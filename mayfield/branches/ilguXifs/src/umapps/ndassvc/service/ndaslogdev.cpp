...
CNdasLogicalDevice::AddUnitDevice(CNdasUnitDevicePtr pUnitDevice)
{
	...
	else if (IsComplete() && m_fMountOnReady) 
	{
			...
			m_mountOnReadyAccess)
		{
			...
			(void) PlugIn(m_mountOnReadyAccess, LdpfFlags, LdpfValues);
			...
			m_fReducedMountOnReadyAccess) 
		{
			...
			(void) PlugIn(GENERIC_READ, LdpfFlags, LdpfValues);
		}
	}
...
namespace
{
	...
	const LDPF_LUR_MAPPING ldpf_lur_mappings[] = {
		...
		NDAS_LDPF_SHARED_WRITE,     LUROPTION_ON_SHAREDWRITE,     LUROPTION_OFF_SHAREDWRITE,
		NDAS_LDPF_OOB_SHARED_WRITE, LUROPTION_ON_OOB_SHAREDWRITE, LUROPTION_OFF_OOB_SHAREDWRITE,
