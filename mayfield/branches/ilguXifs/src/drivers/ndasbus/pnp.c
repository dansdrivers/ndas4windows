...
--*/
{
	...
	Bus_KdPrint (FdoData, BUS_DBG_PNP_ERROR,
		("Bus_PlugInDeviceEx2: Exposing PDO\n"));
	...
	if(PlugIn->SlotNo) {
		...
		for(slotno = 1; slotno <= LSBUS_MAX_DEVICES; slotno ++) {
	...
	status = LSBus_OpenLanscsiAdapter(&pdoData->LanscsiAdapterPDO, PlugIn->MaxRequestBlocks, pEvent, pAlarmEvent);

	Bus_KdPrint_Def(BUS_DBG_SS_ERROR, ("PlugIn->MaxRequestBlocks %d \n", PlugIn->MaxRequestBlocks));
...
--*/
{
	...
	if(Request32Bit) {
		NTSTATUS	status;
		ULONG		length;
		PBUSENUM_PLUGIN_HARDWARE_EX2_32 plugIn32 = (PBUSENUM_PLUGIN_HARDWARE_EX2_32)PlugIn;
		PBUSENUM_PLUGIN_HARDWARE_EX2 thunkPlugIn;

		length = FIELD_OFFSET(BUSENUM_PLUGIN_HARDWARE_EX2, HardwareIDs) +
							plugIn32->HardwareIDLen  * sizeof(WCHAR);
		thunkPlugIn = ExAllocatePool(NonPagedPool, length);

		//
		//	Convert 32 fields to pointer-aligned field.
		//

		thunkPlugIn->Size				= sizeof(BUSENUM_PLUGIN_HARDWARE_EX2);
		thunkPlugIn->SlotNo				= plugIn32->SlotNo;
		thunkPlugIn->MaxRequestBlocks	= plugIn32->MaxRequestBlocks;
