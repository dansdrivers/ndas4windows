...
namespace ndas
{
	namespace settings
	{
#pragma push_macro("DEF_DWORD")
#define DEF_DWORD(x) struct x { static DWORD Value(); };

		DEF_DWORD(AddTargetGap)
		DEF_DWORD(PlugInFailToEjectGap)
		DEF_DWORD(MaxHeartbeatFailure)
		DEF_DWORD(MaxUnitDeviceIdentifyFailure)

#pragma pop_macro("DEF_DWORD")
	}
}
