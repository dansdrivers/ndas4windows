	...
	*received = 0;
	...
	while (len > 0) {
		...
		if(!NT_SUCCESS(ntStatus)) {
			PLANSCSI_SESSION LSS = (PLANSCSI_SESSION)((PBYTE)ConnectionFile - (PBYTE)&((PLANSCSI_SESSION)NULL)->ConnectionFile);
	   ...
	   )
{
	...
	*pTotalSent = 0;
	...
	KDPrintM(DBG_PROTO_NOISE, ("STATUS_PENDING returned.\n"));
			...
			)
{
	...
	if(iDataSegLen) {
		//
		//	Put TDI_FAKE_CONTEXT to enable asynchronous send.
		//
	...
	if(!NT_SUCCESS(ntStatus) || (result != (sizeof(LANSCSI_H2R_PDU_HEADER) + iAHSLen + iDataSegLen))) {
   ...
   )
{
	...
	ntStatus = LspIdeSendRequest_V11(LSS, &pdu);
	if(!NT_SUCCESS(ntStatus)) {
...
//						TDI_FAKE_CONTEXT,
	...
	// Read Reply.
	ntStatus = LspIdeReadReply_V11(LSS, (PCHAR)PduBuffer, &pdu);
	if(!NT_SUCCESS(ntStatus)) {
	 ...
	 )
{
	...
	KDPrintM(DBG_PROTO_INFO, ("UserId:%08lx TargetId:%d HWProtoType:0x%x, HWProtoVersion:0x%x\n",
							LSS->UserID,
							LSS->LanscsiTargetID,
							LSS->HWProtoType,
							LSS->HWProtoVersion
	...
	// Read Reply.
	ntStatus = LspIdeReadReply_V11(LSS, (PCHAR)LSS->PduBuffer, &pdu);
	if(!NT_SUCCESS(ntStatus)) {
	...
	KDPrintM(DBG_PROTO_INFO, ("Version %d Auth %d\n", 
	...
	KDPrintM(DBG_PROTO_ERROR, ("Hash %d, Challenge %d\n", 
	...
	KDPrintM(DBG_PROTO_ERROR, ("Hw Type %d, Hw Version %d, NRSlots %d, W %d, MT %d ML %d\n", 
	...
	if(2 == LSS->HWVersion && LSS->MaxBlocks > 104)
		LSS->MaxBlocks = 104; // set to 52k max
...
//						TDI_FAKE_CONTEXT,
	...
	if(pReplyHeader->Opcode != IDE_RESPONSE){		
		KDPrintM(DBG_PROTO_ERROR, ("BAD Reply Header pReplyHeader->Opcode != IDE_RESPONSE . Flag: 0x%x, Req. Command: 0x%x Rep. Command: 0x%x\n", 
			pReplyHeader->Flags, iCommandReg, pReplyHeader->Command));
		return STATUS_UNSUCCESSFUL;
	}
	if(pReplyHeader->F == 0){		
		KDPrintM(DBG_PROTO_ERROR, ("BAD Reply Header pReplyHeader->F == 0 . Flag: 0x%x, Req. Command: 0x%x Rep. Command: 0x%x\n", 
			pReplyHeader->Flags, iCommandReg, pReplyHeader->Command));
		return STATUS_UNSUCCESSFUL;
	}

	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_PROTO_ERROR, ("Failed. Response 0x%x %d %d Req. Command: 0x%x Rep. Command: 0x%x\n", 
		...
		KDPrintM(DBG_PROTO_ERROR, ("Error register = 0x%x\n", pReplyHeader->Feature_Curr));
	...
	)
{
	...
	case 0x54 : //	case SCSIOP_SEND_OPC_INFORMATION:
	case SCSIOP_SET_READ_AHEAD:
	case SCSIOP_SEND_VOLUME_TAG: //Set Streamming
	case SCSIOP_FORMAT_UNIT:
	case SCSIOP_MODE_SELECT10:	
	...
	case 0x46:
		...
		break;
		// Mix Receove Protocol and Execute Protocol 
	...
	switch(com)
	{
	//	1. Send protcocol No data receive return.
	case SCSIOP_SEND_KEY:
	case SCSIOP_MODE_SELECT10:
	case 0xbf :	//	case SCSIOP_SEND_DVD_STRUCTURE:
	case 0xbb : //	case SCSIOP_SET_CD_SPEED:
...
LspIdeVendorRequest(
	...
	// Read Reply.
	ntStatus = LspReadReply(LSS, (PCHAR)PduBuffer, &pdu);
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(DBG_PROTO_ERROR, ("Can't Read Reply.\n"));
		return ntStatus;
	}
	...
	if(pReplyHeader->Opcode != VENDOR_SPECIFIC_RESPONSE){		
		KDPrintM(DBG_PROTO_ERROR, ("BAD Reply Header pReplyHeader->Opcode != VENDOR_SPECIFIC_RESPONSE . Flag: 0x%x, Req. Command: 0x%x Rep. Command: 0x%x\n", 
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_PROTO_ERROR, ("Failed. Response 0x%x Req. Command: 0x%x Rep. Command: 0x%x\n", 
