...
IoctlToLanscsiBusByWorker(
	...
	LSMP_INIT_WORKITEMCTX(	&WorkitemCtx,
							IoctlToLanscsiBus_Worker,
	  ...
	  )
{
	...
	{
		ADAPTER_SETSTATUSFLAG(HwDeviceExtension, ADAPTERINFO_STATUSFLAG_RECOVERING);
	}
	else
	{
		ADAPTER_RESETSTATUSFLAG(HwDeviceExtension, ADAPTERINFO_STATUSFLAG_RECOVERING);
	}
	...
	if(AdapterStatus != AdapterStatusBefore)
	{
		...
		if(
			!(AdapterStatusBefore & ADAPTERINFO_STATUSFLAG_RECOVERING) &&
			(AdapterStatus & ADAPTERINFO_STATUSFLAG_RECOVERING)
			)
		{
			// ADAPTERINFO_STATUSFLAG_RECOVERING on
		...
		{
			// ADAPTERINFO_STATUSFLAG_RECOVERING off
