...

{
    ...
    case NdisMedium802_3:
        ...
        //
#if DBG
		if(DeviceMaxFrameSize <= 14 || DeviceMaxFrameSize > 1514) {
			DebugPrint(1, ("[LPX] MacReturnMaxDataSize: NdisMedium802_3: DeviceMaxFrameSize=%u\n", DeviceMaxFrameSize));
		}
#endif
		if(DeviceMaxFrameSize <= 14) {
			DebugPrint(1, ("[LPX] MacReturnMaxDataSize: NdisMedium802_3: CAUTION!!!!!!! DeviceMaxFrameSize=%u\n", DeviceMaxFrameSize));
			*MaxFrameSize = 0;
			return;
		}
	...
	default:
		...
		//
		//	Default size: Ethernet packet size
		//	To allow jumbo packet, cut off 1501~1514 size to block malicious NIC drivers that reports wrong size.
		//
#if DBG
		if(DeviceMaxFrameSize <= 14 || DeviceMaxFrameSize > 1514) {
			DebugPrint(1, ("[LPX] MacReturnMaxDataSize: default: DeviceMaxFrameSize=%u\n", DeviceMaxFrameSize));
		}
#endif
		if(DeviceMaxFrameSize <= 14) {
			DebugPrint(1, ("[LPX] MacReturnMaxDataSize: default: CAUTION!!!!!!! DeviceMaxFrameSize=%u\n", DeviceMaxFrameSize));
			*MaxFrameSize = 0;
			return;
		}
		...
		*MaxFrameSize = DeviceMaxFrameSize - 14;
		if((*MaxFrameSize > 1500 && *MaxFrameSize <= 1514) ||
			DeviceMaxFrameSize <= 14) {
			DebugPrint(0, ("[LPX] MacReturnMaxDataSize: default: MaxFrameSize will be adjusted: = %u\n", *MaxFrameSize));
			*MaxFrameSize = 1500;
		}
    }

	ASSERT(*MaxFrameSize >= sizeof(LPX_HEADER2) + 16);
	if(*MaxFrameSize <= sizeof(LPX_HEADER2) + 4) {
		*MaxFrameSize = 0;
		return;
	}
