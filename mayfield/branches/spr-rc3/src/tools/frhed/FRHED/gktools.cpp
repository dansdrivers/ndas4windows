		...
		case BN_CLICKED:
			{
				if( LOWORD (wParam) == IDOK )
				{
					...
					NdasConnectionInfo.bWriteAccess = !SendMessage(hCheckReadOnly, BM_GETCHECK, 0, 0) == BST_CHECKED;
...
//					switch(GetWindowTextLength(hEditID))
					...
					{
					case 12:
						NdasConnectionInfo.address_type = NDASCOMM_CONNECTION_INFO_TYPE_ADDR_LPX;
						STR_TO_MAC(i, NdasConnectionInfo.AddressLPX, buf, 2);
						break;
					case 17:
						NdasConnectionInfo.address_type = NDASCOMM_CONNECTION_INFO_TYPE_ADDR_LPX;
						STR_TO_MAC(i, NdasConnectionInfo.AddressLPX, buf, 3);
						break;
					case 20:
						if(NdasConnectionInfo.bWriteAccess)
						{
							MessageBox(hDlg, "Missing write key", "Wrong format", MB_ICONERROR);
							return FALSE;
						}
						NdasConnectionInfo.address_type = NDASCOMM_CONNECTION_INFO_TYPE_ID_A;
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId, buf, 20);
						break;
					case 23:
						if(NdasConnectionInfo.bWriteAccess)
						{
						...
						NdasConnectionInfo.address_type = NDASCOMM_CONNECTION_INFO_TYPE_ID_A;
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId, buf, 5);
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId +5, buf +6, 5);
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId +10, buf +12, 5);
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId +15, buf +18, 5);
						break;
					case 25:
						NdasConnectionInfo.address_type = NDASCOMM_CONNECTION_INFO_TYPE_ID_A;
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId, buf, 20);
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringKey, buf +20, 5);
						break;
					case 29:
						NdasConnectionInfo.address_type = NDASCOMM_CONNECTION_INFO_TYPE_ID_A;
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId, buf, 5);
						...
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId +10, buf +12, 5);
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringId +15, buf +18, 5);
						CopyMemory(NdasConnectionInfo.DeviceIDA.szDeviceStringKey, buf +24, 5);
						break;
					default:
					...
					NdasConnectionInfo.bSupervisor = FALSE;
					NdasConnectionInfo.login_type = NDASCOMM_LOGIN_TYPE_NORMAL;
