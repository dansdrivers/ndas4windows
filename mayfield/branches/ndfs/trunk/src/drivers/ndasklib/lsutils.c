...
LsuGetBlockACL(
	...
	readBlock = (BlockACLLen + LSS->BlockInBytes) / LSS->BlockInBytes;
	buffer = (PBYTE)ExAllocatePoolWithTag(NonPagedPool, readBlock * LSS->BlockInBytes, LSU_POOLTAG_BLOCKBUFFER);
	if(buffer == NULL)
		return STATUS_INSUFFICIENT_RESOURCES;

	status = LsuReadBlocks(LSS, buffer, LogicalBlockAddress, readBlock, PduFlags);
	if(!NT_SUCCESS(status)) {
