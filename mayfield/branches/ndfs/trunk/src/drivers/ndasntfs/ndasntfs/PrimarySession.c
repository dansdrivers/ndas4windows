	...
	)
{
	...
	primarySession = NtfsAllocatePoolWithTag( NonPagedPool, sizeof(PRIMARY_SESSION), NDNTFS_ALLOC_TAG );
	...
	try {
		...
		DebugTrace( 0, Dbg2, ("Ntfs PrimarySession_Create: primarySession = %p\n", primarySession) );
	...
	)
{
	...
	if (PrimarySession->Irp) {
		...
		outputBuffer = (PULONG)PrimarySession->Irp->AssociatedIrp.SystemBuffer;
		*outputBuffer = (ULONG)PrimarySession;
		DebugTrace( 0, Dbg2, ("PrimarySession_Close: IOCTL_INSERT_PRIMARY_SESSION returned %x\n", PrimarySession) );		
	...
	)
{
    ...
    if(result == 0) {
		...
		ExFreePoolWithTag( PrimarySession, NDNTFS_ALLOC_TAG	);
	...
	)
{
		...
		if (eventStatus == 0) {
			...
			while (primarySessionRequestEntry = ExInterlockedRemoveHeadList( &PrimarySession->RequestQueue,
				...
				if (primarySessionRequest->RequestType == PRIMARY_SESSION_REQ_DISCONNECT) {
					...
					DebugTrace( 0, Dbg2, ("PrimarySessionThreadProc: PRIMARY_SESSION_THREAD_FLAG_SHUTDOWN\n") );
			...
			while (TRUE) {
				...
				if (PrimarySession->Thread.SessionSlot[slotIndex].status == STATUS_SUCCESS) {
					...
					PrimarySession->Thread.SessionSlot[slotIndex].status
						= SendNtfsWinxpMessage( PrimarySession,
												ndfsReplyHeader,
	...
	if (PrimarySession->Irp) {
		...
		outputBuffer = (PULONG)PrimarySession->Irp->AssociatedIrp.SystemBuffer;
		*outputBuffer = (ULONG)PrimarySession;
		DebugTrace( 0, Dbg2, ("FatFsdDeviceControl: IOCTL_INSERT_PRIMARY_SESSION %x\n", *outputBuffer) );		
			...
			if (typeOfOpen == UserFileOpen) {
				...
				do {
					...
					*majorFunction = IRP_MJ_CLEANUP;
					...
					ndfsWinxpRequestHeader.CleanUp.ValidDataLength	= scb->Header.FileSize.QuadPart;
					ndfsWinxpRequestHeader.CleanUp.VaildDataToDisk	= scb->Header.FileSize.QuadPart;
...
//	if(ndfsRequestHeader->MessageSize > (PrimarySession->RequestMessageBufferLength - sizeof(NDFS_REQUEST_HEADER) - sizeof(NDFS_WINXP_REQUEST_HEADER)))
	{
		...
		if(PrimarySession->Thread.SessionSlot[Mid].ExtendWinxpRequestMessagePool == NULL) {
			DebugTrace( 0, Dbg,	("ReceiveNtfsWinxpMessage: failed to allocate ExtendWinxpRequestMessagePool\n"));
