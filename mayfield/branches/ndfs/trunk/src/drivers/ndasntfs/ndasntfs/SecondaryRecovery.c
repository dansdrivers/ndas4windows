	...
	)
{
	...
	DebugTrace( 0, Dbg2, ("RecoverSession: Called Secondary = %p\n", Secondary) );
	...
	if (FlagOn(Secondary->VolDo->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_SHUTDOWN)) {

		//DebugTrace( 0, Dbg2, ("SecondaryToPrimary ND_NTFS_DEVICE_FLAG_SHUTDOWN\n") ); 
		//DbgPrint( "SecondaryToPrimary ND_NTFS_DEVICE_FLAG_SHUTDOWN\n" ); 
	...
	for (status = STATUS_UNSUCCESSFUL, reconnectionTry = 0; reconnectionTry<MAX_RECONNECTION_TRY; reconnectionTry++) {
		...
		DebugTrace( 0, Dbg2, ("SecondaryToPrimary result = %d\n", queryResult) ); 
		...
		if (queryResult == TRUE) {
			...
			try {
				...
				status = NdNtfsMountVolume( tempIrpContext, vcb );
    ...
    for (ccblistEntry = Secondary->RecoveryCcbQueue.Blink; 
		...
		if (FlagOn(ccb->Flags, CCB_FLAG_CLOSE)) {
			...
			SetFlag( ccb->NdNtfsFlags, ND_NTFS_CCB_FLAG_UNOPENED );
			continue;
		}
		...
		if (ndfsWinxpReplytHeader->Status != STATUS_SUCCESS) {
			...
			SetFlag( ccb->NdNtfsFlags, ND_NTFS_CCB_FLAG_UNOPENED | ND_NTFS_CCB_FLAG_CORRUPTED );
			...
			if (scb->CloseCount == scb->CorruptedCcbCloseCount)
				SetFlag( scb->NdNtfsFlags, ND_NTFS_SCB_FLAG_CORRUPTED );
		...
		if (scb->Header.AllocationSize.QuadPart == ndfsWinxpReplytHeader->AllocationSize) {
			...
			if (scb->Header.FileSize.QuadPart == ndfsWinxpReplytHeader->FileSize) {
				...
				DereferenceSecondaryRequest( secondaryRequest );
				SetFlag( ccb->NdNtfsFlags, ND_NTFS_CCB_FLAG_REOPENED );
