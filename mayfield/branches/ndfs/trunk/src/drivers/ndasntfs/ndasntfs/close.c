...

{
...
#ifdef __ND_NTFS__
							(((ULONG)(Vcb->CloseCount - Vcb->CleanupCount)) > NtfsAsyncPostThreshold + NtfsMaxDelayedCloseCount)) {
#else
            ...
            if (FlagOn( Scb->ScbState, SCB_STATE_DELAY_CLOSE ) &&
				...
				if (!NtfsIsTopLevelNtfs(IrpContext)) {
					...
					if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
						...
						if (IrpContext->TopLevelIrpContext->MajorFunction == IRP_MJ_CLEANUP ||
							IrpContext->TopLevelIrpContext->MajorFunction == IRP_MJ_CREATE	||
							FlagOn(IrpContext->TopLevelIrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_TRY_CLOSE_FILES) ) {

						} else {

							DebugTrace( 0, Dbg2, ("NtfsFsdClose: IrpContext->TopLevelIrpContext->MajorFunction = %x\n", 
				...
				if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject))  {
					...
					if (!NtfsIsTopLevelNtfs(IrpContext)) {
						...
						if (secondaryResourceAcquired == FALSE) {
							...
							ASSERT( FlagOn(VolumeDeviceObject->Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_REMOTE_DISCONNECTED) ); // It's not always garented						continue;
							Status = STATUS_PENDING;							
							break;
						}
                ...
                if (Ccb != NULL) {
					...
					if (FlagOn(Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY)) {
						...
						if(FlagOn(Ccb->NdNtfsFlags, ND_NTFS_CCB_FLAG_UNOPENED)) {
							...
							if( FlagOn(Ccb->NdNtfsFlags, ND_NTFS_CCB_FLAG_CORRUPTED) )
								Scb->CorruptedCcbCloseCount --;
					
						} 
							
						ExAcquireFastMutex( &VolumeDeviceObject->Secondary->RecoveryCcbQMutex );
						RemoveEntryList( &Ccb->ListEntry );
						ExReleaseFastMutex( &VolumeDeviceObject->Secondary->RecoveryCcbQMutex );

						InitializeListHead( &Ccb->ListEntry );

						Ccb->FileObject = NULL;
						if(Ccb->Buffer)
							NtfsFreePool( Ccb->Buffer );
					} 
#endif
    ...
    if (Status == STATUS_PENDING) {
		...
		if (FlagOn(Scb->Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY)) {
			...
			SetFlag( Ccb->NdNtfsFlags, ND_NTFS_CCB_FLAG_DELAY_CLOSE );
			Ccb->FileObject = NULL;
		}
        ...
        do {
            ...
            try {
				...
				if (FlagOn(Scb->Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY)) {
					...
					PVOLUME_DEVICE_OBJECT VolumeDeviceObject = (CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb));
					...
					while (TRUE) {
						...
						if (FlagOn(VolumeDeviceObject->Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_REMOTE_DISCONNECTED)) {
							...
							if (FlagOn(VolumeDeviceObject->Vcb.NdNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES)) {
            ...
            if (((Status == STATUS_LOG_FILE_FULL) ||
                ...
                FsRtlExitFileSystem();
//#ifdef __ND_NTFS_SECONDARY__
//				if (!FlagOn(IrpContext->Vcb->NdNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES))
//#endif
    ...
    try {
        ...
        if (CalledFromFsp) {
			...
			if (secondaryCcb) {
				...
				InterlockedDecrement( &Vcb->SecondaryQueuedCloseCount );
		...
		if (send2Primary) {
			...
			secondaryRequest = ALLOC_WINXP_SECONDARY_REQUEST( volDo->Secondary, IRP_MJ_CLOSE, 0 );
    ...
    )
{
	...
	if (FlagOn(((PSCB)IrpContext->OriginatingIrp)->Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY)) {

		InterlockedIncrement( &(IrpContext->Vcb->SecondaryQueuedCloseCount) );
    ...
    )
{
    ...
    if (!IsListEmpty( &NtfsData.AsyncCloseList )) {
        ...
        while (Entry != &NtfsData.AsyncCloseList) {
            ...
            if (!ARGUMENT_PRESENT( Vcb ) ||
				...
				if (ARGUMENT_PRESENT(Vcb) && FlagOn(Vcb->NdNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES)) {
					...
					//ASSERT( FlagOn(((PSCB)IrpContext->OriginatingIrp)->Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY) );
				}
#endif
        ...
        if (ARGUMENT_PRESENT( Vcb )) {
            ...
            while (Entry != &NtfsData.DelayedCloseList) {
                ...
                if (IrpContext->Vcb == Vcb) {
					...
					if (FlagOn(Vcb->NdNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES)) {
	...
	ASSERT( Vcb == NULL || 
			IrpContext == NULL || 
			NtfsIsExclusiveVcb(((PSCB)IrpContext->OriginatingIrp)->Fcb, Vcb) ||
