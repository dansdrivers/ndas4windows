...
#define MAX_TRY_QUERY 2
	...
	ntStatus = PsCreateSystemThread( &secondary->ThreadHandle,
									 THREAD_ALL_ACCESS,
									 &objectAttributes,
									 NULL,
									 NULL,
	...
	ntStatus = ObReferenceObjectByHandle( secondary->ThreadHandle,
										  FILE_READ_DATA,
										  NULL,
										  KernelMode,
	...
	ntStatus = KeWaitForSingleObject( &secondary->Thread.ReadyEvent,
									  Executive,
									  KernelMode,
									  FALSE,
									  &timeOut );
	...
	if (FlagOn(secondary->Thread.Flags, SECONDARY_THREAD_CONNECTED)) {
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_Create: The client thread are ready secondary = %p\n", secondary) );
	...
	)
{
	...
	if (!FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_TERMINATED)) {
	...
	ntStatus = KeWaitForSingleObject( Secondary->ThreadObject,
									  Executive,
									  KernelMode,
									  FALSE,
									  &timeOut );
	...
	if (ntStatus == STATUS_SUCCESS) {
	    ...
	    SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_Close: thread stoped Secondary = %p\n", Secondary) );
	...
	while (secondaryRequestEntry = ExInterlockedRemoveHeadList( &Secondary->Thread.RequestQueue,
	...
	)
{
    ...
    if (result == 0) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_Dereference: Secondary is Freed Secondary  = %p\n", Secondary) );
	...
	)
{
	...
	while (listEntry != &Secondary->FileExtQueue) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_TryCloseFilExts: fcb->FullFileName = %wZ\n", &fileExt->Fcb->FullFileName) );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE, ("Secondary_Ioctl: IRP_MJ_DEVICE_CONTROL Entered\n") );
	...
	do {
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_Ioctl: Volume IOCTL\n") );
	...
	)
{
	...
	if (irpSp->MajorFunction == IRP_MJ_CREATE					||	// 0x00
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_PassThrough: IRP_MJ_PNP %x\n", irpSp->MinorFunction) );

		if (irpSp->MinorFunction == IRP_MN_QUERY_REMOVE_DEVICE) {

			if (FlagOn(Secondary->Flags, SECONDARY_RECONNECTING)) {  // While Disabling and disconnected
	...
	while (1) {
		...
		if (FlagOn(Secondary->Flags, SECONDARY_ERROR)) {
		...
		if (!FlagOn(Secondary->Flags, SECONDARY_RECONNECTING)) {
			...
			ASSERT( FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_CONNECTED) );
		}
		...
		if (LfsObjectCounts.RedirectIrpCount > 3)
			SPY_LOG_PRINT(LFS_DEBUG_SECONDARY_INFO, ("LfsObjectCounts.RedirectIrpCount = %d\n", LfsObjectCounts.RedirectIrpCount) );
		...
		if (redirectStatus == STATUS_DEVICE_REQUIRES_CLEANING) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("redirectStatus == STATUS_DEVICE_REQUIRES_CLEANING\n") );
		...
		if (redirectStatus == STATUS_ABANDONED) {
			...
			SPY_LOG_PRINT(LFS_DEBUG_SECONDARY_INFO, ("redirectStatus == STATUS_ABANDONED\n") );
		...
		if (redirectStatus == STATUS_WORKING_SET_LIMIT_RANGE) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("redirectStatus == STATUS_WORKING_SET_LIMIT_RANGE\n") );
		...
		if (redirectStatus == STATUS_SUCCESS) {
			...
			*NtStatus = Irp->IoStatus.Status = STATUS_PENDING;
			...
			ASSERT( FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_REMOTE_DISCONNECTED) );
			...
			if (FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_REMOTE_DISCONNECTED)) {
	...
	)
{
	if (DataSize > Secondary->Thread.SessionContext.SecondaryMaxDataSize)
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("ALLOC_WINXP_SECONDARY_REQUEST %s(%d) MdataSize = %X\n", 
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession Called Secondary = %p\n", Secondary) );
	...
	for (mid=0; mid < Secondary->Thread.SessionContext.SessionSlotCount; mid++) {
		...
		if (Secondary->Thread.ProcessingSecondaryRequest[mid] != NULL) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession: insert to tempRequestQueue\n") );
...
#define MAX_RECONNECTION_TRY	30
	...
	for (reconnectionTry=0; reconnectionTry<MAX_RECONNECTION_TRY; reconnectionTry++) {
		...
		if (FlagOn(Secondary->LfsDeviceExt->Flags, LFS_DEVICE_STOP) ||
			...
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession: device:%p flags:%x stopped.\n",
		...
		if (Secondary->ThreadHandle) {
			...
			if (waitStatus == STATUS_SUCCESS) {
				...
				SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession: thread stoped\n") );
				ObDereferenceObject( Secondary->ThreadObject );
				...
				InitTransCtx( &Secondary->Thread.SessionContext.TransportCtx, Secondary->Thread.SessionContext.PrimaryMaxDataSize );

				SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR, ("RecoverySession: PriMaxData:%08u SecMaxData:%08u\n",
														  Secondary->Thread.SessionContext.PrimaryMaxDataSize,
														  Secondary->Thread.SessionContext.SecondaryMaxDataSize) );
...
//			if(reconnectionTry >= 2) // first and second lookup primary address
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession: LfsTable_QueryPrimaryAddress tableStatus = %X\n", tableStatus) );

		if (tableStatus == STATUS_SUCCESS) {

			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession: Found PrimaryAddress :%02x:%02x:%02x:%02x:%02x:%02x/%d\n",
		...
		waitStatus = KeWaitForSingleObject( &Secondary->Thread.ReadyEvent,
											Executive,
											KernelMode,
											FALSE,
											&timeOut );
		...
		ASSERT( FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_CONNECTED) && !FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_ERROR) );

		result = TRUE;
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession Success Secondary = %p\n", Secondary) );
	...
	if (result == FALSE) {
		...
		while (!IsListEmpty(&tempRequestQueue)) {
			...
			ASSERT( LFS_BUG );
			ExInterlockedInsertTailList( &Secondary->Thread.RequestQueue, 
    ...
    for (fileExtlistEntry = Secondary->FileExtQueue.Blink;
		...
		if (fileExt->CreateContext.RelatedFileHandle != 0) {
			...
			if (fileExt->RelatedFileObjectClosed == FALSE) {
				...
				SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession: relateFileObject is already closed\n") );
	...
	while (!IsListEmpty(&tempRequestQueue)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("RecoverySession: extract from tempRequestQueue\n") );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("CallRecoverySession entered. Secondary=%p\n", Secondary) );
	...
	) 
{
	...
	secondaryRequest = ExAllocatePoolWithTag( NonPagedPool, requestSize, SECONDARY_MESSAGE_TAG );
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_NOISE, ("AllocSecondaryRequest: secondaryRequest = %p, SecondaryRequestCount = %d\n", 
	...
	)
{
	...
	if (0 == result) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_NOISE, ("FreeSecondaryRequest: SecondaryRequest = %p, SecondaryRequestCount = %d\n", 
													SecondaryRequest, LfsObjectCounts.SecondaryRequestCount) );
	}
}
