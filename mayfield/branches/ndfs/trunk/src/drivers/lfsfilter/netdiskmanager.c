	...
	)
{
	...
	{
		if(!(netdiskInformation.EnabledFeatures & NDASFEATURE_SECONDARY))
	...
	)
{
	...
	ndasBace.BlockStartAddr = LfsDeviceExt->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart/512;
	ndasBace.BlockEndAddr = (LfsDeviceExt->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart + 
							LfsDeviceExt->NetdiskPartitionInformation.PartitionInformation.PartitionLength.QuadPart)/512 - 1;

	ASSERT( ndasBace.BlockStartAddr <= ndasBace.BlockEndAddr );

	result = AddUserBacl( LfsDeviceExt->DiskDeviceObject,
						  &ndasBace,
						  BlockAceId );
	...
	)
{
	...
	RemoveUserBacl( LfsDeviceExt->DiskDeviceObject,
					BlockAceId );
	...
	)
{
	...
	if(!NT_SUCCESS(upgradeStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("[LFS] SecondaryToPrimaryThreadProc: NDCtrlUpgradeToWrite() function failed.\n"));
	} else if(!NT_SUCCESS(ioStatusBlock.Status)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("[LFS] SecondaryToPrimaryThreadProc: Upgrading failed. NetWorking failed.\n"));
	} else {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("[LFS] SecondaryToPrimaryThreadProc: Primary mode.\n"));
	}
			...
			for (volumeIndex = 0; volumeIndex < NETDISK_SECONDARY2PRIMARY+1; volumeIndex++) {
				...
				if (netdiskPartition->NetdiskVolume[volumeIndex][0].LfsDeviceExt &&
					...
					*NetdiskPartitionInformation = netdiskPartition->NetdiskPartitionInformation;
					...
					//DbgPrint( "NetdiskPartitionInformation->VolumeName = %wZ, netdiskPartition->NetdiskPartitionInformation.VolumeName = %wZ\n",
					//		   &NetdiskPartitionInformation->VolumeName, &netdiskPartition->NetdiskPartitionInformation.VolumeName );
