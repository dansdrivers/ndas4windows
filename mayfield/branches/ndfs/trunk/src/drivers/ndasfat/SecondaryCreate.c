	...
	)
{
	...
	DebugTrace2( 0, Dbg, ("NdFatSecondaryCommonCreate: NdNtfsSecondaryCommonCreate: FileName = %Z\n", &fileObject->FileName) );

	if (!FlagOn(((PVOLUME_DEVICE_OBJECT)irpSp->DeviceObject)->NdFatFlags, ND_FAT_DEVICE_FLAG_MOUNTED)) {
	...
	if (volDo->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY) {
		...
		if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_OPEN_TRY_ON_SECONDARY)) {
			
			DebugTrace2( 0, Dbg, ("NdFatSecondaryCommonCreate: NdNtfsSecondaryCommonCreate: FileName = %Z\n", &fileObject->FileName) );
			DebugTrace2( 0, Dbg, ("NdFatSecondaryCommonCreate: ND_FAT_IRP_CONTEXT_FLAG_OPEN_TRY_ON_SECONDARY\n") );
	...
	*DoMore = FALSE;
	...
	try {
...
#ifdef __ND_FAT_SECONDARY_VCB_LOCK__
		...
		if (fullPathName.Length != 0 && (irpSp->Parameters.Create.Options & 0xFF000000) != (FILE_CREATE << 24)) {
			...
			if (!FlagOn(irpSp->Flags, SL_CASE_SENSITIVE)) {
				...
				if (fileObject->RelatedFileObject) {
					...
					if (FlagOn(relatedCcb->NdFatFlags, ND_FAT_CCB_FLAG_UNOPENED)) {
			...
			if (ndfsWinxpReplytHeader->Status == STATUS_SUCCESS) {
				...
				SetFlag( IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE );
				...
				ClearFlag( IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE );
					
				if (fcb == NULL) {
					
					DebugTrace2( 0, Dbg, ("fcb == NULL\n") );
		...
		if (ndfsWinxpReplytHeader->Open.TypeOfOpen == UserFileOpen) {
			...
			ccb->Fcb = fcb;
			InsertTailList( &fcb->CcbQueue, &ccb->FcbListEntry );
		...
		if (ndfsWinxpReplytHeader->Open.TypeOfOpen == UserFileOpen) {
			...
			if (!FlagOn(volDo->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW)) {

				fileObject->SectionObjectPointer = &fcb->NonPaged->SectionObjectPointers;
				ASSERT( fcb->Header.FileSize.QuadPart == 0 );
				goto next_step;
			}			
...
try_exit:  NOTHING;
		...
		if (!NT_SUCCESS(status)) {
			...
			ClearFlag( IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_FILE );
		}
	}
