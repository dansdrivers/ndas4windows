...
#if 0 //def __ND_FAT_INDIRECT_RW__
...

{
    ...
    DebugTrace2( -1, Dbg2, ("FatDeleteVcb -> VOID, VolDo = %p\n", 0, CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)) );
...
#if 0 //def __ND_FAT_PRIMARY__

		if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->FcbTag == 0)
			CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->FcbTag++;

		if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->FcbTag == (_U32)Vcb)
			CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->FcbTag++;

		Dcb->Tag = CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->FcbTag++;
#endif
...

{
    ...
    try {
		...
		if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) {

			SetFlag( Fcb->NdFatFlags, ND_FAT_FCB_FLAG_SECONDARY	);
		}
		...
		Fcb->Tag = CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->FcbTag++;
#endif
		...
		if (!FlagOn(Fcb->NdFatFlags, ND_FAT_FCB_FLAG_SECONDARY)) {
		...
		ExInitializeFastMutex( &Fcb->CcbQMutex );
		InitializeListHead( &Fcb->CcbQueue );
...
#ifdef __ND_FAT_WIN2K_SUPPORT__
	if (NdFatFsRtlTeardownPerStreamContexts)
	    NdFatFsRtlTeardownPerStreamContexts( &Fcb->Header );
#else
...
#ifdef __ND_FAT_DBG__
	ASSERT( Ccb->FcbListEntry.Flink == Ccb->FcbListEntry.Blink );
#endif
...
#if (defined __ND_FAT_DBG__ && defined __ND_FAT_SECONDARY__) 
