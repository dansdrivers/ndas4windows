...
ULONG	PacketTxDropRate = 10000;
ULONG	PacketTxCountForDrop = 0;

ULONG	PacketRxDropRate = 10000;
...
//	ASSERT( LpxControlDeviceContext != DeviceContext) ;
	...
	for (p = DeviceContext->AddressDatabase.Flink;
		...
		IF_NBFDBG (NBF_DEBUG_ADDRESS) {
			NbfPrint2 ("LpxLookupAddress DC %p: found %p ", DeviceContext, address);
		}
	...
	IF_NBFDBG (NBF_DEBUG_ADDRESS) {
		NbfPrint1 ("LpxLookupAddress DC %p: did not find ", address);
	}
...
{   
	...
	IF_NBFDBG (LPX_DEBUG_CURRENT_IRQL)
	...
	case SMP_STATE_CLOSE: {
		...
		NbfReferenceConnection( "Connect", Connection, CREF_LPX_TIMER );
			
		PacketCopy( packet, NULL );
		...
		Connection->LpxSmp.MaxStallTimeOut.QuadPart		= CurrentTime().QuadPart + Connection->LpxSmp.MaxConnectWaitInterval.QuadPart;
		Connection->LpxSmp.RetransmitTimeOut.QuadPart	= CurrentTime().QuadPart + CalculateRTT(Connection);
		RELEASE_DPC_SPIN_LOCK( &Connection->LpxSmp.TimeCounterSpinLock );

		Connection->LpxSmp.StatisticsTimeOut.QuadPart = CurrentTime().QuadPart + Connection->LpxSmp.StatisticsInterval.QuadPart;
		...
		if (result == TRUE) { // Timer is already in system queue. deference myself.
			...
			ASSERT( FALSE );
			NbfDereferenceConnection( "Connect", Connection, CREF_LPX_TIMER );
		}
		...
		if (connectionPrivate) {
			...
			KeQuerySystemTime( &((PCONNECTION_PRIVATE)Irp->Tail.Overlay.DriverContext[0])->CurrentTime );
			((PCONNECTION_PRIVATE)Irp->Tail.Overlay.DriverContext[0])->Connection = Connection;
		}

		RELEASE_C_SPIN_LOCK( &Connection->SpinLock, oldIrql );
		LpxSendPacket( Connection, packet, SMP_TYPE_CONREQ );
...
{   
	...
	DebugPrint( 1, ("LpxListen Connection = %p, Address=%p(%02x:%02x:%02x:%02x:%02x:%02x:%x)\n", 
					 Connection, 
					 Connection->AddressFile->Address, 
					 Connection->AddressFile->Address->NetworkName->LpxAddress.Node[0], 
					 Connection->AddressFile->Address->NetworkName->LpxAddress.Node[1], 
					 Connection->AddressFile->Address->NetworkName->LpxAddress.Node[2], 
					 Connection->AddressFile->Address->NetworkName->LpxAddress.Node[3], 
					 Connection->AddressFile->Address->NetworkName->LpxAddress.Node[4], 
					 Connection->AddressFile->Address->NetworkName->LpxAddress.Node[5],
					 Connection->AddressFile->Address->NetworkName->LpxAddress.Port) );	
	...
	)
{
	...
	case SMP_STATE_CLOSE_WAIT: {
		...
		if (!packet) {
			...
			DebugPrint( 1, ("[LPX] LpxDisconnect ERROR\n") );
	...
	)
{
	...
	if (Connection->LpxSmp.SmpState != SMP_STATE_ESTABLISHED || FlagOn(Connection->LpxSmp.Shutdown, SMP_SEND_SHUTDOWN))
		DebugPrint( 1, ("LpxSend Connection = %p, Connection->LpxSmp.state = %s\n", 
						 Connection, LpxStateName[Connection->LpxSmp.SmpState]) );
	...
	case SMP_STATE_ESTABLISHED: {
		...
		while (remainedUserDataLength) {
			...
			if (!NT_SUCCESS(status)) {
				...
				DebugPrint( 1, ("[LPX]LpxSend: packet == NULL\n") );
				SmpPrintState( 1, "[LPX]LpxSend: PacketAlloc", Connection );
				break;
			}
			
			ASSERT( numberOfPackets < 64 );

			packetArray[numberOfPackets++] = packet;
		...
		if (FlagOn(Connection->LpxSmp.Shutdown, SMP_SEND_SHUTDOWN)) {
			...
			for (index = 0; index < numberOfPackets; index ++) {
				...
				PacketFree( Connection->AddressFile->Address->Provider, packetArray[index] );
			}
		...
		ASSERT( Connection->LpxSmp.SmpState == SMP_STATE_ESTABLISHED );
		...
		for (index = 0; index < numberOfPackets; index ++) {
			...
			lpxHeader->SourcePort		= Connection->AddressFile->Address->NetworkName->LpxAddress.Port;
	...
	)
{
	...
	case SMP_STATE_CLOSE_WAIT: {
...
#if 1
		if (GET_IRP_EXPTIME(Irp)) {
			
			Connection->LpxSmp.ReceiveTimeOut.QuadPart = GET_IRP_EXPTIME(Irp);
			SetFlag( Connection->LpxSmp.SmpTimerFlag, SMP_TIMER_RECEIVE_TIME_OUT );
		}
#endif

		Connection->LpxSmp.LastDataPacketArrived = CurrentTime(); // Assume packet is lost if response is not in timeout.
		...
		LpxSmpProcessReceivePacket( Connection );

		status = STATUS_PENDING;
		break;
	}

	default: // !(SMP_STATE_ESTABLISHED|SMP_STATE_CLOSE_WAIT)
...
//#ifdef __LPX_REQUIRED_TO_RELEASE__
	...
	)
{
	...
	DebugPrint( 1, ("LpxCancelReceive\n") );

    irpSp = IoGetCurrentIrpStackLocation (Irp);

    ASSERT( irpSp->MajorFunction == IRP_MJ_INTERNAL_DEVICE_CONTROL && irpSp->MinorFunction == TDI_RECEIVE );
	...
	NbfReferenceConnection( "Cancelling Recv", connection, CREF_TEMP );
	...
	)
{
	...
	if (!IsListEmpty(&Connection->LpxSmp.TramsmitQueue)) {
		...
		if (Connection->LpxSmp.RetransmitTimeOut.QuadPart <= CurrentTime().QuadPart) {
	...
	if (numberOfPackets != 0) {
		...
		LpxSendPackets( Connection, packetArray, numberOfPackets );
	}
}
	...
	)
{
	...
	if (Connection->LpxSmp.MaxStallTimeOut.QuadPart < CurrentTime().QuadPart) {

		DebugPrint( 1,("SmpTimerDpcRoutine: no response from Remote. stop connection\n") );
		...
		NbfDereferenceConnection( "Timer", Connection, CREF_LPX_TIMER );
	...
	for (irpListEntry = Connection->LpxSmp.ReceiveIrpQueue.Flink; 
		 irpListEntry != &Connection->LpxSmp.ReceiveIrpQueue;
		...
		if (GET_IRP_EXPTIME(irp) && GET_IRP_EXPTIME(irp) <= CurrentTime().QuadPart) {
			...
			DebugPrint( 1,("SmpTimerDpcRoutine: response is too late from Remote. stop connection\n") );
			ASSERT( FALSE );
	...
	if (Connection->LpxSmp.SmpState == SMP_STATE_CLOSE) {
		...
		RELEASE_DPC_C_SPIN_LOCK( &Connection->SpinLock );
		NbfDereferenceConnection( "SmpTimerDpcRoutineRequest", Connection, CREF_LPX_TIMER );
	...
	case SMP_STATE_TIME_WAIT:
		...
		if (Connection->LpxSmp.TimeWaitTimeOut.QuadPart <= CurrentTime().QuadPart) {
	...
	if (Connection->LpxSmp.AliveTimeOut.QuadPart < CurrentTime().QuadPart) { // alive message
		...
		Connection->LpxSmp.AliveTimeOut.QuadPart = CurrentTime().QuadPart + Connection->LpxSmp.AliveInterval.QuadPart;
	...
	if (FlagOn(Connection->LpxSmp.SmpTimerFlag, SMP_TIMER_DELAYED_ACK)) { // alive message
		...
		DebugPrint( 1, ("[LPX]SmpTimerDpcRoutine: Send delayed SMP_TYPE_ACK. SP : %p, S: 0x%x, RS: 0x%x\n", 
						 Connection, Connection->LpxSmp.Sequence, SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence)) );
...
out:
	...
	if (FlagOn(Connection->LpxSmp.SmpTimerFlag, SMP_TIMER_RECEIVE_TIME_OUT)) {

		ASSERT( !IsListEmpty(&Connection->LpxSmp.ReceiveIrpQueue) );

		if (Connection->LpxSmp.ReceiveTimeOut.QuadPart < CurrentTime().QuadPart) {

			ASSERT( FALSE );

			DebugPrint( 1,("SmpTimerDpcRoutine: no data from Remote. stop connection\n") );
	...
	if (Connection->LpxSmp.RetransmitCount) {
		...
		DebugPrint( 4,("CurrentTime().QuadPart %I64d\n", CurrentTime().QuadPart) );
	}

	if (Connection->LpxSmp.StatisticsTimeOut.QuadPart <= CurrentTime().QuadPart) { 
	...
	if (Connection->LpxSmp.SmpState != SMP_STATE_CLOSE) {
		...
		timeInterval.QuadPart = -Connection->LpxSmp.SmpTimerInterval.QuadPart;	
		NbfReferenceConnection( "Timer", Connection, CREF_LPX_TIMER );
	...
	)
{
	...
	case SMP_TYPE_DISCON:
		...
		} else if(PacketType == SMP_TYPE_DATA) {
	...
	)
{
	...
	if (NumberOfPackets > 1) {
		...
		for (index = 0; index < (NumberOfPackets-1); index ++) {
			...
			if (Connection->LpxSmp.DestinationNonDelayedAck == TRUE) {
	...
	if (NumberOfPackets) {
		...
		if (FlagOn(deviceContext->LpxFlags, LPX_DEVICE_CONTEXT_START) && !FlagOn(deviceContext->LpxFlags, LPX_DEVICE_CONTEXT_STOP)) {
			...
			if (PacketTxDropRate) {
				...
				if ((PacketTxCountForDrop % PacketTxDropRate) == 0) {
	...
	)
{
	...
	ASSERT( Status == NDIS_STATUS_SUCCESS || Status == NDIS_STATUS_NOT_ACCEPTED || !NT_SUCCESS(Status) );

	if (Status != NDIS_STATUS_SUCCESS) {

		if (Status != NDIS_STATUS_NO_CABLE)
	...
	)
{
	...
	while (!IsListEmpty( &Connection->LpxSmp.RecvDataQueue)) {
		...
		if (GET_IRP_EXPTIME(irp) && GET_IRP_EXPTIME(irp) <= CurrentTime().QuadPart) {
			...
			DebugPrint( 1, ("[LPX] SmpWorkDpcRoutine IRP expired!! %I64d CurrentTime:%I64d.\n",
							 GET_IRP_EXPTIME(irp), CurrentTime().QuadPart) );
		...
		if (IRP_TRANS_STAT(irp) && Connection->LpxSmp.LastDataPacketArrived.QuadPart != 0) {
	...
	if (Connection->LpxSmp.Shutdown & SMP_RECEIVE_SHUTDOWN) {
		...
		DebugPrint( 1, ("[LPX] LpxSmpProcessReceivePacket: Remotely DisConnected\n") );
		...
		while (thisEntry = ExInterlockedRemoveHeadList( &Connection->LpxSmp.ReceiveIrpQueue, 
			...
			DebugPrint( 1, ("[LPX] LpxSmpProcessReceivePacket: Cancelled Receive IRP %p\n", pendingIrp) );
...
--*/
{
	...
	if (PacketRxDropRate) {
		...
		if ((PacketRxCountForDrop % PacketRxDropRate) == 0) {
	...
	DebugPrint( 4, ("LpxReceiveIndicate, PacketSize = %d, LookAheadBufferSize = %d, LPX_HEADER size = %d\n",
...
--*/
{
	...
	while ((pListEntry = ExInterlockedRemoveHeadList(&deviceContext->PacketInProgressList, 
		...
		for (address = NULL, Flink = deviceContext->AddressDatabase.Flink;
			...
			if (address->NetworkName->LpxAddress.Port == lpxHeader->DestinationPort) {
				...
				if (RtlCompareMemory(&address->NetworkName->LpxAddress.Node, 
		...
		if (lpxHeader->LpxType == LPX_TYPE_STREAM) {
			...
			for (connection = NULL, thisEntry = listHead->Flink;
			    ...
			    if (connection->LpxSmp.SmpState != SMP_STATE_CLOSE && 
					(!memcmp(connection->LpxSmp.DestinationAddress.Node, RESERVED(Packet)->EthernetHeader.SourceAddress, ETHERNET_ADDRESS_LENGTH) && 
					...
					NbfReferenceConnection( "ReceiveCompletion", connection, CREF_REQUEST );
			        refConnection = TRUE;
			...
			if (connection == NULL) {
				...
				for (connection = NULL, thisEntry = listHead->Flink;
					...
					if (connection->LpxSmp.SmpState == SMP_STATE_LISTEN && 
						!memcmp(&connection->LpxSmp.DestinationAddress.Node, zeroNode , 6) && 
						connection->LpxSmp.ListenIrp != NULL) {

						NbfReferenceConnection( "ReceiveCompletion", connection, CREF_REQUEST );
						refConnection = TRUE;

						DebugPrint( 1, ("listenConnection = %p found!\n", connection) );
						break;
				    }
				}
			}
...
loopEnd:
		...
		if(refConnection) {
			NbfDereferenceConnection( "ReceiveCompletion", connection, CREF_REQUEST );
			refConnection = FALSE;
		}
	...
	) 
{
	...
	NbfReferenceConnection( "DoReceiveReq", Connection, CREF_LPX_RECEIVE );
	...
	NbfDereferenceConnection( "DoReceiveReq", Connection, CREF_LPX_RECEIVE );
}
	...
	)
{
	...
	RemoteSeq = SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence);
	...
	if (Connection->LpxSmp.RetransmitCount == 0)
		Connection->LpxSmp.MaxStallTimeOut.QuadPart = CurrentTime().QuadPart + Connection->LpxSmp.MaxStallInterval.QuadPart;
	...
	default:
		...
		DebugPrint( 1, ("[LPX] Dropping packet in %s state. ", LpxStateName[Connection->LpxSmp.SmpState]) );
	...
	) 
{
	...
	case LSCTL_CONNREQ | LSCTL_ACK:
		...
		DebugPrint( 1, ("SmpDoReceive SMP_STATE_LISTEN CONREQ\n") );
		...
		if (FlagOn(lpxHeader->Option, LPX_OPTION_NONE_DELAYED_ACK)) {

			Connection->LpxSmp.DestinationNonDelayedAck = 1;
		...
		NbfReferenceConnection( "Listen", Connection, CREF_LPX_TIMER );
		TimeInterval.QuadPart = -Connection->LpxSmp.SmpTimerInterval.QuadPart;
		...
		if (result ==TRUE) { // Timer is already in system queue. deference myself.
			...
			ASSERT( FALSE );
			NbfDereferenceConnection( "Listen", Connection, CREF_LPX_TIMER );
		}

		RELEASE_DPC_C_SPIN_LOCK( &Connection->SpinLock );

		ReplyPacket = PrepareSmpNonDataPacket( Connection, SMP_TYPE_CONREQ );
		LpxSendPacket( Connection, ReplyPacket, SMP_TYPE_CONREQ );
	...
	) 
{
	...
	if (NTOHS(lpxHeader->Lsctl) & LSCTL_CONNREQ) {
		...
		DebugPrint( 1, ("SmpDoReceive SMP_STATE_SYN_SENT CONREQ\n") );
		...
		SmpPrintState( 1, "LSCTL_CONNREQ", Connection );
	...
	) 
{
	...
	DebugPrint( 1, ("LpxDoReceive SMP_STATE_SYN_RECV CONREQ\n") );
	...
	if (lpxHeader->ServerTag != Connection->LpxSmp.ServerTag) {
		...
		DebugPrint( 1, ("[LPX] SmpDoReceive/SynRecv: Bad Server Tag. Dropping. Connection->LpxSmp.ServerTag 0x%x, lpxHeader->ServerTag 0x%x\n", 
			             Connection->LpxSmp.ServerTag, lpxHeader->ServerTag) );
		return TRUE;	    
	}
	...
	) 
{
	...
	if (lpxHeader->ServerTag != Connection->LpxSmp.ServerTag) {
		...
		DebugPrint( 1, ("[LPX] SmpDoReceive/SMP_STATE_ESTABLISHED: Bad Server Tag. Dropping. Connection->LpxSmp.ServerTag 0x%x, lpxHeader->ServerTag 0x%x\n", 
			             Connection->LpxSmp.ServerTag, lpxHeader->ServerTag) );
	...
	if (NTOHS(lpxHeader->Lsctl) & LSCTL_ACK) {
		...
		ASSERT(((SHORT)(NTOHS(lpxHeader->AckSequence) - SHORT_SEQNUM(Connection->LpxSmp.RemoteAckSequence))) >= 0);
	...
	case LSCTL_DATA | LSCTL_ACK:
		...
		if (NTOHS(lpxHeader->PacketSize & ~LPX_TYPE_MASK) <= sizeof(LPX_HEADER)) {
			...
			DebugPrint(1, ("[LPX] SmpDoReceive/SMP_STATE_ESTABLISHED: Data packet without Data!!!!!!!!!!!!!! SP: %p\n", Connection));
		}
		...
		for (reorderPacketList = Connection->LpxSmp.ReceiveReorderQueue.Flink;
			...
			ASSERT( ((SHORT)(NTOHS(reorderLpxHeader->Sequence) - SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence))) > 0 );
	...
	case LSCTL_DISCONNREQ | LSCTL_ACK:
		...
		if (NTOHS(lpxHeader->Sequence) > SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence)) {
			...
			DebugPrint(1, ("[LPX] SmpDoReceive/SMP_STATE_ESTABLISHED: remote packet lost: HeaderSeq 0x%x, RS: 0x%x\n", NTOHS(lpxHeader->Sequence), SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence)));
		...
		if (NTOHS(lpxHeader->Sequence) < SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence)) {
			...
			DebugPrint(1, ("[LPX] SmpDoReceive/SMP_STATE_ESTABLISHED: Already Received packet: HeaderSeq 0x%x, RS: 0%x\n", NTOHS(lpxHeader->Sequence), SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence)));
			break;
		}

		SetFlag( Connection->LpxSmp.Shutdown, SMP_SEND_SHUTDOWN );

		Connection->LpxSmp.FinSequence = Connection->LpxSmp.RemoteAckSequence;
		Connection->LpxSmp.Sequence = (USHORT)(Connection->LpxSmp.RemoteAckSequence);
		Connection->LpxSmp.NextTransmitSequece = (USHORT)(Connection->LpxSmp.RemoteSequence);
		...
		while (!IsListEmpty(&Connection->LpxSmp.TramsmitQueue)) {
			...
			if (reserved->IrpSp) {
				...
				((PIRP)IRP_SEND_IRP(reserved->IrpSp))->IoStatus.Status = STATUS_REMOTE_DISCONNECT;
				((PIRP)IRP_SEND_IRP(reserved->IrpSp))->IoStatus.Information -= 
					(RESERVED(packet)->PacketRawDataLength - RESERVED(packet)->PacketRawDataOffset);
			}
		...
		DebugPrint( 1, ("[LPX] Receive : LSCTL_DISCONNREQ | LSCTL_ACK: SmpDoReceive from establish to SMP_STATE_CLOSE_WAIT\n") );
	...
	default:
		 ...
		 if (!(NTOHS(lpxHeader->Lsctl) & LSCTL_ACK)) 
			DebugPrint(1, ("[LPX] SmpDoReceive/SMP_STATE_ESTABLISHED: Unexpected packet received %x\n", NTOHS(lpxHeader->Lsctl)));
	...
	) 
{
	...
	if (NTOHS(lpxHeader->Lsctl) == LSCTL_ACK) {
		...
		if (SHORT_SEQNUM(Connection->LpxSmp.RemoteAckSequence) == SHORT_SEQNUM(Connection->LpxSmp.FinSequence)) {
			...
			DebugPrint( 1, ("[LPX] SmpDoReceive: entering SMP_STATE_TIME_WAIT due to RemoteAckSequence == FinSequence\n") );
			
			LpxChangeState( Connection, SMP_STATE_TIME_WAIT, TRUE );

			ACQUIRE_SPIN_LOCK(&Connection->LpxSmp.TimeCounterSpinLock, &oldIrqlTimeCnt);
			Connection->LpxSmp.TimeWaitTimeOut.QuadPart = CurrentTime().QuadPart + Connection->LpxSmp.TimeWaitInterval.QuadPart;
	...
	) 
{
	...
	if (NTOHS(lpxHeader->Lsctl) & LSCTL_DATA) {
		...
		DebugPrint( 1, ("[LPX] SmpDoReceive/SMP_STATE_FIN_WAIT1:  Unexpected data packet\n") );
		//InterlockedIncrement( &Connection->LpxSmp.RemoteSequence );
	
	} else if (NTOHS(lpxHeader->Lsctl) & (LSCTL_CONNREQ | LSCTL_ACKREQ)) {
	
		DebugPrint( 1, ("[LPX] SmpDoReceive/SMP_STATE_FIN_WAIT1:  Unexpected packet %x\n", NTOHS(lpxHeader->Lsctl)) );
	} 
	...
	if (NTOHS(lpxHeader->Lsctl) & LSCTL_ACK) {
		...
		if (SHORT_SEQNUM(Connection->LpxSmp.RemoteAckSequence) == SHORT_SEQNUM(Connection->LpxSmp.FinSequence)) {
			...
			if (Connection->LpxSmp.DisconnectIrp) {
			    ...
			    irp->IoStatus.Status = STATUS_SUCCESS;
			    DebugPrint( 0, ("[LPX]LpxStateDoReceiveWhenFinWait1: Disconnect IRP %p completed.\n ", irp) );
	...
	) 
{
	...
	if (NTOHS(lpxHeader->Lsctl) == LSCTL_ACK) {
		...
		if(SHORT_SEQNUM(Connection->LpxSmp.RemoteAckSequence) == SHORT_SEQNUM(Connection->LpxSmp.FinSequence)) {
			...
			DebugPrint( 2, ("[LPX] SmpDoReceive/SMP_CLOSING: entering SMP_STATE_TIME_WAIT\n") );
			LpxChangeState( Connection, SMP_STATE_TIME_WAIT, TRUE );
	...
	)
{
	...
	Connection->LpxSmp.RetransmitTimeOut.QuadPart = CurrentTime().QuadPart + Connection->LpxSmp.RetransmitInterval.QuadPart;
	RELEASE_SPIN_LOCK( &Connection->LpxSmp.TimeCounterSpinLock, oldIrql );

#if 0
	ACQUIRE_DPC_SPIN_LOCK( &Connection->LpxSmp.TimeCounterSpinLock );
	Connection->LpxSmp.RetransmitEndTime.QuadPart = CurrentTime().QuadPart + (LONGLONG)LpxMaxRetransmitTime;
	RELEASE_DPC_SPIN_LOCK( &Connection->LpxSmp.TimeCounterSpinLock );
#else
	ACQUIRE_SPIN_LOCK( &Connection->LpxSmp.TimeCounterSpinLock, &oldIrql );
	Connection->LpxSmp.MaxStallTimeOut.QuadPart = CurrentTime().QuadPart + (LONGLONG)Connection->LpxSmp.MaxStallInterval.QuadPart;
	RELEASE_SPIN_LOCK( &Connection->LpxSmp.TimeCounterSpinLock, oldIrql );
#endif
	...
	)
{
	...
	DebugPrint(	Debuglevel, (" #Sent %ld, #SentCom %ld, CT %I64d  RT %I64d\n", 
							   NumberOfSent, NumberOfSentComplete, CurrentTime().QuadPart, Connection->LpxSmp.RetransmitTimeOut.QuadPart) );
}
#else
	...
	)
{
	if (Connection->LpxSmp.NumberofSmallSendRequests > 100 ) {
		...
		DebugPrint( DebugLevel2, ("Connection->LpxSmp.ResponseTimeOfLargeRecvRequests = %I64u MiliSecond\n", 
								   Connection->LpxSmp.ResponseTimeOfLargeRecvRequests.QuadPart / MILIHZ) );
		...
		DebugPrint( DebugLevel2, ("Connection->LpxSmp.ResponseTimeOfSmallRecvRequests = %I64u MiliSecond\n", 
								   Connection->LpxSmp.ResponseTimeOfSmallRecvRequests.QuadPart / MILIHZ) );
