	...
	NDASEVENTPROC lpEventProc, LPVOID lpContext)
{
	...
	__try
	{
		...
		{
			XTLTRACE_ERR("Event Subscriber initialization failed.\n");
			return NULL;
		}
		...
		if (NULL == hCallback) 
		{
			XTLTRACE_ERR("Adding callback function failed.\n");
			return NULL;
		}
		return hCallback;
	}
	...
	LPVOID lpContext)
{
	...
	if (0 == m_cRef) 
	{
		...
		{
			XTLTRACE_ERR("QueueUserWorkItem failed.\n");
			::LeaveCriticalSection(&m_cs);
			return NULL;
		}
	}
...
WorkItemStart(LPVOID)
{
	...
	if (WAIT_OBJECT_0 == waitResult)
	{
		XTLTRACE("WorkItem stopped by the the stop event.\n");
		return 0;
	}
...
WaitForEventPipe(DWORD Interval)
{
	do 
	{
		...
		XTLTRACE_ERR("Connecting to %ws failed.\n", NDAS_EVENT_PIPE_NAME);
		PublishConnectionFailed();
		XTLTRACE("Wait for %d ms...\n", Interval);
	...
	NDAS_EVENT_MESSAGE& message)
{
	...
	if (WAIT_OBJECT_0 == waitResult)
	{
		...
		XTLTRACE("Stopped by the stop event.\n");
		return false;
	}
	XTLASSERT(WAIT_OBJECT_0 + 1 == waitResult);
	...
	{
		XTLVERIFY(::CancelIo(hPipe));
		XTLTRACE_ERR("Retrieving version information failed.\n");
		return false;
	}
	...
	{
		XTLVERIFY(::CancelIo(hPipe));
		XTLTRACE_ERR("Getting overlapped result failed.\n");
		return false;
	}

	if (cbRead < cbMessage)
	{
		XTLTRACE("Event message is too small: read %d, expected %d bytes.\n", cbRead, cbMessage);
		::SetLastError(NDASUSER_ERROR_EVENT_VERSION_MISMATCH);
		return false;
	}
