...
{
	XTLTRACE2(NdasVolTrace, 4, "NdasGetNdasScsiSlotNumber(%p)\n", hVolume);
	...
	if (!fSuccess)
	{
		XTLTRACE2_ERR(NdasVolTrace, 1, "NdasGetNdasScsiLocationsForVolumeEx failed.\n");
		return FALSE;
	}

	if (0 == loc.SlotNo) // means not found!
	{
		XTLTRACE2(NdasVolTrace, 1, "Slot No is zero.\n");
		return FALSE;
	}
	...
	IN LPCWSTR FilePath)
{
	...
	{
		XTLTRACE2_ERR(NdasVolTrace, 0, 
			"pGetVolumeMountPointForPath(%ls) failed.\n", FilePath);
		return FALSE;
	}
	...
	{
		XTLTRACE2_ERR(NdasVolTrace, 0, 
			"pGetVolumeDeviceNameForMountPoint(%ls) failed.\n", mountPoint);
		return FALSE;
	}
	...
	PNDAS_REQUEST_DEVICE_EJECT_DATAW EjectData)
{
	...
	if (!fSuccess)
	{
		XTLTRACE("pRequestNdasScsiDeviceEject failed.\n");
		return FALSE;
	}
