...
pOpenDiskDevice(LPCTSTR DiskName)
{
	...
	if (hDisk.IsInvalid())
	{
		XTLTRACE2_ERR(NdasVolTrace, 0, _T("CreateFile(%s) failed.\n"), DiskName);
	...
	LPCTSTR ScsiPortDeviceName)
{
	...
	if (hScsiPort.IsInvalid()) 
	{
		XTLTRACE2_ERR(NdasVolTrace, 0, 
			_T("CreateFile(%s) failed: "), ScsiPortDeviceName);
		return INVALID_HANDLE_VALUE;
	}
	...
	LPDWORD NdasSlotNumber)
{
	...
	if (0 != pSrbIoControl->ReturnCode) 
	{
		XTLTRACE2_ERR(NdasVolTrace, 0, 
			"DeviceIoControl(IOCTL_SCSI_MINIPORT,"
			"NDASSCSI_IOCTL_GET_SLOT_NO) returned error %d.\n",
	...
	*NdasSlotNumber = *reinterpret_cast<DWORD*>(lpInBuffer + cbHeader);
	...
	XTLTRACE2(NdasVolTrace, 2, "NDAS Logical Device Slot: %d\n", *NdasSlotNumber);
	...
	OUT PSCSI_ADDRESS ScsiAddress)
{
	...
	{
		XTLTRACE2_ERR(NdasVolTrace, 0, "pStorageQueryAdapterProperty failed.\n");
		return FALSE;
	}
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(NdasVolTrace, 0, "pScsiGetAddress failed.\n");
		::SetLastError(NDASVOL_ERROR_NON_NDAS_VOLUME);
		return FALSE;
	}

	XTLTRACE2(NdasVolTrace, 2,
		"SCSIAddress: Len: %d, PortNumber: %d, PathId: %d, TargetId: %d, Lun: %d\n",
	...
	LPCTSTR Path)
{
	...
	{
		XTLTRACE2_ERR(NdasVolTrace, 0,
			_T("GetVolumePathName(%s) failed.\n"), Path);
		return NULL;
	}

	XTLTRACE2(NdasVolTrace, 3,
