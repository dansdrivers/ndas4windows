...
UINT32 
CNdasUnitDevice::GetBACLSize() const
{
	return 0;
}
...
CNdasUnitDiskDevice::~CNdasUnitDiskDevice()
{
	...
	if (NULL != m_pBACL) 
	{
		(void) ::HeapFree(::GetProcessHeap(), 0, m_pBACL);
		m_pBACL  = NULL;
	}
}
...
UINT32 
CNdasUnitDiskDevice::GetBACLSize() const
{
	if(m_pBACL)
	{
		XTLTRACE("GetBACLSize() m_pBACL->ElementCount(%d). Size(%d)\n",
			m_pBACL->ElementCount,
			sizeof(NDAS_BLOCK_ACL) + sizeof(NDAS_BLOCK_ACE) * (m_pBACL->ElementCount -1));
	}

	return (m_pBACL) ? 
		sizeof(NDAS_BLOCK_ACL) + sizeof(NDAS_BLOCK_ACE) * (m_pBACL->ElementCount -1) : 0;
}
...
CNdasUnitDiskDevice::FillBACL(void *pNdasBlockAcl) const
{
	...
	l_pNdasBlockAcl->Length = GetBACLSize();
	l_pNdasBlockAcl->BlockACECnt = m_pBACL->ElementCount;
	for(int i = 0; i < l_pNdasBlockAcl->BlockACECnt; i++)
	{
		pNdasBlockAce = &l_pNdasBlockAcl->BlockACEs[i];
		...
		XTLTRACE("FillBACL() pNdasBlockAce : %x %I64d ~ %I64d\n",
			pNdasBlockAce->AccessMode,
