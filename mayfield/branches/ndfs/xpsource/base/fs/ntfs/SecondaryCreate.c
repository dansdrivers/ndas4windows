	...
	)
{
	...
	DebugTrace( 0, Dbg, ("NdNtfsSecondaryCommonCreate: FileName = %Z\n", &fileObject->FileName) );

	if (!FlagOn(((PVOLUME_DEVICE_OBJECT)irpSp->DeviceObject)->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_MOUNTED)) {
	...
	if (volDo->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY) {
		...
		if (FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_OPEN_TRY_ON_SECONDARY)) {
			
			DebugTrace( 0, Dbg, ("ND_NTFS_IRP_CONTEXT_FLAG_OPEN_TRY_ON_SECONDARY\n") );
	...
	try {
		...
		for (index=0; index<ntfsSystemFilesSize; index++) {
			...
			if (NtfsSystemFiles[index].Length != 2 && RtlEqualUnicodeString(&fileObject->FileName, &NtfsSystemFiles[index], TRUE)) {
				...
				DebugTrace(0, Dbg2, ("NdNtfsSecondaryCommonCreate: FileName = %Z\n", &fileObject->FileName) );
				//ASSERT( FALSE );
				try_return( status = STATUS_ACCESS_DENIED );
			}
		}

		RtlInitUnicodeString( &mountMgrRemoteDatabase, L"\\:$MountMgrRemoteDatabase" );
	
		RtlInitUnicodeString( &extendReparse, L"\\$Extend\\$Reparse:$R:$INDEX_ALLOCATION" );	

#ifdef __ND_NTFS_VIEW_INDEX_DONT_SURPORT__
		if (RtlEqualUnicodeString(&fileObject->FileName, &mountMgrRemoteDatabase, TRUE) || 
		    RtlEqualUnicodeString(&fileObject->FileName, &extendReparse, TRUE)) {
#else
		if (RtlEqualUnicodeString(&fileObject->FileName, &mountMgrRemoteDatabase, TRUE)) {
#endif
		...
		*fullFileName = *originalFileName = OplockCleanup->FileObject->FileName;
		...
		secondaryCreateResourceAcquired 
			= SecondaryAcquireResourceExclusiveLite( IrpContext, 
													 &volDo->Secondary->CreateResource, 
		...
		if (fullPathName.Length != 0 && (irpSp->Parameters.Create.Options & 0xFF000000) != (FILE_CREATE << 24)) {
			...
			if (!FlagOn(irpSp->Flags, SL_CASE_SENSITIVE)) {
				...
				if (fileObject->RelatedFileObject) {
					...
					if (FlagOn(relatedCcb->NdNtfsFlags, ND_NTFS_CCB_FLAG_UNOPENED)) {
			...
			if (ndfsWinxpReplytHeader->Status != STATUS_SUCCESS && 
				...
				DebugTrace( 0, Dbg2, ("NdNtfsSecondaryCommonCreate: ndfsWinxpReplytHeader->Status = %x\n", 
			...
			if (ndfsWinxpReplytHeader->Status == STATUS_SUCCESS && ndfsWinxpReplytHeader->Open.TypeOfOpen == UserFileOpen) {
			...
			if (ndfsWinxpReplytHeader->Status == STATUS_SUCCESS) {
				...
				SetFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE );
				...
				ClearFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE );
		...
		if(ndfsWinxpReplytHeader->Status != STATUS_SUCCESS) {
			...
			if(Irp->IoStatus.Status == STATUS_ACCESS_DENIED)
				DebugTrace( 0, Dbg2, ("Irp->IoStatus.Status == STATUS_ACCESS_DENIED DesiredAccess = %x, ndfsWinxpReplytHeader->Status = %x\n", 
			...
			if (createDisposition == FILE_SUPERSEDE ||
				...
				if (scb->Header.AllocationSize.QuadPart != 0) {
					...
					if (FlagOn(scb->ScbState, SCB_STATE_TRUNCATE_ON_CLOSE))
						ASSERT( FlagOn(scb->ScbState, SCB_STATE_TRUNCATE_ON_CLOSE) == ndfsWinxpReplytHeader->Open.TruncateOnClse );
					ASSERT( scb->Header.FileSize.QuadPart == ndfsWinxpReplytHeader->FileSize );
					ASSERT( scb->Header.AllocationSize.QuadPart == ndfsWinxpReplytHeader->AllocationSize );
					...
					if (ndfsWinxpReplytHeader->AllocationSize) {
						...
						scb->Header.AllocationSize.QuadPart = ndfsWinxpReplytHeader->AllocationSize;
						scb->Header.ValidDataLength.QuadPart = ndfsWinxpReplytHeader->ValidDataLength;
#ifdef __ND_NTFS_DBG__
						strncpy(scb->LastUpdateFile, strrchr(__FILE__,'\\')+1, 15); 
						scb->LastUpdateLine = __LINE__; 
#endif				
						if (scb->Header.FileSize.QuadPart == 0) {
							...
							ASSERT( ndfsWinxpReplytHeader->Open.TruncateOnClse == FALSE );
...
try_exit:  NOTHING;
		...
		if (secondaryCreateResourceAcquired) {
			...
			SecondaryReleaseResourceLite( IrpContext, &volDo->Secondary->CreateResource );		
		}
...
//#ifdef __ND_NTFS_DBG__
		if (!NT_SUCCESS(status)) {
			...
			ClearFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_FILE );
		}
//#endif
	}
	...
	)
{
	...
	secondaryRequest = ALLOC_WINXP_SECONDARY_REQUEST( Secondary, IRP_MJ_CLOSE, 0 );
