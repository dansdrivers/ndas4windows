...
PDEVICE_OBJECT NdNtfsControlDeviceObject;
PDEVICE_OBJECT NdNtfsFileSystemDeviceObject;
...
#ifdef __ND_NTFS_AVOID_BUG__
const UNICODE_STRING NdNtfsSDH = CONSTANT_UNICODE_STRING( L"$SDH" );
#endif
...
LONG NtfsstatusFilter = 0xf0ffffffL; // just an non-existent error code
...

{
    ...
    if (NtfsBreakOnCorrupt &&
        ((ExceptionCode == STATUS_FILE_CORRUPT_ERROR) ||
		 ...
		 if(ExceptionCode == STATUS_FILE_CORRUPT_ERROR && FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_FILE))
			 ASSERT( FALSE );
#endif
        if (*KdDebuggerEnabled) {
			...
			//if(ExceptionCode == STATUS_DISK_CORRUPT_ERROR)
				DbgBreakPoint();
#endif
...

{
    ...
    if (ARGUMENT_PRESENT( IrpContext )) {
		...
		if (ARGUMENT_PRESENT( Irp )) {
			...
			ASSERT( !(Irp == IrpContext->CompleteIrp && IoGetCurrentIrpStackLocation( Irp ) == IrpContext->CompleteIrpSp && FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_ALREADY_COMPLETED)) );

			IrpContext->CompleteIrp = Irp;
			IrpContext->CompleteIrpSp = IoGetCurrentIrpStackLocation( Irp );
			SetFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_ALREADY_COMPLETED );
		}
#endif

		//ASSERT( !FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_RESOURCE_ACQUIERED) );
...

{
	...
	if (irpSp->MajorFunction == IRP_MJ_DEVICE_CONTROL && NdNtfsFileSystemDeviceObject == (PDEVICE_OBJECT)VolumeDeviceObject) {
		...
		case IOCTL_SHUTDOWN: {
	        ...
	        for (vcbListEntry = NtfsData.VcbQueue.Flink;
				...
				//DbgPrint( "NtfsFsdDispatch: ND_NTFS_DEVICE_FLAG_SHUTDOWN\n" );		
				SetFlag( volDo->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_SHUTDOWN );
				...
				if (lazyWriter) {
					...
					while (TRUE) {
						...
						if (secondaryResourceAcquired == FALSE) {
							...
							ASSERT( FlagOn(VolDo->Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_REMOTE_DISCONNECTED) ||
									FlagOn(VolDo->Secondary->Flags, SECONDARY_FLAG_RECONNECTING) );
