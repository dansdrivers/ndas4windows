...
#define ND_NTFS_SHUTDOWN			CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 17, METHOD_BUFFERED, FILE_READ_ACCESS)
#define DRVO_BASE_FILESYSTEM_DRIVER	0x00000080   // A driver that is at the bottom of the filesystem stack.
	...
	if ((PVOID)NdNtfsControlDeviceObject == VolumeDeviceObject) {
		...
		if (irpSp->Parameters.FileSystemControl.FsControlCode == ND_NTFS_SHUTDOWN) {

			DebugTrace( 0, Dbg, ("IoControlCode == ND_NTFS_SHUTDOWN occured DeviceObject->ReferenceCount = %x\n", 
			...
			if(NtfsData.FileSystemRegistered == TRUE) {
				...
				NdNtfsControlDeviceObject->DriverObject->Flags &= ~DRVO_BASE_FILESYSTEM_DRIVER;		
    ...
    try {
		...
		if( ((PVOLUME_DEVICE_OBJECT)NdNtfsFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL ) {
			...
			if (queryStatus != STATUS_SUCCESS) {
...
#else
				if (netdiskPartitionInformation.NetdiskInformation.DesiredAccess & GENERIC_WRITE) {

					if (netdiskPartitionInformation.NetdiskInformation.GrantedAccess & GENERIC_WRITE)
						netdiskEnableMode = NETDISK_PRIMARY;
					else
						netdiskEnableMode = NETDISK_SECONDARY;
				...
				DebugTrace( 0, Dbg2, ("netdiskEnableMode = %d, NETDISK_SECONDARY = %d\n", netdiskEnableMode, NETDISK_SECONDARY) );
			}
		}
#endif
...
*/
		...
		if (VolDo->NetdiskEnableMode == NETDISK_SECONDARY) {
			...
			for (reconnectionTry = 0; reconnectionTry < MAX_RECONNECTION_TRY; reconnectionTry ++) {
				...
				queryResult = ((PVOLUME_DEVICE_OBJECT) NdNtfsFileSystemDeviceObject)->
			...
			if (((PVOLUME_DEVICE_OBJECT)NdNtfsFileSystemDeviceObject)->NdfsCallback.AddWriteRange(Vcb->Vpb->RealDevice, &VolDo->BlockAceId) == FALSE) {
...
#ifdef __ND_NTFS_DISABLE_SPARSE__
