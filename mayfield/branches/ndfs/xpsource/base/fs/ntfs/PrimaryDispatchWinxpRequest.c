...
#if	(defined __ND_NTFS_DIRECT_ACCESS__ || defined __ND_NTFS_DIRECT_READWRITE__)
...
#ifndef __ND_NTFS_DIRECT_READWRITE__
		...
		if (NT_SUCCESS(setInformationStatus)) {
			...
			if (!(typeOfOpen == UserFileOpen &&
				...
				*ReplyDataSize = 0;
				...
				NdfsWinxpReplytHeader->Open.TruncateOnClse = BooleanFlagOn( scb->ScbState, SCB_STATE_TRUNCATE_ON_CLOSE );
		...
		if (openFileId == 0) {
			...
			if (NdfsWinxpReplytHeader->Status == STATUS_SUCCESS && fsControlCode == FSCTL_MOVE_FILE) {		// 29
				...
				if (typeOfOpen == UserFileOpen) {
					...
					NdfsWinxpReplytHeader->Open.TruncateOnClse = BooleanFlagOn( moveScb->ScbState, SCB_STATE_TRUNCATE_ON_CLOSE );
