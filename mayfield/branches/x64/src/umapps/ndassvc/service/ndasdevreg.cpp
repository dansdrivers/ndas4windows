...
static BOOL 
pConvertStringToDeviceId(LPCTSTR szAddressString, NDAS_DEVICE_ID* pDeviceId);
	...
	BOOL fAutoRegistered)
{
	...
	// register
	CNdasDevice* pDevice = 
		new CNdasDevice(dwSlotNo, DeviceId, fVolatile, fAutoRegistered);
	if (NULL == pDevice) {
...
CNdasDeviceRegistrar::Register(
	...
	BOOL fAutoRegistered)
{
	return Register(0, DeviceId, fVolatile, fAutoRegistered);
}
...
CNdasDeviceRegistrar::ImportLegacyEntry(DWORD dwSlotNo, HKEY hEntryKey)
{
	...
	CRefObjPtr<CNdasDevice> pDevice = Register(dwSlotNo, deviceId, FALSE, FALSE);
	if (NULL == pDevice.p) {
...
CNdasDeviceRegistrar::Bootstrap()
{
	...
	for (DWORD i = 0; i < MAX_SLOT_NUMBER; ++i) {
		...
		CRefObjPtr<CNdasDevice> pDevice = 
			Register(i, deviceId, FALSE, FALSE);
