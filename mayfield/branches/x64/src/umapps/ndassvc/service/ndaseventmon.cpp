...
CNdasEventMonitor::OnLogicalDeviceAlarmed(DWORD nWaitIndex)
{
	...
	if (!fSuccess) {
		DBGPRT_ERR_EX(_FT("Unable to get alarm status, Ignored: "));
		return TRUE;
	}
	...
	if(pLogDevice->GetAdapterStatus() != ulAdapterStatus)
	{
		...
		// ignore first normal status
		if(!(ADAPTERINFO_STATUSFLAG_MASK & ulAdapterStatus) && // normal mode
			pLogDevice->CheckFirstNormalStatus())
		{
			// do nothing
			DBGPRT_ERR_EX(_FT("First normal status alarm, Ignored"));
		}
		else
		{
			// send event to mgmt
...
BOOL CNdasEventMonitor::OnLogicalDeviceDisconnected(DWORD nWaitIndex)
{
	...
	pLogDevice->CheckFirstNormalStatus(FALSE);
