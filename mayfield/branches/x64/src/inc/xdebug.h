...
struct __declspec(novtable) IXDbgOutput
{
...
public:
	...
	VOID VPrintfEx(
		LPCTSTR szDebugInfo,
		DWORD dwLevel, LPCTSTR szFormat, va_list va)
	{
		//
		// Prevent shadowing of the error
		//
...
__inline LPCTSTR XDebugMemoryDump(
...
__inline BOOL XDbgInit(LPCTSTR szModuleName)
{
...
__inline VOID XDbgSetOutputLevel(DWORD dwOutputLevel)
{
	if (NULL != _PXDebug) {
...
__inline VOID XDbgSetModuleFlags(DWORD dwFlags)
{
	if (NULL != _PXDebug) {
...
__inline VOID XDbgSetLibraryFlags(DWORD dwFlags)
{
	if (NULL != _PXDebug) {
...
__inline BOOL XDbgEnableSystemDebugOutput(BOOL Enable)
{
	if (NULL == _PXDebug) {
		return FALSE;
	}
...
__inline VOID XDbgAttach(IXDbgOutput *pOutput)
{
	if (NULL != _PXDebug) {
...
__inline VOID XDbgDetach(IXDbgOutput *pOutput)
{
	if (NULL != _PXDebug) {
...
static DWORD_PTR _XDbgFilePathOffset = 0;
static __inline LPCTSTR XDbgRemoveFilePath(LPCTSTR szFormat)
{
...
static BOOL XDebugPrintf(DWORD dwLevel, LPCTSTR szFormat, ...)
{
