...
int setdiskencrypt(int argc, TCHAR** argv)
{
	...
	if (pOptID->is_set) {
		...
		if (::IsBadReadPtr(pOptID->pval, sizeof(ci.Address.NdasId.Id))) {
		...
		if (::IsBadReadPtr(pOptWriteKey->pval, sizeof(ci.Address.NdasId.Key))) {
			_tprintf(_T("Error: Device String ID is too short.\n"));
			return 2;
		}

		::CopyMemory(
			ci.Address.NdasId.Key,
			pOptWriteKey->pval,
			sizeof(ci.Address.NdasId.Key));

	} else if (pOptAddr->is_set) {
		if (::IsBadReadPtr(pOptAddr->pval, sizeof(ci.Address.DeviceId.Node))) {
