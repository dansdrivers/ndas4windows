...
#if 1
	KDPrintM(DBG_LURN_ERROR,("Reconnect: ProtoStatus = %08X, NICStatus = %08X, StallCount = %08X, BindFailCount = %08X\n", ProtoStatus, GetNICStatus(), StallCount, BindFailCount));
...
/*
	{ 
		...
		extern VOID HalDisableSystemInterrupt (
			IN ULONG Vector,
			IN KIRQL Irql
			);
...
*/
	...
	if(STATUS_LPX_MINIPORT == ProtoStatus || STATUS_LPX_DRIVER_BEFORE_BIND == ProtoStatus) {
		if(STATUS_NIC_CORRUPTED == GetNICStatus()) {	
			KDPrintM(DBG_LURN_ERROR,("Wait for LPX bind.\n"));
...
/*
			{ 
				...
				extern VOID HalBeginSystemInterrupt (
					IN KIRQL Irql, 
					IN ULONG Vector,
					OUT PKIRQL oldIrql
					);
...
*/
			...
			KDPrintM(DBG_LURN_ERROR,("LspReconnectAndLogin , Status = %08X!!!\n", status ));				
			
			if(NT_SUCCESS(status)) {
				ProtoStatus = STATUS_LPX_DRIVER_BIND;
				BindFailCount = 0;
				KDPrintM(DBG_LURN_ERROR,("LspReconnectAndLogin Success!!!\n" ));				
			}								
/*
			if(	STATUS_NETWORK_UNREACHABLE == status
				) {
				ProtoStatus = STATUS_LPX_MINIPORT;
				
				KDPrintM(DBG_LURN_ERROR,("LspReconnectAndLogin Failed, Status = %08X!!!\n", status ));				
				NetProtoReInitialize();
				status = LspReconnectAndLogin(&IdeExt->LanScsiSession, IdeExt->LstransType, LssEncBuff);
			}	
			else
*/			if(STATUS_OBJECT_NAME_NOT_FOUND == status ||
				...
				KDPrintM(DBG_LURN_ERROR,("Wait for Bind, Status = %08X, BindWaitCount = %08X!!!\n", status, BindWaitCount ));
			}
		...
		if(StallCount>5) {
			...
			*/
			...
			KDPrintM(DBG_LURN_ERROR,("Maybe LPX is unbinded.\n"));
		}		
