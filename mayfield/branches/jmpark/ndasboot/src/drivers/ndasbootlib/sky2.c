...
{
	SK_PACKET     *pSkPacket;
	KIRQL			Irql;   /* for POP/PUSH macros */
...
{
	struct SK_NET_DEVICE  *dev  = (struct SK_NET_DEVICE *)DeferredContext;
	...
	*/
	...
	if (HW_FEATURE(pAC, HWF_WA_DEV_42)) {
		KeAcquireSpinLock(&pAC->SetPutIndexLock, &Irql);
		...
		KeReleaseSpinLock(&pAC->SetPutIndexLock, Irql);
	}
...
*/
	...
	PhysAddr = (SK_U64) virt_to_bus(skb->data);
...
int      Port)  /* port index for which the buffer is used    */
{
	...
	KeAcquireSpinLock(&pAC->TxQueueLock, &LockFlag);
	...
	*/
	...
	while (pSkPacket != NULL) {
		...
		*/
		if (HW_FEATURE(pAC, HWF_WA_DEV_42)) {
			KeAcquireSpinLockAtDpcLevel(&pAC->SetPutIndexLock);
			SkGeY2SetPutIndex(pAC, pAC->IoBase, Y2_PREF_Q_ADDR(Q_XA1,0), &pAC->TxPort[0][0].TxALET);
			KeReleaseSpinLockFromDpcLevel(&pAC->SetPutIndexLock);
		} else {
			/* write put index */
			if (Port == 0) { 
				SK_OUT32(pAC->IoBase, 
	...
	KeReleaseSpinLock(&pAC->TxQueueLock, LockFlag);
...
int      Port)  /* port index of RX                   */
{
	...
	if (NUM_FREE_LE_IN_TABLE(&pAC->RxPort[Port].RxLET) >
		(unsigned) pAC->MaxUnusedRxLeWorking) {
...
{
	SK_PACKET       *pSkPacket;
	SK_PKT_QUEUE     SendCmplPktQ;
	SK_BOOL          DoWakeQueue  = SK_FALSE;
	KIRQL		     Irql;
	unsigned         Put;
	
	SendCmplPktQ.pHead = 0;
	SendCmplPktQ.pTail = 0;
	KeInitializeSpinLock(&SendCmplPktQ.QueueLock);
	...
	*/
	...
	KeAcquireSpinLock(&(pPQ->QueueLock), &Irql);
	...
	*/
	...
	KeReleaseSpinLock(&(pPQ->QueueLock), Irql);
...
*/
	...
	*/
	if (SendCmplPktQ.pHead != NULL) { 
		KeAcquireSpinLock(&(pAC->TxPort[Port][0].TxQ_free.QueueLock), &Irql);
		...
		KeReleaseSpinLock(&pAC->TxPort[Port][0].TxQ_free.QueueLock, Irql);
	}
		...
		while (OWN_OF_FIRST_LE(&pAC->StatusLETable) == HW_OWNER) {
			...
			case OP_RXSTAT:
				...
				*/
				if ((Port != 0) && (Port != 1)) {
				/* Unknown port */
					NbDebugPrint(0, ("sk98lin: Unknown port %d\n",
					Port));
				}
...
SK_AC *pAC)  /* pointer to adapter context */
{
	...
	SK_U32           CurrBuff;
	int	             CurrMac;
	KIRQL		     Irql; /* needed for POP/PUSH functions */
...
SK_AC *pAC)  /* pointer to adapter control context */
{
	...
	*/
	...
	pPhysMemAddr = (dma_addr_t)(((SK_U64) pAC->RxPort[0].RxLET.pPhyLETABHigh << (SK_U64) 32) | 
			((SK_U64) pAC->RxPort[0].RxLET.pPhyLETABLow)); // correct?
...
int         Port)       /* port the packet belongs to             */
{
	...
	pMsgBlock = alloc_skb(pAC->RxPort[Port].RxBufSize, 0);
	if (pMsgBlock == NULL) {
		SK_DBG_MSG(NULL, SK_DBGMOD_DRV,
			SK_DBGCAT_DRV_RX_PROGRESS | SK_DBGCAT_DRV_ERROR,
