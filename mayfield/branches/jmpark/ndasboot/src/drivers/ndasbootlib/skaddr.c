...
int		Level)	/* initialization level */
{
	...
	SK_DBG_MSG(pAC, SK_DBGMOD_ADDR, SK_DBGCAT_CTRL,
		("ADDR: SkPnmiInit: Called, level=%d\n", Level));
	...
	case SK_INIT_IO:
		...
		for (j = 0; j < SK_MAC_ADDR_LEN; j++) {
			...
			NbDebugPrint(3, ("SkAddrInit:pAC->Addr.Net[0].PermanentMacAddress.a[%d] = %02X\n", j,
				pAC->Addr.Net[0].PermanentMacAddress.a[j]));
		}
		...
		for (i = 0; i < (SK_U32) pAC->GIni.GIMacsFound; i++) {
			SK_DBG_MSG(pAC, SK_DBGMOD_ADDR, SK_DBGCAT_INIT,
				("SkAddrInit:Permanent MAC Address (Net%d): %02X %02X %02X %02X %02X %02X\n",
					i,
			...
			SK_DBG_MSG(pAC, SK_DBGMOD_ADDR, SK_DBGCAT_INIT,
				("SkAddrInit:Logical MAC Address (Net%d): %02X %02X %02X %02X %02X %02X\n",
					i,
		...
		for (i = 0; i < (SK_U32) pAC->GIni.GIMacsFound; i++) {
			...
			for (j = 0; j < SK_MAC_ADDR_LEN; j++) {
				...
				NbDebugPrint(3, ("SkAddrInit:pAPort->PermanentMacAddress.a[%d] = %02X\n", j,
				pAC->Addr.Net[0].PermanentMacAddress.a[j]));
			}
...
int		Flags)		/* permanent/non-permanent */
{
	...
	NbDebugPrint(3, ("SkAddrGmacMcAdd: HashBit = %08X\n", HashBit));
...
SK_U32	PortNumber)	/* Port Number */
{
...

	{
		...
		GM_INHASH(IoC, PortNumber, GM_MC_ADDR_H1,
		&Bytes[0]);

		NbDebugPrint(3, ("SkAddrGmacMcUpdate: before GM_MC_ADDR_H1 = %02X %02X %02X %02X %02X %02X %02X %02X\n",
			Bytes[0], 
			Bytes[1],
			Bytes[2],
			Bytes[3],
...

	{
		...
		NbDebugPrint(3, ("SkAddrGmacMcUpdate: after GM_MC_ADDR_H1 = %02X %02X %02X %02X %02X %02X %02X %02X\n",
			Bytes[0], 
			Bytes[1],
			Bytes[2],
			Bytes[3],
	...
	NbDebugPrint(3, ("SkAddrGmacMcUpdate: pAPort->PromMode = %08X\n", pAPort->PromMode));
	if (pAPort->PromMode & SK_PROM_MODE_ALL_MC) {
...

	{
		...
		GM_INADDR(IoC, PortNumber, GM_SRC_ADDR_1L,
		&Bytes[0]);

		NbDebugPrint(3, ("SkAddrGmacMcUpdate: before GM_SRC_ADDR_1L = %02X %02X %02X %02X %02X %02X %02X %02X\n",
			Bytes[0], 
			Bytes[1],
			Bytes[2],
			Bytes[3],
...

	{
		...
		NbDebugPrint(3, ("SkAddrGmacMcUpdate: after GM_SRC_ADDR_1L = %02X %02X %02X %02X %02X %02X %02X %02X\n",
			Bytes[0], 
			Bytes[1],
			Bytes[2],
			Bytes[3],
...

	{
		...
		GM_INADDR(IoC, PortNumber, GM_SRC_ADDR_2L,
		&Bytes[0]);

		NbDebugPrint(3, ("SkAddrGmacMcUpdate: before GM_SRC_ADDR_2L = %02X %02X %02X %02X %02X %02X %02X %02X\n",
			Bytes[0], 
			Bytes[1],
			Bytes[2],
			Bytes[3],
...

	{
		...
		NbDebugPrint(3, ("SkAddrGmacMcUpdate: after GM_SRC_ADDR_2L = %02X %02X %02X %02X %02X %02X %02X %02X\n",
			Bytes[0], 
			Bytes[1],
			Bytes[2],
			Bytes[3],
