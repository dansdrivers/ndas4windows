...
struct s_RxD {
	SK_U32			 RBControl;     /* Receive Buffer Control            */
...
struct s_queue {
 	SK_PACKET       *pHead;
 	SK_PACKET       *pTail;
	KSPIN_LOCK       QueueLock;     /* serialize packet accesses          */
};
...
#define POP_FIRST_PKT_FROM_QUEUE(pQueue, pPacket)	{	\
	KeAcquireSpinLock(&((pQueue)->QueueLock), &Irql);	\
	if ((pQueue)->pHead != NULL) {				\
		(pPacket)       = (pQueue)->pHead;		\
		(pQueue)->pHead = (pPacket)->pNext;		\
		if ((pQueue)->pHead == NULL) {			\
	...
	KeReleaseSpinLock(&((pQueue)->QueueLock), Irql);	\
}
...
struct s_DevNet {
...
#ifdef Y2_RECOVERY
	KTIMER		KernelTimer;	/* Kernel timer struct  */
	KDPC		KernelTimerDpc;	
	int			TransmitTimeoutTimer;
	...
	LONGLONG	LastJiffies;		/* Backup of the jiffies*/
#endif
#endif
	SK_AC		*pAC;
};  
