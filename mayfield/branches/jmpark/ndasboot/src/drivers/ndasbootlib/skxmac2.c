...
static const char SysKonnectFileId[] =
	"@(#) $Id: skxmac2.c,v 2.42 2005/09/19 13:39:52 rschmidt Exp $ (C) Marvell.";
#endif
...
SK_U16	Val)		/* Value */
{
	...
	NbDebugPrint(3, ("SkGmPhyWrite: GMAC_TI_ST_VAL = %08X\n", CurrTime));
...
SK_BOOL	Enable)	/* Enable / Disable */
{
	...
	if (pAC->GIni.GIYukon) {
		...
		NbDebugPrint(3, ("SkMacHashing: before GM_RX_CTRL = %08X\n", RcReg));
		...
		NbDebugPrint(3, ("SkMacHashing: after GM_RX_CTRL = %08X\n", RcReg));
...
int		Port)		/* Port Index (MAC_1 + n) */
{
	...
	if (pPrt->PState == SK_PRT_RESET) {
		...
		/* Auto-negotiation ? */
		NbDebugPrint(3, ("SkGmInitMac:AutoNegotiation Setting, pPrt->PLinkMode = %08X\n", pPrt->PLinkMode));
		if (pPrt->PLinkMode == SK_LMODE_HALF || pPrt->PLinkMode == SK_LMODE_FULL) {
			...
			NbDebugPrint(3, ("SkGmInitMac: before GM_GP_CTRL = %08X\n", SWord));
			...
			{	
				...
				NbDebugPrint(3, ("SkGmInitMac: after GM_GP_CTRL = %08X\n", SWord));
			}
		...
		NbDebugPrint(3, ("SkGmInitMac:Speed Setting, pPrt->PLinkSpeed = %08X\n", pPrt->PLinkSpeed));
		/* speed settings */
		...
		NbDebugPrint(3, ("SkGmInitMac:duplex Setting, pPrt->PLinkMode = %08X\n", pPrt->PLinkMode));
		/* duplex settings */
		if (pPrt->PLinkMode != SK_LMODE_HALF) {
			/* set full duplex */
		...
		NbDebugPrint(3, ("SkGmInitMac:flow-control Setting, pPrt->PFlowCtrlMode = %08X\n", pPrt->PFlowCtrlMode));
		...
		NbDebugPrint(3, ("SkGmInitMac: GMAC_IRQ_SRC = %08X\n", SWord));
	...
	/* setup Transmit Control Register */
	{
		SK_U16 Word;
		GM_IN16(IoC, Port, GM_TX_CTRL, &Word);
		NbDebugPrint(3, ("SkGmInitMac: before GM_TX_CTRL = %04X\n", Word));
	}
...

	{
		SK_U16 Word;
		GM_IN16(IoC, Port, GM_TX_CTRL, &Word);
		NbDebugPrint(3, ("SkGmInitMac: after GM_TX_CTRL = %04X\n", Word));
	}
...

	{
		...
		NbDebugPrint(3, ("SkGmInitMac: before GM_RX_CTRL = %04X\n", Word));
	}
...

	{
		SK_U16 Word;
		GM_IN16(IoC, Port, GM_RX_CTRL, &Word);
		NbDebugPrint(3, ("SkGmInitMac: after GM_RX_CTRL = %04X\n", Word));
	}
...

	{
		...
		NbDebugPrint(3, ("SkGmInitMac: before GM_TX_FLOW_CTRL = %04X\n", Word));
	}
...

	{
		SK_U16 Word;
		GM_IN16(IoC, Port, GM_TX_FLOW_CTRL, &Word);
		NbDebugPrint(3, ("SkGmInitMac: after GM_TX_FLOW_CTRL = %04X\n", Word));
	}
...

	{
		...
		NbDebugPrint(3, ("SkGmInitMac: before GM_TX_PARAM = %04X\n", Word));
	}
...

	{
		SK_U16 Word;
		GM_IN16(IoC, Port, GM_TX_PARAM, &Word);
		NbDebugPrint(3, ("SkGmInitMac: after GM_TX_PARAM = %04X\n", Word));
	}
...

	{
		...
		NbDebugPrint(3, ("SkGmInitMac: before GM_SERIAL_MODE = %04X\n", Word));
	}
...

	{
		SK_U16 Word;
		GM_IN16(IoC, Port, GM_SERIAL_MODE, &Word);
		NbDebugPrint(3, ("SkGmInitMac: after GM_SERIAL_MODE = %04X\n", Word));
	}
	...
	for (i = 0; i < 3; i++) {
		...
		NbDebugPrint(3, ("SkGmInitMac: B2_MAC_2[%d] = %04X\n", i, SWord));
#ifdef WA_DEV_16
		/* WA for deviation #16 */
		...
		NbDebugPrint(3, ("SkGmInitMac: B2_MAC_1[%d] = %04X\n", i, SWord));
...
SK_BOOL	DoLoop)		/* Should a Phy LoopBack be set-up? */
{
	...
	if ((pPrt->PLinkSpeedCap & SK_LSPEED_CAP_1000MBPS) != 0) {
		...
		if (DoLoop) {
			...
			NbDebugPrint(3, ("SkGmInitPhyMarv: PHY_MARV_PHY_CTRL Write Value = %08X\n", SWord));
		}
		else
#endif /* !SK_SLIM */
	...
	NbDebugPrint(3, ("SkGmInitPhyMarv: PHY_MARV_CTRL = %08X\n", PhyCtrl));
...
int		Port)		/* Port Index (MAC_1 + n) */
{
	...
	NbDebugPrint(3, ("SkGmAutoNegDoneMarv: PHY_MARV_PHY_STAT = %08X\n", AuxStat));
	...
	NbDebugPrint(3, ("SkGmAutoNegDoneMarv: PauseMode = %08X\n", PauseMode));
	SK_OUT8(IoC, MR_ADDR(Port, GMAC_CTRL), PauseMode);
	{
		SK_U8 Byte;
		SK_IN8(IoC, MR_ADDR(Port, GMAC_CTRL), &Byte);
		NbDebugPrint(3, ("SkGmAutoNegDoneMarv: MR_ADDR(Port, GMAC_CTRL) = %08X\n", Byte));
	}
...
int		Port)		/* Port Index (MAC_1 + n) */
{
	...
	if (pAC->GIni.GIYukon) {
...

		{
			...
			SK_IN8(IoC, MR_ADDR(Port, GMAC_IRQ_MSK), &Byte);
			NbDebugPrint(3, ("SkMacRxTxEnable: before MR_ADDR(Port, GMAC_IRQ_MSK) = %02X\n", Byte));
		}
...

		{
			...
			SK_IN8(IoC, MR_ADDR(Port, GMAC_IRQ_MSK), &Byte);
			NbDebugPrint(3, ("SkMacRxTxEnable: after MR_ADDR(Port, GMAC_IRQ_MSK) = %02X\n", Byte));
		}

		/* get General Purpose Control */
		GM_IN16(IoC, Port, GM_GP_CTRL, &Reg);

		NbDebugPrint(3, ("SkMacRxTxEnable: before GM_GP_CTRL = %04X\n", Reg));
...

		{
			...
			NbDebugPrint(3, ("SkMacRxTxEnable: after GM_GP_CTRL = %02X\n", Word));
		}
...
//#ifdef XXX
		/* dummy read to ensure writing */
		GM_IN16(IoC, Port, GM_GP_CTRL, &Word);			
		NbDebugPrint(3, ("SkMacRxTxDisable: after GM_GP_CTRL = %02X\n", Word));
...
int		Port)		/* Port Index (MAC_1 + n) */
{
	...
	if (pAC->GIni.GIYukon) {
		/* disable all GMAC IRQs */
		{
			SK_U8 Byte;
			SK_IN8(IoC, MR_ADDR(Port, GMAC_IRQ_MSK), &Byte);
			NbDebugPrint(3, ("SkMacIrqDisable: before MR_ADDR(Port, GMAC_IRQ_MSK) = %02X\n", Byte));
		}
		SK_OUT8(IoC, MR_ADDR(Port, GMAC_IRQ_MSK), 0);
		{
			SK_U8 Byte;
			SK_IN8(IoC, MR_ADDR(Port, GMAC_IRQ_MSK), &Byte);
			NbDebugPrint(3, ("SkMacIrqDisable: after MR_ADDR(Port, GMAC_IRQ_MSK) = %02X\n", Byte));
		}
...
unsigned int Port)	/* Port Index (MAC_1 + n) */
{
	NbDebugPrint(3, ("SkGmUpdateStats: Nothing to do\n"));
	return(0);
}
...
unsigned int Port)	/* Port Index (MAC_1 + n) */
{
	...
	NbDebugPrint(3, ("SkGmResetCounter: GM_PHY_ADDR = %04X\n", Reg));
	...
	for (i = 0; i < GM_MIB_CNT_SIZE; i++) {
		...
		NbDebugPrint(3, ("SkGmResetCounter: GM_MIB_CNT_BASE[%d] = %04X\n", i, Word));
	}
