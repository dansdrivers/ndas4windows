...
int		Level)	/* Initialization Level */
{
	...
	case SK_INIT_RUN:
		...
		for (j = 0; j < SK_MAC_ADDR_LEN; j++) {
			...
			NbDebugPrint(3, ("SkRlmtInit: VirtualMacAddress.a[%d] = %02X\n", j, VirtualMacAddress.a[j]));
		...
		for (j = 0; j < SK_MAC_ADDR_LEN; j++) {
			...
			NbDebugPrint(3, ("SkRlmtInit: PhysicalAMacAddress.a[%d] = %02X\n", j, PhysicalAMacAddress.a[j]));
		...
		NbDebugPrint(3, ("SkRlmtInit: pAC->Rlmt.RlmtOff = %d\n", pAC->Rlmt.RlmtOff));
...
SK_EVPARA	Para)	/* SK_U32 PortNumber; SK_U32 -1 */
{
	...
	NbDebugPrint(3, ("SkRlmtEvtPortStartTim: pAC->Rlmt.Port[Para.Para32[0]].LinkDown = %d\n", pAC->Rlmt.Port[Para.Para32[0]].LinkDown));
...
SK_EVPARA	Para)	/* SK_U32 PortNumber; SK_U32 Undefined */
{
	...
	for (i = 0; i < pRPort->Net->NumPorts; i++) {
		NbDebugPrint(3, ("SkRlmtEvtLinkUp:pRPort->Net->Port[%d]->PortStarted = %d\n", i, pRPort->Net->Port[i]->PortStarted));
...
SK_EVPARA	Para)	/* SK_U32 NetNumber; SK_U32 -1 */
{
	...
	NbDebugPrint(3, ("SkRlmtEvtStart: pAC->Rlmt.Port[PortNumber].UpTimer Start\n"));
...
SK_EVPARA	Para)	/* SK_U32 NewMode; SK_U32 NetNumber */
{
	...
	NbDebugPrint(3, ("SkRlmtEvtModeChange: PrevRlmtMode = %08X, NewRlmtMode = %08X\n", PrevRlmtMode, pAC->Rlmt.Net[Para.Para32[1]].RlmtMode));
	NbDebugPrint(3, ("pAC->Rlmt.Net[Para.Para32[1]].NumPorts = %08X, pAC->Rlmt.Net[Para.Para32[1]].PortsUp = %08X\n", 
		pAC->Rlmt.Net[Para.Para32[1]].NumPorts,
		pAC->Rlmt.Net[Para.Para32[1]].PortsUp
		));
	...
	if ((PrevRlmtMode & SK_RLMT_CHECK_LOC_LINK) !=
		...
		if ((PrevRlmtMode & SK_RLMT_CHECK_OTHERS) == 0 &&
			...
			NbDebugPrint(3, ("pAC->Rlmt.Net[Para.Para32[1]].LocTimer Started\n"));
