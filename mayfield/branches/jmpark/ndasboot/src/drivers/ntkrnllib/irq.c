...
unsigned long do_IRQ_var;

KSPIN_LOCK irq_controller_lock;
...
static void do_none(unsigned int irq)
{
	 ...
	 */
	NbDebugPrint(2, ("unexpected IRQ vector %d on CPU#%d!\n",irq, smp_processor_id()));
...
//unsigned int cached_irq_mask = 0x0000;
 ...
 */
static __inline void mask_and_ack_8259A(unsigned int irq)
{
...
static void do_8259A_IRQ(unsigned int irq)
{
	...
	KeAcquireSpinLock(&irq_controller_lock, &Irql);
	{
		unsigned int status;
	...
	KeReleaseSpinLock(&irq_controller_lock, Irql);
 ...
 */
static __inline void check_smp_invalidate(int cpu)
{
...
static __inline void wait_on_bh(void)
{
	int count = MAXCOUNT;
	do {
		if (!--count) {
 ...
 * KeReleaseSpinLock()/KeAcquireSpinLock() on CPU#1 was racing with
 * KeAcquireSpinLock() on CPU#0. CPU#0 should have noticed KeReleaseSpinLock(), but
 * apparently the KeReleaseSpinLock() information did not make it
...
static __inline void wait_on_irq(int cpu)
{
	int count = MAXCOUNT;
...
static __inline void get_irqlock(int cpu)
{
	if (test_and_set_bit(0,&global_irq_lock)) {
...
void __global_restore_flags(unsigned long flags)
{
	...
	case 3:
		...
		break;
	default:
		NbDebugPrint(2, ("global_restore_flags: %08lx (%08lx)\n",
			flags, (&flags)[-1]));
	}
}
...
//		__sti();
	...
	do {
		status |= action->flags;
		action->handler(NULL, action->dev_id, NULL, NULL);
		action = action->next;
	} while (action);
...
void enable_irq(unsigned int irq)
{
	...
	default:
		...
		break;
	case 0:
//		NbDebugPrint(2, ("enable_irq() unbalanced from %p\n",
...
//	int irq = regs.orig_eax & 0xff; /* subtle, see irq.h */
	...
	NbDebugPrint(2, ("before do_IRQ: nic_irq = %d, irq = %d\n", nic_irq, irq));	
//	kstat.irqs[cpu][irq]++;
//	irq_desc[irq].handler->handle(irq, &regs);
	irq_desc[irq].handler->handle(irq);
...
//	}
	...
	NbDebugPrint(2, ("after do_IRQ: nic_irq = %d, irq = %d\n", nic_irq, irq));	
}
...
//	}
	...
	*p = new;
	...
	if (!shared) {
		//////////////////////////////////////////////////////////////////////////
		if( irq == nic_irq ) SetIntVector(REAL_IRQ(irq),(ULONG)OSNicISR) ;
		//////////////////////////////////////////////////////////////////////////
		
		/*
		{
			extern void OSDefaultISR(void);
			extern ULONG OSDefault;
			extern void OSUserDefault(void);
			
			if( irq == nic_irq ) {
				int i ;
				
				SetIntVector(REAL_IRQ(0x60 + irq),(ULONG)OSNicISR) ;				
			}
...
int request_irq(unsigned int irq, 
		void (*handler)(struct _KDPC *, PVOID, PVOID, PVOID),
		...
		void *dev_id)
{
	...
	NbDebugPrint(2, ("reqeust_irq : irq = %d, handler = %lx\n", irq, handler));
...
//#ifdef LINUX_KERNEL
	...
	NbDebugPrint(2, ("Trying to free free IRQ%d\n",irq));
out:
	KeReleaseSpinLock(&irq_controller_lock,Irql);
...
int probe_irq_off(unsigned long unused)
{
	...
	if (unused != 0x12345678)
		NbDebugPrint(2, ("Bad IRQ probe from %lx\n", (&unused)[-1]));
...
void IRQ_init(void)
{
	...
	KeInitializeSpinLock(&irq_controller_lock);
	...
	*/
	...
	cached_irq_mask = (slavemask<< 8) | mastermask ;	

	cached_irq_mask |= 0x8000;
	cached_irq_mask |= 0x0002; // Keyboard ?

//	cached_irq_mask |= 0x0001; ////////////////////////////////// Timer ///////////////////////////////////

	NbDebugPrint(0, ("cached_irq_mask = %04X\n", cached_irq_mask)) ;
//	cached_irq_mask |= 0x1000;
//	cached_irq_mask |= 0x0200;

//	cached_irq_mask |= 0x0004;	
//	cached_irq_mask |= 0x0040;
	
//	cached_irq_mask &= ~0x0001; // Timer
//	cached_irq_mask &= ~0x0002; // Keyboard ?
//	cached_irq_mask &= ~0x0004; // Interrupts 8-15
//	cached_irq_mask &= ~0x0008; // Com 2 ?
//	cached_irq_mask &= ~0x0010; // Com 1 ?
//	cached_irq_mask &= ~0x0020; // LPT 2 ?
//	cached_irq_mask &= ~0x0040; // Floppy Disk ?
//	cached_irq_mask &= ~0x0080; // LPT 1 ?
//	cached_irq_mask &= ~0x0100; // Real-Time Clock ?
//	cached_irq_mask &= ~0x0200; // Redirect to IRQ 2 ?

	/* 10 - 12 unsigned */
//	cached_irq_mask &= ~0x2000; // 80x87 Co-processor ?
//	cached_irq_mask &= ~0x4000; // Hard Disk ?
	...
	do_IRQ_var = (unsigned long) do_IRQ;
}
