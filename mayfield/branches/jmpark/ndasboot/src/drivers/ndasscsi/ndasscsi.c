...
PVOID  HwExtension;
PUCHAR BootParameters;

#ifndef __ENABLE_LOADER__


NTSTATUS GetBootParameters(PUCHAR *KeyInformation)
{
	HANDLE  key1;
	...
	if (NT_SUCCESS(status2)) {
		...
		KDPrint(0, ("ZwQueryValueKey: status = %X, keyValueLength = %d\n", status, keyValueLength));
		...
		*KeyInformation = ExAllocatePoolWithTag(NonPagedPool,
                                      keyValueLength,
                                      LSMP_PTAG_IOCTL);

		KDPrint(0, ("*KeyInformation = %p, Parameters = %ws\n", *KeyInformation, (PWCHAR)infoBuffer->Data));	

		
		if(NULL != *KeyInformation) {
			_snprintf(*KeyInformation, keyValueLength, "%ws", (PWCHAR)infoBuffer->Data);
		}
...
void DisplayResource(PCM_RESOURCE_LIST list)
{
    ULONG listNumber;
        ...
        for(resourceNumber = 0;
            ...
            switch(partialDescriptor->Type) {
                ...
                case CmResourceTypeMemory: {
					...
					KDPrint(0, ("%04X:Resource[%s] InterfaceType=%04X BusNumber=%04X Address=%I64X Length = %08X TranslatedAddress=%I64X addressSpace=%d\n", 
						resourceNumber,
						addressSpace ? "IO" : "ME",
						fullDescriptor->InterfaceType,
						fullDescriptor->BusNumber,
						partialDescriptor->u.Generic.Start.QuadPart, 
						partialDescriptor->u.Generic.Length, 
						tmp.u.Generic.Start.QuadPart,
						addressSpace
						));
                ...
                case CmResourceTypeInterrupt: {
					...
					KDPrint(0, ("%04X:Resource[%s] InterfaceType=%04X BusNumber=%04X Level=%04X Vector=%08X, TranslatedLevel=%04X TranslatedVector=%08X Affinity=%04X\n",
						resourceNumber,
						"IN",
						fullDescriptor->InterfaceType,
						fullDescriptor->BusNumber,
		...
		)
{
	...
	for(listNumber = 0; listNumber < list->Count; listNumber++) {
        ...
        for(resourceNumber = 0;
			...
			if(partialDescriptor->Type != ResourceType) 
				continue;
            ...
            memcpy(pResourceDescriptor,&tmp, sizeof(tmp));
		}
    ...
    )
{
	...
	KDPrint(0, ("KInterrupt = %p, Context = %p\n", KInterrupt, Context));

	return TRUE;
}

PDRIVER_OBJECT DriverObjectG;
UNICODE_STRING RegistryPathG;
    ...
    )
{
...
#ifdef __ENABLE_LOADER__
	BootParameters = (PVOID)RegistryPath;
	KDPrint(0, ("BootParameters = %s\n", BootParameters));
#else
	KDPrint(1,("%s %s Flags:%08lx\n", __DATE__, __TIME__, DriverObject->Flags));

	GetBootParameters(&BootParameters);
	KDPrint(0, ("BootParameters = %s\n", BootParameters));
#endif

#if 0
	DriverObjectG = DriverObject;
	RegistryPathG.MaximumLength = RegistryPath->Length +
                                          sizeof(UNICODE_NULL);
    RegistryPathG.Length = RegistryPath->Length;
    RegistryPathG.Buffer = ExAllocatePoolWithTag(
                                       PagedPool,
                                       RegistryPathG.MaximumLength,
                                       LSMP_PTAG_IOCTL
                                       );    

    if (!RegistryPathG.Buffer) {

        return STATUS_INSUFFICIENT_RESOURCES;
    }
    
    RtlCopyUnicodeString(&RegistryPathG, RegistryPath);
	...
	
	{
		...
		KDPrint(0, ("before HalTranslateBusAddress ioaddr = %08X\n", ioaddr));
		...
		for(i=0; i<0xff; i++) {
			...
			KDPrint(0, ("HalTranslateBusAddress: b = %d, addressSpace = %d, cardAddress = %I64X\n",
			b, addressSpace, cardAddress.QuadPart));

			if(b)	ioaddr = (ULONG)cardAddress.QuadPart;

			KDPrint(0, ("after HalTranslateBusAddress ioaddr = %08X\n", ioaddr));
		}
	}

#endif

#endif // #ifdef __NDASBOOT__
...
/*
	{
#define PORT_ADDR unsigned long
//
// Vendor and Device IDs.
//
UCHAR VenID[4] = {'1', '0', '0', 'B'};
UCHAR DevID[2] = {'2', '0'};
		...
		hwInitializationData.DeviceIdLength = 2;
		hwInitializationData.DeviceId = DevID;

		hwInitializationData.AdapterInterfaceType = PCIBus;
		hwInitializationData.HwFindAdapter = MiniFindAdapter;
		tStatus = ScsiPortInitialize(DriverObject, RegistryPath,
                (PHW_INITIALIZATION_DATA) &hwInitializationData, &srchContext);

		KDPrint(0,("PCIBus type: NTSTATUS=%08lx.\n", tStatus));
	}
...
/*		ULONG                           Vector;		    
		...
		BOOLEAN  SharedInterrupt = 1;
*/
		unsigned char mastermask, slavemask; 

		mastermask 	= ScsiPortReadPortUchar((PUCHAR)0x21);
		slavemask 	= ScsiPortReadPortUchar((PUCHAR)0xa1);

		KDPrint(0,	("mastermask = %02X, slavemask = %02X\n", mastermask, slavemask));	

		Status = IoConnectInterrupt(&InterruptObject,
                        (PKSERVICE_ROUTINE)TestIsr,
                        NULL,
                        NULL,
						0x3A,		// Vector
                        0x12, // Irql,
                        0x12, // Irql,
                        1, // (KINTERRUPT_MODE)InterruptMode,
                        0, // SharedInterrupt,
                        1, //	InterruptAffinity,
                        FALSE);

		KDPrint(0, ("IoConnectInterrupt: Status = %08X\n", Status));
...
s
			...
			KDPrint(0, ("%d th HalAssignSlotResources: Status = %08X\n\n", i, Status));

			if(NT_SUCCESS(Status)) {
				TranslateResource(AllocatedResources, CmResourceTypeInterrupt, &ResourceDescriptor);

				Irql = (KIRQL)ResourceDescriptor.u.Interrupt.Level;
				Vector = ResourceDescriptor.u.Interrupt.Vector;
				InterruptAffinity = ResourceDescriptor.u.Interrupt.Affinity;


				if (ResourceDescriptor.Flags == CM_RESOURCE_INTERRUPT_LEVEL_SENSITIVE)
				{
				...
				KDPrint(0, ("Irql=%04X Vector=%08X, InterruptAffinity=%04X, InterruptMode=%04X\n",
						Irql, 
						Vector,
						InterruptAffinity,
						InterruptMode
...
*/
	...
	while(NT_SUCCESS(isaStatus)) {
		...
		status = LsuRegisterTdiPnPPowerHandler(&svcName, ClientPnPPowerChange_Delay, &_NdscGlobals.TdiPnP);
...
GetDefaultLurFlags(
	...
	*DefaultLurFlags = 0;
	...
	//
	//	= Flag settings =
	//	This setting apply only for NDAS chip 1.0, 1.1, 2.0.
	//	There should be flag reorganization for newer chip
	//
	//	* Primary/secondary write ( Primary only writes to an NDAS device )
	//		LURFLAG_WRITESHARE_PS: 0n
	//		LURFLAG_LOCKEDWRITE: Off
	//		LURFLAG_SHAREDWRITE: Off
	//		LURFLAG_OOB_SHAREDWRITE: Off
	//
	//	* Primary/secondary write with OOB shared write
	//			( Primary only writes to an NDAS device,
	//				but multi-hosts write to out-of-bound address )
	//		LURFLAG_WRITESHARE_PS: 0n
	//		LURFLAG_LOCKEDWRITE: On
	//		LURFLAG_SHAREDWRITE: Off
	//		LURFLAG_OOB_SHAREDWRITE: On
	//
	//	* Shared write ( multi-hosts write to an NDAS device )
	//		LURFLAG_WRITESHARE_PS: 0n
	//		LURFLAG_LOCKEDWRITE: On
	//		LURFLAG_SHAREDWRITE: On
	//		LURFLAG_OOB_SHAREDWRITE: Off
...
#endif
//	*DefaultLurFlags |= LURFLAG_LOCKEDWRITE;
//	*DefaultLurFlags |= LURFLAG_SHAREDWRITE;
    ...
    )
{
	...
	HwExtension = (PVOID)HwDeviceExtension;
#endif 
	...
	{		
		...
		ioSpace = ScsiPortGetDeviceBase(HwDeviceExtension,
										...
										0,										
                                        ScsiPortConvertUlongToPhysicalAddress(0x2400),
                                        0x100,
                                        TRUE);
		

		KDPrint(0, ("AdapterType = %x, IoBusNumber = %x, IoSpace = %p, virtual IoSpace = %p\n", 
			ConfigInfo->AdapterInterfaceType,
			ConfigInfo->SystemIoBusNumber, 
			ioSpace,
			ScsiPortGetVirtualAddress(HwDeviceExtension, ScsiPortConvertUlongToPhysicalAddress((ULONG_PTR)ioSpace))
		));
		...
		KDPrint(0, ("AdapterType = %x, IoBusNumber = %x, IoSpace = %p\n", 
			ConfigInfo->AdapterInterfaceType,
			ConfigInfo->SystemIoBusNumber, 
			ioSpace, 
			ScsiPortGetVirtualAddress(HwDeviceExtension, ScsiPortConvertUlongToPhysicalAddress((ULONG_PTR)ioSpace))
		...
		KDPrint(0, ("base address value = %08X\n", (ULONG) ScsiPortReadPortUlong((PULONG)(ioSpace))));
		...
		for(i=0;i<0x100;i++) {
			{
				...
				KDPrint(0, ("READ_PORT_UCHAR: %04X th value = %08X\n", i, (ULONG) READ_PORT_UCHAR((PUCHAR)i)));

				KDPrint(0, ("READ_PORT_ULONG: %04X th value = %08X\n", i, (ULONG) READ_PORT_ULONG((PULONG)i)));

				KDPrint(0, ("ScsiPortGetVirtualAddress: %04X th value = %08X\n", i, 
					(ULONG) ScsiPortGetVirtualAddress(HwDeviceExtension, ScsiPortConvertUlongToPhysicalAddress((ULONG_PTR)i))));

				Value = readl(Port);
				KDPrint(0, ("%04X th value = %08X\n", i, Value));
			}
		}
	}
...
#ifdef __ENABLE_LOADER__
	HwDeviceExtension->ScsiportFdoObject			= 0;
#else
	...
	KDPrint(0, ("Initialize NetBoot\n"));

	result = NDASBootInitialize();
	if(result < 0) {
		ScsiPortLogError(
				HwDeviceExtension,
				NULL,
	...
	result = GetScsiAdapterInfo(HwDeviceExtension, ConfigInfo->SystemIoBusNumber, &AddDevInfoLength, &AddDevInfo, &AddDevInfoFlag);
	if(result != SP_RETURN_FOUND) {
		ScsiPortLogError(
				HwDeviceExtension,
				NULL,
...
#ifdef __NDASBOOT__
	KDPrint(2,("MiniStartIo: Srb = %p, Srb->Function=%d, Srb->Cdb[0] = %02X\n", Srb, (LONG)Srb->Function, (LONG)Srb->Cdb[0]));	

#ifdef __INTERRUPT__
	__asm sti // for Keyboard input	
#endif
#endif
