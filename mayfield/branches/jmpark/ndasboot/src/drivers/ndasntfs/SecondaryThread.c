	...
	)
{
	...
	DebugTrace2( 0, Dbg2, ("SecondaryThreadProc: Start Secondary = %p\n", Secondary) );
	...
	Secondary->Thread.SessionContext.MessageSecurity		= Secondary->VolDo->NetdiskInformation.MessageSecurity;
	Secondary->Thread.SessionContext.RwDataSecurity			= Secondary->VolDo->NetdiskInformation.RwDataSecurity;
	...
	DebugTrace2( 0, Dbg2, ("SecondaryThreadProc: Start Secondary = %p Secondary->Thread.SessionStatus = %x\n", 
	...
	while (secondaryThreadTerminate == FALSE) {
		...
		timeOut.QuadPart = -NDFAT_SECONDARY_THREAD_FLAG_TIME_OUT;
		...
		if (eventStatus == STATUS_TIMEOUT) {
			...
			if (Secondary->TryCloseTime.QuadPart > currentTime.QuadPart || 
				(currentTime.QuadPart - Secondary->TryCloseTime.QuadPart) >= NDFAT_TRY_CLOSE_DURATION) {
					...
					if (status == STATUS_SUCCESS) {
						...
						if (requestCount[ndfsWinxpReplyHeader->IrpMajorFunction] %100 == 0) {
							...
							default:
								...
								DebugTrace2( 0, Dbg2, ("%s, requestCount = %d, executionTime = %I64d\n",
   ...
   )
{
	...
	do { // just for structural programing
		...
		RtlCopyMemory( ndfsRequestSetup->NetDiskNode,
					   Secondary->VolDo->NetdiskInformation.NetDiskAddress.Node,
					   6 );

		ndfsRequestSetup->NetDiskPort = HTONS(Secondary->VolDo->NetdiskInformation.NetDiskAddress.Port);
		ndfsRequestSetup->UnitDiskNo = Secondary->VolDo->NetdiskInformation.UnitDiskNo;

		ndfsRequestSetup->StartingOffset = 0;
		ndfsRequestSetup->ResponseLength = 0;

#if 1 //def __ND_FAT_DES__
		{
			...
			MD5Update( &context, Secondary->VolDo->NetdiskInformation.Password, 8 );			
...
SEND_TREE_CONNECT_COMMAND:
		...
		ndfsRequestTreeConnect->StartingOffset = Secondary->VolDo->PartitionInformation.StartingOffset.QuadPart;
