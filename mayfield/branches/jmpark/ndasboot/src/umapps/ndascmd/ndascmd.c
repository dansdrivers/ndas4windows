...
#ifdef _DEBUG
		{ _T("unmountex"), ndascmd_unmount_ex },
#endif
	...
	LPVOID lpContext)
{
	...
	_tprintf(_T("    Unit Device %d: %s"),
		pEntry->UnitNo + 1,
		get_unitdevice_type_string(udinfo.UnitDeviceType));
	...
	LPVOID lpContext)
{
	...
	success = conver_ndas_device_id_to_string(szDisplayID, RTL_NUMBER_OF(szDisplayID), 
	...
	PNDASUSER_LOGICALDEVICE_MEMBER_ENTRY lpEntry, LPVOID lpContext)
{
	...
	{
		TCHAR szDisplayID[30];
		success = conver_ndas_device_id_to_string(
			szDisplayID, 
	...
	*/
	...
	else 
	{
		if (IS_NDAS_LOGICALDEVICE_TYPE_DISK_GROUP(info.LogicalDeviceType)) 
		{
			...
			success = convert_blocksize_to_string(
				szBlockSize, 
...
{
	static LPCTSTR USAGE = _T("[/wait] {<ldn> | /device <dn>} [/lockedwrite[+|-] [/oobsharedrw[+|-] {ro|rw}");
	...
	/* LDPF_LOCKED_WRITE */
	if (-1 != get_named_arg_pos(argc, argv, _T("lockedwrite"), NULL) ||
		-1 != get_named_arg_pos(argc, argv, _T("lockedwrite+"), NULL))
	{
		ldpfFlags |= NDAS_LDPF_LOCKED_WRITE;
		ldpfValues |= NDAS_LDPF_LOCKED_WRITE;
	}
	else if (-1 != get_named_arg_pos(argc, argv, _T("lockedwrite-"), NULL))
	{
		ldpfFlags |= NDAS_LDPF_LOCKED_WRITE;
		ldpfValues &= ~(NDAS_LDPF_LOCKED_WRITE);
	}

	/* NDAS_LDPF_OOB_SHARED_WRITE */
	if (-1 != get_named_arg_pos(argc, argv, _T("oobsharedrw"), NULL) ||
		-1 != get_named_arg_pos(argc, argv, _T("oobsharedrw+"), NULL))
	{
		ldpfFlags |= NDAS_LDPF_OOB_SHARED_WRITE;
		ldpfValues |= NDAS_LDPF_OOB_SHARED_WRITE;
	}
	...
	/* If NDAS_LDPF_OOB_SHARED_WRITE is enabled, 
	   LDPF_LOCKED_WRITE must be enabled.*/

	if ((ldpfFlags & NDAS_LDPF_OOB_SHARED_WRITE) &&
		(ldpfValues & NDAS_LDPF_OOB_SHARED_WRITE))
	{
		ldpfFlags |= NDAS_LDPF_LOCKED_WRITE;
		ldpfValues |= NDAS_LDPF_LOCKED_WRITE;
	...
	if (ejectParam.ConfigRet != CR_SUCCESS)
	{
		if (ejectParam.ConfigRet == CR_REMOVE_VETOED)
		{
			_tprintf(
				...
				get_veto_type_string(ejectParam.VetoType),
				ejectParam.VetoName);
		}
		else
		{
	...
	LPVOID lpContext)
{
	...
	success = convert_guid_to_string(szHostID, RTL_NUMBER_OF(szHostID), lpHostGuid);
	_ASSERTE(success);
