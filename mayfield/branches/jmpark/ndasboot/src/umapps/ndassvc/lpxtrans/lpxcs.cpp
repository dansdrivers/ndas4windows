	...
	CONST BYTE* pbData)
{
	...
	if (WAIT_OBJECT_0 == dwWaitResult || NULL == m_lpSocketAddressList) {
		...
		if (NULL == m_lpSocketAddressList) {
			XTLTRACE("Getting Local Lpx Address list failed, error=0x%X\n",
				GetLastError());
			return FALSE;
		}
	...
	for (DWORD i = 0; i < nLocalAddrs && i < m_nSenders; ++i) {
		...
		if (!fSuccess) {
			XTLTRACE("Creating a socket failed, error=0x%X\n", 
				GetLastError());
			continue;
		}
			...
			sizeof(BROADCAST_ADDR)))
		{
			...
			if (!fSuccess) {
				XTLTRACE("Setting a sock option to broadcast failed, error=0x%X\n",
					GetLastError());
		...
		if (!fSuccess) {
			...
			XTLTRACE("Binding a sock %d to %s failed, error=0x%X\n",
		...
		if (!fSuccess) {
			DWORD error = GetLastError();
			XTLTRACE("Sending at %d failed, error=0x%X\n",
				i, GetLastError());
			(VOID) m_senders[i].Close();
			SetLastError(error);
			continue;
	...
	DWORD nMaxRecvHint)
{
	...
	_ASSERTE(fSuccess);
	if (!fSuccess) {
		XTLTRACE("SetWaitableTimer failed, error=0x%X\n",
			GetLastError());
		return FALSE;
	}
	...
	if (NULL == hWaitingEvents) {
		XTLTRACE("Allocating hWaitingEvents %d failed, error=0x%X\n",
	...
	for (; i < nLocalAddrs && i < m_nSenders; ++i) {
		...
		if (!fSuccess) {
			XTLTRACE("Initiating receive failed, error=0x%X\n",
				GetLastError());
		}
	...
	do {
			...
			dwWaitResult <= WAIT_OBJECT_0 + 1 + m_nSenders)
		{
			...
			if (!fCont) {
				XTLTRACE("Gathering stopped on Processor Request\n");
				return TRUE;
			}
			...
			XTLTRACE("Waiting failed, error=0x%X\n",
				GetLastError());
			_ASSERTE(FALSE);
			return FALSE;
		}
	...
	HANDLE hStopEvent)
{
	...
	do {
		...
		for (DWORD i = 0; i < nLocalAddrs; ++i) {
			...
			if (!fSuccess) {
				XTLTRACE("Creating a sock, address=%s, error=0x%X\n",
			...
			if (!fSuccess) {
				XTLTRACE("bind failed, address=%s, error=0x%X\n",
					CSockLpxAddr(pSockAddr).ToStringA(), GetLastError());
				XTLVERIFY( m_listeners[i].Close() );
				continue;
			}
		...
		do {
			...
			if (WAIT_OBJECT_0 == dwWaitResult) {
				...
				XTLTRACE("SockAddrListChange event issued.\n");
				break;
