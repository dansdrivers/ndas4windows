...
struct EffectiveNdasAccess : std::unary_function<NDAS_SCSI_LOCATION,void> {
	EffectiveNdasAccess() : 
		AllowedAccess(GENERIC_READ | GENERIC_WRITE) {
	}
	void operator()(const NDAS_SCSI_LOCATION& location) {

		XTLTRACE2(TCPnp, TLInfo, "Volume on (%d,%d,%d)\n", 
			location.SlotNo, location.TargetID, location.LUN);

		CNdasLogicalDevicePtr pLogDevice = pGetNdasLogicalDevice(location);
		if (CNdasLogicalDeviceNullPtr == pLogDevice)
		{
			XTLTRACE2(TCPnp, TLWarning, "Logical device is not available. Already unmounted?\n");
			return;
		}
		...
		if (NDAS_LOGICALDEVICE_STATUS_MOUNTED == status ||
			NDAS_LOGICALDEVICE_STATUS_MOUNT_PENDING == status) 
		{
			XTLTRACE2(TCPnp, TLWarning, "Logical device is mounted or mount pending. Already unmounted?\n");
			return;
		}
...
Initialize(HANDLE hRecipient, DWORD dwReceptionFlag)
{
	...
	if (m_hStoragePortNotify.IsInvalid()) 
	{
		XTLTRACE2_ERR(TCPnp, TLError, 
			"Registering Storage Port Device Notification failed.\n");
	}
	...
	{
		XTLTRACE2_ERR(TCPnp, TLError, 
			"Registering Volume Device Notification failed.\n");
	}
	...
	{
		XTLTRACE2_ERR(TCPnp, TLError, 
			"Registering Disk Device Notification failed.\n");
	}
	...
	{
		XTLTRACE2_ERR(TCPnp, TLError, 
			"Registering CDROM Device Notification failed.\n");
	}

	XTLTRACE2(TCPnp, TLInfo, "Storage Port Notify Handle: %p\n", m_hStoragePortNotify);
	XTLTRACE2(TCPnp, TLInfo, "Volume Notify Handle      : %p\n", m_hVolumeNotify);
	XTLTRACE2(TCPnp, TLInfo, "Disk Device Notify Handle : %p\n", m_hDiskNotify);
	XTLTRACE2(TCPnp, TLInfo, "CDROM Device Notify Handle : %p\n", m_hCdRomClassNotify);
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCPnp, TLError, "NdasRoFilterQueryServiceStatus failed.\n");
		return TRUE;
	}
	...
	if (SERVICE_RUNNING != serviceStatus.dwCurrentState) 
	{
		...
		if (!fSuccess) 
		{
			XTLTRACE2_ERR(TCPnp, TLError, "NdasRoFilterStartService failed.\n");
			return TRUE;
		}
	}
	...
	{
		XTLTRACE2_ERR(TCPnp, TLError, "NdasRoFilterCreate failed.\n");
		return TRUE;
	}
...
CNdasServiceDeviceEventHandler::Uninitialize()
{
	...
	//	else
	//	{
	//		XTLTRACE2_ERR(TCPnp, TLWarning, "Failed to stop ROFilter session.\n");
	//		ret = false;
	//	}
	//}
	...
	PDEV_BROADCAST_DEVICEINTERFACE pdbhdr)
{
	...
	do 
	{
		...
		fSuccess = pGetNdasSlotNumberForScsiPortDeviceName(
			pdbhdr->dbcc_name, &dwSlotNo);

		if (!fSuccess && ERROR_DEV_NOT_EXIST == ::GetLastError())
		{
			// Wait for the instance of actual NDASSCSI miniport 
			XTLTRACE2(TCPnp, TLError,
		...
		{
			XTLTRACE2_ERR(TCPnp, TLError, 
				"pGetNdasSlotNumberForScsiPortDeviceName(%ls) failed.\n", 
				(LPCTSTR) pdbhdr->dbcc_name);
		}

		++retry;

	} while (!fSuccess && ERROR_DEV_NOT_EXIST == ::GetLastError() && retry < MaxWait);
...
//	if (!fSuccess) 
//	{
//		XTLTRACE2_ERR(TCPnp, TLWarning, "NdasDmGetNdasLogDevSlotNoOfDisk(%p) failed.\n", hStoragePort);
//	//	XTLTRACE2_ERR(TCPnp, TLWarning, "May not be NDAS logical device. Ignoring.\n");
//	//	return TRUE;
//	}
//	XTLTRACE2(TCPnp, TLInfo, "NdasDmGetNdasLogDevSlotNoOfScsiPort returned %d.\n", dwSlotNo2);
//#endif
	...
	XTLTRACE2(TCPnp, TLInfo, "NDAS Logical Device Slot No: %d.\n", dwSlotNo);
	...
	{
		XTLTRACE2_ERR(TCPnp, TLWarning, 
			"Registering StoragePort device handle %p for notification failed.\n",
			hStoragePort);
	}
	else
	{
	...
	const DeviceNotifyData& Data)
{
	...
	{
		XTLTRACE2_ERR(TCPnp, TLError, 
			"Registering device handle %p failed.\n", hDevice);
		return false;
	}
...
namespace
{
	typedef std::vector<NDAS_SCSI_LOCATION> NdasScsiLocationVector;
	BOOL CALLBACK 
	OnNdasVolume(
		const NDAS_SCSI_LOCATION* NdasScsiLocation,
		LPVOID Context)
	{
		NdasScsiLocationVector* v = 
			static_cast<NdasScsiLocationVector*>(Context);
		v->push_back(*NdasScsiLocation);
		return TRUE;
	}
}
...
//	}
	...
	NdasScsiLocationVector ndasScsiLocations;
	ndasScsiLocations.reserve(1);
	...
	{    
		XTLTRACE2_ERR(TCPnp, TLError, 
			"NdasEnumNdasScsiLocationsForVolume(hVolume %p) failed.\n", hVolume);
		XTLTRACE2(TCPnp, TLError, "May not span a NDAS logical device!!!\n");
...
//#endif
	...
	std::for_each(
		ndasScsiLocations.begin(),
		ndasScsiLocations.end(),
		getEffectiveAccess);

	ACCESS_MASK effectiveAccess = getEffectiveAccess.AllowedAccess;


	XTLTRACE2(TCPnp, TLInfo, "Volume has granted as 0x%08X (%s %s).\n", 
		effectiveAccess,
	...
	{
//		XTLTRACE2(TCPnp, TLInfo, "Loading ROFilter.\n");
//
//		//
...
//			if (!fSuccess) 
//			{
//				XTLTRACE2_ERR(TCPnp, TLError, "NdasRoFilterStartFilter failed.\n");
//				// TODO: Return BROADCAST_QUERY_DENY?
//				return TRUE;
//			}
//			XTLTRACE2(TCPnp, TLInfo, "RoFilter filtering started a session.\n");
//		}
//
...
OnVolumeDeviceInterfaceRemoveComplete(
	PDEV_BROADCAST_DEVICEINTERFACE pdbhdr)
{
	XTLTRACE2(TCPnp, TLInfo, "Remove complete %ls\n", pdbhdr->dbcc_name);
	return TRUE;
}
	...
	PDEV_BROADCAST_HANDLE pdbch)
{
	...
	//		if (CNdasLogicalDeviceNullPtr == pLogDevice) 
	//		{
	//			XTLTRACE2(TCPnp, TLWarning, "Logical Device not found at slot %d.\n", dni.LogDevSlotNo);
	//		}
	//		else
	//		{
...
{
	XTLTRACE2(TCPnp, TLTrace, "CNdasServicePowerEventHandler::Initialize.\n");
	return true;
}
...
{
	XTLTRACE2(TCPnp, TLInfo, "OnQuerySuspend.\n");
...
{
	XTLTRACE2(TCPnp, TLInfo, "OnQuerySuspendFailed.\n");
	return;
}
...
OnSuspend()
{
	XTLTRACE2(TCPnp, TLInfo, "OnSuspend.\n");
	return;
}
...
OnResumeAutomatic()
{
	XTLTRACE2(TCPnp, TLInfo, "OnResumeAutomatic.\n");
	return;
}
...
OnResumeCritical()
{
	XTLTRACE2(TCPnp, TLInfo, "OnResumeCritical.\n");
	return;
}
...
OnResumeSuspend()
{
	XTLTRACE2(TCPnp, TLInfo, "OnResumeSuspend.\n");
	return;
}
	...
	HANDLE hDisk)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCPnp, TLWarning, "GetStorageHotplugInfo failed.\n");
		return;
	} 
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCPnp, TLWarning, "StorageHotplugInfo failed.\n");
	}
