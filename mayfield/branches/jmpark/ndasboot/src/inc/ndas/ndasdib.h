...
static const NDAS_BLOCK_LOCATION NDAS_BLOCK_LOCATION_DMBI	= -6;		// Divided Multi-boot Location

static const unsigned _int64 NDAS_DMBI_SIGNATURE = 0x3F404A9FF4495F9F;
static const unsigned _int32 NDAS_DMBI_VERSION_MAJOR = 0;
static const unsigned _int32 NDAS_DMBI_VERSION_MINOR = 1;

#define MAX_HOST_ID_LEN	14
typedef struct _HOST_DISK_LOCATION
{
	unsigned _int8	Valid;
	unsigned _int8	Shared;
	unsigned _int8	HostID[MAX_HOST_ID_LEN];
	unsigned _int64 Start;
	unsigned _int64 Sectors;
} HOST_DISK_LOCATION, *PHOST_DISK_LOCATION;

#define MAX_HOST_ID		12
typedef struct _NDAS_DMBI_INFORMATION
{
	union{
		struct {
			...
			unsigned _int32 HostCount;
			HOST_DISK_LOCATION	Host[MAX_HOST_ID];
		};
		unsigned char bytes_512[512];
	};
} NDAS_DMBI_INFORMATION, *PNDAS_DMBI_INFORMATION;

typedef struct _FDISK_PARTITION {
  unsigned _int8 Bootid;   /* bootable?  0=no, 128=yes  */
  unsigned _int8 Beghead;  /* beginning head number */
  unsigned _int8 Begsect;  /* beginning sector number */
  unsigned _int8 Begcyl;   /* 10 bit nmbr, with high 2 bits put in begsect */	
  unsigned _int8 Systid;   /* Operating System type indicator code */
  unsigned _int8 Endhead;  /* ending head number */
  unsigned _int8 Endsect;  /* ending sector number */
  unsigned _int8 Endcyl;   /* also a 10 bit nmbr, with same high 2 bit trick */
  unsigned _int32 Relsect;            /* first sector relative to start of disk */
  unsigned _int32 Numsect;            /* number of sectors in partition */
} PARTITION, *PPARTITION;

typedef struct _MASTER_BOOT_RECORD {
  _int8    Bootinst[446];   /* space to hold actual boot code */
  _int8    Parts[4 * sizeof (PARTITION)];
  unsigned _int16  Signature;       /* set to 0xAA55 to indicate PC MBR format */
...
#define NMT_RAID1R			10		// with repair, from 3.11
#define NMT_RAID4R			11		// with repair, from 3.11
