...
BOOL DisableDiskWriteCache(HANDLE hDisk)
{
	...
	if (!fSuccess) {
		DBGPRT_ERR_EX(_FT("DeviceIoControl(IOCTL_DISK_GET_CACHE_INFORMATION) failed: "));
		return FALSE;
	}

	DBGPRT_INFO(_FT("Disk Write Cache Enabled: %d.\n"), diskCacheInfo.WriteCacheEnabled);
	if (!diskCacheInfo.WriteCacheEnabled) {
		DBGPRT_INFO(_FT("Disk Write Cache Already Disabled. Ignoring.\n"));
		return TRUE;
	}
	...
	if (!fSuccess) {
		DBGPRT_ERR_EX(_FT("DeviceIoControl(IOCTL_DISK_SET_CACHE_INFORMATION) failed: "));
		return FALSE;
	}

	DBGPRT_INFO(_FT("Disable Write Cache (%p) completed successfully.\n"), hDisk);
	return TRUE;
...
BOOL DisableDiskWriteCache(LPCTSTR szDiskPath)
{
	...
	if (INVALID_HANDLE_VALUE == hDisk) {
		DBGPRT_ERR_EX(_FT("CreateFile(%s) failed: "), szDiskPath);
		return FALSE;
	}
...
LPCTSTR 
NDIS_PHYSICAL_MEDIUM_String(sysutil::NDIS_PHYSICAL_MEDIUM m)
{
	switch (m) {
