...
typedef struct _NTFS_DATA {
    ...
    UNICODE_STRING EncryptionService;
...
#define NtfsInitializeRestartPointers(P) {          \
    ...
    ExInitializeResource( &(P)->Resource );         \
...
typedef struct _NTFS_CLUSTER_RUN {

    LCN Lcn;
    LONGLONG RunLength;

} NTFS_CLUSTER_RUN, *PNTFS_CLUSTER_RUN;

typedef struct _NTFS_CACHED_RUNS {

    //
    //  Pointer to the pool for the information.
    //  This buffer has two equal sizes.  One sorted by Lcn, the
    //  other sorted by run length.
    //

    PNTFS_CLUSTER_RUN ClusterArray;

    //
    //  Indexes which indicate the total size of the
    //  arrays and the number used.
    //
...
#define NtfsConvertWcharToUlong(B,L,U,H) {      \
    PWCHAR _Current = (B);                      \
...
typedef struct _VCB {
    ...
    ULONG CachedRunsMaximumSize;
    ULONG CachedRunsTrimToSize;
...
#define VCB_STATE_TARGET_DEVICE_STOPPED     (0x00008000)
#define VCB_STATE_EXPLICIT_LOCK             (0x00010000)
#define VCB_STATE_DISALLOW_DISMOUNT         (0x00020000)
#define VCB_STATE_USN_JOURNAL_ACTIVE        (0x00040000)
#define VCB_STATE_VALID_OBJECT_ID           (0x00080000)
#define VCB_STATE_OBJECT_ID_CLEANUP         (0x00100000)
#define VCB_STATE_USN_DELETE                (0x00200000)
#define VCB_STATE_USN_JOURNAL_PRESENT       (0x00400000)
...
typedef struct _FCB {
    ...
    //
    //  The encryption context for the unnamed stream, passed to the encryption
    //  driver for subsequent named stream creates.
    //

    PVOID EncryptionContextHint;
...
typedef struct _SCB {
    ...
    //
    //  High threshold for lazy write.  We want to throw away lazy
    //  write beyond the last write for the file.  This is to
    //  prevent a stale write from advancing valid data to disk
    //  over an unwritten range.
    //

    LONGLONG LazyWriteHighThreshold;
...
#define SCB_STATE_WRITE_COMPRESSED          (0x00100000)  
...
#define SCB_STATE_PROTECT_SPARSE_MCB        (0x10000000)  
#define SCB_STATE_MULTIPLE_OPENS            (0x20000000)  
#define SCB_STATE_COMPRESSION_CHANGE        (0x40000000)  
#define SCB_STATE_WRITE_FILESIZE_ON_CLOSE   (0x80000000)
...
//
//  BUGBUG this flag's value is now superseded by SCB_STATE_WRITE_FILESIZE_ON_CLOSE
...
#define IRP_CONTEXT_STATE_ATTEMPTED_LOG_FLUSH           (0x00040000)
...
typedef struct _OPLOCK_CLEANUP {
    ...
    PCREATE_COMPLETION_CONTEXT CompletionContext;
