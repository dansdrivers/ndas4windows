...
/*
BOOL CNBListView::InsertNBNdasDevice(CNBNdasDevicePtr NBNdasDevice)
{
	if(dynamic_cast<const CNBLogicalDevice *>(NBNdasDevice.get()) != NULL)
	{		
		CNBLogicalDevicePtr NBLogicalDevice = 
			boost::dynamic_pointer_cast<CNBLogicalDevice>(NBNdasDevice);

		return InsertNBLogicalDevice(NBLogicalDevice);
	}
	else if(dynamic_cast<const CNBUnitDevice *>(NBNdasDevice.get()) != NULL)
	{
		CNBUnitDevicePtr NBUnitDevice =
			boost::dynamic_pointer_cast<CNBUnitDevice>(NBNdasDevice);
