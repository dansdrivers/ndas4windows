...
// return TRUE if normal status is never found
BOOL
CNdasLogicalDevice::CheckFirstNormalStatus(BOOL bNormalStatusCalled)
{
	BOOL bOldStatus = m_bNormalStatusCalled;
	m_bNormalStatusCalled = TRUE;
	return !bOldStatus;
}
...
DWORD 
CNdasLogicalDevice::GetUnitDeviceCountSpare()
{
	ximeta::CAutoLock autolock(this);
...
DWORD 
CNdasLogicalDevice::GetUnitDeviceCountInRaid()
{
	ximeta::CAutoLock autolock(this);
...
CNdasLogicalDevice::GetUnitDevice(DWORD ldSequence)
{
	...
	CNdasUnitDevice* pUnitDevice = m_pUnitDevices[ldSequence];
...
static bool 
IsLogicalDVDMountedAsRW(CNdasLogicalDevice* pLogDevice)
{
