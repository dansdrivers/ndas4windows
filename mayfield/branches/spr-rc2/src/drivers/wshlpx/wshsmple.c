...
MAPPING_TRIPLE TcpMappingTriples[] = { AF_UNSPEC,   SOCK_STREAM, IPPROTO_LPXTCP,
                                       AF_UNSPEC,   SOCK_STREAM, 0,
                                       AF_UNSPEC,   0,           IPPROTO_LPXTCP,
                                       AF_UNSPEC, 0,           IPPROTO_LPXTCP,
                                       AF_UNSPEC, SOCK_STREAM, IPPROTO_LPXTCP };

MAPPING_TRIPLE UdpMappingTriples[] = { AF_UNSPEC,   SOCK_DGRAM,  IPPROTO_LPXUDP,
                                       AF_UNSPEC,   SOCK_DGRAM,  0,
                                       AF_UNSPEC,   0,           IPPROTO_LPXUDP,
                                       AF_UNSPEC, 0,           IPPROTO_LPXUDP,
                                       AF_UNSPEC, SOCK_DGRAM,  IPPROTO_LPXUDP };

MAPPING_TRIPLE RawMappingTriples[] = { AF_UNSPEC,   SOCK_RAW,    0 };
...
//                | XP1_EXPEDITED_DATA
...

        {
            ...
            IPPROTO_LPXRAW,                         // iProtocol
            255,                                    // iProtocolMaxOffset
            BIGENDIAN,                              // iNetworkByteOrder
            SECURITY_PROTOCOL_NONE,                 // iSecurityScheme
            UDP_MESSAGE_SIZE,                       // dwMessageSize
            0,                                      // dwProviderReserved
            L"MSAFD Lpx [RAW/LPX]"                 // szProtocol
#endif
        }
...
{
    UNALIGNED SOCKADDR_LPX *sockaddr = (PSOCKADDR_LPX)Sockaddr;
    ULONG i;
	// Added by jgahn 2002/ 7/30
	int		count;
	ULONG	sum;
	// End Adding.
	...
	*/ 
	...
	for(count = 0; count < 6; count++)
		sum += sockaddr->LpxAddress.Node[count];
...

{
    ...
    if ( Level != SOL_SOCKET &&
         Level != IPPROTO_LPXTCP &&
         Level != IPPROTO_LPXUDP &&
         Level != IPPROTO_IP ) {
        return WSAEINVAL;
    }
...

{
                    ...
                    *Protocol ) )
    {
        ...
        if (TransportDeviceName->Buffer == NULL) {
			DebugPrint((1, "[WshLpx]WSHOpenSocket2: TransportDeviceName->Buffer == NULL\n"));
...

{
    ...
    if ( Level == IPPROTO_LPXTCP && OptionName == TCP_NODELAY ) {
    ...
    if ( Level == IPPROTO_LPXTCP && OptionName == TCP_EXPEDITED_1122 ) {
    ...
    if ( Level == IPPROTO_IP ) {
        ...
        if ( OptionName == IP_OPTIONS ) {
            ...
            if ( context->IpOptionsLength < OptionLength ) {
                ...
                if (temp == NULL) {
					DebugPrint((1, "[WshLpx]WSHSetSocketInformation: temp == NULL\n"));
...

{
    ...
    if ( ARGUMENT_PRESENT( lpiProtocols ) ) {
        ...
        for ( i = 0; lpiProtocols[i] != 0; i++ ) {
            if ( lpiProtocols[i] == IPPROTO_LPXTCP ) {
                useTcp = TRUE;
            }
            if ( lpiProtocols[i] == IPPROTO_LPXUDP ) {
                useUdp = TRUE;
            }
        }
...

{
	...
	DebugPrint((2, "[WshLpx]SetTdiInformation: Entered\n"));
    ...
    if ( ioStatusBlock == NULL ) {
		DebugPrint((1, "[WshLpx]SetTdiInformation: ioStatusBlock == NULL\n"));
    ...
    if ( WaitForCompletion ) {
       ...
       if (event == NULL) {
			...
			DebugPrint((1, "[WshLpx]SetTdiInformation: event == NULL\n"));
    ...
    if ( !NT_SUCCESS(status) ) {
		...
		DebugPrint((1, "[WshLpx]SetTdiInformation: !NT_SUCCESS(status)\n"));
...
{
	DebugPrint((2, "[WshLpx]CompleteTdiActionApc: Entered\n"));
...

{
	...
	DebugPrint((2, "[WshLpx]WSHJoinLeaf: Entered\n"));
...

{
		...
		DebugPrint((2, "[WshLpx]MyInetAddr: Entered\n"));
    ...
    *NeedsCompletion = TRUE;
    ...
    case SIO_GET_INTERFACE_LIST :
        ...
        if( NT_SUCCESS(status) ) {
			...
			DebugPrint((1, "[WshLpx]WSHIoctl: status == STATUS_BUFFER_TOO_SMALL\n"));
...

{
	...
	DebugPrint((2, "[WshLpx]GetTcpipInterfaceList: Entered OutputBuffer = %p, OutputBufferLength= %d\n", 
    ...
    if( INVALID_HANDLE_VALUE == deviceHandle ) {
		...
		DebugPrint((2, "[WshLpx]GetTcpipInterfaceList: CreateFile Error\n"));
        goto exit;
	...
	for( ; ; ) {
		...
		DebugPrint((2, "[WshLpx]GetTcpipInterfaceList: dwReturn = %d, status = 0x%x &socketLpxAddressList = %p\n", dwReturn, status, &socketLpxAddressList));
