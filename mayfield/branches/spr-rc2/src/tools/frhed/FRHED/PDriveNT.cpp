...
BOOL PNtPhysicalDrive::Open( int iDrive )
{
	...
	m_hDevice = CreateFile( szPath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0 );
	if(m_hDevice != INVALID_HANDLE_VALUE)
	{
		DISK_GEOMETRY dg;
...
BOOL PNtPhysicalDrive::WriteAbsolute( LPBYTE lpbMemory, DWORD dwSize, INT64 Sector )
{
	LARGE_INTEGER li;
	li.QuadPart = Sector;
	li.QuadPart *= m_BytesPerSector;
	...
	return WriteFile(m_hDevice, lpbMemory, dwSize, &li.LowPart, 0 );
} // WriteAbsolute()
