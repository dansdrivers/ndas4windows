...
#define LSAssocSetStatusFlag(RI, CCB_POINTER) LSCcbSetStatusFlag( (CCB_POINTER), \
	( (RAID_STATUS_RECOVERING == (RI)->RaidStatus) ? CCBSTATUS_FLAG_RECOVERING : (ULONG)NULL) | \
	( (RAID_STATUS_EMERGENCY == (RI)->RaidStatus) ? CCBSTATUS_FLAG_LURN_STOP : (ULONG)NULL) | \
	( (RAID_STATUS_EMERGENCY_READY == (RI)->RaidStatus) ? CCBSTATUS_FLAG_LURN_STOP : (ULONG)NULL) )
