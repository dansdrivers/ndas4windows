...
LSBUSCTLAPI BOOL WINAPI
LsBusCtlRecoverTarget(DWORD SlotNo)
{
	BOOL fSuccess = FALSE;
	HANDLE hDevice = INVALID_HANDLE_VALUE;
	DWORD cbReturned = 0;
	...
	LANSCSI_RECOVER_TARGET_DATA RecoverTargetData;

	DebugPrint(3, _T("LsBusCtlRecoverTarget: Slot %d\n"), SlotNo);
	...
	ZeroMemory(&RecoverTargetData, sizeof(RecoverTargetData));

	RecoverTargetData.ulSlotNo = SlotNo;

    fSuccess = DeviceIoControl(
			hDevice,
			IOCTL_LANSCSI_RECOVER_TARGET,
            &RecoverTargetData,
	...
	if (!fSuccess) {
		DebugPrintErrEx(_T("LsBusCtlRecoverTarget at slot %d failed: "), SlotNo);
	} else {
		DebugPrint(3, _T("LsBusCtlRecoverTarget at slot %d completed successfully.\n"), SlotNo);
	}
