...
{
	NBUnitDevicePtrList listSelected = m_wndListSingle.GetSelectedDiskObjectList();
	...
	m_pSelectedDisk = listSelected.front();
	// Check if the selected disk's size is not smaller than the source disk's size
	if ( m_pSelectedDisk->GetCapacityInByte() < m_pSourceDisk->GetCapacityInByte() )
	{
		WTL::CString strMsg;
