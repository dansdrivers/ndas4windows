...
///////////////////////////////////////////////////////////////////////////////
const CUnitDiskObjectPtr CDiskObjectComposite::GetAvailableUnitDisk() const
{
...
BOOL CDiskObjectComposite::GetUserCount(int *nRW, int *nRO) const
{
	const_iterator itr;
	for ( itr = begin(); itr != end(); ++itr )
	{
		if(!(*itr)->GetUserCount(nRW, nRO))
			return FALSE;
	}
...
void CDiskObjectComposite::HIXChangeNotify(CNdasHIXChangeNotify *hix)
{
	...
	for ( itr = begin(); itr != end(); ++itr )
	{
		...
		unitDeviceId.UnitNo = 
			unitDisk->GetLocation()->GetUnitDiskLocation()->UnitNumber;
		hix->Notify(unitDeviceId);
	}
}
...
UINT CDiskObjectComposite::GetMissingMemberCount() const
{
	UINT nMissingMemberCount = 0;
	const_iterator itr;
	for ( itr = begin(); itr != end(); ++itr )
	{
		nMissingMemberCount += (*itr)->GetMissingMemberCount();
	}

	return nMissingMemberCount;
}
...
void CDiskObjectComposite::RMDLoad()
{
	const_iterator itr;
	for ( itr = begin(); itr != end(); ++itr )
	{
		...
		if((*itr)->IsUnitDisk())
		{
			...
			::CopyMemory(ci.DeviceID, unitDisk->GetLocation()->GetUnitDiskLocation()->MACAddr, sizeof(ci.DeviceID));
			ci.UnitNo = unitDisk->GetLocation()->GetUnitDiskLocation()->UnitNumber;
			NdasOpRMDRead(&ci, &m_rmd);
			break;
		}
	}
}

int CDiskObjectComposite::RMDStatus()
{
	BOOL bHasFault = FALSE;
	for(int i = 0; i < m_rmd.nDiskCount; i++)
	{
		if(NDAS_UNIT_META_BIND_STATUS_FAULT & m_rmd.UnitMetaData[i].UnitDeviceStatus)
		{
			if(bHasFault)
				return 2;
			bHasFault = TRUE;
...
UINT8 CDiskObjectComposite::RMDUnitStatus(int idx)
{
	for(int i = 0; i < m_rmd.nDiskCount + m_rmd.nSpareCount; i++)
	{
		if(i >= m_rmd.nDiskCount)
			return NDAS_UNIT_META_BIND_STATUS_SPARE;

		if(idx == m_rmd.UnitMetaData[i].iUnitDeviceIdx)
			return m_rmd.UnitMetaData[i].UnitDeviceStatus;
	}

	return 0;
}

UINT8 CDiskObjectComposite::RMDUnitStatus(const UNIT_DISK_LOCATION *location)
{
	const_iterator itr;
	int i;
	for (i =0, itr = begin(); itr != end(); i++, ++itr )
	{
		if(dynamic_cast<CUnitDiskObject*>((*itr).get()) != NULL)
		{
			continue;
		}
		...
		if(!memcmp(unitDisk->GetLocation()->GetUnitDiskLocation(),
			location, sizeof(UNIT_DISK_LOCATION)))
		{
			return RMDUnitStatus(i);
		}
	}
...
BOOL CUnitDiskObject::GetUserCount(int *nRW, int *nRO) const
{
	BOOL bRet;
	...
	ConnectionInfo.UnitNo = GetLocation()->GetUnitDiskLocation()->UnitNumber;
	...
	CopyMemory(ConnectionInfo.AddressLPX, 
		GetLocation()->GetUnitDiskLocation()->MACAddr,
		LPXADDR_NODE_LENGTH);
	...
	*nRW += UnitDeviceStat.NRRWHost;
	*nRO += UnitDeviceStat.NRROHost;
...
{
	WTL::CString strNameWithUnitNo;
	strNameWithUnitNo.Format(_T("%s:%d"), 
		m_strName, 
		GetLocation()->GetUnitDiskLocation()->UnitNumber);
	return strNameWithUnitNo;
}
...
BOOL CEmptyDiskObject::GetUserCount(int *nRW, int *nRO) const
{
	return TRUE;
}
