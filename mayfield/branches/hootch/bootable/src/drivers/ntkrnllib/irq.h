...
static __inline int irq_cannonicalize(int irq)
{
	return ((irq == 2) ? 9 : irq);
}
...
#define NR_RESERVED_IRQS	0x08
#define REAL_IRQ(irq)	((irq & 8) ? ((irq) + 0x60 + NR_RESERVED_IRQS) : ((irq)+NR_RESERVED_IRQS))
