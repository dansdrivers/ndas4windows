	...
	do {
		...
		if (result == 1) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsAttachToFileSystemDevice: Name compare %d %wZ %wZ\n",
											     result, &lfsDeviceExt->FileSystemVolumeName, &ntfsName) );
		...
		if (result == 1) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsAttachToFileSystemDevice: Name compare %d %wZ %wZ\n",
											     result, &lfsDeviceExt->FileSystemVolumeName, &fastFatName) );
		...
		RtlInitUnicodeString( &fastFatName, L"\\NdFat" );
		...
		RtlInitUnicodeString( &ndNtfsName, L"\\NdNtfs" );
		...
		if (result == 1) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsAttachToFileSystemDevice: Name compare %d %wZ %wZ\n",
				result, &lfsDeviceExt->FileSystemVolumeName, &ndNtfsName) );
	...
	)
{
	...
	{ // dump all IRP
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE,
				("LfsPassThrough: Thread=%p, %s %s File=%s, LfsExt=%p\n",
		...
		if (irpSp->MajorFunction == IRP_MJ_DIRECTORY_CONTROL &&
			...
			if (irpSp->Flags & SL_INDEX_SPECIFIED) {
				SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO,("Query SL_INDEX_SPECIFIED\n"));
			}
			if (irpSp->Flags & SL_RESTART_SCAN) {
				SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO,("Query SL_RESTART_SCAN\n"));
			} else {
				SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO,("Query SL_RESTART_SCAN is not set\n"));
			}
			if (irpSp->Flags & SL_RETURN_SINGLE_ENTRY) {
				SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO,("Query SL_RETURN_SINGLE_ENTRY\n"));
			}
		}
#endif		
	}
	...
	)
{
	...
	netdiskPartitionInfo.UnitDiskNo = NetdiskInformation->UnitDiskNo;
	netdiskPartitionInfo.StartingOffset.QuadPart = 0;
	
	return LfsTable_QueryPrimaryAddress( GlobalLfs.LfsTable,
