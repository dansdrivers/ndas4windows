...
//	KIRQL				oldIrql;
	...
	case NDFT_PRIMARY_UPDATE_MESSAGE: {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LIB_TRACE, ( "[LFS] DispatchDGReqPkt: BCASTOWNER: %02x:%02x:%02x:%02x:%02x:%02x/%d.\n",
...
RedirDataGramSvrThreadProc(
	...
	while(1) {
		...
		if(0 == ntStatus) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LIB_TRACE, ( "[LFS] RedirDataGramSvrThreadProc: DatagramRecv event received. NTSTATUS:%lu\n", ntStatus));
		} else if(2 == ntStatus) {
			//
			// Renew server datagram port
			//
			SPY_LOG_PRINT( LFS_DEBUG_LIB_TRACE, ( "[LFS] RedirDataGramSvrThreadProc: Networking event received. NTSTATUS:%lu\n", ntStatus));
...
*/
	...
	while(1) {
		...
		if(NetEvent) {
			SPY_LOG_PRINT( LFS_DEBUG_LIB_TRACE, ("DataGramNotifier: Renew the socket.\n"));
			//
			//	renew the socket.
			//
			LfsCloseDGSocket(
