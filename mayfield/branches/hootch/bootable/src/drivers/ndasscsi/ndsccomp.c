...
PUCHAR tstrstr(PUCHAR str, PUCHAR ptn)
{
	PUCHAR pCurrent, pPattern;

	if(!str || !ptn) return NULL;
	
	while(*str) {
		pCurrent = str; pPattern = ptn;
		while(*pCurrent == *pPattern) {	
			pPattern ++;	if(*pPattern == 0) return str;
			pCurrent ++;	if(*pCurrent == 0) return NULL;			
		}		
		str ++;
	}
...
PUCHAR tstrchr(PUCHAR str, UCHAR c)
{
	if(!str) return NULL;
...
UCHAR *ParseNetBootAddress(PUCHAR BootOption,  PUCHAR LpxAddr)
{
	PCHAR pCurrent = BootOption;
	int i = 0;
	UCHAR s = 0;

	if( (pCurrent = tstrstr(pCurrent, "LPXADDRESS")) != NULL ) {
		if( (pCurrent = tstrchr(pCurrent, '=')) != NULL ) {
			pCurrent ++; // skip '='
			while(i<LPX_NODE_LEN) {
				if(tisdigit(*pCurrent)) {
					s = s << 4;
					s += (*pCurrent) - '0';
				}
				else if(tisxdigit(*pCurrent)) {
					s = s << 4;
					s += (*pCurrent) - 'A' + 10;
				}
				else {
...
UCHAR *ParseHWVersion(PUCHAR BootOption,  PUCHAR HWVersion)
{
	PCHAR pCurrent = BootOption;
	int i = 0;
	UCHAR s = 0;

	if( (pCurrent = tstrstr(pCurrent, "HWVERSION")) != NULL ) {
		if( (pCurrent = tstrchr(pCurrent, '=')) != NULL ) {
			pCurrent ++; // skip '='			
			*HWVersion = *pCurrent - '0';				
		}
	}
...
#ifdef __ENABLE_LOADER__
#define MAX_BLOCKS_PER_REQUEST		0x8	// 32 * 512 = 16K
#else 
#define MAX_BLOCKS_PER_REQUEST		0x80	// 128 * 512 = 64K
#endif 
#define NUMBER_OF_UNITDISKLIST		1
...
GetScsiAdapterInfo(
	...
	ULONG							i;
	UCHAR							NetBootAddr[LPX_NODE_LEN];
	...
	HwDeviceExtension->AdapterMaxBlocksPerRequest	= MAX_BLOCKS_PER_REQUEST;
	HwDeviceExtension->EnumFlags					= 0;
	*AddDevInfoFlags								= 0;
	...
	addTargetDataLength = sizeof(LANSCSI_ADD_TARGET_DATA) +
							(NUMBER_OF_UNITDISKLIST - 1) * sizeof(LSBUS_UNITDISK);
	...
	RtlZeroMemory(addTargetData, addTargetDataLength);
			
	addTargetData->ulSize = addTargetDataLength;
	addTargetData->ucTargetType = NDASSCSI_TYPE_DISK_NORMAL;
	addTargetData->ulTargetBlocks = 0; // m_UnitDisks[iUnitNumber].m_Blocks;
	addTargetData->ulNumberOfUnitDiskList = 1;
	
	addTargetData->CntEcrMethod = NDAS_CONTENTENCRYPT_METHOD_NONE; 
	addTargetData->CntEcrKeyLength = 0;
	RtlZeroMemory(addTargetData->CntEcrKey, NDAS_CONTENTENCRYPT_KEY_LENGTH);

	addTargetData->LurOptions = 0; 


	ParseNetBootAddress(BootParameters, NetBootAddr);
	memcpy(addTargetData->UnitDiskList[0].Address.Node, NetBootAddr, LPX_NODE_LEN); // Test Board

#if 0
//	RtlCopyMemory(addTargetData->UnitDiskList[0].Address.Node, "\x00\x0B\xD0\xFE\x00\x04", 6); // IOMega NetDisk
	RtlCopyMemory(addTargetData->UnitDiskList[0].Address.Node, "\x00\x0B\xD0\x00\x80\x0D", 6); // Test Board
//	RtlCopyMemory(addTargetData->UnitDiskList[0].Address.Node, "\x00\x0B\xD0\x38\x1C\xE9", 6); // Test Board2
//	RtlCopyMemory(addTargetData->UnitDiskList[0].Address.Node, "\x00\x0B\xD0\x01\x72\xFA", 6); // Demo
#endif

	RtlCopyMemory(addTargetData->UnitDiskList[0].NICAddr.Node, GetNICAddr(), 6);
 
	addTargetData->UnitDiskList[0].Address.Port = HTONS(LPX_PORT_NUMBER);
	addTargetData->UnitDiskList[0].ucUnitNumber = 0; // iUnitNumber;
	addTargetData->UnitDiskList[0].ulUnitBlocks = 0; // m_UnitDisks[iUnitNumber].m_Blocks;
	addTargetData->UnitDiskList[0].ucHWType = 0;

	ParseHWVersion(BootParameters, &addTargetData->UnitDiskList[0].ucHWVersion);

#if 0
//	addTargetData->UnitDiskList[0].ucHWVersion = LANSCSIIDE_VERSION_1_0;
#endif

	addTargetData->UnitDiskList[0].iPassword = HASH_KEY_USER;// m_Path.iPassword;

#ifdef __REDUCED_REQUEST_SIZE_TO_NDAS_DEVICE_PATCH__
	addTargetData->UnitDiskList[0].UnitMaxRequestBlocks = 32;
#else

if(addTargetData->UnitDiskList[0].ucHWVersion != LANSCSIIDE_VERSION_2_0)
	addTargetData->UnitDiskList[0].UnitMaxRequestBlocks = MAX_BLOCKS_PER_REQUEST;
else
	addTargetData->UnitDiskList[0].UnitMaxRequestBlocks = 104;
#endif

	// Access Right

	addTargetData->DesiredAccess = GENERIC_READ | GENERIC_WRITE;
	
	for(i = 0; i < addTargetData->ulNumberOfUnitDiskList; i++) {
			addTargetData->UnitDiskList[i].iUserID = 0x00010001;
#ifdef __DIVIDED_MULTI_BOOT__
			addTargetData->UnitDiskList[i].iUserID = 0x00000001;
...
#ifdef __NDASBOOT__
			completionSrb->TimeOutValue = 1000;
