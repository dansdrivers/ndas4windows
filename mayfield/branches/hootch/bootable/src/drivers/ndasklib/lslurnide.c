	...
	case LurPrimaryLurnInformation:
		{
			...
			if(IdeDev) {
				LurnInfo->UnitDiskId = IdeDev->LuData.LanscsiTargetID;
...
ULONG BindFailCount = 0;
ULONG BindWaitCount = 0;

#define RECONNECT_TRY_COUNT		20
...
//		ASSERT(FALSE);
	...
	TimeInterval.QuadPart = - HZ * 5;
	KDPrintM(DBG_LURN_ERROR,("Reconnect2: wait for %d seconds\n", (int)(-TimeInterval.QuadPart/HZ)));
	...
	KDPrintM(DBG_LURN_ERROR,("Reconnect2: ProtoStatus = %08X, NICStatus = %08X, StallCount = %08X, BindFailCount = %08X\n", ProtoStatus, GetNICStatus(), StallCount, BindFailCount));
#endif
...
#ifdef __ENABLE_LOADER__
	NetProtoReInitialize();

	status = LspReconnectAndLogin(&IdeExt->LanScsiSession, IdeExt->LstransType, LssEncBuff);
#else

	if(STATUS_LPX_MINIPORT == ProtoStatus || STATUS_LPX_DRIVER_BEFORE_BIND == ProtoStatus) {
		if(STATUS_NIC_CORRUPTED == GetNICStatus()) {
			KDPrintM(DBG_LURN_ERROR,("LOSTNIC: Lost control over the NIC. Try LPX driver instead embedded LPX\n"));

			ProtoStatus = STATUS_LPX_DRIVER_BEFORE_BIND;

			status = LspReconnectAndLogin(&IdeExt->LanScsiSession, IdeExt->LstransType, LssEncBuff);

			KDPrintM(DBG_LURN_ERROR,("LOSTNIC: LspReconnectAndLogin , Status = %08X!!!\n", status ));

			if(NT_SUCCESS(status)) {
				ProtoStatus = STATUS_LPX_DRIVER_BIND;
				BindFailCount = 0;
				KDPrintM(DBG_LURN_ERROR,("LOSTNIC: LspReconnectAndLogin Success with LPX driver!!!\n" ));
			}
			if(STATUS_OBJECT_NAME_NOT_FOUND == status ||
				STATUS_INVALID_ADDRESS_COMPONENT == status ||
				STATUS_NETWORK_UNREACHABLE == status
				) {
				InterlockedDecrement(&Lurn->NrOfStall);
				InterlockedIncrement(&BindWaitCount);
				if(BindWaitCount > 10) {
					ProtoStatus = STATUS_LPX_MINIPORT;

					KDPrintM(DBG_LURN_ERROR,("LOSTNIC: BindWaitCount reached maximum. Try embedded LPX.\n"));

					NetProtoReInitialize();
					status = LspReconnectAndLogin(&IdeExt->LanScsiSession, IdeExt->LstransType, LssEncBuff);
					InterlockedExchange(&BindWaitCount, 0);

				}
				KDPrintM(DBG_LURN_ERROR,("LOSTNIC: Status = %08X, BindWaitCount = %08X!!!\n", status, BindWaitCount));
			}

			if(BindFailCount > 5) {
				KDPrintM(DBG_LURN_ERROR,("LOSTNIC: BindFailCount reached maximum. Try embedded LPX.\n"));
			...
			if(StallCount > 10) {
				KDPrintM(DBG_LURN_ERROR,("LOSTNIC: StallCount reached maximum. Try embedded LPX.\n"));

				ProtoStatus = STATUS_LPX_MINIPORT;
				InterlockedIncrement(&BindFailCount);
				NetProtoReInitialize();
				status = LspReconnectAndLogin(&IdeExt->LanScsiSession, IdeExt->LstransType, LssEncBuff);
			}
		}
		else {				
				KDPrintM(DBG_LURN_ERROR,("EMBEDDED_LPX: Lost connection. reconnecting with embedded LPX and NIC driver.\n"));
				NetProtoReInitialize();
				status = LspReconnectAndLogin(&IdeExt->LanScsiSession, IdeExt->LstransType, LssEncBuff);
				KDPrintM(DBG_LURN_ERROR,("EMBEDDED_LPX: LspReconnectAndLogin: Status = %08X\n", status));
		}
	}
	else if(STATUS_LPX_DRIVER_BIND == ProtoStatus) {
		KDPrintM(DBG_LURN_ERROR,("LPXDRIVER: Lost connection. reconnecting with LPX driver.\n"));

		if(StallCount>5) {
			KDPrintM(DBG_LURN_ERROR,("LPXDRIVER: Maybe LPX is unbinded. Back to embeded LPX.\n"));
			ProtoStatus = STATUS_LPX_MINIPORT;
			NetProtoReInitialize();
		}
		...
		KDPrintM(DBG_LURN_ERROR,("LPXDRIVER: LspReconnectAndLogin: Status = %08X\n", status));
	}
	...
	if(NT_SUCCESS(status)) {
		...
		KDPrintM(DBG_LURN_ERROR,("failed. Status = %08X\n", status));
...
//		ASSERT(FALSE);
	...
	// Recover the original time out.
	genericTimeout.QuadPart = LURNIDE_GENERIC_TIMEOUT;
	LspSetTimeOut(&IdeExt->LanScsiSession, &genericTimeout);
