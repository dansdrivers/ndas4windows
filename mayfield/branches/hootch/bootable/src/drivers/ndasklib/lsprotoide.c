	   ...
	   )
{
	...
	*received = 0;
	...
	while (len > 0) {
		...
		if(CONNECTION_TYPE_SOCKET == ConnectionFile->ConnectionType) {
			ntStatus = LpxReceive(
								ConnectionFile->ConnectionSock, 
								buf, 
								len,
								0, 
								NULL,
...
SendIt(
	...
	if(OverlappedData) {
		...
		else if (CONNECTION_TYPE_FILE_OBJECT == ConnectionFile->ConnectionType){
...
LspIdeVendorRequest(
	...
	pRequestHeader->VendorParameter = HTONLL(PduDesc->Param64);
