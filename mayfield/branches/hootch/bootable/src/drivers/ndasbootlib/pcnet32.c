...
/**
  ...
  */
static __inline int is_valid_ether_addr( u8 *addr )
{
...
static int  pcnet32_probe_pci(struct pci_dev *, unsigned long, struct net_device *);
static int  pcnet32_probe1(unsigned long, unsigned int, int, struct net_device *dev, struct pci_dev *pdev);
...
static int	pcnet32_get_status(struct device *dev);
...
static u16 pcnet32_dwio_read_csr (unsigned long addr, int index)
{
    ...
    return (u16)(inl (addr+PCNET32_DWIO_RDP) & 0xffff);
}
...
{
    outl (index, addr+PCNET32_DWIO_RAP);
    return (u16)(inl (addr+PCNET32_DWIO_BDP) & 0xffff);
}
...
{
    return (u16)(inl (addr+PCNET32_DWIO_RAP) & 0xffff);
}
...
pcnet32_probe_pci(struct pci_dev *pdev, unsigned long ioaddr, struct net_device *dev)
{
//    unsigned long ioaddr;
	...
	//
	// Do not enable device in the boot loader environment.
	//

	err = pci_enable_device(pdev);
    if (err < 0) {
	NbDebugPrint(0, (KERN_ERR PFX "failed to enable device -- err=%d\n", err));
	return err;
    }
#endif
	pci_set_master(pdev);
    ...
    if (!ioaddr) {
        NbDebugPrint(0, (KERN_ERR PFX "card has no PCI IO resources, aborting\n"));
        return -ENODEV;
    }
    if (!pci_dma_supported(pdev, PCNET32_DMA_MASK)) {
	NbDebugPrint(0, (KERN_ERR PFX "architecture does not support 32bit PCI busmaster DMA\n"));
	return -ENODEV;
    }
#endif
	return pcnet32_probe1(ioaddr, pdev->irq, 1, dev, pdev);
}
...
pcnet32_probe1(unsigned long ioaddr, unsigned int irq_line, int shared,
		struct net_device *dev, struct pci_dev *pdev)
{
...
//    struct net_device *dev;
    ...
    if (pcnet32_debug > 2)
	NbDebugPrint(0, (KERN_INFO "  PCnet chip version is %#x.\n", chip_version));
...
#if 0
	if (pcnet32_debug > 2)
	    NbDebugPrint(0, (KERN_DEBUG PFX "media value %#x.\n",  media));
	media &= ~3;
	media |= 1;
#endif
	if (pcnet32_debug > 2)
	    NbDebugPrint(0, (KERN_DEBUG PFX "media reset to %#x.\n",  media));
	a->write_bcr(ioaddr, 49, (u16)media);
	break;
    case 0x2627:
	...
	break;
    default:
	NbDebugPrint(0, (KERN_INFO PFX "PCnet version %#x, no PCnet32 chip.\n",
			chip_version));
	return -ENODEV;
    }
...
//////////////////////////////////////////////////////////////////////////
	...
	return -ENOMEM;
#endif
    NbDebugPrint(0, (KERN_INFO PFX "%s at %#3lx,", chipname, ioaddr));
...
#endif
	    NbDebugPrint(0, (" warning: CSR address invalid,\n"));
	    NbDebugPrint(0, (KERN_INFO "    using instead PROM address of"));
    ...
    for (i = 0; i < 6; i++)
	NbDebugPrint(0, (" %2.2x", dev->dev_addr[i] ));

    if (((chip_version + 1) & 0xfffe) == 0x2624) { /* Version 0x2623 or 0x2624 */
		i = a->read_csr(ioaddr, 80) & 0x0C00;  /* Check tx_start_pt */
		NbDebugPrint(0, ("\n" KERN_INFO "    tx_start_pt(0x%04x):",i));
		switch(i>>10) {
			case 0: NbDebugPrint(0, ("  20 bytes,")); break;
			case 1: NbDebugPrint(0, ("  64 bytes,")); break;
			case 2: NbDebugPrint(0, (" 128 bytes,")); break;
			case 3: NbDebugPrint(0, ("~220 bytes,")); break;
		}
		i = a->read_bcr(ioaddr, 18);  /* Check Burst/Bus control */
		NbDebugPrint(0, (" BCR18(%x):",i&0xffff));
		if (i & (1<<5)) NbDebugPrint(0, ("BurstWrEn "));
		if (i & (1<<6)) NbDebugPrint(0, ("BurstRdEn "));
		if (i & (1<<7)) NbDebugPrint(0, ("DWordIO "));
		if (i & (1<<11)) NbDebugPrint(0, ("NoUFlow "));
		i = a->read_bcr(ioaddr, 25);
		NbDebugPrint(0, ("\n" KERN_INFO "    SRAMSIZE=0x%04x,",i<<8));
		i = a->read_bcr(ioaddr, 26);
		NbDebugPrint(0, (" SRAM_BND=0x%04x,",i<<8));
		i = a->read_bcr(ioaddr, 27);
		if (i & (1<<14)) NbDebugPrint(0, ("LowLatRx"));
	}
...
//    spin_lock_init(&lp->lock);
    ...
    if (!a) {
      NbDebugPrint(0, (KERN_ERR PFX "No access methods\n"));
    ...
    a->write_csr (ioaddr, 1, (u16)((lp->dma_addr + offsetof(struct pcnet32_private, init_block)) & 0xffff));
    a->write_csr (ioaddr, 2, (u16)((lp->dma_addr + offsetof(struct pcnet32_private, init_block)) >> 16));
...
//////////////////////////////////////////////////////////////////////////
	...
	if (dev->irq >= 2) {
		...
		if (dev->irq) {
			NbDebugPrint(0, (", probed IRQ %d.\n", dev->irq));
		} else {
			NbDebugPrint(0, (", failed to detect IRQ line.\n"));
			pci_free_consistent(lp->pci_dev, sizeof(*lp), lp, lp->dma_addr);
#if 0
//////////////////////////////////////////////////////////////////////////
//
...
//////////////////////////////////////////////////////////////////////////
	...
	dev->get_status = &pcnet32_get_status;
...
//////////////////////////////////////////////////////////////////////////
	register_netdev(dev);
#endif
    NbDebugPrint(0, (KERN_INFO "%s: registered as %s\n",dev->name, lp->name));
    cards_found++;
    return 0;
}
	...
	struct device *dev)
{
	...
	NbDebugPrint(0, ("pcnet32_scan_pcibus Entered\n"));
...
//	DbgPrint("rtl8139_probe Entered\n");
	...
	for (; pci_index < 0xff; pci_index++) {
		...
		NbDebugPrint(0, ("pci index=%x\n", pci_index));

		if (pcibios_find_class (PCI_CLASS_NETWORK_ETHERNET << 8, pci_index,
								&pci_bus, &pci_device_fn))
			continue;

		NbDebugPrint(0, ("Device function=%x\n", pci_device_fn));

		pcibios_read_config_word(pci_bus, pci_device_fn,
								 PCI_VENDOR_ID, &vendor);
		pcibios_read_config_word(pci_bus, pci_device_fn,
								 PCI_DEVICE_ID, &device);
		for (chip_idx = 0; pcnet32_pci_tbl[chip_idx].vendor; chip_idx++) {
			if (vendor == pcnet32_pci_tbl[chip_idx].vendor
				&& (device & 0xffff) ==
				pcnet32_pci_tbl[chip_idx].device)
				break;
		}
		if (pcnet32_pci_tbl[chip_idx].vendor == 0) 		/* Compiled out! */
			continue;
...
activate:
		...
		if (ioaddr == 0) {
			NbDebugPrint(0, (" Pcnet32 network adapter has been found, "
...
//		dev = pcnet32_probe1(pci_bus, pci_device_fn, dev, ioaddr,
//									   irq, chip_idx, nr_cards_found);
//		iret = pcnet32_probe1(ioaddr, irq, 1 /* Don't know */, dev, pdev);
		iret = pcnet32_probe_pci(pdev, ioaddr, dev);

		if (iret >= 0  /*&& (pcnet32_pci_tbl[chip_idx].flags & PCI_COMMAND_MASTER)*/) {
			UCHAR pci_latency;
			pcibios_read_config_byte(pci_bus, pci_device_fn,
...
//////////////////////////////////////////////////////////////////////////
	...
	NbDebugPrint(0, ("nr_cards_found = %d\n", nr_cards_found)) ;
	return nr_cards_found ? 0 : -ENODEV;
}
...
//////////////////////////////////////////////////////////////////////////
    ...
    if (pcnet32_debug > 1)
	NbDebugPrint(0, (KERN_DEBUG "%s: pcnet32_open() irq %d tx/rx rings %#x/%#x init %#x.\n",
	       dev->name, dev->irq,
    ...
    lp->a.write_csr (ioaddr, 1, (u16)((lp->dma_addr + offsetof(struct pcnet32_private, init_block)) &0xffff));
    lp->a.write_csr (ioaddr, 2, (u16)((lp->dma_addr + offsetof(struct pcnet32_private, init_block)) >> 16));
	...
	NbDebugPrint(0, (KERN_DEBUG "pcnet32 #1\n"));
#ifdef __NDASBOOT__
#if 1
	link_initialized = 1;
#endif
    ...
    if (pcnet32_debug > 2)
	NbDebugPrint(0, (KERN_DEBUG "%s: pcnet32 open after %d ticks, init block %#x csr0 %4.4x.\n",
	...
	KeInitializeDpc( &lp->PollingTimerDpc, pcnet32_interrupt, dev );
	KeInitializeTimer( &lp->PollingTimer );

	deltaTime.QuadPart = - INTERRUPT_TIME;    /* 2.4 sec. */
	KeSetTimer( &lp->PollingTimer, deltaTime, &lp->PollingTimerDpc);
...
//////////////////////////////////////////////////////////////////////////
//
// not supported API. Use devkfree_skb
//
//////////////////////////////////////////////////////////////////////////
...
pcnet32_init_ring(struct net_device *dev)
{
	...
	NbDebugPrint(0, (KERN_DEBUG "Init ring\n"));
	...
	if (rx_skbuff == NULL) {
		...
		NbDebugPrint(0, (KERN_ERR "%s: pcnet32_init_ring dev_alloc_skb failed.\n",dev->name));
		return -1;
	    }
...
pcnet32_restart(struct net_device *dev, unsigned int csr0_bits)
{
	...
	NbDebugPrint(0, (KERN_DEBUG "restart\n"));
    ...
    lp->a.write_csr (ioaddr, 0, (u16)csr0_bits);
}
...
#else
	KeAcquireSpinLock(&lp->lock, &oldIrql);
#endif
    /* Transmitter timeout, serious problems. */
	NbDebugPrint(0, (KERN_ERR "%s: transmit timed out, status %4.4x, resetting.\n",
	       dev->name, lp->a.read_csr(ioaddr, 0)));
	lp->a.write_csr (ioaddr, 0, 0x0004);
	lp->stats.tx_errors++;
	if (pcnet32_debug > 2) {
	    int i;
	    NbDebugPrint(0, (KERN_DEBUG " Ring data dump: dirty_tx %d cur_tx %d%s cur_rx %d.",
	       lp->dirty_tx, lp->cur_tx, lp->tx_full ? " (full)" : "",
	       lp->cur_rx));
	    for (i = 0 ; i < RX_RING_SIZE; i++)
	    NbDebugPrint(0, ("%s %08x %04x %08x %04x", i & 1 ? "" : "\n ",
...
#else
	KeReleaseSpinLock(&lp->lock, oldIrql);
#endif
}
...
pcnet32_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (pcnet32_debug > 3) {
	NbDebugPrint(0, (KERN_DEBUG "%s: pcnet32_start_xmit() called, csr0 %4.4x.\n",
	       dev->name, lp->a.read_csr(ioaddr, 0)));
    }
...
//		NbDebugPrint(0, (KERN_ERR "tx ring full!!!\n"));
		KeReleaseSpinLock(&lp->lock, oldIrql);
    ...
    lp->tx_ring[entry].length = le16_to_cpu(-(signed int)skb->len);
...
//		NbDebugPrint(0, (KERN_ERR "tx ring going to be full!!!\n"));
    }
#if 0
    spin_unlock_irqrestore(&lp->lock, flags);
#else
...
//	struct net_device *dev = dev_id;
	struct net_device *dev = (struct device *)DeferredContext;
    struct pcnet32_private *lp;
    unsigned long ioaddr;
    u16 csr0,rap;
    ...
    if (!dev) {
	NbDebugPrint(0, (KERN_DEBUG "%s(): irq %d for unknown device\n",
...
#ifndef __INTERRUPT__
	KeCancelTimer(&lp->PollingTimer);
#endif


	if( STATUS_NIC_OK != pcnet32_get_status(dev) ) {
#ifndef __INTERRUPT__
#ifndef __ENABLE_LOADER__
    ...
    spin_lock(&lp->lock);
#else
	KeAcquireSpinLockAtDpcLevel(&lp->lock);
#endif
	...
	if (pcnet32_debug > 5)
	    NbDebugPrint(0, (KERN_DEBUG "%s: interrupt  csr0=%#2.2x new csr=%#2.2x.\n",
	...
	if (csr0 & 0x0200) {		/* Tx-done interrupt */
		...
		while (dirty_tx < lp->cur_tx) {
			...
			if (status & 0x4000) {
				...
				NbDebugPrint(0, (KERN_ERR "%s: Tx FIFO error! CSR0=%4.4x\n",
					   dev->name, csr0));
				must_restart = 1;
				}
#else
	    ...
	    if (lp->cur_tx - dirty_tx >= TX_RING_SIZE) {
			NbDebugPrint(0, (KERN_ERR "%s: out-of-sync dirty pointer, %d vs. %d, full=%d.\n",
	...
	if (csr0 & 0x0800) {
	    NbDebugPrint(0, (KERN_ERR "%s: Bus master arbitration failure, status %4.4x.\n",
		   dev->name, csr0));
    ...
    if (pcnet32_debug > 4)
	NbDebugPrint(0, (KERN_DEBUG "%s: exiting interrupt, csr0=%#4.4x.\n",
	       dev->name, lp->a.read_csr (ioaddr, 0)));
    ...
    spin_unlock(&lp->lock);
#else
	KeReleaseSpinLockFromDpcLevel(&lp->lock);
#endif
}
...
pcnet32_rx(struct net_device *dev)
{
	...
	if (status != 0x03) {			/* There was an error. */
	    ...
	    if(pkt_len < 60) {
		NbDebugPrint(0, (KERN_ERR "%s: Runt packet!\n",dev->name));
		lp->stats.rx_errors++;
	    } else {
		int rx_in_place = 0;
		...
		if (skb == NULL) {
                    int i;
		    NbDebugPrint(0, (KERN_ERR "%s: Memory squeeze, deferring packet.\n", dev->name));
		    for (i = 0; i < RX_RING_SIZE; i++)
...
pcnet32_close(struct net_device *dev)
{
    ...
    if (pcnet32_debug > 1)
	NbDebugPrint(0, (KERN_DEBUG "%s: Shutting down ethercard, status was %2.2x.\n",
	       dev->name, lp->a.read_csr (ioaddr, 0)));
...
static int	pcnet32_get_status(
	struct device *dev
){
	struct pcnet32_private *lp = (struct pcnet32_private *)dev->priv;
	long ioaddr = dev->base_addr;
	u32 rx_ring_addr = lp->dma_addr + offsetof(struct pcnet32_private, rx_ring);
	u32 rx_ring_addr_in_NIC;
	u16 saved_addr;
#if 0
	unsigned long flags;
#else
	...
	rx_ring_addr_in_NIC = le32_to_cpu(lp->a.read_csr(ioaddr, 25)); // upper
	rx_ring_addr_in_NIC <<= 16;
	rx_ring_addr_in_NIC += le32_to_cpu(lp->a.read_csr(ioaddr, 24)) & 0xffff; // lower
	...
	if( rx_ring_addr_in_NIC != rx_ring_addr) {
		NbDebugPrint (0, ("pcnet32 status is corrupted !!!!!!! %x %d\n", rx_ring_addr, rx_ring_addr_in_NIC));
...
static void pcnet32_set_multicast_list(struct net_device *dev)
{
	...
	/* Log any net taps. */
	NbDebugPrint(0, (KERN_INFO "%s: Promiscuous mode enabled.\n", dev->name));
...
static int mdio_read(struct net_device *dev, int phy_id, int reg_num)
{
	...
	lp->a.write_bcr(ioaddr, 33, (USHORT)phyaddr);
...
static void mdio_write(struct net_device *dev, int phy_id, int reg_num, int val)
{
	...
	lp->a.write_bcr(ioaddr, 33, ((phy_id & 0x1f) << 5) | (reg_num & 0x1f));
	lp->a.write_bcr(ioaddr, 34, (USHORT)val);
	lp->a.write_bcr(ioaddr, 33, (USHORT)phyaddr);
}
...
{
    NbDebugPrint(0, (KERN_INFO "%s", version));
    ...
    if (cards_found)
	NbDebugPrint(0, (KERN_INFO PFX "%d cards_found.\n", cards_found));
