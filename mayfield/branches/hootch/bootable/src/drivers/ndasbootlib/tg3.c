...
#define readl(x)	*((ULONG *)(x))
#define writel(v,x) *((ULONG *)(x)) = (v)
#define readw(x)	*((USHORT *)(x))
#define writew(v,x)	*((USHORT *)(x)) = (v)
...
#define rmb()
#define smp_mb()
#define BUG() NbDebugPrint(0, ("BUG !!!!!!! BUG !!!!!! BUG !!!!!"))
#define BUG_ON(x)

#define cpu_to_be32(val) (val)
#define swab32(val) (val)

#define prefetch(x)
#define __netif_rx_schedule netif_rx_schedule
...
#define DRV_MODULE_VERSION	"3.43b"
...
KDPC InterruptTimerDpc_tg3;
KTIMER InterruptTimer_tg3;

static __inline int netif_rx_schedule(struct device *dev)
{
	struct tg3 *tp = (struct tg3 *) dev->priv;
	int budget = TG3_RX_RCB_RING_SIZE(tp);

	return dev->poll(dev, &budget);
}

static __inline int netif_rx_schedule_prep(struct device *dev)
{
	return TRUE;
}

extern ULONG OSTimeTick;
__inline void udelay(long delay) 
{
	
//	LARGE_INTEGER	interval;	

//	NbDebugPrint(0, ("udelay:OSTimeTick = %08X, delay = %08X\n", OSTimeTick, delay));
...
static void tg3_write_indirect_reg32(struct tg3 *tp, u32 off, u32 val)
{
	...
	KeAcquireSpinLock(&tp->indirect_lock, &Irql);
	pci_write_config_dword(tp->pdev, TG3PCI_REG_BASE_ADDR, off);
	pci_write_config_dword(tp->pdev, TG3PCI_REG_DATA, val);
	KeReleaseSpinLock(&tp->indirect_lock, Irql);
}
...
static __inline void tw32_mailbox_flush(struct tg3 *tp, u32 off, u32 val)
{
...
static __inline void tg3_cond_int(struct tg3 *tp)
{
...
static __inline unsigned int tg3_has_work(struct tg3 *tp)
{
...
{
//	NbDebugPrint(0, ("tg3_restart_ints: tp->tg3_flags = %08X, tg3_has_work(tp) = %d\n", tp->tg3_flags, tg3_has_work(tp)));
	...
	if (!(tp->tg3_flags & TG3_FLAG_TAGGED_STATUS) &&
		tg3_has_work(tp)) {
		NbDebugPrint(0, ("tg3_restart_ints \n"));
		tw32(HOSTCC_MODE, tp->coalesce_mode |
		     (HOSTCC_MODE_ENABLE | HOSTCC_MODE_NOW));
	}
}

static __inline void tg3_netif_stop(struct tg3 *tp)
{
...
static __inline void tg3_netif_start(struct tg3 *tp)
{
...
static void tg3_switch_clocks(struct tg3 *tp)
{
	...
	NbDebugPrint(0, ("tg3_switch_clocks: TG3PCI_CLOCK_CTRL = %08X\n", clock_ctrl));
...
static int tg3_set_power_state(struct tg3 *tp, int state)
{
	...
	NbDebugPrint(0, ("tg3_set_power_state:pm = %x\n", pm));
	...
	default:
		NbDebugPrint(0, ("%s: Invalid power state (%d) "
		       "requested.\n",
		       tp->dev->name, state));
		return -EINVAL;
	};
...
static void tg3_link_report(struct tg3 *tp)
{
	if (!netif_carrier_ok(tp->dev)) {
		NbDebugPrint(0, ("%s: Link is down.\n", tp->dev->name));
	} else {
		NbDebugPrint(0, ("%s: Link is up at %d Mbps, %s duplex.\n",
		       tp->dev->name,
		...
		NbDebugPrint(0, ("%s: Flow control is %s for TX and "
		       "%s for RX.\n",
		       tp->dev->name,
...
//				       PCI_DMA_TODEVICE);
	...
	if (unlikely(netif_queue_stopped(tp->dev))) {
		KeAcquireSpinLockAtDpcLevel(&tp->tx_lock);
		if (netif_queue_stopped(tp->dev) &&
		    (TX_BUFFS_AVAIL(tp) > TG3_TX_WAKEUP_THRESH))
			netif_wake_queue(tp->dev);
		KeReleaseSpinLockFromDpcLevel(&tp->tx_lock);
	}
}
...
//	pci_unmap_addr_set(map, mapping, mapping);
	...
	desc->addr_hi = (u32)((u64)mapping >> 32);
	desc->addr_lo = (u32)((u64)mapping & 0xffffffff);
...
//			pci_dma_sync_single_for_device(tp->pdev, dma_addr, len, PCI_DMA_FROMDEVICE);
	...
	//	NbDebugPrint(0, ("skb->protocol = %08x\n", skb->protocol));
...
/*
		{
			unsigned char *temp = skb->data;
			int i;

			for(i=0;i<64;i++) {
				NbDebugPrint(0, ("%02X ", *temp++));
			}
		}
*/
...
static int tg3_poll(struct device *netdev, int *budget)
{
	struct tg3 *tp = netdev_priv(netdev);
	struct tg3_hw_status *sblk = tp->hw_status;
	int done;

//	NbDebugPrint(0, ("tg3_poll entered, tp->tg3_flags = %08X\n", tp->tg3_flags));
	...
	if (!(tp->tg3_flags &
		...
		if (sblk->status & SD_STATUS_LINK_CHG) {
			...
			KeAcquireSpinLockAtDpcLevel(&tp->lock);
			tg3_setup_phy(tp, 0);
			KeReleaseSpinLockFromDpcLevel(&tp->lock);

//			NbDebugPrint(0, ("tg3_poll:, sblk->status = %08X\n", sblk->status));
		}
	}
	...
	if (sblk->idx[0].tx_consumer != tp->tx_cons) {
//		NbDebugPrint(0, ("tg3_poll: sblk->idx[0].tx_consumer = %08X, tp->tx_cons = %08X\n", sblk->idx[0].tx_consumer, tp->tx_cons));
		tg3_tx(tp);
	}
...
//		NbDebugPrint(0, ("tg3_poll: sblk->idx[0].rx_producer = %08X, tp->rx_rcb_ptr = %08X\n", sblk->idx[0].rx_producer, tp->rx_rcb_ptr));
...
//	NbDebugPrint(0, ("tg3_poll: done = %d\n", done));
	if (done) {
...
static __inline int tg3_irq_sync(struct tg3 *tp)
{
	return tp->irq_sync;
}
 ...
 */
static __inline void tg3_full_lock(struct tg3 *tp, int irq_sync)
{
	if (irq_sync)
		tg3_irq_quiesce(tp);
	KeAcquireSpinLockAtDpcLevel(&tp->lock);
...
static __inline void tg3_full_unlock(struct tg3 *tp)
{
	KeReleaseSpinLockFromDpcLevel(&tp->tx_lock);
	KeReleaseSpinLockFromDpcLevel(&tp->lock);
}
...
static void tg3_interrupt_tagged(
	IN struct _KDPC *Dpc,
    IN PVOID DeferredContext,
    IN PVOID SystemArgument1,
    IN PVOID SystemArgument2
)
{
	...
	KeCancelTimer(&InterruptTimer_tg3);
#endif

//	NbDebugPrint(0, ("tg3_interrupt_tagged entered\n"));
//	show_net_buffers();
...
out:
    ...
    KeSetTimer( &InterruptTimer_tg3, deltaTime, &InterruptTimerDpc_tg3 );
#endif
...
static void tg3_poll_controller(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
...
//		KeCancelTimer(&tp->timer);
//		deltaTime.QuadPart = - 1;    /* 2.4 sec. */
//		KeSetTimer( &tp->timer, deltaTime, &tp->timerDpc );		
	}
}

static void tg3_tx_timeout(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);

	NbDebugPrint(0, ("%s: transmit timed out, resetting\n",
	       dev->name));
...
static __inline int tg3_4g_overflow_test(dma_addr_t mapping, int len)
{
...
{
	struct sk_buff *new_skb = skb_copy(skb, 0);
			...
			u32 mss_and_is_end)
{
	...
	txd->addr_hi = (u32)((u64) mapping >> 32);
	txd->addr_lo = (u32)((u64) mapping & 0xffffffff);
	txd->len_flags = (len << TXD_LEN_SHIFT) | flags;
	txd->vlan_tag = vlan_tag << TXD_VLAN_TAG_SHIFT;
}


static int tg_get_status(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
	...
	/*
	ring_ptr = inl(ioaddr + RxRingPtr);
	if(  (ring_ptr < np->ring_dma) 
		|| (ring_ptr > np->ring_dma + RX_RING_SIZE * sizeof(struct netdev_desc)) 
		|| (ring_ptr % sizeof(struct netdev_desc) != np->ring_dma % sizeof(struct netdev_desc)) 
		) {
	//	NbDebugPrint (0, ("natsemi status is corrupted, inl(ioaddr + RxRingPtr) = %08X, np->ring_dma = %08X\n",
...
static int tg3_start_xmit(struct sk_buff *skb, struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
	dma_addr_t mapping;
...
//		return NETDEV_TX_LOCKED; 
	...
	KeAcquireSpinLockAtDpcLevel(&tp->tx_lock);
//	NbDebugPrint(0, ("tg3_start_xmit entered\n"));

	/* This is a hard error, log it. */
	if (unlikely(TX_BUFFS_AVAIL(tp) <= (skb_shinfo(skb)->nr_frags + 1))) {
		netif_stop_queue(dev);
		KeReleaseSpinLockFromDpcLevel(&tp->tx_lock);
		NbDebugPrint(0, ("%s: BUG! Tx Ring full when queue awake!\n",
		       dev->name));
...
//			pci_unmap_addr_set(&tp->tx_buffers[entry], mapping, mapping);
	...
	if (would_hit_hwbug) {
		...
		NbDebugPrint(0, ("tg3_start_xmit: would_hit_hwbug = %d\n", would_hit_hwbug));
		...
		if (tigon3_4gb_hwbug_workaround(tp, skb, last_plus_one,
			...
			NbDebugPrint(0, ("tg3_start_xmit: tigon3_4gb_hwbug_workaround is true\n"));
			goto out_unlock;
		}
		entry = start;
	}
...
//	NbDebugPrint(0, ("tg3_start_xmit exit\n"));
	KeReleaseSpinLockFromDpcLevel(&tp->tx_lock);
...
static __inline void tg3_set_mtu(struct device *dev, struct tg3 *tp,
			       int new_mtu)
{
	dev->mtu = new_mtu;
...
static int tg3_change_mtu(struct device *dev, int new_mtu)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_stop_block(struct tg3 *tp, unsigned long ofs, u32 enable_bit, int silent)
{
	...
	if (i == MAX_WAIT_CNT && !silent) {
		NbDebugPrint(0,("tg3_stop_block timed out, "
		       "ofs=%lx enable_bit=%x\n",
		       ofs, enable_bit));
		return -ENODEV;
	}
...
static int tg3_abort_hw(struct tg3 *tp, int silent)
{
	...
	if (i >= MAX_WAIT_CNT) {
		NbDebugPrint(0,("tg3_abort_hw timed out for %s, "
...
static int tg3_chip_reset(struct tg3 *tp)
{
	...
	if (!(tp->tg3_flags2 & TG3_FLG2_SUN_570X)) {
		...
		if (i >= 100000) {
			NbDebugPrint(0, ("tg3_reset_hw timed out for %s, "
...
static int tg3_halt_cpu(struct tg3 *tp, u32 offset)
{
	...
	if (i >= 10000) {
		NbDebugPrint(0, ("tg3_reset_cpu timed out for %s, "
		       "and %s CPU\n",
		       tp->dev->name,
...
static int tg3_load_firmware_cpu(struct tg3 *tp, u32 cpu_base, u32 cpu_scratch_base,
				 unsigned int cpu_scratch_size, struct fw_info *info)
{
	unsigned int i;
	int err;
	void (*write_op)(struct tg3 *, u32, u32);
	...
	if (cpu_base == TX_CPU_BASE &&
	    (tp->tg3_flags2 & TG3_FLG2_5705_PLUS)) {
		NbDebugPrint(0, ("tg3_load_firmware_cpu: Trying to load "
...
static int tg3_load_5701_a0_firmware_fix(struct tg3 *tp)
{
	...
	if (i >= 5) {
		NbDebugPrint(0, ("tg3_load_firmware fails for %s "
...
static int tg3_load_tso_firmware(struct tg3 *tp)
{
	...
	if (i >= 5) {
		NbDebugPrint(0, ("tg3_load_tso_firmware fails for %s "
...
static int tg3_set_mac_addr(struct device *dev, void *p)
{
	struct tg3 *tp = netdev_priv(dev);
...
static void __tg3_set_rx_mode(struct device *);
...
static int tg3_reset_hw(struct tg3 *tp)
{
	...
	if (i >= 2000) {
		NbDebugPrint(0, ("tg3_reset_hw cannot enable BUFMGR for %s.\n",
		       tp->dev->name));
		return -ENODEV;
	}
	...
	tw32(RCVDBDI_STD_BD + TG3_BDINFO_HOST_ADDR + TG3_64BIT_REG_HIGH,
	     (u32)((u64) tp->rx_std_mapping >> 32));
	tw32(RCVDBDI_STD_BD + TG3_BDINFO_HOST_ADDR + TG3_64BIT_REG_LOW,
	     (u32)((u64) tp->rx_std_mapping & 0xffffffff));
	...
	if (tp->tg3_flags2 & TG3_FLG2_5705_PLUS) {
		...
		if (tp->tg3_flags & TG3_FLAG_JUMBO_RING_ENABLE) {
			tw32(RCVDBDI_JUMBO_BD + TG3_BDINFO_HOST_ADDR + TG3_64BIT_REG_HIGH,
			     (u32)((u64) tp->rx_jumbo_mapping >> 32));
			tw32(RCVDBDI_JUMBO_BD + TG3_BDINFO_HOST_ADDR + TG3_64BIT_REG_LOW,
			     (u32)((u64) tp->rx_jumbo_mapping & 0xffffffff));
	...
	tw32(HOSTCC_STATUS_BLK_HOST_ADDR + TG3_64BIT_REG_HIGH,
	     (u32)((u64) tp->status_mapping >> 32));
	tw32(HOSTCC_STATUS_BLK_HOST_ADDR + TG3_64BIT_REG_LOW,
	     (u32)((u64) tp->status_mapping & 0xffffffff));
	...
	if (!(tp->tg3_flags2 & TG3_FLG2_5705_PLUS)) {
		...
		tw32(HOSTCC_STATS_BLK_HOST_ADDR + TG3_64BIT_REG_HIGH,
		     (u32)((u64) tp->stats_mapping >> 32));
		tw32(HOSTCC_STATS_BLK_HOST_ADDR + TG3_64BIT_REG_LOW,
		     (u32)((u64) tp->stats_mapping & 0xffffffff));
...
{
	struct tg3 *tp = (struct tg3 *) DeferredContext;
	LARGE_INTEGER deltaTime;
...
//			schedule_work(&tp->reset_task);
	...
	deltaTime.QuadPart = - (LONGLONG)(tp->timer_offset);   
	KeSetTimer( &tp->timer, deltaTime, &tp->timerDpc );
...
static int tg3_test_msi(struct tg3 *tp)
{
	...
	NbDebugPrint(0, ("%s: No interrupt was generated using MSI, "
...
static int tg3_open(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
	int err;
	LARGE_INTEGER deltaTime;
	...
	NbDebugPrint(0, ("tg3_open entered\n"));	
	...
	if ((tp->tg3_flags2 & TG3_FLG2_5750_PLUS) &&
		 ...
		 */
		if (!(tp->tg3_flags & TG3_FLAG_TAGGED_STATUS)) {
			NbDebugPrint(0, ("%s: MSI without TAGGED? "
			       "Not using MSI.\n", tp->dev->name));
	...
	NbDebugPrint(0, ("tg3_open: irq = %d, tp->tg3_flags2 = %08X\n", tp->pdev->irq, tp->tg3_flags2 ));
...
*/
	...
	err = tg3_init_hw(tp);
	NbDebugPrint(0, ("tg3_open: tg3_init_hw(), err  = %08X\n", err));
	...
	if (err) {
		...
		tp->timer_counter = tp->timer_multiplier =
			(USHORT) ((HZ / tp->timer_offset));
		tp->asf_counter = tp->asf_multiplier =
			(USHORT)(((HZ / tp->timer_offset) * 2));
/*
		init_timer(&tp->timer);
		tp->timer.expires = jiffies + tp->timer_offset;
...
*/
		KeInitializeDpc( &tp->timerDpc, tg3_timer, tp );
		KeInitializeTimer( &tp->timer );
...
//	KeSetTimer( &tp->timer, deltaTime, &tp->timerDpc );
	...
	KeInitializeDpc( &InterruptTimerDpc_tg3, tg3_interrupt_tagged, dev );
	KeInitializeTimer( &InterruptTimer_tg3 );
...
static void tg3_dump_state(struct tg3 *tp)
{
	...
	NbDebugPrint(0, ("DEBUG: PCI status [%04x] TG3PCI state[%08x]\n",
	       val16, val32));

	/* MAC block */
	NbDebugPrint(0, ("DEBUG: MAC_MODE[%08x] MAC_STATUS[%08x]\n",
	       tr32(MAC_MODE), tr32(MAC_STATUS)));
	NbDebugPrint(0, ("       MAC_EVENT[%08x] MAC_LED_CTRL[%08x]\n",
	       tr32(MAC_EVENT), tr32(MAC_LED_CTRL)));
	NbDebugPrint(0, ("DEBUG: MAC_TX_MODE[%08x] MAC_TX_STATUS[%08x]\n",
	       tr32(MAC_TX_MODE), tr32(MAC_TX_STATUS)));
	NbDebugPrint(0, ("       MAC_RX_MODE[%08x] MAC_RX_STATUS[%08x]\n",
	       tr32(MAC_RX_MODE), tr32(MAC_RX_STATUS)));

#if 1
	NbDebugPrint(0, ("DEBUG: MAC_ADDR_0_HIGH[%08x] MAC_ADDR_0_LOW[%08x]\n",
	       tr32(MAC_ADDR_0_HIGH), tr32(MAC_ADDR_0_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_ADDR_1_HIGH[%08x] MAC_ADDR_1_LOW[%08x]\n",
	       tr32(MAC_ADDR_1_HIGH), tr32(MAC_ADDR_1_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_ADDR_2_HIGH[%08x] MAC_ADDR_2_LOW[%08x]\n",
	       tr32(MAC_ADDR_2_HIGH), tr32(MAC_ADDR_2_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_ADDR_3_HIGH[%08x] MAC_ADDR_3_LOW[%08x]\n",
	       tr32(MAC_ADDR_3_HIGH), tr32(MAC_ADDR_3_LOW)));

	NbDebugPrint(0, ("DEBUG: MAC_ACPI_MBUF_PTR[%08x] MAC_ACPI_LEN_OFFSET[%08x]\n",
	       tr32(MAC_ACPI_MBUF_PTR), tr32(MAC_ACPI_LEN_OFFSET)));
	NbDebugPrint(0, ("DEBUG: MAC_TX_BACKOFF_SEED[%08x] MAC_RX_MTU_SIZE[%08x]\n",
	       tr32(MAC_TX_BACKOFF_SEED), tr32(MAC_RX_MTU_SIZE)));
	NbDebugPrint(0, ("DEBUG: MAC_PCS_TEST[%08x]\n",
	       tr32(MAC_PCS_TEST)));
	NbDebugPrint(0, ("DEBUG: MAC_TX_AUTO_NEG[%08x] MAC_RX_AUTO_NEG[%08x]\n",
	       tr32(MAC_TX_AUTO_NEG), tr32(MAC_RX_AUTO_NEG)));
	NbDebugPrint(0, ("DEBUG: MAC_MI_COM[%08x] MAC_MI_STAT[%08x] MAC_MI_MODE[%08x]\n",
	       tr32(MAC_MI_COM), tr32(MAC_MI_STAT), tr32(MAC_MI_MODE)));
	NbDebugPrint(0, ("DEBUG: MAC_AUTO_POLL_STATUS[%08x]\n",
	       tr32(MAC_AUTO_POLL_STATUS)));
	NbDebugPrint(0, ("DEBUG: MAC_TX_LENGTHS[%08x]\n",
	       tr32(MAC_TX_LENGTHS)));

	NbDebugPrint(0, ("DEBUG: MAC_HASH_REG_0[%08x] MAC_HASH_REG_1[%08x] MAC_HASH_REG_2[%08x] MAC_HASH_REG_3[%08x]\n",
	       tr32(MAC_HASH_REG_0), tr32(MAC_HASH_REG_1), tr32(MAC_HASH_REG_2), tr32(MAC_HASH_REG_3)));
    NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_0[%08x] MAC_RCV_VALUE_0[%08x]\n",
	       tr32(MAC_RCV_RULE_0), tr32(MAC_RCV_VALUE_0)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_1[%08x] MAC_RCV_VALUE_1[%08x]\n",
	       tr32(MAC_RCV_RULE_1), tr32(MAC_RCV_VALUE_1)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_2[%08x] MAC_RCV_VALUE_2[%08x]\n",
	       tr32(MAC_RCV_RULE_2), tr32(MAC_RCV_VALUE_2)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_3[%08x] MAC_RCV_VALUE_3[%08x]\n",
	       tr32(MAC_RCV_RULE_3), tr32(MAC_RCV_VALUE_3)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_4[%08x] MAC_RCV_VALUE_4[%08x]\n",
	       tr32(MAC_RCV_RULE_4), tr32(MAC_RCV_VALUE_4)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_5[%08x] MAC_RCV_VALUE_5[%08x]\n",
	       tr32(MAC_RCV_RULE_5), tr32(MAC_RCV_VALUE_5)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_6[%08x] MAC_RCV_VALUE_6[%08x]\n",
	       tr32(MAC_RCV_RULE_6), tr32(MAC_RCV_VALUE_6)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_7[%08x] MAC_RCV_VALUE_7[%08x]\n",
	       tr32(MAC_RCV_RULE_7), tr32(MAC_RCV_VALUE_7)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_8[%08x] MAC_RCV_VALUE_8[%08x]\n",
	       tr32(MAC_RCV_RULE_8), tr32(MAC_RCV_VALUE_8)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_9[%08x] MAC_RCV_VALUE_9[%08x]\n",
	       tr32(MAC_RCV_RULE_9), tr32(MAC_RCV_VALUE_9)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_10[%08x] MAC_RCV_VALUE_10[%08x]\n",
	       tr32(MAC_RCV_RULE_10), tr32(MAC_RCV_VALUE_10)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_11[%08x] MAC_RCV_VALUE_11[%08x]\n",
	       tr32(MAC_RCV_RULE_11), tr32(MAC_RCV_VALUE_11)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_12[%08x] MAC_RCV_VALUE_12[%08x]\n",
	       tr32(MAC_RCV_RULE_12), tr32(MAC_RCV_VALUE_12)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_13[%08x] MAC_RCV_VALUE_13[%08x]\n",
	       tr32(MAC_RCV_RULE_13), tr32(MAC_RCV_VALUE_13)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_14[%08x] MAC_RCV_VALUE_14[%08x]\n",
	       tr32(MAC_RCV_RULE_14), tr32(MAC_RCV_VALUE_14)));
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_15[%08x] MAC_RCV_VALUE_15[%08x]\n",
	       tr32(MAC_RCV_RULE_15), tr32(MAC_RCV_VALUE_15)));
	
	NbDebugPrint(0, ("DEBUG: MAC_RCV_RULE_CFG[%08x]\n",
	       tr32(MAC_RCV_RULE_CFG)));
	NbDebugPrint(0, ("DEBUG: MAC_LOW_WMARK_MAX_RX_FRAME[%08x]\n",
	       tr32(MAC_LOW_WMARK_MAX_RX_FRAME)));

	NbDebugPrint(0, ("DEBUG: MAC_HASHREGU_0[%08x] MAC_HASHREGU_1[%08x] MAC_HASHREGU_2[%08x] MAC_HASHREGU_3[%08x]\n",
	       tr32(MAC_HASHREGU_0), tr32(MAC_HASHREGU_1), tr32(MAC_HASHREGU_2), tr32(MAC_HASHREGU_3)));

	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_0_HIGH[%08x] MAC_EXTADDR_0_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_0_HIGH), tr32(MAC_EXTADDR_0_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_1_HIGH[%08x] MAC_EXTADDR_1_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_1_HIGH), tr32(MAC_EXTADDR_1_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_2_HIGH[%08x] MAC_EXTADDR_2_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_2_HIGH), tr32(MAC_EXTADDR_2_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_3_HIGH[%08x] MAC_EXTADDR_3_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_3_HIGH), tr32(MAC_EXTADDR_3_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_4_HIGH[%08x] MAC_EXTADDR_4_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_4_HIGH), tr32(MAC_EXTADDR_4_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_5_HIGH[%08x] MAC_EXTADDR_5_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_5_HIGH), tr32(MAC_EXTADDR_5_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_6_HIGH[%08x] MAC_EXTADDR_6_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_6_HIGH), tr32(MAC_EXTADDR_6_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_7_HIGH[%08x] MAC_EXTADDR_7_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_7_HIGH), tr32(MAC_EXTADDR_7_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_8_HIGH[%08x] MAC_EXTADDR_8_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_8_HIGH), tr32(MAC_EXTADDR_8_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_9_HIGH[%08x] MAC_EXTADDR_9_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_9_HIGH), tr32(MAC_EXTADDR_9_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_10_HIGH[%08x] MAC_EXTADDR_10_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_10_HIGH), tr32(MAC_EXTADDR_10_LOW)));
	NbDebugPrint(0, ("DEBUG: MAC_EXTADDR_11_HIGH[%08x] MAC_EXTADDR_11_LOW[%08x]\n",
	       tr32(MAC_EXTADDR_11_HIGH), tr32(MAC_EXTADDR_11_LOW)));

	NbDebugPrint(0, ("DEBUG: MAC_SERDES_CFG[%08x] MAC_SERDES_STAT[%08x]\n",
	       tr32(MAC_SERDES_CFG), tr32(MAC_SERDES_STAT)));

	NbDebugPrint(0, ("DEBUG: MAC_TX_MAC_STATE_BASE[%08x] MAC_RX_MAC_STATE_BASE[%08x]\n",
	       tr32(MAC_TX_MAC_STATE_BASE), tr32(MAC_RX_MAC_STATE_BASE)));


#endif

	/* Send data initiator control block */
	NbDebugPrint(0, ("DEBUG: SNDDATAI_MODE[%08x] SNDDATAI_STATUS[%08x]\n",
	       tr32(SNDDATAI_MODE), tr32(SNDDATAI_STATUS)));
	NbDebugPrint(0, ("       SNDDATAI_STATSCTRL[%08x]\n",
	       tr32(SNDDATAI_STATSCTRL)));

	/* Send data completion control block */
	NbDebugPrint(0, ("DEBUG: SNDDATAC_MODE[%08x]\n", tr32(SNDDATAC_MODE)));

	/* Send BD ring selector block */
	NbDebugPrint(0, ("DEBUG: SNDBDS_MODE[%08x] SNDBDS_STATUS[%08x]\n",
	       tr32(SNDBDS_MODE), tr32(SNDBDS_STATUS)));

	/* Send BD initiator control block */
	NbDebugPrint(0, ("DEBUG: SNDBDI_MODE[%08x] SNDBDI_STATUS[%08x]\n",
	       tr32(SNDBDI_MODE), tr32(SNDBDI_STATUS)));

	/* Send BD completion control block */
	NbDebugPrint(0, ("DEBUG: SNDBDC_MODE[%08x]\n", tr32(SNDBDC_MODE)));

	/* Receive list placement control block */
	NbDebugPrint(0, ("DEBUG: RCVLPC_MODE[%08x] RCVLPC_STATUS[%08x]\n",
	       tr32(RCVLPC_MODE), tr32(RCVLPC_STATUS)));
	NbDebugPrint(0, ("       RCVLPC_STATSCTRL[%08x]\n",
	       tr32(RCVLPC_STATSCTRL)));

	/* Receive data and receive BD initiator control block */
	NbDebugPrint(0, ("DEBUG: RCVDBDI_MODE[%08x] RCVDBDI_STATUS[%08x]\n",
	       tr32(RCVDBDI_MODE), tr32(RCVDBDI_STATUS)));

	/* Receive data completion control block */
	NbDebugPrint(0, ("DEBUG: RCVDCC_MODE[%08x]\n",
	       tr32(RCVDCC_MODE)));

	/* Receive BD initiator control block */
	NbDebugPrint(0, ("DEBUG: RCVBDI_MODE[%08x] RCVBDI_STATUS[%08x]\n",
	       tr32(RCVBDI_MODE), tr32(RCVBDI_STATUS)));

	/* Receive BD completion control block */
	NbDebugPrint(0, ("DEBUG: RCVCC_MODE[%08x] RCVCC_STATUS[%08x]\n",
	       tr32(RCVCC_MODE), tr32(RCVCC_STATUS)));

	/* Receive list selector control block */
	NbDebugPrint(0, ("DEBUG: RCVLSC_MODE[%08x] RCVLSC_STATUS[%08x]\n",
	       tr32(RCVLSC_MODE), tr32(RCVLSC_STATUS)));

	/* Mbuf cluster free block */
	NbDebugPrint(0, ("DEBUG: MBFREE_MODE[%08x] MBFREE_STATUS[%08x]\n",
	       tr32(MBFREE_MODE), tr32(MBFREE_STATUS)));

	/* Host coalescing control block */
	NbDebugPrint(0, ("DEBUG: HOSTCC_MODE[%08x] HOSTCC_STATUS[%08x]\n",
	       tr32(HOSTCC_MODE), tr32(HOSTCC_STATUS)));
	NbDebugPrint(0, ("DEBUG: HOSTCC_STATS_BLK_HOST_ADDR[%08x%08x]\n",
	       tr32(HOSTCC_STATS_BLK_HOST_ADDR + TG3_64BIT_REG_HIGH),
	       tr32(HOSTCC_STATS_BLK_HOST_ADDR + TG3_64BIT_REG_LOW)));
	NbDebugPrint(0, ("DEBUG: HOSTCC_STATUS_BLK_HOST_ADDR[%08x%08x]\n",
	       tr32(HOSTCC_STATUS_BLK_HOST_ADDR + TG3_64BIT_REG_HIGH),
	       tr32(HOSTCC_STATUS_BLK_HOST_ADDR + TG3_64BIT_REG_LOW)));
	NbDebugPrint(0, ("DEBUG: HOSTCC_STATS_BLK_NIC_ADDR[%08x]\n",
	       tr32(HOSTCC_STATS_BLK_NIC_ADDR)));
	NbDebugPrint(0, ("DEBUG: HOSTCC_STATUS_BLK_NIC_ADDR[%08x]\n",
	       tr32(HOSTCC_STATUS_BLK_NIC_ADDR)));

	/* Memory arbiter control block */
	NbDebugPrint(0, ("DEBUG: MEMARB_MODE[%08x] MEMARB_STATUS[%08x]\n",
	       tr32(MEMARB_MODE), tr32(MEMARB_STATUS)));

	/* Buffer manager control block */
	NbDebugPrint(0, ("DEBUG: BUFMGR_MODE[%08x] BUFMGR_STATUS[%08x]\n",
	       tr32(BUFMGR_MODE), tr32(BUFMGR_STATUS)));
	NbDebugPrint(0, ("DEBUG: BUFMGR_MB_POOL_ADDR[%08x] BUFMGR_MB_POOL_SIZE[%08x]\n",
	       tr32(BUFMGR_MB_POOL_ADDR), tr32(BUFMGR_MB_POOL_SIZE)));
	NbDebugPrint(0, ("DEBUG: BUFMGR_DMA_DESC_POOL_ADDR[%08x] "
	       "BUFMGR_DMA_DESC_POOL_SIZE[%08x]\n",
	       tr32(BUFMGR_DMA_DESC_POOL_ADDR),
	       tr32(BUFMGR_DMA_DESC_POOL_SIZE)));

	/* Read DMA control block */
	NbDebugPrint(0, ("DEBUG: RDMAC_MODE[%08x] RDMAC_STATUS[%08x]\n",
	       tr32(RDMAC_MODE), tr32(RDMAC_STATUS)));

	/* Write DMA control block */
	NbDebugPrint(0, ("DEBUG: WDMAC_MODE[%08x] WDMAC_STATUS[%08x]\n",
	       tr32(WDMAC_MODE), tr32(WDMAC_STATUS)));

	/* DMA completion block */
	NbDebugPrint(0, ("DEBUG: DMAC_MODE[%08x]\n",
	       tr32(DMAC_MODE)));

	/* GRC block */
	NbDebugPrint(0, ("DEBUG: GRC_MODE[%08x] GRC_MISC_CFG[%08x]\n",
	       tr32(GRC_MODE), tr32(GRC_MISC_CFG)));
	NbDebugPrint(0, ("DEBUG: GRC_LOCAL_CTRL[%08x]\n",
	       tr32(GRC_LOCAL_CTRL)));

	/* TG3_BDINFOs */
	NbDebugPrint(0, ("DEBUG: RCVDBDI_JUMBO_BD[%08x%08x:%08x:%08x]\n",
	       tr32(RCVDBDI_JUMBO_BD + 0x0),
	       tr32(RCVDBDI_JUMBO_BD + 0x4),
	       tr32(RCVDBDI_JUMBO_BD + 0x8),
	       tr32(RCVDBDI_JUMBO_BD + 0xc)));
	NbDebugPrint(0, ("DEBUG: RCVDBDI_STD_BD[%08x%08x:%08x:%08x]\n",
	       tr32(RCVDBDI_STD_BD + 0x0),
	       tr32(RCVDBDI_STD_BD + 0x4),
	       tr32(RCVDBDI_STD_BD + 0x8),
	       tr32(RCVDBDI_STD_BD + 0xc)));
	NbDebugPrint(0, ("DEBUG: RCVDBDI_MINI_BD[%08x%08x:%08x:%08x]\n",
	...
	NbDebugPrint(0, ("DEBUG: SRAM_SEND_RCB_0[%08x%08x:%08x:%08x]\n",
	...
	NbDebugPrint(0, ("DEBUG: SRAM_RCV_RET_RCB_0[%08x%08x:%08x:%08x]\n",
	       val32, val32_2, val32_3, val32_4));
	...
	NbDebugPrint(0, ("DEBUG: SRAM_STATUS_BLK[%08x:%08x:%08x:%08x:%08x]\n",
	       val32, val32_2, val32_3, val32_4, val32_5));

	/* SW status block */
	NbDebugPrint(0, ("DEBUG: Host status block [%08x:%08x:(%04x:%04x:%04x):(%04x:%04x)]\n",
	       tp->hw_status->status,
	...
	/* SW statistics block */
	NbDebugPrint(0, ("DEBUG: Host statistics block [%08x:%08x:%08x:%08x]\n",
	       ((u32 *)tp->hw_stats)[0],
	       ((u32 *)tp->hw_stats)[1],
	       ((u32 *)tp->hw_stats)[2],
	       ((u32 *)tp->hw_stats)[3]));
	...
	/* Mailboxes */
	NbDebugPrint(0, ("DEBUG: SNDHOST_PROD[%08x%08x] SNDNIC_PROD[%08x%08x]\n",
	...
	for (i = 0; i < 6; i++) {
		...
		NbDebugPrint(0, ("DEBUG: NIC TXD(%d)[%08x:%08x:%08x:%08x]\n",
		       i,
	...
	for (i = 0; i < 6; i++) {
		...
		NbDebugPrint(0, ("DEBUG: NIC RXD_STD(%d)[0][%08x:%08x:%08x:%08x]\n",
		       i,
		       readl(rxd + 0x0), readl(rxd + 0x4),
		       readl(rxd + 0x8), readl(rxd + 0xc)));
		rxd += (4 * sizeof(u32));
		NbDebugPrint(0, ("DEBUG: NIC RXD_STD(%d)[1][%08x:%08x:%08x:%08x]\n",
		       i,
		       readl(rxd + 0x0), readl(rxd + 0x4),
		       readl(rxd + 0x8), readl(rxd + 0xc)));
	}
	...
	for (i = 0; i < 6; i++) {
		...
		rxd = tp->regs + NIC_SRAM_WIN_BASE + NIC_SRAM_RX_JUMBO_BUFFER_DESC
			+ (i * sizeof(struct tg3_rx_buffer_desc));
		NbDebugPrint(0, ("DEBUG: NIC RXD_JUMBO(%d)[0][%08x:%08x:%08x:%08x]\n",
		       i,
		       readl(rxd + 0x0), readl(rxd + 0x4),
		       readl(rxd + 0x8), readl(rxd + 0xc)));
		rxd += (4 * sizeof(u32));
		NbDebugPrint(0, ("DEBUG: NIC RXD_JUMBO(%d)[1][%08x:%08x:%08x:%08x]\n",
		       i,
		       readl(rxd + 0x0), readl(rxd + 0x4),
		       readl(rxd + 0x8), readl(rxd + 0xc)));
	}

	read_config_header(tp->pdev);
//	display_config_header(tp->pdev);
}
#endif

static struct net_device_stats *tg3_get_stats(struct device *);
static struct tg3_ethtool_stats *tg3_get_estats(struct tg3 *);

static int tg3_close(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
...
static __inline unsigned long get_stat64(tg3_stat64_t *val)
{
	unsigned long ret;
	...
	ret = val->low;
#else
	ret = (u32)(((u64)val->high << 32) | ((u64)val->low));
#endif
	return ret;
}
...
static struct net_device_stats *tg3_get_stats(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
...
static __inline u32 calc_crc(unsigned char *buf, int len)
{
	u32 reg;
	u32 tmp;
	int j, k;
...
static void __tg3_set_rx_mode(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
	u32 rx_mode;
...
static void tg3_set_rx_mode(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_get_regs_len(struct device *dev)
{
	return TG3_REGDUMP_LEN;
}

static void tg3_get_regs(struct device *dev,
...
static int tg3_get_eeprom_len(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_get_eeprom(struct device *dev, struct ethtool_eeprom *eeprom, u8 *data)
{
	struct tg3 *tp = netdev_priv(dev);
	int ret;
	u8  *pd;
...
static int tg3_set_eeprom(struct device *dev, struct ethtool_eeprom *eeprom, u8 *data)
{
	struct tg3 *tp = netdev_priv(dev);
	int ret;
...
static int tg3_get_settings(struct device *dev, struct ethtool_cmd *cmd)
{
  	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_set_settings(struct device *dev, struct ethtool_cmd *cmd)
{
	struct tg3 *tp = netdev_priv(dev);
...
static void tg3_get_drvinfo(struct device *dev, struct ethtool_drvinfo *info)
{
	struct tg3 *tp = netdev_priv(dev);
...
static void tg3_get_wol(struct device *dev, struct ethtool_wolinfo *wol)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_set_wol(struct device *dev, struct ethtool_wolinfo *wol)
{
	struct tg3 *tp = netdev_priv(dev);
...
static u32 tg3_get_msglevel(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
	return tp->msg_enable;
}
  
static void tg3_set_msglevel(struct device *dev, u32 value)
{
	struct tg3 *tp = netdev_priv(dev);
	tp->msg_enable = value;
}
...
#if TG3_TSO_SUPPORT != 0
static int tg3_set_tso(struct device *dev, u32 value)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_nway_reset(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
	u32 bmcr;
	int r;
...
static void tg3_get_ringparam(struct device *dev, struct ethtool_ringparam *ering)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_set_ringparam(struct device *dev, struct ethtool_ringparam *ering)
{
	struct tg3 *tp = netdev_priv(dev);
	int irq_sync = 0;
...
static void tg3_get_pauseparam(struct device *dev, struct ethtool_pauseparam *epause)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_set_pauseparam(struct device *dev, struct ethtool_pauseparam *epause)
{
	struct tg3 *tp = netdev_priv(dev);
	int irq_sync = 0;
...
static u32 tg3_get_rx_csum(struct device *dev)
{
	struct tg3 *tp = netdev_priv(dev);
	return (tp->tg3_flags & TG3_FLAG_RX_CHECKSUMS) != 0;
}
  
static int tg3_set_rx_csum(struct device *dev, u32 data)
{
	struct tg3 *tp = netdev_priv(dev);
...
#if (LINUX_VERSION_CODE >= 0x20418)
static int tg3_set_tx_csum(struct device *dev, u32 data)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_get_stats_count (struct device *dev)
{
	return TG3_NUM_STATS;
}

static int tg3_get_test_count (struct device *dev)
{
	return TG3_NUM_TEST;
}

static void tg3_get_strings (struct device *dev, u32 stringset, u8 *buf)
{
	switch (stringset) {
	case ETH_SS_STATS:
...
static int tg3_phys_id(struct device *dev, u32 data)
{
	struct tg3 *tp = netdev_priv(dev);
	unsigned int i;
...
static void tg3_get_ethtool_stats (struct device *dev,
...
out:
	NbDebugPrint(0, ("Register test failed at offset %x\n", offset));
	tw32(offset, save_val);
	return -EIO;
}
...
static void tg3_self_test(struct device *dev, struct ethtool_test *etest,
			  u64 *data)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_ioctl(struct device *dev, struct ifreq *ifr, int cmd)
{
...
#if TG3_VLAN_TAG_USED
static void tg3_vlan_rx_register(struct device *dev, struct vlan_group *grp)
{
	struct tg3 *tp = netdev_priv(dev);
...
static void tg3_vlan_rx_kill_vid(struct device *dev, unsigned short vid)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_get_coalesce(struct device *dev, struct ethtool_coalesce *ec)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_set_coalesce(struct device *dev, struct ethtool_coalesce *ec)
{
	struct tg3 *tp = netdev_priv(dev);
...
static int tg3_nvram_read(struct tg3 *tp, u32 offset, u32 *val)
{
	...
	if (tp->tg3_flags2 & TG3_FLG2_SUN_570X) {
		NbDebugPrint(0, ("Attempt to do nvram_read on Sun 570X\n"));
		return -EINVAL;
	}
		...
		u8 *buf)
{
	...
	for (i = 0; i < len; i += 4, offset += 4) {
		...
		/* Must not use write enable on 5752 */
		if ((GET_ASIC_REV(tp->pci_chip_rev_id) != ASIC_REV_5752) &&
		    (tp->nvram_jedecnum == JEDEC_ST) &&
		    (nvram_cmd & NVRAM_CMD_FIRST)) {
...
static int tg3_nvram_write_block(struct tg3 *tp, u32 offset, u32 len, u8 *buf)
{
	...
	if (tp->tg3_flags2 & TG3_FLG2_SUN_570X) {
		NbDebugPrint(0, ("Attempt to do nvram_write on Sun 570X\n"));
		return -EINVAL;
	}
...
static __inline struct subsys_tbl_ent *lookup_by_subsys(struct tg3 *tp)
{
	int i;
...
static void __devinit tg3_get_eeprom_hw_cfg(struct tg3 *tp)
{
	...
	NbDebugPrint(0, ("tg3_get_eeprom_hw_cfg: NIC_SRAM_DATA_SIG = %08X\n", val));
	if (val == NIC_SRAM_DATA_SIG_MAGIC) {
		u32 nic_cfg, led_cfg;
		...
		tg3_read_mem(tp, NIC_SRAM_DATA_CFG, &nic_cfg);
		NbDebugPrint(0, ("tg3_get_eeprom_hw_cfg: NIC_SRAM_DATA_CFG = %08X\n", nic_cfg));
		...
		NbDebugPrint(0, ("tg3_get_eeprom_hw_cfg: NIC_SRAM_DATA_PHY_ID = %08X\n", nic_phy_id));
...
static void __devinit tg3_read_partno(struct tg3 *tp)
{
	...
	for (i = 0; i < 256; i += 4) {
		...
		vpd_data[i + 0] = (u8)((tmp >>  0) & 0xff);
		vpd_data[i + 1] = (u8)((tmp >>  8) & 0xff);
		vpd_data[i + 2] = (u8)((tmp >> 16) & 0xff);
		vpd_data[i + 3] = (u8)((tmp >> 24) & 0xff);
	}
...
static int __devinit tg3_get_invariants(struct tg3 *tp)
{
	...
	tp->pci_chip_rev_id = (u16)((misc_ctrl_reg >>
			       MISC_HOST_CTRL_CHIPREV_SHIFT));

	NbDebugPrint(0, ("tg3_get_invariants: tp->pci_chip_rev_id = %08X\n", tp->pci_chip_rev_id));
	...
	tp->pci_cacheline_sz = (u8)(cacheline_sz_reg >>  0) & 0xff;
	tp->pci_lat_timer    = (u8)(cacheline_sz_reg >>  8) & 0xff;
	tp->pci_hdr_type     = (u8)(cacheline_sz_reg >> 16) & 0xff;
	tp->pci_bist         = (u8)(cacheline_sz_reg >> 24) & 0xff;
...
//		tp->regs = NULL;
	...
	err = tg3_set_power_state(tp, 0);
	if (err) {
		NbDebugPrint(0, ("transition to D0 failed\n"));
		return err;
	}
	...
	if ((pci_state_reg & PCISTATE_CONV_PCI_MODE) == 0 &&
		...
		NbDebugPrint(0, ("tg3_get_invariants:chiprevid = %08X\n", chiprevid));
...
//			void __iomem *sram_base;
			...
			writel(0x00000000, (u32) sram_base + 4);
			writel(0xffffffff, (u32) sram_base + 4);			
	...
	NbDebugPrint(0, ("tg3_get_invariants:grc_misc_cfg = %08X\n", grc_misc_cfg));
	...
	if (err) {
		NbDebugPrint(0, ("phy probe failed, err %d\n", err));
...
static int __devinit tg3_get_device_address(struct tg3 *tp)
{
	...
	if ((hi >> 16) == 0x484b) {
		dev->dev_addr[0] = (u8)(hi >>  8) & 0xff;
		dev->dev_addr[1] = (u8)(hi >>  0) & 0xff;

		tg3_read_mem(tp, NIC_SRAM_MAC_ADDR_LOW_MBOX, &lo);
		dev->dev_addr[2] = (u8)(lo >> 24) & 0xff;
		dev->dev_addr[3] = (u8)(lo >> 16) & 0xff;
		dev->dev_addr[4] = (u8)(lo >>  8) & 0xff;
		dev->dev_addr[5] = (u8)(lo >>  0) & 0xff;
	}
	/* Next, try NVRAM. */
	else if (!(tp->tg3_flags & TG3_FLG2_SUN_570X) &&
		 !tg3_nvram_read(tp, mac_offset + 0, &hi) &&
		 !tg3_nvram_read(tp, mac_offset + 4, &lo)) {
		dev->dev_addr[0] = (u8)((hi >> 16) & 0xff);
		dev->dev_addr[1] = (u8)((hi >> 24) & 0xff);
		dev->dev_addr[2] = (u8)((lo >>  0) & 0xff);
		dev->dev_addr[3] = (u8)((lo >>  8) & 0xff);
		dev->dev_addr[4] = (u8)((lo >> 16) & 0xff);
		dev->dev_addr[5] = (u8)((lo >> 24) & 0xff);
	}
	/* Finally just fetch it out of the MAC control regs. */
		...
		dev->dev_addr[5] = (u8)lo & 0xff;
		dev->dev_addr[4] = (u8)(lo >> 8) & 0xff;
		dev->dev_addr[3] = (u8)(lo >> 16) & 0xff;
		dev->dev_addr[2] = (u8)(lo >> 24) & 0xff;
		dev->dev_addr[1] = (u8)hi & 0xff;
		dev->dev_addr[0] = (u8)(hi >> 8) & 0xff;
	}
...
static int __devinit tg3_do_test_dma(struct tg3 *tp, u32 *buf, dma_addr_t buf_dma, int size, int to_device)
{
	...
	NbDebugPrint(0, ("tg3_do_test_dma: buf_dma = %08X, size = %08X, to_device = %d\n", buf_dma, size, to_device));
	...
	test_desc.addr_hi = (u32)(((u64) buf_dma) >> 32);
	test_desc.addr_lo = (u32)(buf_dma & 0xffffffff);
	test_desc.nic_mbuf = 0x00002100;
	test_desc.len = (USHORT)size;
...
static int __devinit tg3_test_dma(struct tg3 *tp)
{
	...
	NbDebugPrint(0, ("tg3_test_dma entered\n"));
	...
	while (1) {
		...
		if (ret) {
			NbDebugPrint(0, ("tg3_test_dma() Write the buffer failed %d\n", ret));
			break;
		}
		...
		for (i = 0; i < TEST_BUFFER_SIZE / sizeof(u32); i++) {
			...
			if (le32_to_cpu(val) != p[i]) {
				NbDebugPrint(0, ("  tg3_test_dma()  Card buffer corrupted on write! (%d != %d)\n", val, i));
				/* ret = -ENODEV here? */
			}
			p[i] = 0;
		}
		...
		if (ret) {
			NbDebugPrint(0, ("tg3_test_dma() Read the buffer failed %d\n", ret));
		...
		for (i = 0; i < TEST_BUFFER_SIZE / sizeof(u32); i++) {
			...
			if ((tp->dma_rwctrl & DMA_RWCTRL_WRITE_BNDRY_MASK) !=
				...
				NbDebugPrint(0, ("tg3_test_dma() buffer corrupted on read back! (%d != %d)\n", p[i], i));
				ret = -ENODEV;
				goto out;
			}
		}
...
int __devinit tg3_init_one(struct device *dev)
{
	...
	NbDebugPrint(0, ("tg3_init_one Entered ~~~~~\n"));
	NbDebugPrint(0, ("tg3_init_one:OSTimeTick = %08X\n", OSTimeTick));
	...
	//	display_config_header(pdev);
		...
		if (!(pci_resource_flags(pdev, 0) & IORESOURCE_MEM)) {
			NbDebugPrint(0, ("Cannot find proper PCI device "
...
//		if (err) {
//			NbDebugPrint(0,("Cannot obtain PCI resources, "
//				"aborting.\n"));
//			goto err_out_disable_pdev;
//		}
		...
		if (pm_cap == 0) {
			NbDebugPrint(0, ("Cannot find PowerManagement capability, "
				"aborting.\n"));
			err = -EIO;
		...
		if (!err) {
			...
			if (err < 0) {
				NbDebugPrint(0, ("Unable to obtain 64 bit DMA "
			...
			if (err) {
				NbDebugPrint(0, ("No usable DMA configuration, "
					"aborting.\n"));
				goto err_out_free_res;
			}
			pci_using_dac = 0;
...
//		if (!dev) {
//			NbDebugPrint(0, ("Etherdev alloc failed, aborting.\n"));
//			err = -ENOMEM;
//			goto err_out_free_res;
//		}

		NbDebugPrint(0, ("tg3_init_one: tg3reg_base = %08X, tg3reg_len = %08X\n", tg3reg_base, tg3reg_len));
...
//		SET_MODULE_OWNER(dev);
	...
	#endif
		KeInitializeSpinLock(&tp->lock);
		KeInitializeSpinLock(&tp->tx_lock);
		KeInitializeSpinLock(&tp->indirect_lock);
...
//		tp->regs = ioremap_nocache(tg3reg_base, tg3reg_len);
...
#ifdef __ENABLE_LOADER__
		tp->regs = (ULONG) ScsiPortGetDeviceBase(NULL,
                                        PCIBus,
										pdev->bus->number,
                                        ScsiPortConvertUlongToPhysicalAddress((ULONG)tg3reg_base),
                                        tg3reg_len,
                                        FALSE);
#else

		tp->regs = (ULONG) ScsiPortGetDeviceBase(HwExtension,
                                        PCIBus,
										pdev->bus->number,
                                        ScsiPortConvertUlongToPhysicalAddress((ULONG)tg3reg_base),
                                        tg3reg_len,
		...
		NbDebugPrint(0, ("ScsiPortGetDeviceBase:  tp->regs = %08X\n", tp->regs));	

		if (tp->regs == 0UL) {
			NbDebugPrint(0, ("Cannot map device registers, "
				"aborting.\n"));
			err = -ENOMEM;
	...
	//	dev->change_mtu = tg3_change_mtu;
		...
		dev->get_status = &tg_get_status;

		err = tg3_get_invariants(tp);
		if (err) {
			NbDebugPrint(0, ("Problem fetching invariants of chip, "
				"aborting.\n"));
		...
		if (err) {
			NbDebugPrint(0, ("Could not obtain valid ethernet address, "
				"aborting.\n"));
			goto err_out_iounmap;
		}
		...
		*/
		...
		err = tg3_test_dma(tp);
		if (err) {
			NbDebugPrint(0, ("DMA engine test failed, aborting.\n"));
			goto err_out_iounmap;
		}
...
//		if (err) {
//			NbDebugPrint(0, ("Cannot register net device, "
//				"aborting.\n"));
//			goto err_out_iounmap;
//		}
		...
		NbDebugPrint(0, ("%s: Tigon3 [partno(%s) rev %04x PHY(%s)] (%s) %sBaseT Ethernet ",
			dev->name,
		...
		for (i = 0; i < 6; i++)
			NbDebugPrint(0, ("%2.2x%c", dev->dev_addr[i],
				i == 5 ? '\n' : ':'));

		NbDebugPrint(0, ("%s: RXcsums[%d] LinkChgREG[%d] "
		...
		NbDebugPrint(0, ("%s: dma_rwctrl[%08x]\n",
...
#endif
{
	...
	if (err) {
		...
		deltaTime.QuadPart = - tp->timer_offset;    
		KeSetTimer( &tp->timer, deltaTime, &tp->timerDpc );
