...
static const char SysKonnectFileId[] =
	"@(#) $Id: skgeinit.c,v 2.81 2005/09/01 07:25:25 malthoff Exp $ (C) Marvell.";
#endif
...
SK_BOOL PollTxD)	/* SK_TRUE (enable pol.), SK_FALSE (disable pol.) */
{
	...
	if (pPrt->PXSQSize != 0) {
		...
		{
			SK_U32 Word;
			SK_IN32(IoC, Q_ADDR(pPrt->PXsQOff, Q_CSR), &Word);
			NbDebugPrint(3, ("SkGePollTxD: Q_ADDR(pPrt->PXsQOff, Q_CSR) = %08X\n", Word));
		}
	}
	...
	if (pPrt->PXAQSize != 0) {
		...
		{
			SK_U32 Word;
			SK_IN32(IoC, Q_ADDR(pPrt->PXaQOff, Q_CSR), &Word);
			NbDebugPrint(3, ("SkGePollTxD: Q_ADDR(pPrt->PXaQOff, Q_CSR) = %08X\n", Word));
		}
	}
}	/* SkGePollTxD */
...
int		 Port)		/* port index */
{
	...
	for (i = 0; i < pAC->GIni.GIMacsFound; i++) {
		...
		NbDebugPrint(3, ("SkGeCheckQSize: UsedMem = %08X\n", UsedMem));
	...
	for (i = 0; i < pAC->GIni.GIMacsFound; i++) {
		...
		NbDebugPrint(3, ("SkGeCheckQSize: pPrt->PRxQRamStart = %08X, pPrt->PRxQRamEnd = %08X\n", pPrt->PRxQRamStart, pPrt->PRxQRamEnd));
		...
		NbDebugPrint(3, ("SkGeCheckQSize: pPrt->PXsQRamStart = %08X, pPrt->PXsQRamEnd = %08X\n", pPrt->PXsQRamStart, pPrt->PXsQRamEnd));
		...
		NbDebugPrint(3, ("SkGeCheckQSize: pPrt->PXaQRamStartt = %08X, pPrt->PXaQRamEnd = %08X\n", pPrt->PXaQRamStart, pPrt->PXaQRamEnd));
...
int		Port)		/* Port Index (MAC_1 + n) */
{
	...
	if (pAC->GIni.GIYukon) {
		...
		{
			SK_U16 TWord;
			SK_IN16(IoC, MR_ADDR(Port, RX_GMF_CTRL_T), &TWord);
			NbDebugPrint(3, ("SkGeInitMacFifo: before MR_ADDR(Port, RX_GMF_CTRL_T) = %04X\n", TWord));
		}
		SK_OUT16(IoC, MR_ADDR(Port, RX_GMF_CTRL_T), Word);
		{
			SK_U16 TWord;
			SK_IN16(IoC, MR_ADDR(Port, RX_GMF_CTRL_T), &TWord);
			NbDebugPrint(3, ("SkGeInitMacFifo: after MR_ADDR(Port, RX_GMF_CTRL_T) = %04X\n", TWord));
		}
		...
		{
			SK_U16 TWord;
			SK_IN16(IoC, MR_ADDR(Port, RX_GMF_FL_MSK), &TWord);
			NbDebugPrint(3, ("SkGeInitMacFifo: before MR_ADDR(Port, RX_GMF_FL_MSK) = %04X\n", TWord));
		}
		SK_OUT16(IoC, MR_ADDR(Port, RX_GMF_FL_MSK), Word);
		{
			SK_U16 TWord;
			SK_IN16(IoC, MR_ADDR(Port, RX_GMF_FL_MSK), &TWord);
			NbDebugPrint(3, ("SkGeInitMacFifo: after MR_ADDR(Port, RX_GMF_FL_MSK) = %04X\n", TWord));
		}
		...
		{
			SK_U16 TWord;
			SK_IN16(IoC, MR_ADDR(Port, RX_GMF_FL_THR), &TWord);
			NbDebugPrint(3, ("SkGeInitMacFifo: before MR_ADDR(Port, RX_GMF_FL_THR) = %04X\n", TWord));
		}
		SK_OUT16(IoC, MR_ADDR(Port, RX_GMF_FL_THR), Word);
		{
			SK_U16 TWord;
			SK_IN16(IoC, MR_ADDR(Port, RX_GMF_FL_THR), &TWord);
			NbDebugPrint(3, ("SkGeInitMacFifo: after MR_ADDR(Port, RX_GMF_FL_THR) = %04X\n", TWord));
		}
		...
		{
			SK_U16 TWord;
			SK_IN16(IoC, MR_ADDR(Port, TX_GMF_CTRL_T), &TWord);
			NbDebugPrint(3, ("SkGeInitMacFifo: before MR_ADDR(Port, TX_GMF_CTRL_T) = %04X\n", TWord));
		}
...

		{
			SK_U16 TWord;
			SK_IN16(IoC, MR_ADDR(Port, TX_GMF_CTRL_T), &TWord);
			NbDebugPrint(3, ("SkGeInitMacFifo: after MR_ADDR(Port, TX_GMF_CTRL_T) = %04X\n", TWord));
		}

		if ( pAC->GIni.GIChipId == CHIP_ID_YUKON_EC_U ) {

			SK_OUT8(IoC, MR_ADDR(Port, RX_GMF_LP_THR), SK_ECU_LLPP);
			SK_OUT8(IoC, MR_ADDR(Port, RX_GMF_UP_THR), SK_ECU_ULPP);

			if (pAC->GIni.GP[Port].PPortUsage == SK_JUMBO_LINK) {
				/* set Tx GMAC FIFO Almost Empty Threshold */
				SK_OUT32(IoC, MR_ADDR(Port, TX_GMF_AE_THR), SK_ECU_AE_THR);
...
int		QuType)			/* Queue Type (SK_RX_SRAM_Q|SK_RX_BRAM_Q|SK_TX_RAM_Q) */
{
	...
	if (QuStartAddr != QuEndAddr) {
		...
		NbDebugPrint(3, ("DoInitRamQueue:QuStartAddr = %08X, QuEndAddr = %08X, QuType = %08X\n", QuStartAddr, QuEndAddr, QuType));
		...
		{
			SK_U32 DWord;
			SK_IN32(IoC, RB_ADDR(QuIoOffs, RB_START), &DWord);
			NbDebugPrint(3, ("DoInitRamQueue:RB_ADDR(QuIoOffs, RB_START) = %08X\n", DWord));
		}
		SK_OUT32(IoC, RB_ADDR(QuIoOffs, RB_END), QuEndAddr);
		{
			SK_U32 DWord;
			SK_IN32(IoC, RB_ADDR(QuIoOffs, RB_END), &DWord);
			NbDebugPrint(3, ("DoInitRamQueue:RB_ADDR(QuIoOffs, RB_END) = %08X\n", DWord));
		}
		SK_OUT32(IoC, RB_ADDR(QuIoOffs, RB_WP), QuStartAddr);
		{
			SK_U32 DWord;
			SK_IN32(IoC, RB_ADDR(QuIoOffs, RB_WP), &DWord);
			NbDebugPrint(3, ("DoInitRamQueue:RB_ADDR(QuIoOffs, RB_WP) = %08X\n", DWord));
		}
		SK_OUT32(IoC, RB_ADDR(QuIoOffs, RB_RP), QuStartAddr);
		{
			SK_U32 DWord;
			SK_IN32(IoC, RB_ADDR(QuIoOffs, RB_RP), &DWord);
			NbDebugPrint(3, ("DoInitRamQueue:RB_ADDR(QuIoOffs, RB_RP) = %08X\n", DWord));
		}

		NbDebugPrint(3, ("DoInitRamQueue: RxUpThresVal = %08X, RxLoThresVal = %08X\n", RxUpThresVal, RxLoThresVal));
		switch (QuType) {
		case SK_RX_SRAM_Q:
		...
		case SK_RX_BRAM_Q:
			...
			{
				SK_U32 DWord;
				SK_IN32(IoC, RB_ADDR(QuIoOffs, RB_RX_UTPP), &DWord);
				NbDebugPrint(3, ("DoInitRamQueue:RB_ADDR(QuIoOffs, RB_RX_UTPP) = %08X\n", DWord));
			}
			SK_OUT32(IoC, RB_ADDR(QuIoOffs, RB_RX_LTPP), RxLoThresVal);
			{
				SK_U32 DWord;
				SK_IN32(IoC, RB_ADDR(QuIoOffs, RB_RX_LTPP), &DWord);
				NbDebugPrint(3, ("DoInitRamQueue:RB_ADDR(QuIoOffs, RB_RX_LTPP) = %08X\n", DWord));
			}
		...
		case SK_TX_RAM_Q:
			...
			if (pAC->GIni.GP[MAC_1].PPortUsage == SK_JUMBO_LINK ||
				...
				{
					SK_U8 Byte;
					SK_IN8(IoC, RB_ADDR(QuIoOffs, RB_CTRL), &Byte);
					NbDebugPrint(3, ("DoInitRamQueue:RB_ADDR(QuIoOffs, RB_CTRL) = %08X\n", Byte));
				}
			}
			break;
		}
		...
		SK_OUT8(IoC, RB_ADDR(QuIoOffs, RB_CTRL), RB_RST_SET);
		{
			SK_U8 Byte;
			SK_IN8(IoC, RB_ADDR(QuIoOffs, RB_CTRL), &Byte);
			NbDebugPrint(3, ("ELSE:DoInitRamQueue:RB_ADDR(QuIoOffs, RB_CTRL) = %08X\n", Byte));
		}
	}
}	/* DoInitRamQueue */
	...
	if (CHIP_ID_YUKON_2(pAC)) {
		...
		if (pPrt->PXAQSize != 0) {
			...
			if (pAC->GIni.GIChipId == CHIP_ID_YUKON_EC_U) {
				...
				SK_OUT16(IoC, Q_ADDR(pPrt->PXaQOff, Q_AL), 0x1a0);
			}
		}
	}
	else {
		NbDebugPrint(3, ("SkGeInitBmu: pAC->GIni.GIPciSlot64 = %d, pAC->GIni.GIPciClock66 = %d\n", pAC->GIni.GIPciSlot64, pAC->GIni.GIPciClock66));
		...
		SK_OUT32(IoC, Q_ADDR(pPrt->PRxQOff, Q_CSR), CSR_CLR_RESET);
		{
			...
			NbDebugPrint(3, ("SkGeInitBmu: Q_ADDR(pPrt->PRxQOff, Q_CSR) = %08X\n", DWord));
		}
		SK_OUT32(IoC, Q_ADDR(pPrt->PRxQOff, Q_F), RxWm);
		{
			SK_U32 DWord;
			SK_IN32(IoC, Q_ADDR(pPrt->PRxQOff, Q_F), &DWord);
			NbDebugPrint(3, ("SkGeInitBmu: Q_ADDR(pPrt->PRxQOff, Q_F) = %08X\n", DWord));
		}
		...
		if (pPrt->PXSQSize != 0) {
			SK_OUT32(IoC, Q_ADDR(pPrt->PXsQOff, Q_CSR), CSR_CLR_RESET);
			{
				...
				NbDebugPrint(3, ("SkGeInitBmu: Q_ADDR(pPrt->PXsQOff, Q_CSR) = %08X\n", DWord));
			}
			SK_OUT32(IoC, Q_ADDR(pPrt->PXsQOff, Q_F), TxWm);
			{
				SK_U32 DWord;
				SK_IN32(IoC, Q_ADDR(pPrt->PXsQOff, Q_F), &DWord);
				NbDebugPrint(3, ("SkGeInitBmu: Q_ADDR(pPrt->PXsQOff, Q_F) = %08X\n", DWord));
			}
		}
		...
		if (pPrt->PXAQSize != 0) {
			SK_OUT32(IoC, Q_ADDR(pPrt->PXaQOff, Q_CSR), CSR_CLR_RESET);
			{
				...
				NbDebugPrint(3, ("SkGeInitBmu: Q_ADDR(pPrt->PXaQOff, Q_CSR) = %08X\n", DWord));
			}
			SK_OUT32(IoC, Q_ADDR(pPrt->PXaQOff, Q_F), TxWm);
			{
				SK_U32 DWord;
				SK_IN32(IoC, Q_ADDR(pPrt->PXaQOff, Q_F), &DWord);
				NbDebugPrint(3, ("SkGeInitBmu: Q_ADDR(pPrt->PXaQOff, Q_F) = %08X\n", DWord));
			}
		}
	}
	/*
...
int		RstMode)/* Reset Mode (SK_SOFT_RST, SK_HARD_RST) */
{
	...
	if ((Dir & SK_STOP_TX) != 0) {
		...
		NbDebugPrint(3, ("SkGeStopPort: XsCsr = %08X, XaCsr = %08X\n", XsCsr, XaCsr));
		NbDebugPrint(3, ("SkGeStopPort: pAC->GIni.GIAsfEnabled = %d\n", pAC->GIni.GIAsfEnabled));
		if (pAC->GIni.GIAsfEnabled) {
			...
			if (RstMode == SK_SOFT_RST) {
				...
				NbDebugPrint(3, ("SkGeStopPort:SkMacSoftRst\n"));
				SkMacSoftRst(pAC, IoC, Port);				
			}
			else {
#ifndef SK_SLIM
				NbDebugPrint(3, ("SkGeStopPort:SkMacHardRst\n"));
				...
				if (HW_FEATURE(pAC, HWF_WA_DEV_472) && Port == MAC_1 &&
					...
					NbDebugPrint(3, ("SkGeStopPort: set GPHY Control reset\n"));
	...
	if ((Dir & SK_STOP_RX) != 0) {
		...
		if (CHIP_ID_YUKON_2(pAC)) {
			...
			while (--i) {
				...
				NbDebugPrint(3, ("SkGeStopPort: rsl = %02x, rl = %02X\n", rsl, rl));
				if (rsl == rl) {
					break;
				}
			}
			...
			NbDebugPrint(3, ("SkGeStopPort:RxCsr = %08X\n", RxCsr));
...
SK_IOC	IoC)		/* I/O Context */
{
	...
	for (i = 0; i < 4; i++) {
		...
		NbDebugPrint(3, ("SkGeSetUpSupFeatures:pAC->GIni.HwF.Features[%d] = %08X\n", i, pAC->GIni.HwF.Features[i]));
	}
...
SK_IOC	IoC)		/* I/O Context */
{
	...
	NbDebugPrint(3, ("SkGeInit1: BO_CTST = %08X\n", CtrlStat));
	...
	NbDebugPrint(3, ("SkGeInit1: B2_CHIP_ID = %08X\n", Byte));
	...
	NbDebugPrint(3, ("SkGeInit1:PCI_C(pAC, PCI_STATUS) = %08X\n", Word));
	...
	{
		SK_U16 TWord;
		SK_IN16(IoC, PCI_C(pAC, PCI_STATUS), &TWord);
		NbDebugPrint(3, ("SkGeInit1:PCI_C(pAC, PCI_STATUS) = %08X\n", TWord));
	}
	...
	if ((pAC->GIni.GIChipId >= CHIP_ID_YUKON_XL) &&
		...
		NbDebugPrint(3, ("SkGeInit1:B2_MAC_CFG = %08X\n", Byte));
	...
	NbDebugPrint(3, ("SkGeInit1:pAC->GIni.GIChipRev = %08X\n", pAC->GIni.GIChipRev));
	...
	NbDebugPrint(3, ("SkGeInit1:B2_E_0 = %08X\n", Byte));
	...
	if (pAC->GIni.GIChipId != CHIP_ID_GENESIS) {
		...
		NbDebugPrint(3, ("SkGeInit1:pAC->GIni.GIWolOffs = %08X\n", pAC->GIni.GIWolOffs));

		/* get PM Capabilities of PCI config space */
		SK_IN16(IoC, PCI_C(pAC, PCI_PM_CAP_REG), &Word);

		NbDebugPrint(3, ("SkGeInit1:PCI_C(pAC, PCI_PM_CAP_REG) = %08X\n", Word));
...

		{
			...
			NbDebugPrint(3, ("SkGeInit1:B0_POWER_CTRL = %08X\n", Byte));
		}
		...
		if (CHIP_ID_YUKON_2(pAC)) {
			...
			NbDebugPrint(3, ("SkGeInit1:B0_ISRC = %08X\n", DWord));

			if ((DWord & IS_HW_ERR) != 0) {
				/* read the HW Error Interrupt source */
				SK_IN32(IoC, B0_HWE_ISRC, &DWord);
				NbDebugPrint(3, ("SkGeInit1:B0_HWE_ISRC = %08X\n", DWord));
		...
		SK_IN32(IoC, PCI_C(pAC, PCI_OUR_REG_1), &DWord);
		NbDebugPrint(3, ("SkGeInit1:PCI_C(pAC, PCI_OUR_REG_1) = %08X\n", DWord));
		...
		NbDebugPrint(3, ("SkGeInit1:pAC->GIni.GIAsfEnabled = %d\n", pAC->GIni.GIAsfEnabled));
		if (!pAC->GIni.GIAsfEnabled) {
	...
	if (!CHIP_ID_YUKON_2(pAC)) {
		...
		NbDebugPrint(3, ("SkGeInit1:pAC->GIni.GIPciBus = %d, pAC->GIni.GIPciSlot64 = %d, pAC->GIni.GIPciClock66 = %d\n",
			pAC->GIni.GIPciBus,
			pAC->GIni.GIPciSlot64,
			pAC->GIni.GIPciClock66));
	}
	...
	NbDebugPrint(3, ("SkGeInit1:PCI_C(pAC, PCI_REV_ID) = %08X\n", Byte));
	/* read connector type */
	SK_IN8(IoC, B2_CONN_TYP, &pAC->GIni.GIConTyp);

	NbDebugPrint(3, ("SkGeInit1:B2_CONN_TYP = %08X\n", pAC->GIni.GIConTyp));
#endif /* !SK_SLIM */

	/* read the PMD type */
	SK_IN8(IoC, B2_PMD_TYP, &Byte);
	NbDebugPrint(3, ("SkGeInit1:B2_PMD_TYP = %c\n", Byte));
	...
	NbDebugPrint(3, ("SkGeInit1:B2_E_1 = %d\n", Byte));
		...
		if (pAC->GIni.GIYukon) {
			...
			if (pAC->GIni.GICopperType) {
				...
				if (pAC->GIni.GIChipId == CHIP_ID_YUKON_FE ||
					...
					NbDebugPrint(3, ("SkGeInit1:pPrt->PLinkSpeedCap = %08X\n", pPrt->PLinkSpeedCap));
				}
...
SK_IOC	IoC)		/* I/O Context */
{
	...
	NbDebugPrint(3, ("SkGeInit2:pAC->GIni.GIPollTimerVal = %08X\n",  pAC->GIni.GIPollTimerVal));
	...
	if (CHIP_ID_YUKON_2(pAC)) {
		 ...
		 */
		NbDebugPrint(3, ("SkGeInit2:pAC->GIni.GIValHwIrqMask = %08X\n", pAC->GIni.GIValHwIrqMask));
...
int		Port)		/* Port to configure */
{
	...
	NbDebugPrint(3, ("SkGeInitPort Entered\n"));
	...
	if (pPrt->PXSQSize != 0) {
		...
		{
			SK_U8 Byte;
			SK_IN8(IoC, MR_ADDR(Port, TXA_CTRL), &Byte);
			NbDebugPrint(3, ("SkGeInitPort:MR_ADDR(Port, TXA_CTRL) = %08X\n", Byte));
		}
	}
	...
	NbDebugPrint(3, ("SkGeInitPort Exit\n"));
