...
static const char SysKonnectFileId[] =
	"@(#) $Id: skgesirq.c,v 2.23 2005/08/11 14:21:08 rschmidt Exp $ (C) Marvell.";
#endif
...
int		Port)	/* Port Index (MAC_1 + n) */
{
	...
	NbDebugPrint(3, ("SkHWLinkUp: pPrt->PLinkMode = %d\n", pPrt->PLinkMode));
#ifndef SK_SLIM
...
SK_U32	Istatus)	/* Interrupt status word */
{
	...
	if ((Istatus & IS_EXT_REG) != 0) {
		...
		for (i = 0; i < pAC->GIni.GIMacsFound; i++) {
			...
			if (pAC->GIni.GIYukon) {
				...
				NbDebugPrint(3, ("SkGeYuSirqIsr: PHY_MARV_INT_STAT = %04X\n", PhyInt));
	...
	if ((Istatus & IS_TIMINT) != 0) {
		...
		if (((Istatus & IS_HW_ERR) & ~pAC->GIni.GIValIrqMask) != 0) {
			/* read the HW Error Interrupt source */
			SK_IN32(IoC, B0_HWE_ISRC, &RegVal32);
			NbDebugPrint(3, ("SkGeYuSirqIsr: B0_HWE_ISRC = %08X\n", RegVal32));
...
SK_BOOL	AutoNeg)	/* Is Auto-negotiation used ? */
{
	...
	if (pAC->GIni.GICopperType) {
		...
		NbDebugPrint(3, ("SkGePortCheckUpGmac: pPrt->PLinkSpeedCap = %08X\n", pPrt->PLinkSpeedCap));
		if ((pPrt->PLinkSpeedCap & SK_LSPEED_CAP_1000MBPS) != 0) {
		...
		NbDebugPrint(3, ("SkGePortCheckUpGmac: pPrt->PCableLen = %08X\n", pPrt->PCableLen));
	}
#endif /* !SK_SLIM */
...
SK_EVPARA	Para)		/* Event specific Parameter */
{
	...
	NbDebugPrint(3, ("SkGeSirqEvent Entered\n"));
	...
	case SK_HWEV_WATIM:
		...
		NbDebugPrint(3, ("SkGeSirqEvent: SK_HWEV_WATIM : pPrt->PState = %d\n", pPrt->PState));
		if (pPrt->PState == SK_PRT_RESET) {
		...
		NbDebugPrint(3, ("SkGeSirqEvent: pPrt->PWaTimer Start\n"));
	...
	case SK_HWEV_PORT_START:
		NbDebugPrint(3, ("SkGeSirqEvent: SK_HWEV_PORT_START\n"));
#ifndef SK_SLIM
		if (pPrt->PHWLinkUp) {
			/*
			 * Signal directly to RLMT to ensure correct
	...
	NbDebugPrint(3, ("SkGeSirqEvent Exit\n"));
