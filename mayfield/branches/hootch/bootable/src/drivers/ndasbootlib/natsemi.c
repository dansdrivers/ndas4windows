...
static char version[]  =  
  DRV_NAME " dp8381x driver, version "
...
__inline ULONG rdl(ULONG Port)
{
	...
	__asm mov	edx,Port		; EDX: Port No
	__asm in	eax,dx			; Input the value.
	__asm mov	Value, eax
...
/*
__inline void writel(ULONG Port, ULONG Value)
{
	__asm push	ax;
	__asm push	dx;

	__asm mov	edx,Port			; EDX: Port No
	__asm mov	eax,Value			; EAX: Value

	__asm out	dx, eax			; Output the value.
...
__inline USHORT readw(ULONG Port)
{
	USHORT Value;
	...
	__asm mov	edx,Port		; EDX: Port No
	__asm in	ax,dx			; Input the value.
	__asm mov	Value, ax
...
__inline void writew(ULONG Port, ULONG Value)
{
	...
	__asm mov	edx,Port			; EDX: Port No
	__asm mov	ax,temp			; EAX: Value

	__asm out	dx, ax			; Output the value.
...
Thus configuration functions that want to access everything must call
	disable_irq(dev->irq);
	KeAcquireSpinLock_bh(dev->xmit_lock);
...
static struct device * natsemi_probe1(int pci_bus, int pci_devfn,
									  struct device *dev, long ioaddr,
									  int irq, int chp_idx, int fnd_cnt);

static struct pci_id_info pci_tbl[] =
{{ "NatSemi DP8381[56]",
   PCI_VENDOR_ID_NS, PCI_DEVICE_ID_NS_83815, 0xffff, PCI_USES_IO|PCI_USES_MASTER, 0x80, natsemi_probe1},
}; 
...
static int natsemi_scan(struct device *dev, struct pci_id_info pci_tbl[]);
static int eeprom_read(long ioaddr, int location);
static int mdio_read(struct device *dev, int phy_id, int reg);
static void mdio_write(struct device *dev, int phy_id, int reg, u16 data);
static void natsemi_reset(struct device *dev);
static void natsemi_reload_eeprom(struct device *dev);
static void natsemi_stop_rxtx(struct device *dev);
static int netdev_open(struct device *dev);
static void do_cable_magic(struct device *dev);
static void undo_cable_magic(struct device *dev);
static void check_link(struct device *dev);
static void netdev_timer(unsigned long data);
static void dump_ring(struct device *dev);
static void tx_timeout(struct device *dev);
static int alloc_ring(struct device *dev);
static void refill_rx(struct device *dev);
static void init_ring(struct device *dev);
static void drain_tx(struct device *dev);
static void drain_ring(struct device *dev);
static void free_ring(struct device *dev);
static void reinit_ring(struct device *dev);
...
static void netdev_rx(struct device *dev);
static void netdev_tx_done(struct device *dev);
static void __set_rx_mode(struct device *dev);
static void set_rx_mode(struct device *dev);
static void __get_stats(struct device *dev);
static int get_status(struct device *dev);
static struct net_device_stats *get_stats(struct device *dev);
static int netdev_ioctl(struct device *dev, struct ifreq *rq, int cmd);
static int netdev_set_wol(struct device *dev, u32 newval);
static int netdev_get_wol(struct device *dev, u32 *supported, u32 *cur);
static int netdev_set_sopass(struct device *dev, u8 *newval);
static int netdev_get_sopass(struct device *dev, u8 *data);
static int netdev_get_ecmd(struct device *dev, struct ethtool_cmd *ecmd);
static int netdev_set_ecmd(struct device *dev, struct ethtool_cmd *ecmd);
static void enable_wol_mode(struct device *dev, int enable_intr);
static int netdev_close(struct device *dev);
static int netdev_get_regs(struct device *dev, u8 *buf);
static int netdev_get_eeprom(struct device *dev, u8 *buf);

int link_natsemi = 0;

KDPC TimerDpc_NatSemi;
KTIMER Timer_NatSemi;

KDPC InterruptTimerDpc_NatSemi;
KTIMER InterruptTimer_NatSemi;

int natsemi_probe(struct device *dev)
{
...
//		return -ENODEV;
	...
	return natsemi_scan(dev, pci_tbl);
}
...
//	DEBUGCALLXY(1, 16, "votex_scan start ", 7);
	NbDebugPrint(0, ("natsemi_scan start\n"));
	...
	if (pcibios_present()) {
		...
		for (;pci_index < 0xff; pci_index++) {
			...
			NbDebugPrint(0, ("Find Network ethernet class. pci_index = %02X, pci_bus = %04X, pci_device_fn = %04X\n", pci_index, pci_bus, pci_device_fn));
			...
			NbDebugPrint(0, ("pci_vendor_id = %04X, pci_device_id = %04X\n", vendor, device));

			for (chip_idx = 0; pci_tbl[chip_idx].vendor_id; chip_idx++) {
				NbDebugPrint(0, ("chip_idx = %04X, pci_tbl[chip_idx].vendor_id = %04X\n", chip_idx, pci_tbl[chip_idx].vendor_id));
...
activate:
			...
			NbDebugPrint(0, ("pci_command = %04X\n", pci_command));
			new_command = pci_command | PCI_COMMAND_MASTER|PCI_COMMAND_IO;
			NbDebugPrint(0, ("new_command = %04X\n", new_command));

			if (pci_command != new_command) {
				NbDebugPrint(0, ("  The PCI BIOS has not enabled the device "
			...
			NbDebugPrint(0, ("pwr_cmd = %04X\n", pwr_cmd));

/*
			if (pwr_cmd & 0x3) {

				NbDebugPrint(0, ("  A 3Com network adapter is powered down!"
				...
				NbDebugPrint(0, ("  Setting the IRQ to %d, IOADDR to %#lx.\n",
					   irq, ioaddr));
...
*/
			...
			NbDebugPrint(0, ("before HalTranslateBusAddress ioaddr = %08X\n", ioaddr));
			{
				PHYSICAL_ADDRESS IoAddress, cardAddress;
				ULONG	addressSpace;
				BOOLEAN	b;

				IoAddress.QuadPart = ioaddr;
				...
				NbDebugPrint(0, ("HalTranslateBusAddress: b = %d, addressSpace = %d, cardAddress = %I64X\n",
					b, addressSpace, cardAddress.QuadPart));


				if(b)	ioaddr = (ULONG)cardAddress.QuadPart;
			}

			NbDebugPrint(0, ("after HalTranslateBusAddress ioaddr = %08X\n", ioaddr));
#endif

			dev = natsemi_probe1(pci_bus, pci_device_fn, dev, ioaddr, irq,
								chip_idx, cards_found);

			NbDebugPrint(0, ("natsemi_probe1 device = %p\n", dev));
			...
			if (dev) {
				...
				if (pci_latency < new_latency) {					
					NbDebugPrint(0, ("%s: Overriding PCI latency"
...
/*
				pcibios_read_config_dword(pci_bus, pci_device_fn, PCIPM, &tmp);
				if (tmp & PCI_PM_CTRL_STATE_MASK) {
						u32 newtmp = tmp & ~PCI_PM_CTRL_STATE_MASK;
						pcibios_write_config_dword(pci_bus, pci_device_fn, PCIPM, newtmp);
				}		
*/
				dev = 0;
				cards_found++;
			...
			NbDebugPrint(0, ("natsemi_probe1: card_found = %d\n", cards_found));
		}
	}
	...
	/*
	if (compaq_ioaddr) {
		natsemi_probe1(0, 0, dev, compaq_ioaddr, compaq_irq,
					  compaq_device_id, cards_found++);
		dev = 0;
	}
	*/

	NbDebugPrint(0, ("natsemi_scan end, cards_found = %d\n", cards_found));
...
//	if (i) return i;
	...
	{
		void *mem = kmalloc(sizeof(*np) + 15, GFP_KERNEL);
		np =  (void *)(((long)mem + 15) & ~15);
		memset(np, 0, sizeof(*np));
		np->priv_addr = mem;
	}	
	dev->priv = np;
 
	NbDebugPrint(0, ("dev->priv = %p\n", dev->priv));
	...
	NbDebugPrint(0, ("dev->dev_addr = %02X:%02X:%02X:%02X:%02X:%02X\n", 
		dev->dev_addr/*[0]*/,
		dev->dev_addr[1],
		dev->dev_addr[2],
		dev->dev_addr[3],
...
//	np->iosize = iosize;
	KeInitializeSpinLock(&np->lock);
	np->msg_enable = (debug >= 0) ? (1<<debug)-1 : NATSEMI_DEF_MSG;
	np->msg_enable = np->msg_enable | NETIF_MSG_PROBE | NETIF_MSG_PKTDATA | NETIF_MSG_INTR | NETIF_MSG_HW | NETIF_MSG_RX_STATUS | NETIF_MSG_TX_DONE;
	...
	option = fnd_cnt < MAX_UNITS ? options[fnd_cnt] : 0;
	if (dev->mem_start)
	...
	if (fnd_cnt < MAX_UNITS  &&  full_duplex[fnd_cnt])
		np->full_duplex = 1;
...
*/
	...
	if (netif_msg_drv(np)) {
		NbDebugPrint(0, ("%s: %s at %#08lx, ",
			dev->name, pci_tbl[chp_idx].name, ioaddr));
		for (i = 0; i < ETH_ALEN-1; i++)
				NbDebugPrint(1, ("%02x:", dev->dev_addr[i]));
		NbDebugPrint(0, ("%02x, IRQ %d.\n", dev->dev_addr[i], irq));
	}

	np->advertising = (USHORT) mdio_read(dev, 1, MII_ADVERTISE);
	if ((readl(ioaddr + ChipConfig) & 0xe000) != 0xe000
	 && netif_msg_probe(np)) {
		u32 chip_config = readl(ioaddr + ChipConfig);
		NbDebugPrint(0, ("%s: Transceiver default autonegotiation %s "
			"10%s %s duplex.\n",
			dev->name,
	...
	if (netif_msg_hw(np))
		NbDebugPrint(0, ("%s: silicon revision %#04x.\n",
				dev->name, np->srr));
...
/* Delay between EEPROM clock transitions.
   ...
   depricated.
*/
__inline void eeprom_delay(long ee_addr) 
{
...
static int eeprom_read(long addr, int location)
{
	...
	Addr.QuadPart = addr;
	temp = MmMapIoSpace(Addr, 0x100, FALSE);

	NbDebugPrint(0, ("eeprom_read: addr = %08X, MmMapIoSpace = %08X\n", addr, temp));
	for(i=sizeof(ULONG);i<=0x100;i+= sizeof(ULONG)) {
		NbDebugPrint(0, ("%08X ",*temp++));		
		if( i % (sizeof(ULONG)*16 ) == 0 ) NbDebugPrint(0, ("\n"));
	}

	for(i=sizeof(ULONG);i<=0x100;i+= sizeof(ULONG)) {
		NbDebugPrint(0, ("%08X ",(ULONG) ScsiPortReadPortUlong((PULONG)(addr + i))));		
		if( i % (sizeof(ULONG)*16 ) == 0 ) NbDebugPrint(0, ("\n"));
	}

	for(i=sizeof(ULONG);i<=0x100;i+= sizeof(ULONG)) {
		NbDebugPrint(0, ("%08X ",(ULONG) rdl(addr + i)));		
		if( i % (sizeof(ULONG)*16 ) == 0 ) NbDebugPrint(0, ("\n"));
	}
#endif
...
static void natsemi_reset(struct device *dev)
{
	...
	u16 sopass[3];
	struct netdev_private *np = dev->priv;
	LARGE_INTEGER deltaTime_NatSemi;

	NbDebugPrint(0, ("natsemi_reset entered\n"));
	...
	NbDebugPrint(0, ("natsemi_reset chip reset\n"));
	/* now whack the chip */
	...
	for (i=0;i<NATSEMI_HW_TIMEOUT;i++) {
		...
		deltaTime_NatSemi.QuadPart = - 1;
		KeDelayExecutionThread(KernelMode, FALSE, &deltaTime_NatSemi);
//		KeDelayExecution(50);
	}
	if (i==NATSEMI_HW_TIMEOUT) {
		NbDebugPrint(0, ("%s: reset did not complete in %d usec.\n",
			dev->name, i*5));
	} else if (netif_msg_hw(np)) {
		NbDebugPrint(1, ("%s: reset completed in %d usec.\n",
			dev->name, i*5));
	}
	...
	NbDebugPrint(0, ("natsemi_reset exit\n"));
}
...
//		KeDelayExecution(50);
	...
	if (i==NATSEMI_HW_TIMEOUT) {
		NbDebugPrint(0, ("%s: EEPROM did not reload in %d usec.\n",
			dev->name, i*50));
	} else if (netif_msg_hw(np)) {
		NbDebugPrint(0, ("%s: EEPROM reloaded in %d usec.\n",
			dev->name, i*50));
	}
}
...
//		KeDelayExecution(50);
	...
	if (i==NATSEMI_HW_TIMEOUT) {
		NbDebugPrint(0, ("%s: Tx/Rx process did not stop in %d usec.\n",
			dev->name, i*5));
	} else if (netif_msg_hw(np)) {
		NbDebugPrint(1, ("%s: Tx/Rx process stopped in %d usec.\n",
			dev->name, i*5));
	}
}
...
static int netdev_open(struct device *dev)
{
	...
	if (netif_msg_ifup(np))
		NbDebugPrint(1, ("%s: netdev_open() irq %d.\n",
			dev->name, dev->irq));
	...
	if (netif_msg_ifup(np))
		NbDebugPrint(1, ("%s: Done netdev_open(), status: %#08x.\n",
...
//	KeInitializeDpc( &TimerDpc_NatSemi, rtl8129_timer, dev );
//	KeInitializeTimer( &Timer_NatSemi );
//	deltaTime_NatSemi.QuadPart = - (24 * HZ) / 10;    /* 2.4 sec. */
//  (VOID) KeSetTimer( &Timer_NatSemi, deltaTime_NatSemi, &TimerDpc_NatSemi );

	KeInitializeDpc( &InterruptTimerDpc_NatSemi, intr_handler, dev );
	KeInitializeTimer( &InterruptTimer_NatSemi );

	deltaTime_NatSemi.QuadPart = - INTERRUPT_TIME;    /* 2.4 sec. */
	KeSetTimer( &InterruptTimer_NatSemi, deltaTime_NatSemi, &InterruptTimerDpc_NatSemi );

//	deltaTime.QuadPart = 0;    
//  KeSetTimerEx( &InterruptTimer_NatSemi, deltaTime_NatSemi, INTERRUPT_TIME, &InterruptTimerDpc_NatSemi );	
...
//		KeDelayExecution(50);
	...
	*/
	...
	if (np->SavedClkRun & PMEStatus && netif_msg_wol(np)) {
		NbDebugPrint(2, ("%s: Wake-up event %#08x\n",
...
static void netdev_timer(unsigned long data)
{
	...
	if (netif_msg_timer(np)) {
		 ...
		 */
		NbDebugPrint(1, ("%s: Media selection timer tick.\n",
			dev->name));
	}

//	KeAcquireSpinLock(&np->lock, &Irql);
...
//			KeReleaseSpinLock(&np->lock, Irql);
			if (netif_msg_hw(np))
				NbDebugPrint(2, ("%s: possible phy reset: "
					"re-initializing\n", dev->name));
			disable_irq(dev->irq);
//			KeAcquireSpinLock(&np->lock, &Irql);
...
//		KeReleaseSpinLock(&np->lock, Irql);
	...
	deltaTime_NatSemi.QuadPart = - next_tick;
	KeSetTimer( &Timer_NatSemi, deltaTime_NatSemi, &TimerDpc_NatSemi );
}
...
static void dump_ring(struct device *dev)
{
	...
	if (netif_msg_pktdata(np)) {
		int i;
		NbDebugPrint(1, ("  Tx ring at %p:\n", np->tx_ring));
		for (i = 0; i < TX_RING_SIZE; i++) {
			NbDebugPrint(1, (" #%d desc. %#08x %#08x %#08x.\n",
				i, np->tx_ring[i].next_desc,
				np->tx_ring[i].cmd_status,
				np->tx_ring[i].addr));
		}
		NbDebugPrint(1, ("  Rx ring %p:\n", np->rx_ring));
...
static int start_tx(struct sk_buff *skb, struct device *dev)
{
	...
	if(STATUS_NIC_OK != get_status(dev)) {		
		return 1;
	}
...
#if 0
	{
		...
		extern char order_list[100];

		if(order_count < 100) order_list[order_count++] = 'T';		
	}
#endif

	if (netif_msg_tx_queued(np)) {
		NbDebugPrint(1, ("%s: Transmit frame #%d queued in slot %d.\n",
...
{
	struct device *dev = DeferredContext;
	struct netdev_private *np = dev->priv;
	long ioaddr = dev->base_addr;
	...
	NbDebugPrint(2, ("intr_handler entered\n"));
#ifndef __INTERRUPT__
	KeCancelTimer(&InterruptTimer_NatSemi);	
#endif
...
#if 0
	{
		...
		if(order_count >= 2 && order_count <= 5) 
			if(order_count < 100) order_list[order_count++] = 'I';		
	}
#endif
...
#ifndef __INTERRUPT__
	deltaTime_NatSemi.QuadPart = - INTERRUPT_TIME;    
	KeSetTimer( &InterruptTimer_NatSemi, deltaTime_NatSemi, &InterruptTimerDpc_NatSemi );	
#endif

	if (netif_msg_intr(np))
		NbDebugPrint(1, ("%s: exiting interrupt.\n", dev->name));
}
...
static void netdev_error(struct device *dev, int intr_status)
{
	...
	if (intr_status & LinkChange) {
		u16 adv = (u16)mdio_read(dev, 1, MII_ADVERTISE);
		u16 lpa = (u16)mdio_read(dev, 1, MII_LPA);
	...
	if (intr_status & WOLPkt && netif_msg_wol(np)) {
		...
		NbDebugPrint(2, ("%s: Link wake-up event %#08x\n",
			dev->name, wol_status));
	}
	if (intr_status & RxStatusFIFOOver) {
		if (netif_msg_rx_err(np) && netif_msg_intr(np)) {
			NbDebugPrint(2, ("%s: Rx status FIFO overrun\n",
				dev->name));
		}
	...
	if (intr_status & IntrPCIErr) {
		NbDebugPrint(2, ("%s: PCI error %#08x\n", dev->name,
...
static void __set_rx_mode(struct device *dev)
{
	...
	if (dev->flags & IFF_PROMISC) { /* Set promiscuous. */
		...
		NbDebugPrint(2, ("%s: Promiscuous mode enabled.\n",
			dev->name));
...
/*	case ETHTOOL_GSET: {
		...
		KeReleaseSpinLockFromDpcLevel_irq(&np->lock);
...
static void enable_wol_mode(struct device *dev, int enable_intr)
{
	...
	if (netif_msg_wol(np))
		NbDebugPrint(3, ("%s: remaining active for wake-on-lan\n",
			dev->name));
...
static void natsemi_remove1 (struct pci_dev *dev)
{	
//	unregister_netdev (dev);
...
static int natsemi_suspend (struct pci_dev *dev, u32 state)
{
	struct netdev_private *np = dev->priv;
	long ioaddr = dev->base_addr;
	KIRQL Irql;
...
static int natsemi_resume (struct pci_dev *dev)
{	
	struct netdev_private *np = dev->priv;
	KIRQL Irql;
