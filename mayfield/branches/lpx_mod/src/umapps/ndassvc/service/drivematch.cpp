...
BOOL 
WINAPI
GetDiskNumbersForDrvLtr(int drvno, int diskno[], int disknobuff_cnt, int *diskcnt) {
	BOOL	bret = FALSE ;
	HANDLE	volMgr ;
	ULONG	retbytes ;
	TCHAR	volName[] = _T("\\\\.\\A:") ;
...
BOOL 
WINAPI
GetDiskNumbersForHVol(HANDLE volMgr, int diskno[], int disknobuff_cnt, int *diskcnt) {
	BOOL	bret = FALSE ;
	ULONG	retbytes ;
	UINT	i ;
	PVOLUME_DISK_EXTENTS	pvolExts ;
		...
		int *scsiportno_cnt) 
{
	...
	*scsiportno_cnt = 0 ;
	...
	::ZeroMemory(&strgQuery, sizeof(STORAGE_PROPERTY_QUERY));
	...
	pstrgAdtDesc = (PSTORAGE_ADAPTER_DESCRIPTOR)
		HeapAlloc(GetProcessHeap(), 0, buffSz) ;

	if(NULL == pstrgAdtDesc) {
		DPErrorEx(_FT("Memory allocation failed: "));
		return FALSE ;
	}

	// attach to auto heap resource
	AutoProcessHeap pHeap = pstrgAdtDesc;
	...
	for(i = 0 ; i < diskno_cnt ; i ++ ) {
		...
		//
		AutoFileHandle hDevice = CreateFile(phyDrvName,
						GENERIC_READ |  GENERIC_WRITE,
						FILE_SHARE_READ | FILE_SHARE_WRITE,
						NULL,
						OPEN_EXISTING,
		...
		//
		if(BusTypeScsi == pstrgAdtDesc->BusType) {
...
BOOL 
WINAPI
GetNetDisksForScsiports(
		IN int scsiportno[],
		IN int scsiportno_cnt,
		OUT DRIVEMATCH_UNITDISK_INFO unitdisks[],
		OUT int slotno_sz,
		OUT int *slotno_cnt)
{
	BOOL fSuccess = FALSE;
	...
	*slotno_cnt = 0 ;
	...
	for(i = 0 ; i < scsiportno_cnt ; i ++ ) {
		...
		//
		AutoFileHandle hScsiPort = ::CreateFile(
			scsiPortName,
			GENERIC_READ |  GENERIC_WRITE,
			FILE_SHARE_READ | FILE_SHARE_WRITE,
			NULL,
		...
		outbuff_sz = sizeof(SRB_IO_CONTROL) + 
			sizeof(LSMPIOCTL_QUERYINFO) + 
			sizeof(LSMPIOCTL_PRIMUNITDISKINFO) ;
		...
		AutoProcessHeap autoHeap = psrbioctl;
		...
		if(!fSuccess || psrbioctl->ReturnCode != 0) {
		...
		if(slotno_sz > (LONG)PrimUnitDisk->UnitDisk.SlotNo)  {
			...
			::CopyMemory(
				unitdisks[*slotno_cnt].MacAddress, 
				&PrimUnitDisk->UnitDisk.NetDiskAddress.Address[0].Address, 
				LPXADDR_NODE_LENGTH) ;
			(*slotno_cnt) ++ ;
		}
...
BOOL 
WINAPI
GetNetDiskFromDrvLtr(int drvno, DRIVEMATCH_UNITDISK_INFO unitdisks[], int unitdisks_sz, int *unitdisks_cnt) {
	BOOL	bret = FALSE ;
	int		diskno[256] ;
	int		diskcnt = 0 ;
	int		scsiportno[256] ;
...
BOOL 
WINAPI
GetNetDiskFromHVol(HANDLE hVol, DRIVEMATCH_UNITDISK_INFO unitdisks[], int unitdisks_sz, int *unitdisks_cnt) {
	BOOL	bret = FALSE ;
	int		diskno[256] ;
	int		diskcnt = 0 ;
	int		scsiportno[256] ;
...
BOOL 
WINAPI
GetVolNumForDrvNum(int drvno, ULONG *vol) {
	BOOL	bret = FALSE ;
	HANDLE	volMgr ;
	TCHAR	volName[] = _T("\\\\.\\A:") ;
	VOLUME_NUMBER VN ;
...
BOOL 
WINAPI
Handle2LogDrvNo(HANDLE	hFile, int *drvno) {
	BOOL	bret ;
	ULONG	MaxDriveSet ;
	int		i ;
	ULONG volno ;
...
NdasDmIsLANSCSIPortInterface(LPCWSTR wszDbccName)
{
	...
	for (DWORD i = 0; i < LANSCSI_DEV_IFIDW_LEN - 1; ++i, ++pwch) {
