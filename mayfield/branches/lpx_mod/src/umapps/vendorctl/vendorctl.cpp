...
ConvertNDIDStringToChar(
	...
	for(idIdx = 0; idIdx < 20; idIdx++) {
		...
		if( wctomb(Mbcs, NetDiskIDStr[idIdx]) != 1) {
			_ftprintf(stderr, _T("ERROR: NetDisk ID has invalid charaters.\n"));
			return -1;
		}
	...
	for(idIdx = 0; idIdx < 5; idIdx++) {
		...
		if( wctomb(Mbcs, WriteKeyStr[idIdx]) != 1) {
			_ftprintf(stderr, _T("ERROR: Write key has invalid charaters.\n"));
			return -1;
		}
...
VerifyNDID(
	...
	//	set encryption keys.
	RtlCopyMemory(KeyInfo->key1,NDIDV1Key1,8);
	RtlCopyMemory(KeyInfo->key2,NDIDV1Key2,8);
...
CpSetDeviceStandby(int argc, _TCHAR* argv[])
{
	...
	if(keyInfo.writable == FALSE) {
		_ftprintf(stderr, _T("ERROR: Correct write key needed.\n"));
		return -1;
	}
	...
	RtlCopyMemory(address.Node, keyInfo.address, LPXADDR_NODE_LENGTH);
...
void usage()
{
	...
	_tprintf(_T("<WriteKey>   : 5 charaters\n"));
}
