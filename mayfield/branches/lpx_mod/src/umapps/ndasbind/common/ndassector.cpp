...
CDIBSector::CDIBSector(const PNDAS_UNIT_DEVICE_INFO pUnitDeviceInfo)
...
// CDIBV2Sector
///////////////////////////////////////////////////////////////////////////////
CDIBV2Sector::CDIBV2Sector(const PNDAS_UNIT_DEVICE_INFO pUnitDeviceInfo)
...
: CTypedDiskInfoSector<NDAS_DIB_V2>((PNDAS_UNIT_DEVICE_INFO)&pDIBSector->m_UnitDeviceInfo),
  m_nCount(1), m_dataExpanded()
{
	NDAS_DIB_V2 *pDIBV2 = 
		CTypedDiskInfoSector<NDAS_DIB_V2>::get();
...
BOOL CDIBV2Sector::IsValid() const
{
	...
	if ( pDIB->iSequence >= pDIB->nDiskCount )
	{
		return FALSE;
	}
...
void CDIBV2Sector::ReadAccept(CSession *pSession)
{
	...
	pSession->Read( NDAS_BLOCK_LOCATION_DIB_V2, 1, m_pdata );	// NOTE : m_data is a member variable of CTypedDiskSector..
	...
	if ( m_nCount < static_cast<_int16>(CalcRequiredSectorCount(get())) )
	{
		...
		if ( m_nCount > 1 )
		{
			m_dataExpanded = boost::shared_ptr<_int8>( new _int8[m_nCount*NDAS_BLOCK_SIZE] );
			::CopyMemory( m_dataExpanded.get(), m_pdata, NDAS_BLOCK_SIZE );
			m_pdata = m_dataExpanded.get();
		}
	}
...
BOOL CDIBV2Sector::IsMirrored() const
{
	const NDAS_DIB_V2 *pDIB = 
		CTypedDiskInfoSector<NDAS_DIB_V2>::get();
	return ( pDIB->iMediaType == NMT_RAID1 );
}
...
void CDIBV2Sector::Bind(CUnitDiskObjectVector bindDisks, UINT nIndex, int nBindType)
{
	...
	// initialize pDIB	
	pDIB->sizeXArea			= NDAS_SIZE_XAREA / NDAS_BLOCK_SIZE; // 2MB
...
//	ATLASSERT(nSectorsPerBit != 0);
	nSize = nTotalSectorCount- NDAS_SIZE_XAREA/NDAS_BLOCK_SIZE;	// Subtracts area reserved
