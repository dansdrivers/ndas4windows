...
void CUnBindDlg::OnOK(UINT /*wNotifyCode*/, int /*wID*/, HWND /*hwndCtl*/)
{
	...
	// warning message
	{
		strMsg.LoadString(IDS_WARNING_UNBIND);
		int id = MessageBox(
			strMsg,
			strTitle,
	...
	pConnectionInfo = new NDAS_CONNECTION_INFO[nDiskCount];
	ZeroMemory(pConnectionInfo, sizeof(NDAS_CONNECTION_INFO) * nDiskCount);
	...
	if(i == BindResult)
	{
		strMsg.LoadString(IDS_WARNING_UNBIND_AFTER);
		MessageBox(
			strMsg,
			strTitle,
