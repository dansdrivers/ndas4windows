...
//		MESSAGE_HANDLER(m_iTaskbarRestartMsg, OnRestart)
	...
	LRESULT OnContextMenu(LPARAM /*uMsg*/, BOOL& bHandled)
	{
		...
		//
		// Disable multiple active popups
		//
		HWND hWndPopup = ::GetLastActivePopup(m_nid.hWnd);
		if (hWndPopup != m_nid.hWnd) {
			::FlashWindow(hWndPopup, TRUE);
			::SwitchToThisWindow(hWndPopup, FALSE);
...
#define NIN_BALLOONUSERCLICK (WM_USER + 5)
	...
	{
		CTaskBarIconExT<T>* pThis = reinterpret_cast<CTaskBarIconExT<T>* >(this);
		...
		case NIN_BALLOONTIMEOUT: 
			pThis->OnBalloonTimeout();
			break;
		case NIN_BALLOONUSERCLICK: 
			pThis->OnBalloonUserClick();
			break;
		case NIN_SELECT:
			//pThis->OnTaskBarSelect();
			ATLTRACE(_T("NIN_SELECT\n"));
			break;
		case NIN_KEYSELECT:
			//pThis->OnTaskBarKeySelect();
