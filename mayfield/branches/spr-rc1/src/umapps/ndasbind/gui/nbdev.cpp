...
BOOL CNBNdasDevice::UnitDeviceAdd(PNDASUSER_UNITDEVICE_ENUM_ENTRY pBaseInfo)
{
	...
	// already exist
	if(m_mapUnitDevices[pBaseInfo->UnitNo])
	{
...
UINT CNBUnitDevice::GetIconIndex()
{
	return 1;
}

UINT CNBUnitDevice::GetSelectIconIndex()
{
	return 1;
}
...
CNBLogicalDevice::CNBLogicalDevice(UINT32 nIndex):m_Index(nIndex)
{
}
...
WTL::CString CNBLogicalDevice::GetName()
{
	...
	{
		ATLASSERT(1 == m_mapUnitDevices.size() && m_mapUnitDevices[0]);
...
UINT CNBLogicalDevice::GetIconIndex()
{
	ATLASSERT(m_mapUnitDevices.size());
...
UINT CNBLogicalDevice::GetSelectIconIndex()
{
	ATLASSERT(m_mapUnitDevices.size());
...
BOOL CNBLogicalDevice::IsMember(CNBUnitDevice *pUnitDevice)
{
	...
	if (memcmp(DIB()->UnitDisks, 
		pUnitDevice->m_DIB.UnitDisks, 
		DIB()->nDiskCount * sizeof(UNIT_DISK_LOCATION)))
		return FALSE;
...
BOOL CNBLogicalDevice::UnitDeviceAdd(CNBUnitDevice *pUnitDevice)
{
	if(pUnitDevice->IsGroup())
	{
		...
		m_mapUnitDevices[pUnitDevice->m_DIB.iSequence] = pUnitDevice;
	}
	else
	{
...
WTL::CString CNBLogicalDevice::GetStatusString()
{
	...
	if(NDASBIND_LOGICAL_DEVICE_STATUS_NO_WRITE_KEY & status)
	{
		strText.LoadString(IDS_STATUS_READ_ONLY);
	}
...
BOOL CNBLogicalDevice::IsOperatableAll()
{
	for(UINT32 i = 0; i < DevicesTotal(); i++)
	{
		if(!m_mapUnitDevices[i] || !m_mapUnitDevices[i]->IsOperatable())
			return FALSE;
	}
