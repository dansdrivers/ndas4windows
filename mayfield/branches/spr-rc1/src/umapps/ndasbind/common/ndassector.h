...
{
protected:
	NDASCOMM_UNIT_DEVICE_INFO m_UnitDeviceInfo;
public:
	CDiskInfoSector(const PNDASCOMM_UNIT_DEVICE_INFO pUnitDeviceInfo)
	{
		if ( pUnitDeviceInfo != NULL )
			::CopyMemory(&m_UnitDeviceInfo, pUnitDeviceInfo, sizeof(NDASCOMM_UNIT_DEVICE_INFO) );
		else
			::ZeroMemory(&m_UnitDeviceInfo, sizeof(NDASCOMM_UNIT_DEVICE_INFO) );
	}
...
{
public:
	CTypedDiskInfoSector(const PNDASCOMM_UNIT_DEVICE_INFO pUnitDeviceInfo)
...
public:
	...
	CDIBSector(const PNDASCOMM_UNIT_DEVICE_INFO pUnitDeviceInfo);
...
public:
	CDIBV2Sector(const PNDASCOMM_UNIT_DEVICE_INFO pUnitDeviceInfo);
	...
	virtual BOOL GetLastWrittenSectorsInfo(PLAST_WRITTEN_SECTORS pLWS) const;
	virtual UINT32 GetNDASMediaType() const;
