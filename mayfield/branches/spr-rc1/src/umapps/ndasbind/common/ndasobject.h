...
public:
	...
	// Get the number of hosts connected
	virtual BOOL GetUserCount(int *nRW, int *nRO) const = 0;
	...
	// Returns true if disk is not operatable(Check fault tolerance).
	virtual BOOL IsBroken() const = 0;
	// Returns true if one or more disks are missing.
	virtual UINT GetMissingMemberCount() const = 0;
...
public:
	...
	const CUnitDiskObjectPtr GetAvailableUnitDisk() const;
	...
	virtual BOOL GetUserCount(int *nRW, int *nRO) const;
	...
	virtual UINT GetMissingMemberCount() const;

	void HIXChangeNotify(CNdasHIXChangeNotify *hix);

	// RMD base functions
	void RMDLoad();
	int RMDStatus();
	UINT8 RMDUnitStatus(int idx);
	UINT8 RMDUnitStatus(const UNIT_DISK_LOCATION *location);
};
...
public:
	...
	virtual BOOL GetUserCount(int *nRW, int *nRO) const;
	virtual UINT GetMissingMemberCount() const {return 0;}
};
...
public:
	...
	virtual BOOL GetUserCount(int *nRW, int *nRO) const;
	virtual UINT GetMissingMemberCount() const {return 1;}
};
