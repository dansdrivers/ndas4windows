	...
	*received = 0;
	...
	while (len > 0) {
		...
		if(!NT_SUCCESS(ntStatus)) {
			KDPrintM(DBG_PROTO_ERROR, ("Can't Receive!\n"));
			return ntStatus;
		}
	   ...
	   )
{
	...
	*pTotalSent = 0;
	...
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(DBG_PROTO_ERROR, ("Can't send!\n"));
		return ntStatus;
	}
			...
			)
{
	...
	if(!NT_SUCCESS(ntStatus) 
		|| (result != (sizeof(LANSCSI_H2R_PDU_HEADER) + iAHSLen + iDataSegLen))) {
   ...
   )
{
	...
	case WIN_SETFEATURES:
		...
		break;
	default:		
		KDPrintM(DBG_PROTO_ERROR, ("Not Supported IDE Command.\n"));
		return STATUS_INVALID_PARAMETER;
	}
