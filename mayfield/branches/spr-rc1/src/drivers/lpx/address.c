...

{
...
#ifdef __LPX__
	PDEVICE_CONTEXT	tempDeviceContext;
#endif
    ...
    if (!NbfValidateTdiAddress(name, ea->EaValueLength)) {
		DebugPrint(1, ("[Lpx] NbfOpenAddress: invalid address name\n") );
        return STATUS_INVALID_ADDRESS_COMPONENT;
    }
	...
	//	@hootch@ 09112003
	//
	tempDeviceContext = SocketLpxFindDeviceContext(networkName);
	if(tempDeviceContext == NULL) {
		status = STATUS_INVALID_ADDRESS_COMPONENT;
		DebugPrint(1, ("[Lpx] NbfOpenAddress: couldn't find tempDeviceContext\n") );
		return	status;
	}

	DebugPrint(2, ("[Lpx] NbfOpenAddress: DeviceContext is changed into tempDeviceContext\n") );

	ASSERT( SocketLpxDeviceContext == DeviceContext) ;
    ...
    if (!NT_SUCCESS (status)) {
		...
		DebugPrint(1, ("[Lpx] NbfOpenAddress: NbfCreateAddressFile error.\n") );
        return status;
    }
	...
	if(networkName->LpxAddress.Port == 0) {
        ...
        //NbfDereferenceAddress ("lookup", address, AREF_LOOKUP);

        NbfDereferenceAddressFile (addressFile);

		DebugPrint(3, ("[Lpx] NbfOpenAddress: another process has taken specified address!\n") );

		return status ;
		}
	}
    address = LpxLookupAddress (DeviceContext, &networkName->LpxAddress);
	...
	if(address != NULL) {
		...
		DebugPrint(3, ("[Lpx] !!!!!NbfOpenAddress %p: another process has taken specified address!\n", address) );
...
/*
					if((NumberOfAddress %10) == 0)
						DebugPrint(DebugLevel,(" NumberOfAddress %ld\n", NumberOfAddress));
					InterlockedIncrement(&NumberOfAddress);
*/
                }
...
//	if((NumberOfAddress % 10) == 0)
//		DebugPrint(DebugLevel, ("NbfDestroyAddress %lx:.  NumberOfAddress %ld\n", Address, NumberOfAddress));
	InterlockedDecrement(&NumberOfAddress);
...

{
	...
	DebugPrint(2, ("[Lpx] NbfCreateAddressFile: entered\n") );
...

{
    ...
    if (AddressFile->State == ADDRESSFILE_STATE_CLOSING) {
        RELEASE_SPIN_LOCK (&Address->SpinLock, oldirql);
        IF_NBFDBG (NBF_DEBUG_ADDRESS) {
            DebugPrint(1,  ("NbfStopAddressFile %lx: already closing.\n", AddressFile) );
        }
        return STATUS_SUCCESS;
    }

    IF_NBFDBG (NBF_DEBUG_ADDRESS | NBF_DEBUG_PNP) {
        DebugPrint(2, ("NbfStopAddressFile %lx: closing.\n", AddressFile));
    }
...

{
    ...
    IF_NBFDBG (NBF_DEBUG_ADDRESS | NBF_DEBUG_PNP) {
        DebugPrint(2, ("NbfCloseAddress AF %lx:\n", addressFile) );
    }
