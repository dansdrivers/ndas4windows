...
//	UNICODE_STRING		unicodeDosName;
	...
	{	
		...
		Bus_KdPrint_Def (BUS_DBG_SS_TRACE, 
			("KeGetCurrentIrql() = 0x%x, PASSIVE_LEVEL = 0x%x\n",
...
NTSTATUS
Bus_PlugInDeviceEx (
    PBUSENUM_PLUGIN_HARDWARE_EX    PlugIn,
    ULONG                       PlugInSize,
    PFDO_DEVICE_DATA            FdoData,
...
--*/
{
    ...
    length = (PlugInSize - sizeof (BUSENUM_PLUGIN_HARDWARE_EX))/sizeof(WCHAR);
	...
	if (!NT_SUCCESS (status)) {
		Bus_KdPrint_Cont (FdoData, BUS_DBG_PNP_TRACE, ("Error\n"));
		ExFreePool (pdoData->HardwareIDs);
		pdoData->HardwareIDs = NULL;
		IoDeleteDevice(pdo);
		return status;
