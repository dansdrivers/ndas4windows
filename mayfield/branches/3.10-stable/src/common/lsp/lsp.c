				 ...
				 lsp_pdu_pointers *pdu)
{
	...
	// read lsp_pdu_hdr
	err_trans = trans->recv(context->proc_context, buffer, sizeof(lsp_pdu_hdr), &recvd);
	if(LSP_TRANS_SUCCESS != err_trans)
		return ERROR_T_COMPOSITE(LSP_ERR_FUNC_RECV_PDU, 0, LSP_ERR_TYPE_RECV_PDU, 0);
	...
	if(ahs_len > 0)
	{
		err_trans = trans->recv(context->proc_context, buffer, ahs_len, &recvd);
		if(LSP_TRANS_SUCCESS != err_trans)
			return ERROR_T_COMPOSITE(LSP_ERR_FUNC_RECV_PDU, 1, LSP_ERR_TYPE_RECV_PDU, 0);
	...
	if(dataseg_len > 0)
	{
		err_trans = trans->recv(context->proc_context, buffer, dataseg_len, &recvd);
		if(LSP_TRANS_SUCCESS != err_trans)
			return ERROR_T_COMPOSITE(LSP_ERR_FUNC_RECV_PDU, 3, LSP_ERR_TYPE_RECV_PDU, 0);
...
{
	lsp_error_t              err;
	lsp_uint8                buffer[LSP_MAX_REQUEST_SIZE] = {0};
	lsp_handle_context*      context = (lsp_handle_context*) h;
	lsp_session_data*        session = &context->session;
	lsp_transport_proc*      trans   = &context->proc;
...
retry_with_correct_version:
	...
	*hw_proto_version = LSP_PROTO_VERSION_MAX;
	...
	// 1st login phase
	lsp_memset(buffer, 0x00, LSP_MAX_REQUEST_SIZE);
	pdu_hdr = (lsp_pdu_hdr *)buffer;
	...
	param_secu = (lsp_binparm_security_ptr)(buffer + sizeof(lsp_pdu_hdr));
	...
	if(LSP_ERR_SUCCESS != err)
		return ERROR_T_COMPOSITE(LSP_ERR_FUNC_LOGIN, sub_seq_num, LSP_ERR_TYPE_SEND, 0);

	err = _lsp_recv_pdu(context, buffer, &pdu);
	if(LSP_ERR_SUCCESS != err)
		return ERROR_T_COMPOSITE(LSP_ERR_FUNC_LOGIN, sub_seq_num, LSP_ERR_TYPE_RECEIVE, 0);
	...
	*hw_proto_version = 
	...
	/* phase specific */
	param_nego = (lsp_binparm_negotiation_ptr)(buffer + sizeof(lsp_pdu_hdr));
		   ...
		   lsp_handle h)
{
	...
	{
		session->iSessionPhase = LSP_LOGOUT_PHASE;
		return ERROR_T_COMPOSITE(LSP_ERR_FUNC_LOGOUT, 0, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
	}
				 ...
				 lsp_uint16 *data_out_length)
{
	...
	if(LSP_ERR_RESPONSE_SUCCESS != pdu_hdr->response)
		return ERROR_T_COMPOSITE(LSP_ERR_FUNC_TEXT_COMMAND, 0, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
				 ...
				 lsp_handle h)
{
	...
	if(LSP_ERR_RESPONSE_SUCCESS != pdu_hdr->response)
		return ERROR_T_COMPOSITE(LSP_ERR_FUNC_NOOP, 0, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
