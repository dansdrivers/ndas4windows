			   ...
			   )
{
	...
	// The following line prevents multiple unit device
...
//		pParam->NRTarget = 1;
	...
	for(i = 0; i < pParam->NRTarget; i++) {
		...
		//
		// WORKAROUND:
		// NDAS 2.0 does report RO Host Count incorrectly.
		// There is no known method to estimate it.
		// We adjust it as 0xFF which is declared as unknown.
		// Only RW host count is reliable. 
		//

		pPath->PerTarget[iTargetId].NRROHost = pTarget->NRROHost;
		if (2 == pPath->HWVersion)
		{
			//
			// 0xFF is selected as a maximum value of NRROHost (int8)
			// Any above wrapper should report it as -1 (0xFFFF or 0xFFFFFFFF)
			//
			pPath->PerTarget[iTargetId].NRROHost = 0xFF;
		}
	...
	case WIN_STANDBYNOW1:
		{
			...
			DebugPrint(7, ("[LanScsiOpLib]IdeCommand_V1: WIN_STANDBYNOW1\n"));
		}
		break;
