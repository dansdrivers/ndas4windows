...
CNdasLogicalDeviceManager::Register(CNdasUnitDevice& unitDevice)
{
	...
	if (itr == m_LDGroupMap.end()) {
		...
		if (pDevice->IsAutoRegistered()) 
		{
			...
			BOOL fEncryptedOnly = FALSE;

			fSuccess = _NdasSystemCfg.GetValueEx(
				_T("ndassvc"),
				_T("MountOnReadyForEncryptedOnly"),
				&fEncryptedOnly);
			if (fSuccess && fEncryptedOnly)
			{
				if (0 == ldSequence && 
					NDAS_UNITDEVICE_TYPE_DISK == unitDevice.GetType())
				{
					CNdasUnitDiskDevice& unitDiskDevice = 
						reinterpret_cast<CNdasUnitDiskDevice&>(unitDevice);
