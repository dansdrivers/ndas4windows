	...
	ACCESS_MASK autoRegAccess)
{
	...
	CRefObjPtr<CNdasDevice> pDevice = pRegistrar->Register(deviceId);
	if (NULL == pDevice.p) {
		return FALSE;
	}

	pDevice->SetGrantedAccess(autoRegAccess);
	pDevice->SetAutoRegistered(TRUE);
	...
	HRESULT hr = ::StringCchPrintf(
		...
		_T("NDAS AR%02d - %02X:%02X:%02X:%02X:%02X:%02X"),
		pDevice->GetSlotNo(),
		pDevice->GetDeviceId().Node[0],
		pDevice->GetDeviceId().Node[1],
		pDevice->GetDeviceId().Node[2],
		pDevice->GetDeviceId().Node[3],
		pDevice->GetDeviceId().Node[4],
		pDevice->GetDeviceId().Node[5]);
