...
#define INVALID_NDAS_HOST_COUNT(x) (NDAS_HOST_COUNT_UNKNOWN == x)
	...
	BOOL bUpdate)
{
	...
	//
	// Workaround for 2.0 bug for invalid RO count
	// nROHosts or nRWHosts will be -1 for invalid values
	//
	...
	//
	if (!INVALID_NDAS_HOST_COUNT(nRWHosts) && 
		!INVALID_NDAS_HOST_COUNT(nROHosts) &&
		(0 == nRWHosts || 0 == nROHosts))
	{
		*lpnROHosts = nROHosts;
		*lpnRWHosts = nRWHosts;
		return TRUE;
	...
	DWORD nHosts = 
		INVALID_NDAS_HOST_COUNT(nRWHosts) ||
		INVALID_NDAS_HOST_COUNT(nROHosts) ?	(DWORD)(-1) : nROHosts + nRWHosts;
	...
	//
	// Invalid RW Host Count is not handled!!
	//
	if (INVALID_NDAS_HOST_COUNT(nROHosts))
	{
		nROHosts = nRepliedHosts - nRWHosts;
	}
