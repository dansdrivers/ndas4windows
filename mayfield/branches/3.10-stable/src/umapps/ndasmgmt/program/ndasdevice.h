...
CHardwarePage::OnInitDialog(HWND hwndFocus, LPARAM lParam)
{
	...
	const NDAS_DEVICE_HW_INFORMATION* pHWI = m_pDevice->GetHwInfo();
	if (NULL != pHWI && pHWI->MACAddress.IsSet)
	{
		// Hardware Address
		strType = _T("MAC Address");
		strValue.Format(_T("%02X:%02X:%02X:%02X:%02X:%02X"),
			pHWI->MACAddress.Node[0],
			pHWI->MACAddress.Node[1],
			pHWI->MACAddress.Node[2],
			pHWI->MACAddress.Node[3],
			pHWI->MACAddress.Node[4],
			pHWI->MACAddress.Node[5]);
		m_propList.AddItem(PropCreateSimple(strType, strValue));
	}

	const NDAS_DEVICE_PARAMS* pDevParam = m_pDevice->GetDeviceParams();
	if (NULL != pDevParam)
	{
		if (pDevParam->RegFlags)
		{
			// Hardware Address
			strType = _T("Flags");
			strValue = _T("");
			if (pDevParam->RegFlags & NDAS_DEVICE_REG_FLAG_AUTO_REGISTERED)
			{
				strValue += _T("AR ");
			}
			if (pDevParam->RegFlags & NDAS_DEVICE_REG_FLAG_HIDDEN)
			{
				strValue += _T("HI ");
			}
			if (pDevParam->RegFlags & NDAS_DEVICE_REG_FLAG_VOLATILE)
			{
				strValue += _T("VO ");
			}
			strValue.TrimRight();
			DWORD dwKnownFlags = NDAS_DEVICE_REG_FLAG_AUTO_REGISTERED | 
				NDAS_DEVICE_REG_FLAG_HIDDEN | 
				NDAS_DEVICE_REG_FLAG_VOLATILE;
			if (pDevParam->RegFlags & (~dwKnownFlags))
			{
				// unknown flags
				CString strFlagValue; 
				strFlagValue.Format(_T(" (%08X)"), pDevParam->RegFlags);
				strValue += strFlagValue;
			}
			m_propList.AddItem(PropCreateSimple(strType, strValue));
		}
	}
	...
	const NDASUSER_LOGICALDEVICE_INFORMATION* pLogDeviceInfo)
{
	...
	switch (pLogDeviceInfo->LogicalDiskInformation.ContentEncrypt.Type)
	{
	case NDAS_CONTENT_ENCRYPT_TYPE_NONE:
		strEncryptType = _T("None");
		break;
	case NDAS_CONTENT_ENCRYPT_TYPE_SIMPLE:
		strEncryptType = _T("Simple");
		break;
	case NDAS_CONTENT_ENCRYPT_TYPE_AES:
		strEncryptType = _T("AES");
		break;
	default:
		strEncryptType.Format(
			_T("0x%04X"), 
			pLogDeviceInfo->LogicalDiskInformation.ContentEncrypt.Type);
		break;
	}
	str.Format(
		_T("Encrypted Disk with %s (%d bits) R%X.%X"),
		strEncryptType,
		pLogDeviceInfo->LogicalDiskInformation.ContentEncrypt.KeyLength * 8, // bits
		HIWORD(pLogDeviceInfo->LogicalDiskInformation.ContentEncrypt.Revision),
		LOWORD(pLogDeviceInfo->LogicalDiskInformation.ContentEncrypt.Revision));
	return str;
}
...
//	tiLocation.Expand();
	...
	{
		const NDASUSER_LOGICALDEVICE_INFORMATION* pLogDeviceInfo = 
			pLogDev->GetLogicalDeviceInfo();
		if (NULL != pLogDeviceInfo && 
			IS_NDAS_LOGICALDEVICE_TYPE_DISK_GROUP(pLogDev->GetType()) &&
			0 != pLogDeviceInfo->LogicalDiskInformation.ContentEncrypt.Revision &&
			NDAS_CONTENT_ENCRYPT_TYPE_NONE != pLogDeviceInfo->LogicalDiskInformation.ContentEncrypt.Type)
		{
			CString strNode;
			NdasLogicalDiskEncryptString(strNode, pLogDeviceInfo);
			tiMember.AddTail(strNode, 0);
			// m_tvLogDev.InsertItem(strNode, TVI_ROOT, TVI_LAST);
		}
	}
