...
NdasCommConnectionInfoToDeviceId(
	const NDAS_CONNECTION_INFO* pConnectionInfo,
	...
	case NDAS_CONNECTION_INFO_TYPE_IDW:
		{
			...
			{
				bResults = NdasIdValidateW(wszID, wszKey);
				TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASCOMM_ERROR_INVALID_PARAMETER, fail, bResults);
			}

			bResults = NdasIdStringToDeviceW(wszID, &pUnitDeviceID->DeviceId);
			TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASCOMM_ERROR_INVALID_PARAMETER, fail, bResults);
		}
		break;
	case NDAS_CONNECTION_INFO_TYPE_IDA:
		{
			...
			if(pConnectionInfo->bWriteAccess)
			{
				bResults = NdasIdValidateA(szID, szKey);
				TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASCOMM_ERROR_INVALID_PARAMETER, fail, bResults);
			}

			bResults = NdasIdStringToDeviceA(szID, &pUnitDeviceID->DeviceId);
			TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASCOMM_ERROR_INVALID_PARAMETER, fail, bResults);
		}
		break;
	default:
...
conn:
	...
	if(pPath->PerTarget[pConnectionInfo->UnitNo].MediaType == NDAS_UNITDEVICE_MEDIA_TYPE_BLOCK_DEVICE)
	{
		...
		bResults = NdasCommIsHandleValidForRW((HNDAS)pPath);
  ...
  HNDAS hNDASDevice)
{
	...
	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASCOMM_ERROR_INVALID_PARAMETER, !::IsBadReadPtr(pPath, sizeof(LANSCSI_PATH)));
...
//	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASCOMM_ERROR_INVALID_PARAMETER, usSectorCount > 0);
	...
	while(l_ui64SectorCountLeft > 0){
		l_usSectorCount = (UINT16)min(l_ui64SectorCountLeft, pPath->iMaxBlocks);
					...
					)
{
	...
	bResults = NdasCommIsHandleValid((HNDAS)pPath);
	TEST_AND_RETURN_IF_FAIL(bResults);
