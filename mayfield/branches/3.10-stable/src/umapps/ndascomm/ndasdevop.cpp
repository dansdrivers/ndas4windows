...
static VOID
DbgPrint(IN PCHAR	DebugMessage, ...)
{
    va_list ap;
...
// return value
// LOBYTE(LOWORD(lLoginResult)) : login phase 1, 2, 3, 4
// HIBYTE(LOWORD(lLoginResult)) :
//  1 : SendRequest fail
//  2 : ReadReply fail
//  3 : Reply Header check fail
//  4 : response fail, response = LOBYTE(HIWORD(lLoginResult))
//  5 : data segment fail
//  6 : security fail
//  7 : negotiation fail
	  ...
	  )
{
	...
	// HWVersion is detected after 1st step (X)
	// HWVersion is given as parameter
/*
...
//	pPath->HWVersion = LANSCSIIDE_VERSION_1_1;
	...
	if(SendRequest(pPath->connsock, pPath, &pdu) != 0) {
		...
		return MAKELONG(MAKEWORD(1, 1), 0);
	}
	...
	if(iResult == SOCKET_ERROR) {
		...
		return MAKELONG(MAKEWORD(1, 2), 0);
	}
	...
	if((pLoginReplyHeader->Opcode != LOGIN_RESPONSE)
		...
		return MAKELONG(MAKEWORD(1, 3), 0);
	}
	...
	if(pLoginReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		...
		return MAKELONG(MAKEWORD(1, 4), pLoginReplyHeader->Response);
	}
	...
	if((ntohl(pLoginReplyHeader->DataSegLen) < BIN_PARAM_SIZE_LOGIN_FIRST_REPLY)
		...
		return MAKELONG(MAKEWORD(1, 5), 0);
	}
	}
	...
	if(pParamSecu->ParamType != BIN_PARAM_TYPE_SECURITY
		...
		return MAKELONG(MAKEWORD(1, 6), 0);
	}
	...
	if(SendRequest(pPath->connsock, pPath, &pdu) != 0) {
		...
		return MAKELONG(MAKEWORD(2, 1), 0);
	}
	...
	if(iResult == SOCKET_ERROR) {
		...
		return MAKELONG(MAKEWORD(2, 2), 0);
	}
	...
	if((pLoginReplyHeader->Opcode != LOGIN_RESPONSE)
		...
		return MAKELONG(MAKEWORD(2, 3), 0);
	}

	if(pLoginReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		DebugPrint(1, ("[LanScsiOpLib]login: Second Failed. %02x\n", pLoginReplyHeader->Response));
		return MAKELONG(MAKEWORD(2, 4), pLoginReplyHeader->Response);
	}
	...
	if((ntohl(pLoginReplyHeader->DataSegLen) < BIN_PARAM_SIZE_LOGIN_SECOND_REPLY)
		...
		return MAKELONG(MAKEWORD(2, 5), 0);
	}
	}
	...
	if(pParamSecu->ParamType != BIN_PARAM_TYPE_SECURITY
		...
		return MAKELONG(MAKEWORD(2, 6), 0);
	}
	...
	if(SendRequest(pPath->connsock, pPath, &pdu) != 0) {
		...
		return MAKELONG(MAKEWORD(3, 1), 0);
	}
	...
	if(iResult == SOCKET_ERROR) {
		...
		return MAKELONG(MAKEWORD(3, 2), 0);
	}
	...
	if((pLoginReplyHeader->Opcode != LOGIN_RESPONSE)
		...
		return MAKELONG(MAKEWORD(3, 3), 0);
	}

	if(pLoginReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		DebugPrint(1, ("[LanScsiOpLib]login: Third Failed. %02x\n", pLoginReplyHeader->Response));
		return MAKELONG(MAKEWORD(3, 4), pLoginReplyHeader->Response);
	}
	...
	if((ntohl(pLoginReplyHeader->DataSegLen) < BIN_PARAM_SIZE_LOGIN_THIRD_REPLY)
		...
		return MAKELONG(MAKEWORD(3, 5), 0);
	}
	}
	...
	if(pParamSecu->ParamType != BIN_PARAM_TYPE_SECURITY
		...
		return MAKELONG(MAKEWORD(3, 6), 0);
	}
	...
	if(SendRequest(pPath->connsock, pPath, &pdu) != 0) {
		...
		return MAKELONG(MAKEWORD(4, 1), 0);
	}
	...
	if(iResult == SOCKET_ERROR) {
		...
		return MAKELONG(MAKEWORD(4, 2), 0);
	}
	...
	if((pLoginReplyHeader->Opcode != LOGIN_RESPONSE)
		...
		return MAKELONG(MAKEWORD(4, 3), 0);
	}

	if(pLoginReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		DebugPrint(1, ("[LanScsiOpLib]login: Fourth Failed. %02x\n", pLoginReplyHeader->Response));
		return MAKELONG(MAKEWORD(4, 4), pLoginReplyHeader->Response);
	}
	...
	if((ntohl(pLoginReplyHeader->DataSegLen) < BIN_PARAM_SIZE_LOGIN_FOURTH_REPLY)
		...
		return MAKELONG(MAKEWORD(4, 5), 0);
	}
	}
	...
	if(pParamNego->ParamType != BIN_PARAM_TYPE_NEGOTIATION) {
		...
		return MAKELONG(MAKEWORD(4, 7), 0);
	}
	...
	if(2 == pPath->HWVersion && pPath->iMaxBlocks > 104)
	{
    pPath->iMaxBlocks = 104;
	}
...
// return value
// HIBYTE(HIWORD(l)) : logout phase == 1
// LOBYTE(HIWORD(l)) :
//  1 : SendRequest fail
//  2 : ReadReply fail
//  3 : wrong phase
//  4 : response fail, response = LOWORD(l)
	   ...
	   )
{
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		...
		return MAKELONG(MAKEWORD(1, 4), pReplyHeader->Response);
	}
			...
			)
{
	...
	//
	// For compatibility reason, V1.0 chip information
	// should always turn DMA bit on.
	//
	pPath->PerTarget[TargetId].bDma = TRUE;
...
//	Connection scenario
//	MakeConnectionList(cPaths, pPaths)
//		GetInterfaceList()
//		Create socketAddressList->iAddressCount * cPaths of sockets
//		sock[i] = MakeConnection(&pPaths[i], socketAddressList)
...
typedef std::map<NDAS_DEVICE_ID, SOCKADDR> DeviceIdMap;

SOCKET
LpxConnect(
		   IN LPSOCKADDR Sockaddr,
		   IN PNDAS_DEVICE_ID DeviceId)
{
	...
	if(sock == INVALID_SOCKET) {
		PrintError(WSAGetLastError(), "LpxConnect: socket ");
		return INVALID_SOCKET;
	}

	// set local host address
	socketLpx =
		*(PSOCKADDR_LPX)(Sockaddr);
	...
	if(iErrcode == SOCKET_ERROR) {
		PrintError(WSAGetLastError(), "LpxConnect: bind ");
	...
	memcpy(serverSocketLpx.LpxAddress.Node, DeviceId->Node,
		sizeof(serverSocketLpx.LpxAddress.Node));
	serverSocketLpx.LpxAddress.Port = htons(LPX_PORT_NUMBER);

	// connect socket to remote host
	iErrcode = connect(sock, (struct sockaddr *)&serverSocketLpx,
		sizeof(serverSocketLpx));
	if(iErrcode == SOCKET_ERROR)
	{
		PrintError(WSAGetLastError(), "LpxConnect: connect ");
			   ...
			   )
{
	...
	static DeviceIdMap		HintLocalAddress;
	static SOCKADDR			HintLocalAddressRecent;


	// try with local address in hint
	if(HintLocalAddress.find(pPath->UnitDeviceID.DeviceId) != HintLocalAddress.end())
	{
		sock = LpxConnect(&HintLocalAddress[pPath->UnitDeviceID.DeviceId],
			&pPath->UnitDeviceID.DeviceId);

		if(sock != INVALID_SOCKET) {
			// connection successful
			pPath->connsock = sock;
			CopyMemory(&HintLocalAddressRecent, &HintLocalAddress[pPath->UnitDeviceID.DeviceId], sizeof(SOCKADDR));
			return;
		}
	}

	// try with last successful local address
	sock = LpxConnect(&HintLocalAddressRecent, &pPath->UnitDeviceID.DeviceId);
	...
	socketAddressList = (LPSOCKET_ADDRESS_LIST)LocalLock(hMemorySocketAddressList);
//	socketAddressList = (LPSOCKET_ADDRESS_LIST)malloc(socketAddressListLength);
	...
	for(i = 0; i < socketAddressList->iAddressCount; i++) {
		sock = LpxConnect(socketAddressList->Address[i].lpSockaddr,
			&pPath->UnitDeviceID.DeviceId);
		if(INVALID_SOCKET != sock)
		{
			pPath->connsock = sock;
			HintLocalAddress[pPath->UnitDeviceID.DeviceId] =
				*(socketAddressList->Address[i].lpSockaddr);
			CopyMemory(&HintLocalAddressRecent, socketAddressList->Address[i].lpSockaddr, sizeof(SOCKADDR));
			break;
		}
	}
	...
	if(sock == INVALID_SOCKET) {
		DebugPrint(1, ("[LanScsiCli]MakeConnection: No LanDisk (%02X:%02X:%02X:%02X:%02X:%02X)!!!\n",
			(int)pPath->UnitDeviceID.DeviceId.Node[0],
			(int)pPath->UnitDeviceID.DeviceId.Node[1],
			(int)pPath->UnitDeviceID.DeviceId.Node[2],
			(int)pPath->UnitDeviceID.DeviceId.Node[3],
			(int)pPath->UnitDeviceID.DeviceId.Node[4],
			(int)pPath->UnitDeviceID.DeviceId.Node[5]
			));
