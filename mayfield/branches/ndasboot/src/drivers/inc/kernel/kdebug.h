...
#define	KDPrint(DEBUGLEVEL, FORMAT) do { if((DEBUGLEVEL) <= KDebugLevel) { _LSKDebugPrintWithLocation(_LSKDebugPrint FORMAT , __MODULE__, __LINE__, __FUNCTION__); } } while(0);

#define	KDPrintM(DEBUGMASK, FORMAT)	do { if((DEBUGMASK)& KDebugMask) { _LSKDebugPrintWithLocation(_LSKDebugPrint FORMAT , __MODULE__, __LINE__, __FUNCTION__); } } while(0);
