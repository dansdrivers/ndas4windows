...
//	KIRQL			cancelIrql;
	...
	//
	NbfReferenceConnection("LpxConnect", Connection, CREF_P_CONNECT);
	...
	if(!NT_SUCCESS(status)) {
		...
		NbfDereferenceConnection("LpxConnect", Connection, CREF_P_CONNECT);
...
SmpWorkDpcRoutine(
	...
	NbfReferenceConnection("SmpWorkDpcRoutine", servicePoint->Connection, CREF_PROCESS_DATA);
...
TIMEOUT:	
	...
	NbfDereferenceConnection("SmpWorkDpcRoutine", servicePoint->Connection, CREF_PROCESS_DATA);
...
//#endif
		...
		//        retransmits for one send command will sum at NbfDereference in PacketFree
