...
ULONG	DebugMsgLevel ;
CHAR DebugMsgBuffer[DEBUG_BUFFER_LENGTH] ;
...

{
    ...
    if ((DebugPrintLevel <= (DebugMsgLevel & 0x0000ffff)) ||
        ((1 << (DebugPrintLevel + 15)) & DebugMsgLevel)) {

        _vsnprintf(DebugMsgBuffer, DEBUG_BUFFER_LENGTH, DebugMessage, ap);

        DbgPrint((DebugMsgBuffer));
    }
...
//----------------------------------------------------------------------
//      R O F I L T E R   R O U T I N E S
//----------------------------------------------------------------------
//
//
//	prevent FastIo operations:
//		FilemonFastIoWrite
//		FilemonFastIoPrepareMdlWrite
//		FilemonFastIoWriteCompressed
//
//	prevent IRPs:
//       IRP_MJ_CREATE with Create, OverWriteIf, or Overwrite option
//       IRP_MJ_WRITE except for NTFS metadata files, and PAGING IO:delayed Write
...
//
//
BOOLEAN IsNtfsMetadataFile(PHOOK_EXTENSION hookExt, PCHAR fullPathName) {
	BOOLEAN	bret = FALSE ;
	int i ;

	// MetadataFileName format : 'x:\$xxxx'
	if(	fullPathName[1] == ':' &&
		fullPathName[2] == '\\' &&
		fullPathName[3] == '$') {
		for(i =0 ; i < MAX_NTFS_METADATA_FILE ; i++) {
			if( strncmp(NtfsMetadataFileNames[i], &(fullPathName[3]), strlen(NtfsMetadataFileNames[i])) == 0) {
				bret = TRUE ;
				break ;
			}
		}
    ...
    )
{
    ...
    for ( drive = 0; drive < 26; ++drive )  {
        ...
        if( (bit & DriveSet) &&
            ...
            //
			PDEVICE_OBJECT	phookdev = DriveHookDevices[ drive ] ;
			if(phookdev == NULL) break ;

            for( i = 0; i< 26; i++ ) {

                if( DriveHookDevices[i] == phookdev ) {
			...
			hookExt = phookdev->DeviceExtension ;
            IoDetachDevice( hookExt->FileSystem );
            IoDeleteDevice( phookdev );
    ...
    ) 
{
    ...
    if( hookExt->Type == GUIINTERFACE ) {
        ...
        case IOCTL_FILEMON_GETDRIVES:

            //
            // get Hooked drives
            //
            DbgPrint (("Filemon: get drives\n"));

            if(OutputBufferLength >= sizeof(ULONG)) {

                *(ULONG *)OutputBuffer = CurrentDriveSet ;
                IoStatus->Information = sizeof(ULONG);
...
//
//	denial list :
//       IRP_MJ_CREATE with Create or Overwrite option
//       IRP_MJ_WRITE except for NTFS metadata files, and PAGING IO
//       IRP_MJ_SET_INFORMATION
//       IRP_MJ_SET_EA
//       IRP_MJ_SET_VOLUME_INFORMATION
//		 IRP_MJ_SET_SECURITY
    ...
    )
{
    ...
    if( hookExt->Hooked && fullPathName ) {
        ...
        case IRP_MJ_CREATE:
			...
			//	(Options >> 24) & 0xFF
			options = (CHAR)(((currentIrpStack->Parameters.Create.Options) >> 24) & 0xFF) ;
			if( options == FILE_CREATE  ||
				options == FILE_OVERWRITE ||
				options == FILE_OVERWRITE_IF ) {
        ...
        case IRP_MJ_WRITE:
			...
			//
			//	discard IRP_MJ_WRITE except for NTFS file or Paging IO
			//
			if( IsNtfsMetadataFile(hookExt, fullPathName) == FALSE &&
...
//            break;
        ...
        case IRP_MJ_SET_VOLUME_INFORMATION:
            ...
            hookCompletion = LogRecord( TRUE, &seqNum, &dateTime, NULL, 
                                       ...
                                       VolumeInformation[currentIrpStack->Parameters.QueryVolume.FsInformationClass] );
			//
			//	deny changing information of a volume
			//
			Irp->IoStatus.Status = STATUS_MEDIA_WRITE_PROTECTED ;
			Irp->IoStatus.Information = 0 ;
			IoCompleteRequest(Irp, IO_DISK_INCREMENT) ;
...
//                break;
        ...
        case IRP_MJ_SET_SECURITY:
            ...
            hookCompletion = LogRecord( TRUE, &seqNum, &dateTime, NULL, 
                                       ...
                                       name, fullPathName );
			//
			//	deny changing security of a file
			//
			Irp->IoStatus.Status = STATUS_MEDIA_WRITE_PROTECTED ;
			Irp->IoStatus.Information = 0 ;
			IoCompleteRequest(Irp, IO_DISK_INCREMENT) ;
    ...
    )
{
    ...
    WCHAR                   deviceNameBuffer[]  = L"\\Device\\ROFilt";
    UNICODE_STRING          deviceNameUnicodeString;
    WCHAR                   deviceLinkBuffer[]  = L"\\DosDevices\\ROFilt";
    UNICODE_STRING          deviceLinkUnicodeString;
    ULONG                   i;
