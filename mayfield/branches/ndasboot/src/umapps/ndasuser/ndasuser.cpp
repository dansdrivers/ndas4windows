...
#define NDAS_VALIDATE_DEVICE_STRING_ID(lpszDeviceStringId) \
	{ \
	BOOL fSuccess = NdasIdValidateStringIdKeyW(lpszDeviceStringId, NULL, NULL); \
	if (!fSuccess) { \
...
#define NDAS_VALIDATE_DEVICE_STRING_IDA(lpszDeviceStringId) \
	{ \
	BOOL fSuccess = NdasIdValidateStringIdKeyA(lpszDeviceStringId, NULL, NULL); \
	if (!fSuccess) { \
		::SetLastError(NDASUSER_ERROR_INVALID_DEVICE_STRING_ID); \
		return FALSE; \
	} \
...
#define NDAS_FORWARD_DEVICE_STRING_IDW(lpszDeviceStringId, is) \
	...
	NDAS_DEVICE_ID_OR_SLOT is; \
	is.bUseSlotNo = FALSE; \
	BOOL fSuccess = NdasIdConvertStringIdToRealIdW(lpszDeviceStringId,&is.DeviceId); \
...
#define NDAS_FORWARD_DEVICE_STRING_IDA(lpszDeviceStringId, is) \
	...
	NDAS_DEVICE_ID_OR_SLOT is; \
	is.bUseSlotNo = FALSE; \
	BOOL fSuccess = NdasIdConvertStringIdToRealIdA(lpszDeviceStringId,&is.DeviceId); \
	_ASSERT(fSuccess); // should not fail as we already did Validation 
	...
	LPCWSTR lpszDeviceStringKey)
{
	...
	BOOL fSuccess = NdasIdConvertStringIdToRealIdW(lpszDeviceStringId,&lpRequest->DeviceIdOrSlot.DeviceId);
	_ASSERT(fSuccess); // should not fail as we already did Validation
