			 ...
			 )
{
	...
	for(i = 0; i < nDiskCount; i++)
	{
		...
		CopyMemory(ConnectionInfo.MacAddress,
			Ref_DIB_V2.UnitDisks[i].MACAddr, sizeof(ConnectionInfo.MacAddress));

		hNdasDevices[i] = NdasCommConnect(&ConnectionInfo);
		...
		bResults = NdasCommBlockDeviceRead(hNdasDevices[i], NDAS_BLOCK_LOCATION_BITMAP,
			NDAS_BLOCK_SIZE_BITMAP, (PCHAR)Bitmap);
		TEST_AND_GOTO_IF_FAIL(out, bResults);
	...
	if(NMT_RAID1 == iMediaType)
		nFailedDisk = nRecordDisk ^1;
	else if(NMT_RAID4 == iMediaType)
		nFailedDisk = (nRecordDisk == 0) ? nDiskCount -1 : nRecordDisk -1;

	// read bitmap
	bResults = NdasCommBlockDeviceRead(hNdasDevices[nRecordDisk], NDAS_BLOCK_LOCATION_BITMAP,
		NDAS_BLOCK_SIZE_BITMAP, (PCHAR)Bitmap);
	...
	for(i = 0; i < nDiskCount; i++)
	{
		bResults = NdasCommBlockDeviceRead(hNdasDevices[i], NDAS_BLOCK_LOCATION_WRITE_LOG, 1, (PCHAR)&LWSs);
		TEST_AND_GOTO_IF_FAIL(out, bResults);
	...
	for(i = 0; i < iSectorsPerBit * SECTOR_SIZE * 8; i++) // check whole bits in bitmap
	{
		if(NDAS_BITMAP_ISSET(Bitmap, i))
			nBitsSetTotal++;
	}

	BufferRecover = (PCHAR)::HeapAlloc(::GetProcessHeap(), HEAP_ZERO_MEMORY, iSectorsPerBit * SECTOR_SIZE);
	TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASOP_ERROR_NOT_BOUND_DISK, out, BufferRecover);
	
	BufferRecover2 = (PCHAR)::HeapAlloc(::GetProcessHeap(), HEAP_ZERO_MEMORY, iSectorsPerBit * SECTOR_SIZE);
	...
	while(i != 0xFFFFFFFF)
	{
		...
		if(i * iSectorsPerBit < Ref_DIB_V2.sizeUserSpace) // recover only where IS in user space
		{
			...
			if(NMT_RAID1 == iMediaType)
			{
				bResults = NdasCommBlockDeviceRead(hNdasDevices[nRecordDisk], i * iSectorsPerBit, iSectorsToRecover, BufferRecover);
				TEST_AND_GOTO_IF_FAIL(out, bResults);
			}
			else if(NMT_RAID4 == iMediaType)
			{
			...
			bResults = NdasCommBlockDeviceWrite(hNdasDevices[nFailedDisk], i * iSectorsPerBit, iSectorsToRecover, BufferRecover);
			TEST_AND_GOTO_IF_FAIL(out, bResults);
		}
		else
		{
		...
		if(nRecoverSectorInBitmap != NDAS_BITMAP_IDX_TO_SECTOR(i))
		{
			...
			bResults = NdasCommBlockDeviceWrite(hNdasDevices[nRecordDisk], NDAS_BLOCK_LOCATION_BITMAP + nRecoverSectorInBitmap, 1, OneSector);
			TEST_AND_GOTO_IF_FAIL(out, bResults);
		}
	...
	// clear LWSs
	bResults = NdasCommBlockDeviceRead(hNdasDevices[nRecordDisk], NDAS_BLOCK_LOCATION_WRITE_LOG, 1, (PCHAR)&LWSs);

	for(i = 0; i < nDiskCount; i++)
		bResults = NdasCommBlockDeviceWrite(hNdasDevices[i], NDAS_BLOCK_LOCATION_WRITE_LOG, 1, (PCHAR)&LWSs);
		...
		if(NULL != pnDIBSize)
		{
			if(*pnDIBSize >= nDIBSize) // make sure there is enough space
			{
				...
				if(NMT_SINGLE != DIB_V2.iMediaType)
				{
					...
					bResults = NdasCommGetDeviceId(hNdasDevice, MACAddr, &UnitNumber);
