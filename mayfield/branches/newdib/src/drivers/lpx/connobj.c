...

{
	...
	DebugPrint(3, ("!!!!!!CALL NbfDestroyConnection:   for connection %lx.\n",
                    TransportConnection));
//	if((NumberOfConn %10) == 0)
//		DebugPrint(DebugLevel, ("NumberOfConn %lx\n", NumberOfConn));
	InterlockedDecrement(&NumberOfConn);
...
#ifdef __SOCKET_NBF__
	ASSERT(AddressDeviceContext);
	if(AddressDeviceContext == NULL) 
		return;

    DeviceContext = AddressDeviceContext;
#else
    DeviceContext = TransportConnection->Provider;
#endif
...

{
    ...
    IF_NBFDBG (NBF_DEBUG_TEARDOWN | NBF_DEBUG_PNP) {
        DebugPrint(2, ("NbfStopConnection: Entered for connection %lx LSN %x RSN %x.\n",
