...
static
BOOL
_NdasOpConnectWithDIB(NDAS_DIB_V2 *DIB_V2, HNDAS *ahNDAS, BOOL bWritable, BOOL bAllShouldSuccess);

static
BOOL
_NdasOpDisconnectWithDIB(NDAS_DIB_V2 *DIB_V2, HNDAS *ahNDAS);

static
BOOL
_NdasOpClearMBR(HNDAS hNDAS);

static
BOOL
_NdasOpClearXArea(HNDAS hNDAS);

static
DWORD
_NdasOpVerifyDiskCount(UINT32 BindType, UINT32 nDiskCount);
	...
	)
{
	...
	bResults = _NdasOpVerifyDiskCount(BindType, nDiskCount);
	...
	for(i = 0; i < nDiskCount; i++)
	{
		...
		{
			bResults = _NdasOpClearMBR(ahNDAS[i]);
			TEST_AND_GOTO_IF_FAIL(out, bResults);
		}

		// clear X Area
		bResults = _NdasOpClearXArea(ahNDAS[i]);
		TEST_AND_GOTO_IF_FAIL(out, bResults);
    ...
    UINT32 *pnDIBSize)
{
	...
	// check done, copy DIB_V2 information from NDAS Device to pDIB_V2
	{
		...
		TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASOP_ERROR_VERSION_HIGH,
			!IS_NDAS_DIB_V2_VERSION_HIGH(DIB_V2));
...
process_v1:
	...
	pDIB_V2->MajorVersion = 0; // force version to 0.1 as it is in NDAS_DIB
	pDIB_V2->MinorVersion = 1;
...
_NdasOpConnectWithDIB(NDAS_DIB_V2 *DIB_V2, HNDAS *ahNDAS, BOOL bWritable, BOOL bAllShouldSuccess)
{
	...
	_ASSERT(!::IsBadReadPtr(DIB_V2, sizeof(NDAS_DIB_V2)));
	_ASSERT(!::IsBadWritePtr(ahNDAS, sizeof(ahNDAS) * (DIB_V2->nDiskCount + DIB_V2->nSpareCount)));

	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASOP_ERROR_INVALID_PARAMETER,
		NDAS_DIB_V2_SIGNATURE == DIB_V2->Signature);

	ZeroMemory(ahNDAS, sizeof(ahNDAS) * (DIB_V2->nDiskCount + DIB_V2->nSpareCount));

	for(i = 0; i < DIB_V2->nDiskCount + DIB_V2->nSpareCount; i++)
	{
		SET_CONNECTION_INFO_BY_DIB(i, *DIB_V2, bWritable, l_ci);

		ahNDAS[i] = NdasCommConnect(&l_ci, 0, NULL);

		TEST_AND_GOTO_IF_FAIL(fail, !bAllShouldSuccess || ahNDAS[i]);
	}
...
BOOL
NDASOPAPI
NdasOpGetFaultSectorSize(
	CONST NDASCOMM_CONNECTION_INFO *ci,
	PUINT64 pSectorsFault
)
{
	BOOL bResults = FALSE;
	BOOL bReturn = FALSE;
	...
	PBYTE BufferRecover = NULL;
	UINT32 byte_count_bitmap;


	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASOP_ERROR_INVALID_PARAMETER,
		!::IsBadReadPtr(ci, sizeof(NDASCOMM_CONNECTION_INFO)));
	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASOP_ERROR_INVALID_PARAMETER,
		!::IsBadWritePtr((LPVOID)pSectorsFault, sizeof(UINT64)));
	...
	TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASOP_ERROR_NO_MISSING_MEMBER, out,
		i != DIB_V2.nDiskCount);

	unit_recover_info = RMD_IDX_RECOVER_INFO(DIB_V2.nDiskCount, i);

	// read bitmap
	BufferRecover = (PBYTE)::HeapAlloc(::GetProcessHeap(), HEAP_ZERO_MEMORY, NDAS_BLOCK_SIZE_BITMAP * SECTOR_SIZE);
	TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASOP_ERROR_NOT_ENOUGH_MEMORY, out, BufferRecover);

	SET_CONNECTION_INFO_BY_DIB(rmd.UnitMetaData[unit_recover_info].iUnitDeviceIdx, DIB_V2, FALSE, l_ci);

	hNDAS = NdasCommConnect(&l_ci, 0, NULL);
	TEST_AND_GOTO_IF_FAIL(out, hNDAS);

	bResults = NdasCommBlockDeviceRead(hNDAS, NDAS_BLOCK_LOCATION_BITMAP, NDAS_BLOCK_SIZE_BITMAP, BufferRecover);
	TEST_AND_GOTO_IF_FAIL(out, bResults);

	bResults = NdasCommDisconnect(hNDAS);
	TEST_AND_GOTO_IF_FAIL(out, bResults);

	byte_count_bitmap = (UINT32)(DIB_V2.sizeUserSpace / (DIB_V2.iSectorsPerBit * 8));
	*pSectorsFault = 0;
	for(i = 0; i < byte_count_bitmap; i++)
	{
		for(j = 0; j < 8; j++)
		{
			if(BufferRecover[i] & (0x01 << j))
			{
				*pSectorsFault += DIB_V2.iSectorsPerBit;
			}
		}
	}
...
out:
	if(BufferRecover)
	{
		::HeapFree(::GetProcessHeap(), NULL, BufferRecover);
		BufferRecover = NULL;
	}
	return bReturn;
}
	...
	CONST NDASCOMM_CONNECTION_INFO *ci)
{
	...
	TEST_AND_GOTO_WITH_ERROR_IF_FAIL(
		NDASOP_ERROR_VERSION_HIGH,
		out,
		!IS_NDAS_DIB_V2_VERSION_HIGH(DIB_V2));

	ahNDAS = (HNDAS *)::HeapAlloc(::GetProcessHeap(), HEAP_ZERO_MEMORY, 
		(DIB_V2.nDiskCount + DIB_V2.nSpareCount) * sizeof(HNDAS));
	TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASOP_ERROR_NOT_ENOUGH_MEMORY, out, ahNDAS);

	// DIB V1 -> DIB V2

	if (0 == DIB_V2.MajorVersion &&
		1 == DIB_V2.MinorVersion &&)
	{
		if(NMT_MIRROR == DIB_V2.iMediaType)
		{
			// migrate NMT_MIRROR
			bResults = _NdasOpConnectWithDIB(&DIB_V2, ahNDAS, TRUE, TRUE);
			TEST_AND_GOTO_IF_FAIL(out, bResults);

			// version
			DIB_V2.MajorVersion = 1;
			DIB_V2.MinorVersion = 1;
			...
			// sizeUserSpace
			{
				// read sector count of primary
				bResults = NdasCommGetUnitDeviceInfo(ahNDAS[0], &UnitInfo);
				TEST_AND_GOTO_IF_FAIL(out, bResults);

				_ASSERT(UnitInfo.SectorCount - NDAS_BLOCK_SIZE_XAREA == DIB_V2.sizeUserSpace);
			}

			// iSectorsPerBit
			for(j = 7; TRUE; j++)
			{
				if(DIB_V2.sizeUserSpace <= 1 << (23 + j)) // 512 GB : 128 SPB
				{
					DIB_V2.iSectorsPerBit = 1 << j;
					break;
				}
				_ASSERT(j <= 12); // protect overflow
			}
			...
			// iMediaType
			DIB_V2.iMediaType = NMT_RAID1;

			// nDiskCount
			_ASSERT(2 == DIB_V2.nDiskCount);

			// iSequence : set on writing
			...
			bResults = _NdasOpDisconnectWithDIB(&DIB_V2, ahNDAS);
			TEST_AND_GOTO_IF_FAIL(out, bResults);
		}
		else if(NMT_AGGREGATE == DIB_V2.iMediaType)
		{
			// migrate NMT_AGGREGATE
			bResults = _NdasOpConnectWithDIB(&DIB_V2, ahNDAS, TRUE, TRUE);
			TEST_AND_GOTO_IF_FAIL(out, bResults);
			...
			// iSectorsPerBit
			DIB_V2.iSectorsPerBit = 0;

			// iMediaType
			_ASSERT(NMT_AGGREGATE == DIB_V2.iMediaType);
			...
			for(i = 0; i < DIB_V2.nDiskCount + DIB_V2.nSpareCount; i++)
			{
				// sizeUserSpace
				// read sector count
				bResults = NdasCommGetUnitDeviceInfo(ahNDAS[i], &UnitInfo);
				TEST_AND_GOTO_IF_FAIL(out, bResults);
		...
		{
			_ASSERT(NMT_SINGLE == DIB_V2.iMediaType);
		}
	}
	...
	// 1.0 -> 1.1
	// difference : CRC
	// actually, 1.0 has not been release. not necessary.
	if(1 == DIB_V2.MajorVersion && 0 == DIB_V2.MinorVersion)
	{
		bResults = _NdasOpConnectWithDIB(&DIB_V2, ahNDAS, TRUE, TRUE);
		TEST_AND_GOTO_IF_FAIL(out, bResults);
	...
	// 1.1 -> 1.2
	// difference : spare disk, RMD. use RMD to check failed device
	if(1 == DIB_V2.MajorVersion && 1 == DIB_V2.MinorVersion)
	{
		bResults = _NdasOpConnectWithDIB(&DIB_V2, ahNDAS, TRUE, TRUE);
		TEST_AND_GOTO_IF_FAIL(out, bResults);
		...
		// Init RMD for RAID 1, 4
		NDAS_RAID_META_DATA rmd;
		if (NMT_RAID1 == DIB_V2.iMediaType ||
			NMT_RAID4 == DIB_V2.iMediaType)
		{
			...
			for(i = 0; i < DIB_V2.nDiskCount; i++)
			{
				...
				{
					bResults = NdasCommBlockDeviceRead(ahNDAS[i], 
						NDAS_BLOCK_LOCATION_BITMAP + j, 1, emptyData);
					TEST_AND_RETURN_IF_FAIL(bResults);

					for(k = 0; k < SECTOR_SIZE / sizeof(k); k++)
					{
						if(0 != ((UINT32 *)emptyData)[k])
						{
							rmd.UnitMetaData[i].UnitDeviceStatus = NDAS_UNIT_META_BIND_STATUS_FAULT;
							break;
						}
			...
			for(i = 0; i < DIB_V2.nDiskCount + DIB_V2.nSpareCount; i++)
			{
				rmd.UnitMetaData[i].iUnitDeviceIdx = i;
			}
		}
...
out:
	...
	_NdasOpDisconnectWithDIB(&DIB_V2, ahNDAS);
	HEAP_SAFE_FREE(ahNDAS);
	...
	CONST UINT32 nReplace)
{
	...
	// fast check
	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASOP_ERROR_INVALID_PARAMETER,
		NDASCOMM_CONNECTION_INFO_TYPE_ID_DEVICE == ci_replace->address_type);
	...
	// check replace size
	hNDAS = NdasCommConnect(ci_replace, 0, NULL);
	TEST_AND_GOTO_IF_FAIL(out, hNDAS);
	...
	// replace into DIB
	::CopyMemory(DIB_V2.UnitDisks[nReplace].MACAddr, 
		ci_replace->DeviceID, sizeof(ci_replace->DeviceID));
	...
	if(nReplace < DIB_V2.nDiskCount)
	{
		...
		for(i = 0; i < DIB_V2.nDiskCount; i++)
		{
			TEST_AND_GOTO_WITH_ERROR_IF_FAIL(NDASOP_ERROR_TOO_MANY_MISSING_MEMBER, out,
				!(NDAS_UNIT_META_BIND_STATUS_FAULT & rmd.UnitMetaData[i].UnitDeviceStatus) ||
				nReplace == i);
		}
		// Set fault to RMD at nReplace
		rmd.UnitMetaData[nReplace].UnitDeviceStatus |= NDAS_UNIT_META_BIND_STATUS_FAULT;
	...
	CONST UINT32 nReplace)
{
	...
	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASOP_ERROR_INVALID_PARAMETER,
		!::IsBadReadPtr(ci_replace, sizeof(NDASCOMM_CONNECTION_INFO)));
	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASOP_ERROR_INVALID_PARAMETER,
		ci->bWriteAccess); // use write access. this function does not support run time replace yet.
	TEST_AND_RETURN_WITH_ERROR_IF_FAIL(NDASOP_ERROR_INVALID_PARAMETER,
		ci_replace->bWriteAccess);
