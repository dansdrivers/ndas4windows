...
BOOL CNBLogicalDevice::GetCommandAbility(int nID)
{
	...
	case IDM_TOOL_MIGRATE:
		return (IsHDD() && IsOperatableAll() && NeedMigrate());
	case IDM_TOOL_REPLACE_DEVICE:
		return (IsHDD() && IsOperatable() && (1 == DevicesTotal(FALSE) - DevicesTotal(TRUE)));
	case IDM_TOOL_REPLACE_UNIT_DEVICE:
		if(IsHDD() && IsOperatable() && (1 == DevicesTotal(FALSE) - DevicesTotal(TRUE)))
		{
			...
			for(UINT32 i = 0; i < DevicesTotal(); i++)
			{
				if(!m_mapUnitDevices.count(i))
				{
					...
					CopyMemory(ci.AddressLPX, DIB()->UnitDisks[i].MACAddr, sizeof(ci.AddressLPX));

					HNDAS hNDAS = NdasCommConnect(&ci, 0, NULL);
					if(hNDAS)
					{
...
BOOL CNBLogicalDevice::NeedMigrate()
{
	if(!IsHDD())
		return FALSE;
	...
	if(IS_NDAS_DIB_V2_VERSION_LOW(*DIB()))
		return TRUE;
