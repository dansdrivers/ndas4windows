...
extern HINSTANCE _NdasUpdateDllInstance;
	...
	LPCTSTR szCaption)
{
    ...
    TCHAR szText[MAX_STR_CAPTION] = {0};
    ...
    if (!m_hwndProgress)
    {
        ...
        m_hwndProgress = WIN::CreateDialogParam(
			m_hInst, 
        ...
        // set icon
        HICON hIcon = (HICON) WIN::LoadIcon(m_hInst, MAKEINTRESOURCE(IDI_UPDATE));
        if (hIcon)
...
irmProgress 
CDownloadUI::SetBannerText(LPCTSTR szBanner)
{
    if (!m_fInitialized)
...
irmProgress 
CDownloadUI::SetActionText(LPCTSTR szAction)
{
    if (!m_fInitialized)
        return irmNotInitialized;
...
VOID 
CDownloadUI::InitProgressBar(ULONG ulProgressMax)
{
    ...
    HWND hwndProgBar = WIN::GetDlgItem(
		m_hwndProgress, 
		IDC_DOWNLOAD_PROGRESSBAR);

	WIN::SendMessage(
		hwndProgBar, 
		PBM_SETRANGE32, 
		/* WPARAM = */ 0, 
		/* LPARAM = */ m_ulProgressMax);
...
VOID 
CDownloadUI::IncrementProgressBar(ULONG ulProgress)
{
...
CDownloadBindStatusCallback::CDownloadBindStatusCallback(
	CDownloadUI* pDownloadUI) : 
	m_pDownloadUI(pDownloadUI), 
	m_iRefCnt(1), 
	m_ulProgressSoFar(0)
{
}
