...
--*/
{
    ...
    switch (controlCode) {
		case LANSCSIMINIPORT_IOCTL_GET_DVD_STATUS:
			{
				...
				if(srbStatus == SRB_STATUS_SUCCESS) {
					KDPrint(4,("LANSCSIMINIPORT_IOCTL_GET_DVD_STATUS: Successful.\n"));
		...
		case LANSCSIMINIPORT_IOCTL_UPGRADETOWRITE: {
			...
			if(!NT_SUCCESS(status)) {
				...
				ScsiPortLogError(
						...
						EVTLOG_UNIQUEID(EVTLOG_MODULE_COMPLETION, EVTLOG_FAIL_UPGRADEIOCTL, 0)
					);
				LSCcbFree(Ccb);
		...
		case LANSCSIMINIPORT_IOCTL_RECOVER_TARGET: {
			PLANSCSI_RECOVER_TARGET_DATA	RecoverTargetData;

			RecoverTargetData = (PLANSCSI_RECOVER_TARGET_DATA)srbIoctlBuffer;

            KDPrint(5, ("Recover Raid.\n"));

			srbStatus = SrbIoctlRecoverRaid(
							HwDeviceExtension,
							LuExtension,
							RecoverTargetData,
							&srbIoControl->ReturnCode,
