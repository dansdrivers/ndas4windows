	...
	PLANSCSI_SESSION LanScsiSession)
{
	if(LanScsiSession->SendResult.Retransmits > 0)
	{
		...
		(LanScsiSession->SplitSize >= 64) ? 32 :
		(LanScsiSession->SplitSize >= 32) ? 16 :
		(LanScsiSession->SplitSize >= 16) ? 8 :
		(LanScsiSession->SplitSize >= 8) ? 4 : 2;
	...
	else
	{
		...
		if(LanScsiSession->NrWritesAfterLastRetransmits > 16000) // 1GB for 64KB
		{
			LanScsiSession->NrWritesAfterLastRetransmits = 0;

			if(LanScsiSession->SplitSize < 128)
			{
				LanScsiSession->SplitSize *= 2;
				if(LanScsiSession->SplitSize >= 128)
					LanScsiSession->SplitSize = 128;
...
//			ASSERT(Lurn->LurnStatus == LURN_STATUS_STALL);
	...
	else
	{
		...
		if(IdeDisk->CntEcrMethod) {
			...
			//
			IdeDisk->CntEcrBuffer = ExAllocatePoolWithTag(NonPagedPool, LurnDesc->MaxBlocksPerRequest * LurnDesc->BytesInBlock, LURNEXT_ENCBUFF_TAG);
			if(IdeDisk->CntEcrBuffer) {
				IdeDisk->CntEcrBufferLength = LurnDesc->MaxBlocksPerRequest * LurnDesc->BytesInBlock;
			} else {
...
error_out:
	...
	if(IdeDisk->CntEcrKey) {
		CloseCipherKey(IdeDisk->CntEcrKey);
	}
	if(IdeDisk->CntEcrCipher) {
		CloseCipher(IdeDisk->CntEcrCipher);
	}
	if(IdeDisk) {
...
)
{
	...
	LURNIDE_INITIALIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_READ, logicalBlockAddress,transferBlocks, DataBuffer);
		...
		case LANSCSI_RESPONSE_T_COMMAND_FAILED:
			{
				...
				for(i = 0; i < transferBlocks; i++) {
					...
					LURNIDE_INITIALIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_READ,BadSector,1, &((PUCHAR)DataBuffer)[i]);
					ntStatus = LspRequest(
									&IdeDisk->LanScsiSession,
									&PduDesc,
									&response
...
{		
...
#ifdef _ADD_VENDER_COMMAND_ON_WRITE_
	...
	if(!NT_SUCCESS(ntStatus)) {
		...
		KDPrintM(DBG_LURN_ERROR, ("Error when LspTextTargetList 0x%x\n", ntStatus));
	...
	RecoverBuffer = FALSE;
	LURNIDE_INITIALIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_WRITE,logicalBlockAddress,transferBlocks, DataBuffer);
	...
	if(IdeDisk->CntEcrMethod) {
		...
		//
			...
			PduDesc.DataBuffer = IdeDisk->CntEcrBuffer;
			PduDesc.Flags |= PDUDESC_FLAG_VOLATILE_BUFFER;
		...
		case LANSCSI_RESPONSE_T_COMMAND_FAILED:
			{
				...
				for(i = 0; i < transferBlocks; i++) {
					...
					LURNIDE_INITIALIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_WRITE,BadSector,1, &((PUCHAR)DataBuffer)[i]);
					ntStatus = LspRequest(
									&IdeDisk->LanScsiSession,
									&PduDesc,
									&response
...
)
{
	...
	{
		// linked list
		switch(((PCMD_COMMON)pExtendedCommand)->Operation)
		{
		case CCB_EXT_READ:
			{
				...
				case EXT_BLOCK_OPERATION:
					ntStatus = LurnIdeDiskRead(Ccb, IdeDisk, pCmd->pByteData, pCmd->logicalBlockAddress, pCmd->LengthBlock);
					break;
				case EXT_BYTE_OPERATION_COPY:
					ntStatus = LurnIdeDiskRead(Ccb, IdeDisk, DataBuffer, pCmd->logicalBlockAddress, 1);
					RtlCopyMemory(pCmd->pByteData, DataBuffer + pCmd->Offset, pCmd->LengthByte);
					// write back
					break;
				default:
					ASSERT(FALSE);
		...
		case CCB_EXT_WRITE:
			{
				// block based
				BYTE DataBuffer[BLOCK_SIZE]; // one sector
				PCMD_BYTE_OP pCmd = (PCMD_BYTE_OP)pExtendedCommand;
...
//				RtlZeroMemory(DataBuffer, sizeof(DataBuffer));
				...
				{
				case EXT_BLOCK_OPERATION:
					ntStatus = LurnIdeDiskWrite(Ccb, IdeDisk, pCmd->pByteData, pCmd->logicalBlockAddress, pCmd->LengthBlock);
					break;
				case EXT_BYTE_OPERATION_COPY:
					RtlZeroMemory(DataBuffer, BLOCK_SIZE);
					RtlCopyMemory(DataBuffer + pCmd->Offset, pCmd->pByteData, pCmd->LengthByte);
					// write back
...
//		inquiryData.EnclosureServices;
	...
	case SCSIOP_VERIFY: 
	{
		...
		totalTransferBlocks = CDB10_TRANSFER_BLOCKS(Ccb->Cdb);
...
DO_MORE:
	...
	case SCSIOP_WRITE: 
	{
		...
		while(1)
		{
			if(transferBlocks <= SplitSize * (i +1))
			{
				ntStatus = LurnIdeDiskWrite(Ccb, IdeDisk,
					(PVOID)((PCHAR)Ccb->DataBuffer + (SplitSize * i) * BLOCK_SIZE),
					logicalBlockAddress + (SplitSize * i),
					transferBlocks - SplitSize * i);
...
/*
		...
		else
		{
			...
			ntStatus = LurnIdeDiskWrite(Ccb, IdeDisk, (PVOID)((PCHAR)Ccb->DataBuffer + 64 * BLOCK_SIZE), logicalBlockAddress + 64, transferBlocks - 64);
		}
#else
		ntStatus = LurnIdeDiskWrite(Ccb, IdeDisk, Ccb->DataBuffer, logicalBlockAddress, transferBlocks);
#endif
