...
{
	_tprintf(
		_T("NDAS Device Auto Registration Scope Management Version 1.0\n")
...
int viewscope(int argc, TCHAR** argv)
{
	...
	for (DWORD i = 0; i < arsData.GetCount(); ++i) {
		...
		_tprintf(_T("%d: %s - %s (%s)\n"),
			i,
			xs::CXSByteString(sizeof(NDAS_DEVICE_ID), rBegin.Node, _T(':')).ToString(),
			xs::CXSByteString(sizeof(NDAS_DEVICE_ID), rEnd.Node, _T(':')).ToString(),
