...
namespace ximeta {
	...
	protected:
		...
		BOOL LogMessageToStdOut(WORD wType, DWORD dwEventID, PSID lpUserSid, WORD wNumStrings, DWORD dwDataSize, LPCTSTR* lpStrings, LPVOID lpRawData);
	...
	public:
		...
		BOOL EnableEventLog(BOOL bEnable = TRUE)
		{
			...
			if (m_bUseEventLog) {
				fSuccess = RegisterEventSource();
			} else {
				fSuccess = DeregisterEventSource();
			}
...
NdasLogEventError(
	...
	LPVOID lpRawData = NULL)
{ 
	return NdasLogEvent(EVENTLOG_ERROR_TYPE, dwEventID, NULL, 0, dwDataSize, NULL, lpRawData); 
}
