...
CNdasEventMonitor::OnLogicalDeviceAlarmed(DWORD nWaitIndex)
{
	...
	{		
			...
			)
		{
			...
			(VOID) pEventPublisher->
				LogicalDeviceReconnecting(pLogDevice->GetLogicalDeviceId());
		}
		else
		{
...
BOOL CNdasEventMonitor::OnLogicalDeviceDisconnected(DWORD nWaitIndex)
{
	...
	//	}
	//}
	DPInfo(_FT("Unplugging disconnected logical device\n"));
	pLogDevice->Unplug();
	...
	do {
		...
		do {
			...
			if (WAIT_OBJECT_0 == dwWaitResult) {
				...
				//
				// Clear Mount Flag if the current tick is
				// biased more than 3 min from the Mount Tick 
				//
				PCNdasLogicalDeviceVector::const_iterator lditr = 
					m_vLogDevices.begin();
				for (; lditr != m_vLogDevices.end(); ++lditr) {
					PCNdasLogicalDevice pLogDevice = *lditr;
					if (pLogDevice->IsRiskyMount() &&
						0 != pLogDevice->GetMountTick()) {
						DWORD dwBiased = ::GetTickCount() - 
							pLogDevice->GetMountTick();
						//
						// In case of rollover (in 19 days),
						// it is safe to clear that flag.
						// This is not a strict time tick check
						//
						if (dwBiased > LOGICALDEVICE_RISK_MOUNT_INTERVAL) {
