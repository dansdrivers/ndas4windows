...
#include "ndasevent.h" // for #define NDAS_SRF_READ, NDAS_SRF_WRITE
	...
	const NDAS_HIX::DISCOVER::REQUEST* pRequest)
{
	...
	for (DWORD i = 0; i < pRequest->Data.EntryCount; ++i) {
		...
		CNdasLogicalDevice* pLogDevice = pGetNdasLogicalDevice(udid);
...
//		}
	...
	BOOL fSuccess = sock.SendTo(pRemoteAddr, cbLength, (CONST BYTE*)pReply);
	if (!fSuccess) {
	...
	const NDAS_HIX::SURRENDER_ACCESS::REQUEST* pRequest)
{
	...
	CNdasDevice* pDevice = pGetNdasDevice(unitDeviceId.DeviceId);
	if (NULL == pDevice) {
		fSuccess = pSendSurrenderReply(
			sock,
			pRemoteAddr,
	...
	// TODO: Remove AddRef from here later
	pDevice->AddRef();
	...
	//
	typedef std::map<PCNdasLogicalDevice,NHIX_UDA> LogDevSet;
