...
DWORD pGetOSVerInfo(DWORD cbBuffer, LPBYTE lpbBuffer)
{
	...
	pVerInfo->VersionMajor = htons(osvi.dwMajorVersion);
	pVerInfo->VersionMinor = htons(osvi.dwMinorVersion);
	pVerInfo->VersionBuild = htons(osvi.dwBuildNumber);
	pVerInfo->VersionPrivate = htons(osvi.wServicePackMajor);
...
DWORD pGetNDASSWVerInfo(DWORD cbBuffer, LPBYTE lpbBuffer)
{
	...
	pVerInfo->VersionMajor = htons(FV_MAJOR);
	pVerInfo->VersionMinor = htons(FV_MINOR);
	pVerInfo->VersionBuild = htons(FV_BUILD);
	pVerInfo->VersionPrivate = htons(FV_PRIVATE);
...
{
	_ASSERTE(sizeof(pEntry->DeviceId) == 
