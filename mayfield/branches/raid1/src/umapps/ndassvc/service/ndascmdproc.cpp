	...
	DWORD cbExtraInData)
{
	...
	CNdasDevice::HARDWARE_INFO hwInfo = pDevice->GetHardwareInfo();
	...
	DWORD cbExtraInData)
{
	...
	for (DWORD i = 0; i < CNdasDevice::MAX_NDAS_UNITDEVICE_COUNT; ++i) {
		CNdasUnitDevice* pUnitDevice = pDevice->GetUnitDevice(i);
		if (NULL != pUnitDevice) {
	...
	DWORD cbExtraInData)
{
	...
	CNdasUnitDevice* pUnitDevice = pDevice->GetUnitDevice(pRequest->UnitNo);
	if (NULL == pUnitDevice) { 
	...
	DWORD cbExtraInData)
{
	...
	CNdasUnitDevice* pUnitDevice = pDevice->GetUnitDevice(pRequest->dwUnitNo);
	if (NULL == pUnitDevice) {
		return WriteErrorReply(
			NDAS_CMD_STATUS_FAILED, 
			NDASHLPSVC_ERROR_UNITDEVICE_ENTRY_NOT_FOUND); 
	...
	DWORD cbExtraInData)
{
	...
	PCNdasLogicalDevice pLogDevice = pLogDevMan->Find(pRequest->LogicalDeviceId);
	if (NULL == pLogDevice) {
		return WriteErrorReply(
			NDAS_CMD_STATUS_FAILED, 
	...
	DWORD cbExtraInData)
{
	...
	PCNdasLogicalDevice pLogDevice = pGetNdasLogicalDevice(unitDeviceId);
	if (NULL == pLogDevice) {
		return WriteErrorReply(
			NDAS_CMD_STATUS_FAILED, 
			NDASHLPSVC_ERROR_LOGICALDEVICE_ENTRY_NOT_FOUND); 
	...
	DWORD cbExtraInData)
{
	...
	for (; itr != pLogDevMan->end(); ++itr) {
		...
		entry.LogicalDeviceId = pLogDev->GetLogicalDeviceId();
	...
	DWORD cbExtraInData)
{
	...
	PCNDAS_HOST_INFO pHostInfo = pHostInfoCache->GetHostInfo(&pRequest->HostGuid);
	...
	DWORD cbExtraInData)
{
	...
	//
	// we have to broadcast the change through HIX
	//
//	CNdasHIXChangeNotify hixNotify;
//	BOOL fSuccess = hixNotify.Initialize();
//	// ignore failure
//	if (fSuccess) {
//		hixNotify.Notify(pUnitDevice->GetUnitDeviceId());
//	}
