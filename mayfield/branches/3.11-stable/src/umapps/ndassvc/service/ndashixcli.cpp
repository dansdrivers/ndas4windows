...
namespace
{
	...
	{
		XTLC_ASSERT_EQUAL_SIZE(entry.DeviceId, UnitDeviceId.DeviceId.Node);
		::CopyMemory(
...
CNdasHIXDiscover::OnReceive(CLpxDatagramSocket& sock)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError,
			"Failed to receive data at %ws.\n", 
			boundAddr.ToString());
		// continue receiving
		return TRUE;
	}

	XTLTRACE2(TCHixClient, TLInfo, "Data received in %d ms.\n", 
		::GetTickCount() - m_dwInitTickCount);

	if (cbReceived < sizeof(NDAS_HIX::HEADER)) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError,
			"Data received %d bytes from %ws at %ws "
	...
	{
		XTLTRACE2(TCHixClient, TLWarning,
	...
	for (DWORD i = 0; i < pData->EntryCount; ++i) 
	{
		...
		if (NULL == pHostData) 
		{
			// MEM ALLOC FAILED!
			XTLTRACE2_ERR(TCHixClient, TLError,
				"Allocating Host Data for %d bytes failed.\n", 
				sizeof(HOST_DATA));
			// still try again to recv another
			continue;
		}
	...
	DWORD dwTimeout)
{
	...
	{
		XTLTRACE2_ERR(TCHixClient, TLError,
			"Memory allocation for %d bytes failed: ", cbData);
		return FALSE;
	}
	...
	{
		XTLC_ASSERT_EQUAL_SIZE(pData->Entry[i].DeviceId, pUnitDeviceId[i].DeviceId.Node);
		::CopyMemory(
...
CNdasHIXQueryHostInfo::OnReceive(CLpxDatagramSocket& dgSock)
{
	...
	{
		XTLTRACE2_ERR(TCHixClient, TLError,
			"Data Part received %d bytes from %ws at %ws "
	...
	{
		XTLTRACE2_ERR(TCHixClient, TLError,
			"memory allocation for %d bytes failed.\n", 
	...
	DWORD dwMaxRecvHint /* = 1 */)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError, "LPX Dgram Multiclient SendReceive failed.\n");
		return FALSE;
	}
...
CNdasHIXSurrenderAccessRequest::Initialize()
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError, "LpxDatagramSocket init failed.\n");
		return FALSE;
	}
	...
	DWORD dwTimeout /* = 1000 */)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError, "SendToSync failed.\n");
		return FALSE;
	}
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError, "RecvFrom failed.\n");
		return FALSE;
	}
	...
	{
		XTLTRACE2_ERR(TCHixClient, TLError, "Host not replied in %d ms.\n", dwTimeout);
		return FALSE;
	}
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError, "GetRecvFromResult failed.\n");
		return FALSE;
	}
...
CNdasHIXChangeNotify::Initialize()
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError, "CLpxDatagramMultiClient init failed.\n");
		return FALSE;
	}
	...
	const NDAS_UNITDEVICE_ID& unitDeviceId)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixClient, TLError, "Broadcast failed.\n");
		return FALSE;
	}
