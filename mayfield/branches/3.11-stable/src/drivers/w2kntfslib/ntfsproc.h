...
#define NtfsAcquireExclusivePagingIo(IC,FCB) {                          \
    ...
    ExAcquireResourceExclusive(((PFCB)(FCB))->PagingIoResource, TRUE);  \
    (IC)->CleanupStructure = (FCB);                                     \
}

#define NtfsReleasePagingIo(IC,FCB) {                                   \
    ASSERT((IC)->CleanupStructure == (FCB));                            \
    ExReleaseResource(((PFCB)(FCB))->PagingIoResource);                 \
...
//
// ValAlign(): Aligns P on the next V boundary.
//

#define ValAlign(P,V) (((P)) + (V-1) & (-(V)))

//
// TypeAlign(): Aligns P according to the alignment requirements of type T
//

#define TypeAlign(P,T) ValAlign( P, NTFS_TYPE_ALIGNMENT(T) )
...
NTSTATUS
NtfsFsdQueryQuota (                     // implemented in Quota.c
    IN PVOLUME_DEVICE_OBJECT VolumeDeviceObject,
    IN PIRP Irp
    );

NTSTATUS
NtfsFsdSetQuota (                     // implemented in Quota.c
    IN PVOLUME_DEVICE_OBJECT VolumeDeviceObject,
    IN PIRP Irp
    );
...
NtfsCommonCreate (                      //  implemented in Create.c
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp,
    IN PCREATE_COMPLETION_CONTEXT CompletionContext OPTIONAL,
    ...
    (!FlagOn((S)->Vcb->VcbState, VCB_STATE_VOLUME_MOUNTED) ||                           \
     ...
     : ((((S)->CompressionUnit == 0) &&                                                 \
         ...
         (((S)->Vcb->UsnJournal == NULL) ||                                             \
          (((S)->Fcb->FcbUsnRecord != NULL) &&                                          \
           !FlagOn(~(S)->Fcb->FcbUsnRecord->UsnRecord.Reason,                           \
                   (((S)->AttributeName.Length != 0) ?                                  \
