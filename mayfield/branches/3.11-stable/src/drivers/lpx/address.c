...
#define LpxDbgShowAddr(TNA)\
	{ \
		if ((TNA) == NULL) { \
			...
			LpxPrint7("%02x:%02x:%02x:%02x:%02x::%02x(%d)\n", \
				(TNA)->Node[0], \
				(TNA)->Node[1], \
				(TNA)->Node[2], \
				(TNA)->Node[3], \
...

{
	...
	PLPX_ADDRESS networkName = NULL;	// Network name string.
	...
	if (tdiLpxAddress != NULL) {
		if (tdiLpxAddress != (PVOID)-1) {
			...
			// Broadcast address. Is this ever used?
			RtlFillBytes(networkName->Node, ETHERNET_ADDRESS_LENGTH, (UCHAR)0xff);
			networkName->Port = 0;
		}
	} else {
		IF_LPXDBG (LPX_DEBUG_ADDRESS) {
			LpxPrint1("OpenAddress: IRP %p has no address\n", Irp);
		}
		if (networkName)
...

{
	...
	Address->DestroyAddressQueueItem = IoAllocateWorkItem(&DeviceContext->DeviceObject);
	if (Address->DestroyAddressQueueItem == NULL) {
		PANIC("LPX: Could not allocate WorkItem\n");
		*TransportAddress = NULL;
		ExFreePool(Address);
		return;
	}
...
{
	IoFreeWorkItem(TransportAddress->DestroyAddressQueueItem);
...

{
	...
	//
	InterlockedExchange(&pAddress->ReferenceCount, 1);
...

{
	...
	if (result == 0) {
		...
		IoQueueWorkItem(Address->DestroyAddressQueueItem, LpxDestroyAddress, DelayedWorkQueue, (PVOID)Address);
	}
} /* LpxDerefAddress */
...

{
	...
	InterlockedExchange(&addressFile->ReferenceCount, 1);
...

{
	...
	if (AddressFile->State == ADDRESSFILE_STATE_CLOSING) {
		RELEASE_SPIN_LOCK (&Address->SpinLock, oldirql);
		IF_LPXDBG (LPX_DEBUG_ADDRESS) {
			DebugPrint(1,  ("LpxStopAddressFile %p: already closing.\n", AddressFile) );
		}
		return STATUS_SUCCESS;
	}

	IF_LPXDBG (LPX_DEBUG_ADDRESS | LPX_DEBUG_PNP) {
		DebugPrint(2, ("LpxStopAddressFile %p: closing.\n", AddressFile));
	}
	...
	if (AddressFile->Irp != NULL) {
		PIRP TempIrp=AddressFile->Irp;

		AddressFile->Irp = NULL;
		RELEASE_SPIN_LOCK (&Address->SpinLock, oldirql);
		TempIrp->IoStatus.Information = 0;
		TempIrp->IoStatus.Status = STATUS_DUPLICATE_NAME;

		IoCompleteRequest (TempIrp, IO_NETWORK_INCREMENT);
...

{
	...
	IF_LPXDBG (LPX_DEBUG_ADDRESS | LPX_DEBUG_PNP) {
		DebugPrint(2, ("LpxCloseAddress AF %p:\n", addressFile) );
	}
