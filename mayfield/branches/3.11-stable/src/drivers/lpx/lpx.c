	...
	case SMP_ESTABLISHED:
		{
			...
			while (userDataLength) 
			{
				...
				status = TransmitDataPacket(deviceContext, ServicePoint, packet, copy);
...
/*		RtlCopyMemory((PUCHAR)&protocol, &((PUCHAR)LookAheadBuffer)[18], sizeof(USHORT));
	...
	if (ulPacketDropRate) {
		LONG DropCount;
		DropCount = InterlockedIncrement(&ulPacketCountForDrop);	
		if(DropCount % 1000 < (LONG)ulPacketDropRate ) {
			            ...
			            )
{
	...
	if(LpxAllocateMemoryWithLpxTag(&lpxHeader, sizeof(LPX_HEADER2)) != NDIS_STATUS_SUCCESS) {
