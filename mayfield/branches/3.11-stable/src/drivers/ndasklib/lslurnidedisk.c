...
//			ASSERT(Lurn->LurnStatus == LURN_STATUS_STALL);
	...
	if(Lurn->Lur->LowestHwVer > LurnDesc->LurnIde.HWType) {
		Lurn->Lur->LowestHwVer = LurnDesc->LurnIde.HWType;
	}
...
)
{
	...
	for(transferedBlocks = 0; transferedBlocks < transferBlocks; transferedBlocks += splitBlocks)
	{
		UINT32 DataBufferLength = (transferBlocks - transferedBlocks <= splitBlocks) ? transferBlocks - transferedBlocks : splitBlocks;
		UINT32 RetryCount = 0;
retry:
		LURNIDE_INITIALIZE_PDUDESC(
			IdeDisk, 
		...
		KDPrintM(DBG_LURN_NOISE,
			...
			logicalBlockAddress + transferedBlocks, DataBufferLength));
		...
		{
			// We need to check written data is correct.
			LURNIDE_INITIALIZE_PDUDESC(
				IdeDisk, 
				&PduDesc, 
				IDE_COMMAND, 
			...
			if (NT_SUCCESS(ntStatus) && response == LANSCSI_RESPONSE_SUCCESS) {
				...
				if (RtlCompareMemory(
					...
					RetryCount++;
					if (RetryCount>3) {
						DbgPrint("Write-check failed. Stop retrying\n");
/*						KDPrintM(DBG_LURN_ERROR,
								("Write-check failed. Stop retrying\n"));*/
					} else {
						DbgPrint("Write-check failed. Retrying\n");
/*						KDPrintM(DBG_LURN_ERROR,
