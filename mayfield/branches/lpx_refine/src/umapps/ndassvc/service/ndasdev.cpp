...
{
	static const BYTE V1_SAMPLE[8] = {0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00};
	static const BYTE V1_RETAIL[8] = {0xbb,0xea,0x30,0x15,0x73,0x50,0x4a,0x1f};
	static const BYTE V1_RUTTER[8] = {0xbb,0xea,0x30,0x15,0x73,0x50,0x4a,0x1f};

	C_ASSERT(sizeof(V1_SAMPLE) == sizeof(NDAS_OEM_CODE));
	C_ASSERT(sizeof(V1_RETAIL) == sizeof(NDAS_OEM_CODE));
	C_ASSERT(sizeof(V1_RUTTER) == sizeof(NDAS_OEM_CODE));
	...
	{
		return reinterpret_cast<const NDAS_OEM_CODE&>(V1_SAMPLE);
	} 
	//
	...
	{
		return reinterpret_cast<const NDAS_OEM_CODE&>(V1_RUTTER);
	}
	//
	// Retail Devices
	//
	...
	{
		return reinterpret_cast<const NDAS_OEM_CODE&>(V1_RETAIL);
	}
}
...
#define IS_FLAG_SET(flags, target) ((flags & target) == (target))
...
CNdasDevice::CNdasDevice(
	...
	m_OemCode(GetDefaultNdasOemCode(deviceId))
{
...
void 
CNdasDevice::SetOemCode(const NDAS_OEM_CODE* pCode)
{
	if (NULL == pCode)
	{
		m_OemCode = GetDefaultNdasOemCode(m_deviceId);
