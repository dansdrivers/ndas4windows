	...
	IN CONST NDAS_DEVICE_REGISTRATIONW* Registration)
{
	...
	PASSERT_STR_PTRW(Registration->DeviceName, -1);

	NDAS_CMD_REGISTER_DEVICE::REQUEST buffer; 
	NDAS_CMD_REGISTER_DEVICE::REQUEST* pRequest = &buffer; 
	NDAS_CMD_REGISTER_DEVICE::REPLY* pReply; 
	...
	if (!fConnected) 
	{
		DBGPRT_ERR_EX(_FT("Service Connection Failure: "));
		::SetLastError(NDASUSER_ERROR_SERVICE_CONNECTION_FAILURE); 
		return 0; 
	} 
	...
	{ 
		...
		if (NDAS_CMD_STATUS_SUCCESS != cmdStatus) 
		{ 
			DBGPRT_ERR(_FT("Command failed with error code: %08X\n"), cmdStatus); 
	...
	// Process the result here
	//
	DBGPRT_INFO(_FT("Registered at slot %d.\n"), pReply->SlotNo);
	...
	IN CONST NDAS_DEVICE_REGISTRATIONA* Registration)
{
	...
	PASSERT_STR_PTRA(Registration->DeviceName, -1);
...
BOOL
NdasUnregisterDeviceImpl(const NDAS_DEVICE_ID_OR_SLOT& DeviceIdOrSlot)
{
...
BOOL 
NdasEnableDeviceImpl(const NDAS_DEVICE_ID_OR_SLOT& DeviceIdOrSlot, BOOL bEnable)
{
...
NdasSetDeviceNameImplW(
	const NDAS_DEVICE_ID_OR_SLOT& DeviceIdOrSlot, 
	LPCWSTR lpszDeviceName)
{
	CHECK_STR_PTRW(lpszDeviceName, -1);
	NDAS_DEFINE_TYPES(NDAS_CMD_SET_DEVICE_PARAM,lpRequest,lpReply,lpErrorReply,scc);
	...
	LPCSTR lpszDeviceStringKey)
{
	...
	// Prepare the parameters here
	lpRequest->DeviceIdOrSlot.bUseSlotNo = FALSE;
	BOOL fSuccess = NdasIdStringToDeviceA(
		lpszDeviceStringId,
		&lpRequest->DeviceIdOrSlot.DeviceId);
	_ASSERT(fSuccess); // should not fail as we already did Validation
	...
	NDAS_UNITDEVICE_ERROR* pLastError)
{
	...
	CHECK_WRITE_PTR(pLastError, sizeof(NDAS_UNITDEVICE_ERROR));
...
NdasQueryUnitDeviceInformationImplW(
	const NDAS_DEVICE_ID_OR_SLOT& DeviceIdOrSlot, DWORD dwUnitNo, 
