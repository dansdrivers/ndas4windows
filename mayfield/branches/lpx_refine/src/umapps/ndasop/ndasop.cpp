...
#define SET_CONNECTION_INFO_BY_DIB(IT, DIB, WRITE_ACCESS, CONN_INFO) \
	...
	(CONN_INFO).Size = sizeof(NDASCOMM_CONNECTION_INFO), \
	(CONN_INFO).AddressType = NDASCOMM_CIT_DEVICE_ID, \
	(CONN_INFO).WriteAccess = (WRITE_ACCESS), \
	(CONN_INFO).PrivilegedOEMCode.UI64Value = 0, \
	(CONN_INFO).LoginType = NDASCOMM_LOGIN_TYPE_NORMAL, \
	(CONN_INFO).OEMCode.UI64Value = 0, \
	(CONN_INFO).Protocol = NDASCOMM_TRANSPORT_LPX, \
	::CopyMemory((CONN_INFO).DeviceId.Node, (DIB).UnitDisks[(IT)].MACAddr, sizeof((CONN_INFO).DeviceId.Node)), \
	...
	)
{
	...
	for(i = 0; i < nDiskCount; i++)
	{
		...
		if(NMT_SINGLE != BindType)
		{
			...
			{
				C_ASSERT(sizeof(pConnectionInfo[j].DeviceId.Node) ==
					sizeof(DIBs_V2[i].UnitDisks[j].MACAddr));

				CopyMemory(
					DIBs_V2[i].UnitDisks[j].MACAddr, 
					pConnectionInfo[j].DeviceId.Node, 
					sizeof(DIBs_V2[i].UnitDisks[j].MACAddr));
	...
	CONST UINT32 nReplace)
{
	...
	::CopyMemory(
		...
		ci_replace->DeviceId.Node, 
		sizeof(ci_replace->DeviceId.Node));
	C_ASSERT(sizeof(ci_replace->DeviceId.Node) == 
	...
	CONST NDASCOMM_CONNECTION_INFO *ciSpare)
{
	...
	::CopyMemory(
		...
		ciSpare->DeviceId.Node, 
		sizeof(ciSpare->DeviceId.Node));
...
)
{
	...
	for(i = 0; i < nTotalDiskCount; i++)
	{
		C_ASSERT(sizeof(DIB_V2.UnitDisks[i].MACAddr) ==
			sizeof(ci_spare->DeviceId.Node));
		if(!memcmp(DIB_V2.UnitDisks[i].MACAddr, ci_spare->DeviceId.Node, 
