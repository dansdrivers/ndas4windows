...
namespace 
{
	...
	NdasHandleToContext(HNDAS hNdasDevice)
	{
	...
	case NDASCOMM_CIT_DEVICE_ID:
		{
			C_ASSERT(sizeof(pUnitDeviceID->DeviceId.Node) ==
				sizeof(pConnectionInfo->DeviceId.Node));

			::CopyMemory(
				pUnitDeviceID->DeviceId.Node, 
				pConnectionInfo->DeviceId.Node, 
	...
	case NDASCOMM_CIT_NDAS_IDW:
		{
			...
			::CopyMemory(wszID, pConnectionInfo->NdasIdW.Id, sizeof(WCHAR) * 20);
			::CopyMemory(wszKey, pConnectionInfo->NdasIdW.Key, sizeof(WCHAR) * 5);
	...
	case NDASCOMM_CIT_NDAS_IDA:
		{
			...
			::CopyMemory(szID, pConnectionInfo->NdasIdA.Id, 20);
			::CopyMemory(szKey, pConnectionInfo->NdasIdA.Key, 5);
	...
	CONST VOID* hint)
{
	...
	ASSERT_PARAM(
		...
		NDASCOMM_LOGIN_TYPE_DISCOVER == pConnectionInfo->LoginType);
	...
	else
	{
		...
		// only 1.1 and 2.0 requires this
		if (1 == hardwareVersion || 2 == hardwareVersion)
		{
...
NdasCommImpIsHandleValidForRW(
	HNDAS hNdasDevice)
{
	if (!NdasCommImpIsHandleValid(hNdasDevice))
	{
		return FALSE;
	}
	...
	OUT OPTIONAL LPDWORD lpdwCounter)
{
	...
	ASSERT_PARAM( NULL == lpdwCounter || ! ::IsBadWritePtr(lpdwCounter, sizeof(DWORD)) );
	...
	IN CONST NDASCOMM_CONNECTION_INFO* ConnInfo)
{
	...
	APITRACE( hNdasDevices[0] = NdasCommConnect(&ci, 1000, NULL) );
	...
	{
		APITRACE( hNdasDevices[i] = NdasCommConnect(&ci, 1000, NULL) );
	}
