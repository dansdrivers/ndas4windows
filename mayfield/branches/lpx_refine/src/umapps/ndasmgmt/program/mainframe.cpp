...
class CFootprintWindow :
	public CWindowImpl<
		CFootprintWindow, 
		CWindow, 
		CWinTraits<WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, WS_EX_TOOLWINDOW> >
{
public:
	...
	CFootprintWindow(HWND hParent = NULL)
	{
		...
		int cx = ::GetSystemMetrics(SM_CXVIRTUALSCREEN) + 1,
			cy = ::GetSystemMetrics(SM_CYVIRTUALSCREEN) + 1;
...
CMainFrame::OnCreate(LPCREATESTRUCT lParam)
{
	...
	fSuccess = pGetAppConfigValue(_T("UseHotKey"),&fUseHotkey);
	
	if (fSuccess && fUseHotkey)
	{
		fSuccess = ::RegisterHotKey(
			m_hWnd, 
			NDASMGMT_POPUP_HOTKEY, 
			MOD_WIN, 
			'N' /* VK_N */);
		if (fSuccess) m_fHotKeyRegistered = TRUE;
	}

	CFootprintWindow* fp = new CFootprintWindow(::GetDesktopWindow());
...
CMainFrame::OnDestroy()
{
	...
	{
		fSuccess = ::UnregisterHotKey(m_hWnd, NDASMGMT_POPUP_HOTKEY);
		ATLASSERT(fSuccess);
	}
...
CMainFrame::OnMountDeviceRW(NDAS_LOGICALDEVICE_ID logDevId)
{
	...
	if (!fSuccess && 
		NDASSVC_ERROR_MULTIPLE_WRITE_ACCESS_NOT_ALLOWED == ::GetLastError())
	{
...
VOID 
CMainFrame::OnNdasLogDevAlarmed(
	NDAS_LOGICALDEVICE_ID logDevId,
	ULONG AdapterStatus)
{
	...
	pCreateLogicalDeviceString(szDevices, logDevId);
	CString strInfoFmt, strInfo, strInfoTitle;
	strInfoTitle.LoadString(IDS_BT_ALARMED_INFO_TITLE);

	if(NDAS_DEVICE_ALARM_RECONNECTIONG == AdapterStatus)
	{
		strInfoFmt.LoadString(IDS_BT_ALARMED_RECONNECTING);
	}
	else if(NDAS_DEVICE_ALARM_RECONNECTED == AdapterStatus)
	{
		strInfoFmt.LoadString(IDS_BT_ALARMED_RECONNECTED);
	}
	else if(NDAS_DEVICE_ALARM_MEMBER_FAULT == AdapterStatus)
	{
		strInfoFmt.LoadString(IDS_BT_ALARMED_MEMBER_FAULT);
	}
	else if(NDAS_DEVICE_ALARM_RECOVERING == AdapterStatus)
	{
		strInfoFmt.LoadString(IDS_BT_ALARMED_RECOVERING);
	}
	else if(NDAS_DEVICE_ALARM_RECOVERRED == AdapterStatus)
	{
		strInfoFmt.LoadString(IDS_BT_ALARMED_RECOVERRED);
	}
...
LRESULT 
CMainFrame::OnTaskBarMenu(LPARAM uMsg, BOOL& bHandled)
{
...
CMainFrame::OnHotKey(int id, UINT fsModifier, UINT vk)
{
	...
	BOOL bHandled;
	CTaskBarIconExT<CMainFrame>::OnContextMenu(0, bHandled);
