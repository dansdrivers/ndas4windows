...
LSLURNASSOC_INLINE
PCHAR 
NDAS_RAID_INFO_BUFFER(
	PRAID_INFO RaidInfo, 
	SIZE_T Index
	)
{
	if (NDAS_RAID_META_DATA_SIGNATURE == RaidInfo->rmd.Signature)
	{
		return RaidInfo->DataBuffers[RaidInfo->rmd.UnitMetaData[Index].iUnitDeviceIdx];
	}
	else
	{
		return RaidInfo->DataBuffers[Index];
	}
}
