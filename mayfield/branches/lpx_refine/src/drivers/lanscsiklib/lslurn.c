...
LurModifyConfiguration(
	...
	if(	(LurDesc->LurOptions & LUROPTION_OFF_UDMA_TO3) &&
		(LurDesc->LurOptions & LUROPTION_ON_UDMA_TO3) ) {

			return;
	}

	if(LurDesc->LurOptions & LUROPTION_OFF_UDMA_TO3) {
		Lur->LurFlags &= ~LURFLAG_LOWERUDMA_TO3;
	} else if(LurDesc->LurOptions & LUROPTION_ON_UDMA_TO3) {
		Lur->LurFlags |= LURFLAG_LOWERUDMA_TO3;
	}
	...
	if(Lur->LurFlags & LURFLAG_SHAREDWRITE) {
		...
		ASSERT(!(Lur->LurFlags & LURFLAG_WRITESHARE_PS));
