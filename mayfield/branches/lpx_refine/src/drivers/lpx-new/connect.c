...

{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (fileObject->DeviceObject == &deviceContext->DeviceObject) {
            ...
            IF_LPXDBG (LPX_DEBUG_CONNECT) {
                LpxPrint3 ("LpxTdiAssociateAddress: Connection %lx Irp %lx AddressFile %lx \n",
            ...
            if ((fileObject->FsContext2 == (PVOID) TDI_TRANSPORT_ADDRESS_FILE) &&
                ...
                if ((addressFile->Address->Flags & ADDRESS_FLAGS_STOPPING) == 0) {
                    ...
                    IF_LPXDBG (LPX_DEBUG_CONNECT) {
                        LpxPrint2 ("LpxTdiAssociateAddress: Valid Address %lx %lx\n",
...

{
    ...
    IF_LPXDBG (LPX_DEBUG_CONNECT) {
        LpxPrint1 ("LpxOpenConnection: Opened Connection %lx.\n",
              connection);
    }
...

{
    ...
    if ((connection->Flags2 & CONNECTION_FLAGS2_CLOSING) != 0) {
        RELEASE_SPIN_LOCK (&connection->TpConnectionSpinLock, irql);
        LpxPrint1("Closing already-closing connection %lx\n", connection);
        return STATUS_INVALID_CONNECTION;
    }
    RELEASE_SPIN_LOCK (&connection->TpConnectionSpinLock, irql);
...

{
    ...
    IF_LPXDBG (LPX_DEBUG_DYNAMIC) {
        LpxPrint1 ("ExAllocatePool Connection %08x\n", Connection);
    }
...

{
    ASSERT( LpxControlDeviceContext == DeviceContext) ;
    IF_LPXDBG (LPX_DEBUG_DYNAMIC) {
        LpxPrint1 ("ExFreePool Connection: %08x\n", TransportConnection);
    }
...

{
    ...
    IF_LPXDBG (LPX_DEBUG_CONNECT) {
        LpxPrint1 ("LpxCreateConnection:  Connection at %lx.\n", Connection);
    }
...

{
    ...
    IF_LPXDBG (LPX_DEBUG_CONNECT) {
        LpxPrint1 ("LpxDestroyAssociation:  Entered for connection %lx.\n",
                    TransportConnection);
    }
...

{
    ...
    IF_LPXDBG (LPX_DEBUG_CONNECT) {
        LpxPrint1 ("LpxDestroyConnection:  Entered for connection %lx.\n",
                    TransportConnection);
    }
    DebugPrint(3, ("!!!!!!CALL LpxDestroyConnection:   for connection %lx.\n",
                    TransportConnection));

#if DBG
     if (!(TransportConnection->Flags2 & CONNECTION_FLAGS2_STOPPING)) {
        LpxPrint1 ("attempt to destroy unstopped connection 0x%lx\n", TransportConnection);
        DbgBreakPoint ();
    }
 #endif
    ...
    if (CloseIrp != (PIRP)NULL) {
        ...
        RELEASE_SPIN_LOCK (&TransportConnection->TpConnectionSpinLock, oldirql);    
        LpxPrint1("Connection %x destroyed, no CloseIrp!!\n", TransportConnection);
    }
...

{
    ...
    IF_LPXDBG (LPX_DEBUG_CONNECT) {
        LpxPrint2 ("LpxReferenceConnection: entered for connection %lx, "
                    "current level=%ld.\n",
                    TransportConnection,
...

{
    ...
    IF_LPXDBG (LPX_DEBUG_CONNECT) {
        LpxPrint2 ("LpxDereferenceConnection: entered for connection %lx, "
                    "current level=%ld.\n",
                    TransportConnection,
                    TransportConnection->ReferenceCount);
    }
...

{
    ...
    IF_LPXDBG (LPX_DEBUG_CONNECT) {
        LpxPrint3 ("LpxDereferenceConnectionSpecial: entered for connection %lx, "
                    "current level=%ld (%ld).\n",
                    TransportConnection,
                    TransportConnection->ReferenceCount,
