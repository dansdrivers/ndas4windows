...
//		ConfigInfo->MaximumTransferLength = HwDeviceExtension->MaxBlocksPerRequest * BLOCK_SIZE;
		ConfigInfo->MaximumTransferLength = 256 * BLOCK_SIZE;
...
MiniStopAdapter(
	...
	if(ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_POWERSAVING_PENDING)) {
		...
		NDScsiLogError(
			...
			0,
			0,
			NDASSCSI_IO_STOPREQ_WHILE_POWERSAVING,
	...
	case SCSIOP_INQUIRY: {
		...
		if(ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_POWERSAVING_PENDING)) {
			...
			NDScsiLogError(
				...
				Srb->TargetId,
				Srb->Lun,
				NDASSCSI_IO_INQUIRY_WHILE_POWERSAVING,
	...
	case SCSIOP_START_STOP_UNIT: {
		...
		if((int)cdb->START_STOP.LoadEject == 0) {
			...
			if(cdb->START_STOP.Start == STOP_UNIT_CODE) {
				...
				if(ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_POWERSAVING_PENDING)) {
					KDPrint(1,("SCSIOP_START_STOP_UNIT: STOP_UNIT_CODE! WARNING!!!!!: ADAPTER_STATUSFLAG_POWERSAVING_PENDING has been set already.\n"));
					NDScsiLogError(
						HwDeviceExtension,
						Srb,
						Srb->PathId,
						Srb->TargetId,
						Srb->Lun,
						NDASSCSI_IO_DOUBLE_ENTER_POWERSAVING,
				...
				if(ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_POWERSAVING_PENDING)) {
					...
					RELEASE_SPIN_LOCK(&HwDeviceExtension->LanscsiAdapterSpinLock, oldIrql);
					KDPrint(1,("SCSIOP_START_STOP_UNIT: START_UNIT_CODE! WARNING!!!!!: ADAPTER_STATUSFLAG_POWERSAVING_PENDING has been reset already Will not forward to LURNs!\n"));
					...
					NDScsiLogError(
						...
						Srb->TargetId,
						Srb->Lun,
						NDASSCSI_IO_DOUBLE_EXIT_POWERSAVING,
	...
	default: {
		...
		if(ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_POWERSAVING_PENDING)) {
			...
			NDScsiLogError(
				...
				Srb->TargetId,
				Srb->Lun,
				NDASSCSI_IO_SCSIOP_WHILE_POWERSAVING,
    ...
    )
{
	...
	if(Srb->Cdb[0] != SCSIOP_READ 
		...
		KDPrint(1,("SCSIOP = 0x%x %s hwDeviceExtension->RequestExecuting = %x\n",
