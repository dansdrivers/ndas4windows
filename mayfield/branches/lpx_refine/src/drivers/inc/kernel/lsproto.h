...
//
typedef NTSTATUS (*LSPROTO_SENDREQUEST)(PLANSCSI_SESSION LSS, PLANSCSI_PDU_POINTERS Pdu);
typedef NTSTATUS (*LSPROTO_READREPLY)(PLANSCSI_SESSION LSS, PCHAR Buffer, PLANSCSI_PDU_POINTERS Pdu);
...
typedef struct _LANSCSI_SESSION {
	...
	//
	// write size control information	
	//
	TDI_SEND_RESULT SendResult;
	UINT32			NrWritesAfterLastRetransmits;
	UINT32			SplitSize; // 128, 64, 32, 16, 8
