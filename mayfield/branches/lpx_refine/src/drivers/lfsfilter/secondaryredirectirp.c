...
/*
//
//	Do not clear the rest of output buffer that is not filled with data.
//	E.G) teamup.exe ( reported by Bert Mercier)
//
			...
			if(returnedDataSize)
			{
				...
				if(IRP_MJ_QUERY_VOLUME_INFORMATION == irpSp->MajorFunction) {
					...
					if(queryVolume->FsInformationClass == FileFsAttributeInformation) {
						PFILE_FS_ATTRIBUTE_INFORMATION info = (PFILE_FS_ATTRIBUTE_INFORMATION)secondaryRequest->OutputBuffer;
						info->FileSystemAttributes &= ~(FILE_PERSISTENT_ACLS);
						SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_ERROR, ("SECONDARY: FileFsAttributeInformation: turn off FILE_PERSISTENT_ACLS\n"));
					}
				}
#endif
			}
