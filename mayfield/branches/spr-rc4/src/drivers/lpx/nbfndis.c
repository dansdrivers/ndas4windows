    ...
    if (NdisStatus == STATUS_SUCCESS) {
        ...
        if (NdisRequest2->RequestType == NdisRequestSetInformation) {
            NbfPrint2 ("Nbfdrvr: Set OID %lx failed: %s.\n",
                NdisRequest2->DATA.SET_INFORMATION.Oid, NbfGetNdisStatus(NdisStatus));
        } else {
            NbfPrint2 ("Nbfdrvr: Query OID %lx failed: %s.\n",
                NdisRequest2->DATA.QUERY_INFORMATION.Oid, NbfGetNdisStatus(NdisStatus));
        }
#endif
        if (NdisStatus != STATUS_INVALID_DEVICE_STATE) {
            ...
            NbfWriteOidErrorLog(
                DeviceContext,
                NdisRequest2->RequestType == NdisRequestSetInformation ?
...
//	DbgPrint("SelectedMedium is 0x%x\n", SelectedMedium);
