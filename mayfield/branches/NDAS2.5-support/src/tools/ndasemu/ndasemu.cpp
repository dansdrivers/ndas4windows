...
typedef struct _PROM_DATA {
	unsigned _int32 MaxConnTime;
	unsigned _int32 MaxRetTime;
...
unsigned _int8	HeaderEncryptAlgo = 0;
unsigned _int8	DataEncryptAlgo = 0;
...
//	PLANSCSI_R2H_PDU_HEADER	pReplyHeader;
	...
	while(pSessionData->iSessionPhase != LOGOUT_PHASE) {
		...
		if (pdu.pH2RHeader->Opcode!=IDE_COMMAND && pdu.pH2RHeader->Opcode!=NOP_H2R) {
			if (OptVerbose >=2)
		...
		case LOGIN_REQUEST:
			{
				...
				case 0:
					{
						...
						if (EiHangFirstLogin) {
							fprintf(stderr, "Hang after first login. Waiting 60 seconds before disconnect\n");
							Sleep(60 * 1000);
							goto EndSession;
						}
						// Check Parameter.
				...
				case 2:
					{						
						...
						if((unsigned)ntohl(pAuthChapParam->CHAP_I) != pSessionData->EncryptInfo.CHAP_I) {
...
//						pParamNego->HWVersion = HW_VERSION_CURRENT;
						...
						pParamNego->HeaderEncryptAlgo = HeaderEncryptAlgo;
						pParamNego->HeaderDigestAlgo = 0;
						pParamNego->DataEncryptAlgo = DataEncryptAlgo;
				...
				case BIN_PARAM_TYPE_TARGET_LIST:
					{
						...
						for(int i = 0; i < pParam->NRTarget; i++) {
							...
							pParam->PerTarget[i].v1.NRRWHost = PerTarget[i].NRRWHost;
							pParam->PerTarget[i].v1.NRROHost = PerTarget[i].NRROHost;
			...
			if (thisHWVersion == LANSCSIIDE_VERSION_1_0) {
				...
				if(pRequestHeader->DataTransferLength) {
					...
					if(	pRequestHeader->Command == WIN_IDENTIFY ||
						...
						dataTransferLength = (((unsigned)pRequestHeader->SectorCount_Prev << 8)
							+ (pRequestHeader->SectorCount_Curr)) * 512;
					}
				}
				...
				bRet = NdemuDevRequest(targetDev, &ataCommand);
				if (bRet == FALSE) {
		...
		case VENDOR_SPECIFIC_COMMAND:
			{
				...
				switch(pRequestHeader->VendorOp) {
					case VENDOR_OP_SET_MAX_RET_TIME:
						Prom.MaxRetTime = NTOHL(pRequestHeader->VendorParameter1);
						break;
					case VENDOR_OP_SET_MAX_CONN_TIME:
						Prom.MaxConnTime = NTOHL(pRequestHeader->VendorParameter1);
						break;
					case VENDOR_OP_GET_MAX_RET_TIME:
						pRequestHeader->VendorParameter0 = 0;
						pRequestHeader->VendorParameter1 = HTONL(Prom.MaxRetTime);
						break;
					case VENDOR_OP_GET_MAX_CONN_TIME:
						pRequestHeader->VendorParameter0 = 0;
						pRequestHeader->VendorParameter1 = HTONL(Prom.MaxConnTime);
						break;
...
main(int argc, char* argv[])
{
	...
	if (argc>1) {
		...
		for(i=1;i<argc;i++) {
			...
			if (argv[i][0] == 'v' || argv[i][0] == 'V') {
				OptVerbose = argv[i][1] - '0';
