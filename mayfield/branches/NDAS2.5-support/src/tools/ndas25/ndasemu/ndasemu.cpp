...
unsigned char HostMacAddr[] = { 0x00, 0x08, 0x74, 0x4f, 0xf2, 0x97 }; // BJR
...
// Global Variable.
_int16			G_RPID = 0;
unsigned _int8	thisHWVersion = LANSCSIIDE_VERSION_2_5;
	...
	case 1:
		{
			...
			if(!(ntohl(pAuthChapParam->CHAP_A) & HASH_ALGORITHM_AES128)) {
			...
			case LOGIN_TYPE_NORMAL:
				{
					...
					switch(Permission) {
						case USER_PERMISSION_EW:	PerTarget[0].NREWHost++; break;
						case USER_PERMISSION_SW:	PerTarget[0].NRSWHost++; break;
						case USER_PERMISSION_RO:	PerTarget[0].NRROHost++; break;
					}
	...
	case BIN_PARAM_TYPE_TARGET_LIST:
		{
			...
			for(int i = 0; i < pParam->NRTarget; i++) {
				...
				pParam->PerTarget[i].NRRWHost = PerTarget[i].NRSWHost;
...
main(int argc, char* argv[])
{
	...
	if(PerTarget[0].Export < 0) {
		...
		memset(buffer, 0, 512);
		loc = _lseeki64(PerTarget[0].Export, DEFAULT_DISK_SIZE - 512, SEEK_END);
	...
	for(i=0; i<NR_MAX_TARGET;i++) {
		...
		PerTarget[i].NRSWHost = 0;
		PerTarget[i].NRROHost = 0;
		PerTarget[i].TargetData = 0;
		PerTarget[i].Size = DEFAULT_DISK_SIZE;
