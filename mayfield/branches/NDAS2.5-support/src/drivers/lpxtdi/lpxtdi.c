...
LpxTdiIoCallDriver(
	...
	if (TimeOut) {
		LtDebugPrint(0, ("[LpxTdi] LpxTdiIoCallDriver: Irp->Tail.Overlay.DriverContext[0] = %x, [1]=%x\n",
...
//
//	Completion routine for send/recv routine
//
...
IrpCompletionForUserNotification(
	...
	if(overlapped) {
		if(overlapped->IoCompleteRoutine)
			overlapped->IoCompleteRoutine(overlapped, Irp->UserIosb);
	}
...
#if DBG
	if(ntStatus != STATUS_SUCCESS) {
		LtDebugPrint(1, ("[LpxTdi]TdiConnect: Failed. STATUS=%08lx\n", ntStatus));
	}
#endif
	...
	)
{
	...
	if(ntStatus == STATUS_SUCCESS) {
		*Result = (ULONG)ioStatusBlock.Information;
		...
		LtDebugPrint(1, ("[LpxTdi] LpxTdiSendDataGram: Failed. STATUS=%08lx\n", ntStatus));
		*Result = 0;
	}
