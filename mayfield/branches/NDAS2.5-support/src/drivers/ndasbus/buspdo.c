			       ...
			       )
{
	...
	//
	if((!Packatus = Bus_PDO_QueryDeviceId(DeviceData, Irp);
...
#ifndef __USE_REAL_RESOURCE__
	descriptor->Type = CmResourceTypeNull;
	descriptor->ShareDisposition = 0;
	descriptor->Flags = 0;
	descriptor->u.Port.Length = 0;
	descriptor->u.Port.Alignment = 0;
	descriptor->u.Port.MinimumAddress.QuadPart = 0;
	descriptor->u.Port.MaximumAddress.QuadPart = 0;
#else
...
/*	
	...
	descriptor->u.BusNumber.MaxBusNumber = 15;
*/
    Irp->IoStatus.Information = (ULONG_PTR)resourceList;
    status = STATUS_SUCCESS;    
    return status;
