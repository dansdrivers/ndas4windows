	...
	)
{
	...
	ioStatusBlock.Information = 0;
	if (OpenFile->CreateOptions & FILE_DIRECTORY_FILE) {
...
//		ULONG				originalFileSpyDebugLevel; 
    ...
    case IRP_MJ_CREATE: //0x00
	{
		...
		do 
		{
			...
			} else if (createStatus == STATUS_ACCESS_DENIED) {
				//
				// to do: If file attribute has changed after file opening, file re-open can be failed.
				//	ex. File has changed to read-only after opened for writing.
				//		We need to handle this case...
				//
				SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
					("DispatchWinXpRequest: IoCreateFile status %x\n", createStatus));
				break;
			} else {
				SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
					("DispatchWinXpRequest: IoCreateFile status %x\n", createStatus));
...
//		BOOLEAN					closeOnExit = FALSE;
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_READ\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
...
RETRY_WRITE:
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_WRITE\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			writeStatus = STATUS_ACCESS_DENIED;
			ioStatusBlock.Information = 0;
...
write_out:
		...
		*replyDataSize = 0;
		...
		if(writeStatus == STATUS_ACCESS_DENIED)
		{
			...
			else if(openFile->AlreadyClosed == TRUE) 
			{
				ASSERT(LFS_UNEXPECTED);
    ...
    case IRP_MJ_QUERY_INFORMATION: // 0x05
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_QUERY_INFORMATION\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			queryInformationStatus = STATUS_ACCESS_DENIED;
			returnedLength = 0;
...
query_out:		
    ...
    case IRP_MJ_SET_INFORMATION:  // 0x06
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_SET_INFORMATION\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			setInformationStatus = STATUS_ACCESS_DENIED;
		...
		else if(NdfsWinxpRequestHeader->SetFile.FileInformationClass == FileDispositionInformation) 
		{
			...
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
						("DispatchWinXpRequest: DeleteFile on close %d (FileName= %ws)\n",
							dispositionInformation->DeleteFile, openFile->FullFileNameBuffer));
...
set_info_out:		
	...
	case IRP_MJ_DIRECTORY_CONTROL: // 0x0C
	{
        ...
        if(NdfsWinxpRequestHeader->IrpMinorFunction == IRP_MN_QUERY_DIRECTORY) 
		{
			...
			if (openFile == NULL) {
				SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
						("DispatchWinXpRequest: Reopen failed while IRP_DIRECTORY_CONTROL\n"));
			
				// Failed to reopen file.
				// We should prevent this case, but currently it happens. 
				// Return error until we fix it.
				queryDirectoryStatus = STATUS_ACCESS_DENIED;
				ioStatusBlock.Information = 0;
			
				goto dir_control_out;
			}
...
dir_control_out:
	...
	case IRP_MJ_FILE_SYSTEM_CONTROL: // 0x0D
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_FILE_SYSTEM_CONTROL\n"));
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.

			ioStatusBlock.Information = 0;
			fileSystemControlStatus = STATUS_ACCESS_DENIED;
...
fs_control_out:
	...
	//	case IRP_MJ_INTERNAL_DEVICE_CONTROL:  // 0x0F 
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_DEVICE_CONTROL\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			deviceControlStatus = STATUS_ACCESS_DENIED;
			ioStatusBlock.Information = 0;
			outputBufferLength = 0;
			goto dev_control_out;
		}			
#endif
		ASSERT(openFile && openFile->FileObject);
		//ASSERT(openFile && openFile->OpenFileId == (_U32)openFile);
...
dev_control_out:
	...
	case IRP_MJ_LOCK_CONTROL: // 0x11
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_LOCK_CONTROL\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			lockControlStatus = STATUS_ACCESS_DENIED;
...
lock_ctrl_out:		
    ...
    case IRP_MJ_QUERY_SECURITY:
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_QUERY_SECURITY\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			querySecurityStatus = STATUS_ACCESS_DENIED;
...
query_sec_out:
    ...
    case IRP_MJ_SET_SECURITY:
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_SET_SECURITY\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			setSecurityStatus = STATUS_ACCESS_DENIED;
			length = 0;
			goto set_sec_out;
		}				
#endif
...
set_sec_out:
	...
	case IRP_MJ_QUERY_EA:
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_QUERY_EA\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			queryEaStatus = STATUS_ACCESS_DENIED;
			goto query_ea_out;
		}				
#endif
		ASSERT(openFile && openFile->FileObject);
...
query_ea_out:
    ...
    case IRP_MJ_SET_EA:
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_SET_EA\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			setEaStatus = STATUS_ACCESS_DENIED;
			ioStatusBlock.Information = 0;
			goto set_ea_out;
		}				
#endif
...
set_ea_out:
	...
	case IRP_MJ_QUERY_QUOTA:
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_QUERY_QUOTA\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			queryQuotaStatus = STATUS_ACCESS_DENIED;
			goto query_quota_out;
		}					
#endif
		ASSERT(openFile && openFile->FileObject);
		//ASSERT(openFile && openFile->OpenFileId == (_U32)openFile);
...
query_quota_out:
	...
	case IRP_MJ_SET_QUOTA:
	{
		...
		if (openFile == NULL) {
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
					("DispatchWinXpRequest: Reopen failed while IRP_MJ_SET_QUOTA\n"));
		
			// Failed to reopen file.
			// We should prevent this case, but currently it happens. 
			// Return error until we fix it.
			setQuotaStatus = STATUS_ACCESS_DENIED;
			ioStatusBlock.Information = 0;
			goto set_quota_out;
		}			
#endif
...
//	ASSERT(NdfsWinxpReplytHeader->Status != STATUS_INVALID_HANDLE); // this can happen...
	...
	if(openFile->AlreadyClosed == TRUE)
	{
		...
		while(1) {
			...
			if(fileHandle == NULL)
			{
				...
				if (RetryCount > 3) {
					SPY_LOG_PRINT(LFS_DEBUG_PRIMARY_INFO,
						("PrimarySession_FindOrReopenOpenFile: Gave up reopening\n"));	
					ASSERT(LFS_UNEXPECTED);
					return NULL;
				}
