	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
		("SecondaryThreadProc: PriMaxData:%08u SecMaxData:%08u\n",
		Secondary->Thread.SessionContext.PrimaryMaxDataSize,
		Secondary->Thread.SessionContext.SecondaryMaxDataSize));
...
#define CLEANUP_FILEEXT_COUNT	10
				...
				) 
			{
				...
				else if(secondaryRequest->RequestType == SECONDARY_REQ_SEND_MESSAGE)
				{
					...
					do 
					{
...
#ifdef __PROCESS_SECONDARY_REQUEST_LOCALLY__
						//
						//	Try to process a request locally
						//	NOTE: To allow RAID module of NDASSCSI to recover
						//			fault, disable local processing of IOs.
						//
...
#endif
						//
						//	Allocate an empty slot for a secondary request.
						//
						...
						//
						//	Send the request message to the primary.
						//
						...
						if(Secondary->Thread.SessionContext.RequestsPerSession > 1)
						{
							...
							if(InterlockedIncrement(&Secondary->Thread.WaitReceive) == 1)
							{
								...
								{
									ASSERT(LFS_BUG);
									secondaryRequest->ExecuteStatus = tdiStatus;
						...
						//
						//	Receive the reply message including data
						//
						...
						//
						//	Quit this secondary thread if failure.
						//
						...
						//
						//	Set the reply status to the secondary request
						//
				...
				//
				//	Complete the secondary request if failure.
				//
		...
		{
			//
			//	Recv completion event signaled
			//
			NTSTATUS			processStatus;
			PSECONDARY_REQUEST	secondaryRequest;
			...
			//
			//	If received header has wrong length,
			//	Notify the requester error status.
			//
			...
			//
			//	Receive message including header and body
			//
   ...
   )
{
	...
	do // just for structural programing
	{
		...
		//
		//	STEP1: Negotiate versions and buffer size.
		//
		...
		secondaryRequest = AllocSecondaryRequest(
							...
							(sizeof(NDFS_REPLY_HEADER)+sizeof(NDFS_REPLY_NEGOTIATE)),
							FALSE
							);
		...
		//
		//	Receive the negotiate reply: header
		//
		...
		//
		//	Receive the negotiate reply: body
		//
		...
		if(ndfsReplyNegotiate->MaxBufferSize)
		{
			...
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
				("ConnectToPrimary: PriMaxData:%08u SecMaxData:%08u\n",
				Secondary->Thread.SessionContext.PrimaryMaxDataSize,
				Secondary->Thread.SessionContext.SecondaryMaxDataSize));
		}
		...
		//	STEP2:
		//
		//	Send the SETUP request with NDAS device information
		//

		secondaryRequest = AllocSecondaryRequest(
							Secondary,
							(sizeof(NDFS_REQUEST_HEADER)+sizeof(NDFS_REQUEST_SETUP) > sizeof(NDFS_REPLY_HEADER)+sizeof(NDFS_REPLY_SETUP))
								? (sizeof(NDFS_REQUEST_HEADER)+sizeof(NDFS_REQUEST_SETUP)) :
								(sizeof(NDFS_REPLY_HEADER)+sizeof(NDFS_REPLY_SETUP)),
							FALSE
							);
		...
		//
		//	Receive the SETUP reply: header
		//
		...
		if(Secondary->Thread.SessionContext.NdfsMinorVersion == NDFS_PROTOCOL_MINOR_1)
		{
			...
			//
			//	Send the TREE_CONNECT request with partition information
			//
...
SEND_TREE_CONNECT_COMMAND:
			...
			//
			//	Receive the TREE_CONNECT reply: header
			//
			...
			//
			//	Receive the TREE_CONNECT reply: body
			//
				...
				|| ndfsReplyTreeConnect->Status == NDFS_TREE_CONNECT_NO_PARTITION)
			{
					...
					|| ndfsReplyTreeConnect->Status == NDFS_TREE_CONNECT_NO_PARTITION && sendCount <= 12)
				{
					...
					// Wait 10 seconds. Wait up to 120(12x10) seconds.
				...
				if(ndfsReplyTreeConnect->Status == NDFS_TREE_CORRUPTED ||
					...
					SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_ERROR,("Unplugging the NDAS device in slot %u. TreeStatus=%u\n",
	...
	do // just for structural programing
	{
		...
		secondaryRequest = AllocSecondaryRequest(
							...
							(sizeof(NDFS_REQUEST_HEADER)+sizeof(NDFS_REQUEST_LOGOFF) > sizeof(NDFS_REPLY_HEADER)+sizeof(NDFS_REPLY_LOGOFF))
								...
								(sizeof(NDFS_REPLY_HEADER)+sizeof(NDFS_REPLY_LOGOFF)),
							FALSE
							);
	...
	)
{
	...
	case IRP_MJ_READ:
	{
		...
		if(Secondary->LfsDeviceExt->FileSystemType != LFS_FILE_SYSTEM_NTFS) {
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_ERROR,("Direct read from secondary is supported only for NTFS\n"));
		...
		//
		// Do not read directly if last modification time is later than opentime or file is modified less than 10 minutes.
		//		(Because primary may have not flushed data??)
		//
