...
LspLookupProtocol(
	...
	if(NdasHwVersion == LANSCSIIDE_VERSION_1_0) {
		...
		*Protocol = LSPROTO_IDE_V11;
	} else if (NdasHwVersion == LANSCSIIDE_VERSION_2_5) {
		*Protocol = LSPROTO_IDE_V11;
	} else {
...
LspBuildLoginInfo(
	...
	LoginInfo->HWRevision			= LSS->HWRevision;	// not necessary.
	...
	RtlCopyMemory((PVOID)LoginInfo->Password_v2, (PVOID)LSS->Password_v2, 16);
