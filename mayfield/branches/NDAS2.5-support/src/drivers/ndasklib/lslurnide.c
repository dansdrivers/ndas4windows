	...
	case LurPrimaryLurnInformation:
		{
			...
			if(IdeDev) {
				...
				RtlCopyMemory(LurnInfo->Password_v2, IdeDev->LanScsiSession.Password_v2, LSPROTO_PASSWORD_V2_LENGTH);
			} else {
				...
				RtlZeroMemory(LurnInfo->Password_v2, LSPROTO_PASSWORD_V2_LENGTH);
			}
		}
		break;
...
*/
		...
		//
		NewLanScsiSession = (PLANSCSI_SESSION) ExAllocatePoolWithTag(NonPagedPool, sizeof(LANSCSI_SESSION), LSS_BUFFER_POOLTAG);
		...
		if(LurnUpdate->UpdateClass == LURN_UPDATECLASS_WRITEACCESS_USERID) {
			if ((LoginInfo.HWVersion == LANSCSIIDE_VERSION_2_5 &&
						LoginInfo.UserID & (USER_PERMISSION_SW|USER_PERMISSION_EW)) ||
				(LoginInfo.HWVersion < LANSCSIIDE_VERSION_2_5 &&
						LoginInfo.UserID & 0xffff0000)) {
			...
			//
			if (LoginInfo.HWVersion == LANSCSIIDE_VERSION_2_5) {
				LoginInfo.UserID = MAKE_USER_ID(DEFAULT_USER_NUM, USER_PERMISSION_EW);
			} else {
			...
			if ((LoginInfo.HWVersion == LANSCSIIDE_VERSION_2_5 &&
						LoginInfo.UserID & (USER_PERMISSION_RO)) ||
				(LoginInfo.HWVersion < LANSCSIIDE_VERSION_2_5 &&
						!(LoginInfo.UserID & 0xffff0000))) {
		...
		if(!NT_SUCCESS(status)) {
			LurnRecordFault(Lurn, LURN_ERR_LOGIN, NewLanScsiSession->LastLoginError, NULL);
...
LSLurnIdeReconnect(
	...
	if(Lurn->Lur->LurFlags & LURFLAG_WRITESHARE_PS) {
		...
		if (Lurn->LurnParent && LURN_REDUNDENT_TYPE(Lurn->LurnParent->LurnType)) {
			//
			// If this LURN is part of RAID with redundency, reduce reconnect trial to improve responsiveness.
			//
...
//		ASSERT(FALSE);
	...
	status = LspReconnect(&IdeExt->LanScsiSession, IdeExt->LstransType);
	...
	status = LspRelogin(&IdeExt->LanScsiSession, LssEncBuff);
	if(!NT_SUCCESS(status)) {
