...
//
//
// 	To do: seperate DRIX (protocol) part, DRAID implementation part, and export to ndasscsi part.
...
#if 0
__forceinline
PUCHAR DrixGetCmdString(UCHAR Cmd)  
{
	PUCHAR Str;		
	switch(Cmd){	
	case DRIX_CMD_REGISTER: Str = "Register"; break; 
	case DRIX_CMD_UNREGISTER: Str = "Unregister"; break; 
	case DRIX_CMD_NODE_CHANGE: Str = "NodeChange"; break; 
	case DRIX_CMD_ACQUIRE_LOCK: Str = "Ackquire"; break; 
	case DRIX_CMD_RELEASE_LOCK: Str = "Release"; break; 
	case DRIX_CMD_RETIRE: Str = "Retire"; break; 
	case DRIX_CMD_CHANGE_STATUS: Str="ChangeStatus"; break; 
	case DRIX_CMD_STATUS_SYNCED: Str="Synced"; break;
	case DRIX_CMD_GRANT_LOCK: Str="Grant"; break;
	case DRIX_CMD_REQ_TO_YIELD_LOCK: Str="Yield"; break; 
...
#define DRIX_NODE_FLAG_DEFECTIVE 		0x08
#define DRIX_NODE_FLAG_OUT_OF_SYNC 	0x10	// Replaced with Arbiter->OutOfSyncNode. Only used by client side.
...
typedef struct _DRIX_NODE_CHANGE {
	...
	struct {
		...
		UCHAR	DefectCode;  // Valid only with DRIX_NODE_STATUS_DEFECTIVE. NDAS_UNIT_DEFECT_UNKNOWN, NDAS_UNIT_DEFECT_*
		UCHAR	Reserved[1];
	} Node[1];
...
#define DRAID_MINIMUM_LOCK_GRANULARITY 16
...
#define DRAID_RANGE_SRC2_CONTAINS_SRC1		4
