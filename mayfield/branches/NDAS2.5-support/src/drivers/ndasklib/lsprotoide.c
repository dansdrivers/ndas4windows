...
LspIdeReadReply_V11(
	...
	if(LSS->SessionPhase == FLAG_FULL_FEATURE_PHASE) {
		if (LSS->HeaderEncryptAlgo == ENCRYPT_ALGO_NKCDE_1) {
			Decrypt32SP(
				(unsigned char*)pPdu->pH2RHeader,
				sizeof(LANSCSI_H2R_PDU_HEADER),
				(unsigned char *)&(LSS->DecryptIR[0])
				);
		} else if (LSS->HeaderEncryptAlgo == ENCRYPT_ALGO_AES128) {
			// Decrypt first 32 byte only to get AHS size
			Decrypt128(
				(unsigned char*)pPdu->pH2RHeader,
				32,
				(unsigned char*)LSS->CHAP_C_v2,
				LSS->Password_v2
			);
		}
	}
	...
	if(ahsLen) {
		...
		if(LSS->HWProtoVersion >= LSIDEPROTO_VERSION_1_1) {
			if(LSS->SessionPhase == FLAG_FULL_FEATURE_PHASE
				&& LSS->HeaderEncryptAlgo == ENCRYPT_ALGO_NKCDE_1) {
				Decrypt32SP(
	...
	// Read Header Dig.
	if (LSS->SessionPhase == FLAG_FULL_FEATURE_PHASE &&
		LSS->HeaderDigestAlgo == DIGEST_ALGO_CRC32) {
		status = RecvIt(
				&LSS->ConnectionFile,
				pPtr,
				4,
		...
		if(recv != 4) {
			KDPrintM(DBG_PROTO_ERROR, ("Can't Recv CRC...\n"));
	...
	if (LSS->HWVersion == LANSCSIIDE_VERSION_2_5 && ulTotalRecieved %16 !=0) {
		// Read padding if HW 2.5
		int PaddingSize = 16 - (ulTotalRecieved %16);
		status = RecvIt(
				&LSS->ConnectionFile,
				pPtr,
				PaddingSize,
		...
		if(recv != PaddingSize) {
			KDPrintM(DBG_PROTO_ERROR, ("Can't Recv Padding...\n"));

			return status;
		} else 
			ulTotalRecieved += PaddingSize;
		pPtr += PaddingSize;
	}
	...
	// Decrypt remaing bytes if NDAS 2.5
	if(LSS->SessionPhase == FLAG_FULL_FEATURE_PHASE 
		&& LSS->HeaderEncryptAlgo == ENCRYPT_ALGO_AES128) {
		Decrypt128(
	...
	if (LSS->SessionPhase == FLAG_FULL_FEATURE_PHASE &&
		LSS->HeaderDigestAlgo == DIGEST_ALGO_CRC32) {
		unsigned int hcrc = ((unsigned int*) pPdu->pHeaderDig)[0];
		unsigned int rcrc;
		Crc32((unsigned char*)pPdu->pH2RHeader,
			(unsigned char*)&rcrc,
			sizeof(LANSCSI_R2H_PDU_HEADER) + ahsLen
		);
		if (rcrc != hcrc) {
			KDPrintM(DBG_PROTO_ERROR, ("Crc mismatch while reading reply\n"));
			...
			)
{
	...
	hdrLen = sizeof(LANSCSI_H2R_PDU_HEADER) + iAHSLen + iDataSegLen;
	...
	// Write digest at the header.
	if (LSS->SessionPhase == FLAG_FULL_FEATURE_PHASE
		&& LSS->HeaderDigestAlgo == DIGEST_ALGO_CRC32) {
		Crc32((unsigned char*)pHeader, 
			((unsigned char*)pHeader)+hdrLen,
			hdrLen);
		hdrLen +=4;
	}
	...
	//
	if(LSS->SessionPhase == FLAG_FULL_FEATURE_PHASE
		&& LSS->DataEncryptAlgo == ENCRYPT_ALGO_NKCDE_1
		&& iDataSegLen > 0) {
	...
	// Add padding if NDAS 2.5
	if (LSS->HWVersion == LANSCSIIDE_VERSION_2_5) {
		hdrLen = (hdrLen + 15) & 0xfffffff0;
	}
	 ...
	 )
{
	...
	UINT32								CHAP_I;
	NTSTATUS							ntStatus;
	unsigned char password0[16] = {0x1f, 0x4a, 0x50, 0x73, 0x15, 0x30, 0xea, 0xbb,
	...
	if(!NT_SUCCESS(ntStatus)) {
		...
		LSS->LastLoginError = LSS_LOGIN_RECV_FAIL_1;
		return ntStatus;
	}
	...
	if (LSS->HWVersion == LANSCSIIDE_VERSION_2_5) {
		LSS->CHAP_C_v2[0] = pParamChap->V2.CHAP_CR[0];
		LSS->CHAP_C_v2[1] = pParamChap->V2.CHAP_CR[1];
		LSS->CHAP_C_v2[2] = pParamChap->V2.CHAP_CR[2];
		LSS->CHAP_C_v2[3] = pParamChap->V2.CHAP_CR[3];		
		KDPrintM(DBG_PROTO_ERROR, ("Hash %d, Challenge %08x %08x %08x %08x\n", 
		...
		KDPrintM(DBG_PROTO_TRACE, ("Hash %d, Challenge %08x\n", 
			NTOHL(pParamChap->CHAP_A), 
			LSS->CHAP_C)
			);
	}
	...
	if (LSS->HWVersion == LANSCSIIDE_VERSION_2_5) { // && (USER_NUM_FROM_USER_ID(LSS->UserID) != 1 || USER_PERM_FROM_USER_ID(LSS->UserID) == 0)) {
		// temp. override password & account
//		KDPrintM(DBG_PROTO_ERROR, ("Temporarily set user id from %x to %x\n", LSS->UserID, MAKE_USER_ID(DEFAULT_USER_NUM, USER_PERMISSION_EW)));
//		LSS->UserID = MAKE_USER_ID(DEFAULT_USER_NUM, USER_PERMISSION_EW);
		KDPrintM(DBG_PROTO_ERROR, ("Login id with %x\n", LSS->UserID));	
	...
	//
	if (LSS->HWVersion == LANSCSIIDE_VERSION_2_5) {
		KDPrintM(DBG_PROTO_ERROR, ("Temporarily set password\n"));
		RtlCopyMemory(LSS->Password_v2, password0, 16);

		AesCipher((unsigned char*)LSS->CHAP_C_v2, 
			(unsigned char*)pParamChap->V2.CHAP_CR, 
			LSS->Password_v2);
		KDPrintM(DBG_PROTO_ERROR, ("CHAP R %08x %08x %08x %08x\n", 
			NTOHL(pParamChap->CHAP_A), 
			pParamChap->V2.CHAP_CR[0], 
			pParamChap->V2.CHAP_CR[1], 
			pParamChap->V2.CHAP_CR[2], 
			pParamChap->V2.CHAP_CR[3])
			);
		KDPrintM(DBG_PROTO_ERROR, ("Password: %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x\n", 
			LSS->Password_v2[0], LSS->Password_v2[1], LSS->Password_v2[2], LSS->Password_v2[3],
			LSS->Password_v2[4], LSS->Password_v2[5], LSS->Password_v2[6], LSS->Password_v2[7],
			LSS->Password_v2[8], LSS->Password_v2[9], LSS->Password_v2[10], LSS->Password_v2[11],
			LSS->Password_v2[12], LSS->Password_v2[13], LSS->Password_v2[14], LSS->Password_v2[15])
			);
	} else {
		Hash32To128(
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		if (LSS->HWVersion == LANSCSIIDE_VERSION_2_5) { 
			// We can have more response from NDAS 2.5
			switch(pReplyHeader->Response) {
			case LANSCSI_RESPONSE_T_COMMAND_FAILED:
				KDPrintM(DBG_PROTO_ERROR, ("Third Failed. RESPONSE: Password mismatch\n"));
				LSS->LastLoginError = LSS_LOGIN_INVALID_PASSWORD;
				break;
			case LANSCSI_RESPONSE_T_NO_PERMISSION:
				KDPrintM(DBG_PROTO_ERROR, ("Third Failed. RESPONSE: No permission\n"));
				LSS->LastLoginError = LSS_LOGIN_NO_PERMISSION;
				break;
			case LANSCSI_RESPONSE_T_CONFLICT_PERMISSION:
				KDPrintM(DBG_PROTO_ERROR, ("Third Failed. RESPONSE: Another user loggin in\n"));
				LSS->LastLoginError = LSS_LOGIN_CONFLICT_PERMISSION;
				break;
			default:
...
//	LSS->MaxBlocks = NTOHL(pParamNego->MaxBlocks);
	...
	if (LSS->HWVersion == LANSCSIIDE_VERSION_2_5) {
		LSS->HeaderDigestAlgo = pParamNego->HeaderDigestAlgo;
		LSS->DataDigestAlgo = pParamNego->DataDigestAlgo;
	} else {
		LSS->HeaderDigestAlgo = 0;
		LSS->DataDigestAlgo = 0;
	}
	...
	if(2 == LSS->HWVersion && 0 == LSS->HWRevision && NTOHL(pParamNego->MaxBlocks) > 104) {
	...
	)
{
	...
	for(i = 0; i < pParam->NRTarget; i++) {
		...
		if (LSS->HWVersion == LANSCSIIDE_VERSION_2_5) {
			KDPrintM(DBG_PROTO_INFO, ("NR Targets  %d: Target ID: 0x%x, NR_EW: %d, NR_SW: %d, NR_RO: %d,Data:0x%x \n", i, 
			...
			TargetList->TargetInfoEntry[i].NRROHost = pTarget->v2.NRROHost;
			TargetList->TargetInfoEntry[i].NRSWHost = pTarget->v2.NRSWHost;;
			TargetList->TargetInfoEntry[i].NREWHost = pTarget->v2.NREWHost;
			...
			TargetList->TargetInfoEntry[i].NRSWHost = 0;
			TargetList->TargetInfoEntry[i].NREWHost = pTarget->v1.NRRWHost;
			TargetList->TargetInfoEntry[i].TargetData = pTarget->TargetData;
		}
	}
...
//		TDI_SEND_PARAM SendResult;
		int		SendLength = PduDesc->DataBufferLength << BLOCK_SIZE_BITS;

		// Calculate digest
		if (LSS->DataDigestAlgo == DIGEST_ALGO_CRC32) {
			// Need to add CRC and input buffer may have no room for it. So copy it.
			ActualDataBuffer = LSS->DigestPatchBuffer;
			RtlCopyMemory(ActualDataBuffer, PduDesc->DataBuffer, SendLength);
			Crc32(
				ActualDataBuffer,
				(unsigned char*) &ActualDataBuffer[SendLength],
				SendLength);
			SendLength+=16;
		} else {
			ActualDataBuffer = PduDesc->DataBuffer;
		}
		
		//
		//	Protocol encryption
		//
		if(LSS->DataEncryptAlgo == ENCRYPT_ALGO_NKCDE_1) {
			//
			//	If the data buffer of PDUDESC is volatile, encrypt it directly. 
			//
			if(PduDesc->Flags & PDUDESC_FLAG_VOLATILE_BUFFER) {
		...
		} else if (LSS->DataEncryptAlgo == ENCRYPT_ALGO_AES128) {
			//
			//	If the data buffer of PDUDESC is volatile, encrypt it directly. 
			//
			if (LSS->DataDigestAlgo == DIGEST_ALGO_CRC32) {
				Encrypt128(
					(unsigned char*)ActualDataBuffer,
					SendLength,
					(unsigned char*)LSS->CHAP_C_v2,
					LSS->Password_v2
				);
				...
				//
						...
						LSS->EncryptBufferLength >= (PduDesc->DataBufferLength << BLOCK_SIZE_BITS)) {
						RtlCopyMemory(LSS->EncryptBuffer, ActualDataBuffer, SendLength);
						Encrypt128(
							(unsigned char*)LSS->EncryptBuffer,
							SendLength,
							(unsigned char*)LSS->CHAP_C_v2,
							LSS->Password_v2
						);
		...
		if(	RecoverBuffer) {
			...
			} else if (LSS->DataEncryptAlgo == ENCRYPT_ALGO_NKCDE_1) {
				Decrypt128(
					(unsigned char*)ActualDataBuffer,
					SendLength,
					(unsigned char*)LSS->CHAP_C_v2,
	...
	case WIN_READ:
		{
			...
			// Receive CRC data and check it
			if (LSS->DataDigestAlgo == DIGEST_ALGO_CRC32) {
				int DigeBuf[4];
				int DigeResult;
				ntStatus = RecvIt(
					&LSS->ConnectionFile, 
					(unsigned char*) DigeBuf,
					16,
					&result,
					&LSS->TransStat,
					&LSS->TimeOuts[0]
				...
				if(!NT_SUCCESS(ntStatus) 
					|| result != 16) {
					KDPrintM(DBG_PROTO_ERROR, ("Error when Receive CRC for READ\n"));
					return ntStatus;
				}

				if (LSS->DataEncryptAlgo == ENCRYPT_ALGO_AES128) {
					Decrypt128(
						(unsigned char*)DigeBuf,
						16,
						(unsigned char*)LSS->CHAP_C_v2,
				...
				Crc32((unsigned char*)PduDesc->DataBuffer,
					(unsigned char*)&DigeResult,
					PduDesc->DataBufferLength << BLOCK_SIZE_BITS);
				if (DigeResult != DigeBuf[0]) {
					KDPrintM(DBG_PROTO_ERROR, ("CRC Error when READ\n"));
					return STATUS_CRC_ERROR;
				}
			}
		}
...
LspIdeVendorRequest(
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_PROTO_ERROR, ("Failed. Response 0x%x Req. VendorOp: 0x%x Rep. VendorOp: 0x%x\n", 
