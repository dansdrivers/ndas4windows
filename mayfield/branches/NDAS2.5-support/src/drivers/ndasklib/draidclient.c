		...
		if (ReplyMsg->Length == HTONS((UINT16)sizeof(DRIX_ACQUIRE_LOCK_REPLY)) &&
			...
			if (LockFound == FALSE) {
				...
				if (ReplyMsg->Result == DRIX_RESULT_GRANTED) {
					Lock->LockStatus = LOCK_STATUS_GRANTED;
					KDPrintM(DBG_LURN_INFO, ("Reply - lock granted lock id = %I64x, range= %I64x:%x\n", Lock->LockId, 
						Lock->LockAddress, Lock->LockLength));
				} else {
					Lock->LockStatus = LOCK_STATUS_PENDING;
					Lock->Contented = TRUE;
					KDPrintM(DBG_LURN_INFO, ("Reply - lock pending pending lock id = %I64x, range= %I64x:%x\n", Lock->LockId, 
						Lock->LockAddress, Lock->LockLength));
				}
				ACQUIRE_SPIN_LOCK(&Client->SpinLock, &oldIrql); 
...
//	RequestMsg->Version = DRIX_CURRENT_VERSION;
	...
	if (DRIX_CMD_NODE_CHANGE == Command) {
		...
		RtlCopyMemory(RegMsg->RaidSetId, &Client->Rmd.guid, sizeof(GUID));
		...
		KDPrintM(DBG_LURN_INFO, ("Sending RELEASE_LOCK message lock id = %I64x:%x\n", CmdParam2));
	}
	...
	if (Client->HasLocalArbiter) {
		...
		ArbMsgEntry->Message =  ExAllocatePoolWithTag(NonPagedPool, sizeof(DRIX_MSG_CONTEXT), DRAID_MSG_LINK_POOL_TAG);
...
//		ExFreePoolWithTag(RequestMsg, DRAID_CLIENT_REQUEST_MSG_POOL_TAG); // Freed when reply arrive or when terminating.
		...
		status = LpxTdiRecvWithCompletionEvent(
						...
						sizeof(DRIX_HEADER),	0, &ReqTimeout,NULL
		);
		if (NT_SUCCESS(status)) {
			return STATUS_SUCCESS;
		} else {
...
//	NTSTATUS				status;
	...
	KDPrintM(DBG_LURN_INFO, ("Client handling message %02x\n", Message->Command));
...
//	ReplyMsg->Version = DRIX_CURRENT_VERSION;
	...
	if (pClientInfo->HasLocalArbiter) {
		...
		KDPrintM(DBG_LURN_INFO, ("DRAID Sending reply %x to local arbiter, event %p\n", Message->Command, &pClientInfo->NotificationReplyChannel->Event));
		...
		KDPrintM(DBG_LURN_INFO, ("DRAID Sending reply %x to remote arbiter\n", Message->Command));
...
DraidClientUpdateNodeFlags(
	...
	NewFlags = DraidClientLurnStatusToNodeFlag(ChildLurn->LurnStatus);
	NewFlags |= FlagsToAdd;
...
DraidClientSendCcbToNondefectiveChildren(
	...
	if (DefectiveExist) {
		KDPrintM(DBG_LURN_INFO, ("Defective disk exist. Send Ccb only to non-defective disk %d\n", DiskNonDefective));
	...
	case SCSIOP_WRITE16:		
		{
			...
			// Fake write mode. 
			// to do: handle ndfat case after merging.
			if(!(Lurn->AccessRight & GENERIC_WRITE)){	// Read-only access write
				...
				if(Lurn->Lur->LurFlags & LURFLAG_SHAREDWRITE) {
					...
					LSCcbGetAddressAndLength((PCDB)&Ccb->Cdb[0], &LogicalAddr, &TransferLength);

					if(LogicalAddr + TransferLength > Lurn->UnitBlocks) {
						overRange = TRUE;
					} else {
						overRange = FALSE;
					}
					...
					if(Lurn->Lur->LurFlags & LURFLAG_OOB_SHAREDWRITE) {
						...
						//
						if(overRange) {
							fakeWriteFilter = FALSE; // pass through over-range in write-mode
						}
					}
				}
				//
					...
					if(Lurn->Lur->LurFlags & LURFLAG_FAKEWRITE) {	// Fake write
						KDPrintM(DBG_LURN_ERROR, ("SCSIOP_WRITE: Fake Write."
							"  Address:%lu DataBufferLength = %lu, Trans = %lu\n",
								LogicalAddr, Ccb->DataBufferLength,
...
complete_here:
	...
	LSAssocSetStatusFlag(pClientInfo->DRaidStatus, Ccb);
...
DraidClientConnectToArbiter(
	...
	for (i=0;i<Client->TotalDiskCount;i++) {
		...
		if (!IdeDisk)
			continue;
		RtlCopyMemory(&LocalAddr, &IdeDisk->LanScsiSession.SourceAddress.Address[0].Address, sizeof(LPX_ADDRESS));
		LocalAddrFound = TRUE;
		break;
	}
	if (!LocalAddrFound) {
...
DraidClientRegisterToRemoteArbiter(
	...
	RtlCopyMemory(&RegMsg.RaidSetId, &pClientInfo->Rmd.guid, sizeof(GUID));
	...
	default:
		KDPrintM(DBG_LURN_INFO, ("Registration failed with result %x\n", ReplyMsg.Result));
		return STATUS_UNSUCCESSFUL;
	}
}
...
DraidClientEstablishArbiter(
	...
	// Start waiting for notification message.
	...
	// Wait for network event without timeout
	status = LpxTdiRecvWithCompletionEvent(
					pClientInfo->NotificationConnection.ConnectionFileObject,
					&pClientInfo->NotificationConnection.TdiReceiveContext,
					pClientInfo->NotificationConnection.ReceiveBuf,
					sizeof(DRIX_HEADER),
					0,
					NULL,
					NULL
	...
	if(!NT_SUCCESS(status)) {
		DraidClientDisconnectFromArbiter(pClientInfo, &pClientInfo->NotificationConnection);
		DraidClientDisconnectFromArbiter(pClientInfo, &pClientInfo->RequestConnection);		
		KDPrintM(DBG_LURN_ERROR, ("Failed to recv notificaiont message\n"));
		return STATUS_UNSUCCESSFUL;
	}
	...
	)
{
	...
	static const UINT32 MaxArbiterConnectionTrialCount = 5;
	...
	pClientInfo->IsReadonly = LURN_IS_READONLY_NODE(Lurn);
	...
	RtlCopyMemory(&pClientInfo->RaidSetId, &pClientInfo->Rmd.guid, sizeof(GUID));
	...
	while(TRUE) {
		...
		if (DoMore) {
			...
			if (pClientInfo->DRaidStatus == DRIX_RAID_STATUS_REBUILDING || pClientInfo->RequestToTerminate) {
				// Set shorter timeout in rebuilding status to wake-up write-lock clean-up more frequently
			...
			KDPrintM(DBG_LURN_NOISE, ("Client waiting for event: %p %p\n", events[0], events[1]));
...
//				KDPrintM(DBG_LURN_INFO, ("RequestReplyChannel signaled\n"));
		...
		if (pClientInfo->ClientStatus == DRAID_CLIENT_STATUS_INITIALIZING) {
			...
			//
			if (LURN_IS_PRIMARY_NODE(Lurn)) {
				pClientInfo->HasLocalArbiter = TRUE;
				ASSERT(pRaidInfo->pDraidArbiter);

				// Assume local arbiter is initialized already.
				...
				RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);
				ArbiterConnectionTrialCount = 0;
			} else if (LURN_IS_SECONDARY_NODE(Lurn)) {
		...
		if (DRAID_CLIENT_STATUS_ARBITER_CONNECTED == pClientInfo->ClientStatus) {
			if (pClientInfo->HasLocalArbiter) {
				...
				if (KeReadStateEvent(&pClientInfo->RequestConnection.TdiReceiveContext.CompletionEvent)) {
					KeClearEvent(&pClientInfo->RequestConnection.TdiReceiveContext.CompletionEvent);
					if (pClientInfo->RequestConnection.TdiReceiveContext.Result <0) {
						KDPrintM(DBG_LURN_ERROR, ("Receive through request connection failed. Reset current context and enter initialization step.\n"));
						DraidClientResetRemoteArbiterContext(pClientInfo);
					} else {
						status = DraidClientRecvAdditionalDataFromRemote(pClientInfo, &pClientInfo->RequestConnection);
						if (NT_SUCCESS(status)) {
							status = DraidClientHandleReplyForRequest(pClientInfo, (PDRIX_HEADER)MsgLink->Message);
						}
						if (!NT_SUCCESS(status)) {
		...
		if (DRAID_CLIENT_STATUS_ARBITER_CONNECTED == pClientInfo->ClientStatus) {
			...
			} else if (KeReadStateEvent(&pClientInfo->NotificationConnection.TdiReceiveContext.CompletionEvent)) {
				KeClearEvent(&pClientInfo->NotificationConnection.TdiReceiveContext.CompletionEvent);
				if (pClientInfo->RequestConnection.TdiReceiveContext.Result <0) {
					KDPrintM(DBG_LURN_ERROR, ("Receive through request connection failed. Reset current context and enter initialization step.\n"));
					DraidClientResetRemoteArbiterContext(pClientInfo);
				} else {
					status = DraidClientRecvAdditionalDataFromRemote(pClientInfo, &pClientInfo->NotificationConnection);
					if (NT_SUCCESS(status)) {					
						status = DraidClientHandleNotificationMsg(pClientInfo, (PDRIX_HEADER)pClientInfo->NotificationConnection.ReceiveBuf);
					} 
					if (!NT_SUCCESS(status)) {
						KDPrintM(DBG_LURN_ERROR, ("Failed to handle notification message. Reset current context and enter initialization step.\n"));
						DraidClientResetRemoteArbiterContext(pClientInfo);
					} else {
						// Wait for next notification message.
						pClientInfo->NotificationConnection.TdiReceiveContext.Irp = NULL;
						KeInitializeEvent(&pClientInfo->NotificationConnection.TdiReceiveContext.CompletionEvent, NotificationEvent, FALSE) ;
						status = LpxTdiRecvWithCompletionEvent(
										pClientInfo->NotificationConnection.ConnectionFileObject,
		...
		//
		// Check any lock is not used for a while or we have too many locks.
		//
		if (DRAID_CLIENT_STATUS_ARBITER_CONNECTED == pClientInfo->ClientStatus) {		
			ACQUIRE_SPIN_LOCK(&pClientInfo->SpinLock, &oldIrql);
			KeQueryTickCount(&current_time);
...
//
// Called with pClientInfo->SpinLock locked.
//
// Return SUCCESS if given range is covered.
// 		UNSUCCESSFUL if not covered.
...
DraidClientCheckIoPermission(
	...
	PDRAID_CLIENT_LOCK FullOverlap = NULL, HeadOverlap = NULL, TailOverlap = NULL;
	BOOLEAN LockCovered; // We lock covering this IO
	NTSTATUS status;
	...
	if (FullOverlap) {
		...
		*UnauthLength = (UINT32)(OverlapStartAddr - Addr);
		...
		// Partial match. Assume all region is covered.
...
VOID DraidClientAcquireLockCallback(PDRAID_CLIENT_INFO Client, PDRIX_HEADER Reply, PVOID Param1)
{
	...
	if (Reply) {
		...
		ACQUIRE_SPIN_LOCK(&Client->SpinLock, &oldIrql);
		status = DraidClientCheckIoPermission(Client, Addr, Length, TRUE, &AddrToLock, &LengthToLock);
		RELEASE_SPIN_LOCK(&Client->SpinLock, oldIrql);
		if (status == STATUS_SUCCESS) {
			...
			Ccb->CcbStatus = CCB_STATUS_BUSY;
			LSAssocSetStatusFlag(Client->DRaidStatus, Ccb);
			LSCcbCompleteCcb(Ccb);
		}
	} else {
		// Timeout
...
) 
{
	...
	if (LURN_IS_SECONDARY_NODE(Lurn)) {
		KDPrintM(DBG_LURN_INFO, ("Secondary IO with RAID range lock\n"));
		ASSERT(FALSE); // this should happen in ndasfat only?
	}
	LSCcbGetAddressAndLength((PCDB)&Ccb->Cdb[0], &Addr, &Length);

	ACQUIRE_SPIN_LOCK(&pClientInfo->SpinLock, &oldIrql);
	status = DraidClientCheckIoPermission(pClientInfo, Addr, Length, TRUE, &AddrToLock, &LengthToLock);
	RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);
	...
	if (status == STATUS_PENDING) {
		...
		LARGE_INTEGER Timeout;
		Timeout.QuadPart = HZ / 2; 
		KDPrintM(DBG_LURN_INFO, ("Send request for range %I64x:%x\n", AddrToLock, LengthToLock));
			...
			if(NT_SUCCESS(status) && LURN_IS_RUNNING(ChildLurn->LurnStatus))
			{
				...
				if(!NT_SUCCESS(status)) {
					...
					DraidClientUpdateNodeFlags(pClientInfo, pRaidInfo->MapLurnChildren[i], 
						DRIX_NODE_FLAG_DEFECTIVE, NDAS_UNIT_DEFECT_BAD_DISK);
					continue;
				}
				//
				// Check RMD signature
				...
				if (NDAS_RAID_META_DATA_SIGNATURE != Rmd.Signature  ||
					...
					DraidClientUpdateNodeFlags(pClientInfo, pRaidInfo->MapLurnChildren[i], 
						DRIX_NODE_FLAG_DEFECTIVE, NDAS_UNIT_DEFECT_NOT_A_MEMBER);				
					continue;
				}
				
				// Check RMD's RAID set is same
				if (RtlCompareMemory(&Rmd.guid, &pClientInfo->Rmd.guid, sizeof(GUID)) != sizeof(GUID)) {
					KDPrintM(DBG_LURN_INFO, ("RAID Set ID for node %d is not matched.Mark as defective.\n", i));
					DraidClientUpdateNodeFlags(pClientInfo, pRaidInfo->MapLurnChildren[i], 
						DRIX_NODE_FLAG_DEFECTIVE, NDAS_UNIT_DEFECT_NOT_A_MEMBER);
					continue;
				}
				...
				KDPrintM(DBG_LURN_INFO, ("Failed to revive node %d\n", pClientInfo->RoleToNodeMap[i]));
			}
		}		
	}
