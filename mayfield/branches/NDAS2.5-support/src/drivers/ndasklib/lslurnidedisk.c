...
{
	if (IdeDisk->Lurn->Lur->LurFlags & LURFLAG_DYNAMIC_REQUEST_SIZE) {
		return IdeDisk->LanScsiSession.WriteSplitSize;
	} else {
		return IdeDisk->LanScsiSession.MaxBlocksPerRequest;
	}
}
	...
	PLURNEXT_IDE_DEVICE IdeDisk)
{
	...
	ULONG Retranmits;
	ULONG PacketLoss;
	PLANSCSI_SESSION LanScsiSession = &IdeDisk->LanScsiSession;
	if (!(IdeDisk->Lurn->Lur->LurFlags & LURFLAG_DYNAMIC_REQUEST_SIZE)) 
		return IdeDisk->LanScsiSession.MaxBlocksPerRequest;
		
	// Clear packet loss count not for read-error during writing to cause changes read-split.
...
//		KDPrintM(DBG_LURN_ERROR, ("Cleared packet loss count\n"));
	...
	if(Retranmits > 0)
	{
		...
		LanScsiSession->WriteSplitSize = 
			...
			(LanScsiSession->WriteSplitSize >= 11) ? 10:8; // 4kbyte minimum.
...
//			(LanScsiSession->WriteSplitSize >= 6) ? 4 : 2;
	...
	else
	{
		...
		// Size-up policy:
		// Split size under 32 cause big performance drop. Increase block size faster.
		// After that slower increase speed proportionally to current split size.
		if( 
			(LanScsiSession->WriteSplitSize < 32 && LanScsiSession->NrWritesAfterLastRetransmits > LanScsiSession->WriteSplitSize) ||
			(LanScsiSession->NrWritesAfterLastRetransmits > LanScsiSession->WriteSplitSize * 8)) 
		{
			LanScsiSession->NrWritesAfterLastRetransmits = 0;
...
#define MINUMUN_READ_SPLIT_SIZE 8 // 4kbytes
	...
	PLURNEXT_IDE_DEVICE IdeDisk)
{
	...
	if(PacketLoss > 0)
	{
		...
		if (LanScsiSession->ReadSplitSize < MINUMUN_READ_SPLIT_SIZE)
		    LanScsiSession->ReadSplitSize =MINUMUN_READ_SPLIT_SIZE;
	...
	else
	{
		...
		// After that slower increase speed proportionally to current split size.
		if( 
			(LanScsiSession->ReadSplitSize < 32 && LanScsiSession->NrReadAfterLastPacketLoss > LanScsiSession->ReadSplitSize) ||
			(LanScsiSession->NrReadAfterLastPacketLoss > LanScsiSession->ReadSplitSize * 8)) 
		{
...
{
	if (IdeDisk->Lurn->Lur->LurFlags & LURFLAG_DYNAMIC_REQUEST_SIZE) {
		return IdeDisk->LanScsiSession.ReadSplitSize;
	} else {
		return IdeDisk->LanScsiSession.MaxBlocksPerRequest;
	}
}
	...
	do {
		...
		if(IdeDisk->HwVersion >= LANSCSIIDE_VERSION_2_0 &&
			...
			if(info.hw_config & 0x2000) {
				...
				// If 2.0 HW revision is 0, limit to UDMA4, if revision is 0x01f, limit to UDMA 3. (Revision 0x01f is for HW testing)
				// If 2.5 HW, no limit
				if(info.dma_ultra & 0x0010) {
					DmaMode = 4;
				}
				if (!(IdeDisk->HwVersion == LANSCSIIDE_VERSION_2_0 && 
					IdeDisk->HwRevision == 0)) {
					//
					// Limit UDMA mode to 4 only to reduce write-failure.
					// And some PCB board(from Sarotech) has problem with high speed transfer mode.
				...
				if (IdeDisk->HwVersion == LANSCSIIDE_VERSION_2_0 && IdeDisk->HwRevision == 0x01f) {
					if (DmaMode > 2)
						DmaMode = 2;
				}
	...
	)
{
	...
	if(!LurnDesc)
	{
		...
		status = LspReconnect(&IdeDisk->LanScsiSession, IdeDisk->LstransType);
		...
		status = LspRelogin(&IdeDisk->LanScsiSession, LssEncBuff);
...
//			ASSERT(Lurn->LurnStatus == LURN_STATUS_STALL);
	...
	else
	{
		...
		IdeDisk->HwRevision = LurnDesc->LurnIde.HWRevision;
		...
		if(!(LurnDesc->AccessRight & GENERIC_WRITE)) {
			if (LurnDesc->LurnIde.HWVersion == LANSCSIIDE_VERSION_2_5) {
				LurnDesc->LurnIde.UserID = MAKE_USER_ID(DEFAULT_USER_NUM, USER_PERMISSION_RO);
			} else {
		...
		RtlCopyMemory((PVOID)LoginInfo.Password_v2, (PVOID)LurnDesc->LurnIde.Password_v2, LSPROTO_PASSWORD_V2_LENGTH);	
...
LurnIdeDiskRequest(
	...
	if(		!LURN_IS_RUNNING(Lurn->LurnStatus)
		...
		} else if (Lurn->LurnStatus == LURN_STATUS_DEFECTIVE) {
			KDPrintM(DBG_LURN_ERROR, ("LURN%d(STATUS %08lx)"
						" is in LURN_STATUS_DEFECTIVE. Return with CCB_STATUS_STOP.\n", Lurn->LurnId, Lurn->LurnStatus));
			LSCcbSetStatusFlag(Ccb, CCBSTATUS_FLAG_BUSRESET_REQUIRED|CCBSTATUS_FLAG_LURN_STOP);
			Ccb->CcbStatus = CCB_STATUS_STOP;
			Lurn->LurnStatus = LURN_STATUS_DEFECTIVE;
		} else {
...
try_reconnect:
		...
		if(	!LSCcbIsFlagOn(Ccb, CCB_FLAG_RETRY_NOT_ALLOWED) &&
			...
			ASSERT(Ccb->Srb || Ccb->OperationCode == CCB_OPCODE_RESETBUS || 
				Ccb->OperationCode ==  CCB_OPCODE_NOOP ||Ccb->CompletionEvent);
...
/*			ACQUIRE_SPIN_LOCK(&Lurn->LurnSpinLock, &oldIrql);
			...
			if(!NT_SUCCESS(status) || pdu_response != LANSCSI_RESPONSE_SUCCESS) {
				...
				if(Lurn->LurnStatus != LURN_STATUS_STALL) {
					...
					RELEASE_SPIN_LOCK(&Lurn->LurnSpinLock, oldIrql);
#if 0			// not required any more due to RAID will revive node anyway.				
					PCCB ccb;
					//
					// No more request came after busy returning after stalling. This can be happen for RAID.
					// We need to reconnect in this case and it is better to send this disk only.
					//
					KDPrintM(DBG_LURN_ERROR, ("Lurn%d:"
											" No op failed in stall state. Resend NOOP to trigger reconnect.\n",
											Lurn->LurnId));
					RELEASE_SPIN_LOCK(&Lurn->LurnSpinLock, oldIrql);
	...
	if(	IdeDisk->Lurn->Lur->LurFlags & LURFLAG_LOCKEDWRITE){
		...
		while(1) {
			ntStatus = LspAcquireLock(	&IdeDisk->LanScsiSession,
										SCSISTRG_WRITE_LOCKIDX_BASE + IdeDisk->LuData.UnitNumber,
										NULL);
			...
			if(LockContention >= 10000) {
				...
				LurnRecordFault(IdeDisk->Lurn, LURN_ERR_NDAS_OP, LURN_FAULT_LOCK_CLEANUP, NULL);
				break;
			}
		}
		if(!NT_SUCCESS(ntStatus)) {
...
retry_io:
		...
		if(!NT_SUCCESS(ntStatus)) {
			...
			if (LurnGetCauseOfFault(IdeDisk->Lurn) & LURN_FCAUSE_BAD_SECTOR) {
				KDPrintM(DBG_LURN_ERROR,
					("LurnIdeDiskRead: Multiple failure of reading same place. Handle as bad sector address start=%I64x, length=%x \n",
					logicalBlockAddress + transferedBlocks,
					BlocksToTransfer)
				);

				if (IdeDisk->Lurn->LurnParent && LURN_REDUNDENT_TYPE(IdeDisk->Lurn->LurnParent->LurnType)) {
					KIRQL			oldIrql;
					// In redundent RAID mode, handle bad sector as defective disk.
					ACQUIRE_SPIN_LOCK(&IdeDisk->Lurn->LurnSpinLock, &oldIrql);
					IdeDisk->Lurn->LurnStatus = LURN_STATUS_DEFECTIVE;
					ASSERT(FALSE); // sjcho temp for debug
					RELEASE_SPIN_LOCK(&IdeDisk->Lurn->LurnSpinLock, oldIrql);
				}
					
				Ccb->ResidualDataLength = (ULONG)(transferBlocks - transferedBlocks) * BLOCK_SIZE;
			...
			case LANSCSI_RESPONSE_T_COMMAND_FAILED:
				...
				if((!(PduDesc.Feature & ID_ERR) && (PduDesc.Feature & ABRT_ERR)) ||
					...
					if (senseData) {
						...
						senseData->AdditionalSenseLength = 0xb;
						senseData->AdditionalSenseCode = SCSI_ADSENSE_VOLUME_OVERFLOW;
						senseData->AdditionalSenseCodeQualifier = 0;
					} else {
						KDPrintM(DBG_LURN_ERROR, ("Error SenseBuffer is NULL\n"));
					}
...
#else  
					// Old code. 
					// Bad sector search is not neccessary. Other parameter other than SenseKey will be ignored. 
					// 		See ddk sample storage/class/classpnp/class.c:ClassInterpretSenseInfo for more info

					{
						...
						for(i = 0; i < BlocksToTransfer; i++) {
							...
							LURNIDE_INITIALIZE_PDUDESC(
												...
												IDE_COMMAND,
												WIN_READ,
												logicalBlockAddress + transferedBlocks + i,
												1,
												((PUCHAR)DataBuffer) +
												((transferedBlocks+i) << BLOCK_SIZE_BITS));
							ntStatus = LspRequest(
											&IdeDisk->LanScsiSession,
											&PduDesc,
											&response
							...
							if(!NT_SUCCESS(ntStatus)) {
								...
								//
								// Disk may be in unstable state after previous error.
								//
							...
							if(response != LANSCSI_RESPONSE_SUCCESS) {
								...
								BadSector = logicalBlockAddress + transferedBlocks + i;
							}
						}

						if(BadSector == (UINT64)-1) {
		...
		if(!NT_SUCCESS(ntStatus)) {
			...
			case LANSCSI_RESPONSE_T_COMMAND_FAILED:
				...
				if (PduDesc.Feature & ECC_ERR) {
					...
					//
					// Don't need to transit into defective mode. Let file system handle bad sector.
					//					
#if 1
					Ccb->ResidualDataLength = (ULONG)(transferBlocks - transferedBlocks) * BLOCK_SIZE;
...
//		inquiryData.EnclosureServices;
	...
	case SCSIOP_VERIFY16:
	{
		...
		LSCcbGetAddressAndLength((PCDB)Ccb->Cdb, &logicalBlockAddress, &totalTransferBlocks);
...
DO_MORE:
		...
		if(!NT_SUCCESS(ntStatus)) {
			LurnRecordFault(IdeDisk->Lurn, LURN_ERR_DISK_OP, LURN_FAULT_IDENTIFY, NULL);
			...
			case LANSCSI_RESPONSE_T_NOT_EXIST:
				...
				LurnRecordFault(IdeDisk->Lurn, LURN_ERR_DISK_OP, LURN_FAULT_NOT_EXIST, NULL);	
				return STATUS_SUCCESS;
			...
			case LANSCSI_RESPONSE_T_COMMAND_FAILED:
				...
				if (PduDesc.Feature & ECC_ERR) {
					...
					LurnRecordFault(IdeDisk->Lurn, LURN_ERR_READ, LURN_FAULT_BAD_SECTOR, &Lfi);
					Ccb->ResidualDataLength = transferBlocks * BLOCK_SIZE;
	...
	case SCSIOP_READ16:
	{
		...
		LSCcbGetAddressAndLength((PCDB)Ccb->Cdb, &logicalBlockAddress, &transferBlocks);
		...
		SplitSize = LurnIdeDiskGetReadSplit(IdeDisk);
	...
	case SCSIOP_WRITE16:
	{
		...
		SplitSize = LurnIdeDiskGetWriteSplit(IdeDisk);
