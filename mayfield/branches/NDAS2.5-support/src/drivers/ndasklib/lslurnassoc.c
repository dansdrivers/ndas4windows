...
)
{
	...
	for(idx_child = 0; idx_child < ChildrenCnt; idx_child++)
	{
		...
		status = LSCcbInitializeByCcb(Ccb, pLurnChildren[idx_child], 0, NextCcb[idx_child]);
		if(!NT_SUCCESS(status))
		{
			LONG	idx;
...
//					LurnSetRaidInfoStatus(Lurn->LurnRAIDInfo, RAID_STATUS_INITIAILIZING);
					// to do: Notify client to connect local arbiter?
				}
			}
...
//	Mirroring Lurn
//
///	Called by each child's CcbCompletion
//
NTSTATUS
...
//		inquiryData.EnclosureServices;
	...
	case SCSIOP_READ_CAPACITY16:
	{
		...
		blockSize = BLOCK_SIZE * Lurn->LurnChildrenCnt;
		REVERSE_BYTES(&readCapacityDataEx->BytesPerBlock, &blockSize);
		...
		KDPrintM(DBG_LURN_TRACE, ("SCSIOP_READ_CAPACITY16: %I64d : %04x\n", logicalBlockAddress, blockSize));
		LSCcbCompleteCcb(Ccb);
		break;
	}
	...
	)
{
	...
	if (pClient == NULL || pClient->DRaidStatus == DRIX_RAID_STATUS_TERMINATED) {
		// Client is alreay terminated.
		DraidStatus = DRIX_RAID_STATUS_TERMINATED;
	} else {
		DraidStatus = pClient->DRaidStatus;
		//
		// To do: set defect info If defective.
		//
	...
	LSAssocSetStatusFlag(DraidStatus, OriginalCcb);
...
complete_here:
	...
	LSAssocSetStatusFlag(DraidStatus, Ccb);
						...
						pRaidInfo->nDiskCount -1 != pRaidInfo->iChildFault)
					{
						...
						for(j = 0; j < BlocksPerDisk; j++)
						{
							pDataBufferToRecover = 
								...
								(pRaidInfo->iChildFault + j * (pRaidInfo->nDiskCount -1)) * BLOCK_SIZE);
...
)
{
	...
	for(i = 0; i < Lurn->LurnChildrenCnt; i++)
	{
		...
		{
			if (uiUSNMax !=0 && RtlCompareMemory(&PrevGuid, &rmd->guid, sizeof(GUID)) != sizeof(GUID)) {
				KDPrintM(DBG_LURN_INFO, ("Node %d has different RAID set ID from %d\n", i, LatestNode));
				uiUSNMax = 0;
				break;
			}
	...
	if(0 == uiUSNMax)
	{
		KDPrintM(DBG_LURN_INFO, ("Cannot find valid RMD\n"));
