...
DraidReallocEventArray(
	...
	KDPrintM(DBG_LURN_INFO, ("Allocating event array to count %d\n", NewCount));
	if (*Events)
...
restart:
		...
		// Check whether listen event has been signalled.
...
DraidReceptionThreadProc(
	...
	if (status == STATUS_SUCCESS) {
			...
			listEntry = listEntry->Flink) 
		{
			...
			if (RtlCompareMemory(&Arbiter->Rmd.guid, RegMsg.RaidSetId, sizeof(GUID)) == sizeof(GUID)) {
				MatchFound = TRUE;		
				break;
			}
		}
