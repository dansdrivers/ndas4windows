...
//#define DISABLE_PACKET_REORDERING 1
...
#define    SMP_RETRANSMIT_ACKED    0x0002
	...
	// Send jumbo packet for this connection.
	BOOL SendJumbo;

	// Set TRUE when ack with set jumbo is expected to arrive. Set after sending jumbo ACKREQ
	BOOL WaitingSetJumbo;
	
	// Peer does not support delayed ack. 
	// Need to set OPC not all packets.
	// Currently set by NDAS 2.5 only.
	BOOL PeerWithoutDelayedAck;
	UINT32	UnsentAckCount;
...
typedef enum _PACKET_TYPE {
    ...
    RETRAN,
    ACKREQ,
    ACKREQJ,	// Jumbo ack request
    ACKJ,		// Jumbo ACK reply
	ACKREQNJ,	// No-jumbo ACKREQ
    ACKNJ,		// No-Jumbo ACK reply	
} PACKET_TYPE;
...
#define DEFAULT_MAX_RETRANSMIT_TIME		4000

#define DEFAULT_MAXIMUM_TRANSFER_UNIT	1488	// Bytes. As of NDAS 2.5. Size of packets to netdisk needs be multiple of 16


// NDAS HW support this length MTU. Temporary implementatin...
#define NDAS_JUMBO_FRAME_LENGTH (8208 + 16) // This size excluses MAX header
