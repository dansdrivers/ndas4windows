...
//        ASSERT( LpxControlDeviceContext != DeviceContext) ;
	...
	//
	//	Allocate memory for packet header(send type) or whole packet(recv type)
	//
	...
	if(!NT_SUCCESS(status)) {
		NdisFreePacket(packet);
		*Packet = NULL;
		LpxFreePacketMemory(DeviceContext, PacketLength, packetData);
		   ...
		   )
{
	...
	case SEND_TYPE:
		...
		if(pNdisBuffer) {
			...
			LpxFreePacketMemory(reserved->DeviceContext, uiLength, packetData);
	...
	case RECEIVE_TYPE:
		...
		if(reserved->LpxSmpHeader)
			LpxFreePacketMemory(reserved->DeviceContext, sizeof(LPX_HEADER2), reserved->LpxSmpHeader);
...
{
#if 1 // Use look aside list
	UINT32 i;
	static const UINT32 LookasideMemsizes[] = LOOK_ASIDE_PACKET_MEM_SIZES;
	
	for(i=0;i<NUMBER_OF_PACKET_MEM_LOOKASIDE_LIST;i++) {
		if (Length <= LookasideMemsizes[i]) {
			*VirtualAddress = NdisAllocateFromNPagedLookasideList(
	...
	DebugPrint(0, ("[LPX] LpxAllocatePacketMemory: Unexpected large packet memory request %d\n", Length));
	return NdisAllocateMemoryWithTag(VirtualAddress, Length,'PxpL');
#else
	...
	)
{
	...
	for(i=0;i<NUMBER_OF_PACKET_MEM_LOOKASIDE_LIST;i++) {
		if (Length <= LookasideMemsizes[i]) {
			NdisFreeToNPagedLookasideList(&DeviceContext->PacketMemLookAsideList[i], VirtualAddress);
			return;
		}
	}
