...
//	UCHAR			Reserved2[2];		// 48 bytes
	...
	//
	//	Max request blocks for the unit device ( LURN )
	//

	ULONG			UnitMaxRequestBlocks;
...
#define NDASSCSI_TYPE_DISK_RAID1R		0x0A	// V2 nDiskCount >= 2
#define NDASSCSI_TYPE_DISK_RAID4R		0x0B	// V2 nDiskCount >= 3
...
typedef struct _BUSENUM_PLUGIN_HARDWARE
{
	...
	//
	//	Specify Windows IO request block length for the SCSI adapter.
...
typedef struct _BUSENUM_INFORMATION_PDOENUM {
	UINT32	MaxBlocksPerRequest;	// if zero, the adapter use the default value.
	UINT32	Flags;
#ifdef	_NTDDK_
