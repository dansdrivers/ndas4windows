...
typedef struct _NDAS_DEVICE_HARDWARE_INFO
{
	...
	/* NDAS device hardware revision. 0 for 1.0, 1.1, 2.0. 1 for 2.0 Green. 2~ for 2.5~ */
...
#define NDAS_LDPF_SHARED_WRITE      0x00000020
#define NDAS_LDPF_OOB_SHARED_WRITE  0x00000040
...
======================     ===========                              =======
NDAS_LDPF_PSWRITESHARE     Use PS write share feature.              ON
NDAS_LDPF_FAKE_WRITE       Use fake write feature in ROFilter mode  ON
NDAS_LDPF_LOCKED_WRITE     Use Locked Write                         OFF
NDAS_LDPF_SHARED_WRITE     Enable Shared Write in all regions.      OFF
NDAS_LDPF_LOCKED_WRITE must be also set. 
NDAS_LDPF_OOB_SHARED_WRITE Enable Shared Write in OOB regions.      OFF
NDAS_LDPF_LOCKED_WRITE must be also set.
...
<CODE>
//
// This example shows how to enable NDAS_LDPF_OOB_SHARED_WRITE feature
...
// To enable NDAS_LDPF_OOB_SHARED_WRITE, NDAS_LDPF_LOCKED_WRITE 
// also must be specified.
param.LdpfFlags = NDAS_LDPF_LOCKED_WRITE | NDAS_LDPF_OOB_SHARED_WRITE;

// The following line will turn on LOCKED_WRITE and OOB_SHARED_WRITE
param.LdpfValues |= NDAS_LDPF_LOCKED_WRITE;
param.LdpfValues |= NDAS_LDPF_OOB_SHARED_WRITE;
