...
#define LSP_ERR_RESPONSE_T_NO_PERMISSION		0x24 
#define LSP_ERR_RESPONSE_T_CONFLICT_PERMISSION	0x25 // Returned when user has access permission, 
...
typedef struct _lsp_login_info
{
	...
	lsp_uint64 password;
	lsp_uint8	password_v2[16];
	lsp_uint64 supervisor_password;
	lsp_uint8	supervisor_password_v2[16];
//	lsp_uint8	user_num;
	lsp_uint8 unit_no;
...
typedef enum _lsp_handle_info_type
{
	...
	lsp_handle_info_hw_header_digest_algo,
	lsp_handle_info_hw_data_encrypt_algo,
	lsp_handle_info_hw_data_digest_algo,
