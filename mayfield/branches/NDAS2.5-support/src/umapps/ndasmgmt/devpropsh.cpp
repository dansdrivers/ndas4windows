...
{
	ATLASSERT(n < m_pUnitDevices.size());
	if (n < m_pUnitDevices.size())
	{
...
//	tiLocation.Expand();
	...
	//
	{
		if (NULL != pLogDeviceInfo && 
			IS_NDAS_LOGICALDEVICE_TYPE_DISK_GROUP(pLogDevice->GetType()) &&
...
CDeviceHardwarePage::OnInitDialog(HWND hwndFocus, LPARAM lParam)
{
	...
	for (DWORD i = 0; i < unitDevices.size(); ++i) 
	{
		...
		if (pHWI->PIO) strValue += _T("PIO, ");
		if (pHWI->DMA) strValue += _T("DMA, ");
		if (pHWI->UDMA) strValue += _T("UDMA, ");
		...
		strValue = _T("");
		if (pHWI->LBA) strValue += _T("LBA, ");
		if (pHWI->LBA48) strValue += _T("LBA48, ");
		if (strValue.GetLength() > 2) 
		{
			// Trimming ", "
			strValue = strValue.Left(strValue.GetLength() - 2);
