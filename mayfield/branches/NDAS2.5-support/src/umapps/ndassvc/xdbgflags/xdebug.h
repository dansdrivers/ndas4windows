#ifdef _XDEBUG_H_
#error XDEBUG should not overlapped! Do not include xdebug.h from the header file
#else
#define _XDEBUG_H_
...
public:
		...
		DWORD dwLevel = 0)
	{
		...
		DWORD ccbOutputString = 
			min(4096, (cchOutputString + 1) * sizeof(TCHAR));
...
public:
	...
	static const DWORD OL_NONE = 0x0000;
	...
	inline VOID VRawPrintf(DWORD dwLevel, LPCTSTR szFormat, va_list va)
	{
		//
		...
		va_list ap)
	{
		...
		if (dwError & APPLICATION_ERROR_MASK) {
			...
			hr = ::StringCchPrintfEx(pszNext, cchRemaining,
				&pszNext, &cchRemaining, 0, 
				TEXT("Error %d (0x%08X):"), dwError, dwError);
			_ASSERT(SUCCEEDED(hr));
...
inline LPCTSTR XDebugMemoryDump(
...
#ifdef NO_XDEBUG
static XDebug* _PXDebug = NULL;
#else /* NO_XDEBUG */

#ifdef XDEBUG_MAIN_MODULE
...
inline BOOL XDbgInit(LPCTSTR szModuleName)
{
...
inline VOID XDbgSetOutputLevel(DWORD dwOutputLevel)
{
	if (NULL != _PXDebug) {
...
inline VOID XDbgSetModuleFlags(DWORD dwFlags)
{
	if (NULL != _PXDebug) {
...
inline VOID XDbgAttach(IXDebugOutput* pOutput)
{
	if (NULL != _PXDebug) {
...
inline VOID XDbgDetach(IXDebugOutput* pOutput)
{
	if (NULL != _PXDebug) {
...
static const DWORD _xdbgModuleFlag = XDEBUG_MODULE_FLAG;

#define XDEBUG_IS_ENABLED(_level_) \
	(_PXDebug && \
	(_level_  <= _PXDebug->dwOutputLevel) && \
	(XDEBUG_MODULE_FLAG & _PXDebug->dwEnabledModules))
...
#define _FT(x) _T(_LINE_STR_) _T(",") _T(__FUNCTION__) _T(",") _T(x)
