...
CNdasUnitDevice::GetDeviceUserID(ACCESS_MASK access)
{
	...
	if (pParentDevice->GetHardwareVersion() == 3) {
		if (GENERIC_WRITE & access) 
			return 0x00010004; // MAKE_USER_ID(DEFAULT_USER_NUM, USER_PERMISSION_EW);
		else
			return 0x00010001; //MAKE_USER_ID(DEFAULT_USER_NUM, USER_PERMISSION_RO);
	} else {
...
CNdasUnitDevice::GetOptimalMaxRequestBlock()
{
	...
	UCHAR hardwareRevision = pParentDevice->GetHardwareRevision();
	...
	//
	// 100Mbps or higher: 128 (or 104 in V2.0R0)
	//
	// 
	...
	const DWORD MAX_REQUEST_BLOCK_LIMIT_DEFAULT = 
		(2 == hardwareVersion && 0 == hardwareRevision) ? 104 : 128;
	...
	CNdasUnitDiskDevice &NdasUnitDiskDevice)
{
	...
	if (NdasUnitDiskDevice.m_pDIBv2 == NULL || m_pDIBv2 == NULL) {
		XTLTRACE2_ERR(TCUnitDevice, TLError,
			"DIB is NULL: This unit DIB - %p, Target unit DIB - %p\n", 
