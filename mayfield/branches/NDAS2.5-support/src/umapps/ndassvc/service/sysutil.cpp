...
BOOL DisableDiskWriteCache(HANDLE hDisk)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCSysUtils, TLError, 
			"DiskGetCacheInformation failed.\n");
		return FALSE;
	}

	XTLTRACE2_ERR(TCSysUtils, TLInfo, 
	...
	{
		XTLTRACE2_ERR(TCSysUtils, TLError, 
			"DiskSetCacheInformation failed.\n");
		return FALSE;
	}
...
DisableDiskWriteCache(LPCTSTR szDiskPath)
{
	...
	{
		XTLTRACE2_ERR(TCSysUtils, TLInfo, 
			_T("CreateFile(%s) failed: "), szDiskPath);
		return FALSE;
	}
	...
	LPBOOL lpbDeviceHotplug)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2(TCSysUtils, TLError, "StorageGetHotPlugInfo failed.\n");
	}
	else
	{
	...
	BOOL bDeviceHotplug)
{
	...
	if (!fSuccess)
	{
		XTLTRACE2_ERR(TCSysUtils, TLError, "StorageSetHotPlugInfo failed.\n");
	}
	return fSuccess;
}
