...
{
	return (0 == ucType) && (0 == ucVersion || 1 == ucVersion || 2 == ucVersion || 3 == ucVersion);
}

static const BYTE OEMCODE_V1_SAMPLE[8] = {0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00};
static const BYTE OEMCODE_V1_RETAIL[8] = {0xbb,0xea,0x30,0x15,0x73,0x50,0x4a,0x1f};
static const BYTE OEMCODE_V1_RUTTER[8] = {0xbb,0xea,0x30,0x15,0x73,0x50,0x4a,0x1f};

XTLC_ASSERT(sizeof(OEMCODE_V1_SAMPLE) == sizeof(NDAS_OEM_CODE));
XTLC_ASSERT(sizeof(OEMCODE_V1_RETAIL) == sizeof(NDAS_OEM_CODE));
XTLC_ASSERT(sizeof(OEMCODE_V1_RUTTER) == sizeof(NDAS_OEM_CODE));

struct DefaultNdasOemCode : std::unary_function<NDAS_DEVICE_ID,NDAS_OEM_CODE>{
	const NDAS_OEM_CODE& operator()(const NDAS_DEVICE_ID& deviceID) const {		
		// Sample hardware range: 00:F0:0F:xx:xx:xx
		if (0x00 == deviceID.Node[0] && 0xF0 == deviceID.Node[1] && 0x0F == deviceID.Node[2])
		{
			return reinterpret_cast<const NDAS_OEM_CODE&>(OEMCODE_V1_SAMPLE);
		} 
		...
		{
			return reinterpret_cast<const NDAS_OEM_CODE&>(OEMCODE_V1_RUTTER);
		}
		// Retail Devices
		return reinterpret_cast<const NDAS_OEM_CODE&>(OEMCODE_V1_RETAIL);
	}
};
...
static const NDAS_DEVICE_HARDWARE_INFO NullHardwareInfo = {0};
...
CNdasDevice::CNdasDevice(
	...
	m_fAutoRegistered(FlagIsSet(dwFlags,NDAS_DEVICE_REG_FLAG_AUTO_REGISTERED)),
	m_fVolatile(FlagIsSet(dwFlags,NDAS_DEVICE_REG_FLAG_VOLATILE)),
	m_fHidden(FlagIsSet(dwFlags,NDAS_DEVICE_REG_FLAG_HIDDEN)),
	m_OemCode(DefaultNdasOemCode()(deviceId)),
	m_dstat(CNdasDeviceStat()),
	m_hardwareInfo(NullHardwareInfo),
	CStringizer(_T("{%03X}%s"),	dwSlotNo, CNdasDeviceId(deviceId).ToString())
{
	...
	XTLVERIFY(SUCCEEDED(
		::StringCchPrintf(m_szCfgContainer, 30, TEXT("Devices\\%04d"), dwSlotNo)));
...
UINT16
CNdasDevice::GetHardwareRevision()
{
	InstanceAutoLock autolock(this);
	return static_cast<UINT16>(m_hardwareInfo.HardwareRevision);
}
...
CNdasDevice::SetOemCode(const NDAS_OEM_CODE& OemCode)
{
	InstanceAutoLock autolock(this);
	m_OemCode = OemCode;
	BOOL fSuccess = SetConfigValueSecure(
		_T("OEMCode"), 
		&m_OemCode, 
...
{
	InstanceAutoLock autolock(this);
	m_OemCode = DefaultNdasOemCode()(m_deviceId);
