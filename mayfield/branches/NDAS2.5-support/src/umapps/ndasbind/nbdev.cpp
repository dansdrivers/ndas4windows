...
BOOL CNBUnitDevice::IsThis(BYTE Node[6], DWORD UnitNo)
{
	return !memcmp(m_pDevice->m_DeviceId.Node, Node, sizeof(m_pDevice->m_DeviceId.Node))
...
BOOL CNBUnitDevice::Initialize()
{
	...
	if (NMT_RAID1R == m_DIB.iMediaType ||
		NMT_RAID4R == m_DIB.iMediaType)
	{
...
CString CNBUnitDevice::GetTypeString()
{
	...
	case NMT_RAID1R: strText.LoadString(IDS_LOGDEV_TYPE_SINGLE_DISK); break;
	case NMT_RAID4: strText.LoadString(IDS_LOGDEV_TYPE_SINGLE_DISK); break;
	case NMT_RAID4R: strText.LoadString(IDS_LOGDEV_TYPE_SINGLE_DISK); break;
...
DWORD CNBUnitDevice::GetFaultTolerance()
{
	...
	if (NDAS_UNIT_META_BIND_STATUS_NOT_SYNCED & m_RMD.UnitMetaData[m_cSequenceInRMD].UnitDeviceStatus)
		status |= NDASBIND_UNIT_DEVICE_RMD_FAULT;

	if (NDAS_UNIT_META_BIND_STATUS_DEFECTIVE & m_RMD.UnitMetaData[m_cSequenceInRMD].UnitDeviceStatus)
		status |= NDASBIND_UNIT_DEVICE_RMD_DEFECTIVE;

	return status;
}

CString CNBUnitDevice::GetFaultToleranceString()
{
	CString strText;
	...
	else if (NDASBIND_UNIT_DEVICE_RMD_DEFECTIVE & status)
	{
		strText.LoadString(IDS_FT_DEFECT_CHILD);
	}
...
CString CNBLogicalDevice::GetTypeString()
{
	CString strText;
	...
	case NMT_RAID1R: strText.LoadString(IDS_LOGDEV_TYPE_DISK_RAID1R); break;
	case NMT_RAID4: strText.LoadString(IDS_LOGDEV_TYPE_DISK_RAID4); break;
	case NMT_RAID4R: strText.LoadString(IDS_LOGDEV_TYPE_DISK_RAID4R); break;
...
UINT CNBLogicalDevice::GetIconIndex(const UINT *anIconIDs, int nCount)
{
	...
	const struct { 
		...
		NMT_RAID1R,      IDI_DEVICE_BOUND,
		NMT_RAID4,      IDI_DEVICE_BOUND,
		NMT_RAID4R,      IDI_DEVICE_BOUND,
...
UINT64 CNBLogicalDevice::GetCapacityInByte()
{
	...
	case NMT_RAID1:
	case NMT_RAID1R:
		size = AnyUnitDevice()->GetCapacityInByte();
		break;
	case NMT_RAID4:
	case NMT_RAID4R:
...
CString CNBLogicalDevice::GetFaultToleranceString()
{
	CString strText;
	DWORD status = GetFaultTolerance();
...
BOOL CNBLogicalDevice::GetCommandAbility(int nID)
{
	...
	case IDM_TOOL_REPLACE_DEVICE:
		return (IsHDD() && IsFaultTolerant() && IsOperatable() && (1 == DevicesTotal(FALSE) - DevicesTotal(TRUE)));
	case IDM_TOOL_REPLACE_UNIT_DEVICE:
		if(IsHDD() && IsFaultTolerant() && IsOperatable() && (1 == DevicesTotal(FALSE) - DevicesTotal(TRUE)))
		{
			// make sure the missing device is able to be operated.
			for(UINT32 i = 0; i < DevicesTotal(); i++)
			{
				if(!m_mapUnitDevices.count(i))
				{
					...
					ci.AddressType = NDASCOMM_CIT_DEVICE_ID;
					ci.LoginType = NDASCOMM_LOGIN_TYPE_NORMAL;
					ci.UnitNo = DIB()->UnitDisks[i].UnitNumber;
					ci.WriteAccess = TRUE;
					ci.OEMCode.UI64Value = 0;
					ci.PrivilegedOEMCode.UI64Value = 0;
					ci.Protocol = NDASCOMM_TRANSPORT_LPX;
					...
					C_ASSERT(
						sizeof(ci.Address.DeviceId.Node) == 
						sizeof(DIB()->UnitDisks[i].MACAddr));
