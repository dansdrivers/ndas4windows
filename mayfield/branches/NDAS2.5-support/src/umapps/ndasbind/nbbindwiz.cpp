...
pGetDiskSizeString(CString &strSize, int nBindType, int nDiskCount, NBUnitDevicePtrList &listUnitDevices)
{
	...
	strSize = CNBDevice::GetCapacityString(
		...
		(NMT_RAID1R == nBindType) ? nSizeMin * nDiskCount /2 :
		(NMT_RAID4R == nBindType) ? nSizeMin * (nDiskCount -1) : 0);
}
...
#ifdef _DISABLE_RAID4BTN_ON_BINDDLG_
...
{	
	if(BN_CLICKED == msg)
	{
		...
		case IDC_BIND_TYPE_RAID1:
			UpdateControls(NMT_RAID1R);
			break;
		case IDC_BIND_TYPE_RAID4:
			UpdateControls(NMT_RAID4R);
			break;
		default:
			break;
		}
...
*/
	...
	m_pWizData->m_nBindType = 
		...
		(m_nBindType == 2) ? NMT_RAID1R : NMT_RAID4R;
...
void CSelectDiskPage::UpdateControls()
{
	GetDlgItem(IDC_BTN_ADD).EnableWindow(
		...
		m_wndListBind.GetSelectedDiskObjectList().size() <= m_pWizData->m_nDiskCount
		)
		);
...
CSelectDiskPage::OnWizardNext()
{
	...
	ATLASSERT( 
		...
		NMT_RAID1R == m_pWizData->m_nBindType || 
		NMT_RAID4R == m_pWizData->m_nBindType);
	...
	if(bResults)
	{
		for (i = 0; i < m_pWizData->m_nDiskCount; i++ )
		{
			...
			C_ASSERT(
				...
				sizeof(pConnectionInfo[i].Address.DeviceId.Node));
			CopyMemory(
				unitDeviceId.DeviceId.Node, 
				pConnectionInfo[i].Address.DeviceId.Node,
				sizeof(unitDeviceId.DeviceId.Node));
...
CCompletionPage::OnSetActive()
{
	...
	(NMT_RAID1R == m_pWizData->m_nBindType) ? IDS_LOGDEV_TYPE_DISK_RAID1R :
	(NMT_RAID4R == m_pWizData->m_nBindType) ? IDS_LOGDEV_TYPE_DISK_RAID4R : IDS_LOGDEV_TYPE
		);
