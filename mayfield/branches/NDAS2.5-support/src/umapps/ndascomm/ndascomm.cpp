	...
	void **wait_handle_ptr)
{
	...
	else
	{
		...
		AutoProcessHeap aBuffer = heapBuffer;
	...
	IN DWORD ReceiveTimeout OPTIONAL)
{
	...
	{
		DBGPRT_ERR_EX(_FT("SetSocketTimeout(Send:%d) failed: "), SendTimeout);
		return INVALID_SOCKET;
	}
	...
	{
		DBGPRT_ERR_EX(_FT("SetSocketTimeout(Recv:%d) failed: "), ReceiveTimeout);
		return INVALID_SOCKET;
	}
	...
	if (SOCKET_ERROR == sockret)
	{
		DBGPRT_ERR_EX(_FT("connect failed: "));
		return INVALID_SOCKET;
	}
	...
	CONST NDASCOMM_CONNECTION_INFO* pci)
{
	...
	if (NULL != pSockAddrList && !IsValidSocketAddressList(pSockAddrList))
	{
		...
		DBGPRT_ERR_EX(_FT("Invalid Binding Socket Address List:"));
		return FALSE;
	}
	return TRUE;
}
...
// temp.
unsigned char def_password0[16] = {0x1f, 0x4a, 0x50, 0x73, 0x15, 0x30, 0xea, 0xbb,
	0x3e, 0x2b, 0x32, 0x1a, 0x47, 0x50, 0x13, 0x1e};
unsigned char def_superuser_password0[16] = {
	...
	CONST NDASCOMM_CONNECTION_INFO* pConnectionInfo)
{
	...
	if (!g_bInitialized) 
	{ 
		DBGPRT_ERR(_FT("NDASCOMM_ERROR_NOT_INITIALIZED!\n"));
	...
	DBGPRT_ERR_EX(_FT("Temp: Hardcoded user password"));

	login_info.password = 
		(pConnectionInfo->OEMCode.UI64Value) ? 
			pConnectionInfo->OEMCode.UI64Value :
	        NdasCommImpGetPassword(unitDeviceID.DeviceId.Node);
	memcpy(login_info.password_v2, def_password0, 16);


	login_info.unit_no = (lsp_uint8)unitDeviceID.UnitNo;
//	login_info.user_num = 0; // hard-coded default user number
	login_info.write_access = (lsp_uint8)(pConnectionInfo->WriteAccess) ? 1 : 0;
	DBGPRT_ERR_EX(_FT("Temp: Hardcoded user password"));

	login_info.supervisor_password = pConnectionInfo->PrivilegedOEMCode.UI64Value;
	memcpy(login_info.supervisor_password_v2, def_superuser_password0, 16);
	...
	else
	{
		...
		// only 1.1 and 2.0R0 requires this
		if (1 == hardwareVersion || (2 == hardwareVersion && 0 == hardwareRevision))
		{
...
fail:

	{
		...
		if(context)
		{
			(void) NdasCommImpDisconnect(context);
		}
