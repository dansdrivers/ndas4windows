...
int __cdecl wmain()
{
	...
	if (NULL == (h = NdasHeartbeatRegisterNotification(NdasHeartbeatCallback, NULL)))
	{
		_tprintf(_T("Error %d\n"), GetLastError());
	}
	
	if (NULL == (h2 = NdasHeartbeatRegisterNotification(NdasHeartbeatCallback2, NULL)))
	{
	...
	if (h && !NdasHeartbeatUnregisterNotification(h))
	{
	...
	if (h2 && !NdasHeartbeatUnregisterNotification(h2))
	{
