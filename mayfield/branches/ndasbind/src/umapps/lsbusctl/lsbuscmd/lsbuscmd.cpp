...
void usage()
{
	_tprintf(TEXT("lsbuscmd [version | mpversion | slotlist | fdoinfo | status | pdoevent] slotno\n"));
}
...
int __cdecl _tmain(int argc, LPTSTR* argv)
{
	...
	if (lstrcmpi(argv[1],TEXT("version")) == 0) {
		...
		if(bret == FALSE) {
			_tprintf(TEXT("LanscsiMiniport control failed. LastError:%lu\n"), GetLastError());
		} else {
			_tprintf(TEXT("- LanscsiMiniport version\n"));
			_tprintf(	TEXT("Major   : %u\n")
				TEXT("Minor   : %u\n")
				TEXT("Build   : %u\n")
				TEXT("Private : %u\n"),
	  ...
	  ULONG ulStatus;
		...
		if(bret == FALSE) {
		  ...
		  _tprintf(TEXT("ADAPTERINFO_STATUS : %s\n"),
  		  ADAPTERINFO_ISSTATUS(ulStatus, ADAPTERINFO_STATUS_INIT) ? TEXT("ADAPTERINFO_STATUS_INIT") :
  		  ADAPTERINFO_ISSTATUS(ulStatus, ADAPTERINFO_STATUS_RUNNING) ? TEXT("ADAPTERINFO_STATUS_RUNNING") :
  		  ADAPTERINFO_ISSTATUS(ulStatus, ADAPTERINFO_STATUS_STOPPING) ? TEXT("ADAPTERINFO_STATUS_STOPPING") :
  		  ADAPTERINFO_ISSTATUS(ulStatus, ADAPTERINFO_STATUS_IN_ERROR) ? TEXT("ADAPTERINFO_STATUS_IN_ERROR") :
  		  ADAPTERINFO_ISSTATUS(ulStatus, ADAPTERINFO_STATUS_STOPPED) ? TEXT("ADAPTERINFO_STATUS_STOPPED") :
  		  TEXT("Unknown status"));

#define PRINT_FLAG(STATUS, FLAG) \
  		_tprintf(TEXT("") TEXT(#FLAG) TEXT(": %s\n"), ADAPTERINFO_ISSTATUSFLAG((STATUS), (FLAG)) ? TEXT("ON") : TEXT("off"));
  		PRINT_FLAG(ulStatus, ADAPTERINFO_STATUSFLAG_RECONNECT_PENDING);
  		...
  		PRINT_FLAG(ulStatus, ADAPTERINFO_STATUSFLAG_MEMBER_FAULT);
  		PRINT_FLAG(ulStatus, ADAPTERINFO_STATUSFLAG_RECOVERING);
#undef PRINT_FLAG
		}
		...
		if(bret == FALSE) {
			...
			for(idx_ud = 0; idx_ud < lurFullInfo->UnitDiskCnt; idx_ud++) {
				...
				_tprintf(TEXT("NetDiskAddress.Address[0].Address        :%02X %02X | %02X %02X %02X %02X %02X %02X\n"),
													(int)unitDisk->NetDiskAddress.Address[0].Address[0],
													(int)unitDisk->NetDiskAddress.Address[0].Address[1],
													(int)unitDisk->NetDiskAddress.Address[0].Address[2],
													(int)unitDisk->NetDiskAddress.Address[0].Address[3],
													(int)unitDisk->NetDiskAddress.Address[0].Address[4],
													(int)unitDisk->NetDiskAddress.Address[0].Address[5],
													(int)unitDisk->NetDiskAddress.Address[0].Address[6],
													(int)unitDisk->NetDiskAddress.Address[0].Address[7]
													);
				...
				_tprintf(TEXT("BindingAddress.Address[0].Address        :%02X %02X | %02X %02X %02X %02X %02X %02X\n"),
													(int)unitDisk->BindingAddress.Address[0].Address[0],
													(int)unitDisk->BindingAddress.Address[0].Address[1],
													(int)unitDisk->BindingAddress.Address[0].Address[2],
													(int)unitDisk->BindingAddress.Address[0].Address[3],
													(int)unitDisk->BindingAddress.Address[0].Address[4],
													(int)unitDisk->BindingAddress.Address[0].Address[5],
													(int)unitDisk->BindingAddress.Address[0].Address[6],
													(int)unitDisk->BindingAddress.Address[0].Address[7]
													);
