...
int CpRecover(int argc, _TCHAR* argv[])
{
	BOOL fSuccess;
	NDAS_LOGICALDEVICE_ID ldid = 0;

	if (argc != 1) {
		_tprintf(_T("usage: recover <logical device number>\n"));
		return -1;
	}

	ldid = _ttoi(argv[0]);

	_tprintf(_T("Recovering a logical device (%d)...\n"), ldid);
...
DEF_COMMAND_1(_c_recover, "recover", "<logical device number>")
