...
#define NDAS_VALIDATE_DEVICE_STRING_ID(lpsDeviceStringId) \
	{ \
	BOOL fSuccess = NdasIdValidateStringIdKeyW(lpsDeviceStringId, NULL, NULL); \
	if (!fSuccess) { \
...
#define NDAS_VALIDATE_DEVICE_STRING_IDA(lpsDeviceStringId) \
	{ \
	BOOL fSuccess = NdasIdValidateStringIdKeyA(lpsDeviceStringId, NULL, NULL); \
	if (!fSuccess) { \
		::SetLastError(NDASUSER_ERROR_INVALID_DEVICE_STRING_ID); \
		return FALSE; \
	} \
...
#define NDAS_FORWARD_DEVICE_STRING_IDW(lpsDeviceStringId, is) \
	CHECK_STR_PTRW(lpsDeviceStringId, NDAS_DEVICE_STRING_ID_LEN) \
	NDAS_VALIDATE_DEVICE_STRING_ID(lpsDeviceStringId) \
	NDAS_DEVICE_ID_OR_SLOT is; \
	is.bUseSlotNo = FALSE; \
	BOOL fSuccess = NdasIdConvertStringIdToRealIdW(lpsDeviceStringId,&is.DeviceId); \
	_ASSERT(fSuccess); // should not fail as we already did Validation 

#define NDAS_FORWARD_DEVICE_STRING_IDA(lpsDeviceStringId, is) \
	CHECK_STR_PTRA(lpsDeviceStringId, NDAS_DEVICE_STRING_ID_LEN) \
	NDAS_VALIDATE_DEVICE_STRING_IDA(lpsDeviceStringId) \
	NDAS_DEVICE_ID_OR_SLOT is; \
	is.bUseSlotNo = FALSE; \
	BOOL fSuccess = NdasIdConvertStringIdToRealIdA(lpsDeviceStringId,&is.DeviceId); \
	_ASSERT(fSuccess); // should not fail as we already did Validation 
...
NdasValidateStringIdKeyW(
	LPCWSTR lpsDeviceStringId, 
	LPCWSTR lpsDeviceStringKey)
{
	//
	// pointer validation check - ALWAYS DO THIS FOR API CALLS
	// lpsDeviceStringKey can be NULL
	//

	CHECK_STR_PTRW(lpsDeviceStringId, NDAS_DEVICE_STRING_ID_LEN)
	CHECK_STR_PTRW_NULLABLE(lpsDeviceStringKey, NDAS_DEVICE_STRING_KEY_LEN)

	BOOL bWritable;
	BOOL fSuccess = NdasIdValidateStringIdKeyW(
		lpsDeviceStringId, 
		lpsDeviceStringKey, 
		&bWritable);
...
BOOL NdasValidateStringIdKeyA(
	LPCSTR lpsDeviceStringId, 
	LPCSTR lpsDeviceStringKey)
{
	//
	// pointer validation check - ALWAYS DO THIS FOR API CALLS
	// lpsDeviceStringKey can be NULL
	...
	CHECK_STR_PTRA(lpsDeviceStringId, NDAS_DEVICE_STRING_ID_LEN)
	CHECK_STR_PTRA_NULLABLE(lpsDeviceStringKey, NDAS_DEVICE_STRING_KEY_LEN)

	BOOL bWritable;
	return ValidateStringIdKeyA(lpsDeviceStringId, lpsDeviceStringKey, &bWritable);
	...
	LPCWSTR lpszDeviceName)
{
	...
	BOOL bWritable;
	BOOL fSuccess = NdasIdValidateStringIdKeyW(
		lpsDeviceStringId, lpsDeviceStringKey, &bWritable);
	...
	fSuccess = NdasIdConvertStringIdToRealIdW(
		lpsDeviceStringId, 
		&lpRequest->DeviceId);
...
BOOL 
WINAPI
NdasUnregisterDeviceByIdW(LPCWSTR lpsDeviceStringId)
{
	NDAS_FORWARD_DEVICE_STRING_IDW(lpsDeviceStringId, is)
	return NdasUnregisterDeviceImpl(is);
}
...
NdasEnumDevicesW(NDASDEVICEENUMPROCW lpEnumFunc, LPVOID lpContext)
{
	...
	if (lpEnumFunc) {
		for (DWORD i = 0; i < lpReply->nDeviceEntries && bCont; ++i) {
			...
			BOOL fSuccess = NdasIdConvertRealIdToStringIdW(
				&pDeviceEntry->DeviceId, userEntry.szDeviceStringId);
			_ASSERT(fSuccess);
...
BOOL 
WINAPI
NdasEnableDeviceByIdW(LPCWSTR lpsDeviceStringId, BOOL bEnable)
{
	NDAS_FORWARD_DEVICE_STRING_IDW(lpsDeviceStringId, is)
	return NdasEnableDeviceImpl(is, bEnable);
}
	...
	LPCWSTR lpsDeviceStringKey)
{
	...
	BOOL fSuccess = NdasIdConvertStringIdToRealIdW(lpsDeviceStringId,&lpRequest->DeviceIdOrSlot.DeviceId);
	_ASSERT(fSuccess); // should not fail as we already did Validation
	...
	NDAS_DEVICE_ID_OR_SLOT DeviceIdOrSlot, NDASUSER_DEVICE_INFORMATIONW* pDevInfo)
{
	...
	BOOL fSuccess = NdasIdConvertRealIdToStringIdW(&lpReply->DeviceId,pDevInfo->szDeviceId);
	_ASSERT(fSuccess);
...
NdasQueryDeviceInformationByIdW(
	LPCWSTR lpsDeviceStringId, NDASUSER_DEVICE_INFORMATIONW* pDevInfo)
{
	NDAS_FORWARD_DEVICE_STRING_IDW(lpsDeviceStringId, is)
	return NdasQueryDeviceInformationImplW(is, pDevInfo);
}
...
NdasQueryUnitDeviceInformationByIdW(
	LPCWSTR lpsDeviceStringId, DWORD dwUnitNo, 
	PNDASUSER_UNITDEVICE_INFORMATIONW pDevInfo)
{
	NDAS_FORWARD_DEVICE_STRING_IDW(lpsDeviceStringId, is)
	return NdasQueryUnitDeviceInformationImplW(is, dwUnitNo, pDevInfo);
...
NdasQueryUnitDeviceInformationByIdA(
	LPCSTR lpsDeviceStringId, DWORD dwUnitNo, 
	PNDASUSER_UNITDEVICE_INFORMATIONA pDevInfo)
{
	NDAS_FORWARD_DEVICE_STRING_IDA(lpsDeviceStringId, is)
	return NdasQueryUnitDeviceInformationImplA(is, dwUnitNo, pDevInfo);
}
...
NdasFindLogicalDeviceOfUnitDeviceImpl(
	NDAS_DEVICE_ID_OR_SLOT DeviceIdOrSlot, DWORD dwUnitNo,
	PNDAS_LOGICALDEVICE_ID pLogicalDeviceId)
{
