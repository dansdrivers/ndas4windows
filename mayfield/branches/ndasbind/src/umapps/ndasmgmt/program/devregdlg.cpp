...
CRegisterDeviceDlg::OnRegister(WORD, WORD wID, HWND, BOOL&)
{
	...
	LPCTSTR lpsDeviceStringId = GetDeviceStringId();
	LPCTSTR lpsDeviceStringKey = GetDeviceStringKey();
	BOOL fEnableOnReg = GetEnableOnRegister();

	DWORD dwSlotNo = ::NdasRegisterDevice(
		lpsDeviceStringId, lpsDeviceStringKey, lpszDeviceName);
...
{
	LPCTSTR lpsDeviceStringKey = (m_strDeviceKey.GetLength() == 0) ? 
		NULL : (LPCTSTR) m_strDeviceKey;
	return lpsDeviceStringKey;
}
