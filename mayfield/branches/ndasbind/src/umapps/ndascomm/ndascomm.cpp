...
void *
lpx_connect(u_char *node, u_char *host_node, LPSOCKET_ADDRESS_LIST addr_list_hint)
{
	...
	if(addr_list_hint)
	{
		addr_list = addr_list_hint;
	}
	else
	{
		// init socket
		s = socket(AF_UNSPEC, SOCK_STREAM, LPXPROTO_STREAM);

		// query list_size
		result = WSAIoctl(s, SIO_ADDRESS_LIST_QUERY, 0, 0, addr_list, 0, (LPDWORD)&list_size, 0, 0);
		// -1 == result , cause addr_list

		// query addr_list
		addr_list = (LPSOCKET_ADDRESS_LIST)::HeapAlloc(::GetProcessHeap(), HEAP_ZERO_MEMORY, list_size);
		API_CALLEX_JMP(NDASCOMM_ERROR_NOT_ENOUGH_MEMORY, fail, NULL != addr_list);

		result = WSAIoctl(s, SIO_ADDRESS_LIST_QUERY, 0, 0, addr_list, list_size, (LPDWORD)&list_size, 0, 0);
		API_CALL_JMP(fail, ERROR_SUCCESS == result);
	...
	::CopyMemory(&ndas_dev_addr.slpx_addr.node, node, sizeof(ndas_dev_addr.slpx_addr.node));
	...
	for (int i = 0; i < addr_list->iAddressCount; ++i)
	{
		...
		result = bind(s, (const struct sockaddr*) &host_addr, sizeof(struct sockaddr_lpx));
		if(0 == result)
		{
			// binded     to lpx_addr_str(&host_addr)
			// connecting to lpx_addr_str(&ndas_dev_addr) at lpx_addr_str(&host_addr)
			result = connect(
				s,
			...
			if(0 == result)
			{
				// connected to lpx_addr_str(&ndas_dev_addr) at lpx_addr_str(&host_addr)
				::CopyMemory(host_node, host_addr.slpx_addr.node, sizeof(host_addr.slpx_addr.node));
				connected = 1;
				break;
			}
		}
	...
	case NDASCOMM_CONNECTION_INFO_TYPE_ID_W:
		{
			...
			{
				BOOL bWriteAccess = pConnectionInfo->bWriteAccess;
				API_CALLEX(	NDASCOMM_ERROR_INVALID_PARAMETER,
					::NdasIdValidateStringIdKeyW(wszID, wszKey, &bWriteAccess) );
				API_CALLEX( NDASCOMM_ERROR_INVALID_PARAMETER,
					bWriteAccess);
			}

			API_CALLEX( NDASCOMM_ERROR_INVALID_PARAMETER,
				::NdasIdConvertStringIdToRealIdW(wszID, &pUnitDeviceID->DeviceId) );
		}
		break;
	case NDASCOMM_CONNECTION_INFO_TYPE_ID_A:
		{
			...
			if (pConnectionInfo->bWriteAccess)
			{
				BOOL bWriteAccess = pConnectionInfo->bWriteAccess;
				API_CALLEX( NDASCOMM_ERROR_INVALID_PARAMETER,
					::NdasIdValidateStringIdKeyA(szID, szKey, &bWriteAccess) );
				API_CALLEX( NDASCOMM_ERROR_INVALID_PARAMETER,
					bWriteAccess);
			}
			API_CALLEX( NDASCOMM_ERROR_INVALID_PARAMETER,
				::NdasIdConvertStringIdToRealIdA(szID, &pUnitDeviceID->DeviceId) );
		}
		break;
	default:
		ASSERT_PARAM(FALSE);
	...
	IN UINT32 uiReadDataLen)
{
	...
	// all version
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_ret_time || TRUE);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_max_conn_time || TRUE);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_supervisor_pw || TRUE);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_user_pw || TRUE);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_reset|| TRUE);

	// V1.1
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_enc_opt || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_standby_timer|| 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_sema || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_free_sema || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_get_sema || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_get_owner_sema || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_dynamic_ret_time || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_get_dynamic_ret_time || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_get_ret_time || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_get_max_conn_time || 1 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_get_standby_timer || 1 <= HwVersion);

	// V2.0
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_delay || 2 <= HwVersion);
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_get_delay || 2 <= HwVersion);

	// Higher version
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_lpx_address || FALSE);	
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_d_enc_opt || FALSE);	
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_get_d_enc_opt || FALSE);	

	// Obsolete
	API_CALLEX(NDASCOMM_ERROR_INVALID_PARAMETER, vop_code != ndascomm_vcmd_set_dynamic_max_conn_time || FALSE);
