...
protected:
	...
	CRITICAL_SECTION m_csSendQueue; LONG m_lSendQueueLocks;
	CRITICAL_SECTION m_csRecvQueue; LONG m_lRecvQueueLocks;

	VOID LockSendQueue() 
	{ 
		::EnterCriticalSection(&m_csSendQueue);
		::InterlockedIncrement(&m_lSendQueueLocks);
	}
	VOID UnlockSendQueue() 
	{ 
		::InterlockedDecrement(&m_lSendQueueLocks);
	...
	{ 
		...
		::InterlockedIncrement(&m_lRecvQueueLocks);
	}

	VOID UnlockRecvQueue() 
	{ 
		::InterlockedDecrement(&m_lRecvQueueLocks);
