...
{
	_ASSERTE(!::IsBadStringPtrW(lpszDeviceStringId,NDAS_DEVICE_STRING_ID_LEN + 1));
	_ASSERTE(!::IsBadReadPtr(pDeviceId, sizeof(NDAS_DEVICE_ID)));
	...
	PBOOL pbWritable)
{
	...
	if (NULL != lpszDeviceStringKey) 
	{
		...
		iConverted = ::MultiByteToWideChar(CP_ACP, 0, 
			lpszDeviceStringKey, NDAS_DEVICE_STRING_KEY_LEN + 1,
			wszDeviceStringKey, NDAS_DEVICE_STRING_KEY_LEN + 1);
		if (0 == iConverted) 
		{
			return FALSE;
		}
		return NdasIdValidateStringIdKeyW(wszDeviceStringId, wszDeviceStringKey, pbWritable);
	} 
	else 
	{
		return NdasIdValidateStringIdKeyW(wszDeviceStringId, NULL, pbWritable);
	}
	...
	PNDAS_DEVICE_ID pDeviceId)
{
	...
	return NdasIdConvertStringIdToRealIdW(wszDeviceStringId, pDeviceId);
}
...
{
	WCHAR wszDeviceStringId[NDAS_DEVICE_STRING_ID_LEN + 1];
	BOOL fSuccess = NdasIdConvertRealIdToStringIdW(pDeviceId, wszDeviceStringId);
	if (!fSuccess) 
	{
		return FALSE;
	}
