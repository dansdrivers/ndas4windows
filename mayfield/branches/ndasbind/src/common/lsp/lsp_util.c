			   ...
			   lsp_uint16 sectors)
{
	...
	p.command.command =	(use_48) ? WIN_VERIFY_EXT : WIN_VERIFY;
	...
	return lsp_ide_command(h, target_id, lun0, lun1, &p, 0);
}
				 ...
				 struct hd_driveid *info)
{
	...
	*packet_device = 0;
	...
	err = lsp_ide_command(h, target_id, lun0, lun1, &p, &data_buf);
