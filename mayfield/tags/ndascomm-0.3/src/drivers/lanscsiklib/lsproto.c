...
LspConnectEx(
	...
	do {
		...
		status = LstransQueryBindingDevices(LSS->HWProtoVersion, addrListLen, addrList, NULL);
		if(!NT_SUCCESS(status)) {
			break;
		}
...
LspAcquireLock(
	...
	if(NT_SUCCESS(status)) {
		KDPrintM(DBG_LURN_INFO,	("Acquired lock #%u\n", LockNo));
		if(LockCount)
...
LspReleaseLock(
	...
	if(NT_SUCCESS(status)) {
		KDPrintM(DBG_LURN_INFO,	("Released lock #%u\n", LockNo));
		if(LockCount)
			*LockCount = NTOHL(pduDesc.Param32[1]);
	}
