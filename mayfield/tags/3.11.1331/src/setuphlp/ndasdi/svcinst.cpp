...
{
	DPInfo(
		_FT("Updating a service %s config (DisplayName: %s, Descr: %s) ")
		_T("%s, %s, %s, ")
		_T("Path %s, LoadOrderGroup %s, Dep %s, Account %s, Pwd %s\n"),
		ServiceName,
		(NULL == DisplayName) ? ServiceName : DisplayName,
	...
	AutoSCHandle schService = ::OpenService( 
		schSCManager,  
		ServiceName, 
		SERVICE_CHANGE_CONFIG);
	...
	if (!fSuccess)
	{
		DBGPRT_ERR_EX(_FT("ChangeServiceConfig failed: "));
		return FALSE;
	}
	...
	DPInfo(_FT("Service(%s) configuration updated successfully.\n"), ServiceName);
	return TRUE;
}
	...
	LPCTSTR Password)
{
	...
	if (NULL == (SC_HANDLE) schSCManager) {
		DPErrorEx(_FT("OpenSCManager failed: "));
		return FALSE;
	}
	...
	LPTSTR CopiedFilePath)
{
	...
	DPInfo(_FT("Copying a driver file to the system32\\drivers")
		_T(" from %s\n"), SourceFilePath);
	...
	if (NULL == lpszFullSrcPath) {
		DBGPRT_ERR_EX(_FT("Resolving full path of %s failed: "), SourceFilePath);
		return FALSE;
	}
	...
	if (0 == cch) {
		DBGPRT_ERR_EX(_FT("Getting a system directory failed: "));
		return FALSE;
	}
	...
	if (FAILED(hr)) {
		DBGPRT_ERR_EX(_FT("Concating %s, %s failed: "), szDriverFilePath, DRIVERS_DIR);
		return FALSE;
	}
	...
	if (FAILED(hr)) {
		DBGPRT_ERR_EX(_FT("Concating %s, %s failed: "), szDriverFilePath, lpFileName);
		return FALSE;
	}
	...
	if (!fSuccess) {
		DBGPRT_ERR_EX(_FT("Copying %s to %s failed: "), lpszFullSrcPath, szDriverFilePath);
		return FALSE;
	}
	...
	if (NULL != CopiedFilePath)
	{
		...
		if (FAILED(hr))
		{
			DBGPRT_ERR_EX(_FT("Copying %s to CopiedDriverPath failed: "), szDriverFilePath);
			return FALSE;
		}
	}
	...
	LPBOOL pbUpdated)
{
	...
	if (!fSuccess) 
	{
		DBGPRT_ERR_EX(_FT("NdasDiCopyDriverFileToSystem(%s) failed: "), DriverFilePath);
		return FALSE;
	}
	...
	if (fFound)
	{
		if (bPendingDeletion)
		{
			...
			DBGPRT_ERR_EX(_FT("Service (%s) is marked for deletion: "), ServiceName);
			return FALSE;
		}
		else
		{
			...
			if (!fStopped)
			{
				DBGPRT_ERR_EX(_FT("Service (%s) cannot be stopped: "), ServiceName);
				*pbUpdated = TRUE;
			...
			if (!fDeleted)
			{
				DBGPRT_ERR_EX(_FT("Deleting a service (%s) failed: "), ServiceName);
				return FALSE;
			}
		}
	}
