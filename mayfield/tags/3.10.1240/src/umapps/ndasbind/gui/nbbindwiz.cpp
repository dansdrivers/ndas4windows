...
//
VOID 
pGetDiskSizeString(WTL::CString &strSize, int m_nBindType, int nDiskCount, CDiskObjectList &listSelectedDiskObjects)
{
	UINT nSizeMinMB = 0xFFFFFFFF;
	UINT nSizeResultMB;

	CDiskObjectList::iterator itr;
	const CObjectUIHandler* pHandler;
	for(itr = listSelectedDiskObjects.begin(); 
		itr != listSelectedDiskObjects.end();
		itr++)
	{
		pHandler = CObjectUIHandler::GetUIHandler(*itr);
		nSizeMinMB = (nSizeMinMB > pHandler->GetSizeInMB(*itr)) ?
			pHandler->GetSizeInMB(*itr) : nSizeMinMB;
	}
	...
	case NMT_AGGREGATE:
	case NMT_RAID0:
		nSizeResultMB = nDiskCount * nSizeMinMB;
		break;
	case NMT_RAID1:
		nSizeResultMB = nDiskCount * nSizeMinMB /2;
		break;
	case NMT_RAID4:
		nSizeResultMB = (nDiskCount -1) * nSizeMinMB;
		break;
	default:
		nSizeResultMB = 0;
	}
	...
	strSize.FormatMessage( 
		...
		nSizeResultMB / 1024, (nSizeResultMB % 1024) / 10 
		);
}
...
void CWizard::SetSingleDisks(CDiskObjectList singleDisks)
{
	m_wizData.m_listSingle = singleDisks;
 }
...
CBindTypePage::OnInitDialog(HWND hWnd, LPARAM lParam)
{
	...
	CButton btnAggr;
	btnAggr.Attach(GetDlgItem(IDC_BIND_TYPE_AGGR));

	btnAggr.SetCheck(BST_CHECKED);
...
CSelectDiskPage::OnInitDialog(HWND hWnd, LPARAM lParam)
{
	...
	m_wndListSingle.AddDiskObjectList(m_pWizData->m_listSingle);
...
CSelectDiskPage::OnWizardNext()
{
	...
	m_pWizData->m_listBind = m_wndListBind.GetDiskObjectList();
	...
	WTL::CString strTitle;
	CUnitDiskObjectPtr UnitDiskObject;
	CDiskObjectList::const_iterator itr;
	
	for (i = 0, itr = listBind.begin(); itr != listBind.end(); ++itr, i++ )
	{
		UnitDiskObject = boost::dynamic_pointer_cast<CUnitDiskObject>(*itr);
		...
		pConnectionInfo[i].UnitNo = UnitDiskObject->GetLocation()->GetUnitDiskLocation()->UnitNumber;
		...
		CopyMemory(pConnectionInfo[i].AddressLPX, 
			UnitDiskObject->GetLocation()->GetUnitDiskLocation()->MACAddr,
			LPXADDR_NODE_LENGTH);
...
CCompletionPage::OnSetActive()
{
	...
	int i;
	CDiskObjectList::iterator itr;
	CUnitDiskObjectPtr UnitDiskObject, UnitDiskObjectFailed;
	for(i = 0, itr = m_pWizData->m_listBind.begin(); itr != m_pWizData->m_listBind.end(); ++itr, i++)
	{
		UnitDiskObject = boost::dynamic_pointer_cast<CUnitDiskObject>(*itr);
		if(m_pWizData->m_BindResult == i)
			UnitDiskObjectFailed = UnitDiskObject;
		if(itr != m_pWizData->m_listBind.begin())
			strText += _T(", "); 

		strText += UnitDiskObject->GetTitle();
	}
	ctlEditResult.AppendText(strText); ctlEditResult.AppendText(_T("\n"));
	...
	if(m_pWizData->m_BindResult == m_pWizData->m_nDiskCount)
	{
		...
		// size
		pGetDiskSizeString(strText2, m_pWizData->m_nBindType, m_pWizData->m_nDiskCount, m_pWizData->m_listBind);
