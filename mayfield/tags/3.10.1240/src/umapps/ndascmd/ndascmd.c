...
{
	static LPCTSTR USAGE = _T("{<logical-device-number> | /device <device-number>}");

	NDAS_LOGICALDEVICE_ID ldID;
	INT iPos;
	BOOL fSuccess;

	if (-1 == argc) return pusage(USAGE);

	iPos = get_named_arg_pos(argc, argv, _T("device"), NULL);
	if (-1 != iPos)
	{
		iPos = get_unnamed_arg_pos(argc, argv, 0);
		fSuccess = NC_ResolveLDIDFromSlotUnit(argv[iPos], &ldID);
		if (!fSuccess) 
		{
			return EXIT_FAILURE;
		}
	...
	{
		iPos = get_unnamed_arg_pos(argc, argv, 0);
		ldID = _ttoi(argv[iPos]);
	}

	fSuccess = NdasRecoverLogicalDevice(ldID);
	if (!fSuccess)
	{
		return NC_PrintLastErrMsg(), EXIT_FAILURE;
	}
