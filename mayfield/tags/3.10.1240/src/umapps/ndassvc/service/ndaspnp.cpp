	...
	PDEV_BROADCAST_DEVICEINTERFACE pdbhdr)
{
	...
	if (NULL == hDevNotify) {
		DBGPRT_WARN_EX(_FT("Registering device handle for NDAS logical device")
...
//	//
//
//	CNdasLogicalDevice* pLogDevice = pGetNdasLogicalDevice(dwSlotNo);
//	if (NULL == pLogDevice) {
...
//
//	DBGPRT_INFO(_FT("Logical device status is set to NDAS_LOGICALDEVICE_STATUS_MOUNTED")
	...
	PDEV_BROADCAST_HANDLE pdbch)
{
	...
	DBGPRT_INFO(_FT("LogDevSlotNo %d, Type %s\n"), dni.LogDevSlotNo, DevNotifyInfoTypeString(dni.Type));
