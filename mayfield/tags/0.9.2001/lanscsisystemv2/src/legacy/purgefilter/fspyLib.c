...
--*/
{
...
#ifdef __LSFILTER__
	{
		...
		SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
						("FileSpy!SpyAttachToFileSystemDevice: Supported Device Type Device Name : %wZ\n",
						DeviceName
						) );
	}		
...
#ifdef __LSFILTER__
	{
		...
		if(result == 1) {
			SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
						("FileSpy!SpyAttachToFileSystemDevice: Name compare %d %wZ %wZ\n",
						result,
						&devExt->DeviceName,
						&ntfsName) );

			devExt->FileSystemType = FILE_SYSTEM_NTFS;
		} else {
			RtlInitUnicodeString( &fastFatName, L"\\Fat" );
			...
			if(result == 1)
				devExt->FileSystemType = FILE_SYSTEM_FAT;
			else
				devExt->FileSystemType = FILE_SYSTEM_UNKNOWN;
		}
	}
...
#else
        status = SpyEnumerateFileSystemVolumes( DeviceObject, &tempName, filespyDeviceObject );
#endif
                ...
                if (Name->Length <= 0) {
                    ...
                    if (NT_SUCCESS( status )) {
						...
						{		
							...
							result = SpyFsFilteringIt(SpyFsDeviceObject, devList[i], diskDeviceObject);
			
							SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
											("SpyEnumerateFileSystemVolumes: SpyFsFilteringIt result = %d\n", result));

							((PFILESPY_DEVICE_EXTENSION)(SpyFsDeviceObject->DeviceExtension))->FilteringIt 
						...
						{
							switch(((PFILESPY_DEVICE_EXTENSION)(SpyFsDeviceObject->DeviceExtension))->FileSystemType)
							{
							case FILE_SYSTEM_FAT:
							{
								...
								SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
									("SpyFsControlMountVolume: WnetFatFilteringIt result = %d\n", result));
							...
							default:
								...
								((PFILESPY_DEVICE_EXTENSION)(SpyFsDeviceObject->DeviceExtension))->FilteringIt = FALSE;
						...
						if(((PFILESPY_DEVICE_EXTENSION)(SpyFsDeviceObject->DeviceExtension))->FilteringIt == FALSE)
							goto CLEANUP;
...
#ifdef __LSFILTER__
					        SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
											("newDeviceObject = %p, newDevExt = %p\n", 
											newDeviceObject, newDevExt));

							newDevExt->FileSystemType = 
								((PFILESPY_DEVICE_EXTENSION)(SpyFsDeviceObject->DeviceExtension))->FileSystemType;
							newDevExt->BaseVolumeDeviceObject = devList[i];
							newDevExt->Buffer = NULL;
