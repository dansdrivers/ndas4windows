...
//#include "RmtHookDevice.h"

#include <ntddscsi.h>

#define IOCTL_SIGNATURE						"XIMETA_1"
#define LANSCSIMINIPORT_IOCTL_GET_SLOT_NO	0x00000001
#define LANSCSIMINIPORT_IOCTL_READ_ONLY		0x00000002
...
--*/
{
	...
	gFileSpyDebugLevel |= SPYDEBUG_TRACE_LEVEL1;
//	gFileSpyDebugLevel |= SPYDEBUG_TRACE_LEVEL3;

	DbgPrint("FileSpy DriverEntry %s %s\n", __DATE__, __TIME__);
#endif
...
#ifdef __LSFILTER__
    DriverObject->MajorFunction[IRP_MJ_DIRECTORY_CONTROL] = SpyFsDirectoryControl;
#endif
...
//
	...
	if(IS_WINDOWS2K())
	{
		...
		SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
...
--*/
{
	...
	switch(((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->FileSystemType)
	{
	case FILE_SYSTEM_NTFS:
	{
		if(IS_WINDOWS2K())
		{
			...
			SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL3, 
							("SpyFastIoQueryOpen flushResult = %d\n", 
							flushResult));
	...
	)
{
	...
	outbuff_sz = sizeof(SRB_IO_CONTROL) + sizeof(ULONG) ;
	...
	memcpy(psrbioctl->Signature, IOCTL_SIGNATURE, 8) ;
	psrbioctl->Timeout = 10 ;
	psrbioctl->ControlCode = LANSCSIMINIPORT_IOCTL_READ_ONLY ;
	psrbioctl->Length = sizeof(ULONG) ;

	status = SpyFsDeviceIoControl(	DiskDeviceObject,
									IOCTL_SCSI_MINIPORT,
									psrbioctl,
									outbuff_sz,
									psrbioctl,
	...
	if(NT_SUCCESS(status)) {
		desiredAccess = *(ACCESS_MASK *)(((unsigned char*)(psrbioctl)) + psrbioctl->HeaderLength) ;
	    SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
						("desiredAccess = %x, GENERIC_WRITE = %x\n", 
...

{
	...
	((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->FilteringIt = FALSE;

	if(IS_WINDOWS2K() || IS_WINDOWSXP())
	{
		if(((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->FileSystemType)
		{
			BOOLEAN	result;
//			NETDISK_CONTEXT	NetdiskContext;


			result = SpyFsFilteringIt(DeviceObject, 
										pIrpSp->Parameters.MountVolume.Vpb->DeviceObject, 
										pIrpSp->Parameters.MountVolume.Vpb->RealDevice
//										&NetdiskContext
										);
			
			((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->FilteringIt 
				= ((result == TRUE) ? TRUE : FALSE); 					

			SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
				("SpyFsControlMountVolume: SpyFsFilteringIt result = %d\n", result));
		}
	}


//	if(((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->FilteringIt == TRUE)
//		PrepareRemoteHookDevice(DeviceObject, NetdiskContext);
...
--*/
{
    ...
    if (NT_SUCCESS( Irp->IoStatus.Status )) {
		...
		if(IS_WINDOWSNET()) {
			switch(((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->FileSystemType)
			{
			case FILE_SYSTEM_FAT:
			{
				BOOLEAN	result;
			
				
				result = WnetFatFilteringIt (
								DeviceObject,
								vpb->DeviceObject
								);
				...
				SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1,
						("SpyFsControlMountVolumeComplete: WnetFatFilteringIt result = %d\n", result));
		...
		if(((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->FilteringIt == FALSE)
			goto CLEANUP;
        ...
        if (!SpyIsAttachedToDevice( vpb->DeviceObject, &attachedDeviceObject )) {
            ...
            if (!NT_SUCCESS( status )) {
				...
				newDevExt->FileSystemType = devExt->FileSystemType;
				newDevExt->BaseVolumeDeviceObject = vpb->DeviceObject;
				newDevExt->Buffer =  ExAllocatePool(NonPagedPool, FS_BUFFER_SIZE);
...

{
...

	{
		...
		SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL3, 
					...
					((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->FileSystemType));
		...
		if(((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->DiskDeviceObject == NULL) // It's Control Device 
		{
			ASSERT(FALSE); 
			goto NEXT;
		}
		...
		case FILE_SYSTEM_NTFS:
		{
			if(IS_WINDOWS2K())
			{
				W2kNtfsFlushOnDirectoryControl(DeviceObject, irpSp);

			} else if(IS_WINDOWSXP())
			{
				WxpNtfsFlushOnDirectoryControl(DeviceObject, irpSp);
			}
		...
		case FILE_SYSTEM_FAT:
		{
			if(IS_WINDOWS2K())
			{
				W2kFatFlushOnDirectoryControl(DeviceObject, irpSp);
			
			} else if(IS_WINDOWSXP())
			{
				WxpFatFlushOnDirectoryControl(DeviceObject, irpSp);
			} else if(IS_WINDOWSNET())
			{
				WnetFatFlushOnDirectoryControl(DeviceObject, irpSp);
			}
