...
VOID
CRegistryCfg::
SetCfgRegKey(LPCTSTR szCfgRegKey)
{
...
VOID
CRegistryCfg::
SetDefaultValueProvider(IRegistryCfgDefaultValue *pProvider)
{
...
BOOL 
CRegistryCfg::
DeleteContainer(LPCTSTR szContainer, BOOL bDeleteSubs)
{
...
BOOL 
CRegistryCfg::
DeleteValue(LPCTSTR szContainer, LPCTSTR szValueName)
{
	BOOL bCreate(FALSE);
	...
	const BYTE* lpValue, DWORD cbValue)
{
	...
	LONG lResult = ::RegSetValueEx(hKey, szValueName, NULL, dwRegValueType, lpValue, cbValue);
	if (ERROR_SUCCESS != lResult) {
		::RegCloseKey(hKey);
		return FALSE;
	}
...
BOOL 
CRegistryCfg::
SetEntropy(LPBYTE lpbEntropy, DWORD cbEntropy)
{
