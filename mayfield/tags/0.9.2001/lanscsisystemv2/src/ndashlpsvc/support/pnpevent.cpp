...
#define XDEBUG_MODULE_FLAG 0x00200000
#include "xdebug.h"
...
LRESULT
CDeviceEventHandler::
OnDeviceEvent(WPARAM wParam, LPARAM lParam)
{
	DPInfo(_FT("%s\n"), DBT_String(wParam));
...
LRESULT
CDeviceEventHandler::
OnCustomEvent(PDEV_BROADCAST_HDR pdbhdr)
{
	return TRUE;
}
...
LRESULT
CDeviceEventHandler::
OnDeviceTypeSpecific(PDEV_BROADCAST_HDR pdbhdr)
{
	return TRUE;
}
...
LRESULT 
CDeviceEventHandler::
OnUserDefined(_DEV_BROADCAST_USERDEFINED* pdbuser)
{
	return TRUE;
}
