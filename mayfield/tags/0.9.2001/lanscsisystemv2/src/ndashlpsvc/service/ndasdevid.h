...
class CNdasDeviceId
{
	static const size_t _CCH_BUFFER = 18;
	static const size_t _CB_BUFFER = _CCH_BUFFER * sizeof(TCHAR);

	TCHAR _szBuffer[_CCH_BUFFER];
...
public:
	...
	{
		::CopyMemory(_szBuffer, cDeviceId._szBuffer, _CB_BUFFER);
	}
...
{
	static const size_t _CCH_BUFFER = 26;
	static const size_t _CB_BUFFER = _CCH_BUFFER * sizeof(TCHAR);
...
public:
	...
	{
		::CopyMemory(_szBuffer, cUnitDeviceId._szBuffer, _CB_BUFFER);
	}
	...
	LPCTSTR ToString()
	{
		HRESULT hr = ::StringCchPrintf(
			_szBuffer, 
			_CCH_BUFFER, 
			TEXT("%02X:%02X:%02X:%02X:%02X:%02X@%d"),
...
{
	return (lhs.Node[0] == rhs.Node[0] && lhs.Node[1] == rhs.Node[1] &&
		lhs.Node[2] == rhs.Node[2] && lhs.Node[3] == rhs.Node[3] &&
		lhs.Node[4] == rhs.Node[4] && lhs.Node[5] == rhs.Node[5]);
}
