...
#define XDEBUG_MAIN_MODULE
#include "xdbgflags.h"
#define XDEBUG_APPNAME TEXT("ndashlpsvc")
#define XDEBUG_MODULE_FLAG XDF_MAIN
#include "xdebug.h"
...
int __cdecl wmain(DWORD argc, LPWSTR *argv)
{
	...
	DPInfo(TEXT("WSAStartup(2,2) returns %d.\n"), iResult);
	if (0 != iResult) {
		DPError(TEXT("WSAStartup failed with error %d\n"), ::WSAGetLastError());
		return -1;
	}

	if (!XDbgInit(_T("ndashlpsvc"))) {
		OutputDebugString(_T("XDBGInit failed.\n"));
	}

	XDebugEventTrace xdEventTracer(
		&XDebugEventTraceControlGuid, 
		&XDebugEventTraceTransactionGuid);

	XDebugConsoleOutput xdConsole;
	XDebugSystemOutput xdSystem;

	fSuccess = xdEventTracer.Register();
	_ASSERT(fSuccess);

	DWORD dwXDebugOutputLevel = XDebug::OL_TRACE;
	DWORD dwXDebugEnabledModules = XDF_NONE;
	BOOL bXDebugUseEventTrace;
	BOOL bXDebugUseConsoleOutput;
	BOOL bXDebugUseSystemDebugOutput;

	fSuccess = _NdasServiceCfg.GetValue(_T("EventTrace"), &bXDebugUseEventTrace);
	if (!fSuccess) {
		DWORD dwError = ::GetLastError();
		bXDebugUseEventTrace = FALSE;
	}

	fSuccess = _NdasServiceCfg.GetValue(_T("ConsoleOutput"), &bXDebugUseConsoleOutput);
	if (!fSuccess) {
		DWORD dwError = ::GetLastError();
		bXDebugUseConsoleOutput = FALSE;
	}

	fSuccess = _NdasServiceCfg.GetValue(_T("SystemDebugOutput"), &bXDebugUseSystemDebugOutput);
	if (!fSuccess) {
		DWORD dwError = ::GetLastError();
		bXDebugUseSystemDebugOutput = FALSE;
	}

	fSuccess = _NdasServiceCfg.GetValue(_T("DebugLevel"), &dwXDebugOutputLevel);
	if (!fSuccess) {
		DWORD dwError = ::GetLastError();
		dwXDebugOutputLevel = XDebug::OL_NONE;
	}

	fSuccess = _NdasServiceCfg.GetValue(_T("DebugFlags"), &dwXDebugEnabledModules);
	if (!fSuccess) {
		DWORD dwError = ::GetLastError();
		dwXDebugEnabledModules = XDF_NONE;
	}


	XDbgSetOutputLevel(dwXDebugOutputLevel);
	XDbgSetModuleFlags(dwXDebugEnabledModules);
	
	if (bXDebugUseEventTrace) {
		XDbgAttach(&xdEventTracer);
	}
	if (bXDebugUseSystemDebugOutput) {
		XDbgAttach(&xdSystem);
	}
	if (bXDebugUseConsoleOutput) {
		XDbgAttach(&xdConsole);
	}

	DPAlways(_T("Debug Level: %d\n"), dwXDebugOutputLevel);
	DPAlways(_T("Debug Flags: 0x%08X "), dwXDebugEnabledModules);
	
	XDbgShowEnabledFlags();

	ximeta::PCService pNdasService;
	pNdasService = CNdasService::Instance();

	if (NULL == pNdasService) {
		DPError(TEXT("Out of memory\n"));
		// out of memory
		...
		((*argv[1] == '-') || (*argv[1] == '/')) )
	{
		if ( lstrcmpi( _T("install"), argv[1]+1 ) == 0 )
		{
			...
			BOOL fSuccess = CNdasServiceInstaller::Install(szBinaryPath);
		...
		{
			BOOL fSuccess = CNdasServiceInstaller::Remove();
...
dispatch:
	...
	if (!fSuccess) {
		_tprintf(
			...
			::GetLastError());
		//	ndasService->ReportEventError(TEXT("StartServiceCtrlDispatcher failed."));
	}
