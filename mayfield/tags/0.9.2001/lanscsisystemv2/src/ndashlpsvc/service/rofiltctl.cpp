...
BOOL NdasRoFilterQueryServiceStatus(SC_HANDLE hSCManager, LPSERVICE_STATUS lpServiceStatus)
{
	...
	if (!fSuccess) {
		// Close service handle may shadow the actual error
...
BOOL NdasRoFilterStartFilter(HANDLE hFilter)
{
	BOOL fSuccess(FALSE);
...
BOOL NdasRoFilterStopFilter(HANDLE hFilter)
{
	BOOL fSuccess(FALSE);
...
BOOL NdasRoFilterEnableFilter(HANDLE hFilter, DWORD dwDriveNumber, BOOL bEnable /* = TRUE */)
{
	BOOL fSuccess(FALSE);
