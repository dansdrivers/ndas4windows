				...
				)
{
	...
	ULONG count = 0;
	ULONG i = 0;
	count = FindVDVDPdoCount(FdoData);
	if(pHdInfo->COUNT > count){
		pHdInfo->COUNT = count;
	}
    PAGED_CODE ();
    ...
    for (entry = FdoData->ListOfPDOs.Flink;
         ...
         entry = entry->Flink) {
             ...
             if((pdoData->LanscsiAdapter.LogicalTarget[0].Llu[0].ucLluType == DISK_TYPE_VDVD)
				 &&(i < count))
			 {
				 PDISC_HEADER pHeader = pdoData->LanscsiAdapter.LogicalTarget[0].Llu[0].Targets[0].TargetConnectionList.pHeader;
				 pHdInfo->DISC[i].SlotNo = pdoData->SlotNo;
				 pHdInfo->DISC[i].DISCS = pHeader->DISCS;
				 pHdInfo->DISC[i].ENABLE_DISCS = pHeader->ENABLE_DISCS;
				 XXprint(4,("COM_GET_VDVD_HD_ALL : #of DISC(%d), #of ENABLED(%d), SlotNo(%d)\n",
							pHdInfo->DISC[i].DISCS, pHdInfo->DISC[i].ENABLE_DISCS, pHdInfo->DISC[i].SlotNo));
				 i++;
			 }
			pdoData = NULL;
		 }
