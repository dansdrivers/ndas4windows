...
//
//	shuold be 1500 because header size( Ethernet 14 ) is subtracted from MaxFrameSize.
//
//		if(*MaxFrameSize > 1514)
//			*MaxFrameSize = 1514;
		if(*MaxFrameSize > 1500) {
			DebugPrint(4, ("[LPX] MacReturnMaxDataSize: MaxFrameSize will be adjusted: = %d\n", *MaxFrameSize));
			*MaxFrameSize = 1500;
		}
        break;
...
//
//	shuold be 1500 because header size( Ethernet 14 ) is subtracted from MaxFrameSize.
//	TODO: should allow to adapt other NIC types.
//
//		if(*MaxFrameSize > 1514)
//			*MaxFrameSize = 1514;
		if(*MaxFrameSize > 1500) {
...
//	DbgPrint("MaxFrameSize : = %d\n", *MaxFrameSize);
	*MaxFrameSize -= sizeof(LPX_HEADER2);
//	DbgPrint("*MaxFrameSize -= sizeof(LPX_HEADER2) : = %d\n", *MaxFrameSize);
	*MaxFrameSize >>= 2;
//	DbgPrint("*MaxFrameSize >>= 2 :  = %d\n", *MaxFrameSize);
	*MaxFrameSize <<= 2;
//	DbgPrint("*MaxFrameSize <<= 2 :  = %d\n", *MaxFrameSize);
	*MaxFrameSize += sizeof(LPX_HEADER2);
//	DbgPrint("*MaxFrameSize += sizeof(LPX_HEADER2) :  = %d\n", *MaxFrameSize);
