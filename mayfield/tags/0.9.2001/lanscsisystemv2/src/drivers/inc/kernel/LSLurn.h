...
#define 	LURN_MIRRORING_V2		0x0005

typedef	UINT16 LURN_TYPE, *PLURN_TYPE;

#define NR_LURN_INTERFACE			6
...
typedef struct _LURELATION LURELATION, *PLURELATION ;
typedef struct _RAID_01_INFO RAID_01_INFO, *PRAID_01_INFO ;
...
typedef struct _LURELATION_NODE {
	...
	union{
		PVOID					LurnExtension;
		PRAID_01_INFO			LurnRaid1MirrorInfo;
	};
...
typedef struct _LURELATION_NODE_DESC {
	...
	union {
		...
		INFO_RAID_1			LurnInfoRAID_1[2]; // 2 disks for each mirrored disks
	};
