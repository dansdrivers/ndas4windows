...
//	extened commands
//
#define		CCB_EXT_BYTE_READ				0x01
#define		CCB_EXT_BYTE_WRITE				0x02
#define		CCB_EXT_BYTE_READ_OPERATE_WRITE	0x03

#define		BYTE_OPERATION_COPY				0x01
#define		BYTE_OPERATION_AND				0x02
#define		BYTE_OPERATION_OR				0x03
...
typedef struct _CCB {
	...
	UINT32				OperationCode;		// 4
	ULONG				CcbStatusFlags;		// 8
	UCHAR				CcbStatus;			// 12
	UCHAR				CdbLength;			// 13
	UCHAR				LurId[LURID_LENGTH];	// 14
	ULONG				DataBufferLength;		// 24
	PVOID				DataBuffer;				// 28
	UCHAR				Cdb[MAXIMUM_CDB_SIZE];	// 32

	// Extended command, used for RAID 1 etc.
	...
	UCHAR				PKCMD[MAXIMUM_CDB_SIZE];
	PVOID				PKDataBuffer;
	ULONG				PKDataBufferLength;
	// Added by ILGU HONG 2004_07_06 End

	ULONG				Flags;
	LIST_ENTRY			ListEntry;				// 48
	PVOID				Srb;
	PVOID				AbortSrb;
	PCCB				AbortCcb;				// 64
	...
	PVOID				HwDeviceExtension;		// 80
	LARGE_INTEGER		CreateTime;

	PKEVENT				CompletionEvent;		// 96
...
#define LSCCB_INITIALIZE(CCB_POINTER) {																		\
	...
	KeQueryTickCount(&(CCB_POINTER)->CreateTime);															\
}
