...
LurnIdeQuery(
	...
	case LurPrimaryLurnInformation: {
			...
			ReturnInfo = (PLURN_PRIMARYINFORMATION)Ccb->DataBuffer;
...
#define CHANGECONNECTION_INTERVAL			(FREQ_PER_SEC*2)
...
#ifdef OEM_RUTTER
//
//	Rutter does not want long reconnecting time.
//
#define RECONNECTION_MAX_TRY				4
#else
#define RECONNECTION_MAX_TRY				16
#endif

#if RECONNECTION_MAX_TRY > 20
#error "RECONNECTION exceeds 20!"
#endif

#define RECONNECTION_MAX_TRY_PRIMARY		((RECONNECTION_MAX_TRY + 1) / 4)
#define RECONNECTION_INTERVAL				(FREQ_PER_SEC*2)
#define RECONNECTION_INTERVAL_SECONDARY		(FREQ_PER_SEC*4)
...
LSLurnIdeReconnect(
	...
	TimeInterval.LowPart = - RECONNECTION_INTERVAL;
	MaxStall = RECONNECTION_MAX_TRY;
#else
	if(Lurn->Lur->DesiredAccess & GENERIC_WRITE) {
		if(Lurn->Lur->GrantedAccess & GENERIC_WRITE) {
			...
			TimeInterval.HighPart = -1;
			TimeInterval.LowPart = - RECONNECTION_INTERVAL;
			MaxStall = RECONNECTION_MAX_TRY_PRIMARY;
		} else {
			//
			//	NDFS Secondary mode.
			//
			TimeInterval.HighPart = -1;
			TimeInterval.LowPart = - RECONNECTION_INTERVAL_SECONDARY;
			MaxStall = RECONNECTION_MAX_TRY;
		}
	} else {
		//
	...
	if(StallCount>MaxStall) {
		KDPrintM(DBG_LURN_ERROR,("reconnection trial reach the maximum! Connection will be lost.\n"));
	...
	status = LspReconnectAndLogin(&IdeExt->LanScsiSession, IdeExt->LstransType);
	if(!NT_SUCCESS(status)) {
	...
	)
{
	...
	// identify.
	LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_IDENTIFY, 0, sizeof(struct hd_driveid), &info);
	...
	do {
		...
		// Set DMA mode if needed.
		if(SetDmaMode) {
			LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_SETFEATURES, 0, 0, NULL);
			PduDesc.Feature = 0x03;
			PduDesc.Addr[0] = DmaFeature;
			ntStatus = LspRequest(LSS, &PduDesc, PduResponse);
			if(!NT_SUCCESS(ntStatus) || *PduResponse != LANSCSI_RESPONSE_SUCCESS) {
	...
	if(IdeDisk->HwVersion == LANSCSIIDE_VERSION_2_0 && IdeDisk->LuData.PduDescFlags & PDUDESC_FLAG_UDMA) {
		...
		transferBlocks = 1;
		LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_WRITE,logicalBlockAddress,transferBlocks, OneSector);
		ntStatus = LspRequest(
	...
	)
{
	...
	if(!NT_SUCCESS(status)) {
		KDPrintM(DBG_LURN_ERROR, ("LspLogin(), Can't Connect to the LS node with UserID:%08lx. STATUS:0x%08x.\n", LoginInfo.UserID, status));
	...
	//	Configure the disk.
	//
	status = LurnIdeDiskConfiure(Lurn, LurnDesc->LurnIde.LanscsiTargetID, &response);
	if(!NT_SUCCESS(status) || response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_LURN_ERROR, 
			("LurnIdeDiskConfiure failed. Response:%d. NTSTATUS:%d\n", response, status));
	}
...
LurnIdeDiskRequest(
	...
	case CCB_OPCODE_STOP:
		...
		LurnIdeCompleteAllCcb(&IdeDisk->ThreadCcbQueue, CCB_STATUS_NOT_EXIST, CCBSTATUS_FLAG_LURN_STOP);
	...
	while(1) {
		...
		case CCB_OPCODE_UPDATE:
			...
			if(NT_SUCCESS(status) && Ccb->CcbStatus == CCB_STATUS_SUCCESS) {
				LurnIdeDiskConfiure(Lurn, IdeDisk->LuData.LanscsiTargetID, &PduResponse);
			}
			break;
...
try_reconnect:
		...
		if(status == STATUS_PORT_DISCONNECTED ||
			...
			if(status == STATUS_CONNECTION_COUNT_LIMIT) {
				...
				//
				status = LurnIdeDiskConfiure(Lurn, IdeDisk->LuData.LanscsiTargetID, &PduResponse);
				if(!NT_SUCCESS(status)) {
					ACQUIRE_SPIN_LOCK(&Lurn->LurnSpinLock, &oldIrql);
					Lurn->LurnStatus = LURN_STATUS_STALL;
					RELEASE_SPIN_LOCK(&Lurn->LurnSpinLock, oldIrql);
   ...
   )
{
	...
	do {
		...
		if(!NT_SUCCESS(status)){
			KDPrintM(DBG_LURN_ERROR, ("KeWaitForMultipleObjects Fail\n"));
			break;
		}
...
//		ASSERT(IdeDisk->LuData.PduDescFlags&PDUDESC_FLAG_LBA48);
	KDPrintM(DBG_LURN_ERROR, ("0x%08x(%d)\n", logicalBlockAddress, transferBlocks));
	LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_READ, logicalBlockAddress,transferBlocks, DataBuffer);
	ntStatus = LspRequest(
					&IdeDisk->LanScsiSession,
					&PduDesc,
					&response
		...
		case LANSCSI_RESPONSE_T_COMMAND_FAILED:
			{
				...
				for(i = 0; i < transferBlocks; i++) {
					...
					LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_READ,BadSector,1, &((PUCHAR)DataBuffer)[i]);
					ntStatus = LspRequest(
									&IdeDisk->LanScsiSession,
									&PduDesc,
									&response
...
{		
	...
	LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_WRITE,logicalBlockAddress,transferBlocks, DataBuffer);
	...
	if(!NT_SUCCESS(ntStatus) || response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_LURN_ERROR, 
			("logicalBlockAddress = %x, transferBlocks = %x\n", 
			logicalBlockAddress, transferBlocks));
	}
		...
		case LANSCSI_RESPONSE_T_COMMAND_FAILED:
			{
				...
				for(i = 0; i < transferBlocks; i++) {
					...
					LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_WRITE,BadSector,1, &((PUCHAR)DataBuffer)[i]);
					ntStatus = LspRequest(
									&IdeDisk->LanScsiSession,
									&PduDesc,
									&response
	...
	)
{
	...
	pExtendedCommand = Ccb->pExtendedCommand;
	while(NULL != pExtendedCommand)
	{
		// linked list
		switch(((PBYTE)pExtendedCommand)[0])
		{
		case CCB_EXT_BYTE_READ:
			{
				BYTE DataBuffer[512]; // one sector
				PCMD_BYTE_OP pCmd = (PCMD_BYTE_OP)pExtendedCommand;

				KDPrintM(DBG_LURN_INFO,	("CCB_EXT_BYTE_READ\n"));

				// read sector

				ntStatus = LurnIdeDiskRead(Ccb, IdeDisk, DataBuffer, pCmd->logicalBlockAddress, 1);
		...
		case CCB_EXT_BYTE_WRITE:
			{
				...
				if(!(Lurn->AccessRight & GENERIC_WRITE)){
					...
					if(Lurn->Lur->LurFlags & LUR_FLAG_FAKEWRITE) {
						KDPrintM(DBG_LURN_ERROR, ("CCB_EXT_BYTE_WRITE: Fake Write. Address:%lu\n", pCmd->logicalBlockAddress));
...
//						return STATUS_SUCCESS; 
				...
				KDPrintM(DBG_LURN_INFO,	("CCB_EXT_BYTE_WRITE\n"));

//				RtlZeroMemory(DataBuffer, sizeof(DataBuffer));

				switch(pCmd->ByteOperation)
				{
				case BYTE_OPERATION_COPY:
					RtlCopyMemory(DataBuffer + pCmd->Offset, pCmd->Byte, pCmd->LengthByte);
					break;
				case BYTE_OPERATION_AND:
				case BYTE_OPERATION_OR:
				default:
					ASSERT(FALSE);
					// NA JUNG YE
					break;
				...
				// write back
				ntStatus = LurnIdeDiskWrite(Ccb, IdeDisk, DataBuffer, pCmd->logicalBlockAddress, 1);
			...
			break;
		case CCB_EXT_BYTE_READ_OPERATE_WRITE:
			{
				BYTE DataBuffer[512]; // one sector
				PCMD_BYTE_OP pCmd = (PCMD_BYTE_OP)pExtendedCommand;

				KDPrintM(DBG_LURN_ERROR,	("CCB_EXT_BYTE_READ_OPERATE_WRITE\n"));
				...
				ntStatus = LurnIdeDiskRead(Ccb, IdeDisk, DataBuffer, pCmd->logicalBlockAddress, 1);
				KDPrintM(DBG_LURN_ERROR,	("CCB_EXT_BYTE_READ_OPERATE_WRITE read status %x\n", ntStatus));
...
//						return STATUS_SUCCESS; 
				...
				KDPrintM(DBG_LURN_ERROR,	("CCB_EXT_BYTE_READ_OPERATE_WRITE write status %x\n", ntStatus));
		...
		pExtendedCommand = (PCMD_COMMON)pExtendedCommand->pNextCmd;
	}
...
//		inquiryData.EnclosureServices;
	...
	case SCSIOP_VERIFY: 
	{
		...
		logicalBlockAddress = ((PCDB)Ccb->Cdb)->CDB10.LogicalBlockByte3 |
							((PCDB)Ccb->Cdb)->CDB10.LogicalBlockByte2 << 8 |
							((PCDB)Ccb->Cdb)->CDB10.LogicalBlockByte1 << 16 |
							((PCDB)Ccb->Cdb)->CDB10.LogicalBlockByte0 << 24;

		totalTransferBlocks = (USHORT)(((PCDB)Ccb->Cdb)->CDB10.TransferBlocksMsb << 8 |
...
DO_MORE:
		...
		LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_VERIFY,logicalBlockAddress,transferBlocks, 0);
		ntStatus = LspRequest(
							&IdeDisk->LanScsiSession,
							&PduDesc,
							&response
			...
			case LANSCSI_RESPONSE_T_COMMAND_FAILED:
				{
					...
					for(i = 0; i < transferBlocks; i++) {
						...
						LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_VERIFY,BadSector,transferBlocks, 0);
						ntStatus = LspRequest(
										&IdeDisk->LanScsiSession,
										&PduDesc,
										&response
	...
	case SCSIOP_START_STOP_UNIT: 
	{
		...
		if(cdb->START_STOP.Start == START_UNIT_CODE) {
			...
			//
			ntStatus = LurnIdeDiskConfiure(Lurn, Ccb->LurId[1], &response);
			if(response != LANSCSI_RESPONSE_SUCCESS) {
				KDPrintM(DBG_LURN_ERROR, 
	...
	case SCSIOP_READ: 
	{
		...
		if (transferBlocks > IdeDisk->LanScsiSession.MaxBlocksPerRequest) {
			KDPrintM(DBG_LURN_ERROR, 
				("SCSIOP_READ: verify too many sectors 0x%x, MAX_TRANSFER_BLOCS 0x%x\n",
				transferBlocks, IdeDisk->LanScsiSession.MaxBlocksPerRequest));
		...
		// Send Read Request.
		//
		ntStatus = LurnIdeDiskRead(Ccb, IdeDisk, Ccb->DataBuffer, logicalBlockAddress, transferBlocks);
		if(!NT_SUCCESS(ntStatus))
		{
			return ntStatus;
		}
	...
	case SCSIOP_WRITE: 
	{
		...
		if (transferBlocks > IdeDisk->LanScsiSession.MaxBlocksPerRequest) {
 			KDPrintM(DBG_LURN_ERROR, 
				("SCSIOP_WRITE: verify too many sectors 0x%x, MAX_TRANSFER_BLOCS 0x%x\n",
                    transferBlocks, IdeDisk->LanScsiSession.MaxBlocksPerRequest));
	...
	case SCSIOP_SYNCHRONIZE_CACHE:
		...
		Ccb->CcbStatus = CCB_STATUS_SUCCESS;
		ntStatus = STATUS_SUCCESS;
		KDPrintM(DBG_LURN_ERROR, ("SCSIOP_SYNCHRONIZE_CACHE CcbStatus:%d!!\n", Ccb->CcbStatus));
		break;
	...
	)
{
	...
	// identify.
	LURNIDE_INITILIZE_PDUDESC(IdeDisk, &PduDesc, IDE_COMMAND, WIN_PIDENTIFY, 0, sizeof(struct hd_driveid), &info);
	ntStatus = LspRequest(LSS, &PduDesc, PduResponse);
	if(!NT_SUCCESS(ntStatus) || *PduResponse != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_LURN_ERROR, ("Identify Failed...\n"));
		return ntStatus;
...
static void TranslateScsi2Packet(PCCB Ccb)
{
	...
	Ccb->PKDataBufferLength = Ccb->DataBufferLength;
	Ccb->PKDataBuffer = Ccb->DataBuffer;
	...
	if(c[0] == SCSIOP_MODE_SENSE || c[0] == SCSIOP_MODE_SELECT) {
		...
		if(sc[0] == SCSIOP_MODE_SELECT){
			...
			KDPrintM(DBG_LURN_TRACE,("MODE_SENSE/SELECT PKCMD 0x%02x\n",c[0]));	
			bufsize = sc_buf[0] + 2;
			KDPrintM(DBG_LURN_ERROR,("Buffer size set (%d): Allocated len (%d) : Ccb Set buffersize %d\n",size, Ccb->DataBufferLength, size));
			buf[1] = sc_buf[0];
			buf[2] = sc_buf[1];
			buf[3] = sc_buf[2];
			buf[7] = sc_buf[3];
			RtlCopyMemory(buf + 8, sc_buf + 4, Ccb->DataBufferLength -4);
		}
		Ccb->PKDataBuffer = buf;
		Ccb->PKDataBufferLength = Ccb->DataBufferLength + 4;
	}
	...
	if(c[0] == 0xD8) {
		...
		DataBufferLength = Ccb->PKDataBufferLength;
			  ...
			  )
{
		...
		((PCDB)&tempCCB.Cdb)->CDB6INQUIRY.AllocationLength = sizeof(SENSE_DATA);
		((PCDB)&tempCCB.Cdb)->CDB6INQUIRY.Control = 0;
		
		RtlCopyMemory(tempCCB.PKCMD, tempCCB.Cdb, 6);
		tempCCB.PKDataBuffer = pSenseData;
		tempCCB.PKDataBufferLength = sizeof(SENSE_DATA);
			  ...
			  )
{
		...
		RtlZeroMemory(&tempCCB.Cdb,sizeof(CDB));
		...
		RtlCopyMemory(tempCCB.PKCMD, tempCCB.Cdb, 6);
		tempCCB.PKDataBuffer = &Data;
		tempCCB.PKDataBufferLength = 36;
			  ...
			  )
{
		...
		tempCCB.PKDataBuffer = &RowSectorCount;
		tempCCB.PKDataBufferLength = sizeof(MY_GET_RAWSECTORCOUT);
						...
						)
{
	...
	*NeedRepeat = 0;
		...
		)
	{
		...
		buf = Ccb->PKCMD;
		KDPrintM(DBG_LURN_ERROR,(
						"(0)[0x%02x],(1)[0x%02x],(2)[0x%02x],(3)[0x%02x],(4)[0x%02x],(5)[0x%02x],(6)[0x%02x],(7)[0x%02x],(8)[0x%02x],(9)[0x%02x],(10)[0x%02x],(11)[0x%02x] : PKDataBufferlen (%d)\n",
						buf[0],buf[1],buf[2],buf[3],buf[4],buf[5],buf[6],buf[7],buf[8],buf[9],buf[10],buf[11],Ccb->PKDataBufferLength));
		if(Ccb->Srb)
		{
			Srb = Ccb->Srb;
			KDPrintM(DBG_LURN_ERROR,("Max time out value %d\n",Srb->TimeOutValue));
		}
	}
...
//
				...
				default:
					{
						if( (Ccb->SenseBuffer)
							&& ( Ccb->SenseDataLength >= sizeof(SENSE_DATA)) )
						{
...
*/
	...
	case SCSIOP_INQUIRY:
		{
			...
			if((ULONG)transferLen != Ccb->PKDataBufferLength)
			{
			
				if(Ccb->PKDataBufferLength < (ULONG)transferLen)
				{
					transferLen = (BYTE)Ccb->PKDataBufferLength;
					Ccb->PKCMD[4] = (transferLen & 0xff);

				}else if(Ccb->PKDataBufferLength > (ULONG)transferLen)
				{
					Ccb->PKCMD[4] = (transferLen & 0xff);
					Ccb->PKDataBufferLength = transferLen;
				}
			}
	...
	case SCSIOP_READ_DVD_STRUCTURE:
		{
			...
			if(transferLen != Ccb->PKDataBufferLength)
			{
			
				if(Ccb->PKDataBufferLength < transferLen)
				{
					transferLen = (USHORT)Ccb->PKDataBufferLength;
					Ccb->PKCMD[8] = ((transferLen >> 8) & 0xff);
					Ccb->PKCMD[9] = (transferLen & 0xff);

				}else if(Ccb->PKDataBufferLength > transferLen)
				{
					Ccb->PKCMD[8] = ((transferLen >> 8) & 0xff);
					Ccb->PKCMD[9] = (transferLen & 0xff);
					Ccb->PKDataBufferLength = transferLen;
	...
	case SCSIOP_READ_CAPACITY:
		if(Ccb->PKDataBufferLength > 8) Ccb->PKDataBufferLength = 8;
		break;
	...
	case SCSIOP_READ:
		{
			if(IdeDisk->DataSectorSize > 0)
			{
				...
				if(Ccb->PKDataBufferLength > TransferSize)
				{
					KDPrintM(DBG_LURN_ERROR,("[SCSIOP_READ] changed size %d\n",TransferSize));
					Ccb->PKDataBufferLength = TransferSize;
				}
	...
	case SCSIOP_REPORT_LUNS:
		{
			...
			lunlist = (PLUN_LIST)Ccb->PKDataBuffer;
			...
			{
				{
					...
					KDPrintM(DBG_LURN_ERROR,("[ERROR ABORT]\n"
									"(0)[0x%02x],(1)[0x%02x],(2)[0x%02x],(3)[0x%02x],(4)[0x%02x],(5)[0x%02x],(6)[0x%02x],(7)[0x%02x],(8)[0x%02x],(9)[0x%02x],(10)[0x%02x],(10)[0x%02x]\n",
		...
		if(status == STATUS_PORT_DISCONNECTED ||
			...
			//
			{
				...
				KDPrintM(DBG_LURN_ERROR,("[ERROR RECONN]\n"
								"(0)[0x%02x],(1)[0x%02x],(2)[0x%02x],(3)[0x%02x],(4)[0x%02x],(5)[0x%02x],(6)[0x%02x],(7)[0x%02x],(8)[0x%02x],(9)[0x%02x]\n",
	...
	) 
{
	...
	if(Lurn->LurnType == LURN_IDE_ODD)
	{
		...
		IdeDisk->DVD_ENC_IR[0] = pPassword[1] ^ pPassword[7] ^ pKey[3];
		IdeDisk->DVD_ENC_IR[1] = pPassword[0] ^ pPassword[3] ^ pKey[0];
		IdeDisk->DVD_ENC_IR[2] = pPassword[2] ^ pPassword[6] ^ pKey[2];
		IdeDisk->DVD_ENC_IR[3] = pPassword[4] ^ pPassword[5] ^ pKey[1];

		IdeDisk->DVD_DEC_IR[0] = ~(pPassword[0] ^ pPassword[3] ^ pKey[0]);
		IdeDisk->DVD_DEC_IR[1] = pPassword[2] ^ pPassword[6] ^ pKey[2];
		IdeDisk->DVD_DEC_IR[2] = pPassword[4] ^ pPassword[5] ^ ~(pKey[1]);
		IdeDisk->DVD_DEC_IR[3] = pPassword[1] ^ pPassword[7] ^ pKey[3];
