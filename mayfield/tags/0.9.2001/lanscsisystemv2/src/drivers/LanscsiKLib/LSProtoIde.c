	...
	*pTotalSent = 0;
	...
	while (len > 0) {
		...
		*pTotalSent += result;
		...
		KDPrintM(DBG_PROTO_NOISE, ("len %d, Total %d \n", len, *pTotalSent));
	}
	...
	)
{
	...
	if(NTOHS(pPdu->pR2HHeader->AHSLen) > 0) {
		...
		if(recv != NTOHS(pPdu->pR2HHeader->AHSLen)) {
	...
	if(NTOHL(pPdu->pR2HHeader->DataSegLen) > 0) {
		...
		if(recv != NTOHL(pPdu->pR2HHeader->DataSegLen)) {
	...
	case WIN_SETFEATURES:
		{
			...
			pRequestHeader->SectorCount_Curr = PduDesc->Addr[0];
	 ...
	 )
{
	...
	KDPrintM(DBG_PROTO_ERROR, ("HWProtoType:0x%x, HWProtoVersion:0x%x\n",
							LSS->HWProtoType,
							LSS->HWProtoVersion
						));
	...
	pRequestHeader = (PLANSCSI_LOGIN_REQUEST_PDU_HEADER)PduBuffer;
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_PROTO_ERROR, ("Second Failed.\n"));
		return STATUS_LOGON_FAILURE;
	}
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_PROTO_ERROR, ("Third Failed.\n"));
		return STATUS_LOGON_FAILURE;
	}
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_PROTO_ERROR, ("Fourth Failed.\n"));
		return STATUS_LOGON_FAILURE;
	}
	...
	// Calc Intermidate Result for Encryption.
	{
		PUCHAR	pPassword;
		PUCHAR	pKey;
	...
	)
{
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_PROTO_ERROR, ("Failed.\n"));
		return ntStatus;
	}
