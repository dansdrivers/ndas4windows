...
#if DBG
	SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO, ("Original Address List\n"));
	DbgPrintLpxAddrList(LFS_DEBUG_PRIMARY_INFO, Original) ;
	SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO, ("Updated Address List\n"));
	DbgPrintLpxAddrList(LFS_DEBUG_PRIMARY_INFO, Updated) ;
	SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO, ("Disabled Address List\n"));
	DbgPrintLpxAddrList(LFS_DEBUG_PRIMARY_INFO, Disabled) ;
	SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO, ("Enabled Address List\n"));
	DbgPrintLpxAddrList(LFS_DEBUG_PRIMARY_INFO, Enabled) ;
#else
...
{
	SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
				("PrimaryPassThroughCompletion: called\n"));
	...
	)
{
		...
		&& (irpSp->MinorFunction == IRP_MN_USER_FS_REQUEST || irpSp->MinorFunction == IRP_MN_KERNEL_CALL)) 
	{
		...
		if(
			...
			&& NetdiskManager_ThisVolumeHasSecondary(GlobalLfs.NetdiskManager, LfsDeviceExt->EnabledNetdisk, LfsDeviceExt)
			) 
		{
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
...
/*
// To clean up terminated PrimarySession is performed hear
VOID
Primary_AcceptConnection(
	IN 	PPRIMARY	Primary,
	IN  ULONG		ListenSocketIndex
...
{
	PPRIMARY_LISTEN_SOCKET	listenSocket = &Primary->Agent.ListenSocket[ListenSocketIndex];
	...
	{	
		LpxTdiDisconnect(listenSocket->ListenFileObject, 0);
		LpxTdiDisassociateAddress(listenSocket->ListenFileObject);
...
start_session:
			...
			//	start a session.
			//
			Primary_AcceptConnection(Primary, connFileHandle, connFileObject, ntStatus-1);
...
/*
    Cancelling IRPs at this point can cause BSOD.
    hootch 02102004
		...
		if(Primary->Agent.ListenSocket[i].TdiListenContext.Irp)
		{
			...
			IoCancelIrp(Primary->Agent.ListenSocket[i].TdiListenContext.Irp);

			timeOut.QuadPart = - LFS_TIME_OUT ;
			ntStatus = KeWaitForSingleObject(
				&Primary->Agent.ListenSocket[i].TdiListenContext.CompletionEvent,
				Executive,
				KernelMode,
				FALSE,
				&timeOut
			...
			KeClearEvent(&Primary->Agent.ListenSocket[i].TdiListenContext.CompletionEvent) ;
			Primary->Agent.ListenSocket[i].TdiListenContext.Irp = NULL;
		}
*/
