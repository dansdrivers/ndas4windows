...
//	gFileSpyDebugLevel |= LFS_DEBUG_LFS_NOISE;
...
//	gFileSpyDebugLevel |= LFS_DEBUG_NETDISK_MANAGER_NOISE;

	gFileSpyDebugLevel |= LFS_DEBUG_PRIMARY_ERROR;
	gFileSpyDebugLevel |= LFS_DEBUG_PRIMARY_INFO;
//	gFileSpyDebugLevel |= LFS_DEBUG_PRIMARY_TRACE;
//	gFileSpyDebugLevel |= LFS_DEBUG_PRIMARY_NOISE;
...
//	gFileSpyDebugLevel |= LFS_DEBUG_SECONDARY_NOISE;

//	gFileSpyDebugLevel |= LFS_DEBUG_LIB_ERROR;
//	gFileSpyDebugLevel |= LFS_DEBUG_LFS_INFO;
//	gFileSpyDebugLevel |= LFS_DEBUG_LIB_TRACE;
//	gFileSpyDebugLevel |= LFS_DEBUG_LFS_NOISE;
    ...
    )
{
	...
	SPY_LOG_PRINT(LFS_DEBUG_LFS_ERROR,
			("newLfsDeviceExt = %p %wZ Irp->IoStatus.Status = %x, vpb->RealDevice = %x, vpb->DeviceObject = %x\n", 
				newLfsDeviceExt, &newLfsDeviceExt->NetdiskPartitionInformation.VolumeName, Irp->IoStatus.Status, newLfsDeviceExt->DiskDeviceObject, newLfsDeviceExt->DiskDeviceObject->Vpb->DeviceObject));
	...
	)
{
			...
			&& BooleanFlagOn(lfsDeviceExt->Flags, LFS_DEVICE_START) && BooleanFlagOn(lfsDeviceExt->Flags, LFS_DEVICE_STOPPING)))
	{
		...
		PrintIrp(LFS_DEBUG_LFS_INFO,
					BooleanFlagOn(lfsDeviceExt->Flags, LFS_DEVICE_STOP) ? "LFS_DEVICE_STOP" : "LFS_DEVICE_STOPPING",
					lfsDeviceExt, Irp);
#endif
	}
	...
	if(lfsDeviceExt->Vpb && lfsDeviceExt->Vpb->RealDevice && lfsDeviceExt->BaseVolumeDeviceObject) 
	{
		SPY_LOG_PRINT(LFS_DEBUG_LFS_NOISE,
				("LfsPassThrough: Entered Dev:%p Ref:%d, DevVpb: Flags:%08lx Ref:%d, Disk:%p Ref:%d Flag:%08lx Irp = %p,"
				"Base:%p Ref:%d Flag:%08lx\n",
						DeviceObject,
	...
	case LFS_SECONDARY:
	{
		...
		{
#if DBG
			PrintIrp(LFS_DEBUG_LFS_ERROR, "TOUCH_VOLUME_FILE_NAME", lfsDeviceExt, Irp);
#endif
		...
		if(irpSp->MajorFunction == IRP_MJ_PNP)
		{
			...
			if(fileObject && fileObject->FileName.Length)
				...
				ASSERT(fileObject == NULL || fileObject->FileName.Length == 0);

			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO,
				("LfsPassThrough lfsDeviceExt: %p fileObject = %x IRP_MJ_PNP MinorFunction = %d lfsDeviceExt->SecondaryState = %d\n",
			...
			if(	irpSp->MinorFunction == IRP_MN_QUERY_DEVICE_RELATIONS ||
				...
				" %p fileObject = %x IRP_MJ_PNP MinorFunction = %d lfsDeviceExt->SecondaryState = %d\n",
					lfsDeviceExt, fileObject, irpSp->MinorFunction, lfsDeviceExt->SecondaryState));
	...
	)
{
...
#if 0
	{
		...
		irp = IoAllocateIrp(
				LfsDeviceExt->AttachedToDeviceObject->StackSize,
				TRUE
				);
		...
		irpSp->Parameters.FileSystemControl.FsControlCode = FSCTL_LOCK_VOLUME;
		...
		irpSp->DeviceObject = LfsDeviceExt->AttachedToDeviceObject;
		...
		{
			SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR,
							("PurgeVolume: LfsDeviceExt = %p FSCTL_LOCK_VOLUME irp->IoStatus.Status = %x\n",
							LfsDeviceExt, irp->IoStatus.Status));
		...
		irpSp->MajorFunction = IRP_MJ_FILE_SYSTEM_CONTROL;
		irpSp->MinorFunction = IRP_MN_USER_FS_REQUEST;
		irpSp->Parameters.FileSystemControl.FsControlCode = FSCTL_UNLOCK_VOLUME;
		irpSp->Parameters.FileSystemControl.InputBufferLength = 0;
		irpSp->Parameters.FileSystemControl.OutputBufferLength = 0;
		irpSp->Parameters.FileSystemControl.Type3InputBuffer = NULL;
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR,
						("PurgeVolume: LfsDeviceExt = %p ZwCreateFile fileHandle =%x, createStatus = %X, ioStatusBlock = %X\n",
	...
	do 
	{
		...
		if(fsControlStatus != STATUS_SUCCESS)
			SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR,
							("PurgeVolume: LfsDeviceExt = %p ZwFsControlFile FSCTL_LOCK_VOLUME fileHandle =%x, fsControlStatus = %X, ioStatusBlock = %X\n",
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR,
						("PurgeVolume: LfsDeviceExt = %p NtUnlockFile fileHandle =%x, fsControlStatus = %X, ioStatusBlock = %X\n",
						LfsDeviceExt, fileHandle, fsControlStatus, ioStatusBlock.Information));
	} while(0);
