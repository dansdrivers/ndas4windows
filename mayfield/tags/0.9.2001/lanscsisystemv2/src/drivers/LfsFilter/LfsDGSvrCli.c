					   ...
					   )
{
	...
	if(	RtlCompareMemory(Pkt->RawHeadDG.Protocol, Protocol, 4) != 4 ||
		...
		SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ("[LFS] LfsSvrDatagramRecvHandler: Invalid reply header.\n"));
		goto not_accepted ;
	}
	...
	SPY_LOG_PRINT( LFS_DEBUG_LIB_NOISE, ( "[LFS] LfsSvrDatagramRecvHandler: received a datagram packet from %02X:%02X:%02X:%02X:%02X:%02X/%d\n",
...
not_accepted:
	...
	SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ( "[LFS] LfsSvrDatagramRecvHandler: a datagram packet rejected.\n"));
...
//	KIRQL				oldIrql ;
	...
	case NDFT_PRIMARY_UPDATE_MESSAGE: {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ( "[LFS] DispatchDGReqPkt: BCASTOWNER: %02x:%02x:%02x:%02x:%02x:%02x/%d.\n",
		...
		LfsTable_UpdatePrimaryInfo(SvrCtx->LfsTable, &NetDiskAddress, (UCHAR)RawData->Owner.UnitDiskNo, &PrimaryAddress) ;
		break ;
	}
	default:
		SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ( "[LFS] DispatchDGReqPkt: invalid request.\n")) ;
	}
...
RedirDataGramSvrThreadProc(
	...
	while(1) {
		...
		if(0 == ntStatus) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ( "[LFS] RedirDataGramSvrThreadProc: DatagramRecv event received. NTSTATUS:%lu\n", ntStatus));
		} else if(2 == ntStatus) {
			//
			// Renew server datagram port
			//
			SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ( "[LFS] RedirDataGramSvrThreadProc: Networking event received. NTSTATUS:%lu\n", ntStatus));
...
RedirDataGramNotifierThreadProc(
	...
	while(1) {
		...
		if(NetEvent) {
			SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ("DataGramNotifier: Renew the socket.\n")) ;
			//
			//	renew the socket.
			//
			LfsCloseDGSocket(
		...
		while(listEntry != &NtcCtx->LfsTable->LfsTabPartitionList) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ("DataGramNotifier: broadcast NetDisk: %02x:%02x:%02x:%02x:%02x:%02x/%d UD:%d\n", 
			...
			RtlCopyMemory(Dgpkt->RawDataDG.Owner.NetDiskNode, LfsTableEntry->LocalNetDiskPartitionInfo.NetDiskPartitionInfo.NetDiskAddress.Node, ETHER_ADDR_LENGTH) ;
...
next_entry:
		...
		while(!IsListEmpty(&NtcCtx->SendPkts)) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LIB_INFO, ( "[LFS] RedirDataGramNotifierThread: updating %02x:%02x:%02x:%02x:%02x:%02x/%d.\n",
							NetDiskAddress.Node[0], NetDiskAddress.Node[1], NetDiskAddress.Node[2],
							NetDiskAddress.Node[3], NetDiskAddress.Node[4], NetDiskAddress.Node[5],
							NTOHS(NetDiskAddress.Port)
			)) ;

			LfsTable_UpdatePrimaryInfo(NtcCtx->LfsTable, &NetDiskAddress, (UCHAR)RawData->Owner.UnitDiskNo, &PrimaryAddress) ;
