...
NetEventDetectorProc(
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("[LFS] NetEventDetectorProc: Initializing Networking event detector...\n")) ;
	//
	...
	if(!NT_SUCCESS(ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] NetEventDetectorProc: LpxTdiGetAddressList() failed. NTSTATUS:%lu\n", ntStatus));
		goto termination ;

	}

	SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("[LFS] NetEventDetectorProc: Networking event detector started...\n")) ;
	...
	while(1) {
		...
		if(0 == ntStatus) {
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] NetEventDetectorProc: ShutDown event received. NTSTATUS:%lu\n", ntStatus));
			break ;
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] NetEventDetectorProc: KeWaitForSingleObject() failed. NTSTATUS:%lu\n", ntStatus));
			break ;
		}
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] NetEventDetectorProc: Networking event detected.\n"));
		...
		for(idx_callbacks = 0 ; idx_callbacks < NetEvtCtx->CallbackCnt ; idx_callbacks ++ ) {
			...
			if(NetEvtCtx->Callbacks[idx_callbacks]) {
				...
				SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("[LFS] NetEventDetectorProc: Callback #%d is NULL.\n", idx_callbacks)) ;
			}
		}
...
termination:
	SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("[LFS] Networking event detector terminiated.\n")) ;
...
NetEvtInit(PNETEVTCTX	NetEvtCtx) {
	...
	if(!NT_SUCCESS(ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("[LFS] couldn't start Redirection server\n")) ;
		return FALSE ;
	}
...
NetEvtTerminate(PNETEVTCTX	NetEvtCtx) {
	...
	if(!NT_SUCCESS(ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] NetEvtTerminate: referencing to the thread object failed\n")) ;
		goto out ;
	}
	...
	if(!NT_SUCCESS(ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] NetEvtTerminate: waiting for the thread failed\n")) ;
	}

	ObDereferenceObject(ThreadObject) ;

	SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] NetEvtTerminate: Shut down successfully.\n")) ;
...
LfsAllocDGPkt(
	...
	*ppkt = ExAllocatePoolWithTag(
	...
	if(NULL == *ppkt) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsAllocPacket: not enough memory.\n") );
		return FALSE ;
	}
...
LfsOpenDGSocket(
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsOpenDGSocket: Entered\n"));
	...
	if(!NT_SUCCESS(ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsOpenDGSocket: LpxTdiGetAddressList() failed.\n")) ;
		return ntStatus;
	}
	if(0 == socketLpxAddressList.iAddressCount) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsOpenDGSocket: No NICs in the host.\n") );
	...
	for(idx_addr = 0 ; idx_addr < socketLpxAddressList.iAddressCount; idx_addr ++) {
		...
		if( (0 == socketLpxAddressList.SocketLpx[idx_addr].LpxAddress.Node[0]) &&
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsOpenDGSocket: We don't use SocketLpx device.\n")) ;
			continue ;
		...
		if(!NT_SUCCESS(ntStatus)) {
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsOpenDGSocket: couldn't open a address %d:'%02X:%02X:%02X:%02X:%02X:%02X/%d'\n",
						idx_addr,
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsOpenDGSocket: opened a address:'%02X:%02X:%02X:%02X:%02X:%02X/%d'\n",
					NICAddr.Node[0],NICAddr.Node[1],NICAddr.Node[2],
					NICAddr.Node[3],NICAddr.Node[4],NICAddr.Node[5],
					(int)NTOHS(NICAddr.Port)
			) );
...
LfsCloseDGSocket(
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsCloseDGSocket: Entered\n"));
...
LfsSendDatagram(
	...
	for(idx_addr = 0 ; idx_addr < socket->SocketCnt ; idx_addr ++ ) {
		...
		if(STATUS_PENDING == ntStatus) {
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsSendDatagram: outgoing packet is pending.\n")) ;
		} else if(!NT_SUCCESS(ntStatus)) {
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsSendDatagram: sending failed.\n")) ;
		} else if(result != pkt->PacketSize) {
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsSendDatagram: unexpected data length sent. len:%lu\n", result)) ;
		}
		//
		//	patch02172004
...
LfsRegisterDatagramRecvHandler(
	...
	for(idx_addr = 0 ; idx_addr < DatagramSocket->SocketCnt ; idx_addr ++) {
		...
		if(!NT_SUCCESS(ntStatus)) {
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsRegisterDatagramRecvHandler: LpxTdiSetReceiveDatagramHandler() failed.\n")) ;
		}
	}

	SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsRegisterDatagramRecvHandler: left...\n") );
...
LfsIsFromLocal(
	...
	if(!NT_SUCCESS(ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsIsFromLocal: LpxTdiGetAddressList() failed.\n")) ;
		return FALSE ;
	}
	if(0 == socketLpxAddressList.iAddressCount) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsIsFromLocal: No NICs in the host.\n")) ;
		return FALSE ;
	}
	...
	for(idx_addr = 0 ; idx_addr < socketLpxAddressList.iAddressCount ; idx_addr ++ ) {
		...
		if( (0 == socketLpxAddressList.SocketLpx[idx_addr].LpxAddress.Node[0]) &&
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ( "[LFS] LfsIsFromLocal: We don't use SocketLpx device.\n") );
			continue ;
