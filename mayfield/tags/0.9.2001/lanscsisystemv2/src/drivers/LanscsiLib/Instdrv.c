...
//                                SERVICE_DEMAND_START,  // start type
    ...
    if ( schService == NULL ) {
		DebugPrint(1, ( "[LanscsiLib] InstallDriver(): failed CreateService()\n")) ;
        return FALSE;
	}

	DebugPrint(1, ( "[LanscsiLib] InstallDriver(): service created successfully\n")) ;
					  ...
					  HANDLE * lphDevice, PDWORD Error )
{
	...
	DebugPrint(1, ( "LoadDeviceDriver():step1\n")) ;
	schSCManager = OpenSCManager( NULL, NULL, SC_MANAGER_ALL_ACCESS );

/*
	// Remove previous instance
	DebugPrint(1, ( "LoadDeviceDriver():step2\n")) ;
	RemoveDriver( schSCManager, Name );

	// Ignore success of installation: it may already be installed.
	if(!DriverExist(schSCManager, Name)) {
		DebugPrint(1, ( "LoadDeviceDriver():step3\n")) ;
		InstallDriver( schSCManager, Name, Path );
	}
*/
	// Ignore success of start: it may already be started.
	DebugPrint(1, ( "LoadDeviceDriver():step4\n")) ;
	StartDriver( schSCManager, Name );

	// Do make sure we can open it.
	DebugPrint(1, ( "LoadDeviceDriver():step5\n")) ;
	okay = OpenDevice( Name, lphDevice );
	*Error = GetLastError();
	DebugPrint(1, ( "LoadDeviceDriver():step6\n")) ;
 	CloseServiceHandle( schSCManager );
	DebugPrint(1, ( "LoadDeviceDriver():step7\n")) ;
