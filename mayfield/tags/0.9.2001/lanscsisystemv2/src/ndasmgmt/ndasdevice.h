...
public:
	...
	void GetDeviceStringId(WTL::CString& stringId)
	{
...
inline void CNdasDevice::CreateMenuItem(WORD wId, MENUITEMINFO& mii)
{
	...
	subMenuItemInfo.wID = ID_ENABLE_DEVICE + m_dwIndex;
	...
	if (m_status == NDAS_DEVICE_STATUS_DISABLED) 
	{
		...
		subMenuItemInfo.wID = ID_UNREGISTER_DEVICE + m_dwIndex;
		subMenuItemInfo.dwTypeData = (LPTSTR)(LPCTSTR)strMenuText;
		m_subMenu.InsertMenuItem(0xFFFF, TRUE, &subMenuItemInfo);
	}
	...
	strMenuText.LoadString(ID_NDD_MOUNT_RO);
	subMenuItemInfo.fMask |= MIIM_DATA;
	subMenuItemInfo.dwItemData = m_dwIndex;
	subMenuItemInfo.wID = ID_NDD_MOUNT_RO;
	subMenuItemInfo.dwTypeData = LPTSTR(LPCTSTR(strMenuText));
	m_subMenu.InsertMenuItem(0xFFFF, TRUE, &subMenuItemInfo);
	subMenuItemInfo.fMask &= ~(MIIM_DATA);

	strMenuText.LoadString(ID_NDD_MOUNT_RW);
	subMenuItemInfo.wID = ID_NDD_MOUNT_RW;
	subMenuItemInfo.dwTypeData = LPTSTR(LPCTSTR(strMenuText));
	m_subMenu.InsertMenuItem(0xFFFF, TRUE, &subMenuItemInfo);

	strMenuText.LoadString(ID_NDD_UNMOUNT);
	subMenuItemInfo.wID = ID_NDD_UNMOUNT;
	subMenuItemInfo.dwTypeData = LPTSTR(LPCTSTR(strMenuText));
	m_subMenu.InsertMenuItem(0xFFFF, TRUE, &subMenuItemInfo);
	...
	subMenuItemInfo.wID = ID_SHOW_DEVICE_PROPERTIES + m_dwIndex;
	subMenuItemInfo.dwTypeData = (LPTSTR)(LPCTSTR)strMenuText;
	m_subMenu.InsertMenuItem(0xFFFF, TRUE, &subMenuItemInfo);
