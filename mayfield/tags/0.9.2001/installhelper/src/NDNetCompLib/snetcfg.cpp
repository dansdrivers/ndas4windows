...
//
HRESULT HrInstallNetComponent(IN INetCfg* pnc, IN PCTSTR szComponentId,
...
HRESULT HrUninstallNetComponent(IN INetCfg* pnc, IN PCWSTR szComponentId)
{
    ...
    // OBO_TOKEN specifies the entity on whose behalf this
    // component is being uninstalled
...
void ShowHrMessage(IN HRESULT hr)
{
    if (SUCCEEDED(hr))
    {
        ...
        {
            LogPrintf(_T("*** You need to reboot your computer for this change to take effect ***\n"));
        }
    }
    else
    {
        LogPrintf(_T("..failed. Error code: 0x%lx\n"), hr);
    }
}
