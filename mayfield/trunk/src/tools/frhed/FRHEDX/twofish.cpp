...
/*
Algorithm twofish (twofish.c)

128 bit key:
Key Setup:    9510 cycles
Encrypt:       371 cycles =    69.0 mbits/sec
Decrypt:       382 cycles =    67.0 mbits/sec
Mean:          377 cycles =    68.0 mbits/sec

192 bit key:
Key Setup:   13115 cycles
Encrypt:       372 cycles =    68.8 mbits/sec
Decrypt:       382 cycles =    67.0 mbits/sec
Mean:          377 cycles =    67.9 mbits/sec

256 bit key:
Key Setup:   16441 cycles
Encrypt:       372 cycles =    68.8 mbits/sec
Decrypt:       382 cycles =    67.0 mbits/sec
Mean:          377 cycles =    67.9 mbits/sec
*/
...
{   u4byte  t0, t1, blk[4];
    ...
    blk[0] = u4byte_in(in_blk     ) ^ l_key[0];
    blk[1] = u4byte_in(in_blk +  4) ^ l_key[1];
    blk[2] = u4byte_in(in_blk +  8) ^ l_key[2];
    blk[3] = u4byte_in(in_blk + 12) ^ l_key[3];

    f_rnd(0); f_rnd(1); f_rnd(2); f_rnd(3);
    f_rnd(4); f_rnd(5); f_rnd(6); f_rnd(7);

    u4byte_out(out_blk,      blk[2] ^ l_key[4]);
    u4byte_out(out_blk +  4, blk[3] ^ l_key[5]);
    u4byte_out(out_blk +  8, blk[0] ^ l_key[6]);
    u4byte_out(out_blk + 12, blk[1] ^ l_key[7]); 
}
...
{   u4byte  t0, t1, blk[4];
    ...
    blk[0] = u4byte_in(in_blk     ) ^ l_key[4];
    blk[1] = u4byte_in(in_blk +  4) ^ l_key[5];
    blk[2] = u4byte_in(in_blk +  8) ^ l_key[6];
    blk[3] = u4byte_in(in_blk + 12) ^ l_key[7];

    i_rnd(7); i_rnd(6); i_rnd(5); i_rnd(4);
    i_rnd(3); i_rnd(2); i_rnd(1); i_rnd(0);

    u4byte_out(out_blk,      blk[2] ^ l_key[0]);
    u4byte_out(out_blk +  4, blk[3] ^ l_key[1]);
    u4byte_out(out_blk +  8, blk[0] ^ l_key[2]);
    u4byte_out(out_blk + 12, blk[1] ^ l_key[3]); 
}
