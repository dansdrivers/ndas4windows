...
//
...
#endif
	{
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("ntfsStatus = %x, ntfsDeviceObject = %p\n", ntfsStatus, ntfsDeviceObject) );
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("fatStatus = %x, fatDeviceObject = %p\n", fatStatus, fatDeviceObject) );
	}
...

{
    ...
    SPY_LOG_PRINT( SPYDEBUG_ERROR, ("result = %d\n", result) );
	...
	if (result == FALSE) {
		...
		if ((enabledNetdisk->NetdiskInformation.LowestHwVer != LANSCSIIDE_VERSION_1_0 && fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_FAT) ||
		    enabledNetdisk->NetdiskInformation.LowestHwVer == LANSCSIIDE_VERSION_1_0 && fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDFAT /*||
		...
		SPY_LOG_PRINT( SPYDEBUG_ERROR, ("enabledNetdisk->NetdiskInformation.LowestHwVer = %d\n", enabledNetdisk->NetdiskInformation.LowestHwVer) );

		if (fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_FAT)
			ASSERT( enabledNetdisk->NetdiskInformation.LowestHwVer == LANSCSIIDE_VERSION_1_0 );

		if (fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDFAT)
			ASSERT( enabledNetdisk->NetdiskInformation.LowestHwVer == LANSCSIIDE_VERSION_1_1 ||
					enabledNetdisk->NetdiskInformation.LowestHwVer == LANSCSIIDE_VERSION_2_0 );
	}
...
--*/
{
    ...
    switch (IoControlCode) 
	{
		...
		case LFS_FILTER_SHUTDOWN: {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR, ("LFS_FILTER_SHUTDOWN \n") );
