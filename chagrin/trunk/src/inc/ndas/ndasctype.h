...
typedef struct _NDAS_LOGICALDEVICE_GROUP {
	...
	BYTE DeviceHwVersions[MAX_NDAS_LOGICALDEVICE_GROUP_MEMBER]; // Byte offset 396. Retreived from DIB
//	GUID RaidSetId;		// RaidSetId and ConfigSetId should be matched too. 
//	GUID ConfigSetId;	// 
} NDAS_LOGICALDEVICE_GROUP, *PNDAS_LOGICALDEVICE_GROUP;

C_ASSERT((428) == sizeof(NDAS_LOGICALDEVICE_GROUP));
