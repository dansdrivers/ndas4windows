...
// #define LSP_HARDWARE_VERSION_10 0x00
// #define LSP_HARDWARE_VERSION_11 0x01
// #define LSP_HARDWARE_VERSION_20 0x02
...
enum { LSP_VENDOR_ID_XIMETA = 0x0001 };
enum { LSP_VENDOR_OP_VERSION_1_0 = 0x01 };
enum { LSP_VENDOR_OP_CURRENT_VERSION = LSP_VENDOR_OP_VERSION_1_0 };
...
C_ASSERT_SIZEOF(lsp_pdu_op_flags_t, 1);
...
C_ASSERT_SIZEOF(lsp_ide_data_v0_t, 16);
...
C_ASSERT_SIZEOF(lsp_ide_header_t, 4);
...
typedef struct _lsp_ide_register_param_t {
	/* use_dma and use_48 is not used in the output register */
...
} lsp_ide_register_param_t;

C_ASSERT_SIZEOF(lsp_ide_register_param_t, 2 + 2 + 5 + 5);
...
C_ASSERT_SIZEOF(lsp_vendor_v1_t, 8);
...
C_ASSERT_SIZEOF(lsp_pdu_op_data_t, 16);
...
C_ASSERT_SIZEOF(lsp_pdu_hdr_t, 60);
...
typedef struct _lsp_request_packet_t {
	...
	struct _lsp_request_packet_t* next_request;
	/* output */
