...
CNdasLogicalDevice::UpdateBindStateAndError()
{
	...
	if(m_NdasPort == FALSE) {
		...
		UCHAR targetType = LogicalDeviceTypeToLurTargetType(m_logicalDeviceGroup.Type);
	    if (pPrimaryUnitDevice && 
	...
	DWORD LdpfValues)
{
	...
	UCHAR nodeType = LogicalDeviceTypeToLurTargetType(m_logicalDeviceGroup.Type);
	...
	{	
		...
		if (CNdasUnitDeviceNullPtr == pUnitDevice) {
			...
			ataSpecific->HardwareRevision = pDevice->GetHardwareRevision();	
			ataSpecific->TransportPortNumber = HTONS(LPXRP_NDAS_PROTOCOL);
