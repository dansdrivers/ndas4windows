	...
	)
{
	...
	for (remaining = Size; remaining > 0; ) {
		...
		if (TransCtx) {
			...
			AdjustMaxDataSizeToSend( TransCtx );

			onceReqSz = (remaining < TransCtx->DynMaxSendBytes) ? remaining : TransCtx->DynMaxSendBytes;
		...
		if (status != STATUS_SUCCESS) {
		    ...
		    SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("SendMessage: sending failed.\n") );
			break;
		}

		NDASFS_ASSERT( result >= 0 );
		...
		if (TransCtx) {
			...
			queryStatus = LpxTdiV2QueryInformation( ConnectionFileObject,
	...
	if (remaining) {
	    ...
	    SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR, ("SendMessage: unexpected data length sent. remaining:%lu\n", remaining) );
