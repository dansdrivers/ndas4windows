	...
	)
{
	...
	if (newPrimarySession == NULL) {
		...
		LpxTdiV2CloseConnection( ListenFileHandle, ListenFileObject );
	}
	...
	) 
{
	...
	ntStatus = LpxTdiV2OpenAddress( &addressFileHandle, &addressFileObject, NICAddr );
	...
	KeClearEvent( &listenSock->ListenOverLapped.CompletionEvent );
	...
	if (!NT_SUCCESS(ntStatus)) {
		...
		ASSERT( LFS_LPX_BUG );
		LpxTdiV2CleanupRoutine( &listenSock->ListenOverLapped );
...
PrimaryAgentNICDisabled (
	...
	for(idx_disabled = 0; idx_disabled < AddressList->iAddressCount; idx_disabled ++ ) {
		...
		if(found) {
			...
			LpxTdiV2CloseConnection(listenSock->ListenFileHandle, listenSock->ListenFileObject);
	...
	)
{
	...
	for (listenSocketIndex = 0; listenSocketIndex < MAX_SOCKETLPX_INTERFACE; listenSocketIndex++) {
		...
		KeInitializeEvent( &Primary->Agent.ListenSocket[listenSocketIndex].ListenOverLapped.CompletionEvent, 
						   NotificationEvent, 
						   FALSE );
	}
	...
	while (primaryAgentThreadExit == FALSE) {
		...
		if (!FlagOn(Primary->Agent.Flags, PRIMARY_AGENT_FLAG_SHUTDOWN)) {
			...
			for (i=0; i<MAX_SOCKETLPX_INTERFACE; i++) {
				...
				if (Primary->Agent.ListenSocket[i].Active) {
					...
					events[eventCnt++] = &Primary->Agent.ListenSocket[i].ListenOverLapped.CompletionEvent;
		...
		if (1 <= ntStatus && ntStatus <= MAX_SOCKETLPX_INTERFACE) {
			...
			LpxTdiV2CleanupRoutine( &listenSocket->ListenOverLapped );
			...
			tpAddr = (PTRANSPORT_ADDRESS)listenSocket->ListenOverLapped.AddressBuffer;
			...
			KeClearEvent( &listenSocket->ListenOverLapped.CompletionEvent );
			...
			} else if (listenSocket->ListenOverLapped.IoStatusBlock.Status != STATUS_SUCCESS) {
...
start_session:
	...
	for (listenSocketIndex = 0; listenSocketIndex < MAX_SOCKETLPX_INTERFACE; listenSocketIndex++)
		KeClearEvent( &Primary->Agent.ListenSocket[listenSocketIndex].ListenOverLapped.CompletionEvent );
	...
	)
{
	...
	for (idx_addr = 0; idx_addr < socketLpxAddressList->iAddressCount; idx_addr ++) {
		...
		ntStatus = LpxTdiV2OpenAddress( &addressFileHandle,
									  &addressFileObject,
									  &NICAddr );
		...
		KeInitializeEvent(&Primary->Agent.ListenSocket[idx_addr].ListenOverLapped.CompletionEvent, NotificationEvent, FALSE);
		...
		if (!NT_SUCCESS(ntStatus)) {
			...
			LpxTdiV2CleanupRoutine( &Primary->Agent.ListenSocket[idx_addr].ListenOverLapped );
	...
	)
{
	...
	for (i = 0; i < MAX_SOCKETLPX_INTERFACE; i++) {
		...
		LpxTdiV2CancelIrpRoutine( &Primary->Agent.ListenSocket[i].ListenOverLapped );

		LpxTdiV2CleanupRoutine( &Primary->Agent.ListenSocket[i].ListenOverLapped );
	...
	)
{
	...
	ntStatus = LpxTdiV2OpenConnection( &listenFileHandle,
									 &listenFileObject,
									 NULL );
