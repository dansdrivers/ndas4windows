	...
	)
{
	...
	while (primarySessionTerminate == FALSE) {
		...
		NDASFS_ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
		...
		if (eventStatus == 0) {
			...
			NDASFS_ASSERT( eventStatus == 2 );  // Receive Event
