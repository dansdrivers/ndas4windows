...
// Added by ILGU HONG			
			if(!PagingIo && ((uint64)(ByteOffset.QuadPart + WriteLength) > PtrFCB->RealAllocationSize)){
				bNeedAlloc = TRUE;
			}
				...
				if(EndLotIndex > CurentEndIndex){
					...
					if(LotCount != AllocatedLotCount){
						if(PtrFCB->RealAllocationSize < RequestStartOffset){
							...
							try_return(RC);
						}else if (Offset > PtrFCB->RealAllocationSize){
							WriteLength = (uint32)(PtrFCB->RealAllocationSize - RequestStartOffset);
			...
			if (!NonBufferedIo) {
				...
				*/
					...
					&&(ByteOffset.QuadPart  > PtrFCB->ValidDataLength.QuadPart))
				{
					...
					ZeroStart.QuadPart = (PtrFCB->ValidDataLength.QuadPart + (LONGLONG)(PtrVCB->SectorSize -1)) & ~((LONGLONG)(PtrVCB->SectorSize -1));
		...
		*/		
		...
		if(!DelayedOP){
			...
			if (RC == STATUS_CANT_WAIT) {
				...
				if(XifsdCheckFlagBoolean(PtrIrpContext->IrpContextFlags, XIFSD_IRP_CONTEXT_ALLOC_IO_CONTEXT)){
					
					if(PtrIrpContext->IoContext) {
						ExFreePool(PtrIrpContext->IoContext);
					}
					XifsdClearFlag(PtrIrpContext->IrpContextFlags, XIFSD_IRP_CONTEXT_ALLOC_IO_CONTEXT);
