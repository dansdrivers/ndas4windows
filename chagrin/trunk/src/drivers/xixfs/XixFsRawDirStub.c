...
)
{
	...
	try{
		...
		DirContext->ChildHashTable = ExAllocatePoolWithTag(NonPagedPool, XIDISK_HASH_VALUE_TABLE_SIZE, TAG_CHILD);
		if(!DirContext->ChildHashTable){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
				("ERROR XixFsInitializeDirContext Fail Alloc ChildHashTable.\n"));
			try_return(RC =  STATUS_INSUFFICIENT_RESOURCES);
		}
		...
		if(!NT_SUCCESS(RC)){
			...
			// Addedd by ILGU HONG 12082006
			if(DirContext->ChildHashTable){
				ExFreePool(DirContext->ChildHashTable);
...
//						("Read ChildBitMap bitmap[%ld](0x%02x).\n",i,DirContext->ChildBitMap[i]));
		...
		// Addedd by ILGU HONG 12082006
		RC = XixFsRawReadDirEntryHashValueTable(
					pVCB->TargetDeviceObject,
					pVCB->LotSize,
					DirContext->LotNumber,
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
				("ERROR XixFsLookupInitialDirEntry Read Dir Hash Info\n"));
			try_return(RC);
		}
		// Addedd by ILGU HONG 12082006 END
...
)
{
	uint32 hash = 0;
	while(len--){
		hash = XixFsPartialNameHash(*name++, hash);
	}
...
)
{
	...
	UpName.Buffer = ExAllocatePool(PagedPool, Name->MaximumLength);
	
	if(!UpName.Buffer) {
		return STATUS_INSUFFICIENT_RESOURCES;
	}
	UpName.MaximumLength = Name->MaximumLength;
	UpName.Length = Name->Length;
	try{
		RC = RtlUpcaseUnicodeString(&UpName, Name, FALSE);

		if(!NT_SUCCESS(RC)){
			//DbgPrint("ERROR XixFsGetHashValueForFile:RtlUpcaseUnicodeString!!\n");
			try_return(RC);
		}

		hash = XixFsNameHash((unsigned char *)UpName.Buffer, UpName.Length);
		*Hashval = (uint16)((hash % 1024) + 1);
		RC = STATUS_SUCCESS;

	}finally{
		ExFreePool(UpName.Buffer);
	}
    ...
    )
{
	...
	RC = XixFsGetHashValueForFile(Name,&Hashval);

	if(!NT_SUCCESS(RC)){
		DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
				("ERROR XixFsGetHashValueForFile\n"));

		//DbgPrint("ERROR GET HASH VAL!!!\n");
		return RC;
	}

	pDirHashTable = (PXIDISK_HASH_VALUE_TABLE)DirContext->ChildHashTable; 
    ...
    while(1) {
		NextIndex = (uint64)XixFsfindSetBitFromMap(1024, NextIndex, DirContext->ChildBitMap);
		if(NextIndex == 1024){
				DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
						("Can't find Dir endtry\n"));
				break;
		...
		if(pDirHashTable->EntryHashVal[NextIndex] != Hashval){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
				("ERROR Searched Index(%I64d) EntryHashVal(%d) ReqHashVal(%d)\n",
						NextIndex,
						pDirHashTable->EntryHashVal[NextIndex],
						Hashval));
			/*
			DbgPrint("ERROR Searched Index(%I64d) EntryHashVal(%d) ReqHashVal(%d)\n",
						NextIndex,
						pDirHashTable->EntryHashVal[NextIndex],
						Hashval);
			*/
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
					("Fail XixFsRawReadDirEntry\n"));
			//DbgPrint("Error XixFsRawReadDirEntry\n");
			return RC;
		}
...
)
{
	...
	try{
		...
		RC = XixFsGetHashValueForFile(ChildName,&Hashval);

		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
					("ADD Dir Entry ERROR XixFsGetHashValueForFile\n"));

			//DbgPrint("ADD Dir Entry ERROR GET HASH VAL!!!\n");
			try_return(RC);
		}
		...
		pTable = (PXIDISK_HASH_VALUE_TABLE)DirContext->ChildHashTable;
		...
		RC = XixFsRawWriteDirEntryHashValueTable(
						pVCB->TargetDeviceObject,
						pVCB->LotSize,
						LotNumber,
						DirContext->ChildHashTable,
...
)
{
	...
	try{
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
					("Change Dir Entry ERROR XixFsGetHashValueForFile\n"));

			//DbgPrint("Change Dir Entry ERROR GET HASH VAL!!!\n");
			try_return(RC);
		}
