...
)
{
	...
	while(1){
		...
		if(RC == 0){
			...
			if(XifsdCheckFlagBoolean(pVCB->ResourceFlag,XIFSD_VCB_RESOURCE_UPDATE_FALAG)){
				XifsdClearFlag(pVCB->ResourceFlag,XIFSD_VCB_RESOURCE_UPDATE_FALAG);
...
)
{
	...
	uint32					blockSize = 0;
	uint64					LotNumber = 0;
	uint32					LotSignature = 0;
	uint64					VolumeSignature = 0;
	uint32					XifsVesion = 0;		
	...
	try{
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
				("Fail  XixfsdCheckBootSector:XixFsRawReadVolumeLotHeader  .\n"));
			try_return(RC);
		}
		...
		VolumeSignature = VolumeLot->VolInfo.VolumeSignature;
		...
		XifsVesion = VolumeLot->VolInfo.XifsVesion;
		LotSignature = VolumeLot->VolInfo.LotSignature;
		LotNumber = VolumeLot->VolInfo.NumLots;

		RtlZeroMemory(buffer, blockSize);

		
		RC = XixFsRawReadBootSector(
						TargetDevice,
						buffer,
						blockSize,
						SectorSize
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
				("Fail  XixfsdCheckBootSector:XixFsRawReadBootSector .\n"));
			try_return(RC);
		}
		...
		{
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
				("Fail(0x%x)  Is XIFS_CURRENT_OEM string .\n",
				VolumeLot->VolInfo.XifsVesion ));
			RC = STATUS_UNRECOGNIZED_VOLUME;
			try_return(RC);		
		}
		
		if((pBootSector->VolumeSignature != VolumeSignature)
			|| (pBootSector->LotNumber != LotNumber)
			|| (pBootSector->LotSignature != LotSignature)
			|| (pBootSector->XifsVesion != XifsVesion)
		)
		{
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
	...
	DebugTrace(DEBUG_LEVEL_TRACE, (DEBUG_TARGET_FSCTL|DEBUG_TARGET_VOLINFO ),
		("Exit Status(0x%x) XixfsdCheckBootSector .\n", RC));
	return RC;
}
...
//	XifsdDebugTarget = DEBUG_TARGET_ALL;
	...
	try{
		...
		*/
		...
		RC = XixfsdCheckBootSector(TargetDevice, DiskGeometry.BytesPerSector);
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
					("Fail(0x%x) XixfsdCheckBootSector .\n", RC));
			try_return(RC);	
		}
	...
	)
{
	...
	if(PtrIoStackLocation->Parameters.FileSystemControl.FsControlCode == NDAS_XIXFS_VERSION)
	{
		...
		RC = STATUS_SUCCESS;
		XixFsdCompleteRequest(IrpContext, RC, sizeof(XIXFS_VER));
		return RC;
