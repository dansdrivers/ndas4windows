...
)
{
	...
	try{
		...
		pAddress->LogicalStartOffset = GetLogicalStartAddrOfFile((uint64)LotIndex,LotSize);
		...
		//DbgPrint("XixFsGetAddressInfoForOffset LotIndex(%I64d)\n",pAddress->LotIndex); 
	...
	try{
		...
		for(i = 0; i<LotCount; i++){
			...
			XixFsInitializeCommonLotHeader((PXIDISK_COMMON_LOT_HEADER)Buff,
											...
											GetLogicalStartAddrOfFile((uint64)AddrInfo->Index[i], pVCB->LotSize),
		...
		*AddedLotCount = LotCount;
		...
		//DbgPrint("Changed Size pFCB(%I64d)  pFCB->RealAllocationSize(%I64d)\n", pFCB->LotNumber, pFCB->RealAllocationSize);
