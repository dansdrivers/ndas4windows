...
)
{
	...
	PAGED_CODE();
	DebugTrace(DEBUG_LEVEL_TRACE, (DEBUG_TARGET_DEVCTL|DEBUG_TARGET_FCB),
		("Enter XixFsRawWReadDirEntryHashValueTable \n"));
	
	ASSERT(BufferSize >=XIDISK_HASH_VALUE_TABLE_SIZE);

	LotOffset.QuadPart = GetAddressOfFileData(LOT_FLAG_BEGIN, LotSize, LotIndex);
	
	RC = XixFsEndianSafeReadDirEntryHashValueTable(
			TargetDevice,
			&LotOffset,
	...
	DebugTrace(DEBUG_LEVEL_TRACE, (DEBUG_TARGET_DEVCTL|DEBUG_TARGET_FCB),
		("Exit XixFsRawWReadDirEntryHashValueTable \n"));
	return RC;
}
...
)
{
	...
	PAGED_CODE();
	DebugTrace(DEBUG_LEVEL_TRACE, (DEBUG_TARGET_DEVCTL|DEBUG_TARGET_FCB),
		("Enter XixFsRawWWriteDirEntryHashValueTable \n"));

	ASSERT(BufferSize >=XIDISK_HASH_VALUE_TABLE_SIZE);

	LotOffset.QuadPart = GetAddressOfFileData(LOT_FLAG_BEGIN, LotSize, LotIndex);
	
	RC = XixFsEndianSafeWriteDirEntryHashValueTable(
			TargetDevice,
			&LotOffset,
			(XIDISK_HASH_VALUE_TABLE_SIZE), 
			Buffer,
	...
	DebugTrace(DEBUG_LEVEL_TRACE, (DEBUG_TARGET_DEVCTL|DEBUG_TARGET_FCB),
		("Exit XixFsRawWWriteDirEntryHashValueTable \n"));
	return RC;
}
...
)
{
	NTSTATUS		RC = STATUS_SUCCESS;
	LARGE_INTEGER	LotOffset;
	LotOffset.QuadPart = 0;

	PAGED_CODE();
	DebugTrace(DEBUG_LEVEL_TRACE, (DEBUG_TARGET_DEVCTL|DEBUG_TARGET_FCB),
		("Enter XixFsRawReadBootSector \n"));

	RC = XixFsEndianSafeReadBootSector(
			TargetDevice,
			&LotOffset,
			Buffer,
			SectorSize
...
)
{
	...
	PAGED_CODE();
	DebugTrace(DEBUG_LEVEL_TRACE, (DEBUG_TARGET_DEVCTL|DEBUG_TARGET_FCB),
		("Enter XixFsRawWriteBootSector \n"));

	ASSERT(BufferSize>=XIDISK_VOLUME_LOT_SIZE);

	LotOffset.QuadPart = GetAddressOfVolLotHeader();
	
	RC = XixFsEndianSafeWriteBootSector(
			TargetDevice,
			&LotOffset,
			Buffer,
			SectorSize
