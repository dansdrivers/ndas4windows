...
#define DRVO_BASE_FILESYSTEM_DRIVER	0x00000080   // A driver that is at the bottom of the filesystem stack.
	...
	if ((PVOID)NdasNtfsControlDeviceObject == VolumeDeviceObject) {
		...
		if (irpSp->Parameters.FileSystemControl.FsControlCode == NDAS_FS_UNLOAD) {
			...
			DebugTrace( 0, Dbg2, ("IoControlCode == NDAS_FS_UNLOAD occured DeviceObject->ReferenceCount = %x\n", 
			...
			DebugTrace( 0, Dbg, ("IoControlCode != NDAS_FS_UNLOAD\n", 0) );
...

{
    ...
    try {
...
#if (__NDAS_NTFS_PRIMARY__ || __NDAS_NTFS_SECONDARY__)

		DebugTrace( 0, Dbg2, ("NdasNtfsMountVolume: VolDo = %p\n", VolDo) );		
		DebugTrace( 0, Dbg2, ("NdasNtfsMountVolume QueryNetdiskInformation \n") );
		...
		if( ((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL ) {
			...
			if (queryStatus != STATUS_SUCCESS) {
				...
				DebugTrace( 0, Dbg2, ("NdasNtfsMountVolume %08lx QueryNetdiskInformation \n", queryStatus) );
				ASSERT( NDASNTFS_BUG );
				...
				NDASNTFS_ASSERT( !FlagOn(netdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) );
				...
				DebugTrace( 0, Dbg2, ("netdiskEnableMode = %d, VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionNumber = %d\n", 
	...
	NDASNTFS_ASSERT( !FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) );
	NDASNTFS_ASSERT( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode != NETDISK_SECONDARY );
...
*/
		...
		if (VolDo->NetdiskEnableMode == NETDISK_SECONDARY &&
			...
			if (Vcb->LogFileObject != NULL) {
				...
				NDASNTFS_ASSERT( Vcb->LogFileObject == NULL );
...

{
	...
	case FSCTL_NDAS_FS_FLUSH_OR_PURGE: {
		...
		DebugTrace(+1, Dbg, ("NtfsUserFsRequest... FSCTL_NDAS_FS_FLUSH_OR_PURGE \n") );
		...
		NDASNTFS_ASSERT( FlagOn(IrpContext->State, IRP_CONTEXT_STATE_WAIT) );
...
#if __NDAS_NTFS_PRIMARY__
	{
		...
		NDASNTFS_ASSERT( volDo->NetdiskEnableMode == NETDISK_PRIMARY			|| 
...
#if __NDAS_NTFS_PRIMARY__
	{
		...
		NDASNTFS_ASSERT( volDo->NetdiskEnableMode == NETDISK_PRIMARY || volDo->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY );
