...
NDASNTFS_FSRTL_NOTIFY_FILTER_REPORT_CHANGE NdasNtfsFsRtlNotifyFilterReportChange = NULL;
NDASNTFS_FSRTL_NOTIFY_FILTER_CHANGE_DIRECTORY NdasNtfsFsRtlNotifyFilterChangeDirectory = NULL;
NDASNTFS_FSRTL_TEARDOWN_PER_STREAM_CONTEXTS NdasNtfsFsRtlTeardownPerStreamContexts = NULL;
NDASNTFS_SET_FILTER_TOKEN NdasNtfsSeFilterToken = NULL;
NDASNTFS_KE_ARE_APCS_DISABLED NdasNtfsKeAreApcsDisabled = NULL;
NDASNTFS_CC_MDL_WRITE_ABORT NdasNtfsCcMdlWriteAbort = NULL;
	...
	)
{
	...
	for (remaining = Size; remaining > 0; ) {
		...
		if (status != STATUS_SUCCESS) {
		    ...
		    DebugTrace( 0, DEBUG_INFO_ALL, ("SendMessage: sending failed.\n") );
			break;
		}

		NDASNTFS_ASSERT( result > 0 );
	...
	if (remaining) {
		...
		DebugTrace( 0, DEBUG_INFO_ALL, ("SendMessage: unexpected data length sent. remaining:%lu\n", remaining) );

		NDASNTFS_ASSERT( !NT_SUCCESS(status) );
	}
	...
	)
{
	...
	for (remaining = Size; remaining > 0;  ) {
		...
		if (status != STATUS_SUCCESS) {
			...
			DebugTrace( 0, DEBUG_INFO_ALL, ("RecvMessage: Receiving failed.\n") );
			break;
		}
	...
	if (remaining) {
		...
		DebugTrace( 0, DEBUG_INFO_ALL, ("RecvMessage: unexpected data length sent. remaining:%lu\n", remaining) );
