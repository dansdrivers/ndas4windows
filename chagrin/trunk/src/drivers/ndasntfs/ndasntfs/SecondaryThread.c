	...
	)
{
	...
	while (secondaryThreadTerminate == FALSE) {
		...
		if (Secondary->Thread.ReceiveWaitingCount != 0) {
			...
			events[eventCount++] = &Secondary->Thread.ReceiveOverLapped.CompletionEvent;
		}
		...
		if (eventStatus == 0) {
					...
					if (Secondary->Thread.SessionContext.SessionSlotCount > 1)	{
						...
						if (Secondary->Thread.ReceiveWaitingCount == 0) {
							...
							if (!NT_SUCCESS(status)) {
								...
								LpxTdiV2CleanupRoutine( &Secondary->Thread.ReceiveOverLapped );
			...
			if (Secondary->Thread.ReceiveOverLapped.IoStatusBlock.Status != STATUS_SUCCESS ||
				Secondary->Thread.ReceiveOverLapped.IoStatusBlock.Information != sizeof(NDFS_REQUEST_HEADER)) {
   ...
   )
{
	...
	for (idx_addr=0; idx_addr<socketLpxAddressList.iAddressCount; idx_addr++) {
		...
		status = LpxTdiV2OpenAddress( &addressFileHandle,
									&addressFileObject,
									&NICAddr );
		...
		status = LpxTdiV2OpenConnection( &connectionFileHandle, &connectionFileObject, NULL );
		...
		if (!NT_SUCCESS(status)) {
			...
			LpxTdiV2CloseAddress( addressFileHandle, addressFileObject );
			addressFileHandle = NULL; addressFileObject = NULL;
			LpxTdiV2CloseConnection( connectionFileHandle, connectionFileObject );
			connectionFileHandle = NULL; connectionFileObject = NULL;
			continue;
		}
