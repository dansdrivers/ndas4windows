...
LspVendorRequest(
	...
	if(!LSS->LanscsiProtocol->LsprotoFunc.LspRequestVendor) {
		return STATUS_NOT_IMPLEMENTED;
	}
	...
	return LSS->LanscsiProtocol->LsprotoFunc.LspRequestVendor(
					LSS,
					PduDesc,
					PduResponse
				);
...
LspWorkaroundCleanupLock(
   ...
   PLARGE_INTEGER	TimeOut
	...
	//
	// Perform clean-up only for NDAS HW 1.1 or 2.0 rev 0.
	//
