	...
	case DRIX_CMD_CHANGE_STATUS:
		{
			...
			for(i=0;i<CsMsg->NodeCount;i++) {
				...
				if (pClientInfo->NodeFlagsRemote[i] & DRIX_NODE_FLAG_DEFECTIVE) {
					//
					// Once defective, defective until user clears it.
					//
					if (!(pClientInfo->NodeFlagsLocal[i] &DRIX_NODE_FLAG_DEFECTIVE)) {
						KDPrintM(DBG_LURN_INFO, ("Setting defective flag to node %d!!!\n", 
							i, pClientInfo->NodeFlagsRemote[i] ,CsMsg->Node[i].NodeFlags));
						pClientInfo->NodeFlagsLocal[i] |= DRIX_NODE_FLAG_DEFECTIVE;
					}
				}
...
//
// Return role index
ULONG DraidClientRAID1RSelectChildToRead(IN PRAID_INFO pRaidInfo, IN PCCB Ccb)
{
	ULONG raid_idx = -1;
	...
	//
	// Read balancing version.
	// Almost no effect on performance. Need more analysis 
	//
#if 0
...
DraidClientSendCcbToNondefectiveChildren (
	...
	IN LURN_CASCADE_OPTION				CascadeMode	// Execute another ccb after previous ccb is completed.
	) 
{
	//
	...
	case SCSIOP_WRITE16:
		{
			...
			if (pClientInfo->ClientState == DRAID_CLIENT_STATE_NO_ARBITER) {
				KDPrintM(DBG_LURN_INFO,("Handling write in no arbiter mode. Attempt to write without range locking\n"));
				...
				DraidClientSendCcbToNondefectiveChildren(pClientInfo, Ccb, LurnRAID1RCcbCompletion, TRUE);
	...
	case SCSIOP_VERIFY16:
		{
			...
			status = DraidClientSendCcbToNondefectiveChildren(pClientInfo, Ccb, LurnRAID1RCcbCompletion, FALSE);
...
DraidClientRefreshRaidStatusWithoutArbiter(
	...
	for(i = 0; i < Client->TotalDiskCount; i++) // i: role based.
	{
		...
		if(NDAS_UNIT_META_BIND_STATUS_DEFECTIVE & Client->Rmd.UnitMetaData[i].UnitDeviceStatus)
		{
			...
			Client->NodeDefectLocal[Client->RoleToNodeMap[i]] = DraidRmdUnitStatusToDefectCode(Client->Rmd.UnitMetaData[i].UnitDeviceStatus);
		}
	}
...
//	ArbiterConnectionTrialCount = 0;
	...
	while(TRUE) {
		...
		if (DoMore) {
			...
			if (pClientInfo->DRaidStatus == DRAID_CLIENT_STATE_INITIALIZING 
...
VOID DraidClientAcquireLockCallback(PDRAID_CLIENT_INFO Client, PDRIX_HEADER Reply, PVOID Param1)
{
	...
	if (Reply) {
		...
		if (status == STATUS_SUCCESS) {
			status = DraidClientSendCcbToNondefectiveChildren(Client, Ccb, LurnRAID1RCcbCompletion, FALSE);
		} else {
			// Lock is not granted
