	...
	)
{
	...
	if (NT_SUCCESS(status)) {
		...
		LpxTdiV2CleanupRoutine( &PrimarySession->Thread.ReceiveOverLapped );
	...
	while (primarySessionTerminate == FALSE) {
		...
		NDASFAT_ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
		...
		if (!FlagOn(PrimarySession->Flags, PRIMARY_SESSION_FLAG_STOPPING)) {
			...
			if (!FlagOn(PrimarySession->Thread.Flags, PRIMARY_SESSION_THREAD_FLAG_SHUTDOWN_WAIT)) {
				...
				if (PrimarySession->Thread.TdiReceiving == TRUE) {
					...
					events[eventCount++] = &PrimarySession->Thread.ReceiveOverLapped.CompletionEvent;
				}
			}
		...
		if (eventStatus == 0) {
			...
			while (primarySessionRequestEntry = ExInterlockedRemoveHeadList( &PrimarySession->RequestQueue,
...
#if __NDAS_FAT_PRIMARY_DISMOUNT__
				...
				} else if (primarySessionRequest->RequestType == PRIMARY_SESSION_REQ_SUPRISE_REMOVAL) {

					SetFlag( PrimarySession->Flags, PRIMARY_SESSION_FLAG_SURPRISE_REMOVAL );
			...
			NDASFAT_ASSERT( eventStatus == 2 );  // Receive Event
			...
			if (PrimarySession->Thread.ReceiveOverLapped.IoStatusBlock.Status != STATUS_SUCCESS ||
				PrimarySession->Thread.ReceiveOverLapped.IoStatusBlock.Information != sizeof(NDFS_REQUEST_HEADER)) {

				DebugTrace2( 0, Dbg,
							   ("DispatchRequest: Disconnected, PrimarySession = Data received:%d\n",
							   PrimarySession->Thread.ReceiveOverLapped.IoStatusBlock.Information) );
			...
			if (PrimarySession->NetdiskPartition) {
				...
				if (NetdiskManager_IsStoppedNetdisk(GlobalLfs.NetdiskManager, EnabledNetdisk)) {
					...
					DebugTrace2( NDASFAT_DEBUG_PRIMARY_ERROR,
								   ("PrimarySessionThread: %p Netdisk is stopped\n", PrimarySession) );

					DebugTrace2( NDASFAT_DEBUG_PRIMARY_ERROR, ("DispatchWinXpRequest: Netdisk is stopped.\n") );
