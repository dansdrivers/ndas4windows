...
#define DRVO_BASE_FILESYSTEM_DRIVER	0x00000080   // A driver that is at the bottom of the filesystem stack.
	...
	if ((PVOID)FatControlDeviceObject == VolumeDeviceObject) {
		...
		if (irpSp->Parameters.FileSystemControl.FsControlCode == NDAS_FS_UNLOAD) {

			DebugTrace2( 0, Dbg, ("IoControlCode == NDAS_FS_UNLOAD occured DeviceObject->ReferenceCount = %x\n", 
			...
			DebugTrace2( 0, Dbg, ("IoControlCode != NDAS_FS_UNLOAD\n", 0) );
    ...
    try {
		...
		if (((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL) {
			...
			if (queryStatus != STATUS_SUCCESS) {
				...
				//NDASFAT_ASSERT( FlagOn(netdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) );
				...
				NDASFAT_ASSERT( VolDo->NetdiskPartitionInformation.VolumeName.Length );

				ASSERT( VolDo->NetdiskPartitionInformation.VolumeName.Buffer == VolDo->NetdiskPartitionInformation.VolumeNameBuffer );

				if (FlagOn(VolDo->NetdiskPartitionInformation.NetdiskInformation.EnabledFeatures, NDASFEATURE_SIMULTANEOUS_WRITE))
...
if (IS_WINDOWS2K()) {
				...
				while (1) {
					...
					NDASFAT_ASSERT( queryStatus == STATUS_SUCCESS || queryStatus == STATUS_BUFFER_OVERFLOW );
...

            {
				...
				if ((VolDo->NetdiskEnableMode == NETDISK_READ_ONLY) &&
					...
					if (IS_WINDOWS2K() && VolDo->NetdiskEnableMode == NETDISK_READ_ONLY) {
						...
						ASSERT( !FlagOn(OldVcb->VcbState, VCB_STATE_FLAG_WRITE_PROTECTED) );
						SetFlag( OldVcb->VcbState, VCB_STATE_FLAG_WRITE_PROTECTED );
					}
				}
...

{
	...
	case FSCTL_NDAS_FS_FLUSH_OR_PURGE: {
		...
		if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_READ_ONLY) {
			
			if (1) { //IrpContext->PrimaryRequestInfo.PrimaryTag != 0) {
