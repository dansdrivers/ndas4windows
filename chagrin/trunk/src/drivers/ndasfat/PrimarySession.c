	...
	)
{
	...
	try {
		...
		primarySession = FsRtlAllocatePoolWithTag( NonPagedPool, sizeof(PRIMARY_SESSION), NDASFAT_ALLOC_TAG );
	...
	if (primarySession == NULL) {
		...
		NDASFAT_ASSERT( NDASFAT_INSUFFICIENT_RESOURCES );
	...
	try {
		...
		KeInitializeEvent( &primarySession->Thread.ReceiveOverLapped.CompletionEvent, NotificationEvent, FALSE );
	...
	)
{
	...
	if (PrimarySession->ConnectionFileHandle) {
		...
		LpxTdiV2CloseConnection( PrimarySession->ConnectionFileHandle, PrimarySession->ConnectionFileObject );
	}
	...
	)
{
	...
	timeOut.QuadPart = -NDASFAT_QUERY_REMOVE_TIME_OUT;
	ntStatus = KeWaitForSingleObject( &primarySessionRequest->CompleteEvent,
									  Executive,
									  KernelMode,
									  FALSE,
	...
	)
{
	...
	primarySessionRequest = AllocPrimarySessionRequest( TRUE );
	primarySessionRequest->RequestType = PRIMARY_SESSION_REQ_SUPRISE_REMOVAL;
	primarySessionRequest->ExecuteStatus = STATUS_PENDING;
	...
	)
{
    ...
    if (result == 0) {
		...
		ExFreePoolWithTag( PrimarySession, NDASFAT_ALLOC_TAG );
