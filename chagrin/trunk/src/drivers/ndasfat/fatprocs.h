...
#define __NDAS_FAT_WIN2K_SUPPORT__			1

#if DBG
#define __NDAS_FAT_DBG__					1
#endif

#define __NDAS_FAT_SECONDARY__				1
#define __NDAS_FAT_PRIMARY__				1
#define __NDAS_FAT_PURGE__					1
#define __NDAS_FAT_FLUSH__					0

#define __NDAS_FAT_RECOVERY_TEST__			0

#define __NDAS_FAT_ROUTER_SUPPORT__			1

#define __NDAS_FAT_PRIMARY_DISMOUNT__		0

#define	__NDAS_FAT_FORCE_WRITE_CACHING__	1
...
#if WINVER >= 0x0501
#define NdasFatDbgBreakPoint()	(KD_DEBUGGER_ENABLED ? DbgBreakPoint() : TRUE)
#else
#define NdasFatDbgBreakPoint()	((*KdDebuggerEnabled) ? DbgBreakPoint() : TRUE) 
#endif

#if DBG

#define NDASFAT_ASSERT( exp )	ASSERT( exp )

#else

#define NDASFAT_ASSERT( exp )	\
	((!(exp)) ?					\
...
#define FatVcbAcquiredExclusive(IRPCONTEXT,VCB) (																				\
	(FlagOn(IRPCONTEXT->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) ?												\
	ExIsResourceAcquiredExclusiveLite(&(VCB)->SecondaryResource) : ExIsResourceAcquiredExclusiveLite(&(VCB)->Resource))  ||		\
    ExIsResourceAcquiredExclusiveLite(&FatData.Resource)																		\
)

#define FatVcbOnlyAcquiredExclusive(IRPCONTEXT,VCB) (																	\
	FlagOn(IRPCONTEXT->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) ?											\
	ExIsResourceAcquiredExclusiveLite(&(VCB)->SecondaryResource) : ExIsResourceAcquiredExclusiveLite(&(VCB)->Resource)	\
)

#define FatFcbAcquiredShared(IRPCONTEXT,VCB) (																		\
	FlagOn(IRPCONTEXT->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) ?										\
	ExIsResourceAcquiredSharedLite(&(VCB)->SecondaryResource) : ExIsResourceAcquiredSharedLite(&(VCB)->Resource)	\
)
...
#define FatNotifyReportChange(I,V,F,FL,A) {                                                         \
	if (!FlagOn((F)->NdasFatFlags, ND_FAT_FCB_FLAG_SECONDARY)) {									\
    if ((F)->FullFileName.Buffer == NULL) {                                                         \
        FatSetFullFileNameInFcb((I),(F));                                                           \
    }                                                                                               \
    ASSERT( (F)->FullFileName.Length != 0 );                                                        \
...
VOID
NdasNtfsPrimarySessionSurpriseRemoval (
	IN PVOLUME_DEVICE_OBJECT	VolDo
	);
...
NTSTATUS
NdasFatSecondaryCommonFileSystemControl (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );
...
NTSTATUS
NdasFatSecondaryCommonQueryInformation (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );
...
NTSTATUS
NdasFatSecondaryCommonQueryEa (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );

NTSTATUS
NdasFatSecondaryCommonSetEa (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );

NTSTATUS
NdasFatSecondaryCommonQueryVolumeInfo (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );

NTSTATUS
NdasFatSecondaryCommonSetVolumeInfo (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );
