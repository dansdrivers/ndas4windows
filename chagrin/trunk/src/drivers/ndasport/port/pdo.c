	...
	__in NDAS_LOGICALUNIT_ADDRESS LogicalUnitAddress)
{
	...
	UNALIGNED NDASPORT_PNP_NOTIFICATION * portNotification;
	...
	RtlCopyMemory(
		...
		&NDASPORT_PNP_NOTIFICATION_GUID, 
		sizeof(GUID));

	customNotification->NameBufferOffset = -1;
	customNotification->Version = 1;
	customNotification->Size = sizeof(buffer);

	portNotification = (NDASPORT_PNP_NOTIFICATION UNALIGNED *) 
	...
	if (!NT_SUCCESS(status))
	{
		NdasPortTrace(NDASPORT_PDO_PNP, TRACE_LEVEL_ERROR,
			...
			status);
		KdPrint(("IoReportTargetDeviceChangeAsynchronous failed, status=0x%08X\n", status));
	}
	...
	__in PNDASPORT_PDO_EXTENSION PdoExtension)
{
	...
	//
	status = NdasPortPdoNotifyPnpEvent(
		fdoExtension, 
	...
	case IOCTL_NDASPORT_LOGICALUNIT_EXIST:
		{
			...
			if (outputBufferLength >= sizeof(GUID))
			{
				RtlCopyMemory(
					outputBuffer,
					&NDASPORT_LOGICALUNIT_IDENTITY_GUID,
					sizeof(GUID));
    ...
    __in PIRP RequestIrp)
{
    ...
    NdasPortTrace(NDASPORT_PDO_IOCTL, TRACE_LEVEL_VERBOSE,
		"NdasPortPdoMiniportIoctl: DeviceObject=%p, Irp=%p\n",
		PdoExtension->DeviceObject, RequestIrp);
