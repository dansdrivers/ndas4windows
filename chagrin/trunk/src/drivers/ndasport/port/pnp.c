	...
	__in PNDASPORT_FDO_EXTENSION FdoExtension)
{
	...
	status = NdasPortWmiDeregister(FdoExtension);
	...
	//
	// Remove Fdo from the global fdo list
	//
	{
		...
		for (entry = driverExtension->FdoList.Flink;
			...
			entry = entry->Flink)
		{
			PNDASPORT_FDO_EXTENSION FdoExtensionEntry = 
				CONTAINING_RECORD(entry, NDASPORT_FDO_EXTENSION, Link);
			if (FdoExtensionEntry == FdoExtension)
			{
	...
	__inout PSTORAGE_ADAPTER_DESCRIPTOR StorageAdapterDescriptor)
{
	...
	//
	StorageAdapterDescriptor->MaximumPhysicalPages = 0x80000000 / PAGE_SIZE;
#if defined(_WIN64)
	StorageAdapterDescriptor->AlignmentMask = FILE_QUAD_ALIGNMENT;
#else
	StorageAdapterDescriptor->AlignmentMask = FILE_LONG_ALIGNMENT;
#endif
	...
	StorageAdapterDescriptor->AcceleratedTransfer = TRUE;
    ...
    __in PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	status = NdasPortWmiRegister(FdoExtension);
