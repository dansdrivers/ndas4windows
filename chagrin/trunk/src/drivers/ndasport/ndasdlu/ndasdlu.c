	...
	__in PNDAS_LOGICALUNIT_EXTENSION	LogicalUnitExtension)
{
	...
	// Set CDB LUN.
	cdb->CDB6INQUIRY.LogicalUnitNumber = LUR->LurId[2];
	...
	__inout PDEVICE_CAPABILITIES Capabilities)
{
	...
	deviceCaps.UINumber = ndasDluExtension->LogicalUnitAddress;
	...
	__in PSCSI_REQUEST_BLOCK Srb)
{
	...
	//
#ifdef __NDASDLU_USE_POOL_FOR_CCBALLOCATION__
...
//	NTSTATUS			status;
	...
	KDPrint(1,("entered.\n"));
	UNREFERENCED_PARAMETER(NdasLUExtension);
	...
	LUR = (PLURELATION)DluWorkItemCtx->Arg1;
	abnormalTermination = (BOOLEAN)DluWorkItemCtx->Arg2;
	SlotNo = PtrToUlong(DluWorkItemCtx->Arg3);
	if(LUR) {
		//
		//	CCB_OPCODE_STOP must succeed.
		//

		ntStatus = NdasDluSendStopCcbToAllLURsSync(1, &LUR);
		ASSERT(NT_SUCCESS(ntStatus));
		NdasDluFreeAllLURs(1, &LUR);
	}
	else {
		KDPrint(1,("======> no LUR\n"));
		return;
	...
	BusSet.AdapterStatus = DLUINTERNAL_STATUS_STOPPED;
	BusSet.SupportedFeatures = BusSet.EnabledFeatures = 0;

	if(abnormalTermination) {
		KDPrint(1,("Abnormal stop.\n"));
		BusSet.AdapterStatus |= DLUINTERNAL_STATUSFLAG_ABNORMAL_TERMINAT;
	}
...
NdasDluRestartWorker(
	...
	logicalUnitExtension = (PNDAS_LOGICALUNIT_EXTENSION)DluWorkItemCtx->Arg1;
	SlotNo = PtrToUlong(DluWorkItemCtx->Arg3);
	...
	for(loop=0; loop < 10; loop++) {
		result = EnumerateLURFromNDASBUS(logicalUnitExtension, TRUE, &firstInstallation);
	...
	if(result == SP_RETURN_FOUND) {
		...
		ACQUIRE_SPIN_LOCK(&hwDeviceExtension->DluSpinLock, &oldIrql);

		ASSERT(DLUINTERNAL_ISSTATUS(hwDeviceExtension, DLUINTERNAL_STATUS_RUNNING));
		ASSERT(DLUINTERNAL_ISSTATUSFLAG(hwDeviceExtension, DLUINTERNAL_STATUSFLAG_RESTARTING));
		...
		hwDeviceExtension->AdapterStatus = DLUINTERNAL_STATUS_RUNNING;

		RELEASE_SPIN_LOCK(&hwDeviceExtension->DluSpinLock, oldIrql);

		adapterStatus = DLUINTERNAL_STATUS_RUNNING |
						DLUINTERNAL_STATUSFLAG_RESETSTATUS;
		//
		...
		hwDeviceExtension->AdapterStatus = DLUINTERNAL_STATUS_STOPPING;
		...
		adapterStatus = DLUINTERNAL_STATUS_STOPPED |
						DLUINTERNAL_STATUSFLAG_ABNORMAL_TERMINAT;
	}
...
NdasDluNoOperationWorker(
	...
	NdasDluSendIoctlSrbAsynch(
			NdasPortExGetWdmDeviceObject(NdasLUExtension),
			NDASSCSI_IOCTL_NOOP,
			&NoopData,
