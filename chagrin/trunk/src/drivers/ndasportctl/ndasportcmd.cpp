...
void usage()
{
	_tprintf(_T("usage: ndasportctl <command> <parameters>\n")
		_T(" -p <address> 00:00:00:00:00:00[:unit-number] <ro|rw> \n")
...
ConvertStringToType(
	...
	if (0 == lstrcmpi(_T("disk"), TypeString)) {
		return LURN_IDE_DISK;
	}
	else if (0 == lstrcmpi(_T("odd"), TypeString)) {
		return LURN_IDE_ODD;
	}
	else if (0 == lstrcmpi(_T("mo"), TypeString)) {
		return LURN_IDE_MO;
	}
	else if (0 == lstrcmpi(_T("aggr"), TypeString)) {
		return LURN_AGGREGATION;
	}
	else if (0 == lstrcmpi(_T("stripe"), TypeString)) {
		return LURN_RAID0;
	}
	else if (0 == lstrcmpi(_T("mirror"), TypeString)) {
		return LURN_RAID1R;
	}
	else {
		return LURN_NULL;
...
InitializeLurNodes(
	...
	for(;;) {
		...
		switch(NdasDluDesc->LurDesc.DevType) {
			case LUR_DEVTYPE_HDD:
				nodeInit.NodeType = LURN_IDE_DISK;
				break;
			case LUR_DEVTYPE_ODD:
				nodeInit.NodeType = LURN_IDE_ODD;
				break;
			case LUR_DEVTYPE_MOD:
				nodeInit.NodeType = LURN_IDE_MO;
				break;
			default:
				return FALSE;
	...
	TCHAR** argv)
{
	...
	if (handle.IsInvalid())
	{
		...
		_tprintf(_T("Error %u (0x%X): %s"), 
			lastError.GetCode(), 
			lastError.GetCode(), 
			lastError.GetDescription());
