...
#else
	if (NdasScsiMmProtectMdlSystemAddress) {
				
		Status = NdasScsiMmProtectMdlSystemAddress(LocalMdl, PAGE_NOACCESS);
        ...
        for (i = 0; i < Adapter->VerifierExtension->CommonBufferBlocks && BlkAddr[i]; i++) {
            ...
            if (MappingInfo->SrbExtMdl != NULL) {
				...
				if (NdasScsiMmProtectMdlSystemAddress) {
					...
					NdasScsiMmProtectMdlSystemAddress(MappingInfo->SrbExtMdl, PAGE_READWRITE);
				}
#endif
            ...
            if (MappingInfo->SenseMdl != NULL) {
                ...
                MmProtectMdlSystemAddress(MappingInfo->SrbExtMdl, PAGE_READWRITE);
#if !__NDAS_MINI_PORT_W2K_SUPPORT__
				MmProtectMdlSystemAddress(MappingInfo->SenseMdl, PAGE_READWRITE);
#else
				if (NdasScsiMmProtectMdlSystemAddress) {

					NdasScsiMmProtectMdlSystemAddress(MappingInfo->SenseMdl, PAGE_READWRITE);
				}
#endif
#endif
    ...
    *((PVOID *) SrbExtension) = Adapter->SrbExtensionListHeader;    
    ...
    if (Adapter->SrbExtensionListHeader != SrbExtension) {
		...
		if (NdasScsiMmProtectMdlSystemAddress) {
			...
			NdasScsiMmProtectMdlSystemAddress(Mdl, PAGE_NOACCESS);
		}
#endif
...
--*/
{
    ...
    if (Mdl != NULL) {
		...
		if (NdasScsiMmProtectMdlSystemAddress) {
			...
			Status = NdasScsiMmProtectMdlSystemAddress(Mdl, PAGE_NOACCESS);
            ...
            if (InvalidPage != NULL) {
                ...
                if (CurrentValue == NULL) {
					...
					if (NdasScsiMmProtectMdlSystemAddress) {
						...
						NdasScsiMmProtectMdlSystemAddress(UnusedPageMdl, PAGE_NOACCESS);
					}
#endif
