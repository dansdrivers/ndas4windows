...
!IF DEFINED(__LATEST_NT_TARGET_VERSION)
!IF $(__LATEST_NT_TARGET_VERSION) >= 0x600
USING_WDK=1
!ENDIF
!ENDIF
...
PUBLISH_CMD=$(NDAS_ROOT)\bin\bldpub.exe
...
NDAS_BINPLACE_DBGFLAGS=$(NDAS_BINPLACE_DBGFLAGS) -y -a -s $(MAKEDIR)\$(NDASTREE)\symbols.pub
NDAS_BINPLACE_DBGFLAGS=$(NDAS_BINPLACE_DBGFLAGS) -x -n $(MAKEDIR)\$(NDASTREE)\symbols
BINPLACE_FLAGS=-r $(NDASTREE) -h $(NDAS_BINPLACE_DBGFLAGS)

#
#!INCLUDE "$(NDAS_INC_PATH)\oem.mk"
#

!if exist ($(NDAS_SRC_PATH)\__blddate__)
!include $(NDAS_SRC_PATH)\__blddate__
!endif

!if !exist ($(NDAS_SRC_PATH)\__productver.h)
!message BUILDMSG: Creating Product Version Header File
!if [$(NDAS_ROOT)\bin\update_version.cmd $(NDAS_ROOT)]
!endif
!endif
...
!if exists($(SYSTEMDRIVE)\WINDDK\Supplement)
XM_VENDOR_PATH=$(SYSTEMDRIVE)\WINDDK\Supplement
!endif
!endif
...
WTL75_INC_PATH=$(XM_VENDOR_PATH)\wtl75\inc
WTL80_INC_PATH=$(XM_VENDOR_PATH)\wtl80\inc
!else
...
!IFNDEF WINSDK_PATH
!IF EXISTS($(XM_VENDOR_PATH)\winsdk60)
WINSDK_PATH=$(XM_VENDOR_PATH)\winsdk60
!ELSE IF EXISTS($(XM_VENDOR_PATH)\winsdk)
WINSDK_PATH=$(XM_VENDOR_PATH)\winsdk
!ENDIF
!ENDIF

!IFDEF WINSDK_PATH

!IFNDEF WINSDK_INC_PATH
WINSDK_INC_PATH=$(WINSDK_PATH)\include
!ENDIF

!IFNDEF WINSDK_LIB_PATH
WINSDK_LIB_PATH=$(WINSDK_PATH)\lib
!ENDIF

!ENDIF # WINSDK_PATH

#
# WDK requires WINSDK (for Vista) or later
#
# if WDK is being used, WINSDK_XXX_PATH overrides PSDK_XXX_PATH
#
!IFDEF USING_WDK

PSDK_LIB_PATH=$(WINSDK_LIB_PATH)
PSDK_INC_PATH=$(WINSDK_INC_PATH)

!ENDIF # USING_WDK

#
# Platform SDK amd64 libraries are in the separate directory
# Concatenate LIB path if necessary
#
!if "$(_BUILDARCH)" == "AMD64"

!if exists( $(PSDK_LIB_PATH)\AMD64 )
PSDK_LIB_PATH=$(PSDK_LIB_PATH)\AMD64
!endif
!if exists( $(PSDK_LIB_PATH)\x64 )
PSDK_LIB_PATH=$(PSDK_LIB_PATH)\x64
!endif

!endif # _BUILDARCH=AMD64
...
ATL_SOURCES_INC=$(NDAS_INC_PATH)\sources_atl.inc
WTL_SOURCES_INC=$(NDAS_INC_PATH)\sources_wtl.inc
