...
static int ifoRead_PGC(ifo_handle_t *ifofile, pgc_t *pgc, unsigned int offset) {
  ...
  if(pgc->program_map_offset != 0) {
    pgc->program_map = (unsigned char *)malloc(pgc->nr_of_programs * sizeof(pgc_program_map_t));
    if(!pgc->program_map) {
...
int ifoRead_PTL_MAIT(ifo_handle_t *ifofile) {
  ...
  for(i = 0; i < ptl_mait->nr_of_countries; i++) {
    ...
    assert((unsigned int)(ptl_mait->countries[i].pf_ptl_mai_start_byte + 
...
static int ifoRead_PGCIT_internal(ifo_handle_t *ifofile, pgcit_t *pgcit, 
  ...
  data = (unsigned char *)malloc(info_length);
  if(!data)
    return 0;
