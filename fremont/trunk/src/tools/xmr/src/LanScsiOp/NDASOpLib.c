#include "../inc/NDASOpLib.h"
#include "../inc/LanScsiOp.h"
#include "../inc/lsbusioctl.h"
...
BOOL NDAS_DisabledByUser(UNIT_DISK_LOCATION *pUnitDisk, int iDisabled)
{
	...
	sprintf_s(szAddress2,100, "%02x:%02x:%02x:%02x:%02x:%02x",
		pUnitDisk->MACAddr[0],
		pUnitDisk->MACAddr[1],
		pUnitDisk->MACAddr[2],
		pUnitDisk->MACAddr[3],
	...
	while(1)
	{
		...
		if(_stricmp(szAddress, szAddress2))
			continue;
		...
		sprintf_s(szUnitDisk, 200, "UnitDisk_%d", (int)pUnitDisk->SlotNumber);
...
BOOL NDAS_GetStatus(UNIT_DISK_LOCATION *pUnitDisk, PNDAS_STATUS pStatus)
{
	...
	sprintf_s(szAddress2,18, "%02x:%02x:%02x:%02x:%02x:%02x",
		pUnitDisk->MACAddr[0],
		pUnitDisk->MACAddr[1],
		pUnitDisk->MACAddr[2],
		pUnitDisk->MACAddr[3],
	...
	if(DISK_INFORMATION_SIGNATURE_V2 == pDiskInfoV2->Signature)
	{
		...
		pStatus->DiskType = 
			...
			(NMT_RAID_01 == pDiskInfoV2->iMediaType) ? DISK_TYPE_BIND_RAID1 :
...
//			return FALSE;		
	...
	pDiskInfoV2->iMediaType = (iMirrorLevel) ? NMT_RAID_01 : NMT_AGGREGATE;
	pDiskInfoV2->nDiskCount = nDiskCount;
	
	// if nDiskCount > MAX_UNITS_IN_V2, aUnitDisks will be copied into cmd[2], cmd[3]...
	CopyMemory(pDiskInfoV2->UnitDisks, aUnitDisks, min(nDiskCount, MAX_UNITS_IN_V2) * sizeof(UNIT_DISK_LOCATION));

	if(nDiskCount > MAX_UNITS_IN_V2) // fill trailing sectors
	{
		int nCopyPoint;
		for(i = 2; i < nDIB; i++)
		{
			nCopyPoint = MAX_UNITS_IN_SECTOR * (i - 2) + MAX_UNITS_IN_V2;
			CopyMemory(cmd[i].data, &aUnitDisks[nCopyPoint], min(nDiskCount - nCopyPoint, MAX_UNITS_IN_SECTOR) * sizeof(UNIT_DISK_LOCATION));
		}
	...
	for(i = 0; i < nDiskCount; i++)
	{
		...
		char szTemp[100];
		sprintf_s(szTemp, 100, "[NDASOpLib]NDAS_Bind: %d\n", i);
...
NDAS_Unbind(UNIT_DISK_LOCATION *pUnitDisk)
{
	...
	// AING_TO_DO : if nr of disks is more than MAX_UNITS_IN_V2, read more sectors...
	...
	for(i = 0; i < pDiskInfoV2->nDiskCount; i++) // for each netdisks
	{
		...
		if(memcmp(pDIV2->UnitDisks, pDiskInfoV2->UnitDisks, min(pDiskInfoV2->nDiskCount, MAX_UNITS_IN_V2) * sizeof(UNIT_DISK_LOCATION)))
			continue;

		if(pDiskInfoV2->nDiskCount > MAX_UNITS_IN_V2)
		{
			for(j = MAX_UNITS_IN_V2; j < pDiskInfoV2->nDiskCount; j++)
			{
				if(memcmp(&cmd_chk[2 + (j - MAX_UNITS_IN_V2) / MAX_UNITS_IN_SECTOR].data[(j - MAX_UNITS_IN_V2) % MAX_UNITS_IN_SECTOR], &pUnitDisk[j], sizeof(UNIT_DISK_LOCATION)))
					continue;
			}
		}
