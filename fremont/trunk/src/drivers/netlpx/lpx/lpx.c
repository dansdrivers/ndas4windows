	...
	)
{
	...
	NDAS_ASSERT( SocketLpxDeviceContext != AddressDeviceContext );
...
{   
	...
	case SMP_STATE_CLOSE: {
		...
		ExInterlockedInsertTailList( &Connection->LpxSmp.TramsmitQueue,
									 &LpxGetReserved(packet)->ListEntry,
...
//	KIRQL			cancelIrql;
	...
	case SMP_STATE_CLOSE_WAIT: {
		...
		userDataOffSet		   = MmGetSystemAddressForMdlSafe(Irp->MdlAddress, NormalPagePriority );
		...
		while (remainedUserDataLength) {
			...
			InsertTailList( &packetQueue, &LpxGetReserved(packet)->ListEntry );
...
//	KIRQL		cancelIrql;
	...
	case SMP_STATE_CLOSE_WAIT: {
		...
		if (Irp->MdlAddress) {
			...
			userData = MmGetSystemAddressForMdlSafe( Irp->MdlAddress, NormalPagePriority );
   ...
   )
{
	...
	NDAS_ASSERT( status != STATUS_PENDING );
	Irp->IoStatus.Status = status;
...
loop:
	...
	if (!IsListEmpty(&Connection->LpxSmp.TramsmitQueue)) {
		...
		if (Connection->LpxSmp.RetransmitTimeOut.QuadPart <= NdasCurrentTime().QuadPart) {
			...
			NDAS_ASSERT( LpxGetReserved(packet) == reserved );

			if (((SHORT)(NTOHS(reserved->LpxHeader.Sequence) - Connection->LpxSmp.NextTransmitSequece)) < 0) {
				
				reserved->LpxHeader.AckSequence = HTONS( SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence) );

				// Remove continuing flag from retransmitting packet.
				reserved->LpxHeader.Option &= ~LPX_OPTION_PACKETS_CONTINUE_BIT;
				...
				Connection->LpxSmp.NextTransmitSequece = NTOHS(reserved->LpxHeader.Sequence) + 1;

				if (NTOHS(reserved->LpxHeader.Lsctl) & LSCTL_CONNREQ) {

					reserved->LpxHeader.SourcePort = Connection->AddressFile->Address->NetworkName->Port;
				}
	...
	)
{
	...
	if (NumberOfPackets > 1) {
		...
		for (index = 0; index < (NumberOfPackets-1); index ++) {
			...
			PLPX_RESERVED reserved = LpxGetReserved(PacketArray[index]);

			SetFlag( reserved->LpxHeader.Option, LPX_OPTION_PACKETS_CONTINUE_BIT );

			if (Connection->LpxSmp.DestinationSmallLengthDelayedAck == TRUE) {

				if ((index+1) % Connection->LpxSmp.PcbWindow == 0) {
					
					ClearFlag( reserved->LpxHeader.Option, LPX_OPTION_PACKETS_CONTINUE_BIT );
				}
			}
		}	
	}
	...
	for (index = 0; index < NumberOfPackets; index ++) {
		...
		KeQuerySystemTime( &LpxGetReserved(PacketArray[index])->SendTime );

#if __LPX_STATISTICS__
		LpxGetReserved(PacketArray[index])->DeviceContext = deviceContext;
#endif
		...
		if (FlagOn(deviceContext->LpxFlags, LPX_DEVICE_CONTEXT_START) && !FlagOn(deviceContext->LpxFlags, LPX_DEVICE_CONTEXT_STOP)) {
			...
			if (PacketTxDropRate) {
				...
				if ((PacketTxCountForDrop % 1000) <= PacketTxDropRate) {
					...
					LpxGetReserved(PacketArray[NumberOfPackets-1])->NdisStatus = NDIS_STATUS_SUCCESS;
			...
			if (NumberOfPackets) {
				...
				for (i=0; i<NumberOfPackets; i++) {
					...
					NdisSendPackets( deviceContext->NdisBindingHandle, &PacketArray[i], 1 );
				}
			}
			...
			for (index = 0; index < NumberOfPackets; index ++) {
				...
				LpxGetReserved(PacketArray[index])->NdisStatus = NDIS_STATUS_NO_CABLE;
	...
	)
{
...
#if __LPX_STATISTICS__
	{
		...
		reserved->DeviceContext->NumberOfCompleteSendPackets ++;
		reserved->DeviceContext->CompleteTimeOfSendPackets.QuadPart += systemTime.QuadPart - reserved->SendTime.QuadPart;
		reserved->DeviceContext->BytesOfCompleteSendPackets.QuadPart += NTOHS(reserved->LpxHeader.PacketSize & ~LPX_TYPE_MASK);

		if (NTOHS(reserved->LpxHeader.PacketSize & ~LPX_TYPE_MASK) > (sizeof(LPX_HEADER) + 12)) {

			reserved->DeviceContext->NumberOfCompleteLargeSendPackets ++;
			reserved->DeviceContext->CompleteTimeOfLargeSendPackets.QuadPart += systemTime.QuadPart - reserved->SendTime.QuadPart;
			reserved->DeviceContext->BytesOfCompleteLargeSendPackets.QuadPart += NTOHS(reserved->LpxHeader.PacketSize & ~LPX_TYPE_MASK);

		} else {

			reserved->DeviceContext->NumberOfCompleteSmallSendPackets ++;
			reserved->DeviceContext->CompleteTimeOfSmallSendPackets.QuadPart += systemTime.QuadPart - reserved->SendTime.QuadPart;
			reserved->DeviceContext->BytesOfCompleteSmallSendPackets.QuadPart += NTOHS(reserved->LpxHeader.PacketSize & ~LPX_TYPE_MASK);
		}
	}
#endif
	...
	)
{
	...
	inFlight = NTOHS(SHORT_SEQNUM(LpxGetReserved(Packet)->LpxHeader.Sequence)) - SHORT_SEQNUM( Connection->LpxSmp.RemoteAckSequence );

	DebugPrint( 4, ("lpxHeader->Sequence = %x, Connection->LpxSmp.RemoteAckSequence = %x\n",
					 NTOHS(LpxGetReserved(Packet)->LpxHeader.Sequence), SHORT_SEQNUM(Connection->LpxSmp.RemoteAckSequence)) );
	...
	)
{
	...
	while (!IsListEmpty( &Connection->LpxSmp.RecvDataQueue)) {
		...
		if (Connection->LpxSmp.LastDataPacketArrived.QuadPart != 0) {
			...
			TimeDiff = (LONG)(reserved->RecvTime.QuadPart - Connection->LpxSmp.LastDataPacketArrived.QuadPart);
		...
		Connection->LpxSmp.LastDataPacketArrived = reserved->RecvTime;
#endif
		...
		bufferData	   += reserved->PacketRawDataOffset;

		bufferLength	= reserved->PacketRawDataLength;
		bufferLength   -= reserved->PacketRawDataOffset;
		
		copied			= (bufferLength < remained) ? bufferLength : remained;
		
		reserved->PacketRawDataOffset += copied;
		...
		if (reserved->PacketRawDataOffset == reserved->PacketRawDataLength) {
...
--*/
{
	...
	DebugPrint( 4, ("[Lpx]LpxReceiveComplete2: Entered %d\n", KeGetCurrentIrql()) );
	...
	while (TRUE) {
		...
		if (NTOHS(lpxHeader->Lsctl) & LSCTL_CONNREQ) {
			...
			DebugPrint( 4, ("*** Recevied %s from %02X%02X%02X%02X%02X%02X:%04X\n",
							...
							reserved->EthernetHeader.SourceAddress[0],
							reserved->EthernetHeader.SourceAddress[1],
							reserved->EthernetHeader.SourceAddress[2],
							reserved->EthernetHeader.SourceAddress[3],
							reserved->EthernetHeader.SourceAddress[4],
							reserved->EthernetHeader.SourceAddress[5],
							lpxHeader->SourcePort ));
		}

		if (reserved->EthernetHeader.DestinationAddress[0] != 0xFF) {
			...
			DebugPrint( 4, ("To %02X%02X%02X%02X%02X%02X:%04X\n",
							reserved->EthernetHeader.DestinationAddress[0],
							reserved->EthernetHeader.DestinationAddress[1],
							reserved->EthernetHeader.DestinationAddress[2],
							reserved->EthernetHeader.DestinationAddress[3],
							reserved->EthernetHeader.DestinationAddress[4],
							reserved->EthernetHeader.DestinationAddress[5],
		...
		for (address = NULL, Flink = deviceContext->AddressDatabase.Flink;
			...
			if (address->NetworkName->Port == lpxHeader->DestinationPort) {
				...
				if (RtlCompareMemory(reserved->EthernetHeader.DestinationAddress,
									 BroadcastAddr, 
				...
				if (RtlCompareMemory(&address->NetworkName->Node, 
									 reserved->EthernetHeader.DestinationAddress, 
									 ETHERNET_ADDRESS_LENGTH) == ETHERNET_ADDRESS_LENGTH) {
		...
		if (lpxHeader->LpxType == LPX_TYPE_STREAM) {
			...
			for (connection = NULL, thisEntry = listHead->Flink;
			    ...
			    if (connection->LpxSmp.SmpState != SMP_STATE_CLOSE							&& 
					RtlCompareMemory(connection->LpxSmp.DestinationAddress.Node, 
									 reserved->EthernetHeader.SourceAddress, 
									 ETHERNET_ADDRESS_LENGTH) == ETHERNET_ADDRESS_LENGTH	&& 
			...
			InterlockedExchange( &reserved->ReorderCount, 0 );
			...
			if (lpxHeader->DestinationPort == NTOHS(LPXRP_HIX_PORT)) {
				...
				DebugPrint( 1, ("[LPX] LpxReceiveComplete: DataGram packet arrived.\n") );
			}
			...
			if (addressFile->RegisteredReceiveDatagramHandler) {
			    ...
			    userDataLength = reserved->PacketRawDataLength - reserved->PacketRawDataOffset;
	...
	)
{
	...
	PLPX_HEADER		lpxHeader = &reserved->LpxHeader;
	...
	) 
{
	...
	case LSCTL_CONNREQ | LSCTL_ACK:
		...
		if (replyPacket) {
			...
			ExInterlockedInsertTailList( &Connection->LpxSmp.TramsmitQueue,
										 &LpxGetReserved(replyPacket)->ListEntry,
										 &Connection->LpxSmp.TransmitSpinLock );
		...
		if (((SHORT)(NTOHS(lpxHeader->Sequence) - SHORT_SEQNUM(Connection->LpxSmp.RemoteSequence))) > 0) {
			...
			for (reorderPacketList = Connection->LpxSmp.ReceiveReorderQueue.Flink;
				...
				if (((SHORT)(NTOHS(reorderLpxHeader->Sequence) - NTOHS(lpxHeader->Sequence))) > 0) {
					...
					InsertTailList( &reorderReserved->ListEntry, &reserved->ListEntry );
					packetConsumed = TRUE;
			...
			if (packetConsumed == FALSE) {
				...
				InsertTailList( &Connection->LpxSmp.ReceiveReorderQueue, &reserved->ListEntry );
				packetConsumed = TRUE;
			}
	...
	) 
{
	...
	NDAS_ASSERT( Connection->LpxSmp.SmpState == SMP_STATE_FIN_WAIT1 );
	...
	) 
{
	...
	NDAS_ASSERT( Connection->LpxSmp.SmpState == SMP_STATE_FIN_WAIT2 );
	...
	) 
{
	...
	NDAS_ASSERT( Connection->LpxSmp.SmpState == SMP_STATE_CLOSING );
	...
	) 
{
	...
	NDAS_ASSERT( Connection->LpxSmp.SmpState == SMP_STATE_CLOSE_WAIT );
	...
	) 
{
	...
	NDAS_ASSERT( Connection->LpxSmp.SmpState == SMP_STATE_LAST_ACK );	
	...
	) 
{
	...
	NDAS_ASSERT( Connection->LpxSmp.SmpState == SMP_STATE_TIME_WAIT );
	...
	)
{
	...
	while (!IsListEmpty(&Connection->LpxSmp.TramsmitQueue)) {
		...
		Connection->LpxSmp.ResponseTimeOfSendPackets.QuadPart += systemTime.QuadPart - reserved->SendTime.QuadPart;					
		...
		if (NTOHS(lpxHeader->PacketSize & ~LPX_TYPE_MASK)+14 <= 60) {
			...
			Connection->LpxSmp.ResponseTimeOfSmallSendPackets.QuadPart += systemTime.QuadPart - reserved->SendTime.QuadPart;					
			...
			Connection->LpxSmp.ResponseTimeOfLargeSendPackets.QuadPart += systemTime.QuadPart - reserved->SendTime.QuadPart;					
