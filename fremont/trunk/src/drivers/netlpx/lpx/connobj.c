...

{
	...
	Connection->LpxSmp.LastAckInterval.QuadPart        = LPX_DEFAULT_LAST_ACK_INTERVAL;
...

{
	...
	DebugPrint( 2, ("LpxDestroyAssociation: connection %p port %X\n",
					TransportConnection, address->NetworkName->Port) );
...

{
    ...
    if (!(Connection->Flags2 & CONNECTION_FLAGS2_STOPPING)) {
		...
		if (Connection->LpxSmp.SmpState == SMP_STATE_CLOSE) { // Already closed
			...
			DebugPrint( 2, ("[LPX]LpxStopConnection: Connection->LpxSmp.SmpState = %s, Connection->Status = %x\n", 
			...
			if (Connection->LpxSmp.ConnectIrp) {
				...
				DebugPrint( 2, ("[LPX]LpxStopConnection: ConnectIrp %p canceled\n", irp) );	
			...
			if (Connection->LpxSmp.ListenIrp) {
				...
				DebugPrint( 2, ("[LPX]LpxStopConnection: ListenIrp %p canceled\n", irp) );	
			...
			if (Connection->LpxSmp.DisconnectIrp) {
				...
				DebugPrint( 2, ("[LPX]LpxStopConnection: DisconnectIrp %p canceled\n", irp) );	
			...
			while (thisEntry = ExInterlockedRemoveHeadList( &Connection->LpxSmp.ReceiveIrpQueue, 
				...
				pendingIrp->IoStatus.Status = STATUS_NETWORK_UNREACHABLE;
				LpxIoCompleteRequest( pendingIrp, IO_NO_INCREMENT );

				DebugPrint( 2, ("[LPX] SmpCancelIrps: Cancelled Receive IRP %p\n", pendingIrp) );
			...
			while (listEntry = ExInterlockedRemoveHeadList(&Connection->LpxSmp.ReceiveReorderQueue, 
				...
				DebugPrint( 2, ("LpxStopConnection: Removing packet from ReorderQueue\n") );
			...
			while (listEntry = ExInterlockedRemoveHeadList( &Connection->LpxSmp.ReceiveQueue, 
				...
				DebugPrint( 2, ("LpxStopConnection: Removing packet from ReceiveQueue\n") );
				reserved = CONTAINING_RECORD( listEntry, LPX_RESERVED, ListEntry );
				packet = reserved->Packet;
				PacketFree( Connection->AddressFile->Address->Provider, packet );
			}
			...
			while (packet = PacketDequeue(&Connection->LpxSmp.TramsmitQueue, &Connection->LpxSmp.TransmitSpinLock)) {
				...
				irpSp = LpxGetReserved(packet)->IrpSp;
				...
				// Check Cloned Packet. 
				if (LpxGetReserved(packet)->Cloned > 0) {

					DebugPrint( 1, ("[LPX]LpxCloseServicePoint: Cloned is NOT 0. %d\n", LpxGetReserved(packet)->Cloned) );
				}
			...
			if (FlagOn(Connection->Flags2, CONNECTION_FLAGS2_ASSOCIATED)) {
			...
			if (Connection->Status != STATUS_LOCAL_DISCONNECT && addressFile) {

				(*addressFile->DisconnectHandler)( addressFile->DisconnectHandlerContext,
												   Connection->Context,
												   0,
												   NULL,
												   0,
