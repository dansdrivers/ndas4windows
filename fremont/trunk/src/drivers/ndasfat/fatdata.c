...
#if __NDAS_FAT__
PDEVICE_OBJECT FatControlDeviceObject;
#endif
...
LONGLONG FatDebugTraceLevel = 0x00000009;
...

{
	...
	if (ExceptionCode == STATUS_CANT_WAIT) {
		...
		NDAS_ASSERT( FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_DONT_POST_REQUEST) &&
					 FlagOn(IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT) );

		ClearFlag( IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_DONT_POST_REQUEST );

		Irp->IoStatus = IrpContext->OriginalIrpIoStatus;
...

{
	...
	NDAS_ASSERT( IrpContext == NULL || !FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_RESOURCE_ACQUIRED) );
