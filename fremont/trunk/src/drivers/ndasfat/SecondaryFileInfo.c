...
NTSTATUS
NdasFatSecondaryCommonSetInformation (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    )
...

{
    ...
    try {
        ...
        case FileBasicInformation:
            ...
            Status = NdasFatSetBasicInfo( IrpContext, Irp, Fcb, Ccb );
            break;
        ...
        case FileDispositionInformation:
            ...
            if ( FlagOn(Vcb->VcbState, VCB_STATE_FLAG_DEFERRED_FLUSH) &&
                ...
                Status = NdasFatSetDispositionInfo( IrpContext, Irp, FileObject, Fcb );
            }
        ...
        case FileRenameInformation:
            ...
            if (!FlagOn(IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT)) {
                ...
                Status = NdasFatSetRenameInfo( IrpContext, Irp, Vcb, Fcb, Ccb );
        ...
        case FilePositionInformation:
            ...
            Status = NdasFatSetPositionInfo( IrpContext, Irp, FileObject );
            break;
        ...
        case FileAllocationInformation:
            ...
            Status = NdasFatSetAllocationInfo( IrpContext, Irp, Fcb, FileObject );
            break;

        case FileEndOfFileInformation:

            Status = NdasFatSetEndOfFileInfo( IrpContext, Irp, FileObject, Vcb, Fcb );
            break;
...

{
    ...
    try {
...
#if defined(_WIN64)
		ndfsWinxpRequestHeader->SetFile.DeleteHandle	= (UINT64)setFile.DeleteHandle;
#else
		ndfsWinxpRequestHeader->SetFile.DeleteHandle	= (UINT32)setFile.DeleteHandle;
#endif
		...
		NDAS_ASSERT( NT_SUCCESS(NTOHL(ndfsWinxpReplytHeader->Status4)) );

#if 0
		if (NT_SUCCESS(NTOHL(ndfsWinxpReplytHeader->Status4))) {
...

{
	...
	try {
		...
		secondaryRequest = AllocateWinxpSecondaryRequest( volDo->Secondary, IRP_MJ_SET_INFORMATION, 0 );
...
#if !__NDAS_FAT_FORCE_HANDLE_0__
		NDAS_ASSERT(FALSE);
		ndfsWinxpRequestHeader->SetFile.ClusterCount	= setFile.ClusterCount;
		ndfsWinxpRequestHeader->SetFile.DeleteHandle	= setFile.DeleteHandle;
#else
...
#if !__NDAS_FAT_FORCE_HANDLE_0__
		NDAS_ASSERT( NT_SUCCESS(status) || status == STATUS_CANNOT_DELETE || status == STATUS_DIRECTORY_NOT_EMPTY );
#else
		//NDAS_ASSERT( NT_SUCCESS(status) || status == 0x9BD084AC || status == STATUS_CANNOT_DELETE || status == STATUS_DIRECTORY_NOT_EMPTY );
#endif
...

{
    ...
    try {
		...
		if (!FlagOn(volDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW)) {
			...
			ndfsWinxpRequestHeader->SetFile.AllocationInformation.AllocationSize  = Buffer->AllocationSize.LowPart;
        ...
        if (NewAllocationSize > Fcb->Header.AllocationSize.LowPart) {
			...
			if (NT_SUCCESS(Status) && NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) != (UINT64)Fcb->Header.AllocationSize.QuadPart) {
				...
				for (index=0, vcn=0; index < NTOHL(ndfsWinxpReplytHeader->NumberOfMcbEntry4); index++, mcbEntry++) {
					...
					if (lookupResut == TRUE && vcn < Fcb->Header.AllocationSize.QuadPart) {
						...
						if (vcn < NTOHLL(ndfsWinxpReplytHeader->AllocationSize8)) {
							...
							ASSERT( lcn == (((LBO)mcbEntry->Lcn) << IrpContext->Vcb->AllocationSupport.LogOfBytesPerSector) );
							ASSERT( clusterCount <= mcbEntry->ClusterCount || 
									Fcb->Header.AllocationSize.QuadPart > (INT64)NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) && (index+1) == NTOHL(ndfsWinxpReplytHeader->NumberOfMcbEntry4) );
							...
							if (clusterCount < mcbEntry->ClusterCount) {
								...
								FatAddMcbEntry ( IrpContext->Vcb, 
												 &Fcb->Mcb, 
												 (VBO)mcbEntry->Vcn, 
												 ((LBO)mcbEntry->Lcn) << IrpContext->Vcb->AllocationSupport.LogOfBytesPerSector, 
				...
				if (Fcb->Header.AllocationSize.QuadPart > (INT64)NTOHLL(ndfsWinxpReplytHeader->AllocationSize8)) {
					...
					Fcb->Header.FileSize.LowPart = (UINT32)NTOHLL(ndfsWinxpReplytHeader->FileSize8);

					DebugTrace2(0, Dbg, ("NtfsSetEndOfFileInfo scb->Header.FileSize.LowPart = %I64x, scb->Header.ValidDataLength.QuadPart = %I64x\n", 
										 Fcb->Header.FileSize.LowPart, Fcb->Header.ValidDataLength.QuadPart) );
			
					if (Fcb->Header.ValidDataLength.QuadPart > Fcb->Header.FileSize.LowPart) {

						Fcb->Header.ValidDataLength.QuadPart = Fcb->Header.FileSize.LowPart;
...

{
    ...
    try {
		...
		if (!FlagOn(volDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW)) {
			...
			ndfsWinxpRequestHeader->SetFile.EndOfFileInformation.EndOfFile = Buffer->EndOfFile.LowPart;
        ...
        if (Fcb->Header.FileSize.LowPart != NewFileSize) {
			...
			ASSERT( STATUS_DISK_FULL == Status || NT_SUCCESS(Status) );
			...
			if (!NT_SUCCESS(Status)) {
				...
				if (NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) != (UINT64)Fcb->Header.AllocationSize.QuadPart) {
				...
				ASSERT( NewFileSize == NTOHLL(ndfsWinxpReplytHeader->FileSize8) );
				NewFileSize = (ULONG)NTOHLL(ndfsWinxpReplytHeader->FileSize8);
			}
    ...
    )
{
	...
	try {
		...
		if (setFile.FileObject == NULL) {
			...
			if (FlagOn(setFileCcb->NdasFatFlags, ND_FAT_CCB_FLAG_UNOPENED)) {

				ASSERT( FlagOn(setFileCcb->NdasFatFlags, ND_FAT_CCB_FLAG_CORRUPTED) );
...
#if defined(_WIN64)
		ASSERT( ndfsWinxpRequestHeader->SetFile.DeleteHandle	== (UINT64)setFile.DeleteHandle );
#else
		ASSERT( ndfsWinxpRequestHeader->SetFile.DeleteHandle	== (UINT32)setFile.DeleteHandle );
#endif
