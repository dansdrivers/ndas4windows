	...
	)
{
	...
	while (primarySessionTerminate == FALSE) {
		...
		if (eventStatus == 0) {
			...
			while (primarySessionRequestEntry = ExInterlockedRemoveHeadList( &PrimarySession->RequestQueue,
				...
				} else if (primarySessionRequest->RequestType == PRIMARY_SESSION_REQ_SURPRISE_REMOVAL) {
			...
			while (TRUE) {
				...
				NDAS_ASSERT( !FlagOn(PrimarySession->Thread.Flags, PRIMARY_SESSION_THREAD_FLAG_DISCONNECTED) );
	...
	NDAS_ASSERT( !LpxTdiV2IsRequestPending(&PrimarySession->Thread.ReceiveOverlapped, 0) );
