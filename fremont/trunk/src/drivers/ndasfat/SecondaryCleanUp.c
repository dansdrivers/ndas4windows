...

{
    ...
    try {
        ...
        case UserFileOpen:
			...
			if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY &&
				...
				if ((TypeOfOpen == UserFileOpen) /*&& 
					FlagOn(FileObject->Flags, FO_FILE_MODIFIED) &&
					FlagOn(Ccb->CreateContext.SecurityContext.DesiredAccess, FILE_WRITE_DATA) */) {
		...
		SetFlag( Ccb->NdasFatFlags, ND_FAT_CLEANUP_COMPLETE );
		...
		if (TypeOfOpen == UserFileOpen) {
			...
			NDAS_ASSERT( Fcb->Header.ValidDataLength.LowPart <= Fcb->Header.FileSize.LowPart );
			NDAS_ASSERT( Fcb->Header.FileSize.LowPart <= Fcb->Header.AllocationSize.LowPart );

			ndfsWinxpRequestHeader->CleanUp.AllocationSize	= Fcb->Header.AllocationSize.LowPart;
			ndfsWinxpRequestHeader->CleanUp.FileSize		= Fcb->Header.FileSize.LowPart;
			ndfsWinxpRequestHeader->CleanUp.ValidDataLength = Fcb->Header.ValidDataLength.LowPart;
