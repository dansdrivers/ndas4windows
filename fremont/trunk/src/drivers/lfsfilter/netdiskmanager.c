	...
	)
{
    ...
    if (result == 0) {
		...
		NDAS_ASSERT( IsListEmpty(&NetdiskManager->EnabledNetdiskQueue) );
...
{	
    ...
    for (enabledNetdisklistEntry = NULL, enabledNetdisklistEntry = NetdiskManager->EnabledNetdiskQueue.Flink;
		...
		NDAS_ASSERT( enabledNetdisk->ReferenceCount > 0 );
	    ...
	    for (netdiskPartition = NULL, netdiskPartitionlistEntry = enabledNetdisk->NetdiskPartitionQueue.Flink;
			...
			NDAS_ASSERT( netdiskPartition->ReferenceCount > 0 );
			...
			QueueingNetdiskManagerRequest( GlobalLfs.NetdiskManager, netdiskManagerRequest );
		}
	}
	...
	)
{
	...
	while (netdiskManagerThreadExit == FALSE) {
		...
		timeOut.QuadPart = -LFS_NETDISK_MANAGER_TIME_OUT;
		...
		if (waitStatus == STATUS_TIMEOUT) {
			...
			//NetdiskManager_TouchVolume( NetdiskManager );
			continue;
		}
		...
		while (netdiskManagerThreadExit == FALSE && 
			...
			if (netdiskManagerRequest->RequestType == NETDISK_MANAGER_REQUEST_DOWN) {
				...
				NDAS_ASSERT( IsListEmpty(&NetdiskManager->Thread.RequestQueue) );
				...
				do {
					...
					SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
								   ("NetdiskManagerThreadProc: NETDISK_MANAGER_REQUEST_TOUCH_VOLUME netdiskManagerRequest->NetdiskPartition = %p\n", 
								    netdiskManagerRequest->NetdiskPartition) );

					retryCount = 6;

					while (retryCount) {

						if (EnabledNetdisk_IsStoppedNetdisk(netdiskManagerRequest->NetdiskPartition->EnabledNetdisk)) {

							SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
											("NetdiskManagerThreadProc: IsStoppedNetdisk\n") );
					...
					if (retryCount) {
						...
						SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
										("NetdiskManagerThreadProc: retryCount = %d\n", retryCount) );
					...
					while (unicodeCreateStatus == STATUS_SUCCESS && retryCount) {
						...
						if (createStatus == STATUS_SUCCESS) {
							...
							NDAS_ASSERT( ioStatusBlock.Information == FILE_CREATED );
							ZwClose( fileHandle );
							break;
						}
				...
				do {
					...
					SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
								   ("NetdiskManagerThreadProc: NETDISK_MANAGER_REQUEST_CREATE_FILE netdiskManagerRequest->NetdiskPartition = %p\n", 
								    netdiskManagerRequest->NetdiskPartition) );
					...
					while (unicodeCreateStatus == STATUS_SUCCESS && retryCount) {
						...
						if (createStatus == STATUS_SUCCESS) {
							...
							NDAS_ASSERT( ioStatusBlock.Information == FILE_CREATED ||  ioStatusBlock.Information == FILE_OPENED );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO, ("NetdiskManager_FileSystemShutdown: Entered\n") );
	...
	)
{
	...
	status = GetScsiportAdapter( NetdiskPartition->NetdiskPartitionInformation.DiskDeviceObject, &scsiportAdapterDeviceObject );
	...
	if (IS_WINDOWS2K()) {
		...
		ndasBace.StartingOffset = NetdiskPartition->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart;
		ndasBace.Length = NetdiskPartition->NetdiskPartitionInformation.PartitionInformation.PartitionLength.QuadPart;

	} else {

		ndasBace.StartingOffset = NetdiskPartition->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart;
		ndasBace.Length = NetdiskPartition->NetdiskPartitionInformation.PartitionInformationEx.PartitionLength.QuadPart;
	}
	...
	//if (!NT_SUCCESS(status)) {
		...
		status = LfsFilterDeviceIoControl( scsiportAdapterDeviceObject,
										   IOCTL_SCSI_MINIPORT,
										   psrbioctl,
										   outbuff_sz,
										   psrbioctl,
	...
	)
{
	...
	if (netdiskPartition) {
		...
		*NetdiskPartitionInformation = netdiskPartition->NetdiskPartitionInformation;
		...
		NDAS_ASSERT( NetdiskPartitionInformation->VolumeName.Buffer == NetdiskPartitionInformation->VolumeNameBuffer );
	...
	)
{
	...
	if (NetdiskInformation->DeviceMode == DEVMODE_SHARED_READWRITE) {
		...
		NDAS_ASSERT(FALSE);
		NDAS_ASSERT( NetdiskInformation->DeviceMode == DEVMODE_EXCLUSIVE_READWRITE );
...
if (IS_WINDOWS2K()) {
	...
	for (partitionIndex = 0; partitionIndex < driveLayoutInformation->PartitionCount; partitionIndex++) {
		...
		if (netdiskPartition) {
			...
			if (netdiskPartition->DiskDeviceObjectReferenceCount) {
...
if (IS_WINDOWS2K()) {
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_PreMountVolume: netdiskPartition = %p, netdiskEnableMode = %d\n", 
				    netdiskPartition, netdiskEnableMode) );

	if (netdiskPartition->NetdiskVolume[netdiskEnableMode].VolumeState != VolumeEnabled && 
	...
	if (FlagOn(netdiskPartition->EnabledNetdisk->NetdiskInformation.SupportedFeatures, NDASFEATURE_SIMULTANEOUS_WRITE)) {
	...
	if (GlobalLfs.NdasFsForceFiltering) {
	...
	*NetdiskPartition = netdiskPartition;
	...
	netdiskPartition->DiskDeviceObjectReferenceCount ++;

	if (netdiskPartition->DiskDeviceObjectReferenceCount == 1) {
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_PostMountVolume: NetdiskPartition = %p, LfsDeviceExt = %p NetdiskEnableMode = %d\n", 
				    NetdiskPartition, LfsDeviceExt, NetdiskEnableMode) );
	...
	if (Mount == FALSE) {
		...
		NDAS_ASSERT( NetdiskPartition->DiskDeviceObjectReferenceCount );

		NetdiskPartition->DiskDeviceObjectReferenceCount --;

		if (NetdiskPartition->DiskDeviceObjectReferenceCount == 0) {
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_MountVolumeComplete: NetdiskPartition = %p, LfsDeviceExt = %p NetdiskEnableMode = %d\n", 
				    NetdiskPartition, NetdiskPartition->NetdiskVolume[NetdiskEnableMode].LfsDeviceExt, NetdiskEnableMode) );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_ChangeMode: NetdiskPartition = %p, LfsDeviceExt = %p NetdiskEnableMode = %d\n", 
				    NetdiskPartition, NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].LfsDeviceExt, NetdiskEnableMode) );
	...
	if (NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumeEnabled) {
		...
		NDAS_ASSERT( GlobalLfs.NdasFsMiniMode == TRUE && NetdiskEnableMode == NULL );	
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_SurpriseRemoval: NetdiskPartition = %p, LfsDeviceExt = %p NetdiskEnableMode = %d\n", 
				    NetdiskPartition, NetdiskPartition->NetdiskVolume[NetdiskEnableMode].LfsDeviceExt, NetdiskEnableMode) );
	...
	if (NetdiskPartition->DiskDeviceObjectReferenceCount == 0) {
		...
		if (NetdiskPartition->NetdiskPartitionInformation.DiskDeviceObject == NULL) {
...
{
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_QueryRemoveMountVolume: NetdiskPartition = %p, LfsDeviceExt = %p NetdiskEnableMode = %d\n", 
				    NetdiskPartition, NetdiskPartition->NetdiskVolume[NetdiskEnableMode].LfsDeviceExt, NetdiskEnableMode) );
...
{
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_CancelRemoveMountVolume: NetdiskPartition = %p, LfsDeviceExt = %p NetdiskEnableMode = %d\n", 
				    NetdiskPartition, NetdiskPartition->NetdiskVolume[NetdiskEnableMode].LfsDeviceExt, NetdiskEnableMode) );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_DisMountVolume: NetdiskPartition = %p, LfsDeviceExt = %p NetdiskEnableMode = %d\n", 
				    NetdiskPartition, NetdiskPartition->NetdiskVolume[NetdiskEnableMode].LfsDeviceExt, NetdiskEnableMode) );
	...
	)
{
	...
	if (!FlagOn(NetdiskPartition->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)	&&
		(NetdiskPartition->FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS || 
		 NetdiskPartition->FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT)) {
	...
	)
{
	...
	if (netdiskPartition) {
		...
		NDAS_ASSERT( !FlagOn(netdiskPartition->Flags, NETDISK_PARTITION_FLAG_ALREADY_DELETED) );
	}
	...
	)
{
    ...
    if (result == 0) {
		...
		NDAS_ASSERT( NetdiskPartition->ListEntry.Flink == NetdiskPartition->ListEntry.Blink );
				
		for (volumeIndex = 0; volumeIndex < NETDISK_SECONDARY2PRIMARY+1; volumeIndex++) {

			NDAS_ASSERT( NetdiskPartition->NetdiskVolume[volumeIndex].LfsDeviceExt == NULL );
		}

		NDAS_ASSERT( NetdiskPartition->ExportCount == 0 );
		NDAS_ASSERT( NetdiskPartition->LocalExportCount == 0 );
		NDAS_ASSERT( NetdiskPartition->NetdiskPartitionInformation.DiskDeviceObject == NULL );

		NetdiskPartition->EnabledNetdisk = NULL;

		NDAS_ASSERT( IsListEmpty( &NetdiskPartition->PrimarySessionQueue ) );
