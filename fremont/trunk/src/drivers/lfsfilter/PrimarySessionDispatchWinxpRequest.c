...
//		ULONG				originalFileSpyDebugLevel; 
		...
		try {
			...
			if (relatedOpenFileId == 0) {
				...
				if (relatedOpenFile == NULL) {
					...
					NDAS_ASSERT(FALSE);
					createStatus = STATUS_FILE_CLOSED;
					leave;
				}
			...
			if (createStatus == STATUS_SUCCESS) {
				...
				openFile = PrimarySession_AllocateOpenFile( PrimarySession, fileHandle, NULL, &fileName );
				...
				NDAS_ASSERT( !NT_SUCCESS(createStatus) );
			}
	...
	case IRP_MJ_CLOSE: { // 0x02
		...
		NDAS_ASSERT( openFile->AlreadyClosed == FALSE );
...
RETRY_WRITE:
	...
	case IRP_MJ_QUERY_INFORMATION: { // 0x05
		...
		NDAS_ASSERT( NdfsWinxpRequestHeader->QueryFile.Length <= PrimarySession->SessionContext.SecondaryMaxDataSize );
		...
		if (queryInformationStatus == STATUS_BUFFER_OVERFLOW) {
			...
			NDAS_ASSERT( length == returnedLength );
		}

		if (NT_SUCCESS(queryInformationStatus)) {

			NDAS_ASSERT(queryInformationStatus == STATUS_SUCCESS);
		}
	...
	case IRP_MJ_SET_INFORMATION: { // 0x06
		...
		if (NdfsWinxpRequestHeader->SetFile.FileInformationClass == FileBasicInformation) {
			...
			NDAS_ASSERT( sizeof(FILE_LINK_INFORMATION) - sizeof(WCHAR) - 8 + NdfsWinxpRequestHeader->SetFile.LinkInformation.FileNameLength <= 
						 NdfsWinxpRequestHeader->SetFile.Length );
			...
			if (NdfsWinxpRequestHeader->SetFile.RenameInformation.RootDirectoryHandle != 0) {
				...
				NDAS_ASSERT( rootDirectoryFile->AlreadyClosed == FALSE );
			...
			RtlCopyMemory( linkInfomation->FileName, ndfsRequestData, linkInfomation->FileNameLength );
			...
			NDAS_ASSERT( sizeof(FILE_RENAME_INFORMATION) - sizeof(WCHAR) - 8 + NdfsWinxpRequestHeader->SetFile.RenameInformation.FileNameLength <= 
						 NdfsWinxpRequestHeader->SetFile.Length );
			...
			if (ndfsRequestData[0] == (UCHAR) OBJ_NAME_PATH_SEPARATOR || renameInformation->RootDirectory != NULL ) {
				...
				RtlCopyMemory( renameInformation->FileName, ndfsRequestData, renameInformation->FileNameLength );
			}
		...
		setInformationStatus = ZwSetInformationFile (openFile->FileHandle,
													 &ioStatusBlock,
													 fileInformation,
													 length,
													 fileInformationClass );
		...
		if (NT_SUCCESS(setInformationStatus)) {
			...
			NDAS_ASSERT( setInformationStatus == STATUS_SUCCESS );
		}
	...
	case IRP_MJ_QUERY_EA: {	//	0x07
		...
		NDAS_ASSERT( openFile->AlreadyClosed == FALSE && openFile->FileObject );
		...
		SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_NOISE, ("LFS: DispatchWinXpRequest: IRP_MJ_QUERY_EA: length:%d\n", length) );
		...
		if (NT_SUCCESS(queryEaStatus)) {
			...
			NDAS_ASSERT( queryEaStatus == STATUS_SUCCESS );
		}
		...
		if (queryEaStatus == STATUS_SUCCESS || queryEaStatus == STATUS_BUFFER_OVERFLOW) {
			...
			if (ioStatusBlock.Information != 0) {
				...
				if (ioStatusBlock.Information > length) {
					...
					SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
									("DispatchWinXpRequest, IRP_MJ_QUERY_EA: Ea is set NdfsWinxpRequestHeader->QueryEa.Length = %d, ioStatusBlock.Information = %d\n",
									 NdfsWinxpRequestHeader->QueryEa.Length, ioStatusBlock.Information) );
				}
				...
				*replyDataSize = 0;
				...
				while (1) {
					...
					if (fileFullEa->NextEntryOffset) {

						if (*replyDataSize + fileFullEa->NextEntryOffset > length) {

							SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_ERROR,
											("DispatchWinXpRequest, IRP_MJ_QUERY_EA: Ea is set NdfsWinxpRequestHeader->QueryEa.Length = %d, ioStatusBlock.Information = %d, *replyDataSize = %d, *replyDataSize2 = %d\n",
											 NdfsWinxpRequestHeader->QueryEa.Length, ioStatusBlock.Information, *replyDataSize, 
											 *replyDataSize + FIELD_OFFSET(FILE_FULL_EA_INFORMATION, EaName) + fileFullEa->EaNameLength + fileFullEa->EaValueLength) );

							NDAS_ASSERT(FALSE);

							fileFullEa->NextEntryOffset = 0;
						...
						*replyDataSize += fileFullEa->NextEntryOffset;
						...
						if (*replyDataSize + FIELD_OFFSET(FILE_FULL_EA_INFORMATION, EaName) + fileFullEa->EaNameLength + fileFullEa->EaValueLength > length) {
		...
		NDAS_ASSERT( *replyDataSize <= length );
	...
	case IRP_MJ_SET_VOLUME_INFORMATION: {	// 0x0B
		...
		if (NT_SUCCESS(setVolumeStatus)) {
			...
			NDAS_ASSERT( setVolumeStatus == STATUS_SUCCESS );
		}
	...
	case IRP_MJ_DIRECTORY_CONTROL: { // 0x0C
        ...
        if (NdfsWinxpRequestHeader->IrpMinorFunction == IRP_MN_QUERY_DIRECTORY) {
			...
			NDAS_ASSERT( NdfsWinxpRequestHeader->QueryDirectory.Length <= PrimarySession->SessionContext.SecondaryMaxDataSize );
			...
			queryDirectoryStatus = RtlAppendUnicodeToString( &fileName, (PWCHAR)ndfsRequestData );
			...
			if (indexSpecified) {
				...
				queryDirectoryStatus = LfsQueryDirectoryByIndex( openFile->FileHandle,
																 fileInformationClass,
																 fileInformation,
																 length,
			...
			if (NT_SUCCESS(queryDirectoryStatus)) {
				...
				NDAS_ASSERT( queryDirectoryStatus == STATUS_SUCCESS );		
				NDAS_ASSERT( queryDirectoryStatus == ioStatusBlock.Status );
			}
			
			if (queryDirectoryStatus == STATUS_BUFFER_OVERFLOW) {
			
				NDAS_ASSERT( length == ioStatusBlock.Information );
			}
	...
	case IRP_MJ_QUERY_QUOTA: {
		...
		if (NT_SUCCESS(queryQuotaStatus)) {
			...
			NDAS_ASSERT( queryQuotaStatus == STATUS_SUCCESS );
		}
		...
		if (queryQuotaStatus == STATUS_SUCCESS || queryQuotaStatus == STATUS_BUFFER_OVERFLOW) {
	...
	case IRP_MJ_SET_QUOTA: {
		...
		setQuotaStatus = NtSetQuotaInformationFile( openFile->FileHandle,
													&ioStatusBlock,
													ndfsRequestData,
													length );
		...
		if (NT_SUCCESS(setQuotaStatus)) {
			...
			NDAS_ASSERT( setQuotaStatus == STATUS_SUCCESS );
		}
	...
	if (NTOHL(NdfsWinxpReplytHeader->Status4) != STATUS_SUCCESS && NTOHL(NdfsWinxpReplytHeader->Status4) != STATUS_BUFFER_OVERFLOW) {

		  NDAS_ASSERT( *replyDataSize == 0 );
	}
	...
	NDAS_ASSERT( NTOHL(NdfsWinxpReplytHeader->Status4) != STATUS_PENDING );
	...
	)
{
	...
	InitializeObjectAttributes( &objectAttributes, &OpenFile->FullFileName, attributes, NULL, NULL );
