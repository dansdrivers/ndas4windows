...
NTSTATUS 
(*LfsOrginalRofsAcquireFileForModWrite) (
	PFILE_OBJECT	FileObject,
	PLARGE_INTEGER	EndingOffset,
	PERESOURCE		*ResourceToRelease,
	PDEVICE_OBJECT DeviceObject) = NULL;
	...
	)
{
	...
	DbgPrint( "FIELD_OFFSET(FILE_GET_EA_INFORMATION, EaName) = %d, FIELD_OFFSET(FILE_GET_EA_INFORMATION, EaName[1]) = %d, sizeof(FILE_GET_EA_INFORMATION) - sizeof(CHAR) = %d\n",
			  FIELD_OFFSET(FILE_GET_EA_INFORMATION, EaName), FIELD_OFFSET(FILE_GET_EA_INFORMATION, EaName[1]), sizeof(FILE_GET_EA_INFORMATION) - sizeof(CHAR) );
	...
	gFileSpyDebugLevel |= LFS_DEBUG_TABLE_ERROR;
	...
	NDAS_ASSERT( DEFAULT_MAX_DATA_SIZE % 8 == 0 );
	NDAS_ASSERT( sizeof(NDFS_WINXP_REQUEST_HEADER) % 8 == 0 );
	NDAS_ASSERT( sizeof(NDFS_WINXP_REPLY_HEADER) % 8 == 0 );
...
#if __NDAS_FS_NDAS_NTFS_MODE__

	if (RtlCheckRegistryKey(RTL_REGISTRY_SERVICES, NDAS_NTFS_NAME) == STATUS_SUCCESS) {
	
		lfs->NdasNtfsSupport = TRUE;

	} else {

#if __NDAS_FS_CHECK_NTFS__
		NDAS_ASSERT(FALSE);
#endif
	}
	...
	if (RtlCheckRegistryKey(RTL_REGISTRY_SERVICES, NDAS_ROFS_NAME) == STATUS_SUCCESS) {
		...
		lfs->NdasRofsSupport = TRUE;

	} else {

#if __NDAS_FS_CHECK_ROFS__
		NDAS_ASSERT(FALSE);
#endif
	}

	if (RtlCheckRegistryKey(RTL_REGISTRY_SERVICES, NDAS_FAT_NAME) == STATUS_SUCCESS) {

		lfs->NdasFatSupport = TRUE;

	} else {

#if __NDAS_FS_CHECK_FAT__
		NDAS_ASSERT(FALSE);
#endif
	}

#if __NDAS_FS_FORCE_FILTERING__

	lfs->NdasFsForceFiltering = TRUE;
	...
	)
{
	...
	if (LfsDeviceExt->SecondaryState == CONNECTED_TO_LOCAL_STATE) {
		...
		HANDLE	purgeVolumeHandle = NULL;

		ExAcquireFastMutex( &LfsDeviceExt->FastMutex );

		if (LfsDeviceExt->PurgeVolumeHandle) {

			if (LfsDeviceExt->ProcessingIrpCount == 0 && IsListEmpty(&LfsDeviceExt->Secondary->FcbQueue)) {

				purgeVolumeHandle = LfsDeviceExt->PurgeVolumeHandle;
				LfsDeviceExt->PurgeVolumeHandle = NULL;
			}
		}

		ExReleaseFastMutex( &LfsDeviceExt->FastMutex );

		if (purgeVolumeHandle) {

			//SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsDeviceExt->Vpb->ReferenceCount = %d\n", LfsDeviceExt->Vpb->ReferenceCount) );
			ZwClose( purgeVolumeHandle );
			//SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsDeviceExt->Vpb->ReferenceCount = %d\n", LfsDeviceExt->Vpb->ReferenceCount) );
		}
	}
    ...
    if (result == 0) {
		...
		NDAS_ASSERT( LfsDeviceExt->Secondary == NULL );
	...
	)
{
	...
	*NdasUsageFlags = 0;
		...
		if (lfsDeviceExt->NetdiskPartitionInformation.NetdiskInformation.SlotNo == SlotNo) {
			...
			if (lfsDeviceExt->FilteringMode == LFS_SECONDARY_TO_PRIMARY || lfsDeviceExt->FilteringMode == LFS_PRIMARY) {
				...
				*NdasUsageFlags |= LFS_NDASUSAGE_FLAG_SECONDARY;
				...
				if ((FlagOn(lfsDeviceExt->Flags, LFS_DEVICE_FLAG_FILTERING) && lfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT		||
					 FlagOn(lfsDeviceExt->Flags, LFS_DEVICE_FLAG_FILTERING) && lfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS	||
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsDeviceExt_QueryNDASUsage: *NdasUsageFlags = %x *MountedFSVolumeCount -= %d\n", 
										*NdasUsageFlags, *MountedFSVolumeCount) );
    ...
    )
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("LfsRofsAcquireFileForModWrite: Entered DeviceObject = %p\n", DeviceObject) );
	...
	if (SpyIsAttachedToDevice(DeviceObject, &attachedDeviceObject)) {
		...
		return LfsOrginalRofsAcquireFileForModWrite( FileObject, EndingOffset, ResourceToRelease, DeviceObject );
	}
    ...
    )
{
	...
	do {
		...
		if (result == 1) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsAttachToFileSystemDevice: Name compare %d %wZ %wZ\n",
		...
		RtlInitUnicodeString( &fsName, NDAS_ROFS_DEVICE_NAME );
		...
		if (result == 1) {
			...
			if (fastIoDispatch->AcquireForModWrite) {
				...
				LfsOrginalRofsAcquireFileForModWrite = fastIoDispatch->AcquireForModWrite;
				fastIoDispatch->AcquireForModWrite = LfsRofsAcquireFileForModWrite;
			}
    ...
    )
{
	...
	} else if (lfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDAS_ROFS && 
			   fastIoDispatch->AcquireForModWrite						 && 
			   LfsOrginalRofsAcquireFileForModWrite) {
		
		fastIoDispatch->AcquireForModWrite = LfsOrginalRofsAcquireFileForModWrite;
		LfsOrginalRofsAcquireFileForModWrite = NULL;	
	} 
    ...
    )
{
	...
	NDAS_ASSERT( LfsDeviceExt->FileSpyDeviceObject );

#else

	NDAS_ASSERT( LfsDeviceExt->InstanceContext || LfsDeviceExt->FileSpyDeviceObject );
	...
	NDAS_ASSERT( !FlagOn(LfsDeviceExt->Flags, LFS_DEVICE_FLAG_CLNEANUPED) );
	SetFlag( LfsDeviceExt->Flags, LFS_DEVICE_FLAG_CLNEANUPED );

	//NDAS_ASSERT( LfsDeviceExt->ReferenceCount == 1 );

	LfsDeviceExt_Dereference( LfsDeviceExt );

	SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR, ("LfsCleanupMountedDevice: return\n") );
    ...
    )
{
	...
	NDAS_ASSERT( LfsDeviceExt->ReferenceCount );
	...
	case LFS_SECONDARY: {
		...
		if (LfsDeviceExt->Secondary) {
			...
			Secondary_Close( LfsDeviceExt, LfsDeviceExt->Secondary );
		}
	...
	)
{
	...
	NDAS_ASSERT( KeGetCurrentIrql() <= APC_LEVEL );

	PrintIrp( (DevExt->LfsDeviceExt.SecondaryState == CONNECTED_TO_LOCAL_STATE) ? LFS_DEBUG_SECONDARY_TRACE : LFS_DEBUG_SECONDARY_TRACE, 
			  "LfsSecondaryPassThrough", &DevExt->LfsDeviceExt, Irp );
	...
	NDAS_ASSERT( !FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_CLNEANUPED) );
	...
	try {
		...
		if (irpSp->MajorFunction == IRP_MJ_CREATE) {
			...
			if (fileObject->FileName.Length >= 16) {
				...
				if (RtlEqualUnicodeString( &Extend, &tempUnicode, TRUE)) {
					...
					NDAS_ASSERT( FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) );
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("DevExt->LfsDeviceExt.Flags = %x\n", DevExt->LfsDeviceExt.Flags) );
		...
		NDAS_ASSERT( FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) && !FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTED) );
		...
		if (FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTING)) {
			...
			NDAS_ASSERT( DevExt->LfsDeviceExt.Secondary == NULL || 
						 Secondary_LookUpCcb(DevExt->LfsDeviceExt.Secondary, fileObject) == NULL );
		...
		if (irpSp->MajorFunction == IRP_MJ_PNP) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, 
						   ("%s: lfsDeviceExt = %p, IRP_MJ_PNP irpSp->MinorFunction = %d\n",
							__FUNCTION__, &DevExt->LfsDeviceExt, irpSp->MinorFunction) );
			...
			case IRP_MN_SURPRISE_REMOVAL: {
				...
				NDAS_ASSERT( DevExt->LfsDeviceExt.NetdiskPartition );
				
				netdiskPartition = DevExt->LfsDeviceExt.NetdiskPartition;
...
#if 0
				if (DevExt->LfsDeviceExt.DiskDeviceObject			&& 
					DevExt->LfsDeviceExt.MountVolumeDeviceObject	&& 
					DevExt->LfsDeviceExt.BaseVolumeDeviceObject) {

					ObReferenceObject( DevExt->LfsDeviceExt.DiskDeviceObject );
					ObReferenceObject( DevExt->LfsDeviceExt.MountVolumeDeviceObject );
					ObReferenceObject( DevExt->LfsDeviceExt.BaseVolumeDeviceObject );
				}
#endif
				...
				NDAS_ASSERT( KeReadStateEvent(&waitEvent) || !NT_SUCCESS(Irp->IoStatus.Status) );
	
				SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR, 
							   ("%s: lfsDeviceExt = %p, IRP_MN_SURPRISE_REMOVAL Irp->IoStatus.Status = %x\n",
			...
			case IRP_MN_QUERY_REMOVE_DEVICE: {
...
#if __NDAS_FS_CALL_CLOSE_ON_PNP__
				...
				NDAS_ASSERT( DevExt->LfsDeviceExt.AttachedToDeviceObject );
				...
				if (NT_SUCCESS(Irp->IoStatus.Status)) {
					...
					if (IS_WINDOWSXP_OR_LATER()) {
						...
						if (DevExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NTFS ||
							...
							NDAS_ASSERT( !FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTED) );
					...
					ClearFlag( DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTING );
					NDAS_ASSERT( DevExt->LfsDeviceExt.ProcessingIrpCount == 1 );
			...
			case IRP_MN_CANCEL_REMOVE_DEVICE: {
				...
				if (irpSp->MinorFunction == IRP_MN_CANCEL_REMOVE_DEVICE) {
					...
					SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO, 
								   ("%s: lfsDeviceExt = %p, IRP_MN_CANCEL_REMOVE_DEVICE\n", __FUNCTION__, &DevExt->LfsDeviceExt) );
		...
		if (irpSp->MajorFunction == IRP_MJ_FILE_SYSTEM_CONTROL) {
			...
			if (irpSp->MinorFunction == IRP_MN_USER_FS_REQUEST || irpSp->MinorFunction == IRP_MN_KERNEL_CALL) {
				...
				case FSCTL_LOCK_VOLUME: 
					...
					NDAS_ASSERT( fileObject && fileObject->FileName.Length == 0 && fileObject->RelatedFileObject == NULL );
				...
				case FSCTL_DISMOUNT_VOLUME: {
					...
					NDAS_ASSERT( IS_WINDOWSVISTA_OR_LATER() );
		...
		if (DevExt->LfsDeviceExt.SecondaryState == WAIT_PURGE_SAFE_STATE) {
			...
			NDAS_ASSERT( irpSp->MajorFunction == IRP_MJ_CREATE );
		...
		NDAS_ASSERT(fileObject);

		if (FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING) && DevExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT	||
			FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING) && DevExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS	||
	...
	)
{
	...
	NDAS_ASSERT( LfsDeviceExt->SecondaryState == SECONDARY_STATE );

	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, 
				   ("LfsSecondaryToPrimary: LfsDeviceExt = %p, LfsDeviceExt->SecondaryState = %d\n",
	...
	)
{
	...
	NDAS_ASSERT( FlagOn(LfsDeviceExt->Flags, LFS_DEVICE_FLAG_FILTERING) );
	...
	LfsDeviceExt->PurgeVolumeHandle = fileHandle;
	...
	do {
		...
		fileNameBuffer = ExAllocatePoolWithTag( PagedPool, NDFS_MAX_PATH*sizeof(WCHAR), LFS_ALLOC_TAG );
		...
		NDAS_ASSERT( desiredAccess == 0x0012019F );
		...
		if (status == STATUS_SUCCESS) {
			...
			NDAS_ASSERT( ioStatusBlock.Information == FILE_OPENED );
	...
	)
{
	...
	status = LfsTable_QueryPrimaryAddressList( GlobalLfs.LfsTable,
											   &netdiskPartitionInfo,
											   PrimaryAddressList,
											   NumberOfPrimaryAddress );
	
	if (status == STATUS_SUCCESS) {
	
		NDAS_ASSERT( NumberOfPrimaryAddress > 0 );

		if (IsLocalAddress) {

			*IsLocalAddress = Lfs_IsLocalAddress( &PrimaryAddressList[0] );
		}
	}
	...
	)
{
	...
	if (ModeChange == FALSE) {
		...
		NDAS_ASSERT( netdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumePreMounting ||
	...
	NDAS_ASSERT( status != STATUS_NO_SUCH_DEVICE );

	if (status == STATUS_SUCCESS) {

		if (ModeChange == TRUE) {

			NetdiskManager_ChangeMode( GlobalLfs.NetdiskManager, netdiskPartition, NULL );
		}
	}
	...
	)
{
	...
	status = NetdiskManager_AddWriteRange( GlobalLfs.NetdiskManager, netdiskPartition, BlockAceId );
	...
	)
{
	...
	NetdiskManager_RemoveWriteRange( GlobalLfs.NetdiskManager, netdiskPartition, BlockAceId );
