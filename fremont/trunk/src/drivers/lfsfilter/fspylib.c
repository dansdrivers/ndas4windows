...
--*/
{
    if (gSpyDynamicFunctions.GetVersion != NULL) {
        ...
        PsGetVersion( &gSpyOsMajorVersion, &gSpyOsMinorVersion, NULL, NULL );
    }
}
...
--*/
{
	...
	NDAS_ASSERT( FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTING)						||
				 ...
				 (devExt->LfsDeviceExt.Flags & ~LFS_DEVICE_FLAG_FILTERING) == LFS_DEVICE_FLAG_MOUNTED	||
				 (devExt->LfsDeviceExt.Flags & ~LFS_DEVICE_FLAG_REGISTERED) == LFS_DEVICE_FLAG_MOUNTED );
			
	if (devExt->LfsDeviceExt.FilteringMode != LFS_FILE_CONTROL && 
		((devExt->LfsDeviceExt.Flags & ~LFS_DEVICE_FLAG_FILTERING) == LFS_DEVICE_FLAG_MOUNTED ||
		 (devExt->LfsDeviceExt.Flags & ~LFS_DEVICE_FLAG_FILTERING) == (LFS_DEVICE_FLAG_MOUNTED | LFS_DEVICE_FLAG_SURPRISE_REMOVED))) {
