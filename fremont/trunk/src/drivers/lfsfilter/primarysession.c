	...
	)
{
	...
	if (ntStatus == STATUS_SUCCESS) {
		...
		NDAS_ASSERT( FlagOn(PrimarySession->Flags, PRIMARY_SESSION_FLAG_STOPPING) || 
	...
	)
{
	...
	NDAS_ASSERT( ntStatus == STATUS_SUCCESS );
	...
	)
{
	...
	NDAS_ASSERT( PrimarySessionRequest->ListEntry.Flink == PrimarySessionRequest->ListEntry.Blink );
	...
	)
{
	...
	if (ndfsRequestHeader->Splitted == 0) {
		...
		NDAS_ASSERT( NTOHL(ndfsRequestHeader->MessageSize4) <= PrimarySession->Thread.SessionSlot[Mid].RequestMessageBufferLength );
	...
	NDAS_ASSERT( ndfsRequestHeader->Splitted == 1 );

	//	Allocate memory for extended WinXP header

	NDAS_ASSERT( PrimarySession->Thread.SessionSlot[Mid].ExtendWinxpRequestMessagePool == NULL );
	...
	NDAS_ASSERT( PrimarySession->Thread.SessionSlot[Mid].ExtendWinxpRequestMessagePool );
	...
	)
{
	...
	if (ReplyDataSize <= PrimarySession->SessionContext.SecondaryMaxDataSize) {
		...
		NDAS_ASSERT( HTONL(NdfsReplyHeader->MessageSize4) <= PrimarySession->Thread.SessionSlot[Mid].ReplyMessageBufferLength );
	...
	NDAS_ASSERT( (UINT8 *)NdfsWinxpReplyHeader == PrimarySession->Thread.SessionSlot[Mid].ExtendWinxpReplyMessagePool );
	NDAS_ASSERT( ReplyDataSize > PrimarySession->SessionContext.SecondaryMaxDataSize );
