...
#define DRVO_BASE_FILESYSTEM_DRIVER	0x00000080   // A driver that is at the bottom of the filesystem stack.
	...
	if ((PVOID)NdasNtfsControlDeviceObject == VolumeDeviceObject) {
		...
		if (irpSp->Parameters.FileSystemControl.FsControlCode == FSCTL_NDAS_FS_UNLOAD) {
			...
			DebugTrace( 0, Dbg, ("IoControlCode == FSCTL_NDAS_FS_UNLOAD occured DeviceObject->ReferenceCount = %x\n", 
...

{
	...
	case FSCTL_NDAS_FS_PURGE: {
		...
		DebugTrace(0, Dbg, ("NtfsUserFsRequest... FSCTL_NDAS_FS_FLUSH_OR_PURGE \n") );
	    ...
	    if (NtfsDecodeFileObject( IrpContext, IrpSp->FileObject, &Vcb, &Fcb, &Scb, &Ccb, FALSE ) != UserVolumeOpen) {
			...
			DebugTrace( 0, Dbg, ("FatUserFsCtrl -> %08lx\n", STATUS_INVALID_PARAMETER) );
		...
		SetFlag( IrpContext->NdasNtfsFlags, NDAS_ROFS_IRP_CONTEXT_FLAG_PURGE );
...

{
    ...
    Status = NtfsDeviceIoControl( IrpContext,
		...
		if (NT_SUCCESS(Status)) {
			...
			LengthInfo.Length = PartitionInfo.PartitionLength;
		}
	}
...
#if !__NDAS_ROFS_WIN2K_SUPPORT__
