...

{
    ...
    DebugTrace( 0, DEBUG_TRACE_ERROR, ("NtfsDeleteVcb, *Vcb = %08lx\n", *Vcb) );
    ...
    if ((*Vcb)->LogFileObject == NULL) {
        ...
        if (FlagOn( (*Vcb)->VcbState, VCB_STATE_TEMP_VPB )) {
...
#else
			NDAS_ASSERT( FlagOn((*Vcb)->Vpb->Flags, VPB_PERSISTENT) );
			...
			if ((*Vcb)->Vpb->ReferenceCount == 0) {
				...
				ClearFlag( (*Vcb)->Vpb->Flags, VPB_PERSISTENT );
			}
#endif
            (*Vcb)->Vpb = NULL;
        }
...
#if !__NDAS_ROFS_AVOID_ALYAK_BUG__
        IoDeleteDevice( (PDEVICE_OBJECT)VolDo );
#else
		if (!FlagOn(IrpContext->NdasNtfsFlags, NDAS_ROFS_IRP_CONTEXT_FLAG_DONT_REMOVE_DEVICE)) {
    ...
    DebugTrace( 0, DEBUG_TRACE_ERROR, ("NtfsDeleteVcb -> VOID\n") );
