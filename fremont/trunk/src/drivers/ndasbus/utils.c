...
LONG DbgLevelBusUtil = DBG_LEVEL_BUS_UTIL;

#define NdasDbgCall(l,x,...) do {						\
	if (l <= DbgLevelBusUtil) {							\
...
NCommGetDIBV1 (
	...
	IN	PTA_NDAS_ADDRESS	NodeAddress,
	...
	)
{
	...
	for (cnt = 0; cnt < NdasBACL->BlockACECnt; cnt++) {
		...
		if (bace->StartingOffset != bacle->ui64StartSector*512) {
		...
		if (bace->Length != bacle->ui64StartSector * 512) {
...
NCommVerifyNdasDevWithDIB (
	...
	IN	   PTA_NDAS_ADDRESS			SecondaryAddress
	) 
{
	NTSTATUS			status;
	LSSLOGIN_INFO		loginInfo;
	PNDASBUS_UNITDISK	unit;
	TA_NDAS_ADDRESS		targetAddr;
	TA_NDAS_ADDRESS		bindingAddr;
	TA_NDAS_ADDRESS		boundAddr;
	ULONG				idx_unit;
	ULONG				fault_cnt;
	BOOLEAN				oriDibValid;
	UINT32				crcunit = 0;
	...
	for (idx_unit = 0; idx_unit < AddTargetData->ulNumberOfUnitDiskList; idx_unit++) {
		...
		targetAddr.TAAddressCount = 1;
		targetAddr.Address[0].AddressLength = TDI_ADDRESS_LENGTH_LPX;
		targetAddr.Address[0].AddressType = TDI_ADDRESS_TYPE_LPX;

		RtlCopyMemory( &targetAddr.Address[0].NdasAddress, &unit->Address, TDI_ADDRESS_LENGTH_LPX );

		bindingAddr.TAAddressCount = 1;
		bindingAddr.Address[0].AddressLength = TDI_ADDRESS_LENGTH_LPX;
		bindingAddr.Address[0].AddressType = TDI_ADDRESS_TYPE_LPX;
		
		RtlCopyMemory( &bindingAddr.Address[0].NdasAddress, &unit->NICAddr, TDI_ADDRESS_LENGTH_LPX );
		...
		if (SecondaryAddress) {
			...
			status = LsuQueryBindingAddress( &boundAddr, &targetAddr, &bindingAddr, SecondaryAddress, TRUE );
		...
		if (!NT_SUCCESS(status)) {
			...
			NdasDbgCall( 1, "LsuQueryBindingAddress() failed. STATUS=%08lx\n", status );
		...
		ASSERT( boundAddr.TAAddressCount == 1 );
		ASSERT( boundAddr.Address[0].AddressLength == TDI_ADDRESS_LENGTH_LPX );
		ASSERT( boundAddr.Address[0].AddressType == TDI_ADDRESS_TYPE_LPX );

		RtlCopyMemory( &unit->NICAddr, &boundAddr.Address[0].NdasAddress, TDI_ADDRESS_LENGTH_LPX );

		bindingAddr = boundAddr;
		...
		case NDASSCSI_TYPE_DISK_NORMAL: {
			...
			if (NT_SUCCESS(status)) {
				...
				NdasDbgCall( 1, "NORMAL: DIBV2 doesn't exist. try DIBV1. NTSTATUS:%08lx\n", status );
		...
		case NDASSCSI_TYPE_DISK_RAID1R3: {			
			...
			if (DIBV2.nDiskCount != AddTargetData->ulNumberOfUnitDiskList) {
				...
				NdasDbgCall( 1, "RAID1R3: DIBv2's Diskcount mismatch numberofunitdisklist=%u ndiskcount=%u\n", 
			...
			if (DIBV2.iSequence != idx_unit) {
				...
				NdasDbgCall( 1, "RAID1R3: DIBv2's sequence number mismatch idx_unit=%u Seq=%u\n", idx_unit, DIBV2.iSequence );
			...
			if (oriDibValid == TRUE) {
				...
				if (DIBV2.crc32_unitdisks != crcunit) {
					...
					NdasDbgCall( 1, "RAID1R3: DIBv2's unit crc mismatch. original=%08lx crc unit=%08lx\n", 
				...
				NdasDbgCall( 1, "RAID1R3: Set original CRC. original=%08lx\n", crcunit );
			}
		...
		case NDASSCSI_TYPE_DISK_RAID5:{
			...
			if (!NT_SUCCESS(status)) {
				...
				NdasDbgCall( 1, "RAID0/4: could not read DIBV2. NTSTATUS:%x\n", status );
				unit->LurnOptions |= LURNOPTION_MISSING;
				fault_cnt ++;
			...
			if (DIBV2.iMediaType != NdasDevType2DIBType(AddTargetData->ucTargetType) || DIBV2.iSequence != idx_unit) {

				NdasDbgCall( 1, "RAID0/4: DIBv2's disktype mismatch. MediaType:%x Sequence:%d\n", DIBV2.iMediaType, DIBV2.iSequence );
			...
			if (oriDibValid == TRUE) {
				...
				if (DIBV2.crc32_unitdisks != crcunit) {
					...
					NdasDbgCall( 1, "RAID0/4: DIBv2's unit crc mismatch. original=%08lx crc unit=%08lx\n", 
									 crcunit, DIBV2.crc32_unitdisks );
				...
				Bus_KdPrint_Def(BUS_DBG_SS_ERROR, ("RAID0/4: Set original CRC. original=%08lx\n", crcunit));
			}
	...
	if (AddTargetData->ucTargetType == NDASSCSI_TYPE_DISK_RAID1R3 ||
		...
		if (fault_cnt) {
			...
			NdasDbgCall( 1, ("Member is missing. Denying auto mount.\n") );
		}
