...
#define RESOURCE_OWNER_ENTRY OwnerThreads[0]
#else
#define RESOURCE_OWNER_ENTRY OwnerEntry
#endif
...
{
	if (Lfcb->Sync->Resource.RESOURCE_OWNER_ENTRY.OwnerThread == ExGetCurrentResourceThread()) {
...
{
	if (Lch->Sync->Resource.RESOURCE_OWNER_ENTRY.OwnerThread == ExGetCurrentResourceThread()) {
		ExReleaseResourceLite( &Lch->Sync->Resource );
    }
}

#undef RESOURCE_OWNER_ENTRY
