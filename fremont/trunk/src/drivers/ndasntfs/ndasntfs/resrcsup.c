...
#define Dbg2                             (DEBUG_INFO_RESRCSUP)
#endif
...

{
	...
	} else if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY) {
...
#ifdef NTFSDBG
			    if (FlagOn( Vcb->VcbState, VCB_STATE_VOLUME_MOUNTED )) {
				    if (1 == ExIsResourceAcquiredExclusiveLite( &resource) ) {
					    NtfsChangeResourceOrderState( IrpContext, NtfsResourceExVcb, TRUE, FALSE );
					}
				}
#endif
...
#else
            if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) ? 
				(ExIsResourceAcquiredSharedLite( &Vcb->SecondaryResource ) ==  0) : (ExIsResourceAcquiredSharedLite( &Vcb->Resource ) == 0)) {
#endif
	...
	if (FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY)) {
		...
		secondaryResourceAcquired 
			= SecondaryAcquireResourceSharedLite( NULL, 
												  &volDo->Resource, 
												  (IoGetTopLevelIrp() == NULL) ? Wait : FALSE );
...
#if __NDAS_NTFS_SECONDARY__
		if (AcquiredFile == FALSE && FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY)) {
