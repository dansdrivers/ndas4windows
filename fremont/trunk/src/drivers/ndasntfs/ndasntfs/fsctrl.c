...
#define DRVO_BASE_FILESYSTEM_DRIVER	0x00000080   // A driver that is at the bottom of the filesystem stack.
	...
	if ((PVOID)NdasNtfsControlDeviceObject == VolumeDeviceObject) {
		...
		if (irpSp->Parameters.FileSystemControl.FsControlCode == FSCTL_NDAS_FS_UNLOAD) {

			DebugTrace( 0, Dbg2, ("IoControlCode == FSCTL_NDAS_FS_UNLOAD occured DeviceObject->ReferenceCount = %x\n", 
								  NdasNtfsFileSystemDeviceObject->ReferenceCount) );

			if (!IsListEmpty(&NtfsData.VcbQueue)) {
			...
			if(NtfsData.FileSystemRegistered == TRUE) {

				IoUnregisterFileSystem( NdasNtfsFileSystemDeviceObject );

				NdasNtfsControlDeviceObject->DriverObject->Flags &= ~DRVO_BASE_FILESYSTEM_DRIVER;		
				NtfsData.FileSystemRegistered = FALSE;
			}
			...
			DebugTrace( 0, Dbg, ("IoControlCode != FSCTL_NDAS_FS_UNLOAD\n", 0) );
...

{
	...
	if(IoGetCurrentIrpStackLocation(Irp)->FileObject == NULL) {
		...
		DebugTrace( 0, DEBUG_TRACE_ALL, ("NtfsCommonFileSystemControl: IrpSp->MinorFunction = %x, IrpSp->Parameters.FileSystemControl.FsControlCode = %d\n", 
										 IrpSp->MinorFunction, (IrpSp->Parameters.FileSystemControl.FsControlCode & 0x00003FFC) >> 2) );
	}

	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		
		Status = NdasNtfsSecondaryCommonFileSystemControl( IrpContext, Irp );
		return Status;
	}
...

{
...
#if __NDAS_NTFS_SECONDARY__
			if (!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) &&
				CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY) {
    ...
    try {
		...
		ExInitializeFastMutex( &VolDo->FastMutex );

		VolDo->ReferenceCount = 1;
		SetFlag( VolDo->NdasNtfsFlags, NDAS_NTFS_DEVICE_FLAG_INITIALIZING );
		...
		KeInitializeEvent( &VolDo->LogFileCloseEvent, NotificationEvent, FALSE );

#endif

#if (__NDAS_NTFS_PRIMARY__ || __NDAS_NTFS_SECONDARY__ || (__NDAS_NTFS_PURGE__ || __NDAS_NTFS_FLUSH__))

		DebugTrace2( 0, Dbg, ("NdasNtfsMountVolume: VolDo = %p\n", VolDo) );		

		if (((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL) {
			...
			queryStatus = ((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->
								NdfsCallback.QueryPartitionInformation( Vpb->RealDevice, 
																		&netdiskPartitionInformation );
			if (queryStatus != STATUS_SUCCESS) {

				DebugTrace2( 0, Dbg2, ("NdasNtfsMountVolume %08lx QueryNetdiskInformation \n", queryStatus) );
				ASSERT( NDASNTFS_BUG );
...
#if __NDAS_NTFS_ROUTER_SUPPORT__
				...
				if (netdiskPartitionInformation.NetdiskInformation.DeviceMode == DEVMODE_SHARED_READWRITE) {

					if (!(netdiskPartitionInformation.NetdiskInformation.EnabledFeatures & NDASFEATURE_SECONDARY)) {
						...
						if (!FlagOn(netdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {

							SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT );
						}					
					}
						
				} else if (netdiskPartitionInformation.NetdiskInformation.DeviceMode == DEVMODE_SHARED_READONLY) {
				...
				VolDo->NetdiskPartitionInformation = netdiskPartitionInformation;

				RtlInitEmptyUnicodeString( &VolDo->NetdiskPartitionInformation.VolumeName,
											VolDo->NetdiskPartitionInformation.VolumeNameBuffer,
											sizeof(VolDo->NetdiskPartitionInformation.VolumeNameBuffer) );

				if (RtlAppendUnicodeStringToString( &VolDo->NetdiskPartitionInformation.VolumeName,
													&netdiskPartitionInformation.VolumeName
													) != STATUS_SUCCESS) {

					ASSERT( NDASNTFS_UNEXPECTED );
				}

				NDAS_ASSERT( VolDo->NetdiskPartitionInformation.VolumeName.Length );

				ASSERT( VolDo->NetdiskPartitionInformation.VolumeName.Buffer == VolDo->NetdiskPartitionInformation.VolumeNameBuffer );

				if (FlagOn(VolDo->NetdiskPartitionInformation.NetdiskInformation.SupportedFeatures, NDASFEATURE_SIMULTANEOUS_WRITE)) {

					SetFlag( VolDo->NdasNtfsFlags, NDAS_NTFS_DEVICE_FLAG_DIRECT_RW );
				}

if (IS_WINDOWS2K()) {

				DebugTrace2( 0, Dbg, ("netdiskEnableMode = %d, VolDo->NetdiskPartitionInformation.PartitionInformation.PartitionNumber = %d\n", 
										netdiskEnableMode, VolDo->NetdiskPartitionInformation.PartitionInformation.PartitionNumber) );

} else {

				DebugTrace2( 0, Dbg, ("netdiskEnableMode = %d, VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionNumber = %d\n", 
										netdiskEnableMode, VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionNumber) );
}
				//DbgPrint( "netdiskEnableMode = %d, VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionNumber = %d\n", 
				//						netdiskEnableMode, VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionNumber );

#if __NDAS_NTFS_ROUTER_SUPPORT__

				baclLen = sizeof(NDAS_BLOCK_ACL);

				while (1) {
				
					bacl = FsRtlAllocatePoolWithTag( NonPagedPool, baclLen, NDASNTFS_ALLOC_TAG );
			
					bacl->Length = baclLen;

					queryStatus = ((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->
											NdfsCallback.GetNdasScsiBacl( Vpb->RealDevice, bacl, TRUE );

					NDAS_ASSERT( queryStatus == STATUS_SUCCESS || queryStatus == STATUS_BUFFER_OVERFLOW );

					if (queryStatus == STATUS_BUFFER_OVERFLOW) {

						baclLen = bacl->Length;
						ExFreePoolWithTag( bacl, NDASNTFS_ALLOC_TAG );
						continue;
					}

					if (queryStatus == STATUS_SUCCESS) {

						DebugTrace2( 0, Dbg, ("bacl.BlockACECnt = %d\n", bacl->BlockACECnt) );

						for (baceCount = 0; baceCount < bacl->BlockACECnt; baceCount++) {

if (IS_WINDOWS2K()) {

							DebugTrace2( 0, Dbg2, ("EndOfPartition = %I64d, BlockStartAddr = %I64d, BlockEndAddr = %I64d, AccessMode = %x\n",
													(VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformation.PartitionLength.QuadPart)/512,
													bacl->BlockACEs[baceCount].BlockStartAddr, bacl->BlockACEs[baceCount].BlockEndAddr,
													bacl->BlockACEs[baceCount].AccessMode) );

							if (VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart/512 >= (INT64)bacl->BlockACEs[baceCount].BlockStartAddr &&
								VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart/512 <= (INT64)bacl->BlockACEs[baceCount].BlockEndAddr) {
									
								DebugTrace2( 0, Dbg2, ("StartOfPartition = %I64d, EndOfPartition = %I64d, BlockStartAddr = %I64d, BlockEndAddr = %I64d, AccessMode = %x\n",
														VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart/512,
														(VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformation.PartitionLength.QuadPart)/512,
														bacl->BlockACEs[baceCount].BlockStartAddr, bacl->BlockACEs[baceCount].BlockEndAddr,
														bacl->BlockACEs[baceCount].AccessMode) );
				
								ASSERT( (VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformation.PartitionLength.QuadPart)/512 - 1 >= (INT64)bacl->BlockACEs[baceCount].BlockStartAddr &&
								        (VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformation.PartitionLength.QuadPart)/512 - 1 <= (INT64)bacl->BlockACEs[baceCount].BlockEndAddr );
							
								VolDo->NetdiskEnableMode = netdiskEnableMode = NETDISK_READ_ONLY;
								SetFlag( VolDo->NdasNtfsFlags, ND_FAT_DEVICE_FLAG_LINKSYS_SUPPORT );
							...
							DebugTrace2( 0, Dbg2, ("EndOfPartition = %I64d, BlockStartAddr = %I64d, BlockEndAddr = %I64d, AccessMode = %x\n",
													(VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionLength.QuadPart)/512,
													bacl->BlockACEs[baceCount].BlockStartAddr, bacl->BlockACEs[baceCount].BlockEndAddr,
													bacl->BlockACEs[baceCount].AccessMode) );

							if (VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart/512 >= (INT64)bacl->BlockACEs[baceCount].BlockStartAddr &&
								VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart/512 <= (INT64)bacl->BlockACEs[baceCount].BlockEndAddr) {
									
								DebugTrace2( 0, Dbg2, ("StartOfPartition = %I64d, EndOfPartition = %I64d, BlockStartAddr = %I64d, BlockEndAddr = %I64d, AccessMode = %x\n",
														VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart/512,
														(VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionLength.QuadPart)/512,
														bacl->BlockACEs[baceCount].BlockStartAddr, bacl->BlockACEs[baceCount].BlockEndAddr,
														bacl->BlockACEs[baceCount].AccessMode) );
				
								ASSERT( (VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionLength.QuadPart)/512 - 1 >= (INT64)bacl->BlockACEs[baceCount].BlockStartAddr &&
								        (VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionLength.QuadPart)/512 - 1 <= (INT64)bacl->BlockACEs[baceCount].BlockEndAddr );
        ...
        try {
			...
			if (!FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) &&
	...
	NDAS_ASSERT( !FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) );
	NDAS_ASSERT( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode != NETDISK_SECONDARY );
...
*/
        ...
        if ((CurrentVersion || UpgradeVolume) &&
            ...
            try {
				...
				} else if (!FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) &&
					       !NtfsIsVolumeReadOnly( Vcb )) {
		...
		KeInitializeEvent( &VolDo->ReadyEvent, NotificationEvent, FALSE );
    	KeInitializeEvent( &VolDo->RequestEvent, NotificationEvent, FALSE );

		KeQuerySystemTime( &VolDo->TryFlushOrPurgeTime );

		InitializeObjectAttributes( &objectAttributes, NULL, OBJ_KERNEL_HANDLE, NULL, NULL );

		Status = PsCreateSystemThread( &VolDo->ThreadHandle,
									   THREAD_ALL_ACCESS,
									   &objectAttributes,
									   NULL,
									   NULL,
		...
		Status = ObReferenceObjectByHandle( VolDo->ThreadHandle,
											FILE_READ_DATA,
											NULL,
											KernelMode,
											&VolDo->ThreadObject,
		...
		Status = KeWaitForSingleObject( &VolDo->ReadyEvent,
										Executive,
										KernelMode,
										FALSE,
										&timeOut );
		...
		if (VolDo->NetdiskEnableMode == NETDISK_SECONDARY &&
			...
			for (reconnectionTry = 0; reconnectionTry < MAX_RECONNECTION_TRY; reconnectionTry ++) {
				...
				VolDo->Secondary = Secondary_Create( IrpContext, VolDo );
				ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
				...
				status = ((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->
								NdfsCallback.SecondaryToPrimary( Vcb->Vpb->RealDevice, FALSE );
			...
			if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasNtfsFlags, NDAS_NTFS_DEVICE_FLAG_DIRECT_RW) &&
				!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) { 
				
				if (((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->NdfsCallback.AddWriteRange(Vcb->Vpb->RealDevice, &VolDo->BlockAceId) != STATUS_SUCCESS) {
			...
			while (1) {
				...
				queryStatus = ((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->
										NdfsCallback.GetNdasScsiBacl( Vpb->RealDevice, bacl, FALSE );
				...
				if (queryStatus == STATUS_SUCCESS) {
					...
					DebugTrace( 0, Dbg, ("bacl.BlockACECnt = %d\n", bacl->BlockACECnt) );

					for (baceCount = 0; baceCount < bacl->BlockACECnt; baceCount++) {

						DebugTrace( 0, Dbg2, ("StartOfPartition = %I64d, EndOfPartition = %I64d, StartingOffset = %I64d, Length = %I64d, AccessMode = %x\n",
												VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart,
												VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart + VolDo->NetdiskPartitionInformation.PartitionInformationEx.PartitionLength.QuadPart,
												bacl->BlockACEs[baceCount].StartingOffset, bacl->BlockACEs[baceCount].Length,
												bacl->BlockACEs[baceCount].AccessMode) );
...
#if __NDAS_NTFS__
		ClearFlag( VolDo->NdasNtfsFlags, NDAS_NTFS_DEVICE_FLAG_INITIALIZING );
		SetFlag( VolDo->NdasNtfsFlags, NDAS_NTFS_DEVICE_FLAG_MOUNTED );
#endif
...

{
	...
	case FSCTL_NDAS_FS_PURGE:
	case FSCTL_NDAS_FS_FLUSH: {
		...
		DebugTrace(+1, Dbg2, ("NtfsUserFsRequest... FSCTL_NDAS_FS_FLUSH_OR_PURGE \n") );

	    if (NtfsDecodeFileObject( IrpContext, IrpSp->FileObject, &Vcb, &Fcb, &Scb, &Ccb, FALSE ) != UserVolumeOpen) {

		    NtfsCompleteRequest( IrpContext, Irp, Status = STATUS_INVALID_PARAMETER );

			DebugTrace( -1, Dbg2, ("FatUserFsCtrl -> %08lx\n", STATUS_INVALID_PARAMETER) );
			
			break;
		}

		NDAS_ASSERT( FlagOn(IrpContext->State, IRP_CONTEXT_STATE_WAIT) );
		
		if (FsControlCode == FSCTL_NDAS_FS_PURGE) {
			
			SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_PURGE );		
			(VOID) NtfsFlushVolume( IrpContext, Vcb, FALSE, TRUE, FALSE, FALSE );

		} else {
		
			SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_FLUSH );
...
#if __NDAS_NTFS_PRIMARY__
	{
		...
		volDo = CONTAINING_RECORD( IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb );

		NDAS_ASSERT( volDo->NetdiskEnableMode == NETDISK_PRIMARY			|| 
		...
		for (primarySessionListEntry = volDo->PrimarySessionQueue.Blink; 
			...
			primarySessionListEntry = primarySessionListEntry->Blink;
		...
		if (!IsListEmpty(&volDo->PrimarySessionQueue)) {
			...
			DebugTrace( 0, Dbg2, ("FSCTL_LOCK_VOLUME: Access PrimarySessionQueue is not Empty\n") );
...
#if __NDAS_NTFS_PRIMARY__
	{
		...
		NDAS_ASSERT( volDo->NetdiskEnableMode == NETDISK_PRIMARY || volDo->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY );
		...
		if (!IsListEmpty(&volDo->PrimarySessionQueue)) {
			...
			DebugTrace( 0, Dbg2, ("FSCTL_DISMOUNT_VOLUME: Access PrimarySessionQueue is not Empty\n") );
			NtfsCompleteRequest( IrpContext, Irp, Status = STATUS_ACCESS_DENIED );
...
#if __NDAS_NTFS_DISABLE_SPARSE__
		DebugTrace( 0, Dbg2, ("FSCTL_SET_SPARSE: STATUS_NOT_IMPLEMENTED\n") );
...
#else
		if ((!FlagOn( IrpContext->State, IRP_CONTEXT_STATE_DISMOUNT_LOG_FLUSH )) &&
			 !FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) &&
			 (!NtfsIsVolumeReadOnly( Vcb ))) {
...

{
    ...
    *BootScb = Scb;
	...
	NDAS_ASSERT( Scb->Header.FileSize.QuadPart <= Scb->Header.AllocationSize.QuadPart );
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = RequestedMapSize;
		IrpContext->outputBuffer = MappingPairs;
		return STATUS_SUCCESS;
	}
...

{
    ...
    *CompressionState = 0;
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->outputBuffer = CompressionState;
		return STATUS_SUCCESS;
	}
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = CompressionStatePtr;
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = NULL;
		IrpContext->outputBuffer = VolumeData;
		return STATUS_SUCCESS;
	}
...
--*/
{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = GetBitmap;
		IrpContext->outputBuffer = VolumeBitmap;
		return STATUS_SUCCESS;
	}
    ...
    try {
        ...
        try {
			...
			if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
				...
				IrpContext->outputBuffer = OutputBuffer;
				Status = STATUS_SUCCESS;
				leave;
			}
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = GetFileRecord;
		IrpContext->outputBuffer = FileRecord;
		return STATUS_SUCCESS;
	}
...

{
    ...
    *VolumeState = 0;
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = NULL;
		IrpContext->outputBuffer = VolumeState;
		return STATUS_SUCCESS;
	}
...
--*/
{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = ReparseBuffer;
		IrpContext->outputBuffer = NULL;
		return STATUS_SUCCESS;
	}
...

{
    ...
    try {
		...
		if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
			...
			IrpContext->InputBuffer	 = EnumData;
			IrpContext->outputBuffer = Irp->UserBuffer;
			Status = STATUS_SUCCESS;
			leave;
		}
#endif
...

{
    ...
    try {
		...
		NDAS_ASSERT( BootFileScb->Header.FileSize.QuadPart <= BootFileScb->Header.AllocationSize.QuadPart );
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = HandleInfo;
		IrpContext->outputBuffer = NULL;
		return STATUS_SUCCESS;
	}
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = FilePrefetch;
		IrpContext->outputBuffer = NULL;
		return STATUS_SUCCESS;
	}
...
#else
			if (NdasNtfsMmPrefetchPages) {

				MmStatus = NdasNtfsMmPrefetchPages( 1, &ReadList );
			
			} else {

				NDAS_ASSERT( FALSE );
				MmStatus = STATUS_NOT_SUPPORTED;
			}
#endif
            //
