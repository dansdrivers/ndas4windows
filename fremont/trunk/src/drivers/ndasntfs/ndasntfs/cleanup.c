...
#define Dbg2                             (DEBUG_INFO_CLEANUP)
#endif
...

{
	...
	if(IoGetCurrentIrpStackLocation(Irp)->FileObject == NULL) {
		...
		DebugTrace( 0, Dbg, ("IrpSp->FileObject is NULL, IrpSp->MajorFunction = %x, IrpSp->MinorFunction = %x\n", 
							 IoGetCurrentIrpStackLocation(Irp)->MajorFunction, IoGetCurrentIrpStackLocation(Irp)->MinorFunction) );
	}

	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {

		Status = NdasNtfsSecondaryCommonCleanup( IrpContext, Irp );
	...
	DebugTrace( 0, Dbg, ("FileObject->FileName =%wZ, Scb->CleanupCount = %d\n", &FileObject->FileName, Scb->CleanupCount) );

#if __NDAS_NTFS_DBG__
	if (TypeOfOpen == StreamFileOpen) {

		DebugTrace( 0, Dbg2, ("NtfsCommonCleanup StreamFileOpen\n") );
		ASSERT( FALSE );
	}
#endif

#if __NDAS_NTFS_DIRECT_CLEANUP__

	if (FlagOn(FileObject->Flags, FO_CLEANUP_COMPLETE)) {
		
		if (Ccb->FileOpenPrimarySessionId != NDASNTFS_LOCAL_PRMARY_SESSION_ID) {
		
			NtfsCompleteRequest( IrpContext, Irp, Status = STATUS_SUCCESS );
			DebugTrace( -1, Dbg, ("FO_CLEANUP_COMPLETE NtfsCommonCleanup -> %08lx\n", Status) );
			return Status;
		}
	}
    ...
    try {
                    ...
                    if (!LcbLinkIsDeleted( Lcb ) &&
                        ...
                        if (ImageScb == NULL) {
                            ...
                            if (IsDirectory( &Fcb->Info )) {
								...
								if (NdasNtfsFsRtlNotifyFilterChangeDirectory) {
                                
									NdasNtfsFsRtlNotifyFilterChangeDirectory( Vcb->NotifySync,
												                              &Vcb->DirNotifyList,
													                          FileObject->FsContext,
															                  NULL,
																	          FALSE,
        ...
        NDAS_ASSERT( Fcb->CleanupCount != 0 );
        NDAS_ASSERT( Scb->CleanupCount != 0 );
                    ...
                    if ((Scb->CleanupCount == 1) &&
                        ...
                        //
							...
							if (!IsFileObjectReadOnly(FileObject)	&& 
								...
								Scb->PrimarySessionId != NDASNTFS_LOCAL_PRMARY_SESSION_ID) {

								ULONG		writeAceessCleanupCount = 0;
								PLIST_ENTRY	ccbListEntry;

								NtfsLockFcb( NULL, Scb->Fcb );

								for (ccbListEntry = Scb->CcbQueue.Flink; 
									...
									if (Ccb != ccb && FlagOn(ccb->FileObject->Flags, FO_CLEANUP_COMPLETE)) {
								...
								NtfsUnlockFcb( NULL, Scb->Fcb );

								NDAS_ASSERT( writeAceessCleanupCount );

								if (writeAceessCleanupCount > 1) {

								} else if (IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader == NULL ) {
								
								} else if ((ULONGLONG)Scb->Header.ValidDataLength.QuadPart < 
										  IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength) {
																		
									NDAS_ASSERT( Scb->Header.AllocationSize.QuadPart == 
												 IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.AllocationSize );
								
									Scb->Header.FileSize.QuadPart = IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.FileSize;
									Scb->Header.ValidDataLength.QuadPart = IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength;
						
									DebugTrace( 0, Dbg, ("Scb->Header.ValidDataLength.QuadPart = %I64d\n", Scb->Header.ValidDataLength.QuadPart) );
