...

{
	...
	NDAS_ASSERT( !FlagOn(Scb->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY) );
...

{
	...
	NDAS_ASSERT( IrpContext->Vcb == NULL || CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode != NETDISK_SECONDARY );
...

{
    ...
    try {
		...
		NDAS_ASSERT( IrpContext->Usn.CurrentUsnFcb == NULL && IrpContext->TransactionId == 0 ||
...
#else
	if (NdasNtfsKeAcquireInStackQueuedSpinLock) {

		NdasNtfsKeAcquireInStackQueuedSpinLock( &TablePointer->SpinLock, &LockHandle );
...
#else
		if (NdasNtfsKeReleaseInStackQueuedSpinLock) {

			NdasNtfsKeReleaseInStackQueuedSpinLock( &LockHandle );
