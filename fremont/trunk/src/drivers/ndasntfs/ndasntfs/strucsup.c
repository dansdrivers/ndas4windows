...
#define Dbg2                             (DEBUG_INFO_STRUCSUP)
#endif
...

{
	...
	RtlInitializeGenericTable( &Vcb->SecondaryFcbTable,
                               NtfsFcbTableCompare,
                               NtfsAllocateFcbTableEntry,
                               NtfsFreeFcbTableEntry,
                               NULL );

	ExInitializeResourceLite( &Vcb->SecondaryResource );
...

{
    ...
    if ((*Vcb)->LogFileObject == NULL) {
		...
		if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_PRIMARY_STOPPING)) {

			NdasNtfsPrimarySessionDisconnect( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb) );
			ClearFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_PRIMARY_STOPPING );
		}
		...
		if (VolDo->BlockAceId) {
			...
			((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->NdfsCallback.RemoveWriteRange( (*Vcb)->Vpb->RealDevice, 
		...
		if (VolDo->Secondary)
			Secondary_Close( VolDo->Secondary );

		VolDo->Secondary = NULL;

		ASSERT( (*Vcb)->SecondaryCleanupCount == 0 );
		ASSERT( (*Vcb)->SecondaryCloseCount == 0 );
		ASSERT( (*Vcb)->SecondaryReadOnlyCloseCount == 0 );
		ASSERT( (*Vcb)->SecondarySystemFileCloseCount == 0 );
		...
		ExDeleteResourceLite( &(*Vcb)->SecondaryResource );
		...
		while(primarySessionListEntry = 
				ExInterlockedRemoveHeadList( &VolDo->PrimarySessionQueue,
		...
		if (CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->ThreadHandle != NULL) {
	
			ExAcquireFastMutex( &CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->FastMutex );

			if (FlagOn(CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->Thread.Flags, VOLDO_THREAD_FLAG_START) &&
				!FlagOn(CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->Thread.Flags, VOLDO_THREAD_FLAG_STOPED)) {

				KeSetEvent( &CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->RequestEvent, IO_DISK_INCREMENT, FALSE );
			} 
			...
			status = KeWaitForSingleObject( CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->ThreadObject,
											Executive,
											KernelMode,
											FALSE,
											&timeOut );
			...
			if (status == STATUS_SUCCESS) {
				...
				DebugTrace( 0, Dbg2, ("NtfsDeleteVcb: thread stoped VolDo = %p\n", CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)) );

				ObDereferenceObject( CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->ThreadObject );

				CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->ThreadHandle = NULL;
				CONTAINING_RECORD((*Vcb), VOLUME_DEVICE_OBJECT, Vcb)->ThreadObject = NULL;
		...
		timeOut.QuadPart = -10*NANO100_PER_SEC;		// 10 sec
		status = KeWaitForSingleObject( &VolDo->ReferenceZeroEvent, Executive, KernelMode, FALSE, &timeOut );
...

{
...
#if __NDAS_NTFS_SECONDARY__
	BOOLEAN LocalReturnedExistingFcb = TRUE;
	PRTL_GENERIC_TABLE fcbTable;
#else
	...
	if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) 
		fcbTable = &Vcb->SecondaryFcbTable;
	else
		fcbTable = &Vcb->FcbTable;

	if ((Entry = RtlLookupElementGenericTableFull( fcbTable, &Key, &NodeOrParent, &SearchResult )) != NULL) {
    ...
    if ((Entry = RtlLookupElementGenericTableFull( &Vcb->FcbTable, &Key, &NodeOrParent, &SearchResult )) != NULL) {
        ...
        if (FlagOn( Fcb->FcbState, FCB_STATE_FILE_DELETED )) {
			...
			if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) {
				...
				ASSERT( FALSE );
				ASSERT( FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY) );
				ASSERT( IsListEmpty( &Fcb->DeletedListEntry ) );
				InsertHeadList( &CONTAINING_RECORD(Fcb->Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Secondary->DeletedFcbQueue,
								&Fcb->DeletedListEntry );
			}
#else
        ...
        *ReturnedExistingFcb = FALSE;
        ...
        try {
			...
			if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) {
				...
				SetFlag( Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY );
			}
#endif				
...

{
    ...
    if (FlagOn( (*Fcb)->FcbState, FCB_STATE_IN_FCB_TABLE )) {
		...
		NtfsDeleteFcbTableEntry( (*Fcb), (*Fcb)->Vcb, (*Fcb)->FileReference );
#else
...
#if __NDAS_NTFS_SECONDARY__
		if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) 
			if (FlagOn(Scb->NdasNtfsFlags, ND_NTFS_SCB_FLAG_CORRUPTED)) 
				continue;
#endif
    ...
    try {
        ...
        if (AttributeTypeCode == $INDEX_ALLOCATION) {
            ...
            if ((AttributeTypeCode == $DATA) ||
				...
				if (FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY)) 
					ASSERT( NtfsSegmentNumber( &Fcb->FileReference ) != BIT_MAP_FILE_NUMBER );
				else
...

{
...
#if __NDAS_NTFS_SECONDARY__
	if (FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY))
		ASSERT( (*Scb)->CloseCount == 0 && IsListEmpty(&(*Scb)->CcbQueue) );
#endif
	...
	if (NdasNtfsFsRtlTeardownPerStreamContexts) {

		if (FlagOn( (*Scb)->Header.Flags2, FSRTL_FLAG2_SUPPORTS_FILTER_CONTEXTS )) {

			NdasNtfsFsRtlTeardownPerStreamContexts( (PFSRTL_ADVANCED_FCB_HEADER)&(*Scb)->Header );
		}
	}
...
#if __NDAS_NTFS_SECONDARY__
		if ((!(FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY)) ? (Scb == Lcb->Scb) : (Scb == Lcb->SecondaryScb))
#else
        if ((Scb == Lcb->Scb)
    ...
    *ReturnedExistingLcb = FALSE;
    ...
    try {
		...
		if (!FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY)) {
			...
			InitializeListHead( &Lcb->ScbLinks );
			Lcb->SecondaryScb = Scb;
		}
#else
        InsertTailList( &Scb->ScbType.Index.LcbQueue, &Lcb->ScbLinks );
        Lcb->Scb = Scb;
...

{
...
#if __NDAS_NTFS__
			ASSERT( NodeType(Vcb) == NTFS_NTC_VCB || NdasNtfsFileSystemDeviceObject == IrpSp->DeviceObject );
#else
	...
	if (ARGUMENT_PRESENT(Irp) && IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
...
#if __NDAS_NTFS_SECONDARY__
            if (NtfsIsExclusiveVcb( IrpContext, Fcb->Vcb )) {
#else
...

{
	...
	if (FlagOn(Scb->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY))
		InterlockedIncrement( &Vcb->SecondaryCleanupCount );
	else
...
#if __NDAS_NTFS_SECONDARY__
	if (FlagOn(Scb->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY))
		InterlockedIncrement( &Vcb->SecondaryCloseCount );
	else
...
#if __NDAS_NTFS_SECONDARY__
		if (FlagOn(Scb->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY))
			InterlockedIncrement( &Vcb->SecondaryReadOnlyCloseCount );
		else
...

{
	...
	if (FlagOn(Scb->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY)) {
		...
		InterlockedDecrement( &Vcb->SecondaryCleanupCount );
	...
	NDAS_ASSERT( Vcb->SecondaryCleanupCount >= 0 );
	NDAS_ASSERT( (LONG)Vcb->CleanupCount >= 0 );
...

{
...
#if __NDAS_NTFS_SECONDARY__
	if (FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY))
		InterlockedDecrement( &Vcb->SecondaryCloseCount );
	else
...
#if __NDAS_NTFS_SECONDARY__
		if (FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY))
			InterlockedDecrement( &Vcb->SecondaryReadOnlyCloseCount );
		else
	...
	ASSERT( Vcb->SecondaryCloseCount >= 0 );
	ASSERT( Vcb->CloseCount >= 0 );
	ASSERT( Vcb->SystemFileCloseCount >= 0 );
	ASSERT( Vcb->SecondaryReadOnlyCloseCount >= 0 );
	ASSERT( Vcb->ReadOnlyCloseCount >= 0 );
	ASSERT( Vcb->SecondaryCloseCount >= 0 );
	//if (Scb->CloseCount == 0)
...
#if __NDAS_NTFS_SECONDARY__
	if (!FlagOn(Scb->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY) || Scb->CloseCount == 0 )
#endif		
...
#if __NDAS_NTFS_SECONDARY__
                ASSERT( NtfsIsExclusiveVcb( Lcb->Fcb, Lcb->Fcb->Vcb ));
#else
