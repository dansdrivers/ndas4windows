	...
	)
{
	...
	try {
		...
		SetFlag( Secondary->VolDo->Vcb.NdasNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES );
		NtfsFspClose( &Secondary->VolDo->Vcb );
		ClearFlag( Secondary->VolDo->Vcb.NdasNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES );

		restartKey = NULL;
		deletedFcbListEntry = NULL;

		SetFlag( (*IrpContext)->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_TRY_CLOSE_FILES );
		...
		do {
			...
			if (deletedFcbListEntry == NULL) {
				...
				nextFcb = NdasNtfsGetNextFcbTableEntry( &Secondary->VolDo->Vcb, &restartKey );
			}
			...
			if (fcb->CloseCount == 0) {
				...
				NDAS_ASSERT( fcb->ReferenceCount == 0 );
				...
				NtfsTeardownStructures( (*IrpContext), fcb, NULL, FALSE, 0, &removedFcb );

				NDAS_ASSERT( removedFcb );
		...
		while (deletedFcbListEntry != &Secondary->DeletedFcbQueue) {
			...
			NDAS_ASSERT( &fcb->DeletedListEntry == deletedFcbListEntry );
		...
		ClearFlag( (*IrpContext)->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_TRY_CLOSE_FILES );
	...
	)
{
		...
		DebugTrace( 0, Dbg2, ("Secondary_ChangeLcbFileName: FullPathName =%wZ\n", FullPathName) );
    ...
    )
{
	...
	if (ARGUMENT_PRESENT(IrpContext) && !NtfsIsTopLevelNtfs(IrpContext)) {
		...
		ASSERT( IrpContext->MajorFunction == IRP_MJ_CLOSE && IrpContext->TopLevelIrpContext->MajorFunction == IRP_MJ_CLEANUP		||
				...
				IrpContext->MajorFunction == IRP_MJ_CLOSE && FlagOn(IrpContext->TopLevelIrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_TRY_CLOSE_FILES) ||
				...
				IrpContext->MajorFunction == IRP_MJ_WRITE && FlagOn(IrpContext->TopLevelIrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_TRY_CLOSE_FILES) );
	}
