...
#if 0
	HwDeviceExtension->IdlePointer = PoRegisterDeviceForIdleDetection( HwDeviceExtension->ScsiportFdoObject, 1000, 1000, PowerDeviceD0 ); 

	NDAS_ASSERT( HwDeviceExtension->IdlePointer );
#endif
    ...
    )
{
	...
	if (Srb->QueueTag == SP_UNTAGGED) {
		...
		NDAS_ASSERT( ADAPTER_ISSTATUS(HwDeviceExtension, NDASSCSI_ADAPTER_STATUS_STOPPING) ||
	...
	) 
{
	...
	MiniSendIoctlSrbAsynch( DeviceObject,
							NDASSCSI_IOCTL_NOOP,
							&NoopData,
...
#if 0
		if (hwDeviceExtension->IdlePointer) {

			DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, ("PoSetDeviceBusy\n") );

			PoSetDeviceBusy( hwDeviceExtension->IdlePointer	); 
		}
#endif
