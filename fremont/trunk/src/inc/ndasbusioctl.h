...
typedef struct _NDASBUS_ADD_TARGET_DATA
{
	...
	UINT32			StartOffset;			// 12 bytes (8)

	UINT64			ulTargetBlocks;			// 20 bytes
	NDAS_DEV_ACCESSMODE	DeviceMode;			// 24 bytes

	ULONG			ulNumberOfUnitDiskList; // 28 bytes

	UCHAR			ucTargetType;			// NDASSCSI_TYPE_* in ndasscsiioctl.h
	UCHAR			ucTargetId;

	BOOLEAN			LockImpossible;			// 31
	BOOLEAN			EmergencyMode;
	UCHAR			Reserved[3];			// 34 bytes
	...
	//	UnitDisk list
	//
	NDASBUS_UNITDISK	UnitDiskList[MAX_NR_UNITDISK_FOR_AGGR];

#define BACL_BUFFER_SIZE	1000
	UCHAR				BaclBuffer[BACL_BUFFER_SIZE]; // reprograming need
...
typedef struct _NDASBUS_PLUGIN_HARDWARE_EX2
{
	...
	union {
		...
		IN	VOID*POINTER_32	DisconEvent32;
		IN	HANDLE			DisconEvent;
		IN	ULONGLONG		Buffer0;
	};
	...
	union {
		...
		IN  VOID*POINTER_32 AlarmEvent32;
		IN  HANDLE			AlarmEvent;
		IN	ULONGLONG		Buffer1;
	};
	...
	IN NDASBUS_ADD_TARGET_DATA	NdasBusAddTargetData;
...
C_ASSERT( FIELD_OFFSET(NDASBUS_PLUGIN_HARDWARE_EX2, DisconEvent32) == FIELD_OFFSET(NDASBUS_PLUGIN_HARDWARE_EX2, DisconEvent) );
C_ASSERT( FIELD_OFFSET(NDASBUS_PLUGIN_HARDWARE_EX2, AlarmEvent32) == FIELD_OFFSET(NDASBUS_PLUGIN_HARDWARE_EX2, AlarmEvent) );
