...
LONG DbgLevelNdasMagm = DBG_LEVEL_NDAS_MAGM;

#define NdasUiDbgCall(l,x,...) do {						\
	if (l <= DbgLevelNdasMagm) {						\
	...
	int nCmdShow)
{
	...
	SystemInfo.wProcessorArchitecture = 1;

	GetSystemInfo(&SystemInfo);

	NdasUiDbgCall( 1, "SystemInfo.wProcessorArchitecture = %d\n", SystemInfo.wProcessorArchitecture );
	...
	COMVERIFY(hr = ::CoInitialize(NULL));
	...
	COMVERIFY( hr = _Module.Init(NULL, hInstance) );
...
int Run (LPTSTR lpstrCmdLine, int nCmdShow)
{
	...
	if (!sapp.Initialize(NDASMGMT_INST_UID)) {
		...
		NdasUiDbgCall( 1, "sapp.Initialize(NDASMGMT_INST_UID) = %d\n", sapp.Initialize(NDASMGMT_INST_UID) );
	...
	if (!sapp.IsFirstInstance()) {
		...
		NdasUiDbgCall( 1, "sapp.IsFirstInstance() = %d\n", sapp.IsFirstInstance() );
		...
		if (cmdinfo.Exit || cmdinfo.Restart) {
			...
			if (appmsg.PostMessage(INTERAPPMSG_EXIT, 0) && sapp.WaitOtherInstances(10000)) {
	...
	//GetModuleFileName(NULL, moduleFileName, RTL_NUMBER_OF(moduleFileName));

	//HINSTANCE resourceModuleHandle = LoadMUILibrary(moduleFileName, MUI_LANGUAGE_NAME, 0);

	//if (NULL != resourceModuleHandle)
	//{
	//	FreeMUILibrary(resourceModuleHandle);
	//}
	...
	// Privilege Check
	//
#define VISTA_UAC_SUPPORT
#ifndef VISTA_UAC_SUPPORT
