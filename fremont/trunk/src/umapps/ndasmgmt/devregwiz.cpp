...
LONG DbgLevelMagmRegWiz = DBG_LEVEL_MAGM_REG_WIZ;

#define NdasUiDbgCall(l,x,...) do {							\
    if (l <= DbgLevelMagmRegWiz) {							\
...
CRegWizard::CRegWizard(HWND hWndParent, PTCHAR NdasStringId) :
	m_fCentered(FALSE),
	...
	CPropertySheetImpl<CRegWizard>(IDS_DRZ_TITLE, 0, hWndParent)
{
	...
	::ZeroMemory( &m_wizData, sizeof(REG_WIZARD_DATA) );

	if (NdasStringId) {

		memcpy( m_wizData.szDeviceId, NdasStringId, sizeof(m_wizData.szDeviceId) );
	}
...
CRegWizIntroPage::OnInitDialog(HWND hWnd, LPARAM lParam)
{
	...
	CString strDeviceId(m_pWizData->szDeviceId);

	NdasUiDbgCall( 2, "strDeviceId.GetLength() = %d\n", strDeviceId.GetLength() );

	m_DisableAutoPnp.Attach( GetDlgItem(IDC_NDAS_PNP) );
	
	if (strDeviceId.GetLength() == 0) {

		m_DisableAutoPnp.ShowWindow(SW_HIDE);
	}
...
CRegWizIntroPage::OnSetActive()
{
	...
	if (strDeviceId.GetLength() == 0) {
		...
		//m_DisableAutoPnp.EnableWindow(FALSE);
		//m_DisableAutoPnp.ShowWindow(SW_HIDE);
		...
		BOOL fSuccess = ::NdasGetServiceParam( NDASSVC_PARAM_AUTO_PNP, &serviceParam );
		...
		if (serviceParam.Value.DwordValue == NDASSVC_AUTO_PNP_ALLOW) {

			m_DisableAutoPnp.SetCheck(BST_UNCHECKED);
	
		} else if (serviceParam.Value.DwordValue == NDASSVC_AUTO_PNP_DENY) {

			m_DisableAutoPnp.SetCheck(BST_CHECKED);
...
VOID 
CRegWizIntroPage::OnNoWriteKeyClick(UINT, int, HWND)
{
	int check = m_DisableAutoPnp.GetCheck();

	NdasUiDbgCall( 2, "check = %d\n", check );

	if (check == 1) {

		NDAS_SERVICE_PARAM serviceParam = {0};

		serviceParam.ParamCode		  = NDASSVC_PARAM_AUTO_PNP;
		serviceParam.Value.DwordValue = NDASSVC_AUTO_PNP_DENY;
		...
		serviceParam.ParamCode		  = NDASSVC_PARAM_AUTO_PNP;
		serviceParam.Value.DwordValue = NDASSVC_AUTO_PNP_ALLOW;
...
CRegWizDeviceIdPage::OnInitDialog(HWND hWnd, LPARAM lParam)
{
	...
	m_NdasStringId[0].Attach(GetDlgItem(IDC_DEV_ID_1));
	m_NdasStringId[1].Attach(GetDlgItem(IDC_DEV_ID_2));
	m_NdasStringId[2].Attach(GetDlgItem(IDC_DEV_ID_3));
	m_NdasStringId[3].Attach(GetDlgItem(IDC_DEV_ID_4));
	m_DevKey.Attach(GetDlgItem(IDC_DEV_KEY));

	//
	// chaining paste support
	//
	m_wndPasteChains[0].Attach(m_hWnd,m_NdasStringId[0], m_NdasStringId[1]);
	m_wndPasteChains[1].Attach(m_hWnd,m_NdasStringId[1] ,m_NdasStringId[2]);
	m_wndPasteChains[2].Attach(m_hWnd,m_NdasStringId[2], m_NdasStringId[3]);
	m_wndPasteChains[3].Attach(m_hWnd,m_NdasStringId[3], m_DevKey);
	m_wndPasteChains[4].Attach(m_hWnd,m_DevKey,NULL);

	m_NdasStringId[0].SetLimitText(5);
	m_NdasStringId[1].SetLimitText(5);
	m_NdasStringId[2].SetLimitText(5);
	m_NdasStringId[3].SetLimitText(5);
...
CRegWizDeviceIdPage::OnSetActive()
{
	...
	if (strDeviceId.GetLength()) {
		...
		NdasUiDbgCall( 2, _T("%s\n"), m_pWizData->szDeviceId );
		NdasUiDbgCall( 2, _T("%s, %d\n"), strDeviceId, strDeviceId.GetLength() );

		INT	stringIdLen = strDeviceId.GetLength();

		for (INT i=0; i<NDAS_DEVICE_STRING_ID_PARTS; i++) {

			INT copyLen = stringIdLen < NDAS_DEVICE_STRING_ID_PART_LEN ? stringIdLen : NDAS_DEVICE_STRING_ID_PART_LEN;

			m_NdasStringId[i].SetWindowText( strDeviceId.Mid(i*NDAS_DEVICE_STRING_ID_PART_LEN, copyLen) );
...
{
	NdasUiDbgCall( 3, "enter\n" );

	::ZeroMemory( m_szDevId, sizeof(TCHAR) * (NDAS_DEVICE_STRING_ID_LEN + 1) );

	for (INT i=0; i<NDAS_DEVICE_STRING_ID_PARTS; i++) {

		m_NdasStringId[i].GetWindowText( &m_szDevId[i * NDAS_DEVICE_STRING_ID_PART_LEN], 
										 m_NdasStringId[i].GetWindowTextLength()+1 );
	}
	
	NdasUiDbgCall( 3, _T("%s\n"), m_szDevId );
}
...
//	wndCurEdit.SetWindowText(s);
	...
	if (m_NdasStringId[0].GetWindowTextLength() == 5 &&
		m_NdasStringId[1].GetWindowTextLength() == 5 &&
		m_NdasStringId[2].GetWindowTextLength() == 5 &&
		m_NdasStringId[3].GetWindowTextLength() == 5 &&
		...
		if (m_DevKey.GetWindowTextLength() != 0) {
			...
			NDAS_EXT_KEY ExtKey;
			NDAS_EXT_DATA ExtData = { 0xCD, NDAS_VID_WINDWOS_RO, 0xFF, 0xFF };			

    		BOOL fSuccess = ::NdasValidateStringIdKeyExW( m_szDevId, 
														  NULL,
														  NULL,
														  &ExtData );
			...
			if (fSuccess) {
				...
				m_DevKey.SetReadOnly(TRUE);
				bEnableNext = TRUE;
				goto out;
			}
		}

		BOOL fSuccess = ::NdasValidateStringIdKeyW( m_szDevId, lpWriteKey);
		...
		if (fSuccess) {
			...
			Edit_ShowBalloonTip(wndCurEdit, &tip);
		}
...
out:
	...
	CString strDeviceId(m_szDevId);

	NdasUiDbgCall( 2, _T("%s\n"), m_szDevId );
	NdasUiDbgCall( 2, _T("%s, %d\n"), strDeviceId, strDeviceId.GetLength() );

	NdasUiDbgCall( 2, "%d\n", m_NdasStringId[0].GetWindowTextLength() );
	NdasUiDbgCall( 2, "%d\n", m_NdasStringId[1].GetWindowTextLength() );

	INT	stringIdLen = strDeviceId.GetLength();

	for (INT i=0; i<4; i++) {

		INT copyLen = stringIdLen < 5 ? stringIdLen : 5;
		...
		if (stringIdLen == 0) {
			...
			if (i<3) {
				...
				if (copyLen < 5) {
					...
					m_NdasStringId[i].SetFocus();
				
				} else {

					m_NdasStringId[i+1].SetFocus();
				}
...
CRegWizDeviceMountPage::PrepareMount()
{
	...
	while (NDAS_DEVICE_STATUS_ONLINE != pDevice->GetStatus() &&
