...
LONG DbgLevelMagmAbout = DBG_LEVEL_MAGM_ABOUT;

#define NdasUiDbgCall(l,x,...) do {						\
	if (l <= DbgLevelMagmAbout) {						\
...
CAboutDialog::OnInitDialog(HWND hWnd, LPARAM lParam)
{
	...
	CStatic wndCopyRight;
	TCHAR szPCopyRight[256] = {0};

	wndCopyRight.Attach( GetDlgItem(IDC_COPYWRITE) );
	wndCopyRight.GetWindowText( szPCopyRight, 256 );

	//if (memcmp(szPCopyRight, _T("Copyright (c) 2002-2008 XIMETA, Inc.\r\nAll rights reserved."), 30) == 0) {

		// NdasUiDbgCall( 1, "overliding copyRight\n" );
		// wndCopyRight.SetWindowText( _T("Copyright (c) 2002-2008 XIMETA, Inc.\r\nAll rights reserved.\r\n") );

		CString strHyperLinkCopyWrite;
		strHyperLinkCopyWrite.SetString(_T("http://www.ximeta.com"));

		m_wndHyperLink_CopyWrite.SubclassWindow(GetDlgItem(IDC_LINK_COPYWRITE));
		m_wndHyperLink_CopyWrite.SetHyperLink(strHyperLinkCopyWrite);
	//}
