...
LONG DbgLevelSvcDevMan = DBG_LEVEL_SVC_DEVMAN;

#define NdasUiDbgCall(l,x,...) do {							\
    if (l <= DbgLevelSvcDevMan) {							\
...
CNdasLogicalUnitManager::Register(INdasUnit* pNdasUnit, INdasLogicalUnit** ppNdasLogicalUnit)
{
	...
	NdasUiDbgCall( 2, "Registering NdasUnit=%p to LDM\n", pNdasUnit );
	...
	*ppNdasLogicalUnit = NULL;
	...
	{
		NdasUiDbgCall( 2, "itr == m_LogicalUnitDefinitionMap.end()\n" );
	...
	else 
	{
		...
		NdasUiDbgCall( 2, "pNdasLogicalUnit = itr->second\n" );
	}
