	...
	__in BSTR RegistrySubPath)
{
	...
	NdasUiDbgCall( 2, "in LogicalUnitId = %d\n", LogicalUnitId );
	...
	NdasUiDbgCall( 2, "m_NdasLogicalUnitId = %d, lastMountAccess = %x, fRiskyMountFlag = %x\n", 
					  m_NdasLogicalUnitId, lastMountAccess, fRiskyMountFlag );
...
CNdasLogicalUnit::AddNdasUnitInstance(INdasUnit* pNdasUnit)
{
	...
	{	
		...
		NdasUiDbgCall( 2, "m_NdasLogicalUnitId = %d, activeLogicalUnit = %d, m_fMountOnReady = %d\n", 
						   m_NdasLogicalUnitId, activeLogicalUnit, m_fMountOnReady );
		...
		if (!activeLogicalUnit && m_fMountOnReady) 
		{
			...
			NdasUiDbgCall( 2, "m_NdasLogicalUnitId = %d, allowedAccess = %d, m_mountOnReadyAccess = %d\n", 
							   m_NdasLogicalUnitId, allowedAccess, m_mountOnReadyAccess );
...
CNdasLogicalUnit::pPlugInNdasBus(
	...
	DWORD LdpfValues)
{
	NdasUiDbgCall( 2, "in m_NdasLogicalUnitId = %d\n", m_NdasLogicalUnitId );
	...
	if (FAILED(hr)) {
		...
		NdasUiDbgCall( 2, "pIsSafeToPlugIn failed hr = %x in m_NdasLogicalUnitId = %d\n", hr, m_NdasLogicalUnitId );
	...
	if (FAILED(hr)) {
		...
		NdasUiDbgCall( 2, "pGetPrimaryNdasUnit failed hr = %x in m_NdasLogicalUnitId = %d\n", hr, m_NdasLogicalUnitId );
	...
	if (FAILED(hr)) {
		...
		NdasUiDbgCall( 2, "get_ParentNdasDevice failed hr = %x in m_NdasLogicalUnitId = %d\n", hr, m_NdasLogicalUnitId );
	...
	ZeroMemory( static_cast<PNDASBUS_ADD_TARGET_DATA>(addTargetData), addTargetDataSize );
	...
	SetLurOptionsFromMappings( addTargetData->LurOptions, 
							   LdpfFlags, 
							   LdpfValues );
	...
	DWORD PlugInFlags)
{
	...
	if(FAILED(hr))
	{
		NdasUiDbgCall( 2, "pCheckRaidStatus failed %x m_NdasLogicalUnitId = %d\n", hr, m_NdasLogicalUnitId );
		return hr;
	}
...
CNdasLogicalUnit::OnDisconnected()
{
	...
	NdasUiDbgCall( 1, "LogicalUnit=%d, Disconnect Event.\n", m_NdasLogicalUnitId );
...
CNdasLogicalUnit::OnDismounted()
{
	...
	if (m_fDisconnected) {
		...
		NdasUiDbgCall( 2, "LogicalUnit=%d, Unmount Completed (by disconnection).\n", m_NdasLogicalUnitId );
	
	} else {

		NdasUiDbgCall( 2, "LogicalUnit=%d, Unmount Completed.\n", m_NdasLogicalUnitId );
	}
