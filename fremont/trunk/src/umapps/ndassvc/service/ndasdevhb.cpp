...
LONG DbgLevelSvcHb = DBG_LEVEL_SVC_HB;
...
CNdasDeviceHeartbeatListener::ThreadStart(HANDLE hStopEvent)
{
	...
	if (!success) {
		...
		NdasUiDbgCall( 1, "CLpxDatagramServer init failed, error=0x%X\n", GetLastError() );
		return ret = 254;
	}

	success = dgramServer.Receive( this, m_ListenPort, sizeof(HEARTBEAT_RAW_DATA), hStopEvent );

	if (!success) {

		NdasUiDbgCall( 2, "port %d failed, error=0x%X\n", m_ListenPort, GetLastError() );
		return ret = 255;
	}
