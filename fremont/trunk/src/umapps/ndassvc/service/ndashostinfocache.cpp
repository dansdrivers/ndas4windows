	...
	CONST BYTE* pData)
{
	...
	case NHIX_HIC_NETBIOSNAME:
		if (_T('\0') == pHostInfo->szHostname[0]) {
			HRESULT hr = ::StringCchPrintf(
				...
				_T("%S"),
				(LPTSTR)pData);
			XTLASSERT(SUCCEEDED(hr) || STRSAFE_E_INSUFFICIENT_BUFFER == hr);
		}
		return;
	case NHIX_HIC_FQDN: 
	...
	case NHIX_HIC_UNICODE_HOSTNAME:
		if (_T('\0') == pHostInfo->szHostname[0]) {
			DWORD cbToCopy = min(cbData,48 * sizeof(WCHAR));
	...
	case NHIX_HIC_ADDR_LPX: 
		if (0 == pHostInfo->LPXAddrs.AddressCount) {
			...
			pHostInfo->LPXAddrs.AddressCount = static_cast<UCHAR>(i);
		}
		return;
	case NHIX_HIC_ADDR_IPV4:
		if (0 == pHostInfo->IPV4Addrs.AddressCount) {
			...
			pHostInfo->IPV4Addrs.AddressCount = static_cast<UCHAR>(i);
		}
		return;
	case NHIX_HIC_ADDR_IPV6:
		if (0 == pHostInfo->LPXAddrs.AddressCount) {
			...
			pHostInfo->IPV6Addrs.AddressCount = static_cast<UCHAR>(i);
		}
		return;
...
BOOL CDataStringizer::AppendF(LPCTSTR szFormat, ...)
{
	...
	BOOL success = VAppendF(szFormat, ap);
	va_end(ap);
	return success;
}
...
CNdasHostInfoCache::Update(DWORD dwTimeout)
{
	...
	BOOL success = query.Initialize();
	if (!success) {
		return FALSE;
	}

	success = query.BroadcastQuery(dwTimeout);
	if (!success) {
		return FALSE;
	}
...
{
	XTLASSERT(!IsBadWritePtr(lpBoundAddr,sizeof(SOCKADDR_LPX)));
	XTLASSERT(!IsBadWritePtr(lpRemoteAddr,sizeof(SOCKADDR_LPX)));
