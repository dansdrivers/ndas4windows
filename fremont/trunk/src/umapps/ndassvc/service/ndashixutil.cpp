	...
	DWORD cbBuffer, PBYTE lpbBuffer)
{
	...
	BOOL success = ::GetComputerNameExA(
		NameType,
		(CHAR*) lpbBuffer,
		&cchSize);
...
DWORD pGetOSVerInfo(DWORD cbBuffer, LPBYTE lpbBuffer)
{
	...
	BOOL success = ::GetVersionEx((LPOSVERSIONINFO)&osvi);
	XTLASSERT(success);
...
{
	XTLASSERT( sizeof(pEntry->Node) == sizeof(UnitDeviceId.DeviceId.Node) );

	::CopyMemory( UnitDeviceId.DeviceId.Node, pEntry->Node, sizeof(UnitDeviceId.DeviceId.Node) );
	...
	NDAS_HIX::UNITDEVICE_ENTRY_DATA* pEntry)
{
	...
	::CopyMemory( pEntry->Node, UnitDeviceId.DeviceId.Node, sizeof(UnitDeviceId.DeviceId.Node) );
