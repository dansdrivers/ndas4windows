...
LONG DbgLevelSvcDev = DBG_LEVEL_SVC_DEV;
	...
	)
{
	...
	if (NDAS_VID_WINDWOS_RO == NdasIdExtension.Vid) {

		return NDAS_OEM_CODE_WINDOWS_RO;
	}

	if (NDAS_VID_AUTO_REGISTRY == NdasIdExtension.Vid) {

		return NDAS_OEM_CODE_AUTO_REGISTRY;
	}
	...
	__in_opt const NDASID_EXT_DATA* NdasIdExtension)
{
	...
	ATLASSERT(DeviceId.Reserved == 0 );

	m_Status = NDAS_DEVICE_STATUS_NOT_REGISTERED;
	...
	m_LocalSocketAddress.iSockaddrLength = 0;
	m_LocalSocketAddress.lpSockaddr = 0;
	m_LocalSocketAddressString[0] = 0;

	m_RemoteSocketAddress.iSockaddrLength = 0;
	m_RemoteSocketAddress.lpSockaddr = 0;
	m_RemoteSocketAddressString[0] = 0;
	...
	__in_opt const CInterfaceArray<INdasUnit>* NewNdasUnits)
{
	...
	case NDAS_DEVICE_STATUS_ONLINE:
		...
		m_DiscoverErrors = 0;
		break;
	case NDAS_DEVICE_STATUS_NOT_REGISTERED:
		m_DiscoverErrors = 0;
		break;
	}
	...
	if (NDAS_DEVICE_STATUS_NOT_REGISTERED == oldStatus && 
		NDAS_DEVICE_STATUS_OFFLINE == newStatus)
	{
		...
		NDAS_DEVICE_STATUS_NOT_REGISTERED == newStatus)
	{
...
STDMETHODIMP CNdasDevice::put_Enabled(__in BOOL Enabled)
{
	...
	if (Enabled) 
	{
		...
		//
		if (NDAS_DEVICE_STATUS_NOT_REGISTERED != m_Status) 
		{
			return S_FALSE;
		} 
	...
	else 
	{
		...
		//
		pChangeStatus(NDAS_DEVICE_STATUS_NOT_REGISTERED);
	}
...
CNdasDevice::NdasHeartbeatReceived(const NDAS_DEVICE_HEARTBEAT_DATA* Data)
{
	...
	if (memcmp(&Data->NdasDeviceId.Node, &m_NdasDeviceId.Node, sizeof(m_NdasDeviceId.Node))) {

		return;
	}

	NdasUiDbgCall( 4, "deviceId=%s, m_Status = %x\n", CNdasDeviceId(m_NdasDeviceId).ToStringA(), m_Status );
	...
	if (m_Status == NDAS_DEVICE_STATUS_ONLINE) {

		if (m_LocalSocketAddress.iSockaddrLength != Data->LocalAddress.iSockaddrLength ||
			!RtlEqualMemory(m_LocalSocketAddress.lpSockaddr,
						    Data->LocalAddress.lpSockaddr,
	...
	if (!IsSupportedHardwareVersion(Data->Type, Data->Version)) {
	...
	if (NDAS_DEVICE_STATUS_ONLINE == m_Status || NDAS_DEVICE_STATUS_CONNECTING == m_Status) {
	...
	if (m_DiscoverErrors) {
		...
		NdasUiDbgCall( 1, "deviceId=%s, m_Status = %x\n", CNdasDeviceId(m_NdasDeviceId).ToStringA(), m_Status );
	}
	...
	if (m_DiscoverErrors >= fatalErrorThreshold) {
		...
		NdasUiDbgCall( 1, "set NDAS_DEVICE_ERROR_DISCOVER_TOO_MANY_FAILURE deviceId=%s\n", 
						  CNdasDeviceId(m_NdasDeviceId).ToStringA() );

		pSetLastDeviceError(NDAS_DEVICE_ERROR_DISCOVER_TOO_MANY_FAILURE);
		return;
	}

	DWORD localAddressStringLength = RTL_NUMBER_OF(m_LocalSocketAddressString);
	DWORD remoteAddressStringLength = RTL_NUMBER_OF(m_RemoteSocketAddressString);

	int ret = WSAAddressToString( Data->LocalAddress.lpSockaddr, 
								  Data->LocalAddress.iSockaddrLength,
								  NULL,
								  m_LocalSocketAddressString, 
	...
	ret = WSAAddressToString( Data->RemoteAddress.lpSockaddr,
							  Data->RemoteAddress.iSockaddrLength,
							  NULL,
	...
	XTLTRACE2( NDASSVC_NDASDEVICE, TRACE_LEVEL_VERBOSE,
			   "NdasDevice=%p, Id=%s, Remote=%ls, Local=%ls.\n",
			   this,
			   CNdasDeviceId(Data->NdasDeviceId).ToStringA(),
			   m_RemoteSocketAddressString, 
			   m_LocalSocketAddressString );

	// Save the local and remote address

	m_LocalSocketAddress.iSockaddrLength = Data->LocalAddress.iSockaddrLength;
	m_LocalSocketAddress.lpSockaddr = &m_LocalSocketAddressBuffer.Address;

	_ASSERT( Data->LocalAddress.iSockaddrLength <= RTL_NUMBER_OF(m_LocalSocketAddressBuffer.RawAddress) );

	CopyMemory( m_LocalSocketAddressBuffer.RawAddress,
			    Data->LocalAddress.lpSockaddr,
			    Data->LocalAddress.iSockaddrLength );

	m_RemoteSocketAddress.iSockaddrLength = Data->RemoteAddress.iSockaddrLength;
	m_RemoteSocketAddress.lpSockaddr = &m_RemoteSocketAddressBuffer.Address;

	_ASSERT( Data->RemoteAddress.iSockaddrLength <= RTL_NUMBER_OF(m_RemoteSocketAddressBuffer.RawAddress) );

	CopyMemory( m_RemoteSocketAddressBuffer.RawAddress,
			    Data->RemoteAddress.lpSockaddr,
				Data->RemoteAddress.iSockaddrLength );
...
CNdasDevice::pDiscoverThreadStart()
{
	...
	if (FAILED(hr)) {
		...
		NDAS_DEVICE_ERROR ndasDeviceError;

		// TODO: EVENTLOG NDAS_DEVICE_ERROR_DISCOVER_FAILED
		
		CAutoInstanceLock autoInstanceLock(this);

		get_DeviceError(&ndasDeviceError);

		if (ndasDeviceError == NDAS_DEVICE_ERROR_GET_SOKADDR_LIST_FAILED) {
	...
	BOOL changed = pChangeStatus( NDAS_DEVICE_STATUS_ONLINE, &ndasUnits );
	...
	)
{
	...
	if (FAILED(hr)) {
		...
		pSetLastDeviceError(NDAS_DEVICE_ERROR_GET_SOKADDR_LIST_FAILED);
		NdasUiDbgCall( 1, "pGetLocalSocketAddressList failed, hr=0x%X\n", hr );
		return hr;
	}
	...
	if ((HNDAS)hNdas == NULL) {
		...
		NdasUiDbgCall( 1, "Discovery failed, hr=0x%X\n", hr );
	...
	if (!success)  {
		...
		NdasUiDbgCall( 1, "NdasCommGetDeviceHardwareInfo failed, hr=0x%X\n", hr );
	...
	NdasUiDbgCall( 2, "NdasCommGetDeviceHardwareInfo success hr=0x%X\n", hr );
	...
	__out NDAS_DEVICE_STAT& DeviceStat)
{
	...
	if (!success) {
		...
		XTLTRACE2( NDASSVC_NDASDEVICE, TRACE_LEVEL_ERROR, "NdasCommGetDeviceStat failed, hr=0x%X\n", hr);
	...
	*ppNdasUnit = 0;
	...
	for (DWORD i = 0; i < failureThreshold + 1; ++i) {
		...
		if (FAILED(hr)) {
			...
			NdasUiDbgCall( 1, "pUpdateStats failed, hr=0x%X.\n", hr );
			return hr;
		}
		...
		NdasUiDbgCall( 1, "Creating UnitNo %d unit device instance failed (%d out of %d), hr=0x%X\n", 
						  UnitNo, i, failureThreshold, hr );
	}
