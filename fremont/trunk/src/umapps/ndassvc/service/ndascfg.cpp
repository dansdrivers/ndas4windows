...
static const struct {
	...
	_T("UnitDeviceIdentifyFailureRetryInterval"), 2500,   10,      300000,	// nscUnitDeviceIdentifyRetryGap
	_T("CmdPipeMaxInstances"),						 2,    1,		   10,  // nscCommandPipeInstancesMax
	...
	_T("AutoPnp"),				NDASSVC_AUTO_PNP_ALLOW,    0, (DWORD)(-1),	// nscAutoPnp
...
{
	BOOL success = SetEntropy((LPBYTE)&SystemCfgGuid, sizeof(GUID));
	_ASSERT(success);
}
...
//	BOOL success = SetEntropy((LPBYTE)&ServiceCfgGuid, sizeof(GUID));
//	_ASSERT(success);
//}
	...
	NDASSVC_CONFIG_DWORD_TYPE Type)
{
	...
	if (success && value >= NSCONFIG_DWORD_DEF[i].Min &&
