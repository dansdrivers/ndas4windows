...
int ndascmd_mount(int argc, TCHAR** argv)
{
	...
	success = NdasPlugInLogicalDevice(rw_mode, TRUE, ld_id);
	if (!success)
	{
		return print_last_error_message(), EXIT_FAILURE;
	}
...
{
#define NVOL_PREFIX _T("\\\\?\\")

	static LPCTSTR USAGE = _T("<volume-path> (e.g. C: or \\\\?\\C:)");
	...
	i = get_unnamed_arg_pos(argc, argv, 0);
	if (-1 == i) return pusage(USAGE);

	path = argv[i];

	if (0 == _tcsncicmp(path, NVOL_PREFIX, RTL_NUMBER_OF(NVOL_PREFIX) - 1))
	{
		StringCchCopy(npath, RTL_NUMBER_OF(npath), path);
	}
	else
	{
		StringCchPrintf(npath, RTL_NUMBER_OF(npath), NVOL_PREFIX _T("%s"), path);
	}

	h = CreateFile(npath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
		NULL, OPEN_EXISTING, 0, NULL);

	if (INVALID_HANDLE_VALUE == h)
	{
		_tprintf(_T("CreateFile(%s) failed.\n"), npath);
		print_last_error_message();
		return EXIT_FAILURE;
	}
	...
	if (!success)
	{
		...
		_tprintf(_T("DeviceIoControl failed.\n"));
		print_error_message(e);
		CloseHandle(h);
		return EXIT_FAILURE;
	}

	_tprintf(_T("Volume %s purged successfully.\n"), npath);
