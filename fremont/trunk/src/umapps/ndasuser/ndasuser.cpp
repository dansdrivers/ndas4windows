...
//////////////////////////////////////////////////////////////////////////
//
// Validate String Id and Key with Extend Key and Data
//
//////////////////////////////////////////////////////////////////////////
...
BOOL 
NDASUSERAPI
NdasValidateStringIdKeyExW(
	LPCWSTR lpszDeviceStringId, 
	LPCWSTR lpszDeviceStringKey,
	NDAS_EXT_KEY* ExtKey,
	NDAS_EXT_DATA* ExtData)
{
	//
	// pointer validation check - ALWAYS DO THIS FOR API CALLS
	// lpszDeviceStringKey can be NULL
	...
	BOOL fSuccess = NdasIdValidateExW(lpszDeviceStringId, lpszDeviceStringKey, (NDASID_EXT_KEY *)ExtKey, (NDASID_EXT_DATA *)ExtData);
	...
	NDAS_EXT_DATA* ExtData)
{
	...
	BOOL fSuccess = NdasIdValidateExA(lpszDeviceStringId, lpszDeviceStringKey, (NDASID_EXT_KEY *)ExtKey, (NDASID_EXT_DATA *)ExtData);
	...
	DWORD dwFlags)
{
	...
	ZeroMemory( lpRequest, sizeof(*lpRequest) );
	...
	{
		XTLTRACE1(TRACE_LEVEL_ERROR,
			"Service Connection failed, error=0x%X\n",
			GetLastError());
		::SetLastError(NDASUSER_ERROR_SERVICE_CONNECTION_FAILURE); 
		return 0; 
	} 
	...
	IN CONST NDAS_DEVICE_REGISTRATIONW* Registration)
{
	...
	NDAS_CMD_REGISTER_DEVICE_V2::REQUEST buffer = {0}; 
	...
	NDAS_LOGICALDEVICE_ID logicalDeviceId)
{
	...
	param.PlugInFlags = 
		bIgnoreWarning ? NDAS_LOGICALUNIT_PLUGIN_FLAGS_IGNORE_WARNINGS : NULL;
	...
	PNDAS_LOGICALDEVICE_EJECT_PARAMW EjectParam)
{
	...
	{
		NDAS_LOCATION NdasLocation = {0};

		BOOL fSuccess = NdasQueryNdasLocation(
		...
		HRESULT hr = NdasPortCtlCreateControlDevice(GENERIC_READ, &ndasportHandle);
		...
		if (SUCCEEDED(hr))
		{
			...
			hr = NdasPortCtlGetPortNumber(ndasportHandle, &portNumber);

			if (FAILED(hr))
			{
				XTLVERIFY( CloseHandle(ndasportHandle) );
				SetLastError(hr);
				return FALSE;
			}

			NdasLocation |= portNumber;
...
// Disable deprecated use warning here as we are using NDAS_SCSI_LOCATION.
	...
	PNDAS_SCSI_LOCATION pNdasScsiLocation)
{
	...
	ZeroMemory(pNdasScsiLocation, sizeof(NDAS_SCSI_LOCATION));
	return NdasQueryNdasLocation(
		LogicalDeviceId, 
		reinterpret_cast<PNDAS_LOCATION>(&pNdasScsiLocation->SlotNo));
}
...
BOOL
NDASUSERAPI
NdasQueryNdasLocation(
	NDAS_LOGICALDEVICE_ID LogicalDeviceId,
	PNDAS_LOCATION pNdasLocation)
{
	PASSERT_WRITE_PTR(pNdasLocation, sizeof(NDAS_LOCATION));
	struct Command : NdasCmd<NDAS_CMD_QUERY_NDAS_LOCATION> {
		NDAS_LOCATION& NdasLocation;
		Command(NDAS_LOGICALDEVICE_ID logDeviceId, NDAS_LOCATION& NdasLocation) : 
			NdasLocation(NdasLocation) {
			NDAS_CMD_QUERY_NDAS_LOCATION::REQUEST* pRequest = m_pRequest;
			pRequest->LogicalDeviceId = logDeviceId;
		}
		BOOL OnSuccess() {
			NDAS_CMD_QUERY_NDAS_LOCATION::REPLY* pReply = m_pReply;
			NdasLocation = pReply->NdasLocation;
			return TRUE;
		}
	};
	return Command(LogicalDeviceId, *pNdasLocation).Process();
}

NDASUSER_LINKAGE
BOOL
NDASUSERAPI
NdasQueryNdasLogicalDeviceId(
	__in NDAS_LOCATION NdasLocation,
	__out PNDAS_LOGICALDEVICE_ID LogicalDeviceId)
{
	PASSERT_WRITE_PTR(LogicalDeviceId, sizeof(NDAS_LOGICALDEVICE_ID));
	struct Command : NdasCmd<NDAS_CMD_QUERY_NDAS_LOGICALDEVICE_ID> {
		NDAS_LOGICALDEVICE_ID& NdasLogicalDeviceId;
		Command(__in NDAS_LOCATION NdasLocation, __out NDAS_LOGICALDEVICE_ID& logDeviceId) : 
			NdasLogicalDeviceId(logDeviceId) {
			NDAS_CMD_QUERY_NDAS_LOGICALDEVICE_ID::REQUEST* pRequest = m_pRequest;
			pRequest->NdasLocation = NdasLocation;
		}
		BOOL OnSuccess() {
			NDAS_CMD_QUERY_NDAS_LOGICALDEVICE_ID::REPLY* pReply = m_pReply;
			NdasLogicalDeviceId = pReply->LogicalDeviceId;
			return TRUE;
		}
	};
	return Command(NdasLocation, *LogicalDeviceId).Process();
}
	...
	PNDASUSER_LOGICALDEVICE_INFORMATIONW pLogDevInfo)
{
	...
	struct Command : NdasCmd<NDAS_CMD_QUERY_LOGICALDEVICE_INFORMATION_V2>
	{
		...
		BOOL OnSuccess()
		{
			...
			if (IS_NDAS_LOGICALDEVICE_TYPE_DISK(pLogDevInfo->LogicalDeviceType)) 
			{
