...
LONG DbgLevelComm = DBG_LEVEL_COMM;
...
typedef struct _NDAS_HANDLE_LIST_ENTRY {

	struct _NDAS_HANDLE_LIST_ENTRY	*Next;
	HNDAS							NdasHandle;

} NDAS_HANDLE_LIST_ENTRY, *PNDAS_HANDLE_LIST_ENTRY;
...
CCriticalSection		NdasHandleListLock;
NDAS_HANDLE_LIST_ENTRY	NdasHandleListHead = {0};

void NdasCommpAddHandle(HNDAS NdasHandle);
void NdasCommpRemoveHandle(HNDAS NdasHandle);

void NdasCommpAddHandle(HNDAS NdasHandle)
{
	NdasHandleListLock.Enter();
	PNDAS_HANDLE_LIST_ENTRY head = &NdasHandleListHead;
	PNDAS_HANDLE_LIST_ENTRY entry = new NDAS_HANDLE_LIST_ENTRY;
	entry->NdasHandle = NdasHandle;
	entry->Next = head->Next;
	head->Next = entry;
	NdasHandleListLock.Leave();
}
...
void NdasCommpRemoveHandle(HNDAS NdasHandle)
{
	NdasHandleListLock.Enter();
	PNDAS_HANDLE_LIST_ENTRY head = &NdasHandleListHead;
	for (PNDAS_HANDLE_LIST_ENTRY entry = head;
		entry->Next != NULL; entry = entry->Next)
	{
		if (entry->Next->NdasHandle == NdasHandle)
		{
			PNDAS_HANDLE_LIST_ENTRY removal = entry->Next;
	...
	)
{
	...
	if (NdasConnInfo->AddressType == NDASCOMM_CIT_UNSPECIFIED) {
	...
	if (InterlockedCompareExchange(&NdasCommInitCount, 0, 0) < 1) {
		...
		XTLTRACE1( TRACE_LEVEL_ERROR, "NDASCOMM_ERROR_NOT_INITIALIZED!\n" );
	...
	if (!NdasCommpIsValidNdasConnectionInfo(NdasConnInfo)) {
		...
		XTLTRACE1( TRACE_LEVEL_ERROR, "Invalid Binding Socket Address List, error=0x%X\n", GetLastError() );
		return NULL;
	}
	...
	BOOL success = NdasCommpConnectionInfoToDeviceID( NdasConnInfo, &unitDeviceID, &ndasIdExtension );

	if (success == FALSE) {

		XTLTRACE1( TRACE_LEVEL_ERROR, "DeviceID is invalid, error=0x%X\n", GetLastError() );
		return NULL;
	}
	...
	if (NULL == context) {
		...
		XTLTRACE1(TRACE_LEVEL_ERROR, "Handle.CreateContext failed, error=0x%X\n", GetLastError() );
		return NULL;
	}
	...
	const SOCKET_ADDRESS_LIST* localSocketAddressList = 
									reinterpret_cast<const SOCKET_ADDRESS_LIST*>(NdasConnInfo->BindingSocketAddressList);
	...
	HRESULT hr = NdasCommTransportConnect( &context->s,
										   LPXPROTO_STREAM,
										   &deviceSocketAddress,
	...
	if (FAILED(hr)) {
		...
		XTLTRACE1( TRACE_LEVEL_ERROR, "NdasCommpConnect failed, hr=0x%X\n", hr );
	...
	// If the caller requests connection only, return here.
	
	if (NdasConnInfo->Flags & NDASCOMM_CNF_CONNECT_ONLY) {
	
		return context;
	}

	context->LspLoginInfo.login_type = 
		(NDASCOMM_LOGIN_TYPE_NORMAL == NdasConnInfo->LoginType ) ? LSP_LOGIN_TYPE_NORMAL : LSP_LOGIN_TYPE_DISCOVER;

	if (NdasConnInfo->OEMCode.UI64Value) {

		CopyMemory( context->LspLoginInfo.password,
					NdasConnInfo->OEMCode.Bytes,
					sizeof(context->LspLoginInfo.password) );
	
	} else {

		UINT64 pwd = NdasCommpGetPassword( unitDeviceID.DeviceId.Node, &ndasIdExtension );
		
		CopyMemory( context->LspLoginInfo.password, &pwd, sizeof(context->LspLoginInfo.password) );
	}

	//CopyMemory( context->LspLoginInfo.password, LSP_LOGIN_PASSWORD_ANY, sizeof(LSP_LOGIN_PASSWORD_ANY) );
	...
	if (NdasConnInfo->Flags & NDASCOMM_CNF_ENABLE_DEFAULT_PRIVILEGED_MODE) {
		...
		CopyMemory( context->LspLoginInfo.supervisor_password,
	...
	context->State = NDASCOMM_STATE_LOGGED_IN;

	context->LspHardwareData = lsp_get_hardware_data(context->LspHandle);

	if (context->LspHardwareData->data_encryption_algorithm) {

		size_t buflen = context->LspHardwareData->maximum_transfer_blocks * NDASCOMM_BLOCK_LENGTH;
	...
	if (LSP_LOGIN_TYPE_NORMAL == context->LspLoginInfo.login_type && !*(UINT64*)&context->LspLoginInfo.supervisor_password) {
...
{
	const lsp_uint8_t NullPassword[8] = {0};

	if (NDASCOMM_STATE_LOGGED_IN == context->State)
	{
		BOOL superuser = (0 != memcmp(
			context->LspLoginInfo.supervisor_password,
			NullPassword,
			sizeof(NullPassword)));

		//
		// When a supervisor login is in effect,
		// logout cannot be performed.
		//
	...
	IN UINT32 uiReadDataLen)
{
	...
	BYTE param_bytes[12] = {0};
	UINT64 *param_8 = (UINT64 *)param_bytes; // 8 bytes if HwVersion <= 2
	lsp_uint8_t param_length = sizeof(param_bytes);
	BYTE ahs_data_buf[12];
	lsp_extended_command_t ahs_request = {0};
	lsp_extended_command_t ahs_response = {0};
	lsp_extended_command_t *ahs_request_ptr = NULL;
	lsp_extended_command_t *ahs_response_ptr = NULL;
	int i;
	...
	if (NdasHandle->LspHardwareData->hardware_version == LSP_HARDWARE_VERSION_2_0 &&
		NdasHandle->LspHardwareData->hardware_revision == 0)
	{
		switch (vop_code)
		{
		case ndascomm_vcmd_set_eep:
		case ndascomm_vcmd_get_eep:
			SetLastError(NDASCOMM_ERROR_HARDWARE_UNSUPPORTED);
			return FALSE;
		}
	}
	...
	*param_8 = 0;
	...
	case ndascomm_vcmd_set_lpx_address:
		*param_8 = 
		...
		break;
	case ndascomm_vcmd_set_eep:
		*param_8 = (UINT64)param->GETSET_EEPROM.Address/16;
		// Put data in reverse order.

		// Parameter2 field after Paramter.
		param_bytes[8] = param->GETSET_EEPROM.Data[15];
		param_bytes[9] = param->GETSET_EEPROM.Data[14];
		param_bytes[10] = param->GETSET_EEPROM.Data[13];
		param_bytes[11] = param->GETSET_EEPROM.Data[12];
		// Additional header segment
		for(i=0;i<12;i++) {
			ahs_data_buf[i] = param->GETSET_EEPROM.Data[11-i];
		}
	
		ahs_request.cmd_buffer = ahs_data_buf;
		ahs_request.cmd_size = 12;
		ahs_request_ptr = &ahs_request;
		break;
	case ndascomm_vcmd_get_eep:
		*param_8 = (UINT64)param->GETSET_EEPROM.Address/16;
		ahs_response.cmd_buffer = ahs_data_buf;
		ahs_response.cmd_size = 12;
		ahs_response_ptr = &ahs_response;
		break;
	case ndascomm_vcmd_set_dynamic_max_conn_time:
	case ndascomm_vcmd_set_d_enc_opt:
	case ndascomm_vcmd_get_d_enc_opt:
	...
	case ndascomm_vcmd_get_eep:
		...
		// From Parameter2
		param->GETSET_EEPROM.Data[15] = param_bytes[8];
		param->GETSET_EEPROM.Data[14] = param_bytes[9];
		param->GETSET_EEPROM.Data[13] = param_bytes[10];
		param->GETSET_EEPROM.Data[12] = param_bytes[11];
		// From additional header segment
		for(i=0;i<12;i++) {
			param->GETSET_EEPROM.Data[11-i] = ahs_data_buf[i];
		}		
		break;
	case ndascomm_vcmd_set_dynamic_max_conn_time:
	case ndascomm_vcmd_set_d_enc_opt:
	...
	)
{
	...
	HRESULT hr = NdasCommiTextCommand( NdasHandle, 
	...
	for (int i = 0; i < NDAS_MAX_UNITDEVICES; ++i) {
		...
		NdasUiDbgCall( 1, "%2x, i = %d, target_list.PerTarget[i].NRRWHost = %d\n", 
						   pConnectionInfo->Address.DeviceId.Node[5], i, target_list.PerTarget[i].NRRWHost );
		NdasUiDbgCall( 1, "%2x, i = %d, target_list.PerTarget[i].NRROHost = %d\n", 
						   pConnectionInfo->Address.DeviceId.Node[5], i, target_list.PerTarget[i].NRROHost );

		if (i >= target_list.NRTarget) {

			pUnitDeviceStat->IsPresent = FALSE;

			pUnitDeviceStat->RwConnectionCount = 0;
			pUnitDeviceStat->RoConnectionCount = 0; 
	
			pUnitDeviceStat->RwHostCount = 0;
			pUnitDeviceStat->RoHostCount = 0;

		} else {

			pUnitDeviceStat->IsPresent = TRUE;

			pUnitDeviceStat->RwConnectionCount = target_list.PerTarget[i].NRRWHost;

			// chip bug : Read only host count is invalid at V 2.0 rev.0

			if (hardwareVersion == LSP_HARDWARE_VERSION_2_0 && hardwareRevision == LSP_HARDWARE_V20_REV_0) {

				pUnitDeviceStat->RoConnectionCount = NDAS_HOST_COUNT_UNKNOWN;
			
			} else {

				pUnitDeviceStat->RoConnectionCount = target_list.PerTarget[i].NRROHost;
			}

			pUnitDeviceStat->RwHostCount = pUnitDeviceStat->RwConnectionCount;
			pUnitDeviceStat->RoHostCount = pUnitDeviceStat->RoConnectionCount;

			if (pUnitDeviceStat->RwHostCount) {

				XTLASSERT( pUnitDeviceStat->RwHostCount > 0 );

				if (pUnitDeviceStat->RoHostCount > 0) {
		...
		CopyMemory( pUnitDeviceStat->TargetData, &target_list.PerTarget[i].TargetData0, sizeof(UINT32) );
		CopyMemory( &pUnitDeviceStat->TargetData[0] + sizeof(UINT32), &target_list.PerTarget[i].TargetData1, sizeof(UINT32) );
	}
	...
	)
{
	...
	if (NdasHandle->LspLoginInfo.unit_no >= target_list.NRTarget) {
		...
		pUnitDeviceStat->RwConnectionCount = target_list.PerTarget[NdasHandle->LspLoginInfo.unit_no].NRRWHost;
		...
		if (hardwareVersion == LSP_HARDWARE_VERSION_2_0 && hardwareRevision == LSP_HARDWARE_V20_REV_0) {
			...
			pUnitDeviceStat->RoConnectionCount = target_list.PerTarget[NdasHandle->LspLoginInfo.unit_no].NRROHost;
		}
	...
	CopyMemory( pUnitDeviceStat->TargetData, 
				&target_list.PerTarget[NdasHandle->LspLoginInfo.unit_no].TargetData0,
				sizeof(UINT32) );

	CopyMemory( &pUnitDeviceStat->TargetData[0] + sizeof(UINT32),
...
{
	ASSERT_PARAM( (NdasHandle && NdasCommpIsValidNdasHandle(NdasHandle)) || 
				  (ConnectionInfo && NdasCommpIsValidNdasConnectionInfo(ConnectionInfo)) );

	ASSERT_PARAM( NdasHandle != NULL || ConnectionInfo != NULL );

	ASSERT_PARAM( DeviceId == NULL || IsValidWritePtr(DeviceId, RTL_FIELD_SIZE(NDAS_DEVICE_ID, Node)) );

	ASSERT_PARAM( UnitNo == NULL || IsValidWritePtr(UnitNo, sizeof(BYTE)) );
	
	ASSERT_PARAM( Vid == NULL || IsValidWritePtr(Vid, sizeof(BYTE)) );

	if (NdasHandle) {

		if (DeviceId) {

			CopyMemory( DeviceId, NdasHandle->NdasDeviceId.Node, sizeof(NdasHandle->NdasDeviceId.Node) );
		}

		if (UnitNo) {

			*UnitNo = NdasHandle->LspLoginInfo.unit_no;
		}
		...
		BOOL fSuccess = NdasCommpConnectionInfoToDeviceID( ConnectionInfo, &unitDeviceID, &ndasIdExtension );

		if (DeviceId) {

			CopyMemory( DeviceId, unitDeviceID.DeviceId.Node, sizeof(unitDeviceID.DeviceId.Node) );
		}

		if (UnitNo) {

			*UnitNo = (BYTE)unitDeviceID.UnitNo;
		}

		if (Vid) {
			
			*Vid = ndasIdExtension.Vid;
		}
	}
	...
	__in CONST NDASID_EXT_DATA	*NdasIdExtension)
{
	...
	if (NdasIdExtension && NDAS_VID_WINDWOS_RO == NdasIdExtension->Vid) {

		return NDAS_OEM_CODE_WINDOWS_RO.UI64Value;
	}

	if (NdasIdExtension && NDAS_VID_AUTO_REGISTRY == NdasIdExtension->Vid) {

		return NDAS_OEM_CODE_AUTO_REGISTRY.UI64Value;
	}

	// password
	// if it's sample's address, use its password

	if (pAddress[0] == 0x00 && pAddress[1] == 0xf0 && pAddress[2] == 0x0f) {
