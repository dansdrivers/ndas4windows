...
C_ASSERT( sizeof(TA_NDAS_ADDRESS) == sizeof(TA_LSTRANS_ADDRESS) );
...
#define LSTRANS_POOLTAG_TIMEOUT		'otTL'
...
//
typedef struct _LS_TRANS_STAT {
    ULONG Retransmits;
    ULONG PacketLoss;
} LS_TRANS_STAT, *PLS_TRANS_STAT;
...
typedef struct _LSTRANS_TIMEOUT_TIMER {
...
} LSTRANS_TIMEOUT_TIMER, *PLSTRANS_TIMEOUT_TIMER;
...
typedef
VOID
(*PLSTRANS_IOCOMPLETE_ROUTINE)(	IN PLSTRANS_OVERLAPPED	OverlappedData);

typedef struct _LSTRANS_OVERLAPPED {

	PVOID						SystemUse;			// Do not use.
													// Reserved for protocol TDI interface
	PLSTRANS_TIMEOUT_TIMER		TimeoutTimer;		// Reserved for TDI.
	PLSTRANS_IOCOMPLETE_ROUTINE	IoCompleteRoutine;
	PVOID						UserContext;
	IO_STATUS_BLOCK				IoStatusBlock;
	PKEVENT						CompletionEvent;

} LSTRANS_OVERLAPPED, *PLSTRANS_OVERLAPPED;
...
typedef NTSTATUS (*LSTRANS_QUERY_INFORMATION_FUNC)(
		IN	PFILE_OBJECT		ConnectionFileObject,
		IN  ULONG				QueryType,
		IN  PVOID				Buffer,
		IN  ULONG				BufferLen
...
typedef NTSTATUS (*LSTRANS_SET_INFORMATION_FUNC)(
		IN	PFILE_OBJECT		ConnectionFileObject,
		IN  ULONG				SetType,
		IN  PVOID				Buffer,
		IN  ULONG				BufferLen
...
typedef struct _LSTRANS_FUNC {
	...
	LSTRANS_QUERY_INFORMATION_FUNC			LstransQueryInformation;
	LSTRANS_SET_INFORMATION_FUNC			LstransSetInformation;
	PVOID									Reserved[15];
