			...
			if (!FlagOn(VolumeDeviceObject->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) &&
				...
				if (IrpContext->MinorFunction == IRP_MN_QUERY_REMOVE_DEVICE	|| 
					...
					for (;;) {
						...
						SetFlag( IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_RESOURCE_ACQUIRED );
					...
					if (Status == STATUS_SUCCESS) {
						...
						if (VolumeDeviceObject->NetdiskEnableMode != NETDISK_SECONDARY) {
							...
							ClearFlag( IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_RESOURCE_ACQUIRED );
...

{
	...
	if (IrpSp->MinorFunction == IRP_MN_QUERY_REMOVE_DEVICE) {
		...
		NdasFatPrimarySessionStopping( OurDeviceObject );
		...
		NdasFatPrimarySessionSurpriseRemoval( OurDeviceObject );
    ...
    if (OurDeviceObject->DeviceObject.Size != sizeof(VOLUME_DEVICE_OBJECT) ||
		...
		if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_PRIMARY_STOPPING)) {
			...
			NdasFatPrimarySessionCancelStopping( OurDeviceObject );
		...
		if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_RESOURCE_ACQUIRED)) {

			if (!FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_VCB_DELETED)) {

				NDAS_ASSERT( ExIsResourceAcquiredSharedLite(&OurDeviceObject->Resource) );
				SecondaryReleaseResourceLite( NULL, &OurDeviceObject->Resource );
			}
        ...
        default:
			...
			if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_RESOURCE_ACQUIRED)) {
				...
				if (!FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_VCB_DELETED)) {
					...
					NDAS_ASSERT( ExIsResourceAcquiredSharedLite(&CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Resource) );
					SecondaryReleaseResourceLite( NULL, &CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Resource );
				}
...

{
    ...
    try {
		...
		if (volDo->Secondary) {
			...
			NDAS_ASSERT( volDo->NetdiskEnableMode == NETDISK_SECONDARY || volDo->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY );
...
#if __NDAS_FAT_CALL_CLOSE_ON_PNP__
			...
			if (Vcb->SecondaryOpenFileCount) {
				...
				NDAS_ASSERT( ccbCount == Vcb->SecondaryOpenFileCount );
		...
		if (NT_SUCCESS(Status)) {
			...
			NDAS_ASSERT( Vcb->SecondaryOpenFileCount == 0 );

			if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->BlockAceId) {

				((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->
					NdfsCallback.RemoveWriteRange( Vcb->Vpb->RealDevice, 
		...
		if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_PRIMARY_STOPPING)) {
			...
			if (NT_SUCCESS(Status)) {
				...
				NdasFatPrimarySessionCancelStopping( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb) );
			}
