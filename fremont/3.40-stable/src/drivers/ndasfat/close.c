...

{
    ...
    try {
		...
		if (Ccb) {
			...
			NDAS_ASSERT( FlagOn(FileObject->Flags, FO_CLEANUP_COMPLETE) );
			...
			SetFlag( Ccb->NdasFatFlags, ND_FAT_CCB_FLAG_DELAY_CLOSE );
...

{
    ...
    while (CloseContext = FatRemoveClose(Vcb, LastVcb)) {
        ...
        try {
			...
			DebugTrace2( 0, Dbg, ("FatFspClose CloseContext = %p CloseContext->TypeOfOpen = %d, CloseContext->Fcb = %p\n", 
								   CloseContext, CloseContext->TypeOfOpen, CloseContext->Fcb) );
...

{
    ...
    try {
        ...
        case UserVolumeOpen:
			...
			if (Ccb->FileOpenPrimarySessionId != NDASFAT_LOCAL_PRMARY_SESSION_ID) {
