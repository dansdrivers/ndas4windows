		...
		if (eventStatus == STATUS_TIMEOUT) {
			...
			if ((VolDo->NetdiskEnableMode == NETDISK_READ_ONLY && 
				...
				do {
					...
					primaryRequestInfo.PrimarySessionId		  = NDASFAT_LOCAL_PRMARY_SESSION_ID;
	...
	)
{
    ...
    //  When this disk is half full, the multiplier is 3,
    //  and when it is 3/4 full, the multiplier is only 1.
    //
	...
	if (ClusterAlignedFileSize == 0) {
		...
		if (FileSize) {
			...
			ClusterAlignedFileSize = 0xffffffff;
		}
	}
