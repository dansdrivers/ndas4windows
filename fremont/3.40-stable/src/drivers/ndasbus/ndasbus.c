...
LONG DbgLevelNdasBus = DBG_LEVEL_NDAS_BUS;

#define NdasDbgCall(l,x,...) do {						\
	if (l <= DbgLevelNdasBus) {							\
...
LurTranslateUnitdiskToLurnNodeDesc(
	...
	case LURN_OPTIOCAL_CARD:
		...
		NodeDesc->LurnIde.BindingAddress.TAAddressCount = 1;
		NodeDesc->LurnIde.BindingAddress.Address[0].AddressLength = TDI_ADDRESS_LENGTH_LPX;
		NodeDesc->LurnIde.BindingAddress.Address[0].AddressType = TDI_ADDRESS_TYPE_LPX;

		RtlCopyMemory( &NodeDesc->LurnIde.BindingAddress.Address[0].NdasAddress,
					   &UnitDiskPointer->NICAddr,
					   TDI_ADDRESS_LENGTH_LPX );

		NodeDesc->LurnIde.TargetAddress.TAAddressCount = 1;
		NodeDesc->LurnIde.TargetAddress.Address[0].AddressLength = TDI_ADDRESS_LENGTH_LPX;
		NodeDesc->LurnIde.TargetAddress.Address[0].AddressType = TDI_ADDRESS_TYPE_LPX;
		
		RtlCopyMemory( &NodeDesc->LurnIde.TargetAddress.Address[0].NdasAddress,
					   &UnitDiskPointer->Address,
					   TDI_ADDRESS_LENGTH_LPX );
...
LurTranslateAddTargetDataToLURDesc(
	...
	LurDesc->StartOffset = AddTargetData->StartOffset;
	LurDesc->LockImpossible = AddTargetData->LockImpossible;
	LurDesc->EmergencyMode = AddTargetData->EmergencyMode;
	...
	case NDASSCSI_TYPE_DISK_MIRROR: {
		...
		LurTranslateUnitdiskToLurnNodeDesc(	cur_lurn,
									...
									(UINT16)((cur_location - (PUINT8)LurDesc) + length),
									0,
									LURN_MIRRORING,
	...
	case NDASSCSI_TYPE_DISK_AGGREGATION:
		...
		for(idx_childlurn = 0; idx_childlurn < ChildLurnCnt; idx_childlurn++ ) {
			...
			//DbgPrint( "cur_unitdisk->ulUnitBlocks = %d\n", cur_unitdisk->ulUnitBlocks );
...
//	}
	if(psrbIoctl->ReturnCode == SRB_STATUS_DATA_OVERRUN) {
...
LSBus_OpenLanscsiAdapter (
	...
	RtlZeroMemory( LanscsiAdapter, sizeof(PDO_LANSCSI_DEVICE_DATA) );
	...
	KeInitializeEvent( &LanscsiAdapter->AddTargetEvent, NotificationEvent, FALSE );
...
#else
	LanscsiAdapter->AddDevInfoLength = 0;
#endif
	return STATUS_SUCCESS;
}
	...
	*OutBufferLenNeeded = OutBufferLength;
	...
	case INFORMATION_PDO: {
		...
		if (PdoData == NULL) {
			...
			NDAS_ASSERT(FALSE);
			NdasDbgCall( 1, "No PDO for SlotNo %d!\n", Query->SlotNo );
		...
		if ((Query->Flags & NDASBUS_QUERYFLAG_EVENTQUEUE) == 0) {
			...
			NdasDbgCall( 2, "LastAdapterStatus:PDO:%p Status:%08lx DevMod:%08lx SupFeat:%08lx EnFeat:%08lx\n",
							 PdoData->Self, PdoData->LanscsiAdapterPDO.LastAdapterStatus,
							 PdoData->LanscsiAdapterPDO.DeviceMode, PdoData->LanscsiAdapterPDO.SupportedFeatures,
			...
			if (ndasPdoStatus) {
				...
				NdasDbgCall( 1, "No more status item for SlotNo %d!\n", Query->SlotNo );

				status = STATUS_NO_MORE_ENTRIES;
				break;
			}

			NdasDbgCall( 2, "QueuedAdapterStatus:PDO:%p Status:%08lx DevMod:%08lx SupFeat:%08lx EnFeat:%08lx\n",
							 PdoData->Self, adapterStatus, PdoData->LanscsiAdapterPDO.DeviceMode,
							 PdoData->LanscsiAdapterPDO.SupportedFeatures, PdoData->LanscsiAdapterPDO.EnabledFeatures );
	...
	case INFORMATION_PDOENUM: {
		...
		NdasDbgCall( 2, "waiting for AddTargetEvent\n" );

		TimeOut.QuadPart = -(10LL * 1000 * 1000 * 240); // 240 seconds

		status = KeWaitForSingleObject( &PdoData->LanscsiAdapterPDO.AddTargetEvent,
										Executive,
										KernelMode,
										FALSE,
										&TimeOut );
		...
		NdasDbgCall( 2, "Completed to wait for AddTargetEvent.\n" );
		...
		lurDescLength = GetLurDescLenFromAddTargetData( &PdoData->LanscsiAdapterPDO.AddDevInfo,
		...
		*OutBufferLenNeeded = FIELD_OFFSET(NDASBUS_INFORMATION, PdoEnumInfo) +
							  FIELD_OFFSET(NDASBUS_INFORMATION_PDOENUM, AddDevInfo) + lurDescLength;
		...
		if (OutBufferLength < *OutBufferLenNeeded) {
			...
			status = LurTranslateAddTargetDataToLURDesc( &PdoData->LanscsiAdapterPDO.AddDevInfo,
			...
			RtlCopyMemory( Information->PdoEnumInfo.AddDevInfo, lurDesc, lurDescLength );
			...
			status = DrGetDeviceProperty( PdoData->Self,
			...
			if (NT_SUCCESS(status)) {
				...
				NdasDbgCall( 1, "deviceInstallState = %d\n", deviceInstallState );

				if (deviceInstallState != InstallStateInstalled) {

					NdasDbgCall( 1, "First time installation. Do not enumerate a device.\n" );
	...
	case INFORMATION_PDOEVENT: {
		...
		if (Query->Flags & NDASBUS_QUERYFLAG_USERHANDLE) {
			...
			if (Request32 == FALSE) {
				...
				status = ObOpenObjectByPointer( PdoData->LanscsiAdapterPDO.DisconEventToService,
												0,
												NULL,
												EVENT_ALL_ACCESS,
												*ExEventObjectType,
				...
				status = ObOpenObjectByPointer( PdoData->LanscsiAdapterPDO.AlarmEventToService,
												0,
												NULL,
												EVENT_ALL_ACCESS,
												*ExEventObjectType,
				...
				status = RtlULongPtrToULong((ULONG_PTR)disconEvent, (PULONG)&Information->PdoEvents32.DisconEvent);
				...
				status = RtlULongPtrToULong((ULONG_PTR)alarmEvent, (PULONG)&Information->PdoEvents32.AlarmEvent);
...
LSBus_PlugInLSBUSDevice(
	...
	status = Bus_PlugInDevice(PlugIn, PlugIn->Size, FdoData, KernelMode, TRUE);
