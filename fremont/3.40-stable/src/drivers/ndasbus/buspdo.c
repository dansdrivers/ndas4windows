...
LONG DbgLevelBusPdoPnp = DBG_LEVEL_BUS_PDO_PNP;

#define NdasDbgCall(l,x,...) do {						\
	if (l <= DbgLevelBusPdoPnp) {						\
...
--*/
{
    ...
    case IRP_MN_REMOVE_DEVICE:
		...
		NdasDbgCall( 2, "IRP_MN_REMOVE_DEVICE in DeviceData->ReferenceCount = %d DeviceData->ReportedMissing = %d\n", 
						 DeviceData->ReferenceCount, DeviceData->ReportedMissing );
        ...
        if (DeviceData->ReportedMissing) {
			...
			LSBus_WaitUntilNdasScsiStop( &DeviceData->LanscsiAdapterPDO );
		...
		NdasDbgCall( 2, "IRP_MN_REMOVE_DEVICE out\n" );
    ...
    case DeviceTextDescription:
        ...
        case 0x00000409 : // English
            if (!Irp->IoStatus.Information) {
                ...
                length  = (USHORT) \
                (wcslen(NDASSCSI_VENDORNAME) + 1 + wcslen(NDASSCSI_MODEL) + 1 + 10) * sizeof(WCHAR);
...
--*/
{
    ...
    resourceList->BusNumber = DeviceData->SerialNo;
	...
	NDAS_ASSERT( resourceList->BusNumber );
...
--*/
{
    ...
    case TargetDeviceRelation:
        ...
        // unregisters for notification on the device.
 
        deviceRelations->Count = 1;
        deviceRelations->Objects[0] = DeviceData->Self;

        ObReferenceObject( DeviceData->Self );
		//InterlockedIncrement( &DeviceData->ReferenceCount );
...
--*/
{
    ...
    busInfo->BusNumber = 0;
#else
    busInfo->BusNumber = DeviceData->SerialNo;
#endif
...
//	PDMA_ADAPTER		dmaAdapter;
	...
	ASSERT(DeviceDescriptor->BusNumber == DeviceData->SerialNo);
