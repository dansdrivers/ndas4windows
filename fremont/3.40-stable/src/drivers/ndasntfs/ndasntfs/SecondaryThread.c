		...
		if (eventStatus == 0) {
			...
			while (!FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_STOPED) && 
				...
				do {
					...
					secondaryRequest->NdfsRequestHeader.Mid2 = NTOHS(slotIndex);
...
if (IS_WINDOWS2K())
		ndfsRequestTreeConnect->StartingOffset8 = HTONLL(Secondary->VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart);
else
	...
	)
{
	...
	while(1)
	{
		ndfsRequestHeader->MessageSize4 = HTONS(sizeof(NDFS_REQUEST_HEADER) + remaninigDataSize);
