        ...
        try {
            ...
            //
...
#if 0
				{
					...
					DebugTrace( 0,  Dbg, ("NtfsFsdWrite: SavedTopLevelIrp = %p, NtfsIsTopLevelRequest() = %d, NtfsIsTopLevelNtfs() = %d, CanWait = %d\n", 
					...
					if (lazyWriter) {
						...
						if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject))
							ASSERT( !NtfsIsTopLevelRequest(IrpContext) && NtfsIsTopLevelNtfs(IrpContext) );
						...
						if (NtfsIsTopLevelRequest(IrpContext)) {
							...
							if (NtfsIsTopLevelNtfs(IrpContext)) {
								...
								if (FlagOn(IrpContext->State, IRP_CONTEXT_STATE_WAIT)) {
									...
									if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject))
										ASSERT( lazyWriter == TRUE ); // || NtfsGetTopLevelContext()->SavedTopLevelIrp == (PIRP)FSRTL_CACHE_TOP_LEVEL_IRP );
								...
								if (IrpContext->TopLevelIrpContext->MajorFunction != IRP_MJ_WRITE && VolumeDeviceObject->Secondary)
									DebugTrace( 0, Dbg, ("NtfsFsdWrite: IrpContext->TopLevelIrpContext->MajorFunction = %d\n", 
				...
				if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject))  {
					...
					if (FlagOn(Irp->Flags, IRP_PAGING_IO)) {
						...
						if (secondaryResourceAcquired == FALSE) {
							...
							//ASSERT( lazyWriter );
							NDAS_ASSERT( FlagOn(VolumeDeviceObject->Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_REMOTE_DISCONNECTED) ||
...

{
	...
	if (((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY &&
		!FlagOn(((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {

		if (!IS_SECONDARY_FILEOBJECT(IrpSp->FileObject)) {
			
			if (FlagOn(Vcb->VcbState, VCB_STATE_RESTART_IN_PROGRESS)		||
				NtfsSegmentNumber( &Fcb->FileReference ) == LOG_FILE_NUMBER	||
				NtfsSegmentNumber( &Fcb->FileReference ) == LOG_FILE_NUMBER && !FlagOn(Vcb->VcbState, VCB_STATE_MOUNT_COMPLETED)			||
				NtfsSegmentNumber( &Fcb->FileReference ) == LOG_FILE_NUMBER && FlagOn(Vcb->VcbState, VCB_STATE_VOL_PURGE_IN_PROGRESS)		||
				NtfsSegmentNumber( &Fcb->FileReference ) == MASTER_FILE_TABLE_NUMBER && FlagOn(Vcb->VcbState, VCB_STATE_LOCK_IN_PROGRESS)	||
				NtfsSegmentNumber( &Fcb->FileReference ) == LOG_FILE_NUMBER && FlagOn(Vcb->VcbState, VCB_STATE_LOCK_IN_PROGRESS) ) {
			
				if (FlagOn(Vcb->VcbState, VCB_STATE_RESTART_IN_PROGRESS)) {
					
					 DebugTrace( 0, Dbg2, ("NtfsSegmentNumber = %x\n", NtfsSegmentNumber(&Fcb->FileReference)) );
				}
		...
		NDAS_ASSERT( NtfsSegmentNumber( &Fcb->FileReference ) >= FIRST_USER_FILE_NUMBER );
	}

	if (((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY &&
		FlagOn(((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->Secondary->Flags, SECONDARY_FLAG_CLEANUP_VOLUME)) {
		...
		if (NtfsSegmentNumber( &Fcb->FileReference ) == MASTER_FILE_TABLE_NUMBER ||
			...
			DebugTrace( 0, Dbg2, ("NtfsCommonWrite: FileName = %wZ\n", &IrpSp->FileObject->FileName) );
			NDAS_ASSERT( FALSE );
		}
    ...
    if (!PagingIo &&
		...
		if (IS_SECONDARY_FILEOBJECT(IrpSp->FileObject)) {
			...
			NDAS_ASSERT( BooleanFlagOn(IrpContext->State, IRP_CONTEXT_STATE_WAIT) );
			SecondaryReleaseResourceLite( NULL, &((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->Resource );
		}

		if (!CcCanIWrite( FileObject,
				          (ULONG)ByteCount,
					      (BOOLEAN)(FlagOn( IrpContext->State,
						                   IRP_CONTEXT_STATE_WAIT | IRP_CONTEXT_STATE_IN_FSP ) == IRP_CONTEXT_STATE_WAIT),
	                      BooleanFlagOn(IrpContext->Flags, IRP_CONTEXT_FLAG_DEFERRED_WRITE))) {
			...
			if (IS_SECONDARY_FILEOBJECT(IrpSp->FileObject)) {
				...
				SecondaryAcquireResourceSharedLite( IrpContext, 
													&((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->Resource, 
													BooleanFlagOn(IrpContext->State, IRP_CONTEXT_STATE_WAIT) );
			}
    ...
    try {
		...
		if(IrpSp->FileObject == NULL) {
			...
			DebugTrace( 0, DEBUG_TRACE_ALL, 
						("IrpSp->FileObject is NULL, IrpSp->MajorFunction = %x, IrpSp->MinorFunction = %x\n", 
						 IrpSp->MajorFunction, IrpSp->MinorFunction) );
		}  

		if (IS_SECONDARY_FILEOBJECT(IrpSp->FileObject)) {

			Status = NdasNtfsSecondaryCommonWrite( IrpContext, Irp );
...
#if (__NDAS_NTFS_SECONDARY__ && __NDAS_NTFS_DBG__)
