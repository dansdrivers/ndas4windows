...

{
	...
	ASSERT( TypeOfOpen == UserFileOpen || TypeOfOpen == UserDirectoryOpen || TypeOfOpen == UserVolumeOpen );
#else
	ASSERT( TypeOfOpen == UserFileOpen || TypeOfOpen == UserDirectoryOpen || TypeOfOpen == UserVolumeOpen || TypeOfOpen == UserViewIndexOpen );
#endif
#endif
    ...
    try {
...
#else
		if (VolumeMounted && !VolumeMountedReadOnly && TypeOfOpen == UserFileOpen) {
#endif
...
#else
									if (ThisLcb->SecondaryScb == Lcb->SecondaryScb) {
...
#else
												NtfsRemoveNtfsMcbEntry( &Scb->Mcb, StartingCluster, 0xFFFFFFFF );
#endif
                                            }
		...
		if(Ccb && FlagOn(Ccb->NdasNtfsFlags, ND_NTFS_CCB_FLAG_UNOPENED)) {
		...
		secondaryRequest = AllocateWinxpSecondaryRequest( volDo->Secondary, IRP_MJ_CLEANUP, 0 );
		...
		if (TypeOfOpen == UserFileOpen) {
			...
			NDAS_ASSERT( Scb->Header.ValidDataLength.QuadPart <= Scb->Header.FileSize.QuadPart );
        ...
        DebugUnwind( NdasNtfsSecondaryCommonCleanup );
