...
#if 0
			setInformationStatus = IoSetInformation( fileObject, fileInformationClass, length, fileInformation );
#else
	...
	)
{
	...
	do {
		...
		fileNameBuffer = NtfsAllocatePool( PagedPool, NDFS_MAX_PATH*sizeof(WCHAR) );
		...
		if (createStatus == STATUS_SUCCESS) {
			...
			case UserFileOpen: {
				...
				NDAS_ASSERT( !FlagOn(scb->AttributeFlags, ATTRIBUTE_FLAG_SPARSE) );
	...
	)
{
	...
	if (openFile == NULL) {
		...
		DebugTrace2( 0, Dbg, 
					("openFile->FileObject->FileName = %wZ openFile->FileObject = %p\n", 
