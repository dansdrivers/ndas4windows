...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdasNtfsCommonFlushBuffers( IrpContext, Irp );
	...
	)
{
	...
	DebugTrace( 0, Dbg, ("fileRecord->BytesAvailable = %d, filerecord1->SequenceNumber = %x\n", 
						  fileRecord->BytesAvailable, fileRecord->SequenceNumber) );
					
	offset = fileRecord->FirstAttributeOffset;

	do {

		attrHeader = (PATTRIBUTE_RECORD_HEADER)((CHAR *)fileRecord + offset);
		
		DebugTrace( 0, Dbg, ("attrHeader1->RecordLength = %d, attrHeader1->TypeCode = %x, attrHeader1->FormCode\n", 
...
#if __NDAS_NTFS_PURGE__
						if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_PURGE) && Pass == 3) {
							...
							do {
								...
								NDAS_ASSERT( Fcb == Vcb->MftScb->Fcb );
							
								NtfsPerformVerifyDiskRead( IrpContext, 
														   Vcb, 
														   CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Buffer,
														   (LONGLONG)LlBytesFromClusters(Vcb, Vcb->MftStartLcn),
														   FS_BUFFER_SIZE );

								found = NdasNtfsFindAttribute( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Buffer, 
															   $DATA, 
														       &attribute );
								...
								if (validDataLength.QuadPart != Vcb->MftScb->Header.ValidDataLength.QuadPart) {
									...
									DebugTrace( 0, Dbg,	("attribute->Form.Nonresident.AllocatedLength = %x\n",
														  attribute->Form.Nonresident.AllocatedLength / 512) );
									DebugTrace( 0, Dbg,	("attribute->Form.Nonresident.ValidDataLength = %x\n",
														  attribute->Form.Nonresident.ValidDataLength / 512) );
									DebugTrace( 0, Dbg,	("attribute->Form.Nonresident.TotalAllocated = %x\n",
								...
								if (validDataLength.QuadPart != Vcb->MftBitmapScb->Header.ValidDataLength.QuadPart) {
