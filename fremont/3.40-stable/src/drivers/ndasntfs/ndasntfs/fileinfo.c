...

{
    ...
    try {
		...
		if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
			...
			Status = NdasNtfsSecondaryCommonQueryInformation( IrpContext, Irp );
			leave;
		}
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdasNtfsSecondaryCommonSetInformation( IrpContext, Irp );
    ...
    try {
        ...
        case FileDispositionInformation:
			...
			if (IrpContext->PrimaryRequestInfo.PrimarySessionId != NDASNTFS_LOCAL_PRMARY_SESSION_ID									&&
				IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->IrpMajorFunction == IRP_MJ_SET_INFORMATION					&&
				IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->SetFile.FileInformationClass == FileAllocationInformation	&&
				IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->SetFile.AllocationInformation.Lcn != 0) {
                    ...
                    if (NtfsIsLinkDeleteable( IrpContext, Scb->Fcb, &NonEmptyIndex, &LastLink )) {
                        ...
                        if (IsDirectory( &Scb->Fcb->Info )) {
							...
							if (NdasNtfsFsRtlNotifyFilterChangeDirectory) {
								...
								NdasNtfsFsRtlNotifyFilterChangeDirectory( Scb->Vcb->NotifySync,
				                                                          &Scb->Vcb->DirNotifyList,
							                                              FileObject->FsContext,
										                                  NULL,
													                      FALSE,
...

{
	...
	DebugTrace( 0, Dbg, ("NtfsSetEndOfFileInfo NewFileSize = %I64d\n", NewFileSize) );
...
#if __NDAS_NTFS_DBG__					
					{		
					VCN StartingVcn = LlClustersFromBytes( Scb->Vcb, Scb->Header.AllocationSize.QuadPart );
					LONGLONG ClusterCount = LlClustersFromBytes(Scb->Vcb, (NewAllocationSize - Scb->Header.AllocationSize.QuadPart));
					LARGE_INTEGER	oleAllocationSize;
					
					oleAllocationSize = Scb->Header.AllocationSize;
					ASSERT( LlBytesFromClusters( Scb->Vcb, StartingVcn ) == oleAllocationSize.QuadPart);
...

{
    ...
    if (NtfsBreakOnCorrupt &&
        ((Status == STATUS_FILE_CORRUPT_ERROR) ||
		 ...
		 if(Status == STATUS_FILE_CORRUPT_ERROR && FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT))
			 ASSERT( FALSE );
#endif
...
#define PAIRCOUNT_IN_LAGREMCB PairCount
#else
#define PAIRCOUNT_IN_LAGREMCB BaseMcb.PairCount
#endif
                      ...
                      if (Count++ != 0) {
                          ...
                          if (Scb->Mcb.NtfsMcbArray[0].NtfsMcbEntry != NULL) {
                              DebugTrace( 0, Dbg, ("First Mcb has %x entries\n",
                                                   Scb->Mcb.NtfsMcbArray[0].NtfsMcbEntry->LargeMcb.PAIRCOUNT_IN_LAGREMCB ));
                          }
                      }
                       );

#undef PAIRCOUNT_IN_LAGREMCB
