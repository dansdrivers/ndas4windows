...
#define Dbg2                             (DEBUG_INFO_CATCH_EXCEPTIONS)
#endif
...
PDEVICE_OBJECT NdasNtfsControlDeviceObject;
PDEVICE_OBJECT NdasNtfsFileSystemDeviceObject;
...
#if __NDAS_NTFS_AVOID_BUG__
const UNICODE_STRING NdasNtfsSDH = CONSTANT_UNICODE_STRING( L"$SDH" );
#endif
...
LONGLONG NtfsDebugTraceLevel = DEBUG_TRACE_ERROR;
...

{
	...
	if(!(ExceptionCode == STATUS_CANT_WAIT ||
		 ...
		 IrpContext->MajorFunction == IRP_MJ_WRITE && ExceptionCode == STATUS_FILE_DELETED || 
		 IrpContext->MajorFunction == IRP_MJ_SET_INFORMATION && ExceptionCode == STATUS_FILE_DELETED)) 
#endif
    DebugTrace( 0, DEBUG_TRACE_UNWIND, ("NtfsExceptionFilter %X\n", ExceptionCode) );
	
#if __NDAS_NTFS__
	if (ExceptionCode == STATUS_INSUFFICIENT_RESOURCES) {
...

{
    ...
    try {
	    ...
	    if (NdasNtfsCcMdlWriteAbort &&
			...
			NdasNtfsCcMdlWriteAbort( IrpSp->FileObject, Irp->MdlAddress );
	       Irp->MdlAddress = NULL;
    }
        ...
        if (ScbSnapshot->SnapshotLinks.Flink != NULL) {
            ...
            do {
                ...
                if (!FlagOn( NextScb->Fcb->FcbState, FCB_STATE_SYSTEM_FILE | FCB_STATE_PAGING_FILE )) {
					...
					NDAS_ASSERT( NextScb->Header.FileSize.QuadPart <= NextScb->Header.AllocationSize.QuadPart );
                }
...

{
    ...
    if (ARGUMENT_PRESENT( IrpContext )) {
		...
		if (ARGUMENT_PRESENT( Irp )) {
			...
			if(FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_FILE)) {
			
				NDAS_ASSERT( IrpContext->RequestIrp == Irp );
				NDAS_ASSERT( IrpContext->RequestIrpSp == IoGetCurrentIrpStackLocation(Irp) );
                NDAS_ASSERT( IrpContext->CurrentIrql == KeGetCurrentIrql() );
				NDAS_ASSERT( IrpContext->VcbSharedCount == ExIsResourceAcquiredSharedLite(&IrpContext->Vcb->Resource) );
			}

			NDAS_ASSERT( !(Irp == IrpContext->CompleteIrp && IoGetCurrentIrpStackLocation( Irp ) == IrpContext->CompleteIrpSp && FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_ALREADY_COMPLETED)) );

			IrpContext->CompleteIrp = Irp;
			IrpContext->CompleteIrpSp = IoGetCurrentIrpStackLocation( Irp );
			SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_ALREADY_COMPLETED );
		}
#endif

		//ASSERT( !FlagOn(IrpContext->NdasNtfsFlags, ND_NTFS_IRP_CONTEXT_RESOURCE_ACQUIERED) );
...
#else
		if (NdasNtfsIoVolumeDeviceToDosName) {

			Status = NdasNtfsIoVolumeDeviceToDosName( IrpContext->Vcb->TargetDeviceObject, &VolumeName );
...

{
	...
	if (irpSp->MajorFunction == IRP_MJ_DEVICE_CONTROL && NdasNtfsFileSystemDeviceObject == (PDEVICE_OBJECT)VolumeDeviceObject) {
		...
		case IOCTL_REGISTER_NDFS_CALLBACK: {
			...
			DebugTrace( 0, Dbg2, ("NtfsFsdDispatch: IOCTL_REGISTER_NDFS_CALLBACK, size = %d\n", inputBuffer->Size) );		
		...
		case IOCTL_UNREGISTER_NDFS_CALLBACK:
			...
			DebugTrace( 0, Dbg2, ("NtfsFsdDispatch: IOCTL_UNREGISTER_NDFS_CALLBACK\n") );		
		...
		case IOCTL_SHUTDOWN: {
			...
			DebugTrace( 0, Dbg2, ("NtfsFsdDispatch: IOCTL_SHUTDOWN\n") );		

	        for (vcbListEntry = NtfsData.VcbQueue.Flink;
			     vcbListEntry != &NtfsData.VcbQueue;
				...
				DebugTrace( 0, Dbg2, ("NtfsFsdDispatch: volDo = %p, KeGetCurrentIrql() = %d\n", volDo, KeGetCurrentIrql()) );		

				vcb = CONTAINING_RECORD(vcbListEntry, VCB, VcbLinks);
				volDo = CONTAINING_RECORD( vcb, VOLUME_DEVICE_OBJECT, Vcb );

				//DbgPrint( "NtfsFsdDispatch: NDAS_NTFS_DEVICE_FLAG_SHUTDOWN\n" );		
				SetFlag( volDo->NdasNtfsFlags, NDAS_NTFS_DEVICE_FLAG_SHUTDOWN );
			...
			Status = Irp->IoStatus.Status = STATUS_SUCCESS;
			Irp->IoStatus.Information = 0;
			DebugTrace( 0, Dbg2, ("NtfsFsdDispatch: IOCTL_SHUTDOWN return\n") );		
    ...
    do {
        ...
        try {
			...
			if (IrpContext->MajorFunction == IRP_MJ_DEVICE_CONTROL && 
				...
				primarySession = PrimarySession_Create( IrpContext, VolDo, inputBuffer );
