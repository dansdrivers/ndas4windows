...
#define __NDAS_FAT_CALL_CLOSE_ON_PNP__			0

#define __NDAS_NTFS_WIN2K_SUPPORT__				1
...
typedef UINT64	PRIMARY_SESSION_ID;
#define NDASNTFS_LOCAL_PRMARY_SESSION_ID	((PRIMARY_SESSION_ID)0)
...
#define NtfsReleaseVcb(IC,V) {												\
 FlagOn((IC)->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) ?	\
...
#define NtfsIsExclusiveVcb(IC, V) (												\
	FlagOn((IC)->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) ?	\
...
#else
#define NtfsAllocateFromStack(S) _ndasntfsalloca(S)
#endif
...
NTSTATUS
NdasNtfsPrimaryDirectFsdCleanup (
    IN PVOLUME_DEVICE_OBJECT		VolumeDeviceObject,
    IN PIRP							Irp,
	IN  PNDFS_WINXP_REQUEST_HEADER	NdfsWinxpRequestHeader
    );
...
NTSTATUS
SecondaryNdasNtfsCommonCleanup (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP			Irp
	);

NTSTATUS
SecondaryNdasNtfsCommonQueryInformation (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP			Irp
	);

NTSTATUS
SecondaryNdasNtfsCommonSetInformation (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP			Irp
	);

NTSTATUS
SecondaryNdasNtfsCommonWrite (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP			Irp
	);
...
NTSTATUS
NdasNtfsCommonSetInformation (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);

NTSTATUS
NdasNtfsCommonQueryInformation (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);
...
NTSTATUS
NdasNtfsSecondaryCommonWrite (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);

NTSTATUS
NdasNtfsSecondaryCommonCleanup (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);

NTSTATUS
NdasNtfsSecondaryCommonClose (
    IN PIRP_CONTEXT IrpContext,
    IN PSCB Scb,
    IN PFCB Fcb,
    IN PVCB Vcb,
...
#if 0
VOID
NdasNtfsDecrementCleanupCounts (
    IN PSCB Scb,
    IN PLCB Lcb OPTIONAL,
    IN BOOLEAN NonCachedHandle
    );
...
NTSTATUS
NdasNtfsSecondaryQueryDirectory (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp,
    IN PVCB Vcb,
    IN PSCB Scb,
...
#if 0
VOID
NdasNtfsIncrementCleanupCounts (
    IN PSCB Scb,
    IN PLCB Lcb OPTIONAL,
    IN BOOLEAN NonCachedHandle
    );
...
#define NDAS_NTFS_VER_SET_CONDITION(_m_,_t_,_c_)  \
	((_m_)=NdasNtfsVerSetConditionMask((_m_),(_t_),(_c_)))
