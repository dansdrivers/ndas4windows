...
)
{
	...
	buffer = xixcore_AllocateBuffer(XIDISK_DUP_DIR_INFO_SIZE);
	...
	try{
		...
		if(pFCB->XixcoreFcb.FCBType  == FCB_TYPE_DIR )
		{
			...
			uint8 	*Name = NULL;
			DirInfo =	(PXIDISK_DIR_INFO)xixcore_GetDataBufferWithOffset(buffer);
			...
			uint8 	*Name = NULL;
			FileInfo =  (PXIDISK_FILE_INFO)xixcore_GetDataBufferWithOffset(buffer);
			...
			if(XIXCORE_TEST_FLAGS(pFCB->XixcoreFcb.FCBFlags, XIXCORE_FCB_MODIFIED_FILE_SIZE)){
				XIXCORE_CLEAR_FLAGS(pFCB->XixcoreFcb.FCBFlags, XIXCORE_FCB_MODIFIED_FILE_SIZE);
				FileInfo->FileSize = ((pFCB->FileSize.QuadPart > pFCB->ValidDataLength.QuadPart)?pFCB->ValidDataLength.QuadPart:pFCB->FileSize.QuadPart);
			...
			xixcore_UpdateChildCacheEntry(
				...
				pFCB->XixcoreFcb.LotNumber,
				FileInfo->FileAttribute,
				FileInfo->FileSize,
				FileInfo->AllocationSize,
				FileInfo->Change_time,
				FileInfo->Change_time,
...
)
{
	...
	if((pFCB->XixcoreFcb.FCBType != FCB_TYPE_DIR) && (pFCB->XixcoreFcb.FCBType != FCB_TYPE_FILE)){
		DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
			("FAIL xixfs_DeleteUpdateFCB STATUS_INVALID_PARAMETER.\n"));	
	...
	if(!buffer) {
		DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
			("FAIL xixfs_DeleteUpdateFCB STATUS_INSUFFICIENT_RESOURCES.\n"));	
		return STATUS_INSUFFICIENT_RESOURCES;
	}
	...
	try{
		if(pFCB->XixcoreFcb.FCBType == FCB_TYPE_DIR )
		{
			if(pFCB->XixcoreFcb.HasLock != FCB_FILE_LOCK_HAS){ 
					...
					&& (pFCB->FCBCleanup == 0))
				{
					...
					if(!NT_SUCCESS(RC)){
						DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
							("FAIL xixfs_DeleteUpdateFCB DIR GET LOCK.\n"));	
						try_return(RC);
					}
				}else{
					RC = STATUS_SUCCESS;
			...
			if (pFCB->XixcoreFcb.HasLock != FCB_FILE_LOCK_HAS) {
				DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
					("FAIL xixfs_DeleteUpdateFCB FILE HAS NO LOCK .\n"));	
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
					("FAIL  xixfs_DeleteUpdateFCB : xixfs_ReadFileInfoFromFcb .\n"));	
		...
		if(pFCB->XixcoreFcb.FCBType  == FCB_TYPE_DIR )
		{
			...
			if(DirInfo->State== XIFS_FD_STATE_DELETED){
				...
				DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
					("FAIL xixfs_DeleteUpdateFCB : Deleted .\n"));	
				...
				&& (DirInfo->LinkCount == 1))
			{
				...
				if(!NT_SUCCESS(RC)){
					DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
						("FAIL xixfs_DeleteUpdateFCB : XixFsdDeleteFileLotAddress .\n"));	
					try_return(RC);
				}
				
				
				RtlZeroMemory(xixcore_GetDataBufferWithOffset(LotHeader),(XIDISK_COMMON_LOT_HEADER_SIZE - XIXCORE_MD5DIGEST_AND_SEQSIZE));
				DirInfo->AddressMapIndex = 0;
				DirInfo->State = XIFS_FD_STATE_DELETED;
				DirInfo->Type =  XIFS_FD_TYPE_INVALID;

				pLotHeader->LotInfo.BeginningLotIndex = 0;
				pLotHeader->LotInfo.LogicalStartOffset = 0;
				pLotHeader->LotInfo.LotIndex = pFCB->XixcoreFcb.LotNumber;
				pLotHeader->LotInfo.NextLotIndex = 0;
				pLotHeader->LotInfo.PreviousLotIndex = 0;
				pLotHeader->LotInfo.StartDataOffset = 0;
				pLotHeader->LotInfo.Type = LOT_INFO_TYPE_INVALID;
				pLotHeader->LotInfo.Flags = LOT_FLAG_INVALID;
			...
			if(pFCB->XixcoreFcb.FCBType == FCB_TYPE_FILE){
				...
				if(XIXCORE_TEST_FLAGS(pFCB->XixcoreFcb.FCBFlags,XIXCORE_FCB_MODIFIED_ADDR_BLOCK)){
					...
					if (RC< 0 ){
						DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
							("Fail(0x%x) Reason(0x%x):xixfs_DeleteUpdateFCB:xixcore_RawWriteAddressOfFile\n",
								RC, Reason));
						...
						&& (FileInfo->LinkCount > 1))
			{
				...
				if(XIXCORE_TEST_FLAGS(pFCB->XixcoreFcb.FCBFlags, XIXCORE_FCB_MODIFIED_FILE_NAME)){
					...
					DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
						("xixfs_DeleteUpdateFCB : Set New Name .\n"));
				}
		...
		if(bDelete){
			...
			if(!NT_SUCCESS(RC)){
				DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL,  
						("FAIL xixfs_DeleteUpdateFCB : xixfs_WriteFileInfoFromFcb .\n"));
				try_return(RC);
			}
		}
	...
	try{
		...
		if(pFCB->XixcoreFcb.FCBType  == FCB_TYPE_DIR )
		{
				...
				&& (FileInfo->LinkCount == 1))
			{
				...
				RtlZeroMemory(xixcore_GetDataBufferWithOffset(LotHeader),(XIDISK_DUP_COMMON_LOT_HEADER_SIZE - XIXCORE_MD5DIGEST_AND_SEQSIZE));
				FileInfo->AddressMapIndex = 0;
				FileInfo->State = XIFS_FD_STATE_DELETED;
				FileInfo->Type =  XIFS_FD_TYPE_INVALID;
