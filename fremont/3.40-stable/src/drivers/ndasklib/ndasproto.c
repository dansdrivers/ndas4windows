...
LspLookupProtocol(
	...
	if(NdasHwVersion == LANSCSIIDE_VERSION_1_0) {
		*Protocol = LSIDEPROTO_VERSION_1_0;
	} else if(NdasHwVersion == LANSCSIIDE_VERSION_1_1) {
		*Protocol = LSIDEPROTO_VERSION_1_1;
...
LspUpgradeUserIDWithWriteAccess(
	...
	if(LSS->UserID & 0xffff0000) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("LanscsiSession(%p) has the write-access UserID(%08lx).\n", LSS->UserID));
	...
	) 
{
	...
	RtlZeroMemory( FromLSS, sizeof(LANSCSI_SESSION) );
...
LspSetDefaultTimeOut(
	...
	NDAS_ASSERT( TimeOut->QuadPart < 0 && (-TimeOut->QuadPart) <= 1000 *NANO100_PER_SEC );
	...
	) 
{
	if (BindingAddress) {
		...
		*BindingAddress = LSS->NdasBindAddress;
	}

	if (TargetAddress) {

		*TargetAddress = LSS->NdasNodeAddress; 
	}
}
	...
	) 
{
	...
	NDAS_ASSERT( SrcAddr->Address[0].NdasAddress.Lpx.Port == 0 );

	do {

		LSS->NdasNodeAddress = *DestAddr;
		LSS->NdasBindAddress = *SrcAddr;
		...
		status = NdasTransOpenConnection( NULL, DestAddr->Address[0].AddressType, &LSS->NdastConnectionFile );
	...
	)
{
	...
	for (idx_addr = 0; idx_addr < (ULONG)BindAddrList->TAAddressCount; idx_addr++) {
		...
		bindNdasAddr.TAAddressCount = 1;
		bindNdasAddr.Address[0].AddressType = bindAddr->AddressType;
		bindNdasAddr.Address[0].AddressLength = bindAddr->AddressLength;
		RtlCopyMemory( &bindNdasAddr.Address[0].NdasAddress, bindAddr->Address, bindNdasAddr.Address[0].AddressLength );

		DebugTrace( NDASSCSI_DBG_TRANSPORT_INFO, 
					("bindAddr %04x/%02x:%02x:%02x:%02x:%02x:%02x\n",
					 bindNdasAddr.Address[0].NdasAddress.Lpx.Port, 
					 bindNdasAddr.Address[0].NdasAddress.Lpx.Node[0], bindNdasAddr.Address[0].NdasAddress.Lpx.Node[1],
					 bindNdasAddr.Address[0].NdasAddress.Lpx.Node[2], bindNdasAddr.Address[0].NdasAddress.Lpx.Node[3],
					 bindNdasAddr.Address[0].NdasAddress.Lpx.Node[4], bindNdasAddr.Address[0].NdasAddress.Lpx.Node[5]) );

		DebugTrace( NDASSCSI_DBG_TRANSPORT_INFO, 
					("DestAddr %04x/%02x:%02x:%02x:%02x:%02x:%02x\n",
					 DestAddr->Address[0].NdasAddress.Lpx.Port, 
					 DestAddr->Address[0].NdasAddress.Lpx.Node[0], DestAddr->Address[0].NdasAddress.Lpx.Node[1],
					 DestAddr->Address[0].NdasAddress.Lpx.Node[2], DestAddr->Address[0].NdasAddress.Lpx.Node[3],
					 DestAddr->Address[0].NdasAddress.Lpx.Node[4], DestAddr->Address[0].NdasAddress.Lpx.Node[5]) );
	...
	) 
{
	...
	NDAS_ASSERT( LSS != NULL );
	NDAS_ASSERT( DestAddr != NULL );
	NDAS_ASSERT( BindAnyAddress == TRUE );
	...
	) 
{
	...
	do {
		...
		addrList = NdasTransAllocateAddr( DestAddr->Address[0].AddressType, 
										  16, 
										  &addrListLen );
		...
		status = NdasTransGetTransportAddress( DestAddr->Address[0].AddressType,
											   addrListLen,
											   addrList,
											   NULL );
		...
		if (!NT_SUCCESS(status)) {
			...
			DebugTrace( NDASSCSI_DBG_TRANSPORT_INFO, ("LstransQueryBindingDevices() failed. STATUS=%08lx\n", status) );

			break;
		}

		if (addrList->TAAddressCount < 1) {
		...
		addrListPrioritized = NdasTransAllocateAddr( DestAddr->Address[0].AddressType, 
												     16, 
													 NULL );
		...
		if (BindAddr1) {
			...
			taAddress->AddressType = BindAddr1->Address[0].AddressType;
			taAddress->AddressLength = BindAddr1->Address[0].AddressLength;

			RtlCopyMemory( taAddress->Address, 
						   &BindAddr1->Address[0].NdasAddress, 
		...
		if (BindAddr2) {
			...
			taAddress->AddressType = BindAddr2->Address[0].AddressType;
			taAddress->AddressLength = BindAddr2->Address[0].AddressLength;

			RtlCopyMemory( taAddress->Address, 
						   &BindAddr2->Address[0].NdasAddress, 
						   taAddress->AddressLength );
	...
	) 
{
	...
	TA_NDAS_ADDRESS	bindAddress;
	TA_NDAS_ADDRESS	nodeAddress;
	...
	LspGetAddresses( LSS, &bindAddress, &nodeAddress );
	...
	LSS->NdasBindAddress = bindAddress;
...
LspRelogin(
	...
	if( !NT_SUCCESS(ntStatus) ) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("HWVersion wrong.\n"));
		return STATUS_NOT_IMPLEMENTED;
	}
	...
	)
{
	...
	pRequestHeader = (PLANSCSI_H2R_PDU_HEADER)LSS->PduBuffer;
	...
	) 
{
	...
	NDAS_ASSERT( OverlappedData == NULL );
	...
	return LSS->LanscsiProtocol->LsprotoFunc.LspSendRequest( LSS,
															 Pdu,
															 OverlappedData,
															 0, 
															 IF_NULL_TIMEOUT_THEN_DEFAULT(LSS, TimeOut) );
	...
	) 
{
	...
	return LSS->LanscsiProtocol->LsprotoFunc.LspReadReply( LSS,
														   Buffer,
														   Pdu,
														   OverlappedData,
														   IF_NULL_TIMEOUT_THEN_DEFAULT(LSS, TimeOut) );
	...
	if(NT_SUCCESS(status)) {
		...
		if(LoginInfo->IsEncryptBuffer) {
			...
			if(!LSS->EncryptBuffer) {
				...
				KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Error! Could not allocate the encrypt buffer! Performace may slow down.\n"));
			} else {
		...
		if(!LSS->DigestPatchBuffer) {
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Error! Could not allocate the digest patch buffer! Performace may slow down.\n"));
		}
#endif
		//
...
LspWorkaroundCleanupLock(
   ...
   PLARGE_INTEGER	TimeOut
	...
	KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("Cleaned #%u\n", cleaned));
