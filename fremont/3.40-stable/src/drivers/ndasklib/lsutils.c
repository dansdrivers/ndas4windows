...
LsuConnectLogin(
	...
	defaultTimeOut.QuadPart = (-1) * LURN_IDE_GENERIC_TIMEOUT * 15 / 10;
	LspSetDefaultTimeOut(LanScsiSession, &defaultTimeOut);

	status = LspConnectMultiBindAddr( LanScsiSession,
									  BindingAddress,
									  NULL,
									  NULL,
									  TargetAddress,
	...
	) 
{
	...
	status = LspConnectMultiBindAddr( LSS, BoundAddr, BindingAddr, BindingAddr2, TargetAddr, BindAnyAddr, NULL, NULL, NULL );
...
LsuConfigureIdeDisk(
	...
	// Even though this setting is enough to be applied once,
	// We don't know when the device resets.
	// Ex) SunPlus SPIF223A
	//

	(VOID)LsuSetPioMode(LSS, NDAS_DEFAULT_PIO_MODE);
#endif
	...
	status = LsuSetPioMode(LSS, pioMode);

	if (status != STATUS_SUCCESS) {

		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Set Defualt PIO mode Feature Failed...\n") );
			...
			if(info.hw_config & 0x2000) {
				...
				if(UdmaRestrict != 0xff) {
					if(DmaMode > UdmaRestrict) {
						...
						KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("UDMA restriction applied. UDMA=%d\n", (ULONG)DmaMode));
					}
				}
...
LsuSetTransferMode(
	...
	LSS_INITIALIZE_PDUDESC(LSS, &pduDesc, IDE_COMMAND, WIN_SETFEATURES, 0, 0, 0, 0, NULL, NULL);
	pduDesc.Feature = SETFEATURES_XFER;
	pduDesc.BlockCount = TransferMode;

	status = LspRequest(LSS, &pduDesc, &PduResponse, NULL, NULL, NULL );
	if(!NT_SUCCESS(status) || PduResponse != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(DBG_OTHER_ERROR, ("Failed...STATUS=%08lx\n", status));
...
LsuSetPioMode(
	...
	pioFeature = 0x08 | PioMode;

	return LsuSetTransferMode(LSS, pioFeature);
}
...
LsuSetDmaMode(
	...
	if(UltraDma) {
		...
		dmaFeature = 0x20 | DmaMode;
	}

	return LsuSetTransferMode(LSS, dmaFeature);
}
	...
	)
{
	...
	NDAS_ASSERT( LogEntry->DumpDataEntry <= LSU_MAX_ERRLOG_DATA_ENTRIES );
...
LsuIncrementTdiClientDevice() {
	...
	KeAcquireSpinLock(&TDICLIENT_CONTEXT.TdiPnPSpinLock, &oldIrql);

	ASSERT(TDICLIENT_CONTEXT.ClientDeviceCount >= 0);
	TDICLIENT_CONTEXT.ClientDeviceCount++;

	KeReleaseSpinLock(&TDICLIENT_CONTEXT.TdiPnPSpinLock, oldIrql);
...
LsuIncrementTdiClientInProgress() {
	...
	NDAS_ASSERT( TDICLIENT_CONTEXT.ClientInProgressIOCount >= 0 );
...
LsuClientPnPBindingChange(
	...
	if(DeviceName && DeviceName->Buffer) {
		KDPrintM(DBG_OTHER_INFO, ("DeviceName=%ws PnpOpcode=%x\n", DeviceName->Buffer, PnPOpcode));
	} else {
		KDPrintM(DBG_OTHER_INFO, ("DeviceName=NULL PnpOpcode=%x\n", PnPOpcode));
	}
	...
	)
{
	...
	NDAS_ASSERT( BlockBytes && Length );
	NDAS_ASSERT( StartingOffset % BlockBytes == 0 && Length % BlockBytes == 0 );
	...
	ace->BlockBytes		= BlockBytes;
	ace->BlockStartAddr = StartingOffset / BlockBytes;
	ace->BlockLength	= Length / BlockBytes;

	ace->BlockAceId = BlockAceId ? BlockAceId : (BLOCKACE_ID)(ULONG_PTR)ace;
	...
	)
{
	...
	for (cnt = 0; cnt < NdasBacl->BlockACECnt; cnt++) {
		...
		lsuBace = LsuCreateBlockAce( accessMode, ndasBace->StartingOffset, ndasBace->Length, BlockBytes, 0 );
	...
	)
{
	...
	requiredBufLen = FIELD_OFFSET(NDAS_BLOCK_ACL, BlockACEs) + sizeof(NDAS_BLOCK_ACE) * LsuBacl->BlockACECnt;
	...
	for (cur = LsuBacl->BlockAclHead.Flink; cur != &LsuBacl->BlockAclHead; cur = cur->Flink) {
		...
		ndasBace->StartingOffset = lsuBlockAce->BlockStartAddr * lsuBlockAce->BlockBytes;
		ndasBace->Length = lsuBlockAce->BlockLength * lsuBlockAce->BlockBytes;
	...
	)
{
	...
	for (cur = BlockAcl->BlockAclHead.Flink; cur != &BlockAcl->BlockAclHead; cur = cur->Flink) {
		...
		if (StartBlockAddr >= lsuBlockAce->BlockStartAddr && 
			StartBlockAddr+BlockLength <= lsuBlockAce->BlockStartAddr+lsuBlockAce->BlockLength) {
