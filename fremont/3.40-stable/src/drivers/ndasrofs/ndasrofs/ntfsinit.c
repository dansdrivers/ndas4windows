...
BOOLEAN
(*NdasNtfsSeAuditingHardLinkEvents)(
    __in BOOLEAN AccessGranted,
    __in PSECURITY_DESCRIPTOR SecurityDescriptor
    );

VOID
(*NdasNtfsSeAuditHardLinkCreation)(
...

{
	...
	NtfsDebugTraceLevel |= DEBUG_TRACE_FSP_DISPATCHER;
	NtfsDebugTraceLevel |= DEBUG_TRACE_LOGSUP;
	NtfsDebugTraceLevel |= DEBUG_TRACE_PNP;
	NtfsDebugTraceLevel |= DEBUG_TRACE_VERFYSUP;
	NtfsDebugTraceLevel |= DEBUG_TRACE_LOGSUP;
...
#if __NDAS_ROFS_DBG__
	NtfsData.NdasNtfsDriverObject = DriverObject;
#endif
	...
	RtlInitUnicodeString( &functionName, L"SeAuditingHardLinkEvents" );
	NdasNtfsSeAuditingHardLinkEvents = MmGetSystemRoutineAddress( &functionName );

	RtlInitUnicodeString( &functionName, L"SeAuditHardLinkCreation" );
	NdasNtfsSeAuditHardLinkCreation = MmGetSystemRoutineAddress( &functionName );
	...
	RtlInitUnicodeString( &UnicodeString, NDAS_ROFS_DEVICE_NAME );
	...
	RtlInitUnicodeString( &UnicodeString, NDAS_ROFS_CONTROL_DEVICE_NAME );
	...
	RtlInitUnicodeString( &linkString, NDAS_ROFS_CONTROL_LINK_NAME );
...
#if !__NDAS_ROFS_DISABLE_REINIT__
	...
	)
{
	...
	DebugTrace( 0, Dbg, ("NdasRofsUnload occured NdasNtfsControlDeviceObject->ReferenceCount = %x\n", 
						 NdasNtfsControlDeviceObject->ReferenceCount) );

	IoDeleteDevice( NdasNtfsControlDeviceObject );

	DebugTrace( 0, Dbg, ("NdasRofsUnload occured NdasNtfsFileSystemDeviceObject->ReferenceCount = %x\n", 
...

{
	...
	if (NdasNtfsRtlVerifyVersionInfo) {
		...
		NDAS_ASSERT( IS_WINDOWS2K() || FALSE );
		return FALSE;
	}
#endif
}
