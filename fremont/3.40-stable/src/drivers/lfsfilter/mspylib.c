...
#ifdef ALLOC_PRAGMA
    #pragma alloc_text(INIT, MiniSpyReadDriverParameters)
#endif
...
--*/
{
    ...
    newRecord = MiniSpyAllocateBuffer( &initialRecordType );
    ...
    if (newRecord == NULL) {
        ...
        if (!InterlockedExchange( &MiniSpyData.StaticBufferInUse, TRUE )) {
            ...
            newRecord = (PMINI_RECORD_LIST)MiniSpyData.OutOfMemoryBuffer;
    ...
    if (newRecord != NULL) {
        ...
        newRecord->LogRecord.Length = sizeof(MINI_LOG_RECORD);
...
--*/
{
    ...
    if (NULL != Name) {
        ...
        if (Name->Length > (MINI_MAX_NAME_SPACE - sizeof( UNICODE_NULL ))) {

            nameCopyLength = MINI_MAX_NAME_SPACE - sizeof( UNICODE_NULL );
        ...
        LogRecord->Length = (ULONG)ROUND_TO_SIZE( (LogRecord->Length +
                                            nameCopyLength +
                                            sizeof( UNICODE_NULL )),
                                           sizeof( PVOID ) );
...
--*/
{
    ...
    PMINI_LOG_RECORD pLogRecord;
    NTSTATUS status = STATUS_NO_MORE_ENTRIES;
    PMINI_RECORD_LIST pRecordList;
    KIRQL oldIrql;
    ...
    while (!IsListEmpty( &MiniSpyData.OutputBufferList ) && (OutputBufferLength > 0)) {
        ...
        pRecordList = CONTAINING_RECORD( pList, MINI_RECORD_LIST, List );
        ...
        if (MINI_REMAINING_NAME_SPACE( pLogRecord ) == MINI_MAX_NAME_SPACE) {
            ...
            pLogRecord->Length += (ULONG)ROUND_TO_SIZE( sizeof( UNICODE_NULL ), sizeof( PVOID ) );
