	...
	try {
		...
		if (!(FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) && !FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTED))) {
			...
			if (InstanceContext->LfsDeviceExt.Secondary != NULL && 
				Secondary_LookUpCcb(InstanceContext->LfsDeviceExt.Secondary, fileObject) != NULL) {
		...
		if (iopb->MajorFunction == IRP_MJ_PNP) {
			...
			if (!FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
				
				NDAS_ASSERT( InstanceContext->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT ||
	...
	try {
		...
		if (iopb->MajorFunction == IRP_MJ_PNP) {
			...
			if (iopb->MinorFunction == IRP_MN_CANCEL_REMOVE_DEVICE || iopb->MinorFunction == IRP_MN_REMOVE_DEVICE) {
				...
				NDAS_ASSERT( FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTING) );
	...
	try {
		...
		if (iopb->MajorFunction == IRP_MJ_CREATE) {
			...
			if (RtlEqualUnicodeString(&GlobalLfs.MountMgrRemoteDatabase, &fileObject->FileName, TRUE)	|| 
				...
				NDAS_ASSERT( FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) );
		...
		if (FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTING)) {
			...
			NDAS_ASSERT( InstanceContext->LfsDeviceExt.Secondary == NULL || 
						   Secondary_LookUpCcb(InstanceContext->LfsDeviceExt.Secondary, fileObject) == NULL );
		...
		NDAS_ASSERT( FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) && 
					 !FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTED) );

		if (FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_SURPRISE_REMOVED)) {

			if (Secondary_LookUpCcb(InstanceContext->LfsDeviceExt.Secondary, fileObject)) {
		...
		if (iopb->MajorFunction == IRP_MJ_PNP) {
			...
			case IRP_MN_SURPRISE_REMOVAL: {
				...
				NDAS_ASSERT( InstanceContext->LfsDeviceExt.NetdiskPartition );
			...
			case IRP_MN_QUERY_REMOVE_DEVICE: {
				...
				Secondary_TryCloseCcbs( InstanceContext->LfsDeviceExt.Secondary );
				...
				NDAS_ASSERT( InstanceContext->LfsDeviceExt.AttachedToDeviceObject );
		...
		if (InstanceContext->LfsDeviceExt.SecondaryState == WAIT_PURGE_SAFE_STATE) {
			...
			NDAS_ASSERT( iopb->MajorFunction == IRP_MJ_CREATE );
			...
			if (iopb->MajorFunction == IRP_MJ_CREATE) {
				...
				if (Data->RequestorMode == KernelMode &&
					(fileObject->RelatedFileObject == NULL ||
					 Secondary_LookUpCcb(InstanceContext->LfsDeviceExt.Secondary, fileObject->RelatedFileObject) == NULL)) {
		...
		if (FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING) && InstanceContext->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT  ||
			FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING) && InstanceContext->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS ||
		...
		status = MiniSecondaryPassThrough( InstanceContext->LfsDeviceExt.Secondary, Data );
