	...
	)
{
	...
	UNREFERENCED_PARAMETER(FastMutexSet);
	...
	)
{
	...
	fullFileNameBuffer = ExAllocatePoolWithTag(NonPagedPool, NDFS_MAX_PATH*sizeof(WCHAR), LFS_ALLOC_TAG);
	...
	RtlInitEmptyUnicodeString( &fullFileName, fullFileNameBuffer, NDFS_MAX_PATH*sizeof(WCHAR) );
