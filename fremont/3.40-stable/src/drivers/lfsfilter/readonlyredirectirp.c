	...
	)
{
	...
	try {
		...
		fullFileNameBuffer = ExAllocatePoolWithTag( NonPagedPool, 
											 NDFS_MAX_PATH*sizeof(WCHAR) + DevExt->LfsDeviceExt.NetdiskPartitionInformation.VolumeName.Length, LFS_ALLOC_TAG );
		...
		RtlInitEmptyUnicodeString( &fullFileName,
								   fullFileNameBuffer,
								   NDFS_MAX_PATH*sizeof(WCHAR) + DevExt->LfsDeviceExt.NetdiskPartitionInformation.VolumeName.Length );
...
#if 1
					outputBuffer = ExAllocatePoolWithTag( NonPagedPool, outputBufferLength, LFS_ALLOC_TAG );
#else
