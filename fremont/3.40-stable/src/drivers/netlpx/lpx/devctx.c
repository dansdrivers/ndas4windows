...

{
	...
	KeInitializeSpinLock (&deviceContext->LpxMediaFlagSpinLock);
	...
	ExInitializeNPagedLookasideList(
				&deviceContext->DeferredRxCompContext,
				NULL,
				NULL,
				0,
				sizeof(RX_COMP_DPC_CTX),
				LPX_MEM_TAG_RXCOMPDPC_CTX,
				0);

#else

	InitializeListHead( &deviceContext->DeferredLpxReceiveCompleteDpcPacketList );
	KeInitializeDpc( &deviceContext->DeferredLpxReceiveCompleteDpc, DeferredLpxReceiveComplete, deviceContext );

	deviceContext->ForcedAckPerPackets = DEFAULT_FORCED_ACK_PER_PACKETS;
#endif
    ...
    deviceContext->Information.MaxTsduSize = LPX_MAX_TSDU_SIZE;
    deviceContext->Information.MaxDatagramSize = LPX_MAX_DATAGRAM_SIZE;
    deviceContext->Information.MaxConnectionUserData = LPX_MAX_CONNECTION_USER_DATA;
    ...
    deviceContext->Information.MinimumLookaheadData = LPX_MIN_LOOKAHEAD_DATA;
    deviceContext->Information.MaximumLookaheadData = LPX_MAX_LOOKAHEAD_DATA;
...
#if 0
	// Remove deferred receive complete look-aside memory.
	ExDeleteNPagedLookasideList(&DeviceContext->DeferredRxCompContext);
#endif
