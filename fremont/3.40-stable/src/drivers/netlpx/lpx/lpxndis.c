    ...
    )
{
	...
	UNREFERENCED_PARAMETER(ProtocolVcContext);
...

{
	...
	NDIS50_PROTOCOL_CHARACTERISTICS	ProtChars = {0};
	UINT							ProtCharsLen;
	...
	ProtCharsLen = sizeof(NDIS50_PROTOCOL_CHARACTERISTICS);
...

{
    ...
    if (LpxNdisProtocolHandle != (NDIS_HANDLE)NULL) {
		...
		NdisDeregisterProtocol( &ndisStatus, LpxNdisProtocolHandle );
...
--*/
{
    ...
    if (NdisStatus == NDIS_STATUS_PENDING) {
        ...
        IF_LPXDBG (LPX_DEBUG_NDIS) {
            LpxPrint1 ("Adapter %p open pended.\n", AdapterString);
        }
...
#if DBG
        IF_LPXDBG (LPX_DEBUG_NDIS) {
            LpxPrint1 ("Adapter %p successfully opened.\n", AdapterString);
        }
#endif
    } else {
#if DBG
        IF_LPXDBG (LPX_DEBUG_NDIS) {
            LpxPrint2 ("Adapter open %p failed, status: %s.\n",
                AdapterString,
                LpxGetNdisStatus (NdisStatus));
        }
#endif
	...
	if (DeviceContext->MaxSendPacketSize < (1024 + 72 + sizeof(LPX_HEADER) + 14)) {
    ...
    case NdisMediumFddi:
        ...
        RtlStoreUlong( (PULONG)LpxDataBuffer,
						(NDIS_PACKET_TYPE_DIRECTED | NDIS_PACKET_TYPE_ALL_MULTICAST | NDIS_PACKET_TYPE_BROADCAST | NDIS_PACKET_TYPE_ALL_LOCAL) );
#endif
		break;
