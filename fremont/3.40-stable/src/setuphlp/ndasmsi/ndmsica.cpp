...
NDMsiInstallLegacyDriver(MSIHANDLE hInstall)
{
	...
	MSILOGINFO(FSB1024(
		_T("iErrorDlg=%d,BinaryPath=%s,ServiceName=%s,DisplayName=%s")
...
NDMsiQuestion(MSIHANDLE hInstall)
{
	...
	XTLTRACE1(TRACE_LEVEL_INFORMATION, _T("iErrorDlg=%d\n"), iErrorDlg);
...
NDMsiStartServiceOnNoDeferredReboot(MSIHANDLE hInstall)
{
	...
	XTL::AutoSCHandle hSCM = ::OpenSCManager(
		NULL, 
...
pNDMsiRemoveNetComponent(MSIHANDLE hInstall, DWORD dwFlags)
{
	...
	XTLTRACE1(TRACE_LEVEL_INFORMATION, _T("Reboot required: %d\n"), fRebootRequired);
...
// CustomActionData: 
//
// <ierrordialog>|<inf-path>|<sectionname>|<flags>
//
// <flags> 0x0000 NONE
// XMSI_INSTALL_INF_FLAGS_NONE                  0x00000000
// XMSI_INSTALL_INF_FLAGS_IGNORE_FILE_REBOOT    0x00000001
// XMSI_INSTALL_INF_FLAGS_IGNORE_SERVICE_REBOOT 0x00000002
// e.g. 25001|ndasfs.inf|DefaultInstall
//

typedef enum _XMSI_INSTALL_INF_FLAGS {
	XMSI_INSTALL_INF_FLAGS_NONE                  = 0x0,
	XMSI_INSTALL_INF_FLAGS_IGNORE_FILE_REBOOT    = 0x1,
	XMSI_INSTALL_INF_FLAGS_IGNORE_SERVICE_REBOOT = 0x2
} XMSI_INSTALL_INF_FLAGS;

EXTERN_C
UINT
__stdcall
xMsiInstallInf(MSIHANDLE hInstall)
{
	RUN_MODE_TRACE(hInstall);
	pMsiIncrementProgressBar(hInstall, 10);
	...
	INT errorDialog = 0;
	LPCTSTR infPath = NULL;
	LPCTSTR infSectionName = NULL;
	ULONG flags = XMSI_INSTALL_INF_FLAGS_NONE;

	CADParser parser;
	parser.AddToken<INT>(errorDialog);
	parser.AddToken<LPCTSTR>(infPath);
	parser.AddToken<LPCTSTR>(infSectionName);
	parser.AddToken<ULONG>(flags);
	...
	MSILOGINFO(FSB256(_T("INF=%s, Section=%s"), infPath, infSectionName));
	...
	while (TRUE) 
	{
		...
		if (FAILED(hr))
		{
			MSILOGINFO(FSB256(_T("InstallFromInf failed, hr=0x%x"), hr));

			UINT response = pMsiErrorMessageBox(hInstall, errorDialog);

			if (IDABORT == response)
			{
				return ERROR_INSTALL_FAILURE;
			}
			else if (IDRETRY == response)
			{
				continue;
			}
		}
		...
		if (SUCCEEDED(hr))
		{
			if (needsReboot)
			{
				MSILOGINFO(_T("Reboot required for the file operation."));
				if (flags & XMSI_INSTALL_INF_FLAGS_IGNORE_FILE_REBOOT)
				{
					MSILOGINFO(_T("Dismissed reboot request for files."));
				}
				else
				{
					pMsiScheduleRebootForDeferredCA(hInstall);
			...
			{
				MSILOGINFO(_T("Reboot required for the service installation."));
				if (flags & XMSI_INSTALL_INF_FLAGS_IGNORE_SERVICE_REBOOT)
				{
					MSILOGINFO(_T("Dismissed reboot request for the service."));
				}
				else
				{
					pMsiScheduleRebootForDeferredCA(hInstall);
...
pDeleteFiles(MSIHANDLE hInstall, LPCTSTR szSearchSpec, BOOL bRecursive)
{
	...
	XTL::AutoFindFileHandle hFindFile = ::FindFirstFile(szSearchSpec, &ffData);
