...
{
	XMSITRACE(L"pxClearGlobalAtom: %ls\n", AtomName);
		...
		at = GlobalFindAtomW(AtomName))
	{
		...
		XMSITRACE(L"pxClearGlobalAtom: Count=%d\n", ++count);
	...
	__in MSIHANDLE hInstall)
{
	...
	{
		XMSITRACEH(hInstall, L"REBOOT: Schedule Reboot to complete the operation\n");
	...
	__in MSIHANDLE hInstall)
{
	if (MsiGetMode(hInstall, MSIRUNMODE_SCHEDULED))
	{
		XMSITRACEH(hInstall, L"FORCEREBOOT: ForceReboot is required to complete the operation\n");
	...
	{
		XMSITRACEH(hInstall, L"ForceReboot to complete the operation\n");
