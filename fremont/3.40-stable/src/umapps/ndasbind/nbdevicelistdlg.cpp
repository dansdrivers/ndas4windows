...
BOOL CNBSelectDeviceDlg::IsOK()
{
	...
	for(NBLogicalDevPtrList::iterator itUnitDevice = m_listDevicesSelected.begin();
		itUnitDevice != m_listDevicesSelected.end(); itUnitDevice++)
	{
		...
		if(!m_fnCallBack((*itUnitDevice), m_hWnd, m_lpContext)) {
			// Unselect item to prevent OnListSelChanged is called again.
			m_wndListSingle.UnselectDiskObject((*itUnitDevice));
			return FALSE;
		}
	}
