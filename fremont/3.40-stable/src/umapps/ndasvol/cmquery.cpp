...
LONG DbgLevelVolCmd = DBG_LEVEL_VOL_CMD;

#define NdasUiDbgCall(l,x,...) do {							\
	if (l <= DbgLevelVolCmd) {								\
	XTLTRACE2(NdasVolTrace, 0, "|%d|%s|%d|",l,__FUNCTION__, __LINE__); 	\
	XTLTRACE2(NdasVolTrace, 0, x,__VA_ARGS__);							\
	} 														\
} while(0)
...
pGetDeviceAddress(
	...
	__out LPDWORD DeviceAddress)
{
	if (IsBadWritePtr(DeviceAddress, sizeof(DWORD)))
	{
		return E_POINTER;
	}
	...
	__in_opt DWORD nNameLength)
{
	...
	for (DWORD i = 0; ; ++i)
	{
		...
		if (UseUINumber)
		{
			hr = pGetDeviceUINumber(deviceInfoSet, &deviceInfoData, &locationId);
		}
		else
		{
			hr = pGetDeviceAddress(deviceInfoSet, &deviceInfoData, &locationId);
		}
