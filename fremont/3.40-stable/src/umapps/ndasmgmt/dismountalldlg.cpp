...
LONG DbgLevelMagmDlg = DBG_LEVEL_MAGM_DLG;

#define NdasUiDbgCall(l,x,...) do {							\
    if (l <= DbgLevelMagmDlg) {								\
...
CDismountAllDialog::PrepareEjectWorkItemCallback(ndas::LogicalDevicePtr LogDevice)
{
	...
	NdasUiDbgCall( 2, _T("dinfo.HardwareInfo.MaximumNumberOfTargets = %d\n"), dinfo.HardwareInfo.MaximumNumberOfTargets );
