	...
	public CInterAppMsgImpl<CMainFrame>
{
	...
	HANDLE m_AutoPnpMutex;

#define NDAS_MAX_AUTO_PNP 32

	UINT  m_NdasStringIdLen;
	PTCHAR m_NdasStringId[NDAS_MAX_AUTO_PNP];
...
public:
	...
	VOID OnNdasAutoPnp (TCHAR *NdasStringId);

private:

	void pWarnUninitalizedDisks(NDAS_LOGICALDEVICE_ID LogicalDeviceId);

	void pShowWelcome();
	void pShowInstPopup();
	HICON pCreateTaskbarIcon(bool NormalOrFailure = true);
	BOOL pUpdateDeviceList();
	void pUpdateMenuItems();
	void ErrorMessageBox(
		ATL::_U_STRINGorID Message, 
		ATL::_U_STRINGorID Title = IDS_ERROR_TITLE,
		DWORD ErrorCode = ::GetLastError());
	void pImportRegistration(LPCTSTR szFileName);
	bool pProcessRegistration(
		LPCTSTR DeviceName,
		LPCTSTR DeviceId,
		LPCTSTR WriteKey);

	void pChangeTaskBarIcon(bool Normal);
