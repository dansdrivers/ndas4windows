...
LONG DbgLevelMagmOption = DBG_LEVEL_MAGM_OPTION;

#define NdasUiDbgCall(l,x,...) do {							\
    if (l <= DbgLevelMagmOption) {							\
...
CAppOptAdvancedPage::OnInitDialog(HWND hwndFocus, LPARAM lParam)
{
	...
	fSuccess = ::NdasGetServiceParam( NDASSVC_PARAM_SUSPEND_PROCESS, &serviceParam );
	...
	m_tree.SetItemData( hItemSub, NDASSVC_PARAM_SUSPEND_PROCESS | NDASSVC_SUSPEND_DENY );
	...
	m_tiGroupSuspendAllow = m_tree.InsertItem( PropCreateOptionCheck(str), 2, 2, hItem );

	m_tree.SetItemData(m_tiGroupSuspendAllow, NDASSVC_PARAM_SUSPEND_PROCESS | NDASSVC_SUSPEND_ALLOW );

	if (m_dwSuspend == NDASSVC_SUSPEND_ALLOW) {

		m_tree.SetCheckState( m_tiGroupSuspendAllow, TRUE );

	} else {

		m_tree.SetCheckState( m_tiGroupSuspendAllow, FALSE );
	}

	// Auto Pnp Support options

	ZeroMemory( &serviceParam, sizeof(NDAS_SERVICE_PARAM) );
	
	fSuccess = ::NdasGetServiceParam( NDASSVC_PARAM_AUTO_PNP, &serviceParam );

	m_tiGroupAutoPnp = NULL;
	...
	m_dwAutoPnp = serviceParam.Value.DwordValue;

	NDASSVC_AUTO_PNP_DENY;
	NDASSVC_AUTO_PNP_ALLOW; //  == serviceParam.DwordValue;

	fSuccess = str.LoadString(IDS_OPTION_GROUP_AUTO_PNP);

	ATLASSERT(fSuccess);
	hItem = m_tree.InsertItem(PropCreateReadOnlyItem(str),13,13,TVI_ROOT);

	m_tiGroupAutoPnp = hItem;

	fSuccess = str.LoadString(IDS_OG_AUTO_PNP_DENY);
	ATLASSERT(fSuccess);
	
	hItemSub = m_tree.InsertItem(PropCreateOptionCheck(str),2,2,hItem);
	
	m_tree.SetItemData(hItemSub, IDS_OG_AUTO_PNP_DENY);
	
	if (m_dwAutoPnp == NDASSVC_AUTO_PNP_DENY) {
	...
	fSuccess = str.LoadString(IDS_OG_AUTO_PNP_ALLOW);
	ATLASSERT(fSuccess);

	hItemSub = m_tree.InsertItem(PropCreateOptionCheck(str),2,2,hItem);

	m_tree.SetItemData(hItemSub, NDASSVC_AUTO_PNP_ALLOW);
	
	if (m_dwAutoPnp == NDASSVC_AUTO_PNP_ALLOW) {
...
CAppOptAdvancedPage::OnTreeNotify(LPNMHDR pnmh)
{
	...
	if (prop->GetItemData() == (NDASSVC_PARAM_SUSPEND_PROCESS | NDASSVC_SUSPEND_ALLOW)) {
...
CAppOptAdvancedPage::OnApply()
{
	...
	if (m_tiGroupSuspend != NULL) {
		...
		for (;NULL != hItem; hItem = m_tree.GetNextSiblingItem(hItem)) {
			...
			if (m_tree.GetCheckState(hItem)) {
				...
				serviceParam.Value.DwordValue = (DWORD)(dwPtr) & ~NDASSVC_PARAM_SUSPEND_PROCESS;
	...
	if (m_tiGroupAutoPnp != NULL) {

		hItem = m_tree.GetChildItem(m_tiGroupAutoPnp);
