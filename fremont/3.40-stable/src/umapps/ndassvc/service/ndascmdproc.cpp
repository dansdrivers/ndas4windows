...
LONG DbgLevelSvcCmdProc = DBG_LEVEL_SVC_COM_PROC;

#define NdasUiDbgCall(l,x,...) do {							\
    if (l <= DbgLevelSvcCmdProc) {							\
	...
	LPDWORD lpcbWritten)
{
	...
	BOOL result = m_pTransport->Send( lpBuffer, cbToWrite, lpcbWritten, &m_overlapped );
	...
	DWORD cbExtraInData)
{
	...
	if (pRequest->DeviceIdOrSlot.UseSlotNo) {
		...
		if (pRequest->DeviceIdOrSlot.SlotNo) {
			...
			for (size_t index = 0; index < count; index++) {
				...
				if (NdasDeviceIsHidden()(pNdasDevice)) {
					...
					pRegistrar->Deregister(pNdasDevice);
				}
			}
		}
	...
	DWORD cbExtraInData)
{
	...
	COMVERIFY(pNdasLogicalUnit->get_MountedDriveSet(
		&pReply->LogicalDeviceParams.MountedLogicalDrives));

	COMVERIFY(pNdasLogicalUnit->get_Abnormalities(
		&pReply->LogicalDeviceParams.Abnormalities));
	...
	DWORD cbExtraInData)
{
	...
	COMVERIFY(pNdasLogicalUnit->get_Id(&reply.NdasLocation));
		...
		DWORD cbExtraInData)
{
	...
	CComPtr<INdasLogicalUnit> pNdasLogicalUnit;
	HRESULT hr = pGetNdasLogicalUnit(pRequest->NdasLocation, &pNdasLogicalUnit);
	...
	)
{
	...
	DWORD nHosts = (NDAS_HOST_COUNT_UNKNOWN == nROHosts) ? NDAS_MAX_CONNECTION_V11 : nROHosts + nRWHosts;

	NdasUiDbgCall( 2, "Expected: nHosts %d, nRoHost %d, nRwHost %d\n",
	...
	if (nHosts > 0) {
		...
		success = hixdisc.HixDiscover( unitDeviceId,uda, nHosts, 1000 );
		
		if (!success) {

			DWORD lastError = ::GetLastError();

			NdasUiDbgCall( 1, "fail HixDiscover %x\n", lastError );
			return ReplyCommandFailed( lastError );
		}
	...
	if (!success) {
		...
		NdasUiDbgCall( 1, "fail WriteHeader %x\n", ::GetLastError() );
		return FALSE;
	}
	...
	if (!success) {
		...
		NdasUiDbgCall( 1, "fail WritePacket %x\n", ::GetLastError() );
		return FALSE;
	}

	NdasUiDbgCall( 1, "nHosts %x\n", nHosts );
	...
	for (DWORD i = 0; i < nHosts; ++i) {
		...
		success = hixdisc.GetHostData( unitDeviceId,
									   i,
									   &hostUda,
									   &hostEntry.HostGuid,
									   NULL,
		...
		if (!success) {
			...
			DWORD lastError = ::GetLastError();
			NdasUiDbgCall( 1, "fail GetHostData %x\n", lastError );
			// warning!!!
		}
...
CNdasCommandProcessor::ProcessCommandRequest<
	...
	DWORD cbExtraInData)
{
	if (pRequest->ParamCode == NDASSVC_PARAM_SUSPEND_PROCESS) {
	...
	if (pRequest->ParamCode == NDASSVC_PARAM_AUTO_PNP) {

		NdasCmdReply<NDAS_CMD_GET_SERVICE_PARAM> reply;

		DWORD dwValue = NdasServiceConfig::Get(nscAutoPnp);

		reply.Param.ParamCode = NDASSVC_PARAM_AUTO_PNP;
		reply.Param.Value.DwordValue = dwValue;
...
CNdasCommandProcessor::ProcessCommandRequest<
	...
	DWORD cbExtraInData)
{
	if (pRequest->Param.ParamCode == NDASSVC_PARAM_SUSPEND_PROCESS) {

		if (pRequest->Param.Value.DwordValue == NDASSVC_SUSPEND_DENY ||
			pRequest->Param.Value.DwordValue == NDASSVC_SUSPEND_ALLOW) {
	...
	if (pRequest->Param.ParamCode == NDASSVC_PARAM_AUTO_PNP) {

		if (pRequest->Param.Value.DwordValue == NDASSVC_AUTO_PNP_DENY ||
			pRequest->Param.Value.DwordValue == NDASSVC_AUTO_PNP_ALLOW) {

			NdasServiceConfig::Set(nscAutoPnp, pRequest->Param.Value.DwordValue);
			return ReplySuccessDummy();
		} 
	}

	if (pRequest->Param.ParamCode == NDASSVC_PARAM_RESET_SYSKEY) {
...
CNdasCommandProcessor::Process()
{
	...
	if (m_requestHeader.VersionMajor != NDAS_CMD_PROTOCOL_VERSION_MAJOR ||
		...
		if (!success) {
			...
			XTLTRACE2( NDASSVC_CMDPROCESSOR, TRACE_LEVEL_ERROR, "HPVM:Fin\n" );
...
#define CASE_PROCESS_COMMAND(cmdType, _CMD_STRUCT_) \
	...
	NdasUiDbgCall( 3, _T("m_command = 0x%X\n"), m_command );
