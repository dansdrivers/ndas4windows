...
LONG DbgLevelSvcEvent = DBG_LEVEL_SVC_EVENT;

#define NdasUiDbgCall(l,x,...) do {							\
    if (l <= DbgLevelSvcEvent) {							\
...
BOOL CNdasEventPublisher::NdasAutoPnp (TCHAR *NdasStringId) {
	
	NDAS_EVENT_MESSAGE msg = {0};

	msg.MessageSize = sizeof(NDAS_EVENT_MESSAGE);
	msg.EventType	= NDAS_EVENT_TYPE_AUTO_PNP;

	CopyMemory( msg.AutoPnpInfo.NdasStringId, NdasStringId, sizeof(msg.AutoPnpInfo.NdasStringId) );

	NdasUiDbgCall( 2, _T("%s\n"), msg.AutoPnpInfo.NdasStringId );
