...
int _dvdcss_open ( dvdcss_t dvdcss )
{
    ...
    _snprintf_s( psz_debug,200,_TRUNCATE,"opening target `%s'", psz_device );
    psz_debug[199] = '\0';
...
static int libc_open ( dvdcss_t dvdcss, char const *psz_device )
{
	int fd = -1;
	errno_t err =0;
	err = _sopen_s(&fd,psz_device, O_BINARY, _SH_DENYNO , (_S_IREAD | _S_IWRITE)  );
	if(err != 0) {
		_dvdcss_error( dvdcss, "failed opening device" );
        return -1;
	}

	dvdcss->i_fd = dvdcss->i_read_fd = fd;
    dvdcss->i_pos = 0;
...
{
    char psz_dvd[16];
    _snprintf_s( psz_dvd, 7,_TRUNCATE, "\\\\.\\%c:", psz_device[0] );
...
static int aspi_open( dvdcss_t dvdcss, char const * psz_device )
{
    ...
    MYGETSUPPORT lpGetSupport;
    MYSENDCOMMAND lpSendCommand;
    ...
    lpGetSupport = (MYGETSUPPORT) GetProcAddress( hASPI, "GetASPI32SupportInfo" );
    lpSendCommand = (MYSENDCOMMAND) GetProcAddress( hASPI, "SendASPI32Command" );
    ...
    fd = (struct w32_aspidev *)malloc( sizeof( struct w32_aspidev ) );
    if( fd == NULL )
    {
...
static int libc_seek( dvdcss_t dvdcss, int i_blocks )
{
    ...
    i_seek = _lseeki64( dvdcss->i_read_fd,
...
static int libc_read ( dvdcss_t dvdcss, void *p_buffer, int i_blocks )
{
    ...
    i_ret = _read( dvdcss->i_read_fd, p_buffer,
    ...
    if( i_ret != i_blocks * DVDCSS_BLOCK_SIZE )
    {
        int i_seek;
...
static int libc_readv ( dvdcss_t dvdcss, struct iovec *p_iovec, int i_blocks )
{
   int i_index, i_len, i_total = 0;
         ...
         i_index--, p_iovec++ )
    {
        ...
        i_bytes = _read( dvdcss->i_fd, p_base, i_len );
...
static int aspi_read_internal( int i_fd, void *p_data, int i_blocks )
{
    ...
    ssc.SRB_PostProc = (unsigned long *)(LPVOID) hEvent;
    ssc.SRB_BufPointer  = (unsigned char *)p_data;
    ssc.SRB_CDBLen      = 12;
