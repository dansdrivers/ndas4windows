...
typedef struct _NDAS_EXT_KEY {
	BYTE Key1[8];
	BYTE Key2[8];
} NDAS_EXT_KEY, *PNDAS_EXT_KEY;

typedef struct _NDAS_EXT_DATA {
	BYTE Seed;
	BYTE VID;
	BYTE Reserved[2];
} NDAS_EXT_DATA, *PNDAS_EXT_DATA;
...
/* <COMBINE NdasValidateStringIdKeyExtW> */
...
/*DOM-IGNORE-BEGIN*/
#ifdef UNICODE
#define NdasValidateStringIdKeyEx NdasValidateStringIdKeyExW
#else
#define NdasValidateStringIdKeyEx NdasValidateStringIdKeyExA
#endif
...
Parameters:
	...
	bIgnoreWarning:
	[in] Set as TRUE to ignore mount warning, otherwise set as FALSE.
...
Summary
	...
	Access mask is GENERIC_READ or GENERIC_READ | GENERIC_WRITE.
	Lower bits contains specific access rights when 
	NDAS_ACCESS_BIT_EXTENDED is set.
...
typedef struct _NDAS_EVENT_INFO {
	...
	union {
		...
		NDAS_EVENT_ATUO_PNP_INFO			AutoPnpInfo;
...
Summary
	...
	NdasQueryNdasLocation queries the NDAS location of 
...
NdasQueryNdasLocation(
	IN NDAS_LOGICALDEVICE_ID LogicalDeviceId,
	OUT PNDAS_LOCATION pNdasLocation);
...
#ifdef MSC_DEPRECATE_SUPPORTED
#pragma deprecated(NdasQueryNdasScsiLocation)
#endif
