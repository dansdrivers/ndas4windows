...
struct AutoHKEYConfig
{
	...
	static void Release(HKEY hKey) 
	{
		XTL_SAVE_LAST_ERROR();
		//
		// Here we also ignores NULL regarding it as an invalid value,
		// as some Windows functions (e.g. RegOpenKeyEx) sets KeyHandle
		// NULL instead of INVALID_HANDLE_VALUE in error cases
		//
		if (NULL == hKey) return;
