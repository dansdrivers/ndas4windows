	...
	__out LPDWORD NdasSlotNumber)
{
	...
	HRESULT hr = pGetScsiAddressForDisk(DiskHandle, &scsiAddress);
	if (FAILED(hr))
	{
	...
	__in HANDLE hVolume)
{
	...
	for (DWORD i = 0; i < extents->NumberOfDiskExtents; ++i)
	{
		...
		//
#define NDASVOL_IMPL_USE_SCSI_ADDRESS
#ifdef NDASVOL_IMPL_USE_SCSI_ADDRESS
		SCSI_ADDRESS scsiAddress = {0};
		...
		{
			XTLTRACE2(NdasVolTrace, 2,
				"PhysicalDrive%d does not seem to an NDAS SCSI.\n", 
				diskExtent->DiskNumber);
			continue;
		}
