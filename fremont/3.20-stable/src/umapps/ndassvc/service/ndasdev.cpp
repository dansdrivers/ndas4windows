...
namespace
{
	...
	struct AutoMountUnitDevice : std::unary_function<CNdasUnitDevicePtr,void> {
		void operator()(const CNdasUnitDevicePtr& pUnitDevice) const {
			if(pUnitDevice->GetLogicalDevice() != CNdasLogicalDeviceNullPtr) {
				pUnitDevice->GetLogicalDevice()->AutoPlugIn();
			}
		}
	};
...
CNdasDevice::CNdasDevice(
	...
	CStringizerA<32>("{%03d}%s", SlotNo, CNdasDeviceId(DeviceId).ToStringA())
{
	XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_INFORMATION,
		"CNdasDevice::ctor(), slot=%d, deviceId=%s\n", 
		SlotNo,
	...
	XTLVERIFY(SUCCEEDED(
		::StringCchPrintf(m_szCfgContainer, 30, _T("Devices\\%04d"), SlotNo)));
...
{
	XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_INFORMATION,
		"CNdasDevice::dtor(), slot=%d, deviceId=%s\n",
		m_dwSlotNo, CNdasDeviceId(m_deviceId).ToStringA());
	...
	LPCTSTR szName, LPCVOID lpValue, DWORD cbValue)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_WARNING,
			...
			szName, m_szCfgContainer, GetLastError());
	}
	return fSuccess;
}
...
CNdasDevice::DeleteConfigValue(LPCTSTR szName)
{
	...
	if (!fSuccess)
	{
		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_WARNING,
			...
			m_szCfgContainer, szName, GetLastError());
	}
	return fSuccess;
}
...
CNdasDevice::GetAllowingAccess()
{
	...
	// TODO: Refine this!
	//
	// (This comment is copied from CNdasUnitDevice::GetAllowingAccess)
	return granted;
}
...
CNdasDevice::UpdateStats()
{
	...
	{
		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_ERROR,
			"CreateLpxSocketAddressList failed, error=0x%X\n", GetLastError());
		return FALSE;
	}
	...
	{
		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_ERROR,
			"NdasCommGetDeviceStat failed, error=0x%X\n", GetLastError());
		return FALSE;
	}
	...
	const LPX_ADDRESS& remoteAddress)
{
	...
	{
		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_ERROR,
			"Discovery failed, error=0x%X\n", GetLastError());
		_SetLastDeviceError(NDAS_DEVICE_ERROR_CONNECTION_FAILED);
		return FALSE;
	}
	...
	if (!fSuccess) 
	{
		...
		//
		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_ERROR,
			"NdasCommGetDeviceHardwareInfo failed, error=0x%X\n", GetLastError());
		_SetLastDeviceError(NDAS_DEVICE_ERROR_DISCOVER_FAILED);
		return FALSE;
	}
...
CNdasDevice::_CreateUnitDevice(DWORD UnitNo)
{
	...
	for (DWORD i = 0; i < dwMaxFailure + 1; ++i)
	{
		...
		::Sleep(dwInterval);
		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_ERROR,
			"Creating a unit device instance failed (%d out of %d), error=0x%X\n", 
			i, dwMaxFailure, GetLastError());
	}
...
CNdasDevice::InvalidateUnitDevice(DWORD UnitNo)
{
	...
	{
		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_INFORMATION,
			"%s: Non-connected device ignored\n", ToStringA());
		return FALSE;
	}
	...
	if (!fSuccess) 
	{
		...
		{
			XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_INFORMATION,
				"%s: Changing to DISCONNECTED.\n", ToStringA());
			//
			// InvalidateUnitDevice can be called at any time. 
			// So _GetDeviceInfo can be failed even if svc recognize it as connected.
			// Just reset state to restart
			//
			_SetLastDeviceError(NDAS_DEVICE_ERROR_NONE);
			_ChangeStatus(NDAS_DEVICE_STATUS_DISCONNECTED);
		}
	...
	if (m_dstat.UnitDevices[UnitNo].IsPresent)
	{
		...
		{
			XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_ERROR,
				"%s: Creating unit device (%d) failed, error=0x%X\n", 
				ToStringA(), UnitNo, GetLastError());
			return FALSE;
		}

		XTLTRACE2(NDASSVC_NDASDEVICE, TRACE_LEVEL_INFORMATION,
			"%s: Unit Device (%d) recreated\n", ToStringA(), UnitNo);
	}
