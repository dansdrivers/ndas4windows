...
//
// Plug in if the logical device is set to mount on ready.
//
BOOL
CNdasLogicalDevice::AutoPlugIn() {
	InstanceAutoLock autolock(this);
	BOOL fSuccess = FALSE;
	...
	if (IsComplete() && !m_fDegradedMode) {
		...
		if ((m_mountOnReadyAccess & GetAllowingAccess()) == m_mountOnReadyAccess)
		{
			...
			{
				XTLTRACE2(NDASSVC_NDASLOGDEVICE, TRACE_LEVEL_INFORMATION,
					"Auto mount (%s) succeeded.\n", this->ToStringA());
			}
			else
			{
				XTLTRACE2(NDASSVC_NDASLOGDEVICE, TRACE_LEVEL_ERROR,
					"Auto mount (%s) failed, error=0x%X\n", 
...
//			pUnitDevice->OnMounted();
		...
		//
		// Initiate auto-mount if necessary
		// conditions are evaluated in the AutoPlugIn function
		//
		AutoPlugIn();
	}
...
CNdasLogicalDevice::Unplug()
{
	...
	{
		XTLTRACE2(NDASSVC_NDASLOGDEVICE, TRACE_LEVEL_ERROR,
			"NdasBusCtlUnplug failed, error=0x%X\n", GetLastError());
...
CNdasLogicalDevice::Eject()
{
	...
	BOOL fSuccess = ::NdasBusCtlEject(m_NdasLocation);
	if (!fSuccess) 
	{
		XTLTRACE2(NDASSVC_NDASLOGDEVICE, TRACE_LEVEL_ERROR,
	...
	LPBOOL lpbPrimary)
{
	...
	//BusEnumQuery.SlotNo = m_NdasLocation.SlotNo;
...
CNdasLogicalDevice::OnUnmounted()
{
	...
	if (!m_fDisconnected)
	{
		...
		//
		// when the unit is dismounted as per the user's request,
		// we should not mount the unit again.
		//
		SetMountOnReady(0);
	}
...
CNdasLogicalDevice::OnPeriodicCheckup()
{
	...
	BOOL fSuccess = ::NdasBusCtlQueryNodeAlive(
		location, 
		&fAlive, 
		&fAdapterError);
