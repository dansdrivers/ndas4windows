    ...
    )
{
	...
	NDASFS_ASSERT( LfsDeviceExt->FileSpyDeviceObject );

#else

	NDASFS_ASSERT( LfsDeviceExt->InstanceContext || LfsDeviceExt->FileSpyDeviceObject );
	...
	NDASFS_ASSERT( !FlagOn(lfsDeviceExt->Flags, LFS_DEVICE_FLAG_CLNEANUPED) );
	...
	)
{
	...
	NDASFS_ASSERT( !FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_CLNEANUPED) );
	...
	try {
		...
		if (irpSp->MajorFunction == IRP_MJ_WRITE) {
			...
			if (DevExt->LfsDeviceExt.Secondary == NULL ||
				Secondary_LookUpFileExtension(DevExt->LfsDeviceExt.Secondary, fileObject) == NULL) {
		...
		if (irpSp->MajorFunction == IRP_MJ_CREATE) {
			...
			if (fileObject->FileName.Length >= 16) {
				...
				if (RtlEqualUnicodeString( &Extend, &tempUnicode, TRUE)) {
					...
					NDASFS_ASSERT( FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) );
		...
		if (FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTING)) {
			...
			NDASFS_ASSERT( DevExt->LfsDeviceExt.Secondary == NULL || 
						   Secondary_LookUpFileExtension(DevExt->LfsDeviceExt.Secondary, fileObject) == NULL );
		...
		NDASFS_ASSERT( FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) && !FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTED) );

		if (FlagOn(DevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_SURPRISE_REMOVED)) {

			if (Secondary_LookUpFileExtension(DevExt->LfsDeviceExt.Secondary, fileObject)) {
		...
		if (irpSp->MajorFunction == IRP_MJ_PNP) {
			...
			case IRP_MN_SURPRISE_REMOVAL: {
				...
				NDASFS_ASSERT( DevExt->LfsDeviceExt.NetdiskPartition );
			...
			case IRP_MN_QUERY_REMOVE_DEVICE: {
				...
				Secondary_TryCloseFilExts( DevExt->LfsDeviceExt.Secondary );
				...
				NDASFS_ASSERT( DevExt->LfsDeviceExt.AttachedToDeviceObject );
				...
				NDASFS_ASSERT( DevExt->LfsDeviceExt.ProcessingIrpCount == 1 );
			...
			if (irpSp->MajorFunction == IRP_MJ_CREATE) {
				...
				if (Irp->RequestorMode == KernelMode &&
					(fileObject->RelatedFileObject == NULL ||
					 Secondary_LookUpFileExtension(DevExt->LfsDeviceExt.Secondary, fileObject->RelatedFileObject) == NULL)) {
		...
		result = SecondaryNtfsPassThrough( DevExt->LfsDeviceExt.Secondary, Irp, &status );
	...
	)
{
	...
	NDASFS_ASSERT( LfsDeviceExt->SecondaryState == SECONDARY_STATE );
	...
	)
{
	...
	if (ModeChange == FALSE) {
		...
		NDASFS_ASSERT( netdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumePreMounting ||
	...
	NDASFS_ASSERT( status != STATUS_NO_SUCH_DEVICE );
