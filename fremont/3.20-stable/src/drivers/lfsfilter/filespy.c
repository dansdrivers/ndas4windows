...
do {
	...
	if (status == STATUS_VOLUME_MOUNTED) {
		...
		NDASFS_ASSERT( GlobalLfs.NdasFsMiniFilterMode == TRUE );
		NDASFS_ASSERT( netdiskEnableMode != NETDISK_READ_ONLY );

		break;
	}

	NDASFS_ASSERT( GlobalLfs.NdasFsMiniFilterMode == FALSE || netdiskEnableMode == NETDISK_READ_ONLY );
