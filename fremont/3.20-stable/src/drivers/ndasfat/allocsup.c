...
#if __NDAS_FAT__
		NDASFAT_ASSERT( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode != NETDISK_SECONDARY				||
						!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_MOUNTED)	||
    ...
    try {
        ...
        if (FcbOrDcb->Header.AllocationSize.LowPart == 0) {
            ...
            FatAllocateDiskSpace( IrpContext,
                                  Vcb,
                                  FatGetIndexFromLbo(Vcb, IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->SetFile.AllocationInformation.Lcn),
                                  &DesiredAllocationSize,
                                  TRUE,
                                  &FcbOrDcb->Mcb );
		...
		if (NodeType(FcbOrDcb) == FAT_NTC_FCB) {
			...
			ULONG				newIndex;
			ULONG				newClusterCount;
			VBO					newVcn;
			LBO					newLcn;

			vcn = 0; index = 0;

			while (FatLookupMcbEntry(FcbOrDcb->Vcb, &FcbOrDcb->Mcb, vcn, &lcn, &clusterCount, NULL)) {

				vcn += clusterCount;
				index++;
			}

			NDASFAT_ASSERT( index * sizeof(NDFS_FAT_MCB_ENTRY) <= DEFAULT_NDAS_MAX_DATA_SIZE ); 

			if (FcbOrDcb->Header.AllocationSize.QuadPart == 0) {

				ASSERT( vcn == NewAllocation );
			
			} else {

				ASSERT( vcn == FcbOrDcb->Header.AllocationSize.QuadPart );

				newVcn = 0; newIndex = 0;

				while (FatLookupMcbEntry(FcbOrDcb->Vcb, &NewMcb, newVcn, &newLcn, &newClusterCount, NULL)) {

					newVcn += newClusterCount;
					newIndex++;
				}

				NDASFAT_ASSERT( newIndex * sizeof(NDFS_FAT_MCB_ENTRY) <= DEFAULT_NDAS_MAX_DATA_SIZE ); 

				ASSERT( newVcn == NewAllocation );
		
				if ((index+newIndex) * sizeof(NDFS_FAT_MCB_ENTRY) > DEFAULT_NDAS_MAX_DATA_SIZE) {
