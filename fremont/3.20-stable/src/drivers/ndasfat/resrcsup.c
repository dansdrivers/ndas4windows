...

{
	...
	if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {
		...
		NDASFAT_ASSERT( !FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) );
	...
	if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
		...
		if (ExAcquireResourceExclusiveLite( &Vcb->SecondaryResource, BooleanFlagOn(IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT))) {
...

{
	...
	if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
	    ...
	    if (ExAcquireResourceSharedLite( &Vcb->SecondaryResource, 
		                                 BooleanFlagOn(IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT))) {
