							...
							try {
								...
								if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION)) {
									...
									for (primarySession = NULL, primarySessionListEntry = VolumeDeviceObject->PrimarySessionQueue.Flink; 
										...
										if (IrpContext->FileOpenPrimarySession == primarySession)
											break;
									}
								...
								NDASNTFS_ASSERT( ExIsResourceAcquiredSharedLite(&VolumeDeviceObject->Resource) );
...
#define FlushScb(IRPC,SCB,IOS) {                                                \
    ...
    if (FlagOn((SCB)->ScbState, SCB_STATE_FILE_SIZE_LOADED)) {                  \
		...
		if (NT_SUCCESS(Status) && (*ThisCcb) && (*ThisCcb)->TypeOfOpen == UserFileOpen) {
			...
			if (IrpContext->PrimaryRequestInfo.PrimarySession)
				ASSERT( NtfsSegmentNumber( &ThisFcb->FileReference ) >= FIRST_USER_FILE_NUMBER );

			DebugTrace( 0, Dbg, ("(*ThisScb)->PrimarySession = %x, IrpContext->PrimaryRequestInfo.PrimarySession = %p\n",
								  (*ThisScb)->PrimarySession, IrpContext->PrimaryRequestInfo.PrimarySession) );
			...
			if ((*ThisScb)->PrimarySession != IrpContext->PrimaryRequestInfo.PrimarySession && (*ThisScb)->CloseCount != 1) {

				ASSERT( (*ThisScb)->CloseCount > 1 && (*ThisScb)->CleanupCount >= 1 );

				if ((*ThisScb)->CleanupCount != 1) {
					
					SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION );
					IrpContext->FileOpenPrimarySession = (*ThisScb)->PrimarySession;
					
					NtfsRaiseStatus( IrpContext, STATUS_CANT_WAIT, NULL, NULL );
				}

				ASSERT( ExIsResourceAcquiredSharedLite((*ThisScb)->Fcb->Resource) );	

				NtfsLockFcb( NULL, (*ThisScb)->Fcb );

				for (unclosedCcbCount = 0, ccbListEntry = (*ThisScb)->CcbQueue.Flink; 
			...
			ASSERT( (*ThisScb)->PrimarySession == IrpContext->PrimaryRequestInfo.PrimarySession || 
			...
			if ((*ThisScb)->PrimarySession != 0)
				SetFlag( (*ThisCcb)->NdasNtfsFlags, ND_NTFS_CCB_FLAG_OPEN_BY_PRIMARY_SESSION );
			...
			if ((*ThisCcb) && (*ThisCcb)->TypeOfOpen == UserFileOpen) {
				...
				if (FlagOn((*ThisScb)->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT)) {
					...
					if((*ThisScb)->AttributeName.Length != 0) {
						...
						DebugTrace( 0, Dbg, ("FileName = %Z, ThisScb->AttributeName = %Z, ThisScb->AttFlagOn(ThisScb->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT) = %d\n", 
											  &(*ThisCcb)->FullFileName, &(*ThisScb)->AttributeName, BooleanFlagOn((*ThisScb)->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT)) );
					}
					else {
                    
						DebugTrace( 0, Dbg, ("FileName = %Z, %Z, ThisCcb->TypeOfOpen = %d, FlagOn( CreateFlags, CREATE_FLAG_OPEN_BY_ID ) = \n", 
...
#endif
						DebugTrace( 0, Dbg, ("ThisScb->AttributeName = %Z, ThisScb->AttFlagOn(ThisScb->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT) = %d\n", 
				...
				ASSERT( !FlagOn((*ThisScb)->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT) ); 
