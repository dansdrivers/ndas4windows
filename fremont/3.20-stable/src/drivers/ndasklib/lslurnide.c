...
//		ASSERT(FALSE);
	...
	status = LspReconnect(&IdeExt->LanScsiSession,  &shortTimeout);
...
LurnIdeDestroy(
	...
	ASSERT(KeGetCurrentIrql() == PASSIVE_LEVEL);
	KDPrintM(DBG_LURN_ERROR,  ("Lurn %p IdeExt %p\n", Lurn, IdeDev));

	if(IdeDev) {
		KDPrintM(DBG_LURN_TRACE,  ("CntEcrBuffer %p EncryptBufferLength %d WriteCheckBuffer %p\n",
			IdeDev->CntEcrBuffer, IdeDev->CntEcrBufferLength, IdeDev->WriteCheckBuffer));
		KDPrintM(DBG_LURN_TRACE,  ("LSS.EncryptBuffer %p LSS.EncryptBufferLength %d\n",
			IdeDev->LanScsiSession.EncryptBuffer, IdeDev->LanScsiSession.EncryptBufferLength));
		...
		//
		// Clean up encryption buffer again just in case.
		//
		if(IdeDev->LanScsiSession.EncryptBuffer && IdeDev->LanScsiSession.EncryptBufferLength
			) {
			ExFreePoolWithTag(IdeDev->LanScsiSession.EncryptBuffer, LSS_ENCRYPTBUFFER_POOLTAG);
			IdeDev->LanScsiSession.EncryptBuffer = NULL;
			IdeDev->LanScsiSession.EncryptBufferLength = 0;
		}
	}
