...
#define LPX_DEFAULT_SMP_TIMER_INTERVAL			(150*MILIHZ)
...
__inline
PLPX_RESERVED
RESERVED(PNDIS_PACKET	Packet) {
	PLPX_RESERVED	reserved = (PLPX_RESERVED)(Packet->ProtocolReserved);

	if(reserved->ExternalReserved == NULL) {
		return (PLPX_RESERVED)Packet->ProtocolReserved;
	} else {
