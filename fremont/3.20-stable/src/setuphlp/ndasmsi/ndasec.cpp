...
NDMsiSetARFlags(MSIHANDLE hInstall)
{
	...
	const static NDAS_DEVICE_ID RUTTER_AR_BEGIN = { 0x00, 0x0B, 0xD0, 0x20, 0x00, 0x00 };
	const static NDAS_DEVICE_ID RUTTER_AR_END =   { 0x00, 0x0B, 0xD0, 0x21, 0xFF, 0xFF };
	const static ACCESS_MASK RUTTER_AR_ACCESS = GENERIC_READ | GENERIC_WRITE;
	...
	else if (0 == ::lstrcmpi(_T("RUTTER"), pszScopeName)) 
	{
		scopeBegin = RUTTER_AR_BEGIN;
		scopeEnd = RUTTER_AR_END;
		grantedAccess = RUTTER_AR_ACCESS;
	}
	else 
	{
