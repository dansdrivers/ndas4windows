...

{
	...
	NDASNTFS_ASSERT( !FlagOn(Scb->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY) );
...

{
	...
	NDASNTFS_ASSERT( IrpContext->Vcb == NULL || CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode != NETDISK_SECONDARY );
...

{
    ...
    try {
		...
		NDASNTFS_ASSERT( IrpContext->Usn.CurrentUsnFcb == NULL && IrpContext->TransactionId == 0 ||
