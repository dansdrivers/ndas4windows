...
#if WINVER >= 0x0501
#define LpxDbgBreakPoint()	(KD_DEBUGGER_ENABLED ? DbgBreakPoint() : TRUE)
#else
#define LpxDbgBreakPoint()	((*KdDebuggerEnabled) ? DbgBreakPoint() : TRUE) 
#endif

#if DBG

#define LPX_ASSERT( exp )	ASSERT( exp )

#else

#define LPX_ASSERT( exp )	\
	((!(exp)) ?				\
	LpxDbgBreakPoint() :	\
	TRUE)
