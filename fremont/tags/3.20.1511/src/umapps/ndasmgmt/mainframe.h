	...
	public CInterAppMsgImpl<CMainFrame>
{
	...
	CFootprintWindow m_wndFootprint;
...
public:
	...
	BEGIN_MSG_MAP_EX(CMainFrame)
		...
		COMMAND_ID_HANDLER_EX(IDR_UNMOUNT_ALL, OnCmdUnmountAll)
	...
	void OnCmdUnmountAll(UINT wNotifyCode, int wID, HWND hWndCtl);

	void OnCmdShowDeviceProperties(DWORD dwSlotNo);
	void OnCmdEnableDevice(DWORD dwSlotNo);
	void OnCmdDisableDevice(DWORD dwSlotNo);
	void OnCmdMountDeviceRW(NDAS_LOGICALDEVICE_ID logDeviceId);
	void OnCmdMountDeviceRO(NDAS_LOGICALDEVICE_ID logDeviceId);
	void OnCmdUnmountDevice(NDAS_LOGICALDEVICE_ID logDeviceId);
