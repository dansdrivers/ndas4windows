...
#if (DBG || __LFS_ALLOW_UNLOAD_DRIVER__) && WINVER >= 0x0501
...
//
...
#endif
	{
		...
		ntfsStatus = IoGetDeviceObjectPointer( &ntfsName,
											   FILE_ALL_ACCESS,
											   &ntfsFileObject,
											   &ntfsDeviceObject );
		...
		ftpStatus = IoGetDeviceObjectPointer( &ftpName,
											  FILE_ALL_ACCESS,
											  &ftpFileObject,
											  &ftpDeviceObject );
...
#endif
	{
		...
		PDEVICE_OBJECT		fatDeviceObject;
		NTSTATUS			fatStatus;


		RtlInitUnicodeString( &ntfsName, NDAS_NTFS_DEVICE_NAME );
		...
		RtlInitUnicodeString( &fatName, NDAS_FAT_DEVICE_NAME );

		fatStatus = IoGetDeviceObjectPointer( &fatName,
											  FILE_ALL_ACCESS,
											  &fatFileObject,
											  &fatDeviceObject );

		if (NT_SUCCESS(fatStatus)) {

			SpyAttachToFileSystemDevice( fatDeviceObject, &fatName );
			ObDereferenceObject( fatFileObject );
		}

		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, 
			           ("fatStatus = %x, fatDeviceObject = %p\n", fatStatus, fatDeviceObject) );
	}
...
#if 0 // Only used when we want to print debug message at completion time
NTSTATUS 
LfsPostIrpRoutine(
    IN PIRP Irp,
...
)
{
	...
	UNREFERENCED_PARAMETER(IsCompletion);

	irpSp = IoGetCurrentIrpStackLocation(Irp);    

	// Show message if IRP operation is failed.
/*	if (Irp->IoStatus.Status != STATUS_SUCCESS) */{
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE,
				("LfsPostIrpRoutine: %s %s %s File=%s, LfsExt=%p, IoStatus=%x, Info=%x\n",
				IsCompletion?"Completion":"Direct",
				irpMajorString, irpMinorString, NameBuf, lfsDeviceExt, Irp->IoStatus.Status, Irp->IoStatus.Information));
#if 0
		if (irpSp->MajorFunction == IRP_MJ_DIRECTORY_CONTROL &&
			irpSp->MinorFunction == IRP_MN_QUERY_DIRECTORY) {
			PrintFileInfoClass(irpSp->Parameters.QueryDirectory.FileInformationClass, 
...
NTSTATUS
LfsPassThroughCompletionRoutine (
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp,
    IN PVOID Context
	)
{
	...
	LfsPostIrpRoutine(Irp, Context, TRUE);
...
--*/
{
	...
	if (irpSp->MajorFunction == IRP_MJ_SHUTDOWN) {
		...
		if (devExt->LfsDeviceExt.FilteringMode == LFS_READONLY) {
			...
			result = ReadonlyPassThrough( DeviceObject, Irp, devExt, &ntStatus );
#else
			result = LfsPassThrough( DeviceObject, Irp, devExt, &ntStatus );
#endif
	...
	ASSERT( oldIrql2 == KeGetCurrentIrql() );
...

{
	...
	if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT && !GlobalLfs.NdasFatRwSupport && !GlobalLfs.NdasFatRoSupport ||
		devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS && !GlobalLfs.NdasNtfsRwSupport && !GlobalLfs.NdasNtfsRoSupport) {
	...
	case NETDISK_READ_ONLY:
		...
		} else if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT) {
		...
		} else if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NTFS) {
		...
		} else if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS) {
    ...
    if (!NT_SUCCESS( status )) 
	{
        ...
        SPY_LOG_PRINT( SPYDEBUG_ERROR,
                       ("FileSpy!SpyFsControlMountVolume: Error creating volume device object, status=%08x\n", 
                        status));
//
//		Do not call SpyPassThrough().
//		LfsPassThrough() will referenece to uninitialized Lfs Extenstion.
//
...
//        return SpyPassThrough(DeviceObject, Irp);
	...
	newDevExt->LfsDeviceExt.Flags = LFS_DEVICE_INITIALIZING;

	newDevExt->LfsDeviceExt.FileSpyDeviceObject	= newDeviceObject;
	newDevExt->LfsDeviceExt.FileSpyDevExt		= newDevExt;
	newDevExt->LfsDeviceExt.NetdiskPartition	= netdiskPartition;
	newDevExt->LfsDeviceExt.NetdiskEnabledMode	= netdiskEnableMode;

	newDevExt->LfsDeviceExt.Purge			= FALSE;
	newDevExt->LfsDeviceExt.HookFastIo		= FALSE;		//Allow FastIo to pass through
	newDevExt->LfsDeviceExt.Filtering		= FALSE;
    ...
    if (IS_WINDOWSXP_OR_LATER()) {
        ...
        SPY_COMPLETION_CONTEXT_WXP_OR_LATER completionContextXP;
        
        IoCopyCurrentIrpStackLocationToNext ( Irp );

        completionContextXP.RecordList = recordList;
        KeInitializeEvent( &completionContextXP.WaitEvent, 
                           NotificationEvent, 
                           FALSE );

        IoSetCompletionRoutine( Irp,
                                SpyFsControlCompletion,
                                &completionContextXP,     //context parameter
                                TRUE,
                                TRUE,
                                TRUE );
    	...
    	if (STATUS_PENDING == status) {
    		...
    		status = KeWaitForSingleObject( &completionContextXP.WaitEvent,
    		                                Executive,
    		                                KernelMode,
    		                                FALSE,
    		                                NULL );
        ...
        ASSERT(KeReadStateEvent(&completionContextXP.WaitEvent) ||
               !NT_SUCCESS(Irp->IoStatus.Status));
    ...
    )
{
	...
	RtlInitUnicodeString( &newLfsDeviceExt->MountPoint, L"\\System Volume Information\\MountPointManagerRemoteDatabase" );	
...
--*/
{
	...
	ASSERT( newDevExt->LfsDeviceExt.ReferenceCount == 2 );
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE,
					("%s: newDevExt = %p %wZ Irp->IoStatus.Status = %x, vpb->RealDevice = %p, vpb->DeviceObject = %p\n", 
					 __FUNCTION__, newDevExt, &newDevExt->DeviceName, Irp->IoStatus.Status, 
    ...
    if (NT_SUCCESS(Irp->IoStatus.Status)) 
	{
        ...
        if (!SpyIsAttachedToDevice( vpb->DeviceObject, &attachedDeviceObject )) {
            ...
            if (!NT_SUCCESS( status )) {
				...
				LfsIoDeleteDevice( NewDeviceObject );
#else
				...
				SetFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_MOUNTING );
				...
				case LFS_READONLY: {
					...
					ClearFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_INITIALIZING|LFS_DEVICE_MOUNTING );
					SetFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_START );
					...
					newDevExt->LfsDeviceExt.Filtering = TRUE;
					newDevExt->LfsDeviceExt.HookFastIo = TRUE;		// disallow FastIo to pass through the next device.
					...
					newDevExt->LfsDeviceExt.AttachedToDeviceObject = newDevExt->AttachedToDeviceObject;
...
#if __LFS_READONLY_PURGE_VERSION__
					status = SpyFsControlReadonlyMountVolumeComplete( NewDeviceObject );
#else
					status = STATUS_SUCCESS;
#endif
				...
				case LFS_PRIMARY: {
					...
					newDevExt->LfsDeviceExt.Filtering = TRUE;
					newDevExt->LfsDeviceExt.HookFastIo = FALSE;		// allow FastIo to pass through the next device.
					ClearFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_INITIALIZING|LFS_DEVICE_MOUNTING );
					SetFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_START );
				...
				case LFS_SECONDARY: {
					...
					status = SpyFsControlSecondaryMountVolumeComplete( NewDeviceObject );
            ...
            SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE,
                           ("SpyFsControlMountVolumeComplete: SpyIsAttachedToDevice() failed.  %p \"%wZ\", already attached\n",
    ...
    } else 
	{
		...
		SPY_LOG_PRINT( (Irp->IoStatus.Status == STATUS_UNRECOGNIZED_VOLUME) ? LFS_DEBUG_LFS_NOISE : SPYDEBUG_DISPLAY_ATTACHMENT_NAMES,
                       ("SpyFsControlMountVolumeComplete: MOUNT IRP failed. %p \"%wZ\", status=%08x\n",
                        DeviceObject,
                        &newDevExt->DeviceName,
                        Irp->IoStatus.Status) );
...
#if __LFS__
		LfsIoDeleteDevice( DeviceObject );
#else
...
#if __LFS__
	LfsIoDeleteDevice( SourceDevice );
#else
...
//	PLFS_DEVICE_EXTENSION		lfsDevExt ;
//    PRECORD_LIST recordList = NULL;
//    BOOLEAN shouldLog;
...
#if DBG
	SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ( "[LFS] SpyPreFsFilterOperation: ") );
	if (FlagOn( gFileSpyDebugLevel, LFS_DEBUG_LFS_NOISE )) {
	...
	lfsDevExt = (PLFS_DEVICE_EXTENSION)(&fileSpyExt->LfsDeviceExt) ;

	ASSERT(fileSpyExt) ;
	ASSERT(lfsDevExt) ;

	//
	//	we do this only for Secondary FastFat volume.
	//
	if(	lfsDevExt != NULL &&
		LFS_FILE_SYSTEM_FAT == lfsDevExt->FileSystemType &&
		LFS_SECONDARY == lfsDevExt->FilteringMode &&
		FS_FILTER_ACQUIRE_FOR_CC_FLUSH == Data->Operation) {

		SPY_LOG_PRINT( SPYDEBUG_TRACE_LEVEL1, ( "[LFS] SpyPreFsFilterOperation: LfsDeviceType == LFS_SECONDARY. Skip ") );
		if (FlagOn( gFileSpyDebugLevel, SPYDEBUG_TRACE_LEVEL1 )) {
		    SpyDumpFsFilterOperation( TRUE, Data );
	    }
...
#if DBG
	SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ( "[LFS] SpyPostFsFilterOperation: ") );
    if (FlagOn( gFileSpyDebugLevel, LFS_DEBUG_LFS_NOISE )) {
...
// Do thunk for 32bit application over 64bit driver.
		...
		case LFS_FILTER_QUERY_NDASUSAGE: {
			...
			if ((InputBufferLength < sizeof(LFS_QUERY_NDASUSAGE_PARAM)) || 
				(InputBuffer == NULL)) {

					IoStatus->Status = STATUS_INVALID_PARAMETER;
					break;
			}
			if ((OutputBufferLength < sizeof(LFS_QUERY_NDASUSAGE_RETURN)) || 
				(OutputBuffer == NULL)) {
			...
			status = LfsDeviceExt_QueryNDASUsage(
							queryUsage->SlotNumber,
		...
		case LFS_FILTER_STOP_SECVOLUME: {
			...
			if ((InputBufferLength < sizeof(LFS_STOPSECVOLUME_PARAM)) || 
				(InputBuffer == NULL)) {
			...
			status = LfsStopSecVolume(stopParam->PhysicalDriveNumber,
										stopParam->VolumeHint);
		...
		case LFS_FILTER_GETVERSION: {
            ...
            if ((OutputBufferLength < sizeof(LFSFILT_VER)) || 
                (OutputBuffer == NULL)) {
			...
			//if (IS_WINDOWS2K())
			//	version.VersionNDFSMajor = NDFS_2K_COMPAT_VERSION;
			//else
				version.VersionNDFSMajor = NDFS_COMPAT_VERSION;

			version.VersionNDFSMinor= NDFS_VERSION;
