...
//
// Read a DWORD value from driver's registry path.
//

VOID
LfsReadDriverParameterDWord(
	IN PUNICODE_STRING	RegistryPath,
	IN PWCHAR			ValueName,
	OUT PLONG			Value
	)
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        pValuePartialInfo = (PKEY_VALUE_PARTIAL_INFORMATION) buffer;
        ASSERT(pValuePartialInfo->Type == REG_DWORD);
        *Value = *((PLONG)&pValuePartialInfo->Data);
...
{        
	...
	LfsCleanupMountedDevice ( DeviceObject );
...
--*/
{
    ...
    LfsInitializeLfsDeviceExt( *FileSpyDeviceObject,
							   NULL,
							   NULL );
...
#if __LFS__
	    LfsIoDeleteDevice( *FileSpyDeviceObject );
#else
...
--*/
{
    ...
    if (IS_WINDOWSXP_OR_LATER()) {
        ...
        status = SpyEnumerateFileSystemVolumes( DeviceObject, filespyDeviceObject, &tempName );
...
#if __LFS__
            LfsIoDeleteDevice( ourAttachedDevice );
#else
...
SpyEnumerateFileSystemVolumes (
	...
	IN PDEVICE_OBJECT FileSpyFsDeviceObject,
...
#if __LFS_ENUMERATION__
							    LfsInitializeLfsDeviceExt( newDeviceObject,
														   NULL,
														   NULL );
...
#if __LFS_ENUMERATION__
									LfsAttachToMountedDevice( FileSpyFsDeviceObject,
															  devList[i],
															  newDeviceObject );
#else
									UNREFERENCED_PARAMETER( FileSpyFsDeviceObject );
#endif
                                } else {
