...
#define __NDAS_NTFS_CONVERT__					1
...
#if WINVER >= 0x0501
#define NdasNtfsDbgBreakPoint()	{if (KD_DEBUGGER_ENABLED) DbgBreakPoint();}
#else
#define NdasNtfsDbgBreakPoint()	{if (*KdDebuggerEnabled) DbgBreakPoint();}
#endif
...
NtfsAllocateClusters (
    ...
    IN BOOLEAN AllocateAll,
    IN LONGLONG ClusterCount,
    IN UNALIGNED PLCN TargetLcn OPTIONAL,
...
BOOLEAN
NtfsAcquireExclusiveSecondaryVcb (
    IN PIRP_CONTEXT IrpContext,
    IN PVCB Vcb,
    IN BOOLEAN RaiseOnCantWait
    );
...
BOOLEAN
NtfsAcquireSharedVcb2 (
    IN PIRP_CONTEXT IrpContext,
    IN PVCB Vcb,
    IN BOOLEAN RaiseOnCantWait
    );

#define NtfsAcquireExclusiveVcb(IrpContext1, Vcb1, RaiseOnCantWait1)											\
	((IrpContext1->VcbAcquiredResult = (NtfsAcquireExclusiveVcb2(IrpContext1, Vcb1, RaiseOnCantWait1))),		\
	(IrpContext1->VcbAcquiredResult ? strncpy(Vcb1->LastAcquireFile, strrchr(__FILE__,'\\')+1, 15) : FALSE),	\
	(IrpContext1->VcbAcquiredResult ? Vcb1->LastAcquireLine = __LINE__ : FALSE),								\
	IrpContext1->VcbAcquiredResult)

#if 0

#define NtfsAcquireSharedVcb(IrpContext1, Vcb1, RaiseOnCantWait1)												\
	((IrpContext1->VcbAcquiredResult = (NtfsAcquireSharedVcb2(IrpContext1, Vcb1, RaiseOnCantWait1))),			\
	(IrpContext1->VcbAcquiredResult ? strncpy(Vcb1->LastAcquireFile, strrchr(__FILE__,'\\')+1, 15) : FALSE),	\
	(IrpContext1->VcbAcquiredResult ? Vcb1->LastAcquireLine = __LINE__ : FALSE),								\
	IrpContext1->VcbAcquiredResult)
...
#define NtfsAcquireSharedVcb(IrpContext1, Vcb1, RaiseOnCantWait1) \
		(NtfsAcquireSharedVcb2 (IrpContext1, Vcb1, RaiseOnCantWait1)) 
...
#define NtfsReleaseVcb(IC,V) {											\
 FlagOn(IC->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_FILE) ?		\
 ExReleaseResourceLite( &(V)->SecondaryResource ) :						\
...
#define NtfsReleaseVcb(IC,V) {										\
	\
	strncpy(V->LastReleaseFile, strrchr(__FILE__,'\\')+1, 15);		\
	V->LastReleaseLine = __LINE__;									\
	NtfsReleaseVcb2 ( IC, V );										\
}
...
#define NtfsIsExclusiveVcb(F, V) (								\
	FlagOn(F->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY) ?		\
	(NtfsIsExclusiveResource(&(V)->SecondaryResource)) :        \
...
#define NtfsDeleteFcbTableEntry(F,V,FR) {									\
    ...
    _Key.FileReference = FR;												\
	if (FlagOn((F)->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY))				\
		RtlDeleteElementGenericTable( &(V)->SecondaryFcbTable, &_Key );		\
	else																	\
...
#define NtfsReportDirNotify(IC,V,FN,O,SN,NPN,F,A,PF)    {       \
    try {                                                       \
																\
		if (NdasNtfsFsRtlNotifyFilterReportChange)				\
        NdasNtfsFsRtlNotifyFilterReportChange( (V)->NotifySync, \
                                       &(V)->DirNotifyList,     \
                                       (PSTRING) (FN),          \
                                       (USHORT) (O),            \
                                       (PSTRING) (SN),          \
...
#define GetIrpName(MajorCode, MinorCode, FsctlCode, MajorCodeName, MinorCodeName)
#define PrintIrp(DebugLevel, Where, VolDo, Irp)
...
VOID
Secondary_DereferenceScb (
	IN PIRP_CONTEXT IrpContext,
	IN	PSCB		Scb
	);

PSCB
Secondary_AllocateScb (
	IN PIRP_CONTEXT			IrpContext,
	IN	PSECONDARY			Secondary,
	IN  PUNICODE_STRING		FullFileName,
	IN  BOOLEAN				IsPagingFile,
...
NTSTATUS
SecondaryNdNtfsCommonCleanup (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP			Irp
	);

NTSTATUS
SecondaryNdNtfsCommonQueryInformation (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP			Irp
	);

NTSTATUS
SecondaryNdNtfsCommonSetInformation (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP			Irp
	);

NTSTATUS
SecondaryNdNtfsCommonWrite (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP			Irp
	);
...
NTSTATUS
NdNtfsCommonSetInformation (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);

NTSTATUS
NdNtfsCommonQueryInformation (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);
...
NTSTATUS
NdNtfsSecondaryCommonSetInformation (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);

NTSTATUS
NdNtfsSecondaryCommonQueryInformation (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);
...
NTSTATUS
NdNtfsSecondaryCommonCleanup (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
	);

NTSTATUS
NdNtfsSecondaryCommonClose (
    IN PIRP_CONTEXT IrpContext,
    IN PSCB Scb,
    IN PFCB Fcb,
    IN PVCB Vcb,
...
NTSTATUS
NdNtfsCommonFlushBuffers (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );

NTSTATUS
NdNtfsSecondaryCommonSetVolumeInfo (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );
...
NTSTATUS
NdNtfsSecondaryCommonQueryQuota (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );
...
NTSTATUS
NdNtfsSecondaryCommonQuerySecurityInfo (
	IN PIRP_CONTEXT IrpContext,
	IN PIRP Irp
	);
...
NTSTATUS
NdNtfsSecondaryCommonSetEa (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp
    );

NTSTATUS
NdNtfsSecondaryQueryDirectory (
    IN PIRP_CONTEXT IrpContext,
    IN PIRP Irp,
    IN PVCB Vcb,
    IN PSCB Scb,
