			...
			)
{
	...
	ReqHdr.SubCOM = WANSCSI_SESSION_INIT_DATA;
				   ...
				   )
{
	...
	memset((char *)&ReqHdr, 0, sizeof(WANSCSI_COMMANDREQ_HEADER));
	ReqHdr.OPCODE = WANSCSI_COM_SESSION;
	ReqHdr.SubCOM = WANSCSI_SESSION_MAKE_NDASCONN;
	iResult = TSOCK_SendIt(ControlSock,(char *)&ReqHdr, sizeof(WANSCSI_COMMANDREQ_HEADER));
	if(iResult == SOCKET_ERROR) {
		PrintError(WSAGetLastError(), "Send WANSCSI_SESSION_MAKE_NDASCONN ");
		return -1;
	}

	iResult = WANRecvIt(ControlSock,(char *)pRepHdr, sizeof(WANSCSI_COMMANDREP_HEADER));
	if(iResult == SOCKET_ERROR) {
		PrintError(WSAGetLastError(), "Recv WANSCSI_SESSION_MAKE_NDASCONN ");
		return -1;
	}else {
		if(pRepHdr->ERR_CODE != WANSCSI_ERR_SUCCESS){
			return (int)pRepHdr->ERR_CODE;
				...
				)
{
	...
	ReqHdr.OPCODE = WANSCSI_COM_SESSION;
	ReqHdr.SubCOM = WANSCSI_SESSION_CLOSE_NDASCONN;
			...
			)
{
	...
	memset((char *)&ReqHdr, 0, sizeof(WANSCSI_COMMANDREQ_HEADER));
	ReqHdr.OPCODE = WANSCSI_COM_SESSION;
	ReqHdr.SubCOM = WANSCSI_SESSION_LOGINED;
