...
#if 1 //def __NDAS_FAT_SECONDARY__
	...
	)
{
    ...
    try {
		...
		if ((FileInformationClass == FileDispositionInformation) ||
            ...
            if (!FatAcquireExclusiveSecondaryVcb( IrpContext, Vcb )) {
    ...
    )
{
    ...
    try_exit: NOTHING;
		...
		//DbgPrint( "FatSetAllocationInfo Fcb->Header.FileSize.LowPart = %x\n", Fcb->Header.FileSize.LowPart );
...

{
    ...
    try {
        ...
        if (Fcb->Header.FileSize.LowPart != NewFileSize) {
			...
			//DbgPrint( "fend ndfsWinxpReplytHeader->FileSize = %x\n", ndfsWinxpReplytHeader->FileSize );
    ...
    try_exit: NOTHING;
		...
		//DbgPrint( "FatSetEndOfFileInfo Fcb->Header.FileSize.LowPart = %x\n", Fcb->Header.FileSize.LowPart );
