...

{
	...
	if (!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
		IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdFatSecondaryCommonWrite3( IrpContext, Irp );
		return Status;
	}
#endif
    ...
    try {
        ...
        if ( TypeOfOpen == UserFileOpen ) {
			...
			if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
				...
				Status = NdFatSecondaryCommonWrite( IrpContext, Irp );
            ...
            if ( ExtendingFile ) {
				...
				//DbgPrint( "w FcbOrDcb->Header.FileSize.LowPart = %x, FcbOrDcb->Header.ValidDataLength.LowPart = %x\n", 
				//		   FcbOrDcb->Header.FileSize.LowPart, FcbOrDcb->Header.ValidDataLength.LowPart );
...
#define LONGMAP_COUNTER
				...
				if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY &&
					!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
					IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {

					SetFlag( IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT );
					Status = NdFatSecondaryCommonWrite2( IrpContext, Irp, BytesToWrite );
            ...
            //
                ...
                if (!FlagOn(IrpContext->MinorFunction, IRP_MN_MDL)) {
                    ...
                    if (!CcCopyWrite( FileObject,
                        ...
                        DebugTrace( 0, Dbg2, "Cached Write could not wait\n", 0 );
