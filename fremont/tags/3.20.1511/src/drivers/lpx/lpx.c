...
{   
	...
	case SMP_STATE_CLOSE: {
		...
		if (!(FlagOn(deviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_CONNECTED) && 
	...
	)
{
	...
	DebugPrint( 1, ("LpxDisconnect Connection = %p, Connection->LpxSmp.state = %s\n", 
					 Connection, LpxStateName[Connection->LpxSmp.SmpState]) );
	...
	case SMP_STATE_CLOSE:
		...
		DebugPrint( 1, ("return disconnect Irp = %p status = %x\n", Irp, status) );
	...
	case SMP_STATE_CLOSE_WAIT: {
		...
		DebugPrint(1, ("Connection = %p Connection->LpxSmp.DisconnectIrp = %p\n", Connection, Connection->LpxSmp.DisconnectIrp) );
		...
		if (Connection->LpxSmp.SmpState == SMP_STATE_CLOSE_WAIT) {
			...
			Connection->LpxSmp.LastAckTimeOut.QuadPart = CurrentTime().QuadPart + Connection->LpxSmp.LastAckInterval.QuadPart;
		...
		if (FlagOn(Connection->Flags2, CONNECTION_FLAGS2_STOPPING)) {
			...
			if (Connection->LpxSmp.DisconnectIrp) {
				...
				DebugPrint( 1, ("CONNECTION_FLAGS2_STOPPING Connection->LpxSmp.DisconnectIrp = %p\n", Connection->LpxSmp.DisconnectIrp) );
	...
	)
{
	...
	case SMP_STATE_CLOSE_WAIT: {
		...
		ASSERT( Connection->LpxSmp.SmpState == SMP_STATE_ESTABLISHED || 
				Connection->LpxSmp.SmpState == SMP_STATE_CLOSE_WAIT );
	...
	)
{
	...
	case SMP_STATE_CLOSE_WAIT: {
		...
		status = LpxSmpProcessReceivePacket( Connection );
	...
	default: 
		...
		if (Connection->Status == STATUS_PENDING) {
			...
			if (!FlagOn(Connection->LpxSmp.Shutdown, SMP_RECEIVE_SHUTDOWN)) {
   ...
   )
{
	...
	/*while (userDataLength)*/ {
		...
		if (status == NDIS_STATUS_SUCCESS) {
			...
			DebugPrint( 0, ("LpxSendDatagram: status = %x\n", status) );
...

{
    ...
    if (Connection->Flags2 & CONNECTION_FLAGS2_AUTOCONNECTING)
        fCanceled = LpxCancelTdiConnect(NULL, Irp);
#endif // RASAUTODIAL
    ...
    if (fCanceled) {
        ...
        IF_LPXDBG (LPX_DEBUG_CONNOBJ) {
            LpxPrint2("LPX: Cancelled in-progress connect/listen %lx on %p\n",
...
#if 0
		LpxCompleteRequest(  Request, STATUS_CANCELLED, 0 );
#endif
	...
	)
{
	...
	case SMP_STATE_LAST_ACK:
		...
		if (Connection->LpxSmp.LastAckTimeOut.QuadPart <= CurrentTime().QuadPart) {
	...
	if (FlagOn(Connection->LpxSmp.SmpTimerFlag, SMP_TIMER_RECEIVE_TIME_OUT)) {
		...
		if (Connection->LpxSmp.ReceiveTimeOut.QuadPart < CurrentTime().QuadPart) {
			...
			while (thisEntry = ExInterlockedRemoveHeadList( &Connection->LpxSmp.ReceiveIrpQueue, 
				...
				pendingIrp->IoStatus.Status = STATUS_IO_TIMEOUT;
...
#if 1	// DBG // Apply always to test LPX
			if (PacketTxDropRate) {
	...
	)
{
	...
	if (Status != NDIS_STATUS_SUCCESS) {
		...
		DebugPrint( 0, ("LpxSendComplete: status = %x\n", Status) );
