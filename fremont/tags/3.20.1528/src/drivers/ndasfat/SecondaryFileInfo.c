...

{
    ...
    try {
		...
		NDASFAT_ASSERT( NT_SUCCESS(ndfsWinxpReplytHeader->Status) );
...

{
	...
	try {
		...
		NDASFAT_ASSERT( NT_SUCCESS(status) || status == STATUS_CANNOT_DELETE || status == STATUS_DIRECTORY_NOT_EMPTY );
