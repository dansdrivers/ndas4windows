...
VOID
LsprotoRequestCompletionRoutine(
	IN PLSTRANS_OVERLAPPED	OverlappedData
){
   ...
   )
{
	...
	LSS->RequestOverlappedData.IoCompleteRoutine = LsprotoRequestCompletionRoutine;
	LSS->RequestOverlappedData.CompletionEvent = &sendRequestCompEvent;
	LSS->RequestOverlappedData.UserContext = LSS;

	ntStatus = LspIdeSendRequest_V11(LSS, &pdu, &LSS->RequestOverlappedData, NULL);
	if(!NT_SUCCESS(ntStatus)) {
	...
	if(sendData) {
		...
		LSS->BodyOverlappedData.IoCompleteRoutine = LsprotoRequestCompletionRoutine;
