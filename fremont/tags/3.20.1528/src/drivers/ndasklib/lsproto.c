...
LspLogin(
	...
	if(NT_SUCCESS(status)) {
		...
		//
		// HWVersion 2.0 Rev 0
		// Clean up locks all the time by repeating acquire/release operation.
		// When performing complete clean up, exclude the current session/connection.
		//

		if(LockCleanup) {
			if(/*LSS->HWVersion == LANSCSIIDE_VERSION_1_1 || */
				(LSS->HWVersion == LANSCSIIDE_VERSION_2_0 &&
...
LspLogout(
	...
	KDPrintM(DBG_LURN_TRACE,  ("LSS %p LanscsiProtocol %p\n",
		LSS, LSS->LanscsiProtocol));

	if(!LSS->LanscsiProtocol) {
		KDPrintM(DBG_LURN_ERROR,  ("Invalid parameter\n"));
		return STATUS_INVALID_PARAMETER;
	}
	if(!LSS->LanscsiProtocol->LsprotoFunc.LsprotoLogout) {
		KDPrintM(DBG_LURN_ERROR,  ("Not implemented\n"));
		return STATUS_NOT_IMPLEMENTED;
	}
	...
	KDPrintM(DBG_LURN_ERROR,  ("EncryptBuffer %p EncryptBufferLength %p\n",
		LSS->EncryptBuffer, LSS->EncryptBufferLength));
	if(/* status == STATUS_SUCCESS && */
		LSS->EncryptBuffer &&
...
LspWorkaroundCleanupLock(
   ...
   PLARGE_INTEGER	TimeOut
	...
	//
	// Perform clean-up only for NDAS HW 2.0 rev 0.
	//
