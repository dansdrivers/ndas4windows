...

{
	...
	case LPX_NDIS_QUERY_GLOBAL_STATS: {
		...
		if (Connection->AddressFile == NULL				|| 
			Connection->AddressFile->Address == NULL	||
			Connection->AddressFile->Address->Provider == NULL) {
		...
		LpxGetMdlChainLength (Irp->MdlAddress, &inputBufferLength);
		inputBuffer = MmGetSystemAddressForMdlSafe(Irp->MdlAddress, NormalPagePriority);

		LpxGetMdlChainLength (Irp->MdlAddress, &outputBufferLength);
		outputBuffer = MmGetSystemAddressForMdlSafe(Irp->MdlAddress, NormalPagePriority);

		ndisOid = (PNDIS_OID)inputBuffer;

		DebugPrint( 1, ("LPX: LPX_NDIS_QUERY_GLOBAL_STATS, ndisOid = %x, inputBuffer = %p, "
						"outputBuffer = %p, Irp->MdlAddress= %p, ((PDEVICE_CONTEXT)(Connection->Provider))->MediumSpeed = %d "
						"outputBufferLength = %d\n",
						 *ndisOid, inputBuffer, outputBuffer, Irp->MdlAddress, 
						 ((PDEVICE_CONTEXT)Connection->AddressFile->Address->Provider)->MediumSpeed, outputBufferLength) );
		...
		if (*ndisOid == OID_GEN_LINK_SPEED) {
			...
			mediumSpeed = ((PDEVICE_CONTEXT)Connection->AddressFile->Address->Provider)->MediumSpeed;

			status = TdiCopyBufferToMdl( &mediumSpeed,
										 0L,
										 sizeof(mediumSpeed),
										 Irp->MdlAddress,
										 0,
