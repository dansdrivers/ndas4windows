...
#pragma alloc_text(PAGE, SpShrinkMappedAddressList)
#pragma alloc_text(PAGE, SpReleaseMappedAddresses)
#pragma alloc_text(INIT, SpInitializeGuidInterfaceMapping)
    ...
    )
{
    ...
    SpGuidInterfaceMappingList = ExAllocatePoolWithTag(
                                    PagedPool,
                                    size,
...

{
    ...
    stringArray = ExAllocatePoolWithTag(PagedPool,
...

{
    ...
    list = RtlDuplicateCmResourceList(NonPagedPool,
                                      AllocatedResources,
...

{
    ...
    irp = IoAllocateIrp((CCHAR) (LogicalUnit->StackSize + 1),
                        FALSE);
...

{
    ...
    try {
        ...
        SpProcessSpecialControllerList(InquiryData, listKey, &flags);
...

{
    ...
    status = ScsiPortGetHardwareIds(InquiryData, &hardwareIds);
    ...
    status = SpMultiStringToStringArray(&hardwareIds,
                                        &hardwareIdList,
                                        FALSE);
...

{
    ...
    emergencySrbData =
        InterlockedCompareExchangePointer(&(Adapter->EmergencySrbData),
                                          SrbData,
                                          NULL);
    ...
    if(emergencySrbData == NULL) {
        ...
        ExFreeToNPagedLookasideList(&Adapter->SrbDataLookasideList,
                                    SrbData);
    }
    return;
}
...

{
    ...
    srbData = ExAllocatePoolWithTag(PoolType,
                                    NumberOfBytes,
    ...
    )
{
    ...
    if(Adapter->MappedAddressList == NULL) {

        //
        // The mapping list allocation might have failed initially.  In that
        // case we can't shrink it any more so just succeed.
        //
    ...
    //
    // Determine how many entries in the mapped address list are actually
    // being used.  Entries are zeroed out when the miniport releases them but
    // they remain in the list.  This will weed out the zero entries.
    //
    ...
    for(i = 0; i < Adapter->MappedAddressCount; i++) {
        ...
        if((Adapter->MappedAddressList[i].NumberOfBytes == 0) &&
           (Adapter->MappedAddressList[i].MappedAddress == 0)) {

            //
            // This entry has been erased - don't allocate space for it.
            //
    ...
    DebugPrint((1, "SpShrinkMappedAddressList: List contained %d entries, %d "
                   "of which were released\n",
                Adapter->MappedAddressCount,
                (Adapter->MappedAddressCount - realCount)));
    ...
    if(realCount > 0) {
        ...
        //
        // Allocate a new block of memory for the array.  This block should
        // be smaller and must be allocated from non-paged pool.
        //

        newSize = sizeof(MAPPED_ADDRESS) * realCount;
        ...
        if(newList == NULL) {
            ...
            //
            // Free the original list and return a failure.
            //

            ExFreePool(Adapter->MappedAddressList);
        ...
        //
        // Copy the original list into the new one.  Make sure each entry points
        // to the next one.
        //

        for(i = 0, j = 0; i < Adapter->MappedAddressCount; i++) {

            if((Adapter->MappedAddressList[i].NumberOfBytes == 0) &&
               (Adapter->MappedAddressList[i].MappedAddress == 0)) {

                //
                // This entry has been erased - don't copy it.
                //

                continue;
            }

            newList[j] = Adapter->MappedAddressList[i];
            newList[j].NextMappedAddress = &(newList[j+1]);
            j++;
        }

        //
        // Clear the next pointer in the last address mapping.
        //

        newList[j-1].NextMappedAddress = NULL;

    } else {
        newList = NULL;
    }

    DebugPrint((1, "SpShrinkMappedAddressList: old list %#p had %d entries. "
                   "new list is %#p and has %d entries\n",
                Adapter->MappedAddressList,
                Adapter->MappedAddressCount,
                newList,
                realCount));

    ExFreePool(Adapter->MappedAddressList);
    Adapter->MappedAddressList = newList;
    Adapter->MappedAddressCount = realCount;
    ...
    for(i = 0; i < Adapter->MappedAddressCount; i++) {
        ...
        if((Adapter->MappedAddressList[i].NumberOfBytes == 0) &&
           (Adapter->MappedAddressList[i].MappedAddress == 0)) {
            ...
            //
            // This entry has been erased - don't release it.
            //
        ...
        MmUnmapIoSpace(
            Adapter->MappedAddressList[i].MappedAddress,
            Adapter->MappedAddressList[i].NumberOfBytes
            );
    }
...
Routine Description:
    ...
    moment so we need to do some silly x86 specific tricks.
...

{
    ...
    if((*Mm64BitPhysicalAddress) == TRUE) {
        DbgPrint("SpDetermine64BitSupport: Mm64BitPhysicalAddress is TRUE\n");
        return TRUE;
    }
    ...
    )
{
    ...
    // 
    // BUGBUG: Indicate that the memory is a "physical view" so that 
    // MM doesn't feel the need to reference the PTE's for this buffer.
    // We have to do this for the time being because MM isn't aware of the 
    // pages the HAL is using for bounce buffers.
    //

    SET_FLAG(mdl->MdlFlags, MDL_PAGES_LOCKED | MDL_PHYSICAL_VIEW);
    ...
    for(sgPage = 0, mdlPage = 0; sgPage < ScatterGatherEntries; sgPage++) {
        ...
        sgLength = 
            ADDRESS_AND_SIZE_TO_SPAN_PAGES(ScatterGatherList[sgPage].PhysicalAddress.QuadPart,
                                           ScatterGatherList[sgPage].Length);

        for(sgSpan = 0; sgSpan < sgLength; sgSpan++, mdlPage++) {
            ULONGLONG pa;
            pa = ScatterGatherList[sgPage].PhysicalAddress.QuadPart;
            pa += sgSpan * PAGE_SIZE;
            pa >>= PAGE_SHIFT;
