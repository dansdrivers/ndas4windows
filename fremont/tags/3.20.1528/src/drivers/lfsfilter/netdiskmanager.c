	...
	)
{
	...
	try {
		...
		if (GlobalLfs.NdasFsMiniFilterMode == TRUE) {
	...
	)
{
	...
	if (status == STATUS_SUCCESS) {
		...
		*BlockAceId = *blockAceId;
		...
		NDASFS_ASSERT( NetdiskPartition->ScsiportAdapterDeviceObjectReferenceCount );
	...
	)
{
	...
	if (netdiskPartition) {
		...
		*NetdiskPartitionInformation = netdiskPartition->NetdiskPartitionInformation;
		...
		NDASFS_ASSERT( NetdiskPartitionInformation->VolumeName.Length );
...
if (IS_WINDOWS2K()) {
	...
	case NETDISK_PRIMARY:
		...
		NDASFS_ASSERT( netdiskEnableMode == NETDISK_PRIMARY );

		break;

	case NETDISK_SECONDARY2PRIMARY:
			
		NDASFS_ASSERT( netdiskEnableMode == NETDISK_PRIMARY || netdiskEnableMode == NETDISK_SECONDARY );
	...
	for (partitionIndex = 0; partitionIndex < driveLayoutInformation->PartitionCount; partitionIndex++) {
		...
		if (netdiskPartition) {
			...
			if (netdiskPartition->ScsiportAdapterDeviceObjectReferenceCount) {
				...
				NDASFS_ASSERT( netdiskPartition->ScsiportAdapterDeviceObject == NULL );
				NDASFS_ASSERT( netdiskPartition->NetdiskPartitionInformation.DiskDeviceObject == NULL );
	...
	)
{
	...
	NDASFS_ASSERT( EnabledNetdisk->ListEntry.Flink == EnabledNetdisk->ListEntry.Blink );

	if (FlagOn(EnabledNetdisk->Flags, ENABLED_NETDISK_INSERT_DISK_INFORMATION)) {
		
		NETDISK_PARTITION_INFO	netdiskPartitionInfo;

		NDASFS_ASSERT( EnabledNetdisk->NetdiskEnableMode == NETDISK_SECONDARY ||
					   EnabledNetdisk->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY );
...
if (IS_WINDOWS2K()) {
	...
	NDASFS_ASSERT( netdiskPartition->NetdiskVolume[netdiskEnableMode].VolumeState == VolumeEnabled || 
	...
	*NetdiskPartition = netdiskPartition;
	...
	if (netdiskPartition->ScsiportAdapterDeviceObject) {
		...
		NDASFS_ASSERT( *ScsiportAdapterDeviceObject == netdiskPartition->ScsiportAdapterDeviceObject );
		NDASFS_ASSERT( DiskDeviceObject == netdiskPartition->NetdiskPartitionInformation.DiskDeviceObject );

		ObDereferenceObject( *ScsiportAdapterDeviceObject );

	} else {

		NDASFS_ASSERT( netdiskPartition->ScsiportAdapterDeviceObjectReferenceCount == 1 );
		NDASFS_ASSERT( DiskDeviceObject && netdiskPartition->NetdiskPartitionInformation.DiskDeviceObject == NULL );

		NDASFS_ASSERT( *ScsiportAdapterDeviceObject );
		...
		NDASFS_ASSERT( netdiskPartition->NetdiskPartitionInformation.VolumeName.Length );
	}
	...
	)
{
	...
	NDASFS_ASSERT( NetdiskPartition->NetdiskVolume[NetdiskEnableMode].VolumeState == VolumePreMounting ||
	...
	)
{
	...
	if (NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumeEnabled) {
		...
		NDASFS_ASSERT( GlobalLfs.NdasFsMiniFilterMode == 1 && NetdiskEnableMode == NULL );	
		NDASFS_ASSERT( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY2PRIMARY].VolumeState == VolumeMounted );

	} else {

		NDASFS_ASSERT( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY2PRIMARY].VolumeState == VolumeEnabled );
		NDASFS_ASSERT( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY2PRIMARY].LfsDeviceExt == NULL );

		if (NetdiskEnableMode) {

			NDASFS_ASSERT( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumeMounted );
	
		} else {

		// called from NdasFat or NdasNtfs

			NDASFS_ASSERT( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumePostMounting  || 
	...
	)
{
	...
	try {
		...
		status = NDCtrlUpgradeToWrite( EnabledNetdisk->NetdiskInformation.SlotNo,
									   &ioStatusBlock );
...
#if !__NDAS_FS_MINI__
	NDASFS_ASSERT( NetdiskPartition->NetdiskVolume[NetdiskEnableMode].VolumeState == VolumeMounted );
#else
	NDASFS_ASSERT( NetdiskPartition->NetdiskVolume[NetdiskEnableMode].VolumeState == VolumeMounted ||
		...
		if (NetdiskPartition->NetdiskVolume[NETDISK_PRIMARY].VolumeState == VolumeMounted && 
			...
			if (NetdiskPartition->NetdiskVolume[NETDISK_PRIMARY].AttachedToDeviceObject == NULL) {
				...
				if (GlobalLfs.NdasFatRwIndirect == TRUE && NetdiskPartition->FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT ||
					...
					NDASFS_ASSERT( status == STATUS_INVALID_DEVICE_REQUEST );
			...
			if (NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY2PRIMARY].AttachedToDeviceObject == NULL) {
				...
				if (GlobalLfs.NdasFatRwIndirect == TRUE && NetdiskPartition->FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT ||
					...
					NDASFS_ASSERT( status == STATUS_INVALID_DEVICE_REQUEST || status == STATUS_INSUFFICIENT_RESOURCES );

				} else {

					NDASFS_ASSERT( status == STATUS_SUCCESS || status == STATUS_INSUFFICIENT_RESOURCES );
				}
			}
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_NOISE,
				   ("NetdiskParttion_Allocate netdiskPartition = %p, netdiskPartition = %d\n", 
