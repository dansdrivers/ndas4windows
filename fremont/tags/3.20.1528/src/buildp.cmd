...
call :run_build -WNET2K %CONF% %_BUILDP_BASE_DIR%
	)
	if defined BUILDP_LFS (
REM call :run_build -WNET2K %CONF% %_BUILDP_BASE_DIR%drivers\w2kfatlib
REM call :run_build -WNET2K %CONF% %_BUILDP_BASE_DIR%drivers\w2kntfslib
REM call :run_build -WNETXP %CONF% %_BUILDP_BASE_DIR%drivers\wxpfatlib
REM call :run_build -WNETXP %CONF% %_BUILDP_BASE_DIR%drivers\wxpntfslib
REM call :run_build -WNET   %CONF% %_BUILDP_BASE_DIR%drivers\wnetfatlib
REM call :run_build -WNETXP %CONF% %_BUILDP_BASE_DIR%drivers\lfsfiltlib
call :run_build -WNETXP %CONF% %_BUILDP_BASE_DIR%drivers\lfsfilter
	)
	if defined BUILDP_NDFS (
call :run_build -WNETXP %CONF% %_BUILDP_BASE_DIR%drivers\ndasfat
call :run_build -WNETXP %CONF% %_BUILDP_BASE_DIR%drivers\ndasntfs
	)
	if defined BUILDP_DIR (
call :run_build -WNET2K %CONF% %BUILDP_DIR%
	)
)
...
if /i "%BUILDP_ARCH%" equ "amd64" (
	...
	if defined BUILDP_BASE (
call :run_build -WNET AMD64 %CONF% %_BUILDP_BASE_DIR%
	)
	if defined BUILDP_LFS (
REM call :run_build -WNET AMD64 %CONF% %_BUILDP_BASE_DIR%drivers\wxpfatlib
REM call :run_build -WNET AMD64 %CONF% %_BUILDP_BASE_DIR%drivers\wnetfatlib
REM call :run_build -WNET AMD64 %CONF% %_BUILDP_BASE_DIR%drivers\lfsfiltlib
call :run_build -WNET AMD64 %CONF% %_BUILDP_BASE_DIR%drivers\lfsfilter
	)
	if defined BUILDP_NDFS (
call :run_build -WNET AMD64 %CONF% %_BUILDP_BASE_DIR%drivers\ndasfat
call :run_build -WNET AMD64 %CONF% %_BUILDP_BASE_DIR%drivers\ndasntfs
	)
	if defined BUILDP_DIR (
call :run_build -WNET AMD64 %CONF% %BUILDP_DIR%
	)
)

if errorlevel 1 echo ERROR: Build failed, error=%ERRORLEVEL%
