...
SkipHotPlug:
        ...
        FileSizes.FileSize.LowPart = sizeof(PACKED_BOOT_SECTOR);
...

{
    ...
    try {
		...
		ExInitializeFastMutex( &Dcb->CcbQMutex );
		InitializeListHead( &Dcb->CcbQueue );
...
#if (defined __NDAS_FAT_DBG__ && defined __NDAS_FAT_SECONDARY__) 
	//ASSERT( IrpContext->CurrentIrql == KeGetCurrentIrql() );
	
	//if (IrpContext->Vcb) {
		
	//	ASSERT( IrpContext->VcbSharedCount == ExIsResourceAcquiredSharedLite(&IrpContext->Vcb->Resource) );
	//	ASSERT( IrpContext->SecondaryVcbSharedCount == ExIsResourceAcquiredSharedLite(&IrpContext->Vcb->SecondaryResource) );
	//}
#endif
