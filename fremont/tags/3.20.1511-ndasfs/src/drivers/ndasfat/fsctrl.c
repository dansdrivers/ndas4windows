...
#if __NDAS_FAT_PURGE_OR_FLUSH__
...
#if (__NDAS_FAT_PRIMARY__ || __NDAS_FAT_SECONDARY__ || __NDAS_FAT_PURGE_OR_FLUSH__)

		DebugTrace2( 0, Dbg, ("NdFatMountVolume: VolDo = %p\n", VolDo) );		
		...
		if (((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL) {
			...
			if (queryStatus != STATUS_SUCCESS) {
				...
				if (FlagOn(VolDo->NetdiskPartitionInformation.NetdiskInformation.EnabledFeatures, NDASFEATURE_SIMULTANEOUS_WRITE))
					SetFlag( VolDo->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW );
...
if (IS_WINDOWS2K()) {
				...
				while (1) {
					...
					bacl = FsRtlAllocatePoolWithTag( NonPagedPool, baclLen, NDFAT_ALLOC_TAG );
					...
					if (queryStatus == STATUS_BUFFER_OVERFLOW) {
						...
						baclLen = bacl->Length;
						ExFreePoolWithTag( bacl, NDFAT_ALLOC_TAG );
						continue;
					}
...
if (IS_WINDOWS2K()) {
							...
							if (VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart/512 >= (INT64)bacl->BlockACEs[baceCount].BlockStartAddr &&
								...
								SetFlag( VolDo->NdFatFlags, ND_FAT_DEVICE_FLAG_LINKSYS_SUPPORT );
        ...
        if ( DoARemount ) {
...

            {
                ...
                FileSizes.FileSize.LowPart = ( 0x40000 + 0x1000 );
				...
				if ((VolDo->NetdiskEnableMode == NETDISK_READ_ONLY) &&
					FlagOn(VolDo->NdFatFlags, ND_FAT_DEVICE_FLAG_LINKSYS_SUPPORT)) {
		...
		if (VolDo->NetdiskEnableMode == NETDISK_SECONDARY) {
			...
			for (reconnectionTry = 0; reconnectionTry < MAX_RECONNECTION_TRY; reconnectionTry ++) {
				...
				status = ((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->
								NdfsCallback.SecondaryToPrimary( Vcb->Vpb->RealDevice );
			...
			if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW)) {
...
if (IS_WINDOWS2K()) {
...
#if __NDAS_FAT__
		ClearFlag( VolDo->NdFatFlags, ND_FAT_DEVICE_FLAG_INITIALIZING );
		SetFlag( VolDo->NdFatFlags, ND_FAT_DEVICE_FLAG_MOUNTED );
#endif
	...
	if (Status == STATUS_SUCCESS && VolDo->NetdiskEnableMode == NETDISK_SECONDARY) {

		SetFlag( Vcb->NdFatFlags, ND_FAT_VCB_FLAG_RESOURCE_DELETED );
		FatDeleteResource( &Vcb->Resource );
		RtlZeroMemory( &Vcb->Resource, sizeof(ERESOURCE) );
	}
...

{
	...
	if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) 
		(VOID)FatAcquireExclusiveSecondaryVcb( IrpContext, Vcb );
	else
...

{
	...
	case FSCTL_ND_FAT_FLUSH_OR_PURGE: {
		...
		DebugTrace(+1, Dbg, "FatUserFsCtrl... FSCTL_ND_FAT_FLUSH_OR_PURGE \n", 0);
...
#if __NDAS_FAT_SECONDARY__
		if (!(FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) ? 
			FatAcquireExclusiveSecondaryVcb(&IrpContext, Vcb) : FatAcquireExclusiveVcb(&IrpContext, Vcb))) {
#else
		...
		if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_READ_ONLY && 
			IrpContext->PrimaryRequestInfo.PrimaryTag != 0) {

			SetFlag( IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_PURGE );
			...
			if (Vcb->RootDcb && Vcb->RootDcb->Specific.Dcb.DirectoryFile) {
				...
				try {
					...
					Vcb->RootDcb->Header.FileSize.LowPart = Vcb->RootDcb->Header.AllocationSize.QuadPart; 
			...
			SetFlag( IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_FLUSH );
...
#if __NDAS_FAT_PRIMARY__
	{
		...
		ASSERT( volDo->NetdiskEnableMode == NETDISK_PRIMARY || volDo->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY ||
				(volDo->NetdiskEnableMode == NETDISK_READ_ONLY /*&& FlagOn(volDo->NdFatFlags, ND_FAT_DEVICE_FLAG_LINKSYS_SUPPORT)*/) );
...
#if __NDAS_FAT_SECONDARY__
			if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) 
				FatAcquireSharedSecondaryVcb( IrpContext, Fcb->Vcb );
			else
...
#if __NDAS_FAT_SECONDARY__
			if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
				FatReleaseSecondaryVcb( IrpContext, Fcb->Vcb );
			} else {
...
#if __NDAS_FAT_SECONDARY__
			if (FlagOn(IrpContext.NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT))
				(VOID)FatAcquireExclusiveSecondaryVcb( &IrpContext, ExistingVcb );
			else
...
#if __NDAS_FAT_SECONDARY__
			if (FlagOn(IrpContext.NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
				FatReleaseSecondaryVcb( &IrpContext, ExistingVcb );
			} else {
...

{
    ...
    try {
        ...
        if ((*RequestedMapSize).QuadPart > Fcb->Header.FileSize.LowPart) {
