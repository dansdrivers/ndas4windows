...
#if __NDAS_FAT_SECONDARY__
				if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
					if (AcquiredVcb) { FatReleaseSecondaryVcb( IrpContext, Vcb ); }
				} else {
                    ...
                    if (FlagOn(Fcb->FcbState, FCB_STATE_DELETE_ON_CLOSE) &&
						...
						if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
							...
							if (IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader == NULL ) {
								...
								ASSERT( Fcb->Header.FileSize.LowPart == IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.FileSize );
