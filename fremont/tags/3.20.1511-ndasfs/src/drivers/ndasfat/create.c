...

{
	...
	UNREFERENCED_PARAMETER( FreeOemBuffer );
			...
			try {
				...
				if (TypeOfOpen == UserFileOpen) {
					...
					if (ThisFcb->PrimarySession != IrpContext->PrimaryRequestInfo.PrimarySession && ThisFcb->OpenCount != 1) {
						...
						ExAcquireFastMutex( &ThisFcb->CcbQMutex );

						for (unclosedCcbCount = 0, ccbListEntry = ThisFcb->CcbQueue.Flink; 
							 ccbListEntry != &ThisFcb->CcbQueue;
						...
						ExReleaseFastMutex( &ThisFcb->CcbQMutex );
...

{
    ...
    try_exit: NOTHING;
		...
		DebugTrace(-1, (Iosb.Status == STATUS_SUCCESS) ? Dbg : Dbg2, "FatOpenVolume -> Iosb.Status = %08lx\n", Iosb.Status);
    }
    ...
    try {
        ...
        *OplockPostIrp = FALSE;
        ...
        if ((CreateDisposition == FILE_OPEN) ||
			...
			UnwindCcb->Fcb = Fcb;
			InsertTailList( &Fcb->CcbQueue, &UnwindCcb->FcbListEntry );
			ExReleaseFastMutex(&Fcb->CcbQMutex);
			FatSetFileObject( FileObject,
                              UserFileOpen,
                              Fcb,
