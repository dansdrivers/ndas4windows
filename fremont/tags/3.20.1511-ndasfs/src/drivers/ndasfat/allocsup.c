...

{
...

    {
        ...
        FileSizes.FileSize.LowPart = (FatReservedBytes( &Vcb->Bpb ) +
...

{
    ...
    *EndOnMax = FALSE;
...
#if __NDAS_FAT_SECONDARY__
	{
		...
		if (volDo->NetdiskEnableMode == NETDISK_SECONDARY && FlagOn(volDo->NdFatFlags, ND_FAT_DEVICE_FLAG_MOUNTED)) {
...

{
    ...
    try {
		...
		if (NodeType(FcbOrDcb) == FAT_NTC_FCB) {
			...
			if (index * sizeof(NDFS_FAT_MCB_ENTRY) > DEFAULT_NDAS_MAX_DATA_SIZE) 
				DbgBreakPoint();
			...
			if (FcbOrDcb->Header.AllocationSize.QuadPart == 0) {
				...
				if (newIndex * sizeof(NDFS_FAT_MCB_ENTRY) > DEFAULT_NDAS_MAX_DATA_SIZE) 
					DbgBreakPoint();
