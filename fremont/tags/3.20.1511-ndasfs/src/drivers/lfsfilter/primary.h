	...
	do {
		...
		switch(waitStatus) {
			...
			*FastMutexSet = TRUE;											
			...
			if((Secondary->Flags & SECONDARY_FLAG_ERROR) ||
			...
			case STATUS_TIMEOUT: {
				...
				if (Secondary->LfsDeviceExt->SecondaryState == VOLUME_PURGE_STATE) {
					...
					if (waitCount++ > 24) { 
						...
						if (((PNDFS_WINXP_REQUEST_HEADER)SecondaryRequest->NdfsRequestData)->IrpMajorFunction == IRP_MJ_CREATE ||
							((PNDFS_WINXP_REQUEST_HEADER)SecondaryRequest->NdfsRequestData)->IrpMajorFunction == IRP_MJ_QUERY_VOLUME_INFORMATION) {
					
							ExReleaseFastMutex( &Secondary->FastMutex );

							SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO,
										   ("ACQUIRE_FASTMUTEX_AND_TEST_CORRUPT_ERROR_RETURN: return ((PNDFS_WINXP_REQUEST_HEADER)secondaryRequest->NdfsRequestData)->IrpMajorFunction = %d "
										    "fileName = %wZ\n",
										     ((PNDFS_WINXP_REQUEST_HEADER)SecondaryRequest->NdfsRequestData)->IrpMajorFunction,
											 (FileExt && FileExt->FileObject) ? &FileExt->FileObject->FileName : NULL) );
							...
							*FastMutexSet = FALSE;
							...
							SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO,
										   ("ACQUIRE_FASTMUTEX_AND_TEST_CORRUPT_ERROR_RETURN: tiemout ((PNDFS_WINXP_REQUEST_HEADER)secondaryRequest->NdfsRequestData)->IrpMajorFunction = %d "
										    "fileName = %wZ\n",
										     ((PNDFS_WINXP_REQUEST_HEADER)SecondaryRequest->NdfsRequestData)->IrpMajorFunction,
											 (FileExt && FileExt->FileObject) ? &FileExt->FileObject->FileName : NULL) );
	...
	)
{
	...
	if (FileObject->RelatedFileObject) {
		...
		ASSERT( relatedFileExt->LfsMark == LFS_MARK );
	...
	*Retry		  = FALSE;
	...
	case IRP_MJ_CLOSE: // 0x02
	{
		...
		if (FlagOn(Secondary->Flags, SECONDARY_FLAG_ERROR) || 
			fileExt->Corrupted == TRUE || 
			!(FlagOn(Secondary->LfsDeviceExt->Flags, LFS_DEVICE_START) && !FlagOn(Secondary->LfsDeviceExt->Flags, LFS_DEVICE_STOP)))
		{
			fileObject->SectionObjectPointer = NULL;
			KeAcquireSpinLock(&Secondary->FcbQSpinLock, &oldIrql);
			if(InterlockedDecrement(&fcb->OpenCount) == 0)
...
*/
	...
	{		
        ...
        else if(irpSp->MinorFunction == IRP_MN_QUERY_DIRECTORY) 
		{
			...
			returnStatus = ACQUIRE_FASTMUTEX_AND_TEST_CORRUPT_ERROR_RETURN( Secondary,
																		    FastMutexSet,
																			fileExt,
																			Retry,
																			secondaryRequest,
