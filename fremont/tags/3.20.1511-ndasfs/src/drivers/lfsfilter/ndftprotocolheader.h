	...
	)
{
	...
	if (irpSp->MajorFunction != IRP_MJ_CREATE) {
		...
		if (ReadonlyLookUpCcb(DevExt, irpSp->FileObject) == NULL) {
	...
	)
{
	...
	try {
		...
		ASSERT( ReadonlyLookUpCcb(DevExt, fileObject) == NULL );
		...
		fullFileNameBuffer = ExAllocatePool( NonPagedPool, 
		...
		if (!NT_SUCCESS(status)) {
			SPY_LOG_PRINT( LFS_DEBUG_READONLY_ERROR,
				("ReadonlyRedirectIrpMajorCreate: STATUS=%08lx\n", status));
	...
	)
{
	...
	if (ReadonlyLookUpCcb(DevExt, fileObject) != ccb) {
        ...
        if (irpSp->MinorFunction == IRP_MN_NOTIFY_CHANGE_DIRECTORY) {
...

			{
				...
				if (indexSpecified) {
					...
					status = LfsQueryDirectoryByIndex( ccb->ReadonlyFileHandle,
	...
	)
{
	...
	try {
		...
		SPY_LOG_PRINT( NT_SUCCESS(status) ? LFS_DEBUG_READONLY_INFO : LFS_DEBUG_READONLY_INFO,
					   ("ReadonlyRedirectIrpMajorFileSystemControl: %wZ, openFileId = %x, function = %d, "
					    "ioStatusBlock.Status = %x, ioStatusBlock.Information = %d\n",
						 &fileObject->FileName, ccb->ReadonlyFileHandle, status, ioStatusBlock.Information) );
	...
	)
{
	...
	try {
		...
		SPY_LOG_PRINT( NT_SUCCESS(status) ? LFS_DEBUG_READONLY_NOISE : LFS_DEBUG_READONLY_TRACE,
					   ("ReadonlyRedirectIrpMajorFileSystemControl: openFileId = %x, function = %d, "
					    "fileSystemControlStatus = %x, ioStatusBlock.Status = %x, ioStatusBlock.Information = %d\n",
						 ccb->ReadonlyFileHandle, (fileSystemControl.FsControlCode & 0x00003FFC) >> 2, status, ioStatusBlock.Information) );
	...
	)
{
	...
	try {
		...
		SPY_LOG_PRINT( NT_SUCCESS(status) ? LFS_DEBUG_READONLY_INFO : LFS_DEBUG_READONLY_INFO,
					   ("ReadonlyRedirectIrpMajorLockControl: %wZ, openFileId = %p, "
