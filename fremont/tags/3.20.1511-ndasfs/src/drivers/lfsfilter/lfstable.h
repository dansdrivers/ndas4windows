...
//		ULONG				originalFileSpyDebugLevel; 
    ...
    case IRP_MJ_CREATE: //0x00
	{
		...
		} else if (NT_SUCCESS(createStatus)) {
...
#else
		if(PrimarySession->SessionContext.NdfsMinorVersion == NDFS_PROTOCOL_MINOR_1 
			&& BooleanFlagOn(NdfsWinxpRequestHeader->IrpFlags, IRP_CLOSE_OPERATION))
#endif
		{
    ...
    )
{
	...
	if(replyDataSize <= (ULONG)(PrimarySession->SessionContext.SecondaryMaxDataSize || sizeof(PrimarySession->Thread.SessionSlot[Mid].ReplyMessageBuffer) - sizeof(NDFS_REPLY_HEADER) - sizeof(NDFS_WINXP_REQUEST_HEADER)))
	{
