...
#if SCSIDBG_ENABLED
        if(ScsiBufferSentinal == NULL) {
            ScsiBufferSentinal = &(ScsiBuffer[DEBUG_BUFFER_LENGTH]);
...

{
    ...
    if (b == FALSE) {
        ...
        DebugPrint((0, "ScsiPortGetDeviceBase: Translate bus address "
                       "failed. %s Address = %lx\n",
        InIoSpace ? "I/O" : "Memory", IoAddress.LowPart));
        return NULL;
    }
    ...
    if ((isReinit == FALSE) && (addressSpace == FALSE)) {
        ...
        newMappedAddress =
            &(adapter->MappedAddressList[adapter->MappedAddressCount]);
        ...
        //
        // Increment the mapped address count.
        //
        ...
        for(i = 0; i < adapter->MappedAddressCount; i++) {
            ...
            if((address->IoAddress.QuadPart == IoAddress.QuadPart) &&
               (address->NumberOfBytes == NumberOfBytes) &&
               (address->BusNumber == SystemIoBusNumber))  {
...
#if defined(_X86_)
            DebugPrint((0, "ScsiPortGetUncachedExtension: Will remap buffers for adapter %#p\n", adapter));
...
#else
        adapter->DmaAdapterObject = HalGetAdapter(
            &deviceDescription,
            &numberOfMapRegisters
            );
#endif
...

{
    ...
    if (DeviceExtension->DmaAdapterObject == NULL) {
        ...
        if((Sp64BitPhysicalAddresses) &&
           (DeviceExtension->Dma64BitAddresses == TRUE)) {
            ...
            //
            // We'll get page aligned memory out of this which is probably better
            // than the requirements of the adapter.
            //
...
#endif
        {
            buffer = HalAllocateCommonBuffer(
    ...
    if (DeviceExtension->AllocateSrbExtension) {
        ...
        DebugPrint((1, "SpGetCommonBuffer: %d entries put onto SrbExtension "
                       "list\n", i));
...

{
    ...
    for (j = 0; j < 8; j++) {
        Context->PortConfig.InitiatorBusId[j] = (CCHAR)SP_UNINITIALIZED_VALUE;
    }
    ...
    while(TRUE){
        ...
        if (_wcsnicmp(keyValueInformation->Name, L"InitiatorTargetId",
            ...
            if (Context->PortConfig.InitiatorBusId[0] > Context->PortConfig.MaximumNumberOfTargets - 1) {
                Context->PortConfig.InitiatorBusId[0] = (CCHAR)SP_UNINITIALIZED_VALUE;
            }
        }
    ...
    for (slotNumber = (*SlotNumber).u.bits.DeviceNumber;
        ...
        for (functionNumber= (*SlotNumber).u.bits.FunctionNumber;
            ...
            *SlotNumber = slotData;
            ...
            status = SpTranslateResources(fdoExtension->AllocatedResources,
...

{
    ...
    if(!fdoExtension->IsInVirtualSlot) {
        ...
        DebugPrint((0,"ScsiPortSetBusDataByOffset: !fdoExtension->"
...

{
    ...
    KeInitializeEvent(&fdoExtension->EnumerationSynchronization,
                      SynchronizationEvent,
                      TRUE);
...

{
    ...
    do {
        ...
        hwDeviceExtension = ExAllocatePoolWithTag(
                                NonPagedPool,
...
NewConfiguration:
        ...
        configInfo = ExAllocatePoolWithTag(
                        NonPagedPool,
...

{
    ...
    *CallAgain = FALSE;
    ...
    // initialized.
    //
    // If this is NULL the miniport will not save the mappings.
    //

    adapter->MappedAddressList = ExAllocatePoolWithTag(
                                    PagedPool,
                                    PAGE_SIZE,
    ...
    RtlZeroMemory(adapter->MappedAddressList, PAGE_SIZE);


    adapter->MappedAddressCount = 0;

    status = HwInitData->HwFindAdapter(adapter->HwDeviceExtension,
                                       HwContext,
                                       NULL,
    ...
    SpShrinkMappedAddressList(adapter);
    ...
    if(ConfigurationContext->DisableTaggedQueueing) {
        adapter->TaggedQueuing = ConfigInfo->TaggedQueuing = FALSE;
    }
...

{
...
#if defined(_X86_)
            DebugPrint((0, "SpAllocateAdapterResources: Will remap buffers for adapter %#p\n", fdoExtension));
            fdoExtension->RemapBuffers = TRUE;
#endif
        }

#ifdef USE_DMA_MACROS
        fdoExtension->DmaAdapterObject = IoGetDmaAdapter(
                                                       fdoExtension->LowerPdo,
                                                       &deviceDescription,
                                                       &numberOfMapRegisters);

#else
        fdoExtension->DmaAdapterObject = HalGetAdapter( &deviceDescription,
                                                        &numberOfMapRegisters);
    ...
    if(((fdoExtension->SrbExtensionSize != 0) || (configInfo->AutoRequestSense)) &&
       (fdoExtension->SrbExtensionBuffer == NULL))  {
    ...
    fdoExtension->InquiryBuffer = ExAllocatePoolWithTag(
    ...
    fdoExtension->InquirySenseBuffer = ExAllocatePoolWithTag(
                                        NonPagedPoolCacheAligned,
                                        SENSE_BUFFER_SIZE,
                                        SCSIPORT_TAG_INQUIRY);
    ...
    fdoExtension->InquiryIrp = IoAllocateIrp(INQUIRY_STACK_LOCATIONS, FALSE);
    ...
    fdoExtension->InquiryMdl = IoAllocateMdl(fdoExtension->InquiryBuffer,
                                             INQUIRYDATABUFFERSIZE,
                                             FALSE,
                                             FALSE,
                                             NULL);
...

{
    ...
    fdoExtension->MapRegisterBase = ExAllocatePoolWithTag(
                                        NonPagedPool,
                                        Length,
...

{
    ...
    if(Pdo == NULL) {
        ...
        DebugPrint((0,"SpGetBusData: NULL PDO, not supported for 64-bits.\n"));
        return STATUS_INVALID_PARAMETER;
    ...
    )
{
    ...
    try {
        ...
        newAdapterList = ExAllocatePoolWithTag(
                        NonPagedPool,
...

{
    ...
    *DriverExtension = NULL;
    ...
    try {
        ...
        if(NT_SUCCESS(status)) {
            switch(busType) {
                ...
                case BusTypeFibre:
                case BusTypeRAID: {
                    DebugPrint((0, "SpAllocateDriverExtension: Bus type set to %d\n", busType));
                    break;
                } 
                default: {
                    busType = BusTypeScsi;
        ...
        for(passes = 0; passes < 2; passes++) {
            ...
            for(count = 0; TRUE; count++) {
                ...
                if(status == STATUS_NO_MORE_ENTRIES) {
                    ...
                    DebugPrint((0, "SpAllocateDriverExtension: Fatal error %#08lx "
    ...
    if(driverExtension->RegistryPath.Buffer == NULL) {
        ...
        DebugPrint((0, "SpAllocateDriverExtension: Fatal error "
        ...
        if(parametersKey != NULL) {
            ...
            if(NT_SUCCESS(status) &&
               ...
               (keyValueInformation->Type == REG_DWORD)) {
                ...
                if(!NT_SUCCESS(status)) {
                    DebugPrint((0, "SpAllocateDriverExtension: Error %#08lx "
