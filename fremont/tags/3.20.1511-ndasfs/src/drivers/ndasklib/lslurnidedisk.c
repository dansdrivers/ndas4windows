...
LurnIdeDiskRequest(
	...
	case CCB_OPCODE_RESETBUS:
		...
		CcbCompleteList(&IdeDisk->ThreadCcbQueue, CCB_STATUS_RESET, Lurn->LurnStopReasonCcbStatusFlags);
		if(	LURN_IS_RUNNING(Lurn->LurnStatus) ) {
...
LurnIdeDiskDispatchCcb(
	...
	while(1) {
		...
		// Stop request must succeed any time.
		//
...
terminate_thread:
	...
	ACQUIRE_SPIN_LOCK(&Lurn->LurnSpinLock, &oldIrql);
	ASSERT(IsListEmpty(&IdeDisk->ThreadCcbQueue));
	Lurn->LurnStatus = LURN_STATUS_STOP_PENDING;
	CcbCompleteList(
...
#ifdef __ENABLE_WRITECACHE_CONTROL__
