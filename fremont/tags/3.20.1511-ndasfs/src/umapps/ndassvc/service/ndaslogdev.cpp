...
CNdasLogicalDevice::ReconcileFromNdasBus()
{
	...
	HANDLE hNdasScsi = INVALID_HANDLE_VALUE;
	fSuccess = ::NdasBusCtlQueryPdoFileHandle(
		m_NdasLocation, 
		&hNdasScsi);
	...
	{
		XTLTRACE2(NDASSVC_NDASLOGDEVICE, TRACE_LEVEL_ERROR,
			"QueryPdoFileHandle failed, slot=%d, error=0x%X\n", 
			m_NdasLocation, GetLastError());
	}
	else
	{
		...
		{
			XTLTRACE2(NDASSVC_NDASLOGDEVICE, TRACE_LEVEL_ERROR,
				"AddDeviceNotificationHandle failed, slot=%d, error=0x%X\n", 
				m_NdasLocation, GetLastError());
		}
	}
