	...
	__in_opt const NDASID_EXT_DATA* NdasIdExtension)
{
	...
	{
		CNdasDevicePtr pExistingDevice = Find(ndasDeviceId);
	...
	// register 
	CNdasDevicePtr pDevice(new CNdasDevice(SlotNo, ndasDeviceId, RegFlags, NdasIdExtension));
	if (0 == pDevice.get()) 
	{
	...
	XTLVERIFY( m_deviceIdMap.insert(std::make_pair(ndasDeviceId, pDevice)).second );
	//DeviceIdMap::value_type(ndasDeviceId, pDevice)).second;
