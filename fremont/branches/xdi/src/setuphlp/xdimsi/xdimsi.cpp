...
const XDIMSI_PROC_FUNC xMsiProcs[] = {
	pxDiMsiInitializeScheduledAction, NULL,
	pxDiMsiInstallFromInfSection, pxDiMsiInstallFromInfSection,
	pxDiMsiInstallLegacyPnpDevice, pxDiMsiInstallLegacyPnpDeviceRollback,
	pxDiMsiInstallPnpDeviceInf, pxDiMsiInstallPnpDeviceInfRollback,
	pxDiMsiInstallNetworkComponent, pxDiMsiInstallNetworkComponentRollback,
	pxDiMsiUninstallPnpDevice, NULL,
	pxDiMsiUninstallNetworkComponent, NULL,
	pxDiMsiCleanupOEMInf, NULL,
	pxDiMsiStartService, NULL,
	pxDiMsiStopService, NULL,
	pxDiMsiQueueScheduleReboot, NULL,
	pxDiMsiCheckServicesInfSection, NULL
};
...
public:
	...
	CXdiMsiProcessRecord(const XDIMSI_PROCESS_RECORD& record)
	{
		this->ProcessType = record.ProcessType;
		// action name is not a duplicate
		this->ActionName = record.ActionName;
		this->HardwareId = DuplicateString(record.HardwareId);
		this->InfPath = DuplicateString(record.InfPath);
		this->Flags = record.Flags;
		this->ErrorNumber = record.ErrorNumber;
		this->RegRoot = record.RegRoot;
		this->RegKey = DuplicateString(record.RegKey);
		this->RegName = DuplicateString(record.RegName);
		this->ProgressTicks = record.ProgressTicks;
	}

	~CXdiMsiProcessRecord()
	{
		// action name is not a duplicate
		// free(this->ActionName);
...
XDIMSI_PROCESS_TYPE 
pxDiMsiGetProcessType(LPCWSTR TypeString)
{
	...
	__deref_out LPWSTR* Template)
{
	...
	*Template = NULL;
	...
	{
		pxMsiTraceW(hInstall, L"MsiDatabaseOpenViewW failed, ret=0x%x\n", ret);
		return ret;
	}
	...
	{
		pxMsiTraceW(hInstall, L"MsiCreateRecord failed.\n");
		return ERROR_OUTOFMEMORY;
	}

	ret = MsiRecordSetStringW(hParamRecord, PActionField, Action);
	if (ret != ERROR_SUCCESS)
	{
		pxMsiTraceW(hInstall, L"MsiRecordSetString failed, ret=0x%x\n", ret);
		return ret;
	}
	...
	if (ret != ERROR_SUCCESS)
	{
		pxMsiTraceW(hInstall, L"MsiViewExecute failed, ret=0x%x\n", ret);
		return ret;
	}
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"MsiRecordGetString('Description') failed, ret=0x%x\n", ret);
		return ret;
	}
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"MsiRecordGetString('Template') failed, ret=0x%x\n", ret);
	...
	__in LPCWSTR ActionTemplate)
{
	...
	if (NULL == hDatabase)
	{
		pxTraceW(L"pxMsiSetActionText: MsiGetActiveDatabase failed.\n");
		return ERROR_INVALID_HANDLE_STATE;
	}
	...
	if (ERROR_SUCCESS !=  ret)
	{
		pxTraceW(L"DELETE: MsiDatabaseOpenViewW failed, ret=0x%x\n", ret);
	}
	else
	{
		...
		if (NULL != hRecord)
		{
			...
			if (ERROR_SUCCESS != ret)
			{
				pxTraceW(L"DELETE: MsiViewExecute failed, ret=0x%x\n", ret);
			}
		}
	}
	...
	if (ERROR_SUCCESS != ret)
	{
		pxTraceW(L"INSERT: MsiDatabaseOpenViewW failed, ret=0x%x\n", ret);
	}
	else
	{
		...
		if (NULL != hRecord)
		{
			...
			if (ERROR_SUCCESS != ret)
			{
				pxTraceW(L"INSERT: MsiViewExecute failed, ret=0x%x\n", ret);
#ifdef _DEBUG
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	...
	pxTraceW(L"\tAction=%s, Type=%d, HardwareId=%s, Inf=%s, Flags=0x%08X, Error=%d, Ticks=%d\n", 
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"pxMsiQueryActionText failed, ret=0x%x\n", ret);
	}
	else
	{
		pxTraceW(L"ActionDescription=%s, Template=%s\n",
			actionDescription, actionTemplate);
	}

	hr = pxDiMsiProcessDataCreate(&processData, ProcessRecord);

	if (FAILED(hr))
	{
		pxMsiTraceW(hInstall, L"pxMsiProcessDataCreate failed, hr=0x%x\n", hr);
	...
	if (FAILED(hr))
	{
		pxMsiTraceW(hInstall, L"pxMsiProcessDataEncode failed, hr=0x%x\n", hr);
		pxDiMsiProcessDataFree(processData);
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"MsiSetProperty(%ls) failed, ret=0x%x\n",
	...
	{
		// non-critical error
		pxMsiTraceW(hInstall, L"pxMsiSetActionText(%ls) failed, ret=0x%x\n", 
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"MsiDoActionW(%ls) failed, ret=0x%x\n",
			XDIMSI_ACTION_PROCESS_DRIVER_ROLLBACK, ret);
	...
	__deref_out LPWSTR* RegValueName)
{
	...
	*RegValueName = NULL;
	...
	if (NULL == hDatabase)
	{
		pxMsiTraceW(hInstall, L"MsiGetActiveDatabase failed\n");
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"MsiDatabaseOpenViewW(`Registry`) failed, ret=0x%x\n", ret);
		return ret;
	}
	...
	if (NULL == hRegParam)
	{
		pxMsiTraceW(hInstall, L"MsiCreateRecord(1) failed, ret=0x%x\n", ret);
		return ret;
	}
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"MsiViewExecute(`Registry`) failed, ret=0x%x\n", ret);
		return ret;
	}
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"MsiViewFetch(`Registry`) failed, ret=0x%x\n", ret);
		return ret;
	}
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"pxMsiRecordGetString(`RegQuery_Key`) failed, ret=0x%x\n", ret);
		return ret;
	}
	ret = pxMsiFormatRecord(hInstall, keyPathFormat, &keyPath, &length);
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"pxMsiFormatRecord(`RegQuery_Key`) failed, ret=0x%x\n", ret);
		free(keyPathFormat);
		return ret;
	}
	free(keyPathFormat);
	...
	pxTraceW(L" RegRoot=0x%x, KeyPath=%ls, ValueName=%ls\n", 
	...
	__in INT Phase)
{
	...
	{
		pxTraceW(L"\tAction=%ls\n", static_cast<LPCWSTR>(actionProperty));

		LPCWSTR actionName = actionProperty;
		LPWSTR actionDescription = NULL;
		LPWSTR actionTemplate = NULL;

		ret = pxMsiQueryActionText(hInstall, actionProperty, &actionDescription, &actionTemplate);
		if (ERROR_SUCCESS == ret)
		{
	...
	if (ret != ERROR_SUCCESS)
	{
		pxMsiTraceW(hInstall, L"MsiRecordSetInteger failed, ret=0x%x\n", ret);
		return ERROR_INSTALL_FAILURE;
	}
	...
	while (TRUE)
	{
		...
		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiViewFetch failed, ret=0x%x\n", ret);
			return ERROR_INSTALL_FAILURE;
		}
		...
		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiRecordGetString(`Condition`) failed, ret=0x%x\n", ret);
			return ERROR_INSTALL_FAILURE;
		}
		...
		{
			pxMsiTraceW(hInstall, L"MsiEvaluateCondition error, condition=%ls\n", condition);
			free(condition);
			return ERROR_INSTALL_FAILURE;
		}
		...
		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiRecordGetStringW(`Type`) failed, ret=0x%x\n", ret);
			return ERROR_INSTALL_FAILURE;
		}

		XDIMSI_PROCESS_TYPE processType = pxDiMsiGetProcessType(processTypeString);

		if (_XDIMSI_PROCESS_TYPE_INVALID == processType)
		{
			pxMsiTraceW(hInstall, L"Invalid process type, type=%ls\n", processTypeString);
			free(processTypeString);
			return ERROR_INSTALL_FAILURE;
		}
		...
		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiRecordGetStringW(`INF`) failed, ret=0x%x\n", ret);
			return ERROR_INSTALL_FAILURE;
		}

		pxTraceW(L"InfPathFormat=%ls\n", infPathFormat);
		...
		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiFormatRecord(`INF`) failed, ret=0x%x\n", ret);
			free(infPathFormat);
			return ERROR_INSTALL_FAILURE;
		}
		...
		pxTraceW(L"infPath=%ls\n", infPath);

		LPWSTR hardwareIdFormat = NULL;
		...
		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiRecordGetStringW(`HardwareId`) failed, ret=0x%x\n", ret);
			free(infPath);
			return ERROR_INSTALL_FAILURE;
		}

		pxTraceW(L"HardwareIdFormat=%ls\n", hardwareIdFormat);
		...
		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiFormatRecord(`HardwareId`) failed, ret=0x%x\n", ret);
			free(hardwareIdFormat);
			free(infPath);
			return ERROR_INSTALL_FAILURE;
		}

		pxTraceW(L"HardwareId=%ls\n", hardwareId);
		...
		ret = pxMsiRecordGetString(hFetchRecord, QActionField, &action, &length);

		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiRecordGetStringW(`Action`) failed, ret=0x%x\n", ret);
			free(hardwareId);
			free(infPath);
			return ERROR_INSTALL_FAILURE;
		}
		...
		DWORD regRoot = 0;
		LPWSTR regKeyPath = NULL;
		LPWSTR regValueName = NULL;
		...
		{
			LPWSTR registryId = NULL;
			ret = pxMsiRecordGetString(hFetchRecord, QRegistry, &registryId, &length);
			if (ERROR_SUCCESS != ret)
			{
				pxMsiTraceW(hInstall, L"MsiRecordGetStringW(`Registry_`) failed, ret=0x%x\n", ret);
				free(hardwareId);
				free(infPath);
				return ERROR_INSTALL_FAILURE;
			}
			
			ret = pxDiMsiGetRegistryData(
				hInstall, registryId, 
				&regRoot, &regKeyPath, &regValueName);

			if (ERROR_SUCCESS != ret)
			{
				pxMsiTraceW(hInstall, L"xDiMsiGetRegistryData failed, ret=0x%x\n", ret);
				free(registryId);
				free(hardwareId);
				free(infPath);
				return ERROR_INSTALL_FAILURE;
		...
		{
			pxMsiTraceW(hInstall, L"CXMsiProcessRecord creation failed, ret=0x%x\n", ERROR_OUTOFMEMORY);
			return ERROR_INSTALL_FAILURE;
		}
		...
		currentRecord->ActionName = action;
		currentRecord->HardwareId = hardwareId;
		currentRecord->InfPath = infPath;
		currentRecord->Flags = processFlags;
		currentRecord->ErrorNumber = errorNumber;
		currentRecord->RegRoot = regRoot;
		currentRecord->RegKey = regKeyPath;
		currentRecord->RegName = regValueName;
	...
	if (ERROR_SUCCESS != ret || 0 == initPropertyLength)
	{
		...
		record.ActionName = XDIMSI_ACTION_INITIALIZE;

		ret = MsiSetPropertyW(hInstall, XDIMSI_PROP_INITIALIZED, L"1");

		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"MsiSetProperty(XDIMSI_PROP_INITIALIZED) failed, ret=0x%X\n", ret);
			return ERROR_INSTALL_FAILURE;
		}

		ret = pxDiMsiProcessDriverRecord(hInstall, &record);

		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"pxMsiProcessRecord(pass0) failed, ret=0x%X\n", ret);
			return ERROR_INSTALL_FAILURE;
		}
	}
		...
		currentRecord = currentRecord->NextRecord)
	{
		...
		case XDIMSI_PROCESS_INSTALL_FROM_INF_SECTION:
			...
			record.ActionName = XDIMSI_ACTION_VALIDATE_DRIVER_SERVICES;
		...
		case XDIMSI_PROCESS_INSTALL_NETWORK_COMPONENT:
			record.ProcessType = _XDIMSI_PROCESS_CHECK_SERVICES_INF_SECTION;
			record.ActionName = XDIMSI_ACTION_VALIDATE_DRIVER_SERVICES;
			record.Flags = XDIMSI_CHECK_SERVICES_USE_HARDWARE_ID | XDIMSI_CHECK_SERVICES_USE_FORCEREBOOT;
			record.ProgressTicks = 2 * xDiProgressTick;
			break;
		...
		if (_XDIMSI_PROCESS_TYPE_INVALID != record.ProcessType)
		{
			...
			if (ERROR_SUCCESS != ret)
			{
				pxMsiTraceW(hInstall, L"pxMsiProcessRecord(pass1) failed, ret=0x%X\n", ret);
				return ERROR_INSTALL_FAILURE;
			}
		}
	}
	...
	for (currentRecord = processRecordList.Head; 
		...
		currentRecord = currentRecord->NextRecord)
	{
		ret = pxDiMsiProcessDriverRecord(hInstall, currentRecord);

		if (ERROR_SUCCESS != ret)
		{
			pxMsiTraceW(hInstall, L"pxMsiProcessRecord(pass2) failed, ret=0x%X\n", ret);
			return ERROR_INSTALL_FAILURE;
		}
	}
		...
		currentRecord = currentRecord->NextRecord)
	{
		...
		if (_XDIMSI_PROCESS_TYPE_INVALID != record.ProcessType)
		{
			...
			if (ERROR_SUCCESS != ret)
			{
				pxMsiTraceW(hInstall, L"pxMsiProcessRecord(pass3) failed, ret=0x%X\n", ret);
				return ERROR_INSTALL_FAILURE;
			}
		}
	}
	...
	__in MSIHANDLE hInstall)
{
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"GetProperty('XDIMSI_PROP_PROCESS_DRIVERS_PHASE') failed, code=0x%x\n", ret);
		return ERROR_INSTALL_FAILURE;
	}
	...
	{
		pxMsiTraceW(hInstall, L"Property 'XDIMSI_PROP_PROCESS_DRIVERS_PHASE' is not an integer.\n");
		// invalid phase
		return ERROR_INSTALL_FAILURE;
	}

	return pxDiMsiProcessDrivers(hInstall, phase);
}
	...
	__in MSIHANDLE hInstall)
{
	...
	PXDIMSI_PROCESS_DATA processData = NULL;
	DWORD size;
	HRESULT hr = pxDiMsiProcessDataDecode(cadata, &processData, &size);
	...
	pxMsiTraceW(hInstall, L"  %ls(S) Type=%d, HardwareId=%s, Inf=%s, Ticks=%d\n",
		actionName, processData->ProcessType, hardwareId, infPath, processData->ProgressTicks);
	...
	{
		MsiRecordSetStringW(hActionDataRecord, 1, actionName);
	...
	{
		pxMsiTraceW(hInstall, L"  Flags=0x%x\n", processData->Flags);
	}

	if (0 != processData->ErrorNumber)
	{
		pxMsiTraceW(hInstall, L"  ErrorDialog=%d\n", processData->ErrorNumber);
	}

	if (regKey && regKey[0])
	{
		pxMsiTraceW(hInstall, L"  Registry Root=0x%x, Key=%s, Name=%s\n",
	...
	processRecord.ActionName = actionName;
...
UINT __stdcall xDiMsiProcessDrivers1(MSIHANDLE hInstall) { return pxDiMsiProcessDrivers(hInstall, 1); }
UINT __stdcall xDiMsiProcessDrivers2(MSIHANDLE hInstall) { return pxDiMsiProcessDrivers(hInstall, 2); }
UINT __stdcall xDiMsiProcessDrivers3(MSIHANDLE hInstall) { return pxDiMsiProcessDrivers(hInstall, 3); }
UINT __stdcall xDiMsiProcessDrivers4(MSIHANDLE hInstall) { return pxDiMsiProcessDrivers(hInstall, 4); }
UINT __stdcall xDiMsiProcessDrivers5(MSIHANDLE hInstall) { return pxDiMsiProcessDrivers(hInstall, 5); }
	...
	__in MSIHANDLE hInstall)
{
	...
	{
		pxMsiTraceW(hInstall, L"xDiMsiUpdateScheduledReboot: Reboot is required to complete the installation\n");
		return MsiSetMode(hInstall, MSIRUNMODE_REBOOTATEND, TRUE);
	}
	else
	{
		pxMsiTraceW(hInstall, L"xDiMsiUpdateScheduledReboot: No reboot is necessary to complete the installation\n");
		return ERROR_SUCCESS;
	}
}
	...
	__in MSIHANDLE hInstall)
{
	...
	{
		pxMsiTraceW(hInstall, L"xDiMsiUpdateForceReboot: Reboot is required to continue the installation\n");
		...
		//
		pxMsiPatchRunOnceEntryInWin64(hInstall);
#endif
		return ret;
	}
	else
	{
		pxMsiTraceW(hInstall, L"xDiMsiUpdateForceReboot: No reboot is necessary to continue the installation\n");
		return ERROR_SUCCESS;
	}
}
	...
	__in MSIHANDLE hInstall)
{
	...
	if (ERROR_SUCCESS != ret)
	{
		pxMsiTraceW(hInstall, L"MsiGetProperty('ProductCode') failed, ret=0x%x\n", ret);
		return ret;
	}

	pxTraceW(L"\tProductCode=%ls\n", productCode);
	...
	if (ERROR_SUCCESS != result) 
	{
		...
		pxMsiTraceW(hInstall, L"RegOpenKeyEx(Wow6432Node...RunOnceEntries) failed, hr=0x%x\n", hr);
		return hr;
	}
	...
	{
		hr = HRESULT_FROM_WIN32(result);
		pxMsiTraceW(hInstall, L"RegQueryInfoKey(Wow6432Node...RunOnceEntries) failed, hr=0x%x\n", hr);
		return hr;
	}
	...
	{
		hr = E_OUTOFMEMORY;
		pxMsiTraceW(hInstall, L"EOUTOFMEMORY %d bytes, hr=0x%x\n", valueNameAllocLength * sizeof(WCHAR), hr);
	...
	for (DWORD index = 0; ; ++index)
	{
		...
		pxTraceW(L"RunOnceEntries: %ls=%ls\n", valueName, roentryValueData);

		if (NULL != StrStrI(roentryValueData, productCode))
		{
			pxMsiTraceW(hInstall, L"Patching RunOnceEntries: %ls=%ls\n", valueName, roentryValueData);
			...
			{
				hr = HRESULT_FROM_WIN32(result);
				pxMsiTraceW(hInstall, L"RegOpenKeyExW(Wow6432Node...RunOnce) failed, hr=0x%x\n", hr);
			...
			{
				hr = HRESULT_FROM_WIN32(result);
				pxMsiTraceW(hInstall, L"RegQueryValueExW(Wow6432Node...RunOnce) failed, hr=0x%x\n", hr);
	...
	if (runonceValueData)
	{
		...
		{
			hr = HRESULT_FROM_WIN32(result);
			pxMsiTraceW(hInstall, L"RegCreateKeyExW(RunOnce) failed, hr=0x%x\n", hr);
		}
		else
		{
			...
			{
				hr = HRESULT_FROM_WIN32(result);
				pxMsiTraceW(hInstall, L"RegSetValueExW(RunOnce) failed, hr=0x%x\n", hr);				
			}
			else
			{
				...
				{
					hr = HRESULT_FROM_WIN32(result);
					pxMsiTraceW(hInstall, L"RegCreateKeyExW(RunOnceEntries) failed, hr=0x%x\n", hr);				
				}
				else
				{
					result = RegSetValueExW(
					...
					{
						hr = HRESULT_FROM_WIN32(result);
						pxMsiTraceW(hInstall, L"RegSetValueExW(RunOnceEntries) failed, hr=0x%x\n", hr);				
					}
