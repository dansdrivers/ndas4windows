...
HRESULT
pxDiInfHandleModelContainsHardwareId(
	HINF InfFileHandle, 
	...
	LPCWSTR HardwareIdList)
{
	...
	do 
	{
		...
		hr = pxDiHardwareIdInList(infHardwareId, HardwareIdList);
...
HRESULT
pxDiInfHandleContainsHardwareId(
	HINF InfFileHandle,
	LPCTSTR HardwareIdList)
{
	...
	do 
	{
		...
		if (fieldCount < 2)
		{
			...
			hr = pxDiInfHandleModelContainsHardwareId(
	...
	LPCWSTR HardwareIdList)
{
	...
	hr = pxDiInfHandleContainsHardwareId(infHandle, HardwareIdList);
	...
	__in_opt LPVOID CallbackContext)
{
	...
	UINT n = GetSystemWindowsDirectory(infDir, MAX_PATH);
	_ASSERTE(0 != n);
	if (0 == n)
	{
	...
	success = PathAppend(infDir, _T("INF"));
	if (!success)
	{
		return E_FAIL;
	}

	hr = StringCchCopy(infFindSpec, MAX_PATH, infDir);
	...
	success = PathAppend(infFindSpec, FileFindSpec ? FileFindSpec : L"oem*.inf");
	_ASSERTE(success);
	if (!success)
	{
		return E_FAIL;
	...
	HANDLE findFileHandle = FindFirstFile(infFindSpec, &findFileData);
	...
	do 
	{
		hr = pxDiInfFileContainsHardwareId(findFileData.cFileName, HardwareIdList);
		if (S_OK == hr)
		{
		...
		success = FindNextFile(findFileHandle, &findFileData);
