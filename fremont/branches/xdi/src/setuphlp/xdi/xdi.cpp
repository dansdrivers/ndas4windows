...
HRESULT
WINAPI
pxDiServiceIsMarkedForDeletionInfContext(
	...
	__deref_out_opt LPWSTR* ServiceName)
{
	...
	while (found)
	{
		hr = pxDiServiceIsMarkedForDeletionInfContext(&infContext, ServiceName);
		if (S_FALSE != hr)
		{
			XTLTRACE1(TRACE_LEVEL_INFORMATION, "pxDiServiceIsMarkedForDeletionInfContext returned, hr=0x%x\n", hr);
			return hr;
		}
	...
	__out_opt LPBOOL NeedsRebootOnService)
{
	...
	if (FAILED(hr))
	{
		...
		XTLTRACE1(TRACE_LEVEL_ERROR, 
			"pxDiSetupInitQueueCallback failed, hr=0x%x\n", hr);
	...
	__deref_out LPWSTR* InfPath)
{
	...
	*InfPath = NULL;
	...
	WCHAR classGuidString[64];
	WCHAR instanceGuidString[64];
	...
	__deref_out_opt LPWSTR* OEMInfName)
{
	...
	if (OEMInfName)
	{
		...
		hr = pxDiGetNetComponentInfPath(&classGuid, &instanceGuid, OEMInfName);
		if (FAILED(hr))
		{
			XTLTRACE1(TRACE_LEVEL_ERROR,
				"pxDiGetNetComponentInfPath failed, hr=0x%x\n", hr);
	...
	__out_opt LPWSTR* AssociatedInfNameList)
{
	...
	HRESULT hr = xDiEnumDevices(
		...
		pxDiEnumCallbackForRemove, &context);
...
BOOL CALLBACK
pxDiEnumDriverFilesCallbackForDelete(
	...
	__in LPVOID Context)
{
	...
	{
		MoveFileExW(TargetFilePath, NULL, MOVEFILE_DELAY_UNTIL_REBOOT);
		*reboot = TRUE;
	}
			...
			__try {
				...
				for (DWORD i = 0; i < depCount; i++ ) 
				{
					...
					__try 
					{
						...
						{
							XTLTRACE1(TRACE_LEVEL_ERROR,
								"pxDiStopService(%ls) failed, hr=0x%x.\n", ess.lpServiceName, hr);
							return hr;
						}
					} 
					__finally 
