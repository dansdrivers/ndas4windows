...
ReplaceEula(MSIHANDLE hInstall)
{
	...
	XTL::AutoProcessHeapPtr<CHAR> lpEula = pReadEulaFromFileA(hInstall);
    ...
    MSIHANDLE hInstall)
{
    ...
    XTL::AutoProcessHeapPtr<TCHAR> pszEulaFileName = 
    ...
    XTL::AutoProcessHeapPtr<TCHAR> pszEulaSourceDir = 
        pMsiGetProperty(hInstall, _T("EulaSourceDir"), &cchEulaSourceDir);
    
    if (NULL == (LPTSTR) pszEulaSourceDir)
    {
    ...
    XTL::AutoProcessHeapPtr<TCHAR> pszEulaFilePath = 
	...
	XTL::AutoProcessHeapPtr<CHAR> lpEula = pReadTextFromFileA(pszEulaFilePath);
    ...
    LPCTSTR szFileName)
{
    ...
    XTL::AutoProcessHeapPtr<CHAR> lpBuffer = (LPSTR) ::HeapAlloc(
        GetProcessHeap(), 
        HEAP_ZERO_MEMORY, 
        cbToRead);
