	...
	lsp_uint8_t subcommand_specific_3 /* lba high register */)
{
	...
	/* set registers */
	idereg.device.dev = (0 == h->session.login_info.unit_no) ? 0 : 1;
	idereg.device.lba = 0;
	...
	__out lsp_uint32_t* fixed_lba24)
{
	...
	*/
	if ((devdata->num_cylinders == 16383 ||
		(devdata->num_cylinders == 4092 && devdata->number_of_current_cylinders == 16383)) &&
		devdata->num_sectors_per_track == 63 &&
		(devdata->num_heads == 15 || devdata->num_heads == 16) &&
		devdata->lba28_capacity >= (lsp_uint32_t)(16383 * 63 * devdata->num_heads))
	{
		return 1;
	}
	...
	chs_sects = devdata->num_cylinders * devdata->num_heads * devdata->num_sectors_per_track;
	...
	__out lsp_large_integer_t* capacity)
{
	...
	lsp_ide_identify_device_data_t *info = (lsp_ide_identify_device_data_t *)_info;
	...
	__in void* cdb)
{
	...
	if (session->handshake_data.lba)
	{
		...
		{
			if (location->u.high >= 0x00000001 || location->u.low >= 0x10000000)
			{
				return LSP_STATUS_INVALID_PARAMETER;
			}
		}
		...
		)
	{
		...
		idereg.command.command = LSP_IDE_CMD_PACKET; // this will set com_type_p to 1
		idereg.use_lba48 = 0; // should be auto set. assert
		idereg.use_lba = !!session->handshake_data.lba; // should be auto set. assert
		
		idereg.device.dev = (0 == session->login_info.unit_no) ? 0 : 1;
	...
	{
		idereg.use_lba48 = !!session->handshake_data.lba48;
		idereg.use_lba = !!session->handshake_data.lba;
		idereg.device.dev = (0 == session->login_info.unit_no) ? 0 : 1;
		idereg.device.lba = !!session->handshake_data.lba;
		idereg.use_dma = !!session->handshake_data.dma;
	...
	else
	{
		...
		idereg.device.lba_head_nr = (lsp_uint8_t)((location->u.low & 0x0F000000) >> 24);
	}
	...
	__in lsp_uint32_t buflen)
{
	return lsp_build_ide_base_command(
		request, h, location, sectors, buf, buflen, LSP_IDE_BASE_WRITE, NULL);
}
...
{
	return lsp_build_ide_base_command(
		request, h, location, sectors, buf, buflen, LSP_IDE_BASE_READ, NULL);
}
...
{
	return lsp_build_ide_base_command(
		request, h, location, sectors, reserved_1, reserved_2, LSP_IDE_BASE_VERIFY, NULL);
}
	...
	__in lsp_uint8_t buf_to_target)
{
	...
	location.u.low = buflen << 8; // see the spec of PACKET section in ATA-ATAPI
...
lsp_build_ide_identify(
	__inout lsp_request_packet_t* request,
	__in lsp_handle_t h,
	__out_bcount(sizeof(lsp_ide_identify_device_data_t)) lsp_ide_identify_device_data_t* ident)
{
	lsp_ide_register_param_t idereg;
	lsp_io_data_buffer_t data_buf;
