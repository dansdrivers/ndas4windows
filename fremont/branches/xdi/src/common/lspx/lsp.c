...
#define LSP_MAKE_ERROR(FUNC, PHASE, TYPE, RESPONSE)			\
	...
	lsp_handle_context_t* context)
{
	...
	pdu_hdr->op_data.login.ver_max = session->hardware_data.hardware_version; // phase specific
	...
	lsp_handle_context_t* context)
{
	...
	{
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_LOGIN, sub_seq_num, LSP_ERR_TYPE_HEADER, 0);
	}

	// Set hardware_data.hardware_version, hardware_data.protocol_version with detected one.
	if (LSP_ERR_RESPONSE_SUCCESS != pdu_hdr->response)
	{
		/* phase specific */
		if (LSP_ERR_RESPONSE_RI_VERSION_MISMATCH == pdu_hdr->response)
		{
			if (session->hardware_data.hardware_version != pdu_hdr->op_data.login_response.ver_active)
			{
				session->hardware_data.hardware_version = pdu_hdr->op_data.login_response.ver_active;
		...
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_LOGIN, sub_seq_num, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
	}
	...
	case LSP_HARDWARE_VERSION_1_0:
		session->hardware_data.protocol_version = LSP_IDE_PROTOCOL_VERSION_1_0;
		break;
	case LSP_HARDWARE_VERSION_1_1:
	case LSP_HARDWARE_VERSION_2_0:
		session->hardware_data.protocol_version = LSP_IDE_PROTOCOL_VERSION_1_1;
		break;
	default:
	...
	/* new in 2.0g */
	session->hardware_data.hardware_revision = lsp_ntohs(pdu_hdr->op_data.login_response.revision);

	status = lspp_is_valid_data_segment(context, LSP_BINPARM_SIZE_LOGIN_FIRST_REPLY);
	...
	{
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_LOGIN, sub_seq_num, LSP_ERR_TYPE_REPLY_PARM, 0);
	}
	...
	lsp_handle_context_t* context)
{
	...
	pdu_hdr->cmd_subpkt_seq = lsp_htons(sub_seq_num);
	pdu_hdr->path_cmd_tag = lsp_htonl(session->path_cmd_tag);
	pdu_hdr->op_data.login.ver_max = session->hardware_data.hardware_version;
	...
	lsp_handle_context_t* context)
{
	...
	status = lspp_is_valid_data_segment(context, LSP_BINPARM_SIZE_LOGIN_SECOND_REPLY);
	if (LSP_STATUS_SUCCESS != status)
	{
		return status;
	}
	...
	lsp_handle_context_t* context)
{
	...
	lspp_set_ahs_or_ds_len(context, LSP_BINPARM_SIZE_LOGIN_THIRD_REQUEST);
	...
	user_id =
		(lsp_htonll(*(lsp_uint64_t *)login_info->supervisor_password)) ? // supervisor_password is set
			LSP_NDAS_SUPERVISOR :
	...
	{
		LSP_ASSERT(session->wlk_pwd_index < countof(LSP_KNOWN_LOGIN_PASSWORD_LIST));
		lsp_memcpy(
			login_info->password,
	...
	lsp_handle_context_t* context)
{
	...
	status = lspp_is_valid_data_segment(context, LSP_BINPARM_SIZE_LOGIN_THIRD_REPLY);
	if (LSP_STATUS_SUCCESS != status)
	{
		return status;
	}
	...
	lsp_handle_context_t* context)
{
	...
	lspp_set_ahs_or_ds_len(context, LSP_BINPARM_SIZE_LOGIN_FOURTH_REQUEST);
	/* additional payload */
	...
	lsp_handle_context_t* context)
{
	...
	// CHECK_DATA_SEGMENT(pdu, pdu_hdr, session->hardware_data.protocol_version, sub_seq_num, LSP_BINPARM_SIZE_LOGIN_FOURTH_REPLY);
	status = lspp_is_valid_data_segment(context, LSP_BINPARM_SIZE_LOGIN_FOURTH_REPLY);
	if (LSP_STATUS_SUCCESS != status)
	{
		return status;
	}
	...
	lsp_handle_context_t* context)
{
	...
	{
		session->session_phase = LSP_PHASE_LOGOUT;
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_LOGOUT, 2, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
	}
...
lsp_ide_command_prepare_v0(lsp_handle_context_t* context)
{
	...
	LSP_ASSERT(LSP_IDE_PROTOCOL_VERSION_1_0 == session->hardware_data.protocol_version);
	...
	/* translate command */
	{
		/* command_ptr is used temporary here to translate 'const' command */
		/* you may see warning like 'initialization discards qualifiers from pointer target type' */
		lsp_uint8_t *command_ptr = &(p->command.command);
		switch (*command_ptr)
		{
		/* commands translation required */
		case LSP_IDE_CMD_READ_SECTORS:
			*command_ptr = LSP_IDE_CMD_READ_DMA;
			break;
		case LSP_IDE_CMD_READ_SECTORS_EXT:
			*command_ptr = LSP_IDE_CMD_READ_DMA_EXT;
			break;
		case LSP_IDE_CMD_WRITE_SECTORS:
			*command_ptr = LSP_IDE_CMD_WRITE_DMA;
			break;
		case LSP_IDE_CMD_WRITE_SECTORS_EXT:
			*command_ptr = LSP_IDE_CMD_WRITE_DMA_EXT;
			break;
...
lsp_ide_command_process_v0(lsp_handle_context_t* context)
{
	...
	if (LSP_ERR_RESPONSE_SUCCESS != pdu_hdr->response)
	{
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_IDE_COMMAND, 4, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
	}
...
lsp_ide_command_prepare_v1(lsp_handle_context_t* context)
{
	...
	LSP_ASSERT(LSP_IDE_PROTOCOL_VERSION_1_1 == session->hardware_data.protocol_version);
	...
	*(lsp_uint32_t *)ide_header = lsp_htonl(*(lsp_uint32_t *)ide_header);
	...
	if (ext_cmd->cmd_size > 0)
	{
		...
		lsp_memcpy(
			...
			ext_cmd->cmd_buffer, 
			ext_cmd->cmd_size);
		// SET_AHS_OR_DATA_SEG(pdu, session->hardware_data.protocol_version, context->pdu_buffer + sizeof(lsp_pdu_hdr));
	}

	/* data for packet command should be aligned to 4 bytes */
	if  (
		LSP_IDE_CMD_PACKET == p->command.command &&
		(
	...
	{
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_IDE_COMMAND, 2, LSP_ERR_TYPE_DATA_LEN, 0);
	}
	...
	lsp_handle_context_t* context)
{
	...
	if (LSP_ERR_RESPONSE_SUCCESS != pdu_hdr->response)
	{
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_VENDOR_COMMAND, 4, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
	}
	...
	lsp_handle_context_t* context)
{
	...
	if (LSP_ERR_RESPONSE_SUCCESS != pdu_hdr->response)
	{
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_TEXT_COMMAND, 4, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
	}
	...
	lsp_handle_context_t* context)
{
	...
	if (LSP_ERR_RESPONSE_SUCCESS != pdu_hdr->response)
	{
		return LSP_MAKE_ERROR(LSP_ERR_FUNC_NOOP, 4, LSP_ERR_TYPE_RESPONSE, pdu_hdr->response);
	}
#else
	context = context;
...
reset_phase:
	...
	case LSP_SP_STATE_PREPARE_SEND_HEADER:
		...
		if (LSP_STATUS_SUCCESS != status)
		{
			...
			return lsp_complete_request(context, context->session.current_request, status);
		}
...
/* TODO: LBA48 unique commands */

/*
switch (ide_register->command)
{
case LSP_IDE_CMD_FLUSH_CACHE_EXT:
case LSP_IDE_CMD_READ_DMA_EXT:
...
case LSP_IDE_CMD_READ_SECTORS_EXT:
case LSP_IDE_CMD_READ_VERIFY_SECTORS_EXT:
case LSP_IDE_CMD_SET_MAX_ADDRESS_EXT:
case LSP_IDE_CMD_WRITE_DMA_EXT:
case LSP_IDE_CMD_WRITE_DMA_FUA_EXT:
case LSP_IDE_CMD_WRITE_DMA_QUEUED_FUA_EXT:
case LSP_IDE_CMD_WRITE_MULTIPLE_EXT:
case LSP_IDE_CMD_WRITE_MULTIPLE_FUA_EXT:
case LSP_IDE_CMD_WRITE_SECTORS_EXT:
}
*/
...
lsp_status_t
lsp_call
lsp_request_ex_ata_handshake_identify_completion(
	lsp_handle_t h,
	struct _lsp_request_packet_t* request);

lsp_status_t
lsp_call
lsp_request_ex_ata_handshake_set_feature_completion(
	__in lsp_handle_t h,
	...
	lsp_request_packet_t* request)
{
	...
	LSP_ASSERT(request->type == LSP_REQUEST_EX_ATA_HANDSHAKE_COMMAND);
	...
	internal_request->original_request = request;
...
lsp_status_t
lsp_call
lsp_request_ex_ata_handshake_integrity_check(
...
int
lsp_call
lsp_request_ex_ata_handshake_transfer_mode(
	__inout lsp_ata_handshake_data_t* handshake_data,
	...
	__in const lsp_ide_identify_device_data_t* ident)
{
	...
	if (ident->capabilities.dma_supported)
	{
			...
			hardware_revision != LSP_HARDWARE_V20_REV_0))
		{
			...
			lsp_debug("UDMA: (bits) support=%02X, active=%02X", support_bits, active_bits);
		...
		if (LSP_IDE_TRANSFER_MODE_UNSPECIFIED == transfer_mode)
		{
			...
			lsp_debug("MWDMA: (bits) support=%02X, active=%02X", support_bits, active_bits);
		...
		if (LSP_IDE_TRANSFER_MODE_UNSPECIFIED == transfer_mode)
		{
			...
			lsp_debug("SWDMA: (bits) support=%02X, active=%02X", support_bits, active_bits);
	...
	if (LSP_IDE_TRANSFER_MODE_UNSPECIFIED == transfer_mode)
	{
		/* TODO: clarify PIO modes */
		/* DMA mode is not specified, use PIO */
		transfer_mode = LSP_IDE_TRANSFER_MODE_PIO_DEFAULT;
		highest_level = 3;
	}

	LSP_ASSERT(transfer_mode != LSP_IDE_TRANSFER_MODE_UNSPECIFIED);

	if (transfer_mode <= LSP_IDE_TRANSFER_MODE_PIO_FLOW_CONTROL)
	{
		handshake_data->dma = 0;
		handshake_data->active_mode =  transfer_mode;
		handshake_data->active_level = highest_level;
	}
	else
	{
		handshake_data->dma = 1;
		handshake_data->active_mode =  transfer_mode;
		handshake_data->active_level = highest_level;
	}
...
lsp_status_t
lsp_call
lsp_request_ex_ata_handshake_identify_packet_device_completion(
	__in lsp_handle_t h,
	__in struct _lsp_request_packet_t* request)
{
	lsp_ide_identify_device_data_t* ident;
	...
	ident = (lsp_ide_identify_device_data_t*) 
		request->u.ide_command.request.data_buf.recv_buffer;
	...
	request->original_request->u.ata_handshake_command.response.handshake_data = handshake_data;
	request->original_request->u.ata_handshake_command.response.identify_data.ata = &h->session.identify_data.ata;
	...
	__in struct _lsp_request_packet_t* request)
{
	...
	if (LSP_STATUS_SUCCESS != request->status)
	{
		if (LSP_IDE_CMD_IDENTIFY_PACKET_DEVICE != request->u.ide_command.request.reg.command.command)
		{
	...
	*/
	...
	{
		status = lsp_request_ex_ata_handshake_integrity_check(ident);
		if (LSP_STATUS_SUCCESS != status)
		{
			return lsp_complete_request(h, request->original_request, status);
		}
	...
	if (ident->capabilities.lba_supported)
	{
		...
		{
			/* LBA 48 */
			handshake_data->lba_capacity.u.low = lsp_letohl(ident->lba48_capacity_lsw);
			handshake_data->lba_capacity.u.high = lsp_letohl(ident->lba48_capacity_msw);
		}
		else
		{
			/* LBA 28 */
			/* 0x0FFFFFFF is the maximum LBA in LBA28 */
			handshake_data->lba_capacity.u.low = lsp_letohl(ident->lba28_capacity);
	...
	{
		/* CHS mode */
		handshake_data->lba_capacity.u.low = lsp_letohs(ident->num_cylinders);
		handshake_data->lba_capacity.u.low *= lsp_letohs(ident->num_heads);
		handshake_data->lba_capacity.u.low *= lsp_letohs(ident->num_sectors_per_track);
		handshake_data->lba_capacity.u.high = 0;
	}
	...
	{
		handshake_data->support.write_cache = ident->command_set_support.write_cache;
		handshake_data->support.smart_commands = ident->command_set_support.smart_commands;
		handshake_data->support.smart_error_log = ident->command_set_support.smart_error_log;
		handshake_data->support.write_fua = ident->command_set_support.write_fua;
	}
	if (0 == ident->command_set_active.valid_clear_to_zero &&
		1 == ident->command_set_active.valid_set_to_one)
	{
		handshake_data->active.write_cache = ident->command_set_active.write_cache;
		handshake_data->active.smart_commands = ident->command_set_active.smart_commands;
		handshake_data->active.smart_error_log = ident->command_set_active.smart_error_log;
	}

	/* transfer mode negotiation */

	require_set_transfer_mode = lsp_request_ex_ata_handshake_transfer_mode(
		handshake_data,
		h->session.hardware_data.hardware_version,
		h->session.hardware_data.hardware_revision,
		ident);

	if (require_set_transfer_mode)
	{
		lsp_request_packet_t* internal_request, *original_request;
		...
		idereg.reg.named.features = LSP_IDE_SET_FEATURES_SET_TRANSFER_MODE;
		idereg.reg.named.sector_count = 
			/* 7:3 5 bits */ handshake_data->active_mode |
			/* 2:0 3 bits */ handshake_data->active_level;
	...
	__in struct _lsp_request_packet_t* request)
{
	...
	internal_request->original_request = original_request;
	internal_request->completion_routine = lsp_request_ex_ata_handshake_identify_completion;
	...
	__inout lsp_request_packet_t* request)
{
	...
	case LSP_REQUEST_IDE_COMMAND:
		...
		if (request != &h->session.internal_packets[1] &&
