...
protected:
	...
	//
	// Device number ( disk number )
	// "PhysicalDrive%d"
	//

	DWORD m_dwDeviceNumberHint;
...
public:
	...
	DWORD GetNdasLogicalUnitAddress();
	...
	DWORD GetDeviceNumberHint();

	DWORD GetRaidSlotNo();
	
	//
	// Set Fault after NDASSCSI_ADAPTERINFO_STATUSFLAG_MEMBER_FAULT
	//
	...
	//
	void OnMounted(DWORD DeviceNumberHint = -1);
...
protected:
	...
	BOOL _CheckPlugInCondForDVD(ACCESS_MASK requestingAccess);
...
//	DWORD _GetMaxRequestBlocks();
	...
	bool _IsWriteAccessAllowed(BOOL fPSWriteShare, CNdasUnitDevicePtr pUnitDevice);
