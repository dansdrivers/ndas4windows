...
)
{
	...
	try{
		RC = xixcore_RawReadVolumeLotHeader(
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
				("Fail  xixfs_CheckBootSector:XixFsRawReadVolumeLotHeader  .\n"));
			try_return(RC);
		}
		...
		pBootSector = (PPACKED_BOOT_SECTOR)xixcore_GetDataBuffer(buffer);
...
*/
	...
	//	Added by ILGU HONG end
		...
		*/
		...
		RC=xixfs_GetSuperBlockInformation(VCB);
...
)
{
	...
	for(i = XIFS_RESERVED_LOT_SIZE; i<pVCB->XixcoreVcb.NumLots; i++){
		...
		if(tLcn >= (uint32)(StartingCluster + BytesToCopy*8)){
			...
			if( xixcore_TestBit(i, xixcore_GetDataBuffer(pVCB->XixcoreVcb.MetaContext.HostFreeLotMap->Data) ) ){
				uint32	NumSetBit = 0;
				uint32	Offset = 0;
				uint32	Margin = 0;
