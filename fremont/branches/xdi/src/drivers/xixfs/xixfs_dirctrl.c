...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, xixfs_InitializeEnumeration)
#pragma alloc_text(PAGE, xixfs_NameInExpression) 
#pragma alloc_text(PAGE, xixfs_EnumerateIndex) 
#pragma alloc_text(PAGE, xixfs_QueryDirectory)
#pragma alloc_text(PAGE, xixfs_NotifyCheck) 
#pragma alloc_text(PAGE, xixfs_NotifyDirectory)
#pragma alloc_text(PAGE, xixfs_CommonDirectoryControl)
#endif
	...
	try
	{
		...
		while(1)
		{
			...
			if (!Found) {
				...
				if(!NT_SUCCESS(RC)){
					if(RC == STATUS_FILE_CORRUPT_ERROR) {
						continue;
					}else {
						try_return(RC);
					}
			...
			try
			{
				...
				case FileDirectoryInformation:
					...
					if(FileContext.pSearchedFCB){
						PXIXFS_FCB pCurrentFCB = FileContext.pSearchedFCB;

						DirInfo->LastWriteTime.QuadPart = 
							DirInfo->ChangeTime.QuadPart = pCurrentFCB->XixcoreFcb.Modified_time;
						DirInfo->LastAccessTime.QuadPart = pCurrentFCB->XixcoreFcb.Access_time;
						DirInfo->CreationTime.QuadPart = pCurrentFCB->XixcoreFcb.Create_time;
						DirInfo->FileAttributes = pCurrentFCB->XixcoreFcb.FileAttribute;
