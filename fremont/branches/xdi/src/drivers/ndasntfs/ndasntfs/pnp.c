...

{
    ...
    try {
		...
		if (OurDeviceObject->Secondary) {
			...
			NDASNTFS_ASSERT( OurDeviceObject->NetdiskEnableMode == NETDISK_SECONDARY || 
            ...
            if ((Vcb->CleanupCount > 0) ||
                ...
                if ((Vcb->CloseCount - (Vcb->SystemFileCloseCount + Vcb->QueuedCloseCount)) > 0) {
                    ...
                    if (NT_SUCCESS( Status )) {
						...
						NDASNTFS_ASSERT( Vcb->SecondaryCleanupCount == 0 );
						NDASNTFS_ASSERT( Vcb->SecondaryCloseCount == 0 );
						NDASNTFS_ASSERT( Vcb->SecondaryReadOnlyCloseCount == 0 );
						NDASNTFS_ASSERT( Vcb->SecondarySystemFileCloseCount == 0 );
#endif
                    }
                }
            }
