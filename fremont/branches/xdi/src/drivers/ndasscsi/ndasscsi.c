...

{
    ...
    KDPrint(2,("Major Ver %d, Minor Ver %d, Build %d Checked:%d\n",
										_NdscGlobals.MajorVersion,
										_NdscGlobals.MinorVersion,
										_NdscGlobals.BuildNumber,
	...
	KDPrint(2,("IsaBus type: NTSTATUS=%08lx.\n", isaStatus));
	...
	while(NT_SUCCESS(isaStatus)) {
		...
		//
		RtlZeroMemory( &_NdscGlobals.DraidGlobal, sizeof(DRAID_GLOBALS) );

		status = NdasRaidStart(&_NdscGlobals.DraidGlobal);
		if (!NT_SUCCESS(status)) {
			ASSERT(FALSE);
		...
		status = LsuRegisterTdiPnPHandler(&svcName, 
			...
			NdasRaidTdPnPPowerHandler, &_NdscGlobals.TdiPnP);
#endif
...
{
	NTSTATUS	status;
	KDPrint(2,("entered. DriverObject=%p\n", DriverObject));
...
#if	__NDAS_SCSI_OLD_VERSION__
	NdasRaidStop(&_NdscGlobals.DraidGlobal);
#endif
...
MiniCleanupWorkItemQueue(
	...
	while(TRUE)	{
		...
		KDPrint(2, ("Canceled work item %p\n", workItem));
		ASSERT(FALSE);
	}
}
...
MiniDriverThreadProc(
	...
	KDPrint(2,("Terminating the worker thread.\n"));
...
AddLurDescDuringFindAdapter(
	...
	KDPrint(2,("SIZE_OF_LURELATION_DESC() = %d\n", SIZE_OF_LURELATION_DESC()));
	KDPrint(2,("SIZE_OF_LURELATION_NODE_DESC(0) = %d\n", SIZE_OF_LURELATION_NODE_DESC(0)));
    ...
    )
{
	...
	if(firstInstallation) {
		...
		KDPrint(2, ("First time installation. Do not enumerate a device.\n"));
	}
	...
	KDPrint(2,("AdapterMaxDataTransferLength= 0x%x\n", HwDeviceExtension->AdapterMaxDataTransferLength));
	KDPrint(2,("Default NumberOfPhysicalBreaks = 0x%x\n", ConfigInfo->NumberOfPhysicalBreaks));
	KDPrint(2,("Default MaximumTranferLength = 0x%x\n", ConfigInfo->MaximumTransferLength));
	...
	KDPrint(2,("Set MaximumTranferLength = 0x%x\n", ConfigInfo->MaximumTransferLength));
    ...
    )
{
	...
	KDPrint(2,("AdapterMaxDataTransferLength:%d\n",
						HwDeviceExtension->AdapterMaxDataTransferLength
		));
    ...
    )
{
	...
	if(!NT_SUCCESS(status)) {
		KDPrint(2, ("SendCcbToAllLIRs() failed. STATUS=0x%08lx\n", status));
	}
...
MiniCompleteLeftoverCcb(
	...
	do {
		...
		KDPrint(2,("completed SRB:%p SCSIOP:%x\n", srb, srb->Cdb[0]));
...
MiniStopAdapter(
	...
	if(ADAPTER_ISSTATUS(HwDeviceExtension, ADAPTER_STATUS_STOPPING)) {
		...
		RELEASE_SPIN_LOCK(&HwDeviceExtension->LanscsiAdapterSpinLock, oldIrql);
		KDPrint(2,("Already stopping in progress.\n"));
	...
	KDPrint(2,("HwDeviceExtension->RequestExecuting:%u\n",
...
MiniRestartAdapter(
	...
	if(ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_ABNORMAL_TERMINAT)) {
		...
		RELEASE_SPIN_LOCK(&HwDeviceExtension->LanscsiAdapterSpinLock, oldIrql);
		KDPrint(2,("Abnormal stop in progress. Can not re-init the LUR.\n"));
		return;
	}
	if(ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_RESTARTING)) {

		RELEASE_SPIN_LOCK(&HwDeviceExtension->LanscsiAdapterSpinLock, oldIrql);
		KDPrint(2,("Already reinitialization in progress.\n"));
		return;
	}
	...
	if(HwDeviceExtension->LURs[0] != NULL) {
		...
		KDPrint(2,("LUR[0] already exists.\n"));
		ASSERT(FALSE);
		return;
	}
    ...
    )
{
	...
	KDPrint(2,("ControlType = %d KeGetCurrentIrql()= 0x%x\n",
    ...
    case ScsiQuerySupportedControlTypes:
		...
		KDPrint(2,("ScsiQuerySupportedControlTypes.\n"));
	...
	case ScsiSetBootConfig:
		...
		KDPrint(2,("ScsiSetBootConfig.\n"));
		status = ScsiAdapterControlSuccess;
		break;

	case ScsiSetRunningConfig:

		KDPrint(2,("ScsiSetRunningConfig.\n"));
		status = ScsiAdapterControlSuccess;
		break;

	case ScsiRestartAdapter:

		KDPrint(2,("ScsiRestartAdapter.\n"));
    ...
    case ScsiAdapterControlMax:
		...
		KDPrint(2,("ScsiAdapterControlMax.\n"));
...
//		ASSERT(FALSE);
	...
	case SCSIOP_INQUIRY: {
		...
		ACQUIRE_SPIN_LOCK(&HwDeviceExtension->LanscsiAdapterSpinLock, &oldIrql);
		if(ADAPTER_ISSTATUS(HwDeviceExtension, ADAPTER_STATUS_STOPPING)) {
			KDPrint(2,("SCSIOP_INQUIRY: ADAPTER_STATUS_STOPPING. returned with error.\n"));
		...
		for(idx_lur = 0; idx_lur < HwDeviceExtension->LURCount; idx_lur ++) {
			...
			if(	LUR &&
				...
				KDPrint(2,("SCSIOP_INQUIRY: set LUR(%p) to LuExtension(%p)\n", LUR, LuExtension));
				break;
			}
		...
		if(LUR == NULL) {
			KDPrint(2,("LUR == NULL LuExtension(%p) HwDeviceExtension(%p), HwDeviceExtension->LURCount(%d)\n",
    ...
    )
{
	...
	if(ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_BUSRESET_PENDING)) {
		...
		KDPrint(2,("Bus Reset status cleared.\n"));
	}
	RELEASE_SPIN_LOCK(&HwDeviceExtension->LanscsiAdapterSpinLock, oldIrql);
	...
	if(ADAPTER_ISSTATUS(HwDeviceExtension, ADAPTER_STATUS_STOPPING) ||
...
#if DBG
		if(ADAPTER_ISSTATUS(HwDeviceExtension, ADAPTER_STATUS_STOPPING)) {
			KDPrint(2,("Error! stopping in progress due to internal error.\n"));
		} else {
			KDPrint(2,("Delay the request due to reinit.\n"));
		}
#endif
		KDPrint(2,("Func:%x %s(0x%02x) Srb(%p) CdbLen=%d\n",
			Srb->Function,
		...
		KDPrint(2,(" TxLen:%d TO:%d SenseLen:%d SrbFlags:%08lx\n",
	...
	if(Srb->Function != SRB_FUNCTION_EXECUTE_SCSI) {
		KDPrint(2,("--->0x%x, Srb->Function = %s, Srb->PathId = %d, Srb->TargetId = %d Srb->Lun = %d\n",
	...
	case SRB_FUNCTION_ABORT_COMMAND:
		KDPrint(2,("SRB_FUNCTION_ABORT_COMMAND: Srb = %p Srb->NextSrb = %p, Srb->CdbLength = %d\n", Srb, Srb->NextSrb, (UCHAR)Srb->CdbLength ));
		NDScsiLogError(
			HwDeviceExtension,
			Srb,
			Srb->PathId,
	...
	case SRB_FUNCTION_SHUTDOWN:
		KDPrint(2,("SRB_FUNCTION_SHUTDOWN: Srb = %p Ccb->AbortSrb = %p, Srb->CdbLength = %d\n", Srb, Srb->NextSrb, (UCHAR)Srb->CdbLength ));
#if 1
	...
	case SRB_FUNCTION_FLUSH: 
		KDPrint(2,("SRB_FUNCTION_FLUSH: Srb = %p Ccb->AbortSrb = %p, Srb->CdbLength = %d\n", Srb, Srb->NextSrb, (UCHAR)Srb->CdbLength ));
#if 1
		break; // Pass to lower LUR
#else
		ScsiPortNotification(
	...
	case SRB_FUNCTION_IO_CONTROL:
		KDPrint(2,("SRB_FUNCTION_IO_CONTROL: Srb = %p Ccb->AbortSrb = %p, Srb->CdbLength = %d\n", Srb, Srb->NextSrb, (UCHAR)Srb->CdbLength ));

		status = MiniSrbControl(HwDeviceExtension, luExtension, Srb);
		if(status == STATUS_PENDING) {
			KDPrint(2,("SRB_FUNCTION_IO_CONTROL: Srb pending.\n" ));
			return TRUE;
			...
			KDPrint(2,("SRB_FUNCTION_IO_CONTROL: Srb completed.\n" ));
			return TRUE;
		}
		KDPrint(2,("SRB_FUNCTION_IO_CONTROL: going to LUR\n" ));
		break;
	...
	case SRB_FUNCTION_EXECUTE_SCSI:
		...
		if(status != STATUS_MORE_PROCESSING_REQUIRED) {
			...
			KDPrint(2,("SRB_FUNCTION_EXECUTE_SCSI: Srb = %p completed.\n", Srb));
			//
			//	Complete the request.
			//
	...
	case SRB_FUNCTION_RESET_DEVICE:
			KDPrint(2,("SRB_FUNCTION_RESET_DEVICE: CurrentSrb is Presented. Srb = %x\n", Srb));

	default:
			KDPrint(2,("Invalid SRB Function:%d Srb = %x\n", 
									Srb->Function,Srb));
...
MiniStopWorker(
	...
	status = IoctlToLanscsiBus(
				...
				NULL);
	if(!NT_SUCCESS(status)) {
		KDPrint(2,("Failed to Update AdapterStatus in NDASSCSI.\n"));
	} else {
		KDPrint(2,("Updated AdapterStatus in NDASSCSI.\n"));
	}
...
MiniRestartWorker(
	...
	for(loop=0; loop < 10; loop++) {
		...
		KDPrint(2,("Loop #%u. try creating LUR again.\n", loop));
		KeDelayExecutionThread(KernelMode, FALSE, &interval);
	};
...
//		KDPrint(2,("LURN_REQUEST_NOOP_EVENT: AdapterStatus %x.\n",
//									AdapterStatus));
	break;
	}

	default:
		KDPrint(2,("Invalid event class:%x\n", LurnEvent->LurnEventClass));
	break;
	}
