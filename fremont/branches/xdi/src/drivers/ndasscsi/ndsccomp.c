...
NdscCopyQueryOutputToSrb(
	...
	switch(lurQuery->InfoClass) {
		...
		case LurEnumerateLurn: { // NdscLurInformation
			...
			info->Length			=	FIELD_OFFSET(NDSCIOCTL_LURINFO, UnitDisks) +
	  ...
	  )
{
	...
	if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_FLAG_VALID)) {
		...
		if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_DEGRADED)) {
			...
			if (!ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_MEMBER_FAULT)) {
				...
				KDPrint(2, ("ADAPTER_STATUSFLAG_MEMBER_FAULT is Set\n") );
			}
		...
		if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_RECOVERING)) {
			...
			if (!ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_RECOVERING)) {

				KDPrint(2, ("ADAPTER_STATUSFLAG_RECOVERING is Set\n") );
			}
		...
		if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_FAILURE)) {
			...
			if (!ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_RAID_FAILURE)) {

				KDPrint(2, ("ADAPTER_STATUSFLAG_RAID_FAILURE is Set\n") );
			}
		...
		if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_NORMAL)) {
			...
			if (!ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_RAID_NORMAL)) {

				KDPrint(2, ("ADAPTER_STATUSFLAG_RAID_NORMAL is Set\n") );
			}
	...
	if (ADAPTER_ISSTATUSFLAG(HwDeviceExtension, ADAPTER_STATUSFLAG_POWERRECYCLED)) {
