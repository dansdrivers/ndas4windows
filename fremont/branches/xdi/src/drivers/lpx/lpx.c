...

{
    ...
    LPX_ASSERT( (IrpSp->MajorFunction == IRP_MJ_INTERNAL_DEVICE_CONTROL) &&
	...
	)
{
	...
	LPX_ASSERT( irpSp->MajorFunction == IRP_MJ_INTERNAL_DEVICE_CONTROL && irpSp->MinorFunction == TDI_SEND );
	...
	)
{
    ...
    LPX_ASSERT( irpSp->MajorFunction == IRP_MJ_INTERNAL_DEVICE_CONTROL && irpSp->MinorFunction == TDI_RECEIVE );
	...
	) 
{
	...
	if (!(NTOHS(lpxHeader->Lsctl) & LSCTL_CONNREQ)) {
		...
		LPX_ASSERT( Connection->AddressFile->Address->PortAssignedByLpx == TRUE );
