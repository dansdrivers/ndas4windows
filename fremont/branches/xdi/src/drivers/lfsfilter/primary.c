...
PrimaryAgentNICDisabled (
	...
	for(idx_disabled = 0; idx_disabled < AddressList->iAddressCount; idx_disabled ++ ) {
		...
		if (found) {
			...
			NDASFS_ASSERT( LpxTdiV2IsRequestPending(&listenSock->ListenOverlapped, 0) );
	...
	while (primaryAgentThreadExit == FALSE) {
		...
		if (1 <= ntStatus && ntStatus <= MAX_SOCKETLPX_INTERFACE) {
			...
			if (listenSocket->Active == FALSE) {
				...
				NDASFS_ASSERT( !LpxTdiV2IsRequestPending(&listenSocket->ListenOverlapped, 0) );
				continue;
			}
	...
	)
{
	...
	for (i = 0; i < MAX_SOCKETLPX_INTERFACE; i++) {
		...
		NDASFS_ASSERT( Primary->Agent.ListenSocket[i].AddressFileObject != NULL );
		NDASFS_ASSERT( Primary->Agent.ListenSocket[i].ListenFileObject != NULL );
