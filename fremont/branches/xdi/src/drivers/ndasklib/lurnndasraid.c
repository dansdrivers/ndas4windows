	...
	) 
{
	...
	if (FlagOn(NdasrGlobals->Flags, NDASR_GLOBALS_FLAG_INITIALIZE)) {
		...
		NdasrGlobals->ReferenceCount ++;
		return STATUS_SUCCESS;
	}

	DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("Starting\n") );

	RtlZeroMemory( NdasrGlobals, sizeof(NDASR_GLOBALS) );

	InitializeListHead( &NdasrGlobals->IdeDiskQueue );
	KeInitializeSpinLock( &NdasrGlobals->IdeDiskQSpinlock );

	InitializeListHead( &NdasrGlobals->ArbiterQueue );
	KeInitializeSpinLock( &NdasrGlobals->ArbiterQSpinlock );
	...
	NDASSCSI_ASSERT( NdasrGlobals->TdiClientBindingHandle );
	...
	) 
{
	...
	NDASSCSI_ASSERT( FlagOn(NdasrGlobals->Flags, NDASR_GLOBALS_FLAG_INITIALIZE) );

	NdasrGlobals->ReferenceCount --;

	if (NdasrGlobals->ReferenceCount) {
...
--*/
{
	...
	DebugTrace(NDASSCSI_DBG_LUR_INFO, ("DeviceName=%ws AddrType=%u AddrLen=%u\n",
		DeviceName->Buffer,
		(ULONG)NetworkAddress->AddressType,
		(ULONG)NetworkAddress->AddressLength));
	...
	if(	RtlPrefixUnicodeString(&lpxPrefix, DeviceName, TRUE)){
		...
		lpxAddr = (PTDI_ADDRESS_LPX)NetworkAddress->Address;
		DebugTrace(NDASSCSI_DBG_LUR_ERROR, ("LPX address deleted: %02x:%02x:%02x:%02x:%02x:%02x\n",
			lpxAddr->Node[0],
			lpxAddr->Node[1],
			lpxAddr->Node[2],
			lpxAddr->Node[3],
...
{
	ExInterlockedInsertTailList( &NdasrGlobals->ArbiterQueue, 
								 &NdasrArbiter->AllListEntry, 
								 &NdasrGlobals->ArbiterQSpinlock );
	...
	) 
{
	...
	ACQUIRE_SPIN_LOCK( &NdasrGlobals->ArbiterQSpinlock, &oldIrql );
	RemoveEntryList( &NdasrArbiter->AllListEntry );
	RELEASE_SPIN_LOCK( &NdasrGlobals->ArbiterQSpinlock, oldIrql );
		...
		for (i=2; i<eventCount; i++) {
			...
			if (KeReadStateEvent(events[i])) {
				...
				if (listenContext->ListenOverlapped.Request[0].IoStatusBlock.Status == STATUS_SUCCESS) {
					...
					NDASSCSI_ASSERT( FALSE ); // May be this can be happen if multiple instances of this function is called with same address
	...
	) 
{
	...
	listenContext->Addr.Port = HTONS(NRIX_ARBITER_PORT_NUM_BASE);
	...
	) 
{
	...
	if (ListenContext->ListenFileHandle) {
		...
		NDASSCSI_ASSERT( ListenContext->ListenFileHandle );
		NDASSCSI_ASSERT( ListenContext->ListenFileObject );
		...
		NDASSCSI_ASSERT( ListenContext->ListenFileHandle == NULL );
		NDASSCSI_ASSERT( ListenContext->ListenFileObject == NULL );
	}
	...
	) 
{
	...
	connection = ExAllocatePoolWithTag( NonPagedPool, sizeof(NDASR_ARBITER_CONNECTION), NDASR_REMOTE_CLIENT_CHANNEL_POOL_TAG );
	...
	RtlZeroMemory( connection, sizeof(NDASR_ARBITER_CONNECTION) );
	...
	) 
{
	...
	do {
		...
		if (NTOHL(registerMsg.Header.Signature) != NRIX_SIGNATURE	||
			registerMsg.Header.ReplyFlag							||
			registerMsg.Header.Command != NRIX_CMD_REGISTER			||
			NTOHS(registerMsg.Header.Length) !=  sizeof(NRIX_REGISTER)) {
		...
		ACQUIRE_SPIN_LOCK( &NdasrGlobalData.ArbiterQSpinlock, &oldIrql );

		for (listEntry = NdasrGlobalData.ArbiterQueue.Flink;
			 listEntry != &NdasrGlobalData.ArbiterQueue;
			 listEntry = listEntry->Flink) {

			PNDASR_ARBITER ndasArbiter;

			ndasArbiter = CONTAINING_RECORD( listEntry, NDASR_ARBITER, AllListEntry );
		
			if (RtlCompareMemory(&ndasArbiter->Lurn->NdasrInfo->Rmd.RaidSetId, &registerMsg.RaidSetId, sizeof(GUID)) == sizeof(GUID) &&
				RtlCompareMemory(&ndasArbiter->Lurn->NdasrInfo->Rmd.ConfigSetId, &registerMsg.ConfigSetId, sizeof(GUID)) == sizeof(GUID)) {
			
				status = ndasArbiter->AcceptClient( ndasArbiter, &registerMsg, &Connection );
		...
		RELEASE_SPIN_LOCK( &NdasrGlobalData.ArbiterQSpinlock, oldIrql );
	...
	if (status != STATUS_SUCCESS) {
		...
		if (Connection) {
			...
			registerReply.Result	= (status == STATUS_SUCCESS) ? NRIX_RESULT_SUCCESS : NRIX_RESULT_RAID_SET_NOT_FOUND;
	...
	)
{
	...
	NDASSCSI_ASSERT( ChildCcb->OperationCode == CCB_OPCODE_STOP );
	...
	)
{
	...
	for (nidx =  0; nidx < Lurn->LurnChildrenCnt; nidx++) { // i is node flag
		...
		if (!FlagOn(Lurn->NdasrInfo->LocalNodeFlags[nidx], NRIX_NODE_FLAG_RUNNING) ||
			FlagOn(Lurn->NdasrInfo->LocalNodeFlags[nidx], NRIX_NODE_FLAG_DEFECTIVE)) {
		...
		if (status != STATUS_SUCCESS) {
			...
			NDASSCSI_ASSERT( !FlagOn(Lurn->NdasrInfo->LocalNodeFlags[nidx], NRIX_NODE_FLAG_RUNNING) ||
							 FlagOn(Lurn->NdasrInfo->LocalNodeFlags[nidx], NRIX_NODE_FLAG_DEFECTIVE) );
		...
		if (Lurn->LurnType == LURN_NDAS_RAID0 || Lurn->LurnType == LURN_NDAS_AGGREGATION) {
			...
			RtlCopyMemory( &Lurn->NdasrInfo->RaidSetId, &rmd_tmp.RaidSetId, sizeof(GUID) );
			RtlCopyMemory( &Lurn->NdasrInfo->ConfigSetId, &rmd_tmp.ConfigSetId, sizeof(GUID) );
			
		} else {

			if (RtlCompareMemory(&Lurn->NdasrInfo->RaidSetId, &rmd_tmp.RaidSetId, sizeof(GUID)) != sizeof(GUID)) {
...
retry:
	...
	lurnDeviceLock.ContentionTimeOut += 60 * HZ;
	lurnDeviceLock.ContentionTimeOut *= (-1);
...
#if __NDAS_SCSI_LOCK_BUG_AVOID__
	AcquireBufferLock = TRUE;
#endif

	if (AcquireBufferLock) {
			
		if (FlagOn(LurNdasFeatures[ChildLurn->Lur->LowestHwVer], NDASFEATURE_SIMULTANEOUS_WRITE)) {
	...
	)
{
	...
	case LURN_STATUS_INIT:
		...
		flags = NRIX_NODE_FLAG_STOP;
		break;
	
	default:
		
		flags = NRIX_NODE_FLAG_UNKNOWN;
		break;
	}
	...
	) 
{
	...
	NDASSCSI_ASSERT( Lurn->NdasrInfo == NULL );

	DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("NdasRaidLurnInitialize, Lurn->LurnType = %x\n", Lurn->LurnType) ); 
	...
	if (Lurn->LurnType != LURN_NDAS_RAID0 && Lurn->LurnType != LURN_NDAS_AGGREGATION) {
		...
		if (LurnDesc->LurnInfoRAID.SectorsPerBit == 0) {
	...
	NDASSCSI_ASSERT( Lurn->ChildBlockBytes == SECTOR_SIZE );
	...
	do {
		...
		case LURN_NDAS_AGGREGATION:
			...
			ndasrInfo->SerialMode = TRUE;

			ndasrInfo->ParityDiskCount = 0;	
			ndasrInfo->InterleaveMode = FALSE;

			break;

		case LURN_NDAS_RAID0:

			ndasrInfo->SerialMode = FALSE;
		...
		if (ndasrInfo->ParityDiskCount == 0) {
			...
			ndasrInfo->ActiveDiskCount	= (UCHAR)(Lurn->LurnChildrenCnt - LurnDesc->LurnInfoRAID.nSpareDisk);
			ndasrInfo->SpareDiskCount	= (UCHAR)LurnDesc->LurnInfoRAID.nSpareDisk;

			ndasrInfo->BlocksPerBit		= LurnDesc->LurnInfoRAID.SectorsPerBit;
		}
		...
		if (ndasrInfo->SerialMode) {
			...
			ndasrInfo->MaxDataSendLength = ChildMinDataSendLength * (ndasrInfo->ActiveDiskCount - ndasrInfo->SpareDiskCount);
			ndasrInfo->MaxDataRecvLength = ChildMinDataRecvLength * (ndasrInfo->ActiveDiskCount - ndasrInfo->SpareDiskCount);
		...
		ndasrInfo->RaidSetId	= LurnDesc->LurnInfoRAID.RaidSetId;
		ndasrInfo->ConfigSetId	= LurnDesc->LurnInfoRAID.ConfigSetId;

		NDASSCSI_ASSERT( ndasrInfo->MaxDataSendLength == ndasrInfo->MaxDataRecvLength );
		NDASSCSI_ASSERT( ndasrInfo->MaxDataSendLength / Lurn->ChildBlockBytes % (ndasrInfo->ActiveDiskCount - ndasrInfo->ParityDiskCount) == 0 );
		NDASSCSI_ASSERT( ndasrInfo->MaxDataRecvLength / Lurn->ChildBlockBytes % (ndasrInfo->ActiveDiskCount - ndasrInfo->ParityDiskCount) == 0 );

		NDASSCSI_ASSERT( (ndasrInfo->ActiveDiskCount - ndasrInfo->ParityDiskCount) == 1 ||
		...
		NDASSCSI_ASSERT( ndasrInfo->SpareDiskCount == 0 );
		...
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO,
					("ndasrInfo->MaxDataSendLength = %x, Lurn->ChildBlockBytes = %x, Lurn->BlockBytes = %x\n", 
					 ndasrInfo->MaxDataSendLength, Lurn->ChildBlockBytes, Lurn->BlockBytes) );
		...
		for (ridx = 0; ridx < Lurn->LurnChildrenCnt; ridx++) { // i : role index. 
			...
			if (FlagOn(unitDeviceStatus, NDAS_UNIT_META_BIND_STATUS_DEFECTIVE)) {
				...
				if (FlagOn(ndasrInfo->LocalNodeFlags[(UCHAR)ndasrInfo->Rmd.UnitMetaData[ridx].iUnitDeviceIdx], NRIX_NODE_FLAG_RUNNING)) {
	...
	) 
{
	...
	if (Lurn->NdasrInfo == NULL) {
		...
		NDASSCSI_ASSERT( Lurn->LurnStatus == LURN_STATUS_INIT );
		return STATUS_SUCCESS;
	}
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("NdasRaidLurnDestroy\n") ); 
	...
	)
{
	...
	if (Ccb->OperationCode == CCB_OPCODE_SHUTDOWN) {
		...
		NDASSCSI_ASSERT( Lurn && Lurn->Lur && Lurn->Lur->AdapterFunctionDeviceObject );
	...
	if (Ccb->OperationCode == CCB_OPCODE_STOP) {
		...
		if (Lurn->NdasrInfo->NdasrArbiter) {

			NdasRaidArbiterStop( Lurn );
		}
	...
	if (Lurn->NdasrInfo->NdasrClient->NdasrState == NRIX_RAID_STATE_TERMINATED) {
	...
	if (Ccb->OperationCode == CCB_OPCODE_EXECUTE) {
		...
		if (Lurn->NdasrInfo->NdasrClient->NdasrState == NRIX_RAID_STATE_TERMINATED) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
						("ndasrClient->ent->Lurn->NdasrInfo->NdasrState == NRIX_RAID_STATE_TERMINATED Returning stop status Ccb->Cdb[0] =%x\n", Ccb->Cdb[0]) );
	...
	) 
{
	...
	if (Parameter->Lurn->NdasrInfo->NdasrArbiter) {

		NdasRaidArbiterShutdown( Parameter->Lurn );
	}
