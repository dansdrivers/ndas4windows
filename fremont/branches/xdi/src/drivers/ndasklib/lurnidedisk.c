	...
	) 
{
	...
	case CCB_OPCODE_RESETBUS:
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, 
					("CCB_OPCODE_RESETBUS, Ccb->Srb = %08x, Ccb->AssociateID = %d\n",
					 Ccb->Srb, Ccb->AssociateID) );
		...
		CcbCompleteList( &IdeDisk->CcbQueue, 
						 CCB_STATUS_RESET, 
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
					("CCB_OPCODE_RESETBUS: Completed Ccb:%p\n", Ccb) );
		
		return STATUS_SUCCESS;

	case CCB_OPCODE_STOP:

		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("CCB_OPCODE_STOP\n") );

		//	Stop request must succeed unconditionally.
		//	clear all CCB in the queue and set LURN_STATUS_STOP_PENDING

		NDASSCSI_ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
		NDASSCSI_ASSERT( Ccb->Flags & CCB_FLAG_SYNCHRONOUS );
		...
		if (IdeDisk == NULL) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("CCB_OPCODE_STOP: IdeDisk NULL!\n") );
			Ccb->CcbStatus =CCB_STATUS_SUCCESS;
			LsCcbCompleteCcb(Ccb);
		...
		CcbCompleteList( &IdeDisk->CcbQueue, 
						 ...
						 CCBSTATUS_FLAG_LURN_STOP | Lurn->LurnStopReasonCcbStatusFlags );

		if (LURN_IS_RUNNING(Lurn->LurnStatus)) {

			DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
						("CCB_OPCODE_STOP: queue CCB_OPCODE_STOP to the thread. LurnStatus=%08lx\n",
						 Lurn->LurnStatus));
			...
			status = KeWaitForSingleObject( IdeDisk->ThreadObject,
											Executive,
											KernelMode,
											FALSE,
											NULL );
...
#if DBG
			if(NT_SUCCESS(status)) {
				DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("CCB_OPCODE_QUERY: Completed Ccb:%p\n", Ccb));
			}
#endif
			LsCcbCompleteCcb(Ccb);
			return STATUS_SUCCESS;
	...
	if (IdeDisk) {
		...
		if (Ccb->Flags & CCB_FLAG_URGENT) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
						("URGENT. Queue to the head and mark pending.\n") );
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("Queue to the tail and mark pending.\n"));
		...
		case STATUS_WAIT_0:
			...
			if (Lurn->IdeDisk->AddAddressOccured == TRUE) {
				...
				do {
					...
					if (RtlEqualMemory(((PTDI_ADDRESS_LPX)IdeDisk->LanScsiSession->NdasBindAddress.Address)->Node,
			...
			if (Lurn->IdeDisk->DisConnectOccured == TRUE) {
				...
				if (status != STATUS_SUCCESS || pdu_response != LANSCSI_RESPONSE_SUCCESS) {
					...
					if (Lurn->LurnStatus != LURN_STATUS_STALL) {
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,
									("Lurn%d: IDLE_TIMEOUT: NoOperation failed. Start reconnecting.\n", 
									 Lurn->LurnId) );
					} else {
		...
		case STATUS_TIMEOUT: {
			...
			if (IdeDisk->LURNHostStop) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, 
							("Lurn%d: IDLE_TIMEOUT: stopped at host side. skip pinging.\n",
							 Lurn->LurnId) );
				break;
			}
			...
			if (status != STATUS_SUCCESS || pdu_response != LANSCSI_RESPONSE_SUCCESS) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, 
							("Lurn%d: IDLE_TIMEOUT: NoOperation is successful.\n", Lurn->LurnId) );
			}
	...
	)
{
	...
	DebugTrace( DBG_LURN_NOISE, ("0x%08x(%d)\n", LogicalBlockAddress, TransferBlocks));
				...
				case CCB_EXT_READ:
					{
						...
						DebugTrace( DBG_LURN_NOISE,	("CCB_EXT_READ\n"));
				...
				case CCB_EXT_WRITE:
					{
						...
						DebugTrace( DBG_LURN_NOISE,	("CCB_EXT_WRITE\n"));
