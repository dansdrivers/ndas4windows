...
#if !__NDAS_SCSI_MULTI_LOCK_GRANT__ // only for debugging ToYieldLink PendingLink must not be used
...
#if __NDAS_SCSI_MULTI_LOCK_GRANT__
	CheckOverlap = FALSE;
#endif
...
DraidArbiterWriteMetaSync(
	...
	for(i = 0; i < pArbiter->Lurn->LurnChildrenCnt; i++)
	{
		...
		DefectCodes = pArbiter->DefectCodes[i];

		if ((NodeFlags & DRIX_NODE_FLAG_DEFECTIVE) && (DefectCodes & DRIX_NODE_DEFECT_BAD_SECTOR)) {
			KDPrintM(DBG_LURN_INFO, ("Node %d flag: %x, defect: %x. Try to update bad sector information\n", i, NodeFlags, pArbiter->DefectCodes[i]));
		...
		if(!LURN_IS_RUNNING(pArbiter->Lurn->LurnChildren[i]->LurnStatus)) {
			KDPrintM(DBG_LURN_INFO, ("Node %d status is not running %x\n", i, pArbiter->Lurn->LurnChildren[i]->LurnStatus));
	...
	for(i = 0; i < pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount; i++) { // i is role index.
		ULONG NodeIdx = pArbiter->RoleToNodeMap[i];
		if (i!=pArbiter->OutOfSyncRoleIndex) {
			...
			if(!LURN_IS_RUNNING(pArbiter->Lurn->LurnChildren[NodeIdx]->LurnStatus)) 
			{
				NodeFlagMismatch[NodeIdx] = TRUE;
				KDPrintM(DBG_LURN_INFO, ("Node %d is not running\n", NodeIdx));
				continue;
			}
...
DraidArbiterUpdateInCoreRmd(
	...
	if (DRAID_ARBITER_STATE_TERMINATING == pArbiter->Status) {
	...
	switch(pArbiter->RaidState) {
	case DRIX_RAID_STATE_DEGRADED :
		...
		break;
	case DRIX_RAID_STATE_NORMAL:
	case DRIX_RAID_STATE_OUT_OF_SYNC:
		if (pArbiter->Rmd.state & NDAS_RAID_META_DATA_STATE_USED_IN_DEGRADED) {
...
//		ASSERT(UsedAddrCount >0);
	...
	for(i=0;i<pArbiter->Lurn->LurnChildrenCnt;i++) { // i is role index.
		...
		if (i >= pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount) {
	...
	case DRIX_CMD_ACQUIRE_LOCK:
		{
			...
			UINT64 OverlapStart, OverlapEnd;
#endif
			UINT64 Addr = NTOHLL(AcqMsg->BlockAddress);
			UINT32 Length = NTOHL(AcqMsg->BlockLength);
			...
			if (NewLock==NULL) {
				...
				if (pArbiter->RaidState == DRIX_RAID_STATE_DEGRADED) {
			...
			if (NewLock) {
				...
				AcqReply->BlockAddress = NTOHLL(NewLock->LockAddress);
				AcqReply->BlockLength = NTOHL(NewLock->LockLength);
			} else {
				PDRIX_ACQUIRE_LOCK_REPLY AcqReply = (PDRIX_ACQUIRE_LOCK_REPLY) ReplyMsg;
				AcqReply->LockType = 0;
				AcqReply->LockMode = 0;
				AcqReply->LockId = 0;
				AcqReply->BlockAddress = 0;
				AcqReply->BlockLength = 0;
			}
		}
		break;
	case DRIX_CMD_RELEASE_LOCK:
...
DraidArbiterUseSpareIfNeeded(
	...
	if (pArbiter->RaidState == DRIX_RAID_STATE_DEGRADED) {
		...
		if (time_diff.QuadPart < NDAS_RAID_SPARE_HOLDING_TIMEOUT) {
			...
			// Find running spare
			for(i=pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount;i<pArbiter->Lurn->LurnChildrenCnt;i++) { // i is indexed by role
			...
			if (SpareFound) {
				...
				KDPrintM(DBG_LURN_INFO, ("Running spare disk found. Swapping defective node %d(role %d) with spare %d\n",
					pArbiter->RoleToNodeMap[pArbiter->OutOfSyncRoleIndex], pArbiter->OutOfSyncRoleIndex, SpareNode));
				ASSERT(SpareNode<pArbiter->Lurn->LurnChildrenCnt);
				ASSERT((pArbiter->NodeFlags[pArbiter->RoleToNodeMap[pArbiter->OutOfSyncRoleIndex]] & DRIX_NODE_FLAG_STOP) ||
					(pArbiter->NodeFlags[pArbiter->RoleToNodeMap[pArbiter->OutOfSyncRoleIndex]] & DRIX_NODE_FLAG_DEFECTIVE));

				OosNode = pArbiter->RoleToNodeMap[pArbiter->OutOfSyncRoleIndex];
				SpareRole = pArbiter->NodeToRoleMap[SpareNode];

				pArbiter->NodeToRoleMap[OosNode] = SpareRole;
				pArbiter->NodeToRoleMap[SpareNode] = pArbiter->OutOfSyncRoleIndex;

				pArbiter->RoleToNodeMap[pArbiter->OutOfSyncRoleIndex] = SpareNode;
				pArbiter->RoleToNodeMap[SpareRole] = OosNode;
				
				// Replaced node is still out-of-sync. Keep pArbiter->OutOfSyncRoleIndex
				...
				if (pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount < pArbiter->Lurn->LurnChildrenCnt) {
	...
	) 
{
	...
	case DRIX_CMD_CHANGE_STATUS:
		MsgLength = SIZE_OF_DRIX_CHANGE_STATUS(pArbiter->Lurn->LurnChildrenCnt);
		break;
...
//	NotifyMsg->Version = DRIX_CURRENT_VERSION;
	...
	if (DRIX_CMD_CHANGE_STATUS == Command) {
		...
		CsMsg->RaidState = (UCHAR)pArbiter->RaidState;
		CsMsg->NodeCount = (UCHAR)pArbiter->Lurn->LurnChildrenCnt;
		...
		for(i=0;i<CsMsg->NodeCount;i++) {
			...
			if (pArbiter->OutOfSyncRoleIndex != NO_OUT_OF_SYNC_ROLE &&  i==pArbiter->RoleToNodeMap[pArbiter->OutOfSyncRoleIndex])
	...
	if (Client->LocalClient) {
		...
		if (MsgEntry) {
			...
			events[1] = pArbiter->ClientTerminatingEvent;
		...
		if (Client->NotificationConnection && Client->NotificationConnection->ConnectionFileObject) {
			...
			KDPrintM(DBG_LURN_ERROR, ("Notification connection does not exist\n"));
			status = STATUS_UNSUCCESSFUL;
		}
		ExFreePoolWithTag(NotifyMsg, DRAID_ARBITER_NOTIFY_MSG_POOL_TAG);
		if (!NT_SUCCESS(status)) {
...
//			KDPrintM(DBG_LURN_INFO, ("pArbiter->AcquiredLockCount %d\n", Arbiter->AcquiredLockCount));
			...
			if (Arbiter->RaidState == DRIX_RAID_STATE_DEGRADED) {
				KDPrintM(DBG_LURN_INFO, ("Granted lock in degraded mode. Mark OOS bitmap %I64x:%x\n",
					Lock->LockAddress, Lock->LockLength));
	...
	)
{
	...
	if (Changed) {
		...
		UINT32 FaultCount = 0;
		UCHAR FaultRole = 0;
		for(i=0;i<pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount;i++) { // i : role index
			if (!(NewNodeFlags[pArbiter->RoleToNodeMap[i]] & DRIX_NODE_FLAG_RUNNING)) {
				FaultCount++;
				if (pArbiter->OutOfSyncRoleIndex != i) {
		...
		if (FaultCount == 0) {
			if (pArbiter->OutOfSyncRoleIndex == NO_OUT_OF_SYNC_ROLE) {
				NewRaidStatus = DRIX_RAID_STATE_NORMAL;
			} else {
				NewRaidStatus = DRIX_RAID_STATE_OUT_OF_SYNC;
			}
			...
			if (pArbiter->OutOfSyncRoleIndex != NO_OUT_OF_SYNC_ROLE) {
				...
				if (NewFaultyNode) { 
					...
					NewRaidStatus = DRIX_RAID_STATE_FAILED;
				} else {	
					// OutOfSync node is still faulty.
					NewRaidStatus = DRIX_RAID_STATE_DEGRADED;
				}
			} else {
				// Faulty node appeared from zero fault.
				pArbiter->OutOfSyncRoleIndex = FaultRole;
		...
		NewRaidStatus = pArbiter->RaidState;
	}
	
	if (pArbiter->RaidState != NewRaidStatus) {
		KDPrintM(DBG_LURN_INFO, ("Changing DRAID Status from %x to %x\n", pArbiter->RaidState, NewRaidStatus));
		if (NewRaidStatus == DRIX_RAID_STATE_DEGRADED) {
			PDRAID_ARBITER_LOCK_CONTEXT Lock;
		...
		if (NewRaidStatus == DRIX_RAID_STATE_OUT_OF_SYNC) {
	...
	if (Changed) {
		pArbiter->SyncState = DRAID_SYNC_REQUIRED;
...
//		KDPrintM(DBG_LURN_INFO, ("pArbiter->AcquiredLockCount %d\n",Arbiter->AcquiredLockCount));
	...
	if (AbandonedLockExist) {
		...
		if (Arbiter->OutOfSyncRoleIndex == NO_OUT_OF_SYNC_ROLE) {
			// Select one node.
			KDPrintM(DBG_LURN_ERROR, ("Client exited without unlocking locks. Marking node %d as out-of-sync.\n", Arbiter->RoleToNodeMap[Arbiter->Lurn->LurnRaidInfo->ActiveDiskCount-1]));
			Arbiter->OutOfSyncRoleIndex = Arbiter->Lurn->LurnRaidInfo->ActiveDiskCount-1;
		}
	}
...
//	UCHAR Flags;
	...
	ASSERT(Lurn);
	ASSERT(LURN_RAID1R == Lurn->LurnType || LURN_RAID4R == Lurn->LurnType || LURN_RAID5 == Lurn->LurnType);
	pRaidInfo = Lurn->LurnRaidInfo;
	ASSERT(pRaidInfo);
	Arbiter = (PDRAID_ARBITER_INFO) pRaidInfo->NdasArbiter;
	ASSERT(Arbiter);
	...
	nActiveDiskCount = Lurn->LurnChildrenCnt - pRaidInfo->SpareDiskCount;
	...
	ACQUIRE_SPIN_LOCK(&Arbiter->SpinLock, &oldIrql);
	Arbiter->Status = DRAID_ARBITER_STATE_RUNNING;
	RELEASE_SPIN_LOCK(&Arbiter->SpinLock, oldIrql);		
...
restart:
			...
			for (listEntry = Arbiter->ClientList.Flink;
				...
				if (Client->RequestConnection) {
					...
					if (!Client->RequestConnection->Receiving) {
						...
						RELEASE_SPIN_LOCK(&Arbiter->SpinLock, oldIrql);
						if (Client->RequestConnection->TdiReceiveContext.Result < 0) {
							KDPrintM(DBG_LURN_INFO, ("Receive result is %x.\n", Client->RequestConnection->TdiReceiveContext.Result));
							ASSERT(FALSE);
							status = STATUS_UNSUCCESSFUL;
						} else {
		...
		if (Arbiter->SyncState == DRAID_SYNC_REQUIRED) {			
			Arbiter->SyncState = DRAID_SYNC_IN_PROGRESS;
			//
			...
			for (listEntry = Arbiter->ClientList.Flink;
				...
				if (!NT_SUCCESS(status)) {
					...
					Arbiter->SyncState = DRAID_SYNC_REQUIRED;
					DoMore = TRUE;
					goto restart;
				}
			}
			...
			Arbiter->SyncState = DRAID_SYNC_DONE;
			DoMore = TRUE;
		}
		...
		ACQUIRE_SPIN_LOCK(&Arbiter->SpinLock, &oldIrql);
		if (Arbiter->RaidState == DRIX_RAID_STATE_OUT_OF_SYNC) {
			RELEASE_SPIN_LOCK(&Arbiter->SpinLock, oldIrql);
		...
		if (Arbiter->StopRequested) {
			Arbiter->Status = DRAID_ARBITER_STATE_TERMINATING;
	...
	ASSERT(DRAID_ARBITER_STATE_TERMINATING== Arbiter->Status);
...
fail:
	ACQUIRE_SPIN_LOCK(&Arbiter->SpinLock, &oldIrql);
	Arbiter->Status = DRAID_ARBITER_STATE_TERMINATING;
	Arbiter->RaidState = DRIX_RAID_STATE_FAILED;
	RELEASE_SPIN_LOCK(&Arbiter->SpinLock, oldIrql);
...
DradArbiterInitializeOosBitmap(
	...
	pArbiter->OosBmpOnDiskSectorCount = (pArbiter->OosBmpBitCount + NDAS_BIT_PER_OOS_BITMAP_BLOCK -1)/NDAS_BIT_PER_OOS_BITMAP_BLOCK;
	if (pArbiter->OosBmpOnDiskSectorCount == 0) {
		KDPrintM(DBG_LURN_ERROR, ("BMP Sector count is 0!!\n"));
		ASSERT(FALSE);
		pArbiter->OosBmpOnDiskSectorCount = 1;
	}
	pArbiter->OosBmpBuffer = ExAllocatePoolWithTag(NonPagedPool, pArbiter->OosBmpByteCount,
								DRAID_BITMAP_POOL_TAG);
	if (pArbiter->OosBmpBuffer == NULL) {
		KDPrintM(DBG_LURN_ERROR, ("Failed to allocate OOS in memory bitmap\n"));
		ASSERT(FALSE);
		status =STATUS_INSUFFICIENT_RESOURCES;
		goto errout;
	}
	pArbiter->OosBmpOnDiskBuffer = ExAllocatePoolWithTag(NonPagedPool, pArbiter->OosBmpOnDiskSectorCount * SECTOR_SIZE, DRAID_BITMAP_POOL_TAG);
	...
	if (pArbiter->LwrBmpBuffer == NULL) {
		ExFreePoolWithTag(pArbiter->OosBmpBuffer, DRAID_BITMAP_POOL_TAG);
		KDPrintM(DBG_LURN_ERROR, ("Failed to allocate OOS in memory bitmap\n"));
		ASSERT(FALSE);
		status =STATUS_INSUFFICIENT_RESOURCES;
		goto errout;
	...
	RtlZeroMemory(pArbiter->OosBmpBuffer, pArbiter->OosBmpByteCount);
	RtlZeroMemory(pArbiter->LwrBmpBuffer, pArbiter->OosBmpByteCount);
	RtlZeroMemory(pArbiter->OosBmpOnDiskBuffer, pArbiter->OosBmpOnDiskSectorCount * SECTOR_SIZE);

	RtlInitializeBitMap(&pArbiter->OosBmpHeader, pArbiter->OosBmpBuffer, 
	...
	if(LURN_STATUS_RUNNING != pArbiter->Lurn->LurnChildren[UpToDateNode]->LurnStatus) {
		...
		status = LurnExecuteSyncRead(pArbiter->Lurn->LurnChildren[UpToDateNode], (PUCHAR)pArbiter->OosBmpOnDiskBuffer,
			NDAS_BLOCK_LOCATION_BITMAP, pArbiter->OosBmpOnDiskSectorCount);
		if(!NT_SUCCESS(status)) {
	...
	if (ValidBitmapBlock == FALSE) {
		...
		//
		for(i=0;i<pArbiter->OosBmpOnDiskSectorCount;i++) {
	...
	if (ValidBitmapBlock == FALSE) {
		...
		for(i=0;i<pArbiter->OosBmpOnDiskSectorCount;i++) {
			pArbiter->OosBmpOnDiskBuffer[i].SequenceNumHead = pArbiter->OosBmpOnDiskBuffer[i].SequenceNumTail = 0;
                    if (i== pArbiter->OosBmpOnDiskSectorCount-1) {
                        UINT RemainingBits = pArbiter->OosBmpBitCount - i * NDAS_BIT_PER_OOS_BITMAP_BLOCK;
                        RTL_BITMAP LastSectorBmpHeader;
                        RtlInitializeBitMap(&LastSectorBmpHeader, (PULONG)pArbiter->OosBmpOnDiskBuffer[i].Bits, 
	...
	for(i=0;i<pArbiter->OosBmpOnDiskSectorCount;i++) {
		...
		RtlCopyMemory(((PUCHAR)pArbiter->OosBmpBuffer)+DRAID_ONDISK_BMP_OFFSET_TO_INCORE_OFFSET(i,0), 
	...
	if (SetBits) {
		...
		if (pArbiter->OutOfSyncRoleIndex == NO_OUT_OF_SYNC_ROLE) {
			...
			for(i = 0; i < pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount; i++) // i : role index.
			{
				if (!(pArbiter->NodeFlags[pArbiter->RoleToNodeMap[i]] & DRIX_NODE_FLAG_RUNNING)) {
					NormalStatus = FALSE;
					pArbiter->OutOfSyncRoleIndex = (UCHAR)i;
					break;
				}
			}
			if (NormalStatus) {
				pArbiter->OutOfSyncRoleIndex = (UCHAR)(pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount-1);
				KDPrintM(DBG_LURN_INFO, 
					("All node is running without out of sync flag. Selecting %d as out-of-sync node\n",pArbiter->OutOfSyncRoleIndex));
			} else {
	...
	if (BitOffset == 0 && NumberOfBit == pArbiter->OosBmpBitCount) {
		...
		for(i=0;i<pArbiter->OosBmpOnDiskSectorCount;i++) {
			...
			RtlCopyMemory(pArbiter->OosBmpOnDiskBuffer[i].Bits,
				pArbiter->OosBmpBuffer+DRAID_ONDISK_BMP_OFFSET_TO_INCORE_OFFSET(i,0),
				ByteCount);
			pArbiter->DirtyBmpSector[i] = TRUE;
		}
	} else {
		for(i=0;i<NumberOfBit;i++) {
			...
			ASSERT(BmpSectorOffset<pArbiter->OosBmpOnDiskSectorCount);
#if 0
			...
			RtlCopyMemory(&pArbiter->OosBmpOnDiskBuffer[BmpSectorOffset].Bits[BmpByteOffset], 
				&pArbiter->OosBmpBuffer[ByteOffset],
				sizeof(ULONG));
#if 0
...
DraidArbiterUpdateOnDiskOosBitmap(
	...
	for(i=0;i<pArbiter->OosBmpByteCount/sizeof(ULONG);i++) {
		...
		BitValues = pArbiter->OosBmpBuffer[i] | pArbiter->LwrBmpBuffer[i];
	...
	)
{
	...
	pRaidInfo->NdasArbiter = ExAllocatePoolWithTag(NonPagedPool, sizeof(DRAID_ARBITER_INFO),
		DRAID_ARBITER_INFO_POOL_TAG);
	if (NULL ==  pRaidInfo->NdasArbiter) {
		return STATUS_INSUFFICIENT_RESOURCES;
	}

	pArbiter = pRaidInfo->NdasArbiter;
	RtlZeroMemory(pArbiter, sizeof(DRAID_ARBITER_INFO));

	KeInitializeSpinLock(&pArbiter->SpinLock);

	pArbiter->Lurn = Lurn;
	pArbiter->Status = DRAID_ARBITER_STATE_INITALIZING;

	pArbiter->RaidState = DRIX_RAID_STATE_INITIALIZING;
	...
	pArbiter->Lurn->LurnChildrenCnt = Lurn->LurnChildrenCnt;
	pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount = Lurn->LurnChildrenCnt - pRaidInfo->SpareDiskCount;
	...
	pArbiter->OutOfSyncRoleIndex = NO_OUT_OF_SYNC_ROLE;
	...
	if(!NT_SUCCESS(ntStatus))
	{
		...
		ACQUIRE_SPIN_LOCK(&pArbiter->SpinLock, &oldIrql);
		pArbiter->Status = DRAID_ARBITER_STATE_TERMINATING;
		pArbiter->RaidState = DRIX_RAID_STATE_FAILED;
		RELEASE_SPIN_LOCK(&pArbiter->SpinLock, oldIrql);
		goto fail;
	}
	...
	if(!NT_SUCCESS(ntStatus))
	{
		ExFreePoolWithTag(pRaidInfo->NdasArbiter, DRAID_ARBITER_INFO_POOL_TAG);
		pRaidInfo->NdasArbiter = NULL;		
	...
	)
{
	...
	PLIST_ENTRY listEntry;
	PRAID_INFO pRaidInfo = Lurn->LurnRaidInfo;
	PDRAID_ARBITER_INFO pArbiterInfo = pRaidInfo->NdasArbiter;
	NTSTATUS status;
	PDRAID_ARBITER_LOCK_CONTEXT Lock;
...
//		KDPrintM(DBG_LURN_INFO, ("pArbiter->AcquiredLockCount %d\n",pArbiterInfo->AcquiredLockCount));		
	...
	ExFreePoolWithTag(pArbiterInfo->OosBmpBuffer, DRAID_BITMAP_POOL_TAG);
...
DraidArbiterUnregisterClient(
	...
	BOOLEAN UnregisterInProgress;
	
	KeInitializeEvent(&UnregisterDoneEvent, NotificationEvent, FALSE) ;	
	ACQUIRE_SPIN_LOCK(&pArbiterInfo->SpinLock, &oldIrql);
	if (pArbiterInfo->Status == DRAID_ARBITER_STATE_TERMINATING) {
		// This client is already removed or will be removed by arbiter thread.
		RELEASE_SPIN_LOCK(&pArbiterInfo->SpinLock, oldIrql);
		return STATUS_SUCCESS;
	}

	if (pClientContext->UnregisterRequest) {
		// Unregister is already in progress
		KDPrintM(DBG_LURN_INFO, ("Unregister is already in progress\n"));
		UnregisterInProgress = TRUE;
	} else {
		UnregisterInProgress = FALSE;
		pClientContext->UnregisterRequest = TRUE;	
		pClientContext->UnregisterDoneEvent = &UnregisterDoneEvent;
	}
	RELEASE_SPIN_LOCK(&pArbiterInfo->SpinLock, oldIrql);

	//
	// Wakeup arbiter to make it handle disappearance of client.
	//
	KeSetEvent(pArbiterInfo->ClientTerminatingEvent, IO_NO_INCREMENT, FALSE);
	KeSetEvent(&pArbiterInfo->ArbiterThreadEvent,IO_NO_INCREMENT, FALSE);

	if (UnregisterInProgress) {
		// Temp fix... poll whether client is removed from the list.
		BOOLEAN ClientExist;
		PLIST_ENTRY listEntry;
		PDRAID_CLIENT_CONTEXT Client;
		while(1) {
			ClientExist = FALSE;
			ACQUIRE_SPIN_LOCK(&pArbiterInfo->SpinLock, &oldIrql);
			for (listEntry = pArbiterInfo->ClientList.Flink;
				listEntry != &pArbiterInfo->ClientList;
				listEntry = listEntry->Flink) 
			{
				Client = CONTAINING_RECORD (listEntry, DRAID_CLIENT_CONTEXT, Link);
		...
		//
		// Wait this client is unregistered 
		//
...
)
{
	...
	if (pArbiterInfo->Status == DRAID_ARBITER_STATE_TERMINATING) {
		...
		RELEASE_SPIN_LOCK(&pArbiterInfo->SpinLock, oldIrql);
		return STATUS_UNSUCCESSFUL;
	} else if (pArbiterInfo->Status != DRAID_ARBITER_STATE_RUNNING) {
	...
	pArbiterInfo->ClientTerminatingEvent = &pClient->LocalClientTerminating;
	...
	pClient->ClientState = DRAID_CLIENT_STATE_ARBITER_CONNECTED;
...
#define DRAID_ADDR_REQUIRES_CDB16(BlockAddr) (BlockAddr >= 0x100000000L)
...
NTSTATUS DraidDoRebuildIo(
	...
	ACQUIRE_SPIN_LOCK(&pArbiter->SpinLock, &oldIrql);
	if (pArbiter->RaidState != DRIX_RAID_STATE_OUT_OF_SYNC) {
		RELEASE_SPIN_LOCK(&pArbiter->SpinLock, oldIrql);
	...
	//
	LurnOutOfSync = pArbiter->Lurn->LurnChildren[pArbiter->RoleToNodeMap[pArbiter->OutOfSyncRoleIndex]];
	j = 0;
	for(i=0;i<pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount;i++) { // i: role index
	...
	if (j!=pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount-1) {
	...
	status = LurnExecuteSyncMulti(
		pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount-1,
		LurnsHealthy,
...
)
{
	...
	for(i=0;i<pArbiter->Lurn->LurnRaidInfo->ActiveDiskCount-1;i++) {
...
//		KDPrintM(DBG_LURN_INFO, ("Rebuild thread is not ready for new request\n"));
	...
	if (0xFFFFFFFF == BitToRecover || BitToRecover>=Arbiter->OosBmpBitCount) {
		...
		KDPrintM(DBG_LURN_INFO, ("Clearing Out of sync node mark from role %d(node %d)\n", 
			Arbiter->OutOfSyncRoleIndex,
			Arbiter->RoleToNodeMap[Arbiter->OutOfSyncRoleIndex]
			));
		Arbiter->OutOfSyncRoleIndex = NO_OUT_OF_SYNC_ROLE;
		RELEASE_SPIN_LOCK(&Arbiter->SpinLock, oldIrql);
