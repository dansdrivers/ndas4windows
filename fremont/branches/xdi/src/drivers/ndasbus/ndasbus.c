...
LurTranslateAddTargetDataToLURDesc(
	...
	if(LurMaxOsRequestLength) {
		...
		maxSendRecvForRootNode = NDSC_DEFAULT_MAXDATATRANSFER;
	}
	...
	case NDASSCSI_TYPE_MO:
		...
		Bus_KdPrint_Def(BUS_DBG_SS_INFO, ("TYPE : MO UserID:%08lx Password:%I64x\n",
										cur_lurn->LurnIde.UserID,
										cur_lurn->LurnIde.Password
									));
		//
	...
	case NDASSCSI_TYPE_DISK_NORMAL:
		...
		Bus_KdPrint_Def(BUS_DBG_SS_INFO, ("UserID:%08lx Password:%I64x\n",
										cur_lurn->LurnIde.UserID,
										cur_lurn->LurnIde.Password
									));
		//
