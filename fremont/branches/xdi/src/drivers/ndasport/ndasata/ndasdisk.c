...
#define SMART_LOG_SECTOR_SIZE 512
	...
	//
	idereg.device.dev = NdasAtaDeviceExtension->LspLoginInfo.unit_no;
	...
	__in PVOID Context)
{
	...
	if (!NT_SUCCESS(LspIoRequest->IoStatus.Status))
	{
		...
		DebugPrint((0, "LspIoRestartSession failed, status=0x%x, errorCount=%d.\n",
			LspIoRequest->IoStatus.Status,
			ndasAtaDeviceExtension->ResetConnectionErrorCount));
			...
			ndasAtaDeviceExtension->ResetConnectionFailThreshold)
		{
			...
			DebugPrint((0, "*** Link failed, NdasAta=%p\n", ndasAtaDeviceExtension));
			DebugPrint((0, "SRB_STATUS_UNEXPECTED_BUS_FREE, NdasAta=%p\n", ndasAtaDeviceExtension));
	...
	{
		ClearFlag(&ndasAtaDeviceExtension->LuFlags, NDASATA_FLAG_RECONNECTING);
		DebugPrint((0, "*** Link established! NdasAta=%p\n", ndasAtaDeviceExtension));
