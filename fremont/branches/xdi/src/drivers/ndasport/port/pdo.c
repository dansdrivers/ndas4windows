	...
	__in ULONG CustomDataSize)
{
	...
	bufferSize = FIELD_OFFSET(TARGET_DEVICE_CUSTOM_NOTIFICATION, CustomDataBuffer) + 
		CustomDataSize;
	...
	buffer = ExAllocatePoolWithTag(
		KeGetCurrentIrql() > APC_LEVEL ? NonPagedPool : PagedPool, 
		bufferSize,
	...
	customNotification->Size = (USHORT) bufferSize;
...
NdasPortpFirePnpEventCompletion(
	__in PVOID Context)
{
	ExFreePoolWithTag(Context, NDASPORT_TAG_PNP_EVENT);
}
	...
	__out PDEVICE_OBJECT* PhysicalDeviceObject)
{
	...
	DECLARE_UNICODE_STRING_SIZE(referenceString, 64);
	...
	__in PIRP Irp)
{
	...
	case StorageAdapterProperty:
		return NpReleaseRemoveLockAndForwardIrp(
			CommonExtension,
			Irp);
	case 2 /* StorageDeviceIdProperty */:  /* Windows XP or later */
		{
	...
	__in PIRP Irp)
{
	...
	WCHAR unicodeStringBuffer[128];
	UNICODE_STRING unicodeStringId = { 0, sizeof(unicodeStringBuffer), unicodeStringBuffer };
	...
	for (index = 0; ; ++index)
	{
		...
		status = RtlStringCbLengthW(
			...
			sizeof(unicodeStringBuffer)- sizeof(WCHAR),
			&length);
		ASSERT(NT_SUCCESS(status));
		ASSERT(unicodeStringId.Length == length);
		requiredBytes += length + sizeof(WCHAR);
	}
	deviceIdCount = index;
