...
//		ULONG				originalFileSpyDebugLevel; 
	...
	case IRP_MJ_CREATE: { //0x00
		...
		try {
			...
			do {
				...
				NDASFS_ASSERT( siblingOpenFile->FileObject );
			...
			if (status == STATUS_SUCCESS) {
				...
				NDASFS_ASSERT( fileHandle != 0 );
				NDASFS_ASSERT( status == ioStatusBlock.Status );
			...
			NdfsWinxpReplytHeader->Status	  = status;

			NDASFS_ASSERT( ioStatusBlock.Information <= 0xffffffff );
	...
	case IRP_MJ_READ: { // 0x03
		...
		if (status != STATUS_SUCCESS) {
			...
			NDASFS_ASSERT( !NT_SUCCESS(status) );
			NDASFS_ASSERT( ioStatusBlock.Information == 0 );
	...
	)
{
	...
	NDASFS_ASSERT( OpenFile->AlreadyClosed == TRUE );
