...
{																			\
	...
	(MndfsWinxpRequestHeader)->IrpFlags			= (Mirp)->IrpFlags;			\
...
//							&timeOut
		...
		Data->IoStatus.Status = ndfsWinxpReplytHeader->Status;
		Data->IoStatus.Information = ndfsWinxpReplytHeader->Information;
	...
	)
{
	...
	if(BooleanFlagOn(iopb->IrpFlags, SL_OPEN_PAGING_FILE))
	{
		ASSERT(LFS_REQUIRED);
...
//						0
...
#endif
	{
		...
		{
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE,
						("RedirectIrpMajorCreate: Directory fileObject->FileName = %wZ BooleanFlagOn( iopb->Parameters.Create.Options, FILE_DIRECTORY_FILE) = %d\n",
