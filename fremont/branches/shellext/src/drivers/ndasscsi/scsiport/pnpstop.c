...
#undef __MODULE__
#endif // __MODULE__
#define __MODULE__ "NDSC_PNPstop"
...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, ScsiPortStopDevice)
...

{
    ...
    if(commonExtension->CurrentPnpState == IRP_MN_STOP_DEVICE) {
        return STATUS_SUCCESS;
    }

    KeInitializeEvent(&(context.Event), SynchronizationEvent, FALSE);

    status = SpEnableDisableLogicalUnit(luExtension,
                                        FALSE,
                                        SpDeviceStoppedCompletion,
                                        &context);
