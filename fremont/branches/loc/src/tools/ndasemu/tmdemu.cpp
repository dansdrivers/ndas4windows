	...
	if(translateAddr) {
		if(Lba48) {
			...
			if(ScsiCommand->ScsiOp.BlockAddress + ScsiCommand->ScsiOp.BlockLength -1 >= 0xffffffffffff) {
				fprintf(stderr, "Can not convert LBA48 addr=%I64x\n", ScsiCommand->ScsiOp.BlockAddress);
				return FALSE;
			}
			...
			if(ScsiCommand->ScsiOp.BlockAddress + ScsiCommand->ScsiOp.BlockLength -1 >= 0xfffffff) {
				fprintf(stderr, "Can not convert LBA28 addr=%I64x\n", ScsiCommand->ScsiOp.BlockAddress);
				return FALSE;
			}
...
TmsPerformBlockIo(
	...
	if(bret == FALSE) {
		fprintf(stderr, "ConvertScsiCommand2AtaCommand() failed.\n");
	...
	if(srbStatus != XPLSRB_STATUS_SUCCESS) {
		...
		result = 0;
		fprintf(stderr, "NdemuDevRequest() failed. STATUS=%x\n", returnStatus);
	} else {
...
TMDiskInitialize(
	...
	bret = RetrieveAtaDiskInfo(
					...
					sizeof(tmDiskCtx->LowerDiskInfo));
	if(bret == FALSE) {
	...
	if(	(tmDiskCtx->LowerDiskInfo.capability & 0x1) &&
		...
		switch(AtaGetBytesPerBlock(&tmDiskCtx->LowerDiskInfo)) {
			case 0:
			case 512:
			...
			case 2048:
				...
				break;
			case 4096:
				tmDiskCtx->LowerDiskBytesInBlockBitShift = 12;
				break;
			default:
				HeapFree(GetProcessHeap(), 0, tmDiskCtx);
				return FALSE;
...
TMDiskDestroy(
	...
	TmdStop(tmDiskCtx->TmDisk, FALSE);
...
TMDiskRequest(
	...
	switch(AtaCommand->Command) {
		...
		case WIN_SETFEATURES:
		{
			...
			switch(Feature) {
				...
				case SETFEATURES_EN_WCACHE:
					...
					tmDiskCtx->WriteCache = TRUE;
					// TODO: turn on write cache on the lower device
					break;
				case SETFEATURES_DIS_WCACHE:
					fprintf(stderr, "SETFEATURES_DIS_WCACHE\n");
					tmDiskCtx->WriteCache = FALSE;
					// TODO: turn off write cache on the lower device
					break;
				default:
		...
		case WIN_PIDENTIFY:	{
			...
			pInfo->sector_bytes = tmDiskCtx->LowerDiskInfo.sector_bytes;
