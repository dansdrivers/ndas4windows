	...
	try {
		...
		if (iopb->MajorFunction == IRP_MJ_PNP) {
			...
			if (iopb->MinorFunction == IRP_MN_CANCEL_REMOVE_DEVICE || iopb->MinorFunction == IRP_MN_REMOVE_DEVICE) {
				...
				NDAS_BUGON( FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTING) );
				...
				if (NT_SUCCESS(Data->IoStatus.Status)) {
					...
					NDAS_BUGON( !FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTED) );
					LfsDismountVolume( &InstanceContext->LfsDeviceExt );
				}
	...
	try {
		...
		if (iopb->MajorFunction == IRP_MJ_CREATE) {
			...
			if (RtlEqualUnicodeString(&GlobalLfs.MountMgrRemoteDatabase, &fileObject->FileName, TRUE)	|| 
				...
				NDAS_BUGON( FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) );
		...
		if (FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTING)) {
			...
			NDAS_BUGON( InstanceContext->LfsDeviceExt.Secondary == NULL || 
						   Secondary_LookUpCcb(InstanceContext->LfsDeviceExt.Secondary, fileObject) == NULL );
		...
		NDAS_BUGON( FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED) && !FlagOn(InstanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTED) );
		...
		if (iopb->MajorFunction == IRP_MJ_PNP) {
			...
			case IRP_MN_SURPRISE_REMOVAL: {
				...
				NDAS_BUGON( InstanceContext->LfsDeviceExt.NetdiskPartition );
			...
			case IRP_MN_QUERY_REMOVE_DEVICE: {
				...
				NDAS_BUGON( InstanceContext->LfsDeviceExt.AttachedToDeviceObject );
		...
		if (InstanceContext->LfsDeviceExt.SecondaryState == WAIT_PURGE_SAFE_STATE) {
			...
			NDAS_BUGON( iopb->MajorFunction == IRP_MJ_CREATE );
