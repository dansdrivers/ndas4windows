...
//		ULONG				originalFileSpyDebugLevel; 
	...
	case IRP_MJ_CREATE: { //0x00
		...
		try {
			...
			do {
				...
				NDAS_BUGON( siblingOpenFile->FileObject );
			...
			NDAS_BUGON( ioStatusBlock.Information <= 0xffffffff );
	...
	case IRP_MJ_CLOSE: { // 0x02
	   ...
	   UINT64			openFileId = NTOHLL(NdfsWinxpRequestHeader->FileHandle8);
		...
		if (openFile->AlreadyClosed == FALSE) {
			...
			NDAS_BUGON( openFile->FileObject );
	...
	case IRP_MJ_READ: { // 0x03
		...
		if (status != STATUS_SUCCESS) {
			...
			NDAS_BUGON( !NT_SUCCESS(status) );
			NDAS_BUGON( ioStatusBlock.Information == 0 );
	...
	)
{
	...
	NDAS_BUGON( OpenFile->AlreadyClosed == TRUE );
