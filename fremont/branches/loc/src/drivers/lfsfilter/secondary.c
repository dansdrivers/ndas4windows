	...
	)
{
	...
	if (Secondary->Thread.SessionStatus == STATUS_DISK_CORRUPT_ERROR) {
		...
		NDAS_BUGON( Secondary_LookUpCcb(Secondary, fileObject) == NULL );
	...
	while (1) {
		...
		NDAS_BUGON( fastMutexSet == FALSE );
		NDAS_BUGON( retry == FALSE );
		...
		if (!FlagOn(Secondary->Flags, SECONDARY_FLAG_RECONNECTING)) {
			...
			NDAS_BUGON( Secondary->ThreadObject );
		}
		...
		if (status == STATUS_INSUFFICIENT_RESOURCES) {
			...
			NDAS_BUGON( status == STATUS_REMOTE_DISCONNECT || status == STATUS_IO_DEVICE_ERROR );
		}

		NDAS_BUGON( fastMutexSet == TRUE );
		...
		NDAS_BUGON( IsListEmpty(&Secondary->RequestQueue) );
