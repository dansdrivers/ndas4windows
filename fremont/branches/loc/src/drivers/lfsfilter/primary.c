...
PrimaryAgentNICDisabled (
	...
	for(idx_disabled = 0; idx_disabled < AddressList->iAddressCount; idx_disabled ++ ) {
		...
		if (found) {
			...
			NDAS_BUGON( LpxTdiV2IsRequestPending(&listenSock->ListenOverlapped, 0) );
	...
	while (primaryAgentThreadExit == FALSE) {
		...
		if (1 <= ntStatus && ntStatus <= MAX_SOCKETLPX_INTERFACE) {
			...
			if (listenSocket->Active == FALSE) {
				...
				NDAS_BUGON( !LpxTdiV2IsRequestPending(&listenSocket->ListenOverlapped, 0) );
				continue;
			}
	...
	)
{
	...
	for (i = 0; i < MAX_SOCKETLPX_INTERFACE; i++) {
		...
		NDAS_BUGON( Primary->Agent.ListenSocket[i].AddressFileObject != NULL );
		NDAS_BUGON( Primary->Agent.ListenSocket[i].ListenFileObject != NULL );
