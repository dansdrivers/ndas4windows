	...
	)
{
	...
	if (status == STATUS_SUCCESS) {
		...
		*BlockAceId = *blockAceId;
		...
		NDAS_BUGON( NetdiskPartition->ScsiportAdapterDeviceObjectReferenceCount );
		NDAS_BUGON( NetdiskPartition->UnreferenceByQueryRemove == FALSE );
	...
	)
{
	...
	if (netdiskPartition) {
		...
		*NetdiskPartitionInformation = netdiskPartition->NetdiskPartitionInformation;
		...
		NDAS_BUGON( NetdiskPartitionInformation->VolumeName.Length );
	...
	)
{
	...
	if (NetdiskInformation->DeviceMode == DEVMODE_SHARED_READWRITE) {
		...
		NDAS_BUGON( FALSE );
		NDAS_BUGON( NetdiskInformation->DeviceMode == DEVMODE_EXCLUSIVE_READWRITE );
...
if (IS_WINDOWS2K()) {
	...
	case NETDISK_PRIMARY:
		...
		NDAS_BUGON( netdiskEnableMode == NETDISK_PRIMARY );

		break;

	case NETDISK_SECONDARY2PRIMARY:
			
		NDAS_BUGON( netdiskEnableMode == NETDISK_PRIMARY || netdiskEnableMode == NETDISK_SECONDARY );
	...
	for (partitionIndex = 0; partitionIndex < driveLayoutInformation->PartitionCount; partitionIndex++) {
		...
		if (netdiskPartition) {
			...
			if (netdiskPartition->ScsiportAdapterDeviceObjectReferenceCount) {
				...
				NDAS_BUGON( netdiskPartition->ScsiportAdapterDeviceObject == NULL );
				NDAS_BUGON( netdiskPartition->NetdiskPartitionInformation.DiskDeviceObject == NULL );
	...
	)
{
	...
	NDAS_BUGON( EnabledNetdisk->ListEntry.Flink == EnabledNetdisk->ListEntry.Blink );

	if (FlagOn(EnabledNetdisk->Flags, ENABLED_NETDISK_INSERT_DISK_INFORMATION)) {
		
		NETDISK_PARTITION_INFO	netdiskPartitionInfo;

		NDAS_BUGON( EnabledNetdisk->NetdiskEnableMode == NETDISK_SECONDARY ||
					   EnabledNetdisk->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY );
...
if (IS_WINDOWS2K()) {
	...
	NDAS_BUGON( netdiskPartition->NetdiskVolume[netdiskEnableMode].VolumeState == VolumeEnabled || 
	...
	*NetdiskPartition = netdiskPartition;
	...
	if (netdiskPartition->ScsiportAdapterDeviceObject) {
		...
		NDAS_BUGON( *ScsiportAdapterDeviceObject == netdiskPartition->ScsiportAdapterDeviceObject );
		NDAS_BUGON( DiskDeviceObject == netdiskPartition->NetdiskPartitionInformation.DiskDeviceObject );

		ObDereferenceObject( *ScsiportAdapterDeviceObject );

	} else {

		NDAS_BUGON( netdiskPartition->ScsiportAdapterDeviceObjectReferenceCount == 1 );
		NDAS_BUGON( DiskDeviceObject && netdiskPartition->NetdiskPartitionInformation.DiskDeviceObject == NULL );

		NDAS_BUGON( netdiskPartition->UnreferenceByQueryRemove = FALSE );
		NDAS_BUGON( *ScsiportAdapterDeviceObject );
		...
		NDAS_BUGON( netdiskPartition->NetdiskPartitionInformation.VolumeName.Length );
	}
	...
	)
{
	...
	NDAS_BUGON( NetdiskPartition->NetdiskVolume[NetdiskEnableMode].VolumeState == VolumePreMounting ||
	...
	)
{
	...
	if (NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumeEnabled) {
		...
		NDAS_BUGON( GlobalLfs.NdasFsMiniMode == 1 && NetdiskEnableMode == NULL );	
		NDAS_BUGON( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY2PRIMARY].VolumeState == VolumeMounted );

	} else {

		NDAS_BUGON( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY2PRIMARY].VolumeState == VolumeEnabled );
		NDAS_BUGON( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY2PRIMARY].LfsDeviceExt == NULL );

		if (NetdiskEnableMode) {

			NDAS_BUGON( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumeMounted );
	
		} else {

		// called from NdasFat or NdasNtfs

			NDAS_BUGON( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumePostMounting  || 
...
#if !__NDAS_FS_MINI__
	NDAS_BUGON( NetdiskPartition->NetdiskVolume[NetdiskEnableMode].VolumeState == VolumeMounted );
#else
	NDAS_BUGON( NetdiskPartition->NetdiskVolume[NetdiskEnableMode].VolumeState == VolumeMounted ||
				   NetdiskPartition->NetdiskVolume[NetdiskEnableMode].VolumeState == VolumeStopping );
#endif
		...
		if (NetdiskPartition->NetdiskVolume[NETDISK_PRIMARY].VolumeState == VolumeMounted && 
			...
			if (NetdiskPartition->NetdiskVolume[NETDISK_PRIMARY].AttachedToDeviceObject == NULL) {
				...
				if (GlobalLfs.NdasFatRwIndirect == TRUE && NetdiskPartition->FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT ||
					...
					NDAS_BUGON( status == STATUS_INVALID_DEVICE_REQUEST || status == STATUS_INSUFFICIENT_RESOURCES );

				} else {

					NDAS_BUGON( status == STATUS_SUCCESS || status == STATUS_INSUFFICIENT_RESOURCES );
				}
			}
