    ...
    )
{
	...
	NDAS_BUGON( Mid == NTOHS(ndfsRequestHeader->Mid2) );
	
    SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_NOISE,
					("DispatchWinXpRequestWorker: entered PrimarySession = %p, ndfsRequestHeader->Command = %d\n", 
					PrimarySession, ndfsRequestHeader->Command) );

	NDAS_BUGON( PrimarySession->Thread.SessionSlot[Mid].State == SLOT_EXECUTING );
	...
	if (replyDataSize <= 
		...
		NDAS_BUGON( PrimarySession->Thread.SessionSlot[Mid].ExtendWinxpReplyMessagePool );
