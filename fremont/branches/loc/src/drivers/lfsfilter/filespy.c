...
--*/
{
	...
	NDAS_BUGON( oldIrql2 == KeGetCurrentIrql() );
...
do {
    ...
    SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("NetdiskManager_PreMountVolume status = %x\n", status) );
	...
	if (status == STATUS_VOLUME_MOUNTED) {
		...
		NDAS_BUGON( GlobalLfs.NdasFsMiniMode == TRUE );
		NDAS_BUGON( netdiskEnableMode != NETDISK_READ_ONLY );

		break;
	}

	NDAS_BUGON( GlobalLfs.NdasFsMiniMode == FALSE || netdiskEnableMode == NETDISK_READ_ONLY );
...
--*/
{
	...
	if (devExt == NULL) {
		...
		NDAS_BUGON( DeviceObject == gControlDeviceObject );
		return FALSE;		
	}
