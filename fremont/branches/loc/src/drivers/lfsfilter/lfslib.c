...

{
	...
	if (status == STATUS_SUCCESS) {
		...
		status = iosb.Status;
		NDAS_BUGON( iosb.Status != STATUS_INSUFFICIENT_RESOURCES );
	...
	) 
{
	...
	if (returnResult == TRUE && NetdiskInformation) {
		...
		NDAS_BUGON( FlagOn(primUnitDisk.Lur.EnabledFeatures, NDASFEATURE_SIMULTANEOUS_WRITE) );
