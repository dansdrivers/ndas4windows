    ...
    try {
		...
		if (((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL) {
			...
			if (queryStatus != STATUS_SUCCESS) {
				...
				//NDAS_BUGON( FlagOn(netdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) );
				...
				NDAS_BUGON( VolDo->NetdiskPartitionInformation.VolumeName.Length );
...
if (IS_WINDOWS2K()) {
				...
				while (1) {
					...
					NDAS_BUGON( queryStatus == STATUS_SUCCESS || queryStatus == STATUS_BUFFER_OVERFLOW );
