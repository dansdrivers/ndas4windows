...

{
    ...
    try {
		...
		NDAS_BUGON( NT_SUCCESS(NTOHL(ndfsWinxpReplytHeader->Status4)) );
...

{
	...
	try {
		...
		NDAS_BUGON( NT_SUCCESS(status) || status == STATUS_CANNOT_DELETE || status == STATUS_DIRECTORY_NOT_EMPTY );
