	...
	)
{
	...
	while (primarySessionTerminate == FALSE) {
		...
		if (eventStatus == 0) {
			...
			while (TRUE) {
				...
				NDAS_BUGON( !FlagOn(PrimarySession->Thread.Flags, PRIMARY_SESSION_THREAD_FLAG_DISCONNECTED) );
			...
			NDAS_BUGON( eventStatus == 2 );  // Receive Event
	...
	NDAS_BUGON( !LpxTdiV2IsRequestPending(&PrimarySession->Thread.ReceiveOverlapped, 0) );
