	...
	)
{
	...
	if (LSS2) {
		...
		NDAS_BUGON( PduDesc0->Command == WIN_READ && PduDesc2->Command == WIN_READ );
		NDAS_BUGON( PduDesc0->DestAddr + PduDesc0->BlockCount == PduDesc2->DestAddr );
	...
	if (PduDesc->Command == WIN_READ	|| 
		...
		UCHAR nodeAddress[6] = {0x00, 0x0B, 0xD0, 0x02, 0x8D, 0xAD};

		lpxAddress = (PTDI_ADDRESS_LPX)LSS->NdasNodeAddress.Address;

		if (RtlEqualMemory(nodeAddress, lpxAddress->Node, 6)) {

			if (PduDesc->DestAddr >= 0x21000 && PduDesc->DestAddr < 0x22000) {
			//if (PduDesc->DestAddr <= 0x21000 && (PduDesc->DestAddr+PduDesc->BlockCount) >= 0x21000) {
...
//
	...
	if (IsDMA == FALSE) {
		...
		NDAS_BUGON( alignedBufLen <= MAX_PIO_LENGTH_4096 );
	}
