	...
	)
{
	...
	ndasrArbitrator->NdasrState = NRMX_RAID_STATE_NORMAL;
	...
	do {
		...
		for (ridx = 0; ridx < Lurn->LurnChildrenCnt; ridx++) { 
			...
			NDAS_BUGON( ndasrArbitrator->Lurn->NdasrInfo->Rmd.UnitMetaData[ridx].iUnitDeviceIdx < Lurn->LurnChildrenCnt );
		...
		for (ridx = 0; ridx < Lurn->LurnChildrenCnt; ridx++) { // i : role index. 
			...
			if (FlagOn(unitDeviceStatus, NDAS_UNIT_META_BIND_STATUS_NOT_SYNCED)) {
				...
				NDAS_BUGON( ndasrArbitrator->Lurn->NdasrInfo->NodeIsUptoDate[ndasrArbitrator->RoleToNodeMap[ridx]] == FALSE );
				
				if (ridx < ndasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount) {

					//NDAS_BUGON( !FlagOn(unitDeviceStatus, NDAS_UNIT_META_BIND_STATUS_DEFECTIVE) );
					NDAS_BUGON( ndasrArbitrator->OutOfSyncRoleIndex == NO_OUT_OF_SYNC_ROLE );
			...
			if (FlagOn(unitDeviceStatus, NDAS_UNIT_META_BIND_STATUS_OFFLINE)) {
				...
				NDAS_BUGON( FlagOn(ndasrArbitrator->NodeFlags[ndasrArbitrator->RoleToNodeMap[ridx]], NRMX_NODE_FLAG_STOP) );
	...
	if (status == STATUS_SUCCESS) {
		...
		NDAS_BUGON( ndasrArbitrator->ThreadHandle );
		return STATUS_SUCCESS;
	}
	...
	)
{
	...
	NDAS_BUGON( KeGetCurrentIrql() == PASSIVE_LEVEL );
	NDAS_BUGON( NdasrArbitrator->Lurn->NdasrInfo->ParityDiskCount != 0 );
	NDAS_BUGON( NdasrArbitrator->Lurn->NdasrInfo->BlocksPerBit > 0 );
	...
	do {
			...
			if (NdasrArbitrator->NdasrRebuilder.RebuildStatus == NDASR_REBUILD_STATUS_SUCCESS		||
				...
				if (status == STATUS_SUCCESS) {
					...
					if (RtlNumberOfSetBits(&NdasrArbitrator->OosBmpHeader) == 0) {
						...
						NDAS_BUGON( NdasrArbitrator->SyncState == NDASR_SYNC_REQUIRED );
					...
					if (NdasRaidArbitratorRefreshRaidStatus(NdasrArbitrator, FALSE) == FALSE) {
						...
						NDAS_BUGON( NdasrArbitrator->NdasrState == NRMX_RAID_STATE_DEGRADED );
					}
		...
		if (NdasrArbitrator->SyncState == NDASR_SYNC_REQUIRED) {			
			...
			if (!FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_REBUILD)) {
				...
				NDAS_BUGON( !FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_CLUSTER_NODE_UNREACHABLE) );
		...
		if (!IsListEmpty(&NdasrArbitrator->TerminatedClientQueue)) {
			...
			if (!FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_CLUSTER_NODE_UNREACHABLE)) {
				...
				if (NdasrArbitrator->SyncState == NDASR_SYNC_IN_PROGRESS) {
					...
					NDAS_BUGON( !FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_REBUILD) );
		...
		while (listEntry != &NdasrArbitrator->ClientQueue) {
			...
			NDAS_BUGON( clientContext->RequestConnection && clientContext->RequestConnection->ConnectionFileObject );
		...
		while (listEntry != &NdasrArbitrator->ClientQueue) {
			...
			NDAS_BUGON( LpxTdiV2IsRequestPending(&clientContext->RequestConnection->ReceiveOverlapped, 0) );
		...
		if (status != STATUS_SUCCESS) {
			...
			NDAS_BUGON( status == STATUS_INSUFFICIENT_RESOURCES );
			NDAS_BUGON( KeGetCurrentIrql() == PASSIVE_LEVEL );
			continue;
		}

		NDAS_BUGON( eventCount <= 4 );
...
SkipSetEvent:
		...
		if (status != STATUS_SUCCESS) {
			...
			NDAS_BUGON( status == STATUS_CONNECTION_DISCONNECTED || status == STATUS_CLUSTER_NODE_UNREACHABLE );
		...
		NDAS_BUGON( NdasrArbitrator->SyncState == NDASR_SYNC_DONE );
		...
		if (NdasrArbitrator->RequestToTerminate) {
			...
			if (!FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_TERMINATING)) {
				...
				NDAS_BUGON( FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_START) );
		...
		if (NdasrArbitrator->RequestToShutdown) {
			...
			if (!FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_SHUTDOWN)) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
							("DRAID Aribter already Send RETIRE message to all client..\n") );	
		...
		NDAS_BUGON( FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_START) );
		NDAS_BUGON( !FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_SHUTDOWN) );
		NDAS_BUGON( !FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_CLUSTER_NODE_UNREACHABLE) );
		NDAS_BUGON( NdasrArbitrator->SyncState == NDASR_SYNC_DONE );
		...
		if (NdasrArbitrator->NdasrState == NRMX_RAID_STATE_OUT_OF_SYNC) {
			...
			if (!FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_REBUILD)) {
				...
				NDAS_BUGON( RtlNumberOfSetBits(&NdasrArbitrator->OosBmpHeader) != 0 );
	...
	NDAS_BUGON( IsListEmpty(&NdasrArbitrator->AcquiredLockList) );
	...
	NDAS_BUGON( NdasrArbitrator->Lurn->NdasrInfo->Rmd.state == NDAS_RAID_META_DATA_STATE_UNMOUNTED ||
	...
	) 
{
	...
	if (!clientContext) {
		...
		NDAS_BUGON( clientContext->LocalClient == FALSE );
	...
	if (RegisterMsg->ConnType == NRMX_CONN_TYPE_NOTIFICATION) {
		...
		clientContext->Usn = NTOHL(RegisterMsg->Usn);
		...
		NDAS_BUGON( clientContext->NotificationConnection );
	...
	) 
{
	...
	NDAS_BUGON( NdasrArbitrator->OosBmpOnDiskSectorCount );
	NDAS_BUGON( NdasrArbitrator->OosBmpOnDiskSectorCount < (2 << 16) );
	...
	for (nidx=0; nidx < NdasrArbitrator->Lurn->LurnChildrenCnt; nidx++) {
		...
		if (NdasrArbitrator->Lurn->NdasrInfo->Rmd.state == NDAS_RAID_META_DATA_STATE_UNMOUNTED) {
			...
			NDAS_BUGON( RtlEqualMemory(NdasrArbitrator->OosBmpOnDiskBuffer, 
		...
		for (i=0; i<NdasrArbitrator->OosBmpOnDiskSectorCount; i++) {
			...
			for (j=0; j<NDAS_BYTE_PER_OOS_BITMAP_BLOCK; j++) {
				...
				if (NdasrArbitrator->Lurn->NdasrInfo->Rmd.state == NDAS_RAID_META_DATA_STATE_UNMOUNTED && 
					...
					NDAS_BUGON( NdasrArbitrator->OosBmpOnDiskBuffer[i].Bits[j] == 0 );
				}
			}
		}
	}

	NDAS_BUGON( secondOosBmp == TRUE );
	...
	) 
{
	...
	while (listEntry != &NdasrArbitrator->ClientQueue) {
		...
		NDAS_BUGON( clientContext->RequestConnection && 
	...
	)
{
	...
	if (NTOHL(RequestMsg->Usn) != NdasrArbitrator->Lurn->NdasrInfo->Rmd.uiUSN) {
		...
		NDAS_BUGON( ((signed _int32)(NTOHL(RequestMsg->Usn) - NdasrArbitrator->Lurn->NdasrInfo->Rmd.uiUSN)) < 0 );
		NDAS_BUGON( NdasrArbitrator->SyncState != NDASR_SYNC_DONE );
	...
	if (NdasrArbitrator->SyncState != NDASR_SYNC_DONE) {
		...
		NDAS_BUGON( NdasrArbitrator->SyncState == NDASR_SYNC_IN_PROGRESS );
		NDAS_BUGON( RequestMsg->Command == NRMX_CMD_NODE_CHANGE );
	...
	NDAS_BUGON( NTOHL(RequestMsg->Usn) == ClientContext->Usn );
	...
	case NRMX_CMD_NODE_CHANGE: {
		...
		NDAS_BUGON( NdasrArbitrator->SyncState == NDASR_SYNC_DONE || NdasrArbitrator->SyncState == NDASR_SYNC_IN_PROGRESS );
	...
	case NRMX_CMD_ACQUIRE_LOCK: {
		...
		for (listEntry = ClientContext->AcquiredLockList.Flink;
			...
			NDAS_BUGON( NdasRaidGetOverlappedRange( blockAddress, 
													blockLength,
													lock->BlockAddress, 
													lock->BlockLength,
													NULL, 
	...
	case NRMX_CMD_RELEASE_LOCK: {
		...
		if (lockId != NRMX_LOCK_ID_ALL && NTOHL(RequestMsg->Usn) != ClientContext->Usn) {
		...
		if (lockId != NRMX_LOCK_ID_ALL) {
			...
			NDAS_BUGON( commonReplyMsg.Result == NRMX_RESULT_SUCCESS );
		}
...
//	case NRMX_CMD_UNREGISTER:
	...
	if (ClientContext->LocalClient) {
		...
		NDAS_BUGON( ClientContext->RequestConnection->ConnectionFileObject );
	...
	)
{
	...
	NDAS_BUGON( changeStatusMsg->WaitForSync );
	...
	)
{
	...
	if (ClientContext->LocalClient) {
		...
		NDAS_BUGON( ClientContext->NotificationConnection->ConnectionFileObject );
...
retry:
	...
	for (nidx = 0; nidx < NdasrArbitrator->Lurn->LurnChildrenCnt; nidx++) {
		...
		if (FlagOn(NdasrArbitrator->NodeFlags[nidx], NRMX_NODE_FLAG_DEFECTIVE)) {
			...
			NDAS_BUGON( FlagOn(newNodeFlags[nidx], NRMX_NODE_FLAG_DEFECTIVE) );
	...
	for (nidx = 0; nidx < NdasrArbitrator->Lurn->LurnChildrenCnt; nidx++) {
		...
		NDAS_BUGON( NdasrArbitrator->NodeFlags[nidx] == NRMX_NODE_FLAG_RUNNING							||
					NdasrArbitrator->NodeFlags[nidx] == NRMX_NODE_FLAG_DEFECTIVE						||
	...
	for (ridx = 0; ridx < NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount; ridx++) { // i : role index
		...
		if (!FlagOn(NdasrArbitrator->NodeFlags[NdasrArbitrator->RoleToNodeMap[ridx]], NRMX_NODE_FLAG_RUNNING)) {
			...
			case NRMX_RAID_STATE_DEGRADED: {
				...
				NDAS_BUGON( NdasrArbitrator->OutOfSyncRoleIndex != NO_OUT_OF_SYNC_ROLE && NdasrArbitrator->OutOfSyncRoleIndex != ridx );
	...
	NDAS_BUGON( newNdasrState == NRMX_RAID_STATE_NORMAL			||
				newNdasrState == NRMX_RAID_STATE_DEGRADED		||
				newNdasrState == NRMX_RAID_STATE_OUT_OF_SYNC	||
				newNdasrState == NRMX_RAID_STATE_FAILED );
	...
	if (ForceChange != TRUE) {
		...
		NDAS_BUGON( NdasrArbitrator->NdasrState != newNdasrState );
	}
	...
	if (changed == FALSE) {
		...
		NDAS_BUGON( FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_INITIALIZING) );
	}
	...
	) 
{
	...
	if (!FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_INITIALIZING) &&
		...
		NDAS_BUGON( NdasrArbitrator->Lurn->NdasrInfo->Rmd.state == NDAS_RAID_META_DATA_STATE_MOUNTED ||
	...
	for (ridx = 0; ridx < NdasrArbitrator->Lurn->LurnChildrenCnt; ridx++) { // i is role index.
		...
		if (FlagOn(nodeFlags, NRMX_NODE_FLAG_OFFLINE)) {
			...
			NDAS_BUGON( FlagOn(nodeFlags, NRMX_NODE_FLAG_STOP) );
			NDAS_BUGON( FlagOn(NdasrArbitrator->Lurn->NdasrInfo->Rmd.UnitMetaData[ridx].UnitDeviceStatus, 
...
NdasRaidArbitratorWriteMetaSync (
	...
	) 
 {
	...
	NDAS_BUGON( BlockAddr > 0 );
	NDAS_BUGON( FlagOn(NdasrArbitrator->Lurn->AccessRight, GENERIC_WRITE) ||
	...
	if (NdasrArbitrator->NdasrState == NRMX_RAID_STATE_NORMAL ||
		...
		NDAS_BUGON( lurnCount == NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount );
	
	} else if (NdasrArbitrator->NdasrState == NRMX_RAID_STATE_DEGRADED) {

		NDAS_BUGON( lurnCount == NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount - 1 );
	...
	if (lurnCount < NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount - 1) {
		...
		NDAS_BUGON( NDAS_BUGON_NODE_UNRECHABLE );
	...
	for (lidx = 0; lidx < lurnCount; lidx++) { 
		...
		NDAS_BUGON( NdasrArbitrator->NodeFlags[lurnChildren[lidx]->LurnChildIdx] == NRMX_NODE_FLAG_RUNNING );
		
		if (!LURN_IS_RUNNING(NdasrArbitrator->Lurn->LurnChildren[lurnChildren[lidx]->LurnChildIdx]->LurnStatus)) {
			
			//NDAS_BUGON( !FlagOn(NdasrArbitrator->Lurn->NdasrInfo->LocalNodeFlags[lurnChildren[ridx]->LurnChildIdx], NRMX_NODE_FLAG_RUNNING) );
		}
	...
	) 
{
	...
	NDAS_BUGON( IsListEmpty(&Lock->ClientAcquiredLink) );
	NDAS_BUGON( IsListEmpty(&Lock->ArbitratorAcquiredLink) );

	NDAS_BUGON( Lock->Owner == NULL );
	...
	) 
{
	...
	if ((Lock->BlockAddress + Lock->BlockLength) > NdasrArbitrator->Lurn->UnitBlocks) {
		...
		NDAS_BUGON( Lock->BlockLength < NdasrArbitrator->Lurn->NdasrInfo->BlocksPerBit );
	}

	NDAS_BUGON( (Lock->BlockAddress + Lock->BlockLength) <= NdasrArbitrator->Lurn->UnitBlocks );
	...
	if (NdasrArbitrator->NdasrState == NRMX_RAID_STATE_DEGRADED) {
		...
		if (((currentTick.QuadPart - NdasrArbitrator->DegradedSince.QuadPart) * KeQueryTimeIncrement()) < NDAS_RAID_SPARE_HOLDING_TIMEOUT) {
			...
			if (spareFound) {
				...
				NDAS_BUGON( spareNode < NdasrArbitrator->Lurn->LurnChildrenCnt );
				NDAS_BUGON( NdasrArbitrator->NodeFlags[NdasrArbitrator->RoleToNodeMap[NdasrArbitrator->OutOfSyncRoleIndex]] & NRMX_NODE_FLAG_STOP ||
				...
				NDAS_BUGON( FlagOn(NdasrArbitrator->NodeFlags[oosNode], NRMX_NODE_FLAG_DEFECTIVE) );
	...
	) 
{
	...
	NDAS_BUGON( NdasrArbitrator->NdasrRebuilder.RebuildStatus == NDASR_REBUILD_STATUS_NONE &&
				NdasrArbitrator->NdasrRebuilder.RebuildRequested == FALSE );
	
	NDAS_BUGON( NdasrArbitrator->NdasrRebuilder.RebuildLock == NULL );
	...
	if ((blockAddr + blockLength) > NdasrArbitrator->Lurn->UnitBlocks) {
		...
		NDAS_BUGON( blockLength < NdasrArbitrator->Lurn->NdasrInfo->BlocksPerBit );
	}

	NDAS_BUGON( (blockAddr + blockLength) <= NdasrArbitrator->Lurn->UnitBlocks );
	...
	)
{
	...
	NDAS_BUGON( ndasrRebuilder->ThreadObject );
	...
	)
{
	...
	NDAS_BUGON( NdasrRebuilder->RebuildStatus == NDASR_REBUILD_STATUS_NONE || NdasrRebuilder->RebuildRequested );
	...
	)
{
	...
	do {
		...
		if (NdasrRebuilder->RebuildRequested) {	
			...
			NDAS_BUGON( NdasrRebuilder->BlockLength );
			NDAS_BUGON( NdasrRebuilder->RebuildStatus == NDASR_REBUILD_STATUS_NONE );
	...
	) 
{
	...
	do {
		...
		while (ridx < NdasrRebuilder->NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount) { 
			...
			if (lurn->Lur->DeviceMode == DEVMODE_EXCLUSIVE_READWRITE ||
				!FlagOn(lurn->Lur->SupportedNdasFeatures, NDASFEATURE_SIMULTANEOUS_WRITE)) {
