	...
	)
{
	...
	NDAS_BUGON( IdeDisk->LuHwData.BlockBytes );
	...
	) 
{
	...
	while (listEntry != &IdeDisk->CcbQueue) {
		...
		if (!LURN_IS_ROOT_NODE(Lurn)) {
			...
			NDAS_BUGON( !LsCcbIsStatusFlagOn(ccb, CCB_FLAG_ACQUIRE_BUFLOCK) );			
		}
		...
		if (Lurn->LurnStatus == LURN_STATUS_STALL) {
			...
			if (requireNotifyReconnect == TRUE) {
				...
				if (IsListEmpty(&IdeDisk->CcbQueue)) {
					...
					ccb = CONTAINING_RECORD( IdeDisk->CcbQueue.Flink, CCB, ListEntry );
	...
	)
{
	...
	NDAS_BUGON( SplitBlockBytes == (32*1024) || SplitBlockBytes == (64*1024) );
	...
	while (transferedBlocks < TransferBlocks) {
		...
		do {
			...
			NDAS_BUGON( blocksToTransfer2 > 0 );
		...
		if (status != STATUS_SUCCESS) {
			...
			if (response != LANSCSI_RESPONSE_SUCCESS) {
				...
				NDAS_BUGON( response == LANSCSI_RESPONSE_SUCCESS );
	...
	)
{
	...
	NDAS_BUGON( SplitBlockBytes == (64*1024) );
...
//		inquiryData.EnclosureServices;
	...
	case SCSIOP_READ16: {
		...
		if (LsCcbIsFlagOn(Ccb, CCB_FLAG_BACKWARD_ADDRESS)) {
			...
			NDAS_BUGON( IdeDisk->LuHwData.SectorCount >= logicalBlockAddress );
	...
	) 
{
	...
	case LURN_UPDATECLASS_READONLYACCESS: {
		...
		NDAS_BUGON( FlagOn(Lurn->AccessRight, GENERIC_WRITE) );
	...
	case LURN_UPDATECLASS_WRITEACCESS_USERID: {
		...
		NDAS_BUGON( !FlagOn(loginInfo.UserID, 0xffff0000) );
	...
	) 
{
	...
	do {
		...
		NDAS_BUGON( IdeDisk->TempLanScsiSession.LanscsiProtocol == NULL );
