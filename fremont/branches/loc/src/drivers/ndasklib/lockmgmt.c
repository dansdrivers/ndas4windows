...
LockIdToTargetLockIdx(ULONG HwVesion, ULONG TargetId, ULONG LockId) {
	...
	NDAS_BUGON(TargetId < MAX_TARGET_ID);
...
OpAcquireDevLock(
	...
	NDAS_BUGON( TimeOut == NULL || TimeOut->QuadPart <= 0 );
...
LurnIdeDiskDeviceLockControl(
	...
	if (devLockControl->LockId == LURNDEVLOCK_ID_BUFFLOCK) {
		...
		NDAS_BUGON( LsCcbIsFlagOn(Ccb, CCB_FLAG_CALLED_INTERNEL) );
	...
	NDAS_BUGON(IdeDisk->LuHwData.LanscsiTargetID < MAX_TARGET_ID);
...
//			Ccb->CcbStatus = CCB_STATUS_COMMAND_FAILED;
		...
		if (devLockControl->LockId == LURNDEVLOCK_ID_BUFFLOCK) {
			...
			NDAS_BUGON( LockCacheIsAcquired(&IdeDisk->LuHwData,devLockControl->LockId) == FALSE ); 
		}
...
LMInitialize(
	...
	KDPrintM(DBG_LURN_ERROR, ("Smart buffer lock control:%x\n",
...
LMEnable(
	IN PBUFFLOCK_CONTROL	BuffLockCtl,
	IN BOOLEAN				EnableBufferLockSmartControl
) {
	BuffLockCtl->BufferLockSmartControl = EnableBufferLockSmartControl;
}
