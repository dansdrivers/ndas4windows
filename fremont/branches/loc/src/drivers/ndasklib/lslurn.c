	...
	)
{
	...
	NDAS_BUGON( tmpLur->StartOffset == 0 || tmpLur->StartOffset == NDAS_RAID_DEFAULT_START_OFFSET );
	...
	if (tmpLur->LockImpossible) {
		...
		NDAS_BUGON( tmpLur->StartOffset == 0 );
	...
	for (idx_lurn = tmpLur->LurDesc->LurnDescCount - 1; idx_lurn >= 0; idx_lurn--) {
		...
		if (tmpLur->DeviceMode == DEVMODE_SHARED_READONLY) {
			...
			//NDAS_BUGON( tmpLur->DeviceMode == DEVMODE_EXCLUSIVE_READWRITE );
	...
	) 
{
	...
	NDAS_BUGON( Ccb->CcbCurrentStackLocation == &Ccb->CcbStackLocation[Ccb->CcbCurrentStackLocationIndex] );
	...
	NDAS_BUGON( Ccb->CcbCurrentStackLocationIndex == NR_MAX_CCB_STACKLOCATION - 2 );
	...
	) 
{
	...
	NDAS_BUGON( Lurn );
	NDAS_BUGON( KeGetCurrentIrql() == PASSIVE_LEVEL );
	NDAS_BUGON( Lurn->LurnStatus == LURN_STATUS_INIT || Lurn->LurnStatus == LURN_STATUS_STOP || Lurn->LurnStatus == LURN_STATUS_DESTROYING );
