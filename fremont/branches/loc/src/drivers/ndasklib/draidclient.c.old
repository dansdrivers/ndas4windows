...
{
	PDRIX_MSG_CONTEXT RequestMsgLink = NULL;
	PDRIX_HEADER RequestMsg = NULL;
	KIRQL	oldIrql;
	...
	KDPrintM(DBG_LURN_TRACE, ("DRAID request result=%x\n", ReplyMsg->Result));

	if (DRIX_CMD_NODE_CHANGE == ReplyMsg->Command) {
		if (ReplyMsg->Result == DRIX_RESULT_REQUIRE_SYNC) {
			ACQUIRE_SPIN_LOCK(&Client->SpinLock, &oldIrql);
			Client->Lurn->LurnRaidInfo->InTransition = TRUE;
			RELEASE_SPIN_LOCK(&Client->SpinLock, oldIrql);				
		...
		for(i=0;i<Client->Lurn->LurnChildrenCnt;i++) {
			Client->Lurn->LurnRaidInfo->NodeChanged[i] &= ~DRAID_NODE_CHANGE_FLAG_UPDATING;
		}
		RELEASE_SPIN_LOCK(&Client->SpinLock, oldIrql);
		...
		if (ReplyMsg->Length == HTONS((UINT16)sizeof(DRIX_ACQUIRE_LOCK_REPLY)) &&
			...
			if (LockFound == FALSE) {
				Lock = DraidClientAllocLock(
					...
					NTOHLL(AcqReply->BlockAddress),
					NTOHL(AcqReply->BlockLength));
				if (Lock==NULL) {
				...
				if (ReplyMsg->Result == DRIX_RESULT_GRANTED) {
					Lock->LockStatus = LOCK_STATUS_GRANTED;
					KDPrintM(DBG_LURN_TRACE, ("Reply - lock is granted lock id = %I64x, range= %I64x:%x\n", Lock->LockId, 
						Lock->LockAddress, Lock->LockLength));
				} else {
...
) 
{
	...
	KIRQL oldIrql;
	TotalDiskCount = Client->Lurn->LurnChildrenCnt;
...
//	RequestMsg->Version = DRIX_CURRENT_VERSION;
	...
	if (DRIX_CMD_NODE_CHANGE == Command) {
		...
		for(i=0;i<NcMsg->UpdateCount;i++) {
			...
			NcMsg->Node[i].NodeFlags = Client->Lurn->LurnRaidInfo->NodeFlagsLocal[i];;
			NcMsg->Node[i].DefectCode = Client->Lurn->LurnRaidInfo->NodeDefectLocal[i];
		}
		...
		RtlCopyMemory(&RegMsg->RaidSetId, &Client->Lurn->LurnRaidInfo->RaidSetId, sizeof(GUID));
		RtlCopyMemory(&RegMsg->ConfigSetId, &Client->Lurn->LurnRaidInfo->ConfigSetId, sizeof(GUID));		
		...
		AckMsg->BlockAddress = HTONLL(CmdParam2);
		AckMsg->BlockLength = HTONL(CmdParam3);
		KDPrintM(DBG_LURN_TRACE, ("Sending ACQUIRE_LOCK message %I64x:%x\n", CmdParam2, CmdParam3));
		...
		KDPrintM(DBG_LURN_TRACE, ("Sending RELEASE_LOCK message lock id = %I64x\n", CmdParam2));
	}
	...
	if (Client->HasLocalArbiter) {
		...
		if (Client->RequestChannel) {
			KDPrintM(DBG_LURN_TRACE, ("Sending to local arbiter(event=%p)\n", &Client->RequestChannel->Event));
			...
			KDPrintM(DBG_LURN_ERROR, ("Local request channel is NULL\n"));
			ASSERT(FALSE);
	...
	case DRIX_CMD_RETIRE:
		{
			...
			pClientInfo->Lurn->LurnRaidInfo->InTransition = TRUE;
	...
	case DRIX_CMD_CHANGE_STATUS:
		{
			...
			if (CsMsg->NodeCount != pClientInfo->Lurn->LurnChildrenCnt) {
			...
			if (CsMsg->RaidState !=pClientInfo->Lurn->LurnRaidInfo->DRaidState) {
				KDPrintM(DBG_LURN_INFO, ("Changing RAID status from %x to %x!!!\n", 
					pClientInfo->Lurn->LurnRaidInfo->DRaidState ,CsMsg->RaidState));
			}
			if (RtlCompareMemory(&CsMsg->ConfigSetId, &pClientInfo->Lurn->LurnRaidInfo->ConfigSetId , sizeof(GUID)) != sizeof(GUID)) {
				KDPrintM(DBG_LURN_INFO, ("Config Set Id changed!!!\n"));
				RtlCopyMemory(&pClientInfo->Lurn->LurnRaidInfo->ConfigSetId, &CsMsg->ConfigSetId, sizeof(GUID));
				RtlCopyMemory(&pRaidInfo->ConfigSetId, &CsMsg->ConfigSetId, sizeof(GUID));
			}

			pClientInfo->Lurn->LurnRaidInfo->DRaidState = CsMsg->RaidState;
			for(i=0;i<CsMsg->NodeCount;i++) {
				if (pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[i] != CsMsg->Node[i].NodeFlags) {
					KDPrintM(DBG_LURN_INFO, ("Changing node %d flag from %x to %x!!!\n", 
						i, pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[i] ,CsMsg->Node[i].NodeFlags));
				}
				pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[i] = CsMsg->Node[i].NodeFlags;
				if (pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[i] & DRIX_NODE_FLAG_DEFECTIVE) {
					//
					// Once defective, defective until user clears it.
					//
					if (!(pClientInfo->Lurn->LurnRaidInfo->NodeFlagsLocal[i] &DRIX_NODE_FLAG_DEFECTIVE)) {
						KDPrintM(DBG_LURN_INFO, ("Setting defective flag to node %d!!!\n", 
							i, pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[i] ,CsMsg->Node[i].NodeFlags));
						pClientInfo->Lurn->LurnRaidInfo->NodeFlagsLocal[i] |= DRIX_NODE_FLAG_DEFECTIVE;
					}
				}
				if (pClientInfo->Lurn->LurnRaidInfo->NodeToRoleMap[i] !=CsMsg->Node[i].NodeRole) {
					KDPrintM(DBG_LURN_INFO, ("Changing node %d role from %x to %x!!!\n", 
						i, pClientInfo->Lurn->LurnRaidInfo->NodeToRoleMap[i], CsMsg->Node[i].NodeRole));
				}

				pClientInfo->Lurn->LurnRaidInfo->NodeToRoleMap[i] =CsMsg->Node[i].NodeRole;
				pClientInfo->Lurn->LurnRaidInfo->RoleToNodeMap[CsMsg->Node[i].NodeRole] = (UCHAR)i;
				pRaidInfo->RoleIndexLurnChildren[CsMsg->Node[i].NodeRole] = Lurn->LurnChildren[i];
			}
			if (CsMsg->WaitForSync) {
				pClientInfo->Lurn->LurnRaidInfo->InTransition = TRUE;
			} else {
				pClientInfo->Lurn->LurnRaidInfo->InTransition = FALSE;
			}
...
//	ReplyMsg->Version = DRIX_CURRENT_VERSION;
	...
	if (pClientInfo->HasLocalArbiter) {
		if (pClientInfo->NotificationReplyChannel)  {
			PDRIX_MSG_CONTEXT ReplyMsgEntry;
			KDPrintM(DBG_LURN_TRACE, ("DRAID Sending reply %s to local arbiter, event %p\n", 
...
DraidClientUpdateNodeFlags(
	...
	KIRQL oldIrql;
	UCHAR NewFlags = pClientInfo->Lurn->LurnRaidInfo->NodeFlagsLocal[ChildLurn->LurnChildIdx];
	...
	//	
	if (pClientInfo->Lurn->LurnRaidInfo->NodeFlagsLocal[ChildLurn->LurnChildIdx] != NewFlags ||
		((NewFlags & DRIX_NODE_FLAG_DEFECTIVE) && pClientInfo->Lurn->LurnRaidInfo->NodeDefectLocal[ChildLurn->LurnChildIdx] != DefectCode)) 
	{
		KDPrintM(DBG_LURN_INFO, ("Changing local node %d flag from %x to %x(Defect Code=%x)\n",
			ChildLurn->LurnChildIdx, pClientInfo->Lurn->LurnRaidInfo->NodeFlagsLocal[ChildLurn->LurnChildIdx], 	NewFlags, DefectCode)
		);
		//
		// Status changed. We need to report to arbiter.
		// Set changed flags
		//
		pClientInfo->Lurn->LurnRaidInfo->NodeFlagsLocal[ChildLurn->LurnChildIdx] = NewFlags;
		pClientInfo->Lurn->LurnRaidInfo->NodeDefectLocal[ChildLurn->LurnChildIdx] = DefectCode;

		if (pClientInfo->Lurn->LurnRaidInfo->NodeChanged[ChildLurn->LurnChildIdx]  & DRAID_NODE_CHANGE_FLAG_UPDATING) {
			KDPrintM(DBG_LURN_INFO, ("Set node changed flag while node is updating\n"));
		}

		pClientInfo->Lurn->LurnRaidInfo->NodeChanged[ChildLurn->LurnChildIdx] |=DRAID_NODE_CHANGE_FLAG_CHANGED;
...
static ULONG DraidClientRAID1RSelectChildToRead(IN PRAID_INFO pRaidInfo, IN PCCB Ccb)
{
	...
	for(i=0;i<pClient->Lurn->LurnRaidInfo->ActiveDiskCount;i++) {
		if ((pClient->Lurn->LurnRaidInfo->NodeFlagsRemote[pClient->Lurn->LurnRaidInfo->RoleToNodeMap[i]]  & DRIX_NODE_FLAG_RUNNING) &&
			!(pClient->Lurn->LurnRaidInfo->NodeFlagsRemote[pClient->Lurn->LurnRaidInfo->RoleToNodeMap[i]]  & (DRIX_NODE_FLAG_OUT_OF_SYNC | DRIX_NODE_FLAG_DEFECTIVE))) {
			raid_idx = i;
			break;
		}
	}
	if (raid_idx == -1) {
		...
		for(i=0;i<pClient->Lurn->LurnRaidInfo->ActiveDiskCount;i++) {
			if (!(pClient->Lurn->LurnRaidInfo->NodeFlagsRemote[pClient->Lurn->LurnRaidInfo->RoleToNodeMap[i]]  & (DRIX_NODE_FLAG_OUT_OF_SYNC | DRIX_NODE_FLAG_DEFECTIVE))) {
				raid_idx = i;
			}
		}
	}
...
#if 0
	if (RAID_STATUS_EMERGENCY == pClient->Lurn->LurnRaidInfo->DRaidState ||
		RAID_STATUS_EMERGENCY_READY == pClient->Lurn->LurnRaidInfo->DRaidState ||
		RAID_STATUS_RECOVERING == pClient->Lurn->LurnRaidInfo->DRaidState)
	{
		for(i=0;i<pRaidInfo->ActiveDiskCount +  pRaidInfo->SpareDiskCount;i++) {
			if ((pClient->Lurn->LurnRaidInfo->NodeFlagsRemote[i]  & DRIX_NODE_FLAG_RUNNING) &&
				!(pClient->Lurn->LurnRaidInfo->NodeFlagsRemote[i]  & DRIX_NODE_FLAG_OUT_OF_SYNC)) {
	...
	{
		if (pRaidInfo->RoleIndexLurnChildren[0]->LurnStatus != LURN_STATUS_RUNNING &&
			pRaidInfo->RoleIndexLurnChildren[1]->LurnStatus != LURN_STATUS_RUNNING
		) {
			static int temp=0; // temp test...
			temp = !temp;
			// To do: if both child is in stall, select alternatively.
			raid_idx = temp;
		} else if (pRaidInfo->RoleIndexLurnChildren[0]->LurnStatus != LURN_STATUS_RUNNING) {
			raid_idx = 1;
		} else if (pRaidInfo->RoleIndexLurnChildren[1]->LurnStatus != LURN_STATUS_RUNNING) {
			raid_idx = 0;
		} else if (pRaidInfo->RoleIndexLurnChildren[0]->LurnStatus == LURN_STATUS_RUNNING &&
				pRaidInfo->RoleIndexLurnChildren[1]->LurnStatus == LURN_STATUS_RUNNING) {
			...
			Diff0 = (INT64)logicalBlockAddress - (INT64)pRaidInfo->RoleIndexLurnChildren[0]->LastAccessedAddress;
			Diff1 = (INT64)logicalBlockAddress - (INT64)pRaidInfo->RoleIndexLurnChildren[1]->LastAccessedAddress;
			if (Diff0 <0 && Diff1 < 0) {
				Diff0 = -Diff0;
				Diff1 = -Diff1;
				if (Diff0 < Diff1) {
...
DraidClientSendCcbToNondefectiveChildren(
	...
	if (pClientInfo->Lurn->LurnRaidInfo->DRaidState == DRIX_RAID_STATE_DEGRADED) {
		if (pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[Lurn->LurnRaidInfo->RoleIndexLurnChildren[0]->LurnChildIdx] &	
			DRIX_NODE_FLAG_DEFECTIVE) 
		{
			DefectiveExist = TRUE;
			DiskNonDefective = 1;
		} else if (pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[Lurn->LurnRaidInfo->RoleIndexLurnChildren[1]->LurnChildIdx] &	
			DRIX_NODE_FLAG_DEFECTIVE) 
		{
			DefectiveExist = TRUE;
			DiskNonDefective = 0;
	...
	if (DefectiveExist) {
		...
		status = LurnAssocSendCcbToChildrenArray(
			&Lurn->LurnRaidInfo->RoleIndexLurnChildren[DiskNonDefective],
			1,
			Ccb,
			CcbCompletion,
			NULL,
		...
		status = LurnAssocSendCcbToChildrenArray(
			Lurn->LurnRaidInfo->RoleIndexLurnChildren,
			Lurn->LurnRaidInfo->ActiveDiskCount,
			Ccb,
			CcbCompletion,
			NULL,
			NULL,
	...
	PCCB Ccb)
{
	...
	PLURELATION_NODE Lurn = pClientInfo->Lurn;
	ASSERT(pClientInfo);
	ASSERT(Ccb->OperationCode == CCB_OPCODE_EXECUTE);
	...
	case SCSIOP_READ16:	
		...
		//
		status = LurnAssocSendCcbToChildrenArray(
			&Lurn->LurnRaidInfo->RoleIndexLurnChildren[role_idx],
			1,
			Ccb,
...
//
NTSTATUS
DraidClientFlush(PDRAID_CLIENT_INFO ClientInfo, PCCB Ccb, CCB_COMPLETION_ROUTINE CompRoutine)
{
	KIRQL oldIrql;
...
DraidClientRefreshRaidStatusWithoutArbiter(
	...
	//
	for(i=0;i<Client->Lurn->LurnChildrenCnt;i++) {
		Client->Lurn->LurnRaidInfo->NodeFlagsRemote[i] =Client->Lurn->LurnRaidInfo->NodeFlagsLocal[i];
	}
	...
	for(i = 0; i < Client->Lurn->LurnChildrenCnt; i++) // i: role based.
	{
		KDPrintM(DBG_LURN_ERROR, ("MAPPING Lurn node %d to RAID role %d\n",
			Client->Rmd.UnitMetaData[i].iUnitDeviceIdx, i));
		Client->Lurn->LurnRaidInfo->RoleToNodeMap[i] = (UCHAR)Client->Rmd.UnitMetaData[i].iUnitDeviceIdx;
		Client->Lurn->LurnRaidInfo->NodeToRoleMap[Client->Rmd.UnitMetaData[i].iUnitDeviceIdx] = (UCHAR)i;

		Client->Lurn->LurnRaidInfo->RoleIndexLurnChildren[i] = Client->Lurn->LurnChildren[Client->Lurn->LurnRaidInfo->RoleToNodeMap[i]];

		if(NDAS_UNIT_META_BIND_STATUS_NOT_SYNCED & Client->Rmd.UnitMetaData[i].UnitDeviceStatus)
		{
			if (i<Client->Lurn->LurnRaidInfo->ActiveDiskCount) {
				Client->Lurn->LurnRaidInfo->NodeFlagsRemote[Client->Lurn->LurnRaidInfo->RoleToNodeMap[i]] |= DRIX_NODE_FLAG_OUT_OF_SYNC;
				KDPrintM(DBG_LURN_ERROR, ("Node %d(role %d) is out-of-sync\n",  Client->Lurn->LurnRaidInfo->RoleToNodeMap[i], i));
			}
		}
		if(NDAS_UNIT_META_BIND_STATUS_DEFECTIVE & Client->Rmd.UnitMetaData[i].UnitDeviceStatus)
		{
			Client->Lurn->LurnRaidInfo->NodeFlagsRemote[Client->Lurn->LurnRaidInfo->RoleToNodeMap[i]] |= DRIX_NODE_FLAG_DEFECTIVE;
		
			// fault device found
			KDPrintM(DBG_LURN_ERROR, ("Node %d(role %d) is defective\n",  Client->Lurn->LurnRaidInfo->RoleToNodeMap[i], i));
			Client->Lurn->LurnRaidInfo->NodeDefectLocal[Client->Lurn->LurnRaidInfo->RoleToNodeMap[i]] = DraidRmdUnitStatusToDefectCode(Client->Rmd.UnitMetaData[i].UnitDeviceStatus);
		}
	}
	...
	FaultCount = 0;
	for(i=0;i<Client->Lurn->LurnRaidInfo->ActiveDiskCount;i++) { // i : role index
		if (!(Client->Lurn->LurnRaidInfo->NodeFlagsRemote[Client->Lurn->LurnRaidInfo->RoleToNodeMap[i]] & DRIX_NODE_FLAG_RUNNING)
			|| (Client->Lurn->LurnRaidInfo->NodeFlagsRemote[Client->Lurn->LurnRaidInfo->RoleToNodeMap[i]] & DRIX_NODE_FLAG_OUT_OF_SYNC)
			|| (Client->Lurn->LurnRaidInfo->NodeFlagsRemote[Client->Lurn->LurnRaidInfo->RoleToNodeMap[i]] & DRIX_NODE_FLAG_DEFECTIVE)
		) {
	...
	if (FaultCount == 0) {
		if (Client->Lurn->LurnRaidInfo->DRaidState !=DRIX_RAID_STATE_NORMAL)  {
			KDPrintM(DBG_LURN_INFO, ("Entering RAID STATUS NORMAL\n"));	
		}
		Client->Lurn->LurnRaidInfo->DRaidState = DRIX_RAID_STATE_NORMAL;
	} else if (FaultCount == 1) {
		if (Client->Lurn->LurnRaidInfo->DRaidState !=DRIX_RAID_STATE_DEGRADED)  {
			KDPrintM(DBG_LURN_INFO, ("Entering DEGRADED status\n"));	
		}
		Client->Lurn->LurnRaidInfo->DRaidState = DRIX_RAID_STATE_DEGRADED;
	} else { // FaultCount > 1
		KDPrintM(DBG_LURN_INFO, ("More than 1 active node is at fault. RAID failure\n"));
		Client->Lurn->LurnRaidInfo->DRaidState = DRIX_RAID_STATE_FAILED;
	} 
...
DraidClientRegisterToRemoteArbiter(
	...
	RtlCopyMemory(&RegMsg.RaidSetId, &pClientInfo->Lurn->LurnRaidInfo->RaidSetId, sizeof(GUID));
	RtlCopyMemory(&RegMsg.ConfigSetId, &pClientInfo->Lurn->LurnRaidInfo->ConfigSetId, sizeof(GUID));	
...
DraidClientEstablishArbiter(
	...
	pClientInfo->RequestConnection.TdiReceiveContext.Result = 0;
	...
	pClientInfo->NotificationConnection.TdiReceiveContext.Irp = NULL;
	pClientInfo->NotificationConnection.TdiReceiveContext.Result = 0;
	...
	pClientInfo->ClientState = DRAID_CLIENT_STATE_ARBITER_CONNECTED;
	RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);
	return STATUS_SUCCESS;
}
...
DraidClientRecvHeaderAsync(
	...
	if (!Connection->ConnectionFileObject) {
		...
		KDPrintM(DBG_LURN_INFO, ("Connection is in error status\n"));
		ASSERT(FALSE); // This case should not happen.But for the case that event is lost.
		RELEASE_SPIN_LOCK(&Client->SpinLock, oldIrql);	
		return STATUS_UNSUCCESSFUL;
	}
...
DraidClientRecvHeaderWithoutWait(
	...
	if (KeReadStateEvent(&Connection->TdiReceiveContext.CompletionEvent)) {
		...
		if (Connection->TdiReceiveContext.Result <0) {
			...
			status = STATUS_UNSUCCESSFUL;
		} else if (Connection->TdiReceiveContext.Result == sizeof(DRIX_HEADER)) {
			status = DraidClientRecvAdditionalDataFromRemote(Client, Connection);
		} else {
			KDPrintM(DBG_LURN_INFO, ("Invalid length of data received. Receive result=%x\n",Connection->TdiReceiveContext.Result));
			status = STATUS_UNSUCCESSFUL;
		}
		if (status == STATUS_SUCCESS) {
		...
		KDPrintM(DBG_LURN_INFO, ("Receive result=%x without event set. Assume disconnected\n",Connection->TdiReceiveContext.Result));
		ASSERT(FALSE);
		status = STATUS_UNSUCCESSFUL;
	} else {
		status = STATUS_PENDING;
...
DraidClientResetRemoteArbiterContext(
	...
	ACQUIRE_SPIN_LOCK(&pClientInfo->SpinLock, &oldIrql);
	pClientInfo->ClientState = DRAID_CLIENT_STATE_INITIALIZING;
	pClientInfo->Lurn->LurnRaidInfo->DRaidState = DRIX_RAID_STATE_INITIALIZING;
	pClientInfo->Lurn->LurnRaidInfo->InTransition = TRUE; // Hold IO until we get information from arbiter
	...
	RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);
	DraidClientRefreshRaidStatusWithoutArbiter(pClientInfo);
	return STATUS_SUCCESS;
}
...
DraidClientProcessTerminateRequest(
	...
	if (LockAcquired) {
		...
		status = DraidClientSendRequest(pClientInfo, DRIX_CMD_RELEASE_LOCK, 0, DRIX_LOCK_ID_ALL, 0, &Timeout, NULL, NULL);
		return FALSE;
	} 
...
//	static const UINT32 MaxArbiterConnectionTrialCount = 30; // This should be larger than LFS's secondary to primary transition time.
	...
	pClientInfo = ( PDRAID_CLIENT_INFO)pRaidInfo->NdasClient;
	ASSERT(pClientInfo);
	...
	//pClientInfo->ActiveDiskCount = nActiveDiskCount = Lurn->LurnChildrenCnt - pRaidInfo->SpareDiskCount;
	//nSpareCount = pRaidInfo->SpareDiskCount;

	nChildCount = Lurn->LurnChildrenCnt;
	nSpareCount = pRaidInfo->SpareDiskCount;

	//ASSERT(nActiveDiskCount == pRaidInfo->ActiveDiskCount);

	pClientInfo->IsReadonly = LUR_IS_READONLY(Lurn->Lur);

	RtlCopyMemory(&pClientInfo->Lurn->LurnRaidInfo->RaidSetId, &pRaidInfo->RaidSetId, sizeof(GUID));
	RtlCopyMemory(&pClientInfo->Lurn->LurnRaidInfo->ConfigSetId, &pRaidInfo->ConfigSetId, sizeof(GUID));	
	...
	while(TRUE) {
		...
		if (DoMore) {
			...
			if (pClientInfo->HasLocalArbiter) {
				...
				eventCount++;
			} else if (pClientInfo->ClientState == DRAID_CLIENT_STATE_ARBITER_CONNECTED) {
				// Remote arbiter
				if (pClientInfo->NotificationConnection.Status == DRAID_CLIENT_CON_REGISTERED) {
					if (pClientInfo->NotificationConnection.TdiReceiveContext.Result <0) {
						KDPrintM(DBG_LURN_ERROR, ("Previous connection result is %x. Reset current context and enter initialization step.\n", 
							pClientInfo->NotificationConnection.TdiReceiveContext.Result));
						ASSERT(FALSE);	// This case should not happen. If this happens, it mean lpx completion event is lost.
						DraidClientResetRemoteArbiterContext(pClientInfo);
						continue;
					} else {
				...
				if (pClientInfo->RequestConnection.Status == DRAID_CLIENT_CON_REGISTERED) {
					if (pClientInfo->RequestConnection.TdiReceiveContext.Result <0) {
						KDPrintM(DBG_LURN_ERROR, ("Previous connection result is %x. Reset current context and enter initialization step.\n", 
							pClientInfo->RequestConnection.TdiReceiveContext.Result));
						ASSERT(FALSE);	// This case should not happen. If this happens, it mean lpx completion event is lost.
						DraidClientResetRemoteArbiterContext(pClientInfo);
						continue;
					} else {
			...
			if (pClientInfo->Lurn->LurnRaidInfo->DRaidState == DRAID_CLIENT_STATE_INITIALIZING 
				|| pClientInfo->Lurn->LurnRaidInfo->DRaidState == DRIX_RAID_STATE_OUT_OF_SYNC 
...
//				KDPrintM(DBG_LURN_INFO, ("RequestReplyChannel signaled\n"));
		...
		if (pClientInfo->ClientState == DRAID_CLIENT_STATE_INITIALIZING ||
			pClientInfo->ClientState == DRAID_CLIENT_STATE_NO_ARBITER) {
			//
			...
			if (LUR_IS_PRIMARY(Lurn->Lur)) {
				...
				if (!NT_SUCCESS(status)) {
					KDPrintM(DBG_LURN_ERROR, ("Failed to start arbiter\n"));
					goto terminate;
				}
				for(i=0;i<500;i++) {				
...
//						ArbiterConnectionTrialCount = 0;
						...
						KDPrintM(DBG_LURN_ERROR, ("Arbiter terminated. Terminate local client too.\n"));
						goto terminate;
					}
				}
				if (i==500) {
					KDPrintM(DBG_LURN_ERROR, ("Failed to register client to arbiter\n"));
					goto terminate;
				}
				DoMore = TRUE;
			} else if (LUR_IS_SECONDARY(Lurn->Lur)) {
				KeQueryTickCount(&current_time);
				time_diff.QuadPart = (current_time.QuadPart - pClientInfo->NextTryConnectToArbiterTime.QuadPart) * KeQueryTimeIncrement();
				...
				if (time_diff.QuadPart >= HZ * ConnectionRetryInterval) {
					...
					KeQueryTickCount(&pClientInfo->NextTryConnectToArbiterTime);
				} else {
				...
				if (NT_SUCCESS(status)) {
					...
					pClientInfo->Lurn->LurnRaidInfo->InTransition = FALSE;
					RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);	
					if (!IsListEmpty(&pClientInfo->CcbQueue)) {
						KDPrintM(DBG_LURN_INFO, ("Handle some CCBs in NO ARBITER mode\n"));
						status = DraidClientHandleCcb(pClientInfo);
						if (!NT_SUCCESS(status)) {	
							goto terminate;
						}
					}

					KDPrintM(DBG_LURN_INFO, ("Failed to establish connection to arbiter. Enter no arbiter mode. Waiting %d seconds\n", 
						ConnectionRetryInterval));
					// Delay for a while or wait event
					TimeOut.QuadPart = - HZ * ConnectionRetryInterval;
					status = KeWaitForSingleObject(
						&pClientInfo->ClientThreadEvent, Executive, KernelMode,	FALSE, &TimeOut);
					KeClearEvent(&pClientInfo->ClientThreadEvent);
					DoMore = TRUE;
				}
		...
		if (pClientInfo->IsReadonly && pClientInfo->ClientState == DRAID_CLIENT_STATE_NO_ARBITER) {
			KeQueryTickCount(&current_time);
			time_diff.QuadPart = (current_time.QuadPart - pClientInfo->NextTryConnectToArbiterTime.QuadPart) * KeQueryTimeIncrement();
		...
		if (DRAID_CLIENT_STATE_ARBITER_CONNECTED == pClientInfo->ClientState) {
			if (pClientInfo->HasLocalArbiter) {
				...
				status = DraidClientRecvHeaderWithoutWait(pClientInfo, &pClientInfo->RequestConnection);
				if (status == STATUS_SUCCESS) {
		...
		if (DRAID_CLIENT_STATE_ARBITER_CONNECTED == pClientInfo->ClientState) {
			if (pClientInfo->HasLocalArbiter) {
				...
				status = DraidClientRecvHeaderWithoutWait(pClientInfo, &pClientInfo->NotificationConnection);
				if (status == STATUS_SUCCESS) {
		...
		for(i=0;i<nChildCount;i++) {
			ACQUIRE_SPIN_LOCK(&pClientInfo->SpinLock, &oldIrql);
			if (!(pClientInfo->Lurn->LurnRaidInfo->NodeChanged[i] & DRAID_NODE_CHANGE_FLAG_UPDATING) &&
				(pClientInfo->Lurn->LurnRaidInfo->NodeChanged[i] & DRAID_NODE_CHANGE_FLAG_CHANGED)) {
				NodeChanged = TRUE;
				pClientInfo->Lurn->LurnRaidInfo->NodeChanged[i] &= ~DRAID_NODE_CHANGE_FLAG_CHANGED;
				pClientInfo->Lurn->LurnRaidInfo->NodeChanged[i] |= DRAID_NODE_CHANGE_FLAG_UPDATING;
		...
		if (NodeChanged) {
			if (DRAID_CLIENT_STATE_ARBITER_CONNECTED == pClientInfo->ClientState) {
				...
				DoMore = TRUE;
			} else if (DRAID_CLIENT_STATE_NO_ARBITER == pClientInfo->ClientState) {
		...
		if (pClientInfo->RequestForFlush) {
			...
			if (LockInUse) {
				...
				if (Flushed) {
					...
					RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);
					// Flushing completed
					if (pClientInfo->FlushCompletionRoutine) {
						pClientInfo->FlushCompletionRoutine(pClientInfo->FlushCcb, NULL);
					}
					ACQUIRE_SPIN_LOCK(&pClientInfo->SpinLock, &oldIrql);
		...
		if (DRAID_CLIENT_STATE_ARBITER_CONNECTED == pClientInfo->ClientState) {		
				...
				listEntry = listEntry->Flink) 
			{
				...
				if (Lock->YieldRequested ||
					...
					((pClientInfo->Lurn->LurnRaidInfo->DRaidState == DRIX_RAID_STATE_OUT_OF_SYNC || Lock->Contented)&&
...
terminate:
	ACQUIRE_SPIN_LOCK(&pClientInfo->SpinLock, &oldIrql);
	pClientInfo->ClientState = DRAID_CLIENT_STATE_TERMINATING;
	pClientInfo->Lurn->LurnRaidInfo->DRaidState = DRIX_RAID_STATE_TERMINATED;
	RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);	
	...
	//
	{
		...
		while(ccbListEntry = ExInterlockedRemoveHeadList(
							&pClientInfo->CcbQueue,
							&pClientInfo->SpinLock
							)) 
		{
			Ccb = CONTAINING_RECORD(ccbListEntry, CCB, ListEntry);
			KDPrintM(DBG_LURN_INFO, ("Client thread is terminating. Returning CCB with STOP status.\n"));
	...
	while(pClientInfo->RequestToTerminate == FALSE) {
		TimeOut.QuadPart = - NANO100_PER_SEC * 1;
		RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);	
	...
	RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);
#else 
	if (pClientInfo->RequestToTerminate == FALSE) {
		KDPrintM(DBG_LURN_INFO, ("Client thread terminated by itself.\n"));
	}
#endif
	...
	)
{
	...
	ASSERT(pRaidInfo->NdasClient == NULL); // Multiple arbiter is not possible.

	//
	// Allocate client and set initial value.
	//
	pRaidInfo->NdasClient = ExAllocatePoolWithTag(NonPagedPool, sizeof(DRAID_CLIENT_INFO),
		DRAID_CLIENT_INFO_POOL_TAG);
	if (NULL ==  pRaidInfo->NdasClient) {
		return STATUS_INSUFFICIENT_RESOURCES;
	}
		
	pClientInfo = pRaidInfo->NdasClient;
	...
	for(i=0;i<Lurn->LurnChildrenCnt;i++) {
		pClientInfo->Lurn->LurnRaidInfo->NodeFlagsLocal[i] = DraidClientLurnStatusToNodeFlag(Lurn->LurnChildren[i]->LurnStatus);
		pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[i] = DRIX_NODE_FLAG_UNKNOWN;
		pClientInfo->Lurn->LurnRaidInfo->RoleToNodeMap[i] = (UCHAR) -1;	// not valid until LurnFlag is not unknown.
	}
	...
	DraidRegisterClient(pClientInfo);
	return STATUS_SUCCESS;
}
	...
	)
{
	...
	PDRAID_CLIENT_LOCK		Lock;
	PDRIX_MSG_CONTEXT	MsgEntry;
	PDRAID_CLIENT_INFO pClientInfo = (PDRAID_CLIENT_INFO)pRaidInfo->NdasClient;
	PDRIX_MSG_CONTEXT MsgContext;
	...
	pRaidInfo->NdasClient  = NULL;
	RELEASE_SPIN_LOCK(&pRaidInfo->SpinLock, oldIrql);	

	DraidUnregisterClient(pClientInfo);
	ACQUIRE_SPIN_LOCK(&pClientInfo->SpinLock, &oldIrql);
...
DraidClientCheckIoPermission(
	...
	KDPrintM(DBG_LURN_NOISE, ("No full match. Addr %I64x, Length %x\n", Addr, Length));	
	
	// No full match. Need to add up partial matched.
	// Pass2. Bitmap search.
	BitmapBitCount = Length;
	BitmapByteCount = ((BitmapBitCount+31)/32)*4; //RtlInitializeBitMap accept multiple of 32bit only.
	if (BitmapByteCount > sizeof(BitmapBuf) ) {
		BitmapBufPtr = ExAllocatePoolWithTag(NonPagedPool, BitmapByteCount , DRAID_LOCKED_RANGE_BITMAP_POOL_TAG);
	...
	ASSERT(pClientInfo);
	RtlInitializeBitMap(&Bitmap, BitmapBufPtr, BitmapByteCount * 8);
	RtlClearAllBits(&Bitmap);
	if (BitmapBitCount %32 != 0) {
		RtlSetBits(&Bitmap, BitmapBitCount, BitmapByteCount * 8 - BitmapBitCount);
	}
	ASSERT(pClientInfo);	
	ACQUIRE_SPIN_LOCK(&pClientInfo->SpinLock, &oldIrql);
	LockIsPended = FALSE;
	...
	if (RtlAreBitsSet(&Bitmap, 0, Length)) {
		...
		if (LockIsPended == TRUE) {
			...
			KDPrintM(DBG_LURN_NOISE, ("Range %I64x:%x is covered. Partial pending\n", Addr, Length));
			status = STATUS_PENDING;
		} else {
			KDPrintM(DBG_LURN_NOISE, ("Range %I64x:%x is covered and all range is granted\n", 
				Addr, Length
			));
			status = STATUS_SUCCESS;
		...
		*UnauthLength = NumberOfClear;
		KDPrintM(DBG_LURN_NOISE, ("Lock range %I64x:%x is not covered.\n", *UnauthAddr, *UnauthLength));
		status = STATUS_UNSUCCESSFUL;
	}
	RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);
...
) 
{
	...
	if (status == STATUS_PENDING) {
		...
		LARGE_INTEGER Timeout;
		Timeout.QuadPart = HZ / 2; 
		KDPrintM(DBG_LURN_TRACE, ("Send lock request for range %I64x:%x\n", AddrToLock, LengthToLock));
	...
	)
{
	...
	while(TRUE) {
		...
		if (pClientInfo->ClientState !=	DRAID_CLIENT_STATE_NO_ARBITER 
			&& pClientInfo->ClientState != DRAID_CLIENT_STATE_ARBITER_CONNECTED) {
			KDPrintM(DBG_LURN_INFO, ("Client is initialized yet.\n"));	
			RELEASE_SPIN_LOCK(&pClientInfo->SpinLock, oldIrql);
			continue;
		}
		if (pClientInfo->Lurn->LurnRaidInfo->InTransition) {
		...
		for(i = 0; i < nChildCount; i++)
		{
			ChildLurn = pRaidInfo->RoleIndexLurnChildren[i];
			...
			if (pClientInfo->Lurn->LurnRaidInfo->NodeFlagsRemote[pClientInfo->Lurn->LurnRaidInfo->RoleToNodeMap[i]] & DRIX_NODE_FLAG_DEFECTIVE) {
				// Node is defect. No need to revive
				KDPrintM(DBG_LURN_INFO, ("Don't revive defective node %d\n", pClientInfo->Lurn->LurnRaidInfo->RoleToNodeMap[i]));
				continue;
			}
		
			if (LURN_STATUS_RUNNING == ChildLurn->LurnStatus)
			{
				if (pClientInfo->Lurn->LurnRaidInfo->NodeFlagsLocal[pClientInfo->Lurn->LurnRaidInfo->RoleToNodeMap[i]] & DRIX_NODE_FLAG_RUNNING) {
			...
			if(NT_SUCCESS(status) && LURN_IS_RUNNING(ChildLurn->LurnStatus))
			{
				...
				if(!NT_SUCCESS(status)) 
				{
					...
					DraidClientUpdateNodeFlags(pClientInfo, pRaidInfo->RoleIndexLurnChildren[i], 
					...
					!IS_RMD_CRC_VALID(crc32_calc, Rmd)) 
				{
					...
					DraidClientUpdateNodeFlags(pClientInfo, pRaidInfo->RoleIndexLurnChildren[i], 
						DRIX_NODE_FLAG_DEFECTIVE, DRIX_NODE_DEFECT_ETC);
					Defected = TRUE;
				} 
				else if (RtlCompareMemory(&Rmd.RaidSetId, &pClientInfo->Lurn->LurnRaidInfo->RaidSetId, sizeof(GUID)) != sizeof(GUID)) 
				...
				else if (RtlCompareMemory(&Rmd.ConfigSetId, &pClientInfo->Lurn->LurnRaidInfo->ConfigSetId, sizeof(GUID)) != sizeof(GUID)) 
				{
				...
				else if ((Rmd.state & NDAS_RAID_META_DATA_STATE_USED_IN_DEGRADED) &&
					i <pRaidInfo->ActiveDiskCount) 
				{
					// NDAS_RAID_META_DATA_STATE_USED_IN_DEGRADED is invalid for spare disk.
					KDPrintM(DBG_LURN_INFO, ("This member is used without this disk in degraded mode. We cannot accept this disk as member\n"));
					DraidClientUpdateNodeFlags(pClientInfo, pRaidInfo->RoleIndexLurnChildren[i], 
						DRIX_NODE_FLAG_DEFECTIVE, DRIX_NODE_DEFECT_ETC);
					Defected = TRUE;
				}
