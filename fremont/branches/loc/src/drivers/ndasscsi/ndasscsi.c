	...
	)
{
	...
	if (NdasMiniGlobalData.DriverThreadHandle) {
		...
		NDAS_BUGON( NdasMiniGlobalData.DriverThreadObject );
    ...
    )
{
	...
	NDAS_BUGON( HwDeviceExtension->SlotNumber );
	...
	if (HwDeviceExtension->AdapterMaxDataTransferLength != 0) {
		...
		if (ConfigInfo->MaximumTransferLength == SP_UNINITIALIZED_VALUE) {
			...
			ConfigInfo->MaximumTransferLength = NDAS_MAX_TRANSFER_LENGTH;
		}
	}
	...
	NDAS_BUGON( ConfigInfo->AutoRequestSense );

	ConfigInfo->MultipleRequestPerLu;
	NDAS_BUGON( !ConfigInfo->MultipleRequestPerLu );
    ...
    )
{
...
#if 0
	if(Srb->Cdb[0] == SCSIOP_GET_EVENT_STATUS
		) {
		ASSERT(FALSE);
	}
#endif
	...
	if (shippingSrbDetected == TRUE) {
		...
		//DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, ("shippingSrbDetected\n") );

		return TRUE;
	}

#if 0
	NdscPrintSrb("StartIo:", Srb);
#endif
	...
	if (ADAPTER_ISSTATUS(HwDeviceExtension, NDASSCSI_ADAPTER_STATUS_STOPPING) ||
		...
		NDAS_BUGON( Srb->SrbFlags & SRB_FLAGS_IS_ACTIVE );
	...
	case SRB_FUNCTION_EXECUTE_SCSI: {
		...
		case SCSIOP_INQUIRY: {
			...
			for (idx_lur = 0; idx_lur < HwDeviceExtension->LURCount; idx_lur ++) {
				...
				if (lur && lur->LurId[0] == Srb->PathId && lur->LurId[1] == Srb->TargetId && lur->LurId[2] == Srb->Lun) {
					...
					DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,("INQUIRY: set LUR(%p) to LuExtension(%p)\n", lur, luExtension) );
					break;
				}
		...
		if (status != STATUS_MORE_PROCESSING_REQUIRED) {
			...
			DebugTrace( NDASSCSI_DBG_MINIPORT_ERROR,("SRB_FUNCTION_EXECUTE_SCSI: Srb = %p completed.\n", Srb) );
    ...
    )
{
	...
	NDAS_BUGON( PathId <= 0xFF );
	NDAS_BUGON( SCSI_MAXIMUM_TARGETS_PER_BUS <= 0xFF );
	NDAS_BUGON( SCSI_MAXIMUM_LOGICAL_UNITS <= 0xFF );
    ...
    )
{
	...
	do {
		...
		listEntry = ExInterlockedRemoveHeadList( &HwDeviceExtension->CcbTimerCompletionList,
	...
	)
{
	if (Srb->Function == SRB_FUNCTION_EXECUTE_SCSI && 
		...
		NDAS_BUGON( FlagOn(Srb->SrbFlags, SRB_FLAGS_BYPASS_FROZEN_QUEUE) );
		NDAS_BUGON( FlagOn(Srb->SrbFlags, SRB_FLAGS_NO_QUEUE_FREEZE) );

		shippedSrb = shippedCcb->Srb;

		NDAS_BUGON( shippedSrb->SrbFlags & SRB_FLAGS_IS_ACTIVE );
