...

{
    ...
    if((*Mm64BitPhysicalAddress) == TRUE) {
		...
		if (NdasScsiDbgPrintEx) {
			...
			NdasScsiDbgPrintEx(DPFLTR_SCSIPORT_ID,
                   DPFLTR_INFO_LEVEL,
                   "SpDetermine64BitSupport: Mm64BitPhysicalAddress is TRUE\n");
    ...
    )
{
    ...
    entry = (PSINGLE_LIST_ENTRY)ExInterlockedPopEntrySList(&(LogicalUnit->BypassSrbDataList),
														   (PSLIST_ENTRY)&(LogicalUnit->BypassSrbDataSpinLock));
