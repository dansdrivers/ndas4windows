    ...
    __in PIRP Irp)
{
	...
	case IOCTL_NDASPORT_PLUGIN_LOGICALUNIT:
		{
			...
			{
				descriptor->Address.PortNumber = 0;
				...
				{
					NTSTATUS status2 = NdasPortFdoRegSaveLogicalUnit(
						FdoExtension, descriptor);

					if (!NT_SUCCESS(status2))
					{
						NdasPortTrace(NDASPORT_FDO_IOCTL, TRACE_LEVEL_ERROR,
							"NdasPortFdoRegSaveLogicalUnit failed, Status=%x\n", 
							status2);
					}
				}
			}
	...
	case IOCTL_NDASPORT_EJECT_LOGICALUNIT:
		{
			...
			{
				param->LogicalUnitAddress.PortNumber = 0;
	...
	case IOCTL_NDASPORT_DELEGATE_LOGICALUNIT_ENTRY:
		{
			PULONG code;
			ULONG bufferLength;
			...
			code = (PULONG) Irp->AssociatedIrp.SystemBuffer;

			Irp->IoStatus.Information = 0;

			if (bufferLength < sizeof(ULONG) && *code != 1)
			{
				status = STATUS_INVALID_PARAMETER;
			}
			else
			{
				//
				// There should be no pending logical unit initialization
				//
				...
				FdoExtension->InitChildDelegated = TRUE;
	...
	__inout PULONG OutputBufferLength)
{
		...
		entry = entry->Flink) 
	{
		...
		if (NdasPortpIsEqualLogicalUnitAddress(
			LogicalUnitAddress, targetPdo->LogicalUnitAddress))
		{
			//
	...
	__in PNDAS_LOGICALUNIT_DESCRIPTOR LogicalUnitDescriptor)
{
	...
	tmp.LogicalUnitAddress.PortNumber = 0;
		...
		entry = entry->Flink) 
	{
		...
		if (NdasPortpIsEqualLogicalUnitAddress(
			tmp.LogicalUnitAddress, pdoEntry->LogicalUnitAddress))
		{
			ExReleaseFastMutex (&FdoExtension->Mutex); 
	...
	PNDASPORT_LOGICALUNIT_EJECT Parameter)
{
	...
	status = NdasPortFdoDeviceControlEjectOrUnplugLogicalUnit(
		FdoExtension,
		Parameter->LogicalUnitAddress,
		Parameter->Flags,
		TRUE);
