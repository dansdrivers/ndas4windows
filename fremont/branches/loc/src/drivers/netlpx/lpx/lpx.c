	...
	)
{
	...
	if (Connection->LpxSmp.SmpState != SMP_STATE_CLOSE) {
		...
		NDAS_BUGON( Connection->AddressFile );
...
//	KIRQL		cancelIrql;
	...
	default: 
		...
		if (Connection->Status == STATUS_PENDING) {
			...
			NDAS_BUGON( FlagOn(Connection->LpxSmp.Shutdown, SMP_RECEIVE_SHUTDOWN) );			
...

{
    ...
    NDAS_BUGON( (IrpSp->MajorFunction == IRP_MJ_INTERNAL_DEVICE_CONTROL) &&
	...
	)
{
	...
	NDAS_BUGON( irpSp->MajorFunction == IRP_MJ_INTERNAL_DEVICE_CONTROL && irpSp->MinorFunction == TDI_SEND );
	...
	)
{
    ...
    NDAS_BUGON( irpSp->MajorFunction == IRP_MJ_INTERNAL_DEVICE_CONTROL && irpSp->MinorFunction == TDI_RECEIVE );
	...
	)
{
	...
	case SMP_STATE_TIME_WAIT:
		...
		if (Connection->LpxSmp.TimeWaitTimeOut.QuadPart <= NdasCurrentTime().QuadPart) {
			...
			NDAS_BUGON( Connection->LpxSmp.DisconnectIrp == NULL );			
	...
	) 
{
	...
	if (!(NTOHS(lpxHeader->Lsctl) & LSCTL_CONNREQ)) {
		...
		NDAS_BUGON( Connection->AddressFile->Address->PortAssignedByLpx == TRUE );
	...
	if (NTOHS(lpxHeader->Lsctl) & LSCTL_ACK) {
		...
		if (SHORT_SEQNUM(Connection->LpxSmp.RemoteAckSequence) == SHORT_SEQNUM(Connection->LpxSmp.FinSequence)) {
			...
			if (Connection->LpxSmp.DisconnectIrp == NULL) {
			    ...
			    irp->IoStatus.Status = STATUS_SUCCESS;
				DebugPrint( 1, ("[LPX]LpxStateDoReceiveWhenFinWait1: Disconnect IRP %p completed.\n ", irp) );
...
//		packet	= CONTAINING_RECORD( reserved, NDIS_PACKET, ProtocolReserved );
	...
	Connection->LpxSmp.RetransmitTimeOut.QuadPart = NdasCurrentTime().QuadPart + Connection->LpxSmp.RetransmitInterval.QuadPart;
