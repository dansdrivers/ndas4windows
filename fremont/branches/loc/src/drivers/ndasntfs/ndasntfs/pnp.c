			...
			if (!FlagOn(VolumeDeviceObject->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) &&
				...
				if ((IrpContext->MinorFunction == IRP_MN_QUERY_REMOVE_DEVICE	|| 
					...
					while (TRUE) {
						...
						NDAS_BUGON( VolumeDeviceObject->Secondary );
					...
					if (Status == STATUS_SUCCESS) {
						...
						try {
							...
							if (VolumeDeviceObject->NetdiskEnableMode != NETDISK_SECONDARY) {
								...
								NDAS_BUGON( VolumeDeviceObject->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY );
...

{
	...
	if (IrpSp->MinorFunction == IRP_MN_QUERY_REMOVE_DEVICE) {
		...
		SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_PRIMARY_STOPPING );
	}
#endif
    ...
    try {
		...
		if (OurDeviceObject->Secondary) {
			...
			NDAS_BUGON( OurDeviceObject->NetdiskEnableMode == NETDISK_SECONDARY || 
			...
			case IRP_MN_QUERY_REMOVE_DEVICE: {
				...
				DebugTrace( 0, Dbg, ("NtfsCommonPnp: IRP_MN_QUERY_REMOVE_DEVICE OurDeviceObject = %p, NetdiskEnableMode = %d\n", 
									   OurDeviceObject, OurDeviceObject->NetdiskEnableMode) );

				secondaryRecoveryResourceAcquired 
					= SecondaryAcquireResourceExclusiveLite( IrpContext, 
															 &OurDeviceObject->RecoveryResource, 
															 FALSE );
				...
				SecondaryReleaseResourceLite( IrpContext, &OurDeviceObject->RecoveryResource );
				...
				secondaryCreateResourceAcquired 
					= SecondaryAcquireResourceExclusiveLite( IrpContext, 
															 &OurDeviceObject->CreateResource, 
															 BooleanFlagOn(IrpContext->State, IRP_CONTEXT_STATE_WAIT) );
				...
				if (Vcb->SecondaryCloseCount) {
					...
					ASSERT( ccbCount == Vcb->SecondaryCloseCount );
					...
					DebugTrace( 0, Dbg, ("IRP_MN_QUERY_REMOVE_DEVICE: Vcb->SecondaryCloseCount = %d, Vcb->SecondaryCleanupCount = %d, Vcb->CloseCount = %d, ccbCount = %d\n",
                                         Vcb->SecondaryCloseCount, Vcb->SecondaryCleanupCount, Vcb->CloseCount, ccbCount) );

					restartKey = NULL;
					fcb = NdasNtfsGetNextFcbTableEntry( &Secondary->VolDo->Vcb, &restartKey );
					...
					SetFlag( Secondary->Flags, SECONDARY_FLAG_DISMOUNTING );
				}
			...
			case IRP_MN_REMOVE_DEVICE: {
				...
				DebugTrace( 0, Dbg, ("NtfsCommonPnp: IRP_MN_REMOVE_DEVICE NetdiskEnableMode = %d\n", ((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode) );
			...
			default: {
				...
				DebugTrace( 0, Dbg, ("NtfsCommonPnp: IrpSp-MinorFunction = %d NetdiskEnableMode = %d\n", 
                ...
                if ((Vcb->CloseCount - (Vcb->SystemFileCloseCount + Vcb->QueuedCloseCount)) > 0) {
                    ...
                    if (NT_SUCCESS( Status )) {
						...
						if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->BlockAceId) {
							...
							((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->NdfsCallback.RemoveWriteRange( 
								Vcb->Vpb->RealDevice, 
						...
						NDAS_BUGON( Vcb->SecondaryCleanupCount == 0 );
						NDAS_BUGON( Vcb->SecondaryCloseCount == 0 );
						NDAS_BUGON( Vcb->SecondaryReadOnlyCloseCount == 0 );
						NDAS_BUGON( Vcb->SecondarySystemFileCloseCount == 0 );
#endif
                    }
                }
            }
    ...
    try_exit: NOTHING;        
		...
		if (secondaryCreateResourceAcquired) {
			...
			SecondaryReleaseResourceLite( IrpContext, &OurDeviceObject->CreateResource );
