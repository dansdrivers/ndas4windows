...

{
	...
	NDAS_BUGON( !FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) );
	NDAS_BUGON( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode != NETDISK_SECONDARY );
...
*/
		...
		if (VolDo->NetdiskEnableMode == NETDISK_SECONDARY &&
			...
			if (Vcb->LogFileObject != NULL) {
				...
				NDAS_BUGON( Vcb->LogFileObject == NULL );
...

{
	...
	case FSCTL_NDAS_FS_FLUSH_OR_PURGE: {
		...
		NDAS_BUGON( FlagOn(IrpContext->State, IRP_CONTEXT_STATE_WAIT) );
...
#if __NDAS_NTFS_PRIMARY__
	{
		...
		NDAS_BUGON( volDo->NetdiskEnableMode == NETDISK_PRIMARY			|| 
...
#if __NDAS_NTFS_PRIMARY__
	{
		...
		NDAS_BUGON( volDo->NetdiskEnableMode == NETDISK_PRIMARY || volDo->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY );
