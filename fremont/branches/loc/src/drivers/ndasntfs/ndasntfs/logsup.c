...

{
	...
	NDAS_BUGON( !FlagOn(Scb->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY) );
...

{
	...
	NDAS_BUGON( IrpContext->Vcb == NULL || CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode != NETDISK_SECONDARY );
...

{
    ...
    try {
		...
		NDAS_BUGON( IrpContext->Usn.CurrentUsnFcb == NULL && IrpContext->TransactionId == 0 ||
