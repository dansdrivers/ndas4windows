	...
	)
{
	...
	NDAS_BUGON( requestIdx < LPXTDIV2_MAX_REQUEST );
	...
	NDAS_BUGON( Irp->IoStatus.Status == STATUS_SUCCESS || !NT_SUCCESS(Irp->IoStatus.Status) );
	...
	)
{
	...
	NDAS_BUGON( lpxAddress->Port < HTONS((UINT16)0x4000) );
	...
	NDAS_BUGON( status == ioStatusBlock.Status );
	...
	)
{
	...
	NDAS_BUGON( KeGetCurrentIrql() == PASSIVE_LEVEL );
	NDAS_BUGON( AddressFileHandle && AddressFileObject );
	...
	) 
{
	...
	NDAS_BUGON( KeGetCurrentIrql() == PASSIVE_LEVEL );
	NDAS_BUGON( ConnectionFileObject && ConnectionFileHandle && OverlappedContext );
	...
	*ConnectionFileObject = connectionFileObject;
	...
	NDAS_BUGON( !FlagOn(OverlappedContext->Flags1, LPXTDIV2_OVERLAPPED_CONTEXT_FLAG1_INITIALIZED) );
	...
	)
{
	...
	for (requestIdx = 0; requestIdx < LPXTDIV2_MAX_REQUEST; requestIdx ++) {
		...
		NDAS_BUGON( OverlappedContext->Request[requestIdx].Irp == NULL );
		NDAS_BUGON( KeReadStateEvent(&OverlappedContext->Request[requestIdx].CompletionEvent) == 0 );
	}
	...
	)
{
	...
	if (TimeOut) {
		...
		NDAS_BUGON( TimeOut->QuadPart != 0 &&
					  ((TimeOut->QuadPart < 0 && (-TimeOut->QuadPart) >= 3*NANO100_PER_SEC && (-TimeOut->QuadPart) <= 1000*NANO100_PER_SEC) ||
					   TimeOut->QuadPart >= (NdasCurrentTime().QuadPart + 3*NANO100_PER_SEC)) );
	}
	...
	if (OverlappedContext == NULL) {
		...
		if (status == STATUS_PENDING) {
			...
			if (status == STATUS_SUCCESS) {
				...
				NDAS_BUGON( status == STATUS_TIMEOUT );
				...
				NDAS_BUGON( irp->IoStatus.Status == STATUS_CANCELLED );
		...
		NDAS_BUGON( status == STATUS_SUCCESS || !NT_SUCCESS(status) );
		...
		NDAS_BUGON( OverlappedContext->Request[0].Irp == NULL );
		NDAS_BUGON( KeReadStateEvent(&OverlappedContext->Request[0].CompletionEvent) == 0 );
		NDAS_BUGON( AddressBuffer == NULL );
	...
	)
{
	...
	NDAS_BUGON( KeGetCurrentIrql() == PASSIVE_LEVEL );
	NDAS_BUGON( SendBuffer && SendLength );

	if (TimeOut) {

		NDAS_BUGON( TimeOut->QuadPart != 0 &&
					  ((TimeOut->QuadPart < 0 && (-TimeOut->QuadPart) >= 3*NANO100_PER_SEC && (-TimeOut->QuadPart) <= 1200*NANO100_PER_SEC) ||
					   TimeOut->QuadPart >= (NdasCurrentTime().QuadPart + 3*NANO100_PER_SEC)) );
	}
	...
	if (OverlappedContext == NULL) {
		...
		NDAS_BUGON( irp->MdlAddress != NULL );
	...
	)
{
	...
	NDAS_BUGON( KeGetCurrentIrql() == PASSIVE_LEVEL );
	NDAS_BUGON( RecvBuffer && RecvLength );
	...
	)
{
	...
	NDAS_BUGON( KeGetCurrentIrql() == PASSIVE_LEVEL );
	NDAS_BUGON( LpxRemoteAddress && SendBuffer && SendLength );
	...
	)
{
	...
	NDAS_BUGON( KeReadStateEvent(&OverlappedContext->Request[RequestIdx].CompletionEvent) == 1 );
	...
	)
{
	...
	NDAS_BUGON( FlagOn(SourceOverlappedContext->Flags1, LPXTDIV2_OVERLAPPED_CONTEXT_FLAG1_INITIALIZED) );
