...
struct hd_driveid {
	...
	unsigned short	words119_124[6];/* reserved words 119-124 */
	unsigned short  pio_bytecount; /* ATAPI only. byte count=0 behavior */
...
//
// Device configuration overlay structure
//
...
C_ASSERT(sizeof(ATA_SMART_ATTRIBUTE) == 12);


//
// Return data for SMART sub-command READ_ATTRIBUTES.
//
#define ATA_SMART_NR_ATTR 30

typedef struct _ATA_SMART_DATA {
	unsigned short int revnumber;
	ATA_SMART_ATTRIBUTE vendor_attributes [30];
...
C_ASSERT(sizeof(ATA_SMART_DATA) == 512);
