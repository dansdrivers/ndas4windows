...
public:
	...
	QueueUserWorkItemEx(
		...
		ULONG Flags) throw()
	{
		WorkItemParamEx* pex = static_cast<WorkItemParamEx*>(
			::HeapAlloc(::GetProcessHeap(), 0, sizeof(WorkItemParamEx)));
		if (NULL == pex)
		{
			::SetLastError(ERROR_OUTOFMEMORY);
			return FALSE;
		...
		pex->MemberFunc = MemberFunc;
		pex->FuncParam = FuncParam;
		if (!::QueueUserWorkItem(WorkItemStartProcEx, pex, Flags))
		{
			XTLVERIFY( ::HeapFree(::GetProcessHeap(), 0, pex) );
			return FALSE;
		}
	...
	static BOOL QueueUserWorkItem(T* ClassInstance,	ULONG Flags) throw()
	{
		return ::QueueUserWorkItem(WorkItemStartProc, ClassInstance, Flags);
	}
	
	static DWORD WINAPI WorkItemStartProc(LPVOID pArgs) throw()
	{
		T* ClassInstance = static_cast<T*>(pArgs);
		P p;
		return ClassInstance->WorkItemStart(p);
	}
	static DWORD WINAPI WorkItemStartProcEx(LPVOID pArgs) throw()
	{
		WorkItemParamEx* pex = static_cast<WorkItemParamEx*>(pArgs);
		DWORD ret = (pex->ClassInstance->*(pex->MemberFunc))(pex->FuncParam);
		XTLVERIFY( ::HeapFree(::GetProcessHeap(), 0, pex) );	
		return ret;
	}

	struct WorkItemParamEx
	{
		T* ClassInstance;
		DWORD (T::*MemberFunc)(P);
		P FuncParam;
...
template <typename T, typename P = LPVOID, typename Traits = Win32UserWorkItemTraits<T,P> > class CUserWorkItem;

template <typename T, typename P, typename Traits>
class CUserWorkItem 
{
	typedef CUserWorkItem<T, P, Traits> ThisClass;
public:
	BOOL QueueUserWorkItem(T* ClassInstance, ULONG Flags = WT_EXECUTEDEFAULT) throw()
	{
		return Traits::QueueUserWorkItem(ClassInstance, Flags);
	}
	BOOL QueueUserWorkItemParam(T* ClassInstance, P p, ULONG Flags = WT_EXECUTEDEFAULT) throw()
	{
		return Traits::QueueUserWorkItemEx(ClassInstance, &T::WorkItemStart, p, Flags);
	}
	BOOL QueueUserWorkItemEx(T* ClassInstance, DWORD (T::*MemberFunc)(P), P p, ULONG Flags = WT_EXECUTEDEFAULT) throw()
	{
		return Traits::QueueUserWorkItemEx(ClassInstance, MemberFunc, p, Flags);
	}
	//
	// ClassInstance should implement the function
	//
	// DWORD WorkItemStart() {...}
	// int Run() { m_workItem.QueueUserWorkItem(this); }
	//
	// DWORD WorkItemStart(P p) {...}
	// int RunParam() { m_workItem.QueueUserWorkItemParam(this, p); }
	//
	// or 
	// 
	// DWORD AnotherWorkItemStart(P p) {} 
	// int Run() { m_workItem.QueueUserWorkItemEx(this, &ThisClass::AnotherWotkItemStart); }
	//
};
...
template <
	typename T, 
	typename P = void, 
	typename WorkItemTraits = Win32UserWorkItemTraits<T,P> >
class CQueuedWorker;

template <typename T, typename P, typename WorkItemTraits>
class CQueuedWorker : public CUserWorkItem<T, P, WorkItemTraits>
{
	typedef CUserWorkItem<T, P, WorkItemTraits> BaseClass;

	DWORD WorkItemStart(P Param)
	{
		XTLC_ASSERT(FALSE && "Derived class should implement this function");
	}
...
public:
	...
	BOOL QueueUserWorkItem(ULONG Flags = WT_EXECUTEDEFAULT)
	{
		T* pT = static_cast<T*>(this);
		return BaseClass::QueueUserWorkItem(pT, Flags);
	}

	BOOL QueueUserWorkItemParam(P p, ULONG Flags = WT_EXECUTEDEFAULT)
	{
		T* pT = static_cast<T*>(this);
		return BaseClass::QueueUserWorkItemParam(pT, p, Flags);
	}

	BOOL QueueUserWorkItemEx(DWORD (T::*MemberFunc)(P), P p, ULONG Flags = WT_EXECUTEDEFAULT)
	{
		T* pT = static_cast<T*>(this);
		return BaseClass::QueueUserWorkItemEx(pT, MemberFunc, p, Flags);
	}
};
