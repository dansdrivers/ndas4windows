...
*/
unsigned
CMainFrame::ThreadAddUnitDevices(
...
/*
* Launch add unit devices each CNBNdasDevice
*/
...
public:
	...
	void operator() (CNBNdasDevice *pNBNdasDevice) const
	{
		...
		{
			// already added & initialized at EnumDevicesCallBack
			goto out;
		}
		...
		*/
		...
		hThread = (HANDLE)_beginthreadex(
			NULL,
			0,
			CMainFrame::_ThreadAddUnitDevices,
			pThreadContext,
			0,
			NULL);
		if(!hThread)
...
out:
		if(NULL == hThread)
		{
			...
			InterlockedDecrement(&m_pMainFrame->m_nAssigned);
			SetEvent(m_pMainFrame->m_hEventThread);
		}
		else
		{
	...
	LPVOID lpContext)
{
	...
	*/
	...
	CNBNdasDevice *pNdasDevice;
	{
		...
		pNdasDevice = new CNBNdasDevice(
			CString(lpEnumEntry->szDeviceName),
			&DeviceId,
			status,
	...
	pMainFrame->m_listDevices.push_back(pNdasDevice);

	/*
	* If NDAS device is not alive, add dummy unit device.
	*/
	if(!pNdasDevice->IsAlive())
	{
		// should not connect
		CNBUnitDevice *pUnitDevice = pNdasDevice->AddUnitDevice(0, NDAS_UNITDEVICE_TYPE_DISK);
		pUnitDevice->Initialize();
		goto out;
	}
...
//	UIEnableForDevice(NULL, IDM_TOOL_SPAREREMOVE);
	...
	*/
	...
	while(1)
	{
		...
		if(0 == InterlockedCompareExchange(&m_nAssigned, 0, 0))
			break;
	}
			...
			*/
				...
				itNdasDevice != m_listDevices.end(); itNdasDevice++)
			{
					...
					pLogicalDevice->RAID_INFO()->Members[i].DeviceId.VID)) 
				{
					...
					*/
					pMissingNdasDevice = new CNBNdasDevice(pNdasDevice);
...
BOOL CALLBACK CheckReplaceDevice(CNBUnitDevice *pUnitDevice, HWND hWnd, LPVOID lpContext)
{
	...
	UINT64 ui64CapacityRequired =
		pLogicalDevice->PrimaryUnitDevice()->GetLogicalCapacityInByte();
	UINT64 ui64CapacityOfNewMember = pUnitDevice->GetPhysicalCapacityInByte();
...
void CMainFrame::OnReplaceDevice(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	//
	UINT32 logicalDeviceType = pLogicalDevice->GetType();
	if (logicalDeviceType != NMT_RAID1R3 &&
		logicalDeviceType != NMT_RAID4R3 &&
		logicalDeviceType != NMT_RAID5) {
			return;
	}
...
void CMainFrame::OnUseAsMember(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	// Unit device should be initialized to get physical capacity
	if (FALSE == pUnitDevice->Initialize(FALSE)) {
...
void CMainFrame::OnFixRaidState(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	}
	{
		...
		CString strMsg;
		strTitle.LoadString(IDS_APPLICATION);
		strMsg.LoadString(IDS_FIX_RAID_WARNING);
		int id = MessageBox(
			strMsg,
			strTitle,
			MB_YESNO|MB_ICONEXCLAMATION
