	...
	__in BSTR RegistrySubPath)
{
	...
	m_NdasUnits.SetCount(m_NdasLogicalUnitDefinition.NdasUnitCount);
...
{
	*UnitCount = m_NdasLogicalUnitDefinition.NdasUnitCount;
	return S_OK;
}
...
{
	if (Sequence >= m_NdasLogicalUnitDefinition.NdasUnitCount)
	{
		return E_INVALIDARG;
	}

	*NdasUnitId = m_NdasLogicalUnitDefinition.NdasUnits[Sequence];
...
{
	return m_NdasLogicalUnitDefinition.NdasUnitCount;
}

DWORD 
CNdasLogicalUnit::pGetUnitDeviceCountSpare() const
{
	return m_NdasLogicalUnitDefinition.SpareNdasUnitCount;
}
...
CNdasLogicalUnit::AddNdasUnitInstance(INdasUnit* pNdasUnit)
{
	...
	XTLASSERT(luseq < m_NdasLogicalUnitDefinition.NdasUnitCount);
	if (luseq >= m_NdasLogicalUnitDefinition.NdasUnitCount) 
	{
		XTLTRACE2(NDASSVC_NDASLOGDEVICE, TRACE_LEVEL_ERROR,
			"Invalid unit device sequence, seq=%d, count=%d\n", 
			luseq, m_NdasLogicalUnitDefinition.NdasUnitCount);
		return E_INVALIDARG;
	}
...
{
	DWORD activeCount = 0;
	for (DWORD i = 0; i < m_NdasLogicalUnitDefinition.NdasUnitCount; ++i)
	{
...
CNdasLogicalUnit::pGetMemberNdasUnit(DWORD Seq, INdasUnit** ppNdasUnit)
{
	*ppNdasUnit = 0;
	...
	XTLASSERT(Seq < m_NdasLogicalUnitDefinition.NdasUnitCount);
	if (Seq >= m_NdasLogicalUnitDefinition.NdasUnitCount)
	{
		return E_INVALIDARG;
	}
...
//	}
	...
	{	
		...
		NDAS_UNITDEVICE_ID ndasUnitId = m_NdasLogicalUnitDefinition.NdasUnits[i];
	...
	if (!success) 
	{
		...
		NdasBusCtlEject(m_NdasLogicalUnitId);
	...
	DWORD LdpfValues)
{
	...
	{	
		...
		if (!pNdasUnit) 
		{
			...
			ZeroMemory(&ataNodeConfig->BindingAddress, sizeof(TA_LSTRANS_ADDRESS));
...
CNdasLogicalUnit::pIsEmergencyMode()
{
	...
	hr = pPrimaryNdasUnit->GetRaidSimpleStatus(&RaidSimpleStatusFlags);
	...
	else
	{
		...
		if (logicalUnitHandle.IsInvalid())
		{
			XTLTRACE2(NDASSVC_NDASLOGDEVICE, TRACE_LEVEL_ERROR,
				...
				m_SystemDevicePath, GetLastError());
		}
		else
		{
