	...
	DWORD LdpfValues)
{
	...
	{	
		...
		if (CNdasUnitDeviceNullPtr == pUnitDevice) {
			...
			nodeInitData.EndLogicalBlockAddress.QuadPart = pPrimaryUnitDevice->GetUserBlockCount() - 1; //  Assume RAID1, RAID4 and use primary device's
	...
	LPBOOL lpbPrimary)
{
	...
	if(ndasPort == FALSE) {
		...
		DWORD deviceNumber = GetDeviceNumberHint();
