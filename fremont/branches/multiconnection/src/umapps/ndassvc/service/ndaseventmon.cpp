...
CNdasEventMonitor::OnLogicalDeviceAlarmed(CNdasLogicalDevicePtr pLogDevice)
{
	...
	while(TRUE)
	{
		...
		if(!ADAPTERINFO_ISSTATUSFLAG(ulAdapterStatus, NDASSCSI_ADAPTER_STATUSFLAG_NEXT_EVENT_EXIST)) {
			break;
		}
	}
}
...
void
CNdasEventMonitor::OnLogicalDeviceAlarmedByPnP(CNdasLogicalDevicePtr pLogDevice, ULONG LogDeviceStatus)
{
	NDAS_LOCATION ndasLocation = pLogDevice->GetNdasLocation();
	NDAS_LOGICALUNIT_ADDRESS	ndasLogicalUnitAddress;
	...
	XTLTRACE2(NDASSVC_EVENTMON, TRACE_LEVEL_INFORMATION,
		"Logical device alarmed by PnP, ndasLocation=%d, adapterStatus=%08X.\n", 
