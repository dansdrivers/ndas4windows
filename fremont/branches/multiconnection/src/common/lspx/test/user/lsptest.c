...
int lsptest_run_cmds(lsptest_context_t** context_list, int context_count)
{
	int ret, ret2, fc;
	lsp_login_info_t lsp_login_info;
	void** session_buffer_list;
	...
	session_buffer_list = calloc(context_count, sizeof(void*));
	if (NULL == session_buffer_list)
	{
	...
	{
		session_buffer_list[i] = malloc(LSP_SESSION_BUFFER_SIZE);
		if (NULL == session_buffer_list[i])
		{
			printf("error: out of memory\n");
			return 1;
		}
		context_list[i]->lsp_handle = lsp_initialize_session(
			session_buffer_list[i], LSP_SESSION_BUFFER_SIZE);
	}

	context = context_list[0];
	session_buffer = session_buffer_list[0];
	...
	for (;;)
	{
		...
		else if (0 == strcmp(token, "login"))
		{
			...
			else
			{
				...
				{
					ret = lsptest_login(context_list[i], &lsp_login_info);
					if (0 != ret)
					{
						fprintf(stderr, "error: login failed[%u].\n", i);
						context_list[i]->lsp_handle = lsp_initialize_session(
							session_buffer_list[i], LSP_SESSION_BUFFER_SIZE);
					}
				}
		...
		else if (0 == strcmp(token, "handshake"))
		{
			for (i = 0; i < context_count; ++i)
			{
				ret = lspsh_ata_handshake(context_list[i]);
			}
		}
		...
		else if (0 == strcmp(token, "read_mpath"))
		{
		...
		else if (0 == strcmp(token, "read"))
		{
			...
			{
				filename = token;
				if (0 == strcmp("nul", filename))
				{
					rmode = lspsh_read_nul;
				}
				else
				{
					...
					rmode = lspsh_read_bin;
				}
			}
			else
			{
				rmode = lspsh_read_text;
			}
...
{
	static const char* usage = "usage: lsptest <device-address> [connections]";
	...
	context_list = calloc(context_count, sizeof(lsptest_context_t*));

	for (i = 0; i < context_count; ++i)
	{
		context_list[i] = lsptest_transport_create();
		if (0 == context_list[i])
		{
		...
		ret = lsptest_transport_connect(context_list[i], ndas_dev_addr);
		if (0 != ret)
		{
	...
	ret = lsptest_run_cmds(context_list, context_count);

	for (i = 0; i < context_count; ++i)
	{
		lsptest_transport_disconnect(context_list[i]);
	}
