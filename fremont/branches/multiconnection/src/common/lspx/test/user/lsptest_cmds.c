...
int lspsh_ata_handshake(lsptest_context_t* context)
{
	...
	fprintf(stdout, "packet device?: %d\n", handshake_data->device_type);
	fprintf(stdout, "lba: %d\n", handshake_data->lba);
	fprintf(stdout, "lba48: %d\n", handshake_data->lba48);
	fprintf(stdout, "dma: %d\n", handshake_data->dma);
	fprintf(stdout, "Transfer mode: ");
	switch (handshake_data->active_mode)
	{
	case LSP_IDE_TRANSFER_MODE_PIO_DEFAULT:
		fprintf(stdout, "PIO %d\n", handshake_data->active_level);
		break;
	case LSP_IDE_TRANSFER_MODE_PIO_FLOW_CONTROL:
		fprintf(stdout, "PIO %d with flow-control\n", handshake_data->active_level);
		break;
	...
	case LSP_IDE_TRANSFER_MODE_ULTRA_DMA:
		...
		break;
	default:
		fprintf(stdout, "mode=%02X, level=%d\n", handshake_data->active_mode, handshake_data->active_level);
		break;
	}
#if defined(_MSC_VER)
...
const char* disp_ata_version(lsp_uint16_t major, lsp_uint16_t minor)
{
	...
	lsptest_context_t* context)
{
	...
	fprintf(out, "* Major Rev: %04X (%u)\n", lsp_letohs(ident->major_revision), lsp_letohs(ident->major_revision));
	fprintf(out, "* Minor Rev: %04X (%u)\n", lsp_letohs(ident->minor_revision), lsp_letohs(ident->minor_revision));
...
int 
lsptest_read(
	lsptest_context_t** context_list,
	int context_count,
	...
	lspsh_read_mode_t parse_mode)
{
	...
	struct _mpath_context_t {
		lsp_uint8_t* buf;
		int complete;
	}* mpath_context;

	hwdata = lsp_get_hardware_data(context_list[0]->lsp_handle);
	bufblks = min(hwdata->maximum_transfer_blocks, sectors);
//	bufblks = min(64, bufblks);

	mpath_context = calloc(context_count, sizeof(struct _mpath_context_t));
	for (i = 0; i < context_count; ++i)
	{
		mpath_context[i].buf = calloc(bufblks, sector_size);
		if (NULL == mpath_context[i].buf)
		{
...
//			++i, sectors - rblocks, sectors, lba.quad, elba.quad, cblocks);
	...
	for (;;)
	{
		...
		for (i = 0; i < context_count; ++i)
		{
			if (mpath_context[i].complete && rblocks > 0)
			{
				lsp_uint16_t cb = min(rblocks, cblocks);
				//fprintf(stderr, "read[%d], lba=%I64X, blocks=%u\n",
				//	i, lba.quad, cb);
				context_list[i]->lsp_status = lsp_ide_read(
					context_list[i]->lsp_handle, 
					&lba, 
					cb, 
					mpath_context[i].buf, 
					cb * sector_size);
				mpath_context[i].complete = 0;
				rblocks -= cb;
				lba.quad += cb;
			}
		}
		...
		for (i = 0; i < context_count; ++i)
		{
			if (!mpath_context[i].complete)
			{
				ret = lsptest_transport_process_transfer_non_wait(context_list[i]);
				if (ret != 1) 
				{
					mpath_context[i].complete = 1;
					ret = lsptest_check_lsp_status(context_list[i], ret);
					if (0 != ret)
					{
						return ret;
					}
	...
	fprintf(stderr, "%u KB, ", sectors * 512 / 1024);
	fprintf(stderr, "read in %.2f seconds, ", duration);
	fprintf(stderr, "%.2f MB/sec.\n", sectors * 512 / duration / 1024 / 1024);
	fprintf(stderr, "done.\n");
