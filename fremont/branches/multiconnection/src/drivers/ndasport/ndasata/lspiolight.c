...
{
	ASSERT(FALSE == TestFlag(LspIoLightSession->SessionFlags, LSP_IO_LIGHT_SESSION_OP_IN_PROGRESS));
	SetFlag(&LspIoLightSession->SessionFlags, LSP_IO_LIGHT_SESSION_OP_IN_PROGRESS);
		...
		case LSP_REQUIRES_RECEIVE:
			{
				...
				receiveTimeout.QuadPart = -10 * SECOND_IN_100_NS;

				tdiIoSession = LspIoLightSessionpGetNextTdiIoSession(LspIoLightSession);
				ASSERT(NULL != tdiIoSession);

				xTdiIoSessionStartTimer(tdiIoSession, receiveTimeout);
	...
	__in PIO_STATUS_BLOCK IoStatus)
{
	...
	ASSERT(TestFlag(LspIoLightSession->SessionFlags, LSP_IO_LIGHT_SESSION_OP_IN_PROGRESS));
	ClearFlag(&LspIoLightSession->SessionFlags, LSP_IO_LIGHT_SESSION_OP_IN_PROGRESS);
