...
#if __NDAS_MINI_PORT_W2K_SUPPORT__

KEREMOVEBYKEYDEVICEQUEUEIFBUSY	NdasScsiKeRemoveByKeyDeviceQueueIfBusy = NULL;
MMPROTECTMDLSYSTEMADDRESS		NdasScsiMmProtectMdlSystemAddress = NULL;
IOFREEERRORLOGENTRY				NdasScsiIoFreeErrorLogEntry = NULL;
DBGPRINGEX						NdasScsiDbgPrintEx = NULL;
VDBGPRINTEXWITHPREFIX			NdasScsivDbgPrintExWithPrefix = NULL;

ULONG NdasScsiOsMajorVersion;
ULONG NdasScsiOsMinorVersion;
...

{
...
#if __NDAS_MINI_PORT__

#if DBG
	DbgPrint( "\nPort Driver %s %s\n", __DATE__, __TIME__ );
#endif

    PsGetVersion( &NdasScsiOsMajorVersion,
                  &NdasScsiOsMinorVersion,
                  NULL,
                  NULL );
...

	{
		...
		RtlInitUnicodeString( &tempUnicode, L"KeRemoveByKeyDeviceQueueIfBusy" );
		NdasScsiKeRemoveByKeyDeviceQueueIfBusy = MmGetSystemRoutineAddress( &tempUnicode );

		RtlInitUnicodeString( &tempUnicode, L"MmProtectMdlSystemAddress" );
		NdasScsiMmProtectMdlSystemAddress = MmGetSystemRoutineAddress( &tempUnicode );

		RtlInitUnicodeString( &tempUnicode, L"IoFreeErrorLogEntry" );
		NdasScsiIoFreeErrorLogEntry = MmGetSystemRoutineAddress( &tempUnicode );

		if (IS_WINDOWSXP_OR_LATER()) {

			// Avoid Bug Check on Windows 2000

			RtlInitUnicodeString( &tempUnicode, L"DbgPrintEx" );
			NdasScsiDbgPrintEx = MmGetSystemRoutineAddress( &tempUnicode );
		}

		RtlInitUnicodeString( &tempUnicode, L"vDbgPrintExWithPrefix" );
		NdasScsivDbgPrintExWithPrefix = MmGetSystemRoutineAddress( &tempUnicode );
	}

#endif

#if __NDAS_MINI_PORT__
	return MiniPortDriverEntry( DriverObject, RegistryPath );
#endif
	...
	)
{
	...
	if (SpGuidInterfaceMappingList) {
		...
		ExFreePool( SpGuidInterfaceMappingList );
		SpGuidInterfaceMappingList = NULL;
	}

	if (ScsiGlobalAdapterList) {

		ExFreePool( ScsiGlobalAdapterList );
		ScsiGlobalAdapterList = NULL;
	}
}
...
#if !__NDAS_MINI_PORT_AVOID_ASSERT__
...

{
	...
	UNREFERENCED_PARAMETER( HwInitData );
