...

{
    ...
    case IRP_MN_USER_FS_REQUEST:
        ...
        Status = NdasFatSecondaryUserFsCtrl( IrpContext, Irp );
        break;
...

{
	...
	try {
		...
		if (Status == STATUS_SUCCESS && fileSystemControl.FsControlCode == FSCTL_MOVE_FILE) {		// 29
			...
			if (typeOfOpen == UserFileOpen && FlagOn(volDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) && ndfsWinxpReplytHeader->FileInformationSet && ndfsWinxpReplytHeader->AllocationSize) {
