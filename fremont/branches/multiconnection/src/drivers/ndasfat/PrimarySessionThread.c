	...
	)
{
	...
	while (primarySessionTerminate == FALSE) {
		...
		if (eventStatus == 0) {
			...
			while (TRUE) {
				...
				NDASFAT_ASSERT( !FlagOn(PrimarySession->Thread.Flags, PRIMARY_SESSION_THREAD_FLAG_DISCONNECTED) );
	...
	NDASFAT_ASSERT( !LpxTdiV2IsRequestPending(&PrimarySession->Thread.ReceiveOverlapped, 0) );
