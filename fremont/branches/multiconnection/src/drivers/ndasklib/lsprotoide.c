...
#endif
{
	...
	if (OverlappedData) {
		...
		ntStatus = SendIt( &LSS->ConnectionFile,
						   (PCHAR)pHeader,
	...
	)
{
	...
	ntStatus = IdeLspSendRequest_V11(LSS, &pdu, &LSS->RequestOverlappedData, 0, NULL);
	if(!NT_SUCCESS(ntStatus)) {
	...
	//ntStatus = IdeLspSendRequest_V11( LSS, &pdu, NULL, 0, NULL );
...
#if 0
	if (LSS->NdastConnectionFile.OverlappedContext.Request[0].IoStatusBlock.Status != STATUS_SUCCESS ||
		LSS->NdastConnectionFile.OverlappedContext.Request[0].IoStatusBlock.Information != sizeof(LANSCSI_H2R_PDU_HEADER)) {
	...
	)
{
	...
	if (LSS2) {
		...
		NDASSCSI_ASSERT( PduDesc0->Command == WIN_READ && PduDesc2->Command == WIN_READ );
		NDASSCSI_ASSERT( PduDesc0->DestAddr + PduDesc0->BlockCount == PduDesc2->DestAddr );
	...
	ntStatus = IdeLspSendRequest_V11(LSS, &pdu0, &LSS->RequestOverlappedData, 0, NULL);
	if(!NT_SUCCESS(ntStatus)) {
		LsprotoWaitForCompEvent(&sendRequestCompEvent, NULL);
		KDPrintM(DBG_PROTO_ERROR, ("Error when Send Request\n"));
		// Assume sending communication error as retransmit.
	...
	if (LSS2) {
		...
		ntStatus = IdeLspSendRequest_V11(LSS, &pdu2, &LSS->RequestOverlappedData, 0, NULL);
		if(!NT_SUCCESS(ntStatus)) {
			LsprotoWaitForCompEvent(&sendRequestCompEvent, NULL);
			KDPrintM(DBG_PROTO_ERROR, ("Error when Send Request\n"));
			// Assume sending communication error as retransmit.
	...
	*PduResponse0 = pReplyHeader->Response;
	...
	if (LSS2) {
		...
		NDASSCSI_ASSERT( recvData );
		NDASSCSI_ASSERT( sendData == FALSE );
