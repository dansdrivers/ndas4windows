	...
	)
{
	...
	do {
		...
		*/
		...
		//	PIO.
		//
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("NetDisk does not support DMA mode. Turn to PIO mode.\n"));
	...
	)
{
	...
	do {
		...
		IdeDisk->MaxStallInterval.QuadPart  = IDE_DISK_DEFAULT_MAX_STALL_INTERVAL;
		IdeDisk->ReConnectInterval.QuadPart = IDE_DISK_DEFALUT_RECONNECT_INTERVAL;
		...
		if (FlagOn(Lurn->AccessRight, GENERIC_WRITE)) {
			...
			NDASSCSI_ASSERT( loginInfo.MaxDataTransferLength == (64*1024) || 
			...
			status = LspLogin( IdeDisk->LanScsiSession,
							   &loginInfo,
							   LSProto,
							   NULL,
							   TRUE );
		...
		if (IdeDisk->LuHwData.LudataFlags & LUDATA_FLAG_SMART_ENABLED) {
			...
			if (status == STATUS_NOT_SUPPORTED) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Initial power recycle count = %I64u\n", IdeDisk->LuHwData.InitialDevPowerRecycleCount));
		
				NDASSCSI_ASSERT( IdeDisk->LuHwData.InitialDevPowerRecycleCount );
			}
	...
	)
{
	...
	NDASSCSI_ASSERT( Lurn->IdeDisk->ThreadHandle != NULL );
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("stopped completely\n") );
	...
	) 
{
	...
	if (Lurn->LurnStatus != LURN_STATUS_RUNNING && Lurn->LurnStatus != LURN_STATUS_STOP_PENDING) {
		...
		NDASSCSI_ASSERT( Lurn->LurnStatus == LURN_STATUS_STALL );
	...
	) 
{
	...
	while (listEntry != &IdeDisk->CcbQueue) {
		...
		NDASSCSI_ASSERT( LURN_IS_RUNNING(Lurn->LurnStatus) );
		...
		if (ccb->OperationCode != CCB_OPCODE_DEVLOCK) {
			...
			NDASSCSI_ASSERT( ccb->CcbStatus == CCB_STATUS_COMMUNICATION_ERROR );
		}

		NDASSCSI_ASSERT( !LsCcbIsFlagOn(ccb, CCB_FLAG_MUST_SUCCEED) );
	...
	do {
		...
		if (Lurn->LurnStatus == LURN_STATUS_RUNNING) {
			...
			if (status == STATUS_SUCCESS) {
				...
				IdeDisk->MaxStallTimeOut.QuadPart = LpxTdiV2CurrentTime().QuadPart + IdeDisk->MaxStallInterval.QuadPart;
				IdeDisk->ReConnectTimeOut.QuadPart = LpxTdiV2CurrentTime().QuadPart;
		...
		if (Lurn->IdeDisk->RequestToTerminate == TRUE) {
			...
			NDASSCSI_ASSERT( IsListEmpty(&IdeDisk->CcbQueue) );
		...
		if (Lurn->LurnStatus == LURN_STATUS_RUNNING) {
			...
			if (Lurn->IdeDisk->AddAddressOccured == TRUE) {
				...
				do {
					...
					status = LurnIdeChangeConnection( Lurn, IdeDisk );
		...
		if (Lurn->LurnStatus == LURN_STATUS_STALL) {
			...
			NDASSCSI_ASSERT( IdeDisk->MaxStallTimeOut.QuadPart != 0 );
			NDASSCSI_ASSERT( IdeDisk->ReConnectTimeOut.QuadPart != 0 );

			if (IdeDisk->MaxStallTimeOut.QuadPart < LpxTdiV2CurrentTime().QuadPart) {
			...
			if (IdeDisk->ReConnectTimeOut.QuadPart > LpxTdiV2CurrentTime().QuadPart) {

				continue;
			}

			IdeDisk->ReConnectTimeOut.QuadPart = LpxTdiV2CurrentTime().QuadPart + IdeDisk->ReConnectInterval.QuadPart;
			...
			do {
				...
				if (FlagOn(Lurn->AccessRight, GENERIC_WRITE)) {
					...
					NDASSCSI_ASSERT( status == STATUS_SUCCESS || status == STATUS_PORT_DISCONNECTED );
				...
				if (IdeDisk->LanScsiSession->LanscsiProtocol == NULL) {
				...
				if (status != STATUS_SUCCESS) {
					...
					status = LspReconnect( IdeDisk->LanScsiSession,  
										   NULL, 
										   NULL, 
										   NULL );
					...
					if (status != STATUS_SUCCESS) {
						...
						LurnRecordFault( Lurn, 
										 LURN_ERR_LOGIN, 
										 IdeDisk->LanScsiSession->LastLoginError, 
										 NULL );
				...
				status = LurnIdeDiskReconnect( Lurn, IdeDisk );
				...
				if (status == STATUS_CONNECTION_COUNT_LIMIT) {
					...
					DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, 
								("RECONN: reconnection count limit. Terminating.\n") );
	...
	)
{
	...
	NDASSCSI_ASSERT( SplitBlockBytes == (32*1024) || SplitBlockBytes == (64*1024) );
	...
	NDASSCSI_ASSERT( splitBlocks <= 128 );

	//NDASSCSI_ASSERT( FlagOn(IdeDisk->Lurn->AccessRight, GENERIC_WRITE) );
	...
	while (transferedBlocks < TransferBlocks) {
		...
		NDASSCSI_ASSERT( blocksToTransfer > 0 );
		...
		do {
			...
			NDASSCSI_ASSERT( blocksToTransfer2 > 0 );
		...
		NDASSCSI_ASSERT( !LsCcbIsFlagOn(Ccb, CCB_FLAG_ACQUIRE_BUFLOCK) );
#endif
		...
		if (status != STATUS_SUCCESS) {
			...
			if (response != LANSCSI_RESPONSE_SUCCESS) {
				...
				NDASSCSI_ASSERT( response == LANSCSI_RESPONSE_SUCCESS );
...
//				Ccb->CcbStatus = CCB_STATUS_INVALID_COMMAND;
	...
	if (status == STATUS_SUCCESS) {
		...
		NDASSCSI_ASSERT( transferedBlocks == TransferBlocks );
	...
	)
{
	...
	{
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO,	("SCSIOP = 0x%x %s \n",
									Ccb->Cdb[0],
...
//				BYTE one_sector[BLOCK_SIZE];
				...
				case CCB_EXT_READ:
					{
						...
						case EXT_BLOCK_OPERATION:
							DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,	("CCB_EXT_READ: ext_cmd_op->pByteData = %p Addr %I64x %x\n", ext_cmd_op->pByteData, logicalBlockAddress, ext_cmd_op->LengthBlock));
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE,	("CCB_EXT_READ\n"));
				...
				case CCB_EXT_WRITE:
					{
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE,	("CCB_EXT_WRITE\n"));
				...
				case CCB_EXT_READ_OPERATE_WRITE:
					{
						DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,	("CCB_EXT_READ_OPERATE_WRITE\n"));
						...
						status = LurnIdeDiskRead(
										...
										1,
										IdeDisk->MaxDataRecvLength);
						DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,	("CCB_EXT_READ_OPERATE_WRITE read status %x\n", status));
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,	("CCB_EXT_READ_OPERATE_WRITE write status %x\n", status));
...
//		inquiryData.EnclosureServices;
	...
	case SCSIOP_RESERVE_UNIT:
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("RESERVE(6): %02x %02x %02x %02x %02x %02x\n",
												Ccb->Cdb[0], Ccb->Cdb[1],
												Ccb->Cdb[2], Ccb->Cdb[3],
												Ccb->Cdb[4], Ccb->Cdb[5]));
	...
	case 0x56:					// RESERVE_UNIT(10)
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("RESERVE(10): %02x %02x %02x %02x %02x %02x\n",
												Ccb->Cdb[0], Ccb->Cdb[1],
												Ccb->Cdb[2], Ccb->Cdb[3],
												Ccb->Cdb[4], Ccb->Cdb[5],
												Ccb->Cdb[6], Ccb->Cdb[7],
	...
	case SCSIOP_RELEASE_UNIT:
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("RELEASE(6): %02x %02x %02x %02x %02x %02x\n",
												Ccb->Cdb[0], Ccb->Cdb[1],
												Ccb->Cdb[2], Ccb->Cdb[3],
												Ccb->Cdb[4], Ccb->Cdb[5]));
		status = LurnIdeDiskReserveRelease(Lurn,IdeDisk, Ccb);
	...
	case 0x57:					// RELEASE_UNIT(10)
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("RELEASE(10): %02x %02x %02x %02x %02x %02x\n",
												Ccb->Cdb[0], Ccb->Cdb[1],
												Ccb->Cdb[2], Ccb->Cdb[3],
												Ccb->Cdb[4], Ccb->Cdb[5],
												Ccb->Cdb[6], Ccb->Cdb[7],
	...
	case SCSIOP_WRITE16:
	{
		...
		if(!(Lurn->AccessRight & GENERIC_WRITE)){	// Read-only LURN
			if(!LsCcbIsFlagOn(Ccb, CCB_FLAG_ALLOW_WRITE_IN_RO_ACCESS)) {
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("WRITE: WRITE command"
	...
	) 
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,
				("entered.(%02x:%02x:%02x)\n", 
				 IdeDisk->LanScsiSession->NdasNodeAddress.Address[5], // Low three byte of target MAC
				 IdeDisk->LanScsiSession->NdasNodeAddress.Address[6],
				 IdeDisk->LanScsiSession->NdasNodeAddress.Address[7]) );
	...
	if (Lurn->Lur->DeviceMode == DEVMODE_SHARED_READWRITE) {
		...
		if (FlagOn(Lurn->AccessRight, GENERIC_WRITE)) {
			...
			NDASSCSI_ASSERT( RECONNECTION_INTERVAL_SECONDARY(Lurn->ReconnInterval) <= NANO100_PER_SEC*9 );
		...
		if (Lurn->LurnParent && LURN_REDUNDENT_TYPE(Lurn->LurnParent->LurnType)) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,("Max reconnection time for RAID member = %d\n", MaxStall));
		}
	...
	if (StallCount > MaxStall) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,
					("reconnection trial reach the maximum %d! Connection will be lost.\n", MaxStall) );

		return STATUS_CONNECTION_COUNT_LIMIT;
	}

	DebugTrace( DBG_LURN_INFO,("reconnection trial count %d\n", StallCount) );

	//	Set the short timeout not to block too long here.

	LspLogout( &IdeDisk->CommLanScsiSession, &shortTimeout );
	LspDisconnect( &IdeDisk->CommLanScsiSession );

	if (FlagOn(Lurn->AccessRight, GENERIC_WRITE)) {

		LspLogout( &IdeDisk->PrimaryLanScsiSession, &shortTimeout );
		LspDisconnect( &IdeDisk->PrimaryLanScsiSession );
	}
	...
	if (FlagOn(Lurn->AccessRight, GENERIC_WRITE)) {
		...
		status = LspRelogin( &IdeDisk->PrimaryLanScsiSession, LssEncBuff, &shortTimeout );
	...
	status = LspRelogin( &IdeDisk->CommLanScsiSession, LssEncBuff, &shortTimeout );
	...
	) 
{
	...
	case LURN_UPDATECLASS_READONLYACCESS: {
		...
		NDASSCSI_ASSERT( FlagOn(Lurn->AccessRight, GENERIC_WRITE) );
	...
	case LURN_UPDATECLASS_WRITEACCESS_USERID: {
		...
		NDASSCSI_ASSERT( !FlagOn(loginInfo.UserID, 0xffff0000) );
