	...
	)
{
	...
	NdasFcStatistics->MinimumRoundTripTime.QuadPart = 
		(HZ << NDAS_FC_LOG_OF_AMPLIFIER) / (NdasFcStatistics->MaxTotalThroughput.QuadPart >> NDAS_FC_LOG_OF_AMPLIFIER);

	NDASSCSI_ASSERT( NdasFcStatistics->TotalThroughput.QuadPart > 0 );
	...
	)
{
	...
	if (((EndTime.QuadPart - StartTime.QuadPart) << NDAS_FC_LOG_OF_AMPLIFIER) > (TotalSend * NdasFcStatistics->MinimumRoundTripTime.QuadPart)) {
		...
		diff.QuadPart = (TotalSend * NdasFcStatistics->MinimumRoundTripTime.QuadPart) >> NDAS_FC_LOG_OF_AMPLIFIER;
#if 1
		NdasFcStatistics->MinimumRoundTripTime.QuadPart -= NdasFcStatistics->MinimumRoundTripTime.QuadPart >> 10;
	...
	NDASSCSI_ASSERT( diff.QuadPart > 0 );
	...
	NdasFcStatistics->Throughput[idx].QuadPart += 
		(((TotalSend * HZ) << NDAS_FC_LOG_OF_AMPLIFIER) / diff.QuadPart) >> NDAS_FC_LOG_OF_FRACTION;
