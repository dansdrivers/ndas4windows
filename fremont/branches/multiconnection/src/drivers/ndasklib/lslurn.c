	...
	)
{
	...
	for (idx_lurn = LurDesc->LurnDescCount - 1; idx_lurn >= 0; idx_lurn--) {
		...
		status = LurnCreate( cur_lurn, tmpLur, cur_lurndesc );
		...
		if (status != STATUS_SUCCESS) {
			...
			for (idx_closinglurn = idx_lurn; 
				...
				LurnClose( lurndesc_table[idx_closinglurn].Lurn );
			}
	...
	) 
{
	...
	status = Lurn->LurnInterface->LurnFunc.LurnCreate( Lurn, LurnDesc );
