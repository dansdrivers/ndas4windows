	...
	) 
{
	...
	NDASSCSI_ASSERT( Overlapped == NULL );
	...
	NDASSCSI_ASSERT( ((PTDI_ADDRESS_LPX)SrcAddr->Address)->Port == 0 );
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
				("ndiskOid = %x, status = %x, mediumSpeed = %d, ndisOid = %p, mediumSpeed = %p\n", 
				 ndisOid, status, mediumSpeed, &ndisOid, &mediumSpeed) );
	...
	)
{
	...
	for (idx_addr = 0; idx_addr < (ULONG)BindAddrList->TAAddressCount; idx_addr++) {
		...
		if (NT_SUCCESS(status)) {
			...
			NDASSCSI_ASSERT( bindAddr->AddressLength == TDI_ADDRESS_LENGTH_LPX );
	...
	) 
{
	...
	NDASSCSI_ASSERT( LSS );
	NDASSCSI_ASSERT( DestAddr );
	NDASSCSI_ASSERT( BindAnyAddress == TRUE );
	...
	) 
{
	...
	do {
		...
		if (addrListPrioritized->TAAddressCount < 1) {
			...
			NDASSCSI_ASSERT( NDASSCSI_ASSERT_NETWORK_FAIL );
	...
	) 
{
	...
	NDASSCSI_ASSERT( OverlappedData == NULL );
	...
	) 
{
	...
	return LSS->LanscsiProtocol->LsprotoFunc.LspRequest( LSS,
														 PduDesc,
														 PduResponse,
														 LSS2,
														 PduDesc2,
