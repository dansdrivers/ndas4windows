...
--*/
{
	...
	RtlInitUnicodeString( &fltMgrUnicode, L"\\REGISTRY\\MACHINE\\SYSTEM\\ControlSet001\\Services\\FltMgr" );
    ...
    if (!NT_SUCCESS(status)) {
		...
		ZwClose( fltMgrRegKey );
	}

#if __NDAS_FS_MINI__
	
	NDASFS_ASSERT( fltMgrRegKey );

#else

	if (fltMgrRegKey != NULL) {
...
--*/
{
	...
	NDASFS_ASSERT( oldIrql2 == KeGetCurrentIrql() );
...
do {
	...
	if (status == STATUS_VOLUME_MOUNTED) {
		...
		NDASFS_ASSERT( GlobalLfs.NdasFsMiniMode == TRUE );
		NDASFS_ASSERT( netdiskEnableMode != NETDISK_READ_ONLY );

		break;
	}

	NDASFS_ASSERT( GlobalLfs.NdasFsMiniMode == FALSE || netdiskEnableMode == NETDISK_READ_ONLY );
...
--*/
{
	...
	if (devExt == NULL) {
		...
		NDASFS_ASSERT( DeviceObject == gControlDeviceObject );
		return FALSE;		
	}
