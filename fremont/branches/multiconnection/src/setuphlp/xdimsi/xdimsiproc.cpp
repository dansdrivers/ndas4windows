...
{
	pxMsiTraceW(hInstall, L"pxMsiInitializeScheduledAction started...\n"); 

	pxMsiClearForceReboot();
	pxMsiClearScheduleReboot();

	pxMsiTraceW(hInstall, L"pxMsiInitializeScheduledAction finished...\n"); 
...
pxDiMsiInstallLegacyPnpDeviceRollback(
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	pxMsiTraceW(hInstall, L"Rollback in pxMsiInstallLegacyPnpDeviceRollback\n");

	LPWSTR oemInfPath = NULL;

	HRESULT hr = pxDiMsiGetInfPathFromRegistry(
		hInstall,
	...
	if (FAILED(hr))
	{
		pxMsiTraceW(hInstall, L"pxMsiGetInfPathFromRegistry failed, hr=0x%x\n", hr);
		//
		// OEM INF is not copied yet. No further processing is required.
		//
		pxMsiTraceW(hInstall, L"No actions are necessary for rollback, hardwareId=%ls\n",
			ProcessRecord->HardwareId);
		return ERROR_SUCCESS;
	}

	hr = pxDiMsiDeleteInfPathFromRegistry(
		hInstall, 		
		ProcessRecord->RegRoot,
		ProcessRecord->RegKey,
		ProcessRecord->RegName);

	pxMsiTraceW(hInstall, L"DeleteInfPathFromRegistry, hr=0x%x\n", hr);
	...
	pxMsiTraceW(hInstall, L"DiRemoveDevices, hardwareId=%ls, hr=0x%x\n", 
		ProcessRecord->HardwareId, hr);

	hr = xDiDeletePnpDriverServices(serviceList);

	pxMsiTraceW(hInstall, L"DiDeletePnpDriverServices, serviceList=%ls, hr=0x%x\n", 
		serviceList, hr);

	hr = xDiDeleteDriverFiles(NULL, oemInfPath, XDI_EDF_NO_CLASS_INSTALLER);

	pxMsiTraceW(hInstall, L"DiDeleteDriverFiles, inf=%ls, hr=0x%x\n", 
		oemInfPath, hr);

	hr = xDiUninstallOEMInf(oemInfPath, 0);

	pxMsiTraceW(hInstall, L"DiUninstallOEMInf, inf=%ls, hr=0x%x\n", 
		oemInfPath, hr);
...
pxDiMsiInstallNetworkComponentRollback(
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	pxMsiTraceW(hInstall, L"Rollback in pxMsiInstallNetworkComponentRollback\n");
	...
	pxMsiTraceW(hInstall, L"DiUninstallNetComponent, componentId=%ls, hr=0x%x\n", 
		ProcessRecord->HardwareId, hr);
...
pxDiMsiInstallPnpDeviceInfRollback(
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	pxMsiTraceW(hInstall, L"Rollback in pxMsiInstallPnpDeviceInfRollback\n");
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	...
	{
		pxMsiTraceW(hInstall, L"Out of memory for %d bytes\n", len * sizeof(WCHAR));
		return ERROR_INSTALL_FAILURE;
	}
	...
	if (FAILED(hr))
	{
		pxMsiTraceW(hInstall, L"xDiUninstallHardwareOEMInf failed, hr=0x%x\n", hr);
	}
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	...
	pxMsiTraceW(hInstall, L"Inspecting services in INF file, inf=%ls, section=%ls, flags=%x\n", 
	...
	else
	{
		...
		pxMsiTraceW(hInstall, L"xDiServiceIsMarkedForDeletionFromInfSection: Section=%ls\n", 
			section);
		...
		pxMsiTraceW(hInstall, L"xDiServiceIsMarkedForDeletionFromInfSection: hr=0x%x\n", hr);
	}

	if (S_OK == hr)
	{
		pxMsiTraceW(hInstall, L"A service is marked for deletion, InfPath=%s, Service=%s\n", 
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	...
	{
		pxMsiTraceW(hInstall, L"XDIMSI_STARTSERVICE: Starting service skipped (ForceReboot is queued)\n");
		return ERROR_SUCCESS;
	}
	if (pxMsiIsScheduleRebootQueued(hInstall))
	{
		pxMsiTraceW(hInstall, L"XDIMSI_STARTSERVICE: Starting service skipped (ScheduleReboot is queued)\n");
		return ERROR_SUCCESS;
	}

	pxMsiTraceW(hInstall, L"XDIMSI_STARTSERVICE: Starting service %ls\n", ProcessRecord->ServiceName);

	HRESULT hr = pxDiStartStopService(ProcessRecord->ServiceName, TRUE);

	pxMsiTraceW(hInstall, L"XDIMSI_STARTSERVICE: Starting service returned, hr=0x%x\n", hr);
	...
	if (FAILED(hr))
	{
		...
		{
			pxMsiTraceW(hInstall, L"XDIMSI_STARTSERVICE: Ignoring failure\n");
		}
		else
		{
			pxMsiTraceW(hInstall, L"XDIMSI_STARTSERVICE: Queue ScheduleReboot on failure\n");
			pxMsiQueueScheduleReboot(hInstall);
		}
	}
...
pxDiMsiStopService(
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	pxMsiTraceW(hInstall, L"XDIMSI_STOPSERVICE: Stopping service %ls\n", ProcessRecord->ServiceName);

	HRESULT hr = pxDiStartStopService(ProcessRecord->ServiceName, FALSE);

	pxMsiTraceW(hInstall, L"XDIMSI_STOPSERVICE: Stopping service returned, hr=0x%x\n", hr);
...
pxDiMsiQueueScheduleReboot(
	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	pxMsiTraceW(hInstall, L"XDIMSI_QUEUESCHEDULEREBOOT: Queue ScheduleReboot.\n");
	pxMsiQueueScheduleReboot(hInstall);
	return ERROR_SUCCESS;
}
	...
	__deref_out LPWSTR* OemInfPath)
{
	*OemInfPath = NULL;
	...
	if (ERROR_SUCCESS != result)
	{
		pxMsiTraceW(hInstall, L"RegQueryValueExW failed, valueName=%ls, hr=0x%x\n",
	...
	__in LPCWSTR RegName)
{
	...
	if (ERROR_SUCCESS != result)
	{
		pxMsiTraceW(hInstall, L"RegCreateKey failed, root=%p, key=%ls, hr=0x%x\n",
			rootKeyHandle, RegKey, HRESULT_FROM_WIN32(result));
	...
	if (ERROR_SUCCESS != result)
	{
		pxMsiTraceW(hInstall, L"RegSetValueExW failed, name=%ls, hr=0x%x\n",
			RegName, HRESULT_FROM_WIN32(result));
	}
	...
	do 
	{
		...
		do
		{
			...
			if (hWndNext != NULL)
			{
				...
				pxTraceW(L"\tWindowHandle=%p, ProcessId=0x%x, ThreadId=0x%x\n", hWndNext, processId, threadId);

				if (GetCurrentProcessId() == processId)
				{
					pxTraceW(L"\tActivating Window, Handle=%p\n", hWndNext);
