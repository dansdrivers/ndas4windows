...
typedef struct _MINI_LOG_RECORD {
...
} MINI_LOG_RECORD, *PMINI_LOG_RECORD;

//
//  How the mini-filter manages the log records.
//

typedef struct _MINI_RECORD_LIST {
    ...
    MINI_LOG_RECORD LogRecord;  

} MINI_RECORD_LIST, *PMINI_RECORD_LIST;
...
#define MINI_MAX_NAME_SPACE ROUND_TO_SIZE( (MINI_RECORD_SIZE - sizeof(MINI_RECORD_LIST)), sizeof( PVOID ))
...
#define MINI_REMAINING_NAME_SPACE(LogRecord) \
    (ASSERT((LogRecord)->Length >= sizeof(MINI_LOG_RECORD)), \
     (USHORT)(MINI_MAX_NAME_SPACE - ((LogRecord)->Length - sizeof(MINI_LOG_RECORD))))

#define MAX_LOG_RECORD_LENGTH  (MINI_RECORD_SIZE - FIELD_OFFSET( MINI_RECORD_LIST, LogRecord ))
