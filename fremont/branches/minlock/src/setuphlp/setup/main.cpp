	...
	DWORD cchBuffer)
{
	...
	SHLWAPI::PathRenameExtension(Buffer, _T(".inf"));
	return TRUE;
}
...
{
	// read the configuration from setup.ini
	CSetupConfiguration configuration;
	configuration.LoadFromCommandLine(argc, argv);
	if (!configuration.LoadFromFile(iniFileName))
	{
		CString msg; 
		msg.Format(IDS_ERR_INVALID_CONFIG, iniFileName);
	...
	if (configuration.SupportedLanguages.GetSize() > 1 && 
		configuration.MUIPrompt)
	{
		CLanguageSelectionDlg langDlg;
		langDlg.SetLanguages(configuration.SupportedLanguages);
		langDlg.SetSelectedLangID(configuration.CurrentLangID);
		...
		configuration.CurrentLangID = langDlg.GetSelectedLangID();
	}

	configuration.MinimumMsiVersion
	return 0;
...
int __cdecl _tmain(int argc, TCHAR** argv)
{
	...
	//
	// This setup bootstrapper requires Windows 2000 or higher
	//
	...
	CSetupLauncher launcher;
	int nRet = launcher.Run(argc, argv);
