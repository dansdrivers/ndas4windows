...
template <typename T>
class CXTaskDialogCallbackImpl
{
...
protected:
	...
	{
		T* pThis = reinterpret_cast<T*>(lpRefData);
	    ...
	    case TDN_BUTTON_CLICKED:
			{
				bRet = pThis->OnButtonClicked(static_cast<int>(wParam));
				break;
			}
	    ...
	    case TDN_TIMER:
			{
				bRet = pThis->OnTimer(static_cast<DWORD>(wParam));
				break;
			}
	    case TDN_NAVIGATED:
			{
	    ...
	    default:
			{
				ATLTRACE("Unknown notification received in CXTaskDialog::TaskDialogCallback\n");
				break;
			}
		}

		return static_cast<HRESULT>(bRet);
	}
...
template <typename T>
class CXTASKDIALOGCLIENT_EXT_CLASS CXTaskDialogClientImpl
{
public:
//Constructors / Destructors
	CXTaskDialogClientImpl(HWND hWndParent = NULL) : m_hWnd(NULL)
	{
		ZeroMemory(&m_tc, sizeof(TASKDIALOGCONFIG));
