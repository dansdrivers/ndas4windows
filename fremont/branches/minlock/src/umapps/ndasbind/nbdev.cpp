...
CNBNdasDevice::CNBNdasDevice(PNDASUSER_DEVICE_ENUM_ENTRY pBaseInfo, NDAS_DEVICE_STATUS status)
{
	...
	ATLTRACE(_T("new CNBNdasDevice(%p) : Name %s, ID %s, Slot %d, Access %08x Status(%x)\n"),
		this, pBaseInfo->szDeviceName, pBaseInfo->szDeviceStringId, pBaseInfo->SlotNo, pBaseInfo->GrantedAccess, m_status);
}
...
{
	return (NDAS_DEVICE_STATUS_CONNECTED == m_status ||
	...
	DWORD RaidFlags)
{
	...
	if(NDAS_UNITDEVICE_TYPE_UNKNOWN == UnitDeviceType)
	{
		...
		if(NdasCommGetUnitDeviceHardwareInfo(hNDAS, &UnitInfo))
		{
			...
			case NDAS_UNIT_ATAPI_CDROM_DEVICE:
				BaseInfo.UnitDeviceType = NDAS_UNITDEVICE_TYPE_CDROM;
				break;
			case NDAS_UNIT_ATAPI_DIRECT_ACCESS_DEVICE:
				BaseInfo.UnitDeviceType = NDAS_UNITDEVICE_TYPE_COMPACT_BLOCK;
				break;
			case NDAS_UNIT_ATAPI_OPTICAL_MEMORY_DEVICE:
				BaseInfo.UnitDeviceType = NDAS_UNITDEVICE_TYPE_OPTICAL_MEMORY;
				break;
	...
	// already exist
	if(m_mapUnitDevices.count(BaseInfo.UnitNo))
	{
		delete m_mapUnitDevices[BaseInfo.UnitNo];
		m_mapUnitDevices[BaseInfo.UnitNo] = NULL;
	}
	...
	m_mapUnitDevices[BaseInfo.UnitNo] = pUnitDevice;
...
BOOL CNBNdasDevice::InitConnectionInfo(PNDASCOMM_CONNECTION_INFO ci, BOOL bWriteAccess, DWORD UnitNo)
{
	// check granted access
	if(!(GENERIC_WRITE & m_BaseInfo.GrantedAccess) && bWriteAccess)
	{
		return FALSE;
	}
...
BOOL CNBUnitDevice::InitConnectionInfo(PNDASCOMM_CONNECTION_INFO ci, BOOL bWriteAccess)
{
	...
	return m_pDevice->InitConnectionInfo(ci, bWriteAccess, m_BaseInfo.UnitNo);
}
...
BOOL CNBUnitDevice::IsEqual(BYTE Node[6], BYTE Vid, DWORD UnitNo)
{
	return !memcmp(
...
void CNBUnitDevice::SetNotInitialized()
{
	m_pLogicalDevice = NULL;
	m_bInitialized = FALSE;
}
...
{
	return (GetOperatableCount() == DevicesTotal());
}
...
BOOL CNBLogicalDevice::IsFixRaidStateRequired()
{
	...
	if(GetOperatableCount() < DevicesTotal() -1)
	{
