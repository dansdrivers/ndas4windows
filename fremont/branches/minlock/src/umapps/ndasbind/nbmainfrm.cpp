...
/*
* Creates unit devices and adds them to pNBNdasDevice.
*/
unsigned
CMainFrame::ThreadCreateNdasDevice(
	CNBNdasDevice *pNBNdasDevice)
{
	ATLASSERT(NULL != pNBNdasDevice);
	...
	*/
	NDAS_DEVICE_STAT devicestat;
	{
		...
		pNBNdasDevice->InitConnectionInfo(&ci, FALSE);
	...
	*/
	...
	for(DWORD unit = 0; unit < devicestat.NumberOfUnitDevices; unit++)
	{
		pUnitDevice = pNBNdasDevice->AddUnitDevice(unit);
		if(NULL == pUnitDevice)
		{
			uReturn = 2;
			goto out;
...
/*
* Call CNBUnitDevice::Initialize() of each unit device
* Because this take a good time, I made this threaded.
*/
unsigned
CMainFrame::ThreadInitializeNdasDevice(
	CNBNdasDevice *pNBNdasDevice)
{
	ATLASSERT(NULL != pNBNdasDevice);
	/*
	* Call CNBUnitDevice::Initialize() of each unit device
	*/
	for(UINT32 i = 0; i < pNBNdasDevice->UnitDevicesCount(); i++)
	{
		if ((*pNBNdasDevice)[i]) {
			(*pNBNdasDevice)[i]->Initialize();
		} else {
			ATLTRACE(_T("Reported as 2 units device but there may be only one unit."));
		}
	}
...
/*
* Launch CMainFrame::_ThreadInitializeNdasDevice for each CNBNdasDevice
*/
struct NdasDeviceInitializer :
	public std::unary_function<CNBNdasDevice *, void>
{
private:
	CMainFrame *m_pMainFrame;
public:

	NdasDeviceInitializer(CMainFrame *pMainFrame) 
		: m_pMainFrame(pMainFrame) {}

	void operator() (CNBNdasDevice *pNBNdasDevice) const
	{
		ATLASSERT(NULL != m_pMainFrame);
		ATLASSERT(NULL != pNBNdasDevice);

		MAINFRAME_CONTEXT *pThreadContext =
			(MAINFRAME_CONTEXT *)malloc(sizeof(MAINFRAME_CONTEXT));
		...
		pThreadContext->pMainFrame = m_pMainFrame;
		pThreadContext->pParameter = (void *)pNBNdasDevice;

		HANDLE hThread = (HANDLE)_beginthreadex(
			NULL,
			0,
			CMainFrame::_ThreadInitializeNdasDevice,
			pThreadContext,
			0,
			NULL);
		if(!hThread)
...
/*
* Used to count NDAS devices in service.
*/
BOOL 
CALLBACK 
CMainFrame::EnumDevicesCountCallBack(
	PNDASUSER_DEVICE_ENUM_ENTRY lpEnumEntry, 
	LPVOID lpContext)
{
	MAINFRAME_CONTEXT *context = (MAINFRAME_CONTEXT *)lpContext;
	CMainFrame *pMainFrame = context->pMainFrame;
	UINT32 *pnNdasDevices = (UINT32 *)context->pParameter;

	ATLASSERT(NULL != pMainFrame);
	ATLASSERT(NULL != pnNdasDevices);
...
/*
* Create CNBNdasDevice and CNBUnitDevice
*/
BOOL 
CALLBACK 
	...
	LPVOID lpContext)
{
	MAINFRAME_CONTEXT *context = (MAINFRAME_CONTEXT *)lpContext;
	CMainFrame *pMainFrame = context->pMainFrame;
	NBNdasDevicePtrList *plistDevices = (NBNdasDevicePtrList *)context->pParameter;
	HANDLE hThread = NULL;

	ATLASSERT(pMainFrame);
	ATLASSERT(plistDevices);
	...
	NDAS_DEVICE_STATUS status;
	{
		...
		if(!fSuccess)
		{
			ATLTRACE(_T("NdasQueryDeviceStatus() Failed\n"));
			goto out;
		}
	}

	/*
	* Create CNBNdasDevice and push it into list
	*/
	CNBNdasDevice *pNBNdasDevice = new CNBNdasDevice(lpEnumEntry, status);
	if(!pNBNdasDevice)
	{
		ATLTRACE(_T("new CNBNdasDevice() Failed\n"));
		goto out;
	}
	...
	*/
	if(!pNBNdasDevice->IsAlive())
	{
		// should not connect
		pNBNdasDevice->AddUnitDevice(0, NDAS_UNITDEVICE_TYPE_DISK);
		goto out;
	}
	...
	*/
	...
	pThreadContext->pMainFrame = pMainFrame;
	pThreadContext->pParameter = (void *)pNBNdasDevice;

	hThread = (HANDLE)_beginthreadex(
		NULL,
		0,
		CMainFrame::_ThreadCreateNdasDevice,
		pThreadContext,
		0,
		NULL);
	if(!hThread)
	{
		ATLTRACE(_T("_beginthreadex(CMainFrame::_ThreadCreateNdasDevice) Failed\n"));
		free(pThreadContext);
	}
...
out:
	if(NULL == hThread)
	{
		...
		InterlockedDecrement(&pMainFrame->m_nAssigned);
		SetEvent(pMainFrame->m_hEventThread);
	}
	else
	{
	...
	// always return TRUE or there might be a thread comflict
	return TRUE;
}

/*
* empty m_listLogicalDevices,	m_listDevices, m_listMissingDevices
*/
void 
...
{
	if(pDevice->IsCommandAvailable(wID))
		return TRUE;
...
{
#ifdef _DEBUG
	static BOOL bPauseFirstRefresh = TRUE;
	if(bPauseFirstRefresh)
	{
		MessageBox(
			_T("Paused for checked version : Click OK to start refresh."),
			_T("Paused"),
			MB_OK);
		bPauseFirstRefresh = FALSE;
	}
#endif
...
//	UIEnableForDevice(NULL, IDM_TOOL_SPAREREMOVE);
	...
	*/
	...
	ClearDevices();
	MAINFRAME_CONTEXT context;
	context.pMainFrame = this;

	// get NDAS device count
	context.pParameter = (void *)&m_nAssigned;
	if(!NdasEnumDevices( EnumDevicesCountCallBack, &context))
		return FALSE;

	// set progress bar (m_nAssigned ~ 0)
	m_wndRefreshProgress.SetRange32(0, m_nAssigned);
	m_wndRefreshProgress.SetStep(1);
	m_wndRefreshProgress.SetPos(m_nAssigned);

	// retrieve all the device & unit device information
	context.pParameter = reinterpret_cast<LPVOID>(&m_listDevices);
	...
	// set progress bar (0 ~ m_listDevices.size())
	m_wndRefreshProgress.SetRange32(0, m_listDevices.size());
	m_wndRefreshProgress.SetStep(1);
	m_wndRefreshProgress.SetPos(0);

	ResetEvent(m_hEventThread);
	m_nAssigned = m_listDevices.size();
	std::for_each(
		m_listDevices.begin(),
		m_listDevices.end(),
		NdasDeviceInitializer(this));
	while(1)
	{
		if(WAIT_OBJECT_0 != WaitForSingleObject(m_hEventThread, INFINITE))
			return FALSE;
		m_wndRefreshProgress.SetPos(m_listDevices.size() - m_nAssigned);
		if(0 == m_nAssigned)
			break;
	}
	...
	/*
	* Step 2. create (grouped) logical devices
	*/
	CNBNdasDevice *pNBNdasDevice;
	CNBUnitDevice *pUnitDevice;
	CNBNdasDevice *pNdasDevice;	
	CNBLogicalDevice *pLogicalDevice;
	NBNdasDevicePtrList::iterator itNdasDevice;
	NBLogicalDevicePtrList::iterator itLogicalDevice;
	NBUnitDevicePtrList listUnusedUnits;
	NBUnitDevicePtrList::iterator itUnitDevice;

	// for each NDAS device
	for(itNdasDevice = m_listDevices.begin();
		itNdasDevice != m_listDevices.end();
		itNdasDevice++)
	{
		pNBNdasDevice = *itNdasDevice;

		// for each unit device
		for(UINT32 i = 0; i < pNBNdasDevice->UnitDevicesCount(); i++)
		{
			pLogicalDevice = NULL;
			pUnitDevice = (*pNBNdasDevice)[i];
			if (!pUnitDevice)
				continue;

			// find already created logical device of this unit device
			for(itLogicalDevice = m_listLogicalDevices.begin();
				itLogicalDevice != m_listLogicalDevices.end();
				itLogicalDevice++)
			{
				if((*itLogicalDevice)->IsMember(pUnitDevice))
				{
					...
					ATLTRACE(
						_T("use CNBLogicalDevice(%p) : %s\n"),
						pLogicalDevice,
						pUnitDevice->GetName());
					pLogicalDevice->UnitDeviceSet(
						pUnitDevice,
						pUnitDevice->GetSequence());
					break;
				}
			}
			...
			// failed to find matching logical device
			if (pUnitDevice->IsGroup() || // in group but logical device is not created yet
				!( // or because this is disconnected, you can't find matching logical device
				(pUnitDevice->GetStatus() & NDASBIND_UNIT_DEVICE_STATUS_DISABLED) ||
				(pUnitDevice->GetStatus() & NDASBIND_UNIT_DEVICE_STATUS_DISCONNECTED)
				)
				) 
			{
				// add to new logical device
				pLogicalDevice = new CNBLogicalDevice();
				m_listLogicalDevices.push_back(pLogicalDevice);
				ATLTRACE(
					_T("new CNBLogicalDevice(%p) : %s\n"),
					pLogicalDevice,
					pUnitDevice->GetName());
				pLogicalDevice->UnitDeviceSet(
					pUnitDevice,
			...
			{
				// We can't sure that this unit device is single or in group
				// We will use this unit device at step 3
				listUnusedUnits.push_back(pUnitDevice);
			}
		}
	}

	/*
	* Step 3. We created all grouped logical device.
	* Set additional info to unit device and fill empty units of logical devices.
	*/
	for(itLogicalDevice = m_listLogicalDevices.begin();
		itLogicalDevice != m_listLogicalDevices.end();
		itLogicalDevice++)
	{
		pLogicalDevice = *itLogicalDevice;
		NDASOP_RAID_INFO* pRaidInfo;

		// we're looking missing device in group only
		if (!pLogicalDevice->IsGroup())
			continue;
		
		bResult = pLogicalDevice->InitRaidInfo();
		if (!bResult)
		{
			ATLTRACE(
				_T("Failed to get RAID info for %s\n"),
				pLogicalDevice->GetName());
			continue;
		}
		pRaidInfo = pLogicalDevice->RAID_INFO();

		for(UINT32 i =0; i<pLogicalDevice->DevicesTotal();i++)
		{
			pUnitDevice = (*pLogicalDevice)[i];
			if (NULL != pUnitDevice)
			{
				// normal state. just update raid status
				pUnitDevice->SetRaidStatus(
					pLogicalDevice->RAID_INFO()->Members[i].Flags);
				continue;
			}
			...
			*/
			for(itNdasDevice = m_listDevices.begin();
				itNdasDevice != m_listDevices.end(); itNdasDevice++)
			{
				pNdasDevice = (*itNdasDevice);
				if (pNdasDevice->IsEqual(
					pLogicalDevice->RAID_INFO()->Members[i].DeviceId.Node,
					pLogicalDevice->RAID_INFO()->Members[i].DeviceId.VID)) 
				{
					/*
					...
					*/
					pMissingNdasDevice = new CNBNdasDevice(pNdasDevice, FALSE);
					m_listMissingDevices.push_back(pMissingNdasDevice);
					pUnitDevice = pMissingNdasDevice->AddUnitDevice(
						pLogicalDevice->RAID_INFO()->Members[i].UnitNo,
			...
			if (pMissingNdasDevice == NULL)
			{
				...
				*/
				...
				pMissingNdasDevice = new CNBNdasDevice((PCTSTR)str, 
					&pLogicalDevice->RAID_INFO()->Members[i].DeviceId,
					NDAS_DEVICE_STATUS_NOT_REGISTERED);
				m_listMissingDevices.push_back(pMissingNdasDevice);
				pUnitDevice = pMissingNdasDevice->AddUnitDevice(
					pLogicalDevice->RAID_INFO()->Members[i].UnitNo,
					NDAS_UNITDEVICE_TYPE_DISK, 
					pLogicalDevice->RAID_INFO()->Members[i].Flags | NDAS_RAID_MEMBER_FLAG_NOT_REGISTERED);
			...
			pLogicalDevice->UnitDeviceSet(pUnitDevice, i);

			// Remove used unit from allunit list
			for(itUnitDevice = listUnusedUnits.begin();
				itUnitDevice != listUnusedUnits.end();
				itUnitDevice++) 
			{
				pUnitDevice = *itUnitDevice;
				if (pUnitDevice->IsEqual(
					pLogicalDevice->RAID_INFO()->Members[i].DeviceId.Node, 
					pLogicalDevice->RAID_INFO()->Members[i].DeviceId.VID,
					pLogicalDevice->RAID_INFO()->Members[i].UnitNo))
				{
					listUnusedUnits.erase(itUnitDevice);
					break;
				}
			}
		}
	...
	/*
	* Step 4. Create logical device for disabled/not-connected devices.
	*/
	for(itUnitDevice = listUnusedUnits.begin();
		itUnitDevice != listUnusedUnits.end();
		itUnitDevice++)
...
{
	UIEnable(nMenuID, pDevice ? pDevice->IsCommandAvailable(nMenuID) : FALSE);
}
...
//	UIEnableForDevice(pDevice, IDM_TOOL_SINGLE);
	UIEnableForDevice(pDevice, IDM_TOOL_APPEND);
...
//	UIEnableForDevice(pDevice, IDM_TOOL_SPAREREMOVE);
	...
	UIEnableForDevice(pDevice, IDM_TOOL_FIX_RAID_STATE);
...
void CMainFrame::OnAppend(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	(*pLogicalDevice)[0]->InitConnectionInfo(&ConnectionInfo[0], TRUE);
	pUnitDevice->InitConnectionInfo(&ConnectionInfo[1], TRUE);
	...
	if(pLogicalDevice->IsGroup())
	{
		// append a disk to aggregated disks

		bResults = NdasOpAppend(
			&ConnectionInfo[0], &ConnectionInfo[1]);

	}
	else
	{
		// append a disk to single disk -> 2 disk aggregated
...
void CMainFrame::OnReplaceDevice(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	if (pLogicalDevice->GetType() == NMT_RAID1R3) {
		...
		// Unsupported RAID type or not implemented
		// To do: handle RAID4,5 case
		if (pLogicalDevice->GetType() == NMT_RAID4R3 || 
			pLogicalDevice->GetType() == NMT_RAID5) {
			CString strMsg;
			strMsg.LoadString(IDS_ERROR_NOT_IMPLEMENTED);
			CString strTitle;
			strTitle.LoadString(IDS_APPLICATION);
			MessageBox(
				strMsg,
...
void CMainFrame::OnClearDefect(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	...
	//
	// Reject clearing defect if this member is the only healthy node.
	//
	for(i=0;i<pLogicalDevice->DevicesTotal();i++) {
		if (pUnitDevice == (*pLogicalDevice)[i])
			continue;
		...
		if ((UnitRaidStatus & NDAS_RAID_MEMBER_FLAG_ACTIVE) &&
			(UnitRaidStatus & (
				NDAS_RAID_MEMBER_FLAG_OUT_OF_SYNC |
				NDAS_RAID_MEMBER_FLAG_INVALID_MEMBER))) {
			//
			// Healthy disk has broken in degraded mode.
			// We cannot continue.
			//
			CString strMsg;
			strMsg.LoadString(IDS_ERROR_CANNOT_CLEAR_DEFECT);
			CString strTitle;
			strTitle.LoadString(IDS_APPLICATION);
			MessageBox(
				strMsg,
...
void CMainFrame::OnFixRaidState(UINT wNotifyCode, int wID, HWND hwndCtl)
{
	CNBDevice *pDevice = m_viewTreeList.GetSelectedDevice();
	...
	pLogicalDevice->PrimaryUnitDevice()->InitConnectionInfo(&ci, TRUE);

	AutoCursor l_auto_cursor(IDC_WAIT);

	BOOL bResult;
	switch(pLogicalDevice->GetType())
	{
	case NMT_RAID1R3:
		bResult = NdasOpRAID1R3IsFixRequired(&ci, TRUE);
		break;
	case NMT_RAID4R3:
	case NMT_RAID5:
		bResult = NdasOpRAID4R3or5IsFixRequired(&ci, TRUE);
		break;
	default:
		bResult = FALSE;
	}
	...
	if(!bResult)
	{
		ShowErrorMessageBox(IDS_MAINFRAME_FIX_RAID_STATE_FAIL);
	}
