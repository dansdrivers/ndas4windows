	...
	std::unary_function<NDAS_LOCATION,void> 
{
	...
	void operator()(NDAS_LOCATION location) 
	{
		...
		HRESULT hr = pGetNdasLogicalUnitByNdasLocation(location, &pNdasLogicalUnit);
...
CNdasServiceDeviceEventHandler::pOnVolumeArrivalOrRemoval(DWORD UnitMask, BOOL Removal)
{
	for (int index = 0; index < ('Z' - 'A' + 1); ++index)
	{
		...
		if (UnitMask & (1 << index))
		{
			...
			CComPtr<INdasLogicalUnit> pNdasLogicalUnit;
			hr = pGetNdasLogicalUnitByNdasLocation(loc, &pNdasLogicalUnit);
			if (FAILED(hr))
			{
				XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_ERROR, 
	...
	PDEV_BROADCAST_DEVICEINTERFACE pdbhdr)
{
	...
	hr = pGetNdasLogicalUnitByNdasLocation(ndasLocation, &pNdasLogicalUnit);
	...
	PDEV_BROADCAST_HANDLE pdbch)
{
	...
	HRESULT hr = pGetNdasLogicalUnitByNdasLocation(notifyData->NdasLocation, &pNdasLogicalUnit);
	...
	PDEV_BROADCAST_HANDLE pdbch)
{
	...
	case PnpStoragePortHandle:

		{
			...
			HRESULT hr = pGetNdasLogicalUnitByNdasLocation(ndasLocation, &pNdasLogicalUnit);
