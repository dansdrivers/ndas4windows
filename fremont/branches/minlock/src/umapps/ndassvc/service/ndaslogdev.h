...
protected:
	...
	//
	NDAS_LOGICALDEVICE_GROUP m_NdasLogicalUnitDefinition;

	CAtlArray<INdasUnit*> m_NdasUnits;

	CComBSTR m_DevicePath;
...
public:
	...
	STDMETHODIMP get_LogicalUnitDefinition(__out NDAS_LOGICALDEVICE_GROUP* LogicalUnitDefinition);

	STDMETHODIMP get_NdasLocation(__out NDAS_LOCATION* NdasLocation);
...
protected:
	...
	//
	const NDAS_LOGICALDEVICE_GROUP& pGetLogicalUnitConfig() const;
	...
	void pAllocateNdasLocation();
	void pDeallocateNdasLocation();
	...
	//
	void pLocateRegContainer();

	BOOL pIsPSWriteShareCapable();
