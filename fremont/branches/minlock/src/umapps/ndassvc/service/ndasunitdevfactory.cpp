	...
	*ppNdasUnit = NULL;
	...
	else 
	{
		...
		case NDAS_UNIT_ATAPI_OPTICAL_CARD_READER_WRITER_DEVICE:
			{
				NDAS_LOGICALDEVICE_GROUP ludef = {0};
				ludef.nUnitDevices = 1;
				ludef.nUnitDevicesSpare = 0;
				ludef.Type = NDAS_LOGICALDEVICE_TYPE_DVD;
				ludef.UnitDevices[0] = m_ndasUnitId;
	...
	__deref_out INdasUnit** ppNdasUnit)
{
	...
	*ppNdasUnit = NULL;
	...
	NDAS_LOGICALDEVICE_GROUP ludef = {0};
	ludef.Type = pGetNdasLogicalUnitTypeFromDIBv2(pDIBv2);
	ludef.nUnitDevices = pDIBv2->nDiskCount + pDIBv2->nSpareCount;
	ludef.nUnitDevicesSpare = pDIBv2->nSpareCount;
...
//	::CopyMemory(&ldGroup.ConfigSetId, &Rmd.ConfigSetId, sizeof(GUID));
	...
	{
		ludef.DeviceHwVersions[i] = pDIBv2->UnitDiskInfos[i].HwVersion;
	}
	...
	for (DWORD i = 0; i < ludef.nUnitDevices; i++)
	{
		CopyMemory(
			&ludef.UnitDevices[i].DeviceId, 
			pDIBv2->UnitDisks[i].MACAddr, 
			sizeof(NDAS_DEVICE_ID));
		ludef.UnitDevices[i].UnitNo = pDIBv2->UnitDisks[i].UnitNumber;
	}
