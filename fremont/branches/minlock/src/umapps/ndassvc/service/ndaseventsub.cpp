...
{
	DWORD cbTransferred;
	BOOL success = ::GetOverlappedResult(m_hPipe, &m_overlapped,&cbTransferred,FALSE);
	if (!success)
	{
...
CNdasEventSubscriber::BeginWriteMessage(const NDAS_EVENT_MESSAGE& msg)
{
	...
	DWORD cbWritten;
	BOOL success = ::WriteFile(m_hPipe, &msg, cbToWrite, &cbWritten, &m_overlapped);
