	...
	do {
	    ...
	    try {
			...
			if (VolumeDeviceObject->Secondary && 
				(IrpContext->PrimaryRequestInfo.PrimarySession == NULL || 
				...
				while (TRUE) {
					...
					if (!FlagOn(VolumeDeviceObject->Vcb.VcbState, VCB_STATE_FLAG_LOCKED)) {
					...
					try {
						...
						if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION)) {
							...
							for (primarySession = NULL, primarySessionListEntry = VolumeDeviceObject->PrimarySessionQueue.Flink; 
								...
								if (IrpContext->FileOpenPrimarySessionId == (PRIMARY_SESSION_ID)primarySession) {
				...
				ASSERT( VolumeDeviceObject->Secondary == NULL || !FlagOn(VolumeDeviceObject->Secondary->Flags, SECONDARY_FLAG_CLEANUP_VOLUME) );
...

{
...

	{
		...
		if (options & FILE_DELETE_ON_CLOSE	||
			...
			((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->ReceiveWriteCommand = TRUE;
			KeQuerySystemTime( &((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->CommandReceiveTime );
	
			DebugTrace( 0, Dbg, "((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->CommandReceiveTime.QuadPart = %I64d\n", 
				                 ((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->CommandReceiveTime.QuadPart );
		}
	}
	...
	if (!FlagOn(((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {
		...
		if (((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY || 
			...
			Status = NdasFatSecondaryCommonCreate( IrpContext, Irp, &doMore );	
			...
			ASSERT( ((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_PRIMARY || 
					((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_READ_ONLY );
		}
	}
...
#if __NDAS_FAT__
				// original source bug fix.

				if (Fcb == Vcb->RootDcb) {

					ASSERT( Fcb->ParentDcb == NULL );
					try_return( Iosb.Status = STATUS_INVALID_PARAMETER );
				}
#endif
    ...
    try_exit: NOTHING;
		...
		if (Status == STATUS_SUCCESS) {
			...
			try {
				...
				ULONG			writeAceessOpenCount = 0;
				ULONG			writeAceessUncloseCount = 0;
				ULONG			writeAceessUncleanCount = 0;

				PLIST_ENTRY		ccbListEntry;

				TypeOfOpen = FatDecodeFileObject( FileObject, &ThisVcb, &ThisFcb, &ThisCcb );
				...
				if (ThisFcb->Header.AllocationSize.QuadPart == FCB_LOOKUP_ALLOCATIONSIZE_HINT) {

					FatLookupFileAllocationSize( IrpContext, ThisFcb );
				}
				...
				ExAcquireFastMutex( &ThisFcb->NonPaged->CcbQMutex );

				for (ccbListEntry = ThisFcb->NonPaged->CcbQueue.Flink; 
					 ccbListEntry != &ThisFcb->NonPaged->CcbQueue;
					 ccbListEntry = ccbListEntry->Flink) {

					 PCCB ccb = CONTAINING_RECORD( ccbListEntry, CCB, FcbListEntry );
					...
					writeAceessUncleanCount ++;
				}
				 
				ExReleaseFastMutex( &ThisFcb->NonPaged->CcbQMutex );

				if (writeAceessOpenCount != 1) {
					
					DebugTrace2( 0, Dbg, ("ThisFcb->PrimarySession = I64%x, IrpContext->PrimaryRequestInfo.PrimarySession = %p\n",
										  ThisFcb->PrimarySessionId, IrpContext->PrimaryRequestInfo.PrimarySession) );

					DebugTrace2( 0, Dbg, ("openTryCount = %d, Scb->CleanupCount = %d,"
										  "Scb->Close = %d, FlagOn(Scb->ScbState, SCB_STATE_DELAY_CLOSE) = %d\n", 
										   IrpContext->OpenTryCount, ThisFcb->UncleanCount, ThisFcb->OpenCount, 
										   FlagOn(ThisFcb->FcbState, FCB_STATE_DELAY_CLOSE)) );
				}

				if (writeAceessOpenCount != 1 &&
					ThisFcb->PrimarySessionId != (PRIMARY_SESSION_ID)IrpContext->PrimaryRequestInfo.PrimarySession &&
					!(ThisFcb->PrimarySessionId == NDASFAT_LOCAL_PRMARY_SESSION_ID && IrpContext->PrimaryRequestInfo.PrimarySession == NULL)) {

					if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW)) {

						NDAS_ASSERT( writeAceessOpenCount > 1 && writeAceessUncleanCount >= 1 );
					}

					if (writeAceessUncleanCount != 1) {
					
						SetFlag( IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION );
						IrpContext->FileOpenPrimarySessionId = ThisFcb->PrimarySessionId;
					...
					if (writeAceessUncloseCount != 1) {
						...
						if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) && 
							ThisFcb->PrimarySessionId != NDASFAT_LOCAL_PRMARY_SESSION_ID) {
						...
						if (IrpContext->OpenTryCount++ < IrpContext->MaxOpenTry) {
							...
							InterlockedIncrement( &ThisFcb->OpenCount );

							(VOID)FatFlushFile( IrpContext, ThisFcb, Flush );

							dataSectionExists = (BOOLEAN)(ThisFcb->NonPaged->SectionObjectPointers.DataSectionObject != NULL);
							imageSectionExists = (BOOLEAN)(ThisFcb->NonPaged->SectionObjectPointers.ImageSectionObject != NULL);

							if (imageSectionExists) {

								(VOID)MmFlushImageSection( &ThisFcb->NonPaged->SectionObjectPointers, MmFlushForWrite );
							}

							if (dataSectionExists) {
								
								CcPurgeCacheSection( &ThisFcb->NonPaged->SectionObjectPointers, NULL, 0, FALSE );
							}

							InterlockedDecrement( &ThisFcb->OpenCount );
				...
				NDAS_ASSERT( writeAceessOpenCount == 1																									||
							 writeAceessUncloseCount == 1																								||
							 ThisFcb->PrimarySessionId == (PRIMARY_SESSION_ID)IrpContext->PrimaryRequestInfo.PrimarySession								|| 
							 (ThisFcb->PrimarySessionId == NDASFAT_LOCAL_PRMARY_SESSION_ID && IrpContext->PrimaryRequestInfo.PrimarySession == NULL) );

	
				ThisCcb->PrimarySession = IrpContext->PrimaryRequestInfo.PrimarySession;
				ThisCcb->DesiredAccess = *DesiredAccess;

				ThisFcb->PrimarySessionId = (PRIMARY_SESSION_ID)IrpContext->PrimaryRequestInfo.PrimarySession;
			
				if (ThisFcb->PrimarySessionId != NDASFAT_LOCAL_PRMARY_SESSION_ID) {

					SetFlag( ThisCcb->NdasFatFlags, ND_FAT_CCB_FLAG_OPEN_BY_PRIMARY_SESSION );
					InterlockedIncrement( &Vcb->PrimaryOpenFileCount );					
				}
				...
				NDAS_ASSERT( IrpContext->PrimaryRequestInfo.PrimarySession );

				TypeOfOpen = FatDecodeFileObject( FileObject, &ThisVcb, &ThisFcb, &ThisCcb );

				if (TypeOfOpen == UserFileOpen) {

					ULONG		unclosedCcbCount = 0;
					PLIST_ENTRY	ccbListEntry;

					if (ThisFcb->UncleanCount != 1) {
					
						DebugTrace2( 0, Dbg, ("ThisFcb->PrimarySession = %x, IrpContext->PrimaryRequestInfo.PrimarySession = %p\n",
										  ThisFcb->PrimarySession, IrpContext->PrimaryRequestInfo.PrimarySession) );
					...
					if (ThisFcb->OpenCount != 1 &&
						...
						if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW)) {
							...
							NDAS_ASSERT( ThisFcb->OpenCount > 1 && ThisFcb->UncleanCount >= 1 );
						}
						...
						//ASSERT( ExIsResourceAcquiredSharedLite(ThisFcb->Header.Resource) );	

						//NtfsLockFcb( NULL, ThisFcb->Fcb );
						ExAcquireFastMutex( &ThisFcb->NonPaged->CcbQMutex );

						for (unclosedCcbCount = 0, ccbListEntry = ThisFcb->NonPaged->CcbQueue.Flink; 
							 ccbListEntry != &ThisFcb->NonPaged->CcbQueue;
							 ccbListEntry = ccbListEntry->Flink) {
						...
						//NtfsUnlockFcb( NULL, ThisFcb->Fcb );
						ExReleaseFastMutex( &ThisFcb->NonPaged->CcbQMutex );
					
						if (unclosedCcbCount != 1) {
							...
							NDAS_ASSERT( unclosedCcbCount > 1 );
							...
							if (IrpContext->OpenTryCount++ < IrpContext->MaxOpenTry) {
								...
								(VOID)FatFlushFile( IrpContext, ThisFcb, Flush );
								//FlushScb( IrpContext, ThisFcb, &ioStatus );
					...
					NDAS_ASSERT( ThisFcb->OpenCount == 1																									||
								 unclosedCcbCount == 1																										||
								 ThisFcb->PrimarySessionId == (PRIMARY_SESSION_ID)IrpContext->PrimaryRequestInfo.PrimarySession								|| 
								 (ThisFcb->PrimarySessionId == NDASFAT_LOCAL_PRMARY_SESSION_ID && IrpContext->PrimaryRequestInfo.PrimarySession == NULL) );
    ...
    try {
        ...
        if (!FlagOn(ShareAccess, FILE_SHARE_WRITE) &&
            ...
            if (FlagOn( *DesiredAccess, (FILE_WRITE_DATA | FILE_APPEND_DATA) ) &&
                (NdasFatFsRtlAreVolumeStartupApplicationsComplete && NdasFatFsRtlAreVolumeStartupApplicationsComplete())) {
#else
            if (FlagOn( *DesiredAccess, (FILE_WRITE_DATA | FILE_APPEND_DATA) ) &&
    ...
    try {
        ...
        *OplockPostIrp = FALSE;
...
#if __NDAS_FAT__
        if (!FlagOn( ShareAccess, FILE_SHARE_WRITE ) &&
            (NdasFatMmDoesFileHaveUserWritableReferences && NdasFatMmDoesFileHaveUserWritableReferences( &Fcb->NonPaged->SectionObjectPointers ))) {
#else
        if (!FlagOn( ShareAccess, FILE_SHARE_WRITE ) &&
        ...
        if ((CreateDisposition == FILE_OPEN) ||
			...
			ExAcquireFastMutex(&Fcb->NonPaged->CcbQMutex);
			UnwindCcb->FileObject = FileObject;
			UnwindCcb->Fcb = Fcb;
			InsertTailList( &Fcb->NonPaged->CcbQueue, &UnwindCcb->FcbListEntry );
			ExReleaseFastMutex(&Fcb->NonPaged->CcbQMutex);
			FatSetFileObject( FileObject,
                              UserFileOpen,
                              Fcb,
    ...
    try_exit: NOTHING;
        ...
        if (AbnormalTermination()) {
			...
			if (UnwindCcb != NULL) {
				...
				ExAcquireFastMutex( &Fcb->NonPaged->CcbQMutex );
				RemoveEntryList( &UnwindCcb->FcbListEntry );
				UnwindCcb->FileObject = NULL;
				UnwindCcb->Fcb = NULL;
				InitializeListHead( &UnwindCcb->FcbListEntry );
				ExReleaseFastMutex( &Fcb->NonPaged->CcbQMutex );
			}
