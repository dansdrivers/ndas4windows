...
LfsAllocDGPkt(
	...
	*ppkt = ExAllocatePoolWithTag(
	...
	rawHead->NdfsMajorVersion2		= HTONS(LFS_DATAGRAM_MAJVER);
	rawHead->NdfsMinorVersion2		= HTONS(LFS_DATAGRAM_MINVER);
...
LfsSendDatagram(
	...
	for(idx_addr = 0 ; idx_addr < socket->SocketCnt ; idx_addr ++ ) {
		...
		ntStatus = LpxTdiV2SendDatagram( socket->Sockets[idx_addr].AddressFile,
										 LpxRemoteAddress,
