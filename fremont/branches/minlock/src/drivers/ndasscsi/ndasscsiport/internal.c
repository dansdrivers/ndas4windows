...

{
	...
	//DbgPrint( "%s srbControl->TimeOutValue = %d, srbData = %p\n", __FUNCTION__, srb->TimeOutValue, srbData );
...
#if DBG==1
            DebugPrint((0,"ScsiPortStartIo: Invalid QueueAction (%02x) SRB:%p irp:%p\n", 
...

{
    ...
    if (TEST_FLAG(LogicalUnit->LuFlags, (LU_QUEUE_FROZEN |
	 ...
	 if (NdasScsiKeRemoveByKeyDeviceQueueIfBusy) {

	     packet = NdasScsiKeRemoveByKeyDeviceQueueIfBusy(
		             &LogicalUnit->CommonExtension.DeviceObject->DeviceQueue,
			         LogicalUnit->CurrentKey);
         ...
         if ((DeviceExtension->MapBuffers) || (SrbData->RemappedMdl)) {
             ...
             if(SrbData->RemappedMdl) {
                 ...
                 if (TEST_FLAG(SrbData->Flags, SRB_DATA_RESERVED_PAGES)) {
                     ...
                     remappedAddress = MmGetSystemAddressForMdl(SrbData->RemappedMdl);
...

{
    ...
    if (DeviceExtension->AllocateSrbExtension) {
        ...
        Srb->SrbExtension = (remappedSrbExt != NULL) ? remappedSrbExt : srbExtension;
        ...
        if ((Srb->Function != SRB_FUNCTION_WMI) &&
            ...
            if (Srb->SenseInfoBufferLength > 
                ...
                //
                // Modify the size of the sense buffer to reflect the size of the
                // one we redirect to.
                //
...

{
	...
	DbgPrint( "%s srbControl->TimeOutValue = %d\n", __FUNCTION__, srbControl->TimeOutValue );
    ...
    )
{
    ...
    if(Flush) {
        ...
        while ((packet =
                (NdasScsiKeRemoveByKeyDeviceQueueIfBusy ? 
				 NdasScsiKeRemoveByKeyDeviceQueueIfBusy( &(LogicalUnit->DeviceObject->DeviceQueue), 0) :
				 KeRemoveByKeyDeviceQueue( &(LogicalUnit->DeviceObject->DeviceQueue), 0)))
            != NULL) {
