	...
	)
{
	...
	while (secondaryThreadTerminate == FALSE) {
		...
		timeOut.QuadPart = -NDASNTFS_SECONDARY_THREAD_FLAG_TIME_OUT;
		...
		if (eventStatus == STATUS_TIMEOUT) {
			...
			if (Secondary->TryCloseTime.QuadPart > currentTime.QuadPart || 
				(currentTime.QuadPart - Secondary->TryCloseTime.QuadPart) >= NDASNTFS_TRY_CLOSE_DURATION) {

				ExAcquireFastMutex( &Secondary->FastMutex );		
				
				if (!Secondary->TryCloseActive && Secondary->VolDo->Vcb.SecondaryCloseCount) {
		...
		if (eventStatus == 0) {
			...
			secondaryRequest = Secondary->Thread.SessionSlot[HTONS(Secondary->Thread.NdfsReplyHeader.Mid2)];
			Secondary->Thread.SessionSlot[HTONS(Secondary->Thread.NdfsReplyHeader.Mid2)] = NULL;
	...
	do { // just for structural programing
		...
		if(NTOHL(ndfsReplyNegotiate->MaxBufferSize4)) {
			...
			Secondary->Thread.SessionContext.PrimaryMaxDataSize
				= (NTOHL(ndfsReplyNegotiate->MaxBufferSize4) <= DEFAULT_NDAS_MAX_DATA_SIZE)
					? NTOHL(ndfsReplyNegotiate->MaxBufferSize4) : DEFAULT_NDAS_MAX_DATA_SIZE;
		...
		ndfsRequestSetup->NetDiskPort2 = HTONS(Secondary->VolDo->NetdiskPartitionInformation.NetdiskInformation.NetDiskAddress.Port);
		ndfsRequestSetup->UnitDiskNo2 = HTONS(Secondary->VolDo->NetdiskPartitionInformation.NetdiskInformation.UnitDiskNo);
...
SEND_TREE_CONNECT_COMMAND:
		...
		ndfsRequestTreeConnect->StartingOffset8 = HTONLL(Secondary->VolDo->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset.QuadPart);
		...
		Secondary->Thread.SessionContext.BytesPerCluster = NTOHL(ndfsReplyTreeConnect->BytesPerCluster4);
		...
		DebugTrace( 0, Dbg, ("NDFS_COMMAND_TREE_CONNECT: ndfsReplyTreeConnect->BytesPerSector = %d, ndfsReplyTreeConnect->BytesPerCluster = %d\n", 
							  NTOHL(ndfsReplyTreeConnect->BytesPerSector4), NTOHL(ndfsReplyTreeConnect->BytesPerCluster4)));
	...
	)
{
	...
	ASSERT(NTOHS(ndfsRequestHeader->Uid2) == Secondary->Thread.SessionContext.Uid);
	ASSERT(NTOHS(ndfsRequestHeader->Tid2) == Secondary->Thread.SessionContext.Tid);
	...
	if(requestDataSize <= Secondary->Thread.SessionContext.PrimaryMaxDataSize)
	{
		...
		else
		{
			...
			desResult = DES_CBCUpdate(&desCbcContext, encryptedMessage, (UINT8 *)ndfsWinxpRequestHeader, ndfsRequestHeader->MessageSize-sizeof(NDFS_REQUEST_HEADER));
			ASSERT(desResult == IDOK);
			tdiStatus = SendMessage(
							Secondary->Thread.ConnectionFileObject,
							encryptedMessage,
	...
	)
{
	...
	NDAS_ASSERT( NTOHL(ndfsReplyHeader->MessageSize4) <= SecondaryRequest->NdfsMessageAllocationSize );
		...
		else
		{
			...
			if (NTOHL(ndfsReplyHeader->MessageSize4) - sizeof(NDFS_REPLY_HEADER) - sizeof(NDFS_WINXP_REPLY_HEADER))
			{
				...
				else
				{
					...
					desResult = DES_CBCUpdate(&SecondaryRequest->DesCbcContext, (UINT8 *)(ndfsWinxpReplyHeader+1), encryptedMessage, NTOHL(ndfsReplyHeader->MessageSize4) - sizeof(NDFS_REPLY_HEADER) - sizeof(NDFS_WINXP_REPLY_HEADER));
					ASSERT(desResult == IDOK);
				}
			}
		}
	...
	while(1) {
...
#endif
		{
			tdiStatus = RecvMessage( Secondary->Thread.ConnectionFileObject,
									 ...
									 splitNdfsReplyHeader->Splitted ? Secondary->Thread.SessionContext.SecondaryMaxDataSize : (NTOHL(splitNdfsReplyHeader->MessageSize4) - sizeof(NDFS_REPLY_HEADER)),
									 NULL );
	...
	)
{
	...
	UINT8							*outputBuffer = (UINT8*)SecondaryRequest->NdfsMessage + sizeof(NDFS_REPLY_HEADER) + sizeof(NDFS_WINXP_REPLY_HEADER); 		

	UINT32						irpTag			 = ndfsWinxpRequestHeader->IrpTag4;
	UINT8							irpMajorFunction = ndfsWinxpRequestHeader->IrpMajorFunction;
	UINT8							irpMinorFunction = ndfsWinxpRequestHeader->IrpMinorFunction;
	UINT32						irpFlags		 = ndfsWinxpRequestHeader->IrpFlags;
	...
	case IRP_MJ_READ:
	{
		...
		if(fcb->OpenTime.QuadPart < lastUpdateTime.QuadPart
			|| (fcb->OpenTime.QuadPart - lastUpdateTime.QuadPart) < 600*NANO100_PER_SEC)
		{
			status = STATUS_UNSUCCESSFUL;
			break;
		}
	...
	if(status == STATUS_SUCCESS)
	{
		...
		case IRP_MJ_READ:
		{
			...
			ndfsWinxpReplyHeader = (PNDFS_WINXP_REPLY_HEADER)(ndfsReplyHeader + 1);
			ndfsWinxpReplyHeader->IrpTag4 = irpTag;
			...
			ndfsWinxpReplyHeader->Information = (UINT32)ioStatusBlock.Information;
