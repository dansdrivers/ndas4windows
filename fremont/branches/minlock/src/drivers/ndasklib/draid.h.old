...
#define DRIX_NODE_FLAG_OUT_OF_SYNC 	0x10	// Replaced with Arbiter->OutOfSyncRoleIndex. Only used by client side.

#if 0	// Replacing disk online is not supported.
#define DRIX_NODE_FLAG_NEW_DISK		0x20	// Disk is replaced with new disk. Always comes with DRIX_NODE_FLAG_OUT_OF_SYNC											// If new disk is small disk, set defective flag only.
#endif
...
__forceinline 
UCHAR 
DraidNodeDefectCodeToRmdUnitStatus (
	UCHAR DefectCode
	) 
{
	UCHAR status = 0;
...
__forceinline 
UCHAR 
DraidRmdUnitStatusToDefectCode (
	UCHAR UnitStatus
	) 
{
	UCHAR defectCode = 0;
...
#define DRIX_RAID_STATE_INITIALIZING	0x00	
#define DRIX_RAID_STATE_NORMAL  		0x01
#define DRIX_RAID_STATE_DEGRADED		0x02
#define DRIX_RAID_STATE_OUT_OF_SYNC		0x03
#define DRIX_RAID_STATE_FAILED			0x04
#define DRIX_RAID_STATE_TERMINATED		0x05
...
typedef struct _DRIX_CHANGE_STATUS {
	...
	UCHAR		RaidState;			// DRIX_RAID_STATE_NORMAL, DRIX_RAID_STATE_*
	UCHAR		NodeCount;
...
extern 
struct _DRAID_GLOBALS *g_DraidGlobals;
