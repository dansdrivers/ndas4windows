...
public:
	...
	BOOL SubclassWindow(HWND hWnd)
	{
		...
		m_useNativeMarquee = GetShellVersion() >= PackVersion(6,0);
	...
	BOOL SetMarquee(BOOL bMarquee, UINT uUpdateTime /* = 0U */)
	{
		if (m_useNativeMarquee)
		{
			return SendMessage(PBM_SETMARQUEE, 
		...
		if (bMarquee)
		{
			return SetTimer(9, uUpdateTime * 10, NULL);
		}
		else
		{
			return KillTimer(9);
...
typedef CMarqueeProgressBarCtrlT<CWindow> CMarqueeProgressBarCtrl;
