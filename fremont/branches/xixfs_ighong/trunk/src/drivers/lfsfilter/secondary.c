	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE, ("Secondary close\n") );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE, ("Secondary stop\n") );
	...
	if (status == STATUS_SUCCESS) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE, ("Secondary_Stop: thread stoped Secondary = %p\n", Secondary));
	...
	ASSERT( Secondary->RequestQueue.Flink == &Secondary->RequestQueue );
	...
	)
{
	...
	if (Secondary->Thread.SessionStatus == STATUS_FILE_CORRUPT_ERROR) {
