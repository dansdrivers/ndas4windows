...
AddUserBacl(
	...
	returnResult = GetScsiportAdapter(DiskDeviceObject, &scsiportDeviceObject);
	...
	outbuff_sz = sizeof(SRB_IO_CONTROL) + sizeof(LSMPIOCTL_QUERYINFO) + sizeof(NDAS_BLOCK_ACE);
	psrbioctl = (PSRB_IO_CONTROL)ExAllocatePool(NonPagedPool , outbuff_sz);
	if(psrbioctl == NULL) {
...
RemoveUserBacl(
	...
	outbuff_sz = sizeof(SRB_IO_CONTROL) + sizeof(LSMPIOCTL_QUERYINFO) + sizeof(BLOCKACE_ID);
	psrbioctl = (PSRB_IO_CONTROL)ExAllocatePool(NonPagedPool , outbuff_sz);
	if(psrbioctl == NULL) {
		ObDereferenceObject(scsiportDeviceObject);
        SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE,
...
NdasScsiQuery(
	...
	if(OutBufferLen < sizeof(LSMPIOCTL_QUERYINFO)) {
	...
	) 
{
	...
	if(returnResult != TRUE) 
	{
	    SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("IsNetDisk: GetScsiportAdapter() failed.\n"));
		return FALSE;
	}
	...
	) 
{
	...
	if(returnResult != TRUE) {
	    ...
	    SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR, ("GetSystemBacl: GetScsiportAdapter() failed.\n"));
		return STATUS_UNSUCCESSFUL;
	}

	status = NdasScsiQuery( scsiportAdapterDeviceObject,
							SystemOrUser?LsmpSystemBacl:LsmpUserBacl,
							(PUCHAR)NdasBacl,
							NdasBacl->Length,
							&srbIoctlReturnCode );
