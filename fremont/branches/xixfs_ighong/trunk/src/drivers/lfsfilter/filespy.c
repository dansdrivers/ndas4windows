...
//
...
#endif
	{
		...
		RtlInitUnicodeString( &ntfsName, L"\\NdNtfs" );
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("ntfsStatus = %x, ntfsDeviceObject = %p\n", ntfsStatus, ntfsDeviceObject) );

		RtlInitUnicodeString( &fatName, L"\\NdFat" );
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("fatStatus = %x, fatDeviceObject = %p\n", fatStatus, fatDeviceObject) );
	}
...

{
	...
	{
		PLFS_DEVICE_EXTENSION	fsLfsDeviceExt = &((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->LfsDeviceExt;

		if (!(fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDFAT    ||

			fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDNTFS     ||

			fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NTFS       ||

			fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_FAT)) {
	...
	result = NetdiskManager_IsNetdiskPartition( GlobalLfs.NetdiskManager,
												pIrpSp->Parameters.MountVolume.Vpb->RealDevice,
												&enabledNetdisk );

    SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("NetdiskManager_IsNetdiskPartition result = %d\n", result) );
	...
	if (result == FALSE) {
		...
		ASSERT( enabledNetdisk != NULL );

		if (!(fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDFAT																								||
			 !FlagOn(enabledNetdisk->NetdiskInformation.EnabledFeatures, NDASFEATURE_SIMULTANEOUS_WRITE) && fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NTFS	||
			 FlagOn(enabledNetdisk->NetdiskInformation.EnabledFeatures, NDASFEATURE_SIMULTANEOUS_WRITE) && fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDNTFS)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("enabledNetdisk->NetdiskInformation.SimulWrite = %d\n", (enabledNetdisk->NetdiskInformation.EnabledFeatures & NDASFEATURE_SIMULTANEOUS_WRITE)) );

		//DbgPrint("fsLfsDeviceExt->FileSystemType = %d, enabledNetdisk->NetdiskInformation.SimulWrite = %d\n", 
		//		fsLfsDeviceExt->FileSystemType, (enabledNetdisk->NetdiskInformation.EnabledFeatures & NDASFEATURE_SIMULTANEOUS_WRITE) );

		//if (fsLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_FAT)
		//	ASSERT( !(enabledNetdisk->NetdiskInformation.EnabledFeatures & NDASFEATURE_SIMULTANEOUS_WRITE) );
	...
	if (result == FALSE) {
		...
		SPY_LOG_PRINT( SPYDEBUG_ERROR, ("enabledNetdisk->NetdiskInformation.SimulWrite = %d\n", (enabledNetdisk->NetdiskInformation.EnabledFeatures & NDASFEATURE_SIMULTANEOUS_WRITE)) );
		...
		result = NetdiskManager_IsNetdiskPartition(
		...
		if(result == TRUE && enabledNetdisk == NULL)
		{
			ASSERT(LFS_REQUIRED);
	        return SpyPassThrough(DeviceObject, Irp);
		}
...
//        return SpyPassThrough(DeviceObject, Irp);
	...
	((PFILESPY_DEVICE_EXTENSION)(newDeviceObject->DeviceExtension))->LfsDeviceExt.FileSystemType =
		((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->LfsDeviceExt.FileSystemType;
	...
	NetdiskManager_MountVolume(
		...
		enabledNetdisk,
		((PFILESPY_DEVICE_EXTENSION)(DeviceObject->DeviceExtension))->LfsDeviceExt.FileSystemType,		
		&((PFILESPY_DEVICE_EXTENSION)(newDeviceObject->DeviceExtension))->LfsDeviceExt,
		pIrpSp->Parameters.MountVolume.Vpb->RealDevice,
		&((PFILESPY_DEVICE_EXTENSION)(newDeviceObject->DeviceExtension))->LfsDeviceExt.NetdiskPartitionInformation,
		&netdiskEnableMode
		);
	...
	case NETDISK_READ_ONLY:
		...
		((PFILESPY_DEVICE_EXTENSION)(newDeviceObject->DeviceExtension))->LfsDeviceExt.FilteringMode 
			= LFS_READ_ONLY;
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoCheckIfPossible (
										FileObject,
										FileOffset,
										Length,
										Wait,
		...
		if(lfsPastIoResult.LfsResult == TRUE)
			return lfsPastIoResult.PastIoResult;
	}
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result = LfsFastIoDispatch->FastIoWrite (
										FileObject,
										FileOffset,
										Length,
										Wait,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoQueryBasicInfo (
									FileObject,
									Wait,
									Buffer,
									IoStatus,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoQueryStandardInfo (
									FileObject,
									Wait,
									Buffer,
									IoStatus,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoUnlockSingle (
									FileObject,
									FileOffset,
									Length,
									ProcessId,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoUnlockAll (
									FileObject,
									ProcessId,
									IoStatus,
									DeviceObject
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoUnlockAllByKey (
									FileObject,
									ProcessId,
									Key,
									IoStatus,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoDeviceControl (
									FileObject,
									Wait,
									InputBuffer,
									InputBufferLength,
...
--*/
{
	...
	if (LfsFastIoDispatch && LfsFastIoDispatch->FastIoDetachDevice)
	LfsFastIoDispatch->FastIoDetachDevice (
			SourceDevice,
    						TargetDevice
			);
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoQueryNetworkOpenInfo (
											FileObject,
											Wait,
											Buffer,
											IoStatus,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->MdlReadComplete (
										FileObject,
										MdlChain,
										DeviceObject
										);
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->PrepareMdlWrite(
										FileObject,
										FileOffset,
										Length,
										LockKey,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->MdlWriteComplete (
										FileObject,
										FileOffset,
										MdlChain,
										DeviceObject
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoReadCompressed (
										FileObject,
										FileOffset,
										Length,
										LockKey,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoWriteCompressed (
										FileObject,
										FileOffset,
										Length,
										LockKey,
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->MdlReadCompleteCompressed (
										FileObject,
										MdlChain,
										DeviceObject
										);
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->MdlWriteCompleteCompressed (
										FileObject,
										FileOffset,
										MdlChain,
										DeviceObject
...
--*/
{
	...
	if(LfsFastIoDispatch)
	{
		...
		lfsPastIoResult.Result 
			= LfsFastIoDispatch->FastIoQueryOpen (
										Irp,
										NetworkInformation,
										DeviceObject
										);
...
--*/
{
    ...
    switch (IoControlCode) 
	{
		...
		{
			//
			//	LfsFilter is not notified shutdown from NDAS service any more.
			//	LfsFilter is registered shutdown event listener.
			//

			SPY_LOG_PRINT(LFS_DEBUG_LFS_ERROR, ("LFSIOCTL: LFS_FILTER_SHUTDOWN from the NDAS service discarded.\n"));
