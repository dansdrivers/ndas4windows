				...
				IN PCMD_COMMON				ExtendedCommand)
{
	NTSTATUS				status;
	PCCB					Ccb;
	UINT32					DataBufferLength = BlockTransfer * BLOCK_SIZE;
	ULONG					i, Waits;
	...
	//
	//	This function support only 10 bytes CDB.
	//	Need to support 16 bytes CDB.
	//

	ASSERT(	CDBOperationCode != SCSIOP_SYNCHRONIZE_CACHE16 &&
			CDBOperationCode != SCSIOP_READ16 &&
			CDBOperationCode != SCSIOP_WRITE16);

	ASSERT(NrLurns < LUR_MAX_LURNS_PER_LUR);

	if (SCSIOP_WRITE == CDBOperationCode && 
		!(GENERIC_WRITE & Lurn[0]->AccessRight))
	{
	...
	for(i = 0; i < NrLurns; i++)
	{
		...
		CDB10_LOGICAL_BLOCK_BYTE_TO_BYTES((PCDB)(Ccb[i].Cdb), (UINT32)BlockAddress );
...
#if 0
				KDPrintM(DBG_LURN_ERROR, ("sectors_to_recover(%08d) 92 * 512 + 376 : %02X %02X %02X %02X %02X %02X %02X %02X \n",
				sectors_to_recover,
				(ULONG)*(bufs_read_from_healthy[0] + (91 * 512 + 376 +0)),
				(ULONG)*(bufs_read_from_healthy[0] + (91 * 512 + 376 +1)),
				(ULONG)*(bufs_read_from_healthy[0] + (91 * 512 + 376 +2)),
				(ULONG)*(bufs_read_from_healthy[0] + (91 * 512 + 376 +3)),
				(ULONG)*(bufs_read_from_healthy[0] + (91 * 512 + 376 +4)),
				(ULONG)*(bufs_read_from_healthy[0] + (91 * 512 + 376 +5)),
				(ULONG)*(bufs_read_from_healthy[0] + (91 * 512 + 376 +6)),
				(ULONG)*(bufs_read_from_healthy[0] + (91 * 512 + 376 +7))
				));
#endif				
//				ASSERT(bit_to_recover == 0);
				...
				tested_size = RtlCompareMemory(buf_for_test, buf_write_to_fault, sectors_to_recover * BLOCK_SIZE);
				ASSERT(tested_size == sectors_to_recover * BLOCK_SIZE);
#endif
			}
			else
			{
...
LurnAggrExecute(
	...
	case SCSIOP_VERIFY:  {
		...
		if(	Ccb->Cdb[0] == SCSIOP_READ ||
			...
			startBlockAddress = CDB16_LOGICAL_BLOCK_BYTE((PCDB)Ccb->Cdb);
		...
		if(endBlockAddress <= ChildLurn->EndBlockAddr) {
			...
			if(cdb16 == FALSE) {
				CDB10_LOGICAL_BLOCK_BYTE_TO_BYTES(pCdb, (UINT32)startBlockAddress);
			} else {
				CDB16_LOGICAL_BLOCK_BYTE_TO_BYTES(pCdb, startBlockAddress);
			}
			...
			if(cdb16 == FALSE) {
				CDB10_LOGICAL_BLOCK_BYTE_TO_BYTES(pCdb, (UINT32)firstStartBlockAddress);
			} else {
				CDB16_LOGICAL_BLOCK_BYTE_TO_BYTES(pCdb, firstStartBlockAddress);
			}

			// transfer length
			firstTransferBlocks = (USHORT)(ChildLurn->EndBlockAddr - startBlockAddress + 1);
			if(cdb16 == FALSE) {
				CDB10_TRANSFER_BLOCKS_TO_BYTES(pCdb, (UINT16)firstTransferBlocks);
			} else {
				CDB16_TRANSFER_BLOCKS_TO_BYTES(pCdb, firstTransferBlocks);
			}
			...
			BlockAddress_0 = 0;
			if(cdb16 == FALSE) {
				CDB10_LOGICAL_BLOCK_BYTE_TO_BYTES(pCdb, (UINT32)BlockAddress_0);
			} else {
				CDB16_LOGICAL_BLOCK_BYTE_TO_BYTES(pCdb, BlockAddress_0);
			}
			// transfer length
			secondTransferBlocks = transferBlocks - firstTransferBlocks;
			ASSERT(secondTransferBlocks > 0);
			if(cdb16 == FALSE) {
				CDB10_TRANSFER_BLOCKS_TO_BYTES(pCdb, (UINT16)secondTransferBlocks);
			} else {
				CDB16_TRANSFER_BLOCKS_TO_BYTES(pCdb, secondTransferBlocks);
			}
	...
	if(OriginalCcb->OperationCode == CCB_OPCODE_UPDATE &&
		OriginalCcb->CcbStatus == CCB_STATUS_SUCCESS
			...
			if(LurnUpdate->UpdateClass == LURN_UPDATECLASS_WRITEACCESS_USERID) {
				...
				if(LURN_RAID1R == Lurn->LurnType || LURN_RAID4R == Lurn->LurnType)
				{
					// we don't support shared write for RAID1, 4 yet
					//ASSERT(FALSE); // now we support
					KDPrintM(DBG_LURN_ERROR,("********** Lurn->LurnType(%d) : R/O ->R/W : Start to initialize **********\n"));
...
//	Data buffer shuffled
	...
	ASSERT(0 == (pRaidInfo->MaxBlocksPerRequest % Lurn->LurnChildrenCnt));

	// service sets max blocks per request as full devices size of 1 I/O
	// ex) 64KB per unit device and 2 devices -> MBR = 128KB;
	ulDataBufferSizePerDisk = (pRaidInfo->MaxBlocksPerRequest / Lurn->LurnChildrenCnt) * BLOCK_SIZE;
			...
			case SCSIOP_READ:
				{
					...
					for(j = 0; j < BlocksPerDisk; j++)
					{
						RtlCopyMemory( // Copy back
							...
							(PCHAR)Ccb->DataBuffer + (j * BLOCK_SIZE),
							BLOCK_SIZE);
					}
				}
				break;
	...
	case SCSIOP_WRITE:
		{
			...
			transferBlocks = CDB10_TRANSFER_BLOCKS((PCDB)Ccb->Cdb);
			ASSERT(transferBlocks <= pRaidInfo->MaxBlocksPerRequest);
			...
			for(i = 0; i < Lurn->LurnChildrenCnt; i++)
			{
				...
				{
					RtlCopyMemory(
						(PCHAR)cdb.DataBuffer[i] + j * BLOCK_SIZE,
	...
	case CCB_STATUS_STOP:		// priority 1/5
		if(!LSCcbIsStatusFlagOn(OriginalCcb, CCBSTATUS_FLAG_LURN_STOP))
		{
			...
			//
			{
				...
				ASSERT(LURN_RAID1R == pLurnOriginal->LurnType);
	...
	) 
{
	...
	if(RAID_STATUS_INITIAILIZING != pRaidInfo->RaidStatus)
	{
		ASSERT(pRaidInfo->nDiskCount > 0 && pRaidInfo->nDiskCount <= NDAS_MAX_RAID1R_CHILD);
	}
	...
	case SCSIOP_READ:
		...
		if (RAID_STATUS_EMERGENCY == pRaidInfo->RaidStatus ||
			...
			RAID_STATUS_RECOVERING == pRaidInfo->RaidStatus)
		{
			idx_child = (pRaidInfo->iChildDefected == 0) ? 1 : 0;
		}
		else
		{
			...
			case SCSIOP_READ:
				{
					...
					if ((RAID_STATUS_EMERGENCY == pRaidInfo->RaidStatus ||
						RAID_STATUS_EMERGENCY_READY == pRaidInfo->RaidStatus ||
						RAID_STATUS_RECOVERING == pRaidInfo->RaidStatus) &&
						pRaidInfo->nDiskCount -1 != pRaidInfo->iChildDefected)
					{
						// parity work
						...
						for(j = 0; j < BlocksPerDisk; j++)
						{
							pDataBufferToRecover = 
								...
								(pRaidInfo->iChildDefected + j * (pRaidInfo->nDiskCount -1)) * BLOCK_SIZE);

							k = BLOCK_SIZE / sizeof(ULONG);
							while(k--)
							{
