	...
	)
{
	...
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(DBG_LURN_ERROR, ("Failed to LsLurnIdeAcquireXifsLock #0\n"));
		return STATUS_UNSUCCESSFUL;
	}
	...
	)
{
		...
		LspReleaseLock(&IdeDisk->LanScsiSession,
						SCSIXIFS_LOCKINDEX,
						&LockCount);
