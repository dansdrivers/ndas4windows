	...
	)
{
	...
	secondary = ExAllocatePoolWithTag( NonPagedPool, sizeof(SECONDARY), NDNTFS_ALLOC_TAG );
...
#define MAX_TRY_QUERY 2
	...
	if (status != STATUS_SUCCESS || isLocalAddress) {
		...
		ExFreePoolWithTag( secondary, NDNTFS_ALLOC_TAG );
		return NULL;
	}
	...
	) 
{
	...
	requestSize = sizeof(NDFS_REQUEST_HEADER) + sizeof(NDFS_WINXP_REPLY_HEADER) + DEFAULT_MAX_DATA_SIZE;
