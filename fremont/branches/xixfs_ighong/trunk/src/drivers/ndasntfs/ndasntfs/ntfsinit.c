...

{
...
#if DBG
	DbgPrint("NdNtfs DriverEntry %s %s\n", __DATE__, __TIME__);
#endif
	...
	RtlInitUnicodeString( &UnicodeString, L"\\NdNtfs" );
	...
	Status = IoCreateDevice( DriverObject,
							 ...
							 0,
							 FALSE,
							 &NdNtfsFileSystemDeviceObject );
	
	if(NT_SUCCESS(Status))
		RtlZeroMemory( (PUCHAR)NdNtfsFileSystemDeviceObject+sizeof(DEVICE_OBJECT), 
					   sizeof(VOLUME_DEVICE_OBJECT)-sizeof(DEVICE_OBJECT) );
	...
	RtlInitUnicodeString( &UnicodeString, L"\\NdNtfsControl" );
...
#endif
		IoDeleteDevice( NdNtfsFileSystemDeviceObject );
		return Status;
	}

	RtlInitUnicodeString( &linkString, L"\\DosDevices\\NdNtfsControl" );
	...
	if (!NT_SUCCESS(Status)) {
		...
		IoDeleteDevice( NdNtfsControlDeviceObject );
		IoDeleteDevice( NdNtfsFileSystemDeviceObject );
		return Status;
	}
    ...
    //
#ifdef __ND_NTFS__
	DriverObject->DriverUnload = NdNtfsUnload;
#endif
...
#ifdef __ND_NTFS__
	IoRegisterFileSystem( NdNtfsFileSystemDeviceObject );
...
#ifdef __ND_NTFS_DISABLE_REINIT__
#else
	...
	)
{
...
#if DBG
	DbgPrint("NdNtfs NdNtfsUnload %s %s\n", __DATE__, __TIME__);
#endif
	...
	DebugTrace( 0, Dbg2, ("NdNtfsUnload occured NdNtfsControlDeviceObject->ReferenceCount = %x\n", 
		NdNtfsControlDeviceObject->ReferenceCount) );

	IoDeleteDevice( NdNtfsControlDeviceObject );

	DebugTrace( 0, Dbg2, ("NdNtfsUnload occured NdNtfsFileSystemDeviceObject->ReferenceCount = %x\n", 
