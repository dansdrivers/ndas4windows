...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdFatSecondaryCommonCleanup( IrpContext, Irp );
...
#if 0 //def __ND_FAT_PURGE_OR_FLUSH__
                ...
                if ( (Fcb->UncleanCount == 1) && (Fcb->FcbCondition != FcbBad) ) {
                    ...
                    if (FlagOn(Fcb->FcbState, FCB_STATE_DELETE_ON_CLOSE) &&
						...
						if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
							TypeOfOpen == UserFileOpen && Fcb->PrimarySession && Fcb->UncleanCount == 1) {

							if (IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader == NULL ) {
								
							} else if ((ULONGLONG)Fcb->Header.ValidDataLength.QuadPart < 
								...
								ASSERT( Fcb->Header.FileSize.QuadPart == IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.FileSize );
								ASSERT( Fcb->Header.AllocationSize.QuadPart == IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.AllocationSize );
								
								Fcb->Header.ValidDataLength.QuadPart = IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength;

								DebugTrace2( 0, Dbg, ("Fcb->Header.ValidDataLength.QuadPart = %d\n", Fcb->Header.ValidDataLength.QuadPart) );
							   ...
							   DebugTrace2( 0, Dbg2, ("Fcb->Header.ValidDataLength.QuadPart = %d,IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength = %d\n", 
													   Fcb->Header.ValidDataLength.QuadPart, IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength) );
								Fcb->Header.ValidDataLength.QuadPart = IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength;
