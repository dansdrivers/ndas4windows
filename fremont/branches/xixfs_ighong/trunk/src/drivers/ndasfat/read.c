		...
		try {
			...
			if (FlagOn(Irp->Flags, IRP_PAGING_IO)) {
				...
				if (!FatIsTopLevelRequest(IrpContext))
					DebugTrace( 0,  Dbg2, ("FatFsdRead: SavedTopLevelIrp = %p, FatIsTopLevelRequest() = %d, FatIsTopLevelFat() = %d, CanWait = %d\n", 
				...
				//if ( !FlagOn(IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT) && VolumeDeviceObject->Secondary )
				//	ASSERT( FatGetTopLevelContext()->SavedTopLevelIrp == 0 );
...

{
    ...
    ASSERT( Vcb != NULL );
		...
		if (!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
			...
			Status = NdFatSecondaryCommonRead3( IrpContext, Irp );
			return Status;
		}
#endif
            ...
            if ( NonCachedIo ) {
                ...
                if ( (StartingVbo & (SectorSize - 1)) ||
					...
					if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY &&
						...
						Status = NdFatSecondaryCommonRead( IrpContext, Irp, ByteCount );
					...
					if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY &&
						...
						Status = NdFatSecondaryCommonRead( IrpContext, Irp, BytesToRead );
...
#if 0 //def __ND_FAT_SECONDARY__
					if (IS_SECONDARY_FILEOBJECT(FileObject))
