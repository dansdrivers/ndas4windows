...
#ifdef __ND_FAT_PURGE_OR_FLUSH__
...

{
    ...
    try {
...
#if (defined(__ND_FAT_PRIMARY__) || defined(__ND_FAT_SECONDARY__) || defined(__ND_FAT_PURGE_OR_FLUSH__))
		...
		if (((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL) {
			...
			if (queryStatus != STATUS_SUCCESS) {
...
#if 0 //ndef __ND_FAT_PURGE_OR_FLUSH__
				...
				//DbgPrint( "netdiskEnableMode = %d, VolDo->NetdiskPartitionInformation.PartitionInformation.PartitionNumber = %d\n", 
				//						netdiskEnableMode, VolDo->NetdiskPartitionInformation.PartitionInformation.PartitionNumber );

#ifdef __ND_FAT_LINKSYS_SUPPORT__
		...
		if (VolDo->NetdiskEnableMode == NETDISK_SECONDARY) {
			...
			if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
				((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->NdfsCallback.AddWriteRange(Vcb->Vpb->RealDevice, &VolDo->BlockAceId) == FALSE) {
    ...
    try_exit: NOTHING;
		...
		DebugTrace2( 0, Dbg2, ("NdFatMountVolume: VolDo = %p Status = %x\n", VolDo, Status) );		
...

{
	...
	if (IrpContext && FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_FILE)) {
