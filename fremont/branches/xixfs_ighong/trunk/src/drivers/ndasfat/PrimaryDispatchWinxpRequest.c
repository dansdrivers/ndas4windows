	...
	)
{
...
#ifdef __ND_FAT_RECOVERY_TEST__
	...
	case IRP_MJ_WRITE: { // 0x04
...
#if	(defined __ND_FAT_DIRECT_ACCESS__ || defined __ND_FAT_DIRECT_READWRITE__)
		...
		do {
			...
			*majorFunction = IRP_MJ_WRITE;
			...
			DebugTrace2( 0, Dbg, ("length = %x, byteOffset = %I64x\n", length, byteOffset.QuadPart) );

			ASSERT( (irpSp->Control & SL_INVOKE_ON_SUCCESS) ||
					(irpSp->Control & SL_INVOKE_ON_ERROR) ||
					(irpSp->Control & SL_INVOKE_ON_CANCEL) );

#ifdef __ND_FAT_DIRECT_READWRITE__
			...
			if (fakeWrite == TRUE) {
				...
				typeOfOpen = FatDecodeFileObject( NULL, openFile->FileObject, &vcb, &fcb, &scb, &ccb, TRUE );
				...
				ASSERT( FALSE );
				writeStatus = STATUS_UNSUCCESSFUL; //NdFatPrimaryDirectFsdWrite( PrimarySession->VolDo, irp );
			}
			else 
#endif
			{

				writeStatus = FatFsdWrite( PrimarySession->VolDo, irp );
			}
		...
		if (fakeWrite && NT_SUCCESS(writeStatus)) {
			...
			if (typeOfOpen == UserFileOpen) {
				...
				while (FatLookupFatMcbEntry( &scb->Mcb, vcn, &lcn, &clusterCount, &startingLcn, NULL, NULL, NULL )) {

					ASSERT( lcn == startingLcn );
					ASSERT( (index+1)*sizeof(NDFS_MCB_ENTRY) <= PrimarySession->SessionContext.SecondaryMaxDataSize );
				...
				testVbo = 0;
				for(index=0; 
					FatGetNextFatMcbEntry( &scb->Mcb, &rangePtr, index, &vbo, &lbo, &clusterCount );
					index++) 
				{
					ASSERT( (index+1)*sizeof(NDFS_MCB_ENTRY) <= PrimarySession->SessionContext.SecondaryMaxDataSize );
					...
					mcbEntry[index].ClusterCount = clusterCount;
					//DebugTrace2( 0, Dbg2, "result = %d\n", result );
					//DebugTrace2( 0, Dbg2, ("index = %d\n", index) );

					//DebugTrace2( 0, Dbg2, "sectorCount = %d\n", sectorCount );
...
#ifndef __ND_FAT_DIRECT_READWRITE__
		NdfsWinxpReplytHeader->CurrentByteOffset = fileObject->CurrentByteOffset.QuadPart;
#endif
	...
	case IRP_MJ_SET_INFORMATION: { // 0x06
		...
		openFileId = NdfsWinxpRequestHeader->FileHandle;
		openFile = PrimarySession_FindOpenFile( PrimarySession, openFileId );
		...
		do {
			...
			fileInformation 
				= ExAllocatePoolWithTag( PagedPool, 
										 NdfsWinxpRequestHeader->SetFile.Length + PrimarySession->NetdiskPartitionInformation.VolumeName.Length,
			...
			if (NdfsWinxpRequestHeader->SetFile.FileInformationClass == FileBasicInformation) {
				...
				if (ndfsRequestData[0] == (UCHAR) OBJ_NAME_PATH_SEPARATOR || renameInformation->RootDirectory != NULL) {
					...
					if (found) {
						...
						renameInformation->FileNameLength = PrimarySession->NetdiskPartitionInformation.VolumeName.Length;
		...
		if (NT_SUCCESS(setInformationStatus)) {
			...
			if (!(typeOfOpen == UserFileOpen &&
				...
				*ReplyDataSize = 0;
				...
				NdfsWinxpReplytHeader->Open.TruncateOnClse = BooleanFlagOn( fcb->FcbState, FCB_STATE_TRUNCATE_ON_CLOSE );

				NdfsWinxpReplytHeader->FileSize			= fcb->Header.FileSize.QuadPart;
				NdfsWinxpReplytHeader->AllocationSize	= fcb->Header.AllocationSize.QuadPart;
				NdfsWinxpReplytHeader->ValidDataLength	= fcb->Header.ValidDataLength.QuadPart;
	...
	case IRP_MJ_FILE_SYSTEM_CONTROL: { // 0x0D
		...
		if (openFileId == 0) {
			...
			if (NdfsWinxpReplytHeader->Status == STATUS_SUCCESS && fsControlCode == FSCTL_MOVE_FILE) {		// 29
				...
				if (typeOfOpen == UserFileOpen) {
					...
					NdfsWinxpReplytHeader->Open.TruncateOnClse = BooleanFlagOn( moveFcb->FcbState, FCB_STATE_TRUNCATE_ON_CLOSE );

					NdfsWinxpReplytHeader->FileSize			= moveFcb->Header.FileSize.QuadPart;
					NdfsWinxpReplytHeader->AllocationSize	= moveFcb->Header.AllocationSize.QuadPart;
					NdfsWinxpReplytHeader->ValidDataLength	= moveFcb->Header.ValidDataLength.QuadPart;
					...
					ASSERT( vcn == moveFcb->Header.AllocationSize.QuadPart );
	...
	case IRP_MJ_LOCK_CONTROL: // 0x11
	{
		...
		DebugTrace2( 0, Dbg, ( "LFS: IRP_MJ_LOCK_CONTROL: Length:%I64d Key:%08lx Offset:%I64d Ime:%d Ex:%d\n",
						length.QuadPart,
						key,
						byteOffset.QuadPart,
						key,
	...
	case IRP_MJ_CLEANUP: // 0x12 
	{
		...
		if (openFileId == 0) {
			...
			{			
				...
				if (NdfsWinxpRequestHeader->CleanUp.FileSize != 0) {
					...
					ASSERT( fcb->Header.FileSize.QuadPart == NdfsWinxpRequestHeader->CleanUp.FileSize );
				}
		...
		if (status == STATUS_SUCCESS) {
			...
			NdfsWinxpReplytHeader->Status	   = cleanupStatus;
			NdfsWinxpReplytHeader->Information = 0;
		}
	...
	case IRP_MJ_QUERY_QUOTA:
	{
		...
		NdfsWinxpReplytHeader->Information = 0;
		*replyDataSize = 0;
		NdfsWinxpReplytHeader->Status	  = STATUS_NOT_SUPPORTED;
		status = STATUS_SUCCESS;
	...
	)
{
	...
	do {
		...
		fileNameBuffer = FsRtlAllocatePoolWithTag( PagedPool, NDFS_MAX_PATH, TAG_FILENAME_BUFFER );
...
#if defined(_WIN64)
			ASSERT( relatedOpenFile && relatedOpenFile->OpenFileId == (_U64)relatedOpenFile );
#else
...
#endif
			DebugTrace2( 0, Dbg, ("openFile->OpenFileId = %x\n", openFile->OpenFileId) );
		...
		if (createStatus == STATUS_SUCCESS) {
			...
			//NdfsWinxpReplytHeader->Open.ScbHandle			= (_U32)scb;
...
#if defined(_WIN64)
				NdfsWinxpReplytHeader->Open.FcbHandle			= (_U64)fcb;
#else
				NdfsWinxpReplytHeader->Open.FcbHandle			= (_U32)fcb;
#endif
...
#if defined(_WIN64)
				NdfsWinxpReplytHeader->Open.FcbHandle			= (_U64)vcb;
#else
				NdfsWinxpReplytHeader->Open.FcbHandle			= (_U32)vcb;
#endif
			}
	...
	)
{
	...
	NdfsWinxpReplytHeader->Status		= closeStatus;
	NdfsWinxpReplytHeader->Information	= 0;
