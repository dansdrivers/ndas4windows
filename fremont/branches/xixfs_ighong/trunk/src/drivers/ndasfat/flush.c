...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdFatCommonFlushBuffers( IrpContext, Irp );

		DebugTrace2( -1, Dbg, ("NtfsCommonFlushBuffers -> %08lx\n", Status) );
        ...
        if ( (NodeType( Fcb ) == FAT_NTC_FCB) &&
            ...
            try {
                ...
                try {
					...
					if (!FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_FLUSH)) {
