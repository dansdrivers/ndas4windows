...
/*		
		if(NewByteCount > PAGE_SIZE) {
			NewByteCount = PAGE_SIZE;
		}
*/
		...
		*/
		...
		if(Offset || (TruncateSize && (RemainOrgByteCount < NewByteCount))){
			...
			*bUnAligned = TRUE;
			...
			IoRun[IoRunIndex].TransferVirtualAddress = Add2Ptr(Irp->UserBuffer,CurrentUserBufferOffset);
		}
    ...
    )
{
	...
	if(TypeOfOpen <= UserVolumeOpen){
		return XixFsdVolumePrepareBuffers (IrpContext,
											Irp,
											FCB,
											ByteOffset,
											ByteCount,
		...
		return XixFsdFilePrepareBuffers (IrpContext,
											Irp,
											FCB,
											ByteOffset,
											ByteCount,
...
)
{
	...
	try{	
		...
		do{
			...
			if((NumberOfRuns == 1) && !UnAligned && FirstPass){
					...
					if(LotCount != AllocatedLotCount){
						if(pFCB->RealAllocationSize < RequestStartOffset){
							...
							try_return(RC);
							 ...
							 Length = (uint32)(Offset - pFCB->RealAllocationSize);
						}
					}
