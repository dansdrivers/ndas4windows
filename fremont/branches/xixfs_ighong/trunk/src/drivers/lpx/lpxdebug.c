...
Abstract:
    ...
    This module contains code that implements debug things for LPX. It is
...

{
    ...
    LpxHeader = (PLPX_HDR_CONNECTION)&(Packet->Header[18]);
    ...
    IF_LPXDBG (LPX_DEBUG_IFRAMES) {
    } else {
        return;                         // if IFRAMES not set, don't print.
    }

    switch (LpxHeader->Command) {
        case LPX_CMD_ADD_GROUP_NAME_QUERY:
            s = "ADD_GROUP_NAME_QUERY"; break;

        case LPX_CMD_ADD_NAME_QUERY:
            s = "ADD_NAME_QUERY"; break;

        case LPX_CMD_NAME_IN_CONFLICT:
            s = "NAME_IN_CONFLICT"; break;

        case LPX_CMD_STATUS_QUERY:
            s = "STATUS_QUERY"; break;

        case LPX_CMD_TERMINATE_TRACE:
            s = "TERMINATE_TRACE"; break;

        case LPX_CMD_DATAGRAM:
            s = "DATAGRAM"; break;

        case LPX_CMD_DATAGRAM_BROADCAST:
            s = "BROADCAST_DATAGRAM"; break;

        case LPX_CMD_NAME_QUERY:
            s = "NAME_QUERY"; break;

        case LPX_CMD_ADD_NAME_RESPONSE:
            s = "ADD_NAME_RESPONSE"; break;

        case LPX_CMD_NAME_RECOGNIZED:
            s = "NAME_RECOGNIZED"; break;

        case LPX_CMD_STATUS_RESPONSE:
            s = "STATUS_RESPONSE"; break;

        case LPX_CMD_TERMINATE_TRACE2:
            s = "TERMINATE_TRACE2"; break;

        case LPX_CMD_DATA_ACK:
            s = "DATA_ACK"; break;

        case LPX_CMD_DATA_FIRST_MIDDLE:
            s = "DATA_FIRST_MIDDLE"; break;

        case LPX_CMD_DATA_ONLY_LAST:
            s = "DATA_ONLY_LAST"; break;

        case LPX_CMD_SESSION_CONFIRM:
            s = "SESSION_CONFIRM"; break;

        case LPX_CMD_SESSION_END:
            s = "SESSION_END"; break;

        case LPX_CMD_SESSION_INITIALIZE:
            s = "SESSION_INITIALIZE"; break;

        case LPX_CMD_NO_RECEIVE:
            s = "NO_RECEIVE"; break;

        case LPX_CMD_RECEIVE_OUTSTANDING:
            s = "RECEIVE_OUTSTANDING"; break;

        case LPX_CMD_RECEIVE_CONTINUE:
            s = "RECEIVE_CONTINUE"; break;

        case LPX_CMD_SESSION_ALIVE:
    ...
    if (HEADER_LENGTH(LpxHeader) != 14) {
        e = "(LENGTH IN ERROR) ";
    } else if (HEADER_SIGNATURE(LpxHeader) != NETBIOS_SIGNATURE) {
    ...
    DbgPrint (" ( D1=%ld, D2=%ld, XC=%ld, RC=%ld )\n",
              (ULONG)LpxHeader->Data1,
              (ULONG)(LpxHeader->Data2Low+LpxHeader->Data2High*256),
              TRANSMIT_CORR(LpxHeader),
              RESPONSE_CORR(LpxHeader));
} /* DisplayOneFrame */
...
Routine Description:
    ...
    before it is sent by LpxSendUIFrame.  This ensures that we have formatted
...

{
    ...
    PLPX_HDR_CONNECTIONLESS LpxHeader;

    //

    DlcHeader = (PDLC_FRAME)&(OuterFrame->Header[14]);
    LpxHeader = (PLPX_HDR_CONNECTIONLESS)&(OuterFrame->Header[17]);
    ...
    for (i=0; i<16; i++) {              // copy NetBIOS names.
        SenderName [i] = LpxHeader->SourceName [i];
        ReceiverName [i] = LpxHeader->DestinationName [i];
    }
    SenderName [16] = 0;                // install zero bytes.
    ReceiverName [16] = 0;

    if (HEADER_LENGTH(LpxHeader) != 44) {
        e = "(LENGTH IN ERROR) ";
    } else if (HEADER_SIGNATURE(LpxHeader) != NETBIOS_SIGNATURE) {
        e = "(SIGNATURE IN ERROR) ";
    } else {
