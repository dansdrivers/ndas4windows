...
SrbIoctlQueryInfo(
	...
	*SrbStatus = SRB_STATUS_SUCCESS;
	...
	case LsmpUserBacl: {
		...
		KDPrint(1,("LsmpSystemBacl/UserBacl: going to default LuExtention 0.\n"));
		if(HwDeviceExtension->LURs[0] == NULL || HwDeviceExtension->LURCount == 0) {
			*SrbStatus = SRB_STATUS_INVALID_REQUEST;
			status = STATUS_INVALID_PARAMETER;
			break;
		...
		if(QueryInfo->InfoClass == LsmpSystemBacl)
	...
	case 	LsmpDriverVersion:				// 5
		{
			...
			if(OutputBufferLength < sizeof(LSMPIOCTL_DRVVER)) {
...
SrbIoctlXifsLock(
	PMINIPORT_DEVICE_EXTENSION	HwDeviceExtension,
	PLSMPIOCTL_GB_LOCK_QUERY	QueryLock,
	PNTSTATUS					NtStatus,
	ULONG						CurSrbSequence
)
{
	...
	if(!NT_SUCCESS(ntStatus)) {
		KDPrint(1,("LSCcbAllocate() failed.\n"));
		QueryLock->Status = STATUS_INSUFFICIENT_RESOURCES;
	...
	LSCCB_INITIALIZE(Ccb, CurSrbSequence);
	switch(QueryLock->Operation){
		case NdasXifsdLockGB:
			Ccb->OperationCode = CCB_OPCODE_XIFS_LOCK_GB;
			//DbgPrint("CCB_OPCODE_XIFS_LOCK_GB\n");
			break;
		case NdasXifsdUnlockGB:
			Ccb->OperationCode = CCB_OPCODE_XIFS_UNLOCK_GB;
			//DbgPrint("CCB_OPCODE_XIFS_UNLOCK_GB\n");
			break;
		case NdasXifsdQueryLockGB:
			Ccb->OperationCode = CCB_OPCODE_XIFS_QUERY_LOCK_GB;
			//DbgPrint("CCB_OPCODE_XIFS_QUERY_LOCK_GB\n");
			break;
		default:
			DbgPrint("Invalid Command\n");
			KDPrint(1, ("Invalid Command\n"));
			QueryLock->Status = STATUS_INVALID_DEVICE_REQUEST;
	...
	LSCcbSetFlag(Ccb, CCB_FLAG_SYNCHRONOUS|CCB_FLAG_ALLOCATED);
	Ccb->DataBuffer = QueryLock->Information;
	Ccb->DataBufferLength = 8;
	

	KDPrint(1,("going to default LUR 0.\n"));
	ntStatus = LurRequest(
						HwDeviceExtension->LURs[0],	// default: 0.
						Ccb
					);
	...
	KDPrint(1,("Information (0x%02x:%02x:%02x:%02x-%02x:%02x:%02x:%02x)\n",
...
--*/
{
    ...
    switch (controlCode) {
		//	Added by ILGU HONG for xixfs 08302006
		case LANSCSIMINIPORT_IOCTL_QUERY_GB_LOCK:
			{
				//DbgPrint("Enter LANSCSIMINIPORT_IOCTL_QUERY_GB_LOCK\n");
				srbStatus = SrbIoctlXifsLock(
								HwDeviceExtension,
								(PLSMPIOCTL_GB_LOCK_QUERY)srbIoctlBuffer,
								&srbIoControl->ReturnCode,
								CurSrbSequence
				);

				if(srbStatus == SRB_STATUS_SUCCESS) {
						KDPrint(1,("LANSCSIMINIPORT_IOCTL_QUERY_GB_LOCK: Successful.\n"));
						//DbgPrint("LANSCSIMINIPORT_IOCTL_QUERY_GB_LOCK: Successful.\n");
						status = STATUS_SUCCESS;
				} else {
						KDPrint(1,("LANSCSIMINIPORT_IOCTL_QUERY_GB_LOCK: Unsuccessful.\n"));
						//DbgPrint("LANSCSIMINIPORT_IOCTL_QUERY_GB_LOCK: Unsuccessful.\n");
		...
		case LANSCSIMINIPORT_IOCTL_ADD_USERBACL: {
		...
		case LANSCSIMINIPORT_IOCTL_REMOVE_USERBACL: {
		...
		case LANSCSIMINIPORT_IOCTL_GET_VERSION: {
			...
			if(srbIoctlBufferLength < sizeof(LSMPIOCTL_DRVVER)) {
				srbStatus = SRB_STATUS_DATA_OVERRUN;
				status = STATUS_UNSUCCESSFUL;
				break;
			}
