...
Routine Description:
    ...
    N.B. This routine is NOT pageable as it may be called at dispatch
    level.
    ...
    switch(irpStack->MinorFunction) {
        ...
        case IRP_MN_SET_POWER: {
            ...
            if(isPdo) {
                ...
                //
                // If we have disabled power then don't allow any non-working
                // power irps through.
                //
                ...
                if((adapter->DisablePower) && 
                   ((state.DeviceState != PowerDeviceD0) || 
                    ...
                    status = SpSetAdapterPower(DeviceObject,
                                               Irp,
                                               type,
                                               state);
                }
        ...
        case IRP_MN_QUERY_POWER: {
            ...
            if(isPdo) {
                ...
                status = SpQueryAdapterPower(DeviceObject,
                                             Irp,
                                             type,
                                             state);
    ...
    )
{
    ...
    if((commonExtension->CurrentDeviceState != PowerDeviceD0) ||
       ((commonExtension->CurrentSystemState != PowerSystemWorking) &&
        ...
        //
        // BUGBUG - if we can't power up the device here we'll need to
...

{
    ...
    if(((commonExtension->CurrentSystemState != PowerSystemWorking) &&
       ...
       (commonExtension->CurrentSystemState == SystemState)) {
        ...
        SpAdjustDisabledBit(
            logicalUnit, 
            (BOOLEAN) ((SystemState == PowerSystemWorking) ? TRUE : FALSE));
...
--*/
{
    ...
    if(!NT_SUCCESS(Status)) {
        ...
        //
        // BUGBUG - figure out a way to deal with this case.
        //
    ...
    )
{
    ...
    try {
        ...
        if(commonExtension->CurrentSystemState != PowerSystemWorking) {
            ...
            //
            // Set the desired power state to D0 and return PENDING.  When the
            // system is woken up scsiport will force the adapter into the
            // desired power state if it's set.
            //
    
            //
            // BUGBUG - this is difficult to do synchronously.  For now just 
            // fail the power up if we're not in a system working state.
            //
