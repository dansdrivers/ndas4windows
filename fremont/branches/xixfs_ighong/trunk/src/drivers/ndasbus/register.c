...
#define NDBUS_REG_UNITMAXBPR	L"UnitDeviceMaxBlocksPerRequest"		// 4 bytes
...
ReadUnitFromRegistry(
	...
	status = DrReadKeyValue(
						UnitDevKey,
						NDBUS_REG_UNITMAXBPR,
						REG_DWORD,
						&Unit->UnitMaxRequestBlocks,
						sizeof(Unit->UnitMaxRequestBlocks),
						NULL
					);
	if(!NT_SUCCESS(status)) {
		Bus_KdPrint_Def(BUS_DBG_SS_ERROR, ("DrReadKeyValue() failed. ValueKeyName:%ws\n", NDBUS_REG_UNITMAXBPR));
...
ReadTargetAndUnitFromRegistry(
	...
	//	Check buffer size.
	//
	outLength = FIELD_OFFSET(LANSCSI_ADD_TARGET_DATA, UnitDiskList) +
				sizeof(LSBUS_UNITDISK) * unitCnt +
				baclLen;
	...
	if(baclLen) {
		...
		ndasBACLOffset = FIELD_OFFSET(LANSCSI_ADD_TARGET_DATA, UnitDiskList) +
			sizeof(LSBUS_UNITDISK) * unitCnt;
