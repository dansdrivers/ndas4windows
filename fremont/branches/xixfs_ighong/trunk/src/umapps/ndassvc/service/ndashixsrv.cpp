	...
	UCHAR Status)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixServer, TLError, 
			"Sending a REPLY_ERROR failed.\n");
		return FALSE;
	}
...
CNdasHIXServer::ThreadStart(HANDLE hStopEvent)
{
	...
	{
		XTLTRACE2_ERR(TCHixServer, TLError, 
			"LPX Datagram Server initialization failed.\n");
		return 254;
	}
	...
	{
		XTLTRACE2_ERR(TCHixServer, TLError, 
			"Listening HIX at port %d failed.\n",
			NDAS_HIX_LISTEN_PORT);
		return 255;
	}
...
CNdasHIXServer::OnReceive(CLpxDatagramSocket& sock)
{
	...
	if (!fSuccess) 
	{
		XTLTRACE2_ERR(TCHixServer, TLError, "HIX Packet Receive failed.\n");
		return;
	}
	...
	const NDAS_HIX::DISCOVER::REQUEST* pRequest)
{
	...
	{
		XTLTRACE2_ERR(TCHixServer, TLError, 
			"Sending a DISCOVER reply to %s at %s failed.\n", 
	...
	const NDAS_HIX::QUERY_HOST_INFO::REQUEST* pRequest)
{
	...
	{
		XTLTRACE2_ERR(TCHixServer, TLError, 
			"Sending a QUERY_HOST_INFO reply to %s at %s failed.\n", 
			CSockLpxAddr(pRemoteAddr).ToStringA(),
			CSockLpxAddr(sock.GetBoundAddr()).ToStringA());
		return;
	}
	...
	const NDAS_HIX::SURRENDER_ACCESS::REQUEST* pRequest)
{
	...
	if (pRequest->Header.Length < cbBasicData) 
	{
		...
		{
			XTLTRACE2_ERR(TCHixServer, TLError,
				"Sending NHIX_SURRENDER_REPLY_ERROR_INVALID_REQUEST failed.\n");
		}
	...
	if (CNdasLogicalDeviceNullPtr == pLogDevice) {
		...
		{
			XTLTRACE2_ERR(TCHixServer, TLError,
				"Sending NHIX_SURRENDER_REPLY_STATUS_NO_ACCESS failed.\n");
		}
	...
	{
		XTLTRACE2_ERR(TCHixServer, TLError,
			"Sending NHIX_SURRENDER_REPLY_STATUS_QUEUED failed.\n");
	}
