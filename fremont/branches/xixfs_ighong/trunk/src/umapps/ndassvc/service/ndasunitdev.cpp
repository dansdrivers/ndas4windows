...
UINT32 
CNdasUnitDiskDevice::GetBACLSize(int nBACLSkipped) const
{
	if (NULL == m_pBACL)
	{
		return 0;
	...
	XTLTRACE("GetBACLSize() m_pBACL->ElementCount(%d). Size(%d)\n",
		...
		sizeof(NDAS_BLOCK_ACL) + sizeof(NDAS_BLOCK_ACE) * (m_pBACL->ElementCount -1 - nBACLSkipped));
...
CNdasUnitDiskDevice::FillBACL(void *pNdasBlockAcl) const
{
	...
	for(int i = 0; i < m_pBACL->ElementCount; i++)
	{
