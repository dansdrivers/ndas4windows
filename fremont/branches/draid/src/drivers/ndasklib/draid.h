...
#define DRIX_NODE_DEFECT_BAD_BLOCK	0x01	// Disk has bad block. 
#define DRIX_NODE_DEFECT_BAD_DISK	0x02	// May be bad disk or bad NDAS HW
#define DRIX_NODE_DEFECT_SMALL_DISK	0x03	// Disk is too small as replacement disk. Used only after hot-swap is implemented.
#define DRIX_NODE_DEFECT_NOT_A_MEMBER 0x04	// Disk is registered as different raid set or single mode disk.
#define DRIX_NODE_DEFECT_INDEPENENT_UPDATE 0x06 // RAID member is indepentently updated.
#define DRIX_NODE_DEFECT_PRIOR_MEMBER	0x7	// Unit is member of RAID set but is not used as RAID member. Set when member is prior active member.	
