	...
	)
{
	...
	if(!LurnDesc)
	{
		...
		status = LspReconnect(&IdeDisk->LanScsiSession, NULL);
...
try_reconnect:
			...
			Ccb->CcbStatus == CCB_STATUS_COMMUNICATION_ERROR)) 
		{
			if (LSCcbIsFlagOn(Ccb, CCB_FLAG_RETRY_NOT_ALLOWED) ||
				(Lurn->LurnParent && LURN_REDUNDENT_TYPE(Lurn->LurnParent->LurnType) && IdeDisk->DiskWriteCacheDirty)) {
				// Reconnection is not allowed 
				//  if unflushed write may be exist on redundent disk.(RAID system should handle this case).
			...
			LurnIdeDiskAllLocksLost(IdeDisk);
...
terminate_thread:
	...
	CcbCompleteList(
			...
			CCB_STATUS_STOP,
			CCBSTATUS_FLAG_LURN_STOP|CCBSTATUS_FLAG_BUSCHANGE
		);
...
UINT32	DevLockIdMap[NDAS_NR_GPLOCK] = {-1, 2, 3};
// Advanced general purpose lock supported by NDAS chip 2.5
UINT32	DevLockIdMap_Adv[NDAS_NR_ADV_GPLOCK] = {-1, 0, 1, 2, 3, 4, 5, 6, 7};
...
static
NTSTATUS
LurnIdeDiskAcquireDevLock(
	IN PLURNEXT_IDE_DEVICE		IdeDisk,
	IN PCCB						Ccb,
	IN ULONG					LockIndex,
	IN PUCHAR					LockData,
	IN UINT64					ContentionTimeOut
){
	NTSTATUS	status;
	ULONG		lockContention;

	UNREFERENCED_PARAMETER(ContentionTimeOut);
	...
	while(1) {
		...
		//
		// STATUS_WAS_LOCKED is informational.
		// NT_SUCCESS() regards it as a success.
		//

		if(status == STATUS_WAS_LOCKED) {
	...
	if(!NT_SUCCESS(status)) {
		KDPrintM(DBG_LURN_ERROR, ("Failed to acquire lock idx=%d\n", LockIndex));
		Ccb->CcbStatus = CCB_STATUS_COMMAND_FAILED;
		return status;
	}
...
LurnIdeDiskReleaseDevLock(
	...
	status = LspReleaseLock(
				...
				NULL);
	if(!NT_SUCCESS(status)) {
		KDPrintM(DBG_LURN_ERROR, ("Failed to release lock idx=%d\n", LockIndex));
		Ccb->CcbStatus = CCB_STATUS_COMMAND_FAILED;
		return status;
	}
...
LurnIdeDiskQueryDevLockOwner(
	...
	status = LspGetLockOwner(
				...
				NULL);
	if(!NT_SUCCESS(status)) {
		KDPrintM(DBG_LURN_ERROR, ("Failed to query lock owner idx=%d\n", LockIndex));
		Ccb->CcbStatus = CCB_STATUS_COMMAND_FAILED;
		return status;
	}
...
LurnIdeDiskGetDevLockData(
	...
	status = LspGetLockData(
				...
				NULL);
	if(!NT_SUCCESS(status)) {
		KDPrintM(DBG_LURN_ERROR, ("Failed to get lock data idx=%d\n", LockIndex));
		Ccb->CcbStatus = CCB_STATUS_COMMAND_FAILED;
		return status;
	}
...
INLINE
VOID
LurnIdeSetDevLockStatusAcquired(
...
INLINE
VOID
LurnIdeSetDevLockStatusReleased(
	IN PLU_HWDATA	LuHwData,
	IN ULONG		LockId
){
	PNDAS_DEVLOCK_STATUS	lockStatus = &LuHwData->DevLockStatus[LockId];
...
LurnIdeSetDevLockStatusLost(
	...
	KDPrintM(DBG_LURN_ERROR, ("Lost lock #%d\n", LockId));
...
INLINE
VOID
LurnIdeSetDevLockStatusLostConfirmed(
	IN PLU_HWDATA	LuHwData,
	IN ULONG		LockId
){
	PNDAS_DEVLOCK_STATUS	lockStatus = &LuHwData->DevLockStatus[LockId];
	...
	KDPrintM(DBG_LURN_ERROR, ("Confirmed lost lock #%d\n", LockId));
...
LurnIdeDiskDeviceLockControl(
	...
	if(IdeDisk->LuHwData.HwVersion == LANSCSIIDE_VERSION_1_1 ||
		...
		lockIdx = DevLockIdMap[devLockControl->LockId];
	} else {
		lockIdx = DevLockIdMap_Adv[devLockControl->LockId];
	}
	...
	case LURNLOCK_OPCODE_ACQUIRE: {
		PNDAS_DEVLOCK_STATUS	lockStatus = &IdeDisk->LuHwData.DevLockStatus[devLockControl->LockId];
		...
		status = LurnIdeDiskAcquireDevLock(
						IdeDisk,
						Ccb,
						lockIdx,
						devLockControl->LockData, 
						devLockControl->ContentionTimeOut);
		if(NT_SUCCESS(status) && Ccb->CcbStatus == CCB_STATUS_SUCCESS) {
			...
			if(!lockStatus->Acquired) {
				...
				LurnIdeSetDevLockStatusAcquired(&IdeDisk->LuHwData, devLockControl->LockId);

				if(devLockControl->AddressRangeValid) {
					lockStatus->AddressRangeValid = TRUE;
					ASSERT(devLockControl->StartingAddress <= devLockControl->EndingAddress);
					lockStatus->StartingAddress = devLockControl->StartingAddress;
					lockStatus->EndingAddress = devLockControl->EndingAddress;
				} else {
					lockStatus->AddressRangeValid = FALSE;
				}
			}
		}
		return status;
	...
	case LURNLOCK_OPCODE_RELEASE: {
		...
		if(lockStatus->Acquired == FALSE) {
			if(lockStatus->Lost) {
				//
				// Clear lost status
				//
				LurnIdeSetDevLockStatusLostConfirmed(&IdeDisk->LuHwData, devLockControl->LockId);
				Ccb->CcbStatus = CCB_STATUS_SUCCESS;
				return STATUS_SUCCESS;
			}
		...
		status = LurnIdeDiskReleaseDevLock(
						IdeDisk,
						Ccb,
						lockIdx,
						devLockControl->LockData);
		if(NT_SUCCESS(status) && Ccb->CcbStatus == CCB_STATUS_SUCCESS) {
			...
			if(lockStatus->Acquired) {
				LurnIdeSetDevLockStatusReleased(&IdeDisk->LuHwData, devLockControl->LockId);
				lockStatus->AddressRangeValid = FALSE;
			}
		}
		return status;
	...
	case LURNLOCK_OPCODE_QUERY_OWNER:
		return LurnIdeDiskQueryDevLockOwner(
						IdeDisk,
						Ccb,
						lockIdx,
						devLockControl->LockData);
	case LURNLOCK_OPCODE_GETDATA:		// Not yet implemented
		return LurnIdeDiskGetDevLockData(
						IdeDisk,
						Ccb,
						lockIdx,
						devLockControl->LockData);
...
//
// Check lost locks during reconnection.
//
...
LurnIdeDiskAllLocksLost(
	...
	for(lockId = 0; lockId < NDAS_NR_MAX_GPLOCK; lockId ++) {
		if(IdeDisk->LuHwData.DevLockStatus[lockId].Acquired) {

			LurnIdeSetDevLockStatusReleased(&IdeDisk->LuHwData, lockId);
			LurnIdeSetDevLockStatusLost(&IdeDisk->LuHwData, lockId);
			KDPrintM(DBG_LURN_ERROR, ("Lost lock #%d\n", lockId));
		}
	}
#if DBG
	if(IdeDisk->LuHwData.LostLockCount == 0) {
		KDPrintM(DBG_LURN_ERROR, ("No Lost lock\n"));
	}
#endif
}
...
LurnIdeCheckLostLockIORange(
	...
	lostLockCnt = IdeDisk->LuHwData.LostLockCount;
	lockId = 0;
	while(lostLockCnt ||
		lockId < NDAS_NR_MAX_GPLOCK) {

		devLockStatus = &IdeDisk->LuHwData.DevLockStatus[lockId];
			...
			if(devLockStatus->AddressRangeValid) {
				...
				if(StartingAddress < devLockStatus->StartingAddress) {
					if(EndingAddress >= devLockStatus->StartingAddress) {
						KDPrintM(DBG_LURN_ERROR, ("The address range is in lost lock's address range(1).\n"));
						return TRUE;
					}
				} else if(StartingAddress <= devLockStatus->EndingAddress) {
					KDPrintM(DBG_LURN_ERROR, ("The address range is in lost lock's address range(2).\n"));
					return TRUE;
				}
			}
...
)
{
	...
	lostLockIO = LurnIdeCheckLostLockIORange(IdeDisk, LogicalBlockAddress, LogicalBlockAddress + TransferBlocks - 1);
	if(lostLockIO) {
	...
	if(LSCcbIsFlagOn(Ccb, CCB_FLAG_ACQUIRE_BUFLOCK)) {
		...
		while(1) {
			status = LspAcquireLock(
							&IdeDisk->LanScsiSession,
							LOCKIDX_BASE_NDASBUFFER + IdeDisk->LuHwData.LanscsiTargetID,
							NULL,
							NULL);
			if(!NT_SUCCESS(status)) {
				break;
			...
			if(	lockContention % 1000 == 0) {
				...
				LspCleanupLock(&IdeDisk->LanScsiSession,
					LOCKIDX_BASE_NDASBUFFER + IdeDisk->LuHwData.LanscsiTargetID, NULL);
			}
...
#ifdef __ENABLE_CHECK_POWER_MODE__
				...
				case CCB_EXT_READ:
					{
						...
						case EXT_BLOCK_OPERATION:
							KDPrintM(DBG_LURN_ERROR,	("CCB_EXT_READ: Addr %I64x\n", logicalBlockAddress));
...
//		inquiryData.EnclosureServices;
	...
	case SCSIOP_READ16:
	{
		...
		if(Ccb->DataBufferLength < (ULONG)(transferBlocks*IdeDisk->LuHwData.BlockBytes)) {
			KDPrintM(DBG_LURN_ERROR,
						("SCSIOP_READ CCB Ccb->DataBufferLength = %d, Trans*BlockSize = %d\n",
	...
	case SCSIOP_WRITE16:
	{
		...
		if(Ccb->DataBufferLength < (ULONG)(transferBlocks * IdeDisk->LuHwData.BlockBytes)) {
			KDPrintM(DBG_LURN_ERROR,
						("SCSIOP_WRITE CCB Ccb->DataBufferLength = %d, Trans*BlockSize = %d\n",
						Ccb->DataBufferLength, (ULONG)(transferBlocks*IdeDisk->LuHwData.BlockBytes)));
	...
	case SCSIOP_SYNCHRONIZE_CACHE16:
		...
		KDPrintM(DBG_LURN_TRACE, ("SCSIOP_SYNCHRONIZE_CACHE\n"));

		{
			...
			if(IdeDisk->LuHwData.HwVersion <= LANSCSIIDE_VERSION_1_0) {
				KDPrintM(DBG_LURN_ERROR, ("SYNCHRONIZE_CACHE: Not supported.\n"));
				Ccb->CcbStatus = CCB_STATUS_SUCCESS;
				status = STATUS_SUCCESS;
				break;
			}
			...
			if(Ccb->Flags & CCB_FLAG_MUST_SUCCEED) {
				KDPrintM(DBG_LURN_TRACE, ("reutun status overrided.\n"));
				KDPrintM(DBG_LURN_ERROR,
