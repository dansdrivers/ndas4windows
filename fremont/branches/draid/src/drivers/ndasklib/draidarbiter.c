...
//		ASSERT(UsedAddrCount >0);
	...
	for(i=0;i<pArbiter->TotalDiskCount;i++) { // i is role index.
		...
		if (NodeFlags & DRIX_NODE_FLAG_DEFECTIVE) {
			if (pArbiter->DefectCodes[NewRmd.UnitMetaData[i].iUnitDeviceIdx] & 
				...
				// Record physical defects only.
				NewRmd.UnitMetaData[i].UnitDeviceStatus |= NDAS_UNIT_META_BIND_STATUS_DEFECTIVE;
				NewRmd.UnitMetaData[i].DefectCode = 
					pArbiter->DefectCodes[NewRmd.UnitMetaData[i].iUnitDeviceIdx] & 
					(DRIX_NODE_DEFECT_BAD_BLOCK|DRIX_NODE_DEFECT_BAD_DISK);
			}
		}
...
#if 0
		if (pArbiter->RaidStatus == DRIX_RAID_STATUS_DEGRADED && !(NodeFlags & DRIX_NODE_FLAG_RUNNING)) {
			// We cannot write to non-running node
			NewRmd.UnitMetaData[i].UnitDeviceStatus |= NDAS_UNIT_META_BIND_STATUS_RMD_OUTDATED;
		}
#endif		
	...
	case DRIX_CMD_ACQUIRE_LOCK:
		{
			...
			if (NewLock==NULL) {
				...
				if (pArbiter->RaidStatus == DRIX_RAID_STATUS_DEGRADED) {
					...
					KDPrintM(DBG_LURN_INFO, ("Granted lock %I64x(%I64x:%Ix).\n",
						NewLock->LockId, NewLock->LockAddress, NewLock->LockLength));
				}
				RELEASE_SPIN_LOCK(&pArbiter->SpinLock, oldIrql);

				// Need to update BMP and LWR before client start to write using this lock
				status = DraidArbiterUpdateOnDiskOosBitmap(pArbiter);
				if (!NT_SUCCESS(status)) {
...
//	case DRIX_CMD_UNREGISTER:
	...
	if (pClient->LocalClient) {
		...
		KDPrintM(DBG_LURN_INFO, ("DRAID Sending reply to request %s with result %s to local client(event=%p)\n", 
...
DraidArbiterUseSpareIfNeeded(
	...
	if (SpareUsed) {
		...
		DraidArbiterUpdateOnDiskOosBitmap(pArbiter);
	}
	return STATUS_SUCCESS;
}
...
#if 0			// Client is already removed from link before calling this.
				ACQUIRE_SPIN_LOCK(&pArbiter->SpinLock, &oldIrql);
				RemoveEntryList(&Client->Link);
				RELEASE_SPIN_LOCK(&pArbiter->SpinLock, oldIrql);
#endif
...
//			KDPrintM(DBG_LURN_INFO, ("pArbiter->AcquiredLockCount %d\n", Arbiter->AcquiredLockCount));
			...
			status = DraidArbiterUpdateOnDiskOosBitmap(Arbiter);
			if (!NT_SUCCESS(status)) {
				KDPrintM(DBG_LURN_ERROR, ("Failed to update bitmap\n"));
			}
...
//			KDPrintM(DBG_LURN_TRACE, ("Setting bit %x:%x\n", BitOffset, NumberOfBit));
	...
	KDPrintM(DBG_LURN_INFO, ("Updated LWR bitmap with %d locks\n", LockCount));		
}
...
DraidArbiterUpdateOnDiskOosBitmap(
	...
	for(i=0;i<pArbiter->OosBmpByteCount/sizeof(ULONG);i++) {
		...
		if (*((PULONG)&pArbiter->OosBmpOnDiskBuffer[Sector].Bits[Offset]) == BitValues) {
			...
			KDPrintM(DBG_LURN_INFO, ("Bitmap offset %x changed from %08x to %08x\n", 
	...
	)
{
	...
	UINT32 UpToDateNode;
	UCHAR NodeFlags[MAX_DRAID_MEMBER_DISK] = {0};
	UCHAR NodeDefectCode[MAX_DRAID_MEMBER_DISK] = {0};
	...
	for(i = 0; i < Lurn->LurnChildrenCnt; i++)
	{
		...
		if (LurnGetCauseOfFault(Lurn->LurnChildren[i]) & (LURN_FCAUSE_BAD_SECTOR|LURN_FCAUSE_BAD_DISK)) {
			...
			NodeDefectCode[i] = (LurnGetCauseOfFault(Lurn->LurnChildren[i]) & LURN_FCAUSE_BAD_SECTOR)?
				DRIX_NODE_DEFECT_BAD_BLOCK:DRIX_NODE_DEFECT_BAD_DISK;
		}
		pArbiter->NodeFlags[i] |= Flags;
		pArbiter->DefectCodes[i] = NodeDefectCode[i];
...
//		KDPrintM(DBG_LURN_INFO, ("pArbiter->AcquiredLockCount %d\n",pArbiterInfo->AcquiredLockCount));		
	...
	ASSERT(InterlockedCompareExchange(&g_ArbiterLockCount, 0, 0)==0);
...
)
{
	...
	ACQUIRE_SPIN_LOCK(&pArbiterInfo->SpinLock, &oldIrql);
	if (pArbiterInfo->Status != DRAID_ARBITER_STATUS_RUNNING) {
...
DraidRebuildIoLockRangeAggressive(
	...
	if (RebuildInfo->RebuildLock) {		
		...
		if (DraidGetOverlappedRange(StartAddr, Length,
			...
			KDPrintM(DBG_LURN_INFO, ("Aggressive rebuild lock %I64x:%x does not cover range %I64x:%x\n",
				pArbiter->RebuildInfo.RebuildLock->LockAddress, 
				pArbiter->RebuildInfo.RebuildLock->LockLength,
				StartAddr, Length));
	...
	)
{
	...
	while(TRUE) {
		...
		if (RebuildInfo->RebuildRequested) {	
			...
			KDPrintM(DBG_LURN_INFO, ("Starting rebuild IO from %I64x to %I64x\n",
				CurAddr, EndAddr-1));
			RestInterval = 0;
			while(TRUE) {
...
//		KDPrintM(DBG_LURN_INFO, ("Rebuild thread is not ready for new request\n"));
	...
	KDPrintM(DBG_LURN_INFO, ("First set dirty bit: %x\n", BitToRecover));
...
VOID DraidRebuildIoAcknowledge(
	...
	if (RebuildStatus == DRAID_REBUILD_STATUS_DONE) {
		...
		// nothing to update
		KDPrintM(DBG_LURN_INFO, ("Rebuilding range %I64x:%x is failed\n", 
			Arbiter->RebuildInfo.Addr, Arbiter->RebuildInfo.Length));
	} else if (RebuildStatus == DRAID_REBUILD_STATUS_CANCELLED) {
		Arbiter->RebuildInfo.CancelRequested = FALSE;
		KDPrintM(DBG_LURN_INFO, ("Rebuilding range %I64x:%x is canceled\n", 
			Arbiter->RebuildInfo.Addr, Arbiter->RebuildInfo.Length));
	} else {
		KDPrintM(DBG_LURN_INFO, ("Rebuilding is not in progess. Nothing to acknowledge\n"));
		RELEASE_SPIN_LOCK(&Arbiter->SpinLock, oldIrql);
		return;
	}
	...
	RELEASE_SPIN_LOCK(&Arbiter->SpinLock, oldIrql);
	DraidArbiterUpdateLwrBitmapBit(Arbiter, NULL, NULL);
	DraidArbiterUpdateOnDiskOosBitmap(Arbiter);
} 
