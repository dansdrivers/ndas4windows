   ...
   )
{
	...
	LSS->TransStat.Retransmits = 0;
	LSS->TransStat.PacketLoss = 0;
	sendData = FALSE;
	recvData = FALSE;
	...
	// LspIdeSendRequest() might encrypt the request depending on the configuration.
	// Do not use this header before receiving the reply
	//
	pdu.pH2RHeader = (PLANSCSI_H2R_PDU_HEADER)pRequestHeader;
	pRequestHeader = NULL;

	ntStatus = LspIdeSendRequest_V11(LSS, &pdu, &LSS->NullOverlappedData, PduDesc->TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
	...
	if(sendData) {
		...
		LSS->BodyOverlappedData.CompletionEvent = NULL;
	...
	ntStatus = LspIdeReadReply_V11(LSS, (PCHAR)PduBuffer, &pdu, NULL, PduDesc->TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
	 ...
	 )
{
	...
	ntStatus = LspIdeSendRequest_V11(LSS, &pdu, NULL, TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
	...
	// Read Reply.
	ntStatus = LspIdeReadReply_V11(LSS, (PCHAR)LSS->PduBuffer, &pdu, NULL, TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
	...
	)
{
	...
	// Read Reply.
	ntStatus = LspIdeReadReply_V11(LSS, (PCHAR)PduBuffer, &pdu, NULL, TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(DBG_PROTO_ERROR, ("Can't Read Reply.\n"));
		return ntStatus;
	}
	...
	)
{
	...
	//
	// NOTE: LspIdeSendRequest() might encrypt the request depending on the configuration.
	// Do not use this header before receiving the reply
	//
	...
	if(sendData){
		...
		if(!NT_SUCCESS(ntStatus) 
			/*|| result != PduDesc->DataBufferBytesLength)*/) {
	...
	if(recvData) {
		...
		if(!NT_SUCCESS(ntStatus) 
			|| result != PduDesc->DataBufferLength) {
			KDPrintM(DBG_PROTO_ERROR, ("Error when Receive Data for READ, SMART, IDENTIFY, PIDENTIFY\n"));
			return STATUS_PORT_DISCONNECTED;
		}
	...
	case 0xaa:
		{
			...
			ntStatus = LspIdeSendRequest_V11(LSS, &pdu, NULL, PduDesc->TimeOut);
			if(!NT_SUCCESS(ntStatus)) {
...
LspIdeVendorRequest(
	...
	ntStatus = LspSendRequest(LSS, &pdu, &LSS->NullOverlappedData, PduDesc->TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(DBG_PROTO_ERROR, ("Error when Send Request\n"));
		return STATUS_PORT_DISCONNECTED;
	}
