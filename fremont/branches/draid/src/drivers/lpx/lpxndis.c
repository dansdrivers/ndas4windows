...
--*/
{
	...
	if (NdisStatus != NDIS_STATUS_SUCCESS) {
		...
		ASSERT( FALSE );
		//SetFlag( DeviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_CONNECTED );
		//return STATUS_INSUFFICIENT_RESOURCES;
		...
		if (ndisMediaState == NdisMediaStateConnected) {
			...
			SetFlag( DeviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_CONNECTED );
			ClearFlag( DeviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_DISCONNECTED );
		
		} else {

			ASSERT( ndisMediaState == NdisMediaStateDisconnected );

			SetFlag( DeviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_DISCONNECTED );
			ClearFlag( DeviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_CONNECTED );
		}

		DebugPrint( 0, ("DeviceContext = %x, ndisMediaState = %d\n", DeviceContext, ndisMediaState) );
	}
   ...
   UNREFERENCED_PARAMETER(StatusBufferSize);
    ...
    switch (NdisStatus) {
		...
		case NDIS_STATUS_MEDIA_CONNECT:
			...
			ACQUIRE_SPIN_LOCK( &DeviceContext->SpinLock, &oldirql2 );
			SetFlag( DeviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_CONNECTED );
			ClearFlag( DeviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_DISCONNECTED );
			RELEASE_SPIN_LOCK( &DeviceContext->SpinLock, oldirql2 );
