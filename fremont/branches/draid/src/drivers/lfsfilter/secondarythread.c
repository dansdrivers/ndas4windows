	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("SecondaryThreadProc: Start Secondary = %p\n", Secondary) );
		...
		if (eventStatus == 0) {
			...
			while (!FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_STOPED) && 
				...
				if (secondaryRequest->RequestType == SECONDARY_REQ_DOWN) {
					...
					SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("SecondaryThread SECONDARY_REQ_DOWN Secondary = %p\n", Secondary) );
