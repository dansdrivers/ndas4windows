	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary close SECONDARY_REQ_DISCONNECT Secondary = %p\n", Secondary) );
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary close SECONDARY_REQ_DISCONNECT end Secondary = %p\n", Secondary) );
	...
	)
{
	...
	while (1) {
		...
		if (redirectStatus == STATUS_DEVICE_REQUIRES_CLEANING) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE, ("redirectStatus == STATUS_DEVICE_REQUIRES_CLEANING\n") );
...
//		if(reconnectionTry >= 2) // first and second lookup primary address
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE, ("RecoverySession Completed. Secondary = %p, result = %d\n", Secondary, result) );
