	...
	)
{
	...
	try {
		...
		result = IsNetdiskPartition( DiskDeviceObject, &netdiskInformation, NULL );
...
if (IS_WINDOWS2K()) {
	...
	result = AddUserBacl( NetdiskPartition->NetdiskPartitionInformation.DiskDeviceObject,
						  &ndasBace,
						  BlockAceId );
	...
	)
{
	...
	*blockAceId = BlockAceId;
	...
	RemoveUserBacl( NetdiskPartition->NetdiskPartitionInformation.DiskDeviceObject,
					BlockAceId );
...
{	
	...
	enabledNetdisk = NetdiskManager_Lookup( NetdiskManager, NetDiskAddress, UnitDiskNo );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_INFO,
				   ("EnabledNetdisk_MountVolumeComplete: NetdiskPartition = %p\n", NetdiskPartition) );
	...
	)
{
	...
	if (netdiskPartition->NetdiskPartitionInformation.DiskDeviceObject && 
		IsNetdiskPartition(netdiskPartition->NetdiskPartitionInformation.DiskDeviceObject, &netdiskInformation, NULL)) {
	...
	while (1) {
		...
		if (!EnabledNetdisk_IsStoppedNetdisk(netdiskPartition->EnabledNetdisk) &&
			FlagOn(netdiskPartition->Flags, NETDISK_PARTITION_FLAG_MOUNT_SUCCESS)) {
	...
	)
{
	...
	if (NetdiskPartition->FileSystemType == LFS_FILE_SYSTEM_NDFAT ||
		NetdiskPartition->FileSystemType == LFS_FILE_SYSTEM_NDNTFS) {
