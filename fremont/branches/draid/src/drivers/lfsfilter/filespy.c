...
NTSTATUS
LfsLockVolumeCompletionRoutine (
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp,
    IN PVOID Context
)
{
	...
	PLFS_DEVICE_EXTENSION LfsDevExt = (PLFS_DEVICE_EXTENSION) Context;
	...
	if (Irp->IoStatus.Status  == STATUS_SUCCESS && 
		(fileSystemControl->FsControlCode == FSCTL_LOCK_VOLUME ||
		fileSystemControl->FsControlCode == FSCTL_UNLOCK_VOLUME)
	) {
		NETDISK_PARTITION_INFO	netdiskPartitionInfo = {0};

		RtlCopyMemory(
			&netdiskPartitionInfo.NetDiskAddress,
			&LfsDevExt->NetdiskPartitionInformation.NetdiskInformation.NetDiskAddress,
			sizeof(netdiskPartitionInfo.NetDiskAddress)
			);
		netdiskPartitionInfo.UnitDiskNo = LfsDevExt->NetdiskPartitionInformation.NetdiskInformation.UnitDiskNo;

if (IS_WINDOWS2K())
		netdiskPartitionInfo.StartingOffset = LfsDevExt->NetdiskPartitionInformation.PartitionInformation.StartingOffset;
else
		netdiskPartitionInfo.StartingOffset = LfsDevExt->NetdiskPartitionInformation.PartitionInformationEx.StartingOffset;


		if (fileSystemControl->FsControlCode == FSCTL_LOCK_VOLUME) {
			LfsDevExt->VolumeLocked = TRUE;
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
					("LfsLockVolumeCompletionRoutine: Removing locked volume from LFS table\n"));
		...
		} else if (fileSystemControl->FsControlCode == FSCTL_UNLOCK_VOLUME) {
			LfsDevExt->VolumeLocked = FALSE;
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
					("LfsLockVolumeCompletionRoutine: Inserting unlocked volume to LFS table\n"));

			if (LfsDevExt->NetdiskPartition)
			LfsTable_InsertNetDiskPartitionInfoUser(
					GlobalLfs.LfsTable,
					&netdiskPartitionInfo,
					&LfsDevExt->NetdiskPartitionInformation.NetdiskInformation.BindAddress,
					TRUE
					);
		} 
	} else {
		SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
					("LfsLockVolumeCompletionRoutine failed to lock/unlock %x\n", Irp->IoStatus.Status));
	}
...
--*/
{
	...
	// Set completion routine for volume lock/unlock to prevent advertize of the locked volume.
		...
		&& (irpSp->MinorFunction == IRP_MN_USER_FS_REQUEST || irpSp->MinorFunction == IRP_MN_KERNEL_CALL)) 
	{
		...
		if (fileSystemControl->FsControlCode == FSCTL_LOCK_VOLUME || 
			...
			// Set lock flag. If volume is lock, do not send broadcast.
			PIO_STACK_LOCATION irpSp2;

			irpSp2 = IoGetNextIrpStackLocation( (Irp) );
			SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO,
				("Primary_PassThrough: Lock/Unlock volume. Setting completion routine\n"));
			ASSERT(irpSp2->CompletionRoutine == NULL);

			if (irpSp2->CompletionRoutine == NULL) {
...
#else // Used to print debug message at completion routine
	if (SkipStack) {
...

{
	...
	if (!(devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDFAT	||
          devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDNTFS	||
	...
	if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDFAT && !GlobalLfs.NdasFatSupport ||
		devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDNTFS && !GlobalLfs.NdasNtfsSupport) {
    ...
    SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("NetdiskManager_IsNetdiskPartition status = %x\n", status) );
...
//        return SpyPassThrough(DeviceObject, Irp);
	...
	case NETDISK_READ_ONLY:
		...
		newDevExt->LfsDeviceExt.FilteringMode = LFS_READ_ONLY;
    ...
    )
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("SpyFsControlSecondaryMountVolumeComplete: Entered newLfsDeviceExt = %p %wZ newLfsDeviceExt->FileSystemType = %d\n", 
        ...
        if (!SpyIsAttachedToDevice( vpb->DeviceObject, &attachedDeviceObject )) {
            ...
            if (!NT_SUCCESS( status )) {
				...
				case LFS_READ_ONLY: {
					...
					SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("LfsFsControlMountVolumeComplete: READONLY PRIMARY newDevExt->LfsDeviceExt = %p Filtering:%d HookFastIo:%d Purge:%d.\n",
														 &newDevExt->LfsDeviceExt, newDevExt->LfsDeviceExt.Filtering, newDevExt->LfsDeviceExt.HookFastIo, newDevExt->LfsDeviceExt.Purge) );
				...
				case LFS_PRIMARY: {
					...
					SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO,
								   ("LfsFsControlMountVolumeComplete: READWRITE PRIMARY newDevExt->LfsDeviceExt = %p Filtering:%d HookFastIo:%d Purge:%d.\n",
									 &newDevExt->LfsDeviceExt, newDevExt->LfsDeviceExt.Filtering, newDevExt->LfsDeviceExt.HookFastIo, newDevExt->LfsDeviceExt.Purge) );
