	...
	if ((PVOID)NdasNtfsControlDeviceObject == VolumeDeviceObject) {
		...
		if (irpSp->Parameters.FileSystemControl.FsControlCode == NDAS_FS_UNLOAD) {
			...
			DebugTrace( 0, Dbg2, ("IoControlCode == NDAS_FS_UNLOAD occured DeviceObject->ReferenceCount = %x\n", 
								  NdNtfsFileSystemDeviceObject->ReferenceCount) );

			if(NtfsData.FileSystemRegistered == TRUE) {

				IoUnregisterFileSystem( NdNtfsFileSystemDeviceObject );
			...
			DebugTrace( 0, Dbg2, ("IoControlCode != ND_NTFS_SHUTDOWN\n", 0) );
...

{
    ...
    try {
		...
		KeInitializeEvent( &VolDo->ReferenceZero, NotificationEvent, FALSE );
		...
		if( ((PVOLUME_DEVICE_OBJECT)NdNtfsFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL ) {
			...
			queryStatus = ((PVOLUME_DEVICE_OBJECT)NdNtfsFileSystemDeviceObject)->
...
*/
		...
		if (VolDo->NetdiskEnableMode == NETDISK_SECONDARY) {
			...
			for (reconnectionTry = 0; reconnectionTry < MAX_RECONNECTION_TRY; reconnectionTry ++) {
				...
				status = ((PVOLUME_DEVICE_OBJECT)NdNtfsFileSystemDeviceObject)->
			...
			if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_DIRECT_RW)) {
				...
				if (((PVOLUME_DEVICE_OBJECT)NdNtfsFileSystemDeviceObject)->NdfsCallback.AddWriteRange(Vcb->Vpb->RealDevice, &VolDo->BlockAceId) != STATUS_SUCCESS) {
			...
			while (1) {
				...
				bacl = FsRtlAllocatePoolWithTag( NonPagedPool, baclLen, NDNTFS_ALLOC_TAG );
				...
				if (queryStatus == STATUS_BUFFER_OVERFLOW) {
					...
					baclLen = bacl->Length;
					ExFreePoolWithTag( bacl, NDNTFS_ALLOC_TAG );
					continue;
				}
...

{
    ...
    *BootScb = Scb;
    ...
    //
    //  Get out if we didn't get an error.  Otherwise try the slotIndexdle sector.
...
//            ASSERT( Scb->NonpagedScb->SegmentObject.SharedCacheMap == NULL );
        ...
        //
        //  Since we can't add zeroes in the slotIndexdle of the file (since we may not
