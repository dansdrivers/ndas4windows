	...
	case IRP_MJ_WRITE: { // 0x04
		...
		do {
			...
			if (!irp) {
				...
				ASSERT(NDNTFS_INSUFFICIENT_RESOURCES);
			...
			*majorFunction = IRP_MJ_WRITE;
			...
			if(fakeWrite == TRUE) {
				...
				ASSERT( FALSE );
				writeStatus = STATUS_UNSUCCESSFUL; //NdNtfsPrimaryDirectFsdWrite( PrimarySession->VolDo, irp );
			}
			else 
#endif
			{
		...
		if(PrimarySession->ExtendWinxpReplyMessagePool != NULL || writeStatus != STATUS_SUCCESS)
			DebugTrace( 0, Dbg,
						...
						openFileId, &openFile->FullFileName, openFile->CleanUp, openFile->DesiredAccess, writeStatus, ioStatusBlock.Status, (_U32)ioStatusBlock.Information) );
#endif
					...
					if (queryDirectoryStatus == STATUS_BUFFER_OVERFLOW)
						DebugTrace( 0, Dbg,
									("DispatchWinXpRequest: NtQueryDirectoryFile: openFileId = %X, queryDirectoryStatus = %X, length = %d, ioStatusBlock.Status = %X, ioStatusBlock.Information = %d\n",
									  openFileId, queryDirectoryStatus, length, ioStatusBlock.Status, (_U32)ioStatusBlock.Information));
		...
		if (openFileId == 0) {
			...
			do {
				...
				DebugTrace( 0, Dbg,
							("DispatchWinXpRequest: IRP_MJ_FILE_SYSTEM_CONTROL: openFileId = %x, function = %d, fileSystemControlStatus = %x, ioStatusBlock.Status = %x, ioStatusBlock.Information = %d\n",
							  openFileId, (fsControlCode & 0x00003FFC) >> 2, fileSystemControlStatus, ioStatusBlock.Status, (_U32)ioStatusBlock.Information));
			...
			if (NdfsWinxpReplytHeader->Status == STATUS_SUCCESS && fsControlCode == FSCTL_MOVE_FILE) {		// 29
				...
				if (typeOfOpen == UserFileOpen) {
					...
					NdfsWinxpReplytHeader->FileSize			= moveScb->Header.FileSize.QuadPart;
					NdfsWinxpReplytHeader->AllocationSize	= moveScb->Header.AllocationSize.QuadPart;
					NdfsWinxpReplytHeader->ValidDataLength	= moveScb->Header.ValidDataLength.QuadPart;
	...
	//	case IRP_MJ_INTERNAL_DEVICE_CONTROL:  // 0x0F 
	{
		...
		DebugTrace( 0, Dbg,
						("DispatchWinXpRequest: IRP_MJ_DEVICE_CONTROL: CtrlCode:%l08x openFileId = %X, deviceControlStatus = %X, ioStatusBlock.Status = %X, ioStatusBlock.Information = %d\n",
										ioControlCode, openFileId, deviceControlStatus, ioStatusBlock.Status, (_U32)ioStatusBlock.Information));
	...
	case IRP_MJ_LOCK_CONTROL: // 0x11
	{
		...
		DebugTrace( 0, Dbg,
					("DispatchWinXpRequest: NtLockFile: openFileId = %X, LockControlStatus = %X, ioStatusBlock.Status = %X, ioStatusBlock.Information = %d\n",
						openFileId, lockControlStatus, ioStatusBlock.Status, (_U32)ioStatusBlock.Information));
	...
	case IRP_MJ_QUERY_EA: {
		...
		DebugTrace( 0, Dbg,
					("DispatchWinXpRequest: ZwQueryEaFile: fileHandle = %p, ioStatusBlock->Information = %d, queryEaStatus = %x\n",
						openFile->FileHandle, (_U32)ioStatusBlock.Information, queryEaStatus) );
    ...
    case IRP_MJ_SET_EA: {
...
#if defined(_WIN64)
		ASSERT( openFile && openFile->OpenFileId == (_U64)openFile );
#else
		ASSERT( openFile && openFile->OpenFileId == (_U32)openFile );
#endif
		...
		DebugTrace( 0, Dbg,
					("DispatchWinXpRequest: ZwSetEaFile: fileHandle = %p, ioStatusBlock->Information = %d, setEaStatus = %x\n",
					  openFile->FileHandle, (_U32)ioStatusBlock.Information, setEaStatus) );
	...
	case IRP_MJ_QUERY_QUOTA: {
		...
		DebugTrace( 0, Dbg,
					("DispatchWinXpRequest: NtQueryQuotaInformationFile: fileHandle = %X, ioStatusBlock->Information = %X, queryQuotaStatus = %X\n",
					  openFile->FileHandle, (_U32)ioStatusBlock.Information, queryQuotaStatus) );
		...
		*replyDataSize = 0;
		...
		DebugTrace( 0, Dbg,
					("DispatchWinXpRequest: NtSetQuotaInformationFile: fileHandle = %X, ioStatusBlock.Information = %X, setQuotaStatus = %X\n",
						openFile->FileHandle, (_U32)ioStatusBlock.Information, setQuotaStatus));
...
#if defined(_WIN64)
			NdfsWinxpReplytHeader->Open.ScbHandle			= (_U64)scb;
#else
...
#if defined(_WIN64)
				NdfsWinxpReplytHeader->Open.Lcb.Scb			= (_U64)ccb->Lcb->Scb;
				//NdfsWinxpReplytHeader->Open.Lcb.Fcb			= (_U64)ccb->Lcb->Fcb;
#else
				NdfsWinxpReplytHeader->Open.Lcb.Scb			= (_U32)ccb->Lcb->Scb;
	...
	)
{
	...
	DebugTrace( 0, Dbg,
				...
				openFileId, synchronousIo, length, readStatus, byteOffset.QuadPart, ioStatusBlock.Status, (_U32)ioStatusBlock.Information) );
