...

{
	...
	if ((PVOID)NdNtfsFileSystemDeviceObject == VolumeDeviceObject) {
					...
					if (VolumeDeviceObject->Secondary && 
						...
						if (Status == STATUS_SUCCESS) {
							...
							try {
								...
								if (FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION)) {
									...
									ClearFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION );
									SetFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_OPEN_TRY_ON_SECONDARY );
								}
...

{
	...
	if (((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY || 
		((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY) {
			...
			Status = NdNtfsSecondaryCommonCreate( IrpContext, Irp, OplockCleanup, NULL, &doMore );	
		...
		if (NT_SUCCESS(Status) && (*ThisCcb) && (*ThisCcb)->TypeOfOpen == UserFileOpen) {
			...
			if ((*ThisScb)->PrimarySession != IrpContext->PrimaryRequestInfo.PrimarySession && (*ThisScb)->CloseCount != 1) {
				...
				if ((*ThisScb)->CleanupCount != 1) {
					...
					SetFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION );
			...
			if ((*ThisScb)->PrimarySession != 0)
				SetFlag( (*ThisCcb)->NdNtfsFlags, ND_NTFS_CCB_FLAG_OPEN_BY_PRIMARY_SESSION );
