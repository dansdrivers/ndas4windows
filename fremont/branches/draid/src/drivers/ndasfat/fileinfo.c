...

{
    ...
    try {
		...
		if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
			...
			Status = NdFatSecondaryCommonQueryInformation( IrpContext, Irp );
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdFatSecondaryCommonSetInformation( IrpContext, Irp );
...

{
    ...
    if (NodeType(Fcb) == FAT_NTC_FCB) {
        ...
        Buffer->EndOfFile.QuadPart = Fcb->Header.FileSize.LowPart;
    }
