...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		if (FlagOn(((PCCB)(IrpSp->FileObject->FsContext2))->NdFatFlags, ND_FAT_CCB_FLAG_UNOPENED)) {

			ASSERT( FlagOn(((PCCB)(IrpSp->FileObject->FsContext2))->NdFatFlags, ND_FAT_CCB_FLAG_CORRUPTED) );
    ...
    case IRP_MN_QUERY_DIRECTORY:
		...
		if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
			...
			Status = NdFatSecondaryQueryDirectory( IrpContext, Irp );
			break;
		}
#endif
