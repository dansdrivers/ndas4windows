...

{
    ...
    try {
		...
		if (NodeType(FcbOrDcb) == FAT_NTC_FCB) {
			...
			if (index * sizeof(NDFS_FAT_MCB_ENTRY) > DEFAULT_MAX_DATA_SIZE) 
				DbgBreakPoint();
			...
			if (FcbOrDcb->Header.AllocationSize.QuadPart == 0) {
				...
				if (newIndex * sizeof(NDFS_FAT_MCB_ENTRY) > DEFAULT_MAX_DATA_SIZE) 
					DbgBreakPoint();

				ASSERT( newVcn == NewAllocation );
		
				if ((index+newIndex) * sizeof(NDFS_FAT_MCB_ENTRY) > DEFAULT_MAX_DATA_SIZE) {
