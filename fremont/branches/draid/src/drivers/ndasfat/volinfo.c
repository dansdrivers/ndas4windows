...

{
    ...
    try {
		...
		if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
			...
			Status = NdFatSecondaryCommonQueryVolumeInfo( IrpContext, Irp );
			leave;
		}
#endif
        ...
        case FileFsVolumeInformation:
			...
			if (!(FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT) ? 
				FatAcquireSharedSecondaryVcb(IrpContext, Vcb) : FatAcquireSharedVcb(IrpContext, Vcb))) {
#else
...
#if __NDAS_FAT_SECONDARY__
		if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
			if (WeAcquiredVcb) { FatReleaseSecondaryVcb( IrpContext, Vcb ); }
		} else {
...

{
    ...
    try {
		...
		if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
			...
			Status = NdFatSecondaryCommonSetVolumeInfo( IrpContext, Irp );
			leave;
		}
#endif
