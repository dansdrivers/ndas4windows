	...
	case IRP_MJ_WRITE: { // 0x04
...
#if (defined __NDAS_FAT_DIRECT_ACCESS__ || defined __NDAS_FAT_DIRECT_READWRITE__)
		...
		do {
			...
			*majorFunction = IRP_MJ_WRITE;
...
#if __NDAS_FAT_DIRECT_READWRITE__
			...
			if (fakeWrite == TRUE) {
				...
				//Scb->Header.FileSize.LowPart = NdfsWinxpRequestHeader->Write.FileSize;
		...
		if (fakeWrite && NT_SUCCESS(writeStatus)) {
			...
			if (typeOfOpen == UserFileOpen) {
				...
				NdfsWinxpReplytHeader->FileSize = scb->Header.FileSize.LowPart;
				...
				DebugTrace2( 0, Dbg, 
							("write scb->Header.FileSize.LowPart = %I64x, scb->Header.ValidDataLength.QuadPart = %I64x\n", 
							scb->Header.FileSize.LowPart, scb->Header.ValidDataLength.QuadPart) );
				...
				while (FatLookupFatMcbEntry( &scb->Mcb, vcn, &lcn, &clusterCount, &startingLcn, NULL, NULL, NULL )) {
					...
					if ((index+1)*sizeof(NDFS_FAT_MCB_ENTRY) > PrimarySession->SessionContext.SecondaryMaxDataSize) 
						DbgBreakPoint();

					mcbEntry[index].Vcn = vcn; 

					ASSERT( ((_U16)lcn & (0x1FF)) == 0 );

					mcbEntry[index].Lcn = (_U32)(lcn >> vcb->AllocationSupport.LogOfBytesPerSector); 
...
#ifndef __NDAS_FAT_DIRECT_READWRITE__
		NdfsWinxpReplytHeader->CurrentByteOffset = fileObject->CurrentByteOffset.QuadPart;
#endif
		...
		if (NT_SUCCESS(setInformationStatus)) {
			...
			if (!(typeOfOpen == UserFileOpen &&
				...
				*ReplyDataSize = 0;
				...
				NdfsWinxpReplytHeader->FileSize			= fcb->Header.FileSize.LowPart;
		...
		if (openFile == NULL) {
			...
			if (NdfsWinxpReplytHeader->Status == STATUS_SUCCESS && fsControlCode == FSCTL_MOVE_FILE) {		// 29
				...
				if (typeOfOpen == UserFileOpen) {
					...
					NdfsWinxpReplytHeader->FileSize			= moveFcb->Header.FileSize.LowPart;
