...
GetScsiAdapterPdoEnumInfo(
	...
	//
	//
	BusInfo= (PBUSENUM_INFORMATION)ExAllocatePoolWithTag(NonPagedPool, BufferNeeded, NDSC_PTAG_IOCTL);
	status = IoctlToLanscsiBus(
						IOCTL_BUSENUM_QUERY_INFORMATION,
						&BusQuery,
						sizeof(BUSENUM_QUERY_INFORMATION),
	...
	*AddDevInfoFlags						= BusInfo->PdoEnumInfo.Flags;
	...
	KDPrint(1,("MaxRequestLength:%d DisconEventToService:%p AlarmEventToService:%p\n",
	...
	if(BusInfo->PdoEnumInfo.Flags & PDOENUM_FLAG_LURDESC) {
		...
		*AddDevInfoLength = LurDescLen;
		...
		//
		addTargetData = (PLANSCSI_ADD_TARGET_DATA)ExAllocatePoolWithTag(NonPagedPool, addTargetDataLength, NDSC_PTAG_ENUMINFO);
...
UpdatePdoInfoInLSBus(
	...
	//	Send to LSBus
	//
	busSet = (PBUSENUM_SETPDOINFO)ExAllocatePoolWithTag(NonPagedPool, sizeof(BUSENUM_SETPDOINFO), NDSC_PTAG_IOCTL);
	if(busSet == NULL) {
		return;
	}
	  ...
	  )
{
	...
	if(LSCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_LURN_DEGRADED))
	{
