...
#endif
		if(DoNotAdjustAccessRight) {
			((PLANSCSI_ADD_TARGET_DATA)AddDevInfo)->LurOptions |= LUROPTION_DONOT_ADJUST_PRIMSEC_ROLE;
		}
    ...
    )
{
	...
	KDPrint(1,("AdapterMaxDataTransferLength:%d DisconEventToService:%p AlarmEventToService:%p\n",
						HwDeviceExtension->AdapterMaxDataTransferLength,
    ...
    )
{
	...
	if(Srb->Cdb[0] == SCSIOP_SYNCHRONIZE_CACHE ||
		...
		//
		// cache synchronization command while entering hibernation.
		//
...
MiniStopWorker(
	...
	DisconEvent = (PKEVENT)NdscWorkItemCtx->Arg2;
