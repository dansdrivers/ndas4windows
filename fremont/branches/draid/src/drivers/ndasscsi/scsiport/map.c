...
Arguments:
    ...
    DeviceObject - the device object we are adding to the device map.
...

{
    ...
    if(commonExtension->IsPdo) {
        return SpBuildLogicalUnitDeviceMapEntry(DeviceObject->DeviceExtension);
    } else {
        return SpBuildAdapterDeviceMap(DeviceObject->DeviceExtension);
    }
}
    ...
    )
{
    ...
    Adapter->BusDeviceMapKeys = ExAllocatePoolWithTag(
                                    PagedPool,
    ...
    )
{
    ...
    if(commonExtension->IsPdo) {
        SpDeleteLogicalUnitDeviceMapEntry(DeviceObject->DeviceExtension);
    } else {
        SpDeleteAdapterDeviceMap(DeviceObject->DeviceExtension);
    }
    return;
}
