...
#undef __MODULE__
#endif // __MODULE__
#define __MODULE__ "NDSC_Internal"
...

{
    ...
    switch (srb->Function) {
        ...
        case SRB_FUNCTION_UNLOCK_QUEUE: {
            ...
            PSCSI_REQUEST_BLOCK currentLock;
            currentLock = InterlockedExchangePointer(
                            &(logicalUnit->LockRequest), 
                            srb);
            ...
            if(currentLock != NULL) {
                KeBugCheckEx(PORT_DRIVER_INTERNAL,
                             ...
                             (ULONG_PTR) logicalUnit->LockRequest,
                             (ULONG_PTR) currentLock);
            }
...

{
    ...
    if (deviceExtension->AllocateSrbExtension) {
        ...
        if(SpAllocateSrbExtension(deviceExtension,
            ...
            if (TEST_FLAG(logicalUnit->LuFlags, LU_PENDING_LU_REQUEST) &&
                !TEST_FLAG(deviceExtension->Flags, PD_PENDING_DEVICE_REQUEST)) {
                SpStartNextPacket(DeviceObject, FALSE);
            }
            return;
        }
    ...
    if (srb->SrbFlags & SRB_FLAGS_UNSPECIFIED_DIRECTION) {
        ...
        if (deviceExtension->MasterWithAdapter) {
            ...
            status = (*deviceExtension->DmaAdapterObject->DmaOperations->GetScatterGatherList)(
            ...
            if (!NT_SUCCESS(status)) {
                ...
                DebugPrint((0, "ScsiPortStartIo: IoAllocateAdapterChannel "
                               "failed(%x)\n",
                            status));
            ...
            if (Irp->MdlAddress) {
                ...
                if(systemAddress != NULL) {
                    ...
                    DebugPrint((0, "ScsiPortStartIo: Couldn't get system "
                                   "VA for irp %#08p\n", Irp));
    ...
    do {
        ...
        if(savedInterruptData.InterruptFlags &
           (PD_FLUSH_ADAPTER_BUFFERS |
            ...
            // If the adapter is being removd then don't do anything.
            //
            ...
            if(TEST_FLAG(savedInterruptData.InterruptFlags,
                ...
                InterlockedExchange(&(deviceExtension->ForceNextBusScan), TRUE);
        ...
        if (savedInterruptData.ReadyLogicalUnit != NULL) {
            ...
            for(logicalUnit = savedInterruptData.ReadyLogicalUnit;
                ...
                if (logicalUnit->ReadyLogicalUnit != NULL) {
...

{
    ...
    switch (irpStack->Parameters.DeviceIoControl.IoControlCode) {
        ...
        case IOCTL_SCSI_GET_INQUIRY_DATA: {
            ...
            SP_ENUMERATION_CONTEXT context;

            //
            // Spawn a worker thread to do the inquiry - we manipulate the 
            // registry during the scan using handles which are only valid 
            // in the system process context.
            //
            ...
            //
            // Return the inquiry data if necessary.
            //
        ...
        case IOCTL_SCSI_GET_DUMP_POINTERS: {
            ...
            if (Irp->RequestorMode != KernelMode) {
                ...
                dumpPointers->CommonBufferSize = deviceExtension->CommonBufferSize;
...

{
    ...
    KeAcquireSpinLockAtDpcLevel(&(logicalUnit->RequestSenseLock));
    ...
    if(logicalUnit->ActiveFailedRequest == NULL) {
        ...
        if(logicalUnit->BlockedFailedRequest != NULL) {
            KeBugCheckEx(PORT_DRIVER_INTERNAL,
                         6,
                         (ULONG_PTR) srbData,
                         (ULONG_PTR) logicalUnit->ActiveFailedRequest,
                         (ULONG_PTR) logicalUnit->BlockedFailedRequest);
        }
    ...
    KeReleaseSpinLockFromDpcLevel(&(logicalUnit->RequestSenseLock));
...

{
    ...
    KeAcquireSpinLock(&(LogicalUnit->RequestSenseLock), &oldIrql);
    ...
    KeReleaseSpinLock(&(LogicalUnit->RequestSenseLock), oldIrql);
...

{
    ...
    if (srbData->ScatterGatherList != NULL) {
        ...
        while (totalLength < srb->DataTransferLength) {
            ...
            scatterList->PhysicalAddress = IoMapTransfer(deviceExtension->DmaAdapterObject,
                                                         Irp->MdlAddress,
                                                         MapRegisterBase,
...
#if defined(_X86_) 
        if(deviceExtension->RemapBuffers) {
...

{
    ...
    srbData->ScatterGatherList = (PSRB_SCATTER_GATHER) ScatterGather->Elements;
...

{
    ...
    //
    // BUGBUG - grab the spinlock and check the queue to make sure it's really
    // busy. This is a hack to workaround an otherwise trivial problem in
    // scsiport.  If a request is returned with scsi BUSY, the device queue
    // busy flag may have already been cleared by the miniport requests a
    // get next lu request in the dispatch routine.  In this case, when the
    // busy request is reissued, SpStartIoSynchronized will set the LU_ACTIVE
    // flag assuming this request came out of the queue.  Unfortunately it did
    // not and get next lu request will foolishly come down here looking for
    // an active queue and assert on a checked build.
    //
    ...
    // HACKHACK -
    //
    // hackish workaround is to check the Busy flag in the device queue before
    // going to pull out of it.  Since we only pull out of the logical unit
    // queues when the port spinlock is held, there's no way for the flag to
    // get cleared if it's set while the lock is held here.
    //
...

{
    ...
    if (TEST_FLAG(srb->SrbFlags, SRB_FLAGS_UNSPECIFIED_DIRECTION)) {
        ...
        //
        // BUGBUG - this will not figure out if a miniport which did not 
        ...
        if ((DeviceExtension->MapBuffers) || (SrbData->RemappedMdl)) {
            ...
            if(SrbData->RemappedMdl) {
                ...
                remappedAddress = MmGetSystemAddressForMdl(
                                    SrbData->RemappedMdl);
...
#ifdef USE_DMA_MACROS
        (*DeviceExtension->DmaAdapterObject->DmaOperations->PutScatterGatherList)(
                              DeviceExtension->DmaAdapterObject,
    ...
    if ((srb->ScsiStatus == SCSISTAT_BUSY ||
        ...
        if ((TEST_FLAG(logicalUnit->LuFlags, LU_LOGICAL_UNIT_IS_BUSY)) &&
            ...
            if (!KeInsertByKeyDeviceQueue(
                ...
                //
                // This can occur if the drivers returns multple busy
                // requests with after they have indicated ready for
                // NextLuRequest.
                //
...

{
    ...
    } else if (TEST_FLAG(Srb->SrbFlags, SRB_FLAGS_QUEUE_ACTION_ENABLE) &&
               !TEST_FLAG(Srb->SrbFlags, SRB_FLAGS_DISABLE_DISCONNECT)) {
        //
...

{
    ...
    //
    // BUGBUG - we shouldn't allow a claim unless pnp has started the device
    // otherwise there's no way to disable them.
    //
...

{
    ...
    for (target = 0; target < NUMBER_LOGICAL_UNIT_BINS; target++) {
        ...
        for(logicalUnit = bin->List;
            ...
            if ((!(logicalUnit->LuFlags & LU_RESCAN_ACTIVE)) &&
    ...
    )
{
    ...
    if(Flush) {
...
#if DBG
        //
        // If the queue isn't busy then don't bother flusing it.
        //

        if(LogicalUnit->DeviceObject->DeviceQueue.Busy == FALSE) {
            listIrp = NULL;
            goto listDone;
        }
#endif
        ...
        while ((packet =
                KeRemoveDeviceQueue(&LogicalUnit->DeviceObject->DeviceQueue))
            != NULL) {
       ...
       if (providerDeviceObject && ((PCOMMON_EXTENSION)
          ...
          if (wmiMiniPortRequestCurrent->TypeOfRequest == WMIReregister) {
              ...
              if (pIOWorkItem) {
                  ...
                  // BUGBUG
                  // Need to check if we cant get the remove lock
                  //
                  ...
                  DebugPrint((0, "ScsiPortCompletionDPC: IoAllocateWorkItem failed for WmiRegistrationControl event\n"));
              }
              ...
              if (NT_SUCCESS(status)) {
                  ...
                  DebugPrint((0, "ScsiPortCompletionDPC: IoWMIWriteEvent failed\n"));
              }
          ...
          if (freeCell) {
              ...
              // BUGBUG - put the cell back onto the free list
              ExFreePool(wmiMiniPortRequestCurrent);
...
{
    PAGED_CODE();
    Context->Status = SpEnumerateAdapter(DeviceObject, FALSE);
    IoFreeWorkItem(Context->WorkItem);
    KeSetEvent(&(Context->Event), FALSE, IO_NO_INCREMENT);
    return;
}
