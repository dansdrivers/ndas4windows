...
#define NDFS_MAX_PATH					(255*sizeof(WCHAR))		
#define DEFAULT_MAX_DATA_SIZE			(1024*1024) // header is not included
#define DEFAULT_MIN_DATA_SIZE			(4*1024) // header is not included
//#define DEFAULT_NTFS_MAX_DATA_SIZE		(128*1024) // header is not included
//#define DEFAULT_NTFS_MIN_DATA_SIZE		(4*1024) // header is not included
...
#define NDFS_PROTOCOL_MAJOR_2	0x0002
...
typedef UNALIGNED struct _WINXP_REQUEST_CREATE {
	...
	struct {
		...
		struct {
			...
			_U32	Flags;
			_U32	RemainingDesiredAccess;
			_U32	PreviouslyGrantedAccess;
			_U32	OriginalDesiredAccess;
...
typedef UNALIGNED struct _WINXP_REQUEST_WRITE {
	...
	_U64	FileSize;
	_U64	ValidDataLength;
	_U64    VaildDataToDisk;
...
typedef UNALIGNED struct _WINXP_DUPLICATED_INFORMATION {
	...
	_S64	CreationTime;
	_S64	LastModificationTime;
	_S64	LastChangeTime;
    _S64	LastAccessTime;
	_S64	AllocatedLength;
	_S64	FileSize;
	_U32	FileAttributes;
...
typedef UNALIGNED struct _WINXP_REPLY_CREATE {
	...
	union {
		...
		struct {
			...
			_U32	SegmentNumberLowPart;       //  offset = 0x000
			_U16	SegmentNumberHighPart;      //  offset = 0x004
			_U16	SequenceNumber;             //  offset = 0x006
	...
	_U32	AttributeTypeCode;
	_U8		TypeOfOpen;
	
	_U8		Reserved1:4;
	_U8		TruncateOnClose:1;
	_U8		SetSectionObjectPointer:1;
	_U8		Reserved2:2;
...
typedef UNALIGNED struct _NDFS_WINXP_REPLY_HEADER {
	...
	_U8		FileInformationSet;

	_S64	FileSize;
	_S64	AllocationSize;
	_S64	ValidDataLength;
	_U32	NumberOfMcbEntry;
