...
typedef struct _NDAS_DIB_V2 {
	union{
		struct {
			...
			unsigned _int32	iMediaType; //  Byte 36. NDAS Media Type. NMT_*
...
//			unsigned _int8	AutoRecover; // Recover RAID automatically. Obsoleted
			...
			unsigned _int8		Reserved1[16]; // Byte 56
...
#define NDAS_UNIT_META_BIND_STATUS_DEFECTIVE	(1<<2) // disk has bad block or is reachable not working.
												// Do not attempt recovery until it is proven non-defective or user clears defective state
												//	or defective disk's RMD can be read but empty.
#define NDAS_UNIT_DEFECT_UNKNOWN	0x00			// 
#define NDAS_UNIT_DEFECT_BAD_BLOCK	0x01			// Disk has bad block. 
#define NDAS_UNIT_DEFECT_BAD_DISK	0x02			// May be bad disk or bad NDAS HW
	// Relative defects are not required to save to disk.
...
typedef struct _NDAS_UNIT_META_DATA {
	...
	unsigned _int8		DefectCode;		// If disk is in defect, record the reason. Used for bind tool to show information.
	unsigned _int8		Reserved[4];
