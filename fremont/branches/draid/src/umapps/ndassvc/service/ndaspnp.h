...
struct DeviceNotifyData
{
	...
	// constructors
	DeviceNotifyData(DataType Type, const NDAS_SCSI_LOCATION& Location) :
		Type(Type), NdasScsiLocation(Location) {}

	DeviceNotifyData(DataType Type, DWORD SlotNo) :
		Type(Type)
	{
		NdasScsiLocation.SlotNo = SlotNo;
		NdasScsiLocation.TargetID = 0;
		NdasScsiLocation.LUN = 0;
	}
