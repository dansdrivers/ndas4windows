...
struct EffectiveNdasAccess : std::unary_function<NDAS_SCSI_LOCATION,void> {
	...
	void operator()(const NDAS_SCSI_LOCATION& location) {
		...
		XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_INFORMATION, "Volume on (%d,%d,%d)\n", 
	...
	PDEV_BROADCAST_DEVICEINTERFACE pdbhdr)
{
	...
	do 
	{
		...
		{
			XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_ERROR, 
				"pGetNdasSlotNumberForScsiPortDeviceName(%ls) failed, error=0x%X\n",  
...
//	}
	...
	{    
		XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_ERROR, 
			"NdasEnumNdasScsiLocationsForVolume(hVolume %p) failed, error=0x%X\n", 
	...
	PDEV_BROADCAST_DEVICEINTERFACE pdbhdr)
{
	...
	{
		XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_WARNING, 
			"pGetNdasScsiLocationForDisk(%p) failed, error=0x%X\n", 
			hDisk, GetLastError());
		XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_WARNING, 
	...
	XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_INFORMATION,
		"NDAS Logical Device Slot No: %s.\n", 
		ndasScsiLocation.ToStringA());
	...
	PDEV_BROADCAST_DEVICEINTERFACE pdbhdr)
{
	...
	{
		XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_WARNING, 
			"pGetNdasScsiLocationForCDROM(%p) failed, error=0x%X\n", 
			hDevice, GetLastError());
		XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_WARNING, 
			"May not be NDAS logical device. Ignoring.\n");
		return TRUE;
	...
	PDEV_BROADCAST_HANDLE pdbch)
{
	...
	XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_INFORMATION, 
		"LogDeviceSlotNo %s, Type %s(0x%X)\n", 
		CNdasScsiLocation(notifyData.NdasScsiLocation).ToStringA(), 
