...
//			pReply->LogicalDiskInfo.Blocks = static_cast<DWORD>(blocks);
	...
	for (DWORD i = 0; i < nEntries; ++i) 
	{
		...
		{
			CNdasUnitDevicePtr registeredUnitDevice = pGetNdasUnitDevice(entry);
			if (registeredUnitDevice)
			{
				registeredUnitDevice->Lock();
				CNdasDevicePtr registeredDevice = registeredUnitDevice->GetParentDevice();
				registeredDevice->Lock();
				replyEntry.NdasIdExtension = registeredDevice->GetNdasIdExtension();
				registeredDevice->Unlock();
				registeredUnitDevice->Unlock();
			}
			else
			{
...
BOOL
CNdasCommandProcessor::ProcessCommandRequest<
	NDAS_CMD_QUERY_NDAS_SCSI_LOCATION::REQUEST>(
	NDAS_CMD_QUERY_NDAS_SCSI_LOCATION::REQUEST* pRequest, 
	DWORD cbExtraInData)
{
	NDAS_CMD_NO_EXTRA_DATA(cbExtraInData);
	...
	NdasCmdReply<NDAS_CMD_QUERY_NDAS_SCSI_LOCATION> reply;

	reply.NdasScsiLocation = pLogDevice->GetNdasScsiLocation();
...
#define CASE_PROCESS_COMMAND(cmdType, _CMD_STRUCT_) \
	...
	switch (m_command) {
		...
		CASE_PROCESS_COMMAND(NDAS_CMD_TYPE_QUERY_NDAS_SCSI_LOCATION,NDAS_CMD_QUERY_NDAS_SCSI_LOCATION)
	}
