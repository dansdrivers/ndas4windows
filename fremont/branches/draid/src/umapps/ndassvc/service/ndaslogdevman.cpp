	...
	CNdasLogicalDevicePtr pLogDevice)
{
	...
	XTLTRACE2(NDASSVC_NDASLOGDEVMANAGER, TRACE_LEVEL_INFORMATION, 
		"RegisterNdasScsiLocation %s of %s\n",
		CNdasScsiLocation(location).ToStringA(),
		pLogDevice->ToStringA());

	bool result = m_NdasScsiLocationMap.insert(
		std::make_pair(location,pLogDevice)).second;
	//
	// result can be false if RAID information is conflicting status .i.e. First RAID member is member of multiple RAID set.
	//
//	XTLASSERT(result);
	...
	CNdasLogicalDevicePtr pLogDevice)
{
	...
	XTLTRACE2(NDASSVC_NDASLOGDEVMANAGER, TRACE_LEVEL_INFORMATION, 
		"Unregistering NdasScsiLocation %s\n",
		CNdasScsiLocation(location).ToStringA());
	...
	if (m_NdasScsiLocationMap.end() == itr)
	{
		XTLTRACE2(NDASSVC_NDASLOGDEVMANAGER, TRACE_LEVEL_ERROR, 
			"NdasScsiLocation not registered at %s", 
			CNdasScsiLocation(location).ToStringA());
		return FALSE;
	}
