	...
	)
{
	...
	DebugPrint(1, _T("LsBusCtlPlugInEx2: Slot %d, MaxRequestLength %d bytes, hEvent %p, hAlarmEvent %p\n"), 
	...
	PNDSCIOCTL_ADAPTERLURINFO	*LurFullInfo) 
{
	NDASSCSI_QUERY_INFO_DATA			query;
	LONG						queryLen;
	NDSCIOCTL_ADAPTERLURINFO	tmpLurFullInfo;
	PNDSCIOCTL_ADAPTERLURINFO	lurFullInfo;
	LONG						infoLen;
	BOOL						bret;
	...
	*LurFullInfo = NULL;
	...
	queryLen				= sizeof(NDASSCSI_QUERY_INFO_DATA) - sizeof(UCHAR);
	query.Length			= queryLen;
	query.InfoClass			= NdscAdapterLurInformation;
	query.SlotNo			= SlotNo;
	query.QueryDataLength	= 0;
	bret = LsBusCtlQueryMiniportInformation(
					&query,
	...
	)
{
	...
	PBUSENUM_PLUGIN_HARDWARE_EX2	registerDev;	
	DWORD cbRegisterDev = 0;

	DebugPrint(1, _T("LsBusCtlRegisterDevice: Slot %d, MaxRequestBlock %d\n"), 
	...
	cbReturned = sizeof (BUSENUM_PLUGIN_HARDWARE_EX2) + 
		LSMINIPORT_HARDWARE_IDS_W_SIZE;
	...
	// and *not* the size of the struct plus the multi_sz
	//
    registerDev->Size = sizeof(BUSENUM_PLUGIN_HARDWARE_EX2);
	registerDev->SlotNo = SlotNo;
	registerDev->MaxOsDataTransferLength = MaxOsDataTransferLength;

	registerDev->HardwareIDLen = LSMINIPORT_HARDWARE_IDS_W_SIZE / sizeof(WCHAR);
	CopyMemory(
	...
	if (!fSuccess) {
		DebugPrintErrEx(_T("LsBusCtlRegisterDevice at slot %d failed: "), SlotNo);
	} else {
		DebugPrintErrEx(_T("LsBusCtlRegisterDevice at slot %d completed successfully.\n"), SlotNo);
	}
	...
	ULONG SlotNo)
{
	...
	LANSCSI_UNREGISTER_NDASDEV unregisterDev;
	...
	ZeroMemory(&unregisterDev, sizeof(LANSCSI_UNREGISTER_NDASDEV));
	unregisterDev.SlotNo = SlotNo;

    fSuccess = DeviceIoControl (
			hDevice,
			IOCTL_LANSCSI_UNREGISTER_DEVICE,
            &unregisterDev,
	...
	if (!fSuccess) {
		DebugPrintErrEx(_T("LsBusCtlUnregisterDevice at slot %d failed :"), SlotNo);
	} else {
		DebugPrint(3, _T("LsBusCtlUnregisterDevice at slot %d completed successfully.\n"), SlotNo);
	}
	...
	PLANSCSI_ADD_TARGET_DATA RegisterTargetData)
{
	...
	if (!fSuccess) {
		DebugPrintErrEx(_T("LsBusCtlAddTarget at slot %d failed :"), RegisterTargetData->ulSlotNo);
	} else {
		DebugPrint(3, _T("LsBusCtlAddTarget at slot %d completed successfully.\n"), RegisterTargetData->ulSlotNo);
	}
...
LSBUSCTLAPI BOOL WINAPI
LsBusCtlUnregisterTarget(DWORD SlotNo, DWORD TargetId)
{
	BOOL fSuccess = FALSE;
	HANDLE hDevice = INVALID_HANDLE_VALUE;
	DWORD cbReturned = 0;
	...
	LANSCSI_UNREGISTER_TARGET unregisterTargetData;
    ...
    fSuccess = DeviceIoControl(
			hDevice,
			IOCTL_LANSCSI_UNREGISTER_TARGET,
