...
pGetBoundDiskSize(int nBindType, int nDiskCount, NBUnitDevicePtrList &listUnitDevices)
{
	...
	return 
		...
		(NMT_RAID1R3 == nBindType) ? nSizeMin * nDiskCount /2 *99/100: // Reserve 1%. See NdasOpBind func.
		(NMT_RAID4R3 == nBindType) ? nSizeMin * (nDiskCount -1) : 0;
}
...
*/
	...
	m_pWizData->m_nBindType = 
		...
		(m_nBindType == 2) ? NMT_RAID1R3 : NMT_RAID4R3;
...
CCompletionPage::OnSetActive()
{
	...
	(NMT_RAID4R3 == m_pWizData->m_nBindType) ? IDS_LOGDEV_TYPE_DISK_RAID4R3 : IDS_LOGDEV_TYPE
		);
