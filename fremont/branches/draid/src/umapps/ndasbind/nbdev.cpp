...
CNBNdasDevice::CNBNdasDevice(PNDASUSER_DEVICE_ENUM_ENTRY pBaseInfo, NDAS_DEVICE_STATUS status)
{
	...
	NdasIdStringToDevice(pBaseInfo->szDeviceStringId, &m_DeviceId);
#if 0
...
CNBNdasDevice::CNBNdasDevice(LPCTSTR DeviceName, PNDAS_DEVICE_ID pDeviceId, NDAS_DEVICE_STATUS status)
{
	...
	NdasIdDeviceToString(pDeviceId, 
...
CNBUnitDevice* CNBNdasDevice::UnitDeviceAdd(PNDASUSER_UNITDEVICE_ENUM_ENTRY pBaseInfo, DWORD RaidFlags, UCHAR DefectCode)
{
	if(!pBaseInfo)
		return FALSE;
	...
	pUnitDevice->m_DefectCode = DefectCode;
...
CString CNBDevice::GetCapacityString()
{
	return GetCapacityString(GetLogicalCapacityInByte());
}
...
BOOL CNBUnitDevice::IsDefective()
{
	if(!IsGroup())
		return FALSE;
	if (GetRaidStatus() & NDAS_RAID_MEMBER_FLAG_DEFECTIVE)
		return TRUE;
	else
		return FALSE;
//	return NDAS_UNIT_META_BIND_STATUS_DEFECTIVE & m_RMD.UnitMetaData[m_cSequenceInRMD].UnitDeviceStatus;
}
...
BOOL CNBUnitDevice::GetCommandAbility(int nID)
{
	...
	case IDM_TOOL_REMOVE_FROM_RAID:
		if (!(NDASBIND_UNIT_DEVICE_STATUS_MOUNTED & GetStatus()) && IsFaultTolerant() && GetLogicalDevice()->DevicesSpare()) {
			// Available only for redundent RAID with spare disk. 
			return TRUE;
		}
		return FALSE;
	case IDM_TOOL_REPLACE_DEVICE:
		if (!(NDASBIND_UNIT_DEVICE_STATUS_MOUNTED & GetStatus()) && IsFaultTolerant() &&
			!(m_pLogicalDevice &&  m_pLogicalDevice->GetStatus() & NDASBIND_LOGICAL_DEVICE_STATUS_MOUNTED)) {
			//
			// Available only for redundent RAID. More check is required by OnReplaceDevice
			//
...
//		return (IsHDD() && IsOperatable());
	case IDM_TOOL_SPAREADD:
		return FALSE;
	case IDM_TOOL_RESET_TYPE:
		return FALSE;
	case IDM_TOOL_CLEAR_DEFECTIVE:
		if (IsOperatable() && (GetRaidStatus() & NDAS_RAID_MEMBER_FLAG_DEFECTIVE)) {
			return TRUE;
		}
		return FALSE;
	case IDM_TOOL_USE_AS_BASIC:
		if (IsFaultTolerant() && IsOperatable() && !IsMissingMember()) {
			return TRUE;
		}
		return FALSE;
...
UINT CNBUnitDevice::GetIconIndex(const UINT *anIconIDs, int nCount)
{
	...
	} else if (m_pLogicalDevice->IsGroup() && !NdasOpIsMemberUsable(m_RaidFlags)) {
...
CString CNBUnitDevice::GetRaidStatusString()
{
	...
	case NMT_RAID4R3: 
		...
		} else if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_DIFFERENT_CONFIG_SET) {
			strText.LoadString(IDS_RAID_STATUS_DIFF_CONFIG_SET);
		...
		} else if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_DEFECTIVE) {
			// RMD has defective code.
			if (m_DefectCode & NDAS_UNIT_DEFECT_BAD_DISK) {
				strText.LoadString(IDS_RAID_STATUS_BAD_DISK);
			} else if (m_DefectCode & NDAS_UNIT_DEFECT_BAD_BLOCK) {
				strText.LoadString(IDS_RAID_STATUS_BAD_SECTOR);
			} else {
				strText.LoadString(IDS_RAID_STATUS_UNKNOWN_DEFECT);
			}
		} else {
			// Normal status.
...
CString CNBUnitDevice::GetCommentString()
{
	...
	case NMT_RAID4R3:
		...
		if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_RMD_CORRUPTED) {
			strText.LoadString(IDS_RAID_COMMENT_RMD_CORRUPTED);
		} else if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_DIFFERENT_RAID_SET) {
			// Hard to happen??
			strText = _T("");
		} else if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_DIFFERENT_CONFIG_SET) {
			// Happens when spare disk is used.
			strText.LoadString(IDS_RAID_COMMENT_DIFF_CONFIG_SET);
		} else if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_IO_FAILURE) {
			strText.LoadString(IDS_RAID_COMMENT_IO_FAILURE);
		} else if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_DIB_MISMATCH) {
			strText.LoadString(IDS_RAID_COMMENT_DIB_MISMATCH);
		} else if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_DEFECTIVE) {
			strText.LoadString(IDS_RAID_COMMENT_DEFECTIVE);
		} else {
			// Normal status.
			if (m_RaidFlags & NDAS_RAID_MEMBER_FLAG_OUT_OF_SYNC) {
		...
		break;
	default:
		strText.LoadString(IDS_RAID_COMMENT_UNKNOWN_TYPE);
		break;
	}
	return strText;
}
...
BOOL CNBLogicalDevice::IsMember(CNBUnitDevice *pUnitDevice)
{
	...
	if (m_DIB.nDiskCount == pUnitDevice->m_DIB.nDiskCount &&
		...
		!memcmp(&m_ConfigSetId, &pUnitDevice->m_RMD.ConfigSetId, sizeof(GUID))) 
	{
		return TRUE;
	}
	return FALSE;
...
BOOL CNBLogicalDevice::UnitDeviceSet(CNBUnitDevice *pUnitDevice, UINT32 SequenceNum)
{
	...
	if (!m_mapUnitDevices.size()) {
		...
		// If this unit has more larger USN, then make this device as primary device.
...
CString CNBLogicalDevice::GetIDString(TCHAR HiddenChar)
{
	...
	{
		ATLASSERT(1 == m_mapUnitDevices.size() && m_mapUnitDevices.count(0));
		strText = AnyUnitDevice()->GetIDString(HiddenChar);
	}
...
UINT64 CNBLogicalDevice::GetLogicalCapacityInByte()
{
	UINT64 size = 0;
	if(!IsGroup())
		return AnyUnitDevice()->GetLogicalCapacityInByte();
	...
	case NMT_AGGREGATE:
		...
		break;
	case NMT_RAID0:
		size = AnyUnitDevice()->GetLogicalCapacityInByte() * DevicesInRaid();
		break;
	case NMT_MIRROR:
	case NMT_RAID1:
	case NMT_RAID1R2:
	case NMT_RAID1R3:		
		size = AnyUnitDevice()->GetLogicalCapacityInByte();
		break;
	case NMT_RAID4:
	case NMT_RAID4R2:
	case NMT_RAID4R3:		
		size = AnyUnitDevice()->GetLogicalCapacityInByte() * (DevicesInRaid() -1);
		break;
	default:
		ATLASSERT(FALSE);
	}
...
CString CNBLogicalDevice::GetRaidStatusString()
{
	...
	case NMT_RAID4R3: 
		if (m_RaidInfo.MountablityFlags & NDAS_RAID_MOUNTABILITY_UNMOUNTABLE) 
		{
			...
			else if (m_RaidInfo.FailReason & NDAS_RAID_FAIL_REASON_INDEPENDENT_UPDATE) 
			{
				strText.LoadString(IDS_RAID_STATUS_INDEPENDENT_UPDATE);
			}
...
CString CNBLogicalDevice::GetCommentString()
{
	...
	case NMT_RAID1: 
	case NMT_RAID1R2: 
		strText.LoadString(IDS_RAID_COMMENT_MIGRATE_MIRROR);	
		break;
	case NMT_RAID4: 
	case NMT_RAID4R2:
		strText.LoadString(IDS_RAID_COMMENT_MIGRATE_RAID4);
		break;
	case NMT_AGGREGATE: 
	case NMT_RAID0: 
	case NMT_RAID1R3: 
	case NMT_RAID4R3: 
		if (m_RaidInfo.MountablityFlags & NDAS_RAID_MOUNTABILITY_UNMOUNTABLE) 
		{
			...
			{
				strText.LoadString(IDS_RAID_COMMENT_INDEPENDENT_UPDATE);				
			}
			else if (m_RaidInfo.FailReason & NDAS_RAID_FAIL_REASON_RMD_CORRUPTED) 
			{
...
BOOL CNBLogicalDevice::GetCommandAbility(int nID)
{
	if (IsGroup()) {
		...
		case IDM_TOOL_USE_AS_MEMBER:
			return FALSE;
		case IDM_TOOL_SPAREADD:
			return (IsFaultTolerant() && !IsMigrationRequired() && IsOperatableAll() && IsHealthy());
		case IDM_TOOL_RESET_TYPE:
			return FALSE;
		case IDM_TOOL_CLEAR_DEFECTIVE:
			return FALSE;
...
BOOL CNBLogicalDevice::InitConnectionInfo(PNDASCOMM_CONNECTION_INFO ci, BOOL bWriteAccess)
{
	...
	return AnyUnitDevice()->InitConnectionInfo(ci, bWriteAccess);
}
