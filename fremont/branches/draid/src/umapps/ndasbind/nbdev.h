...
public:
	...
	CNBUnitDevice* UnitDeviceAdd(PNDASUSER_UNITDEVICE_ENUM_ENTRY pBaseInfo, DWORD RaidFlags = 0, UCHAR DefectCode = 0);
...
private:
	...
	UCHAR			m_DefectCode;	// ORed value of  NDAS_UNIT_DEFECT_*
...
public:
	...
	VOID SetRaidStatus(DWORD Status, UCHAR DefectCode=0) { m_RaidFlags = Status; m_DefectCode = DefectCode; }
...
//	DWORD GetFaultTolerance();
	...
	BOOL IsThisUnit(BYTE Node[6], BYTE Vid, DWORD UnitNo);
	BOOL IsThisDevice(BYTE Node[6], BYTE Vid);
...
private:
	NBUnitDevicePtrMap m_mapUnitDevices;
	NBNdasDevicePtrList m_listVirtualNdasDevices;
...
public:
	CNBLogicalDevice();
	BOOL UnitDeviceSet(CNBUnitDevice *pUnitDevice, UINT32 Sequence);
