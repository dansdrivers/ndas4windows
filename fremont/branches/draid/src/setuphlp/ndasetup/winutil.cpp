...
IsMsiUpgradeNecessary(ULONG ulReqMsiMinVer)
{
	...
	if (0 == dwRet || MAX_PATH < dwRet)
	{
		...
		XTLTRACE1(TRACE_LEVEL_INFORMATION,
			"Can't obtain system directory; assuming upgrade is necessary\n");
		return TRUE;
	}
	...
	if (FAILED(hr)) {
		...
		XTLTRACE1(TRACE_LEVEL_INFORMATION,
			"Can't obtain msi.dll path; assuming upgrade is necessary\n");
		return TRUE;
	}
	...
	if (FAILED(hr)) {
		// failure to get path to msi.dll; assume upgrade is necessary
		XTLTRACE1(TRACE_LEVEL_INFORMATION,
			"Can't get path to msi.dll; assuming upgrade is necessary\n");
		return TRUE;
	}
	...
	if (NULL == hinstMsiSys)
	{
		...
		XTLTRACE1(TRACE_LEVEL_INFORMATION,
			"Can't load msi.dll; assuming upgrade is necessary\n");
		return TRUE;
	}
	...
	if (ERROR_SUCCESS != dwRet)
	{
		...
		XTLTRACE1(TRACE_LEVEL_INFORMATION,
			"Can't obtain version information; assuming upgrade is necessary\n");
		return TRUE;
	}
	...
	if (ulInstalledVer < ulReqMsiMinVer)
	{
		...
		XTLTRACE1(TRACE_LEVEL_INFORMATION,
			"Windows Installer upgrade is required. "
	...
	XTLTRACE1(TRACE_LEVEL_INFORMATION,
		"No upgrade is necessary. System version meets minimum requirements\n");
	return FALSE;
}
...
{
	XTLTRACE1(TRACE_LEVEL_INFORMATION,
		_T("Running update package from --> %s\n"), szUpgradeMsi);
...
//
// C:\Windows\Downloaded Installations\NDAS\{PackageCode}\database.msi
// or 
// C:\Documents and Settings\{username}\Local Settings\Application Data\
// Downloaded Installations\NDAS\{Package Code}\database.msi
// 

static CONST TCHAR MSICACHE_DIRECTORY[] = _T("Downloaded Installations\\NDAS");
	...
	DWORD cchMax)
{
	...
	if (0 == nChars) {
		...
		XTLTRACE1(TRACE_LEVEL_ERROR,
			_T("GetFullPathName %s failed, error=0x%X\n"), 
			szMsiFile, GetLastError());
		return dwErr;
	}
	...
	if (0 == nChars) {
		DWORD dwErr = GetLastError();
		XTLTRACE1(TRACE_LEVEL_ERROR,
			_T("GetSystemWindowsDirectory failed, error=0x%X"),
			GetLastError());
		return dwErr;
	}
	...
	if (FAILED(hr)) {
		XTLTRACE1(TRACE_LEVEL_ERROR,
			"StringCchCat failed, hr=0x%X", hr);
...
pGetMsiPackageCode(LPCTSTR szMsiFile)
{
	...
	if (!fSuccess) {
		XTLTRACE1(TRACE_LEVEL_ERROR,
			"MSIAPI functions are not available.\n");
	...
	if (ERROR_SUCCESS != uiRet) {
		SetLastError(uiRet);
		XTLTRACE1(TRACE_LEVEL_ERROR,
			"MsiGetSummaryInformation failed, error=0x%X\n", GetLastError());
		return NULL;
	}
	...
	if (ERROR_SUCCESS == uiRet) {
		...
		SetLastError(uiRet);
		XTLTRACE1(TRACE_LEVEL_ERROR,
			"MsiSummaryInfoGetProperty PID_REVNUMBER failed, error=0x%X\n", GetLastError());
		return NULL;
	}
	...
	if (ERROR_SUCCESS != uiRet) {
		XTLTRACE1(TRACE_LEVEL_ERROR,
			"SummmaryInfoGetProperty PID_REVNUMBER failed for %d chars, error=0x%X\n", 
			cchMax, GetLastError());
		::GlobalFree((HGLOBAL)szPackageCode);
		return NULL;
	}
...
UINT
CacheMsi(ISetupUI* pSetupUI, LPCTSTR szMsiFile, LPTSTR szCachedMsiFile, DWORD cchMax)
{
