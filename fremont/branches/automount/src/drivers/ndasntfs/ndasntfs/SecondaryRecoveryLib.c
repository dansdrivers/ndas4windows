...

{
    ...
    DebugTrace( -1, Dbg, ("NdNtfsDeleteVcb -> VOID\n") );
		...
		if ((Vcb->CleanupCount > 0) ||
            ...
            if ((Vcb->CloseCount - (Vcb->SystemFileCloseCount + Vcb->QueuedCloseCount)) > 0) {
				...
				if (NT_SUCCESS( Status )) {
					...
					NDASNTFS_ASSERT( Vcb->CloseCount == 0 );
			...
			if (NT_SUCCESS( Status )) {
				...
				VcbDeleted = NdNtfsDeleteVcb( IrpContext, &Vcb );
				NDASNTFS_ASSERT( VcbDeleted == TRUE );
			}
		}
...

{
    ...
    RtlZeroMemory( Vcb, FIELD_OFFSET(VCB, NdNtfsFlags) );
...

{
    ...
    try {
        ...
        NdNtfsInitializeVcb( IrpContext, Vcb, DeviceObjectWeTalkTo, Vpb );
        NtfsAcquireExclusiveVcb( IrpContext, Vcb, TRUE );
        VcbAcquired= TRUE;
