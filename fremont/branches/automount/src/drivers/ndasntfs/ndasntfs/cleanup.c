...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdNtfsSecondaryCommonCleanup( IrpContext, Irp );
	...
	if (FlagOn(FileObject->Flags, FO_CLEANUP_COMPLETE)) {
		...
		if (FlagOn(Ccb->NdNtfsFlags, ND_NTFS_CCB_FLAG_OPEN_BY_PRIMARY_SESSION)) {
                ...
                if (Status == STATUS_SUCCESS) {
                    ...
                    if ((Scb->CleanupCount == 1) &&
                        ...
                        //
							...
							if(TypeOfOpen == UserFileOpen && Scb->PrimarySession && Scb->CleanupCount == 1) {

								if (IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader == NULL ) {
									...
									ASSERT( IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.FileSize );   

									ASSERT( Scb->Header.FileSize.QuadPart == IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.FileSize );
									ASSERT( Scb->Header.AllocationSize.QuadPart == IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.AllocationSize );
									
									Scb->Header.ValidDataLength.QuadPart = IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength;

									DebugTrace( 0, Dbg, ("Scb->Header.ValidDataLength.QuadPart = %d\n", Scb->Header.ValidDataLength.QuadPart) );
