...
#if __NDAS_NTFS_DBG__
					ASSERT( FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_FILE) );
#endif				
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdNtfsSecondaryCommonFileSystemControl( IrpContext, Irp );
		return Status;
	}
...

{
    ...
    try {
		...
		SetFlag( VolDo->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_INITIALIZING );
		...
		DebugTrace( 0, Dbg2, ("NdNtfsMountVolume: VolDo = %p\n", VolDo) );		
		DebugTrace( 0, Dbg2, ("NdNtfsMountVolume QueryNetdiskInformation \n") );
		...
		if( ((PVOLUME_DEVICE_OBJECT)NdasNtfsFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL ) {
			...
			if (queryStatus != STATUS_SUCCESS) {
				...
				DebugTrace( 0, Dbg2, ("NdNtfsMountVolume %08lx QueryNetdiskInformation \n", queryStatus) );
				ASSERT( NDASNTFS_BUG );
				...
				if (FlagOn(VolDo->NetdiskPartitionInformation.NetdiskInformation.EnabledFeatures, NDASFEATURE_SIMULTANEOUS_WRITE))
					SetFlag( VolDo->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_DIRECT_RW );
...
*/
		...
		if (VolDo->NetdiskEnableMode == NETDISK_SECONDARY &&
			...
			if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_DIRECT_RW) &&
			...
			ClearFlag( VolDo->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_INITIALIZING );
			SetFlag( VolDo->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_MOUNTED );
		}
	...
	case FSCTL_NDAS_FS_FLUSH_OR_PURGE: {
		...
		if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_READ_ONLY) {
			...
			if (1) { //IrpContext->PrimaryRequestInfo.PrimaryTag != 0) {
				...
				SetFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_PURGE );
			...
			SetFlag( IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_FLUSH );
