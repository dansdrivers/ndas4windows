...
#define NtfsInsertFcbTableEntry(IC,V,F,FR) {								\
    ...
    _Key.Fcb = (F);															\
    if (FlagOn((F)->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY))				\
...

{
	...
	if (FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) 
        ...
        *ReturnedExistingFcb = FALSE;
        ...
        try {
			...
			if (FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) {
				...
				SetFlag( Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY );
			}
#endif				
...
#if __NDAS_NTFS_SECONDARY__
		if (FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) 
			if (FlagOn(Scb->NdNtfsFlags, ND_NTFS_SCB_FLAG_CORRUPTED)) 
				continue;
#endif
...
#if __NDAS_NTFS_SECONDARY__
		if ((!(FlagOn(Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY)) ? (Scb == Lcb->Scb) : (Scb == Lcb->SecondaryScb))
#else
        if ((Scb == Lcb->Scb)
...

{
    ...
    try {
        ...
        if (Fcb->CleanupCount == 0) {
			...
			//DebugTrace( 0, Dbg2, ("FcbCanBeRemoved = %d\n", FcbCanBeRemoved) );
...
#if __NDAS_NTFS_SECONDARY__
            if (NtfsIsExclusiveVcb( Fcb, Fcb->Vcb )) {
#else
...
#if __NDAS_NTFS_SECONDARY__
	if (!FlagOn(Scb->Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY) || Scb->CloseCount == 0 )
#endif		
