	...
	)
{
	...
	if (FlagOn(volDo->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {
		...
		DebugTrace( 0, Dbg, ("NdNtfsSecondaryCommonCreate: NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT FileName = %wZ\n", 
							  &fileObject->FileName) );
	...
	if (FlagOn(IrpContext->Vcb->VcbState, VCB_STATE_TARGET_DEVICE_STOPPED)) {
		...
		DebugTrace( 0, Dbg2, ("NdNtfsSecondaryCommonCreate: VCB_STATE_TARGET_DEVICE_STOPPED FileName = %wZ\n", &fileObject->FileName) );
	...
	if (FlagOn(volDo->Secondary->Flags, SECONDARY_FLAG_DISMOUNTING)) {
		...
		DebugTrace( 0, Dbg2, ("NdNtfsSecondaryCommonCreate: SECONDARY_FLAG_DISMOUNTING FileName = %wZ\n", &fileObject->FileName) );

		//NDASNTFS_ASSERT( fileObject->FileName.Length == 2 );

		*DoMore = TRUE;
		return STATUS_SUCCESS;
	}

	DebugTrace( 0, Dbg, ("NdNtfsSecondaryCommonCreate: FileName = %wZ\n", &fileObject->FileName) );
