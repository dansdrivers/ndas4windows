	...
	)
{
	...
	while (volDoThreadTerminate == FALSE) {
		...
		if (eventStatus == STATUS_TIMEOUT) {
			...
			if (FlagOn(VolDo->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_SHUTDOWN) || 
				!(FlagOn(VolDo->NdNtfsFlags, ND_NTFS_DEVICE_FLAG_MOUNTED) /*&& !FlagOn(LfsDeviceExt->Flags, LFS_DEVICE_STOP)*/)) {
