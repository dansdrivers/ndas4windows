...

{
    ...
    try {
        ...
        case FileBasicInformation:
            ...
            Status = NdNtfsSetBasicInfo( IrpContext, FileObject, Irp, Scb, Ccb );
            break;

        case FileDispositionInformation:

            Status = NdNtfsSetDispositionInfo( IrpContext, FileObject, Irp, Scb, Ccb );
            break;

        case FileRenameInformation:

            Status = NdNtfsSetRenameInfo( IrpContext, FileObject, Irp, Vcb, Scb, Ccb, &VcbAcquired );
            break;

        case FilePositionInformation:

            Status = NdNtfsSetPositionInfo( IrpContext, FileObject, Irp, Scb );
            break;
        ...
        case FileLinkInformation:
            ...
            //Status = NdNtfsSetLinkInfo( IrpContext, Irp, Vcb, Scb, Ccb, &VcbAcquired );
            break;
        ...
        case FileAllocationInformation:
            ...
            if (TypeOfOpen == UserDirectoryOpen ||
                ...
                Status = NdNtfsSetAllocationInfo( IrpContext, FileObject, Irp, Scb, Ccb );
            }
        ...
        case FileEndOfFileInformation:
            ...
            if (TypeOfOpen == UserDirectoryOpen ||
                ...
                Status = NdNtfsSetEndOfFileInfo( IrpContext, FileObject, Irp, Scb, Ccb, VcbAcquired );
            }
        ...
        case FileValidDataLengthInformation:
            ...
            Status = NdNtfsSetValidDataLengthInfo( IrpContext, Irp, Scb, Ccb );
            break;
        ...
        case FileShortNameInformation:
            ...
            if (Scb->Header.NodeTypeCode == NTFS_NTC_SCB_ROOT_INDEX) {
				...
				ASSERT( FALSE );
				Status = STATUS_INVALID_PARAMETER;
                //Status = NdNtfsSetShortNameInfo( IrpContext, FileObject, Irp, Vcb, Scb, Ccb );
            }
            break;
    ...
    try {
		...
		if (IsDirectory(&Scb->Fcb->Info)) {
			...
			if(setFile.FileObject) {
				...
				if(setFileCcb == NULL || !FlagOn(((PSCB)(setFile.FileObject->FsContext))->Fcb->NdNtfsFlags, ND_NTFS_FCB_FLAG_SECONDARY)) {

					ASSERT(NDASNTFS_BUG);
					try_return( Status = STATUS_IO_DEVICE_ERROR );
				}

				if(FlagOn(setFileCcb->NdNtfsFlags, ND_NTFS_CCB_FLAG_UNOPENED)) {

					ASSERT( FlagOn(setFileCcb->NdNtfsFlags, ND_NTFS_CCB_FLAG_CORRUPTED) );
        ...
        if (Buffer->DeleteFile) {
            ...
            if (FileHandle != NULL) {
                ...
                if (!NT_SUCCESS( Status )) {
                    ...
                    DebugTrace( 0, Dbg2, ("Object manager fails to recognize handle\n") );
...
#else
#if 1				
                    if (!IsDirectory( &Scb->Fcb->Info ) || NtfsIsLinkDeleteable( IrpContext, Scb->Fcb, &NonEmptyIndex, &LastLink )) {
#else
...

{
    ...
    try {
		...
		if (NT_SUCCESS(Status) && setFile.FileObject) {
			...
			DebugTrace( 0, Dbg2, ("NdNtfsSetRenameInfo: fullpathName = %Z\n", &fullPathName) );
...

{
    ...
    if (NewAllocationSize < Scb->Header.FileSize.QuadPart) {
        ...
        Status = NdNtfsPrepareToShrinkFileSize( IrpContext, FileObject, Scb, NewAllocationSize );
        if (Status != STATUS_SUCCESS) {
...

{
    ...
    if (IoGetCurrentIrpStackLocation(Irp)->Parameters.SetFile.AdvanceOnly) {
        ...
        if (NewFileSize < Scb->Header.FileSize.QuadPart) {
            ...
            Status = NdNtfsPrepareToShrinkFileSize( IrpContext, FileObject, Scb, NewFileSize );
            if (Status != STATUS_SUCCESS) {
