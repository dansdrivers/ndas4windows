...

{
    ...
    try {
		...
		if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
			...
			Status = NdNtfsSecondaryCommonQueryInformation( IrpContext, Irp );
			leave;
		}
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdNtfsSecondaryCommonSetInformation( IrpContext, Irp );
...

{
    ...
    if (NtfsBreakOnCorrupt &&
        ((Status == STATUS_FILE_CORRUPT_ERROR) ||
		 ...
		 if(Status == STATUS_FILE_CORRUPT_ERROR && FlagOn(IrpContext->NdNtfsFlags, ND_NTFS_IRP_CONTEXT_FLAG_SECONDARY_FILE))
			 ASSERT( FALSE );
#endif
