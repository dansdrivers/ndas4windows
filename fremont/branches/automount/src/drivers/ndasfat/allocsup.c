...

{
    ...
    *EndOnMax = FALSE;
...
#if __NDAS_FAT_SECONDARY__
	{
		...
		NDASFAT_ASSERT( FlagOn(volDo->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) ||
						volDo->NetdiskEnableMode != NETDISK_SECONDARY || 
						!FlagOn(volDo->NdFatFlags, ND_FAT_DEVICE_FLAG_MOUNTED) );
	}
#endif
