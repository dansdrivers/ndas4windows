...

{
	...
	if (IS_WINDOWSXP_OR_LATER()) { // to prevent incomprehensible error
		...
		NdFatCcMdlWriteAbort = MmGetSystemRoutineAddress( &functionName );
	}
	...
	NdFatFsRtlRegisterFileSystemFilterCallbacks = MmGetSystemRoutineAddress( &functionName );
	...
	if (NdFatFsRtlRegisterFileSystemFilterCallbacks) {
	    ...
	    Status = NdFatFsRtlRegisterFileSystemFilterCallbacks( DriverObject,
															  &FilterCallbacks );
