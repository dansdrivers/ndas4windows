			...
			if (VolumeDeviceObject->Secondary && 
				...
				if (Status == STATUS_SUCCESS) {
					...
					try {
						...
						if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION)) {
							...
							ClearFlag( IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION );
							SetFlag( IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_OPEN_TRY_ON_SECONDARY );
						}
...

{
	...
	if (!FlagOn(((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {
		...
		if (((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY || 
			...
			Status = NdFatSecondaryCommonCreate( IrpContext, Irp, &doMore );	
		...
		if (Status == STATUS_SUCCESS) {
			...
			try {
				...
				if (TypeOfOpen == UserFileOpen) {
					...
					if (ThisFcb->PrimarySession != IrpContext->PrimaryRequestInfo.PrimarySession && ThisFcb->OpenCount != 1) {

						//ASSERT( FALSE );

						if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW))
							ASSERT( ThisFcb->OpenCount > 1 && ThisFcb->UncleanCount >= 1 );

						if (ThisFcb->UncleanCount != 1) {
					
							SetFlag( IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION );
							IrpContext->FileOpenPrimarySession = ThisFcb->PrimarySession;
					...
					ASSERT( ThisFcb->PrimarySession == IrpContext->PrimaryRequestInfo.PrimarySession || 
							ThisFcb->OpenCount == 1 || 
							unclosedCcbCount == 1 );

					ThisFcb->PrimarySession = IrpContext->PrimaryRequestInfo.PrimarySession;
			
					if (ThisFcb->PrimarySession != 0) {

						SetFlag( ThisCcb->NdFatFlags, ND_FAT_CCB_FLAG_OPEN_BY_PRIMARY_SESSION );
...
#if __NDAS_FAT_SECONDARY__
			if (FlagOn(IrpContext->NdFatFlags, ND_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
				FatReleaseSecondaryVcb( IrpContext, LocalVcb );
			} else {
