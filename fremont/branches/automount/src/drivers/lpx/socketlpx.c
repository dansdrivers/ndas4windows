	...
	)
{
	...
	if (RtlEqualMemory (notAssigned, &NetworkName->Node, HARDWARE_ADDRESS_LENGTH) && 
	...
	for (thisEntry = listHead->Flink;
		...
		if (deviceContext != SocketLpxDeviceContext && 
			...
			if (RtlEqualMemory(deviceContext->LocalAddress.Address,
							   &NetworkName->Node,
							   HARDWARE_ADDRESS_LENGTH)) {
			   ...
			   if (!(FlagOn(deviceContext->LpxFlags, LPX_DEVICE_CONTEXT_START) && 
					...
					deviceContext = NULL;
#if 0			   
			   } else if (!(FlagOn(deviceContext->LpxFlags, LPX_DEVICE_CONTEXT_MIDIA_CONNECTED) && 
