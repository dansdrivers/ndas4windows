    ...
    switch(irpStack->MinorFunction) {
        ...
        case IRP_MN_QUERY_DEVICE_RELATIONS: {
            ...
            if(type == BusRelations) {
                ...
                status = SpEnumerateAdapter(DeviceObject, FALSE);
                ...
                if(NT_SUCCESS(status)) {
                    ...
                    status = SpExtractDeviceRelations(
                                DeviceObject,
                                irpStack->Parameters.QueryDeviceRelations.Type,
                                &deviceRelations);
                }
                ...
                if(NT_SUCCESS(status)) {
                    DebugPrint((2, "QUERY_DEVICE_RELATIONS: Found %d devices on fdo "
                                   "%#p\n",
                                   deviceRelations->Count,
                                   DeviceObject));
    
                    for(i = 0; i < deviceRelations->Count; i++) {
                        DebugPrint((2, "/t#%2d: device %#p\n", i, deviceRelations->Objects[i]));
                    }
                } else {
        ...
        case IRP_MN_DEVICE_USAGE_NOTIFICATION: {
            ...
            //
            // BUGBUG - make sure it's okay to send the irp down.
            //
...

{
    ...
    if (adapterExtension->CommonExtension.WmiInitialized == FALSE) {
       ...
       if (adapterExtension->CommonExtension.WmiMiniPortSupport ||
           adapterExtension->CommonExtension.WmiScsiPortRegInfoBuf) {
    ...
    InterlockedExchange(&(adapterExtension->ForceNextBusScan), TRUE);
...

{
    ...
    //
    // BUGBUG - what about vendor specific id's?  Check that the dt is in
    // valid range
    //
...

{
    ...
    //
    // BUGBUG - use serial number if it exists
    //
...

{
    ...
    return ScsiPortStringArrayToMultiString(UnicodeString, stringBuffer);
}
...

{
    ...
    try {
        ...
        status = ScsiPortStringArrayToMultiString(UnicodeString,
                                                  strings);
        leave;
...

{
    ...
    try {
        ...
        configInfo = ExAllocatePoolWithTag(NonPagedPool,
                                           portConfigSize,
    ...
    )
{
    ...
    irp = IoAllocateIrp((CCHAR) (Adapter->DeviceObject->StackSize + 1), FALSE);
