...
SendMessage(
	...
	for(remaining = Size; remaining > 0;  ) 
	{
		...
		if(TransCtx) {
			...
			queryStatus = LpxTdiQueryInformation(
							ConnectionFileObject,
							LPXTDI_QUERY_CONNECTION_TRANSSTAT,
							&onceTransStat,
							sizeof(onceTransStat));
			if(NT_SUCCESS(queryStatus)) {
				TransCtx->AccSendTransStat.Retransmits += onceTransStat.Retransmits;
				TransCtx->AccSendTransStat.PacketLoss += onceTransStat.PacketLoss;
			} else {
				onceTransStat.Retransmits = 0;
			}
...
BOOLEAN
GetPrimaryUnitDisk(
	IN	PDEVICE_OBJECT				ScsiPortDeviceObject,
...
AddUserBacl(
	...
	if (status != STATUS_SUCCESS) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("AddUserBacl: GetScsiportAdapter() failed.\n") );
		return FALSE;
	}

	outbuff_sz = sizeof(SRB_IO_CONTROL) + sizeof(NDASSCSI_QUERY_INFO_DATA) + sizeof(NDAS_BLOCK_ACE);
	psrbioctl = (PSRB_IO_CONTROL)ExAllocatePoolWithTag(NonPagedPool , outbuff_sz, LFS_ALLOC_TAG);
	...
	)
{
	...
	if (status != STATUS_SUCCESS) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("RemoveUserBacl: GetScsiportAdapter() failed.\n") );
		return status;
	}

	outbuff_sz = sizeof(SRB_IO_CONTROL) + sizeof(NDASSCSI_QUERY_INFO_DATA) + sizeof(BLOCKACE_ID);
	psrbioctl = (PSRB_IO_CONTROL)ExAllocatePoolWithTag(NonPagedPool , outbuff_sz, LFS_ALLOC_TAG);
	...
	*blockAceId = BlockAceId;
	...
	status = LfsFilterDeviceIoControl( scsiportDeviceObject,
									   IOCTL_SCSI_MINIPORT,
									   psrbioctl,
									   outbuff_sz,
									   psrbioctl,
...
NdasScsiQuery(
	...
	if(OutBufferLen < sizeof(NDASSCSI_QUERY_INFO_DATA)) {
		return STATUS_BUFFER_TOO_SMALL;
	}

	srbioctlbuff_sz = sizeof(SRB_IO_CONTROL) + OutBufferLen;
	...
	) 
{
	...
	returnResult = GetPrimaryUnitDisk( *ScsiportDeviceObject,
									   &primUnitDisk );


	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("IsNetDisk: GetPrimaryUnitDisk returnResult = %d\n", returnResult) );
	...
	if (returnResult == TRUE && NetdiskInformation) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR, ("NdscId: %x:%x:%x:%x\n", 
		...
		RtlCopyMemory( NetdiskInformation->UserId,
					   primUnitDisk.UnitDisk.UserID,
					   sizeof(NetdiskInformation->UserId) );
		
		RtlCopyMemory( NetdiskInformation->Password,
		...
		RtlCopyMemory( &NetdiskInformation->BindAddress, 
					   &primUnitDisk.UnitDisk.BindingAddress.Address[0].Address, 
					   sizeof(LPX_ADDRESS) );
...
)
{
	...
	if(!NT_SUCCESS(ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlOpenLanScsiBus: IoGetDeviceInterfaces ntStatus = 0x%x\n", ntStatus));
		return ntStatus;
	}
	...
	if(!NT_SUCCESS(ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlOpenLanScsiBus: ntStatus = 0x%x\n", ntStatus));
...
) 
{
	...
	PIRP				irp;
    KEVENT				event;
	NDASBUS_UPGRADE_TO_WRITE	Param;

	ASSERT( KeGetCurrentIrql() <= PASSIVE_LEVEL );
	
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUpradeToWrite: SlotNo:%ld\n", SlotNo));
	...
	Param.Size = sizeof(NDASBUS_UPGRADE_TO_WRITE);
	Param.SlotNo = SlotNo;
	...
	irp = IoBuildDeviceIoControlRequest(
			...
			deviceObject,
			&Param,
			sizeof(NDASBUS_UPGRADE_TO_WRITE),
			NULL,
			0,
			FALSE,
			&event,
    ...
    if (irp == NULL) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUpgradeToWrite: irp NULL\n"));
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUpradeToWrite: NTSTATUS:%lx IOSTATUS:%lx\n", ntStatus, IoStatus->Status));
    if (ntStatus == STATUS_PENDING)
    {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUpradeToWrite: IoCallDriver STATUS_PENDING\n"));
		KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, NULL);
    }
	if (!NT_SUCCESS (ntStatus)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUpradeToWrite: IoCallDriver() failed.\n"));
//		ASSERT(FALSE);
	}
...
) 
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlRemoveTarget: SlotNo:%ld\n", SlotNo));
    ...
    if (irp == NULL) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlRemoveTarget: irp NULL\n"));
	...
	ntStatus = IoCallDriver(deviceObject, irp);
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlRemoveTarget: NTSTATUS:%lx IOSTATUS:%lx\n", ntStatus, ioStatusBlock.Status));
    if (ntStatus == STATUS_PENDING)
    {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlRemoveTarget: IoCallDriver STATUS_PENDING\n"));
		KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, NULL);
    }
...
cleanup:
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlRemoveTarget: Done...\n"));
...
) 
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUnplug: SlotNo:%ld\n", SlotNo));
    ...
    if (irp == NULL) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUnplug: irp NULL\n"));
	...
	ntStatus = IoCallDriver(deviceObject, irp);
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUnplug: NTSTATUS:%lx IOSTATUS:%lx\n", ntStatus, ioStatusBlock.Status));
    if (ntStatus == STATUS_PENDING)
    {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUnplug: IoCallDriver STATUS_PENDING\n"));
		KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, NULL);
    }
	if (!NT_SUCCESS (ntStatus) || !NT_SUCCESS(ioStatusBlock.Status)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, 
...
cleanup:
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlUnplug: Done...\n"));
...
NDCtrlEject(
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlEject: SlotNo:%ld\n", SlotNo));
    ...
    if (irp == NULL) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlEject: irp NULL\n"));
	...
	ntStatus = IoCallDriver(deviceObject, irp);
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlEject: NTSTATUS:%lx IOSTATUS:%lx\n", ntStatus, ioStatusBlock.Status));
    if (ntStatus == STATUS_PENDING)
    {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlEject: IoCallDriver STATUS_PENDING\n"));
		KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, NULL);
    }
	if (!NT_SUCCESS (ntStatus) || !NT_SUCCESS(ioStatusBlock.Status)) {
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlEject: IoCallDriver() failed. STATUS=%08lx\n", ntStatus));
	}
...
cleanup:
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NDCtrlEject: Done...\n"));
