		...
		while (netdiskManagerThreadExit == FALSE && 
			...
			if (netdiskManagerRequest->RequestType == NETDISK_MANAGER_REQUEST_DOWN) {
				...
				do {
					...
					//interval.QuadPart = ( 120 * DELAY_ONE_SECOND );      //delay 5 seconds
	...
	)
{
	...
	try {
		...
		} else if (netdiskInformation.DeviceMode == DEVMODE_EXCLUSIVE_READWRITE) {

			SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("DEVMODE_EXCLUSIVE_READWRITE\n") );
	...
	)
{
	...
	ndasBace2 = (PNDAS_BLOCK_ACE)((PUCHAR)psrbioctl + sizeof(SRB_IO_CONTROL));
	blockAceId = (PBLOCKACE_ID)ndasBace2;
	RtlCopyMemory( ndasBace2, &ndasBace, sizeof(NDAS_BLOCK_ACE) );
	...
	)
{
	...
	if (EnabledNetdisk->ListEntry.Flink != EnabledNetdisk->ListEntry.Blink)
		NDASFS_ASSERT( FALSE );
...
if (IS_WINDOWS2K()) {
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_TRACE,
				   ("EnabledNetdisk_PreMountVolume: DiskDeviceObject = %p\n", DiskDeviceObject) );
	...
	*NetdiskPartition = netdiskPartition;
	...
	if (netdiskPartition->ScsiportAdapterDeviceObject) {
		...
		NDASFS_ASSERT( DiskDeviceObject );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_NETDISK_MANAGER_TRACE,
				   ("EnabledNetdisk_PostMountVolume: NetdiskPartition = %p\n", NetdiskPartition) );
	...
	)
{
	...
	if (NetdiskEnableMode) {
		...
		ASSERT( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumeMounted );
	
	} else {

		// called from NdasFat or NdasNtfs

		ASSERT( NetdiskPartition->NetdiskVolume[NETDISK_SECONDARY].VolumeState == VolumePostMounting || 
	...
	)
{
	...
	ASSERT( NetdiskPartition->NetdiskVolume[NetdiskEnableMode].VolumeState == VolumeMounted );
