...
#define CountIrpMajorFunction( LfsDeviceExt, IrpMajorFunction )
	...
	)
{
	...
	lfsDeviceExt->Flags = LFS_DEVICE_FLAG_INITIALIZING;

	lfsDeviceExt->FileSpyDeviceObject	= FileSpyDeviceObject;
	ObReferenceObject( lfsDeviceExt->FileSpyDeviceObject );
		...
		if (DevExt->LfsDeviceExt.SecondaryState == VOLUME_PURGE_STATE) {
			...
			if (irpSp->MajorFunction == IRP_MJ_CREATE) {
				...
				if (fileObject->FileName.Length == (sizeof(WAKEUP_VOLUME_FILE_NAME)-sizeof(WCHAR)) && 
					...
					status = Irp->IoStatus.Status = STATUS_PORT_DISCONNECTED;
					Irp->IoStatus.Information = 0;
					IoCompleteRequest( Irp, IO_DISK_INCREMENT );
	...
	)
{
	...
	return ( LfsDeviceExt->SecondaryState == CONNECTED_TO_LOCAL_STATE );
}
