...
void usage()
{
	_tprintf(TEXT("ndasbuscmd [version | mpversion | slotlist | fdoinfo | status | pdoevent | pdoeventptr | pdofile] slotno\n"));
}

void
PrintStatus(DWORD PdoStatus) {
	_tprintf(TEXT("ADAPTERINFO_STATUS : %s\n"),
		ADAPTERINFO_ISSTATUS(PdoStatus, NDASSCSI_ADAPTERINFO_STATUS_INIT) ? TEXT("NDASSCSI_ADAPTERINFO_STATUS_INIT") :
		ADAPTERINFO_ISSTATUS(PdoStatus, NDASSCSI_ADAPTERINFO_STATUS_RUNNING) ? TEXT("NDASSCSI_ADAPTERINFO_STATUS_RUNNING") :
		ADAPTERINFO_ISSTATUS(PdoStatus, NDASSCSI_ADAPTERINFO_STATUS_STOPPING) ? TEXT("NDASSCSI_ADAPTERINFO_STATUS_STOPPING") :
		ADAPTERINFO_ISSTATUS(PdoStatus, NDASSCSI_ADAPTERINFO_STATUS_STOPPED) ? TEXT("NDASSCSI_ADAPTERINFO_STATUS_STOPPED") :
		TEXT("Unknown status"));

#define PRINT_FLAG(STATUS, FLAG) \
	_tprintf(TEXT("") TEXT(#FLAG) TEXT(": %s\n"), ADAPTERINFO_ISSTATUSFLAG((STATUS), (FLAG)) ? TEXT("ON") : TEXT("off"));

	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_RECONNECT_PENDING);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_RESTARTING);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_BUSRESET_PENDING);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_MEMBER_FAULT);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_RECOVERING);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_ABNORMAL_TERMINAT);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_POWERRECYCLED);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_RAID_FAILURE);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_RAID_NORMAL);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_RESETSTATUS);
	PRINT_FLAG(PdoStatus, NDASSCSI_ADAPTERINFO_STATUSFLAG_NEXT_EVENT_EXIST);
#undef PRINT_FLAG
}
	...
	if (lstrcmpi(argv[1],TEXT("version")) == 0) {
	  ...
	  ULONG ulStatus;
		...
		if(bret == FALSE) {
			...
			_tprintf(TEXT("Adapter.MaxDataTransferLength        :%u\n"), lurFullInfo->Adapter.MaxDataTransferLength);
