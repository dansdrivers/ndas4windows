...
CDismountAllDialog::OnInitDialog(HWND hWnd, LPARAM lParam)
{
	...
	m_messageText.GetWindowText(m_staticMessage, RTL_NUMBER_OF(m_staticMessage));
...
DWORD 
CDismountAllDialog::WorkItemStart(PWORK_ITEM_PARAM Param)
{
	BOOL success = NdasEjectLogicalDeviceEx(&Param->EjectParam);
	if (!success)
	{
