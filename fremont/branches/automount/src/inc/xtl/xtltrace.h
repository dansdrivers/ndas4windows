...
public:
	...
	void SimpleTraceWithLastError(LPCSTR Format, ...)
	{
		va_list ap;
		va_start(ap, Format);
		FlagLevelTraceVWithError(XTLTRACE_DEFAULT_CATEGORY, XTLTRACE_DEFAULT_LEVEL, ::GetLastError(), Format, ap);
		va_end(ap);
	}
	void SimpleTraceWithLastError(LPCWSTR Format, ...)
	{
		va_list ap;
		va_start(ap, Format);
		FlagLevelTraceVWithError(XTLTRACE_DEFAULT_CATEGORY, XTLTRACE_DEFAULT_LEVEL, ::GetLastError(), Format, ap);
	...
	void SimpleTraceWithError(DWORD Error, LPCSTR Format, ...)
	{
		va_list ap;
		va_start(ap, Format);
		FlagLevelTraceVWithError(XTLTRACE_DEFAULT_CATEGORY, XTLTRACE_DEFAULT_LEVEL, Error, Format, ap);
		va_end(ap);
	}
	void SimpleTraceWithError(DWORD Error, LPCWSTR Format, ...)
	{
		va_list ap;
		va_start(ap, Format);
		FlagLevelTraceVWithError(XTLTRACE_DEFAULT_CATEGORY, XTLTRACE_DEFAULT_LEVEL, Error, Format, ap);
	...
	void LevelTrace(DWORD Level, LPCSTR Format, ...)
	{
		...
		va_list ap;
		va_start(ap, Format);
		XtlTraceV2(XtlTraceGetFileName(m_lpFileName), m_dwLine, XTLTRACE_DEFAULT_CATEGORY, Level, Format, ap);
		va_end(ap);
	}
	...
	void FlagLevelTraceWithLastError(DWORD Category, DWORD Level, LPCSTR Format, ...)
	{
		va_list ap;
		va_start(ap, Format);
		FlagLevelTraceVWithError(Category, Level, ::GetLastError(), Format, ap);
		va_end(ap);
	}
	void FlagLevelTraceWithLastError(DWORD Category, DWORD Level, LPCWSTR Format, ...)
	{
		va_list ap;
		va_start(ap, Format);
		FlagLevelTraceVWithError(Category, Level, ::GetLastError(), Format, ap);
	...
	void FlagLevelTraceWithError(DWORD Category, DWORD Level, DWORD Error, LPCSTR Format, ...)
	{
		va_list ap;
		va_start(ap, Format);
		FlagLevelTraceVWithError(Category, Level, Error, Format, ap);
		va_end(ap);
	}
	void FlagLevelTraceWithError(DWORD Category, DWORD Level, DWORD Error, LPCWSTR Format, ...)
	{
		va_list ap;
		va_start(ap, Format);
		FlagLevelTraceVWithError(Category, Level, Error, Format, ap);
	...
	void FlagLevelTraceVWithError(DWORD Category, DWORD Level, DWORD Error, LPCSTR Format, va_list ap)
	{
		if (!XtlTraceIsEnabled(Category, Level)) return;
		XtlTraceV2WithError(XtlTraceGetFileName(m_lpFileName), m_dwLine, Category, Level, Error, Format, ap);
	}
	void FlagLevelTraceVWithError(DWORD Category, DWORD Level, DWORD Error, LPCWSTR Format, va_list ap)
	{
		if (!XtlTraceIsEnabled(Category, Level)) return;
		XtlTraceV2WithError(XtlTraceGetFileName(m_lpFileName), m_dwLine, Category, Level, Error, Format, ap);
	}
...
#else
#define XTLTRACE XTL::CXtlTraceBufferA(__FILE__,__LINE__).SimpleTrace
// #define XTLTRACE_ERR XTL::CXtlTraceBufferA(__FILE__,__LINE__).SimpleTraceWithLastError
// #define XTLTRACE_ERR_EX XTL::CXtlTraceBufferA(__FILE__,__LINE__).SimpleTraceWithError
#define XTLTRACE1 XTL::CXtlTraceBufferA(__FILE__,__LINE__).LevelTrace
#define XTLTRACE2 XTL::CXtlTraceBufferA(__FILE__,__LINE__).FlagLevelTrace
// #define XTLTRACE2_ERR XTL::CXtlTraceBufferA(__FILE__,__LINE__).FlagLevelTraceWithLastError
// #define XTLTRACE2_ERR_EX XTL::CXtlTraceBufferA(__FILE__,__LINE__).FlagLevelTraceWithError
#endif
...
public:
	...
	{
		XTL::CXtlTraceBufferA(m_file,m_line).FlagLevelTrace(m_cat, xtlTLCall, "ENTER>%s\n", m_func);
	}
	~CXtlCallTrace()
	{
		XTL::CXtlTraceBufferA(m_file,m_line).FlagLevelTrace(m_cat, xtlTLCall, "LEAVE>%s\n", m_func);
	}
};
...
public:
	...
	~CXtlCallTraceEx()
	{
		XTL::CXtlTraceBufferA(m_file,m_line).FlagLevelTrace(m_cat, xtlTLCall, "LEAVE>%s (ret=%x)\n", m_func, m_ret);
	}
private:
