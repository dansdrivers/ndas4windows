	...
	TraceErrorMessageA(DWORD dwError = ::GetLastError())
	{
		if (dwError & APPLICATION_ERROR_MASK) 
		{
			XTLTRACE1(TRACE_LEVEL_ERROR,
				"Error from an application %d (0x%08X)\n", 
				dwError, dwError);
			return;
		} 
		else 
		{
			...
			{
				XTLTRACE1(TRACE_LEVEL_ERROR,
					"Error from system %d (0x%08X): (not available)", 
					dwError, dwError);
				return;
			}
			XTLTRACE1(TRACE_LEVEL_ERROR,
				"Error from system %d (0x%08X): %s", 
...
public:
	...
	BOOL Connect()
	{
		...
		while (TRUE) 
		{
			...
			if (hPipe.IsInvalid() && ERROR_PIPE_BUSY != GetLastError()) 
			{
				XTLTRACE1(TRACE_LEVEL_INFORMATION,
					"Unable to create a named pipe (%ls), error=0x%X\n",
			...
			{
				XTLTRACE1(TRACE_LEVEL_ERROR,
					"Unable to open a pipe (%ls), error=0x%X\n", 
					szPipeName, GetLastError());
				return FALSE;
			}
		}
		...
		{
			XTLTRACE1(TRACE_LEVEL_ERROR,
				"Setting named pipe handle state failed, error=0x%X\n",
				GetLastError());
			return FALSE;
		}
		...
		LPDWORD lpdwOperationStatus)
	{
		...
		default:
			XTLTRACE1(TRACE_LEVEL_ERROR,
				"NDAS Command failed, status=%s(0x%X).\n", 
