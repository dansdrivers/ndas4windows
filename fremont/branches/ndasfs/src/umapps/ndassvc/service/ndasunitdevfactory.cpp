...
CNdasUnitDeviceCreator::_CreateUnknownUnitDevice()
{
	...
	{
		XTLTRACE2(NDASSVC_NDASUNITDEVICE, TRACE_LEVEL_ERROR,
			"new CNdasNullUnitDevice failed\n");
	}
	return pUnitDevice;
}
...
CNdasUnitDeviceCreator::CreateUnitDiskDevice()
{
	...
	{	
		...
		PNDAS_RAID_INFO pIR = 
			reinterpret_cast<PNDAS_RAID_INFO>(pAddTargetInfo);
		...
		::CopyMemory(&pIR->NdasRaidId, &Rmd.RaidSetId, sizeof(pIR->NdasRaidId));	
	...
	PNDAS_CONTENT_ENCRYPT_BLOCK pCEB)
{
	...
	if(!fSuccess) 
	{
		XTLTRACE2(NDASSVC_NDASUNITDEVICE, TRACE_LEVEL_ERROR,
			...
			NDAS_BLOCK_LOCATION_ENCRYPT, GetLastError());
