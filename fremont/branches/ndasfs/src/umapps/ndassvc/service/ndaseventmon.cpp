...
CNdasEventMonitor::OnLogicalDeviceAlarmed(CNdasLogicalDevicePtr pLogDevice)
{
	...
	NDAS_LOGICALUNIT_ADDRESS ndasLogicalUnitAddress = pLogDevice->GetNdasLogicalUnitAddress();
...
CNdasEventMonitor::OnLogicalDeviceAlarmedByPnP(
	__in CNdasLogicalDevicePtr pLogDevice, 
	__in ULONG LogDeviceStatus)
{
	NDAS_LOCATION ndasLocation = pLogDevice->GetNdasLocation();

	XTLTRACE2(NDASSVC_EVENTMON, TRACE_LEVEL_INFORMATION,
		"Alarm Event, ndasLocation=%08X, logDevice=%s\n", 
		ndasLocation, pLogDevice->ToStringA());
	...
	XTLTRACE2(NDASSVC_EVENTMON, TRACE_LEVEL_INFORMATION,
		...
		ndasLocation, LogDeviceStatus);

	// Determine whether an alarm will be issued.
	// Only viable alarm will be published
	if (pIsViableAlarmStatus(pLogDevice->GetAdapterStatus(), LogDeviceStatus))
	{
		CNdasEventPublisher& epub = pGetNdasEventPublisher();

		if(ADAPTERINFO_ISSTATUS(LogDeviceStatus, NDASSCSI_ADAPTER_STATUS_STOPPED) &&
			ADAPTERINFO_ISSTATUSFLAG(LogDeviceStatus, NDAS_DEVICE_ALARM_STATUSFLAG_ABNORMAL_TERMINAT))
		{
			//
			// Notify abnormal removal of the LU device.
			//
			OnLogicalDeviceDisconnected(pLogDevice);
		}
		else
		{
			(void) epub.LogicalDeviceAlarmed(
				pLogDevice->GetLogicalDeviceId(), 
				pGetSignificantAlarm(pLogDevice->GetAdapterStatus(), LogDeviceStatus));
		}
	}

	pLogDevice->SetAdapterStatus(LogDeviceStatus);
}
