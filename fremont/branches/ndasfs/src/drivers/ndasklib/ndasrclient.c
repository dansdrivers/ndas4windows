	...
	)
{
...
#else
	NDAS_ASSERT( NdasrClient->NdasrState == NRMX_RAID_STATE_OUT_OF_SYNC ||
	...
	do {
		...
		for (;;) {
			...
			if (FlagOn(NdasrClient->Status, NDASR_CLIENT_STATUS_ARBITRATOR_DISCONNECTED)) {
		...
		if (!FlagOn(NdasrClient->Status, NDASR_CLIENT_STATUS_NON_ARBITRATOR_MODE)) {
			...
			while (listEntry != &NdasrClient->LockList) {
				...
				if (((currentTick.QuadPart - lock->LastAccesseTick.QuadPart) * KeQueryTimeIncrement()) >= (NANO100_PER_SEC * NRMX_IO_LOCK_CLEANUP_TIMEOUT)) { // 60 NANO100_PER_SEC
			...
			if (Ccb->Cdb[0] == SCSIOP_WRITE ||Ccb->Cdb[0] == SCSIOP_WRITE16) {
				...
				for (ridx = 0; ridx < NdasrClient->Lurn->NdasrInfo->ActiveDiskCount;) { 
					...
					if (nextCcb[ridx]) {
						...
						NDAS_ASSERT( LsCcbIsFlagOn(nextCcb[ridx], CCB_FLAG_ACQUIRE_BUFLOCK) );
	...
	) 
{
	...
	NDAS_ASSERT( newLocalNodeFlags == NRMX_NODE_FLAG_RUNNING ||
					 ...
					 newLocalNodeFlags == (NRMX_NODE_FLAG_STOP | NRMX_NODE_FLAG_DEFECTIVE) );
