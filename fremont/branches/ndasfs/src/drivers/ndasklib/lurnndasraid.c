	...
	) 
{
	...
	do {
		...
		NDAS_ASSERT( ndasrInfo->SpareDiskCount == 0 );
...
#if __NDAS_SCSI_UNRECHABLE_NODE_TEST__
					continue;
#endif
					NDAS_ASSERT( FALSE );
	...
	)
{
	...
	if (Ccb->OperationCode == CCB_OPCODE_SHUTDOWN) {
		...
		NDAS_ASSERT( Lurn && Lurn->Lur && Lurn->Lur->AdapterFunctionDeviceObject );
