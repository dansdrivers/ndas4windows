	...
	)
{
	...
	if (status != STATUS_SUCCESS || response != LANSCSI_RESPONSE_SUCCESS) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, 
					("CMD(0x%02x), MEDIA_TYPE(0x%02x), resStatus (0x%02x), regError(0x%02x)\n",
	...
	if (1																								&&
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
					("CMD(0x%02x), MEDIA_TYPE(0x%02x), STATUS(0x%02x), "
					 "resStatus (0x%02x), regError(0x%02x), ErrorCode(0x%02x), "
...
#if __NDAS_SCSI_B_RECORDER__

	if (Ccb->Cdb[0] == SCSIOP_MODE_SELECT) {

		if (status == STATUS_SUCCESS) {

			if (Ccb->DataBufferLength == 12) {
		...
		if (ntStatus != STATUS_SUCCESS) {
			...
			case LANSCSI_RESPONSE_SUCCESS: {
				...
				if (Command == SCSIOP_MODE_SENSE10) {
					...
					if ((!(Lurn->AccessRight & GENERIC_WRITE)) && (Lurn->LurnType == LURN_IDE_MO)) {
...
//	retryCount = 0;
	...
	while (transferedBlocks < transferBlocks) {
		...
		if (status != STATUS_SUCCESS || *PduResponse != LANSCSI_RESPONSE_SUCCESS) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR,
				("STATUS=%08lx, response=%x,logicalBlockAddress = %I64x, transferBlocks = %d, transferedBlocks = %d splitBlocks = %d\n",
	...
	)
{
	...
	if(IdeDisk->LuHwData.InquiryData.DeviceType != DIRECT_ACCESS_DEVICE) {
		...
		{
			bytesOfBlock = CDROM_RAW_SECTOR_SIZE;
			doSplit = TRUE;
		} else {
