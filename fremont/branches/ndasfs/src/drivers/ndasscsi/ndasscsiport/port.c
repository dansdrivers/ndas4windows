...
#if !defined(__NDAS_MINI_PORT__) || !defined(_WIN64) 

VOID
ScsiPortMoveMemory(
...
UCHAR ScsiBuffer[DEBUG_BUFFER_LENGTH + 1];
PUCHAR ScsiBufferSentinal = NULL;
#endif
...

{
...
#else
		if (NdasScsiDbgPrintEx) {

			NdasScsiDbgPrintEx(DPFLTR_SCSIMINIPORT_ID, DPFLTR_ERROR_LEVEL,
				       "Debug messages in SCSI Miniports are no longer controlled by\n"
					   "scsiport!scsidebug.  Please use the correct debug maski\n\n"
	                   "Kd_ScsiPort_Mask -- controls debug msgs from ScsiPort\n"
		               "Kd_ScsiMiniPort_Mask  --  controls debug msgs from SCSI-Miniports\n\n"
	...
	if (NdasScsivDbgPrintExWithPrefix) {

	    NdasScsivDbgPrintExWithPrefix("ScsiMiniport: ",
		                      DPFLTR_SCSIMINIPORT_ID, 
			                  DebugLevel, 
				              DebugMessage, 
					          ap);
...

{
    ...
    if (ScsiDebug != 0) {
		...
		if (NdasScsiDbgPrintEx) {
			...
			NdasScsiDbgPrintEx(DPFLTR_SCSIPORT_ID, DPFLTR_ERROR_LEVEL,
                   "Debug messages in SCSI Miniports are no longer controlled by\n"
                   "scsiport!scsidebug.  Please use the correct debug maski\n\n"
                   "Kd_ScsiPort_Mask -- controls debug msgs from ScsiPort\n"
                   "Kd_ScsiMiniPort_Mask  --  controls debug msgs from SCSI-Miniports\n\n"
	...
	if (NdasScsivDbgPrintExWithPrefix) {
	    ...
	    NdasScsivDbgPrintExWithPrefix("ScsiPort: ",
                          DPFLTR_SCSIPORT_ID, 
                          DebugLevel, 
                          DebugMessage, 
                          ap);
...
//
#if !defined(_ALPHA_) && !defined(_X86_) && !defined(__NDAS_MINI_PORT__)

UCHAR
