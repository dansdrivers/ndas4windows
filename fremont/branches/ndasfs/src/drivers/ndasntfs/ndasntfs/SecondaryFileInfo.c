...

{
    ...
    try {
		...
		if (NT_SUCCESS(Status) && NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) != (UINT64)Scb->Header.AllocationSize.QuadPart) {
    ...
    if (IoGetCurrentIrpStackLocation(Irp)->Parameters.SetFile.AdvanceOnly) {
		...
		try {
			...
			if (NT_SUCCESS(Status)) {
				...
				if (NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) != (UINT64)Scb->Header.AllocationSize.QuadPart) {
