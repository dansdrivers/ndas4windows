			...
			if (!FlagOn(VolumeDeviceObject->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) &&
				...
				if ((IrpContext->MinorFunction == IRP_MN_QUERY_REMOVE_DEVICE	|| 
					...
					while (TRUE) {
						...
						NDAS_ASSERT( VolumeDeviceObject->Secondary );
...

{
    ...
    try {
		...
		if (OurDeviceObject->Secondary) {
			...
			NDAS_ASSERT( OurDeviceObject->NetdiskEnableMode == NETDISK_SECONDARY || 
            ...
            if ((Vcb->CleanupCount > 0) ||
                ...
                if ((Vcb->CloseCount - (Vcb->SystemFileCloseCount + Vcb->QueuedCloseCount)) > 0) {
                    ...
                    if (NT_SUCCESS( Status )) {
						...
						NDAS_ASSERT( Vcb->SecondaryCloseCount == 0 );
						NDAS_ASSERT( Vcb->SecondaryReadOnlyCloseCount == 0 );
						NDAS_ASSERT( Vcb->SecondarySystemFileCloseCount == 0 );
#endif
                    }
                }
            }
