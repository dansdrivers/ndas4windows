...
--*/
{
...
#if 0 //__NDAS_FS_MINI_CHECK__
	{
		...
		if (gSpyOsMajorVersion == 0x5 && gSpyOsMinorVersion >= 2 ||
			gSpyOsMajorVersion >= 6) {

#if DBG
				DbgPrint( "Mini Filter version works\n" );
#endif
...
#if DBG
		DbgPrint( "Mini Filter version doesn't works\n" );
#endif

#endif

		RtlInitUnicodeString( &tempUnicode, L"FltRegisterFilter" );

		if (MmGetSystemRoutineAddress(&tempUnicode)) {

		//if (FltGetRoutineAddress("FsRtlTeardownPerStreamContexts")) {
...
--*/
{
	...
	if (devExt->LfsDeviceExt.FilteringMode == LFS_READONLY) {
		...
		if (!FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_INDIRECT) &&
			(devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT ||
...

{
    ...
    SPY_LOG_PRINT( SPYDEBUG_ERROR,
                   ("FileSpy!SpyFsControlMountVolume: pIrpSp->Parameters.MountVolume.Vpb->RealDevice = %p, pIrpSp->Parameters.MountVolume.DeviceObject = %p\n",
					pIrpSp->Parameters.MountVolume.Vpb->RealDevice, pIrpSp->Parameters.MountVolume.DeviceObject) );
    ...
    )
{
	...
	if (!FlagOn(newLfsDeviceExt->Flags, LFS_DEVICE_FLAG_INDIRECT) &&
		(newLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT ||
...
if (NewDeviceObject == NULL) {
	...
	if (newDevExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT) {

		if (GlobalLfs.NdasFatRwIndirect)
			SetFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_INDIRECT );
	
	} else if (newDevExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS) {
