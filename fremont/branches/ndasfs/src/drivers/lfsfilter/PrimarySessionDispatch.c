	...
	)
{
	...
	case NDFS_COMMAND_SETUP: {
		...
		do {
			...
			PrimarySession->NetDiskAddress.Port = NTOHS(ndfsRequestSetup->NetDiskPort2);//HTONS(ndfsRequestSetup->NetDiskPort);
    ...
    )
{
	...
	NDAS_ASSERT( Mid == NTOHS(ndfsRequestHeader->Mid2) );
	
    SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_NOISE,
					("DispatchWinXpRequestWorker: entered PrimarySession = %p, ndfsRequestHeader->Command = %d\n", 
					PrimarySession, ndfsRequestHeader->Command) );

	NDAS_ASSERT( PrimarySession->Thread.SessionSlot[Mid].State == SLOT_EXECUTING );
	...
	if (replyDataSize <= 
		...
		NDAS_ASSERT( PrimarySession->Thread.SessionSlot[Mid].ExtendWinxpReplyMessagePool );
