	...
	lsp_handle_context_t* context)
{
	...
	if (LSP_ERR_RESPONSE_SUCCESS != pdu_hdr->response)
	{
		...
		if (LSP_ERR_RESPONSE_RI_VERSION_MISMATCH == pdu_hdr->response)
		{
			if (session->hardware_data.hardware_version != 
				pdu_hdr->op_data.login_response.ver_active)
			{
	...
	session->hardware_data.hardware_revision = 
		lsp_ntohs(pdu_hdr->op_data.login_response.revision);
	...
	lsp_handle_context_t* context)
{
	 ...
	 */
	if (*(lsp_uint64_t *)login_info->supervisor_password)
	{
...
lsp_ide_command_process_v0(lsp_handle_context_t* context)
{
	...
	if (LSP_OPCODE_IDE_RESPONSE != pdu_hdr->op_code ||
		0 == pdu_hdr->op_flags.ide_command.F /* ||
...
lsp_ide_command_prepare_v1(lsp_handle_context_t* context)
{
	...
	*(lsp_uint32_t*) &pdu_hdr->op_data.ide_command.header = 
...
lsp_status_t
lsp_call
lsp_request_ex_ata_handshake_packet_device_set_feature_completion(
	__in lsp_handle_t h,
	__in struct _lsp_request_packet_t* request);
	...
	lsp_request_packet_t* request)
{
	...
	h->session.handshake_set_feature_count = 0;
	...
	__in lsp_uint16_t hardware_revision)
{
	...
	{
		ident->minimum_pio_cycle_time;
		ident->minimum_pio_cycle_time_iordy;
	}
...
lsp_request_ex_ata_handshake_transfer_mode(
	...
	__in const lsp_ide_identify_device_data_t* ident,
	__inout lsp_ata_handshake_data_t* hsdat,
	__out lsp_uint8_t* new_dma_mode,
	__out lsp_uint8_t* new_dma_level)
{
	lsp_int8_t highest_level, active_level;
	lsp_uint8_t support_bits, active_bits;
	lsp_uint32_t require_set_transfer_mode;
	...
	if (ident->capabilities.dma_supported)
	{
			...
			hardware_revision != LSP_HARDWARE_V20_REV_0))
		{
			...
			if (support_bits)
			{
				...
				hsdat->active.dma_mode = LSP_IDE_TRANSFER_MODE_ULTRA_DMA;
				hsdat->active.dma_level = active_level;

				hsdat->support.dma_mode = LSP_IDE_TRANSFER_MODE_ULTRA_DMA;
				hsdat->support.dma_level = highest_level;

				transfer_mode = LSP_IDE_TRANSFER_MODE_ULTRA_DMA;
				*new_dma_mode = LSP_IDE_TRANSFER_MODE_ULTRA_DMA;
				*new_dma_level = highest_level;
			}
		}
		...
		if (LSP_IDE_TRANSFER_MODE_UNSPECIFIED == transfer_mode)
		{
			...
			if (support_bits)
			{
				...
				*/
				...
				hsdat->active.dma_mode = LSP_IDE_TRANSFER_MODE_MULTIWORD_DMA;
				hsdat->active.dma_level = active_level;

				hsdat->support.dma_mode = LSP_IDE_TRANSFER_MODE_MULTIWORD_DMA;
				hsdat->support.dma_level = highest_level;

				transfer_mode = LSP_IDE_TRANSFER_MODE_MULTIWORD_DMA;
				*new_dma_mode = LSP_IDE_TRANSFER_MODE_MULTIWORD_DMA;
				*new_dma_level = highest_level;
			}
		}
		...
		if (LSP_IDE_TRANSFER_MODE_UNSPECIFIED == transfer_mode)
		{
			...
			if (support_bits)
			{
				...
				*/
				...
				hsdat->active.dma_mode = LSP_IDE_TRANSFER_MODE_SINGLEWORD_DMA;
				hsdat->active.dma_level = active_level;

				hsdat->support.dma_mode = LSP_IDE_TRANSFER_MODE_SINGLEWORD_DMA;
				hsdat->support.dma_level = highest_level;

				transfer_mode = LSP_IDE_TRANSFER_MODE_SINGLEWORD_DMA;
				*new_dma_mode = LSP_IDE_TRANSFER_MODE_SINGLEWORD_DMA;
				*new_dma_level = highest_level;
			}
		}
	}
	...
	__in struct _lsp_request_packet_t* request)
{
	...
	*/
	...
	handshake_data->logical_block_size = 0;

	/* we do not negotiate PIO mode */
	...
	if (h->session.handshake_set_feature_count < 2 && require_set_transfer_mode)
	{
	...
	__in struct _lsp_request_packet_t* request)
{
	...
	int require_set_transfer_mode;
	int i;
	lsp_uint8_t new_dma_mode, new_dma_level;
	...
	*/
	...
	if (h->session.handshake_set_feature_count < 2 && require_set_transfer_mode)
	{
		...
		new_dma_mode = handshake_data->support.dma_mode;
		new_dma_level = handshake_data->support.dma_level;
	...
	__in struct _lsp_request_packet_t* request)
{
	...
	++h->session.handshake_set_feature_count;
