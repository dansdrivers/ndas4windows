	...
	)
{
...

	{
		...
		*/
		if ((LSP_HW_VERSION_2_0 < session->HWVersion) ||
			((LSP_HW_VERSION_2_0 == session->HWVersion) && (0 != session->HWRevision)))
		{
			// ultra dma
	...
	//					dma_mode = 7;
					...
					// 2.0G 100M version revision.
					if (dma_mode > 2 && 
						session->HWVersion == LSP_HW_VERSION_2_0 && 
						session->HWRevision == LSP_HW_VER20_REV_100M)
					{ 
						dma_mode = 2;
					}
#ifdef __LSP_CHECK_CABLE80__
		...
		if(set_dma_feature)
		{
			...
			//
			// RAID controllers such as ACCUSYS's ACS-75170 does not seem to report dma mode change correctly.
			// For the workaround, do not check the identify data if setfeature command is successful.
			err = lsp_ide_identify(h, target_id, lun0, lun1, packet_device, &info);
			if(LSP_ERR_SUCCESS != err)
				return err;
