...
#define lsp_byteswap_ulong lsp_imp_byteswap_ulong
#endif
...
LSP_INLINE unsigned long lsp_imp_byteswap_ulong(unsigned long x);
...
#else
	return ((unsigned long) (lsp_byteswap_ushort(x & 0xFFFF) << 16)) |
		((unsigned long) (lsp_byteswap_ushort(x >> 16)));
#endif
}

LSP_INLINE lsp_uint64_t lsp_imp_byteswap_uint64(lsp_uint64_t x)
{
	return (((lsp_uint64_t) lsp_byteswap_ulong((unsigned long)(x & 0xFFFFFFFFULL))) << 32) |
		((lsp_uint64_t) lsp_byteswap_ulong((unsigned long)(x >> 32)));
}
