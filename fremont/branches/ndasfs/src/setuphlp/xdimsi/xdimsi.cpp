	...
	__in const XDIMSI_PROCESS_RECORD* ProcessRecord)
{
	...
	XMSITRACE(L"\tAction=%s, Type=%d, HardwareId=%s, Inf=%s, Flags=0x%08X, Error=%d, Ticks=%d\n", 
	...
	if (ERROR_SUCCESS != ret)
	{
		XMSITRACEH(hInstall, L"pxMsiQueryActionText failed, ret=0x%x\n", ret);
	}
	else
	{
	...
	__in INT Phase)
{
	...
	while (TRUE)
	{
		...
		if (ERROR_SUCCESS != ret)
		{
			XMSITRACEH(hInstall, L"MsiRecordGetStringW(`Action`) failed, ret=0x%x\n", ret);
			free(hardwareId);
			free(infPath);
			return ERROR_INSTALL_FAILURE;
		}
	...
	__in MSIHANDLE hInstall)
{
	...
	XMSITRACEH(hInstall, L"  %ls(S) Type=%d, HardwareId=%s, Inf=%s, Ticks=%d\n",
