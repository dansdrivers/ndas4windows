...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="text">
    <xsl:choose>
      <!-- excludes an empty string -->
      <xsl:when test="not(@id)"></xsl:when>
      <xsl:when test="not(text())"></xsl:when>
      <xsl:when test="substring(text(),string-length(text()),1) = '}'"></xsl:when>
      <!-- 
      <xsl:when test="starts-with(text(),'{') and substring(text(),string-length(text()),1) = '}'"></xsl:when>
      -->
      <xsl:otherwise>
        <string>
