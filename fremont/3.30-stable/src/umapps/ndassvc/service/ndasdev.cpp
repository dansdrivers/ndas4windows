...
CNdasDevice::NdasHeartbeatReceived(const NDAS_DEVICE_HEARTBEAT_DATA* Data)
{
	...
	//
	if (0 != memcmp(&Data->NdasDeviceId.Node, &m_NdasDeviceId.Node, sizeof(m_NdasDeviceId.Node)))
	{
		return;
	}
	...
	if (NDAS_DEVICE_STATUS_ONLINE == m_Status) 
	{
		if (m_LocalSocketAddress.iSockaddrLength != Data->LocalAddress.iSockaddrLength ||
			0 != memcmp(m_LocalSocketAddress.lpSockaddr,
	...
	_ASSERT(Data->LocalAddress.iSockaddrLength <= 
		RTL_NUMBER_OF(m_LocalSocketAddressBuffer.RawAddress));

	CopyMemory(
		m_LocalSocketAddressBuffer.RawAddress,
		Data->LocalAddress.lpSockaddr,
		Data->LocalAddress.iSockaddrLength);
	...
	_ASSERT(Data->RemoteAddress.iSockaddrLength <= 
		RTL_NUMBER_OF(m_RemoteSocketAddressBuffer.RawAddress));

	CopyMemory(
		m_RemoteSocketAddressBuffer.RawAddress,
		Data->RemoteAddress.lpSockaddr,
		Data->RemoteAddress.iSockaddrLength);
