	...
	BOOL fUpdate)
{
	...
	*lpnRWHosts = dstat.UnitDevices[m_unitDeviceId.UnitNo].RWHostCount;
	...
	if (*lpnRWHosts && *lpnROHosts && *lpnROHosts != NDAS_HOST_COUNT_UNKNOWN) {

		*lpnROHosts = *lpnROHosts - 1;
	}
...
CNdasUnitImpl::CheckNDFSCompatibility()
{
	...
	{
		XTLTRACE2(NDASSVC_NDASUNITDEVICE, TRACE_LEVEL_ERROR,
			"Host NDFS %x, Primary NDFS %x failed.\n",
	...
	NDAS_LOGICALUNIT_DEFINITION *NdasLogicalUnitDefinition)
{
	...
	ATLVERIFY(NULL != RaidSimpleStatusFlags);
	...
	hr = NdasVsmGetRaidSimpleStatus(ndasHandle,
...
CNdasUnitImpl::VerifyNdasLogicalUnitDefinition()
{
	...
	if (curLuDef.Type == NDAS_LOGICALDEVICE_TYPE_DISK_SINGLE) {
		...
		curLuDef.NdasUnits[0] = m_unitDeviceId;
	}
	...
	if (0 != result)
	{
		...
		XTLTRACE2(NDASSVC_NDASCOMM, TRACE_LEVEL_WARNING,
			"NdasLogicalUnitDefinition of the NDAS unit is changed, hr=0x%x\n", hr);
