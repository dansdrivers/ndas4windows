...
class CNdasDeviceId
{
	static const size_t CCH_BUFFER = 32;
...
template<> struct std::less<NDAS_DEVICE_ID>;
...
template<>
struct std::less<NDAS_LOGICALUNIT_DEFINITION> :
	public std::binary_function<NDAS_LOGICALUNIT_DEFINITION, NDAS_LOGICALUNIT_DEFINITION, bool>
{
	bool operator()(const NDAS_LOGICALUNIT_DEFINITION& lhs, const NDAS_LOGICALUNIT_DEFINITION& rhs) const
	{
		int r = memcmp(&lhs, &rhs, sizeof(NDAS_LOGICALUNIT_DEFINITION));
		return r < 0;
	}
};
