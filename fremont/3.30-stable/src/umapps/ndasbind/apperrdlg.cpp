...
void
GetDescription(CString &strDescription, DWORD dwError)
{
	/* BOOL fSuccess = FALSE; */
	if (dwError & APPLICATION_ERROR_MASK) {
		...
		//
		// Show no error description if error code is 0 to reduce confusion.
		// 
		if (dwError != 0 && NULL != hModule) {
		...
		LPTSTR lpszErrorMessage = NULL;
		// error code 0 is misleading. Don't show it
		if (dwError) {
			INT iChars = ::FormatMessage(
