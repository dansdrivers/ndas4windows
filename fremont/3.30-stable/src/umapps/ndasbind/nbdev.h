...
public:
	CNBNdasDevice(LPCTSTR DeviceName, PNDAS_DEVICE_ID pDeviceId, NDAS_DEVICE_STATUS status, ACCESS_MASK GrantedAccess = GENERIC_READ | GENERIC_WRITE);	
	CNBNdasDevice(CNBNdasDevice* SrcDev);	// Create copy
	...
	virtual BOOL InitConnectionInfo(NDASCOMM_CONNECTION_INFO *ci, BOOL bWriteAccess)
	{
		return InitConnectionInfo(ci, bWriteAccess, 0);
	}

	BOOL InitConnectionInfo(NDASCOMM_CONNECTION_INFO *ci, BOOL bWriteAccess, DWORD UnitNo);
...
public:
	...
	virtual void HixChangeNotify(LPCGUID guid) = 0;
	virtual BOOL InitConnectionInfo(NDASCOMM_CONNECTION_INFO *ci, BOOL bWriteAccess) = 0;
...
private:
	...
	DWORD			m_Status;
	NDAS_LOGICALUNIT_DEFINITION m_definition; // Can be used for key of CNBLogicalDevice

	
public:

	//	CNBUnitDevice(PNDASUSER_UNITDEVICE_ENUM_ENTRY pBaseInfo);
	CNBUnitDevice(DWORD UnitNo, NDAS_UNITDEVICE_TYPE UnitDeviceType);
	...
	virtual void HixChangeNotify(LPCGUID guid);
	virtual BOOL InitConnectionInfo(NDASCOMM_CONNECTION_INFO *ci, BOOL bWriteAccess);
	...
	BOOL Equals(BYTE Node[6], BYTE Vid, DWORD UnitNo);
};
...
public:
	...
	BOOL IsOperatableAll(CNBUnitDevice *pUnitDeviceException = NULL);
