...
BOOL CNBUnitDevice::IsFlaggedOffline()
{
	if(!IsGroup())
		return FALSE;

	if (GetRaidStatus() & NDAS_RAID_MEMBER_FLAG_OFFLINE)
		return TRUE;
...
{
	// disable all function if RAID is fail
	if (GetLogicalDevice()->IsFail() ||
		GetLogicalDevice()->IsEmergency())
		return FALSE;
	...
	case IDM_TOOL_REMOVE_FROM_RAID:
		if (!(NDASBIND_UNIT_DEVICE_STATUS_MOUNTED & GetStatus()) && 
			IsFaultTolerant() && 
			GetLogicalDevice()->DevicesSpare() && GetLogicalDevice()->IsOperatableAll(this) &&
			!(GetRaidStatus() & NDAS_RAID_MEMBER_FLAG_IRRECONCILABLE) &&
			(IsSpare() || IsDefective() || IsFlaggedOffline())) {
	...
	case IDM_TOOL_USE_AS_MEMBER:
		if (IsFaultTolerant() && IsOperatable() && 
			(GetRaidStatus() & NDAS_RAID_MEMBER_FLAG_INVALID_MEMBER)	&& 
			GetLogicalDevice()->IsOperatableAll(this)&&
			!(GetRaidStatus() & NDAS_RAID_MEMBER_FLAG_IRRECONCILABLE)) {
...
CNBLogicalDevice::CNBLogicalDevice()
{
	...
	m_PrimaryUnit = NULL;
	memset(&m_RaidInfo, 0, sizeof(m_RaidInfo));
}
...
UINT32 CNBLogicalDevice::GetOperatableCount()
{
	...
	{
		if (m_mapUnitDevices.count(i) &&
			m_mapUnitDevices[i]->IsOperatable())
		{
			uiOperatable++;
		}
	}
...
BOOL CNBLogicalDevice::IsOperatableAll(CNBUnitDevice *pUnitDeviceException)
{
	if (GetOperatableCount() == DevicesTotal(FALSE))
	{
		// everything IS ok
		return TRUE;
	}
	...
	// everything 
	if (NULL == pUnitDeviceException)
	{
		// everything MUST BE ok
		return FALSE;
	}

	if ((GetOperatableCount() +1) != DevicesTotal(FALSE))
	{
		// At most, 1 device should be not ok
		return FALSE;
	}

	if (pUnitDeviceException->IsOperatable())
	{
		// This device should not be ok
		return FALSE;
	}
...
BOOL CNBLogicalDevice::IsEmergency()
{
	if(!IsFaultTolerant())
		return FALSE;

	// true with 1 disconnected, 1 not synced and else ok
	BOOL bDisconnected = FALSE, bNotSynced = FALSE;

	UINT32 nDevicesTotal = DevicesInRaid(FALSE);
	...
	for(UINT32 i = 0; i < nDevicesTotal; i++)
	{
		...
		if(m_mapUnitDevices[i]->IsNotSynced())
		{
			if(bNotSynced)
				return FALSE;
			bNotSynced = TRUE;
	...
	if (bDisconnected && bNotSynced)
		return TRUE;

	return FALSE;
}

BOOL CNBLogicalDevice::IsFail()
{
	if(!IsFaultTolerant())
		return FALSE;

	// true if 2 or more units fail
	BOOL bUnitFail = FALSE;
	...
	for(UINT32 i = 0; i < nDevicesTotal; i++)
	{
		if(!m_mapUnitDevices.count(i))
		{
			if(bUnitFail) // second failed device
				return TRUE;
			bUnitFail = TRUE;
			continue;
		}

		CNBUnitDevice *pUnitDevice = m_mapUnitDevices[i];

		if (!pUnitDevice->IsOperatable() ||
			pUnitDevice->IsNotSynced() ||
			pUnitDevice->IsDefective())
		{
			if(bUnitFail) // second failed device
				return TRUE;
			bUnitFail = TRUE;
...
BOOL CNBLogicalDevice::IsCommandAvailable(int nID)
{
	if (IsGroup())
	{
		if ((IDM_TOOL_UNBIND != nID && IDM_TOOL_FIX_RAID_STATE != nID)
			&& (IsFail() || IsEmergency()))
		{
			return FALSE;
		}
...
//			return !(GetStatus() & NDASBIND_LOGICAL_DEVICE_STATUS_MOUNTED);
		...
		case IDM_TOOL_REMOVE_FROM_RAID:
			return FALSE;
		case IDM_TOOL_FIX_RAID_STATE:
			return IsOperatable() && !IsEmergency() && IsFixRaidStateRequired();
		default:
			ATLASSERT(FALSE);
			return FALSE;
		}
