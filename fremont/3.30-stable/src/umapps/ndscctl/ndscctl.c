...
HANDLE
pOpenDeviceByNumber(DWORD DeviceNumber, TCHAR DeviceNamePrefix[])
{
	// Device Name Format: \\.\PhysicalDriveXX
	TCHAR deviceName[24];
	HANDLE hDisk;
	...
	result = StringCchPrintf(
		deviceName, RTL_NUMBER_OF(deviceName), 
		_T("\\\\.\\%s%d"), DeviceNamePrefix, DeviceNumber);
...
NdscCtlGetScsiAddressOfDevice(
	...
	device = pOpenDeviceByNumber(DeviceNumber, DeviceNamePrefix);
	if(device == INVALID_HANDLE_VALUE) {
		return FALSE;
	}

	fSuccess = pGetScsiAddressForDisk(device, ScsiAddress);
	if(!fSuccess) {
		CloseHandle(device);
		return FALSE;
	}
