...
if /i "%BUILDP_ARCH%" equ "i386" (
	...
	if defined BUILDP_LFS (
		(call :run_build -WINXP %CONF% %_BUILDP_BASE_DIR%drivers\lfsfilter) || goto build_error
	)
	if defined BUILDP_NDFS (
		(call :run_build -WINXP %CONF% %_BUILDP_BASE_DIR%drivers\ndasfat) || goto build_error
		(call :run_build -WINXP %CONF% %_BUILDP_BASE_DIR%drivers\ndasntfs) || goto build_error
		(call :run_build -WINXP %CONF% %_BUILDP_BASE_DIR%drivers\ndasrofs) || goto build_error
	)
...
if /i "%BUILDP_ARCH%" equ "amd64" (
	...
	if defined BUILDP_LFS (
		(call :run_build -WS03 AMD64 %CONF% %_BUILDP_BASE_DIR%drivers\lfsfilter) || goto build_error
	)
	if defined BUILDP_NDFS (
