...
typedef enum _NDAS_RAID_MEMBER_FLAG {
	...
	// Disk is marked as having bad sector.
	NDAS_RAID_MEMBER_FLAG_BAD_SECTOR            = 0x00000400,
	// Disk is marked as bad disk.
	NDAS_RAID_MEMBER_FLAG_BAD_DISK			= 0x00000800,
	// Disk is not registered. Cannot be set by ndasop.
	NDAS_RAID_MEMBER_FLAG_REPLACED_BY_SPARE           = 0x00001000,
	// Disk is not registered. Cannot be set by ndasop.
	NDAS_RAID_MEMBER_FLAG_NOT_REGISTERED           = 0x00002000,
	//
	NDAS_RAID_MEMBER_FLAG_IRRECONCILABLE		= 0x00004000,
	NDAS_RAID_MEMBER_FLAG_DEGRADE_MODE_CONFLICT		= 0x00008000,
	...
	NDAS_RAID_MEMBER_FLAG_INVALID_MEMBER = NDAS_RAID_MEMBER_FLAG_OFFLINE|
												...
												NDAS_RAID_MEMBER_FLAG_BAD_SECTOR|
												NDAS_RAID_MEMBER_FLAG_BAD_DISK|
												NDAS_RAID_MEMBER_FLAG_NOT_REGISTERED|
												NDAS_RAID_MEMBER_FLAG_REPLACED_BY_SPARE|
												NDAS_RAID_MEMBER_FLAG_IRRECONCILABLE|
												NDAS_RAID_MEMBER_FLAG_DEGRADE_MODE_CONFLICT
} NDAS_RAID_MEMBER_FLAG;

__inline BOOL NdasOpIsValidMember(DWORD NdasRaidMemberFlags)
{
	return (!(NdasRaidMemberFlags & NDAS_RAID_MEMBER_FLAG_INVALID_MEMBER) && 
		(NdasRaidMemberFlags & NDAS_RAID_MEMBER_FLAG_ONLINE));
}

__inline BOOL NdasOpIsConflictMember(DWORD NdasRaidMemberFlags)
{
	return (NdasRaidMemberFlags & NDAS_RAID_MEMBER_FLAG_IRRECONCILABLE);
}
...
typedef struct _NDASOP_RAID_INFO {
	...
	GUID 				RaidSetIdBackup;	// Set only when Normal -> Degrade mode
...
//		UCHAR			DefectCode; // From primary device's RMD. NDAS_UNIT_DEFECT_*
	...
	} Members[MAX_NDAS_LOGICALDEVICE_GROUP_MEMBER]; // Indexed by unit index.
...
typedef struct _NDAS_LOGICALUNIT_DEFINITION * PNDAS_LOGICALUNIT_DEFINITION; 
...
NdasVsmReadLogicalUnitDefinition(
	__in HNDAS NdasHandle,
	__inout PNDAS_LOGICALUNIT_DEFINITION LogicalUnitDefinition);
