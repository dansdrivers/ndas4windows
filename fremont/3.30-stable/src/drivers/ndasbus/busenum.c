...
--*/
{
    ...
    case IOCTL_NDASBUS_PLUGIN_HARDWARE_EX2:
        if (
            ...
            // field is set to the declared size of the struct
            //
            ((sizeof (NDASBUS_PLUGIN_HARDWARE_EX2) + sizeof(UNICODE_NULL) * 2) <=
             inlen) &&
            ...
            // and *not* the size of the struct plus the multi_sz
            //
            (sizeof (NDASBUS_PLUGIN_HARDWARE_EX2) ==
             ((PNDASBUS_PLUGIN_HARDWARE_EX2) buffer)->Size)) {
	...
	case IOCTL_NDASBUS_ADD_TARGET:
	{
			...
			(sizeof(NDASBUS_ADD_TARGET_DATA) <= inlen)) 
		{
			...
			//DbgPrint( "addTargetData->StartOffset = %x, addTargetData->LockImpossible = %d\n", 
			//		   addTargetData->StartOffset, addTargetData->LockImpossible );
	...
	case IOCTL_NDASBUS_DVD_GET_STATUS:
		{
			...
			if(pdoData == NULL) {
				...
				if(pdoData->LanscsiAdapterPDO.Flags & LSDEVDATA_FLAG_LURDESC) {
					...
					NDAS_ASSERT( FALSE );
#if 0
					PLURELATION_DESC lurDesc = (PLURELATION_DESC)pdoData->LanscsiAdapterPDO.AddDevInfo;
					PLURELATION_NODE_DESC lurnRootNode = lurDesc->LurnDesc;
					...
					if(lurDesc->LurnDescCount == 0) {
						Bus_KdPrint_Cont (fdoData, BUS_DBG_IOCTL_ERROR,
							("IOCTL_NDASBUS_DVD_GET_STATUS  No DVD root Device\n"));
						status = STATUS_UNSUCCESSFUL;
						break;
					}
					if(lurnRootNode->LurnType != NDASSCSI_TYPE_ATAPI_CDROM)
					{
						Bus_KdPrint_Cont (fdoData, BUS_DBG_IOCTL_ERROR,
