...
NCommVerifyNdasDevWithDIB(
	...
	for(idx_unit = 0; idx_unit < AddTargetData->ulNumberOfUnitDiskList; idx_unit++) {
		...
		RtlCopyMemory( &targetAddr.Address[0].NdasAddress,
					   &unit->Address,
		...
		RtlCopyMemory( &bindingAddr.Address[0].NdasAddress,
					   &unit->NICAddr,
					   TDI_ADDRESS_LENGTH_LPX );
		...
		RtlCopyMemory( &unit->NICAddr,
					   &boundAddr.Address[0].NdasAddress,
					   TDI_ADDRESS_LENGTH_LPX );
