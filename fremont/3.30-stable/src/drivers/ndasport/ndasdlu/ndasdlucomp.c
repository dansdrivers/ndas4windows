	  ...
	  )
{
	...
	if(Ccb->CcbStatus == CCB_STATUS_STOP) {
		...
		if(Ccb->CcbStatus == CCB_STATUS_BUSY) {
			...
			status = NdasDluQueueWorkItem(LogicalUnitExtension, &workitemCtx);

			if (!NT_SUCCESS(status))
			{
				//
				// When NdasDluQueueWorkItem fails, we should fail the SRB
				//
				...
				//
				// However, the return status is still the same.
				//
			}
