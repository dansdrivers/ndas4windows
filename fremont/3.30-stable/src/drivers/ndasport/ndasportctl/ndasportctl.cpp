...
NdasPortCtlConvertDeviceIndentiferToLpxTaLsTransAddress(
	...
	PLPX_ADDRESS	lpxAddress = &TaLsTransAddress->Address[0].NdasAddress.Lpx;
	...
	__in ULONG BufferLength)
{
	...
	//
	//	Decode SRB IOCTL return code.
	//
	if( pSrbIoControl->ReturnCode ==  0x12 /* SRB_STATUS_DATA_OVERRUN */) {

		HeapFree(GetProcessHeap(), 0, lpInBuffer);

		return HRESULT_FROM_WIN32(ERROR_INSUFFICIENT_BUFFER);

	} else	if (0 != pSrbIoControl->ReturnCode &&
