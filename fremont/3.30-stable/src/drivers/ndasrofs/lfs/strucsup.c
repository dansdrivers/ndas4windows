...
#else
		if (NdasNtfsIoFreeErrorLogEntry) {

			NdasNtfsIoFreeErrorLogEntry( Lfcb->ErrorLogPacket );
			
		} else {

			RtlZeroMemory( Lfcb->ErrorLogPacket, ERROR_LOG_MAXIMUM_SIZE );
			IoWriteErrorLogEntry( Lfcb->ErrorLogPacket );
		}
#endif
