...
This module contains code which defines the LPX transport 
...

{
	...
	DbgPrint( "KdDebuggerNotPresent = %p *KdDebuggerNotPresent = %d\n", KdDebuggerNotPresent, *KdDebuggerNotPresent );
	...
	//LpxDebug |= LPX_DEBUG_CONNECT;
	//LpxDebug |= LPX_DEBUG_DYNAMIC;
	//LpxDebug |= LPX_DEBUG_TEARDOWN;
	//LpxDebug |= LPX_DEBUG_TEMP;
	//LpxDebug |= LPX_DEBUG_CONNOBJ;
    ...
    RtlCopyMemory(LpxRegistryPath.Buffer, RegistryPath->Buffer, RegistryPath->MaximumLength);
    ...
    LpxAddressTable = LpxAddressFileTable + (LpxConfig->InitAddressFiles + 2);
...

{
    ...
    switch (IrpSp->Parameters.DeviceIoControl.IoControlCode) {
		...
		case IOCTL_LPX_QUERY_ADDRESS_LIST: {
			...
			RtlZeroMemory( &socketLpxAddressList, sizeof(SOCKETLPX_ADDRESS_LIST) );
			...
			if (IsListEmpty(&LpxDeviceList)) {
				...
				RtlCopyMemory( outputBuffer, &socketLpxAddressList, sizeof(SOCKETLPX_ADDRESS_LIST) );
...
loop:
			...
			for (thisEntry = LpxDeviceList.Flink;
				 thisEntry != &LpxDeviceList && thisEntry->Flink != &LpxDeviceList;
				 thisEntry = thisEntry->Flink) {

				PDEVICE_CONTEXT deviceContext;
				PDEVICE_CONTEXT deviceContext2;
    
				deviceContext = CONTAINING_RECORD( thisEntry, DEVICE_CONTEXT, Linkage );
				deviceContext2 = CONTAINING_RECORD( thisEntry->Flink, DEVICE_CONTEXT, Linkage );

				if (deviceContext->MediumSpeed < deviceContext2->MediumSpeed) {

					RemoveEntryList(thisEntry);
					InsertTailList( &LpxDeviceList, thisEntry );
			...
			for (thisEntry = LpxDeviceList.Flink;
				...
				NDAS_ASSERT( thisEntry->Flink == &LpxDeviceList || 
							 deviceContext->MediumSpeed >= CONTAINING_RECORD(thisEntry->Flink, DEVICE_CONTEXT, Linkage)->MediumSpeed );
				...
				if (deviceContext != SocketLpxDeviceContext		&& 
					...
					NDAS_ASSERT( socketLpxAddressList.iAddressCount <= MAX_SOCKETLPX_INTERFACE );
		...
		case IOCTL_LPX_GET_RX_DROP_RATE: {
			...
			*pulData = PacketRxDropRate;
			...
			DebugPrint( 2, ("[LPX]IOCTL_LPX_GET_RX_DROP_RATE: %d\n", *pulData) );
		...
		case IOCTL_LPX_SET_RX_DROP_RATE: {
			...
			DebugPrint( 2, ("[LPX]IOCTL_LPX_SET_RX_DROP_RATE: %d -> %d\n", PacketRxDropRate, *pulData) );
		...
		case IOCTL_LPX_GET_TX_DROP_RATE: {
			...
			*pulData = PacketTxDropRate;
			...
			DebugPrint( 2, ("[LPX]IOCTL_LPX_GET_TX_DROP_RATE: %d\n", *pulData) );
		...
		case IOCTL_LPX_SET_TX_DROP_RATE: {
			...
			DebugPrint( 2, ("[LPX]IOCTL_LPX_SET_TX_DROP_RATE: %d -> %d\n", PacketRxDropRate, *pulData) );
		...
		case IOCTL_NDIS_QUERY_GLOBAL_STATS: {
			...
			ndisOid = (PNDIS_OID)inputBuffer;

			DebugPrint( 2, ("LPX: IOCTL_NDIS_QUERY_GLOBAL_STATS, ndisOid = %x, inputBuffer = %p, "
							"outputBuffer = %p, Irp->MdlAddress= %p, outputBufferLength = %d\n",
							 *ndisOid, inputBuffer, outputBuffer, Irp->MdlAddress, outputBufferLength) );

			if (*ndisOid == OID_GEN_LINK_SPEED) {

				if (outputBufferLength != sizeof(DeviceContext->MediumSpeed) ) {
				...
				if (SocketLpxPrimaryDeviceContext) {

					*((PULONG)outputBuffer) = SocketLpxPrimaryDeviceContext->MediumSpeed;
				
				} else {

					NDAS_ASSERT(FALSE);
					*((PULONG)outputBuffer) = DeviceContext->MediumSpeed;
				}
...

{
	...
	for (thisEntry = LpxDeviceList.Flink;
		...
		if (DeviceContext->MediumSpeed > deviceContext->MediumSpeed) {

			RemoveEntryList( &DeviceContext->Linkage );
			InsertTailList( &deviceContext->Linkage, &DeviceContext->Linkage );
	...
	for (thisEntry = LpxDeviceList.Flink;
	    ...
	    IF_LPXDBG (LPX_DEBUG_DYNAMIC) {
			...
			LpxPrint2 ("%p MediumSpeed %d\n", deviceContext, deviceContext->MediumSpeed);
		}
	}
