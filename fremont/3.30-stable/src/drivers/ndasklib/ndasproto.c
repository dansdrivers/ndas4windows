	...
	)
{
	...
	pduDesc.Command	= VENDOR_OP_RESET;
	...
	status = LspVendorRequest( LSS, &pduDesc, &pduResponse );

	if (NT_SUCCESS(status)) {
	
		if (pduResponse != LANSCSI_RESPONSE_SUCCESS) {
		
			KDPrintM( DBG_LURN_ERROR, ("reset faild\n") );
			status = STATUS_LOCK_NOT_GRANTED;

		} else {

			KDPrintM(DBG_LURN_ERROR, ("reset success\n") );
		}
	}
