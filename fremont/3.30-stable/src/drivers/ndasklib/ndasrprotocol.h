...
#define	NRMX_ARBITRRATOR_LPX_PORT		((UINT16)0x0010)	// stream

#define NRMX_MAX_MEMBER_DISK			 16
#define NRMX_MAX_REQUEST_SIZE			128
#define NRMX_IO_LOCK_CLEANUP_TIMEOUT	120		// in seconds
...
typedef struct _NRMX_HEADER {
	...
	UINT16	Length2;				// byte 8.   Length including this header. 
	UINT16	Sequence2;				// byte 10.  Sequence number. Incease by one for every command. 
	...
	UINT32 	Usn4;					// Rmd Usn when send request
...
C_ASSERT( NRMX_MAX_REQUEST_SIZE >= SIZE_OF_NRMX_NODE_CHANGE(NRMX_MAX_MEMBER_DISK) );
...
typedef struct _NRMX_CHANGE_STATUS {
	...
	UINT32		Usn4;					// If Usn is updated, reload DIB and RMD.
...
typedef struct _NRMX_ACQUIRE_LOCK {
	...
	UINT64		BlockAddress8;	// in sector for block IO lock
	UINT32		BlockLength4;	// in sector.
...
typedef struct _NRMX_ACQUIRE_LOCK_REPLY {
	...
	UINT64		LockId8;		// Valid for result Granted/waiting
	UINT8		LockType;		// NRMX_LOCK_TYPE_BLOCK_IO
	UINT8		LockMode;		// NRMX_LOCK_MODE_*
