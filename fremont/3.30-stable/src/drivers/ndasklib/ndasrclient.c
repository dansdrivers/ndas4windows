	...
	) 
{
	...
	while (!IsListEmpty(&NdasrClient->CcbQueue)) {
		...
		if (ccb->OperationCode == CCB_OPCODE_EXECUTE && 
			...
			DebugTrace( DBG_LURN_NOISE, ("ccb->DataBufferLength %d\n", ccb->DataBufferLength) );
		...
		if (status != STATUS_SUCCESS) {
			...
			NDAS_ASSERT( status == STATUS_SUCCESS || status == STATUS_CLUSTER_NODE_UNREACHABLE );
	...
	) 
{
	...
	if (NdasrClient->EmergencyMode == TRUE || NdasrClient->Lurn->NdasrInfo->ParityDiskCount == 0) {
