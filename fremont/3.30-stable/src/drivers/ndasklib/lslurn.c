...
#define _LUR_SET_OPTIONAL_FEATURE(SUPPORTED_FEAT, ENABLED_FEAT, NDAS_FEATURE) { \
	if((SUPPORTED_FEAT)&(NDAS_FEATURE)) {	\
		...
		KDPrintM(DBG_LURN_INFO, ((#NDAS_FEATURE " not supported.\n")));	\
	} \
}
	...
	)
{
	...
	if(Lur->SupportedNdasFeatures & NDASFEATURE_SIMULTANEOUS_WRITE){
	...
	) 
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("return 0x%08lx : Idx:%d\n", status, Lurn->LurnId) );
