...
#pragma alloc_text (PAGE, DraidPnpAddAddressHandler)
#pragma alloc_text (PAGE, DraidPnpDelAddressHandler)
...
struct _DRAID_GLOBALS *g_DraidGlobals = NULL;
	...
	if (status == STATUS_SUCCESS) {
			...
			listEntry = listEntry->Flink) 
		{
			...
			if (RtlCompareMemory(&Arbiter->Rmd.RaidSetId, &RegMsg.RaidSetId, sizeof(GUID)) == sizeof(GUID) &&
				...
				if (Arbiter->Status != DRAID_ARBITER_STATE_TERMINATING) {
					MatchFound = TRUE;
				} else {
