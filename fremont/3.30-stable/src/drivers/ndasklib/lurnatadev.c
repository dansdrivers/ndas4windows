	...
	)
{
	...
	do {
		...
		if (info.dma_ultra & 0x00ff											&&
			...
			if (info.hw_config & 0x2000) {
				...
				if (Lurn->UDMARestrictValid) {
					...
					if (DmaMode > Lurn->UDMARestrict) {
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("UDMA restriction applied. UDMA=%c\n", DmaMode) );
					}
				}
#if __DETECT_CABLE80__
			}
#endif
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Ultra DMA %c detected.\n", DmaMode) );
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("DMA mode %d detected.\n", (int)DmaMode) );
		...
		if (FlagOn(ideDisk->LuHwData.PduDescFlags, PDUDESC_FLAG_DMA)) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("DMA Feature = %x\n", DmaFeature) );
	...
	if (info.command_set_1 & 0x0001) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Smart supported.\n") );
		
		ideDisk->LuHwData.LudataFlags |= LUDATA_FLAG_SMART_SUPPORT;
		
		if (info.cfs_enable_1 & 0x0001) {
		
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Smart enabled.\n") );
	...
	if (info.command_set_1 & 0x0008) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Power management supported\n") );
		...
		if (info.cfs_enable_1 & 0x0008) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Power management enabled\n") );
		}
	}
	...
	if (info.command_set_1 & 0x20) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Write cache supported\n") );

		if (info.cfs_enable_1 & 0x20) {

			ideDisk->LuHwData.LudataFlags |= LUDATA_FLAG_WCACHE_ENABLED;
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Write cache enabled\n") );
		}
	}
	...
	if (info.cfsse & 0x0040) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Native force unit access support.\n") );
		...
		if (info.csf_default & 0x0040) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Native force unit access enabled.\n") );
		}
	}
	...
	)
{
	...
	do {
		...
		if (KeReadStateEvent(events[1])) {
			...
			status = EnterBufferLockIoIdle( &IdeDisk->BuffLockCtl, IdeDisk->LanScsiSession, &IdeDisk->LuHwData );
...
#if 0
					for (lockIdx=NDAS_LOCK_COUNT - 1; lockIdx<NDAS_LOCK_COUNT; lockIdx++) {
			...
			if (requireNotifyReconnect == TRUE) {
				...
				if (IsListEmpty(&IdeDisk->CcbQueue)) {
					...
					while (listEntry != &IdeDisk->CcbQueue) {
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("CCBSTATUS_FLAG_RECONNECTING\n") );
					}
	...
	)
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("SendCmdIn: BuffSz=%x DrvNo=%x F=%x SC=%x SN=%x CL=%x CH=%x DH=%x CM=%x\n",
	...
	switch(sendCmdIn.irDriveRegs.bCommandReg) {
		...
		case SMART_CMD:
			...
			if(selectDev != IdeDisk->LuHwData.LanscsiTargetID) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Wrong device selected. Select dev=%d. Correct it to %d\n",
	...
	)
{
	...
	status = LurnIdeDiskWriteNoFua( Ccb,
									IdeDisk,
									DataBuffer,
									LogicalBlockAddress,
									TransferBlocks,
