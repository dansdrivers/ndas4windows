...
UINT32	DevLockIdMap[MAX_TARGET_ID][NDAS_NR_GPLOCK] = {
	{-1, 0, 2, 3}, // Target 0 uses lock 0 for buffer lock
	{-1, 1, 2, 3}  // Target 0 uses lock 1 for buffer lock
};
...
LockIdToTargetLockIdx(ULONG HwVesion, ULONG TargetId, ULONG LockId) {
	...
	NDAS_ASSERT(TargetId < MAX_TARGET_ID);
...
LurnIdeDiskDeviceLockControl(
	...
	if (devLockControl->LockId == LURNDEVLOCK_ID_BUFFLOCK) {
		...
		NDAS_ASSERT( LsCcbIsFlagOn(Ccb, CCB_FLAG_CALLED_INTERNEL) );
	...
	NDAS_ASSERT(IdeDisk->LuHwData.LanscsiTargetID < MAX_TARGET_ID);
...
//			Ccb->CcbStatus = CCB_STATUS_COMMAND_FAILED;
		...
		if (devLockControl->LockId == LURNDEVLOCK_ID_BUFFLOCK) {
			...
			NDAS_ASSERT( LockCacheIsAcquired(&IdeDisk->LuHwData,devLockControl->LockId) == FALSE ); 
		}
		...
		if(IdeDisk->BuffLockCtl.BufferLockSmartControl == TRUE) {
			if (devLockControl->LockId != LURNDEVLOCK_ID_BUFFLOCK) {
		...
		if(NT_SUCCESS(status)) {
			...
			//
			if(devLockControl->LockId == LURNDEVLOCK_ID_BUFFLOCK && IdeDisk->BuffLockCtl.BufferLockSmartControl) {
				KDPrintM(DBG_OTHER_ERROR, ("Bufflock control off.\n"));
				IdeDisk->BuffLockCtl.BufferLockSmartControl = FALSE;
			}
...
QueueBufferLockRequest(
	...
	status = LspTextTartgetData(
		...
		TimeOut);
	if(!NT_SUCCESS(status)) {
		KDPrintM(DBG_OTHER_ERROR, ("LspTextTartgetData() failed. Read the target data. STATUS=%08lx\n", status));
		goto exit;
	}
	...
	status = LspTextTartgetData(
		...
		TimeOut);
	if(!NT_SUCCESS(status)) {
		KDPrintM(DBG_OTHER_ERROR, ("LspTextTartgetData() failed. Write target data. STATUS=%08lx\n", status));
		goto exit;
	}
...
WaitForBufferLockRelease(
	...
	*LockAcquired = FALSE;
	...
	ASSERT(LURN_IDE_GENERIC_TIMEOUT >= maximumWaitTime.QuadPart);
...
NdasAcquireBufferLock(
	...
	if(BuffLockCtl->BufferLockSmartControl == FALSE) {
		status = OpAcquireDevLock(
			LSS,
...
NdasReleaseBufferLock(
	...
	if(release) {
		...
		if(BuffLockCtl->BufferLockSmartControl) {
...
LMInitialize(
	...
	KDPrintM(DBG_OTHER_INFO, ("KeQueryTickCount()'s 100nano per tick = %u, LOCKMGMT_QUANTUM = %u\n", 
	...
	BuffLockCtl->AccumulatedIOBytes = 0;
	BuffLockCtl->BufferLockSmartControl = InitialState;

	// If the LURN is not a root LURN, turn off smart buffer lock control.
	if (!LURN_IS_ROOT_NODE(Lurn)) {
		// Must be off when the parent controls the buffer lock.
		BuffLockCtl->BufferLockSmartControl = FALSE;
	}

	if(!LURN_IS_ROOT_NODE(Lurn))
		ASSERT(BuffLockCtl->BufferLockSmartControl == FALSE);
	KDPrintM(DBG_LURN_INFO, ("Smart buffer lock control:%x\n",
		BuffLockCtl->BufferLockSmartControl
		));
