...
#undef __MODULE__
#endif // __MODULE__
#define __MODULE__ "ndasprotoata"
	...
	if (OverlappedData) {
		...
		if (status != STATUS_PENDING) {
			...
			KDPrintM( DBG_PROTO_ERROR, 
					  ("Can't recv! %02x:%02x:%02x:%02x:%02x:%02x, status = %x\n",
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[0],
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[1],
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[2],
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[3],
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[4],
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[5],
					   status) );
		}
	...
	)
{
	...
	if (OverlappedData) {
		...
		if (status != STATUS_PENDING) {
			...
			KDPrintM( DBG_PROTO_ERROR, 
					  ("Can't send! %02x:%02x:%02x:%02x:%02x:%02x, status = %x\n",
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[0],
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[1],
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[2],
					   LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node[3],
	 ...
	 )
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_PROTO_INFO, 
				("HeaderEnc=%d DataEnc=%d\n", LSS->HeaderEncryptAlgo, LSS->DataEncryptAlgo) );
