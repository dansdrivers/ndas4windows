...
typedef enum _DRAID_ARBITER_STATE {
	DRAID_ARBITER_STATE_INITALIZING = 0,
	DRAID_ARBITER_STATE_RUNNING,
	DRAID_ARBITER_STATE_TERMINATING
} DRAID_ARBITER_STATE;

typedef enum _DRAID_SYNC_STATE {
	DRAID_SYNC_DONE = 0, 
	DRAID_SYNC_REQUIRED,
...
typedef struct _DRAID_CLIENT_CONTEXT {
	...
	UCHAR		DefectCode[MAX_DRAID_MEMBER_DISK]; 			// Ored value of DRIX_NODE_DEFECT_*. Indexed by Node number
...
#define DRAID_ARBITER_LOCK_STATUS_NONE		0x0   // Not initialized yet
#define DRAID_ARBITER_LOCK_STATUS_GRANTED	0x1
#define DRAID_ARBITER_LOCK_STATUS_PENDING	0x2
...
typedef struct _DRAID_ARBITER_LOCK_CONTEXT {
	...
	LIST_ENTRY	PendingLink;			// Link that arbiter->PendingLockList.Guard with arbiter spinlock.
	
	PDRAID_CLIENT_CONTEXT Owner;		// Client that has acquired or is waiting for this lock. Can be NULL for rebuild IO.
	...
	BOOLEAN	AlreadySendYiledMessage;

	LIST_ENTRY	SiblingLockLink;
...
#define DRAID_AGGRESSIVE_REBUILD_SIZE (1024 * 1024 / 512)	// In sector count. 1Mbytes
...
typedef struct _DRAID_ARBITER_INFO {
	...
	DRAID_ARBITER_STATE	Status;  // DRAID_ARBITER_STATE_*
	...
	UINT32					RaidState;								// DRIX_RAID_STATE_INITIALIZING, DRIX_RAID_STATE_*
	UCHAR					NodeFlags[MAX_DRAID_MEMBER_DISK]; 		// Ored value of DRIX_NODE_FLAG_*. Indexed by Node number
	UCHAR					DefectCodes[MAX_DRAID_MEMBER_DISK]; 	// Ored value of DRIX_NODE_DEFECT_*. Indexed by Node number
	...
	KEVENT					ArbiterThreadReadyEvent;

	HANDLE					ThreadArbiterHandle;
	PVOID					ThreadArbiterObject;
	KEVENT					ArbiterThreadEvent;		// Used to terminate arbiter thread

#if 0	// To do: support Arbitor side flush. This is required if multi-write is used.
	BOOLEAN			FlushAllRequested;
	BOOLEAN			FlushingInProgress;
	PKEVENT			FlushingDoneEvent;
#endif	
	...
	LIST_ENTRY				NewClientList;
	LIST_ENTRY				ClientList;

	PDRAID_CLIENT_CONTEXT	LocalClient;
	PKEVENT					ClientTerminatingEvent;
...
//	PNDAS_LAST_WRITTEN_REGION_BLOCK LwrBlocks; // Last/Locked write region.Length NDAS_BLOCK_SIZE_LWR_REV1 * block size,  8k
