    ...
    if (ARGUMENT_PRESENT( IrpContext )) {
		...
		if (ARGUMENT_PRESENT( Irp )) {
			...
			if(FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_FILE)) {
				...
				ASSERT( IrpContext->RequestIrp == Irp );
				ASSERT( IrpContext->RequestIrpSp == IoGetCurrentIrpStackLocation(Irp) );
			...
			ASSERT( !(Irp == IrpContext->CompleteIrp && IoGetCurrentIrpStackLocation( Irp ) == IrpContext->CompleteIrpSp && FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_ALREADY_COMPLETED)) );
