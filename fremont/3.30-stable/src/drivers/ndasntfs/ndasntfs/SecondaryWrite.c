	...
	)
{
	...
	if (!FlagOn(IrpContext->State, IRP_CONTEXT_STATE_WAIT)) {
		...
		//ASSERT( FALSE );
		//DebugTrace( 0, Dbg, ("Can't wait in NdFatSecondaryCommonWrite\n") );

		status = NtfsPostRequest( IrpContext, Irp );

		DebugTrace( -1, Dbg2, ("NdasNtfsSecondaryCommonWrite:  FatFsdPostRequest -> %08lx\n", status) );
		return status;
	}
	...
	try {
		...
		if (NTOHL(ndfsWinxpReplytHeader->Status4) != STATUS_SUCCESS) {
			...
			DebugTrace( 0, Dbg2, ("NdasNtfsSecondaryCommonWrite: NTOHL(ndfsWinxpReplytHeader->Status4) = %x\n", NTOHL(ndfsWinxpReplytHeader->Status4)) );
		...
		if (ndfsWinxpReplytHeader->FileInformationSet) {
			...
			if (NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) != scb->Header.AllocationSize.QuadPart) {
				...
				ASSERT( (INT64)NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) > scb->Header.AllocationSize.QuadPart );
