    ...
    )
{
	...
	ExAcquireSpinLock( &HwDeviceExtension->CcbListSpinLock, &oldIrql );
	...
	while (!IsListEmpty(&HwDeviceExtension->CcbCompletionList)) {
		...
		NDAS_ASSERT( srb );
		NDAS_ASSERT( srb->SrbFlags & SRB_FLAGS_IS_ACTIVE );
		//NDAS_ASSERT( LsCcbIsStatusFlagOn(ccb, CCBSTATUS_FLAG_TIMER_COMPLETE) );

		//DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, ("completing CCB:%p SCSIOP:%x\n", ccb, ccb->Cdb[0]) );
		...
		if (ccb->CcbStatus == CCB_STATUS_STOP) {
			...
			DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, ("Adapter stopped abnormally\n") );
	...
	ExReleaseSpinLock( &HwDeviceExtension->CcbListSpinLock, oldIrql );
