...

{
	...
	PERESOURCE UnwindSecondaryResource = NULL;
#endif
    ...
    try {
		...
		if (IrpContext && IrpContext->Vcb &&
			(CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb))->Secondary && 
			FlagOn((CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb))->Secondary->Flags, SECONDARY_FLAG_REMOUNT_VOLUME)) {

	        RtlZeroMemory( Vcb, FIELD_OFFSET( VCB, ClearBoundary ) );
	        ...
	        UnwindSecondaryResource = &Vcb->SecondaryResource;
		}
...
#if __NDAS_FAT__
			DebugTrace2( 0, Dbg2, ("HotplugInfo.WriteCacheEnableOverride = %d\n", HotplugInfo.WriteCacheEnableOverride) );
			DebugTrace2( 0, Dbg2, ("HotplugInfo.MediaRemovable = %d\n", HotplugInfo.MediaRemovable) );
			DebugTrace2( 0, Dbg2, ("HotplugInfo.MediaHotplug = %d\n", HotplugInfo.MediaHotplug) );
			DebugTrace2( 0, Dbg2, ("HotplugInfo.DeviceHotplug = %d\n", HotplugInfo.DeviceHotplug) );
#endif
...
#if !__NDAS_FAT_FORCE_WRITE_CACHING__
...
#if __NDAS_FAT_SECONDARY__
			if (UnwindSecondaryResource != NULL) { FatDeleteResource( UnwindSecondaryResource ); }
#endif
...

{
...
#if __NDAS_FAT__
	NDAS_ASSERT( ExIsResourceAcquiredSharedLite(&FatData.Resource) );
#endif
	...
	if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_PRIMARY_STOPPING)) {
		...
		NdasFatPrimarySessionDisconnect( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb) );
	...
	if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Secondary)
		Secondary_Close( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Secondary );
		
	CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Secondary = NULL;

	ASSERT( Vcb->SecondaryOpenFileCount == 0 );
	...
	if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->ThreadHandle != NULL) {
		...
		if (Status == STATUS_SUCCESS) {
			...
			DebugTrace2( 0, Dbg2, ("FatDeleteVcb: thread stoped VolDo = %p\n", CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)) );
...
#if __NDAS_FAT_SECONDARY__
	FatDeleteResource( &Vcb->SecondaryResource );
#endif
	...
	while (primarySessionListEntry = 
			ExInterlockedRemoveHeadList( &CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->PrimarySessionQueue,
										 &CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->PrimarySessionQSpinLock )) 
	{
	...
	NDAS_ASSERT( Vcb->PrimaryOpenFileCount == 0 );

#endif
			
	VolDo_Dereference( CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb) );

	timeOut.QuadPart = -10*NANO100_PER_SEC;		
	Status = KeWaitForSingleObject( &CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->ReferenceZeroEvent, 
								    Executive, 
									KernelMode, 
									FALSE, 
									&timeOut );
...

{
    ...
    try {
		...
		ExInitializeFastMutex( &Dcb->NonPaged->CcbQMutex );
		InitializeListHead( &Dcb->NonPaged->CcbQueue );
...

{
    ...
    try {
		...
		if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CREATE_FILE)) {
		...
		if (!FlagOn(Fcb->NdasFatFlags, ND_FAT_FCB_FLAG_SECONDARY)) {
			...
			InitializeListHead( &Fcb->ParentDcbLinks );
		}
#else
		...
		ExInitializeFastMutex( &Fcb->NonPaged->CcbQMutex );
		InitializeListHead( &Fcb->NonPaged->CcbQueue );
...

{
	...
	NDAS_ASSERT( IsListEmpty(&Fcb->NonPaged->CcbQueue) );
...

{
	...
	NDAS_ASSERT( (*Ccb)->FcbListEntry.Flink == (*Ccb)->FcbListEntry.Blink );
...

{
	...
	if (VcbDeleted) {
		...
		SetFlag( IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_VCB_DELETED );
	}
