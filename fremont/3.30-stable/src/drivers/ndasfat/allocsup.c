    ...
    if ( FatLookupMcbEntry(Vcb, &FcbOrDcb->Mcb, Vbo, Lbo, ByteCount, Index) ) {
        ...
        *Allocated = TRUE;
		...
		NDAS_ASSERT( !FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)	||
					 ...
					 CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY );
#endif
...

{
    ...
    try {
        ...
        if (FcbOrDcb->Header.AllocationSize.LowPart == 0) {
			...
			if (IrpContext->PrimaryRequestInfo.PrimarySessionId != NDASFAT_LOCAL_PRMARY_SESSION_ID									&&
		...
		if (NodeType(FcbOrDcb) == FAT_NTC_FCB) {
			...
			if (FcbOrDcb->Header.AllocationSize.QuadPart == 0) {
				...
				NDAS_ASSERT( vcn == 0 && index != 0 || vcn == NewAllocation );
			
			} else {

				NDAS_ASSERT( vcn == FcbOrDcb->Header.AllocationSize.QuadPart );
				...
				NDAS_ASSERT( newVcn == NewAllocation );
