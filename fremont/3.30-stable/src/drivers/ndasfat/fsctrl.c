...
#define DRVO_BASE_FILESYSTEM_DRIVER	0x00000080   // A driver that is at the bottom of the filesystem stack.
	...
	if ((PVOID)FatControlDeviceObject == VolumeDeviceObject) {
		...
		if (irpSp->Parameters.FileSystemControl.FsControlCode == FSCTL_NDAS_FS_UNLOAD) {
			...
			DebugTrace2( 0, Dbg, ("IoControlCode == FSCTL_NDAS_FS_UNLOAD occured DeviceObject->ReferenceCount = %x\n", 
								   FatDiskFileSystemDeviceObject->ReferenceCount) );

			if (!IsListEmpty(&FatData.VcbQueue)) {
			...
			if (FatData.FileSystemRegistered == TRUE) {
				...
				FatDiskFileSystemDeviceObject->DriverObject->Flags &= ~DRVO_BASE_FILESYSTEM_DRIVER;		
				FatData.FileSystemRegistered = FALSE;
			}
			...
			DebugTrace2( 0, Dbg, ("IoControlCode != FSCTL_NDAS_FS_UNLOAD\n", 0) );
...
#if (__NDAS_FAT_PRIMARY__ || __NDAS_FAT_SECONDARY__ || __NDAS_FAT_PURGE__ || __NDAS_FAT_FLUSH__)
...
#if (__NDAS_FAT_PURGE__ || __NDAS_FAT_FLUSH__)
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdasFatSecondaryCommonFileSystemControl( IrpContext, Irp );
		return Status;
	}
...

{
    ...
    try {
		...
		SetFlag( VolDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_INITIALIZING );
		...
		DebugTrace2( 0, Dbg, ("NdasFatMountVolume: VolDo = %p\n", VolDo) );		
		...
		if (((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->NdfsCallback.QueryPartitionInformation == NULL) {
			...
			if (queryStatus != STATUS_SUCCESS) {
				...
				DebugTrace2( 0, Dbg2, ("NdasFatMountVolume %08lx QueryNetdiskInformation \n", queryStatus) );
				ASSERT( NDASFAT_BUG );
...
#if __NDAS_FAT_ROUTER_SUPPORT__
				...
				if (FlagOn(VolDo->NetdiskPartitionInformation.NetdiskInformation.SupportedFeatures, NDASFEATURE_SIMULTANEOUS_WRITE)) {
					...
					SetFlag( VolDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW );
				}
...
if (IS_WINDOWS2K()) {
				...
				while (1) {
					...
					bacl = FsRtlAllocatePoolWithTag( NonPagedPool, baclLen, NDASFAT_ALLOC_TAG );
					...
					if (queryStatus == STATUS_BUFFER_OVERFLOW) {
						...
						baclLen = bacl->Length;
						ExFreePoolWithTag( bacl, NDASFAT_ALLOC_TAG );
						continue;
					}
...
if (IS_WINDOWS2K()) {
							...
							if (VolDo->NetdiskPartitionInformation.PartitionInformation.StartingOffset.QuadPart/512 >= (INT64)bacl->BlockACEs[baceCount].BlockStartAddr &&
								...
								SetFlag( VolDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_LINKSYS_SUPPORT );
		...
		if (!FlagOn(VolDo->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {
			...
			if (!FatIsFat32(Vcb)) {
				...
				DebugTrace2( 0, Dbg2, ("NdasFat only supports Fat32 FatIndexBitSize(&Vcb->Bpb) = %d\n", FatIndexBitSize(&Vcb->Bpb)) );
				NDAS_ASSERT( FALSE );
				try_return( Status = STATUS_UNRECOGNIZED_VOLUME );
			}
		}
        ...
        if (FatIsMediaWriteProtected(IrpContext, TargetDeviceObject)) {
			...
			DebugTrace( 0, Dbg2, "Set VCB_STATE_FLAG_WRITE_PROTECTED\n", 0 );

        } else {

            ClearFlag( Vcb->VcbState, VCB_STATE_FLAG_WRITE_PROTECTED );
			DebugTrace( 0, Dbg2, "Unset VCB_STATE_FLAG_WRITE_PROTECTED\n", 0 );
        }
		...
		if (VolDo->NetdiskEnableMode == NETDISK_READ_ONLY) {
			...
			NDAS_ASSERT( FlagOn(Vcb->VcbState, VCB_STATE_FLAG_WRITE_PROTECTED) );
		
		} else {

			NDAS_ASSERT( !FlagOn(Vcb->VcbState, VCB_STATE_FLAG_WRITE_PROTECTED) ); // for test only
		}
		...
		if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
			...
			if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW)) { 
				
				if (((PVOLUME_DEVICE_OBJECT)FatData.DiskFileSystemDeviceObject)->NdfsCallback.AddWriteRange(Vcb->Vpb->RealDevice, &VolDo->BlockAceId) != STATUS_SUCCESS) {
...
#if __NDAS_FAT__
		ClearFlag( VolDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_INITIALIZING );
		SetFlag( VolDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_MOUNTED );
#endif
...

{
    ...
    try_exit: NOTHING;
        ...
        if (Vcb->VcbCondition == VcbNotMounted) {
			...
			if ((VolDo->NetdiskEnableMode == NETDISK_READ_ONLY) &&
				FlagOn(VolDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_LINKSYS_SUPPORT)) {
				...
				if (IS_WINDOWS2K() && VolDo->NetdiskEnableMode == NETDISK_READ_ONLY) {

					ASSERT( !FlagOn(Vcb->VcbState, VCB_STATE_FLAG_WRITE_PROTECTED) );
					SetFlag( Vcb->VcbState, VCB_STATE_FLAG_WRITE_PROTECTED );
				}
			}
...

{
	...
	switch (CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode) {

	case NETDISK_READ_ONLY:

		if (!FlagOn(CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, 
					ND_FAT_DEVICE_FLAG_LINKSYS_SUPPORT)) {

			NDAS_ASSERT( Status == STATUS_MEDIA_WRITE_PROTECTED );
		}
...

{
	...
	case FSCTL_NDAS_FS_FLUSH: {
		...
		DebugTrace(+1, Dbg, "FatUserFsCtrl... FSCTL_NDAS_FS_FLUSH_OR_PURGE \n", 0);
		...
		if (FsControlCode == FSCTL_NDAS_FS_PURGE) {
			...
			SetFlag( IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_PURGE );
			
			(VOID) FatPurgeReferencedFileObjects( IrpContext, Vcb->RootDcb, NoFlush );

			NDAS_ASSERT( Vcb->VirtualVolumeFile );
			...
			NDAS_ASSERT( Vcb->RootDcb );

			if (Vcb->RootDcb && Vcb->RootDcb->Specific.Dcb.DirectoryFile) {

				if (Vcb->RootDcb->FirstClusterOfFile == 0) {
					...
					if (BootBcb == NULL || !FatIsBootSectorFat(BootSector)) {
						...
						Vcb->RootDcb->FirstClusterOfFile = Vcb->Bpb.RootDirFirstCluster;
					}
				}

				FatAcquireExclusiveFcb( IrpContext, Vcb->RootDcb );
				ExAcquireSharedStarveExclusive( Vcb->RootDcb->Header.PagingIoResource, TRUE );

				try {
				
					CcPurgeCacheSection ( &Vcb->RootDcb->NonPaged->SectionObjectPointers,
										  NULL,
										  0,
										  FALSE );

					FatRemoveMcbEntry( Vcb, &Vcb->RootDcb->Mcb, 0, 0xFFFFFFFF );

					Vcb->RootDcb->Header.AllocationSize.QuadPart = FCB_LOOKUP_ALLOCATIONSIZE_HINT;
					FatLookupFileAllocationSize( IrpContext, Vcb->RootDcb );
					Vcb->RootDcb->Header.FileSize.QuadPart = Vcb->RootDcb->Header.AllocationSize.QuadPart; 
					CcSetFileSizes( Vcb->RootDcb->Specific.Dcb.DirectoryFile, (PCC_FILE_SIZES)&Vcb->RootDcb->Header.AllocationSize );
				
				} finally {

					ExReleaseResourceLite( Vcb->RootDcb->Header.PagingIoResource );
					FatReleaseFcb( IrpContext, Vcb->RootDcb );
				}

				DebugTrace2( 0, Dbg, ("Vcb->RootDcb->Header.AllocationSize.QuadPart = %I64x\n", Vcb->RootDcb->Header.AllocationSize.QuadPart) );
			}

		} else {
		
			SetFlag( IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_FLUSH );
	        (VOID) FatFlushVolume( IrpContext, Vcb, FlushWithoutPurge );
		}
...
#if __NDAS_FAT_PRIMARY__
	{
		...
		ASSERT( volDo->NetdiskEnableMode == NETDISK_PRIMARY || volDo->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY ||
				(volDo->NetdiskEnableMode == NETDISK_READ_ONLY /*&& FlagOn(volDo->NdasFatFlags, ND_FAT_DEVICE_FLAG_LINKSYS_SUPPORT)*/) );
		...
		if (!IsListEmpty(&volDo->PrimarySessionQueue)) {
			...
			DebugTrace2( 0, Dbg2, ("FSCTL_LOCK_VOLUME: Access PrimarySessionQueue is not Empty\n") );
...
#if __NDAS_FAT_PRIMARY__
	{
		...
		if (!IsListEmpty(&volDo->PrimarySessionQueue)) {
			...
			DebugTrace2( 0, Dbg2, ("FSCTL_DISMOUNT_VOLUME: Access PrimarySessionQueue is not Empty\n") );
			FatCompleteRequest( IrpContext, Irp, Status = STATUS_ACCESS_DENIED );
...

{
    ...
    try {
		...
		if (FlagOn(IrpContext->NdasFatFlags, NDAS_FAT_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
			...
			if (!AbnormalTermination() || ExIsResourceAcquiredExclusiveLite( &Vcb->SecondaryResource )) {
...

{
    ...
    if (!(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Secondary && 
		  FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Secondary->Flags, SECONDARY_FLAG_CLEANUP_VOLUME))) {
    ...
    if (!FlagOn(Vcb->Vpb->Flags, VPB_LOCKED) &&
        ((Vcb->Vpb->ReferenceCount <= 2 + RemainingUserReferences) ||
		 ((CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb))->Secondary && 
		  FlagOn((CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb))->Secondary->Flags, SECONDARY_FLAG_CLEANUP_VOLUME))) &&
...

{
...
#if __NDAS_FAT__
	StatsSize = sizeof(FILE_SYSTEM_STATISTICS) * (IS_WINDOWSXP_OR_LATER() ? KeNumberProcessors : *((PCHAR)(&KeNumberProcessors)));
#else
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		IrpContext->InputBuffer	 = NULL;
		IrpContext->outputBuffer = Irp->AssociatedIrp.SystemBuffer;
		return STATUS_SUCCESS;
	}
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
	    ...
	    try {
		    ...
		    if (Irp->RequestorMode != KernelMode) {
			    ...
			    ProbeForRead( IrpSp->Parameters.FileSystemControl.Type3InputBuffer, InputBufferLength, sizeof( UCHAR ));
...

{
    ...
    while (Links != &FatData.VcbQueue) {
		...
		if (!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) &&
	        ...
	        if (!ExAcquireResourceExclusiveLite( &(Vcb->SecondaryResource), FALSE )) {
