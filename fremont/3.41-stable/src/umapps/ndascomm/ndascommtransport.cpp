	...
	*Socket = INVALID_SOCKET;
	...
	while (writefds.fd_count > 0)
	{
		...
		for (UINT i = 0; i < nbcount; ++i)
		{
			...
			{
				//
				// Take out the connected socket from the nbs list
				//
				connSocket = nbs[i];
				...
				{
					if (INVALID_SOCKET != nbs[j])
					{
						CancelIo(reinterpret_cast<HANDLE>(nbs[j]));
						XTLVERIFY(0 == closesocket(nbs[j]));
					}
				}
				...
				//
				// Change the connected socket to the blocking mode
				//
				ULONG nonblocking = 0;
	...
	//
	// Sometimes error is not returned even if the connection is not made.
	//
