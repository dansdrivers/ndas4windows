...
LONG DbgLevelSvcPnp = DBG_LEVEL_SVC_PNP;
	...
	)
{
	...
	XTLASSERT( ((DEVICE_NOTIFY_SERVICE_HANDLE & m_dwReceptionFlags) == DEVICE_NOTIFY_SERVICE_HANDLE) ||
	...
	RegisterDeviceInterfaceNotification( &GUID_DEVINTERFACE_STORAGEPORT, "StoragePort" );

	for (DWORD i = 0; i < RTL_NUMBER_OF(LogicalUnitInterfaceGuids); ++i) {

		RegisterDeviceInterfaceNotification( LogicalUnitInterfaceGuids[i].Guid,
	...
	)
{
	...
	const DWORD MaxWait = 2;
	DWORD		retry   = 0;

	if (IsNdasPortMode() == FALSE) {
	...
	do {
		...
		hr = pGetNdasSlotNumberFromDeviceNameW( pdbhdr->dbcc_name, &slotNo );

		if (FAILED(hr) && HRESULT_FROM_WIN32(ERROR_DEV_NOT_EXIST) == hr) {

			// Wait for the instance of actual NDASSCSI miniport 
			
			NdasUiDbgCall( 2, "NDASSCSI miniport is not available yet. Wait (%d/%d)\n", retry + 1, MaxWait );
		
		} else if (FAILED(hr)) {

			NdasUiDbgCall( 1, "pGetNdasSlotNumberForScsiPortDeviceName(%ls) failed, hr=0x%X\n",  
	...
	if (FAILED(hr)) {
		...
		NdasUiDbgCall( 0, "failed return pGetNdasSlotNumberForScsiPortDeviceName(%ls) hr=0x%X\n", 
					   (LPCWSTR) pdbhdr->dbcc_name, hr );
		return;
	}

	XTL::AutoFileHandle deviceHandle = ::CreateFile( (LPCTSTR)pdbhdr->dbcc_name,
	...
	if (deviceHandle.IsInvalid()) {
		...
		NdasUiDbgCall( 0, "CreateFile(%ls) failed, error=0x%X\n", 
	...
	NdasUiDbgCall( 2, "NDAS Logical Device Slot No: %d.\n", slotNo );
	...
	RegisterStoragePortHandleNotification( deviceHandle, slotNo, pdbhdr->dbcc_name );
	...
	__in LPCTSTR DevicePath)
{
	...
	if (FAILED(hr)) {
		...
		NdasUiDbgCall( 1, "Registering device handle %p failed, hr=0x%X\n", DeviceHandle, hr );
	...
	XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_INFORMATION, 
		"DevicePath = %s Device handle %p Notification Registered successfully.\n", 
...
{
	NdasUiDbgCall( 2, "%s, UnitMask=%08X, Flags=%08X %s\n", 
				   Removal ? "Removal" : "Arrival",dbcv->dbcv_unitmask, dbcv->dbcv_flags );

	// We ignore the media or the network volume

	if (dbcv->dbcv_flags != 0) {
...
{
	NdasUiDbgCall( 2, "UnitMask = %X, Removal = %d\n", UnitMask, Removal );
		...
		if (UnitMask & (1 << index)) {
			...
			if (Removal) {
				...
				if (!pNdasLogicalUnit) {
					...
					//ATLASSERT(FALSE);
					NdasUiDbgCall( 1, "Not a registered NDAS drive letter(%C:).\n", driveLetter );
				...
				NdasUiDbgCall( 2, "Dismounted is a volume of the NDAS logical unit, drive=%C:, address=%08X\n", 
								   driveLetter, logicalUnitAddress.Address );
			...
			COMVERIFY( StringCchPrintf(volumeDeviceName, RTL_NUMBER_OF(volumeDeviceName),  _T("\\\\.\\%C:"), driveLetter) );
			...
			if (IsNdasPortMode()) {
				...
				NDAS_LOGICALUNIT_ADDRESS logicalUnitAddress;
				hr = NdasVolumeExtentsNdasLogicalUnitByName(
					volumeDeviceName, 
					&logicalUnitAddress);
				...
				loc = NdasLogicalUnitAddressToLocation(logicalUnitAddress);

			} else {

				WCHAR volumeDeviceName2[MAX_PATH+1];

				hr = StringCchCopy(volumeDeviceName2, MAX_PATH+1, volumeDeviceName);

				XTLASSERT(SUCCEEDED(hr));

				LPWSTR p = volumeDeviceName2;
				while (*p) ++p;

				if (p > volumeDeviceName2 && *(p-1) == L'\\') {

					*(p-1) = L'\0';
				}

				HANDLE volumeHandle = CreateFileW( volumeDeviceName2,
												   GENERIC_READ, 
												   FILE_SHARE_READ | FILE_SHARE_WRITE,
												   NULL,
												   OPEN_EXISTING,
				...
				if (volumeHandle == INVALID_HANDLE_VALUE) {
					...
					NdasUiDbgCall( 2, "CreateFile(%ls) failed, hr=0x%X\n", volumeDeviceName, hr );

					continue;
				}

				hr = pGetNdasSlotNumberFromStorageDeviceHandle(	volumeHandle, &loc );
			...
			if (FAILED(hr)) {
				...
				NdasUiDbgCall( 2, "pGetNdasLogicalDeviceByNdasLocation failed, address=%08X\n", loc );
				continue;
			}
	...
	)
{
	...
	XTL::AutoFileHandle deviceHandle = ::CreateFile( reinterpret_cast<LPCTSTR>(pdbhdr->dbcc_name),
													 GENERIC_READ | GENERIC_WRITE,
													 FILE_SHARE_READ | FILE_SHARE_WRITE,
													 NULL,
													 OPEN_EXISTING,
	...
	if (deviceHandle.IsInvalid()) {
		...
		NdasUiDbgCall( 1, "CreateFile(%ls) failed, error=0x%X\n", (LPCWSTR) pdbhdr->dbcc_name, GetLastError() );
	...
	if (IsNdasPortMode())
	{
		...
		hr = pGetNdasSlotNumberFromStorageDeviceHandle( deviceHandle, &ndasLocation );

		if (FAILED(hr)) {

			NdasUiDbgCall( 2, "failed, handle=%p, hr=0x%X\n", deviceHandle, hr );
			return;
		}
	}

	NdasUiDbgCall( 2, "deviceHandle = %p, ndasLocation=%08Xh.\n", deviceHandle, ndasLocation );
	...
	RegisterLogicalUnitHandleNotification( deviceHandle, ndasLocation, pdbhdr->dbcc_name );
...
{
	NdasUiDbgCall( 2, "OnDeviceHandleQueryRemove: Device Handle %p, Notify Handle %p\n", 
...
{
	NdasUiDbgCall( 2, "OnDeviceHandleQueryRemoveFailed: Device Handle %p, Notify Handle %p\n", 
				   pdbch->dbch_handle, pdbch->dbch_hdevnotify );

	const DEVICE_HANDLE_NOTIFY_DATA* notifyData = GetDeviceHandleNotificationData(pdbch->dbch_hdevnotify);

	if (notifyData == NULL) {

		ATLASSERT(FALSE);
		NdasUiDbgCall( 1, "Notify Handle is not registered.\n" );
		return;
	}
...
VOID 
CNdasServiceDeviceEventHandler::OnDeviceHandleRemovePending (PDEV_BROADCAST_HANDLE pdbch)
{
	NdasUiDbgCall( 2, "Device Handle %p, Notify Handle %p\n", pdbch->dbch_handle, pdbch->dbch_hdevnotify );
	...
	NdasUiDbgCall( 2, "notifyData->HandleType = %d\n", notifyData->HandleType );
	...
	case PnpStoragePortHandle: {
		...
		if (FAILED(hr)) {
			...
			NdasUiDbgCall( 1, "Logical Device not found, ndasLocation=%08X.\n", ndasLocation );
			return;
		}
...
VOID 
CNdasServiceDeviceEventHandler::OnDeviceHandleRemoveComplete (PDEV_BROADCAST_HANDLE pdbch)
{
	// Device Handle Remove Complete is called only on Surprise Removal

	NdasUiDbgCall( 2, "pdch %p Device Handle %p, Notify Handle %p\n", pdbch, pdbch->dbch_handle, pdbch->dbch_hdevnotify );
...
VOID
CNdasServiceDeviceEventHandler::OnDeviceHandleCustomEvent (PDEV_BROADCAST_HANDLE pdbch)
{
	NdasUiDbgCall( 3, "Device Handle %p, Notify Handle %p\n", pdbch->dbch_handle, pdbch->dbch_hdevnotify );

	// Device Handle Remove Complete is called on Surprise Removal

	if (m_bInitialized == FALSE) {

		// ridiculous but it's all I can do at this time

		for (;;) {
			
			UINT retry = 0;

			NdasUiDbgCall( 2, "not yet initialized, retry = %d\n", retry );

			Sleep(2000);
			
			if (m_bInitialized == TRUE || ++retry > 5) {
	...
	if (IsEqualGUID(NDAS_DLU_EVENT_GUID, pdbch->dbch_eventguid)) {
		...
		XTLTRACE2( NDASSVC_PNP, TRACE_LEVEL_WARNING, "IoEvent - IO_VOLUME_NAME_CHANGE\n" );
		...
		XTLTRACE2( NDASSVC_PNP, TRACE_LEVEL_WARNING,
				   "IoEvent - IO_VOLUME_MOUNT, nameoffset=%d, data=%08X\n", pdbch->dbch_nameoffset, *(LPDWORD)pdbch->dbch_data );

		PDEVICE_EVENT_MOUNT mountData = reinterpret_cast<PDEVICE_EVENT_MOUNT>(pdbch->dbch_data);
...
void
CNdasServiceDeviceEventHandler::OnDeviceInterfaceArrival (PDEV_BROADCAST_DEVICEINTERFACE dbcc)
{
	NdasUiDbgCall( 4, "in\n" );
...
HRESULT
CNdasServiceDeviceEventHandler::RegisterLogicalUnits()
{
	NdasUiDbgCall( 2, "in \n" );

	HRESULT hr;

	HDEVINFO devInfoSet = SetupDiGetClassDevsW( NULL,
												NULL,
												NULL,
	...
	if (devInfoSet == INVALID_HANDLE_VALUE) {
		...
		ATLASSERT(FALSE);
		NdasUiDbgCall( 0, "SetupDiGetClassDevsW failed, hr=0x%X\n", hr );
		return hr;
	}
	...
	for (DWORD index = 0; ; index++) {
		...
		if (!success) {
			...
			if (GetLastError() != ERROR_NO_MORE_ITEMS) {
				...
				ATLASSERT(FALSE);
				NdasUiDbgCall( 0, "SetupDiEnumDeviceInfo failed, hr=0x%X\n", hr );
			}
	...
	for (DWORD i = 0; i < RTL_NUMBER_OF(LogicalUnitInterfaces); ++i) {
		...
		RegisterLogicalUnit( devInfoSet, LogicalUnitInterfaces[i].Guid );
	}
...
HRESULT
CNdasServiceDeviceEventHandler::RegisterLogicalUnit (
	...
	)
{
	...
	for (DWORD index = 0; ; ++index) {
		...
		success = SetupDiEnumDeviceInterfaces( DevInfoSet,
											   NULL,
											   InterfaceGuid,
											   index,
											   &devIntfData );
		...
		if (!success) {
			...
			if (GetLastError() == ERROR_NO_MORE_ITEMS) {
				...
				if (index == 0) {
					...
					XTLTRACE2(NDASSVC_PNP, TRACE_LEVEL_WARNING, "No devices\n" );
				}
		...
		success = SetupDiGetDeviceInterfaceDetail( DevInfoSet,
												   &devIntfData,
												   devIntfDetail,
												   devIntfDetailSize,
												   &devIntfDetailSize,
		...
		HANDLE deviceHandle = CreateFile( devIntfDetail->DevicePath,
										  GENERIC_READ,
										  FILE_SHARE_READ | FILE_SHARE_WRITE,
										  NULL,
										  OPEN_EXISTING,
		...
		if (IsNdasPortMode()) {
			...
			hr = pNdasPortGetLogicalUnitAddress(deviceHandle, &luAddress);

			if (FAILED(hr)) {

				XTLVERIFY( CloseHandle(deviceHandle) );
				continue;
			}

			ndasLocation = NdasLogicalUnitAddressToLocation(luAddress);
			...
			if (FAILED(hr)) {
				...
				NdasUiDbgCall( 2, "GetNdasSlot fail %ls hr=0x%X\n", devIntfDetail->DevicePath, hr );
				XTLVERIFY( CloseHandle(deviceHandle) );
				continue;
			}
		}

		NdasUiDbgCall( 2, "GetNdasSlot success %ls\n", devIntfDetail->DevicePath );

		RegisterLogicalUnitHandleNotification( deviceHandle, ndasLocation, devIntfDetail->DevicePath );
		...
		if (IsEqualGUID(*InterfaceGuid, GUID_DEVINTERFACE_DISK)) {
			...
			CComQIPtr<INdasLogicalUnitPnpSink> pNdasLogicalUnitPnpSink(pNdasLogicalUnit);
			pNdasLogicalUnitPnpSink->OnMounted(CComBSTR(devIntfDetail->DevicePath), abnormalities);
		
		} else {

			CComQIPtr<INdasLogicalUnitPnpSink> pNdasLogicalUnitPnpSink(pNdasLogicalUnit);
			pNdasLogicalUnitPnpSink->OnMounted(CComBSTR(devIntfDetail->DevicePath), NDAS_LOGICALUNIT_ABNORM_NONE);
		}
	...
	)
{
	...
	*DevNotifyHandle = NULL;
	...
	HDEVNOTIFY h = RegisterDeviceNotification(RecipientHandle, &filter, ReceptionFlags);

	if (h == NULL) {

		hr = HRESULT_FROM_WIN32(GetLastError());

		NdasUiDbgCall( 2, "RegisterDeviceNotification failed, hDevice=%p, hr=0x%X\n", DeviceHandle, hr );
		return hr;
	}
