...
LONG DbgLevelVolDev = DBG_LEVEL_VOL_DEV;
	...
	__out LPDWORD NdasSlotNumber)
{
	...
	*NdasSlotNumber = *reinterpret_cast<DWORD*>(lpInBuffer + cbHeader);
	...
	NdasUiDbgCall( 2, "NDAS Logical Device Slot: %d\n", *NdasSlotNumber );
	...
	__out LPDWORD NdasSlotNumber)
{
	...
	NdasUiDbgCall( 1, "failed DiskHandle=%p, hr=0x%X. Try the SCSI adapter device directly.\n", 
				   DiskHandle, hr );
	...
	hr = pGetNdasSlotNumberForScsiPortNumber( scsiAddress.PortNumber, NdasSlotNumber );
