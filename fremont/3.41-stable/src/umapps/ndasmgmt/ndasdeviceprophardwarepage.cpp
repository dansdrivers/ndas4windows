...
namespace
{
	bool IsNullNdasDeviceId(const NDAS_DEVICE_ID& deviceId) {
		...
		return( deviceId.Node[0] == 0x00 && deviceId.Node[1] == 0x00 && deviceId.Node[2] == 0x00 &&
				deviceId.Node[3] == 0x00 && deviceId.Node[4] == 0x00 && deviceId.Node[5] == 0x00 );
	}
}
...
CNdasDevicePropHardwarePage::OnInitDialog (HWND hwndFocus, LPARAM lParam)
{
	...
	if (pDevHWInfo->HardwareRevision != 0) {
		...
		strValue.Format( _T("%d"), pDevHWInfo->HardwareRevision );
		m_propList.AddItem( PropCreateSimple(strType, strValue) );
	}
	...
	if (phwi.get() && !::IsNullNdasDeviceId(phwi->NdasDeviceId)) {

		strType.LoadString(IDS_DEVPRO_HW_MAC_ADDRESS);
		
		strValue.Format( _T("%02X:%02X:%02X:%02X:%02X:%02X"), 
						 phwi->NdasDeviceId.Node[0], phwi->NdasDeviceId.Node[1], phwi->NdasDeviceId.Node[2],
						 phwi->NdasDeviceId.Node[3], phwi->NdasDeviceId.Node[4], phwi->NdasDeviceId.Node[5] );
