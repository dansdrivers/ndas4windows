	...
	IN UINT64 cb)
{
	...
	for (; base > 1024 && i < RTL_NUMBER_OF(SUFFIX_IDS) - 1; ++i)
	{
		...
		// 500 / 100 = 5 -> 0.5
		sub = MulDiv(static_cast<int>(base % 1024), 1000, 1024) / 100;
		base /= 1024;
	}
...
CString& 
pHWVersionString(
	__inout CString& strBuffer, 
	__in const NDAS_DEVICE_HARDWARE_INFO* NdasDeviceHardwareInfo)
{
	switch (NdasDeviceHardwareInfo->HardwareVersion) 
	{
	...
	default:
		{
			strBuffer.Format(
				_T("Unsupported Version %d"), 
				NdasDeviceHardwareInfo->HardwareVersion);
		}
	}
	...
	IN NDAS_LOGICALDEVICE_TYPE type)
{
	const ResIDTable<NDAS_LOGICALDEVICE_TYPE> ldtable[] = 
	{
		...
		{NDAS_LOGICALDEVICE_TYPE_DISK_CONFLICT_DIB, IDS_LOGDEV_TYPE_SINGLE_DISK}, // ??
		...
		{NDAS_LOGICALDEVICE_TYPE_DISK_RAID5, IDS_LOGDEV_TYPE_DISK_RAID5},
	...
	IN NDAS_DEVICE_ERROR lastError)
{
	...
	{
		{NDAS_DEVICE_STATUS_ONLINE, IDS_NDAS_DEVICE_STATUS_ONLINE},
		{NDAS_DEVICE_STATUS_NOT_REGISTERED, IDS_NDAS_DEVICE_STATUS_NOT_REGISTERED}
	};

	BOOL fSuccess;

	if (status == NDAS_DEVICE_STATUS_OFFLINE) 
	{

		CString strBuffer;
		fSuccess = strBuffer.LoadString(IDS_NDAS_DEVICE_STATUS_OFFLINE);
		ATLASSERT(fSuccess);
	...
	IN NDAS_UNITDEVICE_SUBTYPE subType)
{
	const ResIDTable<NDAS_UNITDEVICE_DISK_TYPE> diskTypeTable[] = 
	{
		...
		{ NDAS_UNITDEVICE_DISK_TYPE_CONFLICT, IDS_UNITDEV_TYPE_DISK_SINGLE }, //??
		...
		{ NDAS_UNITDEVICE_DISK_TYPE_RAID5,	IDS_UNITDEV_TYPE_DISK_RAID5}
	};
