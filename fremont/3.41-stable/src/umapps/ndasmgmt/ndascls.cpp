...
LONG DbgLevelMagmCls = DBG_LEVEL_MAGM_CLS;

#define NdasUiDbgCall(l,x,...) do {						\
	if (l <= DbgLevelMagmCls) {							\
...
BOOL 
LogicalDevice::PlugIn(BOOL bWritable /* = false */, BOOL bIgnoreWarning /* = false */)
{
	NdasUiDbgCall( 2, "in\n" );

	BOOL fSuccess = ::NdasPlugInLogicalDevice( bWritable, bIgnoreWarning, m_id );
	...
	if (!fSuccess) {
		...
		NdasUiDbgCall( 1, "NdasPlugInLogicalDevice as %s failed: %08X.\n", bWritable ? _T("RW") : _T("RO"), ::GetLastError() );
		::SetLastError(err);
...
LogicalDevice::GetNdasLogicalUnitError()
{
	return m_lastError;
}
