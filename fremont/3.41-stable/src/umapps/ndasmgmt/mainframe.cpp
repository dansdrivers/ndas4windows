...
LONG DbgLevelMagmMain = DBG_LEVEL_MAGM_MAIN;

#define NdasUiDbgCall(l,x,...) do {						\
	if (l <= DbgLevelMagmMain) {						\
...
CMainFrame::CMainFrame() :
	...
	m_AutoPnpMutex(NULL),
	m_NdasStringIdLen(NDAS_MAX_AUTO_PNP),
...
HICON 
CMainFrame::pCreateTaskbarIcon(bool NormalOrFailure /* = true */)
{
	HICON hicon;

	hicon = m_taskbarImageList.GetIcon( m_nTaskbarNormal, NormalOrFailure ? ILD_NORMAL : INDEXTOOVERLAYMASK(1));
...
CMainFrame::OnDestroy()
{
	...
	for (UINT i=0; i<NDAS_MAX_AUTO_PNP; i++) {
		...
		if (m_NdasStringId[i]) {
			...
			::HeapFree( ::GetProcessHeap(), NULL, m_NdasStringId[i] ); 
			m_NdasStringId[i] = NULL;
		}
	}

	CloseHandle(m_AutoPnpMutex);
	m_AutoPnpMutex = NULL;
...
{
	DWORD dwWaitResult = WaitForSingleObject( m_AutoPnpMutex, 0 );
	...
	NdasUiDbgCall( 2, _T("m_AutoPnpMutex = %p, dwWaitResult = %d\n"), m_AutoPnpMutex, dwWaitResult );

	CRegWizard regwiz;
	INT_PTR	result = regwiz.DoModal(m_hWnd);

	ReleaseSemaphore( m_AutoPnpMutex, 1, NULL );

	NDAS_SERVICE_PARAM serviceParam = {0};

	BOOL bresult = ::NdasGetServiceParam( NDASSVC_PARAM_AUTO_PNP, &serviceParam );

	if (bresult && serviceParam.Value.DwordValue == NDASSVC_AUTO_PNP_ALLOW) {

		NdasUnregisterDevice(0);
	}
}
...
retry:
	...
	NDAS_LOGICALUNIT_STATUS status = pLogDevice->GetStatus();

	if (FAILED(hr) ||
		NDAS_LOGICALUNIT_STATUS_MOUNTED == status)
	{
...
VOID 
CMainFrame::OnNdasAutoPnp (TCHAR *NdasStringId)
{
	if (m_NdasStringIdLen == NDAS_MAX_AUTO_PNP) {

		ATLASSERT(FALSE);
		return;
	}

	for (UINT i=0; i<m_NdasStringIdLen; i++) {
		...
		if (!memcmp(NdasStringId, m_NdasStringId[i], sizeof(TCHAR) * (NDAS_DEVICE_STRING_ID_LEN+1))) {
	...
	NdasUiDbgCall( 2, _T("%s m_AutoPnpMutex = %p, dwWaitResult = %d\n"), NdasStringId, m_AutoPnpMutex, dwWaitResult );

	CopyMemory( m_NdasStringId[m_NdasStringIdLen], NdasStringId, sizeof(TCHAR) * (NDAS_DEVICE_STRING_ID_LEN+1) );
	m_NdasStringIdLen ++;

	CRegWizard regwiz( NULL, NdasStringId );
	...
	NdasUiDbgCall( 2, "out, bresult = %d m_NdasStringIdLen = %d\n", bresult, m_NdasStringIdLen ); 
...
{
	NdasUiDbgCall( 2, "InterAppMsg: WPARAM %d, LPARAM %d\n", wParam, lParam );

	switch (wParam) {

	case INTERAPPMSG_POPUP:

		NdasUiDbgCall( 2, "INTERAPPMSG_POPUP\n" );
		pShowInstPopup();
	...
	case INTERAPPMSG_EXIT:
		...
		NdasUiDbgCall( 1, "INTERAPPMSG_EXIT\n" );
		DestroyWindow();
...
bool
CMainFrame::pProcessRegistration(
