	...
	LPBYTE lpbKey128)
{
	...
	hr = md5.GetHashValue(&lpbHash, &cbHash);
	...
	OUT LPDWORD pcbUsed)
{
	...
	hr = md5.HashData(pSysKey, cbSysKey);
	...
	hr = md5.HashData(pDiskKey, cbDiskKey);
	...
	hr = md5.GetHashValue(&pbHash, &cbHash);
...
NdasEncImportSysKeyFromFile(LPCTSTR szFileName)
{
	...
	if (cbRead < FIELD_OFFSET(NDAS_ENC_SYSKEY_FILE_HEADER, KeyData)) 
	{
	...
	if (cbRead != 
		lpFileHeader->KeyDataLength + FIELD_OFFSET(NDAS_ENC_SYSKEY_FILE_HEADER, KeyData))
	{
		return NDASENC_ERROR_INVALID_SYSKEY_FILE;
	}
	...
	CCryptEncrypt des(CALG_DES, MS_DEF_PROV, PROV_RSA_FULL);
...
NdasEncExportKeyToFile(LPCTSTR szFileName, DWORD cbKey, LPBYTE lpKey)
{
	...
	hr = des.Encrypt(
		lpKeyData, 
		&cbKeyData, 
		sizeof(lpBuffer) - FIELD_OFFSET(NDAS_ENC_SYSKEY_FILE_HEADER, KeyData));
	...
	DWORD cbFileSize = FIELD_OFFSET(NDAS_ENC_SYSKEY_FILE_HEADER, KeyData) + cbKeyData;
