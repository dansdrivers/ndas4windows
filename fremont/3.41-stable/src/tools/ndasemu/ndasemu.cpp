...
UCHAR	HeaderEncryptAlgo = 0;
UCHAR	DataEncryptAlgo = 0;
...
//				fprintf(stderr, "IDE_COMMAND received.\n");
			...
			if (thisHWVersion == LANSCSIIDE_VERSION_1_0) {
				...
				if(pRequestHeader->W) {
					// Check access right.
#if 0		// Allow writing in read-only mode in NDAS 1.1~2.0
...
void PrintUsage(void)
{
	...
	fprintf(stderr, "v0, v1, v2: Set verbose level\n");
...
main(INT32 argc, char* argv[])
{
	...
	ataDiskInit.BlockBytes = 512;
	ataDiskInit.BytesInBlockBitShift = 9;
#else
	ataDiskInit.BlockBytes = 4096;
	ataDiskInit.BytesInBlockBitShift = 12;
#endif

	ataDiskInit.Capacity = DiskSize / ataDiskInit.BlockBytes;
