		   ...
		   )
{
	...
	// ...
	// Display the string.
	fprintf(stderr, "\n%s: %s - %d", prefix, (LPCSTR)lpMsgBuf, ErrorCode);
			...
			)
{
	...
	TRACE("[LanScsiCli]GetDiskInfo: Target ID %d, Major 0x%x, Minor 0x%x, Capa 0x%x\n", 
		TargetId, info.major_rev_num, info.minor_rev_num, info.capability);
	
	TRACE("[LanScsiCli]GetDiskInfo: DMA 0x%x, U-DMA 0x%x\n", 
		info.dma_mword, 
		info.dma_ultra);
	...
	TRACE("CAP 2 %I64d, CAP %d\n",
		info.lba_capacity_2,
		info.lba_capacity
		);

	TRACE("LBA %d, LBA48 %d, Number of Sectors: %I64d\n", 
			   ...
			   )
{
	...
	for(i = 0; i < socketAddressList->iAddressCount; i++) {
		...
		TRACE("[LanScsiCli]MakeConnection: NIC %02d: Address %02X:%02X:%02X:%02X:%02X;%02X\n",
			i,
