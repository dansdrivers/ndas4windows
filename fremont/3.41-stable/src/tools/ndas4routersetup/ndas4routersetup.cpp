...
#include <ndas/ndasop.h> /* include this header file to use NDAS Op API */
...
CpWriteBacl(int argc, _TCHAR* argv[])
{
	...
	{		
		for(pchtext = argv[2 +i]; *pchtext; pchtext++)
		{
			...
			else if(_T('E') == *pchtext)
			{
				pBACL_Element->AccessMask |= BACL_ACCESS_MASK_EMBEDDED_SYSTEM;
			}
			else if(_T('D') == *pchtext)
			{
				pBACL_Element->AccessMask |= BACL_ACCESS_MASK_PC_SYSTEM;
			}
	...
	// write BACL on disk
	if (FAILED(NdasOpWriteBACL(hNDAS, BACL_Allocated))) {
...
CpReadBACL(int argc, _TCHAR* argv[])
{
	...
	if (FAILED(NdasOpReadBACL(hNDAS, &BACL))) {
	...
	if (FAILED(NdasOpReadBACL(hNDAS, BACL_Allocated))) {
	...
	{
		_ftprintf(stdout, _T("%03d       : %s %s %s %s %I64u(%I64x) , %I64u(%I64x)\n"),
			i, 
			...
			(BACL_Allocated->Elements[i].AccessMask & BACL_ACCESS_MASK_EMBEDDED_SYSTEM) ? _T("E") : _T(" "),
			(BACL_Allocated->Elements[i].AccessMask & BACL_ACCESS_MASK_PC_SYSTEM) ? _T("D") : _T(" "),
...
CpClearBACL(int argc, _TCHAR* argv[])
{
	...
	// write BACL on disk
	if (FAILED(NdasOpWriteBACL(hNDAS, &BACL_Empty))) {
...
DEF_COMMAND_1(_c_set_bacl, "setbacl", "<NetDisk ID> <Write key> [WRDE(Pn|Sn,n)] {ex : setbacl <Netdisk ID><Write key> WDP0 RES0,63 means writable only to first partition for PC(Desktop), read only to 63 sectors from 0 for embedded system}")
