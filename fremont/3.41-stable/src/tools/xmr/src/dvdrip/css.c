...
static int GetBusKey( dvdcss_t dvdcss )
{
    ...
    for( i = 0; i_ret == -1 && i < 4 ; ++i )
    {
        ...
        dvdcss->css.i_agid = i;
        i_ret = ioctl_InvalidateAgid( dvdcss->i_fd, &dvdcss->css.i_agid );
		if(i_ret == -1) continue;
    ...
    for( i = 0 ; i < 32 ; ++i )
    {
        ...
        {
            _snprintf_s( psz_warning, sizeof(psz_warning), _TRUNCATE,
...
static void PrintKey( dvdcss_t dvdcss, char *prefix, uint8_t const *data )
{
    ...
    sprintf_s( psz_output, 80, "%s%02x:%02x:%02x:%02x:%02x", prefix,
...
int _dvdcss_title ( dvdcss_t dvdcss, int i_block )
{
    ...
    if( dvdcss->psz_cachefile[0] )
    {
        ...
        sprintf_s( dvdcss->psz_block, 50, "%.10x", i_block );
		err = _sopen_s(&i_fd, dvdcss->psz_cachefile, O_RDONLY, _SH_DENYNO, (_S_IREAD | _S_IWRITE) );
        b_cache = 1;
    ...
    if( b_cache )
    {
		err = _sopen_s(&i_fd, dvdcss->psz_cachefile, O_RDWR|O_CREAT|O_EXCL, _SH_DENYNO,  (_S_IREAD | _S_IWRITE));
        if(err == 0)
        {
            _write( i_fd, p_title_key, 5 );
            _close( i_fd );
        }
    }
    ...
    p_newtitle = (struct dvd_title_s *)malloc( sizeof( dvd_title_t ) );
                          ...
                          dvd_key_t p_titlekey )
{
    ...
    { /* Print some statistics. */
        char psz_info[128];
        _snprintf_s( psz_info, sizeof(psz_info), _TRUNCATE,
