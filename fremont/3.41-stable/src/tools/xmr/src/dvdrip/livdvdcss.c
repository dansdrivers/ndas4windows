...
extern dvdcss_t dvdcss_open (const  char *psz_target )
{
    ...
    char *psz_verbose = NULL;
    char *psz_cache = NULL;
	size_t method_size = 0;
	size_t verbose_size = 0;
	size_t cache_size = 0;
	dvdcss_t dvdcss;


	_dupenv_s(&psz_method, &method_size, "DVDCSS_METHOD");
	_dupenv_s(&psz_verbose, &verbose_size, "DVDCSS_METHOD");
	_dupenv_s(&psz_cache, &cache_size, "DVDCSS_METHOD");


    /*
     *  Allocate the library structure
     */
    dvdcss = (struct dvdcss_s *)malloc( sizeof( struct dvdcss_s ) );
    if( dvdcss == NULL )
    {
        return NULL;
    }
    ...
    dvdcss->psz_device = (char *)_strdup( psz_target );
    ...
    if( psz_method != NULL )
    {
        ...
        else
        {
			...
			if(psz_method) free(psz_method);
			if(psz_verbose) free(psz_verbose);
			if(psz_cache) free(psz_cache);
...
#define SHGFP_TYPE_CURRENT 0
        ...
        *psz_home = '\0';
        ...
        if( p_dll )
        {
            p_getpath = (SHGETFOLDERPATH) GetProcAddress( p_dll, "SHGetFolderPathA" );
            if( p_getpath )
            {
        ...
        if( *psz_home )
        {
            _snprintf_s( psz_buffer, PATH_MAX,_TRUNCATE, "%s/dvdcss", psz_home );
    ...
    if( psz_cache )
    {
       ...
       /* We have a disc name or ID, we can create the cache dir */
        i = sprintf_s( dvdcss->psz_cachefile, PATH_MAX, "%s", psz_cache );
		i_ret = _mkdir( dvdcss->psz_cachefile );
        ...
        i += sprintf_s( dvdcss->psz_cachefile + i,(PATH_MAX -i), "/%s#%s", psz_title,
                      psz_serial );
        ...
        i += sprintf_s( dvdcss->psz_cachefile + i, (PATH_MAX -i), "/");

        /* Pointer to the filename we will use. */
        dvdcss->psz_block = dvdcss->psz_cachefile + i;

        sprintf_s( (char *)psz_debug, (PATH_MAX+30), "using CSS key cache dir: %s",
                                          ...
                                          int i_flags )
{
    ...
    else
    {
        ...
        for( i_index = i_ret; i_index; i_index-- )
        {
            _dvdcss_unscramble( dvdcss->css.p_title_key, (unsigned char *)p_buffer );
                                           ...
                                           int i_flags )
{
    ...
    for( i_index = i_ret; i_index; i_index-- )
    {
        ...
        _dvdcss_unscramble( dvdcss->css.p_title_key, (unsigned char *)iov_base );
...
{
    return Ddvdcss_title( dvdcss, i_block );
}
