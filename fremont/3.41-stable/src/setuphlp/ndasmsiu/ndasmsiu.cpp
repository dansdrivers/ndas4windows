...
static
HRESULT 
NdasMsipEnsureNoActiveNdasScsiLogicalUnitInstances(
...
UINT NdasMsiDetectNdasLogicalUnitInstances(MSIHANDLE hInstall)
{
	...
	while (TRUE)
	{
		...
		HRESULT hrScsi = NdasMsipEnsureNoActiveNdasScsiLogicalUnitInstances(
			hInstall,
			NULL,
		...
		if (FAILED(hrScsi))
		{
			xMsiLogPrintf(
				hInstall, _T("CA_NDAS_DETECT:"), 
				_T("NdasMsipEnsureNoActiveNdasScsiLogicalUnitInstances failed, hr=0x%X"),
				hrScsi);
		...
		else if (IDCANCEL == ret)
		{
			xMsiLogPrintf(
				hInstall, _T("CA_NDAS_DETECT:"), 
				_T("MsiProcessMessage returned %d (IDCANCEL)"), ret);
