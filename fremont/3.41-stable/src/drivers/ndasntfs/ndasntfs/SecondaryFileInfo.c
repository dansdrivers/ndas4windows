...

{
    ...
    try {
		...
		if (NT_SUCCESS(Status) && setFile.FileObject) {
			...
			DebugTrace( 0, Dbg2, ("NdasNtfsSetRenameInfo: fullpathName = %wZ\n", &fullPathName) );
...

{
    ...
    try {
		...
		if (NT_SUCCESS(Status) && NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) != Scb->Header.AllocationSize.QuadPart) {
				...
				if (vcn < LlClustersFromBytes(&volDo->Secondary->VolDo->Vcb, Scb->Header.AllocationSize.QuadPart)) {
					...
					if (vcn < LlClustersFromBytes(&volDo->Secondary->VolDo->Vcb, NTOHLL(ndfsWinxpReplytHeader->AllocationSize8))) {
						...
						ASSERT( clusterCount <= mcbEntry->ClusterCount || 
								Scb->Header.AllocationSize.QuadPart > (INT64)(NTOHLL(ndfsWinxpReplytHeader->AllocationSize8)) && (index+1) == NTOHL(ndfsWinxpReplytHeader->NumberOfMcbEntry4) );
			...
			NDAS_ASSERT( LlBytesFromClusters(&volDo->Secondary->VolDo->Vcb, vcn) == NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) );
			...
			if (Scb->Header.AllocationSize.QuadPart > (INT64)NTOHLL(ndfsWinxpReplytHeader->AllocationSize8)) {
				...
				ASSERT( LlBytesFromClusters(&volDo->Secondary->VolDo->Vcb, testVcn) == NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) );
