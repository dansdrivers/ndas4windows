...

{
    ...
    if (FileObject == Vcb->LogFileObject) {
		...
		ExAcquireFastMutex( &VolumeDeviceObject->FastMutex );

		if (FlagOn(VolumeDeviceObject->NdasNtfsFlags, NDAS_NTFS_DEVICE_FLAG_WAIT_CLOSE_LOGFILE)) {

			Vcb->LogFileObject = NULL;
			KeSetEvent( &VolumeDeviceObject->LogFileCloseEvent, 0, FALSE );
		}

		ExReleaseFastMutex( &VolumeDeviceObject->FastMutex );
                ...
                if (Ccb != NULL) {
					...
					if (FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY)) {
						...
						if(FlagOn(Ccb->NdasNtfsFlags, ND_NTFS_CCB_FLAG_UNOPENED)) {
							...
							if( FlagOn(Ccb->NdasNtfsFlags, ND_NTFS_CCB_FLAG_CORRUPTED) )
    ...
    if (Status == STATUS_PENDING) {
		...
		if (Ccb) {
			...
			SetFlag( Ccb->NdasNtfsFlags, ND_NTFS_CCB_FLAG_DELAY_CLOSE );
			Ccb->FileObject = NULL;
    ...
    while (IrpContext = NtfsRemoveClose( ThisVcb, ThrottleCreate )) {
        ...
        do {
            ...
            if (((Status == STATUS_LOG_FILE_FULL) ||
                ...
                FsRtlExitFileSystem();
//#if __NDAS_NTFS_SECONDARY__
//				if (!FlagOn(IrpContext->Vcb->NdasNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES))
//#endif
...

{
	...
	if (Fcb && FlagOn(Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY))
		SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT );
	...
	if (!FlagOn(volDo->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) &&
		...
		if (!FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)) {
			...
			NtfsSetSegmentNumber( &fileReference, 0, FIRST_USER_FILE_NUMBER );
			NDAS_ASSERT( NtfsLtrMftRef( &Fcb->FileReference, &fileReference) );
    ...
    )
{
	...
	if (FlagOn(((PSCB)IrpContext->OriginatingIrp)->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY)) {
    ...
    )
{
    ...
    if (!IsListEmpty( &NtfsData.AsyncCloseList )) {
        ...
        while (Entry != &NtfsData.AsyncCloseList) {
            ...
            if (!ARGUMENT_PRESENT( Vcb ) ||
				...
				if (ARGUMENT_PRESENT(Vcb) && FlagOn(Vcb->NdasNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES)) {
					...
					//ASSERT( FlagOn(((PSCB)IrpContext->OriginatingIrp)->Fcb->NdasNtfsFlags, NDAS_NTFS_FCB_FLAG_SECONDARY) );
				}
#endif
        ...
        if (ARGUMENT_PRESENT( Vcb )) {
            ...
            while (Entry != &NtfsData.DelayedCloseList) {
                ...
                if (IrpContext->Vcb == Vcb) {
					...
					if (FlagOn(Vcb->NdasNtfsFlags, ND_NTFS_VCB_FLAG_TRY_CLOSE_FILES)) {
	...
	ASSERT( Vcb == NULL || 
			IrpContext == NULL || 
			NtfsIsExclusiveVcb(IrpContext, Vcb) ||
