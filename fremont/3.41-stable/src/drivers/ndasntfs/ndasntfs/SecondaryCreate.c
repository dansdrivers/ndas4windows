	...
	try {
		...
		for (index=0; index<ntfsSystemFilesSize; index++) {
			...
			if (NtfsSystemFiles[index].Length != 2 && RtlEqualUnicodeString(&fileObject->FileName, &NtfsSystemFiles[index], TRUE)) {
				...
				DebugTrace(0, Dbg2, ("NdasNtfsSecondaryCommonCreate: FileName = %wZ\n", &fileObject->FileName) );
		...
		*fullFileName  = *originalFileName = OplockCleanup->FileObject->FileName;
		...
		if (fileObjectName->Length == 0) {
			...
			if (attrName.Length != 0 || attrCodeName.Length != 0) {
				...
				DebugTrace( 0, Dbg, ("FileObjectName = %wZ\n", fileObjectName) );
				DebugTrace( 0, Dbg, ("OriginalFileName = %wZ\n", originalFileName) );
				DebugTrace( 0, Dbg, ("AttrName = %wZ\n", &attrName) );
				DebugTrace( 0, Dbg, ("AttrCodeName = %wZ\n", &attrCodeName) );
			}
		}

		fullPathNameBuffer = NtfsAllocatePool( NonPagedPoolCacheAligned, NDFS_MAX_PATH*sizeof(WCHAR) );
		...
		if (attrName.Length != 0 || attrCodeName.Length != 0) {
			...
			DebugTrace( 0, Dbg, ("AttrName = %wZ\n", &attrName) );
			DebugTrace( 0, Dbg, ("AttrCodeName = %wZ\n", &attrCodeName) );
			DebugTrace( 0, Dbg, ("fullPathName = %wZ\n", &fullPathName) );
		}
		...
		if (fullPathName.Length != 0 && (irpSp->Parameters.Create.Options & 0xFF000000) != (FILE_CREATE << 24)) {
			...
			DebugTrace( 0, Dbg, ("ndfsWinxpRequestHeader->Create.AllocationSize = %I64d\n", 
								  ndfsWinxpRequestHeader->Create.AllocationSize) );
			...
			if (NTOHL(ndfsWinxpReplytHeader->Status4) == STATUS_OBJECT_NAME_INVALID) {
				...
				DebugTrace( 0, Dbg, ("orginalFileName = %wZ\n", originalFileName) );

				if (fileObject->RelatedFileObject) {

					DebugTrace( 0, Dbg, ("RelatedFileObjectName = %wZ\n", &fileObject->RelatedFileObject->FileName) );
				}
			}
...
#endif
					NDAS_ASSERT( scb->Handle == ndfsWinxpReplytHeader->Open.ScbHandle || scb->CleanupCount == 0 );
		...
		if (irpSp->Parameters.Create.EaLength == 0) {
			...
			while (fileFullEa->NextEntryOffset) {
				...
				DebugTrace( 0, Dbg, ("create ccb->Lcb->ExactCaseLink.LinkName = %wZ, fileFullea->EaName = %ws\n", 
		...
		if (ndfsWinxpReplytHeader->Open.TypeOfOpen == UserFileOpen) {
			...
			if (Irp->Overlay.AllocationSize.QuadPart) {
				...
				if (secondaryRequestInfo->ExecuteStatus != STATUS_SUCCESS) {
					...
					DebugTrace( 0, Dbg2, ("secondaryRequestInfo->ExecuteStatus != STATUS_SUCCESS file = %s, line = %d\n", __FILE__, __LINE__) );

					NtfsRaiseStatus( IrpContext, secondaryRequestInfo->ExecuteStatus, NULL, NULL );
				}
		...
		NDAS_ASSERT( ndfsWinxpReplytHeader->Open.AttributeTypeCode == $DATA ||
					 ndfsWinxpReplytHeader->Open.AttributeTypeCode == $INDEX_ALLOCATION );
		
		if (attrTypeCode != $END) {

			NDAS_ASSERT( attrTypeCode == ndfsWinxpReplytHeader->Open.AttributeTypeCode );
		}

		if (fcb) {
			
			NDAS_ASSERT( fcb->FileReference.SegmentNumberHighPart == ndfsWinxpReplytHeader->Open.FileReference.SegmentNumberHighPart );
			NDAS_ASSERT( fcb->FileReference.SegmentNumberLowPart	 == ndfsWinxpReplytHeader->Open.FileReference.SegmentNumberLowPart );
			NDAS_ASSERT( fcb->FileReference.SequenceNumber		 == ndfsWinxpReplytHeader->Open.FileReference.SequenceNumber );  

			NDAS_ASSERT( !FlagOn( fcb->FcbState, FCB_STATE_FILE_DELETED) );
			NDAS_ASSERT( FlagOn( fcb->FcbState, FCB_STATE_IN_FCB_TABLE) );
			NDAS_ASSERT( !FlagOn( scb->ScbState, SCB_STATE_ATTRIBUTE_DELETED) );
		...
		if (!IsDirectory(&fcb->Info)) {
			...
			NDAS_ASSERT( ndfsWinxpReplytHeader->Open.TypeOfOpen == UserFileOpen		|| 
		...
		NtfsUpcaseName( IrpContext->Vcb->UpcaseTable, IrpContext->Vcb->UpcaseTableSize, &fullPathName );
		...
		NDAS_ASSERT( !FlagOn(scb->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT) );
        ...
        NtfsIncrementCleanupCounts( scb, lcb, BooleanFlagOn(irpSp->FileObject->Flags, FO_NO_INTERMEDIATE_BUFFERING) );
		...
		if (ndfsWinxpReplytHeader->Open.TypeOfOpen == UserFileOpen) {
			...
			if (Irp->Overlay.AllocationSize.QuadPart) {
				...
				NDAS_ASSERT( Irp->Overlay.AllocationSize.QuadPart == NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) );
			}
			...
			if (scb->CloseCount != 1) {
				...
				if (NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) != scb->Header.AllocationSize.QuadPart) {
					...
					NDAS_ASSERT( createDisposition == FILE_SUPERSEDE || createDisposition == FILE_OVERWRITE || createDisposition == FILE_OVERWRITE_IF );
			...
			if (createDisposition == FILE_SUPERSEDE || createDisposition == FILE_OVERWRITE || createDisposition == FILE_OVERWRITE_IF) {
				...
				if (scb->Header.AllocationSize.QuadPart < (INT64)NTOHLL(ndfsWinxpReplytHeader->AllocationSize8)) {
					...
					NtfsSetBothCacheSizes( fileObject, (PCC_FILE_SIZES)&scb->Header.AllocationSize, scb );
				...
				if (scb->Header.AllocationSize.QuadPart != 0) {
					...
					if (FlagOn(scb->ScbState, SCB_STATE_TRUNCATE_ON_CLOSE)) {
						...
						NDAS_ASSERT( FlagOn(scb->ScbState, SCB_STATE_TRUNCATE_ON_CLOSE) == ndfsWinxpReplytHeader->Open.TruncateOnClose );
					}

					NDAS_ASSERT( scb->Header.FileSize.QuadPart == NTOHLL(ndfsWinxpReplytHeader->FileSize8) );
					NDAS_ASSERT( scb->Header.AllocationSize.QuadPart == NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) );
			
				} else {

					if (NTOHLL(ndfsWinxpReplytHeader->AllocationSize8)) {

						NDAS_ASSERT( scb->CloseCount == 1 );
						...
						NDAS_ASSERT( LlBytesFromClusters(&volDo->Secondary->VolDo->Vcb, vcn) == 
									 NTOHLL(ndfsWinxpReplytHeader->AllocationSize8) );
...
try_exit:  NOTHING;
	...
	if (irpSp->Parameters.Create.EaLength) {
		...
		DebugTrace( 0, Dbg2, ("create originalFileName = %wZ status = %x\n", originalFileName, status) );
	}	
	...
	)
{
	...
	if (FileObject->RelatedFileObject) {
		...
		if (IS_SECONDARY_FILEOBJECT(FileObject->RelatedFileObject)) {
			...
			if ((relatedCcb->Lcb->ExactCaseLink.LinkName.Length + sizeof(WCHAR) + FileObjectName->Length) > FullPathName->MaximumLength) {
