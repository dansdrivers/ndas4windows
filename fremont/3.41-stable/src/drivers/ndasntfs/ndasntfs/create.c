...

{
	...
	if ((PVOID)NdasNtfsFileSystemDeviceObject == VolumeDeviceObject) {
        ...
        try {
            ...
            if (IrpContext == NULL) {
...
#if __NDAS_NTFS_PRIMARY__
				{
					...
					ASSERT(!BooleanFlagOn( irpSp->Flags, SL_OPEN_PAGING_FILE ));

					IrpContext->MaxOpenTry = 1;
					...
					if (ValidPrimaryRequest) {
						...
						DebugTrace( 0, Dbg, ("primaryRequestInfo = %p\n", primaryRequestInfo) );
            ...
            if (FlagOn( IrpContext->State, IRP_CONTEXT_STATE_DASD_OPEN )) {
                ...
                if (IoGetRemainingStackSize( ) >= OVERFLOW_CREATE_THRESHHOLD) {
					...
					if (VolumeDeviceObject->Secondary == NULL) {
						
						Status = NtfsCommonCreate( IrpContext, Irp, &OplockCleanup, NULL );
				
					} else if (IrpContext->PrimaryRequestInfo.PrimarySessionId != NDASNTFS_LOCAL_PRMARY_SESSION_ID	&&
							   IrpContext->PrimaryRequestInfo.PrimarySession->SessionContext.Uid == VolumeDeviceObject->Secondary->Thread.SessionContext.Uid	&&
							   IrpContext->PrimaryRequestInfo.PrimarySession->SessionContext.Tid == VolumeDeviceObject->Secondary->Thread.SessionContext.Tid) {
						...
						NDAS_ASSERT( NtfsIsTopLevelRequest(IrpContext) );
						...
						for (;;) {
							...
							if (!FlagOn(VolumeDeviceObject->Vcb.VcbState, VCB_STATE_TARGET_DEVICE_STOPPED)) {
							...
							if (irpSp->FileObject->FileName.Length == (sizeof(WAKEUP_VOLUME_FILE_NAME)-sizeof(WCHAR)) && 
								...
								if (secondaryResourceAcquired == FALSE) {
									...
									NtfsRaiseStatus( IrpContext, STATUS_OBJECT_NAME_NOT_FOUND, NULL, NULL );
								}
						...
						if (Status == STATUS_SUCCESS) {
							...
							try {
								...
								if (FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION)) {

									PLIST_ENTRY			primarySessionListEntry;
									PPRIMARY_SESSION	primarySession;

									for (primarySession = NULL, primarySessionListEntry = VolumeDeviceObject->PrimarySessionQueue.Flink; 
										 primarySessionListEntry != &VolumeDeviceObject->PrimarySessionQueue; 
										...
										if (IrpContext->FileOpenPrimarySessionId == primarySession->PrimarySessionId) {
											...
											if (primarySession->SessionContext.Uid != VolumeDeviceObject->Secondary->Thread.SessionContext.Uid ||
												primarySession->SessionContext.Tid != VolumeDeviceObject->Secondary->Thread.SessionContext.Tid) {
									...
									ClearFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION );
									SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_OPEN_TRY_ON_SECONDARY );
								}
								...
								NDAS_ASSERT( ExIsResourceAcquiredSharedLite(&VolumeDeviceObject->Resource) );
...

{
	...
	INDEX_CONTEXT	tempIndexContext;
#endif
#endif
	...
	if (((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY || 
		((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY2PRIMARY) {

		BOOLEAN doMore = FALSE;

		Status = NdasNtfsSecondaryCommonCreate( IrpContext, Irp, OplockCleanup, NULL, &doMore );	
	...
	NDAS_ASSERT( !FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_SECONDARY_CONTEXT)	||
				 FlagOn(IrpContext->Vcb->VcbState, VCB_STATE_TARGET_DEVICE_STOPPED)					|| 
				 FlagOn(CONTAINING_RECORD(IrpContext->Vcb, VOLUME_DEVICE_OBJECT, Vcb)->Secondary->Flags, SECONDARY_FLAG_DISMOUNTING) );
#endif
    ...
    try {
        ...
        *FullFileName = *OriginalFileName = OplockCleanup->FileObject->FileName;
        ...
        while (TRUE) {
            ...
            DebugTrace( 0, Dbg, ("Final name     -> %wZ\n", &FinalName) );
            DebugTrace( 0, Dbg, ("Remaining Name -> %wZ\n", &RemainingName) );
...

{
    ...
    try {
		...
		if (AllocationSize) {
			...
			NDAS_ASSERT( FlagOn(ThisScb->ScbState, SCB_STATE_HEADER_INITIALIZED) );
			NDAS_ASSERT( !FlagOn(ThisScb->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT) );
			NDAS_ASSERT( ThisScb->Header.AllocationSize.QuadPart == AllocationSize );
		}
...
#define FlushScb(IRPC,SCB,IOS) {                                                \
    ...
    if (FlagOn((SCB)->ScbState, SCB_STATE_FILE_SIZE_LOADED)) {                  \
		...
		if (NT_SUCCESS(Status) && (*ThisCcb) && (*ThisCcb)->TypeOfOpen == UserFileOpen) {
	
			ULONG		writeAceessCloseCount   = 0;
			ULONG		writeAceessUncloseCount = 0;
			ULONG		writeAceessCleanupCount = 0;

			PLIST_ENTRY	ccbListEntry;

			NDAS_ASSERT( (*ThisCcb)->FileObject == NULL );
			NDAS_ASSERT( IrpSp->FileObject );

			(*ThisCcb)->FileObject = IrpSp->FileObject;

			if (IrpContext->PrimaryRequestInfo.PrimarySessionId != NDASNTFS_LOCAL_PRMARY_SESSION_ID) {

				NDAS_ASSERT( NtfsSegmentNumber( &ThisFcb->FileReference ) >= FIRST_USER_FILE_NUMBER );
			}

			RemoveShareAccess = TRUE;

			NDAS_ASSERT( ExIsResourceAcquiredSharedLite((*ThisScb)->Fcb->Resource) );	

			NtfsLockFcb( NULL, (*ThisScb)->Fcb );

			for (ccbListEntry = (*ThisScb)->CcbQueue.Flink; 
				 ccbListEntry != &(*ThisScb)->CcbQueue;
				 ccbListEntry = ccbListEntry->Flink) {

				 PCCB ccb = CONTAINING_RECORD( ccbListEntry, CCB, CcbLinks );

				if (FlagOn(ccb->NdasNtfsFlags, ND_NTFS_CCB_FLAG_READONLY)) {

					continue;
				}

				writeAceessCloseCount ++;

				if (FlagOn(ccb->NdasNtfsFlags, ND_NTFS_CCB_FLAG_DELAY_CLOSE)) {

					continue;
				}

				writeAceessUncloseCount ++;

				NDAS_ASSERT( ccb == (*ThisCcb) || ccb->FileObject );

				if (ccb != (*ThisCcb) && FlagOn(ccb->FileObject->Flags, FO_CLEANUP_COMPLETE)) {
					 
					continue;
				}

				writeAceessCleanupCount ++;
			}
				 
			NtfsUnlockFcb( NULL, (*ThisScb)->Fcb );

			DebugTrace( 0, Dbg, ("(*ThisScb)->PrimarySessionId = %I64x, IrpContext->PrimaryRequestInfo.PrimarySession = %I64x\n",
								  (*ThisScb)->PrimarySessionId, IrpContext->PrimaryRequestInfo.PrimarySessionId) );

			DebugTrace( 0, Dbg, ("openTryCount = %d, Scb->CleanupCount = %d,"
								  "Scb->Close = %d, FlagOn(Scb->ScbState, SCB_STATE_DELAY_CLOSE) = %d\n", 
								   IrpContext->OpenTryCount, (*ThisScb)->CleanupCount, (*ThisScb)->CloseCount, 
								   FlagOn((*ThisScb)->ScbState, SCB_STATE_DELAY_CLOSE)) );

			if (writeAceessCloseCount != 1						&&
				!IsFileObjectReadOnly((*ThisCcb)->FileObject)	&&
				(*ThisScb)->PrimarySessionId != IrpContext->PrimaryRequestInfo.PrimarySessionId) {

				NDAS_ASSERT( writeAceessCloseCount > 1 && writeAceessCleanupCount >= 1 );

				if (writeAceessCleanupCount != 1) {
					
					SetFlag( IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_ALREADY_OPENED_BY_PRIMARY_SESSION );
					IrpContext->FileOpenPrimarySessionId = (*ThisScb)->PrimarySessionId;
					
					NtfsRaiseStatus( IrpContext, STATUS_CANT_WAIT, NULL, NULL );
				}
					
				if (writeAceessUncloseCount != 1) {
					
					if ((*ThisScb)->PrimarySessionId != NDASNTFS_LOCAL_PRMARY_SESSION_ID) {
					...
					NDAS_ASSERT( writeAceessUncloseCount > 1 );
										
					if (IrpContext->OpenTryCount++ < IrpContext->MaxOpenTry) {
						...
						FlushScb( IrpContext, (*ThisScb), &ioStatus );
						ASSERT( ioStatus.Status == STATUS_SUCCESS );

						dataSectionExists = (BOOLEAN)((*ThisScb)->NonpagedScb->SegmentObject.DataSectionObject != NULL);
						imageSectionExists = (BOOLEAN)((*ThisScb)->NonpagedScb->SegmentObject.ImageSectionObject != NULL);

						if (imageSectionExists) {

							(VOID)MmFlushImageSection( &(*ThisScb)->NonpagedScb->SegmentObject, MmFlushForWrite );
						}

						if (dataSectionExists) {
								
							CcPurgeCacheSection( &(*ThisScb)->NonpagedScb->SegmentObject, NULL, 0, FALSE );
						}
			...
			NDAS_ASSERT( IsFileObjectReadOnly((*ThisCcb)->FileObject)	||
						 writeAceessCloseCount == 1						||
						 writeAceessUncloseCount == 1					||
						 (*ThisScb)->PrimarySessionId == IrpContext->PrimaryRequestInfo.PrimarySessionId );

			if (!IsFileObjectReadOnly((*ThisCcb)->FileObject)) {
			...
			if (IsFileObjectReadOnly((*ThisCcb)->FileObject)) {
				...
				SetFlag( (*ThisCcb)->NdasNtfsFlags, ND_NTFS_CCB_FLAG_READONLY );
			}

			if ((*ThisCcb) && (*ThisCcb)->TypeOfOpen == UserFileOpen) {
		
				ASSERT( (*ThisScb)->AttributeTypeCode == $DATA );

				if (IrpContext->OriginatingIrp->Overlay.AllocationSize.QuadPart) {
				
					NDAS_ASSERT( (*ThisScb)->Header.AllocationSize.QuadPart == IrpContext->OriginatingIrp->Overlay.AllocationSize.QuadPart );
					NDAS_ASSERT( FlagOn((*ThisScb)->ScbState, SCB_STATE_HEADER_INITIALIZED) );
					NDAS_ASSERT( !FlagOn((*ThisScb)->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT) );
				}						
				
				DebugTrace( 0, Dbg, ("(*ThisScb)->Header.AllocationSize.QuadPart = %I64d\n", (*ThisScb)->Header.AllocationSize.QuadPart) );

				if (!FlagOn((*ThisScb)->ScbState, SCB_STATE_HEADER_INITIALIZED) && (*ThisScb)->Header.AllocationSize.QuadPart == 0) {
				...
				if (FlagOn((*ThisScb)->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT)) {
					...
					if ((*ThisScb)->AttributeName.Length != 0) {

						DebugTrace( 0, Dbg, ("FileName = %wZ, ThisScb->AttributeName = %wZ, ThisScb->AttFlagOn(ThisScb->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT) = %d\n", 
											 &(*ThisCcb)->FullFileName, &(*ThisScb)->AttributeName, BooleanFlagOn((*ThisScb)->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT)) );
					
					} else {
                    
						DebugTrace( 0, Dbg, ("FileName = %wZ, %wZ, ThisCcb->TypeOfOpen = %d, FlagOn( CreateFlags, CREATE_FLAG_OPEN_BY_ID ) = \n", 
							  				 &IrpSp->FileObject->FileName, &(*ThisCcb)->FullFileName, (*ThisCcb)->TypeOfOpen) );
					}
					...
					try {
						...
						NtfsLookupAttributeForScb( IrpContext, (*ThisScb), NULL, &attrContext );

						attribute = NtfsFoundAttribute( &attrContext );
						ASSERT( attribute->TypeCode == $DATA );

						NtfsConvertToNonresident( IrpContext, ThisFcb, attribute, FALSE, &attrContext );
...
#endif
						DebugTrace( 0, Dbg, ("ThisScb->AttributeName = %wZ, ThisScb->AttFlagOn(ThisScb->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT) = %d\n", 
										      &(*ThisScb)->AttributeName, BooleanFlagOn((*ThisScb)->ScbState, SCB_STATE_ATTRIBUTE_RESIDENT)) );
				...
				NtfsPreloadAllocation( IrpContext, (*ThisScb), 0, MAXLONGLONG );
			}
		...
		if (*ThisCcb) {
			...
			(*ThisCcb)->FileOpenPrimarySessionId = IrpContext->PrimaryRequestInfo.PrimarySessionId;
		}
#endif
