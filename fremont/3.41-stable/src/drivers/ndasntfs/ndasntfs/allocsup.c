...
#define Dbg2                             (DEBUG_INFO_ALLOCSUP)
#endif
...

{
    ...
    if ((Found = NtfsLookupNtfsMcbEntry( &Scb->Mcb, Vcn, Lcn, ClusterCount, NULL, NULL, RangePtr, RunIndex ))
        ...
        (Scb == Vcb->MftScb
		 ...
		 !FlagOn(IrpContext->NdasNtfsFlags, NDAS_NTFS_IRP_CONTEXT_FLAG_PURGE) 
...

{
    ...
    DebugTrace( +1, Dbg2, ("NtfsAddAllocation\n") );
    ...
    if (FlagOn(Scb->Vcb->VcbState, VCB_STATE_RESTART_IN_PROGRESS)) {
        ...
        DebugTrace( -1, Dbg2, ("NtfsAddAllocation (Nooped for Restart) -> VOID\n") );
	...
	if (IrpContext->PrimaryRequestInfo.PrimarySessionId != NDASNTFS_LOCAL_PRMARY_SESSION_ID									&&
		...
		//ASSERT( LlBytesFromClusters(IrpContext->Vcb, ClusterCount) == 
		//		IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->SetFile.AllocationInformation.AllocationSize );
		
		ASSERT( ClusterCount == DesiredClusterCount );
		ASSERT( Extending );
		...
		try {
            ...
            if (NtfsAllocateClusters( IrpContext,
                                      ...
                                      TRUE,
                                      ClusterCount,
                                      (UNALIGNED PLCN)(&IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->SetFile.AllocationInformation.Lcn),
                                      &DesiredClusterCount )) {
	    ...
	    DebugTrace( -1, Dbg2, ("NtfsAddAllocation -> VOID\n") );
        ...
        do {
            ...
            if (!RtlCheckBit( &CurrentBitmap->Bitmap, FirstBit )) {
                ...
                if (((Vcb->TotalReserved + (Int64ShraMod32( Vcb->TotalReserved, 8 )) +
                     ((LONGLONG)1 << Scb->CompressionUnitShift)) >= Vcb->FreeClusters) &&
            ...
            if (FlippedBit || (MappedFile && (Scb->ScbType.Data.TotalReserved <= TempL))) {
                ...
                if (FlagOn(Scb->ScbState, SCB_STATE_WRITE_ACCESS_SEEN)) {
                    Vcb->TotalReserved += (LONGLONG)1 << Scb->CompressionUnitShift;
                }
        ...
        if (MappedFile || (*DirtyBits != 0)) {
            ...
            while (FirstBit <= CurrentLastBit) {
                ...
                if (MappedFile || RtlCheckBit( &CurrentBitmap->Bitmap, FirstBit )) {
                    ...
                    if (FlagOn(Scb->ScbState, SCB_STATE_WRITE_ACCESS_SEEN)) {
                        ASSERT(Vcb->TotalReserved >= ((LONGLONG)1  << Scb->CompressionUnitShift));
                        Vcb->TotalReserved -= (LONGLONG)1 << Scb->CompressionUnitShift;
                    }
