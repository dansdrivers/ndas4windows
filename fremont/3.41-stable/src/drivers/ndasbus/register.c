...
LONG DbgLevelBusReg = DBG_LEVEL_BUS_REG;
...
#define NDBUS_REG_START_OFFSET		L"StartOffset"			// 4 bytes
#define NDBUS_REG_LOCK_IMPOSSIBLE	L"LockImpossible"		// 2 bytes
...
#define NDBUS_REG_REVISION		L"HWRevision"				// 4 bytes
...
ReadUnitFromRegistry(
	...
	Unit->ucHWRevision = (UINT16)tempUlong;
...
ReadTargetAndUnitFromRegistry(
	...
	outLength = sizeof(NDASBUS_ADD_TARGET_DATA);
	...
	status = DrReadKeyValue(
						TargetKey,
						NDBUS_REG_START_OFFSET,
						REG_BINARY,
						&AddTargetData->StartOffset,
						sizeof(AddTargetData->StartOffset),
						NULL
					);
	if(!NT_SUCCESS(status)) {
		NDAS_ASSERT( FALSE );
	...
	status = DrReadKeyValue(
						TargetKey,
						NDBUS_REG_LOCK_IMPOSSIBLE,
						REG_BINARY,
						&AddTargetData->LockImpossible,
						sizeof(AddTargetData->LockImpossible),
						NULL
					);
	if(!NT_SUCCESS(status)) {
		NDAS_ASSERT( FALSE );
	...
	if(baclLen) {
		...
		ndasBACLOffset = FIELD_OFFSET(NDASBUS_ADD_TARGET_DATA, BaclBuffer);
#endif
	...
	status = DrReadKeyValue(
		...
		NDBUS_REG_RAID_SPB,
		REG_DWORD,
		&AddTargetData->RAID_Info.BlocksPerBit,
		sizeof(AddTargetData->RAID_Info.BlocksPerBit),
		NULL
		);
	if(!NT_SUCCESS(status)) {
		NDAS_ASSERT( FALSE );
	...
	status = DrReadKeyValue(
		...
		REG_DWORD,
		&AddTargetData->RAID_Info.SpareDiskCount,
		sizeof(AddTargetData->RAID_Info.SpareDiskCount),
		NULL
		);
	if(!NT_SUCCESS(status)) {
		NDAS_ASSERT( FALSE );
	...
	status = DrReadKeyValue(
		...
		REG_BINARY,
		&AddTargetData->RAID_Info.NdasRaidId,
		sizeof(AddTargetData->RAID_Info.NdasRaidId),
		NULL
		);
	if(!NT_SUCCESS(status)) {
		NDAS_ASSERT( FALSE );
	...
	) 
{
	...
	for (idxKey = 0 ; idxKey < MAX_DEVICES_IN_NDAS_REGISTRY; idxKey ++) {
		...
		status = ZwEnumerateKey( NdasDeviceReg, idxKey, KeyBasicInformation, keyInfo, 512, &outLength );
		...
		InitializeObjectAttributes( &objectAttributes, &objectName, OBJ_KERNEL_HANDLE, NdasDeviceReg, NULL );
		...
		if (!NT_SUCCESS(status)) {
			...
			NdasDbgCall( 1, "ReadNDASDevInstFromRegistry() failed. NTSTATUS:%08lx\n", status );
		...
		if (!NT_SUCCESS(status)) {
			...
			NdasDbgCall( 1, "Reg_OpenTarget() failed. NTSTATUS:%08lx\n", status );

			if (status == STATUS_OBJECT_NAME_NOT_FOUND) {

				status = DrDeleteAllSubKeys(ndasDevInst);

				NDAS_ASSERT( status == STATUS_SUCCESS );

				if (NT_SUCCESS(status)) {

					status = ZwDeleteKey(ndasDevInst);
				}
			}
		...
		addTargetData->ulSlotNo = plugIn->SerialNo;

		NDAS_ASSERT( plugIn->SerialNo );

		//	Check to see if the NDAS device already plug-ined

		pdoData = LookupPdoData( FdoData, plugIn->SerialNo );

		if (pdoData) {

			NdasDbgCall( 1, "PDO already exists plugIn->SerialNo %d\n", plugIn->SerialNo );
		...
		NdasDbgCall( 2, "Cleanup Success %d\n", plugIn->SerialNo );
	...
	) 
{
	...
	for (idxKey = 0 ; idxKey < MAX_DEVICES_IN_NDAS_REGISTRY; idxKey ++) {
		...
		NdasDbgCall( 2, "start to plugin SerialNo %d\n", plugIn->SerialNo );
		...
		if (AddedAddress) {
			...
			RtlCopyMemory( &bindAddr.Address[0].NdasAddress, AddedAddress->Address, AddedAddress->AddressLength );
		...
		if (!NT_SUCCESS(status)) {
			...
			if (status != STATUS_CLUSTER_NETWORK_NOT_FOUND && status != STATUS_NETWORK_UNREACHABLE && status != NDIS_STATUS_NO_CABLE) {

				NdasDbgCall( 1, "status = %x\n", status );
				NDAS_ASSERT(FALSE);
			}
		...
		RtlCopyMemory( &plugIn->NdasBusAddTargetData, addTargetData, addTargetData->ulSize );
		...
		if (!NT_SUCCESS(status)) {
			...
			NdasDbgCall( 1, "LSBus_PlugInLSBUSDevice() failed %x plugIn->SerialNo %d\n", status, plugIn->SerialNo );
		...
		if(!NT_SUCCESS(status)) {
			LSBus_PlugOutLSBUSDevice(FdoData, plugIn->SerialNo, FALSE);
		...
		NdasDbgCall( 1, "Register Success %d\n", plugIn->SerialNo );
...
typedef struct _NDBUS_ENUMWORKER {
	...
	union {
		...
		UCHAR			buffer[NDBUSWRK_MAX_ADDRESSLEN];			
		TA_ADDRESS		AddedTaAddress;
	};
	...
	)
{
	...
	if (ctx->AddedAddressVaild) {
		...
		PTA_ADDRESS		addedAddress = &ctx->AddedTaAddress;
		LONG			i;

		UCHAR			buffer[FIELD_OFFSET(TRANSPORT_ADDRESS, Address) +
							   (FIELD_OFFSET(TA_ADDRESS, Address) + TDI_ADDRESS_LENGTH_LPX)*16];

		PTRANSPORT_ADDRESS	addrList;
		
		addrList = (PTRANSPORT_ADDRESS)buffer;
		...
		for (i = 0; i < 10; i++) {
			...
			if (LpxTdiV2GetTransportAddress (FIELD_OFFSET(TRANSPORT_ADDRESS, Address) +
											 (FIELD_OFFSET(TA_ADDRESS, Address) + TDI_ADDRESS_LENGTH_LPX)*16,
											 addrList,
											 NULL) != STATUS_SUCCESS) {
			...
			for (idx_addr = 0; idx_addr < addrList->TAAddressCount; idx_addr++) {
				...
				if (addedAddress->AddressType == taAddress->AddressType &&
					addedAddress->AddressLength == taAddress->AddressLength &&
					RtlCompareMemory( &addedAddress->Address,
									  &taAddress->Address,
									  taAddress->AddressLength) == taAddress->AddressLength) {

					break;
				}
			}
	
			if (idx_addr != addrList->TAAddressCount) {
...
Reg_NetworkPnPBindingChange(
	...
	if(DeviceName && DeviceName->Buffer) {
		Bus_KdPrint_Def(BUS_DBG_SS_INFO, ("DeviceName=%ws PnpOpcode=%x\n", DeviceName->Buffer, PnPOpcode));
	} else {
		Bus_KdPrint_Def(BUS_DBG_SS_INFO, ("DeviceName=NULL PnpOpcode=%x\n", PnPOpcode));
	}
...
LSBus_RegisterDevice(
	...
	status = Reg_OpenDeviceInst(&ndasDevInst, Plugin->SerialNo, TRUE, ndasDevRoot);
	if(!NT_SUCCESS(status)) {
...
LSBus_UnregisterDevice(
	...
	Bus_KdPrint_Def(BUS_DBG_SS_ERROR, ("Removing a NDAS device from registry is completed. NTSTATUS:%08lx\n", status));
