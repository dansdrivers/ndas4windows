...

{
	...
	status = SocketLpxFindDeviceContext( networkName, &providerDeviceContext );
	...
	if (status != NDIS_STATUS_SUCCESS) {
		...
		DebugPrint( 1+(status == NDIS_STATUS_NO_CABLE), 
					("[Lpx] LpxOpenAddress: couldn't find providerDeviceContext status = %x\n", status) );
	...
	if (networkName->Port == 0) {
		...
		portAssignedByLpx = TRUE;
	
	} else {

		portAssignedByLpx = FALSE;
	}
	...
	if (address != NULL) {
		...
		DebugPrint( 1, ("[Lpx] !!!!!LpxOpenAddress %p: another process has taken specified address!\n", address) );
...
#if __LPX__
			address->PortAssignedByLpx = portAssignedByLpx;
#endif
...

{
    ...
    if (p == &DeviceContext->AddressPool) {
        ...
        if ((DeviceContext->AddressMaxAllocated == 0) ||
            ...
            LpxWriteResourceErrorLog( DeviceContext,
