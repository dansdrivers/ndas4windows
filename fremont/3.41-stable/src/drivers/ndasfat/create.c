	...
	do {
	    ...
	    try {
			...
			if (IrpContext == NULL) { 
				...
				NDAS_ASSERT( !BooleanFlagOn(irpSp->Flags, SL_OPEN_PAGING_FILE) );
...

{
	...
	if (!FlagOn(((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {
		...
		if (((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_SECONDARY || 
			...
			NDAS_ASSERT( ((PVOLUME_DEVICE_OBJECT)IrpSp->DeviceObject)->NetdiskEnableMode == NETDISK_PRIMARY || 
    ...
    try_exit: NOTHING;
		...
		if (Status == STATUS_SUCCESS && NT_SUCCESS(Iosb.Status)) {
			...
			NDAS_ASSERT( ThisCcb->FileObject );
			...
			try {
				...
				for (ccbListEntry = ThisFcb->NonPaged->CcbQueue.Flink; 
					...
					if (FlagOn(ccb->NdasFatFlags, ND_FAT_CCB_FLAG_READONLY)) {
					...
					if (FlagOn(ccb->NdasFatFlags, ND_FAT_CCB_FLAG_DELAY_CLOSE)) {
				...
				if (writeAceessOpenCount != 1) {
					...
					DebugTrace2( 0, Dbg, ("ThisFcb->PrimarySession = I64%x, IrpContext->PrimaryRequestInfo.PrimarySessionId = %I64x\n",
										  ThisFcb->PrimarySessionId, IrpContext->PrimaryRequestInfo.PrimarySessionId) );
				...
				NDAS_ASSERT( IsFileObjectReadOnly(FileObject)			||
							 writeAceessOpenCount == 1					||
				...
				if (IsFileObjectReadOnly(FileObject)) {
					...
					SetFlag( ThisCcb->NdasFatFlags, ND_FAT_CCB_FLAG_READONLY );
				}
		        ...
		        if (AbnormalTermination()) {
					...
					NDAS_ASSERT( IrpContext->ExceptionStatus == STATUS_CANT_WAIT );
					...
					if (Ccb && NodeType(Fcb) == FAT_NTC_FCB) {
...
try_exit2: NOTHING;
			...
			if (ThisCcb->FileOpenPrimarySessionId != NDASFAT_LOCAL_PRMARY_SESSION_ID) {
    ...
    try {
        ...
        if (AbnormalTermination()) {
			...
			if (LocalCcb2 && NodeType(LocalFcb) == FAT_NTC_FCB) {

				ExAcquireFastMutex( &LocalFcb->NonPaged->CcbQMutex );
				
				LocalCcb2->FileObject = NULL;
				LocalCcb2->Fcb = NULL;
				RemoveEntryList( &LocalCcb2->FcbListEntry );
				InitializeListHead( &LocalCcb2->FcbListEntry );

				ExReleaseFastMutex( &LocalFcb->NonPaged->CcbQMutex );
			}
