...

{
	...
	if (IoGetCurrentIrpStackLocation(Irp)->FileObject == NULL) {
		...
		DebugTrace2( 0, DEBUG_TRACE_ALL, ("IrpSp->FileObject is NULL, IrpSp->MajorFunction = %x, IrpSp->MinorFunction = %x\n", 
										  IoGetCurrentIrpStackLocation(Irp)->MajorFunction, IoGetCurrentIrpStackLocation(Irp)->MinorFunction) );
	}

	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {

		Status = NdasFatSecondaryCommonCleanup( IrpContext, Irp );

		DebugTrace2( -1, Dbg, ("NtfsCommonCleanup -> %08lx\n", Status) );
                ...
                if ( (Fcb->UncleanCount == 1) && (Fcb->FcbCondition != FcbBad) ) {
                    ...
                    if (FlagOn(Fcb->FcbState, FCB_STATE_DELETE_ON_CLOSE) &&
						...
						if (FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW)   &&
							TypeOfOpen == UserFileOpen																				&& 
							Fcb->PrimarySessionId != NDASFAT_LOCAL_PRMARY_SESSION_ID												&& 
							...
							for (ccbListEntry = Fcb->NonPaged->CcbQueue.Flink; 
								...
								NDAS_ASSERT( !FlagOn(Ccb->FileObject->Flags, FO_CLEANUP_COMPLETE) );
							...
							NDAS_ASSERT( writeAceessUncleanCount );

							if (writeAceessUncleanCount > 1) {

							} else if (IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader == NULL ) {
								
							} else if ((ULONGLONG)Fcb->Header.ValidDataLength.LowPart < 
									   IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength) {
																		
								NDAS_ASSERT( Fcb->Header.AllocationSize.LowPart == 
											 IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.AllocationSize );
								
								Fcb->Header.FileSize.LowPart = (ULONG)IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.FileSize;
								Fcb->Header.ValidDataLength.LowPart = (ULONG)IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength;

								DebugTrace2( 0, Dbg, ("Fcb->Header.ValidDataLength.LowPart = %d\n", Fcb->Header.ValidDataLength.QuadPart) );
							   ...
							   DebugTrace2( 0, Dbg2, ("Fcb->Header.ValidDataLength.LowPart = %d,IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength = %d\n", 
													   Fcb->Header.ValidDataLength.LowPart, IrpContext->PrimaryRequestInfo.NdfsWinxpRequestHeader->CleanUp.ValidDataLength) );
