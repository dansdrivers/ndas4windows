...

{
    ...
    if (!NonCachedIo) {
		...
		if (IS_SECONDARY_FILEOBJECT(IrpSp->FileObject)) {
			...
			NDAS_ASSERT( BooleanFlagOn(IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT) );
	...
	if (!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
		IS_SECONDARY_FILEOBJECT(IrpSp->FileObject)) {
		...
		Status = NdasFatSecondaryCommonWrite3( IrpContext, Irp );
		return Status;
	}
#endif
    ...
    try {
        ...
        if ( TypeOfOpen == UserFileOpen ) {
			...
			if (IS_SECONDARY_FILEOBJECT(IrpSp->FileObject)) {
				...
				Status = NdasFatSecondaryCommonWrite( IrpContext, Irp );
				...
				InitialFileSize = StartingVbo + ByteCount;
			}
#endif  
...
#define LONGMAP_COUNTER
				...
				if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY &&
					!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
					IS_SECONDARY_FILEOBJECT(IrpSp->FileObject)) {

					SetFlag( IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT );
					Status = NdasFatSecondaryCommonWrite2( IrpContext, Irp, BytesToWrite );
