...

{
	...
	if (!FlagOn(CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NdasFatFlags, ND_FAT_DEVICE_FLAG_DIRECT_RW) &&
		...
		Status = NdasFatSecondaryCommonRead3( IrpContext, Irp );
		return Status;
	}
#endif
            ...
            if ( NonCachedIo ) {
                ...
                if ( (StartingVbo & (SectorSize - 1)) ||
					...
					if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY &&
						...
						Status = NdasFatSecondaryCommonRead( IrpContext, Irp, ByteCount );
					...
					if (CONTAINING_RECORD(Vcb, VOLUME_DEVICE_OBJECT, Vcb)->NetdiskEnableMode == NETDISK_SECONDARY &&
						...
						Status = NdasFatSecondaryCommonRead( IrpContext, Irp, BytesToRead );
