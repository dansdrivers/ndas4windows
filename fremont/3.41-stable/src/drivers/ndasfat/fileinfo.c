	...
	do {
		...
		try {
			...
			if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
				...
				if (IoGetTopLevelIrp() == (PIRP)FSRTL_FSP_TOP_LEVEL_IRP &&
					IoGetCurrentIrpStackLocation(Irp)->Parameters.QueryFile.FileInformationClass == FileStandardInformation) {
				...
				if (!FatIsTopLevelRequest(IrpContext)) {
					...
					ASSERT( FALSE );
					DebugTrace2( 0, Dbg2, ("TopLevelIrp = %x IoGetCurrentIrpStackLocation(Irp)->Parameters.QueryFile.FileInformationClass = %d\n", 
					IoGetTopLevelIrp(), IoGetCurrentIrpStackLocation(Irp)->Parameters.QueryFile.FileInformationClass) );
				}
...

{
    ...
    try {
		...
		if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
			...
			Status = NdasFatSecondaryCommonQueryInformation( IrpContext, Irp );
...

{
	...
	if (IS_SECONDARY_FILEOBJECT(IoGetCurrentIrpStackLocation(Irp)->FileObject)) {
		...
		Status = NdasFatSecondaryCommonSetInformation( IrpContext, Irp );

		DebugTrace2( -1, Dbg, ("NtfsCommonSetInformation -> %08lx\n", Status) );
...

{
    ...
    DebugTrace(+1, Dbg2, "FatSetBasicInfo...\n", 0);
        ...
        if (FatData.ChicagoMode && (Buffer->CreationTime.QuadPart != 0)) {
            ...
            if ( !FatNtTimeToFatTime( IrpContext,
                ...
                if ((LargeCreationTime.QuadPart >= FatLocalDecThirtyOne1979.QuadPart) &&
                    ...
                    DebugTrace(0, Dbg2, "Invalid CreationTime\n", 0);
                    try_return( Status = STATUS_INVALID_PARAMETER );
                }
        ...
        if (FatData.ChicagoMode && (Buffer->LastAccessTime.QuadPart != 0)) {
            ...
            if ( !FatNtTimeToFatTime( IrpContext,
                ...
                if ((LargeLastAccessTime.QuadPart >= FatLocalDecThirtyOne1979.QuadPart) &&
                    ...
                    DebugTrace(0, Dbg2, "Invalid LastAccessTime\n", 0);
                    try_return( Status = STATUS_INVALID_PARAMETER );
                }
            }
            ...
            if (ModifyLastAccess &&
                ...
                if ( !FatNtTimeToFatTime( IrpContext,
                    ...
                    if ((LargeLastWriteTime.QuadPart >= FatLocalDecThirtyOne1979.QuadPart) &&
                        ...
                        DebugTrace(0, Dbg2, "Invalid LastWriteTime\n", 0);
                        try_return( Status = STATUS_INVALID_PARAMETER );
                    }
                }
        ...
        if (Buffer->FileAttributes != 0) {
            ...
            if (NodeType(Fcb) == FAT_NTC_FCB) {
                ...
                if (FlagOn(Buffer->FileAttributes, FILE_ATTRIBUTE_DIRECTORY)) {
                    ...
                    DebugTrace(0, Dbg2, "Attempt to set dir attribute on file\n", 0);
                    try_return( Status = STATUS_INVALID_PARAMETER );
                }
            ...
            if (FlagOn(Buffer->FileAttributes, FILE_ATTRIBUTE_TEMPORARY)) {
                ...
                if (NodeType(Fcb) == FAT_NTC_DCB) {
                    ...
                    DebugTrace(0, Dbg2, "No temporary directories\n", 0);
                    try_return( Status = STATUS_INVALID_PARAMETER );
                }
    ...
    try_exit: NOTHING;
        ...
        DebugTrace(-1, Dbg2, "FatSetBasicInfo -> %08lx\n", Status);
    }
