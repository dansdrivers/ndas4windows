...
)
{
	...
	*/
	...
	try {
		...
		FileId = DirContext->StartLotIndex;
...
)
{
	...
	FileContext->LotHeaderBuffer = xixcore_AllocateBuffer(XIDISK_DUP_COMMON_LOT_HEADER_SIZE);
	if(!FileContext->LotHeaderBuffer){
		DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
					("Fail XifsdInitializeFileContext LotHeader.\n"));
		return STATUS_INSUFFICIENT_RESOURCES;
	}
	RtlZeroMemory(xixcore_GetDataBuffer(FileContext->LotHeaderBuffer), xixcore_GetBufferSize(FileContext->LotHeaderBuffer));

	FileContext->Buffer =xixcore_AllocateBuffer(XIDISK_DUP_FILE_INFO_SIZE); 
	if(!FileContext->Buffer){
		xixcore_FreeBuffer(FileContext->LotHeaderBuffer);
		DebugTrace(DEBUG_LEVEL_ERROR, DEBUG_TARGET_ALL, 
					("Fail XifsdInitializeFileContext.\n"));
		return STATUS_INSUFFICIENT_RESOURCES;
	}
	RtlZeroMemory(xixcore_GetDataBuffer(FileContext->Buffer), xixcore_GetBufferSize(FileContext->Buffer));
...
)
{
	...
	try{
		...
		if(FileContext->LotHeaderBuffer == NULL){
			FileContext->LotHeaderBuffer = 
				xixcore_AllocateBuffer(XIDISK_DUP_COMMON_LOT_HEADER_SIZE);
		}


		if(FileContext->Buffer == NULL){
			FileContext->Buffer = 
				xixcore_AllocateBuffer(XIDISK_DUP_FILE_INFO_SIZE); 
		}
...
)
{
	...
	try{
		...
		if(FileContext->LotHeaderBuffer){
			xixcore_FreeBuffer(FileContext->LotHeaderBuffer);
			FileContext->LotHeaderBuffer = NULL;
		}
...
)
{
	...
	try{
		...
		xixcore_ZeroBufferOffset(FileContext->LotHeaderBuffer);
		RtlZeroMemory(xixcore_GetDataBuffer(FileContext->LotHeaderBuffer),xixcore_GetBufferSize(FileContext->LotHeaderBuffer)); 
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ALL, (DEBUG_TARGET_DIRINFO|DEBUG_TARGET_FCB|DEBUG_TARGET_FILEINFO),
						("Fail Read File LotHeader(%I64d).\n", FileContext->LotNumber));
			try_return (RC);
		}	



		xixcore_ZeroBufferOffset(FileContext->Buffer);
		RtlZeroMemory(xixcore_GetDataBuffer(FileContext->Buffer),xixcore_GetBufferSize(FileContext->Buffer)); 
		...
		if(!NT_SUCCESS(RC)){
			DebugTrace(DEBUG_LEVEL_ALL, (DEBUG_TARGET_DIRINFO|DEBUG_TARGET_FCB|DEBUG_TARGET_FILEINFO),
						("Fail Read File Header(%I64d).\n", FileContext->LotNumber));
			try_return (RC);
		}	
		
		pLotHeader = (PXIDISK_COMMON_LOT_HEADER)xixcore_GetDataBufferWithOffset(FileContext->LotHeaderBuffer);
...
)
{
	...
	ASSERT(Waitable);
	ASSERT(LotHeader);
	ASSERT(xixcore_GetBufferSize(LotHeader) >= XIDISK_DUP_COMMON_LOT_HEADER_SIZE);
	ASSERT(Buffer);
	ASSERT(xixcore_GetBufferSize(Buffer) >= XIDISK_DUP_FILE_INFO_SIZE);
	...
	try{
		...
		RtlZeroMemory(xixcore_GetDataBuffer(LotHeader), xixcore_GetBufferSize(LotHeader));
		...
		RtlZeroMemory(xixcore_GetDataBuffer(Buffer), xixcore_GetBufferSize(Buffer));
		...
		pFileHeader = (PXIDISK_FILE_INFO)xixcore_GetDataBufferWithOffset(Buffer);
...
)
{
	...
	Buffer = xixcore_AllocateBuffer(XIDISK_DUP_FILE_INFO_SIZE);
	...
	try{
		...
		if(pFCB->XixcoreFcb.FCBType == FCB_TYPE_DIR){
			pDirHeader = (PXIDISK_DIR_INFO)xixcore_GetDataBufferWithOffset(Buffer);
			...
			if(pDirHeader->NameSize != 0){
				uint16 Size = (uint16)pDirHeader->NameSize;
			...
			pFCB->RealFileSize = pDirHeader->FileSize;
			pFCB->XixcoreFcb.RealAllocationSize = pDirHeader->AllocationSize;
			
			pFCB->ValidDataLength.QuadPart =
			pFCB->FileSize.QuadPart = pDirHeader->FileSize;
			pFCB->AllocationSize.QuadPart = pDirHeader->AllocationSize; 
			...
			if(pFileHeader->NameSize != 0){
				uint16 Size = (uint16)pFileHeader->NameSize;
				if(Size > XIFS_MAX_NAME_LEN){
					Size = XIFS_MAX_NAME_LEN;
				}
			...
			pFCB->RealFileSize = pFileHeader->FileSize;
			pFCB->XixcoreFcb.RealAllocationSize = pFileHeader->AllocationSize;
			
			pFCB->ValidDataLength.QuadPart =
			pFCB->FileSize.QuadPart =  pFileHeader->FileSize;
			pFCB->AllocationSize.QuadPart = pFileHeader->AllocationSize;
