...
retry:
	...
	try{
		...
		size = SECTOR_ALIGNED_SIZE(VCB->XixcoreVcb.SectorSizeBit, (uint32) ((VCB->XixcoreVcb.NumLots + 7)/8 + sizeof(XIDISK_BITMAP_DATA) -1));
		BuffSize = size*2;
		...
		RtlZeroMemory(xixcore_GetDataBuffer(pTempFreeLotMap->Data), BuffSize);
		...
		*/
		...
		RtlCopyMemory(xixcore_GetDataBufferOfBitMap(xixcoreCtx->VolumeFreeMap->Data),
		...
		RC = xixcore_WriteBitMapWithBuffer(
				&VCB->XixcoreVcb,
				(xc_sector_t)xixcoreCtx->HostUsedLotMapIndex, 
		...
		RC = xixcore_WriteBitMapWithBuffer(
				&VCB->XixcoreVcb,
				(xc_sector_t)xixcoreCtx->HostUnUsedLotMapIndex, 
...
)
{
	...
	RtlZeroMemory(xixcore_GetDataBuffer(tmpBuf), xixcore_GetBufferSize(tmpBuf));
	...
	RtlZeroMemory(xixcore_GetDataBuffer(tmpLotHeader), xixcore_GetBufferSize(tmpLotHeader));
