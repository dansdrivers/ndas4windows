...

{
...
#else
			if (NdasScsiIoFreeErrorLogEntry) {

				NdasScsiIoFreeErrorLogEntry(Packet);
			
			} else {

				RtlZeroMemory( Packet, sizeof(IO_ERROR_LOG_PACKET) + sizeof(SCSIPORT_ALLOCFAILURE_DATA) );
    ...
    if (InvalidPage != NULL) {
        ...
        if (InvalidPage == CurrentValue) {
			...
			if (NdasScsiMmProtectMdlSystemAddress) {
				...
				NdasScsiMmProtectMdlSystemAddress(DriverExtension->UnusedPageMdl, PAGE_READWRITE);
			}
#endif
	...
	while (DriverExtension->InitChain) {

		PSP_INIT_CHAIN_ENTRY entry;;

		entry = DriverExtension->InitChain;
		DriverExtension->InitChain = entry->NextEntry;
