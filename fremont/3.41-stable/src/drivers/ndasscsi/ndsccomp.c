	...
	) 
{
	...
	return IoctlToLanscsiBus( IOCTL_NDASBUS_SETPDOINFO,
							  &BusSet,
...
{
	if (FlagOn(Ccb->Flags, CCB_FLAG_MUST_SUCCEED)) {
		
		DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, 
					("ccbStatus is overided %p Srb->Cdb[0] = %s\n", Srb, CdbOperationString(Srb->Cdb[0])) );
	...
	if (HwDeviceExtension->Stopping == FALSE) {
...
NdscCopyQueryOutputToSrb(
	...
	case LurPrimaryLurnInformation: { // NdscPrimaryUnitDiskInformation
		...
		RtlCopyMemory( primUnitDisk->UnitDisk.UserID, &lurPrimaryInfo->PrimaryLurn.UserID, sizeof(primUnitDisk->UnitDisk.UserID) );
		RtlCopyMemory( primUnitDisk->UnitDisk.Password, &lurPrimaryInfo->PrimaryLurn.Password, sizeof(primUnitDisk->UnitDisk.Password) );
		...
		DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
					("NDSC_ID: %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	case LurEnumerateLurn: { // NdscLurInformation
		...
		if (HwDeviceExtension->LURs[0]) {
			...
			NDAS_ASSERT(HwDeviceExtension->LURs[0]->NodeCount >= 1);
		...
		info->Length = FIELD_OFFSET(NDSCIOCTL_LURINFO, Lurns) + sizeof(NDSC_LURN_FULL) * nodeCount;
		...
		for (idx_lurn = 0; idx_lurn < info->LurnCnt; idx_lurn++) {
			...
			RtlCopyMemory( unitDisk->UserID, &lurnInformation->UserID, sizeof(unitDisk->UserID) );

			RtlCopyMemory( unitDisk->Password, &lurnInformation->Password, sizeof(unitDisk->Password) );
	...
	)
{
	...
	if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_FLAG_VALID)) {
		...
		if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_DEGRADED)) {
			...
			if (!ADAPTER_ISSTATUSFLAG(HwDeviceExtension, NDASSCSI_ADAPTER_STATUSFLAG_MEMBER_FAULT)) {
				...
				DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
							("NDASSCSI_ADAPTER_STATUSFLAG_MEMBER_FAULT is Set\n") );
			}
		...
		if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_RECOVERING)) {
			...
			if (!ADAPTER_ISSTATUSFLAG(HwDeviceExtension, NDASSCSI_ADAPTER_STATUSFLAG_RECOVERING)) {
				...
				DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
							("NDASSCSI_ADAPTER_STATUSFLAG_RECOVERING is Set\n") );
			}
		...
		if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_FAILURE)) {
			...
			if (!ADAPTER_ISSTATUSFLAG(HwDeviceExtension, NDASSCSI_ADAPTER_STATUSFLAG_RAID_FAILURE)) {
				...
				DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
							("NDASSCSI_ADAPTER_STATUSFLAG_RAID_FAILURE is Set\n") );
			}
		...
		if (LsCcbIsStatusFlagOn(Ccb, CCBSTATUS_FLAG_RAID_NORMAL)) {
			...
			if (!ADAPTER_ISSTATUSFLAG(HwDeviceExtension, NDASSCSI_ADAPTER_STATUSFLAG_RAID_NORMAL)) {
				...
				DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
							("NDASSCSI_ADAPTER_STATUSFLAG_RAID_NORMAL is Set\n") );
			}
	...
	if (AdapterStatus != AdapterStatusBefore) {
		...
		if (!(AdapterStatusBefore & NDASSCSI_ADAPTER_STATUSFLAG_MEMBER_FAULT) &&
			...
			DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
						("Ccb:%p CcbStatus %d. Set member fault.\n", Ccb, Ccb->CcbStatus) );
		}
		...
		if ((AdapterStatusBefore & NDASSCSI_ADAPTER_STATUSFLAG_MEMBER_FAULT) &&
			...
			DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
						("Ccb:%p CcbStatus %d. Reset member fault.\n", Ccb, Ccb->CcbStatus) );
		}
		...
		if ((AdapterStatusBefore & NDASSCSI_ADAPTER_STATUSFLAG_RECONNECT_PENDING) &&
			...
			DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
						("Ccb:%p CcbStatus %d. reconnecting\n", Ccb, Ccb->CcbStatus) );
		}
		...
		if (!(AdapterStatusBefore & NDASSCSI_ADAPTER_STATUSFLAG_RECOVERING) &&
			...
			DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
						("Ccb:%p CcbStatus %d. Started recovering\n", Ccb, Ccb->CcbStatus) );
		}
		...
		if ((AdapterStatusBefore & NDASSCSI_ADAPTER_STATUSFLAG_RECOVERING) &&
			...
			DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
						("Ccb:%p CcbStatus %d. Ended recoverin\n", Ccb, Ccb->CcbStatus) );
		}
		...
		if ((AdapterStatus & NDASSCSI_ADAPTER_STATUSFLAG_RAID_FAILURE)	 &&
			...
			DebugTrace( NDASSCSI_DBG_MINIPORT_INFO,
						("Ccb:%p CcbStatus %d. RAID failure\n", Ccb, Ccb->CcbStatus) );
		}
	...
	if (Ccb->CcbStatus == CCB_STATUS_SUCCESS) {
		...
		case CCB_OPCODE_DEVLOCK: {
			...
			RtlCopyMemory( ioCtlAcReDeviceLock->LockData, lurnAcReDeviceLock->LockData, NDSCLOCK_LOCKDATA_LENGTH );
	...
	if (Ccb->CcbStatus == CCB_STATUS_SUCCESS) {
		...
		if (Ccb->OperationCode == CCB_OPCODE_EXECUTE) {
			...
			switch (Ccb->SrbCdb.AsByte[0]) {

			case 0x3E:		// READ_LONG
			case SCSIOP_READ:
			case SCSIOP_READ16: {

				LsCcbResetStatusFlag( Ccb, CCBSTATUS_FLAG_TIMER_COMPLETE );	
				break;
			}
			...
			default:
				...
				NDAS_ASSERT( Ccb->SrbCdb.AsByte[0] == SCSIOP_INQUIRY			|| Ccb->SrbCdb.AsByte[0] == SCSIOP_START_STOP_UNIT		||
							 Ccb->SrbCdb.AsByte[0] == SCSIOP_READ_CAPACITY		|| Ccb->SrbCdb.AsByte[0] == SCSIOP_READ_CAPACITY16		||							
							 Ccb->SrbCdb.AsByte[0] == SCSIOP_RESERVE_UNIT		|| Ccb->SrbCdb.AsByte[0] == SCSIOP_RESERVE_UNIT10		||							
							 Ccb->SrbCdb.AsByte[0] == SCSIOP_RELEASE_UNIT		|| Ccb->SrbCdb.AsByte[0] == SCSIOP_RELEASE_UNIT10		||							
							 Ccb->SrbCdb.AsByte[0] == SCSIOP_MODE_SENSE			|| Ccb->SrbCdb.AsByte[0] == SCSIOP_MODE_SELECT			||							
							 Ccb->SrbCdb.AsByte[0] == SCSIOP_SYNCHRONIZE_CACHE	|| Ccb->SrbCdb.AsByte[0] == SCSIOP_SYNCHRONIZE_CACHE16	||							
							 Ccb->SrbCdb.AsByte[0] == SCSIOP_SEEK				|| Ccb->SrbCdb.AsByte[0] == SCSIOP_REASSIGN_BLOCKS		||							
							 Ccb->SrbCdb.AsByte[0] == SCSIOP_REQUEST_SENSE		|| Ccb->SrbCdb.AsByte[0] == SCSIOP_FORMAT_UNIT			||							
							 Ccb->SrbCdb.AsByte[0] == SCSIOP_FORMAT_UNIT		|| Ccb->SrbCdb.AsByte[0] == SCSIOP_MEDIUM_REMOVAL );
	...
	if (FlagOn(srb->SrbFlags, SRB_FLAGS_DISABLE_DISCONNECT | SRB_FLAGS_BYPASS_FROZEN_QUEUE | SRB_FLAGS_BYPASS_LOCKED_QUEUE)) {
	...
	InsertTailList( &HwDeviceExtension->CcbCompletionList, &Ccb->ListEntryForMiniPort );
	...
	if (Ccb->AbortCcb != NULL) {
		...
		InsertTailList( &HwDeviceExtension->CcbCompletionList, &Ccb->AbortCcb->ListEntryForMiniPort );
	}
	...
	if (postCompleteIrp) {
		...
		do {
			...
			IoSetCompletionRoutine(	pCompletionIrp, CompletionIrpCompletionRoutine, completionSrb, TRUE, TRUE, TRUE );
