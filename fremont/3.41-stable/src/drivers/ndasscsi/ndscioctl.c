	...
	*SrbStatus = SRB_STATUS_SUCCESS;
	...
	case NdscPrimaryUnitDiskInformation: {
		...
		if (lurBuffer) {
			...
			SetFlag( Ccb->Flags, CCB_FLAG_DATABUF_ALLOCATED );
		}
...
SrbIoctlGetDVDSTatus(
	...
	UINT8				LurBuffer[sizeof(LURN_DVD_STATUS)];
	...
	)
{
	...
	DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, 
				("controlCode %x NDASSCSI_IOCTL_GET_DVD_STATUS = %x\n", controlCode, NDASSCSI_IOCTL_GET_DVD_STATUS) );
	...
	case NDASSCSI_IOCTL_GET_DVD_STATUS: {
		...
		srbStatus = SrbIoctlGetDVDSTatus( HwDeviceExtension,
										  LuExtension,
										  srbIoctlBufferLength,
										  srbIoctlBuffer,
	...
	case NDASSCSI_IOCTL_GET_SLOT_NO:
		...
		DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, ("Get Slot No. Slot number is %d\n", HwDeviceExtension->SlotNumber) );
	...
	case NDASSCSI_IOCTL_QUERYINFO_EX: {
		...
		status = SrbIoctlQueryInfo( HwDeviceExtension,
									LuExtension,
									Srb,
									QueryInfo,
									srbIoctlBufferLength,
	...
	case NDASSCSI_IOCTL_UPGRADETOWRITE: {
		...
		LurnUpdate = ExAllocatePoolWithTag( NonPagedPool, sizeof(LURN_UPDATE), NDSC_PTAG_IOCTL );
	...
	case NDASSCSI_IOCTL_ADD_USERBACL: {
		...
		PLSU_BLOCK_ACE			lsuBaceBackup;

		DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, ("NDASSCSI_IOCTL_ADD_USERBACL Slot number is %d\n", HwDeviceExtension->SlotNumber) );
		...
		NDAS_ASSERT( HwDeviceExtension->LURs[0]->BlockBytes >= 512		&&
					 HwDeviceExtension->LURs[0]->BlockBytes <= 512 * 8	&&
					 HwDeviceExtension->LURs[0]->BlockBytes % 512 == 0 );

		lsuBace = LsuCreateBlockAce( lsuAccessMode, bace->StartingOffset, bace->Length, HwDeviceExtension->LURs[0]->BlockBytes, 0 );
		...
		lsuBaceBackup = LsuCreateBlockAce( lsuAccessMode, bace->StartingOffset, bace->Length, HwDeviceExtension->LURs[0]->BlockBytes, lsuBace->BlockAceId );

		if (lsuBaceBackup == NULL) {

			NDAS_ASSERT(FALSE);

			LsuFreeBlockAce(lsuBace);
		...
		*blockAceId = lsuBace->BlockAceId;
		...
		LsuInsertAce( &HwDeviceExtension->BackupUserBacl[0], lsuBaceBackup );
	...
	case NDASSCSI_IOCTL_REMOVE_USERBACL: {
		...
		PLSU_BLOCK_ACE				lsuBacl2;

		DebugTrace( NDASSCSI_DBG_MINIPORT_INFO, ("NDASSCSI_IOCTL_REMOVE_USERBACL Slot number is %d\n", HwDeviceExtension->SlotNumber) );
		...
		lsuBacl = LsuRemoveAceById( &HwDeviceExtension->BackupUserBacl[0], blockAceId );
	...
	case NDASSCSI_IOCTL_DEVICE_LOCK: {
		...
		status = SrbIoctlDeviceLock( HwDeviceExtension,
										LuExtension,
