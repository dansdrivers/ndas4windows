        ...
        *Irp = NULL;
        ...
        case IRP_MN_SURPRISE_REMOVAL:
			...
			if (FlagOn( Vcb->VcbState, VCB_STATE_VOLUME_MOUNTED )) {
				...
				SetFlag( IrpContext->NdasNtfsFlags, NDAS_ROFS_IRP_CONTEXT_FLAG_DONT_REMOVE_DEVICE );

				NtfsPerformSurpriseRemoval( IrpContext, Vcb );

				ClearFlag( IrpContext->NdasNtfsFlags, NDAS_ROFS_IRP_CONTEXT_FLAG_DONT_REMOVE_DEVICE );
			}
#endif
            break;
