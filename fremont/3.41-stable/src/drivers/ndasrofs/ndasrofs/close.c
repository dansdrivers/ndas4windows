...

{
...
#ifdef __NDAS_ROFS_AVOID_ALYAK_BUG__

	if (Vcb == NULL) {

		NDAS_ASSERT( FileObject && FileObject->Vpb && FileObject->Vpb->DeviceObject && 
					 FileObject->Vpb->DeviceObject->DriverObject == NtfsData.DriverObject );
		
		if (FileObject && FileObject->Vpb && FileObject->Vpb->DeviceObject &&
			FileObject->Vpb->DeviceObject->DriverObject == NtfsData.DriverObject) {

			PDEVICE_OBJECT	tempDeviceObject;

			tempDeviceObject = NtfsData.DriverObject->DeviceObject;
	
			while (tempDeviceObject) {

				if (tempDeviceObject == FileObject->Vpb->DeviceObject) {
			...
			NDAS_ASSERT( tempDeviceObject == NULL );

			if (tempDeviceObject == NULL) {

				FileObject->Vpb->DeviceObject->NextDevice = NtfsData.DriverObject->DeviceObject;
				NtfsData.DriverObject->DeviceObject = FileObject->Vpb->DeviceObject;
			}
		}
		...
		DebugTrace( -1, DEBUG_TRACE_ERROR, ("NtfsFsdClose -> %08lx\n", Status) );
		return Status;
	}
    ...
    if (FileObject == Vcb->LogFileObject) {
        ...
        DebugTrace( -1, Dbg, ("NtfsFsdClose LogFileObject-> %08lx\n", Status) );
        return Status;
    }
