	...
	__in PUNICODE_STRING RegistryPath)
{
    ...
    KdPrint(("NdasDlu DriverEntry build on %s %s at %s MSVC %d\n", 
	...
	KdPrint(("NdasDlu loaded successfully.\n"));

	//
	// Set NDASK library debug level.
	//
	KDebugLevel = 1;
    ...
    __in PDRIVER_OBJECT DriverObject)
{
	...
	KdPrint(("NdasDlu unloaded successfully.\n"));
}
