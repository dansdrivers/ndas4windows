...
static void TranslateScsi2Packet(PCCB Ccb)
{
	...
	if(c[0] == 0xD8) {
		...
		if((DataBufferLength / TransferLength) == 2352) {
			KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("Vendor PKCMD 0x%02x to READ_CD(0xBE)\n",c[0]));
	...
	)
{
	...
	do {
		...
		NDAS_ASSERT( loginInfo.MaxDataTransferLength == (64*1024) || loginInfo.MaxDataTransferLength == (32*1024) );
		...
		status = LspLogin( &IdeDisk->CommLanScsiSession, &loginInfo, LSProto, NULL, TRUE );
		...
		PCCB Ccb)
{
	...
	if(c[0] == SCSIOP_GET_CONFIGURATION)
	{
		...
		for(i = 0; i< max; i++)
		{
			KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("CONGIFURATION: (%02x):(%02x):(%02x):(%02x): (%02x):(%02x):(%02x):(%02x)\n",
	...
	if(c[0] == SCSIOP_READ_CAPACITY)
	{
		...
		for(i = 0; i< max; i++)
		{
			KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("CAPACITY: (%02x):(%02x):(%02x):(%02x): (%02x):(%02x):(%02x):(%02x)\n",
				sc_buf[i*8+0],sc_buf[i*8+1],sc_buf[i*8+2],sc_buf[i*8+3],sc_buf[i*8+4],sc_buf[i*8+5],sc_buf[i*8+6],sc_buf[i*8+7]));
		}
		/*
...
ODD_BusyWait(LONG time)
{
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_NOISE, 
				("Entered\n"));
	...
	)
{
	...
	NDAS_ASSERT( SenseDataLen >= sizeof(SENSE_DATA) );
	...
	)
{
	...
	if (rawSectorCountData->Header.FeatureCode[0] == 0x0 && rawSectorCountData->Header.FeatureCode[1] == 0x10) {
		...
		NDAS_ASSERT( IdeDisk->BytesPerBlock == 2048 );
			  ...
			  )
{
		...
		KDPrintM( NDASSCSI_DBG_LURN_IDE_INFO, 
	...
	)
{
	...
	if (status != STATUS_SUCCESS || response != LANSCSI_RESPONSE_SUCCESS) {
		...
		NDAS_ASSERT( status == STATUS_PORT_DISCONNECTED || status == STATUS_CANCELLED );
	...
	NDAS_ASSERT( IdeDisk->RegStatus == 0x00 && IdeDisk->RegError == 0x00 );
...
//	NDAS_ASSERT( !FlagOn(IdeDisk->RegStatus, BUSY_STAT) );
	...
	do {
		...
		if (senseData->SenseKey == SCSI_SENSE_NO_SENSE				&& // 0x00
			...
			if (Ccb->PKCMD[0] == SCSIOP_READ			||
				...
				//
				NDAS_ASSERT( FlagOn(regStatus, BUSY_STAT) );
#endif
...
//				NDAS_ASSERT( !FlagOn(regStatus, BUSY_STAT) );
			...
			NDAS_ASSERT( NDAS_ASSERT_PAKCET_TEST );
		}
	...
	)
{
	...
	if(	Ccb->Cdb[0] != SCSIOP_READ &&
		...
		NDAS_ASSERT( Ccb->DataBufferLength <= IdeDisk->MaxDataSendLength );
		NDAS_ASSERT( Ccb->DataBufferLength <= IdeDisk->MaxDataRecvLength );
	}
#endif
	if (1												&&
	...
	case SCSIOP_TEST_UNIT_READY: {	//0x00
		...
		NDAS_ASSERT( Ccb->SecondaryBufferLength == 0 );
	...
	case SCSIOP_READ6: {	//0x08 SCSIOP_RECEIVE
		...
		NDAS_ASSERT( logicalBlockAddress & 0x80000000 ||
		...
		NDAS_ASSERT( IdeDisk->BytesPerBlock );
		transferLength = transferBlocks * (IdeDisk->BytesPerBlock);
		...
		NDAS_ASSERT( Ccb->SecondaryBufferLength % 4 == 0 );
	...
	case SCSIOP_SEEK6: {	//0x0B SCSIOP_TRACK_SELECT SCSIOP_SLEW_PRINT SCSIOP_SET_CAPACITY 
		...
		Ccb->PacketCdb.SEEK.LogicalBlockAddress[0] = (UCHAR)((logicalBlockAddress >> 24) & 0xFF);
		Ccb->PacketCdb.SEEK.LogicalBlockAddress[1] = (UCHAR)((logicalBlockAddress >> 16) & 0xFF);
		Ccb->PacketCdb.SEEK.LogicalBlockAddress[2] = (UCHAR)((logicalBlockAddress >> 8) & 0xFF);
		Ccb->PacketCdb.SEEK.LogicalBlockAddress[3] = (UCHAR)((logicalBlockAddress) & 0xFF);
	...
	case SCSIOP_MODE_SELECT: {	//0x15
		...
		NDAS_ASSERT( parameterListLength == Ccb->DataBufferLength );

		parameterListLength += 4;
		NDAS_ASSERT( parameterListLength == Ccb->SecondaryBufferLength );
		...
		NDAS_ASSERT( sizeof(IdeDisk->Buffer) >= Ccb->SecondaryBufferLength );
	...
	case SCSIOP_MODE_SENSE: {	//0x1A
		...
		NDAS_ASSERT( allocationLength == Ccb->DataBufferLength );

		allocationLength += 4;
		NDAS_ASSERT( allocationLength == Ccb->SecondaryBufferLength );
	...
	case SCSIOP_WRITE: {	//0x2A
		...
		NDAS_ASSERT( IdeDisk->LogicalBlockAddress == 0	||
	...
	case SCSIOP_SYNCHRONIZE_CACHE: {	//0x35
		...
		NDAS_ASSERT( Ccb->DataBufferLength == 0 );
	...
	case SCSIOP_REPORT_LUNS: {	// 0xA0
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("size of allocated buffer %d\n",lunsize));
	...
	case SCSIOP_MODE_SELECT10: {
		...
		NDAS_ASSERT( blockDescriptorLength == 0 );
		NDAS_ASSERT( modeDataLength == 0 );
		NDAS_ASSERT( modeDataLength <= MAX_PIO_LENGTH_4096 );
		...
		case MODE_PAGE_WRITE_PARAMETERS: {
			...
			NDAS_ASSERT( pageLength == Ccb->SecondaryBufferLength - 10 );
		...
		while ((pageLength - 8) < (modeDataLength <= MAX_PIO_LENGTH_4096 ? modeDataLength : 0)) {
			...
			if (pageCode == MODE_PAGE_CAPABILITIES) { // 0x2A
				...
				NDAS_ASSERT( cdvdCapabilitiesPage->PageCode == MODE_PAGE_CAPABILITIES );
		...
		NDAS_ASSERT( (pageLength - 8) < (modeDataLength <= MAX_PIO_LENGTH_4096 ? modeDataLength : 0) );
	...
	if (status != STATUS_SUCCESS) {
		...
		if (status == STATUS_CANCELLED) {
			...
			NDAS_ASSERT( Ccb->SrbCdb.AsByte[0] == SCSIOP_GET_PERFORMANCE/*		||
						 Ccb->SrbCdb.AsByte[0] == 0xFC							|| 
						 Ccb->SrbCdb.AsByte[0] == SCSIOP_READ_DVD_STRUCTURE */);

			if (IdeDisk->CanceledCount) {

				NDAS_ASSERT( Ccb->SrbCdb.AsByte[0] == IdeDisk->PrevRequest );
			}
		...
		NDAS_ASSERT( response == LANSCSI_RESPONSE_SUCCESS || response == LANSCSI_RESPONSE_T_COMMAND_FAILED );
			...
			if (Ccb->SenseBuffer && (Ccb->SenseDataLength >= FIELD_OFFSET(SENSE_DATA, FieldReplaceableUnitCode))) {
				...
				if (status == STATUS_SUCCESS && response == LANSCSI_RESPONSE_SUCCESS) {
					...
					if (status == STATUS_CANCELLED) {
						...
						NDAS_ASSERT( Ccb->SrbCdb.AsByte[0] == SCSIOP_MODE_SELECT10			||
		...
		if (IdeDisk->DVD_REPEAT_COUNT > 1) {
			...
			NDAS_ASSERT( IdeDisk->PrevRequest == Ccb->Cdb[0] );
		}
		...
		case SCSIOP_MODE_SENSE10: {	//0x5A
			...
			if (Ccb->SecondaryBufferLength > 9) {
				...
				if (Ccb->PacketCdb.MODE_SENSE10.PageCode != MODE_SENSE_RETURN_ALL &&
					...
					NDAS_ASSERT( Ccb->PacketCdb.MODE_SENSE10.PageCode == (((PUINT8)(Ccb->SecondaryBuffer))[8] & 0x3F) );
				}
			}
	...
	)
{
	...
	KDPrintM( NDASSCSI_DBG_LURN_IDE_NOISE, ("Entered\n") );

	KDPrintM( NDASSCSI_DBG_LURN_IDE_NOISE, ("CMD 0x%02x\n", Ccb->Cdb[0]) );
	...
	if (IdeDisk->RegStatus & BUSY_STAT) {
		...
		NDAS_ASSERT( WaitCount != MAX_WAIT_COUNT );
	}
...
//		ODD_GetSectorCount(IdeDisk);
	...
	if (NeedRepeat == TRUE) {
		...
		NDAS_ASSERT( Ccb->CcbStatus == CCB_STATUS_BUSY );
						...
						)
{
	...
	*NeedRepeat = 0;
	...
	NDAS_ASSERT( Ccb->SecondaryBufferLength <= IdeDisk->MaxDataSendLength );
...
//			Ccb->PKCMD[1] = Ccb->PKCMD[1] & 0xEF;
			...
			{
				KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("Command (0x%02x) Error Read only Access !!!\n",Command));
	...
	case SCSIOP_READ_DVD_STRUCTURE:
		{
			...
			if(transferLen >MAX_PIO_LENGTH_4096)
			{
				KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("Command (0x%02x) Length change Transfer length %d!!!\n",Command, transferLen));
				KDPrintM(NDASSCSI_DBG_LURN_IDE_NOISE, ("PKCMD 0x%02x : buffer size %d\n", Command, transferLen));
				transferLen = MAX_PIO_LENGTH_4096;
...
//		break;
	...
	case SCSIOP_READ_CAPACITY: {
		...
		if (ntStatus != STATUS_SUCCESS) {
			...
			NDAS_ASSERT( ntStatus == STATUS_PORT_DISCONNECTED );
			...
			if (response != LANSCSI_RESPONSE_SUCCESS) {
				...
				NDAS_ASSERT( response == LANSCSI_RESPONSE_T_COMMAND_FAILED );
...
//
							...
							KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("ProcessIdePacketRequest: PKCMD (0x%02x), RegStatus(0x%02x), RegError(0x%02x),ErrorCode(0x%02x),SenseKey(0x%02x), ASC(0x%02x), ASCQ(0x%02x)\n",
							...
							if(IdeDisk->ODD_STATUS == RESET_POWERUP)
							{
								KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("POWER RESET 3 CMD %02x\n",Ccb->PKCMD[0]));
								*NeedRepeat = TRUE;
								Ccb->ResidualDataLength = Ccb->DataBufferLength;
								LsCcbSetStatus(Ccb, CCB_STATUS_BUSY);
								//LSCCB_SETSENSE(Ccb, SCSI_SENSE_NOT_READY , SCSI_ADSENSE_LUN_NOT_READY, 0x08);
							...
							{
								KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("NOT_READY_MEDIA 4 CMD %02x\n",Ccb->PKCMD[0]));
								*NeedRepeat = TRUE;
								Ccb->ResidualDataLength = Ccb->DataBufferLength;
								//LSCCB_SETSENSE(Ccb, SCSI_SENSE_NOT_READY , SCSI_ADSENSE_LUN_NOT_READY, 0x08);
								RtlZeroMemory(pSenseData, sizeof(Ccb->SenseDataLength));
				...
				if ((Ccb->SenseDataLength >= FIELD_OFFSET(SENSE_DATA, FieldReplaceableUnitCode)) && (Ccb->SenseBuffer)) {
					...
					if (IdeDisk->ODD_STATUS == RESET_POWERUP) {
						...
						if (Ccb->PKCMD[0] != 0x0) {
							...
							KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("POWER RESET 1 CMD %02x\n",Ccb->PKCMD[0]));
							*NeedRepeat = TRUE;
							Ccb->ResidualDataLength = Ccb->DataBufferLength;
							//LSCCB_SETSENSE(Ccb, SCSI_SENSE_NOT_READY , SCSI_ADSENSE_LUN_NOT_READY, 0x08);
							pSenseData->ErrorCode = 0x0;
							...
							KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("POWER RESET 2 CMD %02x\n",Ccb->PKCMD[0]));
							pSenseData->ErrorCode = 0x0;
							LSCCB_SETSENSE(Ccb, 0x0 , 0x0, 0x0);
							LsCcbSetStatus(Ccb, CCB_STATUS_BUSY);
					...
					if (IdeDisk->ODD_STATUS == MEDIA_ERASE_ERROR) {
						...
						KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("MEDIA_ERASE_ERROR 4 CMD %02x\n",Ccb->PKCMD[0]));
						*NeedRepeat = TRUE;
						Ccb->ResidualDataLength = Ccb->DataBufferLength;
						//LSCCB_SETSENSE(Ccb, SCSI_SENSE_NOT_READY , SCSI_ADSENSE_LUN_NOT_READY, 0x08);
						RtlZeroMemory(pSenseData, sizeof(Ccb->SenseDataLength));
				...
				)
{
	...
	KDPrintM(NDASSCSI_DBG_LURN_IDE_INFO, ("Reconnection : Send Syncronize cache command for flush controler buffer\n"));
	...
	do {
		...
		if (Lurn->LurnStatus == LURN_STATUS_STALL) {
...
#if 0
			{
				...
				NDAS_ASSERT( status == STATUS_SUCCESS || status == STATUS_PORT_DISCONNECTED || status == STATUS_CANCELLED );

				if (status != STATUS_SUCCESS || PduResponse != LANSCSI_RESPONSE_SUCCESS) {

					NDAS_ASSERT( status == STATUS_CANCELLED || status == STATUS_PORT_DISCONNECTED );
	...
	)
{
	...
	UINT64				blockAddr;
	UINT32				transferBlocks;
	ULONG				writeSplitBlockBytes = IdeDisk->MaxDataSendLength;
...
//	LurnIdeDiskResetProtocolCounters( IdeDisk );
	...
	NDAS_ASSERT( writeSplitBlockBytes == (32*1024) || writeSplitBlockBytes == (64*1024) );
	
	// Calculate the number of split blocks
	
	if (doSplit) {

		chooseDataLength = NdasFcChooseTransferSize( &IdeDisk->LanScsiSession->SendNdasFcStatistics, 
													 (transferBlocks * IdeDisk->LuHwData.BlockBytes < writeSplitBlockBytes) ?
													 (transferBlocks * IdeDisk->LuHwData.BlockBytes) : writeSplitBlockBytes );
...
#if DBG
	if(doSplit) {
		NDAS_ASSERT( splitBlocks * bytesOfBlock <= 65536 );
		NDAS_ASSERT( (Ccb->SecondaryBufferLength % bytesOfBlock) == 0 );
	}
	else {
		NDAS_ASSERT(Ccb->SecondaryBufferLength <= 65536);
	}
#endif
	...
	if (status == STATUS_SUCCESS) {
		...
		if (doSplit) {
			...
			NdasFcUpdateTrasnferSize( &IdeDisk->LanScsiSession->SendNdasFcStatistics, 
									  chooseDataLength, 
									  transferBlocks * bytesOfBlock,
									  startTime, 
									  endTime );
	...
	)
{
	...
	UINT64				blockAddr;
	UINT32				transferBlocks;
	ULONG				readSplitBlockBytes = IdeDisk->MaxDataRecvLength;
	ULONG				transferedBlocks;
	ULONG				blocksToTransfer;
	UINT32				splitBlocks;
...
//	LurnIdeDiskResetProtocolCounters( IdeDisk );
	...
	if (doSplit) {
		...
		chooseDataLength = NdasFcChooseTransferSize( &IdeDisk->LanScsiSession->RecvNdasFcStatistics, 
													 (transferBlocks * IdeDisk->LuHwData.BlockBytes < readSplitBlockBytes) ?
													 (transferBlocks * IdeDisk->LuHwData.BlockBytes) : readSplitBlockBytes );
...
//	retryCount = 0;
	...
	while (transferedBlocks < transferBlocks) {
		...
		status = LspPacketRequest( LSS, &pduDesc, PduResponse, PduRegister );
