...
LANSCSIPROTOCOL_INTERFACE	LsProtoIdeV11Interface = {
		...
		LSIDEPROTO_VERSION_1_1,					// version 1.1
		0,
		0,
		0,
		0,
...
LANSCSIPROTOCOL_INTERFACE	LsProtoIdeV10Interface = {
		sizeof(LANSCSIPROTOCOL_INTERFACE),
		LSSTRUC_TYPE_PROTOCOL_INTERFACE,
		LSIDEPROTO_VERSION_1_0,				// version 1.0
		0,
		0,
		0,
		0,
	...
	)
{
	...
	if (OverlappedData) {
		...
		if (status != STATUS_PENDING) {
			...
			KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, 
...
IdeLspReadReply_V11(
	...
	if(recv != sizeof(LANSCSI_R2H_PDU_HEADER)) {
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Can't Recv Header...\n"));
	...
	// Read Header.
		...
		if(recv != ahsLen) {
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Can't Recv AHS...\n"));
	...
	if(dsLen) {
		...
		if(recv != dsLen) {
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Can't Recv Data segment...\n"));
	...
	)
{
	...
	if (OverlappedData) {
		...
		if (!NT_SUCCESS(ntStatus)) {
			...
			KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send Request 0x%x %d\n", ntStatus, result) );
		}
		...
		if (!NT_SUCCESS(ntStatus)) {
			...
			KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send Request 0x%x\n", ntStatus) );
	...
	)
{
	...
	if(PduResponse == NULL) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("pResult is NULL!!!\n"));
		return STATUS_INVALID_PARAMETER;
	}
	if(((PduDesc->Command == WIN_READ) || (PduDesc->Command == WIN_WRITE))
		&& (PduDesc->DataBuffer == NULL)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Buffer is NULL!!!\n"));
		return STATUS_INVALID_PARAMETER;
	}
	...
	RtlZeroMemory( LSS->PduBuffer, MAX_REQUEST_SIZE );
	
	pRequestHeader = (PLANSCSI_IDE_REQUEST_PDU_HEADER)LSS->PduBuffer;
	...
	case WIN_SETFEATURES:
		...
		break;
	default:
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Not Supported IDE Command(%08x).\n", (ULONG)PduDesc->Command));
	...
	if((PduDesc->Command == WIN_READ)
		...
		if(!(PduDesc->Flags & PDUDESC_FLAG_LBA)) {
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("LBA not supported. CHS not implemented.\n"));
			ASSERT(FALSE);
			return STATUS_INVALID_PARAMETER;
		}
	...
	if (!NT_SUCCESS(ntStatus)) {
		...
		KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send Request\n") );
	...
	if (sendData) {
		...
		chooseDataLength = NdasFcChooseTransferSize( &LSS->SendNdasFcStatistics, PduDesc->DataBufferLength );

		NDAS_ASSERT( chooseDataLength );
		...
		NdasFcUpdateTrasnferSize( &LSS->SendNdasFcStatistics, 
								  chooseDataLength, 
							      PduDesc->DataBufferLength,
							      startTime, 
							      endTime );
	...
	ntStatus = IdeLspReadReply_V11( LSS, LSS->PduBuffer, &pdu, NULL, PduDesc->TimeOut );

	if (!NT_SUCCESS(ntStatus)) {

		DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("Can't Read Reply\n") );
	...
	if((pReplyHeader->Opcode != IDE_RESPONSE)
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Reply Header. 0x%x\n", pReplyHeader->Flags));
		return ntStatus;
	}
#if DBG
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Failed. Response %d %d Command/StatReg: 0x%x ErrReg=0x%x\n", 
	...
	*PduResponse = pReplyHeader->Response;
	...
	if (*PduResponse == LANSCSI_RESPONSE_SUCCESS || SetStatics) {
	 ...
	 )
{
	...
	if(LSS->HWProtoVersion == LSIDEPROTO_VERSION_1_0) {
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Invalid LSPROTO_VERSION.\n"));
		ASSERT(FALSE);
	...
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send First Request\n"));
		LSS->LastLoginError = LSS_LOGIN_SEND_FAIL;
		return ntStatus;
	}

	// Read Reply.
	ntStatus = IdeLspReadReply_V11( LSS, LSS->PduBuffer, &pdu, NULL, TimeOut );

	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("First Can't Read Reply.\n"));
		NDAS_ASSERT(FALSE);
		return STATUS_PORT_DISCONNECTED;
	}
	...
	if((pReplyHeader->Opcode != LOGIN_RESPONSE)
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD First Reply Header.\n"));
		LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
		return STATUS_INVALID_PARAMETER;
	}
	
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("First Failed. RESPONSE:%x\n", (int)pReplyHeader->Response));
		if (pReplyHeader->Response == LANSCSI_RESPONSE_RI_VERSION_MISMATCH) {
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Tried to login with invalid version of LSprotocol.\n"));
	...
	if(LSS->HWProtoVersion == LSIDEPROTO_VERSION_1_0) {
		if((NTOHL(pReplyHeader->DataSegLen) < BIN_PARAM_SIZE_LOGIN_FIRST_REPLY)
			...
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD First Reply Data.\n"));
			LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
			return STATUS_INVALID_PARAMETER;
		}
	} else if(LSS->HWProtoVersion >= LSIDEPROTO_VERSION_1_1) {
	...
	if(pParamSecu->ParamType != BIN_PARAM_TYPE_SECURITY
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD First Reply Parameters.\n"));
		LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
		return STATUS_INVALID_PARAMETER;
	}
	...
	ntStatus = IdeLspSendRequest_V11(LSS, &pdu, NULL, 0, TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send Second Request\n"));
		LSS->LastLoginError = LSS_LOGIN_SEND_FAIL;
		return ntStatus;
	}
	...
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Second Can't Read Reply.\n"));
	...
	if((pReplyHeader->Opcode != LOGIN_RESPONSE)
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Second Reply Header.\n"));
		LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
		return STATUS_INVALID_PARAMETER;
	}
	
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Second Failed. RESPONSE:%x\n", (int)pReplyHeader->Response));
	...
	if(LSS->HWProtoVersion == LSIDEPROTO_VERSION_1_0) {
		if((NTOHL(pReplyHeader->DataSegLen) < BIN_PARAM_SIZE_LOGIN_SECOND_REPLY)
			...
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Second Reply Data.\n"));
			LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
			return STATUS_INVALID_PARAMETER;
		}	
	} else if(LSS->HWProtoVersion >= LSIDEPROTO_VERSION_1_1) {
	...
	if(pParamSecu->ParamType != BIN_PARAM_TYPE_SECURITY
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Second Reply Parameters.\n"));
		LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
		return STATUS_INVALID_PARAMETER;
	}
	...
	LSS->CHAP_C = NTOHL(pParamChap->V1.CHAP_C[0]);
	...
	Hash32To128(
		...
		(unsigned char*)pParamChap->V1.CHAP_R, 
	...
	ntStatus = IdeLspSendRequest_V11(LSS, &pdu, NULL, 0, TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send Third Request\n"));
		LSS->LastLoginError = LSS_LOGIN_SEND_FAIL;
		return ntStatus;
	}
	...
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Third Can't Read Reply.\n"));
		LSS->LastLoginError = LSS_LOGIN_RECV_FAIL;		
		NDAS_ASSERT(FALSE);
		return STATUS_PORT_DISCONNECTED;
	}
	...
	if((pReplyHeader->Opcode != LOGIN_RESPONSE)
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Third Reply Header.\n"));
		LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
		return STATUS_INVALID_PARAMETER;
	}
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Third Failed. RESPONSE:%x\n", (int)pReplyHeader->Response));
		return STATUS_LOGON_FAILURE;
	}
	...
	if(LSS->HWProtoVersion == LSIDEPROTO_VERSION_1_0) {
		if((NTOHL(pReplyHeader->DataSegLen) < BIN_PARAM_SIZE_LOGIN_THIRD_REPLY)
			...
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Third Reply Data.\n"));
			LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
			return STATUS_INVALID_PARAMETER;
		}	
	} else if(LSS->HWProtoVersion >= LSIDEPROTO_VERSION_1_1) {
	...
	if(pParamSecu->ParamType != BIN_PARAM_TYPE_SECURITY
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Third Reply Parameters.\n"));
		LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
		return STATUS_INVALID_PARAMETER;
	}
	...
	ntStatus = IdeLspSendRequest_V11(LSS, &pdu, NULL, 0, TimeOut);
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send Fourth Request\n"));
		LSS->LastLoginError = LSS_LOGIN_SEND_FAIL;
		return ntStatus;
	}
	...
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Fourth Can't Read Reply.\n"));
		LSS->LastLoginError = LSS_LOGIN_RECV_FAIL;
		NDAS_ASSERT(FALSE);
		return STATUS_PORT_DISCONNECTED;
	}
	...
	if((pReplyHeader->Opcode != LOGIN_RESPONSE)
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Fourth Reply Header.\n"));
		LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
		return STATUS_INVALID_PARAMETER;
	}
	
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Fourth Failed. RESPONSE:%x\n", (int)pReplyHeader->Response));
	...
	if(LSS->HWProtoVersion == LSIDEPROTO_VERSION_1_0) {
		if((NTOHL(pReplyHeader->DataSegLen) < BIN_PARAM_SIZE_LOGIN_FOURTH_REPLY)
			...
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Fourth Reply Data.\n"));
			LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
			return STATUS_INVALID_PARAMETER;
		}	
	} else if(LSS->HWProtoVersion >= LSIDEPROTO_VERSION_1_1) {
	...
	if(pParamNego->ParamType != BIN_PARAM_TYPE_NEGOTIATION) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Fourth Reply Parameters.\n"));
		LSS->LastLoginError = LSS_LOGIN_BAD_REPLY;
		return STATUS_INVALID_PARAMETER;
	}
	...
	)
{
	...
	if(!NT_SUCCESS(ntStatus)) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Can't Read Reply.\n"));
		NDAS_ASSERT(FALSE);
		return STATUS_PORT_DISCONNECTED;
	}
	...
	if((pReplyHeader->Opcode != LOGOUT_RESPONSE)
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Reply Header.\n"));
		return ntStatus;
	}
	
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Failed. RESPONSE:%x\n", (int)pReplyHeader->Response));
		return ntStatus;
	}
	...
	)
{
	...
	pRequestHeader = (PLANSCSI_TEXT_REQUEST_PDU_HEADER)LSS->PduBuffer;
	...
	pParam = (PBIN_PARAM_TARGET_LIST)&LSS->PduBuffer[sizeof(LANSCSI_H2R_PDU_HEADER)];
	...
	status = IdeLspReadReply_V11( LSS, LSS->PduBuffer, &pdu, NULL, TimeOut );
	...
	if(LSS->HWProtoVersion == LSIDEPROTO_VERSION_1_0) {
		if(pReplyHeader->DataSegLen < BIN_PARAM_SIZE_REPLY) {
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("No Data Segment.\n"));
			return STATUS_UNSUCCESSFUL;
		}
	}
	...
	if(pParam->ParamType != BIN_PARAM_TYPE_TARGET_LIST) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Bad Parameter Type.\n"));
		return STATUS_UNSUCCESSFUL;
	}
	...
	for(i = 0; i < pParam->NRTarget; i++) {
		...
		TargetList->TargetInfoEntry[i].NRROHost = pTarget->V1.NRROHost;
		TargetList->TargetInfoEntry[i].NRRWHost = pTarget->V1.NRRWHost;
	...
	)
{
	...
	pParam = (PBIN_PARAM_TARGET_DATA)&LSS->PduBuffer[sizeof(LANSCSI_H2R_PDU_HEADER)];
	...
	)
{
	...
	LANSCSI_PDU_POINTERS				pdu0 = {{0}, 0};

#if 0
	LANSCSI_IDE_REQUEST_PDU_HEADER_V1	pRequestHeader2 = {0};
	LANSCSI_PDU_POINTERS				pdu2 = {{0}, 0};
#endif
	...
	case WIN_READ:
		{
			...
			if(PduDesc->Flags & PDUDESC_FLAG_DMA) {
				...
				if(!(PduDesc->Flags & PDUDESC_FLAG_PIO)) {
					KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("WIN_READ: I/O Mode is not determined. 0x%x\n", PduDesc->Flags));
					return STATUS_INVALID_PARAMETER;
				}
	...
	case WIN_WRITE:
		{
			...
			if(PduDesc->Flags & PDUDESC_FLAG_DMA) {
				...
				if(!(PduDesc->Flags & PDUDESC_FLAG_PIO)) {
					KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("WIN_WRITE: I/O Mode is not determined. 0x%x\n", PduDesc->Flags));
					return STATUS_INVALID_PARAMETER;
				}
	...
	case WIN_SMART:
		{
			if(PduDesc->Flags & PDUDESC_FLAG_DATA_IN) {
				if(!(PduDesc->Flags & PDUDESC_FLAG_PIO)) {
					KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("WIN_SMART: I/O Mode is not determined. 0x%x\n", PduDesc->Flags));
					return STATUS_INVALID_PARAMETER;
				}
				pRequestHeader->R = 1;
				recvData = TRUE;
	...
	if((PduDesc->Command == WIN_READ)
		...
		if(!(PduDesc->Flags & PDUDESC_FLAG_LBA)) {
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("CHS not supported...\n"));
			return STATUS_INVALID_PARAMETER;
		}
#if DBG
		if(PduDesc->Flags & PDUDESC_FLAG_PIO)
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("PIO %d\n", (int)pRequestHeader->Command));
#endif
		pRequestHeader->LBA = 1;
	...
	if (LSS2) {
		...
		pduBuffer = LSS->PduBuffer;
		RtlZeroMemory( pduBuffer, sizeof(LSS->PduBuffer) );
	...
	ntStatus = IdeLspReadReply_V11( LSS, LSS->PduBuffer, &pdu0, NULL, PduDesc->TimeOut );
	...
	if (pReplyHeader->Opcode != IDE_RESPONSE) {
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Reply's Opcode BAD. Flag: 0x%x, Req. Command: 0x%x Rep. Command: 0x%x\n", 
	...
	if (pReplyHeader->F == 0) {
		...
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Reply's F option BAD. Flag: 0x%x, Req. Command: 0x%x Rep. Command: 0x%x\n", 
			pReplyHeader->Flags, iCommandReg, pReplyHeader->Command));
		return STATUS_UNSUCCESSFUL;
	}
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Failed. Response 0x%x 0x%x %d %d Command:0x%x Command/StatReg: 0x%x ErrReg=0x%x\n", 
...
#if __NDAS_BAD_SECTOR_TEST__

	if (PduDesc->Command == WIN_READ || PduDesc->Command == WIN_WRITE || PduDesc->Command == WIN_VERIFY) {

		UCHAR nodeAddress[6] = {0x00, 0x0B, 0xD0, 0x03, 0x2D, 0xE5};

		if (RtlEqualMemory(nodeAddress, LSS->NdasNodeAddress.Address[0].NdasAddress.Lpx.Node, 6)) {

			LSS->IoCount++;
			
			if (LSS->IoCount > 10) {

				if (LSS->IoCount % 10000 == 0 || LSS->IoCount % 10000 == 1 || LSS->IoCount % 10000 == 2 ||
					LSS->IoCount % 10000 == 3 || LSS->IoCount % 10000 == 4 || LSS->IoCount % 10000 == 5) {

					pReplyHeader->Response	   = LANSCSI_RESPONSE_T_COMMAND_FAILED;
					pReplyHeader->Feature_Curr = ECC_ERR;

					DbgPrint("set Bad Sector, PduDesc->DestAddr = %I64x, PduDesc->BlockCount =%d\n",
						     PduDesc->DestAddr, PduDesc->BlockCount );
				}
			}
		}
	}
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("pReplyHeader->Response = %x\n", pReplyHeader->Response) ); 

	*PduResponse0 = pReplyHeader->Response;

	if (*PduResponse0 == LANSCSI_RESPONSE_SUCCESS || SetStatics) {

		// Query transport statistics

		// Register return values
		//
		// Only error bit is valid in status register value, but it ORs command code.
		...
		if (PduDesc->Flags & PDUDESC_FLAG_LBA48) {
			...
			PduDesc->DestAddr = ((UINT64)pReplyHeader->LBAHigh_Prev << 40) +
								((UINT64)pReplyHeader->LBAMid_Prev  << 32) +
								((UINT64)pReplyHeader->LBALow_Prev  << 24) +
								((UINT64)pReplyHeader->LBAHigh_Curr << 16) +
								((UINT64)pReplyHeader->LBAMid_Curr  << 8)  +
			...
			PduDesc->BlockCount = ((UINT32)pReplyHeader->SectorCount_Prev << 8) + 
								  ((UINT32)pReplyHeader->SectorCount_Curr);
			...
			PduDesc->DestAddr = ((UINT64)pReplyHeader->Device       << 24) +
								((UINT64)pReplyHeader->LBAHigh_Curr << 16) +
								((UINT64)pReplyHeader->LBAMid_Curr  << 8)  +
								((UINT64)pReplyHeader->LBALow_Curr);
		}
	...
	if (LSS2) {
		...
		if (!NT_SUCCESS(ntStatus)) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("Error when Receive data\n") );
		...
		if (LSS->DataEncryptAlgo != 0) {
			...
			Decrypt32SP( PduDesc->DataBuffer,
						 PduDesc->DataBufferLength,
						 (unsigned char*)&(LSS->DecryptIR[0]) );
		}
		...
		if (pReplyHeader->Opcode != IDE_RESPONSE) {
			...
			KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, 
					  ("Reply's Opcode BAD. Flag: 0x%x, Req. Command: 0x%x Rep. Command: 0x%x\n", 
					   pReplyHeader->Flags, iCommandReg, pReplyHeader->Command) );
		...
		if (pReplyHeader->F == 0) {
			...
			KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, 
					  ("Reply's F option BAD. Flag: 0x%x, Req. Command: 0x%x Rep. Command: 0x%x\n", 
					   pReplyHeader->Flags, iCommandReg, pReplyHeader->Command) );
		...
		if (PduDesc->Command == WIN_CHECKPOWERMODE1){
			...
			KDPrintM( DBG_PROTO_TRACE, 
					  ("Check Power mode = 0x%02x\n", (unsigned char)(pReplyHeader->SectorCount_Curr)) );
		}

		if (pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS) {

			KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, 
					  ("Failed. Response 0x%x 0x%x %d %d Command:0x%x Command/StatReg: 0x%x ErrReg=0x%x\n", 
						pReplyHeader->Response,
						pReplyHeader->Status,
						NTOHL(pReplyHeader->DataTransferLength),
						NTOHL(pReplyHeader->DataSegLen),
		...
		*PduResponse2 = pReplyHeader->Response;
		...
		if (*PduResponse0 == LANSCSI_RESPONSE_SUCCESS && (*PduResponse2 == LANSCSI_RESPONSE_SUCCESS || SetStatics)) {
	...
	)
{
	...
	case SCSIOP_SEND_CUE_SHEET: {
		...
		if ((PduDesc->DVD_TYPE == IO_DATA) || (PduDesc->DVD_TYPE == IO_DATA_9573)) {
	...
	)
{
	...
	if (PduDesc->PKCMD == NULL) {
		...
		KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, ("PKCMD is NULL!!!\n") );
		return STATUS_INVALID_PARAMETER;
	}

	RtlZeroMemory( LSS->PduBuffer, MAX_REQUEST_SIZE );

	pRequestHeader = (PLANSCSI_PACKET_REQUEST_PDU_HEADER)LSS->PduBuffer;
...
//
	...
	pRequestHeader->LBAMid_Curr = (INT8)((len | 0x1) & 0xff); // Low byte count limit
	pRequestHeader->LBAHigh_Curr = (INT8)((len >> 8) & 0xff); // High byte count limit
	...
	pRequestHeader->LBAMid_Curr = (INT8)((len) & 0xff); // Low byte count limit
	pRequestHeader->LBAHigh_Curr = (INT8)((len >> 8) & 0xff); // High byte count limit
	...
	NDAS_ASSERT( sizeof(PduDesc->PKCMD) == 12 );

	RtlCopyMemory( pRequestHeader->PKCMD, PduDesc->PKCMD, sizeof(PduDesc->PKCMD) );
	...
	case SCSIOP_WRITE12: {
		...
		if (!NT_SUCCESS(ntStatus)) {
			...
			KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send Request  0x%02x\n", com));
			return ntStatus;
		}
		...
		if (len > 0) {
			...
			if (pData == NULL) {
				...
				KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, ("No buffer for send data  0x%02x\n", com) );
				return STATUS_UNSUCCESSFUL;
			}
			...
			if (!NT_SUCCESS(ntStatus) || result != alignedBufLen) {

				KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("Error when Send data for WRITE  Com 0x%02x",com));
				return ntStatus;
			}
		}
	...
	default:
		{
			...
			if(len > 0)
			{
				...
				if(pData == NULL)
				{
					KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("No buffer for Receive data  0x%02x\n", com));
					return STATUS_UNSUCCESSFUL;
				}
				...
				if (ntStatus != STATUS_SUCCESS || result != alignedBufLen) {
					...
					KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, ("Can't Read Data. Com 0x%02x alignedBufLen = %d\n", com, alignedBufLen) );
	...
	if (!NT_SUCCESS(ntStatus)) {
		...
		KDPrintM( NDASSCSI_DBG_LURN_IDE_ERROR, ("Can't Read Reply. Com 0x%02x alignedBufLen = %d\n", com, alignedBufLen) );
	...
	if( (pReplyHeader->Opcode != IDE_RESPONSE)
		|| (pReplyHeader->F == 0)) 
	{
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Reply Header pReplyHeader->Opcode != IDE_RESPONSE . Flag: 0x%x, Command: 0x%x\n", 
	...
	if(pReplyHeader->Response != LANSCSI_RESPONSE_SUCCESS)
	{
		...
		//
		PduRegister[1] = pReplyHeader->Command & (~PduDesc->Command); //	status
	}else{
...
IdeLspVendorRequest(
	...
	pRequestHeader = (PLANSCSI_VENDOR_REQUEST_PDU_HEADER)LSS->PduBuffer;
	...
	ntStatus = LspReadReply( LSS, LSS->PduBuffer, &pdu, NULL, PduDesc->TimeOut );
	...
	if(pReplyHeader->Opcode != VENDOR_SPECIFIC_RESPONSE){		
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Reply Header pReplyHeader->Opcode != VENDOR_SPECIFIC_RESPONSE . Flag: 0x%x, Req. VendorOp: 0x%x Rep. VendorOp: 0x%x\n", 
			pReplyHeader->Flags, iCommandReg, pReplyHeader->VendorOp));
		return STATUS_UNSUCCESSFUL;
	}
	if(pReplyHeader->F == 0){		
		KDPrintM(NDASSCSI_DBG_LURN_IDE_ERROR, ("BAD Reply Header pReplyHeader->F == 0 . Flag: 0x%x, Req. VendorOp: 0x%x Rep. VendorOp: 0x%x\n", 
			pReplyHeader->Flags, iCommandReg, pReplyHeader->VendorOp));
		return STATUS_UNSUCCESSFUL;
	}
