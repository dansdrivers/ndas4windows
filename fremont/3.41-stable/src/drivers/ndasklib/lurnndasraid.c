	...
	) 
{
	...
	PsGetVersion( &NdasrGlobals->OsMajorVersion,
				  &NdasrGlobals->OsMinorVersion,
				  NULL,
				  NULL );
	...
	NDAS_ASSERT( NdasrGlobals->TdiClientBindingHandle );
	...
	) 
{
	...
	NDAS_ASSERT( FlagOn(NdasrGlobals->Flags, NDASR_GLOBALS_FLAG_INITIALIZE) );

	refCnt = InterlockedDecrement(&NdasrGlobals->ReferenceCount);
	
	NDAS_ASSERT( refCnt >= 0 );
	...
	)
{
	...
	case TDI_PNP_OP_ADD:
		...
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("TDI_PNP_OP_ADD\n") );
		break;
	
	case TDI_PNP_OP_DEL:
	
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("TDI_PNP_OP_DEL\n") );
		break;
	
	case TDI_PNP_OP_PROVIDERREADY:
		
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("TDI_PNP_OP_PROVIDERREADY\n") );
		break;
	
	case TDI_PNP_OP_NETREADY:
		
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("TDI_PNP_OP_NETREADY\n") );
		break;
...
--*/
{
	...
	if (RtlPrefixUnicodeString(&lpxPrefix, DeviceName, TRUE) &&
		...
		KeAcquireSpinLock( &NdasrGlobalData.IdeDiskQSpinlock, &oldIrql );
		...
		KeReleaseSpinLock( &NdasrGlobalData.IdeDiskQSpinlock, oldIrql );
	...
	)
{
	...
	switch (PowerEvent->NetEvent) {
		...
		case NetEventSetPower:
			...
			DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("SetPower\n") );
	...
	) 
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("DRAID flush all\n") );
	
	// Flush request to all client 
	
	KeAcquireSpinLock( &NdasrGlobals->ClientQSpinlock, &oldIrql );
	...
	KeReleaseSpinLock( &NdasrGlobals->ClientQSpinlock, oldIrql );
	...
	) 
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Allocating event array to count %d\n", newCount) );
	...
	) 
{
	...
	KeAcquireSpinLock( &NdasrGlobals->IdeDiskQSpinlock, &oldIrql );
	RemoveEntryList( &IdeDisk->AllListEntry );
	KeReleaseSpinLock( &NdasrGlobals->IdeDiskQSpinlock, oldIrql );
	...
	) 
{
	...
	KeAcquireSpinLock( &NdasrGlobals->ArbitratorQSpinlock, &oldIrql );
	RemoveEntryList( &NdasrArbitrator->AllListEntry );
	KeReleaseSpinLock( &NdasrGlobals->ArbitratorQSpinlock, oldIrql );
	...
	) 
{
	...
	for (i = 0; i < socketLpxAddressList.iAddressCount; i++) {
		...
		if (status != STATUS_PENDING) {
			...
			KeAcquireSpinLock( &NdasrGlobals->ListenContextSpinlock, &oldIrql );					
			RemoveEntryList( &listenContext->Link );
			KeReleaseSpinLock( &NdasrGlobals->ListenContextSpinlock, oldIrql );					
		...
		for (i = 2; i < eventCount; i++) {
			...
			if (KeReadStateEvent(events[i])) {
				...
				if (listenContext->ListenOverlapped.Request[0].IoStatusBlock.Status == STATUS_SUCCESS) {
					...
					NDAS_ASSERT( NDAS_ASSERT_NETWORK_FAIL ); // May be this can be happen if multiple instances of this function is called with same address
	...
	) 
{
	...
	for (listEntry = NdasrGlobals->ListenContextList.Flink;
		...
		if (!listenContext->Destroy && 
			...
			DebugTrace(NDASSCSI_DBG_LURN_IDE_INFO, ("New LPX address already exist.Ignoring.\n"));
			alreadyExist = TRUE;
			break;
		}
	}
	...
	listenContext->Addr.Port = HTONS(NRMX_ARBITRRATOR_LPX_PORT);
	...
	) 
{
	...
	KeAcquireSpinLock( &NdasrGlobalData.ListenContextSpinlock, &oldIrql );
	...
	for (listEntry = NdasrGlobals->ListenContextList.Flink;
		...
		if (RtlCompareMemory(listenContext->Addr.Node, Addr->Node, 6) == 6) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Found matching address\n") );
	...
	) 
{
	...
	if (ListenContext->ListenFileHandle) {
		...
		NDAS_ASSERT( ListenContext->ListenFileHandle );
		NDAS_ASSERT( ListenContext->ListenFileObject );
		...
		NDAS_ASSERT( ListenContext->ListenFileHandle == NULL );
		NDAS_ASSERT( ListenContext->ListenFileObject == NULL );
	}
	...
	) 
{
	...
	if (status != STATUS_PENDING) {
		...
		NDAS_ASSERT(FALSE); // May be this can be happen if multiple instances of this function is called with same address
	...
	) 
{
	...
	do {
		...
		if (NTOHL(registerMsg.Header.Signature) != NRMX_SIGNATURE	||
			...
			NTOHS(registerMsg.Header.Length2) !=  sizeof(NRMX_REGISTER)) {
		...
		KeAcquireSpinLock( &NdasrGlobalData.ArbitratorQSpinlock, &oldIrql );
		...
		KeReleaseSpinLock( &NdasrGlobalData.ArbitratorQSpinlock, oldIrql );
	...
	if (status != STATUS_SUCCESS) {
		...
		if (Connection) {
			...
			registerReply.Sequence2	= registerMsg.Header.Sequence2;
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Exiting reception thread.\n") );
	...
	)
{
	...
	try {
		...
		for (nidx = 0; nidx < Lurn->LurnChildrenCnt; nidx++) {  // idx_client is index of role
			...
			status = LsCcbInitializeByCcb( Ccb, nextCcb[nidx] );
			...
			NDAS_ASSERT( Ccb->DataBuffer == nextCcb[nidx]->DataBuffer );
	...
	)
{
	...
	NDAS_ASSERT( ChildCcb->OperationCode == CCB_OPCODE_STOP );
	...
	)
{
	...
	UCHAR				upToDateNode;
	BOOLEAN				nodeIsUptoDate[NRMX_MAX_MEMBER_DISK] = {FALSE};
	...
	for (nidx =  0; nidx < Lurn->LurnChildrenCnt; nidx++) { // i is node flag
		...
		if (status != STATUS_SUCCESS) {
			...
			NDAS_ASSERT( !FlagOn(Lurn->NdasrInfo->LocalNodeFlags[nidx], NRMX_NODE_FLAG_RUNNING) ||
						 FlagOn(Lurn->NdasrInfo->LocalNodeFlags[nidx], NRMX_NODE_FLAG_DEFECTIVE) );
	...
	if (NodeIsUptoDate) {
		...
		RtlCopyMemory( NodeIsUptoDate, nodeIsUptoDate, sizeof(BOOLEAN)*NRMX_MAX_MEMBER_DISK );
		*UpToDateNode = upToDateNode;
	}
	...
	)
{
	...
	case LURNLOCK_OPCODE_ACQUIRE: {
		...
		for (;;) {
			...
			NDAS_ASSERT( ChildLurn->NdasLocked[LockId] == FALSE );
			...
			DebugTrace( NDASSCSI_DBG_LURN_NDASR_NOISE, ("LurnRequest status : %08x\n", status) );
			...
			if (ccb.CcbStatus == CCB_STATUS_SUCCESS) {
				...
				NDAS_ASSERT( ChildLurn->NdasLocked[LockId] == TRUE );
				break;
			};
			...
			if (ccb.CcbStatus == CCB_STATUS_LOCK_NOT_GRANTED) {
				...
				if (NdasCurrentTime().QuadPart - startTime.QuadPart > NANO100_PER_SEC) {
					...
					NdasRaidLurnLockSynchrously( ChildLurn, LURNLOCK_OPCODE_CLEANUP, LockId, TRUE );
			...
			NDAS_ASSERT( ccb.CcbStatus == CCB_STATUS_STOP || ccb.CcbStatus == CCB_STATUS_NOT_EXIST );
...
retry:
	...
	if (ForceUnitAccess) {
		...
		} else if (ccb.SrbCdb.CDB16.OperationCode == SCSIOP_WRITE16) {

			ccb.SrbCdb.CDB16.ForceUnitAccess = TRUE;
		}
	}
	...
	) 
{
	...
	NDAS_ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
	NDAS_ASSERT( Lurn->NdasrInfo == NULL );
	...
	do {
		...
		case LURN_NDAS_RAID4:
			...
			NDAS_ASSERT( FlagOn(Lurn->Lur->SupportedNdasFeatures, NDASFEATURE_SIMULTANEOUS_WRITE) );
		...
		if (ndasrInfo->ParityDiskCount == 0) {
			...
			ndasrInfo->ActiveDiskCount	= (UCHAR)(Lurn->LurnChildrenCnt);
		...
		NDAS_ASSERT( ndasrInfo->MaxDataSendLength == ndasrInfo->MaxDataRecvLength );
		
		if (ndasrInfo->Striping) {

			NDAS_ASSERT( ndasrInfo->MaxDataSendLength / Lurn->ChildBlockBytes % (ndasrInfo->ActiveDiskCount - ndasrInfo->ParityDiskCount) == 0 );
			NDAS_ASSERT( ndasrInfo->MaxDataRecvLength / Lurn->ChildBlockBytes % (ndasrInfo->ActiveDiskCount - ndasrInfo->ParityDiskCount) == 0 );
			
			NDAS_ASSERT( (ndasrInfo->ActiveDiskCount - ndasrInfo->ParityDiskCount) == 1 ||
		...
		NDAS_ASSERT( ndasrInfo->SpareDiskCount == 0 ||  ndasrInfo->SpareDiskCount == 1 );
			...
			for (ridx = 0; ridx < Lurn->LurnChildrenCnt; ridx++) { // i : role index. 
				...
				if (FlagOn(unitDeviceStatus, NDAS_UNIT_META_BIND_STATUS_DEFECTIVE)) {
					...
					if (Lurn->Lur->EmergencyMode == FALSE) {
						...
						NDAS_ASSERT( FlagOn(ndasrInfo->LocalNodeFlags[(UCHAR)ndasrInfo->Rmd.UnitMetaData[ridx].iUnitDeviceIdx], 
							NRMX_NODE_FLAG_STOP) );
					}
	...
	) 
{
	...
	if (Lurn->NdasrInfo == NULL) {
		...
		NDAS_ASSERT( Lurn->LurnStatus == LURN_STATUS_INIT );
		return;
	}
	...
	)
{
	...
	if (Ccb->OperationCode == CCB_OPCODE_SHUTDOWN) {
		...
		SetFlag( Lurn->LurnStatus, LURN_STATUS_SHUTDOWN );
	}
	...
	if (!LURN_IS_RUNNING(Lurn->LurnStatus) ||
		Lurn->NdasrInfo->NdasrClient->NdasrState == NRMX_RAID_STATE_TERMINATED) {
	...
	KeAcquireSpinLockAtDpcLevel( &Lurn->NdasrInfo->NdasrClient->SpinLock );
	...
	KeReleaseSpinLockFromDpcLevel( &Lurn->NdasrInfo->NdasrClient->SpinLock );
	...
	) 
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, ("Shutdowning DRAID\n") );
