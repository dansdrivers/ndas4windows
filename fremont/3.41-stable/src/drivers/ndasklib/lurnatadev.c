	...
	)
{
	...
	do {
		...
		IdeDisk->MaxStallInterval.QuadPart  = Lurn->ReconnInterval * Lurn->ReconnTrial;
		...
		if (IdeDisk->LuHwData.HwVersion == LANSCSIIDE_VERSION_2_0 &&
			...
			if (!IdeDisk->WriteCheckBuffer) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Error! Could not allocate the write-check buffer!\n"));
			}
		...
		NDAS_ASSERT( loginInfo.MaxDataTransferLength == (128*1024) || 
					 loginInfo.MaxDataTransferLength == (64*1024)  || 
					 loginInfo.MaxDataTransferLength == (32*1024) );
		...
		status = LurnIdeConfigure( Lurn, &response );
		...
		NDAS_ASSERT( IdeDisk->MaxDataSendLength > 0 && IdeDisk->MaxDataSendLength <= 128*1024 );
		NDAS_ASSERT( IdeDisk->MaxDataRecvLength > 0 && IdeDisk->MaxDataRecvLength <= 128*1024 );
	...
	)
{
	...
	KeAcquireSpinLock( &Lurn->SpinLock, &oldIrql );
	...
	KeReleaseSpinLock( &Lurn->SpinLock, oldIrql );
	...
	)
{
	...
	if (Ccb->OperationCode == CCB_OPCODE_SHUTDOWN) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, 
					("CCB_OPCODE_SHUTDOWN, Ccb->Srb = %08x\n",
					 Ccb->Srb) );
	...
	)
{
	...
	case LURNLOCK_OPCODE_ACQUIRE: {
		...
		LARGE_INTEGER	startTime = NdasCurrentTime();

		for (;;) {

			if (Lurn->LurnStatus != LURN_STATUS_STALL) {

				NDAS_ASSERT( Lurn->NdasLocked[LockId] == FALSE );
			}

			NDAS_ASSERT( Lurn->IdeDisk->LuHwData.LanscsiTargetID < 2 );

			status = LspAcquireLock( Lurn->IdeDisk->LanScsiSession, 
									 LockId + Lurn->IdeDisk->LuHwData.LanscsiTargetID, 
									 LockData, 
									 Timeout );

			if (status == STATUS_SUCCESS) {

				Lurn->NdasLocked[LockId] = TRUE;
				break;
			}
			...
			if (status == STATUS_LOCK_NOT_GRANTED) {
				...
				if ((NdasCurrentTime().QuadPart - startTime.QuadPart) > NANO100_PER_SEC) {

					LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_CLEANUP, LockId, NULL, NULL, TRUE );
	...
	case LURNLOCK_OPCODE_RELEASE: {
		...
		NDAS_ASSERT( Lurn->NdasLocked[LockId] == TRUE );

		status = LspReleaseLock( Lurn->IdeDisk->LanScsiSession, 
								 LockId + Lurn->IdeDisk->LuHwData.LanscsiTargetID, 
								 LockData, 
								 Timeout );
		...
		if (status == STATUS_SUCCESS) {
			...
			Lurn->NdasLocked[LockId] = FALSE;
		}

		break;
	}

	case LURNLOCK_OPCODE_CLEANUP: {

		status = LspWorkaroundCleanupLock( Lurn->IdeDisk->LanScsiSession, 
										   LockId + Lurn->IdeDisk->LuHwData.LanscsiTargetID, 
										   Timeout );
	...
	if (status != STATUS_SUCCESS && status != STATUS_LOCK_NOT_GRANTED) {
	...
	) 
{
	...
	case CCB_OPCODE_FLUSH:
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("CCB_OPCODE_FLUSH, Ccb->Srb = %08x\n", Ccb->Srb) );
	...
	case CCB_OPCODE_UPDATE: {
		...
		NDAS_ASSERT( lurnUpdate->TryCount == 1 || lurnUpdate->TryCount == NDAS_RAID_UPDATE_COUNT );
		
		loop = lurnUpdate->TryCount;
	...
	case CCB_OPCODE_DEVLOCK: {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("CCB_OPCODE_DEVLOCK\n") );

		devLockControl = (PLURN_DEVLOCK_CONTROL)Ccb->DataBuffer;

		lockId = devLockControl->LockId;

		if (lockId == NDAS_USER_LOCK) { // Implement later
		...
		if (LURN_IS_ROOT_NODE(Lurn)) {
			...
			if (lockId != NDAS_RW_LOCK && lockId != NDAS_CONTENT_LOCK) {
		...
		case LURNLOCK_OPCODE_ACQUIRE: 
			...
			status = LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_ACQUIRE, lockId, devLockControl->LockData, &timeout, FALSE );
			...
			if (status == STATUS_LOCK_NOT_GRANTED) {
				...
				Ccb->CcbStatus = CCB_STATUS_LOCK_NOT_GRANTED;
				status = STATUS_SUCCESS;
			}
		...
		case LURNLOCK_OPCODE_RELEASE: 
			...
			status = LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_RELEASE, lockId, devLockControl->LockData, &timeout, FALSE );
		...
		case LURNLOCK_OPCODE_CLEANUP: 
			...
			status = LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_CLEANUP, lockId, devLockControl->LockData, &timeout, FALSE );
	...
	) 
{
	...
	LARGE_INTEGER	lockingStartTime = {0};
	BOOLEAN			lockFailCount;
	...
	while (!IsListEmpty(&IdeDisk->CcbQueue)) {
		...
		if (Lurn->LurnStatus == LURN_STATUS_STOP_PENDING) {
			...
			LsCcbSetStatusFlag( ccb, CCBSTATUS_FLAG_LURN_STOP | CCBSTATUS_FLAG_BUSCHANGE );
		...
		if (LsCcbIsFlagOn(ccb, CCB_FLAG_ACQUIRE_BUFLOCK)) {
			...
			do {
				...
				if (&Lurn->IdeDisk->LanScsiSession->RecvNdasFcStatistics.MinimumIdx == 0) {

					NDAS_ASSERT( ccb->OperationCode == CCB_OPCODE_EXECUTE );

					if (ccb->SrbCdb.AsByte[0] == 0x3E || ccb->SrbCdb.AsByte[0] == SCSIOP_READ || ccb->SrbCdb.AsByte[0] == SCSIOP_READ16) {

						DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("not locking\n") );
						break;
					}
				}

				if (Lurn->NdasLocked[NDAS_RW_LOCK] == TRUE) {

					break;
				}

				lockingStartTime = NdasCurrentTime();

				if (Lurn->CascadeLock) {

					NDAS_ASSERT( Lurn->NdasLocked[NDAS_CONTENT_LOCK] == FALSE );

					status = LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_ACQUIRE, NDAS_CONTENT_LOCK, NULL, NULL, TRUE );
					...
					NDAS_ASSERT( Lurn->NdasLocked[NDAS_CONTENT_LOCK] == TRUE );
				}
				...
				for (;;) {
					...
					status = LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_ACQUIRE, NDAS_RW_LOCK, NULL, NULL, FALSE );
				
					if (status == STATUS_SUCCESS) {

						NDAS_ASSERT( Lurn->NdasLocked[NDAS_RW_LOCK] == TRUE );
						break;
					}
					...
					if (status == STATUS_LOCK_NOT_GRANTED) {
						...
						if (lockFailCount == 0) {
							...
							delay.QuadPart = 
								NDAS_MAX_LOCKING_DELAY <= Lurn->LockingDelay.QuadPart ?
									-NDAS_MAX_LOCKING_DELAY : -Lurn->LockingDelay.QuadPart;
					...
					NDAS_ASSERT( Lurn->NdasLocked[NDAS_RW_LOCK] == FALSE );
					NDAS_ASSERT( status != STATUS_SUCCESS && status != STATUS_LOCK_NOT_GRANTED );
				...
				if (status == STATUS_SUCCESS) {
					...
					NDAS_ASSERT( Lurn->LockingTime.QuadPart == 0 );
					Lurn->LockingTime = NdasCurrentTime();
				}
				...
				if (Lurn->CascadeLock) {
					...
					if (status == STATUS_SUCCESS) {
						...
						status = LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_RELEASE, NDAS_CONTENT_LOCK, NULL, NULL, TRUE );						
					}
					
					if (status != STATUS_SUCCESS) {

						Lurn->NdasLocked[NDAS_CONTENT_LOCK] = FALSE;
					}
				}
		...
		if (status == STATUS_SUCCESS) {
			...
			if (ccb->OperationCode != CCB_OPCODE_DEVLOCK && ccb->OperationCode != CCB_OPCODE_SMART) {
	...
	if (LURN_IS_ROOT_NODE(Lurn)) {
		...
		if (status == STATUS_SUCCESS) {
			...
			if (Lurn->LockingTime.QuadPart) {

				if (lockFailCount == 0) {

					Lurn->CascadeLock = FALSE;

				} else {

					Lurn->CascadeLock = TRUE;
				}

				Lurn->LockingDelay.QuadPart += (NdasCurrentTime().QuadPart - Lurn->LockingTime.QuadPart);
				Lurn->LockingDelay.QuadPart >>= 1;
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, 
							("Lurn->LockingDelay.QuadPart = %I64d\n", Lurn->LockingDelay.QuadPart) );
			}
		}
	}
	...
	)
{
	...
	BOOLEAN				requireNotifyReconnect = FALSE;
	...
	do {
		...
		if (Lurn->LurnStatus != LURN_STATUS_RUNNING || IsListEmpty(&IdeDisk->CcbQueue)) {
			...
			if (Lurn->LockingTime.QuadPart) {
				...
				NDAS_ASSERT( LURN_IS_ROOT_NODE(Lurn) );
				NDAS_ASSERT( Lurn->NdasLocked[NDAS_RW_LOCK] == TRUE );
			...
			if (!NT_SUCCESS(status)) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("terminateThread\n") );
			...
			if (status == STATUS_TIMEOUT) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("LurCallBack\n") );
			}
#endif
		}
		...
		if (Lurn->LurnStatus == LURN_STATUS_RUNNING) {
			...
			if (LURN_IS_ROOT_NODE(Lurn)) {
				...
				if (IsListEmpty(&IdeDisk->CcbQueue)) {
					...
					if (Lurn->NdasLocked[NDAS_RW_LOCK] == TRUE) {
						...
						NDAS_ASSERT( Lurn->LockingTime.QuadPart );

						DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("release lock\n") );

						status = LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_RELEASE, NDAS_RW_LOCK, NULL, NULL, TRUE );

						Lurn->LockingTime.QuadPart = 0;

						if (status != STATUS_SUCCESS) {

							Lurn->NdasLocked[NDAS_RW_LOCK] = FALSE;
							break;
						}
			...
			if (NT_SUCCESS(status)) {
				...
				if (Lurn->LockingTime.QuadPart && 
					(NdasCurrentTime().QuadPart - Lurn->LockingTime.QuadPart) >= NDAS_MAX_LOCKING_DURATION) {
			...
			if (ccb->OperationCode == CCB_OPCODE_EXECUTE) {
				...
				if (ccb->Cdb[0] == SCSIOP_START_STOP_UNIT) {
					...
					if (cdb->START_STOP.Start == STOP_UNIT_CODE) {
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("complete STOP_UNIT_CODE\n") );
					}
				}
			}
		}
		...
		if (Lurn->LurnStatus == LURN_STATUS_RUNNING) {
			...
			if (Lurn->IdeDisk->AddAddressOccured == TRUE) {
				...
				do {
					...
					DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Lurn->IdeDisk->AddAddressOccured\n") );
					...
					interval.QuadPart = (-10*NANO100_PER_SEC);      //wait 10 seconds for boost up network interface
					KeDelayExecutionThread( KernelMode, FALSE, &interval );
					...
					if (IdeDisk->CommLanScsiSession.MediumSpeed >= IdeDisk->TempLanScsiSession.MediumSpeed) {
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
									("medium speed is smaller IdeDisk->CommLanScsiSession.MediumSpeed = %d\n", 
									 IdeDisk->CommLanScsiSession.MediumSpeed) );
					...
					if (RtlEqualMemory(IdeDisk->CommLanScsiSession.NdasBindAddress.Address[0].NdasAddress.Lpx.Node,
									   IdeDisk->TempLanScsiSession.NdasBindAddress.Address[0].NdasAddress.Lpx.Node,
									   6)) {
					...
					LspMove( &IdeDisk->CommLanScsiSession, &IdeDisk->TempLanScsiSession, TRUE );

					for (lockIdx = NDAS_LOCK_COUNT - 1; lockIdx >= 0; lockIdx --) {

						if (Lurn->NdasLocked[lockIdx] == TRUE) {

							status = LurnIdeLockSynchrously( Lurn, LURNLOCK_OPCODE_ACQUIRE, lockIdx, NULL, NULL, TRUE );
					...
					DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("CommLanScsiSession is changed\n") );
		...
		if (Lurn->LurnStatus == LURN_STATUS_STALL) {
			...
			if (requireNotifyReconnect == TRUE) {
				...
				if (IsListEmpty(&IdeDisk->CcbQueue)) {
					...
					while (listEntry != &IdeDisk->CcbQueue) {
						...
						DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Set CCBSTATUS_FLAG_RECONNECTING\n") );
					}

					KeReleaseSpinLock( &Lurn->SpinLock, oldIrql );

					//requireNotifyReconnect = FALSE;
				}
			}
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, 
						("IdeDisk->MaxStallTimeOut.QuadPart = %I64d, NdasCurrentTime().QuadPart = %I64d\n", 
						 IdeDisk->MaxStallTimeOut.QuadPart, NdasCurrentTime().QuadPart) );
			...
			if (IdeDisk->MaxStallTimeOut.QuadPart < NdasCurrentTime().QuadPart) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("timeout terminate.\n") );
				terminateThread = TRUE;
				continue;
			}
			...
			do {
				...
				if (FlagOn(Lurn->AccessRight, GENERIC_WRITE)) {
					...
					if (status != STATUS_SUCCESS) {
						...
						status = LspReconnect( &IdeDisk->PrimaryLanScsiSession, NULL, NULL, NULL );

						if (status != STATUS_SUCCESS) {

							LurnRecordFault( Lurn, LURN_ERR_CONNECT, status, NULL );
							DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("failed status = %x\n", status) );
				...
				status = LurnIdeConfigure( Lurn, &PduResponse );
				...
				if (IdeDisk->LuHwData.InitialDevPowerRecycleCount != 0) {
					...
					if (IdeDisk->DiskWriteCacheDirty) {
						...
						if (IdeDisk->LuHwData.InitialDevPowerRecycleCount != powerRecycleCount) {
							...
							DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("RECONN: HDD got power-recycled. Stop the LURN.\n") );
				...
				NdasFcInitialize( &Lurn->IdeDisk->LanScsiSession->SendNdasFcStatistics );
				NdasFcInitialize( &Lurn->IdeDisk->LanScsiSession->RecvNdasFcStatistics );
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
				("The thread will be terminated IsListEmpty(&IdeDisk->CcbQueue) = %d\n", IsListEmpty(&IdeDisk->CcbQueue)) );
	...
	)
{
	...
	if (Ccb->DataBufferLength < FIELD_OFFSET(SENDCMDOUTPARAMS, bBuffer) + IDENTIFY_BUFFER_SIZE) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, 
					("Buffer too small. Supplied=%u Required=%u\n",
	...
	)
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
				("SendCmdIn: BuffSz=%x DrvNo=%x F=%x SC=%x SN=%x CL=%x CH=%x DH=%x CM=%x\n",
	...
	SendCmdOut->DriverStatus.bDriverError = (pduDesc.Command & ERR_STAT) ? SMART_IDE_ERROR : 0;
	...
	)
{
	...
	NDAS_ASSERT( SplitBlockBytes == (32*1024) || SplitBlockBytes == (64*1024) || SplitBlockBytes == (128*1024) );

	chooseDataLength = NdasFcChooseTransferSize( &IdeDisk->LanScsiSession->RecvNdasFcStatistics, 
												 (TransferBlocks * IdeDisk->LuHwData.BlockBytes < SplitBlockBytes) ?
												 (TransferBlocks * IdeDisk->LuHwData.BlockBytes) : SplitBlockBytes );
...
//				Ccb->CcbStatus = CCB_STATUS_INVALID_COMMAND;
			...
			case LANSCSI_RESPONSE_T_COMMAND_FAILED:
				...
				if (pduDesc.Feature & ECC_ERR || pduDesc.Feature & MARK_ERR) {
	...
	if (status == STATUS_SUCCESS) {
		...
		NdasFcUpdateTrasnferSize( &IdeDisk->LanScsiSession->RecvNdasFcStatistics, 
								  chooseDataLength, 
...
retry:
	...
	for (transferedBlocks = 0; transferedBlocks < TransferBlocks; transferedBlocks += splitBlocks) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("logicalBlockAddress = %I64u, transferBlocks = %d\n",
												 LogicalBlockAddress + transferedBlocks, blocksToTransfer) );

		status = LspRequest( IdeDisk->LanScsiSession, &pduDesc, &response, NULL, NULL, NULL );
		...
		if (!NT_SUCCESS(status)) {
			...
			case LANSCSI_RESPONSE_T_NOT_EXIST:
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("CcbStatus: returned LANSCSI_RESPONSE_T_NOT_EXIST.\n") );
				goto diskwrite_exit;
			...
			case LANSCSI_RESPONSE_T_BAD_COMMAND: // not used by HW
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("CcbStatus: returned LANSCSI_RESPONSE_T_BAD_COMMAND..\n") );
				LurnRecordFault(IdeDisk->Lurn, LURN_ERR_DISK_OP, LURN_FAULT_NOT_EXIST, &Lfi);
			...
			case LANSCSI_RESPONSE_T_COMMAND_FAILED: {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("CcbStatus: returned LANSCSI_RESPONSE_T_COMMAND_FAILED\n") );
				...
				if ((!(pduDesc.Feature & ID_ERR) && (pduDesc.Feature & ABRT_ERR)) ||
					...
					DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("IDE error: ID not found.\n") );
			...
			case LANSCSI_RESPONSE_T_BROKEN_DATA:
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("CcbStatus: returned LANSCSI_RESPONSE_T_BROKEN_DATA or failure..\n") );
...
#if 0 
	// As we don't use UDMA mode for 2.0 rev 0, we don't need to use LurnIdeDiskVerifyWrittenData.
	//
	...
	)
{
	...
	status = LurnIdeDiskWriteNoFua( Ccb, IdeDisk, DataBuffer, LogicalBlockAddress, TransferBlocks, SplitBlockBytes );
	...
	)
{
	...
	if (TransferBlocks == 0) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("Zero transfer block\n") );
...
DO_MORE:
	...
	if (!NT_SUCCESS(status)) {
		...
		case LANSCSI_RESPONSE_T_COMMAND_FAILED:
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("CcbStatus: Verify returned LANSCSI_RESPONSE_T_COMMAND_FAILED\n") );
	...
	)
{
	...
	if (!NT_SUCCESS(status)) {
		...
		LSCCB_SETSENSE( Ccb, SCSI_SENSE_NOT_READY, 0x05 /* LOGICAL UNIT DOES NOT RESPOND TO SELECTION */, 0 );
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Current power mode = %x\n", ataPowerMode) );
	}
	...
	)
{
	...
	if (Cdb->MODE_SENSE.Pc != (MODE_SENSE_CURRENT_VALUES>>6)) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("unsupported page control:%x\n", (ULONG)Cdb->MODE_SENSE.Pc) );
	...
	//
	parameterHeader->ModeDataLength = sizeof(MODE_PARAMETER_HEADER) - sizeof(parameterHeader->ModeDataLength); // exclude ModeDataLength itself
	...
	if (!(Lurn->AccessRight & GENERIC_WRITE)) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("MODE_DSP_WRITE_PROTECT\n") );
	...
	BlockCount = (ULONG)Lurn->DataUnitBlocks;
	...
	if (Cdb->MODE_SENSE.PageCode == MODE_SENSE_RETURN_ALL || Cdb->MODE_SENSE.PageCode == MODE_PAGE_CACHING) {	// all pages
		...
		if (Ccb->DataBufferLength < requiredLen) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Could not fill out caching page. %d.\n", Ccb->DataBufferLength) );
			return STATUS_SUCCESS;
		}
		...
		cachingPage->PageLength = 
			sizeof(MODE_CACHING_PAGE) - (FIELD_OFFSET(MODE_CACHING_PAGE, PageLength) + sizeof(cachingPage->PageLength));
	...
	)
{
	...
	if (Cdb->MODE_SELECT.PFBit == 0) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("unsupported page format:%x\n", (ULONG)Cdb->MODE_SELECT.PFBit) );
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, ("unsupported save page to non-volitile memory:%x.\n", (ULONG)Cdb->MODE_SELECT.SPBit) );
	...
	if ((*modePages & 0x3f) == MODE_PAGE_CACHING) {	// all pages
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Caching page\n") );
...
#else
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Not support write cache disabling\n") );
		Ccb->CcbStatus = CCB_STATUS_INVALID_COMMAND;
		return STATUS_SUCCESS;
#endif
	} else {
	...
	)
{
	...
	case SCSIOP_INQUIRY: {
		...
		if (Ccb->Cdb[1]  & CDB_INQUIRY_EVPD) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("SCSIOP_INQUIRY: got EVPD. Not supported.\n") );
...
//		inquiryData.RelativeAddressing;
	...
	case SCSIOP_READ_CAPACITY: {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_TRACE, ("sectorCount = %d\n", sectorCount/1024/1024/2) );
	...
	case SCSIOP_READ_CAPACITY16: {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("sectorCount = %d\n", sectorCount/1024/1024/2) );
	...
	case SCSIOP_RELEASE_UNIT:
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO,
					("RELEASE(6): %02x %02x %02x %02x %02x %02x\n",
					 Ccb->Cdb[0], Ccb->Cdb[1], Ccb->Cdb[2], Ccb->Cdb[3], Ccb->Cdb[4], Ccb->Cdb[5]) );
	...
	case SCSIOP_RELEASE_UNIT10:	
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, 
					("RELEASE(10): %02x %02x %02x %02x %02x %02x\n",
					 Ccb->Cdb[0], Ccb->Cdb[1], Ccb->Cdb[2], Ccb->Cdb[3], Ccb->Cdb[4], Ccb->Cdb[5],
					 Ccb->Cdb[6], Ccb->Cdb[7], Ccb->Cdb[8], Ccb->Cdb[9]) );
	...
	case SCSIOP_START_STOP_UNIT: {
		...
		LsCcbSetStatusFlag(Ccb, CCBSTATUS_FLAG_TIMER_COMPLETE);
		Ccb->CcbStatus = CCB_STATUS_SUCCESS;
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("START_STOP_UNIT CcbStatus:%d!!\n", Ccb->CcbStatus) );
	...
	case SCSIOP_READ16: {
		...
		if (Ccb->Cdb[0] == SCSIOP_READ || Ccb->Cdb[0] == 0x3E) {
#if DBG
			PCDB	cdb;
			...
			if (Ccb->SrbCdb.CDB16.ForceUnitAccess) {

				DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("READ:CDB16: ForceUnitAccess detected.\n") );
			}
		}

		LsCcbGetAddressAndLength( &Ccb->SrbCdb, &logicalBlockAddress, &transferBlocks );
		...
		status = LurnIdeDiskRead( Ccb, IdeDisk, Ccb->DataBuffer, logicalBlockAddress, transferBlocks, IdeDisk->MaxDataRecvLength );
	...
	case SCSIOP_WRITE16: {
		...
		if (!(Lurn->AccessRight & GENERIC_WRITE)){	// Read-only LURN
			...
			if(!LsCcbIsFlagOn(Ccb, CCB_FLAG_ALLOW_WRITE_IN_RO_ACCESS)) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, 
							("WRITE: WRITE command received without Write accessright.\n") );
	...
	case SCSIOP_SYNCHRONIZE_CACHE16: {
		...
		LURNIDE_INITIALIZE_PDUDESC( IdeDisk, &pduDescTemp, IDE_COMMAND, WIN_FLUSH_CACHE, 0, 0, 0, NULL, NULL );
	...
	case 0xF1:					// Vendor-specific commands
		...
		if (Ccb->Cdb[0] == SCSIOP_MEDIUM_REMOVAL) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_ERROR, 
						("SCSIOP_MEDIUM_REMOVAL: Prevent:%d Persistant:%d Control:0x%x\n",
	...
	) 
{
	...
	do {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("LurnIdeDiskChangeConnection\n") );
	...
	DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("status = %x\n", status) );
