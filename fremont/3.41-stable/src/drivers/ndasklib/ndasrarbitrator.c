	...
	)
{
	...
	do {
		...
		KeAcquireSpinLock( &ndasrArbitrator->SpinLock, &oldIrql );	
	
		for (nidx = 0; nidx < Lurn->LurnChildrenCnt; nidx++) {

			KeAcquireSpinLock( &Lurn->LurnChildren[nidx]->SpinLock, &oldIrql2 );
			...
			if (LurnGetCauseOfFault(Lurn->LurnChildren[nidx]) & (LURN_FCAUSE_BAD_SECTOR | LURN_FCAUSE_BAD_DISK)) {
				...
				ndasrArbitrator->DefectCodes[nidx] = 
					((LurnGetCauseOfFault(Lurn->LurnChildren[nidx]) & LURN_FCAUSE_BAD_SECTOR) ?
						NRMX_NODE_DEFECT_BAD_SECTOR : NRMX_NODE_DEFECT_BAD_DISK );
			}

			DebugTrace( NDASSCSI_DBG_LURN_NDASR_ERROR, 
						("Setting initial node %d flag: %d\n", nidx, ndasrArbitrator->NodeFlags[nidx]) );

			KeReleaseSpinLock( &Lurn->LurnChildren[nidx]->SpinLock, oldIrql2 );
		}

		KeReleaseSpinLock( &ndasrArbitrator->SpinLock, oldIrql );
		...
		for (ridx = 0; ridx < Lurn->LurnChildrenCnt; ridx++) { 
			...
			ndasrArbitrator->RoleToNodeMap[ridx] = 
					(UCHAR)ndasrArbitrator->Lurn->NdasrInfo->Rmd.UnitMetaData[ridx].iUnitDeviceIdx;
		...
		for (ridx = 0; ridx < Lurn->LurnChildrenCnt; ridx++) { // i : role index. 
			...
			if (FlagOn(unitDeviceStatus, NDAS_UNIT_META_BIND_STATUS_OFFLINE)) {
				...
				NDAS_ASSERT( FlagOn(ndasrArbitrator->NodeFlags[ndasrArbitrator->RoleToNodeMap[ridx]], NRMX_NODE_FLAG_STOP) );
	
				SetFlag( ndasrArbitrator->NodeFlags[ndasrArbitrator->RoleToNodeMap[ridx]], NRMX_NODE_FLAG_OFFLINE );
	...
	)
{
	...
	KeClearEvent( &ndasrArbitrator->FinishShutdownEvent );
	...
	)
{
	...
	do {
		...
		clientContext->Usn = NdasrArbitrator->Lurn->NdasrInfo->LocalClientRegisterUsn;
	...
	do {
		...
		if (FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_REBUILD)) {
			...
			if (NdasrArbitrator->NdasrRebuilder.RebuildStatus == NDASR_REBUILD_STATUS_SUCCESS		||
				NdasrArbitrator->NdasrRebuilder.RebuildStatus == NDASR_REBUILD_STATUS_FAILED		||
				NdasrArbitrator->NdasrRebuilder.RebuildStatus == NDASR_REBUILD_STATUS_CANCELLED) {
				...
				if (status == STATUS_SUCCESS) {
					...
					if (RtlNumberOfSetBits(&NdasrArbitrator->OosBmpHeader) == 0) {
						...
						NDAS_ASSERT( NdasrArbitrator->SyncState == NDASR_SYNC_REQUIRED );
					...
					if (NdasRaidArbitratorRefreshRaidStatus(NdasrArbitrator, FALSE) == FALSE) {
						...
						// already changed by clients

						NDAS_ASSERT( NdasrArbitrator->NdasrState == NRMX_RAID_STATE_DEGRADED ||
									 NdasrArbitrator->NdasrState == NRMX_RAID_STATE_FAILED );
					}
		...
		while (listEntry != &NdasrArbitrator->ClientQueue) {
			...
			if (clientContext->LocalClient == TRUE) {
				...
				KeAcquireSpinLockAtDpcLevel( &NdasrArbitrator->Lurn->NdasrInfo->RequestChannel.SpinLock );
				...
				KeReleaseSpinLockFromDpcLevel( &NdasrArbitrator->Lurn->NdasrInfo->RequestChannel.SpinLock );

				continue;
			}

			NDAS_ASSERT( clientContext->RequestConnection && clientContext->RequestConnection->ConnectionFileObject );
			...
			if (!LpxTdiV2IsRequestPending(&clientContext->RequestConnection->ReceiveOverlapped, 0)) {
				...
				if (!NT_SUCCESS(status)) {
					...
					DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
								("Failed to start to recv from client. Terminating this client\n") );
		...
		if (listEntry != &NdasrArbitrator->ClientQueue) {

			// event reallocated
			continue;
		}

		NDAS_ASSERT( eventCount <= 10 );
		...
		if (NdasrArbitrator->RequestToTerminate) {
			...
			if (!FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_TERMINATING)) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO,
							("DRAID Aribter stop requested. Sending RETIRE message to all client..\n") );	
		...
		if (NdasrArbitrator->RequestToShutdown) {
			...
			if (!FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_SHUTDOWN)) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
							("DRAID Aribter shutdown requested.Sending RETIRE message to all client..\n") );	
			...
			if (clean == TRUE) {
				...
				if (NdasRaidArbitratorRefreshRaidStatus(NdasrArbitrator, TRUE) == TRUE) {
	...
	) 
{
	...
	if (!clientContext) {
		...
		NDAS_ASSERT( clientContext->LocalClient == FALSE );
	...
	if (RegisterMsg->ConnType == NRMX_CONN_TYPE_NOTIFICATION) {
		...
		clientContext->Usn = NTOHL(RegisterMsg->Usn4);
	...
	)
{
	...
	while (listEntry != &NdasrArbitrator->NewClientQueue) {
		...
		if (clientContext->NotificationConnection && clientContext->NotificationConnection->NeedReply) {
			...
			registerReply.Length2	= NTOHS((UINT16)sizeof(NRMX_HEADER));
			registerReply.ReplyFlag = 1;
			registerReply.Sequence2	= clientContext->NotificationConnection->RegisterMsg.Header.Sequence2;
			...
			DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
						("LpxTdiSend NotificationConnection status=%x, result=%x.\n", status, result) );
		...
		if (clientContext->RequestConnection && clientContext->RequestConnection->NeedReply) {
			...
			registerReply.Length2	= NTOHS((UINT16)sizeof(NRMX_HEADER));
			registerReply.ReplyFlag = 1;
			registerReply.Sequence2	= clientContext->RequestConnection->RegisterMsg.Header.Sequence2;
			registerReply.Result	= NRMX_RESULT_SUCCESS;
			...
			DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
						("LpxTdiSend RequestConnection status=%x, result=%x.\n", status, result) );
	...
	) 
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
				("NdasrArbitrator->Lurn->NdasrInfo->BlocksPerBit = %d, NdasrArbitrator->OosBmpOnDiskSectorCount =%d\n", 
				 NdasrArbitrator->Lurn->NdasrInfo->BlocksPerBit, NdasrArbitrator->OosBmpOnDiskSectorCount) );			
	...
	NdasrArbitrator->LwrBmpBuffer = ExAllocatePoolWithTag( NonPagedPool, 
														   NdasrArbitrator->OosBmpByteCount, 
														   NDASR_BITMAP_POOL_TAG );
	...
	) 
{
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Before BitmapByte[0]=%x\n", NdasrArbitrator->OosBmpBuffer[0]) );	
	
	if (Set) {
	
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Setting in-memory bitmap offset %x:%x\n", bitOffset, numberOfBit) );
		
		RtlSetBits( &NdasrArbitrator->OosBmpHeader, bitOffset, numberOfBit );

	} else {

		DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Clearing in-memory bitmap offset %x:%x\n", bitOffset, numberOfBit) );
	...
	) 
{
	...
	if (HintAddedLock && HintRemovedLock == NULL) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Setting LWR bit %x:%x\n", bitOffset, numberOfBit) );
		...
		for (listEntry = NdasrArbitrator->AcquiredLockList.Flink;
			...
			// DebugTrace(NDASSCSI_DBG_LURN_NDASR_TRACE, ("Setting bit %x:%x\n", BitOffset, NumberOfBit));
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Updated LWR bitmap with %d locks\n", lockCount) );		
}
	...
	) 
{
	...
	for (i=0; i<NdasrArbitrator->OosBmpByteCount/sizeof(ULONG); i++) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Bitmap offset %x changed from %08x to %08x\n", 
	...
	) 
{
	...
	while (listEntry != &NdasrArbitrator->ClientQueue) {
		...
		if (clientContext->LocalClient) {
			...
			KeAcquireSpinLock( &NdasrArbitrator->Lurn->NdasrInfo->RequestChannel.SpinLock, &oldIrql );

			if (IsListEmpty(&NdasrArbitrator->Lurn->NdasrInfo->RequestChannel.RequestQueue)) {

				KeReleaseSpinLock( &NdasrArbitrator->Lurn->NdasrInfo->RequestChannel.SpinLock, oldIrql );
	...
	)
{
	...
	if (NTOHL(RequestMsg->Usn4) != NdasrArbitrator->Lurn->NdasrInfo->Rmd.uiUSN) {

		NDAS_ASSERT( ((signed _int32)(NTOHL(RequestMsg->Usn4) - NdasrArbitrator->Lurn->NdasrInfo->Rmd.uiUSN)) < 0 );
		NDAS_ASSERT( NdasrArbitrator->SyncState != NDASR_SYNC_DONE );
	...
	if (NdasrArbitrator->SyncState != NDASR_SYNC_DONE) {
		...
		ClientContext->Usn = NTOHL(RequestMsg->Usn4);
	}

	NDAS_ASSERT( NTOHL(RequestMsg->Usn4) == ClientContext->Usn );
	...
	case NRMX_CMD_NODE_CHANGE: {
		...
		commonReplyMsg.Length2		= HTONS((UINT16)replyLength);
		commonReplyMsg.Sequence2	= RequestMsg->Sequence2;
	...
	case NRMX_CMD_ACQUIRE_LOCK: {
		...
		UINT64						blockAddress	 = NTOHLL(acquireLockMsg->BlockAddress8);
		UINT32						blockLength		 = NTOHL(acquireLockMsg->BlockLength4);
		PNDASR_ARBITRATOR_LOCK_CONTEXT newLock;

		NDAS_ASSERT( NdasrArbitrator->SyncState == NDASR_SYNC_DONE );

		DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Arbitrator received ACQUIRE_LOCK message: %I64x:%x\n", blockAddress, blockLength) );
		...
		acquireLockReply.Header.Length2		= HTONS((UINT16)replyLength);
		acquireLockReply.Header.Sequence2	= RequestMsg->Sequence2;
		...
		if (acquireLockReply.Header.Result == NRMX_RESULT_GRANTED) {
			...
			acquireLockReply.LockId8		= NTOHLL(newLock->Id);
			acquireLockReply.BlockAddress8	= NTOHLL(newLock->BlockAddress);
			acquireLockReply.BlockLength4	= NTOHL(newLock->BlockLength);
		
		} else {

			acquireLockReply.LockType		= 0;
			acquireLockReply.LockMode		= 0;
			acquireLockReply.LockId8		= 0;
			acquireLockReply.BlockAddress8	= 0;
			acquireLockReply.BlockLength4	= 0;
		}
	...
	case NRMX_CMD_RELEASE_LOCK: {
		...
		PNDASR_ARBITRATOR_LOCK_CONTEXT	lock;
		UINT64							lockId = NTOHLL(releaseLockMsg->LockId8);
		...
		if (lockId != NRMX_LOCK_ID_ALL && NTOHL(RequestMsg->Usn4) != ClientContext->Usn) {

			NDAS_ASSERT(FALSE);

			commonReplyMsg.Result = NRMX_RESULT_UNSUCCESSFUL;
			break;	
		};
	...
	)
{
	...
	changeStatusMsg->Header.Length2			= HTONS((UINT16)msgLength);
	changeStatusMsg->Header.Sequence2		= RequestMsg->Sequence2;
	changeStatusMsg->Header.Result			= Result;
	
	changeStatusMsg->Usn4				= NTOHL(NdasrArbitrator->Lurn->NdasrInfo->Rmd.uiUSN);
	...
	)
{
	...
	changeStatusMsg->Header.Length2   = HTONS((UINT16)msgLength);
	changeStatusMsg->Header.Sequence2 = HTONS(ClientContext->NotifySequence);
	...
	if (ClientContext->LocalClient) {
		...
		KeAcquireSpinLock( &NdasrArbitrator->Lurn->NdasrInfo->NotitifyChannel.SpinLock, &oldIrql );
		...
		KeReleaseSpinLock( &NdasrArbitrator->Lurn->NdasrInfo->NotitifyChannel.SpinLock, oldIrql );
	...
	)
{
	...
	statusSyncedMsg.Length2	  = HTONS((UINT16)msgLength);
	statusSyncedMsg.Sequence2 = HTONS(ClientContext->NotifySequence);
	...
	)
{
	...
	retireMsg.Length2   = HTONS((UINT16)msgLength);
	retireMsg.Sequence2 = HTONS(ClientContext->NotifySequence);
	...
	)
{
	...
	UCHAR					newNdasrState;
	UCHAR					newNodeFlags[NRMX_MAX_MEMBER_DISK];
	UCHAR					newDefectCodes[NRMX_MAX_MEMBER_DISK];

	NTSTATUS				status;	

	BOOLEAN					spareChanged;
...
retry:
	...
	for (nidx = 0; nidx < NdasrArbitrator->Lurn->LurnChildrenCnt; nidx++) {
		...
		KeAcquireSpinLock( &NdasrArbitrator->Lurn->LurnChildren[nidx]->SpinLock, &oldIrql2 );

		newNodeFlags[nidx] = NdasRaidLurnStatusToNodeFlag( NdasrArbitrator->Lurn->LurnChildren[nidx]->LurnStatus );

		if (FlagOn(NdasrArbitrator->NodeFlags[nidx], NRMX_NODE_FLAG_OFFLINE)) {

			SetFlag( newNodeFlags[nidx], NRMX_NODE_FLAG_OFFLINE );
		}
		...
		KeReleaseSpinLock( &NdasrArbitrator->Lurn->LurnChildren[nidx]->SpinLock, oldIrql2 );
		...
		for (listEntry = NdasrArbitrator->ClientQueue.Flink;
			...
			if (FlagOn(newNodeFlags[nidx], NRMX_NODE_FLAG_DEFECTIVE)) {
				...
				if (FlagOn(clientContext->NodeFlags[nidx], NRMX_NODE_FLAG_DEFECTIVE)) {
					...
					DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
								("newNodeFlags[nidx], NRMX_NODE_FLAG_DEFECTIVE Node %d: Arbitrator->NodeFlag %x Old flag %x, Client Flag %x\n", 
			...
			if (FlagOn(clientContext->NodeFlags[nidx], NRMX_NODE_FLAG_STOP)) {
				...
				DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO, 
							(" clientContext->NodeFlags[nidx], NRMX_NODE_FLAG_STOP Node %d: Arbitrator->NodeFlag %x Old flag %x, Client Flag %x\n", 
							 nidx, NdasrArbitrator->NodeFlags[nidx], newNodeFlags[nidx], clientContext->NodeFlags[nidx] ) );
		...
		if (NdasrArbitrator->NodeFlags[nidx] != newNodeFlags[nidx]) {
			...
			NdasrArbitrator->DefectCodes[nidx] = 0;
			...
			if (NdasrArbitrator->NodeToRoleMap[nidx] >= NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount) {
		...
		if (FlagOn(NdasrArbitrator->NodeFlags[nidx], NRMX_NODE_FLAG_DEFECTIVE)) {
			...
			if ((NdasrArbitrator->DefectCodes[nidx] & ~NRMX_NODE_DEFECT_REPLACED_BY_SPARE) != 
				(newDefectCodes[nidx] & ~NRMX_NODE_DEFECT_REPLACED_BY_SPARE)) {

				DebugTrace( NDASSCSI_DBG_LURN_NDASR_INFO,
							("aaaa NdasrArbitrator->DefectCodes[nidx] = %x, newDefectCodes[nidx] = %x\n", 
							 NdasrArbitrator->DefectCodes[nidx], newDefectCodes[nidx]) );
	...
	for (nidx = 0; nidx < NdasrArbitrator->Lurn->LurnChildrenCnt; nidx++) {
		...
		NDAS_ASSERT( NdasrArbitrator->NodeFlags[nidx] == NRMX_NODE_FLAG_RUNNING							||
					 NdasrArbitrator->NodeFlags[nidx] == NRMX_NODE_FLAG_DEFECTIVE						||
					 NdasrArbitrator->NodeFlags[nidx] == (NRMX_NODE_FLAG_OFFLINE | NRMX_NODE_FLAG_STOP)	||
	...
	if (ForceChange != TRUE) {
		...
		NDAS_ASSERT( NdasrArbitrator->NdasrState != newNdasrState || spareChanged == TRUE );
	}
	...
	if (NdasrArbitrator->NdasrState != newNdasrState) {
		...
		if (newNdasrState == NRMX_RAID_STATE_OUT_OF_SYNC) {
			...
			if (NdasrArbitrator->OutOfSyncRoleIndex == NO_OUT_OF_SYNC_ROLE) {
				...
				NdasrArbitrator->OutOfSyncRoleIndex = 
					NdasrArbitrator->NodeToRoleMap[NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount-1];
			}
		}
	...
	if (changed == TRUE) {
		...
		NdasRaidArbitratorUpdateInCoreRmd( NdasrArbitrator );
	...
	if (changed == FALSE) {
		...
		// if abnormally terminated it can not be changed

		NDAS_ASSERT( FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_INITIALIZING) ||
					 ForceChange && FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_SHUTDOWN) );
	}
	...
	if (changed) {
		...
		if (NdasrArbitrator->SyncState != NDASR_SYNC_REQUIRED) {
			...
			NDAS_ASSERT( ForceChange && FlagOn(NdasrArbitrator->Status, NDASR_ARBITRATOR_STATUS_SHUTDOWN) );
		}
	}
	...
	) 
{
	...
	if (FlagOn(newRmd.state, NDAS_RAID_META_DATA_STATE_MOUNTED) &&
		...
		for (nidx = 0; nidx < NdasrArbitrator->Lurn->LurnChildrenCnt; nidx++) {
			...
			for (j=0; j < usedAddrCount; j++) {
				...
				if (RtlCompareMemory(newRmd.ArbitratorInfo[j].Addr, 
									 ideDisk->LanScsiSession->NdasBindAddress.Address[0].NdasAddress.Lpx.Node, 
									 6) == 6) {
			...
			RtlCopyMemory( newRmd.ArbitratorInfo[usedAddrCount].Addr, 
						   ideDisk->LanScsiSession->NdasBindAddress.Address[0].NdasAddress.Lpx.Node, 
						   6 );
	...
	for (ridx = 0; ridx < NdasrArbitrator->Lurn->LurnChildrenCnt; ridx++) { // i is role index.
		...
		if (FlagOn(nodeFlags, NRMX_NODE_FLAG_OFFLINE)) {

			NDAS_ASSERT( FlagOn(nodeFlags, NRMX_NODE_FLAG_STOP) );
			NDAS_ASSERT( FlagOn(NdasrArbitrator->Lurn->NdasrInfo->Rmd.UnitMetaData[ridx].UnitDeviceStatus, 
						 NDAS_UNIT_META_BIND_STATUS_OFFLINE) );

			newRmd.UnitMetaData[ridx].UnitDeviceStatus = 
				NdasrArbitrator->Lurn->NdasrInfo->Rmd.UnitMetaData[ridx].UnitDeviceStatus;			
		}
	...
	)
{
	...
	status = NdasRaidArbitratorWriteMetaSync( NdasrArbitrator, (PUCHAR) Rmd, (-1*NDAS_BLOCK_LOCATION_RMD_T), 1, TRUE, FALSE );
	...
	status = NdasRaidArbitratorWriteMetaSync( NdasrArbitrator, (PUCHAR) Rmd, (-1*NDAS_BLOCK_LOCATION_RMD), 1, TRUE, FALSE );
...
NdasRaidArbitratorWriteMetaSync (
	...
	) 
 {
	...
	UCHAR				ridx;
	PLURELATION_NODE	lurnChildren[NRMX_MAX_MEMBER_DISK] = {0};
	UCHAR				lurnCount;	
	UCHAR				lidx;
	LARGE_INTEGER		delay;
	...
	if (NdasrArbitrator->Lurn->LockingTime.QuadPart) {

		// for local clients

		delay.QuadPart = -50 * NANO100_PER_MSEC;
	...
	for (ridx = 0; ridx < NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount; ridx ++) { 
		...
		if (!FlagOn(NdasrArbitrator->Lurn->Lur->SupportedNdasFeatures, NDASFEATURE_SIMULTANEOUS_WRITE)) {
		...
		if (NdasrArbitrator->Lurn->LurnChildren[NdasrArbitrator->RoleToNodeMap[ridx]]->NdasLocked[NDAS_RW_LOCK] == TRUE) {

				status = STATUS_SUCCESS;
				continue;
		}

		status = NdasRaidLurnLockSynchrously( NdasrArbitrator->Lurn->LurnChildren[NdasrArbitrator->RoleToNodeMap[ridx]],
...
ERROR_OUT:
	...
	for (ridx = 0; ridx < NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount; ridx ++) { 
		...
		if (!FlagOn(NdasrArbitrator->NodeFlags[NdasrArbitrator->RoleToNodeMap[ridx]], NRMX_NODE_FLAG_RUNNING)) {
			...
			NdasrArbitrator->Lurn->LurnChildren[NdasrArbitrator->RoleToNodeMap[ridx]]->NdasLocked[NDAS_RW_LOCK] = FALSE;
			continue;
		}

		if (NdasrArbitrator->Lurn->LurnChildren[NdasrArbitrator->RoleToNodeMap[ridx]]->NdasLocked[NDAS_RW_LOCK] == TRUE) {

			lockStatus = NdasRaidLurnLockSynchrously( NdasrArbitrator->Lurn->LurnChildren[NdasrArbitrator->RoleToNodeMap[ridx]],
													  LURNLOCK_OPCODE_RELEASE,
													  NDAS_RW_LOCK,
													  TRUE );

			NdasrArbitrator->Lurn->LockingTime.QuadPart = 0;
	...
	) 
{
	...
	if (NdasrArbitrator->NdasrState == NRMX_RAID_STATE_DEGRADED) {
		...
		if (!FlagOn(NdasrArbitrator->NodeFlags[NdasrArbitrator->RoleToNodeMap[NdasrArbitrator->OutOfSyncRoleIndex]], NRMX_NODE_FLAG_DEFECTIVE)) {
		...
		if (((currentTick.QuadPart - NdasrArbitrator->DegradedSince.QuadPart) * KeQueryTimeIncrement()) < NDAS_RAID_SPARE_HOLDING_TIMEOUT) {
			...
			if (spareFound) {
				...
				NDAS_ASSERT( FlagOn(NdasrArbitrator->NodeFlags[oosNode], NRMX_NODE_FLAG_DEFECTIVE) );
				...
				if (NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount < NdasrArbitrator->Lurn->LurnChildrenCnt) {
					...
					DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Spare disk is not running\n") );
	...
	for (i = 0; i < NdasrArbitrator->Lurn->LurnChildrenCnt; i++) {
		...
		for (lockIte = 0; lockIte<acquiredLockCount;lockIte++) {
			...
			for (lockAddr = lockAddrLenPair[lockIte].LockAddress; 
				...
				DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Flushing 1.0 HW range %I64x:%x.\n", lockAddr, lockLength ));
	...
	) 
{
	...
	NDAS_ASSERT( NdasrArbitrator->NdasrRebuilder.RebuildStatus == NDASR_REBUILD_STATUS_NONE &&
	...
	DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Arbitrator->AcquiredLockCount %d\n",NdasrArbitrator->AcquiredLockCount) );	
	...
	if (status != STATUS_SUCCESS) {
		...
		NdasrArbitrator->NdasrRebuilder.RebuildStatus = NDASR_REBUILD_STATUS_NONE;
	...
	) 
{
	...
	} else if (NdasrArbitrator->NdasrRebuilder.RebuildStatus == NDASR_REBUILD_STATUS_FAILED) {

		status = NdasrArbitrator->NdasrRebuilder.Status;
	...
	} else if (NdasrArbitrator->NdasrRebuilder.RebuildStatus == NDASR_REBUILD_STATUS_CANCELLED) {
	...
	NdasrArbitrator->NdasrRebuilder.RebuildRequested = FALSE;
	NdasrArbitrator->NdasrRebuilder.CancelRequested = FALSE;
	NdasrArbitrator->NdasrRebuilder.Status = STATUS_SUCCESS;
	...
	)
{
	...
	ndasrRebuilder->RebuildStatus   = NDASR_REBUILD_STATUS_NONE;
	...
	)
{
	...
	KeAcquireSpinLock( &NdasrRebuilder->NdasrArbitrator->SpinLock, &oldIrql );

	NDAS_ASSERT( NdasrRebuilder->RebuildStatus == NDASR_REBUILD_STATUS_NONE || NdasrRebuilder->RebuildRequested );
	...
	KeReleaseSpinLock( &NdasrRebuilder->NdasrArbitrator->SpinLock, oldIrql );
	...
	)
{
	...
	PLURELATION_NODE	lurn = NdasrRebuilder->NdasrArbitrator->Lurn;
	UCHAR				ridx;
	...
	do {
		...
		DebugTrace( NDASSCSI_DBG_LURN_NDASR_TRACE, ("Waiting rebuild request\n") );
		...
		if (NdasrRebuilder->RebuildRequested) {	
			...
			NDAS_ASSERT( NdasrRebuilder->RebuildStatus == NDASR_REBUILD_STATUS_NONE );
			
			// Rebuild request should be called only when previous result is cleared.
			
			NdasrRebuilder->RebuildStatus = NDASR_REBUILD_STATUS_WORKING;
			...
			if (NdasrRebuilder->BlockLength == 0) {
				...
				NdasrRebuilder->RebuildStatus = NDASR_REBUILD_STATUS_SUCCESS;
			...
			for (;;) {
				...
				if (lockingTime.QuadPart == 0) {

					KeAcquireSpinLock( &lurn->NdasrInfo->SpinLock, &oldIrql );

					if (lurn->NdasrInfo->NdasrClient && 
						(!IsListEmpty(&lurn->NdasrInfo->NdasrClient->CcbQueue) || lurn->LockingTime.QuadPart)) {

						KeReleaseSpinLock( &lurn->NdasrInfo->SpinLock, oldIrql );
	
						// for local clients

						delay.QuadPart = -NDAS_MAX_LOCKING_DURATION;
						KeDelayExecutionThread( KernelMode, FALSE, &delay );
					...
					for (ridx = 0; ridx < NdasrRebuilder->NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount; ridx ++) { 
						...
						if (!FlagOn(lurn->Lur->SupportedNdasFeatures, NDASFEATURE_SIMULTANEOUS_WRITE)) {

							break;
						}

						if (!FlagOn(NdasrRebuilder->NdasrArbitrator->NodeFlags[NdasrRebuilder->NdasrArbitrator->RoleToNodeMap[ridx]], NRMX_NODE_FLAG_RUNNING)) {

							continue;
						}

						if (lurn->LurnChildren[NdasrRebuilder->NdasrArbitrator->RoleToNodeMap[ridx]]->NdasLocked[NDAS_RW_LOCK] == TRUE) {
					...
					lockingTime = NdasCurrentTime();
				
				} else {

					if (NdasCurrentTime().QuadPart-lockingTime.QuadPart >= 100 * NANO100_PER_MSEC) {
						...
						for (ridx = 0; ridx < lurn->NdasrInfo->ActiveDiskCount; ridx ++) { 
							...
							if (lurn->LurnChildren[NdasrRebuilder->NdasrArbitrator->RoleToNodeMap[ridx]]->NdasLocked[NDAS_RW_LOCK] == TRUE) {
								...
								lockStatus = NdasRaidLurnLockSynchrously( lurn->LurnChildren[NdasrRebuilder->NdasrArbitrator->RoleToNodeMap[ridx]],
																		  LURNLOCK_OPCODE_RELEASE,
																		  NDAS_RW_LOCK,
																		  TRUE );
		
								lurn->LockingTime.QuadPart = 0;

								if (lockStatus != STATUS_SUCCESS) {
						
									lurn->LurnChildren[NdasrRebuilder->NdasrArbitrator->RoleToNodeMap[ridx]]->NdasLocked[NDAS_RW_LOCK] = FALSE;
								}
						...
						delay.QuadPart = -1 * NANO100_PER_MSEC;
						KeDelayExecutionThread( KernelMode, FALSE, &delay );
				...
				NDAS_ASSERT( ridx < NdasrRebuilder->NdasrArbitrator->Lurn->NdasrInfo->ActiveDiskCount );
			...
			if (NdasrRebuilder->ExitRequested) {
				...
				NdasrRebuilder->RebuildStatus = NDASR_REBUILD_STATUS_CANCELLED;
			...
			if (status == STATUS_SUCCESS) {
				...
				NdasrRebuilder->RebuildStatus = NDASR_REBUILD_STATUS_FAILED;
			}
	...
	) 
{
	...
	if (NdasrRebuilder->CancelRequested || NdasrRebuilder->ExitRequested) {
		...
		DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Rebuild IO is cancelled or exited.\n") );		

		NdasrRebuilder->Status = STATUS_UNSUCCESSFUL;
		return STATUS_UNSUCCESSFUL;
	}
	...
	do {
		...
		if (ndasrInfo->ActiveDiskCount - ndasrInfo->ParityDiskCount != 1) { // trick for LURN_NDASR_RAID1
			...
			for (ridx = nextChild; ridx < ndasrInfo->ActiveDiskCount; ridx++) {
				...
				for (k = 0; k < NdasrRebuilder->NdasrArbitrator->Lurn->ChildBlockBytes * BlockLength; k ++) {
					...
					NdasrRebuilder->RebuildBuffer[outOfSyncRoleIndex][k] ^=	NdasrRebuilder->RebuildBuffer[ridx][k];									
				}
			}
		}
		...
		if (status != STATUS_SUCCESS) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_NDASR_ERROR, ("Failed to read from healthy disk. RAID failure\n") );
			break;
		}
		...
		if (status != STATUS_SUCCESS) {
			...
			DebugTrace( NDASSCSI_DBG_LURN_IDE_INFO, ("Failed to write to out-of-sync disk\n") );
			break;
		}

	} while (0);

	NdasrRebuilder->Status = status;
