			...
			for (nidx = 0; nidx < NdasrClient->Lurn->LurnChildrenCnt; nidx++) {
				...
				if (Ccb->OperationCode == CCB_OPCODE_UPDATE) {
					...
					if (nidx > 0) {
						...
						lurnUpdate = (PLURN_UPDATE)nextCcb[NdasrClient->NodeToRoleMap[nidx2]]->DataBuffer;

						NDAS_ASSERT( lurnUpdate->TryCount == 1 );

						if (lurnUpdate->UpdateClass == LURN_UPDATECLASS_WRITEACCESS_USERID) {

							oldTryCount = lurnUpdate->TryCount;
							lurnUpdate->TryCount = NDAS_RAID_UPDATE_COUNT;
						}
					}
				}
				...
				if (Ccb->OperationCode == CCB_OPCODE_UPDATE) {
					...
					if (nidx > 0) {
						...
						if (lurnUpdate->UpdateClass == LURN_UPDATECLASS_WRITEACCESS_USERID) {
							...
							lurnUpdate->TryCount = oldTryCount;
						}
					}
				}
