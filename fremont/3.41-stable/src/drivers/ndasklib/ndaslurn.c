...
#if DBG
NDASSCSI_DBG_FLAGS NdasScsiDebugLevel = NDASSCSI_DBG_MINIPORT_ERROR		| 
	...
	) 
{
	...
	if (LUR_GETROOTNODE(Lur) == NULL) {
		...
		DebugTrace( NDASSCSI_DBG_LUR_INFO, ("No LURN.\n") );
	...
	) 
{
	...
	if (Ccb->OperationCode == CCB_OPCODE_UPDATE) {
		...
		NDAS_ASSERT( LurnUpdate->TryCount == 1 );

		if (LurnUpdate->UpdateClass != LURN_UPDATECLASS_WRITEACCESS_USERID &&
			LurnUpdate->UpdateClass != LURN_UPDATECLASS_READONLYACCESS) {
	...
	if (Ccb->OperationCode == CCB_OPCODE_EXECUTE) {
		...
		case SCSIOP_WRITE16:
			...
			if (FlagOn(Lur->SupportedNdasFeatures, NDASFEATURE_SIMULTANEOUS_WRITE)) {
			...
			if (LUR_GETROOTNODE(Lur)->LurnType == LURN_CDROM) {
...
#if __NDAS_KLIB_ENABLE_READ_LOCK__
...
LurnSendStopCcb(
	...
	KDPrintM(NDASSCSI_DBG_LUR_TRACE, ("entered.\n"));
	...
	LsCcbSetFlag( ccb, CCB_FLAG_SYNCHRONOUS | CCB_FLAG_DONOT_PASSDOWN );
...
#define _LUR_SET_OPTIONAL_FEATURE(SUPPORTED_FEAT, ENABLED_FEAT, NDAS_FEATURE) { \
	if((SUPPORTED_FEAT)&(NDAS_FEATURE)) {	\
		...
		KDPrintM(NDASSCSI_DBG_LUR_INFO, ((#NDAS_FEATURE " not supported.\n")));	\
	} \
}
...
#define _LUR_RESET_OPTIONAL_FEATURE(SUPPORTED_FEAT, ENABLED_FEAT, NDAS_FEATURE) { \
	if((SUPPORTED_FEAT)&(NDAS_FEATURE)) {	\
		...
		KDPrintM(NDASSCSI_DBG_LUR_ERROR, ((#NDAS_FEATURE " not supported.\n")));	\
	} \
}
...
FilterWriteByBACL (
	...
	PLSU_BLOCK_ACL	Bacl,
	BOOLEAN			RoFakeWrite,
	UINT64			WriteBlockAddr,
	UINT32			WriteBlockLen,
	PCCB			WriteCommand
	)
{
	NTSTATUS	status;
	...
	bace = LsuGetAce( Bacl, WriteBlockAddr, WriteBlockLen );
	
	if (bace) {

		//	Read-only block area

		if (!FlagOn(bace->AccessMode, LSUBACE_ACCESS_WRITE)) {
...
//
// Interface look-up function will search from top to bottom.
// Do not forget to increase NR_LURN_MAX_INTERFACE after adding a interface.
//

PLURN_INTERFACE	LurnInterfaceList[NR_LURN_MAX_INTERFACE] = {
					
					&LurnNdasAggregationInterface,	// 0
					&LurnNdasRaid0Interface,
					&LurnNdasRaid1Interface,
					&LurnNdasRaid4Interface,	// 10: &LurnRAID4RInterface, RAID4 is temporarily disabled.
					&LurnNdasRaid5Interface,	// 11
					//
					// Following interfaces support all LURN types with their device interfaces.
					// If LURN type-specific interface is needed,
					// insert before these interface.
					//
					&LurnAtaInterface,
					&LurnAtapiInterface,
				};
...
LookupLurnInterface(
	...
	__in UCHAR DeviceInterfaceType
){
	LONG i;
	PLURN_INTERFACE	lurnInterface;

	for(i = 0; i < NR_LURN_MAX_INTERFACE; i ++) {

		lurnInterface = LurnInterfaceList[i];

		if(lurnInterface->DeviceInterfaceType == DeviceInterfaceType) {

			// LURN_NULL mean that the LURN interface support all device type
			// with the specified device interface.
			if(lurnInterface->LurnType == LURN_NULL) {
				return lurnInterface;
			}
			else if(lurnInterface->LurnType == LurnType) {
				return lurnInterface;
			}
		}
	}

	return (PLURN_INTERFACE)NULL;
}
	...
	) 
{
	...
	if (Lurn->LurnDesc->LurnType > LURN_LAST_DEVICE_TYPE ) {
	...
	Lurn->LurnInterface   = LookupLurnInterface(Lurn->LurnDesc->LurnType, Lurn->LurnDesc->LurnDeviceInterface);
	...
	Lurn->DataUnitBlocks  = Lurn->UnitBlocks;
	Lurn->AccessRight	  = Lurn->LurnDesc->AccessRight;
		
	LurnResetFaultInfo( Lurn );

	if (FlagOn(Lurn->LurnDesc->LurnOptions, LURNOPTION_SET_RECONNECTION)) {

		Lurn->ReconnTrial		= Lurn->LurnDesc->ReconnTrial;
		Lurn->ReconnInterval	= (UINT64)Lurn->LurnDesc->ReconnInterval * NANO100_PER_MSEC;
	...
	if (Lurn->LurnDesc->LurnOptions & LURNOPTION_MISSING) {
		...
		DebugTrace( NDASSCSI_DBG_LUR_NDASR_ERROR, 
	...
	DebugTrace( NDASSCSI_DBG_LUR_INFO, ("return 0x%08lx : Idx:%d\n", status, Lurn->LurnId) );
...
{
	DebugTrace( NDASSCSI_DBG_LUR_ERROR, ("In. Lurn %p\n", Lurn) );
