...
typedef struct _NDASR_CLIENT_CONTEXT {
	...
	UINT32		Usn;
	UCHAR		NodeFlags[NRMX_MAX_MEMBER_DISK]; 			// Ored value of NRMX_NODE_FLAG_*. Indexed by Node number
	UCHAR		DefectCodes[NRMX_MAX_MEMBER_DISK]; 		// Ored value of NRMX_NODE_DEFECT_*. Indexed by Node number
...
typedef struct _NDASR_REBUILDER {
	...
	UINT32					RebuildStatus;		// NDASR_REBUILD_STATUS_NONE, NDASR_REBUILD_STATUS_*
	...
	PUCHAR					RebuildBuffer[NRMX_MAX_MEMBER_DISK];
...
typedef struct _NDASR_ARBITRATOR {
	...
	UCHAR				NodeToRoleMap[NRMX_MAX_MEMBER_DISK]; 	// Unit number to RAID role mapping.
	UCHAR				RoleToNodeMap[NRMX_MAX_MEMBER_DISK]; 	// RAID role index to unit number map
