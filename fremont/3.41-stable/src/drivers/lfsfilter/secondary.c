...
#define MAX_TRY_QUERY 2
	...
	for (tryQuery = 0; tryQuery < MAX_TRY_QUERY; tryQuery++) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_Create: LfsTable_QueryPrimaryAddressList %08x\n", status) );
	...
	if (FlagOn(secondary->Thread.Flags, SECONDARY_THREAD_FLAG_UNCONNECTED)) {
		
		secondary->Thread.SessionContext.SessionSlotCount = 1;
	}

	NDAS_ASSERT( secondary->Thread.SessionContext.SessionSlotCount == 1 );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary close Secondary = %p\n", Secondary) );
	...
	NDAS_ASSERT( Secondary->ThreadObject != NULL );
	...
	status = KeWaitForSingleObject( Secondary->ThreadObject, Executive, KernelMode, FALSE, &timeOut );
	...
	if (Secondary->CcbCount) {

		PLIST_ENTRY		ccbListEntry;

		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_ERROR, ("Secondary->CcbCount != 0\n") );

		timeOut.QuadPart = (10 * DELAY_ONE_SECOND);      //delay 10 seconds
		KeDelayExecutionThread( KernelMode, FALSE, &timeOut );

		while (!IsListEmpty(&Secondary->CcbQueue)) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_ERROR, ("Close Ccbs Forcely\n") );

			ccbListEntry = Secondary->CcbQueue.Flink;
	...
	if (Secondary->CcbCount == 0) {
		
		NDAS_ASSERT( IsListEmpty(&Secondary->FcbQueue) );
		NDAS_ASSERT( IsListEmpty(&Secondary->CcbQueue) );
		NDAS_ASSERT( IsListEmpty(&Secondary->RequestQueue) );
	}

#if 0
	if (FlagOn(LfsDeviceExt->Flags, LFS_DEVICE_FLAG_SURPRISE_REMOVED)) {

		ObDereferenceObject( LfsDeviceExt->DiskDeviceObject );
		ObDereferenceObject( LfsDeviceExt->MountVolumeDeviceObject );
		ObDereferenceObject( LfsDeviceExt->BaseVolumeDeviceObject );
	}
#endif
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary close return\n") );
	...
	) 
{
	...
	secondaryRequest = ExAllocatePoolWithTag( NonPagedPool, allocationSize, SECONDARY_MESSAGE_TAG );
	...
	)
{
	...
	if (result == 0) {
		...
		NDAS_ASSERT( SecondaryRequest->ListEntry.Flink == SecondaryRequest->ListEntry.Blink );

		if (SecondaryRequest->SendMdl) {

			MmUnlockPages( SecondaryRequest->SendMdl );
			IoFreeMdl( SecondaryRequest->SendMdl );
		}

		if (SecondaryRequest->RecvMdl) {

			MmUnlockPages( SecondaryRequest->RecvMdl );
	...
	)
{
	...
	if (FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_START) &&
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("QueueingSecondaryRequest return\n") );
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, 
				   ("Secondary_TryCloseCcbs: IsListEmpty(&Secondary->CcbQueue) = %d\n", IsListEmpty(&Secondary->CcbQueue)) );

	Secondary_Reference(Secondary);	
	
	if (ExTryToAcquireFastMutex(&Secondary->CcbQMutex) == FALSE) {

		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_TryCloseCcbs: can't access Mutex\n") );
		Secondary_Dereference(Secondary);
	...
	if (FlagOn(Secondary->Flags, SECONDARY_FLAG_RECONNECTING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, ("Secondary_TryCloseCcbs: reconnecting\n") );
		ExReleaseFastMutex( &Secondary->CcbQMutex );
		Secondary_Dereference(Secondary);
	...
	try {
		...
		while (listEntry != &Secondary->CcbQueue) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, 
						   ("Secondary_TryCloseCcbs: fcb->FullFileName = %wZ fcb->UncleanCount = %d\n", 
							&ccb->Fcb->FullFileName, fcb->UncleanCount) );
			...
			SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_INFO, 
						   ("Secondary_TryCloseCcbs: try close fcb->FullFileName = %wZ\n", &ccb->Fcb->FullFileName) );
	...
	)
{
	...
	if (irpSp->MajorFunction == IRP_MJ_CREATE					||	// 0x00
		...
		if (irpSp->MinorFunction == IRP_MN_MOUNT_VOLUME		||
			...
			PrintIrp( LFS_DEBUG_SECONDARY_ERROR, "Secondary_PassThrough", Secondary->LfsDeviceExt, Irp );
			NDAS_ASSERT(FALSE);

			NDAS_ASSERT( Secondary_LookUpCcb(Secondary, fileObject) == NULL );
	...
	while (1) {
		...
		NDAS_ASSERT( fastMutexSet == FALSE );
		NDAS_ASSERT( retry == FALSE );
		...
		if (!FlagOn(Secondary->Flags, SECONDARY_FLAG_RECONNECTING)) {
			...
			NDAS_ASSERT( Secondary->ThreadObject );
		}
		...
		if (status == STATUS_INSUFFICIENT_RESOURCES) {
			...
			NDAS_ASSERT( status == STATUS_REMOTE_DISCONNECT || status == STATUS_IO_DEVICE_ERROR );
		}

		NDAS_ASSERT( fastMutexSet == TRUE );

		ExAcquireFastMutex( &Secondary->FastMutex );

		NDAS_ASSERT( FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_REMOTE_DISCONNECTED) ||
					 FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_UNCONNECTED) );
		...
		if (FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_REMOTE_DISCONNECTED) ||
			...
			NDAS_ASSERT( !FlagOn(Secondary->Flags, SECONDARY_FLAG_RECONNECTING) );
	...
	)
{
	...
	PLIST_ENTRY				secondaryRequestEntry;
	UINT16					mid;
	UINT16					previousSessionSlotCount = Secondary->Thread.SessionContext.SessionSlotCount;
	UINT16					queuedRequestCount = 0;
	...
	NDAS_ASSERT( Secondary->ThreadHandle );
...
#define MAX_RECONNECTION_TRY	300
	...
	for (reconnectionTry = 0; reconnectionTry < MAX_RECONNECTION_TRY; reconnectionTry++) {
		...
		if (Secondary->ThreadHandle) {
			...
			if (waitStatus == STATUS_SUCCESS) {
				...
				RtlZeroMemory( &Secondary->Thread.SessionContext, sizeof(Secondary->Thread.SessionContext) );
		...
		tableStatus = LfsTable_QueryPrimaryAddressList( GlobalLfs.LfsTable,
														&netdiskPartitionInfo,
														Secondary->PrimaryAddressList,
														&Secondary->NumberOfPrimaryAddress );
		...
		if (NT_SUCCESS(tableStatus)) {
			...
			if (Lfs_IsLocalAddress(&Secondary->PrimaryAddressList[0]) && Secondary->LfsDeviceExt->SecondaryState == SECONDARY_STATE) { // not yet purged
		...
		NDAS_ASSERT( FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_CONNECTED) && 
					 !FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_ERROR) );
    ...
    for (ccblistEntry = Secondary->CcbQueue.Blink;
		...
		//
		//	[64bit issue]
		//	I assume that IrpTag4 may have any dummy value.
		//

		ndfsWinxpRequestHeader->IrpTag4   = HTONL((UINT32)PtrToUlong(ccb));
	...
	while (!IsListEmpty(&tempRequestQueue)) {
		...
		secondaryRequest->NdfsRequestHeader.Tid2	= HTONS(Secondary->Thread.SessionContext.Tid);
	...
	if (result == TRUE) {
		...
		if (previousSessionSlotCount < Secondary->Thread.SessionContext.SessionSlotCount) {
			...
			UINT16	consumeRequireCount = previousSessionSlotCount - Secondary->Thread.SessionContext.SessionSlotCount;
	...
	)
{
	...
	LfsDeviceExt_Dereference( Secondary->LfsDeviceExt ); // for close volume handle
	Secondary_Dereference( Secondary );
	...
	) 
{
	...
	Secondary_Reference( Secondary );
	LfsDeviceExt_Reference( Secondary->LfsDeviceExt ); // for close volume handle
