	...
	while (secondaryThreadTerminate == FALSE) {
...
#endif		
		NDAS_ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
		NDAS_ASSERT( eventCount < THREAD_WAIT_OBJECTS );
		...
		if (eventStatus == 0) {
						...
						if (requestCount[ndfsWinxpReplyHeader->IrpMajorFunction] %100 == 0) {
							...
							default:
								...
								SPY_LOG_PRINT( LFS_DEBUG_SECONDARY_TRACE, 
											   ("%s, requestCount = %d, executionTime = %I64d\n",
			...
			if (Secondary->Thread.ReceiveOverlapped.Request[0].IoStatusBlock.Status != STATUS_SUCCESS ||
				...
				NDAS_ASSERT( slotIndex < Secondary->Thread.SessionContext.SessionSlotCount );
			...
			if (status == STATUS_SUCCESS) {
				...
				NDAS_ASSERT( status == STATUS_REMOTE_DISCONNECT );
   ...
   )
{
	...
	for (idx_addr = 0; idx_addr < socketLpxAddressList.iAddressCount; idx_addr++) {
		...
		for (dest_addr = 0; dest_addr < Secondary->NumberOfPrimaryAddress; dest_addr ++) {
			...
			status = LpxTdiV2OpenAddress( &NICAddr, &addressFileHandle, &addressFileObject );
			...
			serverAddress = &Secondary->PrimaryAddressList[dest_addr];
			...
			NDAS_ASSERT( serverAddress->Port == HTONS(DEFAULT_PRIMARY_PORT) || FlagOn(Secondary->Flags, SECONDARY_FLAG_INITIALIZING) );
...
if (IS_WINDOWS2K())
		...
		NDAS_ASSERT( ndfsReplyTreeConnect->Status == NDFS_TREE_CONNECT_SUCCESS );
   ...
   )
{
	...
	NDAS_ASSERT( Secondary->Thread.AddressFileHandle ); 
	NDAS_ASSERT( Secondary->Thread.AddressFileObject );
	NDAS_ASSERT( Secondary->Thread.ConnectionFileHandle );
	NDAS_ASSERT( Secondary->Thread.ConnectionFileObject ); 
	...
	)
{
	...
	NDAS_ASSERT( !FlagOn(Secondary->Thread.Flags, SECONDARY_THREAD_FLAG_ERROR) );
	...
	NDAS_ASSERT( NTOHL(ndfsRequestHeader->MessageSize4) >= sizeof(NDFS_REQUEST_HEADER) );

	NDAS_ASSERT( NTOHS(ndfsRequestHeader->Uid2) == sessionCtx->Uid );
	NDAS_ASSERT( NTOHS(ndfsRequestHeader->Tid2) == sessionCtx->Tid );
	...
	if (requestDataSize <= sessionCtx->PrimaryMaxDataSize) {
		...
		if (SecondaryRequest->SendBuffer) {

			NDAS_ASSERT( requestDataSize == SecondaryRequest->SendBufferLength );
			...
			if (requestDataSize) {
				...
				if (SecondaryRequest->SendMdl) {
					...
					SecondaryRequest->SendBuffer = MmGetSystemAddressForMdlSafe( SecondaryRequest->SendMdl, NormalPagePriority );
				
				} else {

#ifdef _WIN64
					NDAS_ASSERT( ((ULONGLONG)SecondaryRequest->SendBuffer & (ULONGLONG)0xF000000000000000LL) == (ULONGLONG)0xF000000000000000LL );
#else
					NDAS_ASSERT( ((ULONG)SecondaryRequest->SendBuffer & (ULONG)0xF0000000) == (ULONG)0x80000000 ||
								 ((ULONG)SecondaryRequest->SendBuffer & (ULONG)0xF0000000) == (ULONG)0x90000000	||
								 ((ULONG)SecondaryRequest->SendBuffer & (ULONG)0xF0000000) == (ULONG)0xA0000000	||
								 ((ULONG)SecondaryRequest->SendBuffer & (ULONG)0xF0000000) == (ULONG)0xB0000000	||
								 ((ULONG)SecondaryRequest->SendBuffer & (ULONG)0xF0000000) == (ULONG)0xF0000000	);
#endif
				}
	...
	NDAS_ASSERT( requestDataSize > sessionCtx->PrimaryMaxDataSize );
	...
	while (1) {
		...
		if (SecondaryRequest->SendBuffer) {
			...
			tdiStatus = SendMessage( Secondary->Thread.ConnectionFileObject,
									 &Secondary->Thread.SendNdasFcStatistics,
									 NULL,
									 (UINT8 *)SecondaryRequest->SendBuffer + (requestDataSize - remaninigDataSize),
		...
		NDAS_ASSERT( (INT32)remaninigDataSize > 0 );
	}
}
	...
	)
{
	...
	if (NTOHL(ndfsReplyHeader->MessageSize4) > SecondaryRequest->NdfsMessageAllocationSize) {
		...
		NDAS_ASSERT( SecondaryRequest->RecvMdl || SecondaryRequest->RecvBuffer || FALSE );
	}

	if (SecondaryRequest->RecvBuffer) {
		
		NDAS_ASSERT( NTOHL(ndfsReplyHeader->MessageSize4) - sizeof(NDFS_REPLY_HEADER) - sizeof(NDFS_WINXP_REPLY_HEADER) <= SecondaryRequest->RecvBufferLength );
	
	} else {

		NDAS_ASSERT( NTOHL(ndfsReplyHeader->MessageSize4) - sizeof(NDFS_REPLY_HEADER) - sizeof(NDFS_WINXP_REPLY_HEADER) <= SecondaryRequest->SendBufferLength );
	}
		...
		if (SecondaryRequest->RecvBuffer) {
			...
			if (NTOHL(ndfsReplyHeader->MessageSize4) - sizeof(NDFS_REPLY_HEADER) - sizeof(NDFS_WINXP_REPLY_HEADER)) {
				...
				if (SecondaryRequest->RecvMdl) {
					...
					SecondaryRequest->RecvBuffer = MmGetSystemAddressForMdlSafe( SecondaryRequest->RecvMdl, NormalPagePriority );
				
				} else {

#ifdef _WIN64
					NDAS_ASSERT( ((ULONGLONG)SecondaryRequest->RecvBuffer & (ULONGLONG)0xF000000000000000LL) == (ULONGLONG)0xF000000000000000LL );
#else
					NDAS_ASSERT( ((ULONG)SecondaryRequest->RecvBuffer & (ULONG)0xF0000000) == (ULONG)0x80000000 ||
								 ((ULONG)SecondaryRequest->RecvBuffer & (ULONG)0xF0000000) == (ULONG)0x90000000	||
								 ((ULONG)SecondaryRequest->RecvBuffer & (ULONG)0xF0000000) == (ULONG)0xA0000000	||
								 ((ULONG)SecondaryRequest->RecvBuffer & (ULONG)0xF0000000) == (ULONG)0xB0000000	||
								 ((ULONG)SecondaryRequest->RecvBuffer & (ULONG)0xF0000000) == (ULONG)0xE0000000	||
								 ((ULONG)SecondaryRequest->RecvBuffer & (ULONG)0xF0000000) == (ULONG)0xF0000000	);
#endif
				}
	...
	NDAS_ASSERT( ndfsReplyHeader->Splitted == 1 );

	//  Receive WinXP request header

	ndfsWinxpReplyHeader = (PNDFS_WINXP_REPLY_HEADER)(ndfsReplyHeader+1);
	
	NDAS_ASSERT( ndfsWinxpReplyHeader == (PNDFS_WINXP_REPLY_HEADER)SecondaryRequest->NdfsReplyData );
		...
		if (SecondaryRequest->RecvBuffer) {
			...
			if (NTOHL(ndfsReplyHeader->MessageSize4) - sizeof(NDFS_REPLY_HEADER) - sizeof(NDFS_WINXP_REPLY_HEADER)) {
				...
				tdiStatus = RecvMessage( Secondary->Thread.ConnectionFileObject,
										 &Secondary->Thread.RecvNdasFcStatistics,
										 NULL,
										 (UINT8 *)SecondaryRequest->RecvBuffer + NTOHL(ndfsReplyHeader->MessageSize4) - NTOHL(splitNdfsReplyHeader->MessageSize4),
