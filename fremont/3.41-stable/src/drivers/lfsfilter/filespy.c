...
PDRIVER_OBJECT gFileSpyDriverObject = NULL;
...
#if __NDAS_FS_MINI_CHECK__
BOOLEAN fltMgrInstalled = FALSE;
BOOLEAN miniFilterInstalled = FALSE;
#endif
...
--*/
{
	...
	status = RtlCheckRegistryKey(RTL_REGISTRY_SERVICES, L"FltMgr");

	fltMgrInstalled = (status == STATUS_SUCCESS);

#if __NDAS_FS_MINI__
	
	NDAS_ASSERT( fltMgrInstalled );

#else // __NDAS_FS_MINI__

	status = RtlCheckRegistryKey(RTL_REGISTRY_SERVICES, L"ndasfs");

	miniFilterInstalled = (status == STATUS_SUCCESS);

	if (fltMgrInstalled && miniFilterInstalled) {
		
#if (DBG || __NDAS_FS_ALLOW_UNLOAD_DRIVER__) && WINVER >= 0x0501
...
#endif // __NDAS_FS_MINI__

#endif // __NDAS_FS_MINI_CHECK__
	...
	//
	//	Adjust default values after reading the registry
	//

	if (LfsRegistry.MaxDataTransferPri == 0)
		LfsRegistry.MaxDataTransferPri = DEFAULT_MAX_DATA_SIZE;

	if (LfsRegistry.MaxDataTransferPri < DEFAULT_MIN_DATA_SIZE)
		LfsRegistry.MaxDataTransferPri = DEFAULT_MIN_DATA_SIZE;
	
	if (LfsRegistry.MaxDataTransferSec == 0)
		LfsRegistry.MaxDataTransferSec = DEFAULT_MAX_DATA_SIZE;
	
	if (LfsRegistry.MaxDataTransferSec < DEFAULT_MIN_DATA_SIZE)
		LfsRegistry.MaxDataTransferSec = DEFAULT_MIN_DATA_SIZE;
    ...
    if (STATUS_OBJECT_PATH_NOT_FOUND == status) {
        ...
        if (!NT_SUCCESS( status )) {
			...
			gFileSpyDriverObject->DriverUnload = NULL;
			gFileSpyDriverObject = NULL;
#endif
...
//
	...
	status = IoRegisterShutdownNotification(gControlDeviceObject);
	...
	if (!NT_SUCCESS( status )) {
		...
		SPY_LOG_PRINT( SPYDEBUG_ERROR,
					   ("FileSpy!DriverEntry: Error registering Shutdown notification, status=%08x\n", 
						 status) );
...
#   define DEVOBJ_LIST_SIZE 64
	...
	if (fltMgrInstalled && miniFilterInstalled) {
	    ...
	    NDAS_ASSERT( IS_WINDOWSXP_OR_LATER() );
		return;
#else
		NDAS_ASSERT( FALSE );
		return;
    ...
    for (;;) {
        ...
        interval.QuadPart = (2 * DELAY_ONE_SECOND);      //delay 5 seconds
	...
	interval.QuadPart = (2 * DELAY_ONE_SECOND);      //delay 2 seconds
	KeDelayExecutionThread( KernelMode, FALSE, &interval );
...
--*/
{
	...
	if (irpSp->MajorFunction == IRP_MJ_SHUTDOWN) {
...
#if DBG
			SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ("file system type = %d\n", devExt->LfsDeviceExt.FileSystemType) );
			PrintIrp( LFS_DEBUG_LFS_TRACE, "ShutdownOccured", &devExt->LfsDeviceExt, Irp );
#endif
		...
		return IoCallDriver( ((PFILESPY_DEVICE_EXTENSION)DeviceObject->DeviceExtension)->NLExtHeader.AttachedToDeviceObject, Irp );
	}

	if (devExt->LfsDeviceExt.FilteringMode == LFS_READONLY) {

		if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING) == FALSE) {

			result = LfsGeneralPassThrough( DeviceObject, Irp, devExt, &status );
		
		} else {

			result = ReadonlyPassThrough( DeviceObject, Irp, devExt, &status );
		}

	} else if (devExt->LfsDeviceExt.FilteringMode == LFS_PRIMARY ||
			   devExt->LfsDeviceExt.FilteringMode == LFS_SECONDARY_TO_PRIMARY) {

		result = LfsGeneralPassThrough( DeviceObject, Irp, devExt, &status );

	} else if (devExt->LfsDeviceExt.FilteringMode == LFS_FILE_CONTROL) {

		result = FALSE;
	
	} else if (devExt->LfsDeviceExt.FilteringMode == LFS_SECONDARY) {
		...
		if (!FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
			...
			result = LfsSecondaryPassThrough( DeviceObject, Irp, devExt, &status );
		}
    ...
    if (DeviceObject == gControlDeviceObject) {
        ...
        switch (irpStack->MajorFunction) {
            ...
            case IRP_MJ_DEVICE_CONTROL:
                ...
                status = SpyCommonDeviceIoControl( Irp,
						 Irp->AssociatedIrp.SystemBuffer,
			...
			case IRP_MJ_SHUTDOWN: {
				...
				//
				//	This is a private shutdown notification.
				//

				SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE,
					("SpyDispatch:================= Shutdown occured! ===============\n"));

				//KeAcquireSpinLock(&GlobalLfs.SpinLock, &oldIrqlLfs);
				
				ExAcquireFastMutex( &GlobalLfs.FastMutex );
				
				if(GlobalLfs.ShutdownOccured == FALSE)
				{
					KIRQL			oldIrql;
					PLIST_ENTRY		lfsDeviceExtListEntry;
					LARGE_INTEGER	interval;
					...
					//
					//	Notify shutdown to the applications
					//
...

{
    PFILESPY_DEVICE_EXTENSION devExt = DeviceObject->DeviceExtension;
    PIO_STACK_LOCATION pIrpSp = IoGetCurrentIrpStackLocation( Irp );
#if (__NDAS_FS_MINI__ && __NDAS_FS_ENABLE_MINI_MODE__)
    ...
    NDAS_ASSERT( !FlagOn(devExt->Flags,IsVolumeDeviceObject) );

    SPY_LOG_PRINT( SPYDEBUG_DISPLAY_ATTACHMENT_NAMES,
                   ("FileSpy!SpyFsControlMountVolume: pIrpSp->Parameters.MountVolume.Vpb->RealDevice = %p, "
				    "pIrpSp->Parameters.MountVolume.DeviceObject = %p, devExt->LfsDeviceExt.FileSystemType = %d\n",
					pIrpSp->Parameters.MountVolume.Vpb->RealDevice, pIrpSp->Parameters.MountVolume.DeviceObject, devExt->LfsDeviceExt.FileSystemType) );

#if __NDAS_FS__

do {

	if (!(devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT	||
          devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS	||
		  devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_ROFS	||
	...
	if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT  && !GlobalLfs.NdasFatSupport	||
		devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS && !GlobalLfs.NdasNtfsSupport  ||
		devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_ROFS && !GlobalLfs.NdasRofsSupport) {
    ...
    SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, 
				   ("NetdiskManager_PreMountVolume status = %x, netdiskEnableMode = %d\n", status, netdiskEnableMode) );
	...
	NDAS_ASSERT( GlobalLfs.NdasFsMiniMode == FALSE || netdiskEnableMode == NETDISK_READ_ONLY );

	switch (netdiskEnableMode) {

	case NETDISK_READ_ONLY: {

		if (GlobalLfs.NdasFatSupport) {
			
			if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT) {

				ndasFsFiltering = FALSE;
				status = STATUS_SUCCESS;
				break;
			}
			...
			if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_FAT) {

				ndasFsFiltering = TRUE;
				status = STATUS_SUCCESS;
				break;
			}
		}
		
		if (GlobalLfs.NdasRofsSupport) {
			
			if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_ROFS) {
	...
	case NETDISK_SECONDARY: {
		...
		if (GlobalLfs.NdasFatSupport) {
			...
			if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT) {
				...
				ndasFsFiltering = 
					FlagOn(netdiskPartition->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT) ?
						TRUE : FALSE;	
		...
		if (GlobalLfs.NdasNtfsSupport && 
			!FlagOn(netdiskPartition->NetdiskPartitionInformation.Flags, NETDISK_PARTITION_INFORMATION_FLAG_INDIRECT)) {

			if (devExt->LfsDeviceExt.FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS) {
	...
	if (status == STATUS_UNRECOGNIZED_VOLUME) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("SpyFsControlMountVolume status = STATUS_UNRECOGNIZED_VOLUME\n") );
...
#   define MVInsufResMsg "FileSpy!SpyFsControlMountVolume: Error getting device name, insufficient resources, status=%08x\n"
	...
	RtlZeroMemory( &newDevExt->LfsDeviceExt, sizeof(LFS_DEVICE_EXTENSION) );

    ExInitializeFastMutex( &newDevExt->LfsDeviceExt.FastMutex );
	newDevExt->LfsDeviceExt.ReferenceCount		= 1;

	InitializeListHead( &newDevExt->LfsDeviceExt.LfsQListEntry );

	newDevExt->LfsDeviceExt.Flags = LFS_DEVICE_FLAG_INITIALIZING;

	newDevExt->LfsDeviceExt.FileSpyDeviceObject	= newDeviceObject;
	ObReferenceObject( newDevExt->LfsDeviceExt.FileSpyDeviceObject );

	newDevExt->LfsDeviceExt.NetdiskPartition	= netdiskPartition;
	newDevExt->LfsDeviceExt.NetdiskEnabledMode	= netdiskEnableMode;
	
	if (ndasFsFiltering) {
		
		SetFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING );
	}

	newDevExt->LfsDeviceExt.FilteringMode	= LFS_NO_FILTERING;

	newDevExt->LfsDeviceExt.DiskDeviceObject		= pIrpSp->Parameters.MountVolume.Vpb->RealDevice;
	newDevExt->LfsDeviceExt.MountVolumeDeviceObject	= pIrpSp->Parameters.MountVolume.DeviceObject;

	ExInterlockedInsertTailList( &GlobalLfs.LfsDeviceExtQueue,
								 &newDevExt->LfsDeviceExt.LfsQListEntry,
								 &GlobalLfs.LfsDeviceExtQSpinLock );

	newDevExt->LfsDeviceExt.FileSystemType = devExt->LfsDeviceExt.FileSystemType;

	switch (netdiskEnableMode) {

	case NETDISK_READ_ONLY:

		newDevExt->LfsDeviceExt.FilteringMode = LFS_READONLY;

		break;

	case NETDISK_SECONDARY:

		newDevExt->LfsDeviceExt.FilteringMode = LFS_SECONDARY;
	...
	case NETDISK_SECONDARY2PRIMARY:
		...
		newDevExt->LfsDeviceExt.FilteringMode = LFS_PRIMARY;
	...
	SetFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTING );
...
{
	NTSTATUS				status;
	PLFS_DEVICE_EXTENSION	newLfsDeviceExt = NewLfsDeviceExt;
	ULONG					tryCreateSecondary;


	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, 
				   ("SpyFsControlSecondaryMountVolumeComplete: Entered newLfsDeviceExt = %p %wZ newLfsDeviceExt->FileSystemType = %d\n", 
					 newLfsDeviceExt, &newLfsDeviceExt->NetdiskPartitionInformation.VolumeName, newLfsDeviceExt->FileSystemType) );

	if (!FlagOn(newLfsDeviceExt->Flags, LFS_DEVICE_FLAG_FILTERING)) {
		
		NDAS_ASSERT( newLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT ||
					 newLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS );
...
#define TRY_COUNT	360   // about 10 minutes!!
	...
	for (tryCreateSecondary = 0; tryCreateSecondary < TRY_COUNT; tryCreateSecondary++) {
		...
		if (secondary) {
			...
			if (secondary->Thread.SessionStatus != STATUS_SUCCESS) {
				...
				NDAS_ASSERT( FlagOn(secondary->Thread.Flags, SECONDARY_THREAD_FLAG_UNCONNECTED) );
			}
		...
		status = NetdiskManager_Secondary2Primary( GlobalLfs.NetdiskManager, 
												   newLfsDeviceExt->NetdiskPartition, 
												   newLfsDeviceExt->NetdiskEnabledMode );
    ...
    )
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, 
				   ("SpyFsControlReadonlyMountVolumeComplete: Entered newLfsDeviceExt = %p %wZ newLfsDeviceExt->FileSystemType = %d\n", 
					 newLfsDeviceExt, &newLfsDeviceExt->NetdiskPartitionInformation.VolumeName, newLfsDeviceExt->FileSystemType) );

	if (newLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDAS_FAT	||
		newLfsDeviceExt->FileSystemType == LFS_FILE_SYSTEM_NDAS_NTFS) {
	...
	readonly = Readonly_Create( newLfsDeviceExt );

	if (readonly) {

		newLfsDeviceExt->Readonly = readonly;
		status = STATUS_SUCCESS;
...
if (NewDeviceObject == NULL) {
	...
	ASSERT( newDevExt->LfsDeviceExt.ReferenceCount == 1 );
	...
	SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE,
					("%s: newDevExt = %p %wZ Irp->IoStatus.Status = %x, vpb->RealDevice = %p, vpb->DeviceObject = %p\n", 
					 __FUNCTION__, newDevExt, &newDevExt->NLExtHeader.DeviceName, Irp->IoStatus.Status, 
					 vpb->RealDevice, vpb->DeviceObject) );

#endif

	NetdiskManager_PostMountVolume( GlobalLfs.NetdiskManager,
									newDevExt->LfsDeviceExt.NetdiskPartition,
									newDevExt->LfsDeviceExt.NetdiskEnabledMode,
									TRUE,
									newDevExt->LfsDeviceExt.FileSystemType,		
									&newDevExt->LfsDeviceExt,
									&newDevExt->LfsDeviceExt.NetdiskPartitionInformation );
    ...
    if (NT_SUCCESS( Irp->IoStatus.Status )) {
        ...
        if (!SpyIsAttachedToDevice( vpb->DeviceObject,
            ...
            if (NT_SUCCESS( status )) {
				...
				NDAS_ASSERT( vpb->DeviceObject );

				newDevExt->LfsDeviceExt.Vpb						= newDevExt->LfsDeviceExt.DiskDeviceObject->Vpb; // Vpb will be changed in IrpSp Why ?
				newDevExt->LfsDeviceExt.BaseVolumeDeviceObject	= newDevExt->LfsDeviceExt.Vpb->DeviceObject;

				ExReleaseFastMutex( &gSpyAttachLock );

				switch (newDevExt->LfsDeviceExt.FilteringMode) {
				...
				case LFS_READONLY: {
					...
					SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsFsControlMountVolumeComplete: READONLY newDevExt->LfsDeviceExt = %p "
														"newDevExt->LfsDeviceExt.FileSystemType = %d\n",
														 &newDevExt->LfsDeviceExt, newDevExt->LfsDeviceExt.FileSystemType) );
		
					newDevExt->LfsDeviceExt.AttachedToDeviceObject = newDevExt->NLExtHeader.AttachedToDeviceObject;

					NetdiskManager_MountVolumeComplete( GlobalLfs.NetdiskManager,
														newDevExt->LfsDeviceExt.NetdiskPartition,
														newDevExt->LfsDeviceExt.NetdiskEnabledMode,
														status,
														newDevExt->LfsDeviceExt.AttachedToDeviceObject );

					status = SpyFsControlReadonlyMountVolumeComplete( &newDevExt->LfsDeviceExt );
				...
				case LFS_PRIMARY: {
					...
					ASSERT( newDevExt->LfsDeviceExt.AttachedToDeviceObject );
					...
					//NetdiskManager_CreateFile( GlobalLfs.NetdiskManager,
					//						   newDevExt->LfsDeviceExt.NetdiskPartition );
				...
				case LFS_SECONDARY: {
					...
					SPY_LOG_PRINT( LFS_DEBUG_LFS_INFO, ("LfsFsControlMountVolumeComplete: LFS_SECONDARY newDevExt->LfsDeviceExt = %p "
														"newDevExt->LfsDeviceExt.FileSystemType = %d\n",
														 &newDevExt->LfsDeviceExt, newDevExt->LfsDeviceExt.FileSystemType) );

					status = SpyFsControlSecondaryMountVolumeComplete( &newDevExt->LfsDeviceExt );

					if (newDevExt->LfsDeviceExt.FilteringMode == LFS_SECONDARY_TO_PRIMARY) {
				...
				if (status == STATUS_SUCCESS) {
					...
					ClearFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_INITIALIZING );
					ClearFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTING );

					SetFlag( newDevExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_MOUNTED );
					...
					XevtQueueVolumeInvalidOrLocked( -1,
													newDevExt->LfsDeviceExt.NetdiskPartitionInformation.NetdiskInformation.SlotNo,
													newDevExt->LfsDeviceExt.NetdiskPartitionInformation.NetdiskInformation.UnitDiskNo );
					...
					NDAS_ASSERT( NT_SUCCESS(status2) );
				}
				...
				LfsDismountVolume( &newDevExt->LfsDeviceExt );
...
--*/
{
	...
	if (devExt == NULL) {
		...
		NDAS_ASSERT( DeviceObject == gControlDeviceObject );
		return FALSE;		
	}

	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {

		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoCheckIfPossible entered\n") );

		if (devExt->LfsDeviceExt.FilteringMode == LFS_SECONDARY) {

			return FALSE;

		} else if (devExt->LfsDeviceExt.FilteringMode == LFS_READONLY) {
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoRead entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoWrite entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoQueryBasicInfo entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoQueryStandardInfo entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoLock entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoUnlockSingle entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoUnlockAll entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoUnlockAllByKey entered\n") );
...
--*/
{
    ...
    if (DeviceObject == gControlDeviceObject) {
		...
		if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
			...
			SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoDeviceControl entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoQueryNetworkOpenInfo entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoMdlRead entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoMdlReadComplete entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoPrepareMdlWrite entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoMdlWriteComplete entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoReadCompressed entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoWriteCompressed entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoMdlReadCompleteCompressed entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoMdlWriteCompleteCompressed entered\n") );
...
--*/
{
	...
	if (FlagOn(devExt->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_NOISE, ("SpyFastIoQueryOpen entered\n") );
...
--*/
{
    ...
    switch (IoControlCode) {
		...
		case LFS_FILTER_SHUTDOWN: {

			SPY_LOG_PRINT( LFS_DEBUG_LFS_ERROR, ("SpyCommonDeviceIoControl: LFS_FILTER_SHUTDOWN return\n") );		
		...
		case LFS_FILTER_CREATE_EVTQ: {
			NTSTATUS	status;
			PLFS_CREATEEVTQ_RETURN createReturn = NULL;

#ifdef  _WIN64 // 64-bit driver

			LFS_CREATEEVTQ_RETURN LocalCreateReturn;
			
			if (IoIs32bitProcess(Irp)) { /* 32-bit process IOCTL */

				if ((OutputBufferLength < sizeof(LFS_CREATEEVTQ_RETURN_32)) || 
					(OutputBuffer == NULL)) {
				...
				createReturn = &LocalCreateReturn;
			
			} else {
			
				if ((OutputBufferLength < sizeof(LFS_CREATEEVTQ_RETURN)) || 
					(OutputBuffer == NULL)) {
				...
				createReturn = (PLFS_CREATEEVTQ_RETURN)OutputBuffer;
			}
			...
			do {
				//
				//	Create a event queue for this caller.
				//

				status = XEvtqCreateEventQueue(
										&createReturn->EventQueueHandle,
										&createReturn->EventWaitHandle);
				if(!NT_SUCCESS(status)) {
					break;
				}

				//
				//	Register the event queue to the event manager.
				//

				status = XevtqRegisterEventQueue(&GlobalLfs.EvtQueueMgr,
								createReturn->EventQueueHandle);
				if(!NT_SUCCESS(status)) {
					XEvtqCloseEventQueue(createReturn->EventQueueHandle);
				}

			} while (0);

// Do thunk for 32bit application over 64bit driver.
#ifdef _WIN64 
			if (NT_SUCCESS(status) && IoIs32bitProcess(Irp)) { /* 32-bit process IOCTL */
				status = STATUS_NOT_SUPPORTED;
#if 0	// Current scheme cannot support 32 bit application. Just return not supported error.
				NTSTATUS ThunkStatus;
				PLFS_CREATEEVTQ_RETURN_32 createReturn_32;
			
				createReturn_32 = (PLFS_CREATEEVTQ_RETURN_32)OutputBuffer;
				ThunkStatus = RtlULongPtrToULong(
					(ULONG_PTR) LocalCreateReturn.EventQueueHandle, 
					&((ULONG)createReturn_32->EventQueueHandle));
				if (!NT_SUCCESS(ThunkStatus)) {
					// Assumption that "event created by 32bit process is 32 bit" is not right.
					//    We cannot support 32 bit application.
					status = STATUS_NOT_SUPPORTED;
					ASSERT(0);
				} else {
					ThunkStatus = RtlULongPtrToULong(
						(ULONG_PTR) LocalCreateReturn.EventWaitHandle, 
						&((ULONG)createReturn_32->EventWaitHandle));
					if (!NT_SUCCESS(ThunkStatus)) {
						// Assumption that "event created by 32bit process is 32 bit" is not right.
						//    We cannot support 32 bit application.
						status = STATUS_NOT_SUPPORTED;
		...
		case LFS_FILTER_CLOSE_EVTQ: {
			NTSTATUS	status;
			PLFS_CLOSEVTQ_PARAM closeParam;
#ifdef  _WIN64 // 64-bit driver
			LFS_CLOSEVTQ_PARAM LocalCloseParam;
			if (IoIs32bitProcess(Irp)) { /* 32-bit process IOCTL */
				if ((InputBufferLength < sizeof(LFS_CLOSEVTQ_PARAM_32)) || 
					(InputBuffer == NULL)) {
					IoStatus->Status = STATUS_INVALID_PARAMETER;
					break;
				}
				// Thunk 32 bit value
				LocalCloseParam.EventQueueHandle = ((PLFS_CLOSEVTQ_PARAM_32)InputBuffer)->EventQueueHandle;
				closeParam = (PLFS_CLOSEVTQ_PARAM)&LocalCloseParam;
			} else {
				if ((InputBufferLength < sizeof(LFS_CLOSEVTQ_PARAM)) || 
					(InputBuffer == NULL)) {

					IoStatus->Status = STATUS_INVALID_PARAMETER;
					break;
				}
				closeParam = (PLFS_CLOSEVTQ_PARAM)InputBuffer;			
			}
#else  // 32 bit driver
			if ((InputBufferLength < sizeof(LFS_CLOSEVTQ_PARAM)) || 
				(InputBuffer == NULL)) {
			...
			do {
				//
				//	Unregister the event queue to the event manager.
				//

				status = XevtqUnregisterEventQueue(
							&GlobalLfs.EvtQueueMgr,
							closeParam->EventQueueHandle,
							TRUE);
				if(!NT_SUCCESS(status)) {
					break;
				}
				...
				//
				//	Close the event queue for this caller.
				//

				status = XEvtqCloseEventQueue(
							closeParam->EventQueueHandle);
		...
		case LFS_FILTER_GET_EVTHEADER: {
			NTSTATUS					status;
			PLFS_GET_EVTHEADER_PARAM	evtHeaderParam;
			PLFS_GET_EVTHEADER_RETURN	evtHeaderReturn;

#ifdef  _WIN64 // 64-bit driver
			LFS_GET_EVTHEADER_PARAM LocalGetEvtHeaderParam;
			if (IoIs32bitProcess(Irp)) { /* 32-bit process IOCTL */
				if ((InputBufferLength < sizeof(LFS_GET_EVTHEADER_PARAM_32)) || 
					(InputBuffer == NULL)) {
						IoStatus->Status = STATUS_INVALID_PARAMETER;
						break;
				}
				LocalGetEvtHeaderParam.EventQueueHandle = ((PLFS_GET_EVTHEADER_PARAM_32)InputBuffer)->EventQueueHandle;
				evtHeaderParam = (PLFS_GET_EVTHEADER_PARAM)&LocalGetEvtHeaderParam;
			} else {
				if ((InputBufferLength < sizeof(LFS_GET_EVTHEADER_PARAM)) || 
					(InputBuffer == NULL)) {
						IoStatus->Status = STATUS_INVALID_PARAMETER;
						break;
				}
				evtHeaderParam = (PLFS_GET_EVTHEADER_PARAM)InputBuffer;
			}
#else 
			if ((InputBufferLength < sizeof(LFS_GET_EVTHEADER_PARAM)) || 
				(InputBuffer == NULL)) {
			...
			if ((OutputBufferLength < sizeof(LFS_GET_EVTHEADER_RETURN)) || 
				(OutputBuffer == NULL)) {
			...
			evtHeaderReturn = (PLFS_GET_EVTHEADER_RETURN)OutputBuffer;
			
			status = XEvtqGetEventHeader(evtHeaderParam->EventQueueHandle,
										&evtHeaderReturn->EventLength,
										&evtHeaderReturn->EventClass);

			IoStatus->Status = status;
			IoStatus->Information = sizeof(LFS_GET_EVTHEADER_RETURN);
			break;
		}
		case LFS_FILTER_DEQUEUE_EVT: {
			NTSTATUS				status;
			PLFS_DEQUEUE_EVT_PARAM	dequeParam;

#ifdef  _WIN64 // 64-bit driver
			LFS_DEQUEUE_EVT_PARAM	LocalDequeParam;
			if (IoIs32bitProcess(Irp)) { /* 32-bit process IOCTL */
				if ((InputBufferLength < sizeof(LFS_DEQUEUE_EVT_PARAM_32)) || 
					(InputBuffer == NULL)) {

						IoStatus->Status = STATUS_INVALID_PARAMETER;
						break;
				}
				LocalDequeParam.EventQueueHandle = ((PLFS_DEQUEUE_EVT_PARAM_32)InputBuffer)->EventQueueHandle;
				dequeParam = (PLFS_DEQUEUE_EVT_PARAM)&LocalDequeParam;
			} else {
				if ((InputBufferLength < sizeof(LFS_DEQUEUE_EVT_PARAM)) || 
					(InputBuffer == NULL)) {

						IoStatus->Status = STATUS_INVALID_PARAMETER;
						break;
				}
				dequeParam = (PLFS_DEQUEUE_EVT_PARAM)InputBuffer;
			}
#else // 32bit driver
			...
			if ((OutputBufferLength < FIELD_OFFSET(XEVENT_ITEM, VolumeLocked)) || 
				(OutputBuffer == NULL)) {
			...
			status = XEvtqDequeueEvent(dequeParam->EventQueueHandle, (PXEVENT_ITEM)OutputBuffer);

			IoStatus->Status = status;
			IoStatus->Information = sizeof(XEVENT_ITEM);
			break;
		}
		case LFS_FILTER_QUERY_NDASUSAGE: {

			NTSTATUS					status;
			PLFS_QUERY_NDASUSAGE_PARAM	queryUsage;
			PLFS_QUERY_NDASUSAGE_RETURN	queryUsageReturn;

			if ((InputBufferLength < sizeof(LFS_QUERY_NDASUSAGE_PARAM)) || (InputBuffer == NULL)) {

				IoStatus->Status = STATUS_INVALID_PARAMETER;
				break;
			}

			if ((OutputBufferLength < sizeof(LFS_QUERY_NDASUSAGE_RETURN)) || (OutputBuffer == NULL)) {

				IoStatus->Status = STATUS_INVALID_PARAMETER;
				break;
			}

			queryUsage = (PLFS_QUERY_NDASUSAGE_PARAM)InputBuffer;
			queryUsageReturn = (PLFS_QUERY_NDASUSAGE_RETURN)OutputBuffer;

			status = LfsDeviceExt_QueryNDASUsage( queryUsage->SlotNumber,
												  &queryUsageReturn->NdasUsageFlags,
												  &queryUsageReturn->MountedFSVolumeCount );

			IoStatus->Status = status;
			IoStatus->Information = sizeof(LFS_QUERY_NDASUSAGE_RETURN);

			break;
		}

		case LFS_FILTER_STOP_SECVOLUME: {

			NTSTATUS					status;
			PLFS_STOPSECVOLUME_PARAM	stopParam;

			if ((InputBufferLength < sizeof(LFS_STOPSECVOLUME_PARAM)) || (InputBuffer == NULL)) {

				IoStatus->Status = STATUS_INVALID_PARAMETER;
				break;
			}

			stopParam = (PLFS_STOPSECVOLUME_PARAM)InputBuffer;

			status = LfsStopSecVolume( stopParam->PhysicalDriveNumber, stopParam->VolumeHint );
		...
		case LFS_FILTER_GETVERSION: {

			LFSFILT_VER	version;

            if ((OutputBufferLength < sizeof(LFSFILT_VER)) || (OutputBuffer == NULL)) {
			...
			version.VersionNDFSMajor = NDFS_MAJOR_VERSION;

			// if (IS_WINDOWS2K()) {

			//	version.VersionNDFSMajor = NDFS_2K_MAJOR_VERSION;
			// }
			
			if (GlobalLfs.NdasNtfsSupport == TRUE) {

				version.VersionNDFSMajor |= 0x0100;
			}

			version.VersionNDFSMinor= NDFS_MINOR_VERSION;

            IoStatus->Information = sizeof(LFSFILT_VER);

            try {

                RtlCopyMemory(OutputBuffer, &version, sizeof(LFSFILT_VER));
