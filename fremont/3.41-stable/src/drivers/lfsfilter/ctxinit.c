...
--*/
{
	...
	RtlInitUnicodeString( &ndasRofs, NDAS_ROFS_DEVICE_NAME );
	...
	if (!(RtlEqualUnicodeString(&instanceContext->VolumeProperties->FileSystemDeviceName, &ntfs, TRUE)		||
		  ...
		  RtlEqualUnicodeString(&instanceContext->VolumeProperties->FileSystemDeviceName, &ndasRofs, TRUE)	||
	...
	if (RtlEqualUnicodeString(&instanceContext->VolumeProperties->FileSystemDeviceName, &ndasFat, TRUE)  && 
		...
		RtlEqualUnicodeString(&instanceContext->VolumeProperties->FileSystemDeviceName, &ndasRofs, TRUE) && 
		!GlobalLfs.NdasRofsSoSupport && !GlobalLfs.NdasRofsRoSupport) {
	...
	} else if (RtlEqualUnicodeString(&instanceContext->VolumeProperties->FileSystemDeviceName, &ndasRofs, TRUE)) {
	...
	case NETDISK_READ_ONLY:
		...
		if (RtlEqualUnicodeString(&instanceContext->VolumeProperties->FileSystemDeviceName, &fat, TRUE)) {
			...
			if (GlobalLfs.NdasNtfsRoSupport || GlobalLfs.NdasRofsRoSupport) {
			...
			if (!GlobalLfs.NdasRofsRoSupport) {
	...
	case NETDISK_SECONDARY2PRIMARY:
		...
		if (RtlEqualUnicodeString(&instanceContext->VolumeProperties->FileSystemDeviceName, &fat, TRUE)) {
			...
			if (GlobalLfs.NdasNtfsRwSupport &&
				FlagOn(netdiskPartition->EnabledNetdisk->NetdiskInformation.SupportedFeatures, NDASFEATURE_SIMULTANEOUS_WRITE) ||
				GlobalLfs.NdasRofsSoSupport && netdiskEnableMode == NETDISK_SECONDARY) {
			...
			if !(GlobalLfs.NdasRofsSoSupport && netdiskEnableMode == NETDISK_SECONDARY)) {
	...
	if (canBeDirect == FALSE) {

		SetFlag( instanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_FILTERING );
	}
	...
	if (RtlEqualUnicodeString(&instanceContext->VolumeProperties->FileSystemDeviceName, &ntfs, TRUE)) {
		...
		instanceContext->LfsDeviceExt.FileSystemType = LFS_FILE_SYSTEM_NDAS_ROFS;
...
--*/
{
	...
	if (status == STATUS_SUCCESS) {
		...
		NDAS_ASSERT( FlagOn(instanceContext->LfsDeviceExt.Flags, LFS_DEVICE_FLAG_DISMOUNTED) ||
					   ...
					   (instanceContext->LfsDeviceExt.Flags & ~LFS_DEVICE_FLAG_FILTERING) == LFS_DEVICE_FLAG_MOUNTED );
