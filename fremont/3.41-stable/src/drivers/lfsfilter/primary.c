	...
	) 
{
	...
	RtlCopyMemory( listenSock->NICAddress.Node, NICAddr->Node, ETHER_ADDR_LENGTH );
	...
	if (!NT_SUCCESS(ntStatus)) {
		...
		NDAS_ASSERT( ntStatus == 0xC001001FL );
		return ntStatus;
	}
	...
	)
{
	...
	SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO, ("PrimaryAgentThreadProc: Start\n") );
	...
	while (primaryAgentThreadExit == FALSE) {
		...
		status = KeWaitForMultipleObjects( eventCnt,
											 events,
											 WaitAny,
											 Executive,
											 KernelMode,
		...
		SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_TRACE,
					   ("PrimaryAgentThreadProc: status:%lu\n", status) );
		

		if (!NT_SUCCESS(status) || status >= eventCnt) {
		...
		NDAS_ASSERT( 1 <= status && status < eventCnt ); // LpxEvent 
		
		if (1 <= status && status <= MAX_SOCKETLPX_INTERFACE) {
			...
			listenSocket = &Primary->Agent.ListenSocket[status-1];
		...
		if (irpSp->MajorFunction == IRP_MJ_PNP) {
			...
			if (irpSp->MinorFunction == IRP_MN_CANCEL_REMOVE_DEVICE || irpSp->MinorFunction == IRP_MN_REMOVE_DEVICE) {
				...
				if (irpSp->MinorFunction == IRP_MN_CANCEL_REMOVE_DEVICE) {
					...
					SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO, 
								   ("%s: lfsDeviceExt = %p, IRP_MN_REMOVE_DEVICE\n", __FUNCTION__, &DevExt->LfsDeviceExt) );
				...
				SPY_LOG_PRINT( LFS_DEBUG_PRIMARY_INFO, 
							   ("%s: lfsDeviceExt = %p, IRP_MN_QUERY_REMOVE_DEVICE Irp->IoStatus.Status = %x, DevExt->LfsDeviceExt.FileSystemType = %d\n",
								__FUNCTION__, &DevExt->LfsDeviceExt, Irp->IoStatus.Status, DevExt->LfsDeviceExt.FileSystemType) );
