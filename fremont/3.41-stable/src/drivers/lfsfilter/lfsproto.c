	...
	) 
{
	...
	ntStatus = LpxTdiV2GetAddressList( &NetEvtCtx->AddressList );

	if (!NT_SUCCESS(ntStatus)) {

		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] NetEventDetectorProc: LpxTdiV2GetAddressList() failed. NTSTATUS:%lu\n", ntStatus));
//		goto termination ;
	}
	...
	while (1) {
		...
		ntStatus = KeWaitForSingleObject( &NetEvtCtx->ShutdownEvent, Executive, KernelMode, FALSE, &TimeOut );
		...
		RtlZeroMemory( &addressList, sizeof(SOCKETLPX_ADDRESS_LIST) );

		ntStatus = LpxTdiV2GetAddressList(&addressList);
		...
		FindOutChanges( &NetEvtCtx->AddressList, &addressList, &NetEvtCtx->DisabledAddressList, &NetEvtCtx->EnabledAddressList );

		//	call back

		if (NetEvtCtx->DisabledAddressList.iAddressCount == 0 && NetEvtCtx->EnabledAddressList.iAddressCount == 0 && AgeCount <= 60) {
		...
		for (idx_callbacks = 0 ; idx_callbacks < NetEvtCtx->CallbackCnt ; idx_callbacks ++ ) {
			...
			if (NetEvtCtx->Callbacks[idx_callbacks]) {
				...
				NetEvtCtx->Callbacks[idx_callbacks]( &NetEvtCtx->AddressList,
													 &addressList,
...
LfsAllocDGPkt(
	...
	*ppkt = ExAllocatePoolWithTag(
	...
	rawHead->NdfsMajorVersion2		= HTONS(NDFT_MAJVER);
	rawHead->NdfsMinorVersion2		= HTONS(NDFT_MINVER);

	rawHead->OsMajorType2			= HTONS(OSTYPE_WINDOWS);
	rawHead->Type2					= HTONS(NDFT_PRIMARY_UPDATE_MESSAGE);

	if (IS_WINDOWS2K()) {

		rawHead->OsMinorType2	= HTONS(OSTYPE_WIN2K);
	
	} else if(IS_WINDOWSXP()) {
	
		rawHead->OsMinorType2	= HTONS(OSTYPE_WINXP);
	
	} else if(IS_WINDOWSNET()) {

		rawHead->OsMinorType2	= HTONS(OSTYPE_WIN2003SERV);
	...
	rawHead->MessageSize4	= HTONL(((UINT32)(sizeof(NDFT_HEADER))) + dataSz);
	...
	) 
{
	...
	if (!NT_SUCCESS(ntStatus)) {
		...
		SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] LfsOpenDGSocket: LpxTdiV2GetAddressList() failed.\n") );
		return ntStatus;
	}
	...
	for (idx_addr = 0; idx_addr < socketLpxAddressList.iAddressCount; idx_addr ++) {
		...
		if (socketLpxAddressList.SocketLpx[idx_addr].LpxAddress.Node[0] == 0 &&
			socketLpxAddressList.SocketLpx[idx_addr].LpxAddress.Node[1] == 0 &&
			socketLpxAddressList.SocketLpx[idx_addr].LpxAddress.Node[2] == 0 &&
			socketLpxAddressList.SocketLpx[idx_addr].LpxAddress.Node[3] == 0 &&
			socketLpxAddressList.SocketLpx[idx_addr].LpxAddress.Node[4] == 0 &&
			socketLpxAddressList.SocketLpx[idx_addr].LpxAddress.Node[5] == 0) {
	...
	)
{
	...
	for (idx_addr = 0; idx_addr < socket->SocketCnt; idx_addr ++) {
		...
		ntStatus = LpxTdiV2SendDatagram( socket->Sockets[socket->SocketCnt - idx_addr - 1].AddressFile,
										 LpxRemoteAddress,
...
LfsRegisterDatagramRecvHandler(
	...
	for(idx_addr = 0 ; idx_addr < DatagramSocket->SocketCnt ; idx_addr ++) {
		...
		ntStatus = LpxTdiV2RegisterRecvDatagramHandler(
				DatagramSocket->Sockets[idx_addr].AddressFile,
				EventHandler,
				EventContext
			) ;
		if(!NT_SUCCESS(ntStatus)) {
			SPY_LOG_PRINT( LFS_DEBUG_LFS_TRACE, ( "[LFS] LfsRegisterDatagramRecvHandler: LpxTdiV2SetReceiveDatagramHandler() failed.\n")) ;
		}
	}
